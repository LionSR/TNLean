...
The third route word is \tnval{orth}, which turns at right angles.  A
string that must reach around the picture rather than across it takes its
corners from invisible junction atoms: chain the segments through a
\tnkey{skin=}\tnval{none} atom standing where the turn belongs, and each
segment runs straight while the chain draws the corner.

\begin{tnexample}[
  caption={a string routed around the picture},
  index={The junction is an invisible atom at an ordinary cell address.
    The path leaves $A$ southward, turns at the junction, and enters $B$
    from the west.},
  signature={kernel-boundary|signature=}]
% Ink: two chained segments; the invisible junction atom is the corner,
%   and the joint declares its order as any crossing does.
\tndeclare{species}{gauge}{hue=source:blue}
\begin{tenkz}[rows={wire,wire,wire}, cols=4, bonds=none]
  \tn[at=(1,1), name=a, skin=box]{A}
  \tn[at=(2,4), name=b, skin=box]{B}
  \tn[at=(3,1), skin=none, name=j]{}
  \tnwire[kind=string, species=gauge, name=drop]{a.270}{j}
  \tnwire[kind=string, species=gauge, name=run,
          cross={over at crossing of drop and run}]{j}{b.180}
\end{tenkz}
\end{tnexample}
...
shape comes from its route: a loop that rings a selection travels the
hull, \tnkey{route=}\tnval{\{all of z\}}, and a freer shape is a closed
junction chain, wire by wire through its invisible corner atoms.
...
their ends are junctions with no ports to take.  \tnkey{dir=} marks a
directed index against its dual.  Multiplicity is mathematics carried by
the index label, not a second field on the rail.

\begin{tnexample}[
  wide,
  caption={the three strokes and a direction},
  index={Left: the three declared rail patterns.  The four sites declare
    ...
    Right: one rail carrying the direction mark that distinguishes a
    space from its dual.},
  signature={kernel-boundary|signature=}]
...
\begin{tnexample}[
  ...
  index={The inner contour is the retained block; the outer one, declared
    ...
    acting on the indices the block leaves open.  Their region species
...
\tnkey{species=} tints the contour, as it tints every other record: the
five region words -- \tnval{selected}, \tnval{secondary},
\tnval{complement}, \tnval{collar}, \tnval{neutral} -- are prelude
species carrying the house region palette, and \tnval{complement} also
dashes the contour it names, the grey rest of a picture under
discussion.  A contour naming no species is passive.  \tnkey{tint} lays
ink over the paper inside a contour rather than only stroking it; the
tutorial's blocked run shows the tinted form, and so does the band
below.
...
\tncmd{tnset} carries the same policy at document scope.  Two keys live
there: \tnkey{pitch=}, the one base metric every named ratio follows, and
\tnkey{strict}, which rejects every sugar spelling in the registry.  The
house semantic ink binds when the package loads.  A palette taken from a
cited source reaches ink through a species declaration with \tnkey{hue=}.
No benchmark case sets \tnkey{strict}.  The canonical cases write sugar freely, and
