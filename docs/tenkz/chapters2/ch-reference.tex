...
% command, and key of tenkz 0.6, one entry per object: usage line
...
equation must share.  \tnenv{tenkzlattice} and \tnenv{tenkzplanes}
pictures log the same boundary line (open sheets in place of open
wires); the free and cd dialects log their own events and carry no
boundary line.} that
...
\texttt{chain axis=} & \texttt{east|south} & \texttt{east} &
  ...
  south = vertical) \\
\texttt{trace style=} & \texttt{racetrack|hooks} & flat:
  \texttt{racetrack}, vertical: \texttt{hooks} &
  how a traced side closes; never sets whether (that is
  \texttt{periodic} or \texttt{west=trace}/\texttt{east=trace}) \\
\bottomrule
\end{tabularx}\par}
\medskip
...
closed indices and leave the signature.  This per-site form requires
one centred port on each physical face; asymmetric or multi-port faces
remain open and produce a diagnostic.  The per-column form
...
\texttt{up at=}, \texttt{down at=} & \texttt{center|\{c,\ldots\}} & centre &
  face-specific physical ports at span-local columns \\
\texttt{west at=}, \texttt{east at=} & \texttt{center|\{r,\ldots\}} & rows &
  fused or separate virtual ports at span-local rows \\
...
\texttt{physical=} & \texttt{up|down|updown} & --- &
  per-brick legs on a WIRE row (the row itself draws none); a per-cell
  override of the picture-scope \texttt{physical=} sugar \\
\texttt{role=} & \meta{role} & --- &
  \texttt{operator}, \texttt{marked}, \texttt{extra}, or
  \texttt{passive}; same hue mechanism as \tncmd{tnput}'s
  \texttt{role=} (below, \tnenv{tenkzfree}) \\
\texttt{species=} & \meta{name} & --- &
  hue of a species declared by \tncmd{tnset}; \texttt{role=} prevails
  if both are present \\
\bottomrule
\end{tabularx}\par}
\medskip
...
\tnval{wide=2} spans the isometry across both blocked columns,
\tnval{up at=\{1,2\}} gives two output ports, and
\tnval{down at=center} gives the fused input one port.  The older
\tnval{legs at=\{1,2\}} remains an alias for assigning the same list
to both physical faces; an explicit face key overrides it on that face.
On a virtual face, a sparse list such as \tnval{east at=\{1,3\}}
draws, contracts, and counts only those span-local row slots.
...
\thumbtag{\tncmd{tnspan}}%
\cmdsig{\textbackslash tnspan[\meta{mode}, label pos=\meta{compass}]%
\{\meta{n}\}\{\meta{label}\}\\
\meta{mode}=brace above|brace below|box\qquad
\textbackslash tncut\{\meta{label}\}}

\noindent Write annotation ink directly after its anchor cell.
\tncmd{tnspan} sets a brace over the next $n$ columns or
fits one dashed \tnval{box} to their rendered tensor bodies;
\tncmd{tncut} hangs a dashed stroke across the bond east of its
cell.  Box labels accept the four compass sides, the four corners, and
\tnval{center}; the default is north.  Legs remain outside a box, so its
meaning is ``these local factors form one object'', not a contraction.
Unknown modes, including the removed \tnval{shade}, stop compilation.
\whynote{The brace names columns, the box groups bodies, and the stroke crosses a bond ---
none belongs to a glyph.}  None adds or removes an index: the signature is the
...
\begin{tnexample}[
    formula   = {A^{[3]}_{i_1i_2i_3}=A^{i_1}A^{i_2}A^{i_3}},
    caption   = {a measured blocked-tensor enclosure},
    index     = {The dashed outline measures the three tensor bodies.
                 Their physical legs $i_1,i_2,i_3$ and the two boundary
                 virtual legs remain outside and keep their usual index
                 meaning.  The west label is presentation ink.},
    signature = {boundary|virtual-west=1|virtual-east=1|physical-up=3|physical-down=0},
    label     = {ex:ref-span-box},
  ]
% The body enclosure records a blocked local object, never a trace.
\begin{tenkz}[physical=up, tensor style=box]
  \tn[up=$i_1$]{A}\tnspan[box, label pos=west]{3}{A^{[3]}} &
  \tn[up=$i_2$]{A} & \tn[up=$i_3$]{A}
\end{tenkz}
\end{tnexample}
...
\texttt{boundary=} & \texttt{open|none} & \texttt{none} &
  all four virtual sides open or sealed \\
\texttt{west=}, \texttt{east=},
\texttt{north=}, \texttt{south=} &
  \texttt{open|none|}\newline
  \texttt{trace|cup|}\newline
  \texttt{cup=}\meta{m} & --- &
  override one virtual side; opposing \texttt{trace} values close each
  sheet to itself, while \texttt{cup} contracts matching ket--bra indices \\
\texttt{sheets=} & \meta{n}\texttt{|}\newline
  \texttt{\{ket,bra,\ldots\}} & \texttt{1} &
...
Opposing side traces are periodic virtual closures within each sheet.
Thus \tnval{north=trace, south=trace} closes the row axis but never
ties ket to bra; the site-wise pairing legs make that separate physical
contraction.  A lone \tnval{north=trace} warns and leaves its real
endpoints open rather than inventing a return path.

\begin{tnexample}[
    formula   = {T_C:\bigl(V\otimes\overline V\bigr)^{\otimes R}_{W}
                 \longrightarrow
                 \bigl(V\otimes\overline V\bigr)^{\otimes R}_{E}},
    caption   = {a periodic PEPS-column transfer operator},
    index     = {The body remains a live customization layer: the marked
                 site and edge are drawn together with the declarative
                 closure.  West and east carry the $2R$ free virtual
                 indices; north, south, and every physical pair are closed.},
    label     = {ex:ref-lattice-transfer},
  ]
% T_C : (V tensor Vbar)^R_W -> (V tensor Vbar)^R_E.
% Pairing ink contracts each ket physical index with its bra index;
% in-column bonds contract neighbouring virtual indices; north/south
% trace closes the remaining row-axis virtual indices within each sheet.
\begin{tenkzlattice}[rows=3, cols=1, sheets={ket,bra},
...
\meta{puts, joins, regions}\ \textbackslash end\{tenkzfree\}}
...
widths still apply, so a free picture matches its grid neighbours.  Its
body executes once and remains open to ordinary semantic-style extensions,
for example \tnval{group region/.append style=\{rounded corners=0pt\}}.
...
\tnval{curve} remains the pre-0.6 compatibility alias for \tnval{arc}.

\medskip

\thumbtag{\tncmd{tnregion}}%
\cmdsig{\textbackslash tnregion[\meta{keys}]\{\meta{member names}\}}

\medskip
{\footnotesize
\noindent\begin{tabularx}{\linewidth}{@{}p{20mm}p{32mm}p{14mm}>{\raggedright\arraybackslash}X@{}}
\toprule
\keyhead
\texttt{group} & --- & --- &
  passive dashed grouping outline shared with \tncmd{tnspan}\tnval{[box]} \\
...
Members may be earlier atoms from \tncmd{tnput}.  They may also be named
\tncmd{tnjoin} routes or earlier named regions.  Their actual rendered ink --- including
external bead labels, oblique routes, and route labels --- determines the
enclosure.  An unlabelled \tnval{boundary} atom contributes its endpoint
position and the usual daylight without inventing visible glyph ink.  Region
fills occupy dependency-ordered background layers: an outer named region sits
behind every region it contains, while outlines and labels remain above all
fills.  Unknown, empty, or duplicate names stop compilation rather than
silently dropping ink.

\begin{tnexample}[
    formula   = {\textstyle\sum_{\alpha}
                 A_{\alpha}^{\!\top}\,T\,B_{\alpha}},
    caption   = {typed ports, named joins, and a measured region},
    index     = {The straight wire is the summed virtual index
                 ...
                 The region encloses both atoms and both named routes,
                 but introduces no index.  Joining ports of different
                 kinds warns.},
    label     = {ex:ref-free},
  ]
% typed ports; the directed join reads argument order as
% direction (out of a, into b); the arc carries T
\begin{tenkzfree}
  \tnput[ports={east:virtual, north:physical}]{a}{(0,0)}{A}
  \tnput[ports={west:virtual}]{b}{(14mm,0)}{B}
  \tnjoin[name=jalpha, dir]{a.east}{b.west}
  \tnjoin[name=jT, route=arc, out=90, in=90, label=T]
    {a.north}{b.north}
  \tnregion[slot=secondary, outline, label={$R$},
            label pos=north east]{a,b,jalpha,jT}
\end{tenkzfree}
\end{tnexample}

The entries above are the whole 0.6 language.  A lattice side
\tnval{cup} contracts each matching virtual ket--bra boundary pair;
an optional matrix rides the resolved exterior connector.  This is
distinct from a same-sheet periodic virtual trace.  If a sheet-local
removal leaves only one endpoint of a requested cup, that endpoint remains
open and the picture warns; a zero-length expert side vector likewise leaves
the affected ports open rather than inventing a closure.  Remaining deferred
objects are stated as mathematics in the roadmap of
