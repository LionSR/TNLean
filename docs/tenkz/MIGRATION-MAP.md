...
| TNBNTDecomposition | ch10_bnt:325 | gauge $X$ / basis tensor / $X^{-1}$ threaded by block-index lines $j,q$ with coefficient $\mu$ | needs-judgment (block-index wire type has no tenkz row type; simplest: demote to $A=X(\bigoplus_j\mu_j\otimes A_j)X^{-1}$ display math like its RFP siblings) |
...
| TNMPDORecursiveStructureOperator | ch21_mpdo_rfp_fusion_isometries:610 | `tenkzfree`: degree-four $X_s$ boxes meet open $k_s,\alpha_s,\beta_s,\gamma_s$ copy rails; $\gamma_s$ and $\alpha_{s+1}$ share one open rail, and the final rail selects traced $M_{\gamma_r}$ with its physical pair open | source-faithful free-tier routing; no package change |
