...
\subtitle{{\small 2,221 lines $\cdot$ 14 modules $\cdot$ 0 sorries\;\;$\cdot$\;\;Mathlib v4.27.0\;\;$\cdot$\;\;\cite{Cirac2021Matrix}}}
...
\textcolor{leangreen}{\textbf{[proved in Defs.lean]}}
...
\begin{block}{Theorem (Single-block, injective case) \hfill{\small [\cite{Cirac2021Matrix}, Thm\,4.1]}}
...
\begin{itemize}
\item Gauge equivalence $\Leftrightarrow$ same gapped phase
\item SPT order: symmetry action on gauge d.o.f.\ classifies phases via $H^2(G,U(1))$
\item Injective MPS are fixed points of the RG flow
\item Parent Hamiltonians: unique ground state iff injective; gauge freedom is the only ambiguity
\end{itemize}
\end{frame}
...
{\small\textbf{\textcolor{softred}{Physics proof issues:}}}
\begin{itemize}\itemsep0.1em
\item {\small ``It is easy to see that\dots''}
\item {\small Implicit assumptions ($D>0$, algebraically closed, \dots)}
\item {\small Deep algebra (Skolem--Noether, ring simplicity)}
\item {\small SPT classification depends on correctness}
\end{itemize}

\column{0.48\textwidth}
{\small\textbf{\textcolor{leangreen}{What formalization gives:}}}
\begin{itemize}\itemsep0.1em
\item {\small Every step machine-checked, all hypotheses explicit}
\item {\small 0 sorries = no gaps}
\end{itemize}

\vspace{0.3em}
{\small\textbf{What we found:}}
\begin{itemize}\itemsep0.1em
\item {\small The paper proof is correct}
\item {\small ``Obvious'' steps are genuinely nontrivial}
\item {\small $D>0$ needed in 6+ places}
...
\vspace{0.3em}
{\small\textbf{Analogy:} A proof assistant is a compiler for mathematics.
If it compiles, the proof is correct.}
\end{frame}
...
\begin{itemize}\itemsep0.1em
  ...
  \item {\small For us: matrix algebras, simple rings, Skolem--Noether}
\end{itemize}

\column{0.46\textwidth}
{\small\textbf{Recent successes:}}
\begin{itemize}\itemsep0.1em
  \item {\small Liquid Tensor Experiment (Scholze, 2022)}
  \item {\small PFR Conjecture (Tao et al., 2023)}
  \item {\small Sphere eversion (2023)}
\end{itemize}

\vspace{0.4em}
{\small\textbf{Our project:} Lean 4 + Mathlib v4.27.0}
\end{columns}
\end{frame}
...
\vspace{0.3em}
{\small Compare: ``by a standard argument, $T$ is multiplicative'' $\to$ 159 lines.}
\end{frame}
...
\vspace{0.3em}
Expansion factor: $\sim$3 pages $\to$ 2,221 lines ($\approx 10\times$)
\end{frame}
...
\begin{itemize}
  \item Algebraic injectivity: $\{A^i\}$ span $\MD$\\
    {\small (avoids CP maps, Perron--Frobenius)}
  \item $\GLC(D,\mathbb{C})$ for gauge transforms
  \item Sigma-type indices for block-diagonal\\
    {\small (avoids \texttt{Fin} arithmetic)}
  \item Word evaluation via lists
\end{itemize}
\end{columns}
\end{frame}
...
\vspace{0.1em}
\begin{center}
{\small Pure algebra --- no wavefunctions, no channels, no spectral theory.}
\end{center}
\end{frame}
...
\textbf{Trace nondegeneracy:}  $\mathrm{tr}(MN)=0 \forall N \Longrightarrow  M=0$\quad {\small (22 lines, via single-entry matrices)}
\end{block}
...
\end{lstlisting}
\vspace{-0.1em}
{\small\textbf{Our bridge} (149 lines in \texttt{SkolemNoether.lean}):}
...
\begin{itemize}
  \item[\textcolor{leangreen}{$\blacksquare$}] Trace nondegeneracy (22 lines)
  \item[\textcolor{leangreen}{$\blacksquare$}] Trace pairing map $\Phi_A$ (98 lines)
...
\begin{itemize}
  \item Perron--Frobenius for CP maps is not in Mathlib
  \item The algebraic route uses only linear algebra + ring theory
  \item The paper already notes: injective case doesn't need channel theory
\end{itemize}
\end{frame}
...
\vspace{0.15em}
{\small\textbf{Proof outline} (163 lines in \texttt{MultiBlock.lean}):}
...
\vspace{0.1em}
{\small Reduces multi-block to single-block weighted by $\mu_k^N$.}
...
{\small If $\{\mu_k\}$ are distinct, this is a Vandermonde system.}
...
\vspace{0.15em}
{\small Block Permutation Theorem: automorphisms of $\prod_k M_{n_k}(\mathbb{C})$ permute simple factors (two-sided ideal classification $+$ atoms).}

\vspace{0.1em}
{\small Remaining: linear extension $T$ on the full semisimple algebra (multi-block Steps 2--4).}
\end{frame}
...
\vspace{0.4em}
{\small Hypotheses are explicit. The single-block theorem is fully proven from Mathlib.}
...
\begin{enumerate}
  \item Mathlib's abstract algebra coverage is extensive
...
\vspace{0.8em}
The paper proof is correct. Formalization found no errors, but forced precision about every hypothesis (especially $D>0$).
\end{frame}
...
\node[mod,fill=leangreen!50!black,draw=leangreen] (bpm) at (7.5,3.9) {BlkPermMPS (336)};
\node[mod,fill=amber!40!black] (piae) at (5,6.5) {PiAlgExt (370)};
\draw[arr] (defs) -- (trans);
\draw[arr] (defs) -- (tp);
\draw[arr] (tnd) -- (tp);
\draw[arr] (defs) -- (tnd);
...
{\footnotesize
\texttt{PiAlgebraExtension} 370\,l,
\texttt{BlockPermMPS} 336\,l,\\
\texttt{FundThmMulti} 237\,l,
\texttt{BlockPerm} \textcolor{leangreen}{223}\,l,\\
...
\begin{columns}[T]
\column{0.48\textwidth}
{\small\textbf{Near-term:}}
\begin{itemize}\itemsep0.05em
  \item {\small Parent Hamiltonians}
  \item {\small SPT classification --- $H^2(G,U(1))$}
  \item {\small PEPS (higher-dim tensor networks)}
\end{itemize}

\column{0.48\textwidth}
{\small\textbf{Longer-term:}}
\begin{itemize}\itemsep0.05em
  \item {\small Quantum channel library}
  \item {\small Contribute back to Mathlib}
\end{itemize}
\end{columns}
...
\vspace{0.8em}
\textcolor{leangreen}{\textbf{2,221 lines\quad$\cdot$\quad 14 modules\quad$\cdot$\quad 0 sorries\quad$\cdot$\quad 0 axioms}}
\end{center}
\end{frame}
...
\begin{itemize}\itemsep0.01em
  \item Surjectivity (to find $x$)
  \item Multiplicativity ($T(x\cdot 1) = T(x)T(1)$)
\end{itemize}
This is why Step~3 must precede Skolem--Noether.}
...
\begin{itemize}\itemsep0.01em
  ...
  \item {\scriptsize $\to$ Everything downstream}
\end{itemize}
...
\end{lstlisting}
\vspace{0.02em}
{\scriptsize\textcolor{amber}{\textbf{149 lines}} for this bridge alone!}
\end{columns}
\end{frame}
...
\column{0.48\textwidth}
{\small\textbf{Why 163 lines?}}\\[0.05em]
