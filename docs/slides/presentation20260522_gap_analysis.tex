...
\vspace{0.1em}
{\small \textcolor{leangreen}{2792 lines, 16 modules, 0 sorries, 0 axioms.}\quad
The gap is a hypothesis we can't yet derive, not a sorry.}
\end{frame}
...
\vspace{0.2em}
{\small This looks like Vandermonde invertibility, but it isn't.}
\end{frame}
...
\vspace{0.5em}
{\small Distinct bases $\Rightarrow$ linearly independent columns.}
\end{frame}
...
\vspace{-0.5em}
{\small If $V$ is invertible, $V \cdot c = 0$ implies $c = 0$.}
...
\vspace{-0.3em}
{\small If $\sum_k c_k \mu_k^N = 0$ for $N = 0, \dots, r{-}1$ and the $\mu_k$ are distinct, then every $c_k = 0$.}
...
\vspace{-0.2em}
{\small Each row has $d^N$ equations, but the $\sigma$-spaces for different $N$ are unrelated.}
\end{frame}
...
\vspace{0.3em}
\begin{center}
{\small Both fail: the $N$--$\sigma$ coupling is structural.}
\end{center}
\end{frame}
...
\vspace{-0.3em}
{\small In \cite{Cirac2021Matrix}, \S IV, the separation uses the \textbf{transfer operator} (quantum channel):}
\[
...
\column{0.44\textwidth}
\begin{center}
% Formula: E_A(rho) = sum_i A^i rho (A^i)^dagger.
% Ink: A is the ket tensor and the starred A is its bra-layer adjoint.
...
% The state rides the closing cup: the cup sugar stands the ring on the
% generated cup wire's own station (r_onwire_cup_arc.tex pins the arc).
\begin{tenkz}[rows={ket,bra}, cols=1, west=open, east={cup=$\rho$}]
  \tn[label pos=180]{A} \\
  \tn[label pos=180]{$\overline{A}$}
  \tnmark[form=label, label pos=90]{on open-west-1 0.5}{$\mathcal E_A(\rho)$}
\end{tenkz}
\end{center}
\end{columns}

\vspace{0.3em}
{\small As $N \to \infty$, the dominant block dominates. The spectral gap ensures exponential decay of the rest. Iterate to extract each block.}
\end{frame}
...
\node[rectangle,rounded corners,draw=amber,fill=cardbg,text=white,
  font=\small,align=center,minimum width=11cm,minimum height=0.9cm] {
  The physics proof uses analysis (spectral theory + positivity + convergence)\\
  --- the point where the algebraic bypass reaches its limit.
};
\end{tikzpicture}
\end{center}
\end{frame}
...
\vspace{0.15em}
{\small The gap: $\delta_k(N, \sigma)$ depends on $N$, which is both the Vandermonde exponent and the configuration space size.}
...
\vspace{-0.2em}
{\small Not a standard Vandermonde problem: $V_k(N)$ and $V_k(N')$ live in different spaces for $N \neq N'$.}

\vspace{0.15em}
{\small\textbf{Why it's still true:}}
\vspace{-0.2em}
\begin{itemize}\itemsep0em
  \item {\small $N \mapsto V_k(N)$ arises from matrix traces, not arbitrary}
  \item {\small Transfer operator $\mathcal{E}_{A_k}$ governs asymptotics as $N \to \infty$}
  \item {\small Spectral gap gives exponential convergence $\to$ block extraction}
\end{itemize}
\end{frame}
...
\vspace{0.5em}
{\small The final theorems take per-block \texttt{SameMPV} as hypothesis. Either a purely algebraic proof closes the gap ($\sim$100--200 lines), or Mathlib's functional analysis grows to support the spectral argument ($\sim$3000--5000 lines).}
\end{frame}
...
\end{center}
\vspace{-0.3em}
{\small Everything above and below the gap is machine-verified.}
\end{frame}
