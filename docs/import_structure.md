...
`HANDWRITTEN_IMPORT_OWNERSHIP` table. An exception delegates named production
modules to a directly imported handwritten sub-aggregator; all other handwritten
imports remain incidental and do not count toward manifest coverage. Keeping the
exceptions explicit prevents a policy change from rewriting unrelated umbrellas.
