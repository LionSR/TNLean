...
\end{align}
The source coordinates are lines~3993--4005 of \path{cornerproblem.tex} in
...
The corrected strict-range identity and both contracted formulas are proved in
the formal development under the hypotheses stated above.\footnote{Formal
declarations:
\leanid{MPSTensor.IsReduction.evalWord_eq_reductionResidual_add_strictWindowRangeSum},
\leanid{MPSTensor.IsReduction.mul_evalWord_eq_reductionLeftContractedRangeSum},
and
\leanid{MPSTensor.IsReduction.evalWord_mul_eq_reductionRightContractedRangeSum}.
The corresponding blueprint entries are
\path{thm:mps_reduction_residual_expansion} and
\path{thm:mps_reduction_contracted_expansions} in
