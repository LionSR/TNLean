...
\leanid{MPOTensor.%
blockTwo_isRFPViaTS_of_commonWeightAbsorbedBNTChannels}. The outer projectors
are assumed to form a resolution of the one-site physical identity, and on
one blocked site the measured projection is the reindexed matrix
$P_s\otimes I$. Compression selects the matching absorbed representative at
both required closure lengths; its supplied channel is then applied. This
leaves two independent gaps in the source implication. First, one blocked
channel pair must be constructed for every absorbed representative, by either
\ghissue{6775} or \ghissue{6793}. Second, the projector completeness required
by the conditional assembly must be derived from the source hypotheses. The
present Markov-sector theorem identifies the projector sum only with the
active Markov projection, while the two-sided absorbed-support theorem does
not prove that this projection is the identity. This completeness problem is
tracked in \ghissue{6807}. Its elimination may either prove that the active
Markov projection is the identity, or prove that both relevant closures lie
in its corner and complete the channels trace-preservingly on the orthogonal
complement.
...
orthogonal outer physical sectors is formalized in \ghissue{6632}; deriving
or harmlessly completing the required outer projector resolution remains
\ghissue{6807}.
