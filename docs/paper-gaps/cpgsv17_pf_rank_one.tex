...
is not normal.  The current declaration does not yet package biCF, simplicity,
MPDO positivity, and SAL through the high-level Case-II interfaces.  That
packaging is residual work; the counterexample already decides the scalar
normality inference itself.  Propositions `prop2to3` and `prop2to5`, and the
final Theorem~4.9 implications depending on them, remain not ready rather than
being refuted by this slice. For the rescaling-stable example, the
...
This one-factorization result does not by itself prove the full implication
Theorem~4.9(iv)$\Rightarrow$(v). Condition~(iv) supplies a separate physical
factorization for each outer BNT element, together with mutually orthogonal
outer physical supports. Proposition~C.7 constructs a channel pair within one
such factorization. The formal development does not yet combine the resulting
family into one trace-preserving completely positive coarse-graining map and
one refinement map for the whole BNT sum. A direct concatenation of the inner
...
projectors to control a direct sum of channel pairs. This is a separate
all-sector obligation in addition to the Case-II factorization problem; it is
tracked in \ghissue{6632}.
