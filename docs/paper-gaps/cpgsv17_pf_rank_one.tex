...
The formal development now proves two corrected sufficient rank-one criteria.
The first derives the factorization from positive semidefiniteness, trace
normalization, and the trace-power identities.  The second derives $T^2=T$
from the sector-pairing identity and linear independence of the closed sector
tensors; trace one then gives the same factorization directly.  These are
matrix criteria, not constructions of the full local structure in the source
corollary.  A former statement placed independent Markov and matrix data in a
single hypothesis without relating them to each other or to an MPO tensor. It
has therefore been removed, together with the two results whose only purpose
was to assemble this hypothesis.
...
Theorem~4.9 cannot be obtained by the printed argument.  The two direct
rank-one criteria remain available under positive semidefiniteness or the
appropriate linear-independence hypothesis, but neither criterion supplies
the full structural corollary, and those additional conditions must not be
