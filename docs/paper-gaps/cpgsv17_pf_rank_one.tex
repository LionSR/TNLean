...
that the active Markov projection is the ambient identity. Thus the remaining
gap in the source implication is only the construction of one blocked channel
pair for every absorbed representative, by either \ghissue{6775} or
\ghissue{6793}.
...
matrix refutes only the proof path tied to those four selected sectors. A
general coarsening theorem, or a counterexample to the existence of any such
factorization under the complete absorbed-representative hypotheses, remains
missing in \ghissue{6775}.

The same raw tensor is not the injective normal tensor assumed in Case~I, and
its normal representative loses literal ZCL. It therefore does not refute
Lemma~C.5 under its complete standing hypotheses. The completed normal Case-I
...
refutes the exact printed implication (iv)$\Rightarrow$(v). For the source
(ii)$\Rightarrow$(v) route, \ghissue{6775} retains the printed
per-representative factorization assertion, while \ghissue{6793} records the
possible alternative of constructing the blocked channel pair by a genuine
coarsening or sector-mixing argument. The conditional combination through the
orthogonal outer physical sectors is formalized in \ghissue{6632}, and its
trace-preserving completion outside the active support resolves
\ghissue{6807}.
