...
the congruence
\begin{align}
    O_f^{\mathrm{rev}} & =W^{-1/2}\,O_f\,W^{-1/2},
    \qquad W=\operatorname{diag}\prod_k d_{x_k},
    \label{eq:fib_review}
\end{align}
...
$O_f^{\mathrm{rev}}W=W^{-1/2}O_fW^{1/2}$ are similar to $O_f$
...
operators stand for the operators $O_f^{\mathrm{rev}}W$, composed with the
weight $W$ on plaquette configurations. The formalization records both readings: the literal one fails the fusion rules,
...
This rule is garbled: the first condition names the label $c$ where the
fusion channel $e$ belongs, and the last one is misprinted. The formalized review tensor uses instead the source's F-symbols
