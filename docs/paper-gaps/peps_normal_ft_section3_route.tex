...
with two right-collar contiguous $2\times3$ rectangles. The normalized horizontal and
vertical edge blockings are now both packaged as one-edge blocking data with
...
implies injectivity of all three blocks. For the vertical edge, rotated local
$T$-injectivity now implies injectivity of the complementary block by union
...
\begin{enumerate}
    ...
    \item Prove that the paper's regions $R$, $S$, and $T$ are injective
          ...
          injective once the rotated local $T$-region is injective. The present
          normalized $5\times6$ local-window $T$ model is now known not to
...
\begin{itemize}
    ...
    \item Normalized horizontal and vertical red/blue/complement blocking data:
          ...
          vertical complement injectivity is reduced to rotated
          $T$-injectivity.
