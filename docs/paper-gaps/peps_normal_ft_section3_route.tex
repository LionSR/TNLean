...
\begin{enumerate}
    \item The tensor-level union-of-injective-regions lemma is now proved, with
          ...
          Two exploratory closures of this argument have been removed.  The
          first carried a full $A$-boundary parameter through a row indexed only
          by $B$-boundary configurations; that row cannot see the
          overlap-to-difference edges inside $B$.  The second introduced a
          host-spanning three-block geometry and reduced to the blocked weights
          of $A\setminus B$, but the hypotheses do not make that strict
          subregion injective.  Neither construction contributes to the final
          proof.

          The live route retains the left and right three-block geometries and
          their two inverse applications.  It closes the missing host indices by
          carrying only the $P_0$-outer part of the $A$-boundary, rather than the
          full $A$-boundary or the abandoned host geometry.
