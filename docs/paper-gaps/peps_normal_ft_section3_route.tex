...
\begin{enumerate}
    ...
    \item In the translationally invariant case, show that the edge gauges obtained
          ...
          The precise step that remains is the residual-multiplicity factorization, the inserted-site
          analogue of \leanid{TNLean.PEPS.regionInsertedCoeff_eq_smul_edgeInsertedCoeff}. The
          residual sum at a fixed local configuration \(\eta\) at \(v\) is not the bare blocked weight
          of \(R\): it carries (i) a consistency delta forcing \(\mu\) and \(\eta\) to agree on the
          \(v\)-incident edges running to \((\operatorname{insert} v\,R)^c\) (these are
          \(S\)-boundary but not \(R\)-boundary), and (ii) the interior bond multiplicity of the
          \(S\)-boundary edges away from \(R\), which are free in the residual sum but pinned by
          \(\mu\). Both factors are bond-dimension data, hence identical for \(A\) and for the
          reindexed \(\widetilde B\); they therefore cancel in the quotient of the two region
          proportionalities, leaving \(\sum_\eta A.\mathrm{component}\,v\,\eta \cdot
          (c_R\,\widetilde B\,\text{-residual})\). Inverting the inserted-site tensor map through the
          \(R\)-block injectivity then yields the per-vertex relation
          \(A_v=(c_S/c_R)\cdot\widetilde B_v\). Until this residual factorization is built,
