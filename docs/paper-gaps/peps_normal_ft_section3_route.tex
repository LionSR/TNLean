...
coordinate right and upward edges now store the corresponding named margin
predicates. Consequently this margin package does not apply to horizontal edges
without enough left or right margin, nor to vertical edges without enough bottom
or top margin; the corresponding $7\times7$ examples are recorded explicitly,
including a single four-sided boundary obstruction. Thus the remaining
every-edge construction must include the boundary geometry rather than only
translating the interior margin package; in particular, no family of this
margin-cover data exists over all edges of the present open $7\times7$ graph.
The realigned local and rotated $T$ covers and the construction of the
appropriate data for every edge remain open.
...
\begin{itemize}
    ...
    \item Conditional assembly from translated edge windows:
          ...
          window constructors accept those predicates directly, the per-edge
          margin-cover package stores those predicates, and the resulting
          open-boundary obstruction for
