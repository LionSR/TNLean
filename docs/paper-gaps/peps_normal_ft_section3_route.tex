...
The rotated local-window model for vertical edges has the analogous
obstruction in a $6\times5$ window. The origin-based horizontal
edge-complement model has the same obstruction in the $5\times7$ frame, and
the origin-based vertical edge-complement model has the corresponding
obstruction in the $7\times5$ frame. These no-cover results do not contradict
the paper. They show that the source's injectivity argument for $T$ and $A_3$
must be aligned with the finite PEPS geometry, rather than read directly from
the present exact-cover criterion.
...
of such one-edge data assembles into those hypotheses, and each datum supplies
...
cover. A family of these data also assembles into the normal edge-blocking
hypotheses, and the assembled hypotheses recover the supplied one-edge datum
at each edge.
...
\begin{itemize}
    ...
    \item Conditional assembly from translated edge windows:
          ...
          data assembles into the normal edge-blocking hypotheses. The
