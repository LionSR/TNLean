...
The normal-PEPS part of Ref.~\cite{Molnar2018NormalPEPS} begins after the
injective PEPS theorem. Its purpose is not to replace the injective proof, but
to make
...
The proof in Ref.~\cite{Molnar2018NormalPEPS} has three layers. First, Lemma
\path{injective_union} proves
...
\footnote{The source is
    \path{Papers/1804.04964/paper_normal.tex:1324--1400}, Ref.~\cite{Molnar2018NormalPEPS}.}

Second, the translationally invariant square-lattice theorem of
Ref.~\cite{Molnar2018NormalPEPS} assumes that every $2\times 3$ and
$3\times 2$ region is injective. By
...
\footnote{The regions $R$, $S$, and $T$ are drawn at
    \path{Papers/1804.04964/paper_normal.tex:1407--1443}, Ref.~\cite{Molnar2018NormalPEPS}.}

Third, the proof of Theorem \path{3} in
Ref.~\cite{Molnar2018NormalPEPS} blocks a neighborhood of each edge into
...
\footnote{The general theorem is
    \path{Papers/1804.04964/paper_normal.tex:1574--1585}, Ref.~\cite{Molnar2018NormalPEPS}.}
...
\begin{align}
    ...
    \label{eq:peps_normal_ft_section3_route_local_target_point}        \\
    p \in R^{2\times3}_{x,y}
      & \Longrightarrow q_{23}:=(0,2)\in R^{2\times3}_{x,y},
    \qquad q_{23}\notin T_{\mathrm{loc}},
    \label{eq:peps_normal_ft_section3_route_two_by_three_forced_point} \\
    p \in R^{3\times2}_{x,y}
      & \Longrightarrow q_{32}:=(2,1)\in R^{3\times2}_{x,y},
    \qquad q_{32}\notin T_{\mathrm{loc}}.
    \label{eq:peps_normal_ft_section3_route_three_by_two_forced_point}
\end{align}
By (\ref{eq:peps_normal_ft_section3_route_local_target_point}), any exact cover
of $T_{\mathrm{loc}}$ contains a rectangle covering $p$. Equations
(\ref{eq:peps_normal_ft_section3_route_two_by_three_forced_point}) and
(\ref{eq:peps_normal_ft_section3_route_three_by_two_forced_point}) show that
such a rectangle also covers a point outside $T_{\mathrm{loc}}$. The same
calculation applies to the rotated local-window model for vertical edges in a
$6\times5$ window, the origin-based horizontal edge-complement model in the
$5\times7$ frame, and the origin-based vertical edge-complement model in the
$7\times5$ frame. The forced-point data are summarized in
(\ref{eq:peps_normal_ft_section3_route_forced_point_table}):
\begin{align}
    \begin{array}{c|c|c|c}
    ...
    \end{array}
    \label{eq:peps_normal_ft_section3_route_forced_point_table}
\end{align}
...
\begin{itemize}
    ...
    \item The abstract contraction-chain criterion for Lemma
          ...
          \leanid{TNLean.PEPS.InjectiveUnionContractionChain}. For a boundary
          tensor $X$, write $\mathcal C_I(X)$ for the contraction over the
          indexed subfamily $I$. The criterion assumes
          \begin{align}
              \mathcal C_{\{0,1,2\}}(X)=0
               & \Longrightarrow \mathcal C_{\{2\}}(X)=0,
              \label{eq:peps_normal_ft_section3_route_union_contraction_first_step}  \\
              \mathcal C_{\{2\}}(X)=0
               & \Longrightarrow \mathcal C_{\{1,2\}}(X)=0,
              \label{eq:peps_normal_ft_section3_route_union_contraction_second_step} \\
              \mathcal C_{\{1,2\}}(X)=0
               & \Longrightarrow X=0.
              \label{eq:peps_normal_ft_section3_route_union_contraction_third_step}
          \end{align}
          The implications
          (\ref{eq:peps_normal_ft_section3_route_union_contraction_first_step}),
          (\ref{eq:peps_normal_ft_section3_route_union_contraction_second_step}),
          and
          (\ref{eq:peps_normal_ft_section3_route_union_contraction_third_step})
          imply that the union contraction has trivial kernel.
...
\begin{enumerate}
    \item The tensor-level union-of-injective-regions lemma is now proved, with
          ...
          \leanid{TNLean.PEPS.regionInteriorBondProd_smul_geometryBlueWeight_eq}
          ...
          and the bridge produced the rebuild hypothesis
          $\sum_{b_2}\mathrm{row}(b_2)\cdot\mathrm{cc}_R(b_2,\sigma_{P_2},
              \beta_{P_1})=0$ for the unrestricted bridge row
          \leanid{TNLean.PEPS.overlapBridgeRow}.

          Feeding the summed bridge row to the rebuild and inverting $B$
          exposes one residual freedom: a row over the $B$-boundary alone does
          not separate the $P_0$-outer host indices.  The live closure restricts
          the coefficient family to each $P_0$-outer fiber before applying the
          left strip and bridge.  The resulting fiber row vanishes after the
          right rebuild, and the pair consisting of its $B$-boundary label and
          its $P_0$-outer label determines the union boundary label.  This gives
          the final injectivity statement
          \leanid{TNLean.PEPS.regionBlockedTensorInjective_union_overlap}.

          A full $A$-boundary parameter is unsuitable because a row indexed
          only by $B$-boundary configurations cannot see the
          overlap-to-difference edges inside $B$.  A host-spanning three-block
          geometry also does not close the argument: it reduces to blocked
          weights of $A\setminus B$, while the hypotheses do not make that
          strict subregion injective.

          The proof instead uses the left and right three-block geometries and
          their two inverse applications, carrying only the $P_0$-outer part of
          the $A$-boundary to close the missing host indices.
    ...
    \item Reuse the injective-chain insertion route after this blocking:
          ...
          per-edge gauge matrix.\footnote{
              ...
              \leanid{TNLean.PEPS.RegionInsertionTransfer.fwdAlgEquiv}, and
              \leanid{TNLean.PEPS.exists_regionEdgeGauge_of_transfer}.}

          The identity-insertion bridge is
          (\ref{eq:peps_normal_ft_section3_route_identity_insertion_bridge}).
          Here $C_A(R,f,X;\sigma,\tau)$ denotes the region-inserted coefficient
          for the tensor $A$, the region $R$, the boundary edge $f$, the boundary
          matrix $X$, the region physical configuration $\sigma$, and the
          complement physical configuration $\tau$. The symbol $D_A(e)$ denotes
          the bond dimension of the edge $e$, $\partial R$ denotes the set of
          boundary edges of $R$, and
          ...
          \begin{align}
              ...
              \label{eq:peps_normal_ft_section3_route_identity_insertion_bridge}
          \end{align}
          The proof of
          (\ref{eq:peps_normal_ft_section3_route_identity_insertion_bridge})
          decomposes the closed coefficient into region and complement
          ...
          $\prod_{e\notin\partial R}D_A(e)$ is therefore part of
          (\ref{eq:peps_normal_ft_section3_route_identity_insertion_bridge}), not
          a normalization that can be discarded.\footnote{
              Formal declarations:
          ...
          span and not the per-leg decomposition across the boundary bond.\footnote{
              ...
              \leanid{TNLean.PEPS.exists_regionEdgeGauge_of_coeffTransfer}.}
          ...
          are contracted diagonally (\leanid{TNLean.PEPS.SameAwayFromRBBundle}). Under the
          single-crossing hypothesis this coefficient is read as the ordinary single-edge
          region-inserted coefficient, and against a bond-model-conjugated matrix it expands
          as a configuration sum
          (\leanid{TNLean.PEPS.redBundleInsertedCoeff_singleton},
          \leanid{TNLean.PEPS.redBundleInsertedCoeff_bondModelMatrix_eq_configSum}).
          The relaxed crossing agreement
          ...
          \leanid{TNLean.PEPS.crossingLabel_eq_redBoundaryRBCrossing}, while the agreement
          of the red boundary labels away from the red-to-blue crossings is characterized
          by \leanid{TNLean.PEPS.sameAwayFromRBBundle_regionBoundaryLabel_iff}. The host side collapses
          ...
          (\leanid{TNLean.PEPS.bondLocal_iff_coeffTransfer}). The forward
    ...
    \item In the translationally invariant case, show that the edge gauges obtained
          ...
          crossing carried across.\footnote{
              Formal declarations:
              \leanid{TNLean.PEPS.regionBlockedWeight_transport},
              \leanid{TNLean.PEPS.regionBlockedTensorInjective_transport},
              \leanid{TNLean.PEPS.transportBlockingDataAlong}, and
              \leanid{TNLean.PEPS.isCrossingEdge_transportBlockingDataAlong}.}
          The reference blocking datum at each class's reference edge is
          constructed from the rectangular injectivity hypotheses directly: the
          reference red and blue blocks are the two source
...
Ref.~\cite{Molnar2018NormalPEPS}; source file
...
\begin{enumerate}
    ...
    \item \emph{Bond dimensions derived from the blocking hypotheses.}  The
          ...
          distinguished coarse bond then forces equal sizes; see the remark at
          lines 560--583 of Ref.~\cite{Molnar2018NormalPEPS} in
          \path{Papers/1804.04964/paper_normal.tex}, which states that the
          algebra isomorphism makes the bond dimensions on the two sides agree.
...
\path{Papers/1804.04964/paper_normal.tex}, Ref.~\cite{Molnar2018NormalPEPS}), not a narrowing.  The source's
...
\path{Papers/1804.04964/paper_normal.tex}, Ref.~\cite{Molnar2018NormalPEPS}).  The proof it generalizes
...
\path{Papers/1804.04964/paper_normal.tex}, Ref.~\cite{Molnar2018NormalPEPS}) states that for a translationally
...
layer to named proof nodes.\footnote{Chapter~13a writes the diagrams as native
    bodies beside the corresponding statements. The web blueprint captures
    the same bodies and renders them through the generic tenkz bridge.}
...
kernels (\path{bondLocal_iff_coeffTransfer}), the forward
...
Theorem~3 (lines 1475--1498 of Ref.~\cite{Molnar2018NormalPEPS}) sit on a
$7\times7$
...
\path{TNLean/PEPS/NormalSquareTI.lean}); and the translation-invariant absorption datum and its
...
The first corollary after the theorem \path{normal} in
Ref.~\cite{Molnar2018NormalPEPS} (lines 1585--1622; source file
...
Second, the translationally invariant corollary of
Ref.~\cite{Molnar2018NormalPEPS} (lines 1624--1661,
...
The translation-invariant corollary of Ref.~\cite{Molnar2018NormalPEPS}
(lines 1624--1661: a single gauge $Z$
...
Writing $T$ for the cyclic shift, the Applications corollary in
lines 1801--1890 of Ref.~\cite{Molnar2018NormalPEPS} asserts
\begin{align}
    T\Psi(A_1,\ldots,A_n)=\Psi(A_1,\ldots,A_n)
     & \Longrightarrow D_1=\cdots=D_n,
    \label{eq:peps_normal_ft_section3_route_cyclic_shift_equal_bond_dimensions} \\
    T\Psi(A_1,\ldots,A_n)=\Psi(A_1,\ldots,A_n)
     & \Longrightarrow
    \exists B,\ B\text{ is injective and }
    \Psi(A_1,\ldots,A_n)=\Psi(B,\ldots,B).
    \label{eq:peps_normal_ft_section3_route_cyclic_shift_repeated_tensor}
\end{align}
After telescoping the gauges obtained by comparing the family with its cyclic
...
\path{Papers/1804.04964/paper_normal.tex}, Ref.~\cite{Molnar2018NormalPEPS}), tracked by \ghissue{2921}. Its
...
The plan recorded in the previous section is carried out: Lemma~5 of
Ref.~\cite{Molnar2018NormalPEPS} (lines 2045--2255) is formalized for matrix
tensors, and the
...
The corollary at lines 2297--2318 of Ref.~\cite{Molnar2018NormalPEPS} ---
the torus theorem with one
