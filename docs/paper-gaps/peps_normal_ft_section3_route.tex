...
frame. The origin-based vertical edge-complement model has the corresponding
obstruction whenever the ambient lattice contains the $7\times5$ frame. The
normalized $5\times7$ horizontal-edge geometry is formalized:
$A_3$ is the
...
\begin{enumerate}
    ...
    \item Prove that the paper's regions $R$, $S$, and $T$ are injective
          ...
          frame, and the origin-based vertical edge-complement model has the
          corresponding obstruction whenever the ambient lattice contains the
          $7\times5$ frame. These no-cover diagnostics now use a common
          point-forcing obstruction lemma for exact rectangular covers. The
          remaining coordinate work is to realign the source local and rotated
          $T$
