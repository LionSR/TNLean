...
closure and rectangular injectivity. The vertical-edge $7\times5$ counterpart
is now formalized as well: the corresponding complement is the local-window
$T$-region together with two right-collar contiguous $2\times3$ rectangles, and
the same conditional injectivity consequence follows from union closure. The
normalized $5\times7$ horizontal edge is now packaged with its red, blue, and
complementary regions: the edge endpoints lie in the red and blue regions,
respectively, the three regions are disjoint and cover the full finite square
lattice, and local $T$-injectivity implies injectivity of all three blocks. The
realigned local $T$ cover, the vertical-edge blocking data, and the translated
...
\begin{itemize}
    ...
    \item Normalized $5\times7$ top-collar decomposition of that block:
          ...
          \#2004. This node now also contains the normalized $7\times5$
          right-collar counterpart for vertical edges.
    \item Normalized horizontal-edge red/blue/complement blocking data:
          blueprint
