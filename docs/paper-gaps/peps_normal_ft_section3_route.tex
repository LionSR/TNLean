...
\begin{enumerate}
    ...
    \item In the translationally invariant case, show that the edge gauges obtained
          ...
          The precise step that remains is the scalar extraction: feeding the two region
          proportionalities \(A_R=c_R\,\widetilde B_R\) and \(A_S=c_S\,\widetilde B_S\) (the outputs
          of \leanid{TNLean.PEPS.regionComplement_comparison} at \(R\) and \(S=\operatorname{insert}
          v\,R\)) through the inserted-site factorization and inverting the inserted-site tensor map
          through the \(R\)-block injectivity (linear independence of \(R\)'s blocked family), to
          yield the per-vertex relation \(A_v=(c_S/c_R)\cdot\widetilde B_v\) at matching local
          configurations. Until this scalar extraction is built on top of the landed factorization,
          \path{hPV} remains the conditional input of the torus theorem rather than a derived
          consequence.
