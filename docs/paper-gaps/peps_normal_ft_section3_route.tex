...
\begin{itemize}
    \item \path{lem:peps_injectiveRegion_union} states the union-of-injective
          regions lemma. It is tracked by \ghissue{1374}. Chapter 13a now
          includes both the four-region blocking picture and the graphical proof
          sequence from the source proof, via
          \path{\TNPEPSInjectiveRegionUnion} and
          \path{\TNPEPSInjectiveRegionUnionProof}.
...
are also formalized. Chapter 13a now includes TikZ diagrams for the rectangular
cover criterion and the top-collar decomposition of $A_3$, so these formal
sufficient conditions can be compared directly with the source drawings. The
two edge blocks removed from the displayed $T$-window
...
layer to named proof nodes. Chapter~13a calls the public TikZ commands in
\path{blueprint/src/macros/tn_print.tex}, and the web blueprint renders cached
SVGs from those same commands through \path{blueprint/src/Packages/tn_diagrams.py}.
...
The normal-PEPS diagram commands are registered in
