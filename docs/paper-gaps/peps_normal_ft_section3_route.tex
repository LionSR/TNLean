...
known not to apply to a left-boundary horizontal edge or to a bottom-boundary
vertical edge of the $7\times7$ lattice. Thus the remaining every-edge
construction must include the boundary geometry rather than only translating
the interior margin package; in particular, no family of this margin-cover
data exists over all edges of the present open $7\times7$ graph. The realigned
local and rotated $T$ covers and the construction of the appropriate data for
every edge remain open.
...
\begin{itemize}
    ...
    \item Conditional assembly from translated edge windows:
          ...
          injective regions at each edge. The open-boundary obstruction for a
          left-boundary horizontal edge and a bottom-boundary vertical edge of
          the $7\times7$ lattice is now formalized, so the present margin
          package is explicitly a sufficient interior criterion rather than the
          final every-edge construction. Consequently, the nonexistence of a
          family of this margin-cover data over all edges of the current open
