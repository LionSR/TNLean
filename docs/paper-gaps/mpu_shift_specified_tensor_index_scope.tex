...
\begin{align}
    \label{eq:mpu_shift_specified_tensor_index_scope_blocked_ranks}
    (r,\ell)=(d^{k+2},d^{k}),
\end{align}
...
blocked over $k+1$ sites is also simple. The tensor product of two simple
tensors is simple (\leanid{MPOTensor.IsMPUSimple.tensorProduct}; this is the
step behind ``The case of tensoring is trivial'' in the proof of Theorem
IndexTh (ii), lines 835--836), so $U_2$, $U_3$ and all their blockings over
$k+1$ sites are simple as well
(\leanid{MPOTensor.blockTensor_shiftExampleU₂_isMPUSimple},
\leanid{MPOTensor.blockTensor_shiftExampleU₃_isMPUSimple}). All four values are
therefore values at simple blockings.
...
evolution~\cite{Cirac2021Matrix}, citing the source. Unequal
