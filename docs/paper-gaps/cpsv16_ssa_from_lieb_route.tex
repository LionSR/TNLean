...
from \doclink{TNLean/Entropy/SSAEqualityCharacterization} with its single re-export
...
theorem.\footnote{Lieb concavity is the theorem
\leanid{lieb_concavity_posDef} in \doclink{TNLean/Analysis/LiebConcavity}
(\path{TNLean/Analysis/LiebConcavity.lean}), consumed by
\leanid{lieb_concavity} in \doclink{TNLean/Channel/Schwarz/AndoLieb}.}
The only entropy-side dependency in the closure of
\leanid{strong_subadditivity_general}, formerly the axiom, is now the proved
theorem \leanid{lieb_concavity_posDef}.
...
\begin{enumerate}
  ...
  \item \emph{Klein's inequality.} Nonnegativity $D(\rho\,\|\,\sigma) \ge 0$,
    ...
    operator Jensen. The equality case is now formalized for full-rank $\sigma$:
    \leanid{quantumRelativeEntropy_eq_zero_iff} proves
    $D(\rho\,\|\,\sigma) = 0 \iff \rho = \sigma$ by sharpening the Gibbs
    reduction. When $D=0$ every term of the Gibbs slack is tight, so the strict
    tangent inequality $\log x < x-1$ for $x\neq1$ forces the eigenvalues to match
    on the overlap support; the matching makes the overlap matrix intertwine the
    eigenvalue diagonals, and conjugating $\sigma$ into $\rho$'s eigenbasis
    recovers $\rho$, giving $\rho=\sigma$. This argument too consumes neither Lieb
  ...
  \item \emph{Joint convexity.} Convexity of
    ...
    convexity of the relative entropy. The present formal Lieb theorem is stated
...
only entropy-side input is the Lieb-concavity theorem already present.
...
\begin{itemize}
  ...
  \item Layer (4) is the single consumer of \leanid{lieb_concavity_posDef}: joint
    ...
    consumer of \leanid{lieb_concavity_posDef}) and converges pointwise to $D$ as
  ...
  \item Layer (6) instantiates layer (5) at the tripartite splitting to
    ...
    singular reference. Its only entropy-side dependency, formerly the axiom, is
    now the proved theorem \leanid{lieb_concavity_posDef}, so this theorem
    discharges the content of the standalone strong subadditivity axiom on the
    full density-matrix domain.
\end{itemize}
...
dependency, formerly the axiom, is now the proved theorem
\leanid{lieb_concavity_posDef}. The marginal support lemma places the singular
...
entropy-side dependency on this route, formerly the axiom, is now the proved
theorem \leanid{lieb_concavity_posDef}; the standalone axiom
...
Lieb-concavity theorem (formerly asserted as an axiom, now proved as
\leanid{lieb_concavity_posDef}) available in the same repository; the former
standalone axiom has been removed. The elimination route is the classical relative-entropy
...
through the both-arguments regularization, so the Lieb-concavity theorem is now
...
full density-matrix domain whose only entropy-side dependency, formerly the
axiom, is now the proved theorem \leanid{lieb_concavity_posDef}, and it carries
exactly the hypotheses of the former standalone axiom, so
