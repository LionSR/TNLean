...
In the notation of the BNT comparison, this is the expansion
\[
  V_A^{(N)}
  =
  \sum_{j=1}^{g_A}
...
The former blocked-normal-form records that packaged this boundary have been
...
\[
  ...
  \tag{Cond}
\]
where the comparison package \(\mathcal C_{\mathrm{prod}}(A,B)\) contains
...
denote the weight families on the final produced sectors.  The missing
...
\[
  \tr\bigl(0_{D_0}^{\,N}\bigr)=0\quad(N>0),
  \qquad
  \tr\bigl(0_{D_0}^{\,0}\bigr)=D_0.
  \tag{Zero}
\]
