...
R_oneLabelBNTAlgebraTensorClause}.  There is one BNT label.  Its normalized
vertical component
...
\leanid{MPOTensor.HasBlockedAdjointFixedPointAlgebraTower} does not include these BNT-label
...
nonempty statement can be unpacked directly into a concrete witness carrying
the source predicates, the corresponding same-length and idempotent equations
written with $c^{(L)}_{\alpha,\beta,\gamma}$, and the same two equations with
coefficients written as $\chi$-traces; the blocked-basis $\chi$-family is also
identified as the pullback of the BNT-label $\chi$-family along the comparison
maps.
The existential
...
\leanid{MPOTensor.BNTLabelTheoremWitness.ofChi}.}  Such a witness
...
$F$-matrix data discussed above.  The one-site criterion for
\leanid{MPOTensor.TransferRetractData} also remains separate: it is equivalent to doubled-index transfer-map idempotence rather
than Definition~4.1.
