...
The explicit capstone
\leanid{MPOTensor.RescalingStableLengthDependentRFP.%
R_oneLabelBNTAlgebraTensorClause} constructs the tensor-attached clause.  The
previously established theorem
\leanid{MPOTensor.RescalingStableLengthDependentRFP.%
R_hasBNTAlgebraTensorClause} records the corresponding existential statement.
There is one BNT label.  Its normalized vertical component has $16$ physical letters,
bond dimension $4$, and
\[
    M_0^{(a,b)}=A^a\otimes\overline{A^b}.
\]
The
multiplicity matrix and the coefficient matrix are
\[
