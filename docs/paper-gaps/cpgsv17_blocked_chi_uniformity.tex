...
\section{Closure for the rescaling-stable example R}

Verdict: existential tensor attachment closed; explicit coefficient
identification remains open.

The declaration
\leanid{MPOTensor.RescalingStableLengthDependentRFP.%
R_hasBNTAlgebraTensorClause} applies the formalized implication
Theorem~4.14(i)$\Rightarrow$(ii) to the proved literal canonical form, MPDO
positivity, and renormalization fixed-point property of $R$.  It supplies a
vertical BNT component family, its same-length product law, its coefficient
family, positive diagonal trace-power data, and the length-one idempotent
identity.  Thus a genuine tensor-attached presentation exists for $R$.

Separately, the generic
\leanid{MPOTensor.PositiveBNTLabelChiTracePowerForm} apparatus is instantiated
for the hand-defined one-label family with
$\chi=\operatorname{diag}(1,7/25)$
(\leanid{MPOTensor.RescalingStableLengthDependentRFP.oneLabelChi}).  The
witness
\leanid{MPOTensor.RescalingStableLengthDependentRFP.oneLabelChiTracePowerForm}
gives $c^{(L)}_{0,0,0}=\tr(\chi^L)$ for
\leanid{MPOTensor.RescalingStableLengthDependentRFP.oneLabelCoeffs} at every
positive length.  The identities
\leanid{MPOTensor.RescalingStableLengthDependentRFP.%
wMat_pow_trace_eq_oneLabelChi_matrix_pow_trace} and
\leanid{MPOTensor.RescalingStableLengthDependentRFP.%
oneLabelCoeffs_coeff_eq_wMat_pow_trace} tie this scalar formula to the local
factor $W$ in the closed-operator factorization
\leanid{MPOTensor.RescalingStableLengthDependentRFP.%
mpo_R_eq_B_mul_wN_mul_transpose}.

These two valid results are not yet identified.  In particular, the
existential witness is not proved to have label type $\operatorname{Fin}1$,
its component operator is not identified with the original closed operator
$\rho^{(L)}(R)$, and its attached coefficients and diagonal matrices are not
proved equal to the separately defined one-label families.  The source
operator $O_L(M_\alpha)$ is the closed operator of a vertical BNT component,
not automatically the closed operator of the original tensor.

The tempting direct identification is false.  The ordinary matrix power
$W^L$ is distinct from the $L$-fold Kronecker power $W_L$ appearing inside
$\rho^{(L)}(R)$.  Squaring the factorization retains $W_L^2$, rather than
producing the scalar $\tr(W^L)$.  Already at $L=1$, the proposed law would
force
\[
    \frac{25}{32}W^2=\frac{32}{25}W,
\]
but the $(0,0)$ entries are $337/800$ and $512/625$.  Any explicit
identification must therefore construct the actual vertical BNT components
and retain the multiplicity relating their closed operators to the original
closed MPO.  This is the remaining scope of issues \#5635 and \#5590.
