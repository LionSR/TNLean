...
\leanid{MPOTensor.IsMPU.index_eq_of_mpo_eq}). Positive physical blocking
preserves this representative index for every positive-dimensional MPU, and
physical adjunction reverses its sign
(\leanid{MPOTensor.IsMPU.index_blockTensor},
\leanid{MPOTensor.IsMPU.index_physicalAdjointTensor}). This does not assert that
...
For two canonical-form-II representatives, tensor-product additivity follows without
separately supplied data at a common blocking length. Their independent
tensor product is again in canonical form II; all three tensors become simple
after a sufficiently long positive blocking, and both source-cut ranks
multiply. The logarithmic formula therefore gives
$\operatorname{ind}(\mathcal U\otimes\mathcal V)
    =\operatorname{ind}(\mathcal U)+\operatorname{ind}(\mathcal V)$.
In particular, tensoring with a positive-dimensional identity factor leaves
the index unchanged. Passing to reduced representatives and using the exact
periodic tensor-product identity gives the same additivity and identity-ancilla
invariance for all positive-dimensional MPUs
(\leanid{MPOTensor.IsMPU.index_tensorProduct},
\leanid{MPOTensor.IsMPU.index_tensorProduct_identityMPUTensor}). The remaining
composition hypotheses below are not removed by this tensor-product argument.
