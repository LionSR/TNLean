...
This does not identify indices across a change of ambient bond dimension.
Tensor-product additivity still requires one common blocking length at which
both factors and their tensor product are simple and have source data.
...
The remaining composition and comparison statements still require source
data for each transformed tensor to which the source theorem is applied.
...
composition retain distinct obligations under \ghissue{6139}.
...
The three candidate entries have not yet been consolidated with their
source-labelled counterparts under \ghissue{7660}. The former citation
obstruction is now discharged mathematically, though blueprint consolidation
remains. Eleven retained
...
eleven carry a supplied fixed pair. Redirecting their
dependencies onto the convention-hypothesis entries previously risked
strengthening these statements without saying so. The implication
\begin{align}
...
It concerns the original tensor, without introducing a parallel definition of
canonical form.

There are two independent mathematical stages. The first, concerning the four
entries whose restricted tensor is $\mathcal U$ itself, is proved below and
removes the citation obstruction; its use to merge the three candidates is
the remaining consolidation in \ghissue{7660}. The second concerns the twelve
entries whose restricted tensor is derived from $\mathcal U$. Positive
...
\begin{itemize}
    \item The proved converse of the stabilization theorem, namely
          ...
          candidates can be deleted upon consolidation and the second item is not
...
work it needs is larger than the two stages described above.

The first item is now proved, but the three candidate labels have not yet been
merged under \ghissue{7660}. After that consolidation, thirteen pointwise
entries remain: the generic supplied-pair step, which is kept deliberately,
and the twelve derived-tensor entries. Their positive-blocking,
...
conjugation and transposition comparisons, symmetry transforms, and
composition retain distinct preservation or reduction obligations.

\section{Continuous paths of canonical representatives}

Proposition~IV.5 of Ref.~\cite{Cirac2017MPU} begins with a continuous path
$x\mapsto\mathcal W(x)$ of arbitrary MPU tensors. It does not require the
path to remain in canonical form; indeed, the bond dimension of a reduced
canonical representative can vary along the path. Its conclusion concerns
the source ranks of those representatives.
% Source: paper_v2.tex 733--818.

For a path of fixed physical and bond dimensions whose members are already
in canonical form II, the common block of length $D^4$ is simple. The two
source cuts of this blocked path vary continuously, their ranks are lower
semicontinuous, and their product is the fixed positive number $d^{2D^4}$.
Thus both ranks and the index are constant on the interval. This proves a
special case, not Proposition~IV.5 with its stated hypotheses.

Pointwise existence of reduced representatives and invariance of the index
under a change of representative do not establish the unrestricted result.
The remaining step is to construct, at least locally along the path, a
continuous rank-preserving comparison tensor for the reduced representatives.
The source uses the weighted tensor $L(x)\mathcal W(x)R(x)$ and invertible
changes of auxiliary coordinates to identify its two cut ranks with those of
the reduced canonical tensor. The required continuity and rank comparison
cannot be inferred from an arbitrary pointwise choice of reductions.

This special case is a second scoped counterpart of
\texttt{prop:continuity-index}, beside
\texttt{prop:mpu\_admissible\_continuity\_index}. The two hypothesis sets are
not comparable. The canonical-form-II path requires each tensor of the path to
be in canonical form II and works at the fixed blocking length $D^4$, but asks for no
continuity of the fixed pair $\rho$, the stabilization exponent, or any other
source datum. The admissible proposition allows the path to leave canonical
form, but requires a common blocking length $m$ at which every blocked tensor
is simple and carries a reduced full-support source datum varying continuously
along the path. A canonical-form-II path is not known to carry such a
continuously varying datum, because the fixed point $\rho(x)$ is chosen
pointwise and its diagonality in ambient coordinates is not preserved. An
admissible path need not be in canonical form II. Neither theorem therefore
discharges the other, and \texttt{prop:mpu\_admissible\_continuity\_index}
keeps its label and its dependence on \ghissue{6140}.

The argument does show which part of the admissible datum the rank conclusion
uses. The rank lower semicontinuity and connectedness step needs only
continuity of the actual blocked tensors and a fixed positive rank product
$r\ell=d^{2m}$ at every point. For an admissible path this product follows
pointwise from simplicity and the source datum at that point, by the admissible
simple-tensor equivalence theorem. The continuity of the source data along the
path is not used. Once that equivalence theorem is formalized, the admissible
proposition should be proved by the same argument, and its path hypothesis may
be weakened to a pointwise one. The unrestricted proposition still needs the
comparison of reduced representatives described above.
...
continuous paths. For the continuity proposition alone, a path already in
canonical form II at every point has constant source ranks and constant index
without any continuity of source data; paths that leave canonical form remain
open, through the admissible and unrestricted entries.
...
proved and permits consolidation of the three entries that already carry no
formal restriction, together with the formerly obstructing citations.
Preservation along
