...
tensors, products, and compositions it is the content of \ghissue{6139}.
Physical adjunction already preserves the bundled presentation by
\leanid{MPOTensor.IsMPUCanonicalFormII.physicalAdjointTensor}; the twelve
downstream statements nevertheless retain their own supplied-source
hypotheses until the relevant derived-tensor arguments are consolidated.
...
restricted tensor is derived from $\mathcal U$ and remains open.
...
The second item is not uniformly distant. The bundled presentation is already
preserved under physical adjunction by the constructor cited above. For
blocking and independent tensor products the four clauses are separately
available, while their assembled statements are addressed in their respective
developments. Blocking preserves unitarity,
...
matrix stays fixed. Physical adjunction transports the canonical data together
with their full support and conjugates the transfer map entrywise. Independent
tensor products preserve unitarity, carry product
...
deliberately, and the twelve derived-tensor entries, each waiting on its own
operation in the second item. The twenty-three pathwise entries retain their
continuous-source-data hypotheses.
