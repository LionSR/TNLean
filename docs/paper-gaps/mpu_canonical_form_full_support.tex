...
derived-tensor entries has not yet been consolidated. For the comparison
tensors, products, and compositions, preservation remains the subject of
\ghissue{6139}; their restrictions remain necessary.

The census before the positive-blocking construction was the following.
...
representative, as the last part of this subsection shows. The new
positive-blocking construction supplies exactly the first of those transports;
it does not transfer the source cuts or factors back to $\mathcal U$.
...
\begin{itemize}
    ...
    \item Preservation of the bundled presentation along each operation the
          ...
          \leanid{MPOTensor.IsMPUCanonicalFormII.blockTensor}; the other
...
For physical adjunction and independent tensor products all four clauses are
separately available, but the assembled preservation statement remains
missing. Physical adjunction preserves unitarity, transports the
...
derived-tensor entries. Their positive-blocking case now has the required
preservation theorem, but its downstream entries have not yet been
consolidated; the other derived operations still require their respective
