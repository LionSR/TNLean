...
comparison, the relevant operation exchanges the source-cut ranks and negates
the index. Symmetry identifies the two generated MPUs, so the Admissible Index
Theorem gives
\begin{align}
  \operatorname{ind}(\mathcal U)
    &=\operatorname{ind}(\mathcal U_2)
     ...
     =-\operatorname{ind}(\mathcal U),
\end{align}
and hence $\operatorname{ind}(\mathcal U)=0$. Thus the obstruction is derived
from the supplied source and comparison data rather than imposed as an
external hypothesis. The source definitions
