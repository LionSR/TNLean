...
The census behind the derived-tensor obstruction is the following. The
original development had sixteen formal declarations mentioning the bundled
presentation: the structure itself, thirteen that consume it, and two that
construct one. The thirteen consumers conclude,
...
reduction. The converse proved below constructs a presentation on the same
tensor from its supplied fixed pair. It does not carry a presentation on $\mathcal U$ to a
...
and the twelve derived-tensor entries, each waiting on its own operation in
the second item.
