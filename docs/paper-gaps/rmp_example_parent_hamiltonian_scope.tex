...
for the two matrices displayed in the older review, the local and periodic
ground-space identities are formalized, while uniqueness remains open. The
...
formal content includes the single-window identity
$\mathcal G_3(A_{\mathrm{src}})=\{v : Kv=-v\}$ and, for $N\ge3$, the identity
$\mathcal G_{N,3}(A_{\mathrm{src}})=\bigcap_i\{\psi:K_i\psi=-\psi\}$.
The latter does not determine the dimension, and the review's own
...
\leanid{MPSTensor.clusterSourceTensor},
\leanid{MPSTensor.clusterSource_groundSpace_three_eq_eigenspace_neg_one}, and
...
\begin{enumerate}
    ...
    \item For the cluster state, first connect the existing normality proof to
          ...
          the three-site eigenspace identity and the periodic common-eigenspace
          identity are proved, with eigenvalue $-1$; transporting uniqueness
          still requires the on-site $\sigma^z$ equivalence.
...
older review the local and periodic eigenspace identities are proved. The GHZ Ising
operator identity, the on-site $\sigma^z$ equivalence that would carry the
periodic uniqueness statement to those two matrices, the AKLT polynomial and
