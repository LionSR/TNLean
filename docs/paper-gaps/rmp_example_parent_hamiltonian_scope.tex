...
The cluster and AKLT targets stated in the sources are now formalized. The
...
\begin{enumerate}
    ...
    \item For the cluster state, first connect the existing normality proof to
          ...
          index-shift equivalence. Both parts are done: the three-site parent
          space is the $ZXZ$ stabilizer eigenspace, and the general theorem
          gives the unique ground state.
...
statements with distinct source obligations. The first type and the cluster
stabilizer are formalized; the GHZ Ising operator identity, the AKLT
polynomial and spin-$2$ projector forms, and the open-boundary edge modes
remain open.
