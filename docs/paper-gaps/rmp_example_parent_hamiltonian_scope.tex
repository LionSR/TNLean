...
For the one-dimensional cluster state, write
$\ket{\pm}=(\ket{0}\pm\ket{1})/\sqrt2$. The review gives the MPS matrices
\begin{align}
    A^0
        &=
        \ketbra{0}{+},
...
\end{align}
and supplies the unnormalised matrices, indexed here by $0,1$,
\cite[local TeX lines~374--387]{PerezGarcia2007MPSReps}:
\begin{align}
    A_{\mathrm{src}}^0 &= \begin{pmatrix}0&0\\1&1\end{pmatrix},
    \label{eq:rmp_example_parent_hamiltonian_scope_source_zero}\\
    A_{\mathrm{src}}^1 &= \begin{pmatrix}1&-1\\0&0\end{pmatrix}.
    \label{eq:rmp_example_parent_hamiltonian_scope_source_one}
\end{align}
The formal target
...
The cluster module uses the reflected one-dimensional convention. Write
$A_{\mathrm{cl}}$ for this tensor:
\begin{align}
    A_{\mathrm{cl}}^0
        &=
        \ketbra{+}{0},
...
\path{TNLean/MPS/ParentHamiltonian/UniqueGroundState.lean}:666--709 and
\path{TNLean/MPS/ParentHamiltonian/UniqueGroundState.lean}:734--771.}
...
(\ref{eq:rmp_example_parent_hamiltonian_scope_aklt_intersection}) is proved by
the hand computation the review asks for, and the periodic two-site parent
Hamiltonian on $N\ge3$ sites has the AKLT state as its unique ground
state.\footnote{Formal declarations:
\leanid{MPSTensor.mem_aklt_groundSpace_two_iff},
\leanid{MPSTensor.aklt_groundSpace_intersection_two},
\leanid{MPSTensor.aklt_chainGroundSpace_two_eq_mpvSubmodule}, and
\leanid{MPSTensor.aklt_parentHamiltonian_two_unique_gs} in
\path{TNLean/MPS/Examples/AKLTParentHamiltonian.lean}.} The three-site
cluster parent space is the $+1$ eigenspace of $\sigma^z_1\sigma^x_2\sigma^z_3$
for the reflected convention
(\ref{eq:rmp_example_parent_hamiltonian_scope_formal_cluster_zero})--(\ref{eq:rmp_example_parent_hamiltonian_scope_formal_cluster_one})
as well, and the common $+1$ eigenspace of the translated stabilizers on
$N\ge3$ periodic sites is spanned by the cluster state. On the two matrices
displayed in the older review the stabilizer has eigenvalue $-1$ instead, so
the sign printed in
(\ref{eq:rmp_example_parent_hamiltonian_scope_cluster_three_body}) is correct
for those matrices. For a periodic chain of $N\ge3$ qubits, set
$K_i=\sigma_i^z\sigma_{i+1}^x\sigma_{i+2}^z$, with site indices modulo $N$.
The two states differ by $\sigma^z$ on every site, which conjugates each
stabilizer to its negative and exchanges $\sum_i K_i$ with $-\sum_i K_i$.
For a pair of square matrices $A=(A^0,A^1)$ of the same size and a configuration
$s\in\{0,1\}^N$, define its periodic matrix product vector by the
computational-basis coefficients
\begin{align}
    V^{(N)}(A)_s &= \tr\bigl(A^{s_0}\cdots A^{s_{N-1}}\bigr),
    \label{eq:rmp_example_parent_hamiltonian_scope_mpv_coefficients}
\end{align}
Thus
$\ket{V^{(N)}(A)}=\sum_{s\in\{0,1\}^N}V^{(N)}(A)_s\ket{s}$.
The source tensor
(\ref{eq:rmp_example_parent_hamiltonian_scope_source_zero})--(\ref{eq:rmp_example_parent_hamiltonian_scope_source_one})
and the reflected tensor
(\ref{eq:rmp_example_parent_hamiltonian_scope_formal_cluster_zero})--(\ref{eq:rmp_example_parent_hamiltonian_scope_formal_cluster_one})
satisfy
\begin{align}
    \ket{V^{(N)}(A_{\mathrm{src}})}
    &=(\sqrt2)^N(\sigma^z)^{\otimes N}\ket{V^{(N)}(A_{\mathrm{cl}})}.
    \label{eq:rmp_example_parent_hamiltonian_scope_source_vector}
\end{align}
Equation~(\ref{eq:rmp_example_parent_hamiltonian_scope_source_vector})
transports nonvanishing and the common eigenspace. For every vector $v$, a
sum-of-squares argument gives
$\operatorname{Re}\langle v,(\sum_i K_i)v\rangle\ge -N\|v\|^2$;
equality holds precisely on the source vector's span. Thus the ground energy
is $-N$ and its eigenspace is one-dimensional.\footnote{Formal declarations:
...
identification, for the reflected convention,
of the parent interaction with $\tfrac12(I-K)$, where
$K=\sigma^z\otimes\sigma^x\otimes\sigma^z$ and $I$ is the three-qubit identity,
as an operator identity is not
formalized; the formal statements are the eigenspace
identities.\footnote{Formal declarations:
\leanid{MPSTensor.cluster_groundSpace_three_eq_eigenspace},
\leanid{MPSTensor.cluster_iInf_eigenspace_eq_chainGroundSpace},
\leanid{MPSTensor.cluster_iInf_eigenspace_eq_mpvSubmodule}, and
...
\begin{enumerate}
    ...
    \item For AKLT, first use the two-site criterion
          ...
          the existing two-block injectivity proof; this is done. The
          polynomial Hamiltonian in
