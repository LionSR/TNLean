...
\leanid{MPSTensor.IsBNTCanonicalForm.combined_family_eventually_li}.  The
corrected proportional theorem applies coefficient comparison on the active BNT
families and does not use the retired per-block-projection layer.
...
The source coefficient-comparison mechanism instead uses the combined-family
\texttt{Lem1} statement~\eqref{eq:largeLem1} and keeps the raw power sums
$\sum_q\mu_{j,q}^N$ visible through \leanid{SectorDecomposition}.  The
corrected theorem uses this information together with
\leanid{MPSTensor.IsBNTCanonicalForm.coeff_not_eventually_zero}.  Collapsing a
sector to one strictly ordered weight destroys precisely the information needed
for that argument, so non-dominant per-block projection is a route that must not
be reintroduced.
