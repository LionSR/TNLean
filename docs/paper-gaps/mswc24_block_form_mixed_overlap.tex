...
eigenvalue other than one. In the source's definition
$\xi_{jj}=-1/\ln\lvert\lambda_2^{(j)}\rvert$ every bound $\lvert\lambda_2\rvert$ on
the subleading eigenvalues is admissible, so $\xi_{\mathrm{diag}}$ can be taken
arbitrarily small and $r=e^{-\gamma/\xi_{\mathrm{diag}}}$ can be any number in
$(0,1)$. Take $r<9/25$ and the sequence $q=M$, $N=q^2$, which satisfies $q=o(N)$.
...
hypothesis that the $q$-site states of distinct blocks are orthogonal, under which
(\ref{eq:mswc24_mixed_block_form}) holds with $\lvert\mu_j\rvert^q$ in place of
$\mu_j^q$ and the source's proof goes through. The second keeps the source's
hypotheses and adds to the rate a term controlled by the mixed transfer matrices,
of the form $(N/q)\,\tau^{2q}$ suggested by the counterexample; it needs a
perturbation bound for the square root of $B^\dagger B$ around its block-diagonal
part. Neither is formalized.
