...
The edge-blocking reduction remains a named tensor-network diagram.  The other
geometric steps are drawn directly where they enter the argument, while steps
that are intrinsically algebraic are stated by their component equations.
...
\item The comparison of a virtual insertion in the two closed three-site
  chains is drawn inline in the statement of \path{lem:inj_isomorph}.
...
\item The realization of a virtual insertion as a physical operation on either
  neighboring site is drawn as the corresponding three-panel equality.
...
% each panel has boundary (V,P)=(0,3).
\item The converse implication, in which two neighboring physical operations
...
\item After the edge gauges have been absorbed into the second tensor family,
  the five-site comparison with its distinguished $(2,5)$-edge insertion is
  drawn inline beside \path{eq:inj_equal_edge}.
...
\item The comparison of two injective tensors with a matrix inserted on a
  shared virtual bond is drawn inline in the three-shared-bond subcase, with
  the external virtual indices shown explicitly.
...
\item The four residual-operator forms involving $Z$, $U$, and $W$ are drawn
  inline in the proof of \path{lem:inj_equal_tensors_2}.
...
\item The inserted-edge coefficient is given by its defining contraction.
  Once the edge, physical indices, and inserted matrix are fixed, this
  coefficient is a scalar; a three-open-leg picture would describe a different
  object.
...
\item The orientation convention for an edge gauge is stated by the two
  endpoint actions, rather than by a separate contraction.
...
\item The action of all incident edge gauges on a vertex is stated by its
...
\item Cancellation of the two endpoint actions on an internal edge is stated
  as the corresponding matrix-inverse identity.
...
\item The local gauge obtained from the blocked-middle comparison is stated by
  its component equation at an arbitrary vertex.
...
\item Absorption of the incident edge gauges into $B_v$ is stated by the
  vertexwise equation defining $\widetilde B_v$.
...
\item Extraction of the local gauge is stated by the component equation that
  expresses $B_v$ as the action of the incident edge matrices on $A_v$.
...
\item Global consistency is stated by the single gauge family satisfying the
  local component equation at every vertex.
\end{itemize}

The inline comparison
$A_R A_v=A_S\propto\widetilde B_S=\widetilde B_R\widetilde B_v$
belongs to the normal-region argument of \cite[Section~4]{Molnar2018NormalPEPS},
where $R\subset S=R\cup\{v\}$; it is not part of the one-vertex-versus-complement
theorem in Section~3.

The retained and inline diagrams keep the local dot convention used in the
blueprint, but preserve the distinctions that matter in the paper: tensor
sites, blocked regions, virtual matrix insertions, and physical operations are
not drawn as the same object.  The component equations are used precisely when
a separate picture would obscure the boundary data or assert a contraction not
