...
\paragraph{Systematic positive-bond restriction.} Section~3 uses the standing
assumption that injective PEPS have nonzero-dimensional virtual bond spaces.
If this assumption is omitted, a zero-dimensional edge empties a virtual
configuration, makes an injectivity or equality hypothesis vacuous, and leaves
the conclusion as a genuine constraint that can fail. The affected statements
are the edge-blocked three-site injectivity (\#1366), the physical-to-virtual
recovery (\#1370), the edge-blocked insertion-algebra isomorphism (\#1367), the
global gauge consistency, the gauge uniqueness statement, and the headline
theorem \leanid{TNLean.PEPS.fundamentalTheorem_PEPS}. The restored hypothesis is
$\forall e,\ 0<\dim(e)$, for both tensors where both occur. The two-block
...
with the hypotheses, so the statement is vacuously true. The headline
counterexample is
...
edgeBlockedThreeSiteInjective_all_two_lt_card}
  ...
  lemmas above record the formal non-injectivity obstruction.
