...
edgeBlockedThreeSiteInjective_all_two_lt_card}
  ...
  A faithfulness defect was found and corrected here. The earlier Lean
  statement asserted middle-block injectivity from vertex injectivity alone,
  with no hypothesis on the bond dimensions. The formal obstruction is the
  following: if some complement bond makes
  ...
  have nonzero-dimensional virtual bond spaces, an implicit assumption the
  earlier Lean statement had dropped. The hypothesis
  $\forall f,\ 0<\dim(f)$ has been restored on
  ...
  restored positive-bond hypothesis its proof is now complete (the three facts
  below are proved).

  The per-vertex one-sided-inverse step of that fact is now formalized and
  proved:
  \leanid{TNLean.PEPS.IsVertexInjective.localCoeff_eq_zero_of_contract_zero}
  states that a coefficient family on the local virtual configurations at a
  vertex $v$ that contracts to the zero physical vector vanishes, which is the
  one-sided inverse at $v$. The middle tensor is a genuine per-vertex product,
