...
edgeBlockedThreeSiteInjective_all_two_lt_card}
  ...
  with no hypothesis on the bond dimensions. That statement is false: if some
  edge $f\ne e$ carries a zero-dimensional bond then
  \leanid{TNLean.PEPS.EdgeComplementConfig} is empty, the middle tensor family
  is identically zero
  (\leanid{TNLean.PEPS.edgeMiddleTensorFamily_eq_zero_of_isEmpty}), and the
  family is not linearly independent on a nonempty boundary-label index
  (\leanid{TNLean.PEPS.not_edgeMiddleTensorInjective_of_isEmpty}), whereas
  vertex injectivity survives vacuously because the local virtual configuration
  space at an endpoint of that bond is empty. A four-cycle with a
  zero-dimensional middle-to-middle edge is a concrete witness. The source
  avoids this because injective PEPS have nonzero-dimensional virtual bond
  spaces, an implicit assumption the Lean statement had dropped. The hypothesis
  $\forall f,\ 0<\dim(f)$ has been restored on
  \leanid{TNLean.PEPS.IsVertexInjective.edgeBlockedThreeSiteInjective}, with the
  two lemmas above kept as a checked counterexample.
  ...
  The per-vertex deletion engine of that one-step fact is now formalized and
  proved: \leanid{TNLean.PEPS.IsVertexInjective.localCoeff_eq_zero_of_contract_zero}
  ...
  single vertex $j$ peels off by $\prod_{v\in M_e}=A_j\cdot\prod_{v\in M_e
  \setminus\{j\}}$. Three pieces remain before the kernel descent can be
  instantiated from vertex injectivity:
  \begin{enumerate}
  \item a star-bond split equivalence
    $\textsf{edgeComplementConfigSplitAtStar}$ that decomposes the complement
  ...
  \item the terminal isolation $K_c(\varnothing)\Rightarrow c=0$ over the
    residual endpoint boundary labels.
  \end{enumerate}
  These assemble into
  \leanid{TNLean.PEPS.EdgeMiddleKernelDescentData} and then close the target by
  the existing reductions. The star-bond split equivalence is the load-bearing
  piece.
