...
of that block at length $S$. The simultaneous span also separates distinct
blocks: a scalar similarity between two blocks would impose a linear relation
between the corresponding entries of every word tuple, contradicting the
ability to choose these two matrix components independently. Independent
...
therefore gives $\ker K_{R,W}=G_W(B)$ for every $W\geq R$ as soon as $R\geq S+1$.
