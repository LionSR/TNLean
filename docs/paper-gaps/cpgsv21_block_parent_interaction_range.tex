...
For $W\in\mathbb N$, let $G_W(B)$ be the local MPS space of $B$ on $W$ sites, the
span of the vectors $\sigma\mapsto\operatorname{tr}(B^{\sigma}X)$ for
$\sigma\in\{0,\ldots,d-1\}^W$ and virtual matrices $X$. For $R\leq W$, let
$K_{R,W}$ be the open-chain parent Hamiltonian of range $R$ on $W$ sites, the
sum over the $W-R+1$ initial sites $i$ of the orthogonal projection onto
$G_R(B)^\perp$ acting on sites $i,\ldots,i+R-1$.
Fix $R>0$, and choose $W\geq 2R$ such that $K_{R,W}$ has kernel $G_W(B)$ and the periodic range-$W$ Hamiltonians
...
$\ker h=G_R(A)$. Let $h_R=I-P_{G_R(A)}$ be the orthogonal projection onto
$G_R(A)^\perp$. Finite dimensionality gives constants $\kappa,C>0$ such that
\begin{align}
    \kappa h_R\leq h\leq Ch_R.
    \label{eq:block_range_interaction_local_comparison}
\end{align}
If $h=0$, then $h_R=0$ and any positive constants suffice. Otherwise one may
...
over initial sites preserve (\ref{eq:block_range_interaction_local_comparison}).
Thus, with the same constants for every chain length $N\geq R$,
\begin{align}
    \kappa H_{R,N}(A)\leq H_N(h)\leq C H_{R,N}(A).
    \label{eq:block_range_interaction_periodic_comparison}
\end{align}
