...
The current formal reduction isolates this movement as a block-window equation
which implies the one-site commutation identity. Fix a complementary word
\(\mu\) and a boundary letter \(\eta\). After
...
The block-window equation gives the following commutation identity through the
block-injective commutation lemma: for every physical letter \(k\),
\[
  XA^k=A^kX.
\]
