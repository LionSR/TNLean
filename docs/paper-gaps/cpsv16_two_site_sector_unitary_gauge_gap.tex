...
realization.
% The conditional Gram and unitary results are issues 5282 and 5286.
...
\section{A bond-two singleton boundary}

% Verified against Lean head f0f8a921c3c25bdf3f0ac121d3d0dcb75bfe368d.
A second bond-two model satisfies the terminal condition but fails at two
sites.  Let $A$ be the normal tensor whose four diagonal doubled-physical
...
$P=\ket{\Phi}\!\bra{\Phi}$.  The corresponding two-site deformation
\begin{equation}
  P_X=(X\otimes X)P(X^{-1}\otimes X^{-1})
  \label{eq:singleton-two-site-deformation}
\end{equation}
is not Hermitian.  Hence it cannot be a positive two-site operator.  This is
a sharper necessary-condition boundary than terminal positivity: the explicit
nonunitary deformation survives the one-site test but fails the two-site test.

The converse is also formalized for this model.  For any
...
\end{equation}
Indeed, one-site Hermiticity gives $GJ=JG$, while two-site Hermiticity gives
...
one-site/two-site test, but they do not state the model-specific theorem
\eqref{eq:singleton-scalar-gram}.  The result is a necessary condition and a
no-go theorem for the explicit deformation, not an all-length construction.
In particular, this result does not supply an all-length MPDO
predicate or a tensor-attached BNT algebra clause for $A$ or $B$.
The separate all-length tensor-attached/base-model construction is recorded in
...
remain unchanged.
% Terminal positivity is issue 5288; the conditional Gram and unitary results
% are issues 5282 and 5286.  Issue 5290 is separate from issue 5289.
