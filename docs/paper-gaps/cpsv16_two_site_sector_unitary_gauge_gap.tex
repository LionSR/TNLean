...
\section{A bond-two singleton terminal boundary and Bell comparator}

A bond-two singleton tensor satisfies the terminal condition.  Let $A$ be the
normal tensor whose four diagonal doubled-physical
letters are the matrix units $E_{a,b}$ of $\mathbb C^{2\times2}$.  Its
terminal matrix is
\begin{align}
...
\end{align}
The matrix $X$ is invertible and nonunitary.  The tensors $A$ and $B$ have the
same positive-length closed chains, and their terminal matrices both equal
$J$.  Thus the deformation preserves the positive one-site boundary.

Separately, let $\ket{\Phi}=\ket{0,0}+\ket{1,1}$ and
$P=\ket{\Phi}\!\bra{\Phi}$.  The companion matrix similarity
\begin{align}
  P_X&=(X\otimes X)P(X^{-1}\otimes X^{-1})
  \label{eq:companion-bell-similarity}
\end{align}
is not Hermitian.  Hence this chosen Bell-projector similarity is not positive.
The matrix $P$ is not the retained closed operator $O_2(A)$, and no result
identifies $P_X$ with $O_2(B)$ or with the two-site operator of a deformed
MPDO tensor.  As shown below, $P$ is instead the reindexed two-site operator
of a separate ambient MPO whose vertical tensor is $A$.  Thus the calculation
does not show that $B$ fails an MPDO two-site test.

A scalar-Gram criterion is also formalized for the chosen pair of matrices.
For any
$Y\in\mathrm{GL}(2,\mathbb C)$, put $G=Y^\dagger Y$.  If both
$YJY^{-1}$ and
$(Y\otimes Y)P(Y^{-1}\otimes Y^{-1})$ are Hermitian, then there is a real
number $\omega>0$ such that
\begin{align}
  G&=\omega\mathbf 1.
  \label{eq:singleton-scalar-gram}
\end{align}
Indeed, Hermiticity of the terminal-matrix similarity gives $GJ=JG$, while
Hermiticity of the companion Bell-projector similarity gives
$G_{00}G_{01}+G_{10}G_{11}=0$.  The first identity makes the diagonal entries
equal and the off-diagonal entries equal; positive definiteness and the second
identity then force the off-diagonal entries to vanish.  Thus
$\omega^{-1/2}Y$ is unitary.

Appendix~C.4, lines~2048--2057, and Proposition~4.13 motivate this
finite-dimensional terminal/Bell comparison, but they do not state the
criterion \eqref{eq:singleton-scalar-gram}.  The criterion applies to the
chosen $J$ and companion $P$; it is not a theorem about the retained closed
operators $O_2(A)$ or $O_2(B)$.

The singleton does admit a separate all-length positive ambient realization.
Use pairs in $I\times I$ as a four-dimensional bond index and define
\begin{align}
  M^{i,j}_{(r,s),(r',s')}
...
\end{align}
In particular, after the canonical identification of two-site words with
$I\times I$, $M_2=P$.

For every $N>0$, the raw retained closed operators are diagonal indicators of
cyclically composable directed-edge words.  They are therefore orthogonal
projections and obey the coefficient-one product law.  The normalized
representative $\widehat A=2^{-1/2}A$, together with the unique multiplicity
weight $\sqrt2$, has coefficient $2^{-L/2}$ at length $L$ and reconstructs
$A$.  The canonical
identification of its retained coordinates with the original bond coordinates
is a coisometry.  These data give a genuine one-label tensor-attached BNT
algebra clause for the model $M$, whose closed operators are positive at every
positive length.

This construction completes the attachment tracked in
\href{https://github.com/LionSR/TNLean/issues/5290}{issue 5290}.  The proved
reindexing identity also attaches the nonunitary Bell similarity from
\href{https://github.com/LionSR/TNLean/issues/5289}{issue 5289} to the matrix of
the base operator: $P_X$ is the stated similarity of the reindexed matrix of
$M_2$.  No deformed tensor-attached BNT algebra clause or deformed MPDO tensor
with two-site operator $P_X$ has been constructed.  Hence this is not a
nonunitary full-contract counterexample and does not supply the
identity-dressed marked realization.

Consequently Theorem~4.14 remains partial.  The identity-dressed marked
comparison remains open in
\href{https://github.com/LionSR/TNLean/issues/5284}{issue 5284}, under
\href{https://github.com/LionSR/TNLean/issues/4648}{issue 4648}, and
\href{https://github.com/LionSR/TNLean/issues/4645}{issue 4645} remains open.
The terminal-positivity theorem and the conditional Gram and unitary theorems
remain unchanged.
% Terminal positivity is pull request 5288; the conditional Gram and unitary
% results are pull requests 5282 and 5286.  Issue 5290 is separate from issue 5289.
...
common reflected target.  Its unitary-normalization corollary then constructs
\[
...
target identities used by the conditional Gram theorem directly from the tensor-attached
