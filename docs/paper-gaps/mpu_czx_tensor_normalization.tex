...
second reduction step with the projector $Q$ (read as below) lead to the
...
Two smaller discrepancies of the same passage are recorded here.
...
\emph{The second projector.} Section IV of the review fixes the convention
for the canonical-form construction: an invariant subspace
$\mathcal S_1$ satisfies $B^i\mathcal S_1\subset\mathcal S_1$, $P_1$ is the
orthogonal projector onto $\mathcal S_1$ and $Q_1=\mathbb 1-P_1$, so that
$B^iP_1=P_1B^iP_1$ and $Q_1B^iQ_1=Q_1B^i$. The first projector
$P=\operatorname{diag}(0,1,1,0)$ of the appendix follows this convention.
The second projector $Q=\tfrac12\begin{pmatrix}1&-1\\-1&1\end{pmatrix}$ is
said to give an invariant subspace of the block
$B^{00}=\begin{pmatrix}-1&1\\0&0\end{pmatrix}$,
$B^{11}=\begin{pmatrix}0&0\\1&-1\end{pmatrix}$, but its range is not
invariant: $B^{00}(1,-1)^{T}=(-2,0)^{T}$. The range of $\mathbb 1-Q$, spanned
by $(1,1)^{T}$, is invariant, since both letters annihilate it. Thus the
printed $Q$ plays the role of $Q_1$, with $P_1=\mathbb 1-Q$. The block on
the range of $P_1$ vanishes and the block on the range of $Q$ is
$QB^{ij}Q=(-1)\delta_{ij}Q$, which is the canonical form the review states.
The formalization reads $Q$ as $Q_1$.\footnote{Source traceability:
    \path{Papers/2011.12127/TN-Review-main.tex}, lines 1784--1797 and
    2598--2601. Results: \leanid{CZXCompression.reviewQ_mul_reviewBlock},
    \leanid{CZXCompression.reviewBlock_mul_reviewQ} and
