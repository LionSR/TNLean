...
\paragraph{Classification: resolved definition drift.} The older formal
predicate records doubled-index CP-transfer idempotence, while the source
definition uses the physical-trace transfer. The source definition is now
formalized by \texttt{MPOTensor.IsPhysicalTraceIdempotent}, whose equation
characterization is $\mathcal T_M^2=\mathcal T_M$. The predicate
\texttt{MPOTensor.IsSourceZCL} remains the separate scale-invariant
up-to-scalar relation; it is distinct from the paper's literal diagram and from
...
(\path{Papers/1606.00608/MPDO-22-12-17-2.tex:777}) cannot be represented by
\texttt{IsZCL}: the source implication controls $\mathcal T_M$, whereas that
predicate controls $\mathcal E_M$. After repairing this definition drift, the
printed implication to literal physical-trace idempotence is itself refuted by
\texttt{MPOTensor.exists\_isPRFP\_isMPDO\_physTraceTransfer\_ne\_zero\_not\_isPhysicalTraceIdempotent}.
Its witness satisfies global PRFP and MPDO positivity with $\mathcal T_M\ne0$,
but $\mathcal T_M^2\ne\mathcal T_M$. The stronger failure of the scale-invariant
relation remains a separate project result. Two normalization conventions remain
useful for corrected restricted theorems:
...
The source definition and the scale-invariant relation are now both in place.
\texttt{IsPhysicalTraceIdempotent} records the literal fixed-tensor diagram,
\texttt{IsSourceZCL} records the nonzero up-to-scalar relation, and
\texttt{IsZCL} remains the doubled-index CP-transfer condition. These three
predicates must remain distinct. For the renormalization condition
...
\texttt{MPOTensor.IsPhysicalTraceIdempotent} for Definition~4.2 and
\texttt{MPOTensor.isPhysicalTraceIdempotent\_of\_isRFPViaTS} for its RFP
...
printed global PRFP equivalence is refuted; the local-purification equivalences remain valid only under their stated tensor-level restriction. In
Theorem~4.9, the literal implication
(iv)$\Rightarrow$(v) is now refuted by the unequal repeated-copy example
described below, while the source (ii)$\Rightarrow$(v) channel route remains a
missing statement after the ZCL-derived common-weight reduction.
...
condition. Neither of these four-sector representatives settles the exact
(ii)$\Rightarrow$(v) route. A separate scalar BNT presentation with raw copy
weights $1$ and $1/2$ does satisfy the global normalization and every clause of
condition~(iv), while its two-site blocking fails Definition~4.1. This
kernel-checked counterexample is
\texttt{MPOTensor.CaseIIAbsorptionCounterexample.%
printed\_theorem49\_iv\_to\_v\_is\_false} and refutes the literal
(iv)$\Rightarrow$(v).
...
The theorem
\texttt{MPOTensor.isPhysicalTraceIdempotent\_of\_isRFPViaTS}, using the
physical-trace-square calculation, formalizes the zero-correlation-length part
of Appendix~C, Proposition \texttt{propsimple}, source lines 1333--1340. This proposition proves the first
...
The remaining obligations belong to the stronger route from condition~(ii).
First, the Case-II argument still needs a replacement for the false claim that
coefficient absorption preserves normality; this is the \emph{missing
statement} in the (ii)$\Rightarrow$(iv) node and is analyzed in
\path{docs/paper-gaps/cpgsv17_pf_rank_one.tex}. Second, the earlier Case-II
argument at lines~1646--1665 uses literal ZCL to make repeated-copy weights
independent of the copy label and absorb their common value. Lemma~C.10 is now
linked exactly by
...
assembly problem: it is a refuted \emph{unfaithful statement}, and treating the
one-factorization result as though outer-sector assembly would prove it is
\emph{proof-path drift}. The hidden use of ZCL in the proof of
\texttt{prop3to4}, through \texttt{lemmus}, is a separate instance of the same
proof-path drift. The former doubled-index theorem has been removed rather
than renamed, and none of these surviving gaps is a doubled-index bridge or a
