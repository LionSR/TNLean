...
exists_commonWeightAbsorbedBasisMPOTensor_isSourceZCL} chooses the canonical
BNT data, proves that the weights over each normal representative are
independent of the copy index, and proves source ZCL for every absorbed
representative.  Its final step uses the absorbed-transfer identity
...
accepts these sectorwise source-ZCL conclusions together with sector MPDO
positivity, one-site injectivity, normality, and positive proportional sector
data.  The standing biCF hypothesis at source line~1628 is the simultaneous
one-letter span of the BNT representatives.  The formal implication
\leanid{MPSTensor.WordTupleSpanTop.isInjective_one} therefore supplies
one-site injectivity for every absorbed representative; no passage through
blocked SAL is needed.  Normality comes from the chosen normal BNT
representatives.  Sector MPDO positivity remains an explicit input to the
formal theorem.  It must be supplied by the surrounding positive sector data,
not inferred from SAL of the ambient tensor, which is the conclusion being
proved.  The exact outer equality for the canonical BNT family is provided by
\leanid{MPOTensor.mpo_eq_sum_copies_smul_commonWeightAbsorbedBasisMPOTensor},
and the natural multiplicities are positive.

\paragraph{Blocking alone would give only block-aligned SAL.}
Suppose instead that one began with a merely normal sector tensor and first
chose a positive injective blocking length \(L_0\).  SAL of the blocked tensor
would control only original chain lengths and cuts divisible by \(L_0\):
\[
  I_{rL_0}^{(qL_0)}=I_{(r+1)L_0}^{(qL_0)}.
\]
It would not imply the source equalities
\[
  I_{\ell}^{(N)}=I_{\ell+1}^{(N)}
\]
for every admissible original chain length \(N\) and consecutive cut
\(\ell\).  This explains why ordinary physical blocking is not a substitute
for one-site injectivity in the single-sector theorem.

The source's Case~II application does not make that substitution.  It fixes
$K$ in biCF at line~1628, and biCF supplies the simultaneous one-letter span
before Proposition~\emph{prop4to2} is invoked.  Projecting this span to one
representative gives its one-site injectivity.  Thus there is no unresolved
injectivity gap in the Case~II application.  The all-cut Hayashi decomposition
applies directly to a canonical absorbed sector once the theorem's remaining
explicit hypotheses---sector MPDO positivity and normality---are supplied.
The canonical BNT representative supplies normality; positivity must come
from the positive sector data and not from an assumption of ambient SAL.
...
scalar.  When the explicit sector MPDO and normality hypotheses hold, applying
...
gives SAL for the full tensor.  This is the corrected ambient-ZCL-conditioned
statement; it does not derive sector positivity from ambient SAL.  Neither
global nor sectorwise positive proportionality remains a restriction.  The
exact theorem
\leanid{MPOTensor.%
...
therefore no longer restrictions.  Under ambient source ZCL, copy independence
and the blockwise transfer equation give source ZCL for every canonical
absorbed simple-BNT sector.  The standing biCF one-letter span gives one-site
injectivity of those sectors, while the chosen BNT representatives are normal.
The single-sector theorem therefore retains every original cut when sector
MPDO positivity is supplied independently by the positive sector data.

The literal proposition \emph{prop4to2} is still not proved as printed.  Its
statement assumes only GSNNCH, while Proposition~\emph{4to2}, invoked in its
proof, also assumes ZCL.  The surrounding main theorem supplies ambient ZCL in
condition~(iii), and the corrected ZCL-conditioned canonical-BNT application
is covered by the preceding argument.  An arbitrary GSNNCH presentation does
not inherit tensor-level source ZCL, as the nilpotent-summand example above
shows.  Thus ambient source ZCL remains a separate scope restriction on the
