...
The supplied orthogonal commuting sector family has pairwise orthogonal
one-site projections and a positive commuting bond whose periodic product is
exactly the sector operator, with scalar one, at every length at least two.
This exact identity is stronger than the proportional \emph{ApprhoNComm}
sector form at source lines~1641--1665.  It yields proportional fixed-bond data
with constant one inside the proof.  The theorem also assumes at every
positive length that the outer tensor is the positive-natural-multiplicity sum
of the sectors.  The proportional local orthogonal-sum theorem shows that a
further positive length-dependent scalar would not change the conclusion.
Applying
...
theorem gives SAL for the full tensor.  This is a scope-restricted
specialization rather than a formalization of the printed GSNNCH-only
proposition.
...
together with an exact scalar-one orthogonal realization.  It remains
restricted relative to the printed assertion: sectorwise source ZCL is omitted
in print, one-site injectivity requires an unaddressed blocking argument, and
the exact sector realization is stronger than the proportional source data.
