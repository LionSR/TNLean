...
The full finite-chain equation \emph{sigmaNK2} is now formalized.  The theorem
\leanid{MPOTensor.EtaLocalStructureData.%
exists_finite_chain_eta_decomposition}
combines the positive realization scalar with tensor-power conjugation by the
same one-site unitary.  Thus one unitary, one sector decomposition, and one
positive neighboring family work for every $N\geq2$; only the scalar $c_N>0$
may depend on $N$.  If $E_N$ sends a site configuration to its sector-labelled
cyclic edge configuration, and
$\mathcal R_{E_N}(A)_{s,t}=A_{E_N^{-1}(s),E_N^{-1}(t)}$, then the conclusion is
\[
  \mathcal R_{E_N}\!\left(
    U^{*\otimes N}\sigma^{(N)}(\mathcal K)U^{\otimes N}\right)
  =c_N\bigoplus_k\bigotimes_n\eta_{k_n,k_{n+1}}.
\]
This conclusion uses neither ZCL nor a rank-one trace factorization.
