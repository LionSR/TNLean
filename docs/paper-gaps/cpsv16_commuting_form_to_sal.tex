...
positive length that the outer tensor is exactly the
positive-natural-multiplicity sum of the sectors.  Applying
...
together with an exact scalar-one orthogonal realization and an exact
positive-multiplicity outer sum.  The latter theorem derives sectorwise SAL
rather than assuming it.  It is a restricted theorem rather than a
formalization of the printed assertion: sectorwise source ZCL is omitted in
print, one-site injectivity requires an unaddressed blocking argument, and the
exact realization is stronger than the proportional source data.
