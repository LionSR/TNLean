...
In the BNT specialization the exact outer equality is already provided by
...
decomposition.  Consequently the corollary
\leanid{MPOTensor.isSAL_of_orthogonalCommutingSectorFamily} assumes the exact
outer sum and sectorwise SAL, although exactness of the global outer sum is no
longer required by the entropy argument.
The restricted specialization
\leanid{MPOTensor.%
isSAL_of_proportionalOrthogonalCommutingSectorFamily_of_sectorwise_isSourceZCL}
assumes that every sector generates MPDOs, is one-site injective and normal,
and has source ZCL.  Source ZCL is omitted by the printed proposition.
Normality is inherited in the BNT setting, but one-site injectivity is not a
consequence of BNT normality alone: a normal tensor becomes injective only
after blocking.  The source application therefore leaves an
injectivity-versus-blocking step unaddressed.
...
sectorwise SAL.  The global proportionality scalar is therefore no longer a
restriction.  A further theorem derives sectorwise SAL from explicit
sectorwise MPDO, one-site injectivity, normality, and source-ZCL hypotheses,
together with positive proportional orthogonal sector realizations.  The
sector-product proportionality is therefore no longer a restriction.  The
printed proposition still omits sectorwise source ZCL, and its application to
normal BNT sectors still requires an unaddressed argument passing from
eventual block injectivity to the all-original-cut visibility statements used
by the present single-sector proof.  Under the available blocking and SAL
interfaces, SAL of a blocked sector supplies only block-aligned chains and cuts
and does not by itself discharge this obligation.
