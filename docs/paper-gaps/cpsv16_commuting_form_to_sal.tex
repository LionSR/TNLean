...
\paragraph{Scope restriction (sectorwise source ZCL).}
...
therefore assumes SAL for the summands, a positive proportional
local orthogonal-sum identity at every positive chain length, supplied one-site
projections supporting every nonempty sector marginal, and positive natural
multiplicities.  It must not be presented as a proof that GSNNCH alone implies
SAL.  The structure
\leanid{MPOTensor.ProportionalOrthogonalCommutingSectorFamily} records fixed
one-site projections and commuting sector products whose periodic products
realize the sector operators up to positive scalars.  Projection idempotence,
bond support, and positive proportional realization imply that the normalized
sector chain and every nonempty sector marginal on a chain of length at least
two are supported on the matching projection at the first site; these are
\leanid{MPOTensor.ProportionalOrthogonalCommutingSectorFamily.%
firstSiteMatrix_mul_normalizedMPO} and
\leanid{MPOTensor.ProportionalOrthogonalCommutingSectorFamily.%
firstSiteMatrix_mul_reducedBlockState}.  The scalar factors through the
first-site action and then cancels under trace normalization.  The earlier
...
multiplicities, and therefore cannot by itself supply the proportional outer
sum at every positive length.  The theorem
\leanid{MPOTensor.isSAL_of_proportionalLocalOrthogonalSum} assumes numbers
$c_N>0$ such that
\[
  \sigma^{(N)}(M)
...
sectorwise SAL and proves the non-circular final local direct-sum implication;
it neither derives the sectorwise SAL premise nor uses ZCL.  The earlier exact
theorem \leanid{MPOTensor.isSAL_of_localOrthogonalSum} is its specialization
with $c_N=1$.

In the canonical BNT specialization, ambient source ZCL does supply the
missing sectorwise source-ZCL premise.  For a simple tensor already in the
chosen canonical form,
\leanid{MPOTensor.IsSimpleCanonicalForm.%
exists_weight_copy_independent_of_isSourceZCL} combines the canonical witness
with
\leanid{MPOTensor.weight_copy_independent_of_isSourceZCL} to show that the
weights over each normal representative are independent of the copy index.
After absorbing this common weight, the block equation restricts the ambient
source-ZCL identity to every representative; this is
\leanid{MPOTensor.commonWeightAbsorbedBasisMPOTensor_isSourceZCL}.  Hence every
canonical absorbed simple-BNT sector has source ZCL under ambient source ZCL.
This is precisely the sectorwise hypothesis used when Proposition
\emph{4to2}, at source lines~1597--1619, is applied sector by sector in
Proposition \emph{prop4to2}, at source lines~1801--1808.

This conclusion belongs to the canonical absorbed presentation, not to an
...
one the one-dimensional tensor $(1)$ and the virtual direct sum
\[
  (1)\oplus
  \begin{pmatrix}0&1\\0&0\end{pmatrix}
\]
generate the same scalar operator at every positive length.  The first tensor
has source ZCL, while the second transfer has a nonzero nilpotent summand and
satisfies no equation $E^2=\lambda E$ with $\lambda>0$.  Thus representation-
level source ZCL cannot be inferred from GSNNCH alone without selecting the
canonical absorbed sectors.

The downstream theorem
\leanid{MPOTensor.%
isSAL_of_proportionalOrthogonalCommutingSectorFamily_of_sectorwise_isSourceZCL}
accepts these sectorwise source-ZCL conclusions together with the sector MPDO,
one-site injectivity, normality, and positive proportional sector data.  The
exact outer equality for the canonical BNT family is provided by
\leanid{MPOTensor.mpo_eq_sum_copies_smul_commonWeightAbsorbedBasisMPOTensor},
and the natural multiplicities are positive.  The remaining source application
is therefore not a source-ZCL problem.  It is the separate all-original-cut
problem: a normal BNT sector is only eventually injective, while the present
single-sector proof uses one-site injectivity.  Replacing those uses by
word-length visibility while retaining every original consecutive cut is the
unresolved obligation recorded in issue~\#5208.

\paragraph{The present blocking interface gives only block-aligned SAL.}
Suppose that normality supplies a positive blocking length \(L_0\) for a
sector tensor \(\mathcal K\).  The closed operator of the blocked tensor on
\(q\) blocked sites is the closed operator of \(\mathcal K\) on \(qL_0\)
original sites, up to the canonical identification of physical indices.
Likewise, a prefix of \(r\) blocked sites is a prefix of \(rL_0\) original
sites.  Therefore SAL of the blocked tensor supplies only
\[
  I_{rL_0}^{(qL_0)}=I_{(r+1)L_0}^{(qL_0)}.
\]
The source definition of SAL for the original tensor instead requires
\[
  I_{\ell}^{(N)}=I_{\ell+1}^{(N)}
\]
for every admissible original chain length \(N\) and every consecutive cut
\(\ell\).  Shifting the origin of the blocking does not change the retained
length, and replacing \(L_0\) by a larger injective blocking length still
cannot align both \(\ell\) and \(\ell+1\) unless the blocking length is one.
Consequently, under the present physical-blocking identification and SAL
interface, preservation of MPDO positivity, source ZCL, and the commuting bond
presentation establishes only a block-aligned statement.  Blocked SAL does not
by itself establish SAL for the original tensor.

Any completion that follows the present all-cut Hayashi-decomposition route
must retain the original one-site middle region.  The present proof uses
one-site injectivity to show that the
cyclic-active original one-site matrix family spans the virtual matrix
algebra, to prove that the visible recurrent sectors form one component, and
to prove that the cyclic-active sector set is nonempty.  Along this route,
removing one-site injectivity requires word-length replacements for these
three steps, derived from a positive block-injectivity witness, while the
resulting Hayashi decomposition remains at every one-site cut of the unblocked
chain.  No such word-length visibility theorem is presently available.
Ordinary physical blocking does not by itself provide these replacements.

The proportional orthogonal commuting sector family has pairwise orthogonal
one-site projections and a positive commuting bond whose periodic product is
proportional to the sector operator by a positive scalar at every length at
least two.  This matches the proportional \emph{ApprhoNComm} sector form at
source lines~1641--1665.  The full tensor may also differ from the
positive-natural-multiplicity sector sum by a positive length-dependent
scalar.  Applying
\leanid{MPOTensor.EtaLocalStructureData.isSAL_of_isSourceZCL} to each sector
supplies the sectorwise SAL premise, after which the preceding direct-sum
theorem
\leanid{MPOTensor.isSAL_of_proportionalOrthogonalCommutingSectorFamily}
gives SAL for the full tensor.  Thus neither global nor sectorwise positive
proportionality remains a restriction.  The exact theorem
\leanid{MPOTensor.%
isSAL_of_orthogonalCommutingSectorFamily_of_sectorwise_isSourceZCL}
is the specialization in which all these scalars equal one.
...
formalized under a positive proportional outer sum, conditionally on
sectorwise SAL.  The global and sector-product proportionality scalars are
therefore no longer restrictions.  Ambient source ZCL now descends to every
canonical absorbed simple-BNT sector through copy independence and the
blockwise transfer equation.  This completes the missing sectorwise source-ZCL
step used between source lines~1597--1619 and~1801--1808.  It does not make
source ZCL a property of an arbitrary GSNNCH presentation.

The literal proposition \emph{prop4to2} is not yet complete.  Its canonical
BNT sectors are normal and hence eventually injective, but the available
single-sector theorem assumes one-site injectivity to obtain visibility at
every original cut.  Under the present blocking and SAL interfaces, SAL of a
blocked sector supplies only block-aligned chains and cuts.  The separate
all-original-cut and eventual-injectivity problem in issue~\#5208 remains.
