...
The corrected specialization
\leanid{MPOTensor.%
isSAL_of_orthogonalCommutingSectorFamily_of_sectorwise_isSourceZCL}
assumes that every sector generates MPDOs, is injective and normal, and has
source ZCL.  The fixed bond and its exact periodic-product identity come from
the same orthogonal commuting sector family used in the outer sum.  Applying
...
theorem gives SAL for the full tensor.  Since sectorwise source ZCL is explicit,
this is a scope-restricted specialization rather than a formalization of the
printed GSNNCH-only proposition.
...
The later orthogonal direct-sum implication is formalized both as a general
scope-restricted theorem assuming sectorwise SAL and as a specialization
deriving that premise from sectorwise MPDO positivity, injectivity, normality,
a fixed commuting bond, and source ZCL.  Neither theorem proves the printed
assertion that the GSNNCH form alone implies SAL.
