...
virtual gauge for the selected fixed tensor.

The remaining step is the coefficient comparison at source lines~1606--1617.
One must expand the normalized reduced state after tracing one suffix site and
after tracing two suffix sites.  The two surviving boundary contractions have
coefficients $\widehat T^2$ and $\widehat T^3$, respectively.  Recurrence
supplies a nonzero retained path between every pair of cyclic-active sectors,
so equality of the two reduced states permits entrywise cancellation and
yields
\[
  \widehat T^2=\widehat T^3.
\]
This relation must then be combined with the recurrent support to obtain the
strictly positive rank-one two-step coefficient used by the already
formalized Markov decomposition.  Until this coefficient extraction and its
application to the original source marginal identity are proved, the
commuting-form-to-SAL implication remains open.  The explicit factorization
and positivity assumptions of the proved SAL corollary remain scope
restrictions, not additional hypotheses on Proposition~\emph{4to2}.
