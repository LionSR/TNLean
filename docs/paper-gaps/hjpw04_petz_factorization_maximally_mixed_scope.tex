...
The product-reference recovery identity, the recovered conditional family, the
...
\end{align}
The supported matrices $\widehat\rho_j$ are the trace-one recovered physical
...
supported right factor remains its recovered state. Total positive-semidefinite
...
\leanid{Matrix.exists_recoveredDilationBlockUnitary} proves equation~(15)
...
right factor remains its recovered state, even when its weight is zero. A
