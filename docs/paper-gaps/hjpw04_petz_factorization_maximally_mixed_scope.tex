...
The joint-support block theorem is now applied to this family, and its
coordinates extend to one ambient direct sum
\begin{align}
  H_B\cong\bigoplus_j H_{B_j^L}\otimes H_{B_j^R}
\end{align}
with one-dimensional zero-weight complement sectors. One chosen
...
marginals. Equations~(11), (14), and~(15) now imply
\begin{align}
  W^\dagger\rho_{ABC}W
  =0_\perp\oplus\bigoplus_j\omega_j\otimes\widehat\rho_j,
\end{align}
where $W=\mathbf 1_A\otimes(U_B\otimes\mathbf 1_C)$. The supported blocks are
written in HJPW order $(A B_j^L)\otimes(B_j^R C)$, and every complementary
block is zero. This identity is formalized by
\leanid{Matrix.RecoveredConditionalDilationBlockForm.ambient_tripartite_block_form}.
The singular-support equality-to-raw-recovery implication is now formalized by
the projected finite-Weyl argument on arbitrary finite product coordinates.
...
Probability normalization, zero-weight sector choices, the final quantum-Markov
decomposition, and removal of the forward Hayashi assumption remain open.
...
\begin{align}
  ...
  =0_\perp\oplus\bigoplus_j\omega_j\otimes\widehat\rho_j
\end{align}
in HJPW factor order. Probability normalization, zero-weight sector choices,
the generic quantum-Markov decomposition, and removal of the external forward
assumption remain open.
...
supported-sector states are also formalized. The unnormalized ambient
tripartite state now has zero complementary blocks and supported blocks
$\omega_j\otimes\widehat\rho_j$ in HJPW factor order. Probability normalization,
zero-weight sector choices, the quantum-Markov decomposition, and the generic
Hayashi forward implication remain open.
