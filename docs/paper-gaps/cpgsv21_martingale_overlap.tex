...
without this stronger one-sided estimate.\footnote{The formal anchors are
    ...
    The constant-$\eta$ reductions are
...
\path{TNLean/MPS/ParentHamiltonian/Martingale/Gap.lean}; its hypothesis is
(\ref{eq:cpgsv21_martingale_overlap_cyclic_anticommutator_target}). The entries
\path{thm:overlap_norm_gap_bound} and
\path{thm:overlap_norm_gap_bound_constant} point to the special and
constant-$\eta$ norm-compression theorems in the same file. The special theorem
uses
\begin{align}
    \eta
    &=
