...
The prefactor remains unmatched: the source asserts that it may be
taken equal to $k^2$, where $k$ is the dimension of the auxiliary space of the
pure state. The formal prefactor is the existential rate-dependent constant
$C_\lambda$ of Lemma~5.2. No general quantitative theorem in the inspected
primary sources establishes $C_\lambda=k^2$ for an arbitrary faithful
stationary density and every prescribed $\lambda$, and no theorem identifies
that minimal $k$ with the bond dimension $D$ used here. Accordingly, no
inverse or finite Gram matrix, pseudoinverse, periodic coercivity, comparison
of unequal kernels, replacement by a cyclic tensor, dimension prefactor,
identity $a_-(m)=1-a(m)$, identification $k=D$, or claim $C_\lambda=k^2$ is
used, and the $D^{3/2}$ reshuffle factor no longer appears in any coefficient
on the route to condition C3.
...
satisfies the weighted remainder spectral-radius condition. The remaining
restriction is the prefactor. The formal $c$ is the existential
rate-dependent constant of Lemma~5.2; the dimension-only value $k^2$ in the
source projector estimate is not asserted from the injectivity threshold.
