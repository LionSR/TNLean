...
It remains to identify the next-prefix projector $G_{n+1}^{(N)}$ with the
right-spectator extension of the whole-ground projector in the $(n+1)$-site
volume.  Subtracting that identity from the completed left-prefix identity
will compare the fixed-volume martingale difference $E_n$ with its
$(n+1)$-site counterpart.  Composition with the completed interval-projector
identity and exact preservation of the fiberwise operator norm will then
reduce $\lVert Q_nE_n\rVert\leq\varepsilon_l$ to the existing variable-volume
C3 estimate.  Until this final next-prefix identification and product
calculation are proved, the fixed-final-volume estimate for $n>l$ remains
open.  No periodic coercivity statement can replace it.
