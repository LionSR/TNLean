...
uniform in the positive left length, and
...
estimate and proves the corresponding anticommutator inequality for every
positive left length. Exact blocked three-site identities then connect the
three-block defect to the local terms of the blocked tensor. No
...
remainder $\mathcal E_A-E_\infty^\rho$. The source phrases the same condition
through the transfer eigenvalues, $\lambda_i<\lambda<1$ for $\lambda_i\neq1$.
The two phrasings agree because
\leanid{MPSTensor.IsPrimitiveMPS.fnwRemainder_eigenvalue_norm_lt_one} shows the
remainder to have exactly the nonunit transfer eigenvalues together with $0$;
the equality of the spectral radius with the largest such modulus is not
separately formalized, and the geometric theorem uses only the spectral-radius
form.
