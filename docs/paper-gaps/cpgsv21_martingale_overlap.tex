...
obtain an all-length periodic gap, and both have been deleted.  The compatible
open-chain estimate, cyclic active-block identity, and finite-range inequality
give
(\ref{eq:cpgsv21_martingale_overlap_open_knabe_periodic_gap}) for every
$N\geq2m$, with no divisibility restriction.  What survives from the deleted
development is the pair of exact conjugation identities relating the blocked
parent interaction and the blocked range-two local terms to their original
range-$2p$ counterparts at block-aligned starts, namely
\leanid{MPSTensor.parentInteractionES_blockTensor_conj} and
\leanid{MPSTensor.localTermES_blockTensor_conj_aligned}.  No periodic boundary
coercivity, comparison of unequal kernels, periodic uniqueness argument,
divisibility substitution, or replacement by a different cyclic tensor is used.
