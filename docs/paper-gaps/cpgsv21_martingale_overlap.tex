...
\leanid{MPSTensor.fnwLowerBoundaryConstant_mono}. The weighted spectator
families and aggregates in
\leanid{MPSTensor.norm_inner_overlap_sub_inner_aggregates_le_div_lowerBoundary}
prove the linear contribution $a(m)/a_-(m)$ in FNW Equation~(6.5). The
quadratic contribution and the final projector estimate in Lemma~6.2 remain
open.
...
\end{align}
This is the linear contribution in Equation~(6.5). It assumes
$a_-(m)>0$ directly; the block-injectivity argument that supplies this
...
Ref.~\cite{Nachtergaele1996Spectral}. The linear term $a(m)/a_-(m)$ is now
formalized in
Equation~\eqref{eq:cpgsv21_martingale_overlap_fnw_linear_6_5}. The aggregate
orthogonality estimates needed for the separate term $a(m)^2/a_-(m)$ remain
open, as do the common-subspace projector argument and the final Lemma~6.2
estimate. No inverse or finite Gram matrix, pseudoinverse, periodic coercivity,
comparison of unequal kernels, replacement by a cyclic tensor, dimension
prefactor, identity $a_-(m)=1-a(m)$, identification $k=D$, or claim
$C_\lambda=k^2$ is used in the source-coordinate estimate. The reconstructed
constant is not identified with a source dimension-dependent value; the
available unweighted conversion retains the proved $D^{3/2}$ norm factor
inside $g$.
...
FNW. The source lower boundary constant, Lemma~5.3, and the linear
$a(m)/a_-(m)$ contribution in Equation~(6.5) are formalized. The
block-injectivity positivity argument, the aggregate orthogonality bounds, the
quadratic $a(m)^2/a_-(m)$ contribution, the common-subspace projector
argument, and the final Lemma~6.2 estimate remain open.
