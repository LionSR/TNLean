...
Ref.~\cite{Nachtergaele1996Spectral}. The linear term $a(m)/a_-(m)$ is
formalized in
...
Equation~(5.9) gives the separate quadratic term $a(m)^2/a_-(m)$. The present
estimate has the deliberately unfactored coefficient
\begin{align}
    \frac{a(m)}{a_-(m)}+\frac{a(m)^2}{a_-(m)}.
    \label{eq:cpgsv21_martingale_overlap_fnw_unfactored_sum}
\end{align}
The common-subspace projector argument and final projector-defect estimate in
Lemma~6.2 remain open for issue~\#7601. No inverse or finite Gram matrix,
pseudoinverse, periodic coercivity, comparison of unequal kernels, replacement
by a cyclic tensor, dimension prefactor, identity $a_-(m)=1-a(m)$,
identification $k=D$, or claim $C_\lambda=k^2$ is used in the source-coordinate
estimate. The reconstructed constant is not identified with a source
...
quadratic $a(m)^2/a_-(m)$ contribution, and their unfactored sum are
formalized. The common-subspace projector geometry and final projector-defect
estimate in Lemma~6.2 remain open for issue~\#7601.
