...
The remaining comparison is an exact intertwining statement for the concrete
projectors.  The canonical splitting of an $(n+r)$-site configuration into its
first $n$ sites and final $r$ sites is
...
local term over right-spectator configurations.  Moreover,
\leanid{ContinuousLinearMap.ker_starProjection_rightFiberwiseMap} identifies
the kernel projection of a fiberwise operator with the fiberwise extension of
the base kernel projection.

It remains to apply these identities to the finite prefix and local-interval
Hamiltonian sums and then identify their smaller-volume kernels with the MPS
subspaces \leanid{MPSTensor.openChainLeftGroundSpaceES} and
\leanid{MPSTensor.openChainTailGroundSpaceES}.  This does not follow by
definitional equality.  Until those sum and range identifications are proved,
the fixed-final-volume estimate
$\lVert Q_nE_n\rVert\leq\varepsilon_l$ for $n>l$ remains open.  No periodic
coercivity statement can replace this missing operator identification.
