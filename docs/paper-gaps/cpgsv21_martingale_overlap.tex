...
Ref.~\cite{Fannes1992Finitely}. The formal C3 threshold now uses this
weighted projection estimate. A sufficient range-two estimate after physical
blocking is also available. The source lower
boundary constant and the conclusions of FNW Lemma~5.3 are identified by
\leanid{MPSTensor.fnwLowerBoundaryConstant} and
\leanid{MPSTensor.fnwLowerBoundaryConstant_mono}. Block injectivity at
$L_0$ gives $a_-(L_0)>0$, and monotonicity gives $a_-(m)>0$ for every
$m\geq L_0$, by
\leanid{MPSTensor.fnwLowerBoundaryConstant_pos_of_isNBlkInjective_of_le}.
The weighted spectator families and aggregates prove the linear contribution
$a(m)/a_-(m)$ in Equation~(6.5), the separate quadratic contribution
$a(m)^2/a_-(m)$, and their unfactored sum. The common-subspace projector
geometry then gives the exact coefficient $a(m)(1+a(m))/a_-(m)$ in
Lemma~6.2. The transfer-eigenvalue prescription also supplies the rate in
its geometric form. The dimension-only prefactor at a prescribed rate requires
the correction in Section~\ref{sec:fnw-dimension-prefactor-obstruction}; the
valid eventual replacement is stated in Section~\ref{sec:fnw-prefactor-verdict}.

\subsection{Lower-endpoint scope in Theorem 2.1(i)}

There is a separate lower-endpoint mismatch between the hypotheses and the
printed proof of Theorem~2.1(i) in
Ref.~\cite{Nachtergaele1996Spectral}. Condition C1, at source
lines~1030--1041, states for a window parameter $q$ that
\begin{align}
    0
     & \leq
    \sum_{k=q}^{N}H_{\Lambda_k\setminus\Lambda_{k-q}}
    \leq d_q H_{\Lambda_N}.
    \label{eq:cpgsv21_martingale_overlap_nachtergaele_c1_lower_endpoint}
\end{align}
In the proof of part~(i), the local term indexed by $n$ is
$H_{\Lambda_{n+1}\setminus\Lambda_{n-l}}$. Thus applying C1 with
$q=l+1$ controls the terms with $n\geq l$, not the terms with $0\leq n<l$.
Condition C2, at source lines~1043--1058, gives the corresponding local gap
only when $n+1\geq n_{l+1}$, while C3, at source lines~1083--1094,
assumes
\begin{align}
    \lVert G_{\Lambda_{n+1}\setminus\Lambda_{n-l}}E_n\rVert
     & \leq\epsilon_l
     &                & \text{only for }n\geq n_l.
    \label{eq:cpgsv21_martingale_overlap_nachtergaele_c3_lower_endpoint}
\end{align}

The proof at source lines~1195--1259 first resolves an arbitrary
$\psi\perp\operatorname{ran}G_{\Lambda_N}$ as
\begin{align}
    \psi
    ...
    \label{eq:cpgsv21_martingale_overlap_nachtergaele_full_resolution}
\end{align}
and then applies C2 and C3 to the estimate for each $E_n\psi$ before summing
over the same full range. The stated hypotheses do not supply these estimates
at the omitted lower indices. The convention $G_{\Lambda_0}=I$ does not make
...
\begin{align}
    ...
    \label{eq:cpgsv21_martingale_overlap_nachtergaele_full_range_coefficient}
\end{align}
The associated norm theorem
\leanid{FrustrationFree.NestedGroundProjections.norm_lower_bound_of_nachtergaele_c1_c3_full_range}
...
The source-threshold hypotheses do not justify the full-range summation, and
no convention stated in Ref.~\cite{Nachtergaele1996Spectral} repairs this.
A one-site chain whose leftmost interaction is weakened satisfies C1, C2, and
C3 with constants uniform in $N$ and violates the printed conclusion of
Theorem~2.1(i); the model, the verification of the three conditions, and the
resulting minimal repair are in
\path{docs/paper-gaps/nachtergaele96_theorem_2_1_lower_endpoint.tex}. The
repair is exactly what
Equation~\eqref{eq:cpgsv21_martingale_overlap_threshold_estimate} isolates,
namely the vanishing of the martingale mass below the threshold, and the
...
The nonwrapping open MPS specialization supplies this control and
...
\begin{align}
    ...
    \label{eq:cpgsv21_martingale_overlap_nachtergaele_c2_weight}
\end{align}
after requiring $c_2>0$. When $\epsilon_l=0$, this choice is inadmissible.
The formal proof treats this case separately: C3 gives
$G_{\Lambda_{n+1}\setminus\Lambda_{n-l}}E_n=0$, so the second mixed inner
product in the display labelled \path{Enpsi} vanishes, and the first weighted
estimate with $c_1=1$ gives the same coefficient. This is a local repair of
the printed proof and adds no hypothesis.
...
The numerical open-chain step is complete. The imported FNW estimate is
uniform in the left length, the empty prefix included, and
...
length, the empty prefix included. Exact blocked three-site identities then
connect the three-block defect to the local terms of the blocked tensor. No
excitation-compression estimate is used.

\subsection{Fixed-final-volume C3 comparison}

Nachtergaele's proof of Theorem~2.1(i) sums all martingale differences in one
fixed final-volume Hilbert space.  For interaction length $l+1$, the formal
prefix filtration now gives the two boundary cases exactly.  If $n<l$, both
adjacent prefixes contain no local interaction and hence $E_n=0$.  At $n=l$,
the local interval and the next prefix contain the same single interaction, so
\begin{align}
    Q_lE_l
     & =G_{l+1}(\Id-G_{l+1})=0.
    \label{eq:cpgsv21_martingale_overlap_fixed_volume_endpoint}
\end{align}
The formal anchors are
\leanid{MPSTensor.fixedAmbient_martingaleDifference_eq_zero_of_lt} and
\leanid{MPSTensor.openIntervalGroundProjectionES_comp_martingaleDifference_at_endpoint}.
No assumption $0<\varepsilon_l$ is needed for these identities.

For $n>l$, put $K=n-l$.  The local C3 theorem acts on the $(n+1)$-site space,
whereas the prefix and interval projectors in the martingale sum act on the
fixed $N$-site space.  The general direct-sum map
\leanid{ContinuousLinearMap.rightFiberwiseMap} applies an operator independently
at every right-spectator configuration.  It preserves composition and does not
increase operator norm; when the spectator set is nonempty, its norm is exactly
the original norm.

The canonical splitting of an $(n+r)$-site configuration into its first $n$
sites and final $r$ sites is
\leanid{MPSTensor.rightSpectatorConfigEquiv}; its Euclidean realization is
\leanid{MPSTensor.rightSpectatorConfigLinearIsometryEquiv}.  For every
nonwrapping interaction, the theorem
\leanid{MPSTensor.localTermES_conj_rightSpectatorConfigLinearIsometryEquiv}
proves that the larger-volume local term is the direct sum of the $n$-site
local term over right-spectator configurations.  The theorems
\leanid{MPSTensor.openPrefixParentHamiltonianES_conj_rightSpectatorConfigLinearIsometryEquiv}
and
\leanid{MPSTensor.openSuffixParentHamiltonianES_conj_rightSpectatorConfigLinearIsometryEquiv}
sum this identity over the exact prefix and local-interval start sets.

The smaller-volume kernels are now explicit.  Under block injectivity,
\leanid{MPSTensor.ker_openPrefixParentHamiltonianES_eq_openChainLeftGroundSpaceES}
identifies the prefix kernel with
\leanid{MPSTensor.openChainLeftGroundSpaceES}.  The full suffix window contains
one local interaction, and
\leanid{MPSTensor.ker_openSuffixParentHamiltonianES_eq_openChainTailGroundSpaceES}
identifies its kernel with
\leanid{MPSTensor.openChainTailGroundSpaceES}.  Combining these statements with
\leanid{ContinuousLinearMap.ker_starProjection_rightFiberwiseMap} gives the
fixed-volume projector identities
\leanid{MPSTensor.openPrefixGroundProjectionES_conj_rightSpectatorConfigLinearIsometryEquiv}
and
\leanid{MPSTensor.openIntervalGroundProjectionES_conj_rightSpectatorConfigLinearIsometryEquiv}.
None of these comparisons uses definitional equality between ambient volumes.

The next-prefix comparison is also complete.
The theorem
\leanid{MPSTensor.openPrefixWholeGroundProjectionES_conj_rightSpectatorConfigLinearIsometryEquiv}
identifies $G_{n+1}^{(N)}$ with the right-spectator extension of the whole-ground
projector in the $(n+1)$-site volume.  Subtracting this identity from the left
prefix identity gives
\leanid{MPSTensor.fixedAmbient_martingaleDifference_conj_rightSpectatorConfigLinearIsometryEquiv}.
Fiberwise composition with the interval projector gives
\leanid{MPSTensor.openIntervalGroundProjectionES_comp_martingaleDifference_conj_rightSpectator}.
Isometric conjugacy preserves operator norm, while the right-fiberwise
extension does not increase it.  Hence
\leanid{MPSTensor.norm_openIntervalGroundProjectionES_comp_martingaleDifference_le_openChain}
reduces the fixed-volume norm to the open-chain norm.

Combining this comparison with the two boundary cases proves
\leanid{MPSTensor.IsPrimitiveMPS.exists_fixedAmbient_martingaleDifference_norm_lt_c3_threshold}.
For the same block-injective $l>1$ and the same $\varepsilon_l\geq0$ chosen by
the open-chain theorem, every $N$ and every $n<N$ satisfy
\begin{align}
    \lVert G_{\Lambda_{n+1}\setminus\Lambda_{n-l}}^{(N)}E_n^{(N)}\rVert
     & \leq\varepsilon_l,
     &
    \varepsilon_l
     & <\frac{1}{\sqrt{l+1}}.
    \label{eq:cpgsv21_martingale_overlap_fixed_volume_c3}
\end{align}
The proof treats $n<l$, $n=l$, and $n>l$ separately.  Positivity of
$\varepsilon_l$ is not assumed, and $K=n-l>0$ is used only in the third case.
The comparison uses neither definitional equality between ambient volumes nor
periodic coercivity.
...
This argument neither claims the source prefactor $k^2$ nor uses an
...
without this stronger one-sided estimate.\footnote{The formal anchors are
    ...
    The ordered cross-term reduction in
    \path{TNLean/MPS/ParentHamiltonian/Martingale/Reduction.lean} is
    \leanid{MPSTensor.parentHamiltonianES_gap_bound_of_cyclic_window_ordered_cross_term}.
    The cyclic-window norm-compression reductions
    \path{MPSTensor.parentHamiltonianES_gap_bound_of_cyclic_window_overlap_norm_bound},
    \path{MPSTensor.parentHamiltonianES_gap_bound_of_cyclic_window_overlap_norm_bound_of_le},
    and
    \path{MPSTensor.parentHamiltonianES_gap_bound_of_cyclic_window_overlap_norm_bound_of_lt},
    together with the public spectral-gap reformulation
    \path{MPSTensor.parentHamiltonian_gapped_of_overlap_norm_constant}, have
    been removed; the anticommutator chain named above is the formal route that
    remains.
    The projection-geometry results are in
    \path{QICLean/Analysis/ProjectionGeometry.lean}. This note reserves
    ``anticommutator estimate'' for the source condition and ``norm
    compression'' for the stronger local condition, which is now discussed here
    only as a comparison.}

The finite-overlap cyclic anticommutator formulation is the formal condition
that matches the source, and it is the only one the formal development carries.
A flexible norm-compression variant, kept here as a comparison only, assumes
$\eta\geq0$ and
\begin{align}
    2\eta(L-1)
     & <1,
...
\section{Constants and blocking conventions}
...
principal-angle geometry. The comparison must distinguish the chosen
finite cyclic blocking convention from the weighted FNW estimate used by the
open-chain C3 threshold. The latter is proved with a rate-dependent prefactor;
the fixed dimension-only prefactor is addressed separately below.
...
The source prefactor $k^2$ cannot hold at every prescribed admissible rate
from the fixed intersection threshold, as shown in
Section~\ref{sec:fnw-dimension-prefactor-obstruction}. That assertion is
unnecessary for the proved range-two blocked theorem: the exact
...
\end{align}
for the complementary excitation projections. The weighted FNW estimate
supplies the uniform numerical bound with a rate-dependent constant. A
finite-dimensional principal-angle decomposition alone supplies no such
uniform estimate; the additional decay input is essential.
...
A symmetric operator-product route can be compared in the same way. Assume
\begin{align}
...
\leanid{LinearMap.IsSymmetricProjection.re_inner_anticommutator_ge_neg_of_norm_apply_le}.
The cyclic-window entry points
\path{MPSTensor.parentHamiltonianES_gap_bound_of_cyclic_window_overlap_operator_norm_of_le}
and
\path{MPSTensor.parentHamiltonianES_gap_bound_of_cyclic_window_overlap_operator_norm_of_lt},
together with the public reformulation
\path{MPSTensor.parentHamiltonian_gapped_of_overlap_operator_norm_constant},
have been removed, because the hypothesis they carry is unsatisfiable in the
generic overlapping case and no discharger for it exists. For the
...
is (\ref{eq:cpgsv21_martingale_overlap_cyclic_anticommutator_target}).
The Blueprint entries that recorded the special and constant-$\eta$
cyclic-window norm-compression reductions, and the entry for the
operator-product variant, have been deleted together with the theorems they
cited. The special reduction had used
\begin{align}
    \eta
     & =
...
\end{align}
The finite-row, cyclic-window, non-overlap, anticommutator, and spectral
reductions are formalized, and the blocked range-two MPS specialization is
complete. Comparing the blocked Hamiltonian with an original-tensor parent
interaction in original-site units is tracked under issue~\#5926.
...
$1/8$. The stronger conditional norm-compression route, which assumed
\begin{align}
    \lVert p_ip_jv\rVert
               & \leq\eta\lVert p_iv\rVert,
               &
...
\end{align}
and the symmetric operator-product route are no longer carried in the formal
development: neither hypothesis is discharged anywhere, and the
operator-product one is unsatisfiable whenever the overlapping excitation
ranges share a nonzero vector. The coefficient in
(\ref{eq:cpgsv21_martingale_overlap_special_eta}) was one sufficient choice,
...
Hamiltonian after blocking. The separate claim that the prefactor may be
$k^2$ at every prescribed admissible rate is refuted in
Section~\ref{sec:fnw-dimension-prefactor-obstruction}. The weighted estimate
with a rate-dependent prefactor and its eventual replacement remain valid.

\section{Inverse-Gram identities and the weighted FNW estimate}
...
$0<\lambda<1$ satisfy $|\mu|<\lambda$ for every nonunit eigenvalue $\mu$
of the observable transfer operator, and let $c>0$ be its prefactor.
...
\cite[Section~6]{Nachtergaele1996Spectral}. Here $k^2$ is the square of that
dimension. The physical counterexample in
Section~\ref{sec:fnw-dimension-prefactor-obstruction} shows that this value
cannot be combined with every prescribed admissible rate at the fixed
intersection threshold. The proved geometric estimate instead uses a
rate-dependent $c=C_\lambda>0$, supplied by the weighted transfer estimate.
The source's eigenvalue prescription is sufficient for that estimate, without
a diagonalizability assumption. Choose $l$ beyond an injective length and such that
\begin{align}
    c\lambda^l
     & <1.
    \label{eq:cpgsv21_martingale_overlap_source_transfer_smallness}
\end{align}
The weighted FNW projector theorem proves the quoted rational expression
under these conditions. Any prescribed positive $c$, including $k^2$, also
works after the additional onset described in
Section~\ref{sec:fnw-prefactor-verdict}; this does not give the original fixed
threshold.

The inverse-Gram identities remain valid descriptions of the physical range
projectors. The quantitative C3 estimate uses the weighted FNW argument rather
than an inverse-Gram reconstruction. The following results record both these
identities and the current weighted route.
...
\begin{itemize}
    ...
    \item The C3$'$ whole-increment algebra and a prefix-uniform estimate are
          ...
          These finite-Gram results are no longer assembled into a projector-defect
          estimate. The former inverse-Gram reconstruction of that estimate, whose
          coefficient $Cr^L/(1-cr^L)$ carried an unidentified constant and the
          $D^{3/2}$ reshuffle factor, has been deleted. The whole-increment defect is
          now controlled by Lemma~6.2 of Ref.~\cite{Fannes1992Finitely} in the
          geometric form quoted by Ref.~\cite{Nachtergaele1996Spectral}. Write
          \begin{align}
          ...
          \end{align}
          The theorem
          \leanid{MPSTensor.IsPrimitiveMPS.exists_wholeIncrement_groundProjection_defect_le_fnw_geometric}
          gives $c>0$, $0<\lambda<1$, and an injective length $L_0\geq1$ such that,
          whenever $L\geq L_0$, $c\lambda^L<1$, $Q\geq1$, and $K\geq0$,
          \begin{align}
              \left\lVert
              P^{\mathrm{tail}}_{K;L,Q}P^{\mathrm{left}}_{K+L,Q}
              -P_{\mathcal G_{K+L+Q}^{\mathrm{ES}}(A)}
              \right\rVert
               & \leq\frac{c\lambda^L(1+c\lambda^L)}{1-c\lambda^L}.
              \label{eq:cpgsv21_martingale_overlap_whole_increment_defect}
          \end{align}
          The prefix length is unrestricted: the estimate holds at $K=0$, so the
          domain of the martingale difference $E_n$ is imposed only where that
          difference itself appears. The prefactor $c$ is the existential
          rate-dependent constant of Lemma~5.2; the source value $k^2$ is not
          asserted.
...
\end{align}
These constants no longer enter any live estimate: the reconstructed open-chain
coefficient $C_*r^l/(1-cr^l)$ has been deleted together with its assembly. A
one-site suffix increment needs no reassociation, so
\leanid{MPSTensor.openChain_groundProjection_defect_eq_wholeIncrement} identifies
the open-chain defect at overlap length $l$ and prefix length $K$ with the
whole-increment defect at $r=K$, $m=l$, and $\ell=1$. Hence
\leanid{MPSTensor.IsPrimitiveMPS.exists_openChain_groundProjection_defect_le_fnw_geometric}
proves, for every prefix length $K$ and every $l$ above an injective length
with $c\lambda^l<1$,
\begin{align}
    \left\lVert
...
\end{align}
This is the estimate imported by Ref.~\cite{Nachtergaele1996Spectral} from
Ref.~\cite{Fannes1992Finitely}. No physical projector is identified with a
transfer fixed-point projector.

The source coefficient suffices for Nachtergaele's C3 condition. The
theorem
\leanid{MPSTensor.IsPrimitiveMPS.exists_openChain_groundProjection_defect_lt_c3_threshold}
uses
\begin{align}
...
\end{align}
The same $l$ works for every prefix length $K$, including $K=0$. The theorem
\leanid{MPSTensor.IsPrimitiveMPS.exists_re_inner_openChain_anticommutator_ge_c3_threshold}
...
The weighted virtual-space foundation is formalized. For every
...
$\C^{D^2}$. The formal results also include the transfer remainder estimate
of Lemma~5.2, the lower-boundary assertions of Lemma~5.3, the weighted
spectator families, the two source aggregates, their middle-block
decomposition, the spectator norm identities, and the linear estimate
\begin{align}
    \bigl|\langle L\Phi,R\Psi\rangle
    -\langle A_\varphi,A_\psi\rangle_\rho\bigr|
     & \leq\frac{a(m)}{a_-(m)}\lVert L\Phi\rVert\lVert R\Psi\rVert.
    \label{eq:cpgsv21_martingale_overlap_fnw_linear_6_5}
\end{align}
This is the linear contribution in Equation~(6.5). Block injectivity at the
interaction length $L_0$ supplies $a_-(L_0)>0$, and the recursion from
Equation~(5.5) propagates positivity to every $m\geq L_0$ by Lemma~5.3.

The linear term $a(m)/a_-(m)$ is formalized in
Equation~\eqref{eq:cpgsv21_martingale_overlap_fnw_linear_6_5}. Testing the
orthogonality hypotheses against the special full-ground families, using the
boundary identities from Equations~(5.5) and~(5.8), and applying
Equation~(5.9) gives the separate quadratic term $a(m)^2/a_-(m)$. The theorem
\leanid{MPSTensor.wholeIncrement_groundProjection_defect_le_fnw} preserves the
unfactored coefficient
\begin{align}
    \frac{a(m)}{a_-(m)}+\frac{a(m)^2}{a_-(m)}.
    \label{eq:cpgsv21_martingale_overlap_fnw_unfactored_sum}
\end{align}
The two overlap ranges contain the full boundary range. After removing this
common subspace, the overlap estimate bounds the product of the two orthogonal
projections. Global physical reversal identifies these ranges with the two
whole-increment ranges and the full ground space. Consequently,
\leanid{MPSTensor.wholeIncrement_groundProjection_defect_le_fnw_factored}
proves the coefficient in Lemma~6.2:
\begin{align}
    \left\lVert
    P^{\mathrm{tail}}_{r;m,\ell}P^{\mathrm{left}}_{r+m;\ell}
    -P_{\mathcal G_{r+m+\ell}^{\mathrm{ES}}(A)}
    \right\rVert
     & \leq\frac{a(m)(1+a(m))}{a_-(m)}
    \label{eq:cpgsv21_martingale_overlap_fnw_projector_defect}
\end{align}
for $L_0,r,\ell\geq1$ and $m\geq L_0$, where $L_0$ is an injective length.
The theorem
\leanid{MPSTensor.IsPrimitiveMPS.exists_wholeIncrement_groundProjection_defect_le_fnw}
chooses such an $L_0$ for a primitive tensor.

The later prescribed-rate expression
\begin{align}
...
Ref.~\cite{Nachtergaele1996Spectral} is now derived from the exact FNW
quantities above. The theorem
\leanid{MPSTensor.wholeIncrement_groundProjection_defect_le_fnw_geometric}
replaces the denominator $a_-(m)$ by $1-c\lambda^m$ using the inequality
$1-a(m)\leq a_-(m)$ of Equation~(5.9), which is weaker than the identity
$a_-(m)=1-a(m)$, and bounds the numerator $a(m)(1+a(m))$ by
$c\lambda^m(1+c\lambda^m)$ using monotonicity. The prescribed rate is supplied
by
\leanid{MPSTensor.IsPrimitiveMPS.exists_fnwMixingQuantity_le_geometric}: any
$\lambda$ strictly above the weighted spectral radius of the transfer
remainder $\mathcal E_A-E_\infty^\rho$. The source instead phrases its
condition through the transfer eigenvalues, $\lambda_i<\lambda<1$ for
$\lambda_i\neq1$. The source eigenvalue prescription now implies this spectral-radius
condition. Write $E=\mathcal E_A$ and $P=E_\infty^\rho$. Since
$PE=P$ and $P^2=P$, a nonzero eigenvalue $\nu$ of $E-P$ with eigenvector
$X$ satisfies $PX=0$, and hence $EX=\nu X$. Primitivity excludes $\nu=1$
for the remainder. Thus, if $\lambda>0$ exceeds the moduli of all nonunit
transfer eigenvalues, every remainder eigenvalue has modulus below
$\lambda$. Finite dimensionality gives the required strict spectral-radius
bound, also in the weighted norm. The declarations
\leanid{MPSTensor.IsPrimitiveMPS.fnwWeightedRemainderSpectralRadius_lt_of_transfer_eigenvalues}
and
\leanid{MPSTensor.IsPrimitiveMPS.exists_fnwMixingQuantity_le_geometric_of_transfer_eigenvalues}
therefore establish the prescribed-rate estimate without a diagonalizability
assumption.

The dimension-only prefactor requires a correction when the decay rate is
prescribed. Section~\ref{sec:fnw-dimension-prefactor-obstruction} gives an
explicit physical counterexample to taking $c=k^2$ at every admissible rate
from the fixed intersection threshold. The geometric estimate used here
retains the rate-dependent constant $C_\lambda$ of Lemma~5.2. Alternatively,
any prescribed positive prefactor is valid after an additional onset depending
on the state, rate, and prefactor. Neither conclusion requires a
$D^{3/2}$ reshuffle factor, a finite Gram inverse, or periodic coercivity.
...
\section{A counterexample at a prescribed rate}
\label{sec:fnw-dimension-prefactor-obstruction}

The simultaneous assertions that $c=k^2$ and that every prescribed admissible
rate may be used in Ref.~\cite[Section~6, equation~(6.1)]{Nachtergaele1996Spectral}
require a qualification on the overlap threshold. The following calculation concerns the physical projections themselves.
The full statement, including injectivity, primitivity, a faithful trace-one
stationary density, and the admissible rate, is formalized by
\leanid{MPSTensor.FNWDimensionConstant.exists_dimensionFour_counterexample}.

Let $e=(1,1,1,1)^{\mathsf T}$ and put
\begin{align}
    u & =(1,1,-1,-1)^{\mathsf T}, &
    v & =(1,-1,1,-1)^{\mathsf T}, &
    w & =(1,-1,-1,1)^{\mathsf T},   \\
    P & =\frac{ee^{\mathsf T}}4+
    \frac{uv^{\mathsf T}+vw^{\mathsf T}}{16}.
\end{align}
These four vectors are orthogonal and have squared norm $4$. The entries of
$P$ belong to $\{1/8,1/4,3/8\}$, and its row and column sums are one. Thus
\begin{align}
    P^2=\frac{ee^{\mathsf T}}4+\frac{uw^{\mathsf T}}{64},\qquad
    P^3=\frac{ee^{\mathsf T}}4,\qquad Pu=0,\qquad w^{\mathsf T}P=0.
\end{align}
Define $A_{ij}=\sqrt{P_{ij}}E_{ij}$ on a physical alphabet of ordered pairs.
This tensor has physical dimension $16$ and bond dimension $4$. Its letters
span the full matrix algebra, and both $\sum_{ij}A_{ij}A_{ij}^*$ and
$\sum_{ij}A_{ij}^*A_{ij}$ equal the identity. Its stationary density is $I/4$.
The Schrödinger transfer map kills off-diagonal entries and acts by $P$ on
diagonal entries; its trace adjoint is the observable transfer map, which
acts by $P^{\mathsf T}$ on diagonal entries. Both third powers equal
$X\mapsto\operatorname{tr}(X)I/4$. Both maps are therefore primitive,
and all their nonunit eigenvalues are zero.
One-site injectivity gives $m_0=1$, so the source's intersection threshold
can be taken to be $\ell_0=m_0+1=2$.

Fix endpoints $a,d\in\{0,1,2,3\}$. Using the physical basis of consistent
four-edge paths, define
\begin{align}
    x & =2\sum_{b,c,e}u_b\sqrt{P_{ab}P_{bc}P_{ce}P_{ed}}
    |ab,bc,ce,ed\rangle,                                 \\
    y & =2\sum_{b,c,e}w_e\sqrt{P_{ab}P_{bc}P_{ce}P_{ed}}
    |ab,bc,ce,ed\rangle.
\end{align}
Then $x$ belongs to the range of $I\otimes G_{[1,3]}$, and $y$ belongs to
the range of $G_{[0,2]}\otimes I$. Both vectors have norm one, since
$u_b^2=w_e^2=1$ and $(P^4)_{ad}=1/4$. The four-site ground vector with
endpoints $a,d$ has the same path coefficients without the factor $2u_b$.
Its inner product with $x$ vanishes by $Pu=0$ and $(P^3)_{bd}=1/4$.
Ground vectors with other endpoints have disjoint supports, so
$x\perp\mathcal G_{[0,3]}$. Finally,
\begin{align}
    \langle y,x\rangle
    =4\sum_{b,e}P_{ab}u_b(P^2)_{be}w_eP_{ed}=\frac1{16}.
\end{align}
Indeed, the constant term in $P^2$ contributes zero and its rank-one term
contributes $4/64$. Taking this matrix coefficient between unit vectors gives
\begin{align}
    \left\|(G_{[0,2]}\otimes I)(I\otimes G_{[1,3]})-G_{[0,3]}\right\|
    \geq\frac1{16}.
\end{align}
For the admissible rate $\lambda=1/1000$, $c=16$, and overlap $m=2$,
\begin{align}
    c\lambda^m=\frac1{62500}<1,\qquad
    \frac{c\lambda^m(1+c\lambda^m)}{1-c\lambda^m}
    =\frac{62501}{62500\cdot62499}<\frac1{16}.
\end{align}
Thus the dimension-only prefactor cannot hold for every prescribed rate
starting at the fixed intersection threshold. The auxiliary dimension here
is minimal: the one-site density matrix is diagonal with strictly positive
entries $P_{ij}/4$, hence has rank $16$. Any pure GVBS/MPS representation of
auxiliary dimension $k'$ has one-site support rank at most $(k')^2$.
Thus $k'\geq4$.
This leaves intact the eventual estimate, the bound with a rate-dependent
prefactor, and the possibility of choosing a suitable rate after fixing
the dimension-only prefactor.

\section{Verdict}
\label{sec:fnw-prefactor-verdict}

\textbf{Verdict: false as printed for the dimension-only prefactor, with
    scope restrictions elsewhere.} The classification of this note is
\path{false-source}: the tensor of
Section~\ref{sec:fnw-dimension-prefactor-obstruction} is injective and
primitive, has a faithful trace-one stationary density and minimal auxiliary
dimension $k=4$, and admits the rate $\lambda=1/1000$ above every nonunit
transfer eigenvalue modulus; it therefore satisfies the hypotheses under which
the source asserts the projector estimate with $c=k^2$ from the intersection
threshold $\ell_0=2$, and it violates that estimate. The remaining deviations
recorded below are scope restrictions of the formal theorems and do not
concern the truth of the printed assertions. The exact whole-increment algebra, the
prefix-uniform geometric C3$'$-type estimate, the three-block coefficient
$7/16$, and the resulting cyclic range-two gap $1/8$ are proved for a suitable
blocked tensor. Before blocking, lengths are measured in sites of the input
tensor; the final chain length counts blocked sites. The source lower boundary
constant, Lemma~5.3, block-injectivity positivity, the linear $a(m)/a_-(m)$
contribution in Equation~(6.5), the quadratic $a(m)^2/a_-(m)$ contribution,
their unfactored sum, and the common-subspace projector geometry are
formalized, and the resulting whole-increment theorem proves the exact
coefficient $a(m)(1+a(m))/a_-(m)$ from Lemma~6.2. The threshold that feeds the
gap theorems now runs through that estimate and its geometric form
$c\lambda^l(1+c\lambda^l)/(1-c\lambda^l)$; the earlier inverse-Gram
reconstruction and its unidentified constant have been deleted. The eigenvalue-rate condition is now formalized: every positive
$\lambda$ strictly above the moduli of the nonunit transfer eigenvalues
satisfies the weighted remainder spectral-radius condition. The prefactor
requires a correction: the dimension-only value $k^2$ cannot be used at
every prescribed admissible rate from the fixed intersection threshold, as
Section~\ref{sec:fnw-dimension-prefactor-obstruction} demonstrates. The formal
$c$ is the rate-dependent constant of Lemma~5.2.

A corrected eventual statement permits any prescribed $c>0$, for every rate
$\lambda>0$ strictly above the moduli of the nonunit transfer eigenvalues;
the projector bound additionally requires $\lambda<1$. Choose a rate
$\mu$ strictly between the remainder spectral radius and $\lambda$. The bound
$a(n)\leq C_\mu\mu^n$ implies $a(n)\leq c\lambda^n$ for all sufficiently
large $n$, since $(\mu/\lambda)^n\to0$. Increasing this onset to exceed an
injectivity length and to ensure $c\lambda^n<1$ gives the same rational
projector bound, uniformly in both exterior interval lengths. The two
formal statements are
\leanid{MPSTensor.IsPrimitiveMPS.eventually_fnwMixingQuantity_le_of_pos_prefactor}
and
\leanid{MPSTensor.IsPrimitiveMPS.eventually_projector_defect_le_of_pos_prefactor}.
The onset depends on the state, rate, and prefactor. For no value of $c$,
including $c=k^2$, is this onset shown to equal the injectivity threshold. The
printed assertion, with prefactor $k^2$ at every admissible rate from that
threshold, is refuted by
Section~\ref{sec:fnw-dimension-prefactor-obstruction}.

An earlier blocked-to-original comparison derived the bound
$\operatorname{gap}(H_{Np,2p}(A))\geq1/8$ for $N\geq4$, on periodic lengths
divisible by the chosen block length.  Its engine was a finite cyclic
sparse-sum domination obtained by discarding the local terms whose start is not
block-aligned.  Ref.~\cite{Nachtergaele1996Spectral} instead argues
the corresponding statement by rescaling to the increasing sequence of finite
volumes
$\tilde\Lambda_n=\Lambda_{ln}$ and applying part~(i) of its Theorem~2.1 with
$l=1$, so the finite cyclic domination was a reconstruction rather than the
printed route.  That theorem and its sparse-sum engine are no longer needed to
obtain an all-length periodic gap, and both have been deleted.  The removed
declarations are
\path{MPSTensor.IsPrimitiveMPS.exists_parentHamiltonianES_gap_eighth_mul},
which carried the divisible-length bound; its engine
\path{MPSTensor.blockTensor_parentHamiltonianES_conj_le}, which carried the
sparse aligned-sum domination; the pair of exact conjugation identities
\path{MPSTensor.parentInteractionES_blockTensor_conj} and
\path{MPSTensor.localTermES_blockTensor_conj_aligned}, which related the
blocked parent interaction and the blocked range-two local terms to their
original range-$2p$ counterparts at block-aligned starts; and the private
helpers serving them.  The replacement for the first is
\leanid{MPSTensor.IsPrimitiveMPS.exists_parentHamiltonianES_gap_of_finiteRangeKnabe},
whose compatible open-chain estimate, cyclic active-block identity, and
finite-range inequality give
(\ref{eq:cpgsv21_martingale_overlap_open_knabe_periodic_gap}) for every
$N\geq2m$, with no divisibility restriction on the chain length.  The two
conjugation identities had no consumer once the sparse-sum domination was gone,
so they needed no replacement and were deleted with the rest; nothing survives
from that development.  The dated audit note
\path{docs/audits/2026-08-29_fbc25_local_orthogonality_rg_scope.md} was amended
in the same change so that it cites the surviving theorem rather than the
removed one.  No periodic boundary coercivity, comparison of unequal kernels,
periodic uniqueness argument, divisibility substitution, or replacement by a
different cyclic tensor is used.

The conditional norm-compression and operator-product branch has been
removed.  It consisted of
\path{MPSTensor.parentHamiltonian_gapped},
\path{MPSTensor.parentHamiltonian_gapped_of_overlap_norm_constant},
\path{MPSTensor.parentHamiltonian_gapped_of_overlap_operator_norm_constant},
\path{MPSTensor.parentHamiltonianES_quadratic_form_of_finite_overlap_norm_bound}
and its constant-$\eta$ companion,
\path{MPSTensor.parentHamiltonianES_gap_bound_of_cyclic_window_overlap_norm_bound}
with its two constant-$\eta$ companions, and the two operator-product
reductions
\path{MPSTensor.parentHamiltonianES_gap_bound_of_cyclic_window_overlap_operator_norm_of_le}
and
\path{MPSTensor.parentHamiltonianES_gap_bound_of_cyclic_window_overlap_operator_norm_of_lt}.
The replacement for all of them is the source-anchored anticommutator route,
whose public statement is
\leanid{MPSTensor.parentHamiltonian_gapped_of_anticommutator} and whose
cyclic-window reduction is
\leanid{MPSTensor.parentHamiltonianES_gap_bound_of_cyclic_window_overlap_anticommutator}.
None of the removed theorems had a consumer, the all-vector estimate they
assume has no discharger, and the operator-product form is unsatisfiable
whenever the overlapping excitation ranges share a nonzero vector.  The nine
Blueprint entries that stated them were deleted with them, and the Blueprint
entry \path{thm:parent_hamiltonian_gapped} now states the anticommutator
hypothesis alone.

The abstract caveat is now settled against the source.  The printed
hypotheses of \cite[Theorem~2.1(i)]{Nachtergaele1996Spectral} do not control
the initial indices in the full martingale sum, and the printed conclusion is
false without such control; the counterexample and the minimal repair are in
\path{docs/paper-gaps/nachtergaele96_theorem_2_1_lower_endpoint.tex}.  The
primitive MPS theorem applies because the compatible open-chain Hamiltonians
make the early C1 and C2 terms vanish and the boundary C3 product is zero.
Thus the MPS specialization is complete, while the unrestricted abstract
full-range identification is not asserted.
