...
For the adjacent range-two conditions in the three-site Hilbert space, the
individual identifications are now explicit. The coordinate equalities between
last-site restriction and the cyclic window at site $0$, and between the
one-site-prefix tail restriction, first-site restriction, and the cyclic window
at site $1$, give
\[
  \ker h_0=\leanid{openChainLeftGroundSpaceES}(B,2),\qquad
  \ker h_1=\leanid{openChainTailGroundSpaceES}(B,1,2).
\]
These are formalized as \leanid{ker_localTermES_two_three_zero} and
\leanid{ker_localTermES_two_three_one}. Since each $h_i$ is a symmetric
projection, uniqueness from its range then gives
\[
  \leanid{openChainLeftGroundSpaceES}(B,2)^\perp\text{-projection}=h_0,
  \qquad
  \leanid{openChainTailGroundSpaceES}(B,1,2)^\perp\text{-projection}=h_1.
\]
No injectivity or nonzero bond-dimension assumption enters these individual
kernel and projector identities.
