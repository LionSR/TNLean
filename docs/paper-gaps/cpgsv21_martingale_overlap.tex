...
$\lambda_i\neq1$. The two phrasings are not shown here to describe the same
rates. What is formalized about the remainder's spectrum is
\leanid{MPSTensor.IsPrimitiveMPS.fnwRemainder_eigenvalue_norm_lt_one}: every
eigenvalue of the remainder has modulus below one. That is a bound, not a
characterization. It does not exhibit the remainder's spectrum as the nonunit
transfer eigenvalues together with $0$, and it does not identify the weighted
spectral radius with the largest modulus among the nonunit transfer
eigenvalues. The geometric theorem therefore uses the spectral-radius form
alone, and the source's eigenvalue prescription remains unformalized.

Also unmatched is the source's assertion that the prefactor may be
...
reconstruction and its unidentified constant have been deleted. Two
restrictions remain. The first is the rate: the formal condition admits any
$\lambda$ strictly above the weighted spectral radius of the transfer
remainder, whereas the source prescribes $\lambda$ through the nonunit transfer
eigenvalues, and the two phrasings are not shown here to describe the same
rates, so the source's eigenvalue prescription remains unformalized. The second
is the prefactor: the formal $c$ is the existential rate-dependent constant of
Lemma~5.2, and the source value $k^2$ is not asserted.
...
The finite threshold $N\geq2m$ can also be removed without comparing
Hamiltonians with different kernels. For each fixed $N$, the restriction of
$H_{N,L}(A)$ to $(\ker H_{N,L}(A))^\perp$ is injective on a finite-dimensional
space, hence has a positive lower norm bound $\delta_N$. Taking the minimum
of the large-length bound and the finitely many $\delta_N$ for $N<2m$
gives a uniform positive constant at every length. This is proved by
\leanid{MPSTensor.parentHamiltonianES_exists_gap} and
\leanid{MPSTensor.parentHamiltonianES_gap_of_eventual_gap}.
The normalized primitive gauge of an arbitrary normal tensor preserves its
parent Hamiltonian. Consequently
\leanid{MPSTensor.exists_parentHamiltonianES_uniform_gap_of_isNormal}
gives the uniform gap at a suitable range $L=l+1$ for every $N\geq2L$,
without a normalization hypothesis on the tensor. The resulting constant
need not equal the explicit Knabe coefficient. The block-injective gap
assertion remains separate from this normal-tensor result.
