...
Ref.~\cite{Nachtergaele1996Spectral} is not identified with the exact FNW
quantities above. No inverse or finite Gram matrix, pseudoinverse, periodic
coercivity, comparison of unequal kernels, replacement by a cyclic tensor,
dimension prefactor, identity $a_-(m)=1-a(m)$, identification $k=D$, or claim
$C_\lambda=k^2$ is used. The available unweighted conversion retains the
...
FNW. The source lower boundary constant, Lemma~5.3, block-injectivity
positivity, the linear $a(m)/a_-(m)$ contribution in Equation~(6.5), the
quadratic $a(m)^2/a_-(m)$ contribution, their unfactored sum, and the
common-subspace projector geometry are formalized. The resulting
whole-increment theorem proves the exact coefficient
$a(m)(1+a(m))/a_-(m)$ from Lemma~6.2. The separate prescribed-rate
identification with the expression quoted by Nachtergaele remains outside
this result.
...
block-aligned.  Ref.~\cite{Nachtergaele1996Spectral} proves the corresponding
statement instead by rescaling to the increasing sequence of finite volumes
