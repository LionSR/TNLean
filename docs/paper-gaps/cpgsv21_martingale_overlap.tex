...
operators.  One must prove that the fixed-volume prefix kernel projection and
local-interval kernel projection are the right-spectator sums of the
corresponding $(n+1)$-site open-chain projectors.  This does not follow by
definitional equality: the fixed-volume operators are kernels of sums of
\leanid{MPSTensor.localTermES}, while the local theorem uses the MPS subspaces
\leanid{MPSTensor.openChainLeftGroundSpaceES} and
\leanid{MPSTensor.openChainTailGroundSpaceES}.  The existing cyclic-window
configuration decomposition in
\path{TNLean/MPS/ParentHamiltonian/LocalSupportTransport.lean} is private and
specialized to a three-site window, so it cannot establish this arbitrary
prefix identity.  Until an arbitrary-window configuration equivalence and its
local-term intertwining theorem are proved, the fixed-final-volume estimate
