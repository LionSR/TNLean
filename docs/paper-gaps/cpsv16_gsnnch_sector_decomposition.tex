...
common-weight theorem retains the joint five-identity package
\[
  ...
  \texttt{AppPsiPhi}.
\]
It combines the supported sector bonds with the positive-length BNT sum and
its natural copy multiplicities to obtain the GSNNCH form.  The trace
factorization and normalization remain explicit hypotheses, although only
positivity is needed in this forward implication.

There is an independent common-copy-weight interface:
\[
...
Under the additional common-weight hypothesis, the theorem derives the
orthogonal commuting sector family from the printed identities before
applying the general direct-sum result.  No reverse comparison with a
single-bond predicate is needed.
...
projections.  The common-weight theorem combines these bonds with the BNT
copy-number identity while retaining \texttt{Apptralktrrk} and
\texttt{AppPsiPhi} in its statement.

Proposition~\texttt{prop3to4} nevertheless remains partial under its printed
hypotheses.  Copy independence is absent from the five listed identities,
and the proof invokes Lemma~\texttt{lemmus}, whose hypothesis includes source
ZCL.  The formal theorem therefore carries an explicit scope restriction
rather than treating that earlier lemma as an unstated assumption.
