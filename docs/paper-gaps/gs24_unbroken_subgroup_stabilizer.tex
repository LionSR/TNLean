...
and therefore for the cyclic examples worked out in the first source. It
fails for members of the general construction of the second source, which
builds a symmetry-broken state for every subgroup $H\leq G$ trivializing
$\omega$~\cite{GarreRubio2023MPOSym}.\footnote{Source traceability:
\path{Papers/2203.12563/REsubmission.tex}, lines 1997--2005 and
2098--2100.}
...
\begin{enumerate}
    ...
    \item \emph{Reconstruction.} The two-cocycle $\psi_{x_0}$ is defined on
          ...
          representatives do not correspond to blocks. The restriction of
          $L^{x_0}$ to $H$ is a trivializing two-cochain of $\omega|_H$; only
          after a choice of trivializing gauge $\beta$ is
          $\psi_{x_0}=L^{x_0}/\beta$ a two-cocycle. For fixed $(G,\omega,H)$
          the gauge classes of solutions therefore form a torsor for
          $\mathcal H^2(H,U(1))$, as stated at line 761 of the second
          source, and the classifying data are the stabilizer $H$ up to
          conjugacy together with a point of that torsor, the data
          classifying the indecomposable module categories of
          $\mathrm{Vec}_G^\omega$.
\end{enumerate}
...
\textbf{Verdict: false as printed, resolved.} With the unbroken subgroup
defined as the kernel of the action, the printed degeneracy $|G/H|$ fails for
$S_3$ acting transitively on three blocks. The correct statement, available in
arXiv:2203.12563, reads $H$ as the stabilizer of one block; the kernel agrees
with it whenever the stabilizer is normal, in particular for every abelian
group, so the cyclic examples of arXiv:2405.00439 are unaffected.
