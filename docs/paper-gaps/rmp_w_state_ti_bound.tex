...
The bound (\ref{eq:rmp_w_state_ti_bound_conditional}) is conditional. Three
...
\begin{itemize}
    ...
    which yields $D=\Omega(N^{1/5})$ for primitive blocks; the bound $D^2$
    is formalized only when every $A^j_0$ is invertible.

    \item The reduction to blocks that are pairwise distinct in the sense of
    the direct-sum lemma. The source assumes without loss of generality only
    that the block states are pairwise different
    \cite[lines~1329--1330]{PerezGarcia2007MPSReps}; the stricter
    distinctness used here is a hypothesis of the formal bound.
\end{itemize}
...
$D=\Omega(N^{1/5})$. The review's bound
(\ref{eq:rmp_w_state_ti_bound_asymptotic_lower_bound}) and the source's
unconditional $D\succeq O(N^{1/3})$ remain unformalized: they need a
reduction of an arbitrary tensor to a canonical form with condition C1 at a
fixed length and pairwise distinct blocks, and the explicit $O(D^2)$ or
$O(D^2\log D)$ Wielandt index.
