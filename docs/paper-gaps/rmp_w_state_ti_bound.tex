...
\begin{enumerate}
    ...
    \item With $L_0=O(D^2)$ and $b\leq D$ the bound $D\succeq O(N^{1/3})$
    follows.
\end{enumerate}
...
equal size are related by a gauge transformation and a phase. This
distinctness is stricter than the source's assumption that the block states
$\ket{\phi_{A^j}}$ are pairwise different
\cite[lines~1329--1330]{PerezGarcia2007MPSReps}: two equal blocks with
different weights, or two blocks related by a gauge transformation and a
phase $\omega$ with $\omega^N\neq1$, give different states. Step~5 is
formalized when the first matrix of every block is invertible. The formal
...
\begin{itemize}
    ...
    words of length $n$ span the product of the block matrix algebras. The
    three canonical conditions make every block irreducible.\footnote{Formal
    declarations:
    \leanid{MPSTensor.PGVWC07CanonicalFormData.wordTupleSpanTop_of_ge},
    \leanid{MPSTensor.PGVWC07CanonicalFormData.isIrreducibleFamily_blocks}, and
    ...
    $x_1x_2g_3-(x_1y_2+x_2y_1)g_2+y_1y_2g_1=0$ gives $y_1y_2=0$. Then
    $g_1$ and $g_2$ reduce to the terms of the remaining block $k$, and $c\,y_k=y_k^2x_k^{N-1}=x_kg_2=0$, so $y_k=0$ and
    ...
    \item The conclusion:\footnote{Formal declarations:
    \leanid{MPSTensor.lt_of_sum_mpv_eq_smul_wIndicator} and
    ...
    \end{align}
    The same bound holds for $\sum_jc_j\ket{\phi_{A^j}}=c\ket{W_N}$ with
    arbitrary nonzero coefficients $c_j$ in place of $\lambda_j^N$, for blocks
    that satisfy the three conditions of the canonical form.

    \item Step~5 when every $A^j_0$ is invertible. The source proves that
    condition C1 then holds at $L_0=D^2$
    \cite[lines~2115--2118]{PerezGarcia2007MPSReps}. The formal counterpart
    is Wolf's bound $D_j^2-k+1$ for a family with an invertible
    letter \cite{Wolf2012Quantum}, which gives C1 at $L_0=D^2+1$ from C1 at
    any positive length. With $b\leq D$,
    (\ref{eq:rmp_w_state_ti_bound_conditional}) becomes
    $N<2\max\bigl(D^2+1,\,3(D-1)(D^2+2)\bigr)$, that is,
    $D=\Omega(N^{1/3})$.\footnote{Formal declaration:
    \leanid{MPSTensor.PGVWC07CanonicalFormData.lt_of_mpv_eq_smul_wIndicator_of_isUnit}.}

    \item An explicit bound in $D$ for primitive blocks, weaker than the
    source's. If the adjoint
...
\section{Formalized reduction at prime length}

The source removes the peripheral eigenvalues of modulus one by grouping the
spins into blocks or by taking $N$ prime, and obtains condition C1 from
condition C2 \cite[lines~2090--2097]{PerezGarcia2007MPSReps}. The second
device is formalized, with Wolf's general Wielandt bound in place of
Conjecture~2. Let $A$ be an arbitrary tensor of bond dimension $D$, let $N$
be prime, and let the periodic vector of $A$ on $N$ sites be $c\ket{W_N}$
with $c\neq0$. Then
\begin{align}
    N
        &<
        2\max\bigl((D^2+1)^2,\,3(D-1)((D^2+1)^2+1)\bigr),
    \label{eq:rmp_w_state_ti_bound_prime}
\end{align}
that is, $D=\Omega(N^{1/5})$ along the prime lengths.\footnote{Formal
declarations: \leanid{MPSTensor.lt_of_mpv_eq_smul_wIndicator_of_prime} and
\leanid{MPSTensor.IsPeriodic.period_le_sq}.} The proof runs as follows.
\begin{enumerate}
    \item For $N\leq D^2$ there is nothing to prove. Otherwise write the
    periodic vector as $\sum_j\lambda_j^N\ket{\phi_{A^j}}$ with unital blocks
    $A^j$ that have a positive definite fixed point of the dual map and only
    scalar fixed points (the canonical form of
    \cite[lines~742--763]{PerezGarcia2007MPSReps}, with at most $D$ blocks in
    total dimension at most $D$). These conditions make every block
    irreducible.

    \item Rescaled and gauged to trace-preserving form, each block is periodic
    with a period $m_j$. The $m_j$ peripheral eigenvalues are distinct
    eigenvalues of an endomorphism of $\mathcal{M}_{D_j}$, so
    $m_j\leq D_j^2\leq D^2<N$.

    \item If $m_j\neq1$, then $m_j$ does not divide the prime $N$, and the block
    state vanishes at length $N$ by Theorem~5 of
    \cite[lines~849--858]{PerezGarcia2007MPSReps}.

    \item If $m_j=1$, the block is primitive, and the general quantum Wielandt
    bound $(D_j^2-k+1)D_j^2\leq(D^2+1)^2$
    \cite{Sanz2010Quantum,Wolf2012Quantum} gives condition C1 at
    $L_0=(D^2+1)^2$.

    \item Group the blocks whose states agree up to a phase power. The class
    representatives are pairwise not related by a gauge transformation and a
    phase, and the vector becomes $\sum_jc_j\ket{\phi_{A^j}}$ over the
    representatives. Dropping the classes with $c_j=0$ or with period other
    than one leaves at most $D$ blocks, and the coefficient form of
    (\ref{eq:rmp_w_state_ti_bound_conditional}) gives
    (\ref{eq:rmp_w_state_ti_bound_prime}).
\end{enumerate}
Step~5 also supplies, at prime length, the distinctness used by the
direct-sum lemma: the grouping absorbs the blocks whose states differ only by
a weight or a phase power, which is what separates the source's pairwise
different block states \cite[lines~1329--1330]{PerezGarcia2007MPSReps} from
...
The bound (\ref{eq:rmp_w_state_ti_bound_conditional}) is conditional, and
the unconditional bound (\ref{eq:rmp_w_state_ti_bound_prime}) holds at prime
lengths only. Two inputs are missing for
(\ref{eq:rmp_w_state_ti_bound_asymptotic_lower_bound}) and for the source's
$D\succeq O(N^{1/3})$.
\begin{itemize}
    \item Composite lengths. The reduction to a canonical form whose blocks
    satisfy condition C1 at a controlled length, and to blocks that are
    pairwise distinct in the sense of the direct-sum lemma, is formalized for
    prime $N$. For composite $N$ the source's other device, grouping the spins
    into blocks of the least common multiple of the periods
    \cite[lines~2094--2096]{PerezGarcia2007MPSReps}, is not formalized; it
    changes the length to $N/p$ and needs the periods to divide $N$.

    \item The explicit Wielandt index $L_0=O(D^2)$ (Conjecture~2) or
    $L_0=O(D^2\log D)$ (Micha\l{}ek and Shitov) for blocks without an
    invertible matrix. Wolf's general bound $(D^2-k+1)D^2$ is formalized,
    which yields $D=\Omega(N^{1/5})$; the bound $D^2$ is formalized only when
    every $A^j_0$ is invertible.
\end{itemize}
...
corrected. When every $A^j_0$ is invertible it gives the source's
$D=\Omega(N^{1/3})$, and with primitive blocks it gives
$D=\Omega(N^{1/5})$. For an arbitrary tensor at prime length the reduction is
formalized, which gives the unconditional
(\ref{eq:rmp_w_state_ti_bound_prime}), $D=\Omega(N^{1/5})$ along the primes.
The review's bound
(\ref{eq:rmp_w_state_ti_bound_asymptotic_lower_bound}) and the source's
unconditional $D\succeq O(N^{1/3})$ remain unformalized: they need the
explicit $O(D^2)$ or $O(D^2\log D)$ Wielandt index, and composite lengths
need the reduction by blocking.
