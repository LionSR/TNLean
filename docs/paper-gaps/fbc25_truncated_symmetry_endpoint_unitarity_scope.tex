...
\end{align}
Here \(\sigma,\tau\) are words of bulk length \(N\). The source superscript
is the total length \(L=N+2\geq2\), including the two endpoint triangles. The
...
(\ref{eq:fbc25_truncated_symmetry_formal_coisometry}) by induction on the
bulk length \(N\). The base case \(N=0\) has total length two:
\(U^2=u\) after removing the empty-word coordinates.
The rank product in
