...
- `cpgsv21_block_diagonal_parent_ground_space.tex` records the degenerate
  ...
  PGVWC07 \(C^j,D^j\) comparison is now formalized under cut-tail word-span
  hypotheses of length \(N-i\) for crossing intervals; the remaining boundary is
  the source-range block-diagonal boundary representation and discharge of those
  span hypotheses.
