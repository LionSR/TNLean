...
The correction is to restore the tilde in that textual equation. A second omission of the same kind occurs in the displayed definition of the comparison $K$ itself, where the weight of the chosen normalization is dropped; that omission and its repair are recorded in the next section. The placement of the gauge on its virtual leg must also be respected when translating the diagram into matrices; the formula below fixes that index order explicitly.
...
The original factors are the normalized source factors constructed using this same weight $\rho$, so $\widetilde X_1^\dagger(I_d\otimes\rho)\widetilde X_1=I_r$; this is the normalization chosen when the factors are introduced
\cite[lines~487--494]{Cirac2017MPU}. The second omission of the argument is in the comparison itself: the source defines it without that weight, as
$K=\widetilde X_1^\dagger X_1=\widetilde Y_1Y_1^\dagger$
\cite[lines~5436--5439]{FrancoRubio2025MPUGauging}. Under the chosen normalization the unweighted $\widetilde X_1^\dagger$ is not a left inverse of $\widetilde X_1$, so the printed expression is not in general a left-inverse comparison and does not satisfy $X_1=\widetilde X_1K$. Restoring the weight repairs this, and gives the comparison used throughout:
\begin{align}
...
\section{An explicit check on the displayed CZX factors}

The explicit factors displayed for the CZX tensor in
Ref.~\cite[lines~4559--4659]{FrancoRubio2025MPUGauging} make the role of the weight visible in coordinates. There $d=4$, $D=2$, the maintained gauge is the Pauli matrix $Y$, and the recorded weight is $\rho=I_2/2$. The displayed factors are the original ones, written $\widetilde X_1,\widetilde Y_1$ as in §2; the candidates of
(\ref{eq:fbc25_inverse_tilde_candidates}) built from them are
$X_1=(I_4\otimes\overline Y)Y_2^\dagger$ and
$Y_1=X_2^\dagger(Y^{\mathsf T}\otimes I_4)$.
Both candidates are the negatives of the displayed factors, and the three overlaps of
Ref.~\cite[lines~5438--5441]{FrancoRubio2025MPUGauging} take the values
\begin{align}
    \widetilde X_1^\dagger X_1                 & =-2I_4,
    \label{eq:fbc25_inverse_tilde_czx_raw}               \\
    \widetilde X_1^\dagger(I_4\otimes\rho)X_1  & =-I_4,
    \label{eq:fbc25_inverse_tilde_czx_weighted}          \\
    \widetilde Y_1Y_1^\dagger                  & =-I_4.
    \label{eq:fbc25_inverse_tilde_czx_right}
\end{align}
The unweighted left overlap (\ref{eq:fbc25_inverse_tilde_czx_raw}) has Gram matrix $4I_4$, so it is not unitary, and it differs from the right overlap
(\ref{eq:fbc25_inverse_tilde_czx_right}). The weighted comparison
(\ref{eq:fbc25_inverse_tilde_czx_weighted}), which is the $K$ of
(\ref{eq:fbc25_inverse_tilde_weighted_comparison}), is unitary and recovers both displayed factors as $\widetilde X_1=X_1K^\dagger$ and $\widetilde Y_1=KY_1$, the second being
(\ref{eq:fbc25_inverse_tilde_corrected_factor}) itself.
This is a calculation on the explicit displayed witness rather than on choice-selected factors, so it diagnoses the intermediate formula on those factors; it is not a counterexample to the proposition proved in
Ref.~\cite[lines~5350--5487]{FrancoRubio2025MPUGauging}, and it changes no gauge phase.\footnote{The three overlaps, the failure of unitarity of the unweighted one, and the weighted recovery are recorded in
    \leanid{MPOTensor.CZX.displayedComparison_coordinates},
    \leanid{MPOTensor.CZX.displayedRawComparison_not_isUnitaryBetween}, and
    \leanid{MPOTensor.CZX.displayedWeightedComparison_spec}, in
    \doclink{TNLean/MPS/MPDO/CZXComparisonNormalization}.}

\section{Conclusion}

These are local corrections of the factor's name and of the omitted weight, not a new hypothesis or a change of gauge. The first is justified by the already distinguished factors and by the explicit tilde in the proof diagram; the second by the source's own normalization of the first factor, which the unweighted comparison does not invert. The elimination criterion for both is that the factor substituted into the old gate be
