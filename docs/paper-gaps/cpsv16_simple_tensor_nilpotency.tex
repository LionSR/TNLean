...
source-facing predicate is \leanid{MPOTensor.IsSourceSimple}: it
existentially quantifies over the positive physical blocking required at
line~815, chooses a basis of normal tensors for the blocked doubled-index
tensor, and records this non-nilpotency condition for every basis element. The
...
Verdict: the formal source predicate matches lines~815--822 under the
identification of the line-819 partial-trace criterion with matrix nilpotency of
the physical-trace transfer. Positive-length nonvanishing is available only in
the separate strengthened project predicate and is not needed for this source
reading.
