...
closed MPO to be nonzero at some positive length, existentially quantifies over
the positive physical blocking required at line~815, chooses a basis of normal
tensors for the blocked doubled-index tensor, and records this non-nilpotency
condition for every basis element. The existential nontriviality clause reflects
the standing active canonical-block convention at lines~217--246 and excludes
the identically zero family. It permits isolated vanishing lengths and does not
in general exclude dormant extra candidates in an arbitrary BNT presentation.
The strengthened project predicate
...
the physical-trace transfer. It requires existential positive-length
nontriviality but not nonvanishing at every positive length. The latter is
available only in the separate strengthened project predicate.
