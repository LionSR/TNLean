...
\begin{enumerate}
    ...
    and for the contracted network on the torus: the virtual string that
    multiplies $\pi(g)$ for each bond with label $g$ it crosses can be
    pulled through every rectangle of sites, since on the support
    $\pi(t)\pi(r)=\pi(l)\pi(b)$ at each site and the two boundary paths
    of a grid of such squares have the same product. This item is
    resolved.
...
The note is a scope restriction: of the review's two statements about the
contracted network, the second is not formalized. The
...
conventions and need no further work. The pulling-through
identity of the representation operators across the contracted primal
network is formalized on the torus. For the remaining scope restriction,
place the dual tensor on the torus and state the coloring superposition
as the trivial-holonomy sector of the Gauss-law configurations.
