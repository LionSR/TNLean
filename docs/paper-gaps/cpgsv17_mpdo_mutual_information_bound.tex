...
\[
  ...
  \leq r.
\]
A direct endpoint comparison gives
\[
  D(\rho\Vert\omega)\leq\log Q_2(\rho,\omega)
\]
for trace-one $\rho,\omega\geq0$ with
$\ker\omega\subseteq\ker\rho$.  For a faithful reference, set
$R=\sqrt\rho$, $T=\omega^{-1/2}$,
$\Delta=R^T\otimes T$, and $v=\operatorname{vec}(R)$.  Under column-stacking,
\[
  (B\otimes A)\operatorname{vec}(X)=\operatorname{vec}(AXB^T),
\]
so the factor order in $\Delta$ gives
$\Delta v=\operatorname{vec}(T\rho)$ and
\[
  m=\operatorname{Re}\langle v,\Delta v\rangle
   =\langle R,RTR\rangle_{\mathrm{HS}}.
\]
The support projection of $\Delta$ fixes $v$.  The support projectors in
\[
  \log(A\otimes B)
  =(\log A)\otimes P_{\operatorname{supp}B}
   +P_{\operatorname{supp}A}\otimes(\log B)
\]
are essential when an eigenvalue vanishes: with the convention $\log0=0$, the
identity with ambient identities in their place is false.  Together with
$\log\sqrt\rho=\frac12\log\rho$,
$\log(\omega^{-1/2})=-\frac12\log\omega$, and transpose covariance, this gives
\[
  \operatorname{Re}\langle v,(\log\Delta)v\rangle
  =\frac12D(\rho\Vert\omega).
\]
Support-aware Jensen yields $\frac12D(\rho\Vert\omega)\leq\log m$, while
Hilbert--Schmidt Cauchy--Schwarz gives
\[
  m^2\leq\lVert R\rVert_2^2\lVert RTR\rVert_2^2
  =Q_2(\rho,\omega).
\]
The auxiliary divergence and vector-state Jensen mechanism are adapted from
\cite[arXiv:1306.3142v4, Definition~5 and Lemma~19, used in the proof of
  Theorem~7]{MullerLennert2013Renyi}.  Only the direct order-two endpoint is
used here, followed by the endpoint Cauchy--Schwarz estimate; the shortened
proof is not Beigi's interpolation argument.  Compression to the support of
$\omega$ proves the singular-reference statement.  Applying it to
$\omega=\rho_A\otimes\rho_B$ therefore gives
\[
  I(A:B)_\rho
  =D(\rho_{AB}\Vert\rho_A\otimes\rho_B)
  \leq\log\operatorname{Tr}(W^2)
  \leq\log r.
\]
...
\[
  ...
  \qquad A\geq0,\quad B>0,\quad\tr A=1.
\]
This faithful comparison is now proved; no normalization of $B$ is required.
...
application to the finite-chain MPO estimate is formalized by
...
\doclink{TNLean/MPS/MPDO/MutualInfoAreaLaw}.  Thus the finite-chain work
tracked by \href{https://github.com/LionSR/TNLean/issues/5213}{\#5213} is
complete.  This does not formalize the false thermodynamic-limit clause of
Proposition~4.5 or alter its separate negative status.
...
in \doclink{TNLean/Algebra/MatrixRankBaseChange}.  The resulting
...
The thermodynamic-limit clause of Proposition~4.5 is formally refuted at its
stated hypothesis set by
\leanid{MPOTensor.ThermodynamicLimitCounterexample.proposition45_limit_counterexample}.
The finite-chain monotonicity and bond-dimension bounds remain valid separate
results.  Primitivity, aperiodicity, or another convergence condition would
exclude the parity obstruction, but such a condition is a genuine boundary of
a corrected theorem and is not a hypothesis of the printed proposition.  This
resolves the false-source limit clause tracked by
