...
\end{align}
\footnote{Source traceability: line 361 for
    (\ref{eq:glm23_fusion}); lines 458--460 for the multiplicities; lines
    491--492 for associativity; lines 564--565 for
    (\ref{eq:glm23_nimrep}); line 683 for the group case
    $M_{gh,x}^y=\sum_z M_{g,z}^y M_{h,x}^z$.}
Evaluating the invariance with periodic boundaries, the source obtains
\begin{align}
    O_a\lvert\psi_{A_x}\rangle & =\sum_y M_{a,x}^y\lvert\psi_{A_y}\rangle
    \label{eq:glm23_periodic_symmetry}
\end{align}
for general fusion rules, and recalls this form when it constructs boundary
states.\footnote{Source traceability: lines 567--568 and line 1801.}
...
(\ref{eq:glm23_periodic_symmetry}) at every positive length, with complex
coefficients independent of the length. The source derives
(\ref{eq:glm23_periodic_symmetry}) from its definition, so the formal
...
tensors instead.\footnote{Formal definitions:
    ...
    \leanid{MPOTensor.exists_equiv_mpo_mulVec_eq_of_isInvertibleLabel}.}
...
S_A\subset\mathcal S_A$; the exact fusion and action tensors that the source
obtains from the closedness condition; the $F$- and $L$-symbols and the
pentagon equations; and the gauge of the fusion tensors by invertible matrices
on the multiplicity spaces (line 416). Formalized instead are fusion and action
tensors for periodic boundaries, derived from the fusion rules and from
(\ref{eq:glm23_periodic_symmetry}) through the multi-block compression theorem.
They carry one block per copy of each channel, are mutually biorthogonal, and
decompose the stacked tensor up to a nilpotent remainder.\footnote{Results:
    \leanid{MPOTensor.exists_fusionTensors_of_mpo_mul_eq_sum} and
    \leanid{MPOTensor.exists_actionTensors_of_mpo_mulVec_eq_sum}.} With only a
nilpotent remainder the gauge is not a multiplicity-space transformation.
Conjugating the block coordinates by a unipotent block upper triangular matrix
adds to one copy's fusion tensor a multiple of another's. Uniqueness up to a
scalar is formalized only when the whole stacked tensor reduces onto a single
block.\footnote{Result:
    \leanid{MPOTensor.exists_boundary_dressed_proportional_of_mpo_mul_eq}.} Consequently the single-block
...
multiplicity, $M_{a,x}^x=1$,\footnote{Source traceability: lines 610--636 and line 738.}
is not formalized. What is formalized is a different, fusion-ring
obstruction: a single normal symmetric block on
