...
\section{Scope restriction: weights, length, representation, and normalization}
...
\textbf{Restricted theorem.} The declaration
\path{PeriodicOverlapHypothesis.ofIsIrreducibleForm} treats an irreducible-form
representation with one positive real scalar weight attached to each block and
assumes that distinct listed blocks are not repeated. From nonzero
proportionality at every positive length, with a scalar allowed to depend on the
length, it derives a non-decaying mixed-overlap partner for every block in both
families. The proof makes the paper's terse contradiction at line 631 explicit:
it partitions one family by mixed-overlap decay, passes to a common-period
subsequence, and compares coefficients in an eventually linearly independent
coproduct-indexed family.

The local representation also differs from the source irreducible form in
three further respects. First, its periodic blocks are left-canonical, whereas the
source assumes only that their transfer maps are irreducible and have spectral
radius one. This is an additional normalization hypothesis. Second, its
defining field asserts equality of matrix-product-vector families with the
scalar-weight direct-sum tensor; it does not assert that the original tensor
is literally equal to that tensor. The restricted theorem consequently
concerns the periodic families carried by these witnesses, not a literal
block-diagonal presentation of the original tensors.

Third, the local equality of matrix-product-vector families includes the empty
word and therefore forces equality of the associated bond-dimension traces.
The source compares physical states at positive lengths and explicitly allows
the irreducible-form representative to have smaller bond dimension
\cite[lines~266--271]{DeLasCuevas2017Irreducible}. Thus the local witness has an
additional empty-word restriction absent from the source theorem.
...
They are conditional components toward \emph{thm:bd}, not formalizations of
the source theorem itself.

This is a mathematically correct restriction, but it is not the full statement
of \emph{thm:bd}: replacing $R_j$ by a single scalar removes the grouped
repeated-copy data. Consequently the restricted theorem must not be used to mark
the unrestricted proportional Fundamental Theorem as complete.

\textbf{Elimination plan.} Formalize a multiplicity-bearing periodic basis, or
prove a reduction from the matrices $R_j,S_k$ to grouped power-sum coefficients
without treating repeated copies as linearly independent. Replace the
MPV-equivalent witness by a literal block-diagonal presentation, or prove a
transport theorem between the two formulations. Replace the empty-word
equality by a positive-length comparison so that the representative may have
the source-allowed smaller bond dimension. Formalize the source's
block-diagonal similarity to trace-preserving form and transport the overlap
and repeated-block conclusions back, thereby removing the extra normalization
hypothesis. Then repeat the coefficient comparison at the grouped level and
derive the scalar-weight theorem as a corollary. The mixed-overlap step remains
tracked by
