...
$\widehat M=(1/2)M$ is a separate task. The full GSNNCH exclusion is already a
statement about the normalized family because \leanid{MPOTensor.IsGSNNCH}
quantifies over \leanid{MPOTensor.normalizedMPO}. It may therefore be proved as
$\neg\operatorname{IsGSNNCH}(M)$ for the printed tensor and remains tracked in
\ghissue{6045}. Transport to a separately named $\widehat M$ is optional
...
open in \ghissue{6044}; no RFP verdict for $\widehat M$ is claimed here. Any
formal treatment of $\widehat M$ must use tensor-level scalar rescaling and
\leanid{MPOTensor.mpo_smul} to identify its periodic MPO family with the
normalization of the printed family, rather than introducing an unrelated
second tensor definition.
