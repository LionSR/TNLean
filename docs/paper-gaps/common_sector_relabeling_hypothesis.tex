...
physically blocked blocks agrees, as an MPV family, with the same family assembled from the
explicitly reindexed blocks of \(F\):
\[
  \forall F,\quad
  \bigoplus_k \mu_k^p \cdot A_k^{[p]} \sim_{\mathrm{MPV}}
  \bigoplus_k \mu_k^p \cdot F.\text{commonReindexedBlock}(k).
\]
Here \(A_k^{[p]}\) denotes the length-\(p\) blocking of the \(k\)-th primitive block, and
\(F.\text{commonReindexedBlock}(k)\) is the same tensor read through the explicit word
relabeling of blocked physical indices.
