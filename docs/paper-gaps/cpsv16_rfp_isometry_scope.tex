...
hypotheses.  If $A$ has positive bond dimension and is normal in the sense of
CPSV16, then
\begin{align}
...
Perron representative.  Gauge invariance then proves that every
positive-bond-dimension normal tensor is a renormalization fixed point if and
only if it admits the displayed square-root form, with no left-canonical
...
is explicit.  For every positive-bond-dimension normal tensor, transfer
idempotence is now equivalent to the trace-normalized square-root form by
\leanid{MPSTensor.IsNormalTensor.isTransferIdempotent_iff_isIsometryCanonicalForm}.
