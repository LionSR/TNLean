...
\]
formalized by \leanid{MPSTensor.mixedMapLM_conj_apply}.  Cancelling the
...
Corollary~3.12 applies the joint residual-isometry conclusion to a basis of
normal tensors of a canonical-form renormalization fixed point. Under the
nonzero-coefficient convention of
\path{docs/paper-gaps/cpsv16_bnt_uniqueness_zero_coefficient.tex}, every
basis element is gauge-phase equivalent to a block of the canonical form,
so the statement is one about the phase-class representatives. Write
\begin{align}
  A^i=U^\dagger\!\left(\bigoplus_k\mu_kA_k^i\right)U,
  \qquad UU^\dagger=I,
\end{align}
with every $A_k$ normal and every $\mu_k$ nonzero, and choose one
representative $C_j$ from each phase class. If $A$ has idempotent transfer
map, then there are invertible matrices $X_j$, positive numbers
...
direct-sum hypothesis, and Corollary~3.12 is formalized up to the square-root
diagonal correction of this note.
...
$\mu_{j,q}$ satisfying $|\mu_{j,q}|=1$.  Read as an ordinary virtual block
diagonal at each fixed physical letter, one physical letter and the scalar
representative $B^0=(1)$ with copy phases $1$ and $-1$ would give
$A^0=\operatorname{diag}(1,-1)$, a tensor satisfying neither \path{AA=A} nor
whole-tensor transfer idempotence.

That reading is not the source's.  The prose immediately after the display
states that the indices $j,q$ connect the physical and virtual indices and
therefore give a direct sum in \emph{both} spaces, up to the physical isometry
(local source \path{Papers/1606.00608/MPDO-22-12-17-2.tex:559--563}).  A single
physical letter cannot carry a nontrivial physical direct sum, so
$\operatorname{diag}(1,-1)$ is outside the class the theorem describes and is
not evidence against it.  The development records no counterexample here.

What the source does leave open is the coordinate data for the physical routing
of $q$.  The displayed isometry equation at lines 551--554 contains $j$ but no
...
for such an isometry.  In particular, a formal construction would also require a
...
\[
  \mathcal E_{\mu B,\nu C}
    = \mu\overline\nu\,\mathcal E_{B,C}.
\]
It is formalized by \leanid{Kraus.mixedMapLM_smul}.  Combining it
...
For CPSV canonical-form data, every listed block has unit-modulus weight and
is itself a renormalization fixed point. After choosing one representative
from each phase class, the trace-normalized square-root forms can be chosen
