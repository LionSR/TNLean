...
The structural datum in Appendix~\cpsvref{AppendixPure} used in the retained
commutation proof adopts the unit pair-index convention. Its cyclic coefficient
formula therefore uses the source normalization. The same appendix of
...
\end{align}
where $\varphi_j$ is shared between $b_t$ and $a_{t+1}$. The rejected even-chain
physical-pair factorization construction and its conditional parent-Hamiltonian
theorems have been retired without replacement. They did not express this
source identity; the proved commutation theorem uses adjacent commutation
directly.
...
for an individual physical word $\sigma$. The source basic-vector expression
in Eq.~\ref{eq:cpsv16_basic_vector_expression} gives the cyclic contraction
$C_L(\sigma)$, not a product over disjoint physical pairs. The following
example distinguishes these two coefficient expressions.
...
\section{Corollary~\cpsvref{III_cor3}: phase-class representatives}
