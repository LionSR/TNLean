...
$K$ is unitary. The formal development first exhibited only a separately
constructed two-sided inverse, which made its statement a restriction of the
printed one and fixed the classification of this note; that restriction has
since been removed, as recorded in the verdict.
...
as constructed. For a simple canonical-form-II tensor whose physical adjoint is
carried by a unitary gauge with $T\overline T=\sigma I_D$, both $K$ unitary and
$J=K^\dagger$ are now formalized, so the pairing holds in its printed
$K^\dagger$ form. The formal statement still keeps the rectangular index types
$r$ and $\ell$ rather than transporting along the proved equality
$r=\ell$.\footnote{Formal declarations:
    ...
    \leanid{MPOTensor.inverseCompatibleComparisonK_isUnitaryBetween} and
    \leanid{MPOTensor.inverseCompatibleComparisonJ_eq_conjTranspose}
    in \doclink{TNLean/MPS/MPU/InverseCompatibleComparisonUnitarity}.}
...
factorization. It was the live content of this note, and the reason for the
scope-restriction classification: the formal development at first proved only
that $K$ has a two-sided inverse, exhibited explicitly, so its pairing was the
printed pairing restricted to that explicit inverse rather than $K^\dagger$.

That restriction is now eliminated. For a simple canonical-form-II tensor whose
physical adjoint is carried by a unitary gauge with $T\overline T=\sigma I_D$,
the weighted comparison is unitary, and substituting that unitarity into
(\ref{eq:fbc25_inverse_compatible_comparison_matrix_kj}) gives
$J=K^\dagger$. The pairing therefore holds in the printed form, on the
hypotheses the appendix itself uses, and the note is resolved. What remains is
presentational only: the formal statement keeps the rectangular index types $r$
and $\ell$ instead of transporting along the proved equality $r=\ell$.
