...
identity used in Wolf's block-positivity step in the proof of
  ...
  be relabelled with, complete positivity.  It does not by itself prove the
  Jensen inequalities; the axioms remain open until the integral step below is
  supplied.
...
\leanid{TNLean.OperatorJensen.integral_posSemidef_of_ae}: a matrix-valued
...
\leanid{CFC.concaveOn_log}.  In particular,
\leanid{CFC.convexOn_rpow} is no longer a missing scalar input; the remaining
missing theorem is the positive-map Jensen step described above.
