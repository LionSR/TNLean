...
proof, and the first concrete step toward eliminating each gap.  The three
positive-map Jensen axioms underlie \cite[Corollary~5.2]{Wolf2012Quantum} (the
second corollary of Wolf's \S5.4) and rest on the operator-convexity and
...
\leanid{IsPositiveMap} plus subunitality recorded in
\leanid{cor52_item1_rpow_of_subunital} and its companions.
...
therefore applies to a bare positive map and proves the inequalities under the
\leanid{IsPositiveMap} hypothesis as stated --- it does not require, and must not
be relabelled with, complete positivity.

What is missing is therefore not a faithful proof route but the analytic step
that closes the existing one: the Löwner-integral argument carrying the pointwise
finite-POVM resolvent inequality (\leanid{povm_resolvent_inv_le}) through the
...
\cite[Theorems~5.10--5.13 and Corollary~5.2]{Wolf2012Quantum}.  Their
elimination is blocked on the positive-map operator Jensen step (the
Choi--Davis--Jensen inequality), which is absent from Mathlib~4.31; the integral
...
concavity theorem (\cite[Theorem~5.15, the Ando--Lieb functional]{Wolf2012Quantum}),
the trace functional underlying the Wigner--Yanase--Dyson conjecture.
