...
and, through it, the operator concavity of $x\mapsto x^p$ on $[0,1]$ and of
$\log$, and the operator convexity of $x\mapsto x^p$ on the relevant range.  The
scalar input functions are therefore operator-concave (resp.\ operator-convex)
...
\begin{enumerate}
  ...
  \item prove the Choi--Davis--Jensen inequality by a positivity-only argument
    (for instance the block-matrix / unitary-dilation proof of
    Hansen--Pedersen, which uses only $2$-positivity, available for any positive
    map between matrix algebras via the $2\times 2$ amplification), keeping the
    bare \leanid{IsPositiveMap} hypothesis.
\end{enumerate}

\subsection{The concrete first step}
...
\leanid{posMap_rpow_convex_jensen}.  It is obtained from the case
$p\in[0,1]$ already in the library through the identity $x^p = x\cdot x^{p-1}$
with $p-1\in[0,1]$.  This is the smallest reusable increment toward the convex
power case.
