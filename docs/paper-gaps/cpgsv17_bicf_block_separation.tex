...
It remains useful to distinguish the selector length from the full tuple-span
length in the present proof.  Put $g$ for the number of minimal
representatives and set $L_0=D^4$.  The proved pair-separation argument has
length $3(L_0+1)$, and multiplying one pair selector for each of the other
representatives gives block selectors of length
\[
    3(g-1)(D^4+1).
\]
Since every representative has positive dimension and occurs at least once,
$g\leq D$; hence this selector length is at most the source length
$3(D^4+1)(D-1)$.  The existing conversion from selectors to the whole product
algebra, however, concatenates an additional injective prefix of length
...
$D^4+1$ in the extremal case $g=D$.  Thus the remaining mathematical point is
not quantum Wielandt and is not the construction of the block selectors.  It
is the source's direct-sum step which obtains arbitrary matrices in the chosen
block without paying a separate injective prefix.  Adding an assumed finite
separation witness would conceal precisely this point and is not an acceptable
substitute.
...
The sole remaining blocker identified here is the direct-sum step: from the
minimal BNT representatives and the block selectors of length
$3(L+1)(D-1)$, obtain arbitrary matrices in the selected block without
concatenating a further injective prefix.  This gives the stated $3D^5$ bound.
The source-level Lemma L needs only the existence of a suitable finite length,
not this sharp estimate.
