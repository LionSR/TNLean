...
The finite-length trace-separation witness cannot be recovered from blockwise
injectivity, left-canonicality, and nonzero weights alone. Take two
one-dimensional scalar blocks over a one-letter alphabet, with both block
tensors equal to $1$ and with weights $1$ and $2$. Every word evaluation is the
same on the two blocks, so the tuple span is the diagonal line in
$\mathbb C\oplus\mathbb C$ rather than the whole product algebra, and
$\Delta_1=1$, $\Delta_2=-1$ makes the trace pairing vanish at every length.

Two identical blocks are, however, related by similarity and phase, so they are
never a basis of normal tensors in the sense of the source characterization,
which the source's sharp block-injectivity proposition assumes throughout. The
observation therefore explains why the finite-length witness is carried as its
own field rather than derived from the others; it is not a counterexample to the
source, and none is recorded for it.

\section{Consequence for Lemma L}

The same field independence marks the restriction in the older theorem
