...
\section{Current status: pair-span gap for homogeneous lengths (2026-05-07)}

The formalization now separates the source inputs into three explicit layers.

\subsection{Layer 1: BNT $\to$ cumulative pair span (proved)}

The subdirect-product analysis in \path{PairHomogenization.lean} proves that
for two injective, left-canonical, non-gauge-equivalent blocks \(A,B\) of the
same dimension, the simultaneous pair algebra generated by
\(\{(A_i,B_i)\}_i\) is the full product \(M_D(\mathbb C)\times M_D(\mathbb C)\).
Dualizing gives \leanid{PairTraceSeparatingAll}, and finite-dimensional
stabilization gives \leanid{PairTraceSeparatingUpTo} at some finite cutoff
length \(S\).

The formal chain is:
\[
  \text{BNT} \;\Rightarrow\; \text{PairTraceSeparatingAll}
    \;\Rightarrow\; \exists S,\ \text{PairTraceSeparatingUpTo}\ A\ B\ S .
\]
and is stated in the equal-dimension declaration
\leanid{pair\allowbreak Trace\allowbreak Separating\allowbreak All\_\allowbreak
and\_\allowbreak exists\_\allowbreak pair\allowbreak Trace\allowbreak
Separating\allowbreak Up\allowbreak To\_\allowbreak of\_\allowbreak
injective\_\allowbreak not\_\allowbreak gauge\allowbreak Phase\allowbreak Equiv}
and the BNT-family declaration
\leanid{forall\_\allowbreak pair\allowbreak Trace\allowbreak Separating\allowbreak
All\_\allowbreak and\_\allowbreak exists\_\allowbreak pair\allowbreak
Trace\allowbreak Separating\allowbreak Up\allowbreak To\_\allowbreak
of\_\allowbreak is\allowbreak Canonical\allowbreak Form\allowbreak BNT}.

\subsection{Layer 2: Cumulative $\to$ homogeneous span (gap)}

The remaining step converts cumulative pair-word fullness to homogeneous
fixed-length fullness. The conversion theorem
\leanid{pairWordTupleSpanTop\_of\_pairCumulativeWordTupleSpanTop\_of\_identity\_padding}
requires identity padding: for every \(l\le S\), the pair identity \((1,1)\)
must belong to the homogeneous pair-word span at length \(T-l\). This
identity padding is not yet supplied by the BNT data.

The source paper (arXiv:1606.00608, lines~317--345) supplies the homogeneous
conclusion through a different route (the \(3D^5\) block-injective bound
via Wielandt and the P\'erez-Garc\'ia--Verstraete--Wolf--Cirac result). The
formalized subdirect-product route isolates the identity-padding input as the
remaining step.

\subsection{Layer 3: Conditional downstream theorems}

The following theorems are already proved conditional on the identity-padding
period-window hypothesis:

\begin{itemize}
  \item \leanid{exists\_pairTraceSeparatingAt\_of\_not\_gaugePhaseEquiv\_of\_pairWordTupleSpanTop\_period\_window}
    (single pair, same dimension, takes
    \leanid{PairWordTupleSpanTop} period window)
  \item \leanid{exists\_forall\_pairTraceSeparatingAt\_of\_isCanonicalFormBNT\_of\_identity\_period\_windows}
    (BNT family, takes identity-padding period windows for each ordered pair)
  \item \leanid{exists\_pos\_productWordSpan\_of\_isCanonicalFormBNT\_of\_identity\_period\_windows}
    (BNT product-word span, same hypotheses)
\end{itemize}

The statement
\leanid{exists\_pairTraceSeparatingAt\_of\_pairTraceSeparatingUpTo\_of\_eventual\_identity\_padding}
has hypotheses \( \mathrm{PairTraceSeparatingUpTo}(A,B,S) \) and eventual
identity padding; it concludes \( \exists T,\mathrm{PairTraceSeparatingAt}(A,B,T) \).

\subsection{Next step}

The missing identity-padding input has the shape:
\begin{quote}
For each ordered pair of distinct BNT blocks \((A_k, A_j)\), there exist
\(\text{start}, \text{period}\in\mathbb N\) with \(\text{period}>0\) such
that \((1,1)\) belongs to the homogeneous pair-word span at length
\(\text{period}\) and at every length \(\text{start}+r\) for
\(r<\text{period}\).
\end{quote}

Once this input is supplied, the conditional theorems listed above apply.  The
complete non-periodic comparison still also depends on the other BNT comparison
inputs recorded earlier in this note.  A plausible source-faithful route should
use the 2021 review's normal-tensor Wielandt bound
(arXiv:2011.12127, lines~1942--1945) together with an arithmetic period-window
construction, or else state a precise algebraic padding theorem with matching
hypotheses.
