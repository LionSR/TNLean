...
feed the resulting families into the common primitive BNT-cover hypotheses.  The
paper suppresses this bookkeeping in notation; Lean must make it explicit.

The key Lean statements involved in this boundary are the conditional
\leanid{BlockedNormalFormHypotheses} interface, the
\leanid{fundamentalTheorem_afterBlocking_from_blockedNormalFormHypotheses}
statement, the \leanid{CommonPrimitiveBNTCoverHypotheses} cover structure, the
bounded/common injective-blocking theorems for normal-CF-BNT blocks, and the
already-injective direct-sum trace-separation theorems.  These names are listed
here only to identify the formal surface; the mathematical comparison above is
the intended classification of which source-paper inputs have become theorems
