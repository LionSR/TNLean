...
\section{Current status: Burnside--Jacobson pair-span gap (2026-05-07)}
...
\[
  \text{BNT} \;\Rightarrow\; \text{PairTraceSeparatingAll}
    ...
    \;\Rightarrow\; \text{PairCumulativeWordTupleSpanTop}\ A\ B\ S
\]
and is packaged in
\leanid{cumulative_pairSpanData_of_injective_not_gaugePhaseEquiv}
(equal dimension) and \leanid{cumulative_pairSpanData_of_isCanonicalFormBNT}
(BNT family).

\subsection{Layer 2: Cumulative $\to$ homogeneous span (gap)}

The remaining Burnside--Jacobson step converts cumulative pair-word fullness
to homogeneous fixed-length fullness. The conversion theorem
\leanid{pairWordTupleSpanTop_of_pairCumulativeWordTupleSpanTop_of_identity_padding}
...
formalization chose the algebraic subdirect-product $\to$ Burnside--Jacobson
route, which isolates the identity-padding input as the remaining step.
...
\begin{itemize}
  \item \leanid{exists_pairTraceSeparatingAt_of_not_gaugePhaseEquiv_of_pairWordTupleSpanTop_period_window}
    (single pair, same dimension, takes
    \leanid{PairWordTupleSpanTop} period window)
  \item \leanid{exists_forall_pairTraceSeparatingAt_of_isCanonicalFormBNT_of_identity_period_windows}
    (BNT family, takes identity-padding period windows for each ordered pair)
  \item \leanid{exists_pos_productWordSpan_of_isCanonicalFormBNT_of_identity_period_windows}
    (BNT product-word span, same hypotheses)
\end{itemize}

The gap connector theorems
\leanid{pairTraceSeparatingAt_of_cumulative_bnt_pairSpan_of_identity_padding}
and \leanid{gap_exists_forall_pairTraceSeparatingAt_of_isCanonicalFormBNT}
make the Burnside--Jacobson hypothesis explicit so that the downstream
dependency is visible in the theorem statements.

\subsection{Next step}

The missing Burnside--Jacobson input has the shape:
\begin{quote}
...
Once this input is supplied, all downstream theorems close automatically.
The input is expected to follow either from the 2021 review's normal-tensor
Wielandt bound (arXiv:2011.12127, lines~1942--1945) together with an
arithmetic period-window construction, or from a direct Burnside--Jacobson
argument applied to the pair product algebra after the subdirect-product
analysis.
