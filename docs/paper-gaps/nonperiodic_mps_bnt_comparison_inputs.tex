...
\section{Current status as of 2026-05-10}

This section updates the 2026-05-07 pair-span note against the current
\path{origin/main} snapshot used for the audit.  The short version is that the
formalization has closed the already-injective BNT direct-sum route, while the
after-blocking canonical-form pipeline still has a smaller number of explicit
transport and coefficient-comparison boundaries.

\begin{center}
\begin{tabular}{p{0.22\linewidth}p{0.36\linewidth}p{0.34\linewidth}}
\hline
Paper input or formal surface & Current Lean status & Extra-assumption verdict \\
\hline
Identity padding and pair-span period windows for injective BNT representatives
& Discharged on main for the already-injective
  \leanid{IsCanonicalFormBNT} surface by the direct-sum injective-block route
  of \ghpr{1534} and \ghpr{1535}.  The relevant declarations include
  \path{forall_pairTraceSeparatingAt_of_isCanonicalFormBNT_of_directSum_injectiveBlocks},
  \path{exists_forall_pairSpanTop_period_window_of_isCanonicalFormBNT_of_directSum_injectiveBlocks},
  \path{exists_pos_productWordSpan_of_isCanonicalFormBNT_of_directSum_injectiveBlocks}, and
  \path{exists_wordTupleSpanTop_of_isCanonicalFormBNT_of_directSum_pairSpanTop_period_windows}.
& No longer a genuine source gap for an already-injective canonical BNT family.
  It remains an interface boundary only when callers have not yet produced the
  direct-sum injective-block hypotheses for the final reblocked family. \\
\hline
Bounded injective blocking and uniform common injective blocking
& Discharged on main by \ghpr{1545} and \ghpr{1550}.  The bounded single-block
  surface is
  \path{exists_pos_blockTensor_isInjective_le_pow_four_of_isNormal_leftCanonical};
  the finite-family surfaces are
  \path{IsNormalCanonicalFormBNT.exists_common_blockTensor_isInjective} and
  \path{exists_common_blockTensor_isInjective_two_of_isNormalCanonicalFormBNT}.
& Not an additional paper hypothesis at the normal-left-canonical or
  normal-CF-BNT level.  The remaining work is to use these results at the exact
  sectors and blocking length chosen by the after-blocking construction. \\
\hline
Power-limit and proportional-decomposition data
& Still explicit in the proportional comparison surface, especially through
  \path{ProportionalDecompositionData} and the separated-data comparison
  theorems.  The data record coefficient limits and nonzero-limit information
  used to separate BNT representatives and compare the power sums of weights.
& A real theorem-surface restriction.  It is not derivable from
  \leanid{IsNormalCanonicalFormBNT} alone; it represents the still-unproved
  BNT coefficient-comparison part of the CPSV argument when the statement is
  not specialized to already supplied decomposition data. \\
\hline
Blocked not-GPE and reblocking transport into
\path{CommonPrimitiveBNTCoverHypotheses}
& The mathematical injective-blocking input is present, but current main has
  not yet plugged the blocked not-gauge-phase-equivalence data, BNT separation,
  and nested-to-flat reblocking transports into the common primitive BNT-cover
  constructor.  \ghpr{1551} was open with changes requested at the time of this
  audit.
& Pending/in flight, not merged.  This is the main remaining after-blocking
  transport boundary and should not be described as closed on main. \\
\hline
Zero-tail equality and common-sector relabeling
& Common-sector relabeling is a coordinate-packaging issue handled by the
  grouped-block cast and common-sector transport route.  Zero-tail equality is
  a length-zero artifact of $\mathrm{SameMPV}_2$; it is partly
  discharged by the zero-tail-identity constructors and by results such as
  \path{zeroTail_eq_of_proportionalDecompositionConclusion} and
  \path{sameMPV₂Pos_and_zeroTail_identity_of_sameMPV₂}.
& Packaging rather than a paper hypothesis.  The source works at positive
  lengths and treats zero blocks as unused dimension; Lean must still transport
  the length-zero identity through the chosen common-sector coordinates. \\
\hline
\end{tabular}
\end{center}

The key formal surfaces for the two source papers are as follows.

\begin{itemize}
  \item \textbf{CPSV16 proportional and equal-MPV theorem surfaces.}
  The strict already-grouped statements are
  \path{fundamentalTheorem_proportionalMPV_CFBNT},
  \path{fundamentalTheorem_proportionalMPV_normalCFBNT},
  \path{fundamentalTheorem_equalMPV_CFBNT},
  \path{fundamentalTheorem_equalMPV_full}, and
  \path{fundamentalTheorem_equalMPV_CFBNT_hetero}.  Their common comparison
  cores include
  \path{fundamentalTheorem_of_separated_CFBNT_data},
  \path{fundamentalTheorem_of_separated_normalCFBNT_data}, and
  \path{exists_eq_numBlocks_and_equiv_gaugePhase_of_proportional_decomp_of_irreducible_TP}.

  \item \textbf{CPSV16 after-blocking surfaces.}
  The conditional pipeline is represented by
  \path{BlockedNormalFormHypotheses},
  \path{BlockedNormalFormSectorMatching},
  \path{fundamentalTheorem_afterBlocking_from_blockedNormalFormHypotheses}, and
  \path{CommonPrimitiveBNTCoverHypotheses}.  These are packaging surfaces for
  the final common primitive nonzero sectors, not extra hypotheses in the paper
  statement itself.

  \item \textbf{PGVWC07 injective-chain Fundamental Theorem surfaces.}
  The already formalized injective-block route includes
  \path{fundamentalTheorem_singleBlock},
  \path{sameMPV_iff_gaugeEquiv_of_injective},
  \path{MPSChainTensor.fundamentalTheorem_injective_chain},
  \path{MPSChainTensor.SameStateBridgeHyp},
  \path{fundamentalTheorem_injective_chain_of_sameState}, and
  \path{fundamentalTheorem_blockedChain}.  The full multi-block
  translationally invariant uniqueness theorem of PGVWC07 remains outside this
  closed single-block/injective-chain surface.

  \item \textbf{PGVWC07 direct-sum input surfaces.}
  The exact-length algebraic input is represented by
  \path{Matrix.span_range_mul_nonzero_mul_eq_top},
  \path{MPSTensor.span_range_evalWord_mul_nonzero_mul_evalWord_eq_top}, the
  pair-trace separation wrappers in
  \path{TNLean/MPS/CanonicalForm/BlockDiagonalCommutant.lean}, and the
  injective-block direct-sum BNT declarations listed in the first row of the
  table.  This is the formal version of the fixed-length direct-sum span used
  by CPSV16's block-injective canonical-form citation.
\end{itemize}
