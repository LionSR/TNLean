...
Explicit decomposition coefficients for a conditional proportional comparison
are recorded separately.\footnote{\leanid{ProportionalDecompositionData},
same file.}
The corresponding conditional proportional comparison theorems are therefore
strict-representative special cases, not the general BNT theorem stated in
the source paper.\footnote{\leanid{fundamentalTheorem_proportionalMPV_CFBNT}
and \leanid{fundamentalTheorem_proportionalMPV_normalCFBNT},
\path{TNLean/MPS/FundamentalTheorem/EqualProportional.lean}.}

The after-blocking theorem is stated in conditional form.\footnote{%
\path{TNLean/MPS/CanonicalForm/SectorComparison.lean}.}
One structure collects the comparison data for the common primitive
nonzero-sector families produced from the two original
tensors,\footnote{\leanid{BlockedNormalFormHypotheses}.}
and the conclusion states the BNT sector matching after one positive common
blocking length.\footnote{\leanid{BlockedNormalFormSectorMatching}.}

The route from produced primitive sectors to the BNT-level comparison theorem
passes through a common-primitive BNT cover
boundary.\footnote{\leanid{CommonPrimitiveBNTCoverHypotheses},
\path{TNLean/MPS/CanonicalForm/SectorComparison/ProportionalComparison.lean}.}
That boundary is paper-facing only when its hypotheses are supplied for the
selected representative BNT families rather than for an intermediate
cyclic-sector list.
...
The comparison with CPSV16 should be read by separating the mathematical layers
of the paper proof, rather than by following implementation history.  The present formalization has closed the exact-length
span input for the already-injective BNT surface, and it has also formalized the
Wielandt blocking input needed to make normal blocks injective after a further
blocking.  What remains is the transport of the final reblocked BNT cover and
the coefficient comparison that CPSV16 carries out by power sums.

For an already-injective canonical BNT family, the identity-padding and pairwise
trace-separation input is no longer a separate paper assumption.  The formal
result specializes the three-block direct-sum argument to the case in which the
selected BNT blocks are one-site injective: one-site injectivity supplies the
fixed length-two and length-six span inputs used by the selector construction.
Thus the theorem proves the exact-length direct-sum span needed in this
specialized surface, but it should not be read as the general statement with an
arbitrary pre-existing injectivity length \(L_0\).  Passing from normal
canonical blocks to such an injective surface is a distinct Wielandt-type
blocking step.

That Wielandt step is now formalized in the canonical-form context.  A
...
length for all of its blocks.  These theorems remove the injectivity hypothesis
as a mathematical gap, but they do not by themselves identify the iterated
blocked alphabet with the direct alphabet used by the after-blocking comparison.
The remaining transport problem is to carry the BNT separation and zero-tail
bookkeeping through this refinement of the blocking length.

The proportional-decomposition coefficient limits remain a genuine theorem-surface
restriction.  CPSV16 compares the weights by the power-sum argument in the BNT
proof; the formal structure \leanid{ProportionalDecompositionData}\footnote{The
main consumers are the separated-data BNT comparison theorems and the
\leanid{CommonPrimitiveBNTCoverHypotheses} constructions.} still asks explicitly
for coefficient limits and nonzero-limit hypotheses.  This is not a harmless
coordinate convention: the block-diagonal expansion involves the powers
\((\mu_j)^N\), and nonzero weights alone do not give nonzero full-sequence
limits.  A source-faithful removal of this hypothesis therefore has to formalize
the relevant power-sum/phase comparison, or replace the present limit-based
surface by an equivalent one.
...
bond dimension.  Lean's \(N=0\) convention for \(\mathrm{SameMPV}_2\) forces an
...
these identities through the same reblocking coordinates used for the nonzero
BNT sectors.

The remaining main boundary is therefore the following.  Starting from the
normal-CF-BNT sectors produced by canonical-form reduction, one must transport
blocked not-gauge-phase equivalence, zero-tail identities, and the common
injective blocking length through the nested-to-flat reblocking map, and then
feed the resulting families into the common primitive BNT-cover hypotheses.\footnote{%
The formal interface for this boundary is the conditional
\leanid{BlockedNormalFormHypotheses} structure and the
\leanid{fundamentalTheorem_afterBlocking_from_blockedNormalFormHypotheses}
statement; the target side is the \leanid{CommonPrimitiveBNTCoverHypotheses}
cover constructor; the now-available inputs are the bounded and common
injective-blocking theorems for normal-CF-BNT blocks and the already-injective
direct-sum trace-separation theorems.}  The
