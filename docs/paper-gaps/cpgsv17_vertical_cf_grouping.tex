...
with the actual grouped sector and distinguished copy is proved once, for any
tensor with the grouped-corner Gram-dressing property, in
\path{TNLean/MPS/MPDO/GroupedSectorGram.lean}. Normalized BNT-refined
horizontal form and literal CPSV canonical form each supply that property
independently, by
\path{MPOTensor.IsHorizontalCF.hasGroupedCornerGramDressing} and
\path{MPSTensor.IsCPSVCanonicalForm.hasGroupedCornerGramDressing} in
\path{TNLean/MPS/MPDO/VerticalBNTGrouping.lean}. Neither proof identifies
either corner separately with the raw reflected adjoint.
...
\end{align}
The formal results hold for any tensor with the grouped-corner Gram-dressing
property: \path{MPOTensor.grouped_sector_gram_eq_pos_smul_one_of_dressing} in
\path{TNLean/MPS/MPDO/GroupedSectorGram.lean} and
\path{MPOTensor.grouped_sector_exists_unitary_normalization_of_dressing} in
\path{TNLean/MPS/MPDO/GroupedGramNormalization.lean}. Both the normalized
BNT-refined hypothesis and literal CPSV canonical form furnish that property,
so each gives these identities without any implication between the two
...
every grouped gauge. The normalized gauges are composed with the physical
sector isometries by
\path{MPOTensor.exists_normalized_grouped_sector_maps_of_dressing} in
\path{TNLean/MPS/MPDO/NormalizedGroupedSectors.lean}, again from the
Gram-dressing property alone, so both hypotheses reach it. In either case, the
maps $W_{\alpha,q}$ have mutually orthogonal isometric ranges,
...
\path{MPOTensor.exists_normalized_grouped_sector_maps_of_dressing} applied to
the literal Gram dressing, and
