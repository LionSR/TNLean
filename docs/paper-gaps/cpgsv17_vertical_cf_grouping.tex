...
Proposition~4.13, so the literal theorem remains open.  For the literal
hypothesis, active regrouping and the representative-grouped Lemma~L on the
retained coordinates are complete; the listed-gauge and ambient-coisometry
passage to the physical vertical tensor is not.  The proved restricted result
follows the grouped vertical sectors used in the paper and does not use the
...
Literal Proposition~4.13 nevertheless remains open.  The physical first-site
and marked-chain identities are stated for the original vertically viewed
tensor.  They must still be carried through the listed block gauge in
\eqref{eq:completed-active-regrouping} and the ambient coisometry in
\eqref{eq:completed-active-reconstruction} to obtain the corresponding
full-coordinate hypotheses for $T_{\mathrm{grp}}$.  In particular, the local
Gram-conjugation identity of source lines~1909--1919 has not been obtained from
the physical compression identity on this literal-CF surface.  The
invariant-projector conclusion, the physical Gram step, and the final
coisometric vertical assembly therefore remain open without
