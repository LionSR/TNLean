...
vertical block decomposition, finite-chain separation of every nonzero
vertical corner, and vertical phase-class grouping with positive grouped
coefficients are now complete.  Grouped Figure~8 normalization and the final
coisometry remain open.
...
\end{align}
The omitted orthogonal complement is an all-zero sector.  The phase-class theorem below partitions these $B_k$ into gauge-equivalent
representative families and proves positivity of the grouped coefficients. It
does not prove the grouped Figure~8 normalization or final coisometry of
Proposition~4.13.
...
nor vertical phase-class grouping.  It proves the separation clause at source line~1898.  Combined with the
literal normal-corner decomposition and the phase-class argument below, it supplies
the nonzero compression needed by the later trace argument.

The normalized corners are now grouped by matrix-product-vector phase class
under both hypotheses.  The common argument is
\path{MPOTensor.exists_verticalBNTGrouping_with_isometry_of_decomposition}
in \path{TNLean/MPS/MPDO/VerticalBNT.lean}.  Its literal specialization is
\path{MPSTensor.IsCPSVCanonicalForm.%
exists_verticalBNTGrouping_with_isometry} in
\path{TNLean/MPS/MPDO/CPSVVerticalBNT.lean}; the normalized BNT-refined
specialization retains the name
...
normalization, or the final coisometry.  Under the normalized BNT-refined
horizontal hypothesis, the gauge normalization is supplied below; under
literal CPSV canonical form it remains open.  Equality of
...
Under both literal CPSV canonical form and the normalized BNT-refined
horizontal hypothesis, the positive-weight step of source lines~1898--1902 is
complete.  For each grouped sector, write
...
the phase classes, their loop identities, and the positivity of every grouped
coefficient are supplied by the literal specialization above.  The pairwise
Gram-conjugation identity of the second displayed diagram at source
lines~1914--1919 is proved in
\path{TNLean/MPS/MPDO/FigureEightPairwise.lean}.  Its literal CPSV
specialization in \path{TNLean/MPS/MPDO/CPSVFigureEight.lean} applies to two
supplied corners satisfying the displayed similarity equations.  These
conditional pairwise statements do not identify those corners with copies in
the literal grouping theorem.

The dependent identification with an actual grouped sector and its
distinguished copy in \path{TNLean/MPS/MPDO/GroupedFigure8.lean} currently
uses the normalized BNT-refined horizontal grouping theorem.  Consequently,
the positive scalar Gram identity and unitary normalization in
\path{TNLean/MPS/MPDO/GroupedGramNormalization.lean}, and the normalized
physical maps in \path{TNLean/MPS/MPDO/NormalizedGroupedSectors.lean}, are
proved only under \path{MPOTensor.IsHorizontalCF}.  They give
\[
X_{\alpha,k}^\dagger X_{\alpha,k}
=\omega_{\alpha,k}X_{\alpha,1}^\dagger X_{\alpha,1},
\qquad \omega_{\alpha,k}>0,
\]
and normalize the relative gauge after division by
$\sqrt{\omega_{\alpha,k}}$.  No theorem yet connects this actual-sector
Figure~8 argument to
\path{MPSTensor.IsCPSVCanonicalForm.%
exists_verticalBNTGrouping_with_isometry}.  Thus grouped Figure~8
normalization remains open under the literal CPSV hypothesis, even though the
phase classes and positive grouped coefficients are now available there.
...
BNT-refined hypothesis \path{MPOTensor.IsHorizontalCF}.  Under the literal
CPSV canonical form assumed in Proposition~4.13, the remaining obligations
begin after the phase-class grouping proved here.  Under the literal
...
separation, vertical phase-class grouping with positive coefficients and exact
reconstruction, the pairwise Figure~8 Gram-dressing identity, and the positive
...
actual representative families.  Grouped Figure~8 normalization and the final vertical canonical-form theorem
with its coisometry remain open.  The proved statements retain every
