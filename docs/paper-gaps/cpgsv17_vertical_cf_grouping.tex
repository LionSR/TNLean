...
\eqref{eq:source-vertical-reconstruction}.  The proof retains every inactive
listed coordinate with weight zero until the vertical decomposition and does
not use the unsupported diagonal-restriction statement from \ghissue{2537}.
...
The active representative sector decomposition and its positive-length
matrix-product-vector identity are explicit in
\path{TNLean/MPS/CanonicalForm/ActiveBNTRefinement.lean}. Its effective copy
weight is
\begin{equation}
  \lambda_{\alpha,q}
...
zero-weight listed coordinate. The definition
