...
Injective spanning gives
$\mathcal A_S+\mathcal A_C=M_D(\mathbb C)$, while both spaces are nonzero.
The one-sided product obstruction then gives a contradiction, without any
assumption on products in the reverse order.  This directed-cut argument is
now formalized for the active-sector virtual matrix spaces.  Nondegeneracy of
the trace pairing gives a closed support walk of length two through every
active sector, while triangle closure gives one of length three.  Strong
connectivity and these coprime return lengths yield a common positive power of
the active trace matrix.  The abstract four-sector phase example remains a
counterexample to cyclic positivity alone, but it does not satisfy the
additional inverse-map and injectivity consequences used here.
...
statement restricts the trace matrix to this subtype while leaving the
