...
finite-ring state. It may assume
(\ref{eq:pgvwc07_ti_uniqueness_scope_finite_injectivity}), uniqueness of the
OBC canonical representation, and
(\ref{eq:pgvwc07_ti_uniqueness_scope_finite_size_bound}), which are the
printed hypotheses; the formal route below needs only the first and the last.
Its conclusion must provide a unitary $U$ and an $N$th-root phase $\xi$ satisfying
...
declaration requires it. Downstream CPSV16 uses must not cite these
same-structure statements as PGVWC07 canonical-form uniqueness. A use that
needs that uniqueness cites the corrected theorem of the previous section,
which assumes finite injectivity and the finite-size bound
(\ref{eq:pgvwc07_ti_uniqueness_scope_finite_size_bound}), does not assume OBC
uniqueness, and yields the corrected unitary-phase conclusion
