...
\begin{align}
  ...
  \label{eq:block_parent_simultaneous_length}
\end{align}
...
(\ref{eq:block_parent_simultaneous_length}). It does not assert the
...
\begin{align}
  ...
  \label{eq:block_parent_cyclic_intertwiner}
\end{align}
Iterating (\ref{eq:block_parent_cyclic_intertwiner}) once around the ring
shows that $X_i^j$ commutes with every length-$N$ word. The word span of
each block is the full matrix algebra, so $X_i^j$ is scalar. The boundary
