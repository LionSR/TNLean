...
\cite[Theorem~\emph{Degeneracy of the ground space v2}]{PerezGarcia2007MPSReps}.
For a fixed boundary-crossing interval with left endpoint \(i\), the local
block-sum constraint and the word-tuple span at the cut-tail length \(N-i\) now
give matrices \(C^j_{i,\rho}\) satisfying
\begin{align}
...
hypothesis: the present proof boundary is the cut-tail word-span hypotheses
\(N-i\) for crossing intervals and the still-used block-diagonal boundary
representation of a vector \(\psi\in\mathcal K_{N,L}(\widehat A)\).
...
remaining external boundary-representation input and discharge the cut-tail
