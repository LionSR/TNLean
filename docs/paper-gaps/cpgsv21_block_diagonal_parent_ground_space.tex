...
The older representation paper \cite[Theorem ``Degeneracy of the ground space
v2'']{PerezGarcia2007MPSReps} gives the finite-chain identity used by the
...
direct-sum boundary formula, and 3972--4004 for the final span statement.} Write
\begin{align}
  \widetilde A_i=\bigoplus_{j=1}^b\mu_j A_i^j
\end{align}
and
\begin{align}
  \sum_{j=1}^b\operatorname{ChainGround}_{N,L}(A_j)
  =
  \left\{
    \sum_{j=1}^b \psi_j:
    \psi_j\in\operatorname{ChainGround}_{N,L}(A_j)
  \right\}.
\end{align}
...
\begin{align}
  ...
  \Longrightarrow
  \notag\\
  \operatorname{ChainGround}_{N,L}(\widetilde A)
  =
  \sum_{j=1}^b \operatorname{ChainGround}_{N,L}(A_j).
\end{align}
The corresponding blueprint entry records the weight hypothesis but weaker
length hypotheses:
\begin{align}
  \forall j,\ \mu_j\ne 0,
  \qquad
...
\begin{align}
  \mathcal K_{N,L}(A)=\operatorname{ChainGround}_{N,L}(A),
  \qquad
...
parent-Hamiltonian uniqueness theorem applied to each block, and the last
equation is an element of
$\operatorname{span}\{V^{(N)}(A_j):j=1,\ldots,b\}$.
