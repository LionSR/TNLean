...
comparison in
\leanid{MPSTensor.exists_blockDiagonal_boundary_chainGroundSpace_pgvwc07_of_dualFixedPoint};
this resolves
...
\section{Injectivity length and interaction range}

Two different lengths enter the source comparison. The common injectivity
length of the individual blocks in Theorem~12 of
Ref.~\cite{PerezGarcia2007MPSReps} need not be a simultaneous injectivity
length. In the block-injective definition of Ref.~\cite{Cirac2021Matrix},
the physical coefficients must represent every tuple of matrices at once.
Writing $S$ for this latter length, the hypothesis is
\begin{align}
  \operatorname{span}_{\mathbb C}
  \{(A_j^\omega)_j:|\omega|=S\}
  &=\prod_j M_{D_j}(\mathbb C).
  \label{eq:cpgsv21_block_diagonal_parent_ground_space_simultaneous_length}
\end{align}
The relevant definition occurs at
\path{Papers/2011.12127/TN-Review-main.tex}:2114--2122.

The interaction range is also part of the source assertion. The paragraph
at lines~2126--2129 refers back to the inversion and regrowth argument and
to the normal-tensor theorem. That theorem specifies interactions on
$S+1$ sites at lines~2087--2094; the preceding argument at
lines~2078--2079 allows larger interactions by regrowing the tensors.
Thus the intended strengthened block statement has
$S+1\leq L\leq N$, with $S\geq1$ satisfying
(\ref{eq:cpgsv21_block_diagonal_parent_ground_space_simultaneous_length}). It does not assert the
ground-space equality for every $L\leq N$ independently of $S$.

The distinction is visible for the GHZ tensor. Its two scalar blocks are
$(1,0)$ and $(0,1)$, so their simultaneous injectivity length is $S=1$.
The one-site local ground space is all of $\mathbb C^2$, and the canonical
range-one Hamiltonian is zero. For $N\geq2$ its kernel has dimension $2^N$,
whereas the two block vectors span a space of dimension two. The
range-two parent Hamiltonian is the one covered by the source theorem.

The longer-range theorem below uses individual block injectivity.
The simultaneous-span argument in the final section establishes the
$S+1$ interaction bound, including the minimal ring $N=S+1$, without
substituting the larger separation bound.
...
\leanid{MPSTensor.chainGroundSpace_toTensorFromBlocks_le_iSup_groundSpace_pgvwc07_of_dualFixedPoint},
\leanid{MPSTensor.exists_unique_sum_groundSpace_of_chainGroundSpace_toTensorFromBlocks_pgvwc07_of_dualFixedPoint},
\leanid{MPSTensor.exists_blockDiagonal_boundary_chainGroundSpace_pgvwc07_of_dualFixedPoint},
and
\leanid{MPSTensor.ker_parentHamiltonian_toTensorFromBlocks_eq_bntMPSVectorSpan_pgvwc07_of_dualFixedPoint}.}
...
\leanid{MPSTensor.exists_blockDiagonal_boundary_chainGroundSpace_pgvwc07_of_dualFixedPoint},
and
\leanid{MPSTensor.chainGroundSpace_toTensorFromBlocks_eq_iSup_pgvwc07_of_dualFixedPoint}.
...
\end{align}
For $b\geq2$, $L_0\geq1$, and nonzero weights $\mu_j$, the proved
source-range statement is
\begin{align}
  &\left[
    L\geq3(b-1)(L_0+1)+1
    \ \wedge\
...
declarations:
\leanid{MPSTensor.pgvwc07_complementary_word_cde_identities_of_trace_decomposition}
applied with the boundary matrix \(X_j\) replaced by \(\mu_j^NX_j\),
...
\leanid{MPSTensor.exists_blockDiagonal_boundary_of_chainGroundSpace_toTensorFromBlocks_pgvwc07_of_dualFixedPoint}
and
\leanid{MPSTensor.pgvwc07_iSup_restriction_intersection_of_ge_of_bnt_directSum_unital_c1_pgvwc07_of_dualFixedPoint}.}
...
\section{Closure at the simultaneous injectivity length}

Assume positive physical and bond dimensions, nonzero block weights, and
(\ref{eq:cpgsv21_block_diagonal_parent_ground_space_simultaneous_length}) at a positive length $S$.
For every $S+1\leq L\leq N$, the strengthened conclusion is
\begin{align}
  \mathcal K_{N,L}(\widehat A)
  &=\ker H_L^{(N)}(\widehat A)
    =\operatorname{span}\{V^{(N)}(A_j):1\leq j\leq b\}.
  \label{eq:cpgsv21_block_diagonal_parent_ground_space_simultaneous_kernel}
\end{align}
There is no normalization or separate inequivalence hypothesis on the
original blocks. This includes $N=L=S+1$.

To prove (\ref{eq:cpgsv21_block_diagonal_parent_ground_space_simultaneous_kernel}), first normalize each
block independently. A nonzero rescaling by $\zeta_j$ multiplies its
length-$S$ words by $\zeta_j^S$, and a virtual gauge conjugates every
word by the same invertible matrix. These operations preserve the full
simultaneous span at exactly $S$, all component ground spaces, and the
span of the periodic component vectors. In trace-preserving form, the
simultaneous span propagates to every length at least $S$.

The open-chain intersection property then supplies boundary matrices
$X_i^j$ at every cyclic cut $i\in\mathbb Z/N\mathbb Z$ for a periodic
ground vector. Comparing adjacent cuts against all complementary words
of length $N-1$ gives, for every physical letter $a$,
\begin{align}
  X_i^j A_j^a&=A_j^a X_{i-1}^j.
  \label{eq:cpgsv21_block_diagonal_parent_ground_space_cyclic_intertwiner}
\end{align}
Iterating (\ref{eq:cpgsv21_block_diagonal_parent_ground_space_cyclic_intertwiner}) once around the ring
shows that $X_i^j$ commutes with every length-$N$ word. This is the
cyclic argument of the theorem entitled \emph{Uniqueness with TI and PBC} in
Ref.~\cite{PerezGarcia2007MPSReps}, which the proof of Theorem~12 there
invokes for several blocks.\footnote{Source location:
\path{Papers/quant-ph_0608197/MPSarchive.tex}:1272--1289, the theorem
labelled \path{uniqueGS} and its proof, invoked at lines 1454--1455.}
The word span of each block is the full matrix algebra, so $X_i^j$ is
scalar. The boundary
representation at cut zero is therefore a linear combination of the
periodic component vectors. The reverse containment follows because
each component vector satisfies every local parent constraint.

The conclusion also holds for positive parent terms that are not
projections. If $K_i\geq0$ has the same kernel as the canonical term at
site $i$, then
\begin{align}
  \ker\Bigl(\sum_i K_i\Bigr)
  &=\bigcap_i\ker K_i=\ker H_L^{(N)}(\widehat A).
  \label{eq:cpgsv21_block_diagonal_parent_ground_space_positive_kernel_sum}
\end{align}
The formal statements are in
\path{TNLean/MPS/ParentHamiltonian/BlockGroundSpaceAtInjectivityLength.lean}.

For an original tensor in canonical form, repeated multiplicity copies
have the same component local spaces as their distinct normal
representatives. Coisometric reconstruction also preserves these spaces
at every positive length. Thus the same conclusion applies to the original
tensor, including its multiplicities, at the same supplied $S$.
The corresponding statements for canonical projectors and arbitrary positive
parent terms are in
\path{TNLean/MPS/ParentHamiltonian/CanonicalBlockGroundSpaceAtInjectivityLength.lean}.
