...
sectors. Combining equations~(11), (14), and~(15) now gives the unnormalized
ambient tripartite identity
\begin{align}
  W^\dagger\rho_{ABC}W
  =0_\perp\oplus\bigoplus_j\omega_j\otimes\widehat\rho_j,
\end{align}
where $W=\mathbf 1_A\otimes(U_B\otimes\mathbf 1_C)$, each supported block is
written in the HJPW order $(A B_j^L)\otimes(B_j^R C)$, and every complementary
block is zero. This is formalized by
\leanid{Matrix.RecoveredConditionalDilationBlockForm.ambient_tripartite_block_form}.
The factors $\omega_j$ are still unnormalized. Their probability normalization,
the zero-weight choices, and the final quantum-Markov decomposition remain
...
supported-sector states with the prescribed common-factor marginals. In HJPW
factor order, the resulting tripartite state has zero complementary blocks and
supported blocks $\omega_j\otimes\widehat\rho_j$. Thus only the probability
normalization, the choices at zero weight, and the final quantum-Markov
decomposition are missing. The family-level structural implication needed here
...
action are packaged on the minimum joint support. Applying this theorem to
...
preserving-operation action of $\varphi$.
\leanid{Matrix.exists_recoveredConditionalBipartiteBlockForm_jointSupport}
rescales active slices, proves inactive positive slices vanish, and reconstructs
...
equation~(14).
\leanid{Matrix.exists_recoveredConditionalAmbientBipartiteBlockForm} completes
the support coordinates to all of $B$, adds one-dimensional zero-weight tensor
sectors on the complement, and proves the ambient equation~(14) with positive
factors whose conditional traces sum to one.
\leanid{Matrix.exists_recoveredDilationBlockUnitary} proves the block-coordinate
recovery dilation of equation~(15), supported agreement with the Petz
operation, and the exact recovery identity. It also constructs positive
trace-one recovered states on the supported common-factor sectors with the
prescribed common-factor marginals.
\leanid{Matrix.exists_recoveredConditionalTripartiteBlockForm} combines
equations~(11), (14), and~(15) in the orientation
$W^\dagger\rho_{ABC}W$ and gives zero complementary blocks together with
supported blocks $\omega_j\otimes\widehat\rho_j$ in HJPW factor order.
Defining the probabilities, normalizing the left factors, handling zero-weight
sectors, and constructing the quantum-Markov decomposition remain necessary
...
reverse direction and a postulated forward direction. For the forward
direction, equality now gives the unnormalized ambient tripartite blocks in
HJPW factor order. The remaining step is their probability normalization and
inclusion in the quantum-Markov decomposition.
...
supported-sector states are now formalized. Equations~(11), (14), and~(15) now
also give zero complementary tripartite blocks and supported blocks
$\omega_j\otimes\widehat\rho_j$ in HJPW factor order. Probability normalization,
zero-weight sector choices, construction of the quantum-Markov decomposition,
and removal of the forward external assumption remain open.
