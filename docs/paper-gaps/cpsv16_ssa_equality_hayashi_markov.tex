...
\doclink{TNLean/Entropy/SSAEqualityCharacterization}. The reverse implication is
...
\end{align}
The supported matrices $\widehat\rho_j$ are the sector output states:
...
sector output state $\widehat\rho_j$. When $p_j>0$, the left density operator is
...
on the supported input, the invariant conditional family, its common invariant
...
are the joint-support reduction and its application to the invariant
...
\begin{center}
  ...
  \leanid{Matrix.exists_markovTripartiteBlockForm}
\end{center}
produce the positive unnormalized factors $\omega_j$, prove that their traces
sum to one, produce the trace-one supported sector output states
...
normalization. Every supported right factor remains its sector output state,
including in a zero-weight supported sector. A zero-weight supported
...
gives the raw support-Petz recovery identity, and on the Petz output marginal the
...
summing to one, preserves the trace-one sector output states on supported sectors,
