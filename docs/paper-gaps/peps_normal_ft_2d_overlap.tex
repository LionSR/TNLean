...
Theorem~3 of Ref.~\cite{Molnar2018NormalPEPS};\footnote{Theorem~3 is at
    ...
    \path{TNLean/PEPS/TorusUnconditionalFundamentalTheorem.lean}.} only the hypotheses
...
$P_B(W_0)E_B(e,\cdot,X)$.  The relations
...
$\lambda^{nm}=1$.\footnote{\leanid{TNLean.PEPS.exists_torusCovariantAbsorbedGaugeFamily},
    ...
    \path{TNLean/PEPS/TorusUnconditionalFundamentalTheorem.lean}.} Two of these layers
...
Layers 1--4 and the interface of layer~5 are complete.  The cross-tensor
transfer that feeds layer~5 and the re-anchoring in layer~6 remain. Until the
capstone lands,
...
This section separates three statements that had previously been conflated:
...
\subsection{The coarse super-bond is an auxiliary theorem, but is unavailable
    for the single-window red block at the minimal size}

The rectangle formalization has a reusable \emph{coarse three-site} route to
$\text{\path{hmul}}$: three
...
needs only blocked-region injectivity of the three regions.  This theorem is a
valid auxiliary result, but it is not the overlapping-window composition used
by Ref.~\cite{Molnar2018NormalPEPS}.
...
The coarse three-site multiplicativity theorem needs a third injective block
--- the complement of the red-blue pair --- and at the minimal size, with the
red block a single window and the single-crossing partner forced to be the
opposite window, that complement is $\mathrm{univ}\setminus S$, non-injective.
It is therefore not the source-faithful route here.  The landed end-factor
relations instead make multiplicativity a direct calculation: compose the two
...
The multiplicativity is carried by the gauge assembly
\leanid{TNLean.PEPS.exists\_regionEdgeGauge\_of\_coherentFrames}
(\path{TNLean/PEPS/RegionBlock/CoarseThreeSite11.lean}): two coherent coarse blocking
...
multiplicative, with no single-vertex injectivity. Its $\text{\path{hmul}}$ step is the
identification of the chosen forward transfer with the concrete single-edge transfer,
\leanid{TNLean.PEPS.coeffTransferMap\_eq\_regionEdgeTransfer}, followed by the concrete
transfer's multiplication law \leanid{TNLean.PEPS.regionEdgeTransfer\_mul}; a consumer
holding only the coefficient transfer obtains the multiplicativity from that pair in
three rewrites. It makes the verdict auditable in Lean: the window route's
$\text{\path{hmul}}$ is one application of that pair with $\mathrm{red}=W$, and the unmet
hypothesis is exactly the frame's $\text{\path{complement\_injective}}$ at
$\mathrm{univ}\setminus S$, the documented obstruction.  The pair remains available for
any future coherent frame, but the
fixed-edge common-state family below does not require such a frame.  The remaining local
work is the cross-tensor identification that feeds the direct end-composition
...
multiplicativity from a coherent three-region blocking frame.\footnote{Formal
    declaration:
    \leanid{TNLean.PEPS.exists\_regionEdgeGauge\_of\_coherentFrames}.}  Neither
