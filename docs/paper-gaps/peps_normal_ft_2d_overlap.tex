...
$X$ is also complete.  What remains is to prove by composition that the
assignments from $O_1$ and $O_3^{\mathsf T}$ to $X$ are algebra
homomorphisms, use them for the cross-tensor transfer, and assemble the back
...
The argument that does \emph{not} transfer verbatim is the single-window
square-matrix-span proof used in the final extraction of the matrix $X$ in one
dimension. In Lemma~5 of Ref.~\cite{Molnar2018NormalPEPS}, that move compares the
...
no such span.  The landed two-dimensional proof instead applies
operator-Schmidt uniqueness to the two genuine end-window families
simultaneously.
...
$X$ are now formalized.  What remains is the composition argument for
Lemma~5's maps $O_1\mapsto X$ and $O_3^{\mathsf T}\mapsto X$, the cross-tensor
transfer, and the back-half assembly.  The failed single-window span above is an
...
The following surrounding results are formalized at the corollary's minimal
size:
...
The remaining gates are the $O_1\mapsto X$ and
$O_3^{\mathsf T}\mapsto X$ composition laws, the cross-tensor transfer, and
the back-half assembly that re-anchors the torus Theorem~3 layers to the
$(L+1) \times (K+1)$ comparison pair.

\paragraph{3. The remaining composition and cross-tensor extraction.}
...
$P_B(W_0)E_B(e,\cdot,X)$.  The outstanding inputs are the composition laws
for the maps from $O_1$ and $O_3^{\mathsf T}$, the resulting cross-tensor
coefficient transfer between $A$ and $B$, and its multiplicativity
\path{hmul}.  The back-half assembly follows only after this transfer is
available.  Existing single-window and coarse-three-site attempts do not
derive these inputs at the minimal size; a source-faithful extraction must use
...
end-pair operation now follow this description literally.  The remaining
formal work is the composition argument for Lemma~5's $O_1$ and
$O_3^{\mathsf T}$ maps, the cross-tensor transfer, and the back-half assembly.
...
same-tensor family and unique end-pair extraction are complete; the remaining
work is the two composition maps, the cross-tensor transfer, and the back-half
assembly tracked separately.
...
The boundary-configuration form of this geometry is also formalized.  The
equivalences
\path{horizontalStaircaseLeftWindowBoundaryConfigEquiv} and
\path{horizontalStaircaseRightWindowBoundaryConfigEquiv} split the boundary
configuration of either end window into its index on $e$ and its external
portion of $\partial S$, while
\path{horizontalStaircaseEndPairBoundaryConfigEquiv} splits a configuration
on $\partial S$ into its left and right external portions.  Their evaluation
rules preserve each underlying lattice-edge label in both directions.  Thus
the dependent index spaces required for the single-bond sum are in place; no
cardinality replacement of a window boundary by a square matrix space is used.
...
window block on the compatible fibre and vanishes off it, and
\path{existsUnique_virtualOperation_of_horizontalStaircaseEndPair} extracts
the unique matrix $X$.  The residual is the composition argument for the maps
$O_1\mapsto X$ and $O_3^{\mathsf T}\mapsto X$, followed by the cross-tensor
transfer and back-half assembly.

\subsection{Step 5: the rest of the proof}

With the per-edge extraction in place, the remainder follows the proof of
Theorem~3 specified by Ref.~\cite{Molnar2018NormalPEPS}.  The existing torus
formalization supplies a chain of layers to be re-anchored after the remaining
cross-tensor coefficient identity is available: the insertion
...
end-pair-operation layers of the derivation above are complete. The remaining
source steps following
\path{Papers/1804.04964/paper_normal.tex:2368--2444} of
Ref.~\cite{Molnar2018NormalPEPS} are the composition laws for the Lemma~5 maps
from $O_1$ and $O_3^{\mathsf T}$, the cross-tensor transfer, and the back-half
assembly.  The layer list below records the campaign around these steps:
...
\begin{enumerate}
  ...
  \item \emph{The Lemma~5 converse on the staircase}: compare the first and
    ...
    $X$ (complete); then prove the $O_1\mapsto X$ and
    $O_3^{\mathsf T}\mapsto X$ homomorphisms by composition (remaining).
  \item \emph{The per-edge witness}: the landed insertion-correspondence and
    ...
    interface consumes.
  \item \emph{Re-anchoring the downstream} (remaining): the covariant family,
    the
...
Layers 1--3, the unique-$X$ part of layer~4, and the interface of layer~5 are
complete.  The composition part of layer~4, the cross-tensor transfer that
feeds layer~5, and the re-anchoring in layer~6 remain. Until the capstone lands,
the corollary of Ref.~\cite{Molnar2018NormalPEPS} at
\path{Papers/1804.04964/paper_normal.tex:2297--2318} remains
unformalized, and no blueprint entry should claim it.

Layer 1 is complete. The seam-avoiding window geometry --- the general
$a \times b$ injectivity by sliding unions, the staircase end pair, and
the single-crossing characterization --- is in
\path{TNLean/PEPS/TorusWindowRegion.lean}. The
...
contraction-extension content of
(\ref{eq:peps_normal_ft_2d_overlap_consecutive_window_open_equality}) is also
complete: \path{horizontalConsecutiveWindow_extend_eq} writes each
single-window deformed state as the union deformed state of the
corner-extended insert $C_j \sqcup T_{W_{j+1} \setminus W_j}$ and applies the
consecutive-window engine; its vertical counterpart supplies the same step
for the descending arm of the staircase.
...
completed region as tensors. This route is now implemented by
\path{staircasePair_insert_eq_open}: it chains the consecutive-window
open-boundary equalities across the patch by contraction and transitivity,
then cancels the completed corner locally by its injectivity alone. It never
collapses the comparison to the patch closed state or assumes injectivity of
...
The finite-dimensional ``compare the two ends'' argument of Lemma~5 and the
torus boundary reindexing needed to apply it are now landed.  The theorem
$\text{\path{existsUnique\_virtualOperation\_of\_endPair}}$ starts from the exact
operator-Schmidt identity
\begin{align}
  \sum_\mu C_{1,\mu}\otimes A_{2,\mu}
    &= \sum_\nu A_{1,\nu}\otimes C_{2,\nu}
\end{align}
and, assuming only linear independence of the two genuine families $A_1$ and
$A_2$, produces the unique matrix $X$ for which
$C_{1,\nu}=\sum_\mu A_{1,\mu}X_{\mu\nu}$ and
$C_{2,\mu}=\sum_\nu X_{\mu\nu}A_{2,\nu}$.  No injectivity of the modified
families $C_1,C_2$ is used.  Thus the algebraic inversion step in
\cite[Lemma~5]{Molnar2018NormalPEPS} no longer requires a separate PEPS
argument.  The three boundary-configuration equivalences above provide the
exact reindexing for the open legs.  The normalized coefficient factorization
is now also landed.  On each end, incompatible external labels make
\path{ThreeBlockGeometry.threeBlockBlueCoeff} vanish; on the compatible fibre,
its normalized value is the genuine opposite-window blocked tensor at the same
reference-edge coordinate and the opposite external configuration.  This gives
the two explicit single-bond formulas for \path{extendInsert}.  Translation
invariance identifies their external crossing-bond products, so equality in
(\ref{eq:peps_normal_ft_2d_overlap_formal_end_pair_insert_equality}) cancels to
the operator-Schmidt identity above.  The theorem
\path{existsUnique_virtualOperation_of_horizontalStaircaseEndPair} consequently
extracts the unique highlighted-edge matrix $X$ assuming injectivity only of
the two genuine end windows.  No injectivity of a modified insert, a single
vertex, the end pair, or its complement enters this step.
The boundary-label and normalized-coupling arguments are in
\path{TNLean/PEPS/TorusWindowPeeling/EndWindowCoupling.lean}; the single-bond
contraction and unique-operation extraction are in
\path{TNLean/PEPS/TorusWindowPeeling/EndOperation.lean}.

The remaining Step~4 work is the composition argument for the maps
$O_1\mapsto X$ and $O_3^{\mathsf T}\mapsto X$, followed by their use in the
cross-tensor assembly and the back half of the proof.

\subsection{The back-half geometry supports a window witness region}

The torus back-half argument of Theorem~3 consumes host injectivity in exactly
two places, and at the corollary's minimal size $(2L+1)\times(2K+1)$ both are
...
\emph{Verdict.} The back-half geometry transfers to the corollary's minimal size with the
...
remaining back-half work is therefore an \emph{assembly} of landed machinery
--- the peeling of
...
\medskip
\noindent\textbf{The local residual.}  What remains of \S5.1 is the production of the
...
The local residual is the composition argument and resulting cross-tensor
multiplicative transfer: the homomorphism the rectangle route reads from the
physical realization of the inserted operator.  The region-level recovery
...
After this transfer is available, the back-half assembly remains.

\section{The multiplicativity verdict: it is the coarse three-site
super-bond, not a single-window span}

This section records the definitive resolution of one local residual question the
...
span or an assumed coarse frame.  The back-half assembly follows only after this
...
fixed-edge common-state family below does not require such a frame.  The remaining local
work is the two-map composition and cross-tensor multiplicativity argument; the
back-half assembly is the subsequent step.
...
prove that transfer multiplicative; those are the remaining Lemma~5-style composition
and cross-tensor steps, after which the back-half assembly remains.
...
The remaining source work is to prove the maps $O_1\mapsto X$ and
$O_3^{\mathsf T}\mapsto X$ multiplicative by the composition argument of
Lemma~5, use them for the cross-tensor transfer, and complete the back-half
assembly.
