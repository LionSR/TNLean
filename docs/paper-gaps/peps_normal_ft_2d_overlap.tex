...
and $M'$. The per-step transport does not assemble such a family. The bond $e$ is a
boundary edge of only the two end windows; the intermediate windows touch neither endpoint
of $e$, so a bond insert of $M$ on them is not even typed, and their inserts must be the
deformed window tensors realizing $M$'s physical action --- the two-dimensional analogue of
the bond-operator pushing of Lemma~5 (lines 2045--2255). Moreover the per-step transport
agrees the two scaled inserts only up to the \emph{neighbour-dependent} interior-bond
rescaling: step $j$ scales by $\mathrm{ribp}(W_{j+1})$ on the left and $\mathrm{ribp}(W_j)$
on the right, and these scalings differ from step to step because the window translates need
not have equal interior-bond products (the bond dimensions need not be translation
invariant). A telescoping to one common state would require all the interior-bond products
equal, a translation invariance the normal-PEPS setting does not assume. So the per-step
transports do not glue into a single common-state family, and the end-pair peeling has no
family to consume for an arbitrary $M$.

This is exactly the residual: the two-dimensional bond-operator extraction that builds, from
one virtual operation $M$ on $e$, the common-state family of deformed window tensors on all
$L+K$ windows --- the matrix-tensor analogue of
\leanid{MPSChainTensor.overlapWindow\_exists\_bondOperator}. With that family the end-pair
...
\leanid{MPSChainTensor.exists\_insertionHom}. The per-step transport above is the foundation
of that extraction; the extraction itself --- the deformed-window-tensor family on the
intermediate windows and its single common state --- is the unbuilt block-level construction,
and it is where the multiplicativity, not the single-operation transport, lives.
