...
\path{TNLean/PEPS/TorusWindowChain3.lean}). What remains is the kernel triviality
itself: that the vanishing of $\path{bareExtendInsert} (S \subseteq R)\, X$ as a tensor
on $R$ forces $X = 0$. It needs the host-boundary-edge embedding and the $Q$-weight
span lemma, neither yet landed.
...
and the kernel reduction \path{extendInsert_injective_of_kernel_trivial}. They are in
...
the single kernel-triviality identity, the piece not yet landed.
