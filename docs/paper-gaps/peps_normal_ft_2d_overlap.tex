...
argument.  The three boundary-configuration equivalences above now provide
the exact reindexing for the open legs.  The remaining Step~4 work is to expand
(\ref{eq:peps_normal_ft_2d_overlap_formal_end_pair_insert_equality}) into the
single-bond contraction identity.  Concretely, the present definition of
\path{extendInsert} is expressed through
\path{ThreeBlockGeometry.threeBlockBlueCoeff} and the
$\mathrm{univ}\setminus S$ interior-bond multiplicity.  The missing lemma must
identify that normalized blue coupling, for the two-window geometry, with the
genuine opposite-window block indexed by the same $e$-coordinate and the
opposite external configuration.  This is a coefficient factorization, not an
additional injectivity hypothesis.  After it, one must formalize the composition
argument for the maps $O_1\mapsto X$ and $O_3^{\mathsf T}\mapsto X$.
