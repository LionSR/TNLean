...
\path{MPOTensor.IsHorizontalCF}. The source's all-length condition for its
distinguished periodic-sector projector is formalized as
\leanid{MPOTensor.PeriodicSectorProjector.not_commute}. The stronger uniform
predicate requiring this noncommutation at every length for every displaced
Hermitian idempotent remains formalized, but deprecated, as
\leanid{MPOTensor.NoninvariantProjectorNoncommuting}; it is used by neither
active proof and is retained only for API compatibility.
