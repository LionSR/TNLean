...
\leanid{MPSTensor.exists\_pos\_productWordSpan\_of\_isCanonicalFormBNT\_of\_directSum\_injectiveBlocks}, and
...
been formalized for the already-injective BNT theorem surface.  In that setting,
the BNT representatives, their pairwise not-gauge-phase-equivalence data, and a
common injective length imply the homogeneous pair-trace separation and
product-word-span conclusions needed downstream.  Thus the old warning against
replacing exact-length span by cumulative span remains mathematically important,
but it is no longer an open gap for an already-injective
\leanid{IsCanonicalFormBNT} family.

For the full after-blocking normal-CF-BNT pipeline, the conclusion is still more
...
on main: the proof still has to transport the blocked BNT separation data through
...
exact-length direct-sum input as formalized for already-injective BNT data, while
keeping the after-blocking CPSV statement visibly conditional until the
reblocking transport and blocked BNT-separation handling have landed.
