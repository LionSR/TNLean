...
up to gauge and phase. PGVWC07 instead assumes that the corresponding
finite-ring component vectors are pairwise distinct. The missing comparison is
a theorem deriving the required gauge-phase separation from the source
hypothesis. To remove the extra premise, one must prove that comparison and use
it to state the direct-sum and ground-space results with the source hypothesis.
...
span are proved under gauge-phase separation, while PGVWC07 assumes only
pairwise distinct finite-ring component vectors. Elimination requires proving
that the source hypothesis implies gauge-phase separation for the canonical
blocks, then restating the direct-sum and ground-space theorems through that
comparison. Until this implication is formalized, the sharp source input is
not fully closed at its printed hypothesis set.
