...
normalization-free Definition~4.7 predicate. It asks for some positive blocking
and a basis of normal tensors whose physical-trace transfers are all
non-nilpotent. It imposes neither positive-length nonvanishing nor the line-246
unit-weight convention. The strengthened project predicate
...
unconditional. The strengthened implication
\leanid{MPOTensor.IsSimple.isNonvanishingSourceSimple} uses
\eqref{eq:positive-length-nonzero}.

The compiled tensor \leanid{MPOTensor.SimpleVanishingCounterexample.M}, whose
...
the tensor is normalized-simple and source-simple, but not nonvanishing
source-simple because $\rho_1(M)=0$. This witness does not refute
...
and the conditional implication to nonvanishing source simplicity using
