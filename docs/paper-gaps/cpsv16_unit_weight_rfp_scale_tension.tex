...
convention at lines~217--246 and Definition~4.7. It requires a positive length
$N$ for which
\begin{align}
    \rho_N(M)&\ne0,
    \label{eq:some-positive-length-nonzero}
\end{align}
and asks for some positive blocking and a basis of normal tensors whose
physical-trace transfers are all non-nilpotent. The existential clause excludes
the identically zero family but permits isolated vanishing lengths. It does not
in general exclude dormant extra candidates in an arbitrary BNT presentation.
The predicate does not impose the line-246 unit-weight convention. The
strengthened project predicate
...
unconditional. Its proof uses eventual linear independence of the normal
sectors, a sector coefficient that is not eventually zero, transport through
the canonical-form gauge, and physical unblocking to obtain
\eqref{eq:some-positive-length-nonzero}. The strengthened implication
...
$\rho_2(M)=2I\ne0$
supplies existential nontriviality. It is not nonvanishing source-simple because
...
whose proof obtains existential nontriviality by eventual linear independence,
a non-eventually-zero sector coefficient, gauge transport, and unblocking, and
