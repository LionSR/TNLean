...
The declaration \leanid{MPOTensor.IsSimple} is the formalization of
lines~217--246 and Definition~4.7. It asks for some positive blocking and a BNT
sector presentation whose representative physical-trace transfers are all
non-nilpotent. Every representative has a positive number of copies and every
copy has nonzero weight, the nonzero-coefficient convention of
\path{docs/paper-gaps/cpsv16_bnt_uniqueness_zero_coefficient.tex}. The theorem
\leanid{MPOTensor.IsSimple.exists_mpo_ne_zero} derives a positive length
$N$ for which
\begin{align}
    \rho_N(M)&\ne0,
...
Isolated vanishing lengths remain possible.

The predicate does not impose the line-246 unit-weight convention. Requiring a
unit-weight witness of the blocked tensor is a hyper-literal reading that the
source's own conventions exclude: the dimer $R$ of the section below is
simple in the source's sense (its physical-trace transfer is non-nilpotent)
and carries the canonical-form weight $\sqrt{337/512}$, so it would not be
simple under a unit-weight reading. (Example~4.12 is not a second such
witness: its second BNT basis element has vanishing one-by-one
physical-trace transfer, so it is not simple under any reading, and its
canonical-form weight $1/\sqrt2$ is therefore beside the point; see
\path{docs/paper-gaps/cpsv16_simple_tensor_nilpotency.tex}.) The library
therefore reads line~246 as a normalization the source may apply, not as a
clause of Definition~4.7. The predicates it once separated, and the
...
displayed canonical-form representative, supplying the Appendix~C.2 hypothesis
on an already-blocked tensor. It is not a quotient by nonzero scalar rescaling,
and no equivalence with \leanid{MPOTensor.IsSimple} is asserted.
...
normalized horizontal canonical form: no rescaling of $R$ reconciles the
line-246 normalization with the scale that Definition~4.1 pins.

This is not a nilpotency statement, and it does not bear on Definition~4.7.
The retained-block theorem proves that the displayed canonical presentation has
non-nilpotent physical-trace transfer. Together with positivity and the
one-site BNT refinement, it witnesses
\begin{align}
    \leanid{MPOTensor.IsSimple}\,R,
\end{align}
by the theorem \leanid{R_isSimple} in the namespace
\leanid{MPOTensor.RescalingStableLengthDependentRFP}. The source quantifier is
existential and is witnessed by $L=1$; no assertion is made about every
positive blocking. Reading line~246 into Definition~4.7 would instead make $R$
non-simple, which is the reading this note rejects.
