...
calculation at lines~1333--1340. The source-facing formal statement is
\leanid{MPOTensor.isPhysicalTraceIdempotent_of_isRFPViaTS}; its proof uses the
equation theorem \leanid{MPOTensor.physTraceTransfer_sq_of_isRFPViaTS}.
...
normalization-free reading of lines~217--246 and Definition~4.7. It asks for
some positive blocking and a BNT sector presentation whose representative
...
BNT sector presentation. It is not nonvanishing source-simple because
...
It does not itself decide the claimed all-sector factorization. The
non-Cartesian witness in
\path{TNLean/MPS/MPDO/NonCartesianActiveSectorCounterexample.lean} refutes the
implication from injectivity, SAL, literal ZCL, and scalar relation to a normal
tensor. Its singleton coefficient $\mu$ and the Perron value $r$ of the
witness's unnormalized transfer, related by $|\mu|^2=r$ and both defined in
\path{docs/paper-gaps/cpgsv17_pf_rank_one.tex}, satisfy the exact strict
bound $0<|\mu|=\sqrt r<1$, formalized in
...
convention has no witness. Rescaling to unit weight
destroys literal ZCL. Thus \ghissue{6775} remains open at the ambient
source-context boundary. This is also the remaining input to the source's
channel construction: Proposition~\texttt{3to5} constructs the representative
maps from the neighboring-trace factorization, and
Proposition~\texttt{prop2to5} combines them after projecting onto the
orthogonal outer subspaces. The latter combination is formalized in
\ghissue{6632}. The direct sector-mixing alternative formerly tracked in
\ghissue{6793} is not a separate source step.
...
whose normalized sector decomposition gives a BNT sector presentation after gauge
