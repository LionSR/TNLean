...
\begin{align}
    ...
    \label{eq:rmp_peps_czx_printed_tensor}
\end{align}
...
\eqref{eq:rmp_peps_czx_printed_tensor} with equal labels at the two ends of
every bond, the
...
\begin{align}
    ...
    \quad\text{for every site } v.
    \label{eq:rmp_peps_czx_literal_constraints}
\end{align}
The conditions~\eqref{eq:rmp_peps_czx_literal_constraints} join the
top-right qubit of $v$ to bottom-left qubits of two different sites, and they
chain the qubits along the diagonals
...
plaquette states. For instance, on a torus of $3\times 3$ sites let $\sigma$ set
the four qubits around one plaquette to $1$ and every other qubit to $0$.
This configuration has coefficient $1$ in the product of plaquette states,
but the top-right qubit $j_v=1$ of the site $v$ at the lower left of that
plaquette differs from the bottom-left qubit $l_{v+e_2}=0$ of its upper
neighbour, so $\sigma$ violates~\eqref{eq:rmp_peps_czx_literal_constraints}
and has coefficient $0$ in the literal contraction. The printed claim thus
fails on nondegenerate data, and the note is filed as a false source claim.
The corrected statement below keeps the printed tensor and changes only how
its labels are joined across a bond.
...
in \cite{Cirac2021Matrix}. Equivalently, the tensor
\eqref{eq:rmp_peps_czx_printed_tensor} is used with its down and left labels
reversed. With this reading the contraction gives
\begin{align}
    c(\sigma)=\prod_{v}\delta\bigl[j_v=i_{v+e_1}=k_{v+e_2}
    =l_{v+e_1+e_2}\bigr],
    \label{eq:rmp_peps_czx_plaquette_coefficient}
\end{align}
where $c(\sigma)$ is the coefficient of the configuration $\sigma$.
Formula~\eqref{eq:rmp_peps_czx_plaquette_coefficient} is the product over
plaquettes of the
unnormalized states
