...
superposition.\footnote{Formal declarations: \leanid{MPSTensor.majumdarGhoshReviewTensor},
...
\begin{align}
    \mu^m
        &=
        cY_{01}Y_{10}Y_{01}^{m-2}
        =
        -c,
    \label{eq:rmp_majumdar_ghosh_tensor_gap_shifted}
\end{align}
and together with $c=\mu^m$ this gives $\mu=0$. In either case $M=0$, so
$U$ is singular and the left side
...
the obstruction. This basis-change argument is not formalized; the formal
statement is the scalar non-proportionality above.
