...
For an MPU in canonical form II,
\leanid{MPOTensor.IsMPUCanonicalFormII.rightRank_mul_leftRank_blockTensor}
now derives each endpoint identity from the formal counterpart of
Theorem~III.8 of Ref.~\cite{Cirac2017MPU} whenever the positive block is
simple. Consequently the index of this canonical representative is
independent of the chosen positive simple blocking; no endpoint identity
is imposed as an additional hypothesis.
...
the proof. The general saturation theorem retains that identity as an explicit
hypothesis; its application to a canonical-form-II representative derives
the identity from simplicity and canonical form.
