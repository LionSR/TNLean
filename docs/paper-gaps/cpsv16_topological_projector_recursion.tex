...
\section{Physical-space completion and chain-length boundary}
...
this analogue. Extending the retained one-site energy by zero on the orthogonal
physical complement gives a finite physical-space Hamiltonian, and transporting
the retained projectors through the adjoint sitewise coisometry gives physical
projectors. This completes the source decomposition for every chain length
$L\geq2$.\footnote{Formal declaration:
\leanid{MPOTensor.physicalTopologicalGibbsDecomposition_of_isRFPViaTS_of_cpsvCanonicalForm}
in \doclink{TNLean/MPS/MPDO/CPSVTopologicalPhysicalGibbs}. The corresponding
Blueprint entry is
\path{thm:mpdo_literal_cpsv_physical_coordinate_gibbs_at_least_two}.}
The complementary-subspace construction is recorded in
\path{docs/paper-gaps/cpsv16_topological_gibbs_physical_complement.tex}.

Ref.~\cite{Cirac2016MPDO_arXiv} defines the local term on two spins but gives no
separate convention for a one-site periodic chain. Thus the physical-space
theorem covers precisely the source-defined range $L\geq2$; no length-one
statement is asserted. This boundary is recorded in
