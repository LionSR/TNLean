...
| Complete | CPSV16 | Lemma C.5 (`SALZCL`) | Under the standing normal Case-I hypotheses, SAL and literal physical-trace idempotence give the active rank-one coefficients. The theorem does not extend to scale-invariant `IsSourceZCL` or to Case II | `TNLean/MPS/MPDO/InverseMapLemmaC5CaseI.lean` (`MPOTensor.exists_activeSectorTraceMatrix_rank_one_coefficients_of_isSAL_of_literal_ZCL`); normalization boundary audited in `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
