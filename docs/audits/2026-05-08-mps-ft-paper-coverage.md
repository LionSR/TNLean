...
| **Definition 4.7** (l.821) | 815–823 | A simple MPDO has no nilpotent element in a blocked BNT | `TNLean/MPS/MPDO/SourceSimpleTensor.lean` (`MPOTensor.IsSourceSimple`); `SimpleTensor.lean` (`MPOTensor.IsSimple`); `SimpleVanishingCounterexample.lean` (`MPOTensor.SimpleVanishingCounterexample.isSimple_and_not_isSourceSimple`); Blueprint `thm:mpdo_simple_not_source_simple_counterexample`; `docs/paper-gaps/cpsv16_unit_weight_rfp_scale_tension.tex` | **partial as an exact source definition; strengthened project interfaces complete** — `IsSourceSimple` is normalization-free but strengthens the printed definition by requiring nonzero MPOs at every positive length. `IsSimple` is a separate normalized fixed-representative predicate. Their project bridge is strictly conditional on positive-length nonvanishing: the compiled bond-two witness has $\rho_N(M)=(1+(-1)^N)I$, satisfies `IsSimple` after blocking two sites, and fails `IsSourceSimple` because $\rho_1(M)=0$. This proves that the added hypothesis is load-bearing for the project bridge; it does not refute Definition 4.7 and is not an RFP or coefficient-rigidity claim |
