...
| **Theorem 4.9** (l.851, `thm:main-simple`) | 851–893 | Simple-MPDO implication chain \((i)\Rightarrow(ii)\Leftrightarrow(iii)\Rightarrow(iv)\Rightarrow(v)\) | `TNLean/MPS/MPDO/RFPViaTSSAL.lean`; `TNLean/MPS/MPDO/CyclicActiveAreaLaw.lean`; `TNLean/MPS/MPDO/ActiveSectorSpanningAreaLaw.lean`; `TNLean/MPS/MPDO/PhysicalSectorFactorization.lean`; `TNLean/Analysis/EntropyMarkovForward.lean`; `TNLean/Channel/PetzProductReference.lean` | **partial** — implication \((i)\Rightarrow(ii)\) is complete at the explicit density-family boundary: positive semidefinite rings, nonzero trace at every positive length, and the Definition 4.1 local RFP equations imply source ZCL and SAL. The fixed-bond and source-ZCL implication to SAL is complete for the original injective MPDO tensor, without normality. The HJPW/Hayashi equality characterization and the exact Petz-recovery construction are also complete. The printed theorem remains partial because its SAL-and-ZCL structural implication to \((iv)\) passes through formally false Lemma C.5 |
...
| **Theorem 4.14** (source label `thm:IV.13`, l.972) | 972–993; proof 1929–2088 | RFP iff the tensor-attached BNT algebra condition iff the fusion-isometry condition | `TNLean/MPS/MPDO/BNTAlgebraTensorClause.lean`; `BNTAlgebraTensorClausePositivity.lean`; `BNTAlgebraTensorClauseSpectrum.lean`; `BNTFusionTensorClauseFromRFP.lean`; `PositiveMinimalRealizationCounterexample.lean`; `PositiveLinearExtensionCounterexample.lean`; `BondTwoSingletonGramBoundary.lean`; `BondTwoSingletonBaseModel.lean`; `BondTwoSingletonPhysicalGauge.lean`; `AlgebraFusionCounterexample.lean` | **partial** — several directions and the tensor-attached data are complete. Terminal positivity excludes the generic nonunitary-similarity witness from retained positive sectors. The explicit injective one-block `SameMPV` pair has a nonpositive generic per-block extension. Thus injectivity and closed-chain equality alone cannot supply the positivity needed for unitary implementation. The bond-two singleton is an explicit all-length MPDO with a tensor-attached one-label BNT algebra clause. Its concrete nonunitary physical-similarity deformation has a genuine `BNTAlgebraTensorClause`, exact one-site and two-site reindexing identities, and literal horizontal CPSV canonical form. Its length-two operator is not positive, so the deformation fails `IsMPDO`. Neither example is a full-contract counterexample or constructs an `IdentityMarkedRealization`. The marked comparison remains open in #5284 under #4648, and #4645 remains open. Appendix C.4 lines 2048–2057 and Proposition 4.13 lines 1898–1921 are motivation and a proof template, not statements of these model-specific results |
...
| Restatement/proof of Theorem 4.14 | 1929–2088 | Algebra and fusion characterizations of RFP | `TNLean/MPS/MPDO/BNTAlgebraTensorClause.lean`; `BNTAlgebraTensorClausePositivity.lean`; `BNTAlgebraTensorClauseSpectrum.lean`; `BNTFusionTensorClauseFromRFP.lean`; `PositiveMinimalRealizationCounterexample.lean`; `PositiveLinearExtensionCounterexample.lean`; `BondTwoSingletonGramBoundary.lean`; `BondTwoSingletonBaseModel.lean`; `BondTwoSingletonPhysicalGauge.lean`; `AlgebraFusionCounterexample.lean` | **partial restatement** — Terminal positivity excludes the generic nonunitary-similarity witness from retained positive sectors. The explicit injective one-block `SameMPV` pair has a nonpositive generic per-block extension. Thus injectivity and closed-chain equality alone cannot supply the positivity needed for unitary implementation. The bond-two singleton is an all-length MPDO with a tensor-attached one-label BNT algebra clause. Its concrete nonunitary physical-similarity deformation has a genuine `BNTAlgebraTensorClause` and literal horizontal CPSV canonical form, but its length-two operator is not positive, so it fails `IsMPDO`. Neither example is a full-contract counterexample or constructs an `IdentityMarkedRealization`. The marked comparison remains open in #5284 under #4648, and #4645 remains open |
...
| Partial | CPSV16 | Theorem 4.9 | Implication \((i)\Rightarrow(ii)\), the HJPW/Hayashi equality characterization, and the exact Petz-recovery construction are complete. The remaining printed obstruction is the SAL-and-ZCL structural implication, whose proof uses formally false Lemma C.5 | `TNLean/Analysis/EntropyMarkovForward.lean`; `TNLean/Channel/PetzProductReference.lean`; `TNLean/MPS/MPDO/ActiveSectorSpanningAreaLaw.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
| Partial | CPSV16 | Theorem 4.14 | The conditional Gram and unitary conclusions are formalized, while the tensor-attached marked comparison and CPTP maps remain open. Terminal positivity excludes the generic nonunitary-similarity witness from retained positive sectors. The explicit injective one-block `SameMPV` pair has a nonpositive generic per-block extension. Thus injectivity and closed-chain equality alone cannot supply the positivity needed for unitary implementation. The concrete nonunitary bond-two deformation has a genuine `BNTAlgebraTensorClause` and literal horizontal CPSV canonical form, but its length-two operator is not positive, so it fails `IsMPDO`. Neither example is a full-contract counterexample or constructs an `IdentityMarkedRealization` | `TNLean/MPS/MPDO/PositiveLinearExtensionCounterexample.lean`; `TNLean/MPS/MPDO/BondTwoSingletonPhysicalGauge.lean`; #5284 under #4648; #4645 remains open |
