...
| **Example 4.12** (unlabeled, l.932) | 932–939 | The toric-code boundary tensor is claimed to be SAL, ZCL, and RFP, but not of the simple GSNNCH form | `TNLean/MPS/MPDO/CPSVExample412Literal.lean`; `TNLean/MPS/MPDO/CPSVExample412NormalizedRFP.lean`; Blueprint `thm:cpsv_example412_literal_formula`, `thm:cpsv_example412_literal_sal`, `thm:cpsv_example412_literal_zcl_rfp_gap`, and `thm:cpsv_example412_normalized_rfp_maps`; issues #5919, #6044, and #6045 | **partially resolved; literal error and normalized channel equations complete** — the printed formula, positivity, one-site trace loss, and SAL are complete. Its physical-trace transfer satisfies $\mathcal T_M^2=2\mathcal T_M\ne\mathcal T_M$, so the printed representative fails literal Definition 4.2 ZCL and cannot satisfy the trace-preserving channel equations. For $\widehat M=(1/2)M$, the positive-length periodic family is normalized and explicit parity Kraus maps prove the bare two-channel predicate without added hypotheses. The two horizontal normal-block weights of $\widehat M$ have modulus $1/\sqrt2$, so this result does not supply the line-246 unit-weight convention. The full exclusion from GSNNCH remains open in #6045; parity across different lengths is not a proof because the commuting realization may depend on $N$ |
...
| Partial and partly refuted | CPSV16 | Theorem 4.9 | Implication \((i)\Rightarrow(ii)\), HJPW/Hayashi equality, Petz recovery, and sectorwise injective+MPDO+SAL+literal-ZCL inheritance are complete. The printed absorbed-normality bridge is refuted, leaving (ii)$\Rightarrow$(iv) as a missing statement with proof-path drift. Separately, the exact scalar repeated-copy capstone gives explicit component witnesses for every standing hypothesis, condition (iv), and MPDO positivity of the blocked tensor, but fails (v), so literal (iv)$\Rightarrow$(v) is a refuted unfaithful statement. The viable source (ii)$\Rightarrow$(v) route remains missing after ZCL-derived common-weight absorption and projector-controlled channel assembly | `TNLean/MPS/MPDO/BNTSectorAreaLaw.lean`; `TNLean/MPS/MPDO/CaseIIAbsorptionCounterexample.lean` (`printed_absorbed_normality_step_is_false`, `printed_theorem49_iv_to_v_is_false`); `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
