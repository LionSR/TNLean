...
| **Theorem 4.14** (source label `thm:IV.13`, l.972) | 972–993; proof 1929–2088 | RFP iff the tensor-attached BNT algebra condition iff the fusion-isometry condition | `TNLean/MPS/MPDO/BNTAlgebraTensorClause.lean`; `BNTAlgebraTensorClausePositivity.lean`; `BNTAlgebraTensorClauseSpectrum.lean`; `BNTFusionTensorClauseFromRFP.lean`; `PositiveMinimalRealizationCounterexample.lean`; `AlgebraFusionCounterexample.lean` | **partial** — several directions and the tensor-attached data are complete. Terminal sector positivity is now a formal necessary condition and excludes the existing generic nonunitary-similarity witness from the full tensor-attached setting. The marked/common-target comparison remains open in #5284, followed by unitary normalization, the CPTP construction, and #3949; Proposition 4.13 does not supply that comparison |
...
| Restatement/proof of Theorem 4.14 | 1929–2088 | Algebra and fusion characterizations of RFP | `TNLean/MPS/MPDO/BNTAlgebraTensorClause.lean`; `BNTAlgebraTensorClausePositivity.lean`; `BNTAlgebraTensorClauseSpectrum.lean`; `BNTFusionTensorClauseFromRFP.lean`; `PositiveMinimalRealizationCounterexample.lean`; `AlgebraFusionCounterexample.lean` | **partial restatement** — terminal sector positivity excludes the existing generic nonunitary-similarity witness, but the tensor-attached marked comparison remains open in #5284; unitary normalization and the CPTP construction still follow |
...
| Partial | CPSV16 | Theorem 4.14 | Algebra⇒RFP still needs the tensor-attached Gram comparison, unitary normalization, and CPTP maps; terminal sector positivity excludes the existing generic nonunitary-similarity witness but does not close the gap | #5284, followed by unitary normalization and the CPTP construction |
