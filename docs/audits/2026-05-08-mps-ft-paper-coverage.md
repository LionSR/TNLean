...
| **Definition 4.7** (l.821) | 815–822, with standing convention 217–246 | A simple MPDO has no nilpotent element in a blocked BNT | `TNLean/MPS/MPDO/SourceSimpleTensor.lean` (`MPOTensor.IsSourceSimple`, `MPOTensor.activeBNT_basis_not_isNilpotent_iff`, `MPOTensor.IsSourceSimple.exists_mpo_ne_zero`, `MPOTensor.IsNonvanishingSourceSimple`, `MPOTensor.IsSimple.isSourceSimple`, `MPOTensor.IsSimple.isNonvanishingSourceSimple`); `ActiveBNTRefinement.lean` (`MPSTensor.IsActiveCPSVBasisOfNormalTensors`, `equiv_of_sameMPV₂Pos`); `SimpleTensor.lean` (gauge-phase invariance of doubled-transfer nilpotency and `MPOTensor.IsSimple`); `SimpleVanishingCounterexample.lean` (`MPOTensor.SimpleVanishingCounterexample.M_isSimple`, `M_isSourceSimple`, `M_not_isNonvanishingSourceSimple`); Blueprint `def:mpdo_source_simple_tensor` and `thm:mpdo_simple_vanishing_source_simple_status`; `docs/paper-gaps/cpsv16_simple_tensor_nilpotency.tex`; `docs/paper-gaps/cpsv16_bnt_uniqueness_zero_coefficient.tex` | **complete for the documented active canonical-block reading, with a separate strengthened predicate** — the literal BNT definition admits dormant candidates and does not support the unrestricted uniqueness sentence at line 1148. `IsSourceSimple` records the intended nonzero canonical-block construction by existentially choosing a positive blocking and an active BNT sector presentation. Every representative has a positive number of copies and all copy weights are nonzero, so dormant candidates are absent; active representatives are unique up to permutation, dimension identification, gauge, and phase, under which doubled-transfer nilpotency is invariant. Positive-length nontriviality follows from the active presentation and is not a defining hypothesis. `IsNonvanishingSourceSimple` strengthens this to $\rho_N(M)\ne0$ for every $N>0$. Normalized fixed-representative simplicity implies source simplicity because its normalized sector decomposition gives an active presentation after gauge transport. The bond-two witness has $\rho_2(M)=2I\ne0$ but $\rho_1(M)=0$, so it is normalized-simple and source-simple after blocking two sites but fails the strengthened predicate. This is not a counterexample to the active reading of Definition 4.7 and is not an RFP or coefficient-rigidity claim |
...
| **Theorem 4.9** (l.851, `thm:main-simple`) | 851–893 | Simple-MPDO implication chain \((i)\Rightarrow(ii)\Leftrightarrow(iii)\Rightarrow(iv)\Rightarrow(v)\) | Blueprint `thm:cpsv_theorem49_printed_status` and `thm:mpdo_theorem49_iv_to_v_counterexample`; `TNLean/MPS/MPDO/RFPViaTSSAL.lean`; `TNLean/MPS/MPDO/CyclicActiveAreaLaw.lean`; `TNLean/MPS/MPDO/BNTSeparatingProjectors.lean`; `TNLean/MPS/MPDO/BNTSourceSectorProjectors.lean`; `TNLean/MPS/MPDO/BNTSectorAnalyticProperties.lean`; `TNLean/MPS/MPDO/BNTSectorAreaLaw.lean`; `TNLean/MPS/MPDO/CaseIIAbsorptionCounterexample.lean`; `TNLean/MPS/MPDO/Theorem49RepeatedCopyCounterexample.lean`; `TNLean/Analysis/EntropyMarkovForward.lean`; `TNLean/Channel/PetzProductReference.lean` | **partial and partly refuted** — implication \((i)\Rightarrow(ii)\) is complete at the explicit density-family boundary, and the HJPW/Hayashi and Petz-recovery constructions are complete. The explicit two-sector bond-one construction refutes the printed claim that coefficient absorption preserves normality, so the (ii)$\Rightarrow$(iv) proof has proof-path drift. The remaining source assertion is the existence of some rank-one neighboring-trace factorization for every common-weight absorbed representative, not rank one of the particular inverse-map sectors. Independently, the scalar one-representative tensor with raw copy weights $1,1/2$ has explicit component witnesses for the exact BNT canonical assembly, global unit weight, MPDO and simplicity hypotheses, biCF one-letter span, every clause of (iv), and MPDO positivity after two-site blocking, while that block fails Definition 4.1. Thus the literal (iv)$\Rightarrow$(v) is a refuted unfaithful statement. The viable source (ii)$\Rightarrow$(v) route remains not ready: #6775 retains the printed factorization question, #6793 records the possible direct blocked-channel route, and #6632 records the later outer-sector combination |
| **Example 4.10** (`ExZCLnoSAL`, l.898) | 898–906 | A mixed state has ZCL but does not saturate the area law | Blueprint `thm:cpsv_examples410_412_status`; `TNLean/MPS/MPDO/CPSVExamples410411Arithmetic.lean`; `docs/paper-gaps/cpsv16_examples_4_10_4_11_entropy.tex`; issue #6301 | **source reconciled; scalar endpoints complete; formalization pending** — the printed channel repeats the left-qubit label; the corrected left-right flip reproduces the printed entropies. The exact integer comparison and log-ratio positivity are certified, while the spectra, entropy identity, tensor link, and final $I_2>I_1$ theorem remain open. The printed exception list also misses $p=1$ |
...
| Prop `prop2to3` | 1740–1743 | SAL and ZCL imply the blockwise structural form | `TNLean/MPS/MPDO/BNTSeparatingProjectors.lean`; `BNTSourceSectorProjectors.lean`; `SimpleTensor.lean` (`MPOTensor.weighted_basis_physTraceTransfer_sq_of_literal_ZCL`); `BNTSectorAnalyticProperties.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_physTraceTransfer_sq_of_literal_ZCL`); `BNTSectorAreaLaw.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_caseII_properties_of_literal_ZCL`); `ActiveSectorSpanningCounterexample.lean` (`MPOTensor.ActiveSectorSpanningCounterexample.exists_oneSector_neighboringTraceFactorization`); `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **not-ready** — the separating projectors and the full sectorwise analytic inheritance are formalized: every absorbed BNT representative is injective, MPDO-positive, SAL, and literally physical-trace idempotent. The printed spectral-normality step after coefficient absorption is refuted by the explicit two-sector bond-one construction: absorption gives transfer radius \(1/2\) in its first sector. The selected four-sector inverse-map factorization in the active-trace witness is nonminimal: the same tensor now has a proved one-sector factorization with a positive trace-one neighboring operator. The remaining missing statement is existence of some physical-sector factorization with normalized rank-one neighboring traces for every absorbed representative. This obstruction is distinct from the separate repeated-copy counterexample to (iv)$\Rightarrow$(v) |
...
| Prop `prop2to5` | 1810–1813 | SAL and ZCL give the two trace-preserving maps | `TNLean/MPS/MPDO/PhysicalSectorCoarseGrainingIdentity.lean`; `PhysicalSectorRefinementIdentity.lean`; `PhysicalSectorBlockedRFP.lean`; `ActiveSectorSpanningRFP.lean`; `BNTSectorAreaLaw.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_caseII_properties_of_literal_ZCL`); `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **not-ready, but not refuted by the raw-copy example** — this proposition starts from condition (ii). The earlier Case-II derivation uses ZCL to make copy weights independent of the copy label and absorb their common value. The conditional one-factor maps are constructed. Issue #6775 retains the printed per-representative factorization assertion. Issue #6793 records the alternative possibility of constructing each blocked channel pair by a genuine coarsening or sector-mixing argument. Once either route supplies the channel pairs, their projector-controlled outer-sector combination remains #6632 |
...
| Partial and partly refuted | CPSV16 | Theorem 4.9 | Implication \((i)\Rightarrow(ii)\), HJPW/Hayashi equality, Petz recovery, and sectorwise injective+MPDO+SAL+literal-ZCL inheritance are complete. The printed absorbed-normality inference is refuted under all conditions used in Appendix C.2 at the normalized fixed-representative boundary, leaving (ii)$\Rightarrow$(iv) as a missing statement with proof-path drift. Separately, the exact scalar repeated-copy theorem gives explicit component witnesses for every standing hypothesis, condition (iv), and MPDO positivity of the blocked tensor, but fails (v), so literal (iv)$\Rightarrow$(v) is a refuted unfaithful statement. The viable source (ii)$\Rightarrow$(v) route remains missing after ZCL-derived common-weight absorption and projector-controlled channel assembly | `TNLean/MPS/MPDO/BNTSectorAreaLaw.lean`; `TNLean/MPS/MPDO/CaseIIAbsorptionCounterexample.lean` (`ambient_isSAL_isSimpleCanonicalForm_and_firstAbsorbed_not_isNormalTensor`); `TNLean/MPS/MPDO/Theorem49RepeatedCopyCounterexample.lean` (`printed_theorem49_iv_to_v_is_false`); `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
...
| Not-ready | CPSV16 | Proposition `prop2to3` | Sectorwise injectivity, MPDO positivity, SAL, and literal physical-trace idempotence are complete for every absorbed BNT representative. The explicit bond-one construction refutes preservation of spectral normality under coefficient absorption. The active-trace witness also proves that its selected four-sector inverse-map factorization is nonminimal by exhibiting a one-sector positive trace-one factorization. The remaining assertion is existence of some such factorization for every absorbed representative; it is not a rank-one claim about a prescribed inverse-map decomposition | `TNLean/MPS/MPDO/CaseIIAbsorptionCounterexample.lean`; `TNLean/MPS/MPDO/ActiveSectorSpanningCounterexample.lean` (`MPOTensor.ActiveSectorSpanningCounterexample.exists_oneSector_neighboringTraceFactorization`); `TNLean/MPS/MPDO/SimpleTensor.lean` (`MPOTensor.weighted_basis_physTraceTransfer_sq_of_literal_ZCL`); `BNTSectorAnalyticProperties.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_physTraceTransfer_sq_of_literal_ZCL`); `BNTSectorAreaLaw.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_caseII_properties_of_literal_ZCL`); issue #6775 |
...
| Not-ready | CPSV16 | Proposition `prop2to5` | This is the viable source (ii)$\Rightarrow$(v) route, not the refuted raw (iv)$\Rightarrow$(v) claim. After the earlier ZCL derivation makes repeated-copy weights common and absorbs them, #6775 retains the printed factorization route and #6793 records a possible direct coarsening or sector-mixing construction of the blocked maps. The later projector-controlled combination of per-representative channel pairs remains #6632 | `TNLean/MPS/MPDO/PhysicalSectorBlockedRFP.lean`; `TNLean/MPS/MPDO/PhysicalSectorPhysicalTransport.lean`; `TNLean/MPS/MPDO/CaseIIAbsorptionCounterexample.lean`; `TNLean/MPS/MPDO/BNTSectorAreaLaw.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
