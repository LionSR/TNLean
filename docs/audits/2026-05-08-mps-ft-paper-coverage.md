...
| Corollary | 1503–1506 | SAL and ZCL imply the displayed structural form | `TNLean/MPS/MPDO/InverseMapActiveSectorPrimitivity.lean`; direct corrected rank-one criteria in `SimpleLocalStructure.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **not-ready** — the source inverse-map construction reaches a primitive active trace matrix, but the only printed derivation of its rank-one factorization uses false Lemma C.5; the direct corrected criteria require additional hypotheses and do not prove the corollary |
