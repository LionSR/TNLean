...
| Lemma `SALZCL` / Lemma C.5 | 1484–1502 | SAL and ZCL force the active trace matrix to have rank one | `TNLean/MPS/MPDO/InverseMapLemmaC5CaseI.lean` (`MPOTensor.exists_activeSectorTraceMatrix_rank_one_coefficients_of_isSAL_of_literal_ZCL`); `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **partial; supported-sector theorem complete** — under the standing normal Case-I hypotheses, injectivity, normality, SAL, and literal physical-trace idempotence give normalized rank-one coefficients on the nonzero-weight Hayashi sectors. The source-labelled full-sector statement still requires extension across TNLean's ambient zero-weight filler sectors. This neither replaces literal ZCL by scale-invariant `IsSourceZCL` nor gives a Case-II conclusion. The raw four-sector tensor remains outside the normal Case-I hypotheses, and its normal representative loses literal ZCL |
| Corollary | 1503–1506 | SAL and ZCL imply the displayed structural form | `TNLean/MPS/MPDO/PhysicalSectorFactorization.lean`; `InverseMapLemmaC5CaseI.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **not-ready** — the normal Case-I rank-one coefficients are available only on the active Hayashi sector. A declaration still must extend them across the zero-weight sectors and combine them with the physical isometry and left--right tensors in the all-sector structural statement printed in the corollary |
...
| Prop `prop2to3` | 1740–1743 | SAL and ZCL imply the blockwise structural form | `TNLean/MPS/MPDO/BNTSeparatingProjectors.lean`; `BNTSourceSectorProjectors.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **not-ready** — the separating projectors are formalized, but no theorem yet proves that every BNT tensor inherits MPDO positivity, SAL, and literal ZCL and then supplies the all-sector Case-I structural datum. The normal Case-I rank-one theorem does not by itself provide this Case-II passage |
...
| Partial | CPSV16 | Lemma C.5 (`SALZCL`) | The supported-sector theorem is complete: under the standing normal Case-I hypotheses, SAL and literal physical-trace idempotence give normalized rank-one coefficients on the nonzero-weight Hayashi sectors. The source-labelled full-sector statement still requires extension across TNLean's ambient zero-weight fillers. The theorem does not extend to scale-invariant `IsSourceZCL` or to Case II | `TNLean/MPS/MPDO/InverseMapLemmaC5CaseI.lean` (`MPOTensor.exists_activeSectorTraceMatrix_rank_one_coefficients_of_isSAL_of_literal_ZCL`); normalization boundary audited in `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
| Not-ready | CPSV16 | Structural corollary, lines 1503–1506 | The active Case-I coefficients must still be extended across zero-weight sectors and combined with the physical isometry and left--right tensors in one all-sector statement | `TNLean/MPS/MPDO/PhysicalSectorFactorization.lean`; `InverseMapLemmaC5CaseI.lean` |
...
| Not-ready | CPSV16 | Proposition `prop2to5` | Conditional maps exist, but the printed implication still depends on the unformalized all-sector BNT datum required by Proposition `prop2to3` | No unconditional source theorem available |
