...
| Prop `prop3to4` | 1786–1796 | Blockwise structure gives the GSNNCH form | `TNLean/MPS/MPDO/CommutingFormBridge.lean` (`EtaLocalStructureData.hasGSNNCHForm`); `GSNNCHOrthogonalSectors.lean` (`hasGSNNCHForm_of_orthogonalCommutingSectorFamily`, `hasGSNNCHForm_of_commonWeightAbsorbedBasisMPOTensor`); `BNTPhysicalSectorGSNNCH.lean` (`hasGSNNCHForm_of_bntLayerOrthogonal_of_physicalSectorFactorization`) | **partial** — the one-sector positive commuting product and the generic orthogonal outer-sector sum are proved with natural multiplicities. The new capstone derives the supported sector family from the five printed identities when the BNT copy weights are independent of the copy index. This extra hypothesis comes from `lemmus` under source ZCL and is absent from the printed proposition, so the unrestricted implication remains open |
...
| Partial | CPSV16 | Proposition `prop3to4` | The five printed identities give the GSNNCH form under the additional common-copy-weight hypothesis; the paper obtains that hypothesis from `lemmus` using source ZCL, but it is absent from the proposition statement | #5065, assigned to `LionSR` |
