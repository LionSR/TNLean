...
| **Definition 4.2** (`DefinitionZCL`, l.736) and its correlation consequence | 735–742, using `Corr` at 490–496 | ZCL is literal idempotence of the physical-trace transfer; the following prose says that MPDO correlation functions are length-independent | Blueprint `def:mpo_physical_trace_idempotence`, `def:mpdo_periodic_two_point_correlation`, `thm:mpdo_zcl_periodic_correlation_positive_wrap_gaps_independent`, and `thm:mpdo_zcl_periodic_correlation_positive_gaps_independent`; `TNLean/MPS/MPDO/ZCL.lean` (`MPOTensor.IsPhysicalTraceIdempotent`, `MPOTensor.isPhysicalTraceIdempotent_iff`); `TNLean/MPS/MPDO/Correlations.lean` (`MPOTensor.periodicTwoPointCorrelation`, `MPOTensor.periodicTwoPointCorrelation_positiveWrapGaps_independent`, `MPOTensor.periodicTwoPointCorrelation_positiveGaps_independent`); `docs/paper-gaps/cpsv16_mpdo_zcl_correlation_length_boundary.tex` | **scope-restricted completion at the documented positive-varied-gap boundary** — the named predicate is exactly $\mathcal T_M^2=\mathcal T_M$. The unnormalized correlation is $\tr(\mathbb E_{O_1}\mathcal T_M^{g_{\mathrm{mid}}}\mathbb E_{O_2}\mathcal T_M^{g_{\mathrm{wrap}}})$ with the `Corr` orientation. At fixed arbitrary middle gap, literal idempotence gives ring-length independence between any two positive wrapping gaps; this includes adjacent marked sites when the unchanged middle gap is zero. Separately, it gives fixed-ring distance independence for observables nonadjacent on both complementary arcs when all four compared gaps are positive and the two gap sums agree. The unrestricted claim comparing a zero varied gap with a positive one is not made because idempotence does not identify $\mathcal T_M^0$ with $\mathcal T_M$. The nonzero up-to-scalar `IsSourceZCL` relation remains separate and is not mapped to the source label |
| **Definition 4.3** (`def:Puri-RFP`, l.758) | 744–764 | Purification RFP from the one-site ancillary-contraction presentation | Blueprint `def:mpdo_is_prfp`; `TNLean/MPS/MPDO/PRFP.lean` (`MPOTensor.IsPRFP`); `TNLean/MPS/MPDO/LocalPurificationRFP.lean` (`MPOTensor.IsPRFP.hasPurificationRFPWitness`) | **complete under the intended-presentation convention** — `IsPRFP` records the local tensor presentation introduced at `Psipuri` and requires its purifying tensor to be a pure-state RFP. The positive-length global purification equation is a proved consequence, not the definition by itself |
| **Theorem 4.4** (unlabeled theorem environment, l.777; counted) | 777–784 | Puri-RFP iff ZCL and the stated purification form | Blueprint `thm:prfp_physical_trace_idempotent` and `thm:cpsv_theorem44_printed_status`; `TNLean/MPS/MPDO/LocalPurificationRFP.lean` (`MPOTensor.IsPRFP.isPhysicalTraceIdempotent`, `MPOTensor.isPRFP_iff_isLPDO_and_physTraceTransfer_sq`); `docs/paper-gaps/cpsv16_purification_rfp_definition.tex` | **partial** — on the intended one-site presentation, the equivalence between PRFP and physical-trace idempotence is complete without a nonzero assumption. The equivalence with the repeated-copy density form remains not ready, and no converse is claimed from the weaker bare global-family equation. The nilpotent $Q$ example separates that global-family equality from the local presentation and is not a source counterexample |
...
| **Theorem 4.9** (l.851, `thm:main-simple`) | 851–893 | Simple-MPDO implication chain \((i)\Rightarrow(ii)\Leftrightarrow(iii)\Rightarrow(iv)\Rightarrow(v)\) | Blueprint `thm:cpsv_theorem49_printed_status`, `thm:mpdo_bond_one_physical_sector_factorization`, `thm:mpdo_sal_zcl_bnt_sector_structure`, and `thm:mpdo_theorem49_iv_to_v_counterexample`; `TNLean/MPS/MPDO/NeighboringTraceObstructionAmbientCounterexample.lean`; `TNLean/MPS/MPDO/Theorem49RepeatedCopyCounterexample.lean`; `TNLean/Analysis/EntropyMarkovForward.lean`; `TNLean/Channel/PetzProductReference.lean` | **partial and refuted in two directions** — implication \((i)\Rightarrow(ii)\) is complete at the explicit density-family boundary, and the HJPW/Hayashi and Petz-recovery constructions are complete. At virtual bond dimension one, SAL and literal physical-trace idempotence give a normalized one-sector neighboring-trace factorization. In arbitrary virtual dimension, the five-letter ambient witness has a two-representative BNT presentation in biCF. It satisfies the complete MPDO, SAL, literal-ZCL, simple-canonical, and line-246 global-normalization hypotheses, while its first absorbed representative has no normalized rank-one neighboring-trace factorization. Thus the printed representativewise assertion in (ii)$\Rightarrow$(iv) is false. Complementary one-site supports give the ambient SAL identity, virtual-gauge invariance transfers a hypothetical factorization to the embedded obstruction, and isometric descent returns it to the four-letter tensor. Independently, the scalar repeated-copy witness refutes literal (iv)$\Rightarrow$(v). |
...
| Prop `prop2to3` | 1740–1743 | SAL and ZCL imply the blockwise structural form | `NeighboringTraceObstructionAmbientCounterexample.lean` (`MPOTensor.NeighboringTraceObstructionAmbientCounterexample.printed_theorem49_ii_to_iv_is_false`); `PhysicalIsometricEmbeddingNeighboringTrace.lean`; <https://sirui-lu.com/QICLean/paper-gaps/cpgsv17_pf_rank_one.pdf> | **false as printed** — the five-letter ambient tensor has normalized BNT weights $(\mu,1)$ with $0<|\mu|<1$, a two-representative BNT presentation in biCF, simple canonical form, MPDO positivity, SAL, literal physical-trace ZCL, and complementary physical supports. Its first absorbed representative has no normalized rank-one neighboring-trace factorization. Virtual-gauge invariance and isometric physical descent reduce any hypothetical factorization to the proved four-letter obstruction. |
...
| Prop `prop2to5` | 1810–1813 | SAL and ZCL give the two trace-preserving maps | `TNLean/MPS/MPDO/CPSVBlockingChannelCounterexample.lean`; `CPSVBlockingChannelEmbeddedCounterexample.lean`; `CPSVBlockingChannelAmbientCounterexample.lean` (`MPOTensor.CPSVBlockingChannelCounterexample.printed_propositionC15_and_theorem49_ii_to_v_are_false`); `docs/paper-gaps/cpsv16_prop_c15_blocking_channel_false.tex` | **false as printed** — the complete five-letter simple-biCF MPDO witness satisfies SAL, literal ZCL, and global normalization. A Hermitian virtual-boundary closure has trace norm $337/250$ at length two and $27/20$ at lengths three and four. Trace-norm contractivity, sitewise physical recovery, and a single ambient boundary valid at every length exclude the asserted two-to-three map and its twice-applied two-to-four refinement. The blocked tensor is not an RFP. Conditional channel constructions from supplied factorizations remain valid. |
...
| Paper label | Lines | Paper description | Lean location | Status |
|---|---|---|---|---|
| Alternative pure RFP up to virtual gauge (`RFP-gauge`) | 2094–2107 | Blocking is allowed to return the tensor up to a virtual gauge; the paper claims equivalence with ordinary pure RFP | Blueprint `thm:cpsv_pure_rfp_gauge_status` (the printed-status stub `thm:cpsv_rfp_gauge_printed_status` was removed in favor of this `\leanok` refutation node); `TNLean/MPS/RFP/PhaseOscillation.lean` supplies the cube-phase obstruction, and `TNLean/MPS/RFP/GaugeBlockingCounterexample.lean` formalizes the exact source-shaped gauge identity and counterexample; issue #5920 is closed | **false as printed; exact refutation complete** — the cube-phase tensor blocks to itself up to swap gauge-phase but has non-idempotent transfer, so the printed spectral argument fails |
...
| Proposition `prop-inj` (l.911) | 911–936 | Condition C1 forces a single canonical block and gives reduced-density rank exactly $D^2$ across every cut with both sides of length at least $L_0$ | `TNLean/MPS/CanonicalForm/PGVWC07CanonicalForm.lean` (`PGVWC07CanonicalFormData.not_isNBlkInjective_of_ne`, `PGVWC07CanonicalFormData.exists_weight_sum_mul_conjTranspose_eq_of_isNBlkInjective`); `TNLean/MPS/CanonicalForm/FixedLengthIntertwiner.lean` (`linearIndependent_leftCutVector`, `linearIndependent_rightCutVector`); `TNLean/Wielandt/SpanGrowth/CumulativeSpan.lean` (`isNBlkInjective_of_le`) | **partial** — the single-block conclusion is packaged for the block form of Theorem `Th:TIcanonical`, and the cut vectors of the proof are linearly independent across every cut with both sides of length at least $L_0$; the reduced-density-rank statement itself is not packaged |
...
| **Theorem `thm-uniq`** (l.1002) | 1002–1015 | Fixed-length uniqueness of a TI canonical representation under C1, uniqueness of its OBC canonical form, and $N>2L_0+D^4$, with a unitary conjugacy conclusion | `TNLean/MPS/CanonicalForm/TranslationInvariantUniqueness.lean` (`MPSTensor.pgvwc07_uniqueness_of_canonical_form`, `_rephased`, `_unitGaugePhaseEquiv`; single-block cases `pgvwc07_uniqueness_of_unital`, `pgvwc07_uniqueness_of_weighted`) | **complete in corrected form** — the printed exact-conjugacy conclusion is false on a finite ring (`docs/paper-gaps/pgvwc07_quadratic_reconstruction_phase.tex`); the formal theorem concludes $B_i=\xi UC_iU^\dagger$ with $\lvert\xi\rvert=1$ and $\xi^N=1$ for two canonical representations in the block form of Theorem `Th:TIcanonical` (`MPSTensor.IsPGVWC07CanonicalForm`) of the same nonzero length-$N$ state under C1 and $N>2L_0+D^4$; C1 forces the single block of $B$, and the invertible intertwiner transports it to $C$. The OBC-uniqueness hypothesis is not consumed: the intertwiner chain is obtained from the cut vectors of Proposition `prop-inj` (`docs/paper-gaps/pgvwc07_ti_uniqueness_scope.tex`) |
...
| Partial | CPSV16 | Purification RFP equivalence | Under the intended one-site presentation, the equivalence between PRFP and literal physical-trace idempotence is complete. Only equivalence with the repeated-copy density form remains not ready; nilpotent hidden sectors show only that positive-length global-family equality does not recover the local presentation | `TNLean/MPS/MPDO/LocalPurificationRFP.lean`; `docs/paper-gaps/cpsv16_purification_rfp_definition.tex` |
...
| Partial and refuted in three directions | CPSV16 | Theorem 4.9 | Implication \((i)\Rightarrow(ii)\), HJPW/Hayashi equality, and Petz recovery are complete. The five-letter ambient witness satisfies the full condition-(ii) hypotheses and line-246 normalization. Its first absorbed representative has no normalized rank-one neighboring-trace factorization, refuting the printed representativewise assertion in (ii)$\Rightarrow$(iv). The same ambient witness has no two-to-three or two-to-four refinement channel, so its two-site blocking is not an RFP; this refutes Proposition C.15 and the corresponding condition-(ii) route to (v). Separately, the scalar repeated-copy witness refutes literal (iv)$\Rightarrow$(v). | `TNLean/MPS/MPDO/NeighboringTraceObstructionAmbientCounterexample.lean` (`printed_theorem49_ii_to_iv_is_false`); `TNLean/MPS/MPDO/CPSVBlockingChannelAmbientCounterexample.lean` (`printed_propositionC15_and_theorem49_ii_to_v_are_false`); `TNLean/MPS/MPDO/Theorem49RepeatedCopyCounterexample.lean` (`printed_theorem49_iv_to_v_is_false`); `docs/paper-gaps/cpsv16_prop_c15_blocking_channel_false.tex`; <https://sirui-lu.com/QICLean/paper-gaps/cpgsv17_pf_rank_one.pdf> |
...
| Refuted as printed | CPSV16 | Proposition `prop2to3` | The five-letter ambient tensor has normalized BNT weights $(\mu,1)$ with $0<|\mu|<1$, a two-representative BNT presentation in biCF, simple canonical form, MPDO positivity, SAL, literal physical-trace ZCL, and complementary supports. Its first absorbed representative has no normalized rank-one neighboring-trace factorization. | `TNLean/MPS/MPDO/NeighboringTraceObstructionAmbientCounterexample.lean` (`MPOTensor.NeighboringTraceObstructionAmbientCounterexample.printed_theorem49_ii_to_iv_is_false`); <https://sirui-lu.com/QICLean/paper-gaps/cpgsv17_pf_rank_one.pdf> |
...
| Refuted as printed | CPSV16 | Proposition `prop2to5` | The complete ambient witness satisfies the source's condition-(ii) standing hypotheses, but trace-norm contractivity excludes every two-to-three refinement channel with the asserted arbitrary-boundary action. The same calculation at length four proves that its two-site block is not an RFP. Conditional constructions from supplied representative factorizations remain valid. | `TNLean/MPS/MPDO/CPSVBlockingChannelAmbientCounterexample.lean` (`MPOTensor.CPSVBlockingChannelCounterexample.printed_propositionC15_and_theorem49_ii_to_v_are_false`); `docs/paper-gaps/cpsv16_prop_c15_blocking_channel_false.tex` |
...
| Complete (corrected form) | PGVWC07 | Theorem `thm-uniq` | The source fixed-length theorem assumes C1, uniqueness of the OBC canonical form, and $N>2L_0+D^4$, and concludes exact unitary conjugacy, which is false on a finite ring. The formal theorem keeps C1 and the size bound, drops the unconsumed OBC-uniqueness hypothesis, and concludes unitary conjugacy up to an $N$th-root phase for two canonical representations in the block form of Theorem `Th:TIcanonical` of the same nonzero state | `MPSTensor.pgvwc07_uniqueness_of_canonical_form` |
