...
| Missing statement; conditional helper; printed route fails | CPSV16 | Proposition `prop2to5` | The representative-channel and outer-sector constructions are proved for supplied factorizations. The source proposition remains `\notready` with no Lean declaration: its printed route uses the false Proposition `prop2to3`, while the bare conclusion is neither proved nor refuted. | `TNLean/MPS/MPDO/PhysicalSectorBlockedRFP.lean`; `TNLean/MPS/MPDO/PhysicalSectorPhysicalTransport.lean`; `TNLean/MPS/MPDO/NeighboringTraceObstructionAmbientCounterexample.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
