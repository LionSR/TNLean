...
| Defn Puri-RFP (l.758) | 758–764 | Purification RFP | `TNLean/MPS/MPDO/PRFP.lean` (`MPOTensor.HasGlobalPurificationEquation`, `MPOTensor.HasPurificationRFPWitness`, `MPOTensor.IsPRFP`) records the global purification equation `eq:MPDO-Puri-1` and the pure-state RFP condition on the purifying spin-ancilla tensor. The trace-preserving-map structure discussed after tracing ancillas is recorded separately as `MPOTensor.IsPRFPWithTracePreservingSpinReduction`. `MPOTensor.IsLocalPurificationRFP` remains only a weaker local tensor condition. See `docs/paper-gaps/cpsv16_purification_rfp_definition.tex`. | **partial** — the definition surface is formalized; the theorem `PRFP ↔ ZCL` remains open and is tracked by #2421. |
