...
| **Theorem 3.10** (l.534, `thm:main-MPS`) | 534–541 | **RFP ⇔ ZCL ⇔ NNCPH** (three-way equivalence) | `TNLean/MPS/RFP/Assembly.lean` (`rfp_iff_zcl` for RFP↔ZCL); `ParentHamiltonian/Commuting.lean` (`rfp_implies_nncph`, `nncph_implies_rfp`) | **partial** — `rfp_implies_nncph` uses `Axioms.rfp_to_nncph_commute` (axiom-backed); `nncph_implies_rfp` now consumes `HasNNCPHGroundSpaces` but still uses `Axioms.beigi_nncph_to_rfp` (axiom-backed). Tracked by issues #1484/#1485/#2633. |
...
| Defn Puri-RFP (l.758) | 758–764 | Purification RFP | No faithful Lean definition yet. `MPOTensor.IsLocalPurificationRFP` in `TNLean/MPS/MPDO/LocalPurificationRFP.lean` records only the weaker local tensor condition: `IsLPDO M` together with the purifying tensor `A` being a pure-state RFP (`MPSTensor.IsRFP`). It proves `IsLocalPurificationRFP.isMPDO` via `IsLPDO.isMPDO`, but does not include the global purification equation or post-ancilla trace-preserving map required by the source. See `docs/paper-gaps/cpsv16_purification_rfp_definition.tex`. | **open** — the source PRFP definition and `PRFP ↔ ZCL` theorem remain to be formalized; tracked by #2421. |
