...
| Prop `prop4to2` | 1801–1804 | GSNNCH gives SAL | `TNLean/MPS/MPDO/OrthogonalSectorAreaLaw.lean`; `LocalOrthogonalSumAreaLaw.lean` (`MPOTensor.isSAL_of_proportionalLocalOrthogonalSum`); `CyclicActiveAreaLaw.lean`; `docs/paper-gaps/cpsv16_commuting_form_to_sal.tex` | **partial** — positive length-dependent proportionality of the global orthogonal-sector sum now cancels in the normalized states, so it is no longer a restriction. The printed statement still omits sectorwise ZCL. The restricted sector theorem also assumes per-sector MPDO, one-site injectivity, normality, and ZCL, together with an exact scalar-one sector realization stronger than the proportional source form. One-site injectivity is not implied merely by BNT normality, leaving an injectivity-versus-blocking gap in the source application |
...
| Partial | CPSV16 | Proposition `prop4to2` | Positive global proportionality of the orthogonal-sector sum is handled by `MPOTensor.isSAL_of_proportionalLocalOrthogonalSum`. The restricted sector theorem still assumes sectorwise MPDO, one-site injectivity, normality, and ZCL, plus an exact scalar-one sector realization stronger than the proportional source form; the source omits ZCL and leaves the injectivity-versus-blocking step unaddressed | Source statement requires correction |
