...
| **Proposition 4.13** (source label `Prop:IV.12`, l.945) | 945–952; proof 1863–1922 | Vertical CF: a tensor in literal CPSV CF is also in CF vertically, with the rectangular sector map and positive diagonal multiplicities | `TNLean/MPS/CanonicalForm/ActiveBNTRefinement.lean`; `TNLean/MPS/MPDO/SourceBNTBlocking.lean`; `TNLean/MPS/MPDO/CPSVRepresentativeGroupedLemmaL.lean`; stronger-hypothesis capstone `TNLean/MPS/MPDO/VerticalCanonicalForm.lean` | **partial** — active class-copy regrouping, retention of inactive zero-weight coordinates, the active representative sector decomposition with weights $\lambda_{j,q}=\nu_{k(j,q)}\zeta_{k(j,q)}$, eventual representative word-tuple span, positive-length MPV agreement, and retained-coordinate marked/first-site Lemma L are formalized. The theorem for normalized BNT-refined horizontal form is complete. Under literal CPSV CF, passage through the listed gauge and ambient coisometry to the physical vertical tensor, the physical Gram-conjugation step, and final coisometric assembly remain open; tracked by #4964 and the Proposition 4.13 gap note. |
