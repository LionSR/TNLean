...
| **Definition 4.7** (l.821) | 815–823 | A simple MPDO has no nilpotent element in a blocked BNT | `TNLean/MPS/MPDO/SourceSimpleTensor.lean` (`MPOTensor.IsSourceSimple`); `SimpleTensor.lean` (`MPOTensor.IsSimple`); `SimpleVanishingCounterexample.lean` (`MPOTensor.SimpleVanishingCounterexample.isSimple_and_not_isSourceSimple`); Blueprint `thm:mpdo_simple_not_source_simple_counterexample`; `docs/paper-gaps/cpsv16_unit_weight_rfp_scale_tension.tex` | **complete with explicit interface separation** — `IsSourceSimple` records the normalization-free source condition, including nonzero MPOs at every positive length. `IsSimple` is the normalized fixed-representative interface. The implication from the latter to the former is strictly conditional on positive-length nonvanishing: the compiled bond-two witness has $\rho_N(M)=(1+(-1)^N)I$, is normalized-simple after blocking two sites, and is not source-simple because $\rho_1(M)=0$. This is not an RFP or coefficient-rigidity claim |
