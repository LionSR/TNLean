...
| Partial | CPSV16 | Proposition `prop4to2` | Ambient source ZCL descends to the canonical absorbed simple-BNT sectors through copy independence and `MPOTensor.commonWeightAbsorbedBasisMPOTensor_isSourceZCL`; arbitrary GSNNCH presentations do not inherit representation-level ZCL. The proportional sector consumer is complete, but the all-original-cut visibility bridge from eventual injectivity remains open | #5208 |
