...
| **Definition 4.7** (l.821) | 815–822, with standing convention 217–246 | A simple MPDO has no nilpotent element in a blocked BNT | `TNLean/MPS/MPDO/SourceSimpleTensor.lean` (`MPOTensor.IsSourceSimple`, `MPOTensor.IsSourceSimple.exists_mpo_ne_zero`, `MPOTensor.IsNonvanishingSourceSimple`, `MPOTensor.IsSimple.isSourceSimple`, `MPOTensor.IsSimple.isNonvanishingSourceSimple`); `SimpleTensor.lean` (`MPOTensor.IsSimple`); `SimpleVanishingCounterexample.lean` (`MPOTensor.SimpleVanishingCounterexample.M_isSimple`, `M_isSourceSimple`, `M_not_isNonvanishingSourceSimple`); Blueprint `thm:mpdo_simple_vanishing_source_simple_status`; `docs/paper-gaps/cpsv16_unit_weight_rfp_scale_tension.tex` | **complete as the source definition, with a separate strengthened interface** — `IsSourceSimple` requires $\rho_N(M)\ne0$ for some $N>0$, excluding the identically zero family while permitting isolated vanishing lengths, and then imposes the blocked-BNT nonnilpotency condition. This existential clause does not in general exclude dormant extra BNT candidates. `IsNonvanishingSourceSimple` strengthens it to $\rho_N(M)\ne0$ for every $N>0$. Normalized fixed-representative simplicity implies source simplicity unconditionally via eventual linear independence, a non-eventually-zero sector coefficient, gauge transport, and unblocking. The bond-two witness has $\rho_2(M)=2I\ne0$ but $\rho_1(M)=0$, so it is normalized-simple and source-simple after blocking two sites but fails the strengthened predicate. This is not a counterexample to Definition 4.7 and is not an RFP or coefficient-rigidity claim |
...
| **Example 4.12** (unlabeled, l.932) | 932–939 | The toric-code boundary tensor is claimed to be SAL, ZCL, and RFP, but not of the simple GSNNCH form | `TNLean/MPS/MPDO/CPSVExample412Literal.lean`; Blueprint `thm:cpsv_example412_literal_formula`, `thm:cpsv_example412_literal_sal`, and `thm:cpsv_example412_literal_zcl_rfp_gap`; issues #5919, #6044, and #6045 | **partially resolved; literal normalization error proved** — the printed formula, positivity, one-site trace loss, and SAL are complete. Its physical-trace transfer satisfies $\mathcal T_M^2=2\mathcal T_M\ne\mathcal T_M$, so the printed representative fails literal Definition 4.2 ZCL and cannot satisfy Definition 4.1 RFP. The likely normalized tensor $(1/2)M$ and its channels remain open in #6044. The full exclusion from GSNNCH remains open in #6045; parity across different lengths is not a proof because the commuting realization may depend on $N$ |
...
| Prop `3to5` | 1510–1517 | The structural data give trace-preserving coarse-graining and refinement maps | `TNLean/MPS/MPDO/PhysicalSectorCoarseGrainingIdentity.lean`; `PhysicalSectorRefinementIdentity.lean`; `PhysicalSectorBlockedRFP.lean`; `PhysicalSectorPhysicalTransport.lean` (`NeighboringTraceFactorization.blockTwo_isRFPViaTS`); `docs/paper-gaps/cpgsv17_mpdo_blocked_rfp_physical_transport.tex`; `docs/paper-gaps/cpgsv17_mpdo_zero_weight_preparation_completion.tex`; `docs/paper-gaps/cpgsv17_mpdo_theorem_4_9_implication_label.tex`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **complete for one supplied factorization; all-BNT assembly missing** — the physical-transport theorem carries one sector-coordinate channel pair back to its original blocked tensor. Zero-weight preparation is completed on the quotient. Theorem 4.9(iv), however, supplies separate factorizations for the outer BNT elements; combining their channel pairs through the mutually orthogonal outer supports is not yet formalized, so this conditional theorem is not the full implication \((iv)\Rightarrow(v)\). The assembly is tracked in #6632 |
...
| Prop `prop2to5` | 1810–1813 | SAL and ZCL give the two trace-preserving maps | `TNLean/MPS/MPDO/PhysicalSectorCoarseGrainingIdentity.lean`; `PhysicalSectorRefinementIdentity.lean`; `PhysicalSectorBlockedRFP.lean`; `ActiveSectorSpanningRFP.lean`; `BNTSectorAreaLaw.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_caseII_properties_of_literal_ZCL`); `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **not-ready** — the conditional maps are constructed, and positivity, SAL, injectivity, and literal ZCL are complete for every absorbed BNT representative. Two distinct inputs remain: an all-sector structural factorization replacing the false absorbed-normality inference for \(\mu_s A_s\), tracked in #5996, and an outer-BNT projector-controlled assembly of the resulting per-element channel pairs into single trace-preserving maps, tracked in #6632 |
...
| Partial | CPSV16 | Theorem 4.9 | Implication \((i)\Rightarrow(ii)\), HJPW/Hayashi equality, Petz recovery, and sectorwise injective+MPDO+SAL+literal-ZCL inheritance are complete. The printed absorbed-normality bridge is formally refuted: \(\rho(\mathcal E_{\mu_s A_s})=|\mu_s|^2\rho(\mathcal E_{A_s})\), and the explicit first sector has radius \(1/2\). An alternative all-sector factorization and the outer-BNT assembly of its per-element channels remain absent, so the final theorem is not refuted and remains partial | `TNLean/MPS/MPDO/BNTSectorAreaLaw.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_caseII_properties_of_literal_ZCL`); `TNLean/MPS/MPDO/CaseIIAbsorptionCounterexample.lean` (`MPOTensor.CaseIIAbsorptionCounterexample.printed_absorbed_normality_step_is_false`); `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
...
| Not-ready | CPSV16 | Proposition `prop2to5` | The conditional channels and all sectorwise analytic hypotheses are complete. The missing inputs are the all-sector structural factorization from `prop2to3`, whose printed absorbed-normality bridge is refuted, and a projector-controlled assembly of the per-BNT channel pairs | `TNLean/MPS/MPDO/PhysicalSectorBlockedRFP.lean`; `TNLean/MPS/MPDO/PhysicalSectorPhysicalTransport.lean`; `TNLean/MPS/MPDO/CaseIIAbsorptionCounterexample.lean`; `TNLean/MPS/MPDO/BNTSectorAreaLaw.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
