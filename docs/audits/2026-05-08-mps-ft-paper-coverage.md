...
| Prop `prop4to2` | 1801–1804 | GSNNCH gives SAL | `TNLean/MPS/MPDO/OrthogonalSectorAreaLaw.lean`; `LocalOrthogonalSumAreaLaw.lean`; `CyclicActiveAreaLaw.lean`; `docs/paper-gaps/cpsv16_commuting_form_to_sal.tex` | **partial** — the printed statement omits sectorwise ZCL. The restricted theorem also assumes per-sector MPDO, one-site injectivity, normality, and ZCL, an exact scalar-one orthogonal sector realization stronger than the proportional source form, and an exact positive-multiplicity outer sum. One-site injectivity is not implied merely by BNT normality, leaving an injectivity-versus-blocking gap in the source application |
...
| Partial | CPSV16 | Proposition `prop4to2` | The restricted theorem assumes sectorwise MPDO, one-site injectivity, normality, and ZCL, plus an exact scalar-one sector realization stronger than the proportional source form and the exact positive-multiplicity outer sum; the source omits ZCL and leaves the injectivity-versus-blocking step unaddressed | Source statement requires correction |
