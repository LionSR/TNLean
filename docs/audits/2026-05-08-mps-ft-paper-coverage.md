...
| **Theorem 4.14** (source label `thm:IV.13`, l.972) | 972–993; proof 1929–2088 | RFP iff the tensor-attached BNT algebra condition iff the fusion-isometry condition | `TNLean/MPS/MPDO/BNTAlgebraTensorClause.lean`; `BNTAlgebraTensorClausePositivity.lean`; `BNTAlgebraTensorClauseSpectrum.lean`; `BNTFusionTensorClauseFromRFP.lean`; `PositiveMinimalRealizationCounterexample.lean`; `BondTwoSingletonGramBoundary.lean`; `BondTwoSingletonBaseModel.lean`; `BondTwoSingletonPhysicalGauge.lean`; `AlgebraFusionCounterexample.lean` | **partial** — several directions and the tensor-attached data are complete. Terminal sector positivity excludes the generic nonunitary-similarity witness. The bond-two singleton is an explicit all-length MPDO with a tensor-attached one-label BNT algebra clause. Its natural physical-similarity deformation has exact one-site and two-site reindexing identities; if the deformation remains an MPDO, then $X^\dagger X$ is a positive scalar multiple of the identity. The concrete nonscalar-Gram deformation is therefore not an MPDO. This excludes that natural deformation family, but it is not a deformed `BNTAlgebraTensorClause` counterexample and does not construct `IdentityMarkedRealization`. The marked comparison remains open in #5284 under #4648, and #4645 remains open. Appendix C.4 lines 2048–2057 and Proposition 4.13 lines 1898–1921 are motivation and a proof template, not statements of these model-specific results |
...
| Restatement/proof of Theorem 4.14 | 1929–2088 | Algebra and fusion characterizations of RFP | `TNLean/MPS/MPDO/BNTAlgebraTensorClause.lean`; `BNTAlgebraTensorClausePositivity.lean`; `BNTAlgebraTensorClauseSpectrum.lean`; `BNTFusionTensorClauseFromRFP.lean`; `PositiveMinimalRealizationCounterexample.lean`; `BondTwoSingletonGramBoundary.lean`; `BondTwoSingletonBaseModel.lean`; `BondTwoSingletonPhysicalGauge.lean`; `AlgebraFusionCounterexample.lean` | **partial restatement** — terminal positivity excludes the generic witness. The bond-two singleton is an all-length MPDO with a tensor-attached one-label BNT algebra clause. Its natural physical-similarity deformation can remain an MPDO only when its Gram matrix is scalar; the concrete nonscalar-Gram deformation is not an MPDO. This is not a deformed `BNTAlgebraTensorClause` counterexample and does not construct `IdentityMarkedRealization`. The marked comparison remains open in #5284 under #4648, and #4645 remains open |
...
| Partial | CPSV16 | Theorem 4.14 | The conditional Gram and unitary conclusions are formalized, while the tensor-attached marked comparison and CPTP maps remain open. Terminal positivity excludes the generic witness. For the bond-two singleton, exact one-site and two-site identities show that the natural physical-similarity deformation can remain an MPDO only when $X^\dagger X$ is scalar; the concrete nonscalar-Gram deformation is not an MPDO. This is not a deformed `BNTAlgebraTensorClause` counterexample and does not construct `IdentityMarkedRealization` | `TNLean/MPS/MPDO/BondTwoSingletonPhysicalGauge.lean`; #5284 under #4648; #4645 remains open |
