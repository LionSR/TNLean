...
| **Theorem 4.14** (source label `thm:IV.13`, l.972) | 972–993; proof 1929–2088 | RFP iff the tensor-attached BNT algebra condition iff the fusion-isometry condition | `TNLean/MPS/MPDO/BNTAlgebraTensorClause.lean`; `BNTAlgebraTensorClausePositivity.lean`; `BNTAlgebraTensorClauseSpectrum.lean`; `BNTFusionTensorClauseFromRFP.lean`; `PositiveMinimalRealizationCounterexample.lean`; `BondTwoSingletonGramBoundary.lean`; `AlgebraFusionCounterexample.lean` | **partial** — several directions and the tensor-attached data are complete. Terminal sector positivity excludes the generic nonunitary-similarity witness. The bond-two singleton deformation preserves a positive terminal matrix but produces a non-Hermitian two-site Bell deformation; conversely, one- and two-site Hermiticity force its gauge Gram matrix to be a positive scalar. This is a necessary-condition/no-go boundary, not an all-length MPDO or tensor-attached BNT model; #5290 records that separate construction. The marked/common-target comparison remains open in #5284 under #4648, and #4645 remains open; Proposition 4.13 supplies motivation but not this model-specific theorem |
...
| Restatement/proof of Theorem 4.14 | 1929–2088 | Algebra and fusion characterizations of RFP | `TNLean/MPS/MPDO/BNTAlgebraTensorClause.lean`; `BNTAlgebraTensorClausePositivity.lean`; `BNTAlgebraTensorClauseSpectrum.lean`; `BNTFusionTensorClauseFromRFP.lean`; `PositiveMinimalRealizationCounterexample.lean`; `BondTwoSingletonGramBoundary.lean`; `AlgebraFusionCounterexample.lean` | **partial restatement** — terminal positivity excludes the generic witness, and the bond-two singleton theorem shows that one-site positivity can coexist with a nonunitary deformation only until two-site Hermiticity is imposed. It is not an all-length tensor-attached model; #5290 records that separate problem. The marked comparison remains open in #5284 under #4648, and #4645 remains open |
...
| Partial | CPSV16 | Theorem 4.14 | The conditional Gram and unitary conclusions are formalized, while the tensor-attached marked comparison and CPTP maps remain open. Terminal positivity excludes the generic witness; the bond-two singleton theorem adds a one-/two-site scalar-Gram necessary condition but no all-length model | #5284 under #4648; #4645 remains open; #5290 is the separate all-length tensor-attached/base-model problem |
