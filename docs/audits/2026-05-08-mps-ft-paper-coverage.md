...
| **Theorem 4.14** (source label `thm:IV.13`, l.972) | 972–993; proof 1929–2088 | RFP iff the tensor-attached BNT algebra condition iff the fusion-isometry condition | `TNLean/MPS/MPDO/BNTAlgebraTensorClause.lean`; `BNTAlgebraTensorClausePositivity.lean`; `BNTAlgebraTensorClauseSpectrum.lean`; `BNTFusionTensorClauseFromRFP.lean`; `PositiveMinimalRealizationCounterexample.lean`; `BondTwoSingletonGramBoundary.lean`; `BondTwoSingletonBaseModel.lean`; `AlgebraFusionCounterexample.lean` | **partial** — several directions and the tensor-attached data are complete. Terminal sector positivity excludes the generic nonunitary-similarity witness. The bond-two singleton is now attached to an explicit bond-four MPDO whose operators are positive rank-one GHZ operators at every positive length, whose two-site operator becomes the Bell projector after the proved canonical reindexing, and whose normalized retained sector carries a genuine one-label BNT algebra clause, with the retained operator laws proved at every positive length. Through the proved reindex identity, the #5289 non-Hermitian Bell similarity is now a similarity of the base two-site operator matrix, but no deformed tensor-attached MPDO clause with that two-site operator is constructed. The marked/common-target comparison remains open in #5284 under #4648, and #4645 remains open; Proposition 4.13 supplies motivation but not these model-specific calculations |
...
| Restatement/proof of Theorem 4.14 | 1929–2088 | Algebra and fusion characterizations of RFP | `TNLean/MPS/MPDO/BNTAlgebraTensorClause.lean`; `BNTAlgebraTensorClausePositivity.lean`; `BNTAlgebraTensorClauseSpectrum.lean`; `BNTFusionTensorClauseFromRFP.lean`; `PositiveMinimalRealizationCounterexample.lean`; `BondTwoSingletonGramBoundary.lean`; `BondTwoSingletonBaseModel.lean`; `AlgebraFusionCounterexample.lean` | **partial restatement** — terminal positivity excludes the generic witness. The bond-two singleton has an explicit all-length positive ambient MPO: its two-site operator is the Bell projector, and its normalized retained sector gives a tensor-attached one-label BNT algebra clause. Through the proved reindex identity, the #5289 non-Hermitian Bell similarity is attached to the base two-site operator matrix, but no deformed tensor-attached MPDO clause is constructed. The marked comparison remains open in #5284 under #4648, and #4645 remains open |
...
| Partial | CPSV16 | Theorem 4.14 | The conditional Gram and unitary conclusions are formalized, while the tensor-attached marked comparison and CPTP maps remain open. Terminal positivity excludes the generic witness. The bond-two singleton has an all-length positive ambient MPO whose length-two operator is the Bell projector and whose normalized retained sector carries a tensor-attached BNT algebra clause. Through the proved reindex identity, the #5289 non-Hermitian Bell similarity is attached to the base two-site operator matrix, but no deformed tensor-attached MPDO clause or nonunitary full-contract counterexample is constructed | `TNLean/MPS/MPDO/BondTwoSingletonBaseModel.lean`; #5284 under #4648; #4645 remains open |
