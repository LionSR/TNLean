...
| Partial | CPSV16 | Proposition `prop4to2` | The restricted theorem assumes sectorwise MPDO, one-site injectivity, normality, proportional fixed-bond data and ZCL, plus independent exact scalar-one sector realizations and the exact positive-multiplicity outer sum; the source omits ZCL and leaves the injectivity-versus-blocking step unaddressed | Source statement requires correction |
