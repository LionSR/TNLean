...
| Prop `4to2` | 1597–1601 | Commuting form and ZCL imply SAL | `TNLean/MPS/MPDO/CyclicActiveAreaLaw.lean` (`EtaLocalStructureData.isSAL_of_isSourceZCL`); `CyclicActiveMarkovDecomposition.lean`; `FixedBondPositivePhysicalSectorRepresentative.lean`; `docs/paper-gaps/cpsv16_commuting_form_to_sal.tex` | **complete** — all-cut Hayashi decompositions for the original injective normal tensor in the selected physical coordinates imply SAL there, and the selected one-site isometry transports SAL back to the source tensor without any property assumed of the selected fixed tensor |
...
| Prop `prop4to2` | 1801–1804 | GSNNCH gives SAL | `TNLean/MPS/MPDO/OrthogonalSectorAreaLaw.lean` (`MPOTensor.isSAL_of_proportionalOrthogonalCommutingSectorFamily_of_sectorwise_isSourceZCL`); `LocalOrthogonalSumAreaLaw.lean` (`MPOTensor.isSAL_of_proportionalLocalOrthogonalSum`); `CyclicActiveAreaLaw.lean`; `docs/paper-gaps/cpsv16_commuting_form_to_sal.tex` | **partial** — positive length-dependent proportionality of both the global orthogonal-sector sum and each fixed-bond sector product now cancels in the normalized states, so neither is a restriction. The printed statement still omits sectorwise ZCL. The restricted sector theorem retains per-sector MPDO, one-site injectivity, normality, and ZCL. Normal BNT sectors are injective only after blocking; under the present blocking and SAL interfaces, blocked SAL supplies only block-aligned chain lengths and cuts and does not by itself establish unblocked SAL. A completion following the present Hayashi-decomposition route requires word-length replacements for the all-original-cut sector-visibility steps |
...
| Partial | CPSV16 | Proposition `prop4to2` | Positive global and sectorwise fixed-product proportionality are handled by `MPOTensor.isSAL_of_proportionalOrthogonalCommutingSectorFamily_of_sectorwise_isSourceZCL`. The restricted theorem retains sectorwise MPDO, one-site injectivity, normality, and ZCL; the source omits ZCL, and its application to normal BNT sectors still lacks the injectivity-versus-blocking step | Source statement requires correction |
