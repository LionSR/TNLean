...
| Partial | CPSV16 | Theorem 4.14 | The conditional Gram and unitary conclusions are formalized, while the tensor-attached marked comparison and CPTP maps remain open. Terminal positivity excludes the generic witness. The concrete nonunitary bond-two deformation has a genuine `BNTAlgebraTensorClause` and literal horizontal CPSV canonical form, but its length-two operator is not positive, so it fails `IsMPDO`. It is not a full-contract counterexample and constructs no `IdentityMarkedRealization` | `TNLean/MPS/MPDO/BondTwoSingletonPhysicalGauge.lean`; #5284 under #4648; #4645 remains open |
