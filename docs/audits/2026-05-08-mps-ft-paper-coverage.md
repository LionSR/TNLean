...
| Prop `prop2to5` | 1810–1813 | SAL and ZCL give the two trace-preserving maps | `TNLean/MPS/MPDO/PhysicalSectorCoarseGrainingIdentity.lean`; `PhysicalSectorRefinementIdentity.lean`; `PhysicalSectorBlockedRFP.lean`; `NeighboringTraceObstructionAmbientCounterexample.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **conditional; printed route fails** — the representative-channel theorem and projector-controlled outer assembly remain valid for supplied absorbed-sector factorizations. The ambient counterexample disproves the preceding representativewise assertion used by the printed derivation, so that derivation does not establish this proposition. No counterexample to Proposition~C.15 itself is claimed. |
...
| Conditional; printed route fails | CPSV16 | Proposition `prop2to5` | The representative-channel and outer-sector constructions remain valid for supplied factorizations. The printed route uses the false Proposition `prop2to3`, but no counterexample to Proposition `prop2to5` itself is claimed. | `TNLean/MPS/MPDO/PhysicalSectorBlockedRFP.lean`; `TNLean/MPS/MPDO/PhysicalSectorPhysicalTransport.lean`; `TNLean/MPS/MPDO/NeighboringTraceObstructionAmbientCounterexample.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
