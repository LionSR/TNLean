...
| **Theorem 4.14** (source label `thm:IV.13`, l.972) | 972–993; proof 1929–2088 | RFP iff the tensor-attached BNT algebra condition iff the fusion-isometry condition | `TNLean/MPS/MPDO/BNTAlgebraTensorClause.lean`; `BNTAlgebraTensorClausePositivity.lean`; `BNTAlgebraTensorClauseSpectrum.lean`; `BNTFusionTensorClauseFromRFP.lean`; `PositiveMinimalRealizationCounterexample.lean`; `BondTwoSingletonGramBoundary.lean`; `AlgebraFusionCounterexample.lean` | **partial** — several directions and the tensor-attached data are complete. Terminal sector positivity excludes the generic nonunitary-similarity witness. The bond-two singleton deformation preserves a positive terminal matrix and all positive-length closed vectors. Independently, a companion Bell-projector similarity is non-Hermitian, and Hermiticity of the chosen terminal and Bell similarities forces a positive scalar Gram matrix. No Bell-to-singleton two-site identification is proved; #5290 tracks that tensor attachment and the all-length positive model. The marked/common-target comparison remains open in #5284 under #4648, and #4645 remains open; Proposition 4.13 supplies motivation but not these model-specific calculations |
...
| Restatement/proof of Theorem 4.14 | 1929–2088 | Algebra and fusion characterizations of RFP | `TNLean/MPS/MPDO/BNTAlgebraTensorClause.lean`; `BNTAlgebraTensorClausePositivity.lean`; `BNTAlgebraTensorClauseSpectrum.lean`; `BNTFusionTensorClauseFromRFP.lean`; `PositiveMinimalRealizationCounterexample.lean`; `BondTwoSingletonGramBoundary.lean`; `AlgebraFusionCounterexample.lean` | **partial restatement** — terminal positivity excludes the generic witness. The bond-two singleton has a terminal-positive nonunitary closed-vector deformation; separately, the Bell comparator supplies a non-Hermitian tensor-square similarity and a scalar-Gram criterion. It is not identified with a singleton-derived two-site operator. Issue #5290 tracks that attachment and the all-length model. The marked comparison remains open in #5284 under #4648, and #4645 remains open |
...
| Partial | CPSV16 | Theorem 4.14 | The conditional Gram and unitary conclusions are formalized, while the tensor-attached marked comparison and CPTP maps remain open. Terminal positivity excludes the generic witness. The bond-two singleton preserves its terminal matrix under a nonunitary closed-vector deformation; an independent terminal-$J$/Bell comparator gives a scalar-Gram criterion. No Bell-to-singleton two-site attachment or all-length model is proved | #5284 under #4648; #4645 remains open; #5290 tracks the singleton attachment and all-length tensor-attached/base-model problem |
