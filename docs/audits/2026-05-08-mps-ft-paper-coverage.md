...
| Prop `prop4to2` | 1801–1808 | GSNNCH gives SAL | `TNLean/MPS/MPDO/BNTSectorAnalyticProperties.lean` (`MPOTensor.IsSimpleCanonicalForm.exists_commonWeightAbsorbedBasisMPOTensor_isSourceZCL`); `BiCFDerivation/Core.lean` (`MPSTensor.WordTupleSpanTop.isInjective_one`); `OrthogonalSectorAreaLaw.lean` (`MPOTensor.isSAL_of_proportionalOrthogonalCommutingSectorFamily_of_sectorwise_isSourceZCL`); `docs/paper-gaps/cpsv16_commuting_form_to_sal.tex` | **partial** — under the surrounding main theorem's ambient source-ZCL hypothesis, every canonical absorbed simple-BNT sector has source ZCL, while the standing Case II biCF one-letter span gives one-site injectivity at every original cut. The formal sector theorem still retains sector MPDO positivity and normality as explicit inputs: the chosen canonical representatives supply normality, while positivity must come from the positive sector data and not from ambient SAL. The printed GSNNCH-only proposition omits ambient ZCL, and an arbitrary GSNNCH presentation does not inherit tensor-level source ZCL |
...
| Partial | CPSV16 | Proposition `prop4to2` | Under ambient source ZCL, the canonical absorbed simple-BNT sectors inherit source ZCL, and the standing biCF one-letter span gives their one-site injectivity. The formal sector theorem still explicitly assumes sector MPDO positivity and normality; canonical representatives supply normality, while positivity must come from the positive sector data rather than ambient SAL. The printed GSNNCH-only statement omits ambient ZCL, and arbitrary GSNNCH presentations do not inherit tensor-level source ZCL | `docs/paper-gaps/cpsv16_commuting_form_to_sal.tex` |
