...
`python3 scripts/audit_cpsv16_labels.py`.  The validator also requires all 58
equation or figure labels inside theorem statements and proofs to inherit the
enclosing result's status and semantic boundary explicitly.  The four duplicate
...
| **Example 3.4** (`Ex:ZCL`, l.453) | 453–464 | A permutation-invariant state has CID but is not an RFP | Blueprint `thm:cpsv_example34_status`; exact displayed-tensor formalization tracked by assigned issue #5916 | **active source target** |
...
| **Theorem 4.9** (l.851, `thm:main-simple`) | 851–893 | Simple-MPDO implication chain \((i)\Rightarrow(ii)\Leftrightarrow(iii)\Rightarrow(iv)\Rightarrow(v)\) | Blueprint `thm:cpsv_theorem49_printed_status`; `TNLean/MPS/MPDO/RFPViaTSSAL.lean`; `TNLean/MPS/MPDO/CyclicActiveAreaLaw.lean`; `TNLean/MPS/MPDO/BNTSeparatingProjectors.lean`; `TNLean/MPS/MPDO/BNTSourceSectorProjectors.lean`; `TNLean/MPS/MPDO/BNTSectorAnalyticProperties.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_physTraceTransfer_sq_of_literal_ZCL`); `TNLean/MPS/MPDO/BNTSectorAreaLaw.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_caseII_properties_of_literal_ZCL`); `TNLean/Analysis/EntropyMarkovForward.lean`; `TNLean/Channel/PetzProductReference.lean` | **partial** — implication \((i)\Rightarrow(ii)\) is complete at the explicit density-family boundary, and the HJPW/Hayashi and Petz-recovery constructions are complete. In Case II every absorbed BNT representative is now proved one-site injective, MPDO-positive, SAL, and literally physical-trace idempotent. The remaining structural implication is blocked by normality after coefficient absorption: the transfer spectral radius of \(\mu_s A_s\) is multiplied by \(|\mu_s|^2\), while the global canonical normalization supplies only one unit-modulus coefficient and does not give \(|\mu_s|=1\) for every sector. Thus the normal Case-I all-sector factorization cannot yet be applied to every absorbed BNT representative |
...
| **Example 4.12** (unlabeled, l.929) | 929–942 | The toric-code boundary tensor is SAL and ZCL but is not of the simple GSNNCH form | Blueprint `thm:cpsv_examples410_412_status`; tracked with Examples 4.10--4.11 by issue #5919 | **active source target** |
...
| Strong unitary-conjugation RFP diagram (`Strong-RFP`) | 2109–2117 | Both renormalization maps are required to be unitary conjugations | Blueprint `thm:cpsv_fibonacci_rank_obstruction_status`; issue #5921 | **not-ready** — the diagrammatic predicate remains ambiguous until translated algebraically; its claimed geometric rank consequence is separated from the concrete Fibonacci calculation |
...
| Partial | CPSV16 | Theorem 4.9 | Implication \((i)\Rightarrow(ii)\), HJPW/Hayashi equality, Petz recovery, and sectorwise injective+MPDO+SAL+literal-ZCL inheritance are complete. The remaining Case-II factorization is blocked by normality after coefficient absorption: \(\rho(\mathcal E_{\mu_s A_s})=|\mu_s|^2\rho(\mathcal E_{A_s})\), and no sectorwise unit-modulus conclusion is available | `TNLean/MPS/MPDO/BNTSectorAreaLaw.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_caseII_properties_of_literal_ZCL`); `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
...
| Not-ready | CPSV16 | Proposition `prop2to3` | Sectorwise injectivity, MPDO positivity, SAL, and literal physical-trace idempotence are complete for every absorbed BNT representative. Coefficient absorption need not preserve spectral normality, so the normal Case-I structural theorem cannot yet supply the all-sector factorization | `TNLean/MPS/MPDO/SimpleTensor.lean` (`MPOTensor.weighted_basis_physTraceTransfer_sq_of_literal_ZCL`); `BNTSectorAnalyticProperties.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_physTraceTransfer_sq_of_literal_ZCL`); `BNTSectorAreaLaw.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_caseII_properties_of_literal_ZCL`) |
...
| Not-ready | CPSV16 | Proposition `prop2to5` | The conditional channels and all sectorwise analytic hypotheses are complete. The missing input is the all-sector structural factorization from `prop2to3`, whose precise blocker is normality after coefficient absorption | `TNLean/MPS/MPDO/PhysicalSectorBlockedRFP.lean`; `TNLean/MPS/MPDO/BNTSectorAreaLaw.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
...
- The CPSV16 every-label inventory is validated by `scripts/audit_cpsv16_labels.py` against `docs/audits/data/cpsv16-label-dispositions.tsv`; it counts 187 lexical occurrences and 183 lexical names, derives the 186 active occurrences and 182 active names by stripping unescaped TeX comments, compares source activity with the ledger occurrence by occurrence, checks the four duplicate names listed in §2, and verifies explicit status inheritance for all 58 theorem/proof-contained equation and figure labels.
