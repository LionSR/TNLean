...
| **Definition 4.7** (l.821) | 815–822, with standing convention 217–246 | A simple MPDO has no nilpotent element in a blocked BNT | `TNLean/MPS/MPDO/SourceSimpleTensor.lean` (`MPOTensor.IsSourceSimple`, `MPOTensor.bnt_basis_not_isNilpotent_iff`, `MPOTensor.IsSourceSimple.exists_mpo_ne_zero`, `MPOTensor.IsNonvanishingSourceSimple`, `MPOTensor.IsSimple.isSourceSimple`, `MPOTensor.IsSimple.isNonvanishingSourceSimple`); `BNTRefinement.lean` (`MPSTensor.IsBNTSectorPresentation`, `equiv_of_sameMPV₂Pos`); `SimpleTensor.lean` (gauge-phase invariance of doubled-transfer nilpotency and `MPOTensor.IsSimple`); `SimpleVanishingCounterexample.lean` (`MPOTensor.SimpleVanishingCounterexample.M_isSimple`, `M_isSourceSimple`, `M_not_isNonvanishingSourceSimple`); Blueprint `def:mpdo_source_simple_tensor` and `thm:mpdo_simple_vanishing_source_simple_status`; `docs/paper-gaps/cpsv16_simple_tensor_nilpotency.tex`; `docs/paper-gaps/cpsv16_bnt_uniqueness_zero_coefficient.tex` | **complete for the canonical-block reading, with a separate strengthened predicate** — `IsSourceSimple` records the nonzero canonical-block construction by existentially choosing a positive blocking and a BNT sector presentation. Every representative has a positive number of copies and all copy weights are nonzero; presentations are unique up to permutation, dimension identification, gauge, and phase, under which doubled-transfer nilpotency is invariant. Positive-length nontriviality follows from the presentation and is not a defining hypothesis. `IsNonvanishingSourceSimple` strengthens this to $\rho_N(M)\ne0$ for every $N>0$. Normalized fixed-representative simplicity implies source simplicity because its normalized sector decomposition gives a presentation after gauge transport. The bond-two witness has $\rho_2(M)=2I\ne0$ but $\rho_1(M)=0$, so it is normalized-simple and source-simple after blocking two sites but fails the strengthened predicate. This is not a counterexample to Definition 4.7 and is not an RFP or coefficient-rigidity claim |
