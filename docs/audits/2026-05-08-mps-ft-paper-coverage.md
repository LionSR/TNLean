...
| Prop `prop4to2` | 1801–1808 | GSNNCH gives SAL | `TNLean/MPS/MPDO/BNTSectorAnalyticProperties.lean` (`MPOTensor.IsSimpleCanonicalForm.exists_commonWeightAbsorbedBasisMPOTensor_isSourceZCL`); `PhysicalSupportRestriction.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_isInjective`); `OrthogonalSectorAreaLaw.lean` (`MPOTensor.isSAL_of_proportionalOrthogonalCommutingSectorFamily_of_sectorwise_isSourceZCL`); `docs/paper-gaps/cpsv16_commuting_form_to_sal.tex` | **partial** — under the surrounding main theorem's ambient source-ZCL hypothesis, every canonical absorbed simple-BNT sector has source ZCL, while the standing Case II biCF one-letter span and the nonzero common weight give one-site injectivity at every original cut. The sector theorem still retains sector MPDO positivity and absorbed-sector normalized normality as explicit independent inputs. The printed GSNNCH-only proposition omits ambient ZCL, and an arbitrary GSNNCH presentation does not inherit tensor-level source ZCL |
...
| Partial | CPSV16 | Proposition `prop4to2` | Under ambient source ZCL, the canonical absorbed simple-BNT sectors inherit source ZCL, and the standing biCF one-letter span together with the nonzero common weight gives their one-site injectivity. The sector theorem still explicitly assumes sector MPDO positivity and absorbed-sector normalized normality as independent inputs. The printed GSNNCH-only statement omits ambient ZCL, and arbitrary GSNNCH presentations do not inherit tensor-level source ZCL | `docs/paper-gaps/cpsv16_commuting_form_to_sal.tex` |
