...
| Lemma `SALZCL` / Lemma C.5 | 1484–1502 | SAL and ZCL force the active trace matrix to have rank one | `TNLean/MPS/MPDO/ActiveSectorSpanningAreaLaw.lean` (`ActiveSectorSpanningCounterexample.tensor_has_sal_sourceZCL_and_non_rankOne_activeTraceMatrix`); `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **not-ready** — the formal tensor has SAL, literal ZCL, the source-selected inverse-map factorization, and a non-rank-one active trace matrix, but it is not the injective normal tensor assumed in Case I. Its normal representative loses literal ZCL. This is a normalization obstruction, not a counterexample under the complete source hypotheses |
| Corollary | 1503–1506 | SAL and ZCL imply the displayed structural form | Corrected direct rank-one criteria in `TNLean/MPS/MPDO/SimpleLocalStructure.lean`; `TNLean/MPS/MPDO/ActiveSectorSpanningAreaLaw.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **not-ready** — the printed derivation uses the unresolved rank-one step of Lemma C.5; the retained criteria require positive semidefiniteness or linear independence, neither of which is supplied by the source |
...
| Prop `prop2to3` | 1740–1743 | SAL and ZCL imply the blockwise structural form | `TNLean/MPS/MPDO/BNTSeparatingProjectors.lean`; `BNTSourceSectorProjectors.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **not-ready** — the printed proof requires the unresolved rank-one step of Lemma C.5; the raw obstruction lies outside its normal-tensor hypothesis and the normal representative loses literal ZCL |
...
| Prop `prop2to5` | 1810–1813 | SAL and ZCL give the two trace-preserving maps | `TNLean/MPS/MPDO/PhysicalSectorCoarseGrainingIdentity.lean`; `PhysicalSectorRefinementIdentity.lean`; `PhysicalSectorBlockedRFP.lean`; `ActiveSectorSpanningRFP.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **not-ready** — the conditional maps are constructed, but deriving their neighboring trace factorization from the printed hypotheses uses the unresolved rank-one step of Lemma C.5. The raw four-sector witness has direct maps and literal diagrammatic ZCL, but its sole BNT coefficient is strictly subunit. The globally normalized representative preserves SAL and TNLean's scale-invariant `IsSourceZCL` interpretation, whereas its two-site blocking has physical-trace transfer $(16/5)\operatorname{diag}(1,0)$ and cannot satisfy Definition 4.1; however, that representative fails the paper's literal ZCL diagram. The two endpoints are kernel-checked, but neither is an unqualified counterexample to the exact source implication. Its normalization boundary remains unsettled |
...
| Partial | CPSV16 | Theorem 4.9 | Implication \((i)\Rightarrow(ii)\), the HJPW/Hayashi equality characterization, and the exact Petz-recovery construction are complete. The remaining SAL-and-ZCL structural implication uses the unresolved normalization-sensitive rank-one step of Lemma C.5 | `TNLean/Analysis/EntropyMarkovForward.lean`; `TNLean/Channel/PetzProductReference.lean`; `TNLean/MPS/MPDO/ActiveSectorSpanningAreaLaw.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
...
| Not-ready | CPSV16 | Lemma C.5 (`SALZCL`) | The raw four-sector tensor has SAL, literal ZCL, and a non-rank-one active trace matrix, but is not the normal tensor assumed in Case I; its normal representative loses literal ZCL | Normalization boundary audited in `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
| Not-ready | CPSV16 | Structural corollary, lines 1503–1506 | Its printed proof depends on the unresolved rank-one step of Lemma C.5 | No source-faithful replacement known |
...
| Not-ready | CPSV16 | Proposition `prop2to3` | Its printed SAL+ZCL proof depends on the unresolved rank-one step of Lemma C.5 | No source-faithful replacement known |
...
| Not-ready | CPSV16 | Proposition `prop2to5` | Conditional maps exist, but the printed derivation of their factorization uses the unresolved rank-one step of Lemma C.5 | No unconditional source theorem available |
