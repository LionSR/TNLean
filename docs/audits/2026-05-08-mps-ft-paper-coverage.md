...
| **Definition 4.7** (l.821) | 815–823 | A simple MPDO has no nilpotent element in a blocked BNT | `TNLean/MPS/MPDO/SourceSimpleTensor.lean` (`MPOTensor.IsSourceSimple`, `MPOTensor.IsNonvanishingSourceSimple`, `MPOTensor.IsSimple.isSourceSimple`, `MPOTensor.IsSimple.isNonvanishingSourceSimple`); `SimpleTensor.lean` (`MPOTensor.IsSimple`); `SimpleVanishingCounterexample.lean` (`MPOTensor.SimpleVanishingCounterexample.M_isSimple_and_isSourceSimple_and_not_isNonvanishingSourceSimple`); Blueprint `thm:mpdo_simple_vanishing_source_simple_status`; `docs/paper-gaps/cpsv16_unit_weight_rfp_scale_tension.tex` | **complete as the source definition, with a separate strengthened interface** — `IsSourceSimple` is the normalization-free Definition 4.7 predicate and does not require positive-length nonvanishing. `IsNonvanishingSourceSimple` adds $\rho_N(M)\ne0$ for every $N>0$. Normalized fixed-representative simplicity implies source simplicity unconditionally, while its bridge to the strengthened predicate assumes nonvanishing. The compiled bond-two witness has $\rho_N(M)=(1+(-1)^N)I$ and is both normalized-simple and source-simple after blocking two sites, but it fails the strengthened predicate because $\rho_1(M)=0$. This is not a counterexample to Definition 4.7 and is not an RFP or coefficient-rigidity claim |
