...
| Prop `prop2to5` | 1810–1813 | SAL and ZCL give the two trace-preserving maps | `TNLean/MPS/MPDO/PhysicalSectorCoarseGrainingIdentity.lean`; `PhysicalSectorRefinementIdentity.lean`; `PhysicalSectorBlockedRFP.lean`; `NeighboringTraceObstructionAmbientCounterexample.lean`; <https://sirui-lu.com/QICLean/paper-gaps/cpgsv17_pf_rank_one.pdf> | **missing statement; conditional helper; printed route fails** — the source proposition remains `\notready` with no Lean declaration. The representative-channel theorem and projector-controlled outer assembly are proved for supplied absorbed-sector factorizations. The ambient counterexample disproves the preceding representativewise assertion used by the printed derivation, so that derivation does not establish this proposition. It does not exclude different channel maps, and the bare conclusion is neither proved nor refuted. |
...
| Partial and refuted in two directions | CPSV16 | Theorem 4.9 | Implication \((i)\Rightarrow(ii)\), HJPW/Hayashi equality, and Petz recovery are complete. The five-letter ambient witness has a two-representative BNT presentation in biCF, and it satisfies the full condition-(ii) hypotheses and line-246 normalization. Its first absorbed representative has no normalized rank-one neighboring-trace factorization, refuting the printed representativewise assertion in (ii)$\Rightarrow$(iv). Complementary supports prove SAL, while virtual-gauge invariance and isometric descent reduce any hypothetical factorization to the four-letter obstruction. Separately, the scalar repeated-copy witness refutes literal (iv)$\Rightarrow$(v). | `TNLean/MPS/MPDO/NeighboringTraceObstructionAmbientCounterexample.lean` (`printed_theorem49_ii_to_iv_is_false`); `TNLean/MPS/MPDO/Theorem49RepeatedCopyCounterexample.lean` (`printed_theorem49_iv_to_v_is_false`); <https://sirui-lu.com/QICLean/paper-gaps/cpgsv17_pf_rank_one.pdf> |
...
| Missing statement; conditional helper; printed route fails | CPSV16 | Proposition `prop2to5` | The representative-channel and outer-sector constructions are proved for supplied factorizations. The source proposition remains `\notready` with no Lean declaration: its printed route uses the false Proposition `prop2to3`, while the bare conclusion is neither proved nor refuted. | `TNLean/MPS/MPDO/PhysicalSectorBlockedRFP.lean`; `TNLean/MPS/MPDO/PhysicalSectorPhysicalTransport.lean`; `TNLean/MPS/MPDO/NeighboringTraceObstructionAmbientCounterexample.lean`; <https://sirui-lu.com/QICLean/paper-gaps/cpgsv17_pf_rank_one.pdf> |
