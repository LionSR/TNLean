...
| Partial | CPSV16 | Proposition `prop4to2` | Under ambient source ZCL, the canonical absorbed simple-BNT sectors inherit source ZCL, and the standing biCF one-letter span gives their one-site injectivity, so the all-original-cut sector theorem applies. The printed GSNNCH-only statement omits ambient ZCL, and arbitrary GSNNCH presentations do not inherit tensor-level source ZCL | `docs/paper-gaps/cpsv16_commuting_form_to_sal.tex` |
