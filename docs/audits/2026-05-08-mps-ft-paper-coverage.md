...
| Not-ready | CPSV16 | Proposition `prop2to3` | Sectorwise injectivity, MPDO positivity, SAL, and literal physical-trace idempotence are complete for every absorbed BNT representative. The explicit bond-one construction refutes preservation of spectral normality under coefficient absorption. The remaining assertion is existence of some physical-sector factorization with normalized rank-one neighboring traces for every absorbed representative; it is not a rank-one claim about a prescribed inverse-map decomposition | `TNLean/MPS/MPDO/CaseIIAbsorptionCounterexample.lean`; `TNLean/MPS/MPDO/SimpleTensor.lean` (`MPOTensor.weighted_basis_physTraceTransfer_sq_of_literal_ZCL`); `BNTSectorAnalyticProperties.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_physTraceTransfer_sq_of_literal_ZCL`); `BNTSectorAreaLaw.lean` (`MPOTensor.commonWeightAbsorbedBasisMPOTensor_caseII_properties_of_literal_ZCL`); issue #6775 |
