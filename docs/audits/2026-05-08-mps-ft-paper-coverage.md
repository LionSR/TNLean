...
| Lemma `SALZCL` / Lemma C.5 | 1484–1502 | SAL and ZCL force the active trace matrix to have rank one | `TNLean/MPS/MPDO/InverseMapLemmaC5CaseI.lean` (`MPOTensor.exists_activeSectorTraceMatrix_rank_one_coefficients_of_isSAL_of_literal_ZCL`); `docs/paper-gaps/cpgsv17_pf_rank_one.tex` | **complete** — on the source's supported-sector convention, under the standing normal Case-I hypotheses, injectivity, normality, SAL, and literal physical-trace idempotence give the rank-one coefficients on the nonzero-weight Hayashi sectors. TNLean's ambient zero-weight filler sectors are excluded by the active subtype. This neither replaces literal ZCL by scale-invariant `IsSourceZCL` nor gives a Case-II conclusion. The raw four-sector tensor remains outside the normal Case-I hypotheses, and its normal representative loses literal ZCL |
...
| Complete | CPSV16 | Lemma C.5 (`SALZCL`) | On the source's supported-sector convention, under the standing normal Case-I hypotheses, SAL and literal physical-trace idempotence give the rank-one coefficients on the nonzero-weight Hayashi sectors; TNLean's ambient zero-weight fillers are excluded by the active subtype. The theorem does not extend to scale-invariant `IsSourceZCL` or to Case II | `TNLean/MPS/MPDO/InverseMapLemmaC5CaseI.lean` (`MPOTensor.exists_activeSectorTraceMatrix_rank_one_coefficients_of_isSAL_of_literal_ZCL`); normalization boundary audited in `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
