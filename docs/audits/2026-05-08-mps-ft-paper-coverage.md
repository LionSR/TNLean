...
| Paper label | Lines | Paper description | Lean location | Status |
|---|---|---|---|---|
| Renormalization flow convergence | 1209–1244 | Renormalization flow from canonical form converges | `TNLean/MPS/RFP/Convergence.lean` (`rg_flow_converges_of_cf`) | **partial proof segment; uncounted and distinct from the completed Theorem 3.1 equivalence** — the current theorem proves convergence for each basic block. The full weighted canonical-form assembly, including repeated copies and mixed-block terms, remains unformalized |
