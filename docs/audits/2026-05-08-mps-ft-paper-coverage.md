...
| Prop `prop4to2` | 1801–1804 | GSNNCH gives SAL | `TNLean/MPS/MPDO/OrthogonalSectorAreaLaw.lean`; `LocalOrthogonalSumAreaLaw.lean`; `CyclicActiveAreaLaw.lean`; `docs/paper-gaps/cpsv16_commuting_form_to_sal.tex` | **partial** — the statement omits the ZCL hypothesis used in its proof; the formal orthogonal-sum theorem therefore assumes SAL for every sector, while Proposition `4to2` now supplies that premise only when the required sectorwise source-ZCL and fixed-bond hypotheses are available |
...
| Partial | CPSV16 | Proposition `prop4to2` | The statement omits the ZCL hypothesis used in the proof; the formal orthogonal-sum result assumes sectorwise SAL, which Proposition `4to2` supplies only under explicit sectorwise source-ZCL and fixed-bond hypotheses | Source statement requires correction |
