...
| **Theorem 4.9** (l.851, `thm:main-simple`) | 851–893 | Simple-MPDO implication chain \((i)\Rightarrow(ii)\Leftrightarrow(iii)\Rightarrow(iv)\Rightarrow(v)\) | Blueprint `thm:cpsv_theorem49_printed_status` and `thm:mpdo_theorem49_iv_to_v_counterexample`; `TNLean/MPS/MPDO/RFPViaTSSAL.lean`; `TNLean/MPS/MPDO/CyclicActiveAreaLaw.lean`; `TNLean/MPS/MPDO/BNTSeparatingProjectors.lean`; `TNLean/MPS/MPDO/BNTSourceSectorProjectors.lean`; `TNLean/MPS/MPDO/BNTSectorAnalyticProperties.lean`; `TNLean/MPS/MPDO/BNTSectorAreaLaw.lean`; `TNLean/MPS/MPDO/CaseIIAbsorptionCounterexample.lean`; `TNLean/MPS/MPDO/Theorem49RepeatedCopyCounterexample.lean`; `TNLean/Analysis/EntropyMarkovForward.lean`; `TNLean/Channel/PetzProductReference.lean` | **partial and partly refuted** — implication \((i)\Rightarrow(ii)\) is complete at the explicit density-family boundary, and the HJPW/Hayashi and Petz-recovery constructions are complete. The explicit two-sector bond-one construction refutes the printed claim that coefficient absorption preserves normality, so the (ii)$\Rightarrow$(iv) proof has proof-path drift and its all-sector factorization remains a missing statement. Independently, the scalar one-representative tensor with raw copy weights $1,1/2$ has explicit component witnesses for the exact BNT canonical assembly, global unit weight, MPDO and simplicity hypotheses, biCF one-letter span, every clause of (iv), and MPDO positivity after two-site blocking, while that block fails Definition 4.1. Thus the literal (iv)$\Rightarrow$(v) is a refuted unfaithful statement. The viable source (ii)$\Rightarrow$(v) route remains not ready after ZCL-derived common-weight absorption and projector-controlled channel assembly |
...
| Not-ready | CPSV16 | Proposition `prop2to5` | This is the viable source (ii)$\Rightarrow$(v) route, not the refuted raw (iv)$\Rightarrow$(v) claim. After the earlier ZCL derivation makes repeated-copy weights common and absorbs them, the remaining inputs are the all-sector structural factorization from `prop2to3` and projector-controlled assembly of the conditional per-BNT channel pairs | `TNLean/MPS/MPDO/PhysicalSectorBlockedRFP.lean`; `TNLean/MPS/MPDO/PhysicalSectorPhysicalTransport.lean`; `TNLean/MPS/MPDO/CaseIIAbsorptionCounterexample.lean`; `TNLean/MPS/MPDO/BNTSectorAreaLaw.lean`; `docs/paper-gaps/cpgsv17_pf_rank_one.tex` |
