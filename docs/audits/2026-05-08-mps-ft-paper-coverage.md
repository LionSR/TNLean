...
| Partial | CPSV16 | Theorem 4.14 | The conditional Gram and unitary conclusions are formalized, while the tensor-attached marked comparison and CPTP maps remain open. Terminal positivity excludes the generic witness from retained sectors, and the explicit injective `SameMPV` pair has a nonpositive generic per-block extension. Thus closed-chain equality alone cannot justify the positive unitary-implementation step. The concrete nonunitary bond-two deformation has a genuine `BNTAlgebraTensorClause` and literal horizontal CPSV canonical form, but its length-two operator is not positive, so it fails `IsMPDO`. Neither example is a full-contract counterexample or constructs an `IdentityMarkedRealization` | `TNLean/MPS/MPDO/PositiveLinearExtensionCounterexample.lean`; `TNLean/MPS/MPDO/BondTwoSingletonPhysicalGauge.lean`; #5284 under #4648; #4645 remains open |
