...
was dropped. Redirecting it to the inherited parent projection was tried first
and rejected: the projection is generated by the structure declaration rather
than written out, so the declaration scanner that reads Lean source cannot see
it. The tag was one of seventeen on that node, no node text changed, and the
node's own statement of the forgetful passage is now carried by the structure
declaration itself. The new raw-representation blocking result is tagged on the
common-blocking theorem of `blueprint/src/chapter/ch29_mpu_gauging.tex`, whose
statement it proves without the simplicity clause. Every other tag on the
affected structures keeps its name, because `extends` preserves the names of
both structures.
