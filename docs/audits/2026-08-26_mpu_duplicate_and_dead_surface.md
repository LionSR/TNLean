...
| `MPOTensor.doubledPhysicalAncillaMiddleShuffle` (private, `TNLean/MPS/MPU/PhysicalAncilla.lean`) | `Equiv.prodProdProdComm` (Mathlib, `Mathlib/Logic/Equiv/Prod.lean`) |
| `MPOTensor.doubledPhysicalAncillaMiddleShuffle_apply` (private, `TNLean/MPS/MPU/PhysicalAncilla.lean`) | `Equiv.prodProdProdComm_apply` (Mathlib, generated by `@[simps apply]`) |
...
The doubled-index middle shuffle is the same story one dimension up. Its
forward and inverse maps and its target type agree character for character with
the four-way commutation of products, so instantiating that equivalence at
\((\mathrm{Fin}\,d, \mathrm{Fin}\,x, \mathrm{Fin}\,d, \mathrm{Fin}\,x)\)
reproduces the local definition definitionally, and the local apply-lemma
duplicates the one Mathlib generates for it. Again no import was added:
`Mathlib.Logic.Equiv.Prod` already supplies the product congruence used two
lines below the rewritten definition. This deletion shifts the line citations
recorded in `docs/tactic_patterns.md` for the same file a second time.
