...
| **Thm 4.9 (main, simple case)** `thm:main-simple` | For simple K in biCF: (i)RFP ⟹ (ii)ZCL+SAL ⟺ (iii)GSNNCH+ZCL ⟹ (iv)structural ⟹ (v)blocked RFP | **VERY HARD** | Partial and partly refuted. The forward RFP implication and several component results are complete. The exact scalar repeated-copy capstone refutes literal (iv)⟹(v). After the false absorbed-normality step, the viable (ii)⟹(iv) and (ii)⟹(v) routes remain at the source-context per-representative factorization tracked in #6775; the projector-controlled outer-sector assembly is formalized in #6632. See the Chapter 21 printed-status node and `docs/paper-gaps/cpgsv17_pf_rank_one.tex`. |
