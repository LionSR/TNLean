...
| **Def 4.7: Simple tensor** | None of BNT elements is nilpotent | Easy | Formalized for the canonical-block reading as `MPOTensor.IsSourceSimple`: every representative has a positive number of copies, all copy weights are nonzero by the nonzero-coefficient convention, and nonnilpotency is independent of the chosen BNT sector presentation. Positive-length nontriviality is derived rather than assumed; `MPOTensor.IsNonvanishingSourceSimple` is the stronger all-positive-length predicate. |
