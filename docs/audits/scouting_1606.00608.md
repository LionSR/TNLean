...
| Item | Description | Formalizability | Our Status |
|------|-------------|-----------------|------------|
| **Def 4.2: ZCL (mixed)** `DefinitionZCL` | Physical-trace transfer idempotence $\mathcal T_M^2=\mathcal T_M$ (graphical), followed by the claim that MPDO correlations are length-independent | Medium | Historical scouting status: not formalized. Current status (2026-08-29): the literal definition is formalized by `MPOTensor.IsPhysicalTraceIdempotent`. Using the unnormalized `Corr` orientation, `MPOTensor.periodicTwoPointCorrelation_positiveWrapGaps_independent` gives a scope-restricted completion of length independence at fixed arbitrary middle gap between any two positive wrapping gaps; this includes adjacent marked sites. `MPOTensor.periodicTwoPointCorrelation_positiveGaps_independent` gives fixed-ring distance independence for observables nonadjacent on both complementary arcs when all four compared gaps are positive and the gap sums agree. No comparison of a zero varied gap with a positive one is claimed. Blueprint nodes `def:mpo_physical_trace_idempotence`, `def:mpdo_periodic_two_point_correlation`, `thm:mpdo_zcl_periodic_correlation_positive_wrap_gaps_independent`, and `thm:mpdo_zcl_periodic_correlation_positive_gaps_independent` record these statements; `docs/paper-gaps/cpsv16_mpdo_zcl_correlation_length_boundary.tex` records the zero-power boundary. |
...
| **Def 4.7: Simple tensor** | None of BNT elements is nilpotent | Easy | Formalized as `MPOTensor.IsSimple`: every representative has a positive number of copies, all copy weights are nonzero by the nonzero-coefficient convention, and nonnilpotency is independent of the chosen BNT sector presentation. Positive-length nontriviality is derived rather than assumed. |
| **Def 4.8: GSNNCH** `defrhoNComm` | ρ^(N) ∝ ⊕_x n_x exp(-Σ τ_j(h^(x))) with commuting h | Hard | Not formalized. |
| **Thm 4.9 (main, simple case)** `thm:main-simple` | For simple K in biCF: (i)RFP ⟹ (ii)ZCL+SAL ⟺ (iii)GSNNCH+ZCL ⟹ (iv)structural ⟹ (v)blocked RFP | **VERY HARD** | Partial and refuted in two directions. The forward RFP implication and several component results are complete. A five-letter ambient witness has a two-representative BNT presentation in biCF, satisfies all condition-(ii) and line-246 normalization hypotheses, and has a first absorbed representative with no normalized rank-one neighboring-trace factorization. This refutes the printed representativewise assertion in (ii)⟹(iv). The exact scalar repeated-copy capstone refutes literal (iv)⟹(v). See the Chapter 21 false-source node and <https://sirui-lu.com/QICLean/paper-gaps/cpgsv17_pf_rank_one.pdf>. |
