...
| **Def 4.7: Simple tensor** | None of BNT elements is nilpotent | Easy | Formalized for the documented active canonical-block reading as `MPOTensor.IsSourceSimple`: every representative has a positive number of copies, all copy weights are nonzero, and nonnilpotency is independent of the chosen active presentation. The unrestricted literal-BNT reading remains false because dormant candidates may be adjoined. Positive-length nontriviality is derived rather than assumed; `MPOTensor.IsNonvanishingSourceSimple` is the stronger all-positive-length predicate. |
| **Def 4.8: GSNNCH** `defrhoNComm` | ρ^(N) ∝ ⊕_x n_x exp(-Σ τ_j(h^(x))) with commuting h | Hard | Not formalized. |
| **Thm 4.9 (main, simple case)** `thm:main-simple` | For simple K in biCF: (i)RFP ⟹ (ii)ZCL+SAL ⟺ (iii)GSNNCH+ZCL ⟹ (iv)structural ⟹ (v)blocked RFP | **VERY HARD** | Partial and partly refuted. The forward RFP implication and several component results are complete. The exact scalar repeated-copy capstone refutes literal (iv)⟹(v); the viable (ii)⟹(iv) and (ii)⟹(v) routes remain not ready after the false absorbed-normality step and projector-controlled assembly. See the Chapter 21 printed-status node and `docs/paper-gaps/cpgsv17_pf_rank_one.tex`. |
