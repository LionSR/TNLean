...
| **Def 4.7: Simple tensor** | None of BNT elements is nilpotent | Easy | Formalized as `MPOTensor.IsSourceSimple` using an active BNT sector presentation: all copy weights are nonzero, and nonnilpotency is independent of the chosen active presentation. Positive-length nontriviality is derived rather than assumed; `MPOTensor.IsNonvanishingSourceSimple` is the stronger all-positive-length interface. |
