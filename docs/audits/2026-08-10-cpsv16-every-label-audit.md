...
| Example 4.10, `ExZCLnoSAL` | 898 | ZCL does not imply SAL | The printed repeated-left channel is corrected to a left-right flip at $p=1/4$. `CPSVExample410Operator` proves the corrected tensor is an MPDO with source ZCL, and `CPSVExample410Spectrum` proves the full one- through four-site operator spectra, including zero multiplicities $0$, $8$, $48$, and $248$. These spectra are project-derived consequences of the corrected reading, not formulas printed in CPSV16. The tensor block-entropy identifications and non-SAL conclusion remain under issue #6301; no universal claim in $p$ is made |
