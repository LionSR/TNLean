...
After the rectangular-span universality relocation, repository-wide searches
found no non-Archive use of any removed `MPSTensor` name and no blueprint
`\lean{...}` tag citing one. The deleted module had two remaining importers:
...
matrix declaration above, all internal uses are absent, no blueprint tag refers
to an old name, and the pull request records the same replacement map.
