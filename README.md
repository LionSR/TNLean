...
- physical-rotation symmetry corollary: physical `u`-action on MPS implies virtual Z-gauge equivalence (`gaugeEquiv_of_sameMPV_rotatePhysical`; Corollary 4.1 of [arXiv:1708.00029](https://arxiv.org/abs/1708.00029)); preservation of transfer map, left-canonical normalization, irreducibility, periodicity, and irreducible-form structure under physical-index rotation all proved sorry-free (`transferMap_rotatePhysical`, `isLeftCanonical_rotatePhysical`, `isIrreducibleTensor_rotatePhysical`, `isPeriodic_rotatePhysical`, `isIrreducibleForm_rotatePhysical`, `zGaugeEquiv_of_isIrreducibleForm_sameMPV_rotatePhysical`; `MPS/FundamentalTheorem/Applications.lean`)
...
- string order and local symmetry: `twistedTransferMap`, `stringOrderParam`, `IsLocalSymmetry`, `condC2_iff_condC3`, `condC1_imp_condC2`, and `stringOrder_iff_localSymmetry` stated; SPT-phase classification predicate `IsSameSPTPhase` (two tensors in the same phase when their virtual cocycles are cohomologous) defined with `IsSameSPTPhase.isOnSiteSymmetric_left/right` proved and `stringOrder_invariant_of_samePhase` proved; remaining spectral-theory proofs (requiring canonical fixed-point primitivity) carry sorry stubs (`MPS/Symmetry/StringOrder.lean`)
...
- concrete MPS examples in `MPS/Examples/`: GHZ state (`ghzTensor`, d=2 D=2, `ghz_isRFP`, `ghz_isOnSiteSymmetric_Z2`); AKLT state (`akltTensor`, d=3 D=2, `aklt_isNormal`, `aklt_isOnSiteSymmetric_Z2`); even-parity / cluster state (`evenParityTensor`, d=2 D=2, `evenParity_isRFP`, `evenParity_isOnSiteSymmetric_Z2`); shared `ZMod2` helpers
