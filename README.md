...
- peripheral eigenvalue group structure (`PeripheralSpectrum` namespace, `Channel/Peripheral/GroupStructure.lean`): peripheral eigenvalues of irreducible channels form a cyclic group with period dividing dimension (Wolf Thm 6.6, `peripheral_eigenvalues_form_cyclic_group`, `channel_period_divides_dim` proved); multiplicity-one result (`peripheral_eigenvalue_multiplicity_one`) retains one sorry; product closure of peripheral eigenvalues proved sorry-free in `Channel/Peripheral/CyclicGroup.lean`
- spectral gap of injective channels proved (`spectral_gap_of_injective` in `Spectral/QuantitativeGap.lean`); exponential convergence rates (`exponential_convergence_of_primitive`) have 2 remaining sorrys requiring a unique fixed-point adapter and a geometric-bound lemma
...
The repository ships a LeanBlueprint in `blueprint/` covering both the MPS development and the channel-side Wolf material. As of 2026-04-01, the blueprint chapters `ch02b_mpdo.tex`, `ch04_channels.tex`, `ch05_schwarz.tex`, `ch06_spectral.tex`, `ch07_wielandt.tex`, `ch08_canonical.tex`, `ch11_assembly.tex`, `ch12_semigroup.tex`, `ch13_algebraic_ft.tex`, `ch13b_symmetry.tex`, and `ch14_parent_hamiltonian.tex` have been synchronized to reflect current Lean status, including the stationary-support package, periodic tensor definitions, semigroup sorry closures, MPDO foundations, symmetry / string-order / coboundary entries, and chain FT declarations.
