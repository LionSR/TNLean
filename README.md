...
- parent Hamiltonian scaffold (`parentHamiltonian`, `IsFrustrationFree`, `localTerm`), commuting parent Hamiltonians (`IsCommutingParentHam`, `IsNNCPH`, `isNNCPH_of_isRFP`), and decorrelation theorem (`Decorrelation.commutingHam_isDecorrelated`: commuting parent Hamiltonian implies decorrelation, backward direction of arXiv:1606.00608 Prop D.3; forward direction deferred)
- MPO, MPDO, and LPDO foundations (`MPOTensor`, `MPOTensor.mpo`, `IsMPDO`, `IsLPDO`, `toMPSTensor`; following arXiv:1606.00608 §4; LPDO → MPDO proof deferred pending Kronecker / partial-trace infrastructure)
- periodic overlap dichotomy type skeleton (Prop 3.3 of arXiv:1708.00029): `periodicOverlapDichotomy` and supporting lemmas `sectorMatch_propagation`, `periodicSelfOverlap_tendsto` in `MPS/FundamentalTheorem/PeriodicOverlap.lean`; all main proof bodies are sorry'd
- scalar 2-cocycle cohomology (`ScalarCocycle.IsCoboundary`, `CohomologousTo`, `CohomologousTo.equivalence`, `isCoboundary_iff_cohomologousTo_one` in `Algebra/CocycleCohomology.lean`) and gauge independence of the cocycle class for injective symmetric MPS (`MPSTensor.cohomologousTo_of_isInjective` in `MPS/Symmetry/CocycleCoboundary.lean`)
- virtual symmetry equation (`virtual_symmetry_eq`): for an injective on-site-symmetric MPS tensor, physical U(g)-rotation implies gauge-phase equivalence of the virtual representation (`MPS/Symmetry/SymmetricMPS.lean`)
...
- peripheral eigenvalue group structure (`PeripheralSpectrum` namespace, `Channel/Peripheral/GroupStructure.lean`): peripheral eigenvalues of irreducible channels form a cyclic group (Wolf Thm 6.6), with multiplicity-one and period-divides-dimension results; partial sorry stubs building on existing `ClosureFixedPoint` and `CyclicDecomposition` infrastructure; now extended by `Channel/Peripheral/CyclicGroup.lean` with fully proved product closure (`peripheralEigenvalues_mul_mem_of_irreducible_unital_of_adjoint_fixedPoint`) and cyclic characterization (`peripheralEigenvalues_eq_range_primitiveRoot`, 2 sorrys for the Lagrange cardinality step), and the invertibility lemma for peripheral eigenvectors (`isUnit_peripheral_eigenvector`)
...
The repository ships a LeanBlueprint in `blueprint/` covering both the MPS development and the channel-side Wolf material. As of 2026-03-30, the blueprint chapters `ch04_channels.tex`, `ch05_schwarz.tex`, `ch06_spectral.tex`, `ch07_wielandt.tex`, `ch08_canonical.tex`, `ch11_assembly.tex`, `ch12_semigroup.tex`, `ch13_algebraic_ft.tex`, and `ch14_parent_hamiltonian.tex` have been synchronized to reflect current Lean status, including the stationary-support package, periodic tensor definitions, semigroup sorry closures, chain FT declarations, cocycle cohomology entries, virtual symmetry theorem, commuting parent Hamiltonian definitions, and decorrelation theorem.
