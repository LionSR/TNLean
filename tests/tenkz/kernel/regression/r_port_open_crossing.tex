...
%% port.  That one demanded leg enters the saved-route crossing pipeline;
%% unrelated generated legs remain direct strokes.
...
\AtEndDocument
  {
    \int_compare:nNnF { \g__tenkz_test_promoted_port_open_int } = {1}
      {
        \errmessage
          {crossing~demand~did~not~promote~exactly~one~open~port~route}
      }
  }
\ExplSyntaxOff
\makeatother
