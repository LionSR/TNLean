...
/-!
# Channel determinants — definitions, basic properties, and Wolf Thm 6.1 helpers

This companion file contains:
* the core definitions (`channelMatrix`, `channelDet`, `unitaryChannel`);
* the basic determinant identities (`channelDet_eq_linearMap_det`, etc.);
* the eigenvalue-norm bounds for positive trace-preserving maps (Wolf Thm 6.1(1));
* the Heisenberg-dual / Kadison–Schwarz / Skolem–Noether helper chain leading to
  `forward_det_one_implies_unitaryChannel` (Wolf Thm 6.1(2) forward direction).

The user-facing main theorems assembling these pieces are in `Determinant.lean`.
