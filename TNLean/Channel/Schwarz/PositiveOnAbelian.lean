...
section JointDiagonalization

local notation "E" => EuclideanSpace ℂ (Fin D)
local notation "Mat" => Matrix (Fin D) (Fin D) ℂ

private theorem lift_commute_toEuclideanLin {X Y : Mat}
    (hXY : Commute X Y) :
    Commute (Matrix.toEuclideanLin X) (Matrix.toEuclideanLin Y) := by
  rw [Commute]
  apply LinearMap.ext
  intro v
  apply (WithLp.ofLp_injective 2)
  simp [Matrix.ofLp_toEuclideanLin_apply, Matrix.mulVec_mulVec, hXY.eq]

private theorem inner_apply_eq_sum_of_orthonormalBasis
    {s : Type*} [Fintype s] [DecidableEq s]
    (b : OrthonormalBasis s ℂ E)
    (L : E →ₗ[ℂ] E) (μ : s → ℂ)
    (hL : ∀ i, L (b i) = μ i • b i)
    (x y : E) :
    inner ℂ y (L x) = ∑ i, star (b.repr y i) * μ i * b.repr x i := by
  calc
    inner ℂ y (L x) = inner ℂ y (L (∑ i, b.repr x i • b i)) := by
      rw [b.sum_repr x]
    _ = inner ℂ y (∑ i, b.repr x i • L (b i)) := by
      simp [map_sum]
    _ = inner ℂ y (∑ i, (μ i * b.repr x i) • b i) := by
      congr 1
      apply Finset.sum_congr rfl
      intro i hi
      simp [hL, smul_smul, mul_comm, mul_left_comm, mul_assoc]
    _ = ∑ i, star (b.repr y i) * μ i * b.repr x i := by
      simp [inner_sum, Finset.sum_mul, inner_smul_right, b.repr_apply_apply,
        mul_comm, mul_left_comm, mul_assoc]

set_option maxHeartbeats 1500000 in
private theorem exists_common_eigenbasis_of_pairwise_commuting_star_family
    {ι : Type} [Fintype ι] [DecidableEq ι]
    (A : ι → Mat)
    (hA_comm : ∀ i j, Commute (A i) (A j))
    (hA_star_comm : ∀ i j, Commute (A i) (A j)ᴴ) :
    ∃ (b : OrthonormalBasis (Fin (Module.finrank ℂ E)) ℂ E)
      (eig : ι → Fin (Module.finrank ℂ E) → ℂ),
      ∀ i k, Matrix.toEuclideanLin (A i) (b k) = eig i k • b k := by
  classical
  let H : ι → Mat := fun i => ((1 / 2 : ℂ)) • (A i + (A i)ᴴ)
  let K : ι → Mat := fun i => ((Complex.I / 2 : ℂ)) • ((A i)ᴴ - A i)
  have hH : ∀ i, (H i).IsHermitian := by
    intro i
    ext r s
    simp [H, add_comm]
  have hK : ∀ i, (K i).IsHermitian := by
    intro i
    ext r s
    simp [K, sub_eq_add_neg]
    ring
  have hHH : ∀ i j, Commute (H i) (H j) := by
    intro i j
    have hleft : Commute (A i + (A i)ᴴ) (A j) := by
      exact (hA_comm i j).add_left ((hA_star_comm j i).symm)
    have hright : Commute (A i + (A i)ᴴ) (A j)ᴴ := by
      exact (hA_star_comm i j).add_left ((hA_comm i j).star_star)
    simpa [H] using
      ((hleft.add_right hright).smul_left ((1 / 2 : ℂ))).smul_right ((1 / 2 : ℂ))
  have hHK : ∀ i j, Commute (H i) (K j) := by
    intro i j
    have hleft : Commute (A i + (A i)ᴴ) (A j)ᴴ := by
      exact (hA_star_comm i j).add_left ((hA_comm i j).star_star)
    have hright : Commute (A i + (A i)ᴴ) (A j) := by
      exact (hA_comm i j).add_left ((hA_star_comm j i).symm)
    have hsum : Commute (A i + (A i)ᴴ) ((A j)ᴴ - A j) := by
      rw [sub_eq_add_neg]
      simpa using hleft.add_right (hright.smul_right (-1 : ℂ))
    simpa [H, K] using
      (hsum.smul_left ((1 / 2 : ℂ))).smul_right ((Complex.I / 2 : ℂ))
  have hKK : ∀ i j, Commute (K i) (K j) := by
    intro i j
    have hleft : Commute ((A i)ᴴ - A i) (A j)ᴴ := by
      rw [sub_eq_add_neg]
      simpa using ((hA_comm i j).star_star).add_left ((hA_star_comm i j).smul_left (-1 : ℂ))
    have hright : Commute ((A i)ᴴ - A i) (A j) := by
      rw [sub_eq_add_neg]
      simpa using ((hA_star_comm j i).symm).add_left ((hA_comm i j).smul_left (-1 : ℂ))
    have hsum : Commute ((A i)ᴴ - A i) ((A j)ᴴ - A j) := by
      rw [sub_eq_add_neg]
      simpa using hleft.add_right (hright.smul_right (-1 : ℂ))
    simpa [K] using
      (hsum.smul_left ((Complex.I / 2 : ℂ))).smul_right ((Complex.I / 2 : ℂ))
  let T : ι ⊕ ι → E →ₗ[ℂ] E
    | Sum.inl i => Matrix.toEuclideanLin (H i)
    | Sum.inr i => Matrix.toEuclideanLin (K i)
  have hT_symm : ∀ x, (T x).IsSymmetric := by
    intro x
    cases x with
    | inl i =>
        simpa [T] using ((Matrix.isHermitian_iff_isSymmetric (A := H i)).mp (hH i))
    | inr i =>
        simpa [T] using ((Matrix.isHermitian_iff_isSymmetric (A := K i)).mp (hK i))
  have hT_comm : Pairwise (fun x y => Commute (T x) (T y)) := by
    intro x y hxy
    cases x with
    | inl i =>
        cases y with
        | inl j => simpa [T] using lift_commute_toEuclideanLin (hHH i j)
        | inr j => simpa [T] using lift_commute_toEuclideanLin (hHK i j)
    | inr i =>
        cases y with
        | inl j => simpa [T] using lift_commute_toEuclideanLin ((hHK j i).symm)
        | inr j => simpa [T] using lift_commute_toEuclideanLin (hKK i j)
  let V : (ι ⊕ ι → ℂ) → Submodule ℂ E := fun χ => ⨅ j, Module.End.eigenspace (T j) (χ j)
  have hV_sum : DirectSum.IsInternal V := by
    simpa [V] using
      (LinearMap.IsSymmetric.LinearMap.IsSymmetric.directSum_isInternal_of_pairwise_commute
        (T := T) hT_symm hT_comm)
  let bFamily : ∀ χ : ι ⊕ ι → ℂ,
      OrthonormalBasis (Fin (Module.finrank ℂ (V χ))) ℂ (V χ) :=
    fun χ => stdOrthonormalBasis ℂ (V χ)
  let s := Σ χ : ι ⊕ ι → ℂ, Fin (Module.finrank ℂ (V χ))
  let cBasis : Module.Basis s ℂ E :=
    hV_sum.collectedBasis (fun χ => (bFamily χ).toBasis)
  letI : Fintype s :=
    cBasis.fintypeIndexOfRankLtAleph0 (Module.rank_lt_aleph0 ℂ E)
  have hOrthoC : Orthonormal ℂ cBasis := by
    exact DirectSum.IsInternal.collectedBasis_orthonormal
      (hV := LinearMap.IsSymmetric.orthogonalFamily_iInf_eigenspaces (T := T) hT_symm)
      (hV_sum := hV_sum)
      (v_family := fun χ => (bFamily χ).toBasis)
      (hv_family := fun χ => (bFamily χ).orthonormal)
  let b : OrthonormalBasis s ℂ E := cBasis.toOrthonormalBasis hOrthoC
  let eig : ι → s → ℂ := fun i a => a.1 (Sum.inl i) + Complex.I * a.1 (Sum.inr i)
  have hb_eq (a : s) : b a = (((bFamily a.1) a.2 : V a.1) : E) := by
    rw [show b a = cBasis a by
      simpa [b] using congrFun (Module.Basis.coe_toOrthonormalBasis cBasis hOrthoC) a]
    change cBasis a = (((bFamily a.1).toBasis a.2 : V a.1) : E)
    exact congrFun (hV_sum.collectedBasis_coe (fun χ => (bFamily χ).toBasis)) a
  have hTb (a : s) (x : ι ⊕ ι) : T x (b a) = a.1 x • b a := by
    have hx : b a ∈ V a.1 := by
      rw [hb_eq a]
      exact ((bFamily a.1 a.2).2)
    have hxall (y : ι ⊕ ι) : b a ∈ Module.End.eigenspace (T y) (a.1 y) :=
      (iInf_le (fun j => Module.End.eigenspace (T j) (a.1 j)) y) hx
    exact (Module.End.mem_eigenspace_iff).mp (hxall x)
  have hA_decomp (i : ι) : A i = H i + Complex.I • K i := by
    ext r s
    simp [H, K, sub_eq_add_neg]
    ring_nf
    norm_num [Complex.I_sq]
    ring
  have hA_lin_decomp (i : ι) :
      Matrix.toEuclideanLin (A i) = T (Sum.inl i) + Complex.I • T (Sum.inr i) := by
    simpa [T, H, K] using congrArg Matrix.toEuclideanLin (hA_decomp i)
  have hEig : ∀ i k, Matrix.toEuclideanLin (A i) (b k) = eig i k • b k := by
    intro i k
    rw [hA_lin_decomp i]
    simp [eig, hTb k (Sum.inl i), hTb k (Sum.inr i), add_smul, smul_smul]
  let efin : s ≃ Fin (Module.finrank ℂ E) :=
    Fintype.equivFinOfCardEq (by rw [← Module.finrank_eq_card_basis b.toBasis])
  let b' : OrthonormalBasis (Fin (Module.finrank ℂ E)) ℂ E := b.reindex efin
  let eig' : ι → Fin (Module.finrank ℂ E) → ℂ := fun i k => eig i (efin.symm k)
  have hEig' : ∀ i k, Matrix.toEuclideanLin (A i) (b' k) = eig' i k • b' k := by
    intro i k
    simpa [b', eig'] using hEig i (efin.symm k)
  exact ⟨b', eig', hEig'⟩

end JointDiagonalization
...
theorem quadraticForm_nonneg_of_isPositiveMap_of_commuting_images
  ...
  classical
  -- Step 1: Prove block adjoint a j i = (a i j)ᴴ from block-positivity.
  -- From ha, the block quadratic form is always a nonneg real. The imaginary
  -- part being zero for specific ψ forces a(j,i,q,p) = star(a(i,j,p,q)).
  have hBlockAdj : ∀ i j, a j i = (a i j)ᴴ := by
    intro i₀ j₀; ext p₀ q₀
    by_cases hsame : i₀ = j₀ ∧ q₀ = p₀
    · rcases hsame with ⟨rfl, rfl⟩
      have him : (a i₀ i₀ p₀ p₀).im = 0 := by
        have h := (Complex.nonneg_iff.mp (ha (fun i p => if i = i₀ ∧ p = p₀ then 1 else 0))).2
        simpa [dotProduct, Matrix.mulVec] using h.symm
      apply Complex.ext <;> simp [him]
    · -- Need: a j₀ i₀ p₀ q₀ = star (a i₀ j₀ q₀ p₀)
      suffices key : ∀ c : ℂ,
          (starRingEnd ℂ c * (a i₀ j₀ q₀ p₀) + c * (a j₀ i₀ p₀ q₀)).im = 0 by
        have h1 := key 1; have hI := key Complex.I
        simp only [map_one, one_mul, Complex.conj_I, Complex.add_im, Complex.mul_im,
          Complex.neg_im, Complex.neg_re, Complex.I_im, Complex.I_re,
          zero_mul, mul_zero, mul_one, neg_zero, add_zero, zero_add] at h1 hI
        have hre : (a j₀ i₀ p₀ q₀).re = (a i₀ j₀ q₀ p₀).re := by
          linarith [hI]
        have him : (a j₀ i₀ p₀ q₀).im = -(a i₀ j₀ q₀ p₀).im := by
          linarith [h1]
        apply Complex.ext <;> simp [hre, him]
      intro c
      let ψc : Fin n → Fin D → ℂ := fun i p =>
        if i = i₀ ∧ p = q₀ then 1 else if i = j₀ ∧ p = p₀ then c else 0
      have hQc_im := (Complex.nonneg_iff.mp (ha ψc)).2
      change 0 = (∑ i : Fin n, ∑ j : Fin n,
        star (ψc i) ⬝ᵥ (a i j).mulVec (ψc j)).im at hQc_im
      have h := hQc_im
      simp [ψc, hsame, dotProduct, Matrix.mulVec, Finset.sum_ite_irrel,
        Finset.sum_ite_eq', Complex.add_im, mul_comm, mul_left_comm, mul_assoc] at h
      simpa using h.symm
  -- Step 2: Set up the commuting family and get the common eigenbasis.
  let F : (Fin n × Fin n) → Matrix (Fin D) (Fin D) ℂ := fun ij => T (a ij.1 ij.2)
  have hF_comm : ∀ x y : Fin n × Fin n, Commute (F x) (F y) := by
    intro x y; exact hcomm x.1 x.2 y.1 y.2
  have hF_adj : ∀ x : Fin n × Fin n, (F x)ᴴ = F (x.2, x.1) := by
    intro x
    calc
      (F x)ᴴ = T ((a x.1 x.2)ᴴ) := by
        simp [F, (hT.map_conjTranspose (a x.1 x.2)).symm]
      _ = T (a x.2 x.1) := by rw [hBlockAdj x.1 x.2]
      _ = F (x.2, x.1) := rfl
  have hF_star_comm : ∀ x y : Fin n × Fin n, Commute (F x) (F y)ᴴ := by
    intro x y; rw [hF_adj]; exact hF_comm x (y.2, y.1)
  obtain ⟨b, eig, hEig⟩ :=
    exists_common_eigenbasis_of_pairwise_commuting_star_family
      (A := F) hF_comm hF_star_comm
  let s := Fin (Module.finrank ℂ (EuclideanSpace ℂ (Fin D)))
  -- Step 3: Show eigenvalue matrix is PSD for each eigenvector k.
  -- For each eigenvector b_k, the n×n matrix (eig(i,j,k))_ij is PSD because
  -- T applied to the PSD matrix ∑ij star(z_i)*z_j•a_ij gives a PSD result,
  -- and evaluating at eigenvector b_k extracts the eigenvalue quadratic form.
  have hquad (k) (z : Fin n → ℂ) :
      0 ≤ ∑ i, ∑ j, star (z i) * eig (i, j) k * z j := by
    have hXz_psd : (∑ i, ∑ j, (star (z i) * z j) • a i j).PosSemidef := by
      rw [Matrix.posSemidef_iff_dotProduct_mulVec]; constructor
      · -- Hermitian: uses hBlockAdj
        have : (∑ i, ∑ j, (star (z i) * z j) • a i j)ᴴ =
            ∑ i, ∑ j, (star (z i) * z j) • a i j := by
          simp only [Matrix.conjTranspose_sum, Matrix.conjTranspose_smul, hBlockAdj,
            map_mul, star_star, RCLike.star_def]
          rw [Finset.sum_comm]; congr 1; ext i; rw [Finset.sum_comm]; congr 1; ext j
          ring_nf
        exact this
      · -- Nonneg quadratic form: from ha with ψ_i = z_i • w
        intro w
        simpa [Matrix.sum_mulVec, Matrix.smul_mulVec, dotProduct_sum,
          ...
          ha (fun i => z i • w)
    -- Evaluate at eigenvector
    let e : Fin D → ℂ := (b k).ofLp
    have hEig_ofLp (ij : Fin n × Fin n) :
        (T (a ij.1 ij.2)).mulVec e = eig ij k • e := by
      have h := congrArg WithLp.ofLp (hEig ij k)
      simp only [F] at h
      convert h using 1 <;> rfl
    have hTXz_e : (T (∑ i, ∑ j, (star (z i) * z j) • a i j)).mulVec e =
        (∑ i, ∑ j, star (z i) * eig (i, j) k * z j) • e := by
      calc
        (T (∑ i, ∑ j, (star (z i) * z j) • a i j)).mulVec e
            = ∑ i, ∑ j, ((star (z i) * z j) * eig (i, j) k) • e := by
                simp [map_sum, Matrix.sum_mulVec, Matrix.smul_mulVec, hEig_ofLp,
                  smul_smul, mul_assoc]
        _ = (∑ i, ∑ j, star (z i) * eig (i, j) k * z j) • e := by
              simp [Finset.sum_smul, Finset.smul_sum, smul_smul,
                mul_assoc, mul_left_comm, mul_comm]
    have hdot_one : star e ⬝ᵥ e = 1 := by
      have : star e ⬝ᵥ e = @inner ℂ _ _ (b k) (b k) := by
        rw [EuclideanSpace.inner_eq_star_dotProduct]
        simp [e, dotProduct_comm]
      rw [this]
      exact inner_self_eq_one_of_norm_eq_one (b.norm_eq_one k)
    have hn := (hT _ hXz_psd).dotProduct_mulVec_nonneg e
    rw [hTXz_e, dotProduct_smul, hdot_one, smul_eq_mul, mul_one] at hn
    exact hn
  -- Step 4: Decompose the block quadratic form using the eigenbasis.
  have hterm (i j : Fin n) :
      star (ψ i) ⬝ᵥ (T (a i j)).mulVec (ψ j) =
        ∑ k, star (b.repr (WithLp.toLp 2 (ψ i)) k) * eig (i, j) k *
          b.repr (WithLp.toLp 2 (ψ j)) k := by
    simpa [EuclideanSpace.inner_eq_star_dotProduct, dotProduct_comm, F]
      using inner_apply_eq_sum_of_orthonormalBasis (b := b)
        (L := Matrix.toEuclideanLin (F (i, j))) (μ := fun k => eig (i, j) k)
        (hL := fun k => hEig (i, j) k)
        (x := WithLp.toLp 2 (ψ j)) (y := WithLp.toLp 2 (ψ i))
  -- Step 5: Rearrange sums and conclude nonnegativity.
  unfold blockQuadraticForm
  let f : Fin n → Fin n → s → ℂ := fun i j k =>
    star (b.repr (WithLp.toLp 2 (ψ i)) k) * eig (i, j) k *
      b.repr (WithLp.toLp 2 (ψ j)) k
  have hswap : (∑ i : Fin n, ∑ j : Fin n, ∑ k, f i j k) = ∑ k, ∑ i : Fin n, ∑ j : Fin n, f i j k := by
    simp_rw [Finset.sum_comm]
    rw [Finset.sum_comm]
  rw [show (∑ i : Fin n, ∑ j : Fin n, star (ψ i) ⬝ᵥ (T (a i j)).mulVec (ψ j)) =
      ∑ i : Fin n, ∑ j : Fin n, ∑ k, f i j k by
      simp_rw [hterm, f], hswap]
  exact Finset.sum_nonneg (fun k _ =>
    by simpa [f] using hquad k (fun i => b.repr (WithLp.toLp 2 (ψ i)) k))
