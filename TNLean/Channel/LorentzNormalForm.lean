...
lemma infimum_is_attained
  ...
  -- Local Frobenius norm instances for matrices
  letI : NormedAddCommGroup (Matrix (Fin D) (Fin D) ℂ) := Matrix.frobeniusNormedAddCommGroup
  letI : NormedAddCommGroup (Matrix (Fin D × Fin D) (Fin D × Fin D) ℂ) := Matrix.frobeniusNormedAddCommGroup
  let Md := Matrix (Fin D) (Fin D) ℂ
  let MD2 := Matrix (Fin D × Fin D) (Fin D × Fin D) ℂ
  set F : Md × Md → ℝ := fun p => (Matrix.trace (((p.2 ⊗ₖ p.1) * τ * ((p.2 ⊗ₖ p.1)ᴴ)))).re with hF_def
  have hF_cont : Continuous F := by dsimp [F]; continuity
  let Ω : Set Md := {S | S.det = (1 : ℂ)}
  have hΩ_closed : IsClosed Ω := isClosed_singleton.preimage (Continuous.matrix_det continuous_id)
  have hΩ_nonempty : Ω.Nonempty := by refine ⟨1, ?_⟩; simp [Ω]
  have hΩprod_closed : IsClosed (Ω ×ˢ Ω) := IsClosed.prod hΩ_closed hΩ_closed
  have hΩprod_nonempty : (Ω ×ˢ Ω).Nonempty := by
    rcases hΩ_nonempty with ⟨S, hS⟩; exact ⟨(S, S), hS, hS⟩
  have h_lower : ∃ (r : ℝ), 0 < r ∧ ∀ (S : Md), S.det = (1 : ℂ) → r ≤ ‖S‖ := by
    have h_closed : IsClosed {S : Md | S.det = (1 : ℂ)} := hΩ_closed
    have h_ne : (0 : Md) ∉ {S | S.det = (1 : ℂ)} := by simp
    have h_nonempty : {S : Md | S.det = (1 : ℂ)}.Nonempty := hΩ_nonempty
    have h_dist_pos : 0 < Metric.infDist (0 : Md) {S | S.det = (1 : ℂ)} := by
  ...
  obtain ⟨r, hr_pos, hr_bound⟩ := h_lower
  have h_kron_norm (A B : Md) : ‖A ⊗ₖ B‖ = ‖A‖ * ‖B‖ := by
    have h_norm_sq : ‖A ⊗ₖ B‖ ^ 2 = (‖A‖ * ‖B‖) ^ 2 := by
      calc
        ‖A ⊗ₖ B‖ ^ 2 = ∑ i : Fin D × Fin D, ∑ j : Fin D × Fin D, ‖(A ⊗ₖ B) i j‖ ^ 2 := by rw [PiLp.norm_sq_eq_of_L2 (A ⊗ₖ B)]
        _ = ∑ i : Fin D × Fin D, ∑ j : Fin D × Fin D, ‖A i.1 j.1 * B i.2 j.2‖ ^ 2 := by simp [Matrix.kroneckerMap_apply]
        _ = ∑ i : Fin D × Fin D, ∑ j : Fin D × Fin D, (‖A i.1 j.1‖ ^ 2 * ‖B i.2 j.2‖ ^ 2) := by simp [norm_mul, mul_pow]
        _ = (∑ i₁ : Fin D, ∑ j₁ : Fin D, ‖A i₁ j₁‖ ^ 2) * (∑ i₂ : Fin D, ∑ j₂ : Fin D, ‖B i₂ j₂‖ ^ 2) := by simp [Finset.sum_product, Finset.mul_sum, Finset.sum_mul]
        _ = ‖A‖ ^ 2 * ‖B‖ ^ 2 := by simp_rw [PiLp.norm_sq_eq_of_L2 A, PiLp.norm_sq_eq_of_L2 B]
        _ = (‖A‖ * ‖B‖) ^ 2 := by ring
    ...
    nlinarith
  have h_coercivity : ∃ (c : ℝ), 0 < c ∧ ∀ (A : MD2), c * ‖A‖ ^ 2 ≤ (Matrix.trace (A * τ * Aᴴ)).re := by
    let E := MD2
    ...
    have h_sphere_nonempty : (Metric.sphere (0 : E) 1).Nonempty := by
      refine ⟨1, ?_⟩; rw [Metric.mem_sphere, dist_eq_norm, sub_zero, norm_one]
    let f : E → ℝ := fun A => (Matrix.trace (A * τ * Aᴴ)).re
    have hf_cont : Continuous f := by unfold f; continuity
    have hf_contOn : ContinuousOn f (Metric.sphere (0 : E) 1) := hf_cont.continuousOn
    obtain ⟨A₀, hA₀, hmin⟩ := h_sphere_compact.exists_isMinOn h_sphere_nonempty hf_contOn
    have h_norm_A₀ : ‖A₀‖ = 1 := by rw [Metric.mem_sphere, dist_eq_norm, sub_zero] at hA₀; exact hA₀
    have h_A₀_ne_zero : A₀ ≠ 0 := by intro hzero; rw [hzero, norm_zero] at h_norm_A₀; linarith
    have h_f_pos : 0 < f A₀ := by
      have h_psd : τ.PosSemidef := _hτ_posDef.posSemidef
      have h_nonzero_rows : ∃ i : Fin (D * D), (fun j => A₀ i j) ≠ 0 := by
        contrapose! h_A₀_ne_zero; ext i j; exact h_A₀_ne_zero i ▸ rfl
      rcases h_nonzero_rows with ⟨i, hi⟩
      have h_pos_row : 0 < RCLike.re (star (fun j => A₀ i j) ⬝ᵥ (τ.mulVec (fun j => A₀ i j))) := _hτ_posDef.re_dotProduct_pos hi
      unfold f
      have h_nonneg_rows : ∀ i, 0 ≤ RCLike.re (star (fun j => A₀ i j) ⬝ᵥ (τ.mulVec (fun j => A₀ i j))) := by
        intro k; exact h_psd.re_dotProduct_nonneg (fun j => A₀ k j)
      have h_trace_sum : (Matrix.trace (A₀ * τ * A₀ᴴ)).re = ∑ i : Fin (D * D), RCLike.re (star (fun j => A₀ i j) ⬝ᵥ (τ.mulVec (fun j => A₀ i j))) := by
        simp [Matrix.trace, Matrix.mul_apply, Matrix.conjTranspose_apply, Finset.sum_mul, Finset.mul_sum, Complex.re_sum, Matrix.mulVec]
      rw [h_trace_sum]
      exact Finset.sum_pos' h_nonneg_rows ⟨i, Finset.mem_univ _, h_pos_row⟩
    set c := f A₀ with hc_def
    have hc_pos : 0 < c := h_f_pos
    refine ⟨c, hc_pos, fun A => ?_⟩
    by_cases hA_zero : A = 0
    · subst hA_zero; simp [f, c, hc_def, Matrix.trace]
    · have h_norm_pos : 0 < ‖A‖ := norm_pos_iff.mpr hA_zero
      set A' := (‖A‖⁻¹ : ℝ) • A with hA'_def
      have hA'_sphere : A' ∈ Metric.sphere (0 : E) 1 := by
        rw [Metric.mem_sphere, dist_eq_norm, sub_zero, hA'_def]
        calc
          ‖(‖A‖⁻¹ : ℝ) • A‖ = ‖(‖A‖⁻¹ : ℝ)‖ * ‖A‖ := norm_smul _ _
          _ = |‖A‖⁻¹| * ‖A‖ := by simp
          _ = ‖A‖⁻¹ * ‖A‖ := by rw [abs_of_pos (by positivity : 0 < ‖A‖⁻¹)]
          _ = 1 := by field_simp [ne_of_gt h_norm_pos]
      have h_bound : c ≤ f A' := hmin hA'_sphere
      have h_scale : f A' = ((‖A‖⁻¹ : ℝ) ^ 2) * f A := by
        dsimp [f, A']
        calc
          (Matrix.trace (((‖A‖⁻¹ : ℝ) • A) * τ * (((‖A‖⁻¹ : ℝ) • A)ᴴ))).re = (Matrix.trace (((‖A‖⁻¹ : ℂ) ^ 2) • (A * τ * Aᴴ))).re := by
            simp [smul_mul_assoc, mul_smul_comm, smul_smul, mul_assoc, Matrix.conjTranspose_smul]
          _ = RCLike.re (((‖A‖⁻¹ : ℂ) ^ 2) * Matrix.trace (A * τ * Aᴴ)) := by simp [Matrix.trace_smul]
          _ = ((‖A‖⁻¹ : ℝ) ^ 2) * RCLike.re (Matrix.trace (A * τ * Aᴴ)) := by simp [RCLike.re_ofReal_mul]
          _ = ((‖A‖⁻¹ : ℝ) ^ 2) * f A := by simp [f]
      have h_scale_inv : f A = (‖A‖ ^ 2) * f A' := by
        rw [h_scale]
        have h_norm_sq_pos : ‖A‖ ^ 2 ≠ 0 := by positivity
        field_simp [h_norm_sq_pos]; ring
      calc
        c * ‖A‖ ^ 2 ≤ f A' * (‖A‖ ^ 2) := by nlinarith
        _ = f A := by rw [h_scale_inv]
  obtain ⟨c, hc_pos, hc_bound⟩ := h_coercivity
  obtain ⟨⟨S₀₁, S₀₂⟩, hS₀₁, hS₀₂⟩ := hΩprod_nonempty
  set M := max (F (S₀₁, S₀₂)) 0 with hM_def
  let K : Set (Md × Md) := {p | p ∈ Ω ×ˢ Ω ∧ F p ≤ M}
  have hK_closed : IsClosed K := by
    apply IsClosed.inter hΩprod_closed; exact isClosed_Iic.preimage hF_cont
    ...
    · have h_det₁ : p.1.det = (1 : ℂ) := hp₁
      ...
      have h_coer : F p ≥ c * ‖p.1‖ ^ 2 * ‖p.2‖ ^ 2 := by
        calc
          ...
          _ = c * (‖p.1‖ * ‖p.2‖) ^ 2 := by rw [h_kron_norm]
      ...
      have h_sq_bound : ‖p.1‖ ^ 2 ≤ M / (c * r ^ 2) := by
        have h_denom_pos : 0 < c * r ^ 2 := by positivity; nlinarith
  ...
  have h_ball_compact : IsCompact (Metric.closedBall (0 : Md × Md) R) := isCompact_closedBall _ _
  have hK_compact : IsCompact K := IsCompact.of_isClosed_subset h_ball_compact hK_closed h_K_subset_ball
  have hK_nonempty : K.Nonempty := by
    refine ⟨(S₀₁, S₀₂), ⟨hS₀₁, hS₀₂⟩, ?_⟩
    rw [hM_def]; exact le_max_left _ _
  ...
  · have h_mem : (T₁, T₂) ∈ K := ⟨⟨hT₁, hT₂⟩, hF_le_M⟩
    have hle := hmin h_mem; simpa [hF_def] using hle
  · nlinarith
