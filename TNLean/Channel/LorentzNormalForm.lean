...
2. **Doubly-stochastic maps.**  A linear map T is *doubly-stochastic* if both
   ...
   for the generic construction (Wolf Proposition 2.9).

3. **Generic normal form (Wolf Proposition 2.9).**  For a CP map with full Kraus rank
...
The source statement of Wolf Proposition 2.9
(`Notes/WolfNoteTexSource/ch02_representations.tex`, lines 923–929) allows
independent input and output dimensions: for a completely positive map
`T : M_{d₁}(ℂ) → M_{d₂}(ℂ)` with full Kraus rank (positive-definite Choi
matrix), there are SL-filterings on each factor making `Φ₂ ∘ T ∘ Φ₁`
doubly-stochastic.  The proof follows the same path as the square case:
attainment of the infimum (`infimum_is_attained`, now rectangular) followed
by the AM–GM optimality argument on each partial trace. -/
...
private lemma choiMatrixRect_unitaryConj_comp (A : Matrix (Fin d₂) (Fin d₂) ℂ)
    (T : Matrix (Fin d₁) (Fin d₁) ℂ →ₗ[ℂ] Matrix (Fin d₂) (Fin d₂) ℂ) :
...
private lemma choiMatrixRect_comp_unitaryConj (B : Matrix (Fin d₁) (Fin d₁) ℂ)
    (T : Matrix (Fin d₁) (Fin d₁) ℂ →ₗ[ℂ] Matrix (Fin d₂) (Fin d₂) ℂ) :
