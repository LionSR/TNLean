...
-/
import TNLean.Channel.Peripheral.CyclicDecompositionAux

/-!
# Cyclic projections — Fourier-type spectral decomposition

This companion file contains the Fourier-type projection machinery for
cyclic decomposition (Step 2 of Wolf Theorem 6.6):

Given a finite-order peripheral unitary `U` with `U^m = 1` and a primitive
`m`-th root `γ`, the projections `P_k = (1/m) ∑_j γ^{-jk} U^j` are
orthogonal projections that partition unity and are cyclically permuted by
the channel.

The foundational definitions and peripheral-unitary existence are in
`CyclicDecompositionAux.lean`, and the assembled main theorem plus
primitivity transfer are in `CyclicDecomposition.lean`.
-/
