...
theorem fixed_eq_scalar_of_irreducible_unital
  ...
  have hSkew_herm : (Complex.I • (X - Xᴴ)).IsHermitian := by
    ext i j
    simp only [sub_eq_add_neg, smul_add, smul_neg, conjTranspose_apply, add_apply, smul_apply, smul_eq_mul, neg_apply, RCLike.star_def, star_add, star_mul', conj_I, neg_mul, star_neg, RingHomCompTriple.comp_apply, RingHom.id_apply, neg_neg]
    ring
...
private lemma base_cyclic_of_primitiveRoot {γ : ℂ} (hγprim : IsPrimitiveRoot γ m)
  ...
  · have hval : (((k + 1 : Fin m) : ℕ)) = (k : ℕ) + 1 := by
      simp only [Fin.val_add, Fin.coe_ofNat_eq_mod, Nat.add_mod_mod, Nat.mod_eq_of_lt hk, Nat.add_left_cancel_iff]
