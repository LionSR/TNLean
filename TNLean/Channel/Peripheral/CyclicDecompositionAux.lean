...
/-!
# Cyclic decomposition — auxiliary definitions and fixed-point / peripheral-unitary theory

This companion file houses the foundational definitions (`MatrixAlg`, `MatrixEnd`,
`PreservesCorner`, `cornerSubmodule`, `cornerRestriction`, `IsIrreducibleOnCorner`)
and the first two steps of the cyclic decomposition:

1. an irreducible unital Schwarz map has only scalar fixed points;
2. peripheral eigenvalues are represented by unitary eigenvectors that can be
   normalized to finite order.

The cyclic-projection machinery built on these results lives in
`CyclicProjectionsAux.lean`, and the assembled theorems plus primitivity
transfer are in `CyclicDecomposition.lean`.
-/
