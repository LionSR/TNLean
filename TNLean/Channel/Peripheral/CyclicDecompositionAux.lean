...
/-!
# Cyclic decomposition of periodic irreducible channels — Auxiliary definitions

This file contains the algebraic type definitions and the `PeripheralUnitary` section that
are used by the cyclic-projections construction and primitivity results in
`CyclicDecomposition.lean`.

## Contents

- `MatrixAlg D` / `MatrixEnd D` — type aliases for the ambient matrix algebra and its
  endomorphisms.
- `PreservesCorner`, `cornerSubmodule`, `cornerRestriction`, `IsIrreducibleOnCorner` —
  corner-algebra infrastructure.
- Private helpers in `MPSTensor`: spectral decomposition of hermitian fixed points, scalar
  characterization of fixed points of irreducible unital maps.
- Section `PeripheralUnitary` (inside `MPSTensor`): normalization of peripheral eigenvectors
  to unitaries, power formula, and existence of a normalized peripheral unitary of exact
  order `m`.
-/
