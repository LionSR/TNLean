...
/-- Every one of the sixteen standard matrix units belongs to the algebra
generated by `A` and `B`. -/
theorem matrix_unit_mem_adjoin_A_B (i j : Fin 4) :
    e i j ∈ Algebra.adjoin ℂ ({A, B} : Set Mat4) := by
  rw [adjoin_A_B_eq_top]
  trivial
