...
theorem rfp_nt_structural_full (A : MPSTensor d D) [NeZero D]
    ...
    calc
      ∑ p : Fin D × Fin D, E p * Y * (E p)ᴴ
        = ∑ p : Fin D × Fin D,
            ...
            calc
              E p * Y * (E p)ᴴ
                  = (c • Matrix.single p.1 p.2 (1 : ℂ)) * Y *
                      ((c • Matrix.single p.1 p.2 (1 : ℂ))ᴴ) := by
                        rfl
              _ = (c * star c) •
                    (Matrix.single p.1 p.2 (1 : ℂ) * Y *
                      (Matrix.single p.1 p.2 (1 : ℂ))ᴴ) := by
                        simp [Matrix.conjTranspose_smul, Matrix.conjTranspose_single,
                          Matrix.mul_assoc, smul_mul_assoc, mul_smul_comm, smul_smul]
      _ = (c * star c) •
            (∑ p : Fin D × Fin D,
              Matrix.single p.1 p.2 (1 : ℂ) * Y *
                (Matrix.single p.1 p.2 (1 : ℂ))ᴴ) := by
      ...
      _ = (Matrix.trace Y / (D : ℂ)) • (1 : Mat) := by
            rw [hc_norm]
            simp [smul_smul, div_eq_mul_inv, hD_neC, mul_comm]
  ...
  have hRangeCard : D * D ≤ (Set.range B).toFinset.card := by
    have hspan_finrank : Module.finrank ℂ ↥(Submodule.span ℂ (Set.range B)) = D * D := by
      rw [hB_inj, Submodule.finrank_top, Module.finrank_matrix]
      simp
    have hspan_le : Module.finrank ℂ ↥(Submodule.span ℂ (Set.range B)) ≤ (Set.range B).toFinset.card :=
  ...
  have hCard : Fintype.card (Fin D × Fin D) ≤ Fintype.card (Fin d) := by
    have hDD_le_d : D * D ≤ d := by
      calc
        D * D ≤ (Set.range B).toFinset.card := hRangeCard
        _ = Fintype.card (Set.range B) := by simp
        _ ≤ d := by simpa using (Fintype.card_range_le B)
