...
`R` and the complement-side realization of the region-inserted coefficient that
endpoint carries. They are the second endpoint and the second reading feeding the
region resonate step behind the normal PEPS Fundamental Theorem (remaining
obligation 4 of `docs/paper-gaps/peps_normal_ft_section3_route.tex`).

## The out-of-region endpoint
...
The endpoint identity and the basic structural facts that the complement-side
realization needs come first, isolating the dependent-type reindexing
