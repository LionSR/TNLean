...
/-!
# Global-configuration expansion of the region-inserted coefficient
...
This module proves the expansions and fiber identities used for the corresponding
gauge-absorption equality. The region weight of a gauged tensor
...
theorem sum_regionBoundary_fiber (A : Tensor G d) (R : Finset V)
...
/-! ### The double-global-configuration single-vertex-product form

Combining the general-matrix double-global-configuration form
`regionInsertedCoeff_eq_doubleSum` with the agreeing-pair product
`regionComplProd_eq` brings the region-inserted coefficient to a sum, over pairs of
global virtual configurations agreeing off the boundary edge `f`, of one global vertex
product over all vertices, read through `pairOuter`. -/
