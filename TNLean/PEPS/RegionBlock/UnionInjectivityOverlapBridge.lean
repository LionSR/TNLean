...
/-!
# The overlapping union lemma: the bridge row, the overlap re-insertion, and the closure
...
`UnionInjectivityOverlapSetup` defines the left and right three-block geometries, proves the first
inverse application `overlap_firstStrip` and the right-geometry blue-side rebuild, and identifies
the overlap-crossing edges. `UnionInjectivityOverlap3` constructs the `P₀`-outer and
overlap-crossing glues, the indicator-gathering identities, and the bridge row with its
coefficient identity. `RegionBlock.Basic` supplies transport of boundary and physical
configurations and of blocked weights along region equalities, and `UnionInjectivityGeneral`
supplies surjectivity of the fused physical leg of a three-block geometry.

This file first reduces the left first strip through the crossing collapse and proves the bridge
host-coefficient identity in the right geometry's native types. It then restricts the coefficient
family to each `P₀`-outer fiber, applies that chain fiberwise, and extracts every union host
coefficient from the vanishing fiber rows.
...
theorem overlapFiber_bridge_rightCoupling_eq_zero {R₁ R₂ : Finset V}
  ...
  have hcoeff : ∀ bc' : RegionBoundaryConfig (G := G) A
    ...
    exact overlapBridge_geometry_host_coeff_eq (G := G) (A := A)
      (cFiber (A := A) c δ) bβ bc' hHR
