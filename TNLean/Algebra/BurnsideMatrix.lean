...
1. `IsIrreducibleTensor A` → `IsIrreducibleAction A` (true but not formalized here;
   needs orthogonal projection onto a subspace)
2. `IsIrreducibleAction A` → `algSpan A = ⊤` (**Burnside's theorem**, not in Mathlib)
3. `algSpan A = ⊤` → `∃ N, cumulativeSpan A N = ⊤` (**proven**, Part 1)
...
/-! ## Part 3: Burnside's theorem (gap statement and conditional results) -/

/-- Under the hypothesis `algSpan A = ⊤` (Burnside's theorem), `IsIrreducibleAction`
implies the cumulative span reaches ⊤.

This shows the composition of steps 2 and 3 in the bridge:
```
IsIrreducibleAction A
  → algSpan A = ⊤                   (Burnside's theorem — not yet in Mathlib)
  → ∃ N, cumulativeSpan A N = ⊤     (exists_cumulativeSpan_eq_top_of_algSpan_eq_top)
```

**Burnside's theorem for matrix algebras** (over ℂ):
If a set of matrices in `M_D(ℂ)` has no nontrivial common invariant subspace,
then the subalgebra they generate equals all of `M_D(ℂ)`. This is classical
but not yet in Mathlib.

Once Burnside is available, the full sorry-free chain is:
`IsIrreducibleTensor → IsIrreducibleAction → algSpan = ⊤ → ∃ N, cumulativeSpan N = ⊤`. -/
theorem exists_cumulativeSpan_eq_top_of_isIrreducibleAction [NeZero D]
    (A : MPSTensor d D) (_hIrr : IsIrreducibleAction A)
    (hBurnside : algSpan A = ⊤) :
    ∃ N : ℕ, cumulativeSpan A N = ⊤ :=
  exists_cumulativeSpan_eq_top_of_algSpan_eq_top A hBurnside
