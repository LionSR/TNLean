...
This file relates `Kraus.IsIrreducibleFamily` (no nontrivial invariant orthogonal
...
* `isIrreducibleTensor_of_isIrreducibleAction`: `Kraus.IsIrreducibleFamily` follows from
...
theorem evalWord_mem_algSpan (A : MPSTensor d D) (w : List (Fin d)) :
    Kraus.evalWord A w ∈ algSpan A := by
  induction w with
...
theorem evalWord_mul_evalWord_mem_cumulativeSpan
    ...
    Kraus.evalWord A w₁ * Kraus.evalWord A w₂ ∈ Kraus.cumulativeSpan A (m + n) := by
...
theorem mul_mem_cumulativeSpan_add (A : MPSTensor d D) {m n : ℕ}
  ...
  apply LinearMap.BilinMap.apply_apply_mem_of_mem_span
    ...
    (s := {M | ∃ w : List (Fin d), w.length ≤ m ∧ M = Kraus.evalWord A w})
    (t := {M | ∃ w : List (Fin d), w.length ≤ n ∧ M = Kraus.evalWord A w})
...
lemma isIrreducibleTensor_of_isIrreducibleAction
    ...
    Kraus.IsIrreducibleFamily (d := d) (D := D) A := by
