...
block decomposition. -/
noncomputable def blockProjection [(i : ι) → DecidableEq (n i)] (k : ι) :
    Matrix ((i : ι) × n i) ((i : ι) × n i) R :=
...
def IsBlockDiagonal (X : Matrix ((i : ι) × n i) ((i : ι) × n i) R) : Prop :=
...
theorem isBlockDiagonal_iff_offBlock_zero
...
/-- Right multiplication by a block projection keeps exactly the corresponding
column block. -/
@[simp] theorem mul_blockProjection_apply_same
  ...
  rw [Matrix.mul_apply]
  let f : ((i : ι) × n i) → R :=
    fun y => X ⟨i, a⟩ y * blockProjection (n := n) (R := R) j y ⟨j, b⟩
  change ∑ y, f y = X ⟨i, a⟩ ⟨j, b⟩
  calc
    ∑ y, f y = f ⟨j, b⟩ := by
      simpa using Finset.sum_eq_single (s := Finset.univ) (f := f) ⟨j, b⟩
        (by
          rintro ⟨k, c⟩ _ hkc
          by_cases hkj : k = j
          · subst k
            have hcb : c ≠ b := by
              ...
              subst hcb
              rfl
            simp [f, blockProjection, hcb]
          · change X ⟨i, a⟩ ⟨k, c⟩ *
                blockProjection (n := n) (R := R) j ⟨k, c⟩ ⟨j, b⟩ = 0
    ...
    _ = X ⟨i, a⟩ ⟨j, b⟩ := by simp [f, blockProjection]

/-- Left multiplication by a block projection kills rows in all other blocks. -/
@[simp] theorem blockProjection_mul_apply_of_ne
    [Fintype ι] [(i : ι) → Fintype (n i)] [(i : ι) → DecidableEq (n i)]
    (X : Matrix ((i : ι) × n i) ((i : ι) × n i) R)
...
theorem isBlockDiagonal_of_commutes_blockProjection
    [Fintype ι] [(i : ι) → Fintype (n i)] [(i : ι) → DecidableEq (n i)]
    {X : Matrix ((i : ι) × n i) ((i : ι) × n i) R}
    (hComm : ∀ k : ι, X * blockProjection (n := n) (R := R) k =
      blockProjection (n := n) (R := R) k * X) :
    IsBlockDiagonal X := by
  rw [isBlockDiagonal_iff_offBlock_zero]
  intro i j hij a b
