...
theorem exists_partial_isometry_transporting_compressions
  ...
  refine ⟨U, ?_, ?_, ?_, ?_⟩
  · rw [show P * U * Q = P * (VP * Wmat * VQᴴ) * Q from rfl,
      ← Matrix.mul_assoc P VP, hPVP, Matrix.mul_assoc, hVQstarQ]
  · rw [show Uᴴ * U = (VP * Wmat * VQᴴ)ᴴ * (VP * Wmat * VQᴴ) from rfl]
    simp only [Matrix.conjTranspose_mul, Matrix.conjTranspose_conjTranspose]
    simp [Matrix.mul_assoc, hVP_iso, hWstarW, hVQ_range]
  · rw [show U * Uᴴ = (VP * Wmat * VQᴴ) * (VP * Wmat * VQᴴ)ᴴ from rfl]
    simp only [Matrix.conjTranspose_mul, Matrix.conjTranspose_conjTranspose]
    simp [Matrix.mul_assoc, hVQ_iso, hWWstar, hVP_range]
  · intro X
    rw [hφP, hφQ]
    change VP * (Wmat * X * Wmatᴴ) * VPᴴ =
      (VP * Wmat * VQᴴ) * (VQ * X * VQᴴ) * (VP * Wmat * VQᴴ)ᴴ
    simp only [Matrix.conjTranspose_mul, Matrix.conjTranspose_conjTranspose]
    simp [Matrix.mul_assoc, hVQ_iso]
...
theorem exists_partial_isometry_implementing_corner_linearEquiv
    ...
    (hStar : ∀ X : cornerSubmodule Q, (f ⟨X.1ᴴ, by
      rw [← X.2, Matrix.conjTranspose_mul, Matrix.conjTranspose_mul,
        hQ.1.eq, Matrix.conjTranspose_conjTranspose]⟩).1 = (f X).1ᴴ) :
    ∃ U : MatrixAlg D,
      U = P * U * Q ∧
      Uᴴ * U = Q ∧
      U * Uᴴ = P ∧
  ...
  let g : Matrix (Fin nQ) (Fin nQ) ℂ →⋆ₙₐ[ℂ]
    ...
    { toFun := gLinear
      ...
      map_star' := by
        ...
        simp only [gLinear, LinearEquiv.trans_apply, LinearEquiv.apply_symm_apply]
        rw [hφP_star]
        have hfstar := hStar (φQ X)
        rw [hφQ_star] at hfstar
        exact hfstar }
  ...
  | zero =>
      have hQzero : Q = 0 := by
        ...
        rw [← ProjectionSpectralSplit.htrace
          (ProjectionSpectralSplit.ofOrthogonalProjection Q hQ)]
        simp
      have hPzero : P = 0 := by
