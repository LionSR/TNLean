...
/-!
# Recovering Hermitian spectral multiplicities from simple spectrum

For Hermitian matrices of the same finite dimension, equality of their spectra
...
comparing its cardinality with the common root support.  No nonempty-dimension
assumption is needed.

## Main declarations

* `Matrix.IsHermitian.roots_charpoly_eq_of_spectrum_eq_of_roots_nodup`:
  set-spectrum equality upgrades to root-multiset equality when one side has
  simple spectrum.
* `Matrix.IsHermitian.trace_pow_eq_of_spectrum_eq_of_roots_nodup`: the
  resulting equality of all trace powers.
-/
...
/-- If two Hermitian matrices of the same finite dimension have equal spectra
...
theorem roots_charpoly_eq_of_spectrum_eq_of_roots_nodup
  ...
  have hCardA : A.charpoly.roots.card = Fintype.card n := by
    rw [← hA.splits_charpoly.natDegree_eq_card_roots,
      Matrix.charpoly_natDegree_eq_dim]
  have hCardB : B.charpoly.roots.card = Fintype.card n := by
    rw [← hB.splits_charpoly.natDegree_eq_card_roots,
      Matrix.charpoly_natDegree_eq_dim]
...
/-- Under the same hypotheses, all trace powers agree.  This is the
power-sum form of the set-to-multiplicity comparison. -/
theorem trace_pow_eq_of_spectrum_eq_of_roots_nodup
    (hA : A.IsHermitian) (hB : B.IsHermitian)
    (hSpectrum : spectrum ℂ B = spectrum ℂ A)
    (hNodup : A.charpoly.roots.Nodup) (k : ℕ) :
    Matrix.trace (B ^ k) = Matrix.trace (A ^ k) := by
  have hRoots := hA.roots_charpoly_eq_of_spectrum_eq_of_roots_nodup
    hB hSpectrum hNodup
