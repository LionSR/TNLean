...
block** $J_D(\lambda) = \lambda I + N$.  Wolf~\cite[§8]{Wolf2012Quantum}.
...
These algebraic facts are the foundation for the two‑sided operator‑norm
estimate of Wolf~\cite[Eq.~(8.104)]{Wolf2012Quantum}; the norm bounds
themselves are not yet formalized because they require an $\ell^2$ operator‑norm
API (entry bound, $\|N\|=1$, diagonal norm).

## Main declarations

* `nilpotentShift`, `nilpotentShift_pow_apply` — the shift matrix $N$ and
  its superdiagonal power formula.
* `nilpotentShift_pow_D_eq_zero` — $N^D=0$.
* `jordanBlock`, `jordanBlock_commute` — the Jordan block $J_D(\lambda)$ and
  the fact that $\lambda I$ commutes with $N$.
* `jordanBlock_pow` — truncated binomial expansion.
...
* `jordanBlock`, `jordanBlock_commute` — the Jordan block $J_D(\lambda)$ and
  the fact that $\lambda I$ commutes with $N$.
* `jordanBlock_pow` — truncated binomial expansion (blocked, see above).
-/
...
Source: Wolf (2012), lines 1211-1214. -/
...
lemma nilpotentShift_pow_eq_zero_of_gt_sub_one [NeZero D] {k : ℕ} (hk : D - 1 < k) :
    nilpotentShift D ^ k = (0 : Matrix (Fin D) (Fin D) ℂ) := by
...
diagonal matrix. -/
lemma smul_one_eq_scalar (a : ℂ) : a • (1 : Matrix (Fin D) (Fin D) ℂ) = Matrix.scalar (Fin D) a := by
  ext i j; simp [Matrix.scalar_apply, Matrix.one_apply, Matrix.diagonal_apply]

/-- Powers of a scalar matrix: $(a \cdot 1)^m = a^m \cdot 1$. -/
lemma smul_one_pow (a : ℂ) (m : ℕ) : (a • (1 : Matrix (Fin D) (Fin D) ℂ)) ^ m = (a ^ m) • (1 : Matrix (Fin D) (Fin D) ℂ) := by
  calc
    (a • (1 : Matrix (Fin D) (Fin D) ℂ)) ^ m = (Matrix.scalar (Fin D) a) ^ m := by rw [smul_one_eq_scalar]
    _ = Matrix.scalar (Fin D) (a ^ m) := by simpa using (RingHom.map_pow (Matrix.scalar (Fin D)) a m).symm
    _ = (a ^ m) • (1 : Matrix (Fin D) (Fin D) ℂ) := by rw [smul_one_eq_scalar]

/-- The natural-number cast into the matrix ring factors through the scalar
matrix: $n_{\text{matrix}} = n_{\mathbb{C}} \cdot 1$. -/
lemma natCast_matrix_eq (n : ℕ) : ((n : ℕ) : Matrix (Fin D) (Fin D) ℂ) = (n : ℂ) • (1 : Matrix (Fin D) (Fin D) ℂ) := by
  ext i j
  simp [Matrix.natCast_apply, Matrix.one_apply]

/-- One term of the commuting binomial expansion:
\[
(a\cdot 1)^m \, N^{\,n-m} \, (n.choose m)_{\text{matrix}}
= \bigl(\binom{n}{m} a^m\bigr) \bullet N^{\,n-m}.
\] -/
lemma jordanBlock_term (a : ℂ) (n m : ℕ) :
  ...
  calc
    (a • (1 : Matrix (Fin D) (Fin D) ℂ)) ^ m * (nilpotentShift D) ^ (n - m) *
        (Nat.choose n m : Matrix (Fin D) (Fin D) ℂ)
      = ((a ^ m) • (1 : Matrix (Fin D) (Fin D) ℂ)) * (nilpotentShift D) ^ (n - m) *
        (Nat.choose n m : Matrix (Fin D) (Fin D) ℂ) := by
      rw [smul_one_pow]
    _ = ((a ^ m) • (nilpotentShift D) ^ (n - m)) * (Nat.choose n m : Matrix (Fin D) (Fin D) ℂ) := by
      rw [Matrix.smul_mul, Matrix.one_mul]
    _ = ((a ^ m) • (nilpotentShift D) ^ (n - m)) * ((Nat.choose n m : ℂ) • (1 : Matrix (Fin D) (Fin D) ℂ)) := by
...
Wolf~\cite[lines 1211--1212]{Wolf2012Quantum}. -/
lemma jordanBlock_pow (a : ℂ) (n : ℕ) :
  ...
  -- RHS = ∑_{m=0}^{n} (a•I)^m * N^(n-m) * ↑(n.choose m)
  rw [Finset.sum_congr rfl (fun m hm => by rw [jordanBlock_term D a n m])]
  -- Now: ∑_{m=0}^{n} ((n.choose m : ℂ) * a^m) • N^(n-m)
  -- Reindex via m ↦ n-m (a bijection of range (n+1))
  have h_reindex : (∑ m ∈ Finset.range (n + 1), ((n.choose m : ℂ) * a ^ m) • (nilpotentShift D ^ (n - m))) =
      (∑ k ∈ Finset.range (n + 1), ((n.choose k : ℂ) * a ^ (n - k)) • (nilpotentShift D ^ k)) := by
    have hsub_one : (n + 1 : ℕ) - 1 = n := by omega
    have h_reflect := Finset.sum_range_reflect
      (fun j : ℕ => ((n.choose j : ℂ) * a ^ j) • (nilpotentShift D ^ (n - j))) (n + 1)
    simp [hsub_one] at h_reflect
    -- h_reflect: ∑ ((n.choose (n-j)) * a^(n-j)) • N^(n-(n-j)) = ∑ ((n.choose j) * a^j) • N^(n-j)
    have h_reflect' : (∑ j ∈ Finset.range (n + 1), ((n.choose (n - j) : ℂ) * a ^ (n - j)) •
        (nilpotentShift D ^ j)) =
        (∑ j ∈ Finset.range (n + 1), ((n.choose j : ℂ) * a ^ j) • (nilpotentShift D ^ (n - j))) := by
      calc
        (∑ j ∈ Finset.range (n + 1), ((n.choose (n - j) : ℂ) * a ^ (n - j)) • (nilpotentShift D ^ j))
            = (∑ j ∈ Finset.range (n + 1), ((n.choose (n - j) : ℂ) * a ^ (n - j)) •
              (nilpotentShift D ^ (n - (n - j)))) := by
          refine Finset.sum_congr rfl (fun j hj => ?_)
          have hj' : j ≤ n := by
            rw [Finset.mem_range] at hj; omega
          rw [Nat.sub_sub_self hj']
        _ = (∑ j ∈ Finset.range (n + 1), ((n.choose j : ℂ) * a ^ j) • (nilpotentShift D ^ (n - j))) := by
          simpa using h_reflect
    calc
      (∑ m ∈ Finset.range (n + 1), ((n.choose m : ℂ) * a ^ m) • (nilpotentShift D ^ (n - m)))
          = (∑ j ∈ Finset.range (n + 1), ((n.choose (n - j) : ℂ) * a ^ (n - j)) •
            (nilpotentShift D ^ j)) := by
        rw [← h_reflect']
      _ = (∑ k ∈ Finset.range (n + 1), ((n.choose k : ℂ) * a ^ (n - k)) • (nilpotentShift D ^ k)) := by
        refine Finset.sum_congr rfl (fun j hj => ?_)
        have hj' : j ≤ n := by
          rw [Finset.mem_range] at hj; omega
        rw [Nat.choose_symm hj']
  rw [h_reindex]
  -- Now we have ∑_{k=0}^{n} ((n.choose k : ℂ) * a^(n-k)) • N^k
  -- Truncate to k ≤ min(n, D-1) using nilpotence N^D = 0
  -- Terms with D ≤ k ≤ n are zero
  have h_trunc : (∑ k ∈ Finset.range (n + 1), ((n.choose k : ℂ) * a ^ (n - k)) • (nilpotentShift D ^ k)) =
      (∑ k ∈ Finset.range (min n (D - 1) + 1), ((n.choose k : ℂ) * a ^ (n - k)) • (nilpotentShift D ^ k)) := by
    have h_subset : Finset.range (min n (D - 1) + 1) ⊆ Finset.range (n + 1) :=
      Finset.range_subset_range.mpr (by omega)
    have h_zero : ∀ (k : ℕ), k ∈ Finset.range (n + 1) → k ∉ Finset.range (min n (D - 1) + 1) →
