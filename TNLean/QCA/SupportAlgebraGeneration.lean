...
The result concerns the norm closure of the algebraic join \(G\); it does not assert that \(G\)
itself equals the quasi-local algebra or that \(G\) is not already norm closed. Only the finite
local observables are represented on finite regions; a general quasi-local observable enters
through density and continuity.
...
This is one algebraic star-subalgebra of the quasi-local algebra. No algebraic equality with the
whole quasi-local algebra, or separate claim about whether this join is norm closed, is asserted.
...
norm-closed sense appropriate to the quasi-local C-star algebra. The theorem identifies the norm
closure of the algebraic join with the whole chain algebra and makes no stronger claim about the
join itself. See the homogeneous-chain scope statement in the module docstring. -/
