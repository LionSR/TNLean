...
/-- An inverse-Gram estimate at one length is bounded by the corresponding
estimate at any shorter base length. -/
private theorem inverseGram_norm_le_geometric_base
    {A : MPSTensor d D}
    (Iinf : EuclideanSpace ℂ (Fin D × Fin D) →L[ℂ]
      EuclideanSpace ℂ (Fin D × Fin D))
    {c r : ℝ}
    (hc : 0 < c) (hr_pos : 0 < r) (hr_lt_one : r < 1)
    {l n : ℕ} (hln : l ≤ n) (hsmall : c * r ^ l < 1)
    (hLocal : Function.Injective (groundSpaceMapES A n))
    (hRaw : ‖ContinuousLinearMap.inverseGram (groundSpaceMapES A n) hLocal‖ ≤
      (1 - c * r ^ n)⁻¹ * ‖Iinf‖) :
    ‖ContinuousLinearMap.inverseGram (groundSpaceMapES A n) hLocal‖ ≤
      (1 - c * r ^ l)⁻¹ * ‖Iinf‖ := by
  calc
    _ ≤ (1 - c * r ^ n)⁻¹ * ‖Iinf‖ := hRaw
    _ ≤ (1 - c * r ^ l)⁻¹ * ‖Iinf‖ :=
      mul_le_mul_of_nonneg_right
        (inverse_one_sub_geometric_le_base hc hr_pos hr_lt_one hln hsmall)
        (norm_nonneg Iinf)
...
theorem IsPrimitiveMPS.wholeIncrement_groundProjection_defect_le_geometric
  ...
  have hInvTail : ‖ContinuousLinearMap.inverseGram
    ...
    simpa only [B, q] using inverseGram_norm_le_geometric_base
      (A := A) (c := c) (r := r) (l := L) (n := L + Q) Iinf
      hc hr_pos hr_lt_one (by omega) hsmall hLocalTail hInvTailRaw
  have hInvLeft : ‖ContinuousLinearMap.inverseGram
      (groundSpaceMapES A (K + L)) hLocalLeft‖ ≤ B := by
    simpa only [B, q] using inverseGram_norm_le_geometric_base
      (A := A) (c := c) (r := r) (l := L) (n := K + L) Iinf
      hc hr_pos hr_lt_one (by omega) hsmall hLocalLeft hInvLeftRaw
  have hInvFull : ‖ContinuousLinearMap.inverseGram
      (groundSpaceMapES A (K + L + Q)) hFull‖ ≤ B := by
    simpa only [B, q] using inverseGram_norm_le_geometric_base
      (A := A) (c := c) (r := r) (l := L) (n := K + L + Q) Iinf
      hc hr_pos hr_lt_one (by omega) hsmall hFull hInvFullRaw
  have hSqrtTailLeft :
      Real.sqrt ‖ContinuousLinearMap.inverseGram
          (groundSpaceMapES A (L + Q)) hLocalTail‖ *
        Real.sqrt ‖ContinuousLinearMap.inverseGram
    ...
    ∃ C c r : ℝ,
      ...
      ∀ K l : ℕ, 1 ≤ l → c * r ^ l < 1 →
        ‖openChainTailGroundProjectionES A K (l + 1) ∘L
          ...
          (1 - c * r ^ l)⁻¹ * (C * r ^ l) := by
  obtain ⟨C, c, r, hC, hc, hr_pos, hr_lt_one, hBound⟩ :=
    hP.wholeIncrement_groundProjection_defect_le_geometric hρ
  refine ⟨C, c, r, hC, hc, hr_pos, hr_lt_one, ?_⟩
  intro K l hl hsmall
  simpa only [reassocTailBoundaryMapES_one, openChainTailGroundProjectionES,
    openChainLeftGroundProjectionES, range_tailBoundaryMapES,
    range_leftBoundaryMapES_one] using hBound K l 1 hl hsmall
