...
/-!
# Tail virtual Gram operator: adjoint-self identity and fourth-power entry-mass bound
-/
...
lemma frobenius_vec_mul_norm_sq_le (X M : Matrix (Fin D) (Fin D) ℂ) :
  ...
  have hU_sq (A : Matrix (Fin D) (Fin D) ℂ) :
      ‖U A‖ ^ 2 = ∑ i : Fin D, ∑ j : Fin D, ‖A i j‖ ^ 2 := by
    calc
      ‖U A‖ ^ 2 = (∑ p : (Fin D × Fin D), ‖(U A) p‖ ^ 2) := by
        rw [EuclideanSpace.norm_sq_eq]
      _ = (∑ p : (Fin D × Fin D), ‖A p.2 p.1‖ ^ 2) := by
        dsimp [U, Matrix.frobeniusEquivEuclidean]
      _ = (∑ j : Fin D, ∑ i : Fin D, ‖A i j‖ ^ 2) := by
        simpa only [Finset.univ_product_univ] using
          (Finset.sum_product'
            (s := (Finset.univ : Finset (Fin D)))
            (t := (Finset.univ : Finset (Fin D)))
            (f := fun j i ↦ ‖A i j‖ ^ 2))
      _ = ∑ i : Fin D, ∑ j : Fin D, ‖A i j‖ ^ 2 := Finset.sum_comm
  have hUX := hU_sq X
  have hUM := hU_sq M
  have hUprod := hU_sq (X * M)
  rw [hUprod, hUX, hUM]
  calc
...
theorem tailVirtualMapES_norm_four_pow_le_transferMap_pow_one_entry_norm_sq
  ...
  have hRHS : (∑ p : (Fin D × Fin D), ‖M p.1 p.2‖ ^ 2) = ‖U M‖ ^ 2 := by
    rw [hU_sq M, Fintype.sum_prod_type]
  calc
