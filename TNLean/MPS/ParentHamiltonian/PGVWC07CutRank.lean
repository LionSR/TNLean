...
**Formalized here.** The block expansion of the amplitudes of a canonical form; the
simultaneous product span of the blocks of a canonical form at every length at least
\(\max(L_0, 3(b-1)(L_0+1))\), from condition C1 in each block and the direct-sum lemma; and
the rank inequality of the claim at lines 2193–2204, in the form: if the functions of the
first piece obtained by fixing the second piece lie in a subspace \(V\), then
\(\sum_j D_j^2\le\dim V\).

## Main results

* `Kraus.isIrreducibleFamily_of_isNBlkInjective` — block injectivity at some length rules out
  a nontrivial invariant projection.
...
* `MPSTensor.PGVWC07CanonicalFormData.trace_evalWord_eq_sum`,
  `MPSTensor.PGVWC07CanonicalFormData.mpv_eq_sum` — the amplitudes of a canonical form are the
  weighted sums of the block amplitudes.
...
/-- Bridge: a block-injective family is irreducible. If the words of one length span the full
matrix algebra, an invariant orthogonal projection `P`, with `(1 - P) K_i P = 0`, satisfies
`(1 - P) X P = 0` for every matrix `X`, hence is `0` or `1`. This is the implication from
condition C1 to irreducibility used implicitly in arXiv:quant-ph/0608197, lines 1314–1326. -/
theorem isIrreducibleFamily_of_isNBlkInjective {K : Fin d → Matrix (Fin D) (Fin D) ℂ} {L : ℕ}
    (hK : IsNBlkInjective K L) : IsIrreducibleFamily K := by
  rintro ⟨P, ⟨-, hPP⟩, hP0, hP1, hinv⟩
  have hword : ∀ w : List (Fin d), (1 - P) * evalWord K w * P = 0 := by
    intro w
    induction w with
    | nil => simp [Matrix.sub_mul, hPP]
    | cons i w ih =>
      have hE : evalWord K w * P = P * evalWord K w * P := by
        rw [Matrix.sub_mul, Matrix.sub_mul, Matrix.one_mul] at ih
        exact sub_eq_zero.mp ih
      calc (1 - P) * evalWord K (i :: w) * P = (1 - P) * K i * (evalWord K w * P) := by
            rw [evalWord_cons]; simp only [Matrix.mul_assoc]
        _ = (1 - P) * K i * P * (evalWord K w * P) := by
            conv_lhs => rw [hE]
            simp only [Matrix.mul_assoc]
        _ = 0 := by rw [hinv i, Matrix.zero_mul]
  have hall : ∀ X : Matrix (Fin D) (Fin D) ℂ, (1 - P) * X * P = 0 := by
    intro X
    have hX : X ∈ Submodule.span ℂ
        (Set.range fun σ : Fin L → Fin d => evalWord K (List.ofFn σ)) := by
      rw [hK.span_eq_top]; exact Submodule.mem_top
    induction hX using Submodule.span_induction with
    | mem x hx => obtain ⟨σ, rfl⟩ := hx; exact hword _
    | zero => simp
    | add x y _ _ hx hy => rw [Matrix.mul_add, Matrix.add_mul, hx, hy, add_zero]
    | smul a x _ hx => rw [Matrix.mul_smul, Matrix.smul_mul, hx, smul_zero]
  obtain ⟨j, b, hjb⟩ : ∃ j b, P j b ≠ 0 := by
    by_contra hcon
    push Not at hcon
    exact hP0 (Matrix.ext hcon)
  obtain ⟨a, i, hai⟩ : ∃ a i, (1 - P) a i ≠ 0 := by
    by_contra hcon
    push Not at hcon
    exact hP1 (sub_eq_zero.mp (Matrix.ext hcon)).symm
  have h := congrFun (congrFun (hall (Matrix.single i j 1)) a) b
  simp only [Matrix.mul_apply, Matrix.single_apply, Matrix.zero_apply] at h
  simp only [ite_and, mul_ite, mul_one, mul_zero, Finset.sum_ite_eq, Finset.mem_univ,
    ↓reduceIte, ite_mul, zero_mul] at h
  exact mul_ne_zero hai hjb h
...
theorem isNBlkInjective_of_isNBlkInjective_conjTranspose
  ...
  have key : ∀ Y ∈ Submodule.span ℂ
      ...
      Yᴴ ∈ Submodule.span ℂ (Set.range fun σ : Fin L → Fin d => evalWord K (List.ofFn σ)) := by
    intro Y hY
...
The source's standing assumption that the block states are pairwise different (line 1327) is
taken in the form `BlocksNotGaugePhaseEquiv`, as in the direct-sum lemma it invokes. The
length `L₀` in the threshold covers a single block, where the direct-sum lemma is vacuous and
the source length `3(b-1)(L₀+1)` is zero; see `docs/paper-gaps/rmp_w_state_ti_bound.tex`. -/
theorem wordTupleSpanTop_of_ge {L₀ : ℕ} (hL₀ : 0 < L₀)
  ...
  · choose Λ hΛ _ hΛfix using h.dual_fixedPoint
    refine wordTupleSpanTop_of_ge_of_bnt_directSum_unital_c1_pgvwc07_of_dualFixedPoint
      h.blocks hr (HasIrreducibleBlocks.ofForall fun k =>
        Kraus.isIrreducibleFamily_of_isNBlkInjective (hC1 k))
...
noncomputable def blockTracePairing {r : ℕ} {dim : Fin r → ℕ}
    ...
    ((k : Fin r) → Matrix (Fin (dim k)) (Fin (dim k)) ℂ) →ₗ[ℂ] ((Fin R → Fin d) → ℂ) where
...
theorem sum_sq_dim_le_finrank_of_forall_mem {r : ℕ} {dim : Fin r → ℕ}
  ...
  have hT : Submodule.span ℂ (Set.range T) = ⊤ := by
    refine eq_top_iff.mpr fun M _ => ?_
    have hM : (fun k => (c k)⁻¹ • M k) ∈ Submodule.span ℂ (Set.range (wordTuple A R')) := by
