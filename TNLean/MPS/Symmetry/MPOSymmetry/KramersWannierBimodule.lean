...
* `KWExample.idTensor`, `KWExample.flipTensor`: the bond-one tensors of `1` and `η`.
...
private theorem toMPSTensor_finProdFinEquiv {d D : ℕ} (M : MPOTensor d D) (i j : Fin d) :
...
matrices. -/
theorem mpo_apply_of_bondOne [NeZero N] {d : ℕ} (M : MPOTensor d 1) (σ τ : Fin N → Fin d) :
...
/-- The bond-one tensor of the identity, `δ_{s's}`. -/
def idTensor : MPOTensor 2 1 := fun i j => Matrix.of fun _ _ => if i = j then 1 else 0
...
/-- The identity tensor generates the identity at every positive length. -/
theorem mpo_idTensor [NeZero N] : idTensor.mpo N = 1 := by
  ext σ τ
  rw [mpo_apply_of_bondOne, Matrix.one_apply]
  simp only [idTensor, Matrix.of_apply]
...
theorem z2Tensor_isNormal (a : Fin 2) : Kraus.IsNormal (z2Tensor a).toMPSTensor := by
  fin_cases a
  · exact isNormal_of_bondOne idTensor 0 0 (by simp [idTensor])
  · exact isNormal_of_bondOne flipTensor 1 0 (by simp [flipTensor])

theorem mpo_z2Tensor_zero [NeZero N] : (z2Tensor 0).mpo N = 1 := mpo_idTensor
