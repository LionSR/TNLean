...
`MPOTensor.perronFrobeniusDim_eq_of_dimension_relation`, the invertibility of `ψ` and
non-invertibility of `σ`, and the identity `O_σ (O_1 - O_ψ) = 0` of the periodic operators.
...
* `IsingTwist.perronFrobeniusDim_isingFusion_psi`, `IsingTwist.perronFrobeniusDim_isingFusion_one`:
  `d_ψ = d_1 = 1`.
* `IsingTwist.mpo_isingBlock_sigma_mul_sub`: `O_σ (O_1 - O_ψ) = 0`.
...
/-- **`O_σ` annihilates the range of `O_1 - O_ψ`** (arXiv:1601.07185, lines 1051–1055): the
fusion rules `σ × 1 = σ × ψ = σ` give `O_σ (O_1 - O_ψ) = 0` at every positive length. -/
theorem mpo_isingBlock_sigma_mul_sub {N : ℕ} (hN : 0 < N) :
    mpo (isingBlock 2) N * (mpo (isingBlock 0) N - mpo (isingBlock 1) N) = 0 := by
  have h0 := isMPOFusionAlgebra_ising 2 0 N hN
  have h1 := isMPOFusionAlgebra_ising 2 1 N hN
  simp only [Fin.sum_univ_three, isingFusion, Fin.isValue, Matrix.cons_val_zero,
    Matrix.cons_val_one, Matrix.cons_val_two, Matrix.head_cons, Matrix.tail_cons,
    Nat.cast_one, Nat.cast_zero, one_smul, zero_smul, zero_add] at h0 h1
  rw [mul_sub, h0, h1, sub_self]
