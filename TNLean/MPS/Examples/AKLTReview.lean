...
are computed exactly. They give `A^- = σ_x/√2` and `A^0 = σ_z/√2` as printed, and
`A^+ = -σ_y/√2`. The printed `A^+ = σ_y/√2` holds exactly in the basis whose vector
`|+⟩` carries the phase `-i` in place of `i`. The review's tensor is a nonzero scalar
...
the tensor is `-σ_y/√2`, not `σ_y/√2`; the printed Pauli form holds for
`|+⟩ = -i(|-1⟩+|+1⟩)/√2`. Documented in `docs/paper-gaps/rmp_aklt_pauli_basis_sign.tex`.
...
* `MPSTensor.physicalBasisChange` : coordinates of a tensor in an orthonormal physical basis
...
## Main results
* `MPSTensor.mpv_physicalBasisChange` : the tensor coordinates are the coordinates of the
  state in the product basis
* `MPSTensor.physicalBasisChange_akltRMPBasis` : the review's basis gives
  `(σ_x, -σ_y, σ_z)/√2`
* `MPSTensor.physicalBasisChange_akltRMPBasisFixed` : the conjugated basis gives the
...
/-- Project result: the coordinates of a tensor in the physical basis whose `a`-th vector is
the column `V · a`, `B^a = ∑_s conj(V_{s a}) A^s`. For an orthonormal basis these are the
tensor components of the same state in the new basis (`mpv_physicalBasisChange`). -/
def physicalBasisChange {d D : ℕ} (V : Matrix (Fin d) (Fin d) ℂ) (A : MPSTensor d D) :
    MPSTensor d D :=
  fun a => ∑ s, star (V s a) • A s

/-- Project result: the state coefficients of `physicalBasisChange V A` are the inner
products of the state of `A` with the product vectors `e_{τ₁} ⊗ ⋯ ⊗ e_{τ_N}`, where
`e_a` is the `a`-th column of `V`. -/
theorem mpv_physicalBasisChange {d D : ℕ} (V : Matrix (Fin d) (Fin d) ℂ) (A : MPSTensor d D)
    {N : ℕ} (τ : Fin N → Fin d) :
    mpv (physicalBasisChange V A) τ =
      ∑ σ : Fin N → Fin d, (∏ k, star (V (σ k) (τ k))) * mpv A σ := by
  classical
  simp only [mpv_eq, coeff_eq]
  rw [evalWord_ofFn_eq_prod]
  simp_rw [physicalBasisChange]
  rw [List.prod_ofFn_sum, Matrix.trace_sum]
  refine Finset.sum_congr rfl (fun σ _ => ?_)
  rw [List.prod_ofFn_smul, evalWord_ofFn_eq_prod, Matrix.trace_smul, smul_eq_mul]
...
component reversed. Documented in `docs/paper-gaps/rmp_aklt_pauli_basis_sign.tex`. -/
theorem physicalBasisChange_akltRMPBasis :
    physicalBasisChange akltRMPBasis akltTensorRMP =
      fun a => (if a = 1 then -1 else 1 : ℂ) • akltTensorRMPPauli a := by
  funext a
  fin_cases a <;> ext i j <;> fin_cases i <;> fin_cases j <;>
    simp [physicalBasisChange, akltRMPBasis, akltTensorRMPPauli, Fin.sum_univ_three,
...
theorem physicalBasisChange_akltRMPBasisFixed :
    physicalBasisChange akltRMPBasisFixed akltTensorRMP = akltTensorRMPPauli := by
  funext a
  fin_cases a <;> ext i j <;> fin_cases i <;> fin_cases j <;>
    simp [physicalBasisChange, akltRMPBasisFixed, akltTensorRMPPauli, Fin.sum_univ_three,
      akltTensorRMP_zero, akltTensorRMP_one, akltTensorRMP_two, SpinCover.pauli] <;>
    ring_nf
...
/-- On-site symmetry is invariant under a nonzero rescaling of the tensor. -/
private lemma isOnSiteSymmetric_smul {A : MPSTensor d D} {U : G →* Matrix (Fin d) (Fin d) ℂ}
    (c : ℂ) (hA : IsOnSiteSymmetric A U) : IsOnSiteSymmetric (c • A) U := by
  intro g N σ
  have htw : twistedTensor (c • A) U g = fun i => c • twistedTensor A U g i := by
    funext i
    simp only [twistedTensor, Pi.smul_apply, Finset.smul_sum, smul_comm c]
  rw [htw]
  change mpv (fun i => c • A i) σ = _
  rw [mpv_smul, mpv_smul, hA g N σ]

/-- On-site symmetry is invariant under a virtual gauge transformation. -/
private lemma isOnSiteSymmetric_of_gaugeEquiv {A B : MPSTensor d D}
...
theorem akltTensorRMP_isOnSiteSymmetric_Z2Z2 :
  ...
  isOnSiteSymmetric_of_gaugeEquiv (isOnSiteSymmetric_smul _ aklt_isOnSiteSymmetric_Z2Z2)
    akltTensorRMP_gaugeEquiv
...
theorem akltTensorRMPPauli_isOnSiteSymmetric_SO3 :
  ...
  isOnSiteSymmetric_smul _ aklt_isOnSiteSymmetric_SO3
