...
/-!
# GHZ and product states: the upper-triangular sources generate the review's states

**Source.** Cirac, Pérez-García, Schuch, Verstraete 2021 (arXiv:2011.12127), Appendix A,
"One dimension: MPS", `Papers/2011.12127/TN-Review-main.tex` lines 2332–2345: a product state is
a bond-one MPS whose amplitude multiplies the local amplitudes (lines 2332–2336), and the GHZ
state `∑_i |i, …, i⟩` is the MPS with `A^i_{αβ} = δ_{i=α=β}` (lines 2338–2345). The
upper-triangular source tensors `ghzB` and `repB` are constructions of this development, not
models of the review.

**Formalized here.** For positive chain length, `ghzB` generates the unnormalized GHZ state
$|0\cdots0\rangle+|1\cdots1\rangle$ of the review's `ghzTensor`, and the bond-one sector tensors
`ghzC` generate the two product states. The tensor `repB` generates the product state
$2(|0\rangle+2|1\rangle)^{\otimes N}$ at every length. These identifications use the proved
word-trace identities of the two examples.

At length zero, `ghzB` has amplitude four, whereas `ghzTensor` has amplitude two, so their
equality is restricted to positive length. The factor two for `repB` already agrees with its
empty-word trace.

## Main results

* `MPSTensor.ghzC_mpv`, `MPSTensor.ghzTensor_mpv`: the product sectors and the review's GHZ
  tensor.
* `MPSTensor.ghzB_mpv`, `MPSTensor.ghzB_sameMPV₂Pos_ghzTensor`, `MPSTensor.ghzB_mpvState_eq`:
  `ghzB` generates the GHZ state at every positive length.
* `MPSTensor.ghzB_not_sameMPV₂_ghzTensor`: the identification fails at length zero.
* `MPSTensor.repA_mpv`, `MPSTensor.repB_mpv`, `MPSTensor.repB_mpvState_eq`: the product state
  generated by the repeated target and by `repB`.
* `MPSTensor.ghzB_mpvState_norm_sq`, `MPSTensor.repA_mpvState_norm_sq`,
  `MPSTensor.repB_mpvState_norm_sq`: the squared norms of these states.
...
The tensors `ghzB` and `repB` were first recorded as Examples B and C of
`Notes/OpenProblemsTN/strategies/p5_asymmetric_compression_theorem.tex` (lines 939–995,
`ex:p5ft-ghz` and `ex:p5ft-jordan`); these are verification records, not the source.
-/
