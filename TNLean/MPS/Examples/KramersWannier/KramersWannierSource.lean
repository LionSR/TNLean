...
`TNLean.MPS.Examples.KramersWannier.KramersWannierAction` identifies these states with
...
`TNLean.MPS.Examples.KramersWannier.KramersWannierCircuit` identifies `𝖣` with the Hadamard image of their
