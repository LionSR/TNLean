...
import TNLean.MPS.CanonicalForm.AssemblyReduction
import TNLean.MPS.CanonicalForm.AssemblyCyclicBridge
open scoped Matrix BigOperators ComplexOrder MatrixOrder
open Filter

/-!
# Assembly — blocked irreducibility, weak FT, and structural FT

This file assembles the remaining pieces of the MPS canonical form construction:

* `isIrreducibleTensor_blockTensor_of_tp_primitive_irr` — TP + primitive + irreducible original
  → irreducible blocked tensor (the most complex proof in the assembly).
* `weakFundamentalTheorem_conditional` — two proportional tensors with normal canonical form
  → block matching.
* `fundamentalTheorem_after_blocking_1606_structural` — SameMPV₂ tensors → both admit
  TP-primitive decompositions.

The main reduction theorem, per-block normality, and blocking infrastructure live in
`AssemblyReduction.lean`. The cyclic sector bridge is in `AssemblyCyclicBridge.lean`.
-/
