...
* MPSTensor.PGVWC07CanonicalFormData.isIrreducibleFamily_blocks - the three
  canonical conditions make every block irreducible.
...
/-- Bridge: every block of a canonical form is irreducible. Conditions 1–3 of
arXiv:quant-ph/0608197, Theorem 4 (lines 753–757) force it: if `(1 - P) A_i P = 0` for an
orthogonal projection `P`, then `P - ∑_i A_i P A_i^†` is the positive matrix
`∑_i B_i B_i^†`, `B_i = P A_i (1 - P)`, whose pairing with the full-rank fixed point `Λ` of
the dual map vanishes; so `P` is a fixed point of `X ↦ ∑_i A_i X A_i^†`, hence a scalar
projection, `0` or `1`. -/
theorem isIrreducibleFamily_blocks (k : Fin h.r) : Kraus.IsIrreducibleFamily (h.blocks k) := by
  classical
  rintro ⟨P, ⟨hPh, hPP⟩, hP0, hP1, hinv⟩
  obtain ⟨Λ, hΛ, -, hΛfix⟩ := h.dual_fixedPoint k
  have hPh' : star P = P := hPh
  have hterm : ∀ i, h.blocks k i * P * (h.blocks k i)ᴴ =
      P * (h.blocks k i * (h.blocks k i)ᴴ) * P -
        P * h.blocks k i * (1 - P) * (P * h.blocks k i * (1 - P))ᴴ := fun i => by
    have hKP : h.blocks k i * P = P * h.blocks k i * P := by
      have := hinv i
  ...
  have hsum : P - Kraus.transferMap (d := d) (D := h.dim k) (h.blocks k) P =
      ∑ i, P * h.blocks k i * (1 - P) * (P * h.blocks k i * (1 - P))ᴴ := by
    rw [Kraus.transferMap_apply, Finset.sum_congr rfl fun i _ => hterm i, Finset.sum_sub_distrib,
      ← Finset.sum_mul, ← Finset.mul_sum, h.unital k, Matrix.mul_one, hPP, sub_sub_cancel]
  have hpsd : (P - Kraus.transferMap (d := d) (D := h.dim k) (h.blocks k) P).PosSemidef := by
    rw [hsum]
    exact Matrix.posSemidef_sum _ fun i _ => Matrix.posSemidef_self_mul_conjTranspose _
  have htr : Matrix.trace
      (Λ * (P - Kraus.transferMap (d := d) (D := h.dim k) (h.blocks k) P)) = 0 := by
  ...
  have hfix : Kraus.transferMap (d := d) (D := h.dim k) (h.blocks k) P = P :=
    (sub_eq_zero.mp (Matrix.posSemidef_eq_zero_of_posDef_trace_mul_eq_zero hpsd hΛ htr)).symm
  obtain ⟨c, hc⟩ := h.fixedPoint_scalar k P hfix
