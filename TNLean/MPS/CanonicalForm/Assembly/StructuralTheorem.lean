...
/-- **Common live-block sector comparison from a BNT proportional-decomposition conclusion.**

A proportional-decomposition comparison supplies a common MPV phase cover of the two
live-block families.  Therefore the common-cover sector theorem applies without an
extra finite-length span hypothesis. -/
theorem fundamentalTheorem_after_blocking_1606_sector_of_common_blocks_proportionalConclusion
    {d D₁ D₂ p rA rB : ℕ}
    {dimA : Fin rA → ℕ} {dimB : Fin rB → ℕ}
    [∀ k : Fin rA, NeZero (dimA k)]
    [∀ k : Fin rB, NeZero (dimB k)]
...
/-- **Zero-tail sector comparison from a BNT proportional-decomposition conclusion.**

This zero-tail-aware variant combines the exact live-block cancellation step with
the BNT proportional-decomposition comparison.  The proportional comparison gives
the common MPV phase cover, hence the finite-length span equality for the live
blocks; injectivity supplies the remaining primitive overlap-span input. -/
theorem fundamentalTheorem_after_blocking_1606_sector_of_common_blocks_proportionalConclusion_zeroTail
    {d D₁ D₂ p rA rB zeroTailA zeroTailB : ℕ}
    {dimA : Fin rA → ℕ} {dimB : Fin rB → ℕ}
    [∀ k : Fin rA, NeZero (dimA k)]
    [∀ k : Fin rB, NeZero (dimB k)]
