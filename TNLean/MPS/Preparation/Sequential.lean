...
/-- The first-row matrix `|0⟩⟨φ|`-type left boundary with row `0` equal to `v`. -/
private def rowMat (v : Fin D → ℂ) : Matrix (Fin D) (Fin D) ℂ :=
  fun α β => if α.val = 0 then v β else 0

private theorem rowSupp_rowMat (v : Fin D → ℂ) : RowSupp 1 (rowMat v) := fun α β hα => by
  simp [rowMat, show α.val ≠ 0 by omega]

private theorem rowMat_mul_mulVec_zero (hD : 0 < D) (v w : Fin D → ℂ)
    (M : Matrix (Fin D) (Fin D) ℂ) :
    ((rowMat v * M) *ᵥ w) ⟨0, hD⟩ = v ⬝ᵥ (M *ᵥ w) := by
  rw [← Matrix.mulVec_mulVec]
  simp [rowMat, Matrix.mulVec, dotProduct]

/-- The basis vector `|0⟩` of the ancilla. -/
private def e₀ (D : ℕ) : Fin D → ℂ := fun β => if β.val = 0 then 1 else 0

private theorem dotProduct_e₀ (hD : 0 < D) (w : Fin D → ℂ) :
    star (e₀ D) ⬝ᵥ w = w ⟨0, hD⟩ := by
...
private theorem star_e₀_dotProduct_e₀ (hD : 0 < D) : star (e₀ D) ⬝ᵥ e₀ D = 1 := by
  rw [dotProduct_e₀ hD]; simp [e₀]
...
theorem isProbabilisticallyGenerated_of_hasOBCRep {ψ : (Fin N → Fin d) → ℂ} {c : ℂ}
  ...
  obtain ⟨B, hB⟩ := h
  have hD := B.pos_bound
  refine ⟨fun k => OBCChainTensor.zeroPad B (Fin.rev k), c⁻¹ • e₀ D, e₀ D, funext fun τ => ?_⟩
  ...
  rw [seqAmplitude, chainProd_rev_rev (fun p => OBCChainTensor.zeroPad B p),
    Matrix.mulVec_smul, dotProduct_smul, dotProduct_e₀ hD]
  have : (chainProd (fun p => OBCChainTensor.zeroPad B p) τ *ᵥ e₀ D) ⟨0, hD⟩ =
      chainProd (fun p => OBCChainTensor.zeroPad B p) τ ⟨0, hD⟩ ⟨0, hD⟩ := by
    rw [Matrix.mulVec, dotProduct, Finset.sum_eq_single ⟨0, hD⟩]
    · simp [e₀]
    · intro b _ hb
      have : b.val ≠ 0 := fun h => hb (Fin.ext h)
      simp [e₀, this]
    · simp
  rw [this, ← hτ, smul_eq_mul, ← mul_assoc, inv_mul_cancel₀ hc, one_mul]
...
theorem isProbabilisticallyGenerated_of_isDeterministicallyGenerated [NeZero d]
  ...
  have hF' : ∑ α, star (φF α) * φF α = 1 := hF
  rw [← mul_one (ψ τ), ← hF', Finset.mul_sum]
...
theorem isDeterministicallyGenerated_of_hasOBCRep [NeZero d] {ψ : (Fin N → Fin d) → ℂ}
  ...
  | zero =>
    have hψ' : star (ψ Fin.elim0) * ψ Fin.elim0 = 1 := by
      simpa [dotProduct, Unique.eq_default] using hψ
    refine ⟨fun k => Fin.elim0 k, ψ Fin.elim0 • e₀ D, e₀ D, fun k => Fin.elim0 k, ?_,
      star_e₀_dotProduct_e₀ hD, fun τ α => ?_⟩
    · rw [star_smul, smul_dotProduct, dotProduct_smul, star_e₀_dotProduct_e₀ hD]
      simpa [mul_comm] using hψ'
    · rw [show τ = Fin.elim0 from Subsingleton.elim _ _]
      simp [jointState, mul_comm]
  | succ n =>
    obtain ⟨b, Q, r', hb0, hbD, hrow, hcol, hiso, hr', hprod⟩ :=
      exists_isometric_chain (n + 1) 1 hD (rowMat (fun β => c⁻¹ * e₀ D β)) (rowSupp_rowMat _)
        (fun p => OBCChainTensor.zeroPad B p) (e₀ D)
    -- The chain output is `ψ τ • |0⟩`.
    have hout : ∀ τ, chainProd Q τ *ᵥ r' = fun α => e₀ D α * ψ τ := by
      intro τ
      funext α
      by_cases hα : α.val = 0
      · have hα' : α = ⟨0, hD⟩ := Fin.ext hα
        ...
        rw [OBCChainTensor.coeff_eq_chainProd_zeroPad] at hτ
        simp only [Pi.smul_apply, smul_eq_mul] at hτ
        have : (fun β => c⁻¹ * e₀ D β) ⬝ᵥ
            (chainProd (fun p => OBCChainTensor.zeroPad B p) τ *ᵥ e₀ D) =
            c⁻¹ * chainProd (fun p => OBCChainTensor.zeroPad B p) τ ⟨0, hD⟩ ⟨0, hD⟩ := by
          rw [dotProduct, Finset.sum_eq_single ⟨0, hD⟩]
          · simp [e₀, Matrix.mulVec, dotProduct, Finset.sum_ite_eq']
          · intro b _ hb
            have : b.val ≠ 0 := fun h => hb (Fin.ext h)
            simp [e₀, this]
          · simp
        rw [this, ← hτ, ← mul_assoc, inv_mul_cancel₀ hc, one_mul]
        simp [e₀]
    ...
    have hr'norm : star r' ⬝ᵥ r' = 1 := by
      ...
      · intro b _ hb
        ...
        simp [e₀, this]
      · simp
    refine ⟨fun k => U (Fin.rev k), r', e₀ D, fun k => hU _, hr'norm,
      star_e₀_dotProduct_e₀ hD, fun τ α => ?_⟩
