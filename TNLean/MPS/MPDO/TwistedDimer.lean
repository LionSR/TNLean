...
open question at `Papers/1606.00608/MPDO-22-12-17-2.tex` line 995: whether
there are RFP tensors whose structure coefficients
$c^{(L)}_{\alpha,\beta,\gamma}=\operatorname{tr}(\chi^L_{\alpha,\beta,\gamma})$
of Theorem 4.14 (lines 972--993) depend on $L$. The fixed-point property of
Definition 4.1 (lines 657--660) is `MPOTensor.TwistedDimer.isRFPViaTS_T`
(`TNLean.MPS.MPDO.TwistedDimerViaTS`); the length-dependent coefficients
$\alpha^L,\beta^L$ with $\alpha=7/10$, $\beta=1/10$ are attached to the
displayed flag-sector presentation in `TNLean.MPS.MPDO.TwistedDimerProductLaw`
and `TNLean.MPS.MPDO.TwistedDimerBNTAlgebraClause`, and are shown to survive
every positive rescaling of the two labels in
`TNLean.MPS.MPDO.TwistedDimerCoefficients`. The flag factor of the operator is
the density-normalized tensor of Example 4.12 (lines 932--939); see
`TNLean.MPS.MPDO.TwistedDimerFactorStates`.
...
The construction, its channels, and exact-arithmetic checks were first recorded
in `Notes/OpenProblemsTN/strategies/p6_round44_graded_dimer_twist.tex`,
Section `sec:p6-round44-z2` and Theorem `thm:p6-r44-z2`; they are verification
