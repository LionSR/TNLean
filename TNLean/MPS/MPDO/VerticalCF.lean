...
structure HorizontalCFData {r : ℕ} {dim : Fin r → ℕ}
  ...
  **Scope restriction (per-block separation):** the paper's Lemma L consumes
  the block-injectivity proposition at the level of basis-of-normal-tensors
  representatives, not per block: gauge-equivalent copies of one
  representative are grouped before the trace-separation step. Demanding
  `biCF` per block as here is strictly stronger and fails on gauge-equivalent
  copies (`duplicateScalarBlocks_not_hasBiCF` in
  `TNLean/MPS/MPDO/BiCFDerivation/Counterexample.lean`). Documented in
...
IV.12. The proof consumes the `biCF` field of `HorizontalCFData` directly: the
weighted blockwise difference at the biCF blocking length pairs to zero
against every block word, so `biCF` forces it to vanish.

**Scope restriction (per-block separation):** the source's Lemma L (lines
1835--1858) separates at the level of basis-of-normal-tensors representatives,
grouping gauge-equivalent copies of one representative and disposing of the
resulting weight sum by the nonvanishing power-sum step of Lemma app_simple.
The `biCF` hypothesis instead demands trace separation simultaneously across
every block, including gauge-equivalent copies, which is a strictly stronger
condition: `duplicateScalarBlocks_not_hasBiCF` in
`TNLean/MPS/MPDO/BiCFDerivation/Counterexample.lean` exhibits two identical
blocks with distinct nonzero weights for which `biCF` fails. Documented in
`docs/paper-gaps/cpgsv17_bicf_block_separation.tex`. -/
