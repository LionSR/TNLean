...
of the appendix proof of Theorem `thm:main-simple` (`AppendixMixed`, lines
1324–1825) in arXiv:1606.00608, in order to conclude
...
`MPOTensor.ExplicitEtaOperators.ofHayashiMarkov`. The remaining `AppendixMixed`
proof step is no longer merely to write down such sector-local operators; it is
to
...
The intended future Proposition `3to4` (line 1569) proof should construct
...
* [Cirac--Perez-Garcia--Schuch--Verstraete 2017] arXiv:1606.00608,
  `AppendixMixed` (lines 1324–1825), Proposition `3to4` (line 1569)
-/
...
presentation `ρ⁽ᴺ⁾ = c · ∏ᵢ B_{i,i+1}`.  In the appendix proof of Theorem
`thm:main-simple` (`AppendixMixed`, lines 1324–1825), this bond is the object
eventually assembled from the explicit neighboring operators `η_{k,h}`. -/
...
/-- The explicit `η`-local structure needed for the `AppendixMixed` proof after
the sector-local neighboring operators have been assembled in the original
tensor coordinates.
...
Source: arXiv:1606.00608, `AppendixMixed`, Proposition 3to4, lines 1571--1593:
...
`3to4` (line 1569) of arXiv:1606.00608.
...
form. Adding ZCL gives item (iii) of Theorem `thm:main-simple` (line 851) in the
simple-MPDO case. -/
