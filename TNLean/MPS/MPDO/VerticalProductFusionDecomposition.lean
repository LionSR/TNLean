...
theorem transportedVerticalSector_exists_positiveFusionDecomposition
    ...
    ∃ (chi : DiagonalChiFamily (Fin g₁))
      (U : ∀ α β : Fin g₁,
        Matrix
          ((γ : Fin g₁) × (Fin (chi.dim α β γ) × Fin (h.dim₁ γ)))
          (Fin (h.dim₁ α * h.dim₁ β)) ℂ),
      chi.PosEntries ∧
      (∀ α β, U α β * (U α β)ᴴ = 1) ∧
      (∀ (α β : Fin g₁) (i j : Fin D),
        U α β *
            (mulTensor (verticalBNTMPO (h.A₁ α))
              (verticalBNTMPO (h.A₁ β))) i j *
            (U α β)ᴴ =
          Matrix.blockDiagonal' fun γ =>
            chi.matrix α β γ ⊗ₖ verticalBNTMPO (h.A₁ γ) i j) ∧
      ∀ (α β : Fin g₁) (i j : Fin D),
        (mulTensor (verticalBNTMPO (h.A₁ α))
          (verticalBNTMPO (h.A₁ β))) i j =
          (U α β)ᴴ *
