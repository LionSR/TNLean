...
`(A i, B i)` is the full product algebra.  The remaining admitted input is the
exact homogeneous padding step needed here: from that full pair algebra,
represent the pair identity `(1, 1)` in every sufficiently long homogeneous
pair-word span. -/
