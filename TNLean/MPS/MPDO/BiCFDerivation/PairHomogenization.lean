...
/- Burnside-Jacobson period-window identity padding for a full pair algebra.

If pair words of length at most `S` span the full product algebra, then the pair
identity occurs in one positive-period length and in every residue class of a
sufficiently large complete window, after allowing multiplication by words of
length at most `S`. -/
private theorem
  ...
  -- Burnside-Jacobson period-window identity padding.
  sorry
...
The pair algebra generated by the pairs `(A i, B i)` is the full product
algebra.  Hence the pair identity `(1, 1)` belongs to the homogeneous pair-word
span at every sufficiently large length. -/
