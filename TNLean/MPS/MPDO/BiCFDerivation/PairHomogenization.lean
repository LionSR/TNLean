...
/- The remaining Burnside-Jacobson homogenization input for the full-product
pair algebra.

The proved subdirect-product theorem already gives
`pairAlgSpan A B = M_D(ℂ) × M_D(ℂ)` for injective, non-gauge-equivalent blocks.
What remains is not another trace-separation statement: it is the homogeneous
padding step saying that the unit `(1, 1)` can be represented by words of every
sufficiently large single length.  A stronger way to prove this would be to
...
The subdirect-product branch proves that the pair algebra generated by
`(A i, B i)` is the full product algebra.  This theorem still depends on the
private admitted certificate
`exists_pairIdentity_mem_pairWordTupleSpan_padding_window_of_pairCumulativeWordTupleSpanTop`:
from that full pair algebra, one must represent the pair identity `(1, 1)` in
every sufficiently long homogeneous pair-word span. -/
theorem pairIdentity_mem_pairWordTupleSpan_eventually_of_not_gaugePhaseEquiv
    ...
    ∃ L : ℕ, ∀ n : ℕ, n ≥ L →
      ((1 : Matrix (Fin D) (Fin D) ℂ), (1 : Matrix (Fin D) (Fin D) ℂ)) ∈
        Submodule.span ℂ (Set.range (pairWordTuple A B n)) := by
  exact exists_pairIdentity_mem_pairWordTupleSpan_eventually_of_pairAlgSpan_eq_top
    A B (pairAlgSpan_eq_top_of_injective_not_gaugePhaseEquiv A B hA_inj hB_inj hNot)
...
span at every length `≥ L`.  This conclusion is supplied through
`pairIdentity_mem_pairWordTupleSpan_eventually_of_not_gaugePhaseEquiv`, which
still depends on the private admitted Burnside--Jacobson period-window
certificate above. -/
