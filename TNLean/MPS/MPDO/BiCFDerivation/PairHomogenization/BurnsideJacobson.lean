...
This module contains the finite-family separation and Burnside–Jacobson bridge
to `PairTraceSeparatingAt`.  It imports `Algebra` and `Span` from the
pair-homogenization subpackage and completes the pair-span homogenization layer.
-/
