...
/-- The algebra clause of arXiv:1606.00608, Theorem IV.13(ii), for fixed
BNT-label operators, trace scalars, and diagonal matrices.

The coefficients are those canonically determined by the diagonal matrices,
`c⁽ᴸ⁾_{α,β,γ} = tr(χ_{α,β,γ}^L)`. The clause asserts positivity of the
diagonal entries, this trace-power form at every positive length, the
same-length product law, and the length-one idempotent scalar law. It does not
include the separate blocked support-algebra comparison.

Source: arXiv:1606.00608, Theorem IV.13(ii), lines 972--985, and Appendix C.4,
lines 1929--1942 of `Papers/1606.00608/MPDO-22-12-17-2.tex`. -/
structure BNTAlgebraClause {Λ : Type*} [Fintype Λ] {O : ℕ → Type*}
    ...
    (m : BNTLabelTraceScalarFamily Λ) (χ : DiagonalChiFamily Λ) : Prop where
  /-- Every diagonal entry of `χ_{α,β,γ}` is positive.

  Source: arXiv:1606.00608, Theorem IV.13(ii), lines 972--985. -/
  posEntries : χ.PosEntries
  /-- The coefficients are traces of powers of `χ_{α,β,γ}` at positive lengths.

  Source: arXiv:1606.00608, Theorem IV.13(ii), lines 972--980. -/
  tracePower :
    (BNTLabelCoefficientFamily.ofChi χ).HasPositiveLengthChiTracePowerForm χ
  /-- The operators satisfy the same-length product law with the trace-power coefficients.
...
`Papers/1606.00608/MPDO-22-12-17-2.tex`. -/
theorem to_bntAlgebraClause
    (m : BNTLabelTraceScalarFamily Λ)
    (hIdempotent :
      m.HasIdempotentCoefficientForm (BNTLabelCoefficientFamily.ofChi Fam.chi)) :
    BNTAlgebraClause Fam.toOperatorFamily m Fam.chi := by
  exact ⟨Fam.posEntries,
    BNTLabelCoefficientFamily.ofChi_hasPositiveLengthChiTracePowerForm Fam.chi,
    Fam.toOperatorFamily_hasSameLengthProductForm, hIdempotent⟩
