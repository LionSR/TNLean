...
Two small overlap / inner-product auxiliary lemmas, used by the heterogeneous
equal-case BNT block matching in this file's siblings
(`Full.NondecayingOverlap`, `Full.BlocksMatch`):
...
in $\mathbb{C}$ to norm convergence, lives in
`TNLean.MPS.Overlap.SelfOverlapAux` (imported above) to avoid duplication with the
BNT permutation-rigidity argument.
