...
the entrywise image of a matrix along it, the compatibility of that image with products, sums,
the bond-space product of matrix product operator tensors, and the normality criterion of
`P6Compression.isNormal_of_single_eq_smul` transported to this ring. The two-object Ising example
...
* `MPSTensor.complexOfZsqrt2_mul`, `MPSTensor.mulTensor_complexOfZsqrt2`,
  `MPSTensor.actTensor_complexOfZsqrt2`: matrix products, the bond-space product and the
  bond-space action commute with the embedding.
...
noncomputable def complexOfZsqrt2 {m n : Type*} (X : Matrix m n (ℤ√2)) : Matrix m n ℂ :=
  X.map zsqrt2ToComplex
...
theorem complexOfZsqrt2_mul {m n o : Type*} [Fintype n] (X : Matrix m n (ℤ√2))
    (Y : Matrix n o (ℤ√2)) :
    complexOfZsqrt2 (X * Y) = complexOfZsqrt2 X * complexOfZsqrt2 Y := by
  ext i j
  simp only [complexOfZsqrt2_apply, Matrix.mul_apply, map_sum, map_mul]

theorem complexOfZsqrt2_one {n : Type*} [DecidableEq n] :
    complexOfZsqrt2 (1 : Matrix n n (ℤ√2)) = 1 := by
  ext i j
  simp only [complexOfZsqrt2_apply, Matrix.one_apply]
  split <;> simp

theorem complexOfZsqrt2_zero {m n : Type*} :
    complexOfZsqrt2 (0 : Matrix m n (ℤ√2)) = 0 := by
  ext i j
  simp

theorem complexOfZsqrt2_add {m n : Type*} (X Y : Matrix m n (ℤ√2)) :
    complexOfZsqrt2 (X + Y) = complexOfZsqrt2 X + complexOfZsqrt2 Y := by
  ext i j
  simp

theorem complexOfZsqrt2_sum {m n κ : Type*} (t : Finset κ) (f : κ → Matrix m n (ℤ√2)) :
    complexOfZsqrt2 (∑ x ∈ t, f x) = ∑ x ∈ t, complexOfZsqrt2 (f x) := by
  ext i j
  simp [Matrix.sum_apply]

theorem complexOfZsqrt2_single {m n : Type*} [DecidableEq m] [DecidableEq n] (i : m) (j : n) :
    complexOfZsqrt2 (Matrix.single i j (1 : ℤ√2)) = Matrix.single i j (1 : ℂ) := by
  ext a b
  simp only [complexOfZsqrt2_apply, Matrix.single_apply]
  split <;> simp

theorem complexOfZsqrt2_smul {m n : Type*} (c : ℤ√2) (X : Matrix m n (ℤ√2)) :
    complexOfZsqrt2 (c • X) = zsqrt2ToComplex c • complexOfZsqrt2 X := by
  ext i j
  simp [Matrix.smul_apply]

theorem complexOfZsqrt2_submatrix {m n m' n' : Type*} (X : Matrix m n (ℤ√2)) (f : m' → m)
    (g : n' → n) : complexOfZsqrt2 (X.submatrix f g) = (complexOfZsqrt2 X).submatrix f g := rfl

theorem complexOfZsqrt2_transpose {m n : Type*} (X : Matrix m n (ℤ√2)) :
    complexOfZsqrt2 Xᵀ = (complexOfZsqrt2 X)ᵀ := rfl

variable {d D D₁ D₂ : ℕ}

/-- The bond-space product of two tensors over `ℤ√2`,
`(M · N)^{ik} = ∑_j M^{ij} ⊗ N^{jk}`, in the bond order of `finProdFinEquiv`. -/
def mulZsqrt2Tensor (M : Fin d → Fin d → Matrix (Fin D₁) (Fin D₁) (ℤ√2))
...
/-- The bond-space product commutes with the entrywise embedding of `ℤ√2`. -/
theorem mulTensor_complexOfZsqrt2 (M : Fin d → Fin d → Matrix (Fin D₁) (Fin D₁) (ℤ√2))
    (N : Fin d → Fin d → Matrix (Fin D₂) (Fin D₂) (ℤ√2)) (i k : Fin d) :
    MPOTensor.mulTensor (fun i j => complexOfZsqrt2 (M i j))
        (fun i j => complexOfZsqrt2 (N i j)) i k =
      complexOfZsqrt2 (mulZsqrt2Tensor M N i k) := by
  ext x y
  simp only [MPOTensor.mulTensor_apply, mulZsqrt2Tensor, Matrix.submatrix_apply,
    Matrix.sum_apply, Matrix.kroneckerMap_apply, complexOfZsqrt2_apply, map_sum, map_mul]

/-- The bond-space action of a matrix product operator tensor over `ℤ√2` on a state tensor over
`ℤ√2`, `(M · A)^i = ∑_j M^{ij} ⊗ A^j`, in the bond order of `finProdFinEquiv`, matching
`MPOTensor.actTensor`. -/
def actZsqrt2Tensor (M : Fin d → Fin d → Matrix (Fin D₁) (Fin D₁) (ℤ√2))
    (A : Fin d → Matrix (Fin D₂) (Fin D₂) (ℤ√2)) (i : Fin d) :
    Matrix (Fin (D₁ * D₂)) (Fin (D₁ * D₂)) (ℤ√2) :=
  (∑ j : Fin d, (M i j) ⊗ₖ (A j)).submatrix finProdFinEquiv.symm finProdFinEquiv.symm

/-- The bond-space action commutes with the entrywise embedding of `ℤ√2`. -/
theorem actTensor_complexOfZsqrt2 (M : Fin d → Fin d → Matrix (Fin D₁) (Fin D₁) (ℤ√2))
    (A : Fin d → Matrix (Fin D₂) (Fin D₂) (ℤ√2)) (i : Fin d) :
    MPOTensor.actTensor (fun i j => complexOfZsqrt2 (M i j)) (fun j => complexOfZsqrt2 (A j)) i =
      complexOfZsqrt2 (actZsqrt2Tensor M A i) := by
  ext x y
  simp only [MPOTensor.actTensor_apply, actZsqrt2Tensor, Matrix.submatrix_apply,
    Matrix.sum_apply, Matrix.kroneckerMap_apply, complexOfZsqrt2_apply, map_sum, map_mul]
