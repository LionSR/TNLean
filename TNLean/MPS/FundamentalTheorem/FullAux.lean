...
/-!
# Full FT auxiliary — overlap dichotomy and non-decaying overlap existence

This companion file houses the overlap-based core lemmas used by the full
heterogeneous equal-case Fundamental Theorem (`Full.lean`):

* `mpvOverlap_eq_selfOverlap_of_forall_mpv_eq` — overlap identity for pointwise-equal MPVs;
* `gaugePhaseEquiv_of_block_sameMPV₂` — per-block SameMPV₂ implies dim-equality + GPE;
* `exists_nondecaying_overlap_of_sameMPV₂_CFBNT` — strong-induction proof that every block
  has a non-decaying cross-family overlap partner (CPSV17, Appendix A).

The main theorem (`fundamentalTheorem_equalMPV_CFBNT_hetero`) and the block-matching
composition lemma remain in `Full.lean`.
-/
