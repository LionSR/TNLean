...
BNT canonical forms.  The primary matcher assumes only:

* both decompositions are `IsBNTCanonicalForm`, and
* eventual nonzero proportionality $V^{(N)}(P) = c_N V^{(N)}(Q)$
  (`EventuallyNonzeroProportionalMPV₂`).

It is the proportional analogue of the exact equal-MPV matcher: partition the
`P`-sectors by whether all overlaps against `Q` decay, rewrite the complementary
sectors as exact scalar multiples of `Q`-sector states, and apply fixed-length
linear independence to the resulting relation.  The proportionality scalar
$c_N$ only changes the aggregated `Q`-coefficients; the chosen `P`-coefficient
is still isolated exactly and contradicts `IsBNTCanonicalForm.coeff_not_eventually_zero`.

The older Cesàro/projection route, which assumes per-sector unit-modulus copy
weights and proves `joint_coeff_not_tendsto_zero`, remains below as legacy API
but is no longer used by the proportional sector-matching chain.
...
lemma mpvState_eq_smul_of_mpv_eq_mul
    ...
    mpvState (d := d) A N = c • mpvState (d := d) B N := by
  apply PiLp.ext
  intro σ
