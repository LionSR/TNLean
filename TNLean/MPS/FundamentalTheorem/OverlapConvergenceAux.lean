...
- `TNLean.Convergence.bounded_mul_tendsto_zero`: bounded geometric × tending-to-zero → 0
- `TNLean.Convergence.geometric_mul_bounded_tendsto_zero`:
    geometric(< 1) × bounded → 0
- `TNLean.Convergence.sum_tendsto_one_of_diag`: diagonal-dominant sum → 1
- `MPSTensor.tendsto_mpvInner_zero`: mpvOverlap → 0 implies mpvInner → 0
- `MPSTensor.tendsto_mpvInner_one`: mpvOverlap → 1 (self) implies mpvInner → 1
- `MPSTensor.geometric_mul_inner_tendsto_zero`:
    geometric(< 1) × Cauchy-Schwarz-bounded inner → 0
-/

open scoped Matrix BigOperators
open Filter

namespace TNLean.Convergence

/-- If `‖c‖ ≤ 1` and `f N → 0`, then `c ^ N * f N → 0`.

This is a convenience wrapper around `Filter.Tendsto.zero_mul_isBoundedUnder_le`
(from `Mathlib.Analysis.Normed.Ring.Lemmas`) specialised to the common `c ^ N` pattern
where the bound `‖c‖ ≤ 1` gives a simpler API than assembling `IsBoundedUnder` manually. -/
...
This is a convenience wrapper around `tendsto_pow_atTop_nhds_zero_of_norm_lt_one` composed
with `Filter.Tendsto.zero_mul_isBoundedUnder_le` (from `Mathlib.Analysis.Normed.Ring.Lemmas`),
specialised to the explicit-constant bound pattern used throughout the FT proof. -/
...
lemma tendsto_mpvInner_zero {D₁ D₂ : ℕ} (X : MPSTensor d D₁) (Y : MPSTensor d D₂)
...
`mpvOverlap = star ∘ mpvInner`. -/
lemma tendsto_mpvInner_one {D : ℕ} (X : MPSTensor d D)
    (hOv : Tendsto (fun N => mpvOverlap (d := d) X X N) atTop (nhds 1)) :
    Tendsto (fun N => mpvInner (d := d) X X N) atTop (nhds 1) := by
  simpa [mpvOverlap_eq_star_mpvInner] using hOv.star

/-- `‖mpvState Z N‖ ^ 2 = ‖mpvInner Z Z N‖` via `inner_self_eq_norm_sq_to_K`. -/
private lemma norm_sq_eq_norm_mpvInner {D : ℕ} (Z : MPSTensor d D) (N : ℕ) :
    ‖mpvState (d := d) Z N‖ ^ 2 = ‖mpvInner (d := d) Z Z N‖ := by
  have heq : mpvInner (d := d) Z Z N = ↑(‖mpvState (d := d) Z N‖ ^ 2 : ℝ) := by
    unfold mpvInner
...
lemma geometric_mul_inner_tendsto_zero {D₁ D₂ : ℕ} (c : ℂ)
  ...
  apply TNLean.Convergence.geometric_mul_bounded_tendsto_zero c _ ((C_X + C_Y) / 2) hc
  intro N
  ...
  calc ‖mpvInner (d := d) X Y N‖
    ...
    _ = (‖mpvInner (d := d) X X N‖ + ‖mpvInner (d := d) Y Y N‖) / 2 := by
          rw [norm_sq_eq_norm_mpvInner X N, norm_sq_eq_norm_mpvInner Y N]
