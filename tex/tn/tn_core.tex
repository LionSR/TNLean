...
% document.
%
% Every index contraction is solid.  Virtual and physical contractions share
% one baseline stroke width; their named ports and semantic styles retain the
% index type.  Morphism arrows, comparison arrows, annotations, and region
% boundaries are separate semantic marks and do not represent indices.  Dashed
% curves delimit grouping regions and never represent periodic contraction.
% Tensor labels do not change the glyph used for the tensor.
...
\cs_new_protected:Npn \tn_alias_assert_acyclic:nn #1#2
  {
    \str_if_eq:eeTF { \tl_to_str:e {#1} } { \tl_to_str:e {#2} }
      {
        \PackageError{tnlean-tikz}{Cyclic tensor-network alias `#1'}
          {Aliases must terminate at an atom port.}
      }
      {
        \prop_if_in:NeT \g__tn_alias_source_prop { \tn_port_key:n {#2} }
          {
            \prop_get:NeN \g__tn_alias_source_prop
              { \tn_port_key:n {#2} } \l_tmpa_tl
            \exp_args:NnV \tn_alias_assert_acyclic:nn {#1} \l_tmpa_tl
          }
      }
  }
...
\newcommand{\TN@normalprofile}{%
    ...
    \def\TN@periodictraceheight{0.92}%
...
\newcommand{\TN@compactprofile}{%
    ...
    \def\TN@periodictraceheight{0.74}%
...
% Every port-to-port contraction emits one connection event.  The repository
% audit rebuilds the diagram graph from these lines, so the field order is
% fixed and must match across all routes.
\newcommand{\TN@connectevent}[4]{%
    \TNEventInternal{connection|picture=\the\TN@pictureid|type=#1|from=#2|to=#3|route=#4}%
}
...
\newcommand{\TN@vconnectports}[3][]{%
    ...
    \TN@connectevent{virtual}{#2}{#3}{straight}%
}
...
\newcommand{\TN@pconnectports}[3][]{%
    ...
    \TN@connectevent{physical}{#2}{#3}{straight}%
}
...
\newcommand{\TN@vconnectportshv}[3][]{%
    ...
    \TN@connectevent{virtual}{#2}{#3}{orthogonal-hv}%
}
...
\newcommand{\TN@vconnectportsvh}[3][]{%
    ...
    \TN@assertporttype{#3}{virtual}%
    \draw[tn virtual wire,rounded corners=3pt,#1] (#2) |- (#3);%
    \TN@connectevent{virtual}{#2}{#3}{orthogonal-vh}%
}
...
\newcommand{\TN@pconnectportshv}[3][]{%
    ...
    \TN@connectevent{physical}{#2}{#3}{orthogonal-hv}%
}
...
\newcommand{\TN@pconnectportsvh}[3][]{%
    ...
    \TN@assertporttype{#3}{physical}%
    \draw[tn physical wire,rounded corners=3pt,#1] (#2) |- (#3);%
    \TN@connectevent{physical}{#2}{#3}{orthogonal-vh}%
}
...
\newcommand{\TN@mconnectports}[3][]{%
    ...
    \TN@connectevent{morphism}{#2}{#3}{straight}%
}
...
\newcommand{\TN@mconnectportshv}[3][]{%
    ...
    \TN@connectevent{morphism}{#2}{#3}{orthogonal-hv}%
}
...
\newcommand{\TN@mconnectportsvh}[3][]{%
    ...
    \TN@assertporttype{#3}{morphism}%
    \draw[tn morphism arrow,rounded corners=3pt,#1] (#2) |- (#3);%
    \TN@connectevent{morphism}{#2}{#3}{orthogonal-vh}%
}
...
\newcommand{\TN@vtraceportsbelow}[5][]{%
    ...
    \draw[tn virtual trace,#1] (#2) -- ++(-#4,0) -- ++(0,-#5)
        -| ($(#3)+(#4,0)$) -- (#3);%
    \TN@connectevent{virtual}{#2}{#3}{trace-below}%
}
...
\newcommand{\TN@vtraceportsabove}[5][]{%
    ...
    \draw[tn virtual trace,#1] (#2) -- ++(-#4,0) -- ++(0,#5)
        -| ($(#3)+(#4,0)$) -- (#3);%
    \TN@connectevent{virtual}{#2}{#3}{trace-above}%
}
...
\newcommand{\TN@vtraceportsright}[5][]{%
    ...
    \draw[tn virtual trace,#1] (#2) -- ++(0,#5)
        -| ($(#3)+(#4,-#5)$) -- ++(-#4,0) -- (#3);%
    \TN@connectevent{virtual}{#2}{#3}{trace-right}%
}
...
\newcommand{\TN@ptraceportsbelow}[5][]{%
    ...
    \draw[tn physical trace,#1] (#2) -- ++(-#4,0) -- ++(0,-#5)
        -| ($(#3)+(#4,0)$) -- (#3);%
    \TN@connectevent{physical}{#2}{#3}{trace-below}%
}
...
\newcommand{\TN@ptraceportsabove}[5][]{%
    ...
    \draw[tn physical trace,#1] (#2) -- ++(-#4,0) -- ++(0,#5)
        -| ($(#3)+(#4,0)$) -- (#3);%
    \TN@connectevent{physical}{#2}{#3}{trace-above}%
}
...
\newcommand{\TN@ptraceportsright}[5][]{%
    ...
    \draw[tn physical trace,#1] (#2) -- ++(0,#5)
        -| ($(#3)+(#4,-#5)$) -- ++(-#4,0) -- (#3);%
    \TN@connectevent{physical}{#2}{#3}{trace-right}%
}
