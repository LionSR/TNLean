...
\newcommand{\TN@trivalentmap}[6][]{%
    ...
    \ifx\TN@triorientation\TN@triright
        \TN@trivalentport{#6}{#2Combined}{#2}{west}%
        \TN@trivalentbetweenport{#6}{east}{#2FactorOne}{#2}
            {\TN@branchfirstfraction}%
        \TN@trivalentbetweenport{#6}{east}{#2FactorTwo}{#2}
            {\TN@branchsecondfraction}%
    \else\ifx\TN@triorientation\TN@trileft
        \TN@trivalentport{#6}{#2Combined}{#2}{east}%
        \TN@trivalentbetweenport{#6}{west}{#2FactorOne}{#2}
            {\TN@branchfirstfraction}%
        \TN@trivalentbetweenport{#6}{west}{#2FactorTwo}{#2}
            {\TN@branchsecondfraction}%
    \else\ifx\TN@triorientation\TN@tridown
        \TN@trivalentport{#6}{#2Combined}{#2}{south}%
        \TN@trivalentport{#6}{#2FactorOne}{#2}{north west}%
        \TN@trivalentport{#6}{#2FactorTwo}{#2}{north east}%
    \else\ifx\TN@triorientation\TN@triup
        \TN@trivalentport{#6}{#2Combined}{#2}{north}%
        \TN@trivalentport{#6}{#2FactorOne}{#2}{south west}%
        \TN@trivalentport{#6}{#2FactorTwo}{#2}{south east}%
    \else
...
% confined to tex/tn.  Each semantic atom exposes long role names in addition
% to the short internal names used by older complete diagrams.

% A complete diagram and all of its renderer metadata are declared together.
% The seven fields are name, ordered argument schema, display role, layout
% profile, audit sample, chapter context, and body.  Python reads precisely the
% same declarations; it maintains no parallel registry.
\ExplSyntaxOn
\prop_new:N \g__tn_diagram_arguments_prop
\prop_new:N \g__tn_diagram_role_prop
\prop_new:N \g__tn_diagram_profile_prop
\prop_new:N \g__tn_diagram_sample_prop
\prop_new:N \g__tn_diagram_context_prop

\cs_new_protected:Npn \tn_declare_diagram:nnnnnnn #1#2#3#4#5#6#7
  {
    \cs_if_exist:cTF {#1}
      {
        \PackageError{tnlean-tikz}{Duplicate diagram declaration `#1'}
          {Every chapter-facing diagram must be declared exactly once.}
      }
      {
        \tl_if_blank:nTF {#2}
        ...
        \prop_gput:Nnn \g__tn_diagram_arguments_prop {#1} {#2}
        \prop_gput:Nnn \g__tn_diagram_role_prop {#1} {#3}
        \prop_gput:Nnn \g__tn_diagram_profile_prop {#1} {#4}
        \prop_gput:Nnn \g__tn_diagram_sample_prop {#1} {#5}
        \prop_gput:Nnn \g__tn_diagram_context_prop {#1} {#6}
      }
  }
\cs_new_eq:NN \TNDeclareDiagram \tn_declare_diagram:nnnnnnn
\ExplSyntaxOff
...
\newcommand{\TNStackedOmission}[2]{%
    \TNOmission{#1Upper}{#2}%
    \node[tn label,below=2\TN@layerpitch cm of #1Upper]
...
\newcommand{\TNPoint}[2]{\TN@annotationterminal{#1}{#2}}
\newcommand{\TNPointBetween}[3]{%
    \TN@annotationterminal{#1}{($(#2)!0.5!(#3)$)}%
}
...
\newcommand{\TNSectorGauge}[3]{\TN@sectorgauge{#1}{#2}{#3}}
\newcommand{\TNInverseSectorGauge}[3]{\TN@inverseSectorgauge{#1}{#2}{#3}}
...
\newcommand{\TNLabel}[3]{\TN@label{#1}{#2}{#3}}
...
% A placed annotation is not a tensor-network object and therefore carries no
% index ports.  Naming the point keeps later labels and arrows compositional.
\newcommand{\TNPlacedLabel}[3]{%
    \TN@annotationterminal{#1}{#2}%
    \TN@label{#1}{(0,0)}{#3}%
}
\newcommand{\TNPlacedLabelAbove}[3]{%
    \TN@annotationterminal{#1}{#2}%
    \TN@label[anchor=south]{#1}{(0,0)}{#3}%
}
\newcommand{\TNPlacedLabelBelow}[3]{%
    \TN@annotationterminal{#1}{#2}%
    \TN@label[anchor=north]{#1}{(0,0)}{#3}%
}
\newcommand{\TNPlacedLabelLeft}[3]{%
    \TN@annotationterminal{#1}{#2}%
    \TN@label[anchor=east]{#1}{(0,0)}{#3}%
}
\newcommand{\TNPlacedLabelRight}[3]{%
    \TN@annotationterminal{#1}{#2}%
...
\newcommand{\TNCanvas}[2]{\TN@canvas{#1}{#2}}
...
\newcommand{\TNGroupBoundary}[1]{\TN@grouppath{#1}}
\newcommand{\TNFactorBoundary}[1]{\TN@factorpath{#1}}
\newcommand{\TNAnnotation}[1]{\TN@annotationpath{#1}}
\newcommand{\TNSelectedPath}[1]{\TN@selectedpath{#1}}
\newcommand{\TNSecondaryPath}[1]{\TN@secondarypath{#1}}
\newcommand{\TNRegion}[4]{\TN@region{#1}{#2}{#3}{#4}}
\newcommand{\TNSelectedRegionPath}[1]{\path[tn region selected] #1;}
\newcommand{\TNSecondaryRegionPath}[1]{\path[tn region secondary] #1;}
\newcommand{\TNComplementRegionPath}[1]{%
    \path[tn region complement, fill=none] #1;%
}
...
\newcommand{\TNStackedMPOProduct}[5]{%
    \TNPlacementDispatch{#2}{%
        ...
        \TNMPOSite{#1Lower}
            {below=2\TN@layerpitch cm of #1Upper}{#4}%
...
\newcommand{\TNIndexedPeriodicMPOWord}[7]{%
    ...
    \TN@label{#1RN}{(0,\TN@physlabeloff)}{#4}%
    \TN@label{#1RS}{(0,-\TN@physlabeloff)}{#5}%
    \TNPoint{#1Length}{(2.10,-0.62)}%
    \TNLabel{#1Length}{(0,0)}{#6}%
}
