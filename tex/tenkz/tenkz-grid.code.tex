...
\cs_new_protected:Npn \tenkz_record:nnn #1#2#3
  {
    % parse the options ONCE, here; the memo hands the resolution to every
    % later pass, and the model record carries the normalized fields
    \tenkz_cell_opts:n {#2}
    \__tenkz_model_record_atom:e
      {kind={#1},at={\tenkz_cell_key:},
       options={\exp_not:n{#2}},label={\exp_not:n{#3}},
       skin={\bool_if:NT \l__tenkz_cshapeexplicit_bool {\exp_not:V \l__tenkz_cshape_tl}},
       wide={\int_use:N \l__tenkz_cwide_int},
       wires={\int_use:N \l__tenkz_cwires_int},
       role={\exp_not:V \l__tenkz_crole_tl},
       species={\exp_not:V \l__tenkz_cspec_tl}}
...
% Within one picture the parser is a pure function of its option string
% (the skin policy and strut are picture options, fixed before the body).
% The resolved locals are snapshotted per distinct string at first parse
% and restored on every repeat; \tenkz_record:nnn primes the memo, so a
% recorded cell's options are parsed ONCE, at record time, and every later
% pass restores the resolution instead of reparsing.  The memo lives in a
                        ...
                        \tl_if_eq:NNF \l__tenkz_mdb_oa_tl \l__tenkz_mdb_ob_tl
                          {
                            ...
                            \bool_lazy_and:nnT
                              ...
                              { ! \l__tenkz_mdb_combined_bool }
                              {
                                \__tenkz_model_record_wire:e
                                  {kind={index},
                                   from={##1-\int_use:N \l__tenkz_mdb_prev_int},
                                   to={##1-####1},
                                   row={##1},origin={bond},
                                   dir={\tenkz_model_dir_word:},
                                   style={\tl_use:N \l__tenkz_mdb_style_tl},
...
\cs_new_protected:Npn \tenkz_glyph_node_aux:nnn #1#2#3
  {
    ...
    % places it, then geometry and the render stage own the ink
    % (glyph shapes never rotate -- text stays upright)
    \tenkz_map:nn { \l__tenkz_x_dim } { \l__tenkz_y_dim }
    \tenkz_render_cell_place:nn {#1}{#2}
