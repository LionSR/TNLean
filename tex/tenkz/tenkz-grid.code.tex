...
\cs_new_protected:Npn \tenkz_fuse_resolve_combined:
  {
    ...
    \str_if_eq:VnTF \l__tenkz_cwestat_tl {center}
      ...
      { \str_if_eq:VnTF \l__tenkz_ceastat_tl {center}
          ...
          {
            % Naming only the separate face determines the opposite
            % implicit face as centred.  A bare bar retains west centre.
            \tl_if_empty:NTF \l__tenkz_fcombined_tl
              { \tl_if_empty:NTF \l__tenkz_cwestat_tl
                  { \tl_set:Nn \l__tenkz_fcombined_tl {west} }
                  { \tl_if_empty:NTF \l__tenkz_ceastat_tl
                      { \tl_set:Nn \l__tenkz_fcombined_tl {east} }
                            ...
                            \tl_if_eq:NNF \l_tmpa_tl \l_tmpb_tl
                              {
                                ...
                                \tenkz_cell_fuse_row_port:nnnN {##1}
...
% in that row?  Non-fusion cells keep their ordinary one-port-per-row
% behaviour.  For a fusion bar, `center' is its one base-row attachment,
% `rows' means every row in span=, and an integer list selects exactly
% those span-local row slots.  Boundary drawing, bond pairing, and their
% event counts all consult this predicate.
\int_new:N \l__tenkz_fuseport_int
\cs_new_protected:Npn \tenkz_cell_fuse_row_port:nnnN #1#2#3#4
  {
    \bool_set_true:N #4
    \int_set:Nn \l__tenkz_fusebase_int {#1}
    \tenkz_get:NeN \l__tenkz_kind_prop
    ...
    \bool_lazy_or:nnT
      ...
      { \str_if_eq_p:Vn \l_tmpa_tl {fusec} }
      {
        ...
        \str_case:VnF \l__tenkz_faceports_tl
          {
            {center}
              { \int_compare:nNnT {\l__tenkz_fuseport_int} = {1}
                  { \bool_set_true:N #4 } }
            {rows}
              { \bool_lazy_and:nnT
                  { \int_compare_p:nNn {\l__tenkz_fuseport_int} > {0} }
                  { \int_compare_p:nNn
                      {\l__tenkz_fuseport_int} < {\l__tenkz_frows_int + 1} }
                  { \bool_set_true:N #4 } }
          }
          { \clist_if_in:NeT \l__tenkz_faceports_tl
              {\int_use:N\l__tenkz_fuseport_int}
              { \bool_set_true:N #4 } }
      }
  }
