...
\cs_new_protected:Npn \tenkz_combined_stub:nn #1#2
  {
    % A two-sided trace consumes both endpoint faces.  Its racetrack or hook
    % path begins at the node anchor itself, so a separate open-face stroke
    % would be redundant; for hooks, its outer half would remain visibly
    % dangling.  A mismatched periodic face is redrawn explicitly by the
    % periodic-face pass below.
    \bool_lazy_and:nnF
      ...
      { \str_if_eq_p:Vn \l__tenkz_eastpol_tl {trace} }
      {
        \int_compare:nNnT { \l__tenkz_cwires_int } > {1}
          {
            % West and east are independent faces.  In particular a gauge on
            % a fused virtual space may have one centred index on both sides.
