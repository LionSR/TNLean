...
\cs_new_protected:Npn \tenkz_tall_resolve_combined:
  {
    \str_if_eq:VnTF \l__tenkz_cwestat_tl {center}
      ...
      { \str_if_eq:VnTF \l__tenkz_ceastat_tl {center}
          ...
          {
            % A primary declaration on the compatibility side replaces it.
            % Thus `combined=west, west at={1,2}' has no hidden centre stub.
            \str_if_eq:VnTF \l__tenkz_fcombined_tl {west}
              { \tl_if_empty:NF \l__tenkz_cwestat_tl
                  { \tl_clear:N \l__tenkz_fcombined_tl } }
...
% glyph.  These are genuine open indices, so the same pass records their
% contribution to the boundary signature.
...
\cs_new_protected:Npn \tenkz_face_has_slot:nnnN #1#2#3#4
  {
    ...
    \str_case:nnF {#1}
      {
        {center}
          { \int_compare:nNnT {#3} = {1} { \bool_set_true:N #4 } }
        {rows}
          { \bool_lazy_and:nnT
