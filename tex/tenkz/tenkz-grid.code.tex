...
\cs_new_protected:Npn \tenkz_normalize_virtual_face:Nn #1#2
  {
    \tl_if_empty:NF #1
      {
    ...
    \bool_if:NF \l__tenkz_cnolegs_bool
      {
        ...
        \tl_if_empty:NF \l__tenkz_cupat_tl
          { \tenkz_emit_faceports_one:nnn {#1}{#2}{up} }
        \tl_if_empty:NF \l__tenkz_cdownat_tl
          { \tenkz_emit_faceports_one:nnn {#1}{#2}{down} }
        % A cell-level physical= declaration on a wires=k brick has one port
        % per spanned row unless a primary or compatibility face key replaces
        % it.  Record that implicit split face as `rows', matching its ink and
...
\cs_new_protected:Npn \tenkz_emit_tracefacewarnings:
  {
    \prop_map_inline:Nn \l__tenkz_tracefacewarning_prop
      { \tenkz@event{warning|code=pair-trace-face-ports|cell=##1} }
  }
        ...
        \clist_map_inline:nn {west,east}
          {
            ...
            \str_if_eq:VnT \l__tenkz_faceports_tl {center}
              {
                \draw[bond]
                  (\tenkz_node_name:nn{#1}{#2}.\tenkz_compass:n{##1}) --
                  ++\tenkz_stub_vec:n {##1};
              }
          }
      }
...
\prop_new:N \l__tenkz_lowersurplus_prop
...
\prop_new:N \l__tenkz_pwcenterused_prop
...
\cs_new_protected:Npn \tenkz_lower_port_matched:nnnN #1#2#3#4
  {
    ...
    \prop_get:NeNF \l__tenkz_owner_prop
      {#1-\int_use:N\l_tmpa_int} \l__tenkz_pwbase_tl
      { \tl_set:Ne \l__tenkz_pwbase_tl { \int_use:N\l_tmpa_int } }
    \tenkz_get:NeN \l__tenkz_kind_prop
      {#1-\l__tenkz_pwbase_tl} \l__tenkz_pkind_tl
    \bool_lazy_or:nnT
      { \str_if_eq_p:Vn \l__tenkz_pkind_tl {tn} }
      { \str_if_eq_p:Vn \l__tenkz_pkind_tl {tnc} }
      {
        \tenkz_get:NeN \l__tenkz_opts_prop
          {#1-\l__tenkz_pwbase_tl} \l_tmpb_tl
        \exp_args:NV \tenkz_cell_opts:n \l_tmpb_tl
        \bool_if:NF \l__tenkz_cnolegs_bool
          {
...
% when it is 1.  An open= interface already contributes the full declared
% face arity in the signature pass, so that caller uses 0 while retaining
% exactly the same visible legs and labels.
\cs_new_protected:Npn \tenkz_draw_lower_surplus:nnn #1#2#3
  {
    \tl_set:Ne \l__tenkz_pwlowbase_tl {#2}
    \prop_if_in:NeF \l__tenkz_lowersurplus_prop
      {#1-\l__tenkz_pwlowbase_tl}
      {
        \prop_put:Nen \l__tenkz_lowersurplus_prop
          {#1-\l__tenkz_pwlowbase_tl} {}
        \tenkz_get:NeN \l__tenkz_name_prop
        ...
        \str_if_eq:VnF \l__tenkz_pwlowports_tl {center}
          {
                ...
                \bool_if:NF \l__tenkz_pwupperport_bool
                  {
                    ...
                    \tl_if_blank:VF \l__tenkz_pwlowlabel_tl
                      {
                        \node[label, anchor=\tenkz_compass:n{south},
                              shift={\tenkz_leg_vec:nn{1}
                                { \tenkz@dim{\tenkz@r@physleg}
                                  + \tenkz@dim{\tenkz@r@labelclear} }}]
                          at \l__tenkz_pwlow_tl
                          { \tl_use:N \l__tenkz_pwlowlabel_tl };
                      }
          ...
          { \int_eval:n{#1+1} - \l__tenkz_pwbase_tl } \l__tenkz_pb_tl
          {
            \tenkz_leg_candidate:nnT
              { \int_eval:n{#1+1} }{ \l__tenkz_pwbase_tl }
              {
                ...
                \str_if_eq:VnTF \l__tenkz_faceports_tl {center}
                  { \prop_if_in:NeF \l__tenkz_pwcenterused_prop
                      {#1-\tl_use:N\l__tenkz_pwbase_tl}
                      { \bool_set_true:N \l__tenkz_pwlowerport_bool
                        \prop_put:Nen \l__tenkz_pwcenterused_prop
                          {#1-\tl_use:N\l__tenkz_pwbase_tl} {} } }
                  { \clist_if_in:NeT \l__tenkz_faceports_tl
    ...
    \str_if_eq:VnTF \l__tenkz_faceports_tl {center}
      {
        ...
        \prop_if_in:NeF \l__tenkz_pwcenterused_prop
          {#1-\tl_use:N\l__tenkz_pwbase_tl}
          {
            \prop_put:Nen \l__tenkz_pwcenterused_prop
              {#1-\tl_use:N\l__tenkz_pwbase_tl} {}
            \tenkz_leg_open:enn { \int_eval:n{#1+1} }
              {\l__tenkz_pwbase_tl}{up}
          }
      }
      {
        \tenkz_draw_lower_surplus:nnn {#1}{\l__tenkz_pwbase_tl}{0}
        \clist_if_in:NeT \l__tenkz_faceports_tl
...
\cs_new_protected:Npn \tenkz_pair_below_wire:nn #1#2
  {
    ...
    \str_if_eq:VnTF \l__tenkz_faceports_tl {center}
      {
        \prop_if_in:NeTF \l__tenkz_pwcenterused_prop
          {#1-\tl_use:N\l__tenkz_pwbase_tl}
          {
            \tenkz_leg_open:nnn {#1}{#2}{down}
            \int_incr:N \l__tenkz_internalpd_int
...
\cs_new_protected:Npn \tenkz_brick_face:nnnNn #1#2#3#4#5
  {
    ...
    \str_if_eq:nnTF {#3}{north}
      {
        \tenkz_face_ports:nN {up} \l__tenkz_bkports_tl
        \tl_if_empty:NT \l__tenkz_cupat_tl
          { \tl_if_empty:NT \l__tenkz_clegsat_tl
      ...
      {
        \tenkz_face_ports:nN {down} \l__tenkz_bkports_tl
        \tl_if_empty:NT \l__tenkz_cdownat_tl
          { \tl_if_empty:NT \l__tenkz_clegsat_tl
              { \tl_clear:N \l__tenkz_bkports_tl
                \int_step_inline:nn {\l__tenkz_cwires_int}
...
\cs_new_protected:Npn \tenkz_draw_legs:
  {
    ...
    \prop_clear:N \l__tenkz_lowersurplus_prop
    % A centred lower face is one index for the entire horizontal
    % interface, even when several separate upper glyphs meet its span.
    \prop_clear:N \l__tenkz_pwcenterused_prop
    \int_step_inline:nn { \l__tenkz_nrows_int }
      {
        \int_step_inline:nnn {1} { \l__tenkz_ncols_int }
          {
                        ...
                        {skip}
                          {
                            \prop_get:NeNF \l__tenkz_owner_prop
                              {\int_eval:n{##1+1}-####1}
                              \l__tenkz_pwbase_tl
                              { \tl_set:Ne \l__tenkz_pwbase_tl {####1} }
                            \tenkz_get:NeN \l__tenkz_opts_prop
                              {\int_eval:n{##1+1}-\l__tenkz_pwbase_tl}
                              \l_tmpb_tl
                            \exp_args:NV \tenkz_cell_opts:n \l_tmpb_tl
                            \tenkz_face_ports:nN {up}
...
% in that row?  Ordinary one-row cells return true.  Fusion bars and tall
% wires=k glyphs resolve their base declaration, then test center, rows,
% or the explicit sparse list through \tenkz_face_has_slot:nnnN.  Boundary
% drawing, bond pairing, and their event counts all consult this predicate.
...
\int_new:N \l__tenkz_centerneighbor_int
% Column of the neighbour actually joined to (row, column) by the bond
% scan.  Empty grid columns are transparent, whereas a `skip' cell breaks
% the chain.  This is the same consecutive-owner rule as \tenkz_draw_bonds:.
\cs_new_protected:Npn \tenkz_center_neighbor_on_row:nnnN #1#2#3#4
  {
    \int_zero:N #4
    \str_if_eq:nnTF {#3}{west}
      {
      ...
      {
        \int_step_inline:nnn {#2+1}{\l__tenkz_ncols_int}
          {
...
\cs_new_protected:Npn \tenkz_center_face_contracted:nnnN #1#2#3#4
  {
    ...
    \int_step_inline:nnn {#1}{\l__tenkz_centerlast_int}
      {
        \tenkz_center_neighbor_on_row:nnnN {##1}{#2}{#3}
          \l__tenkz_centerneighbor_int
        \int_compare:nNnT {\l__tenkz_centerneighbor_int} > {0}
          {
            \str_if_eq:nnTF {#3}{west}
              { \tenkz_cell_virtual_row_port:nnnN {##1}
                  {\l__tenkz_centerneighbor_int}{east} \l_tmpa_bool }
              { \tenkz_cell_virtual_row_port:nnnN {##1}
                  {\l__tenkz_centerneighbor_int}{west} \l_tmpa_bool }
            \bool_if:NT \l_tmpa_bool
              {
                \bool_set_true:N #4
              }
...
\cs_new_protected:Npn \tenkz_cell_physical_port:nnnN #1#2#3#4
  {
    \bool_set_false:N #4
    \prop_get:NeNF \l__tenkz_owner_prop
      {\int_eval:n{#1}-\int_eval:n{#2}} \l__tenkz_pwbase_tl
      { \tl_set:Ne \l__tenkz_pwbase_tl {\int_eval:n{#2}} }
    \tenkz_get:NeN \l__tenkz_kind_prop
      {\int_eval:n{#1}-\l__tenkz_pwbase_tl} \l__tenkz_pkind_tl
    \bool_lazy_or:nnT
      { \str_if_eq_p:Vn \l__tenkz_pkind_tl {tn} }
      { \str_if_eq_p:Vn \l__tenkz_pkind_tl {tnc} }
      {
        \tenkz_get:NeN \l__tenkz_opts_prop
          {\int_eval:n{#1}-\l__tenkz_pwbase_tl} \l_tmpb_tl
        \exp_args:NV \tenkz_cell_opts:n \l_tmpb_tl
        \bool_if:NF \l__tenkz_cnolegs_bool
          {
            \tenkz_face_ports:nN {#3} \l__tenkz_faceports_tl
                        ...
                        \bool_if:NTF \l_tmpa_bool
                          { \tenkz_phtrace_site:n {##1} }
                          {
                            \msg_warning:nnnn {tenkz}{phtrace-face-ports}
                              {1}{##1}
                            \tenkz@event{warning|code=phtrace-face-ports|cell=1-##1}
                          }
                      } }
              }
          }
