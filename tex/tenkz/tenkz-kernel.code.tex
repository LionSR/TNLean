...
\cs_new_protected:Npn \__tenkz_kernel_emit:
  {
    ...
    % states it, since pitch-normalized coordinates cannot show it.
...
\cs_new_protected:Npn \__tenkz_kernel_tngroup:nn #1#2
  {
    ...
    % picture's pitch, so a group-scope profile is refused, not scaled.
...
\tl_new:N \l__tenkz_kernel_leg_record_phys_tl
\cs_new_protected:Npn \__tenkz_kernel_policy_leg_records:
  {
    \__tenkz_model_map_ids:nn {atom}
      {
        \__tenkz_model_get:nnN {##1} {physical}
          \l__tenkz_kernel_leg_record_phys_tl
        \quark_if_no_value:NF \l__tenkz_kernel_leg_record_phys_tl
          {
            \str_case:Vn \l__tenkz_kernel_leg_record_phys_tl
              {
                {up}     { \__tenkz_kernel_policy_leg_record:nn {##1} {n} }
                {down}   { \__tenkz_kernel_policy_leg_record:nn {##1} {s} }
                {updown}
                  {
                    \__tenkz_kernel_policy_leg_record:nn {##1} {n}
                    \__tenkz_kernel_policy_leg_record:nn {##1} {s}
                  }
              }
          }
      }
...
% row's own business: a bare row hangs nothing, an open index hangs one leg,
% and a named one hangs its leg and the band its name occupies.  The scan
% reads the open-index records the model already carries, keeps the ones whose
% host stands in the rows the return passes and whose page bearing points the
% way the return runs, and answers with the deepest of them.  A virtual index
% is longer than a physical one, so the type it declares chooses its length.
%
% A leg the picture's own physical policy grew is an open index exactly as an
% authored port is, and it records its face under its own key; a cell-level or
% boundary opening is one again, and carries no host at all.  All three
% readings run, so no row is passed over the way one was once passed over when
% the closure read authored cells alone.
%
% A bearing is asked of the production that draws it.  An authored port leaves
% along its host's own face; a physical leg leaves along the frame's physical
% axis, which on a projected plane is a page-space vector of its own and not
% the host's local north, so reading it as a face would put an upward leg
% sideways and drop it from the answer.
...
\tl_new:N \l__tenkz_kernel_r_trace_ink_turn_tl
...
% The reach standing in the reach register belongs to a row the return passes
% and points the way the return runs: take it.
\cs_new_protected:Npn \__tenkz_kernel_r_trace_ink_take:
  {
    \bool_lazy_all:nT
      {
        {
        ...
        }
        {
          \fp_compare_p:nNn
            {
              \l__tenkz_kernel_r_yc_fp
              * sind( \l__tenkz_kernel_r_trace_ink_turn_tl )
            }
            > {0}
        }
      }
...
% The length an open index reaches, and the band its name occupies past the
% tip when it carries one.
\cs_new_protected:Npn \__tenkz_kernel_r_trace_leg_depth:n #1
  {
    \__tenkz_model_get:nnN {#1} {port-type}
      \l__tenkz_kernel_r_trace_ink_type_tl
    \str_if_eq:VnTF \l__tenkz_kernel_r_trace_ink_type_tl {physical}
      {
...
% An authored port: its host's row, and its host's own face.
        ...
        \bool_if:NT \l__tenkz_kernel_r_trace_ink_bool
          {
            ...
            \quark_if_no_value:NF \l__tenkz_kernel_r_trace_ink_face_tl
              {
                \tl_set:Ne \l__tenkz_kernel_r_trace_ink_face_tl
                  {
                ...
                \exp_args:NVV \__tenkz_kernel_r_page_face:nnN
                  \l__tenkz_kernel_r_trace_ink_host_tl
                  \l__tenkz_kernel_r_trace_ink_face_tl
                  \l__tenkz_kernel_r_trace_ink_turn_tl
                \__tenkz_kernel_r_trace_leg_depth:n {#1}
                \__tenkz_kernel_r_trace_ink_take:
              }
          }
      }
  }
% A policy leg: its host's row, and the frame's physical axis on the face the
% record names.  On every carrier but a projected plane that axis is the
% host's own north or south, which is why a flat picture reads the same
% either way.
...
% A cell-level or boundary opening.  It has no host: the picture asked for it,
% so the side it opens gives both its row and its outward page bearing.  A
% physical opening stands on the first row and leaves along the frame's
% physical axis; a north or south boundary opening stands on the first or the
% last row and leaves the way it opens.  A west or east opening leaves along
% the row and hangs nothing on any return's side; so does an opening at an
% interface, which points at the row across the interface from it, and which
% no return can reach -- a return whose rows include the row such a stub
% hangs from also passes at least one further row line, so it already stands
% a row spacing and a trace reach beyond the stub.
%
% Two lengths are budgeted rather than resolved, and both are a leg a caller
% has grown past the metric that names it: one lengthened to reach a
% waypoint, an enclosure it pierces, or a crossing it must clear, and, on a
% projected plane, a physical opening the transverse renderer starts at the
% glyph's own outward support point rather than at the site.  Resolving
% either means asking the leg-plan or glyph-anchor productions, which write
% the very coordinate registers the reach is computed in and reads back for
% the passed rows.  A leg carried out to meet other ink crosses the return
% rather than ending on it, which is a different reading from the one this
% clearance is about.
\cs_new_protected:Npn \__tenkz_kernel_r_trace_open_ink:n #1
  {
    \tl_clear:N \l__tenkz_kernel_r_trace_ink_turn_tl
    \__tenkz_model_get:nnN {#1} {mode}
      \l__tenkz_kernel_r_trace_ink_type_tl
    \str_if_eq:VnTF \l__tenkz_kernel_r_trace_ink_type_tl {physical}
      {
        ...
        \__tenkz_model_get:nnN {#1} {from-host}
          \l__tenkz_kernel_r_trace_ink_host_tl
        \quark_if_no_value:NTF \l__tenkz_kernel_r_trace_ink_host_tl
          {
            % The ordinary route hands the opening to the wire renderer,
            % which stands a free end one open index out, not one leg.
            \tl_set:Nn \l__tenkz_kernel_r_trace_ink_turn_tl {90}
            \fp_set:Nn \l__tenkz_kernel_r_trace_ink_reach_fp
              { \__tenkz_metric_ratio:n {stub} }
          }
          {
            % The plane route inks a leg along the frame's physical axis.
            \fp_set:Nn \l__tenkz_kernel_r_trace_ink_reach_fp
              { \__tenkz_metric_ratio:n {physleg} }
            \exp_args:NV \__tenkz_kernel_r_physical_axis:nnNN
              \l__tenkz_kernel_r_trace_ink_host_tl {n}
              \l__tenkz_kernel_r_trace_ink_turn_tl
              \l__tenkz_kernel_r_trace_ink_side_tl
          }
      }
      {
        ...
        \quark_if_no_value:NF \l__tenkz_kernel_r_trace_ink_side_tl
          {
            \str_case:Vn \l__tenkz_kernel_r_trace_ink_side_tl
              {
                {north}
                  {
                    \tl_set:Nn \l__tenkz_kernel_r_trace_ink_row_tl {1}
                ...
                {south}
                  {
                    ...
                    \tl_set:Nn \l__tenkz_kernel_r_trace_ink_turn_tl {270}
                  }
              }
            \fp_set:Nn \l__tenkz_kernel_r_trace_ink_reach_fp
              { \__tenkz_metric_ratio:n {stub} }
    ...
    \tl_if_empty:NF \l__tenkz_kernel_r_trace_ink_turn_tl
      { \__tenkz_kernel_r_trace_ink_take: }
  }
...
% do not change under the render, so the three walks the answer costs are paid
% once per range and per picture rather than once per reader.
...
\cs_new_protected:Npn \__tenkz_kernel_r_trace_side_reach:
  {
    ...
    % side, and it keeps its own trace reach as a floor where they hang
    % nothing.  Every further return crossed still adds one trace reach,
    % which is what that reach measures here: the paper between two returns,
    % not the drop of the first below its row.
    %
...
% The outward standoff the return owes the rows it passes, measured from its
% own row line: the deepest open index those rows hang on its side, one
% daylight beyond it, and the outward extreme of the rows themselves, since a
% return passing further rows must clear their ink where they stand and not
% where its own row does.  A flat rail runs out of the row line and answers
% with a page distance from it; a ring's sector stands off no row line and
% answers with the word for a row that has none.  The record publishes the
% standoff beside the contour so a reader of the stream can hold the drawn
% ink to the clearance the metric table declares, which is the reading a
% wire's own ink cannot supply: a wire carries no measured box.
...
\cs_new_protected:Npn \__tenkz_kernel_r_rail_arc:
  {
    \tl_set:Nn \l__tenkz_kernel_r_rail_clear_tl {none}
    \tl_clear:N \l__tenkz_kernel_r_rail_tl
    ...
    \tl_if_blank:nTF {#3}
      {
        ...
        \exp_args:NV \__tenkz_string_touch_all:n
          \l__tenkz_kernel_leg_name_tl
