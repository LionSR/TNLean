...
% live; spanning records contribute only their anchor, including anchors just
% outside the declared window.  For each member pair the plan either enumerates
% its sparse record product or marks it dense, whichever is bounded by the
% finite translation window.  Geometry owns the raw support calculation and
% asks this kernel callback about model realization only for negative margins.
