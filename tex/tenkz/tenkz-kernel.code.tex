    ...
    \quark_if_no_value:NF \l__tenkz_kernel_r_trace_ink_host_tl
      {
        ...
        \bool_if:NT \l__tenkz_kernel_r_trace_ink_bool
          {
            % A glyph that spans wires stands on several rows, and a port on
            % its south face is placed on the last of them.  The carrier
            % answers with the anchor row, so the span moves a south port to
            % the row it is actually drawn from; otherwise that row's own
            % return would scan past its deepest index.
            \__tenkz_model_get:nnN {#1} {port-face}
