                  ...
                  \l__tenkz_kernel_port_policy_label_tl
                  {
                    \str_if_eq:VnT
                      \l__tenkz_kernel_port_policy_field_tl {physical}
                      {
                        ...
                        \__tenkz_model_get:nnN {#1} {dir}
                          \l__tenkz_kernel_scratch_tl
                        \quark_if_no_value:NF \l__tenkz_kernel_scratch_tl
                          {
                            \str_if_eq:VnF
                              \l__tenkz_kernel_scratch_tl {none}
                              {
                                \msg_error:nnee
                                  {tenkz}{kernel-port-direction} {#1}
                                  { \tl_use:N \l__tenkz_kernel_scratch_tl }
                              }
                          }
                      }
    ...
    \prop_if_in:NeTF \l__tenkz_model_record_prop { #1 / closed }
      { \seq_set_from_clist:Nn #2 {n,w,s,e} }
      {
        \__tenkz_kernel_route_generated_lane:nT {#1}
          {
            % Four open corner points span three segments.  The east face is
            % the gap between the two bare ends, not part of the open path.
            \seq_set_from_clist:Nn #2 {n,w,s}
          }
      }
  }
        ...
        \str_case:Vn \l__tenkz_kernel_ro_side_tl
          {
            ...
            {all}
              {
                % Address-bearing open arcs remain geometry-dependent and
                % crossing police stays authoritative for them.
    ...
    \bool_if:NT \l_tmpa_bool
      {
        ...
        \quark_if_no_value:NTF \l__tenkz_kernel_r_b_tl
          ...
          {
            % Reach is the maximum distance to every referencing string's
            % nearest waypoint, plus a visible margin.
            \exp_args:NV \clist_map_inline:nn \l__tenkz_kernel_r_b_tl
              { \__tenkz_kernel_r_leg_reach:n {##1} }
            \__tenkz_kernel_r_leg_ink:nnn {#1} {#2}
              {
...
\cs_new_protected:Npn \__tenkz_kernel_end:
  {
    ...
    % as ink.  Prepare geometry before route derivation so non-cell hosts can
    % contribute their actual projected exits while the model is still open.
