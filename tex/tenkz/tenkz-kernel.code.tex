...
\cs_new_protected:Npn \__tenkz_kernel_r_species_hue:nN #1#2
  {
    \prop_get:NnNTF \g__tenkz_kernel_species_prop
      ...
      {
        % The last five words are the house region palette, prelude species
        % since the mark's slot= key retired into species=: the region under
        % discussion is red, a second region beside it shares the marked
        % blue, the complement shares the passive grey (its dashed stroke is
        % the contour's), a collar shares the extra violet, and a neutral
        % window is plain ink.
        \str_case:nnF {#1}
          {
...
\cs_new_protected:Npn \__tenkz_kernel_r_mark_text_ink:nN #1#2
  {
    ...
    \quark_if_no_value:NF \l__tenkz_kernel_r_species_tl
      {
        % A region word tints the contour it names and nothing else: the
        % label keeps the plain ink the retired slot= key always left it,
        % while a semantic species reaches the text as it reaches every
        % other ink the record draws.
