...
% and the reach in \l__tenkz_kernel_r_reach_fp.  The closure stroke and the
% annotation layering of \__tenkz_kernel_r_mark_rails:n both read the rail
% through this one function, so the two cannot drift.
\cs_new_protected:Npn \__tenkz_kernel_r_trace_side_reach:
  {
    ...
    % One trace reach past the outward extreme of the passed
    % row lines per return crossed; the outward extreme is the
    % signed maximum, so one formula serves both sides.
    \fp_set:Nn \l__tenkz_kernel_r_reach_fp
      {
        ( \l__tenkz_kernel_r_xc_fp + 1 )
...
% (\__tenkz_kernel_r_trace_side_reach:), on the flat frame, where the hull's
% axes are the page's and a rail is one page ordinate.  The vertical descents
% of a return run outside every column, so the east and west sides need no
% step today; neither do the column returns of a north/south trace, which no
% picture yet combines with an east or west bracket.
...
\cs_new_protected:Npn \__tenkz_kernel_r_mark_rails:n #1
  {
    ...
    \str_if_eq:eeT { \__tenkz_kernel_r_frame_word: } {flat}
      {
...
% daylight stays between their tips and the rail they stepped over.
