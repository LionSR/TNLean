...
\keys_define:nn { tenkz-kernel-picture }
  {
    cols .code:n =
      {
        \regex_match:nnTF { \A \s* [1-9]\d* \s* \Z } {#1}
          { \__tenkz_kernel_stage_put:nn {cols} {#1} }
          {
...
\cs_new_protected:Npn \__tenkz_kernel_sugar_flag:nnn #1#2#3
  {
    \str_case:nnF {#2}
      {
        {true}  {#3}
        % A false-valued sugar flag is still the sugar spelling --
        % strict mode refuses it like the bare word -- and it stands
        % the expansion down, so a later false overrides an earlier
        % flag under the last-wins doctrine the alphabet follows.
        {false}
          {
