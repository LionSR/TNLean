...
\cs_new_protected:Npn \__tenkz_kernel_addr_try:nN #1#2
  {
    ...
    \regex_extract_once:nnNTF
      ...
      {
    % production 10: a .. b -- the records between two addresses in the
    % frame's own order.  It is tried before every production whose tail
    % pattern would swallow the joiner, and it is spelt with a range, not a
    % hyphen: a cluster member is named `a-2-2`, and a hyphen cannot be told
    % from a name the language itself generates.
    \regex_extract_once:nnNTF
      { \A (.+?) \s* \.\. \s* (.+) \Z } {#1}
      \l__tenkz_kernel_match_seq
...
% A selector names a SET of records, and one grammar serves everywhere a set
% is named: an address, a braced list of addresses, or a range.  What the
% model keeps is the resolved membership -- the record ids -- and never the
% expression that produced it, so containment between two selections is a
% fact the model holds rather than a number an author types.
...
\msg_new:nnn {tenkz}{kernel-selector-range}
  { [TKZ-KERNEL-SELECTOR-RANGE]~a~range~needs~two~cells;~'#1'~gives~
    ...
    \str_if_eq:VnTF \l__tenkz_kernel_sel_tl {picture}
      {
        ...
        \__tenkz_model_map_ids:nn {atom} { \seq_put_right:Nn #2 {##1} }
      }
      {
...
% the atom occupying the cell or carrying the name #1
    ...
    \quark_if_no_value:NTF \l_tmpb_tl
      {
        ...
        \str_if_eq:eeT
          { \__tenkz_kernel_node_item:Nn \l_tmpb_tl {kind} } {cell}
          {
            \exp_args:Nee \__tenkz_kernel_select_cell:nnN
              ...
              \l_tmpb_tl
            \quark_if_no_value:NF \l_tmpb_tl
              { \seq_put_right:NV #2 \l_tmpb_tl }
          }
      }
      { \seq_put_right:NV #2 \l_tmpb_tl }
  }

% a range of cells corners a block; every occupied cell inside joins
\cs_new_protected:Npn \__tenkz_kernel_select_range:nN #1#2
  {
    \exp_args:Ne \__tenkz_kernel_select_corner:nNN
      { \__tenkz_kernel_node_item:nn {#1} {first} }
      \l__tenkz_kernel_r_row_tl \l__tenkz_kernel_r_col_tl
    \exp_args:Ne \__tenkz_kernel_select_corner:nNN
      { \__tenkz_kernel_node_item:nn {#1} {second} }
      \l__tenkz_kernel_r_b_tl \l__tenkz_kernel_r_c_tl
    \int_step_inline:nnn
      { \int_min:nn { \l__tenkz_kernel_r_row_tl } { \l__tenkz_kernel_r_b_tl } }
      { \int_max:nn { \l__tenkz_kernel_r_row_tl } { \l__tenkz_kernel_r_b_tl } }
      {
        \int_step_inline:nnn
          { \int_min:nn { \l__tenkz_kernel_r_col_tl } { \l__tenkz_kernel_r_c_tl } }
          { \int_max:nn { \l__tenkz_kernel_r_col_tl } { \l__tenkz_kernel_r_c_tl } }
          {
            \__tenkz_kernel_select_cell:nnN {##1} {####1} \l_tmpb_tl
            \quark_if_no_value:NF \l_tmpb_tl
              { \seq_put_right:NV #2 \l_tmpb_tl }
          }
      }
...
\cs_new_protected:Npn \__tenkz_kernel_select_corner:nNN #1#2#3
  {
    \str_if_eq:eeTF { \__tenkz_kernel_node_item:nn {#1} {kind} } {cell}
      ...
      {
        \msg_error:nne {tenkz}{kernel-selector-range}
          { \__tenkz_kernel_node_item:nn {#1} {text} }
        \tl_set:Nn #2 {1} \tl_set:Nn #3 {1}
      }
  }
    ...
    \seq_map_inline:Nn #1
      {
        ...
        \__tenkz_kernel_hull_skin:nN {##1} \l__tenkz_kernel_hull_skin_tl
        \seq_put_right:Ne #2
          {
...
\prop_new:N \l__tenkz_kernel_ro_box_prop     % wire id -> {x}{xb}{y}{yb}
\prop_new:N \l__tenkz_kernel_ro_legs_prop    % leg path id -> outermost lane
\prop_new:N \l__tenkz_kernel_ro_taken_prop   % route text -> lanes already out
...
% declaration order, so two routes on one side never overlap and the author
% never counts them.
    ...
    \tl_if_empty:NF \l__tenkz_kernel_ro_side_tl
      {
        ...
        \prop_get:NVN \l__tenkz_kernel_ro_taken_prop
          \l__tenkz_kernel_ro_sel_tl \l__tenkz_kernel_ro_lane_tl
        ...
        \__tenkz_kernel_r_route_edge:nnN   {0} { } \l__tenkz_kernel_ro_xb_fp
        \__tenkz_kernel_r_route_edge:nnN  {90} { } \l__tenkz_kernel_ro_yb_fp
        \__tenkz_kernel_r_route_edge:nnN {180} {-} \l__tenkz_kernel_ro_x_fp
        \__tenkz_kernel_r_route_edge:nnN {270} {-} \l__tenkz_kernel_ro_y_fp
        ...
        \__tenkz_model_map_ids:nn {mark}
          {
            ...
            \str_if_eq:VnT \l__tenkz_kernel_leg_tl {enclosure}
              {
                \__tenkz_model_get:nnN {##1} {target} \l__tenkz_kernel_leg_tl
                \quark_if_no_value:NF \l__tenkz_kernel_leg_tl
                  {
                    \__tenkz_kernel_select:VN \l__tenkz_kernel_leg_tl
        ...
        \quark_if_no_value:NF \l__tenkz_kernel_leg_reach_tl
          {
            \fp_set:Nn \l__tenkz_kernel_r_reach_fp
              {
                max( \l__tenkz_kernel_r_reach_fp ,
                     abs( \l__tenkz_kernel_leg_reach_tl
                          - \l__tenkz_kernel_r_y_fp )
...
\cs_new_protected:Npn \__tenkz_kernel_r_leg_ink:nnn #1#2#3
  {
    ...
    \tl_if_blank:nTF {#3}
      ...
      {
        % A route and an enclosure may both request the same pierced leg.
        % The reach above already folds every consumer into one maximum, so
        % the first request owns the saved path and later requests reuse it.
        \prop_if_in:NeF \l__tenkz_kernel_r_legdrawn_prop {#3}
          {
            \use:e
              {
...
% least as far as the outermost lane that crosses it
\cs_new_protected:Npn \__tenkz_kernel_r_route_leg:nn #1#2
  {
    ...
    \tl_set_eq:NN \l__tenkz_kernel_ro_row_tl \l__tenkz_kernel_ro_lane_tl
    \prop_get:NVN \l__tenkz_kernel_ro_legs_prop
      \l__tenkz_kernel_ro_b_tl \l__tenkz_kernel_ro_reach_tl
    \quark_if_no_value:NF \l__tenkz_kernel_ro_reach_tl
      {
        \tl_set:Ne \l__tenkz_kernel_ro_row_tl
          {
            \fp_eval:n
              {
                abs( \l__tenkz_kernel_ro_reach_tl )
                  > abs( \l__tenkz_kernel_ro_lane_tl )
                  ? \l__tenkz_kernel_ro_reach_tl
                  : \l__tenkz_kernel_ro_lane_tl
              }
          }
...
\cs_new_protected:Npn \__tenkz_kernel_r_route_string:n #1
  {
    ...
    \exp_last_unbraced:NV \__tenkz_kernel_r_route_box:nnnn
...
% the measured hull box back into the four route registers
\cs_new_protected:Npn \__tenkz_kernel_r_route_box:nnnn #1#2#3#4
  {
...
% turn #1 of the hull, counted anticlockwise from the northeast
...
% which turn a point stands nearest, or 0 when there is no such point
...
% An enclosure strokes the offset hull of what it selects.
\cs_new_protected:Npn \__tenkz_kernel_r_mark_enclosure:n #1
  {
    \__tenkz_model_get:nnN {#1} {target} \l__tenkz_kernel_r_tl
    \__tenkz_kernel_select:VN \l__tenkz_kernel_r_tl \l__tenkz_kernel_sel_seq
    \__tenkz_kernel_hull_obstacles:NN
    ...
    \__tenkz_kernel_hull_reach:NnnN \l__tenkz_kernel_sel_obst_seq {0}
      { \l__tenkz_kernel_r_reach_fp } \l__tenkz_kernel_r_xb_tl
    \__tenkz_kernel_hull_reach:NnnN \l__tenkz_kernel_sel_obst_seq {180}
      { \l__tenkz_kernel_r_reach_fp } \l__tenkz_kernel_r_x_tl
    \__tenkz_kernel_hull_reach:NnnN \l__tenkz_kernel_sel_obst_seq {90}
      { \l__tenkz_kernel_r_reach_fp } \l__tenkz_kernel_r_yb_tl
    \__tenkz_kernel_hull_reach:NnnN \l__tenkz_kernel_sel_obst_seq {270}
      { \l__tenkz_kernel_r_reach_fp } \l__tenkz_kernel_r_y_tl
    \fp_set:Nn \l__tenkz_kernel_r_x_fp  { - \l__tenkz_kernel_r_x_tl }
    \fp_set:Nn \l__tenkz_kernel_r_xb_fp {   \l__tenkz_kernel_r_xb_tl }
    \fp_set:Nn \l__tenkz_kernel_r_y_fp  { - \l__tenkz_kernel_r_y_tl }
    \fp_set:Nn \l__tenkz_kernel_r_yb_fp {   \l__tenkz_kernel_r_yb_tl }
