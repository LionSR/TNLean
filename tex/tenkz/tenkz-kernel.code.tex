...
\cs_new_protected:Npn \__tenkz_kernel_declare_species:nn #1#2
  {
    ...
    % from, so a declared species is usable on every record class.
        ...
        {west-east}
          {
            ...
            \bool_lazy_and:nnTF
              ...
              }
              {
                ...
                % Each side branch states only its constants: the arc
                % angles, the outward sign, the rows passed, and the
                % envelope range.  The reach and lead arithmetic below is
                % written once, so a change to it cannot reach one side
                % and miss the other.
                \int_compare:nNnTF
