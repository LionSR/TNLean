...
\prop_new:N \l__tenkz_kernel_leg_addr_prop    % leg path ids named by addresses
    ...
    \str_if_eq:VnTF \l_tmpa_tl {cell}
      {
        ...
        % A leg address names a generated path.  Record that demand at the
        % grammar boundary so the leg pass saves the path whether this node
        % is a crossing operand or a place in its own right.
        \prop_put:Nen \l__tenkz_kernel_leg_addr_prop
          {
            leg - \__tenkz_kernel_node_item:Nn
              \l__tenkz_kernel_parent_tl {face}
            - \__tenkz_kernel_node_item:Nn \l__tenkz_kernel_child_tl {row}
            - \__tenkz_kernel_node_item:Nn \l__tenkz_kernel_child_tl {col}
          }
          { }
      }
      {
    ...
    \str_case:nnF {#1}
      {
        ...
        {leg} { \__tenkz_kernel_r_leg_place:n {#2} }
        {crossing}
          {
...
% A leg used as a place is its record-side endpoint.  Generated leg paths run
% from the carrying record outwards, so this is arclength fraction zero of the
% same saved path a crossing operand names.
\cs_new_protected:Npn \__tenkz_kernel_r_leg_place:n #1
  {
    \__tenkz_kernel_r_opid:nN {#1} \l__tenkz_kernel_ow_sid_tl
    \exp_args:NV \__tenkz_kernel_r_curve_point:nnn
      \l__tenkz_kernel_ow_sid_tl {0} {#1}
  }

% Convert one arclength point of a saved path into placement geometry.
\cs_new_protected:Npn \__tenkz_kernel_r_curve_point:nnn #1#2#3
  {
    \__tenkz_string_bead:nnn {#1} {#2} {tenkz-on-#3}
    \pgfpointanchor {tenkz-on-#3} {center}
    \__tenkz_geom_store:nnn {#3}
      {
...
\cs_new_protected:Npn \__tenkz_kernel_r_onwire_curve:n #1
  {
    ...
    \exp_args:Nee \__tenkz_kernel_r_curve_point:nnn
    ...
    \bool_if:NT \l_tmpa_bool
      {
        ...
        \quark_if_no_value:NTF \l__tenkz_kernel_r_b_tl
          {
            \prop_get:NeN \l__tenkz_kernel_leg_addr_prop
              {
                leg - #2 - \tl_use:N \l__tenkz_kernel_leg_row_tl
                - \tl_use:N \l__tenkz_kernel_leg_col_tl
              }
              \l__tenkz_kernel_leg_reach_tl
            \quark_if_no_value:NTF \l__tenkz_kernel_leg_reach_tl
...
\cs_new_protected:Npn \__tenkz_kernel_r_mark_label:n #1
  {
    ...
    \quark_if_no_value:NF \l__tenkz_kernel_r_tl
      {
        \exp_args:NV \__tenkz_kernel_r_turn:nN
          \l__tenkz_kernel_r_tl \l__tenkz_kernel_r_tau_tl
      }
...
\cs_new_protected:Npn \__tenkz_kernel_begin:n #1
  {
    ...
    \prop_clear:N \l__tenkz_kernel_leg_addr_prop
