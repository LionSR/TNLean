...
\cs_new_protected:Npn \__tenkz_kernel_signature_port_open:nN #1#2
  {
    ...
    \exp_args:NV \__tenkz_kernel_signature_bearing:nN
      \l__tenkz_kernel_port_face_tl \l__tenkz_kernel_port_face_tl
...
\cs_new_protected:Npn \__tenkz_kernel_signature_physical_bearing:n #1
  {
    \__tenkz_kernel_signature_bearing:nN {#1}
      \l__tenkz_kernel_port_face_tl
    \seq_put_right:Ne \l__tenkz_kernel_emit_seq
      { phys: \tl_use:N \l__tenkz_kernel_port_face_tl }
  }

\cs_new_protected:Npn \__tenkz_kernel_signature_physical:n #1
  {
    \str_case:nn {#1}
      {
        {up}     { \__tenkz_kernel_signature_physical_bearing:n {90} }
        {down}   { \__tenkz_kernel_signature_physical_bearing:n {270} }
        {updown}
          {
            \__tenkz_kernel_signature_physical_bearing:n {90}
            \__tenkz_kernel_signature_physical_bearing:n {270}
          }
      }
  }
        ...
        \__tenkz_kernel_policy_leg_renders:nT {##1}
          {
            ...
            \quark_if_no_value:NF \l__tenkz_kernel_scratch_tl
              {
                \exp_args:NV \__tenkz_kernel_signature_physical:n
                  \l__tenkz_kernel_scratch_tl
              }
          }
      }
...
\cs_new_protected:Npn \__tenkz_kernel_grid_wire_end:nnnnn #1#2#3#4#5
  {
    ...
    \prop_get:NeN \l__tenkz_kernel_cell_prop {#2-#3}
      \l__tenkz_kernel_cell_atom_tl
        ...
        \int_compare:nNnTF {#2} = {#4}
          {
            \tl_set:Ne \l__tenkz_kernel_port_face_tl
              { \int_compare:nNnTF {#3} < {#5} {0} {180} }
            \tl_set:Ne \l__tenkz_kernel_port_slot_tl
              { \int_eval:n { #2 - \l__tenkz_kernel_claim_row_int + 1 } }
          }
          {
            \tl_set:Ne \l__tenkz_kernel_port_face_tl
              { \int_compare:nNnTF {#2} < {#4} {270} {90} }
            \tl_set:Ne \l__tenkz_kernel_port_slot_tl
              { \int_eval:n { #3 - \l__tenkz_kernel_claim_col_int + 1 } }
          }
        \__tenkz_kernel_node_put:Nnn
          \l__tenkz_kernel_port_node_tl {kind} {port}
        \__tenkz_kernel_node_put:NnV
...
\cs_new_protected:Npn \__tenkz_kernel_port_open_atom:n #1
  {
    ...
    \quark_if_no_value:NF \l__tenkz_kernel_scratch_tl
      {
        \exp_args:NV \clist_map_inline:nn \l__tenkz_kernel_scratch_tl
                ...
                \str_if_eq:eeT
                  ...
                  {port}
                  {
                    \str_if_eq:eeT
                      {
                        \__tenkz_kernel_node_item:Nn
                          \l__tenkz_kernel_port_record_tl {kind}
                      }
                      {port}
                      {
                        \prop_get:NeN \l__tenkz_kernel_node_prop
                        ...
                        \prop_get:NeN \l__tenkz_kernel_node_prop
                          {
                            \l__tenkz_kernel_port_record_tl / port-type
                          }
                          \l__tenkz_kernel_port_policy_label_tl
...
\prop_new:N \l__tenkz_kernel_r_cross_demand_prop
...
% Generated port legs stay direct strokes unless a parsed crossing address
% names them.  Only those demanded paths enter the saved-route ledger, so a
% large family of otherwise independent open ports pays no crossing cost.
\cs_new_protected:Npn \__tenkz_kernel_r_cross_demand_operand:n #1
  {
    \__tenkz_kernel_r_opid:nN {#1} \l__tenkz_kernel_r_b_tl
    \prop_get:NVN \l__tenkz_kernel_named_prop
      \l__tenkz_kernel_r_b_tl \l__tenkz_kernel_r_c_tl
    \quark_if_no_value:NF \l__tenkz_kernel_r_c_tl
      {
        \__tenkz_model_get:nnN
          { \l__tenkz_kernel_r_c_tl } {origin} \l__tenkz_kernel_r_tl
        \str_if_eq:VnT \l__tenkz_kernel_r_tl {port-open}
          {
            \prop_put:NVn \l__tenkz_kernel_r_cross_demand_prop
              \l__tenkz_kernel_r_c_tl { }
          }
      }
  }
\cs_generate_variant:Nn \__tenkz_kernel_r_cross_demand_operand:n { e }

\cs_new_protected:Npn \__tenkz_kernel_r_cross_demands:
  {
    \prop_clear:N \l__tenkz_kernel_r_cross_demand_prop
    \int_step_inline:nn { \l__tenkz_kernel_node_int }
      {
        \tl_set:Ne \l__tenkz_kernel_r_tl
          { \__tenkz_kernel_r_item:nn { addr - ##1 } {kind} }
        \str_if_eq:VnT \l__tenkz_kernel_r_tl {crossing}
          {
            \__tenkz_kernel_r_cross_demand_operand:e
              { \__tenkz_kernel_r_item:nn { addr - ##1 } {first} }
            \__tenkz_kernel_r_cross_demand_operand:e
              { \__tenkz_kernel_r_item:nn { addr - ##1 } {second} }
          }
      }
  }
...
\cs_new_protected:Npn \__tenkz_kernel_r_port_open_geometry:n #1
  {
    ...
    \str_if_eq:VnTF \l__tenkz_kernel_port_type_tl {physical}
      ...
      {
        \fp_set:Nn \l__tenkz_kernel_r_port_reach_fp
    ...
    \tl_if_empty:NF \l__tenkz_kernel_ro_side_tl
      {
        ...
        \prop_if_in:NeTF \l__tenkz_model_record_prop { #1 / closed }
          { \bool_set_true:N \l__tenkz_kernel_ro_bool }
          {
            ...
            \clist_map_inline:nn {from, to}
              {
                \__tenkz_model_get:nnN {#1} {##1} \l__tenkz_kernel_r_end_tl
                \quark_if_no_value:NF \l__tenkz_kernel_r_end_tl
                  { \bool_set_false:N \l__tenkz_kernel_ro_bool }
                \__tenkz_model_get:nnN
                  {#1} {##1-open} \l__tenkz_kernel_r_end_tl
                \quark_if_no_value:NTF \l__tenkz_kernel_r_end_tl
                  { \bool_set_false:N \l__tenkz_kernel_ro_bool }
                  {
                    \str_if_eq:VnF \l__tenkz_kernel_r_end_tl {route}
                      { \bool_set_false:N \l__tenkz_kernel_ro_bool }
                  }
              }
          }
...
\cs_new_protected:Npn \__tenkz_kernel_r_wire_port_open:n #1
  {
    ...
    \tl_set:Ne \l__tenkz_kernel_r_wire_style_tl
      {
        \str_if_eq:VnTF \l__tenkz_kernel_port_type_tl {physical}
          { physical~leg } { bond } ,
        \tl_use:N \l__tenkz_kernel_r_wire_ink_tl
      }
    ...
    \prop_if_in:NnTF \l__tenkz_kernel_r_cross_demand_prop {#1}
      {
        \tl_set:Ne \l__tenkz_kernel_r_tl
          {
            \str_if_eq:VnTF \l__tenkz_kernel_port_type_tl {physical}
      ...
      {
        \exp_args:NVV \__tenkz_render_stroke:nn
          \l__tenkz_kernel_r_wire_style_tl \l__tenkz_kernel_r_pts_tl
      }
...
\cs_new_protected:Npn \__tenkz_kernel_r_prescan:
  {
    \__tenkz_kernel_r_cross_demands:
