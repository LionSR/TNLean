...
%
% The S4 swap made this stage the package surface: tenkz.sty calls
% \tenkzkernel at load, so `tenkz`, `tenkzeq`, and the body commands are
% bound as soon as the package is.  The switch remains callable and rebinds
% the same meanings, keeping the per-group opt-in spelling of existing
% documents inert; its deletion is the 1.0 freeze's corpus rewrite.  Nothing
% here draws; rendering consumes the records this stage validates.
%
...
\msg_new:nnn {tenkz}{kernel-check-word}
  { [TKZ-EQ-CHECK-WORD]~the~audit~does~not~know~'#1';~check=~states~the~
    audit~(signature),~the~equivalence~it~is~taken~up~to~
    (modulo=bundles),~and~its~recorded~opt-outs~(off={<relation>:~
    <reason>}). }
...
\fp_new:N   \l__tenkz_kernel_r_arc_sf_fp
    ...
    \bool_if:NTF \l__tenkz_kernel_r_arc_wrap_bool
      {
        ...
        % (the walk spends the angle registers), the folds are congruent
        % so one walked length serves both, and the target settles into
        % whichever stretch holds its share.
        ...
        \fp_set:Nn \l__tenkz_kernel_r_arc_tgt_fp
          {
            ( \seq_item:Nn \l__tenkz_kernel_ow_run_stack_seq {-1} )
            * ( 2 * \l__tenkz_kernel_r_arc_lone_fp
        ...
        \fp_compare:nNnTF
          ...
          {
            % fold two: N to the saved far end, walked afresh
            \fp_set_eq:NN \l__tenkz_kernel_r_x_fp
              \l__tenkz_kernel_r_arc_nx_fp
            \fp_set_eq:NN \l__tenkz_kernel_r_y_fp
              \l__tenkz_kernel_r_arc_ny_fp
            \fp_set_eq:NN \l__tenkz_kernel_r_xb_fp
              \l__tenkz_kernel_r_arc_ex_fp
            \fp_set_eq:NN \l__tenkz_kernel_r_yb_fp
              \l__tenkz_kernel_r_arc_ey_fp
          ...
          }
          {
            \fp_compare:nNnTF
              ...
              {
                % fold one, already walked: locate directly
...
% between them at drawing curvatures -- within the audit's own certified
% sub-point tolerance.
...
% ---------- index-wire ink ---------------------------------------------------------------------
% Every ordinary route is saved page-resolved before it is stroked, and the
% auditor must read the same resolved object the renderer inks.  The record
% therefore walks the just-saved soft path back segment by segment: the
% points list opens on the path's first point, a bare x,y pair extends a
% polyline, and a c:-prefixed sextuple is a cubic to its last pair with the
% two control pairs before it.  Coordinates are integer scaled points, the
% stream's one coordinate system, and the stroke is the closure-rail
% half-width.  One record covers one drawn component; a route is a single
% chain today, so that is one record per route.  Caller-declared strings
% keep their own ink pass and emit no wire-ink record.
%
...
\cs_new_protected:Npn \__tenkz_kernel_r_arc_controls:n #1
  {
    ...
    \fp_set_eq:NN \l__tenkz_kernel_r_arc_sf_fp \l__tenkz_kernel_r_arc_s_fp
        ...
        \fp_set:Nn \l__tenkz_kernel_r_arc_d_fp
          {
            ...
            * \fp_compare:nNnTF
                { \l__tenkz_kernel_r_arc_delta_fp } > { 1 }
                {
                  ( 4 * tand ( \l__tenkz_kernel_r_arc_delta_fp / 4 ) )
                  / ( 6 * sind ( \l__tenkz_kernel_r_arc_delta_fp / 2 ) )
                }
                { ( 1 / 3 ) }
          }
        \fp_set:Nn \l__tenkz_kernel_r_arc_cax_fp
...
% cubic can only retrace it.  The route instead folds a half circle out
% of each face and joins the two folds by a run parallel to the chord,
% half a chord to the side of it, on the side the from-turn names; the
% travel tangents at the ends still honour the faces exactly.  The two
% run stations stay in the m/n registers for the early on-wire resolver.
\cs_new_protected:Npn \__tenkz_kernel_r_arc_wrap:
  {
    % half the chord, and the side the from-turn names
    \fp_set:Nn \l__tenkz_kernel_r_arc_d_fp
      {
        sqrt (
          ( \l__tenkz_kernel_r_xb_fp - \l__tenkz_kernel_r_x_fp ) ^ 2 +
    ...
    \fp_set:Nn \l__tenkz_kernel_r_arc_mx_fp
      {
        \l__tenkz_kernel_r_x_fp + \l__tenkz_kernel_r_arc_d_fp
          * cosd ( \l__tenkz_kernel_r_arc_delta_fp )
      }
    \fp_set:Nn \l__tenkz_kernel_r_arc_my_fp
      {
        \l__tenkz_kernel_r_y_fp + \l__tenkz_kernel_r_arc_d_fp
          * sind ( \l__tenkz_kernel_r_arc_delta_fp )
      }
    \fp_set:Nn \l__tenkz_kernel_r_arc_nx_fp
      {
        \l__tenkz_kernel_r_xb_fp + \l__tenkz_kernel_r_arc_d_fp
          * cosd ( \l__tenkz_kernel_r_arc_delta_fp )
      }
    \fp_set:Nn \l__tenkz_kernel_r_arc_ny_fp
      {
        \l__tenkz_kernel_r_yb_fp + \l__tenkz_kernel_r_arc_d_fp
          * sind ( \l__tenkz_kernel_r_arc_delta_fp )
      }
    % each fold is a half circle of chord d: control distance (2/3) d
    \fp_set:Nn \l__tenkz_kernel_r_arc_d_fp
      { 2 * \l__tenkz_kernel_r_arc_d_fp / 3 }
    \tl_set:Ne \l__tenkz_kernel_r_arc_path_tl
      {
        \__tenkz_kernel_r_pair:nn
          { \l__tenkz_kernel_r_x_fp } { \l__tenkz_kernel_r_y_fp }
...
% ---------- automatic label station ---------------------------------------------------------------
% The label doctrine (tenkz-core.code.tex) promises the automatic station a
% free quadrant.  A dot without `label pos=` takes the first face carrying no
% ink in the fixed order s, n, e, w: south is the historical station, so a
% dot whose south face is free does not move, and when every face carries
% ink the station falls back to south.
%
...
%   * every other endpoint -- a cell, a basis member, or a bare record name
%     -- reaches its atom through the cell it stands at, and its ink leaves
%     toward the wire's next point: the waypoint on that end's own side of a
%     string's route, and the opposite end for every wire that draws the
...
%
% A departure along neither axis leaves through a corner and blocks no face.
% The answer is therefore independent of ink order and identical on every
% compile.
...
% pass stands down -- the circle guards, an endpoint atom with no cell, a
% bead on a route that bends through its own waypoints -- records the atom
...
% reading of issue 6195 is what empties this ledger.
...
% Every other endpoint reaches its atom through the cell it stands at, and
% leaves toward the next point of the route: waypoint #4 -- `first` or
% `last` -- when the wire names any, and the opposite endpoint #3 when it
% names none.
    ...
    \quark_if_no_value:nF {#2}
      {
        \str_if_eq:eeTF { \__tenkz_kernel_node_item:nn {#2} {kind} } {port}
          {
            \__tenkz_kernel_port_node_record:nN {#2}
              \l__tenkz_kernel_lab_host_tl
            \quark_if_no_value:NF \l__tenkz_kernel_lab_host_tl
              {
                \__tenkz_kernel_r_label_mark:Ve
                  \l__tenkz_kernel_lab_host_tl
                  { \__tenkz_kernel_node_item:nn {#2} {face} }
                \tl_set_eq:NN \l__tenkz_kernel_lab_face_tl
                  \l__tenkz_kernel_r_face_side_tl
                \__tenkz_kernel_r_label_station_lanes:V
                  \l__tenkz_kernel_lab_host_tl
                \__tenkz_kernel_r_label_port_lane:e
                  { \int_eval:n { \__tenkz_kernel_node_item:nn {#2} {slot} } }
              }
          }
          {
            \__tenkz_kernel_r_label_end_atom:nNNNN {#2}
              ...
              \l__tenkz_kernel_lab_row_tl \l__tenkz_kernel_lab_col_tl
              \l__tenkz_kernel_lab_cell_bool
            \bool_if:NF \l__tenkz_kernel_lab_cell_bool
              {
                % an endpoint atom with no cell carries this wire's ink,
                % and no difference of cell numbers says which way it goes
            ...
            \bool_if:NT \l__tenkz_kernel_lab_cell_bool
              {
                \__tenkz_kernel_r_label_station_lanes:V
                  \l__tenkz_kernel_lab_atom_tl
                \__tenkz_kernel_r_label_next_cell:nnn {#1} {#3} {#4}
                \bool_if:NTF \l__tenkz_kernel_lab_far_bool
                  {
                    % A wire whose far cell belongs to the same atom runs
                    % under one glyph and leaves it by no face at all.  A
                    ...
                    \str_if_eq:VVTF
                      ...
                      {
                        \__tenkz_kernel_r_label_mark_toward:V
                          \l__tenkz_kernel_lab_atom_tl
                      }
                  }
                  {
                    \__tenkz_kernel_r_label_open_face:Vnn
                      \l__tenkz_kernel_lab_atom_tl {#1} {#4}
                  }
              }
          }
      }
...
% other wire runs to its opposite endpoint, and reads that end even where a
% waypoint field is set, because only the string stage draws through one.
% A waypoint that names no cell falls back to the opposite end rather than
% losing the face.
\cs_new_protected:Npn \__tenkz_kernel_r_label_next_cell:nnn #1#2#3
  {
    \bool_set_false:N \l__tenkz_kernel_lab_far_bool
    ...
    \quark_if_no_value:NF \l__tenkz_kernel_lab_via_tl
      {
        % the waypoints arrive as one field, and a waypoint is read as an
        % address, so the list is parsed before an item is taken from it
        ...
        \tl_if_empty:NF \l__tenkz_kernel_lab_node_tl
          {
...
% A wire with one authored end and one open end keeps the open side as a
% word rather than as a node, so no cell says where the ink goes.  The four
% compass words say it outright: the stub leaves the authored end by that
% face.  The field also carries words that name no direction of their own --
% `route`, which defers to the route, and the physical axis words -- and
% those answer nothing.
\cs_new_protected:Npn \__tenkz_kernel_r_label_open_face:nnn #1#2#3
  {
    \str_if_eq:nnTF {#3} {first}
      { \__tenkz_model_get:nnN {#2} {to-open} \l__tenkz_kernel_lab_face_tl }
      { \__tenkz_model_get:nnN {#2} {from-open} \l__tenkz_kernel_lab_face_tl }
    \quark_if_no_value:NF \l__tenkz_kernel_lab_face_tl
      {
        \str_case:Vn \l__tenkz_kernel_lab_face_tl
          {
            {n} { \__tenkz_kernel_r_label_mark_side:nn {#1} {n} }
            {e} { \__tenkz_kernel_r_label_mark_side:nn {#1} {e} }
            {s} { \__tenkz_kernel_r_label_mark_side:nn {#1} {s} }
            {w} { \__tenkz_kernel_r_label_mark_side:nn {#1} {w} }
          }
      }
  }
\cs_generate_variant:Nn \__tenkz_kernel_r_label_open_face:nnn { Vnn }
% The endpoint stands at the near cell and its ink leaves toward the far
% one: a step along a row or a column takes the face it crosses.  The ink
% also runs the length of the lane it leaves by, so it takes any station
% standing in that lane as well: a wire crossing a spanning atom's own row
% takes the face whose station lands in that row, and one crossing its own
% column takes the face whose station lands in that column.
%
% All of that reads a difference of cell numbers as a direction, which holds
% while the frame lays its cells out along its own axes.  A circle does not:
% its columns close into a ring, so the step from the last station to the
% first runs the short way and not back across the ring, and a chord between
% two stations further apart leaves each of them at an angle to the tangent
% that the column numbers say nothing about.  On a circle this reading
% therefore answers nothing rather than answering wrongly, and the faces
% that carry an explicit word -- ports, policy legs, transverse hosts, open
% ends -- go on answering there as everywhere.  Reading the departure as a
% bearing in the endpoint's own frame is what answers a circle; issue 6195.
\cs_new_protected:Npn \__tenkz_kernel_r_label_mark_toward:n #1
  {
    \str_if_eq:eeTF { \__tenkz_kernel_r_frame_word: } {circle}
      { \__tenkz_kernel_r_label_blind:n {#1} }
      { \__tenkz_kernel_r_label_mark_axis:n {#1} }
  }
    ...
    \int_compare:nNnTF
      { \l__tenkz_kernel_lab_far_row_tl } = { \l__tenkz_kernel_lab_row_tl }
      {
        ...
        \int_compare:nNnTF
          ...
          = { \l__tenkz_kernel_lab_col_tl }
          {
            % two basis members of one outer cell share its row and
            % column: the difference is zero and no direction comes out
            % of it, so the ink between them stays uncharted
            \__tenkz_kernel_r_label_blind:n {#1}
          }
          {
            \int_compare:nNnT
...
\cs_generate_variant:Nn \__tenkz_kernel_r_label_mark_toward:n { V }
...
\cs_new_protected:Npn \__tenkz_kernel_r_label_carrier_run:nn #1#2
  {
    ...
    % Two model ends: the run is read from their cells, and that reading
    % stands down on a circle exactly as the endpoint one does.  An open
    % side names its direction outright, so it answers on every frame.
    % Either stand-down leaves the bead's ink uncharted, so the bead joins
    % the blind ledger rather than passing for a verified station.
    \bool_lazy_and:nnT
      { ! \quark_if_no_value_p:N \l__tenkz_kernel_lab_from_tl }
      { ! \quark_if_no_value_p:N \l__tenkz_kernel_lab_to_tl }
      {
            ...
            \tl_if_empty:NF \l__tenkz_kernel_r_skin_label_tl
              {
                ...
                % The claim states who chose the station: the chooser's face
                % word rides the bbox as station=, and provenance= says
                % whether the kernel promised that face free or the author
                % named it.  The claim covers exactly this label's node and
                % is released after it.  An atom on the blind ledger got a
                % default, not a promise: its station stays unclaimed.  A
                % circle frame stands down whole: its occupancy reading is
                % built on lanes, and a circle's closed ring of stations is
                % no lane, so no station on it is a verified promise until
                % the bearing reading of issue 6195 lands.
                \bool_lazy_or:nnTF
                  { \quark_if_no_value_p:N \l__tenkz_kernel_r_c_tl }
                  { \str_if_eq_p:Vn \l__tenkz_kernel_r_c_tl {auto} }
                  {
                    \__tenkz_kernel_r_label_auto_side:nN {#1}
                      \l__tenkz_kernel_r_c_tl
                    \bool_lazy_or:nnF
    ...
    \quark_if_no_value:NTF \l__tenkz_kernel_r_species_tl
      {
        ...
        % The five slots take the house region palette: what a paper marks as
        % the region under discussion is red, a second region beside it is
        % blue, the complement is the grey dashed rest, a collar around a
        % region is violet, and a neutral window is plain ink.
        ...
        \quark_if_no_value:NTF \l__tenkz_kernel_r_hue_tl
          ...
          {
            \str_if_eq:VnT \l__tenkz_kernel_r_hue_tl {complement}
              {
            ...
            \tl_set:Ne \l__tenkz_kernel_r_hue_tl
              {
                \str_case:Vn \l__tenkz_kernel_r_hue_tl
                  {
                    {selected}   { tenkzAction }
                    {secondary}  { tenkzMarked }
                    {complement} { tenkzPassive }
                    {collar}     { tenkzExtra }
                    {neutral}    { tenkzInk }
                  }
              }
...
\keys_define:nn { tenkz-check-spec }
  {
    ...
    modulo .choice: ,
    modulo / bundles .code:n =
      { \bool_set_true:N \l__tenkz_kernel_check_bundles_bool } ,
    modulo / unknown .code:n =
      { \msg_error:nne {tenkz}{kernel-check-word} { modulo=\l_keys_value_tl } } ,
...
\cs_new_protected:Npn \__tenkz_kernel_eq_begin:n #1
  {
    ...
    \prop_clear:N \l__tenkz_kernel_stage_prop
    \__tenkz_kernel_keys:nn { tenkz-kernel-picture } {#1}
    % check={signature, modulo=bundles, off={k: reason}}: the specification
    % is a key list and is read as one.  A reason is prose an author writes
...
% pair leaves the signature, a directed `to' continuing as a `from', weight
% and type intact, and the remaining entries of both factors concatenate.
...
% One panel's signature as a sequence of detokenized entries, bundles
% already expanded when the check runs modulo bundles.
    ...
    \bool_if:NT \l__tenkz_kernel_check_bundles_bool
      {
        \__tenkz_kernel_signature_expand_bundles:NN
          \l__tenkz_kernel_side_sig_tl \l__tenkz_kernel_side_sig_tl
      }
    \seq_clear:N #2
...
% field -- type, weight -- is the same index's and rides along unchanged.
...
% side kinds; the side signatures arrive in \l_tmpa_tl and \l_tmpb_tl with
...
\cs_new_protected:Npn \__tenkz_kernel_eq_panel_compare:n #1
  {
    ...
    \bool_if:NT \l__tenkz_kernel_check_bundles_bool
      {
        \__tenkz_kernel_signature_expand_bundles:NN \l_tmpa_tl \l_tmpa_tl
        \__tenkz_kernel_signature_expand_bundles:NN \l_tmpb_tl \l_tmpb_tl
      }
    \__tenkz_kernel_eq_compare_core:nee {#1}
...
% names, and tenkz.sty calls it at load, which is what makes the kernel the
% package surface.
