...
% the label band against the ink actually laid.  The style is played in a
% throwaway group and the resolved line width read back.
\cs_new_protected:Npn \__tenkz_kernel_r_wire_ink_width:n #1
  {
    \group_begin:
      \tikzset {#1}
...
\cs_generate_variant:Nn \__tenkz_kernel_r_wire_ink_width:n { V }
...
\cs_new_protected:Npn \__tenkz_kernel_r_trace_stroke:nn #1#2
  {
    \__tenkz_kernel_r_sid:nN {#1} \l__tenkz_kernel_r_sid_tl
    \__tenkz_kernel_r_wire_ink_width:n { trace }
    ...
    \tl_if_blank:nTF {#3}
      {
        ...
        \__tenkz_kernel_r_wire_ink_width:n { bond }
...
\cs_new_protected:Npn \__tenkz_kernel_r_index_route:n #1
  {
    ...
    \prop_get:NnN \l__tenkz_kernel_r_index_style_prop {#1}
      \l__tenkz_kernel_r_wire_style_tl
    % The wire-ink record is written here, where the route is stroked: the
    % saved path now carries whatever gaps crossing surgery cut into it, so
    % the auditor reads exactly the ink the reader sees, at the width the
    % route's own resolved style strokes it with.
    \exp_args:NV \__tenkz_kernel_r_wire_ink_width:n
      \l__tenkz_kernel_r_wire_style_tl
