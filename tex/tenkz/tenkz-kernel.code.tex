...
%
% One length is budgeted rather than resolved.  On a projected plane the
% transverse renderer starts a physical opening at the glyph's own outward
% support point, so its tip stands that support beyond the leg length counted
% here.  Reading it would mean asking the glyph-anchor production, which
% writes the very coordinate registers the reach is computed in.
