...
\cs_new_protected:Npn \__tenkz_kernel_r_restyle_guard:
  {
    \clist_map_inline:nn { bond , physical~leg , trace }
      {
        \group_begin:
        \cs_set_eq:NN \tikz@options \prg_do_nothing:
        \tikzset {##1}
        % the sync hides inside the deferred option's own group, which a
        % token-level search cannot enter: read the options as a string
        \tl_set:Ne \l__tenkz_kernel_scratch_tl
          { \tl_to_str:N \tikz@options }
