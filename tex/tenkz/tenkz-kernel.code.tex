...
\cs_new_protected:Npn \__tenkz_kernel_hull_measure_glyph:n #1
  {
    ...
    \bool_if:NT \l__tenkz_kernel_r_basis_plane_bool
      {
        % A paired ellipsis is itself page-upright, including the backing
        % node that erases wires.  Move that page-space padding inside the
        % inverse envelope instead of letting the affine node scale it.
        \str_if_eq:VnT \l__tenkz_kernel_hull_skin_tl {dots}
          {
            \prop_if_in:NnT
              \l__tenkz_kernel_skin_pairing_host_prop {#1}
              {
                \tl_put_right:Nn \l__tenkz_kernel_hull_b_tl
                  { , inner~sep = 0pt }
              }
          }
      }
...
\cs_new_protected:Npn \__tenkz_kernel_r_leg_plan_enrol:nn #1#2
  {
    \__tenkz_kernel_r_leg_plan_id:nnN {#1}{#2}
...
% The lane planner's semantic identity includes the explicit basis member.
% Ink and crossing path names retain their established cell-level spelling.
\cs_new_protected:Npn \__tenkz_kernel_r_leg_plan_id:nnN #1#2#3
  {
    ...
    \tl_set:Ne #3
      {
        leg-#2-member-\tl_use:N \l__tenkz_kernel_leg_member_tl
        -\tl_use:N \l__tenkz_kernel_leg_row_tl
        -\tl_use:N \l__tenkz_kernel_leg_col_tl
      }
  }

% Default-member engine paths keep their public crossing name.  An explicit
% later member needs a private suffix so two valid legs cannot redeclare one
% saved string; crossing lookup itself remains cell-level and unchanged.
\cs_new_protected:Npn \__tenkz_kernel_r_leg_path_id:nnN #1#2#3
  {
    \__tenkz_model_get:nnN {#1} {member} \l__tenkz_kernel_leg_member_tl
    \quark_if_no_value:NT \l__tenkz_kernel_leg_member_tl
      { \tl_set:Nn \l__tenkz_kernel_leg_member_tl {1} }
    \tl_set:Ne #3
      {
        leg-#2-
        \int_compare:nNnF {\l__tenkz_kernel_leg_member_tl} = {1}
          {member-\tl_use:N \l__tenkz_kernel_leg_member_tl-}
        \tl_use:N \l__tenkz_kernel_leg_row_tl
        -\tl_use:N \l__tenkz_kernel_leg_col_tl
      }
...
\cs_new_protected:Npn \__tenkz_kernel_r_leg_consume:nnn #1#2#3
  {
    ...
    \__tenkz_kernel_r_leg_path_id:nnN {#1}{#2}
      \l__tenkz_kernel_leg_plan_cid_tl
