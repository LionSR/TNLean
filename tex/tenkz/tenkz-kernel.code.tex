        ...
        \seq_map_inline:Nn \l__tenkz_kernel_sel_seq
          {
            ...
            \bool_if:NTF \l__tenkz_kernel_turn_bool
              {
                % The rows a member occupies run from its anchor through
                % its `wires=` span, so a spanning glyph anchored off the
                % boundary row still reaches it.
                \__tenkz_kernel_r_span:nNN {##1}
                  \l__tenkz_kernel_r_b_tl \l__tenkz_kernel_r_c_tl
...
% basis ny converts it, and on the flat frame the two coincide.
