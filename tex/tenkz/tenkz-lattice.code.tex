...
\cs_new_protected:Npn \tenkzl_draw_all:
  {
    ...
    \tenkzl_draw_side_traces:  % (k) same-sheet opposing-side closures
    \tenkzl_emit_boundary:     % (l) event stream only, no ink
  }
...
\cs_new_protected:Npn \tenkzl_side_cup_corridor:nnn #1#2#3
  {
    ...
    % Site labels occupy the paper north-east quadrant.  Choose the opposite
    % transverse exterior for every cup on this side.  The sign is the
    % projection of paper north-east onto the side's transverse axis.
    \dim_compare:nNnTF
    ...
    \dim_set:Nn \l__tenkzl_cuplane_dim
      { \l__tenkzl_cupsil_dim + \tenkz@dim{\tenkz@r@dotdia} / 2
        + \tenkz@dim{\tenkz@r@daylight} }
    ...
    % node bounds recorded by \tenkzl_sitelabel:nnnn keeps arbitrary body
        ...
        \bool_if:NT \l_tmpa_bool
          {
            ...
            \dim_add:Nn \l__tenkzl_cupproj_dim
              { \tenkz@dim{\tenkz@r@dotdia} / 2 }
            \tenkzl_side_cup_from_projections:NNNN
              \l__tenkzl_cupproj_dim \l__tenkzl_labelshift_dim #8 #9
...
% -- (k) same-sheet periodic virtual closures -------------------------
...
% The side-cup silhouette scanner already measures every surviving site and
% boundary tip in the chosen direction.  Extend that exact silhouette by
% every already-rendered body site-label rectangle; body content therefore
% remains materialize-once even when it controls periodic routing.
\cs_new_protected:Npn \tenkzl_trace_silhouette:n #1
  {
    \tenkzl_side_cup_silhouette:n {#1}
    \prop_map_inline:Nn \l__tenkzl_labelxmin_prop
      {
        \dim_set:Nn \l__tenkzl_labelxmin_dim {##2}
        \prop_get:NnN \l__tenkzl_labelxmax_prop {##1}
          \l__tenkzl_labelbbox_tl
        \dim_set:Nn \l__tenkzl_labelxmax_dim {\l__tenkzl_labelbbox_tl}
        \prop_get:NnN \l__tenkzl_labelymin_prop {##1}
          \l__tenkzl_labelbbox_tl
        \dim_set:Nn \l__tenkzl_labelymin_dim {\l__tenkzl_labelbbox_tl}
        \prop_get:NnN \l__tenkzl_labelymax_prop {##1}
          \l__tenkzl_labelbbox_tl
        \dim_set:Nn \l__tenkzl_labelymax_dim {\l__tenkzl_labelbbox_tl}
