...
% A (1,0) winding in the exterior corridor.  Its first edge is the seam
% representative through the lattice; the remaining edges are the exterior
% return.  Unlike a generic closed string, these segments must stay linear:
% smoothing the whole cycle with Hobby turns the seam representative into an
% unrelated oval.
\cs_new_protected:Npn \__tenkz_string_declare_corridor_wind:nn #1#2
  {
    ...
    \tenkz@event{string|id=#1|kind=wind|class=1,0|
