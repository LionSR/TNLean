...
% box, circle, pill, mpo) always override the policy.  The event stream
% records the RESOLVED skin.
...
\def\tenkz@r@glyphref{0.50}    % the size-class reference WIDTH: the one
                               ...
                               % glyphs it joins.  A label wider than the
                               % reference still grows its glyph -- the
                               % reference is a floor, and unequal ink says
                               % the name outgrew its class
...
\tikzset{
  ...
  % The three labelled skins share ONE reference width (\tenkz@r@glyphref):
  ...
  % Height stays per class -- a box is square at its floor, a pill and an
  % on-wire capsule are deliberately flatter.
  ...
  % capsule: it carries mathematics inside a plain-ink circular outline.
  % The common glyph reference is its diameter floor.  TikZ's circle shape
  % then enlarges both axes together when a wider label requires more room.
  ...
  % Compatibility-object size classes act on the metric floor, not by an
  % affine canvas transform: the audited glyph measures the actual enlarged
  % silhouette.  This is the transitional free renderer's content-sized
  % contract, not the canonical kernel's fixed two-axis atom contract.
  % Labelled skins take 3/4, 1, or 5/4 of their common reference width; dots
  % use the existing cluster, ordinary, and inscribed-glyph diameters.
  % Medium repeats the established reference extent exactly.
  tenkz inscribed size s/.style={
      minimum width=\dimexpr
        \tenkz@r@glyphref\tenkz@pitch * 3 / 4\relax},
  tenkz inscribed size m/.style={
      minimum width=\tenkz@dim{\tenkz@r@glyphref}},
...
% ---------- label placement: one bearing, two consequences ----------
% A label's placement is a bearing read in its host's own axes; the compass
% words are sugar for the eight right and half-right angles of that bearing,
% and nothing else.  ONE bearing settles both quantities a placed label
% needs: the outward step from the host's boundary point, and the anchor,
% which is the same bearing turned through half a turn.  Choosing the two
% separately is how a label comes to step one way and hang the other, so
% they are derived here, from one table, and never stated side by side.
%
% The table holds each word's outward direction as a pair of signs.  The
% anchor is read back out of the same table under the negated pair, and the
% step is that direction made a unit vector and scaled by the caller's
% clearance band -- so a diagonal label stands exactly as far from its host
% as a cardinal one, and no factor is written down.
\prop_new:N \g__tenkz_bearing_prop      % word -> "sx,sy"
\prop_new:N \g__tenkz_bearing_word_prop % "sx,sy" -> word
\cs_new_protected:Npn \__tenkz_bearing_declare:nnn #1#2#3
  {
    \prop_gput:Nnn \g__tenkz_bearing_prop {#1} {#2,#3}
    \prop_gput:Nnn \g__tenkz_bearing_word_prop {#2,#3} {#1}
  }
\__tenkz_bearing_declare:nnn {east}       { 1}{ 0}
\__tenkz_bearing_declare:nnn {north~east} { 1}{ 1}
\__tenkz_bearing_declare:nnn {north}      { 0}{ 1}
\__tenkz_bearing_declare:nnn {north~west} {-1}{ 1}
\__tenkz_bearing_declare:nnn {west}       {-1}{ 0}
\__tenkz_bearing_declare:nnn {south~west} {-1}{-1}
\__tenkz_bearing_declare:nnn {south}      { 0}{-1}
\__tenkz_bearing_declare:nnn {south~east} { 1}{-1}

% \tenkz_label_bearing:nnNNN {word} {clearance band} \anchor \dx \dy
% True when the word is a placement; then the anchor tl holds the opposite
% compass word and the two dimensions hold the outward step.
\tl_new:N \l__tenkz_bearing_tl
\clist_new:N \l__tenkz_bearing_clist
\int_new:N \l__tenkz_bearing_sx_int
\int_new:N \l__tenkz_bearing_sy_int
\dim_new:N \l__tenkz_bearing_band_dim
\prg_new_protected_conditional:Npnn \tenkz_label_bearing:nnNNN #1#2#3#4#5
    { T, F, TF }
  {
    \prop_get:NnNTF \g__tenkz_bearing_prop {#1} \l__tenkz_bearing_tl
      {
        \clist_set:NV \l__tenkz_bearing_clist \l__tenkz_bearing_tl
        \int_set:Nn \l__tenkz_bearing_sx_int
          { \clist_item:Nn \l__tenkz_bearing_clist {1} }
        \int_set:Nn \l__tenkz_bearing_sy_int
          { \clist_item:Nn \l__tenkz_bearing_clist {2} }
        \prop_get:NeN \g__tenkz_bearing_word_prop
          {
            \int_eval:n { -\l__tenkz_bearing_sx_int } ,
            \int_eval:n { -\l__tenkz_bearing_sy_int }
          } #3
        \dim_set:Nn \l__tenkz_bearing_band_dim {#2}
        \dim_set:Nn #4 { \__tenkz_bearing_step:N \l__tenkz_bearing_sx_int }
        \dim_set:Nn #5 { \__tenkz_bearing_step:N \l__tenkz_bearing_sy_int }
        \prg_return_true:
      }
      { \prg_return_false: }
  }
% one component of the unit bearing, scaled by the band
\cs_new:Npn \__tenkz_bearing_step:N #1
  {
    \fp_to_dim:n
      {
        \int_use:N #1 /
          sqrt( \l__tenkz_bearing_sx_int * \l__tenkz_bearing_sx_int
              + \l__tenkz_bearing_sy_int * \l__tenkz_bearing_sy_int )
          * \dim_to_fp:n { \l__tenkz_bearing_band_dim }
      }
  }
\prg_generate_conditional_variant:Nnn \tenkz_label_bearing:nnNNN
  { VnNNN } { T, F, TF }
