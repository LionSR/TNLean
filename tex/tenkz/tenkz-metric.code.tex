...
% Page-round default beads reduce the two body supports to one diameter.
\cs_new:Npn \__tenkz_metric_round_bead_spacing_floor:
  {
    \fp_eval:n
      {
...
\__tenkz_metric_declare:nnn {daylight} {0.15}
  { the~one~gap~between~a~silhouette~and~closure~or~annotation~ink:~
    a~traced~row's~return~stands~this~far~beyond~the~row's~open~indices,~
    a~span~brace~this~far~beyond~the~sites~it~spans }
