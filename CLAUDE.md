...
| `Complex.invSqrtTwo` | helper definition | Writing the normalization `1 / √2` as a complex number, with `invSqrtTwo_mul_self`, `invSqrtTwo_pow_mul_self`, and `star_invSqrtTwo`, instead of a local copy | `TNLean/Algebra/ComplexSqrt.lean` |
...
| `MPSTensor.MultiBlockCompression.ofGolden`, `MPSTensor.MultiBlockCompression.remainder_eq_zero_of_goldenGauge` | helper definition and theorem | Assembling a multi-block compression datum of a worked example over `ℤ[σ]` from decided identities between explicit golden matrices, and showing its remainder vanishes when the conjugated letters are block diagonal | `TNLean/MPS/FundamentalTheorem/Reduction/Examples/GoldenCompression.lean` |
| `MPSTensor.isNormal_of_golden_single` | helper theorem | Proving normality of a tensor over `ℤ[σ]` from a decided table expressing every matrix unit as a combination of words of one positive length | `TNLean/MPS/FundamentalTheorem/Reduction/Examples/GoldenCompression.lean` |
