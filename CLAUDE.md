...
| `MPSTensor.MultiBlockCompression.ofGolden`, `MPSTensor.MultiBlockCompression.remainder_eq_zero_of_goldenGauge` | helper definition and theorem | Assembling a multi-block compression datum of a worked example over `ℤ[σ]` from decided identities between explicit golden matrices, and showing its remainder vanishes when the conjugated letters are block diagonal | `TNLean/MPS/Examples/Rings/GoldenCompression.lean` |
...
| `MPSTensor.isNormal_of_golden_single` | helper theorem | Proving normality of a tensor over `ℤ[σ]` from a decided table expressing every matrix unit as a combination of words of one positive length | `TNLean/MPS/Examples/Rings/GoldenCompression.lean` |
