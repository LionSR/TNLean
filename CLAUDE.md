...
| `MPOTensor.mpo_eq_of_conj`, `MPSTensor.mpo_mul_eq_of_zsqrt2_conj` | helper theorems | Equating periodic operators of tensors related letterwise by a bond similarity, or deriving a fusion rule `O_L(X) O_L(Y) = O_L(T)` from a letter identity over `ℤ[√2]` that conjugates the stacked tensor into `T` plus a zero block | `TNLean/MPS/MPDO/BondSimilarity.lean`, `TNLean/MPS/Examples/Ising/Zsqrt2OperatorFusion.lean` |
