...
| [`docs/proof_debt.md`](docs/proof_debt.md) | Weekly proof-debt loop, ledger format, shrink rhythm; the texra-lean-skills `find-simplification` skill, with its TNLean addendum in `.claude/skills/`, is its hand-driven audit |
...
| `MPOTensor.mpo_apply_of_bondOne`, `MPOTensor.isNormal_of_bondOne` | helper theorems | Evaluating the periodic operator of a bond-one (on-site) tensor as the product of its one-site entries, or proving such a tensor normal from one one-site matrix equal to `1` | `TNLean/MPS/MPDO/BondOneOperator.lean` |
...
| `Complex.ofReal_sqrt_inv_mul_self` | helper theorem | Multiplying the inverse of the complex coercion of a nonnegative real square root by itself, in any spelling of `1 / √x` | `TNLean/Algebra/ComplexSqrt.lean` |
| `Complex.invSqrtTwo` | helper definition | Writing the normalization `1 / √2` as a complex number, with `invSqrtTwo_ne_zero`, `invSqrtTwo_mul_self`, `invSqrtTwo_sq`, `invSqrtTwo_pow_mul_self`, `star_invSqrtTwo`, and `conj_invSqrtTwo`, instead of a local copy | `TNLean/Algebra/ComplexSqrt.lean` |
...
| `LinearMap.exists_smulRight_of_finrank_range_eq_one` | helper theorem | Factoring a linear map with one-dimensional range into a scalar functional and a nonzero range vector with a preimage satisfying `φ x = 1` | `TNLean/Algebra/RankOneFactorization.lean` |
| `Matrix.trace_ofFn_prod_eq_sum_cyclic` | helper theorem | Expanding the trace of an ordered product of finitely many square matrices as a sum over cyclic index paths, one factor per site and its cyclic successor | `TNLean/Algebra/MatrixCyclicPathSum.lean` |
| `Matrix.trace_ofFn_prod_smul_single`, `MPOTensor.mpo_apply_of_eq_smul_single` | helper theorems | Evaluating the trace of a ring of scaled matrix units, or a periodic operator whose letters are scaled matrix units, as the product of the scalars when the units chain cyclically and zero otherwise | `TNLean/Algebra/MatrixCyclicPathSum.lean`, `TNLean/MPS/MPDO/BondSimilarity.lean` |
| `MPOTensor.mpo_eq_of_conj`, `MPOTensor.mpo_eq_of_diagonal_conj`, `MPSTensor.mpo_mul_eq_of_zsqrt2_conj` | helper theorems | Equating periodic operators of tensors related letterwise by a bond similarity (a diagonal one needs only nowhere-zero entries), or deriving a fusion rule `O_L(X) O_L(Y) = O_L(T)` from a letter identity over `ℤ[√2]` that conjugates the stacked tensor into `T` plus a zero block | `TNLean/MPS/MPDO/BondSimilarity.lean`, `TNLean/MPS/Examples/Ising/Zsqrt2OperatorFusion.lean` |
| `Matrix.trace_prod_ofFn_sum_smul`, `Matrix.finKronecker_mul_apply_of_eq_ite`, `Matrix.mul_finKronecker_apply_of_eq_ite` | helper theorems | Expanding the trace of an ordered product of linear combinations `∑_j f_{lj} • B_{lj}` into a sum over choice functions, as when twisting a physical leg of a tensor; moving configurations under a Kronecker power of a permutation-like matrix | `TNLean/Algebra/FinKronecker.lean` |
| `Fintype.sum_reverse_three`, `sum_last_two_first_four`, `sum_last_first_four`, `sum_last_two_first_five`, `sum_permute_five` | helper theorems | Permuting three to five independent nested finite-sum binders in tensor-network contractions | `TNLean/Algebra/FinSumPermutation.lean` |
| `Fintype.sum_prod_neg_one_pow_val_mul` | helper theorem | Summing a character `a ↦ ∏ i, (-1) ^ (a i * c i)` over all binary configurations `a : ι → Fin 2`, which gives `2 ^ card ι` when `c = 0` and zero otherwise, instead of re-running the product-of-sums expansion when a contraction of two sign kernels collapses to a delta constraint | `TNLean/Algebra/BinaryCharacterSum.lean` |
| `Finset.sum_congr₂` | helper theorem | Descending through two nested finite sums whose inner index type may depend on the outer index | `TNLean/Algebra/FinSumPermutation.lean` |
| `commonBufferLength` | helper definition | Choosing one positive length `L` with `N i ≤ L + 1` for a finite family of nilpotency lengths; pair with `MPSTensor.IsReduction.blockTensor_commonBufferLength_one` to make one blocked exterior site suffice for every member of a finite family of reductions | `TNLean/Algebra/CommonBufferLength.lean`, `TNLean/MPS/Core/ReductionBlocking.lean` |
| `generalize_decide` | tactic macro | Closing a bit or residue identity whose only free data are finitely many values in small finite types occurring inside larger terms; abstracts the listed terms and decides the closed statement | `TNLean/Algebra/GeneralizeDecide.lean` |
| `Kraus.evalWord_compress_of_left_absorb`, `Kraus.trace_evalWord_compress_of_left_absorb`, `Kraus.evalWord_compress_of_right_absorb`, `Kraus.trace_evalWord_compress_of_right_absorb` | helper theorems | Replacing letters `B i` by rectangular compressions `W * B i * V` in nonempty words and their traces, when `V * W * B i = B i` or `B i * (V * W) = B i` (one step of a canonical-form reduction onto a single diagonal block) | `TNLean/MPS/FundamentalTheorem/Reduction/AbsorbingCompression.lean` |
| `MPOTensor.mpo_apply_eq_prod_of_forced_bond`, `MPOTensor.mpo_apply_eq_sum_cyclic` | helper theorems | Computing an entry of the periodic operator of a monomial tensor (such as a CZX matrix product unitary) as the product along the one closed bond configuration that every other configuration meets a vanishing entry against | `TNLean/MPS/MPDO/OperatorCyclicSum.lean` |
| `MPOTensor.mpo_apply_of_forced_right_bond`, `MPOTensor.mpo_apply_of_forced_left_bond` | helper theorems | Computing the periodic operator of a monomial tensor whose output is a sitewise function of the input and whose outgoing or incoming bond is a function of the physical indices; pass the entrywise `_apply` lemma of the tensor and close with the phase sum | `TNLean/MPS/MPDO/OperatorCyclicSum.lean` |
| `ite_zero_mul_ite_zero` | helper theorem | Combining a product of two zero-or-value indicator functions into one conjunction before simplifying coordinate conditions | `Mathlib/Algebra/Ring/Defs.lean` |
| `Matrix.monomial_mulVec_eq_self_iff` | helper theorem | Rewriting invariance of a vector under a monomial (permutation-times-phase) matrix as the coordinate transport rule `v (σ s) = φ s * v s` | `TNLean/Algebra/MonomialMatrix.lean` |
...
| `MPSTensor.groundSpaceMap_mul_eq_of_restrict`, `MPSTensor.mem_groundSpace_succ_of_intertwine`, `MPSTensor.groundSpaceMap_mul_eq_of_cyclicTranslate_groundSpaceMap_eq` | helper theorems | Running the intersection-property and ring-closing comparisons for a tensor that is not injective: compare the boundary matrices of two restrictions or two cuts without an injectivity hypothesis, and grow the boundary matrix back from any left inverse `∑ j, R j * A j = 1` (such as the adjoints of a left-canonical tensor) | `TNLean/MPS/ParentHamiltonian/IntersectionProperty.lean`, `TNLean/MPS/ParentHamiltonian/UniqueGroundState.lean` |
| `Matrix.eq_vecCons_fin_two`, `Matrix.eq_vecCons_fin_three`, `Matrix.eq_vecCons_fin_four` | helper theorems | Rewriting a configuration on a two-, three- or four-site window into vector notation before a coordinate-by-coordinate evaluation of an example tensor | `TNLean/Algebra/FinVecEta.lean` |
| `MPSTensor.MultiBlockCompression.ofRing`, `MPSTensor.MultiBlockCompression.ofRingBlockDiagonal`, `MPSTensor.MultiBlockCompression.remainder_ofRing`, `MPSTensor.MultiBlockCompression.triangular_of_offDiag` | helper definitions and theorems | Assembling a multi-block compression datum of a worked example from decided entrywise identities of its conjugated letters over any ring `R →+* ℂ` (or from one block-diagonal letter identity), and showing its remainder vanishes when those letters are block diagonal; unfold the datum before applying the remainder lemma. `ofGolden`, `ofEisenstein`, `ofConjMatrix` and `ofScalarFlagFour` are instances | `TNLean/MPS/FundamentalTheorem/Reduction/ExplicitGauge.lean` |
| `MPSTensor.complexOfRing_mul_eq_one`, `MPSTensor.evalWord_complexOfRing`, `MPSTensor.isNormal_of_complexOfRing_smul_single` | helper theorems | Transporting a decided identity of a worked example over an exact ring into the complex matrices: a pair of mutually inverse gauges, a word evaluation, or a table of scaled matrix units certifying normality | `TNLean/Algebra/ComplexOfRing.lean`, `TNLean/MPS/FundamentalTheorem/Reduction/RingEmbedding.lean` |
| `MPSTensor.isNormal_of_complexOfRing_single`, `MPSTensor.isNormal_of_complexOfRing_letter_eq_smul_single` | helper theorems | Proving normality of a tensor over an exact ring from a decided table expressing every matrix unit as a combination of words of one positive length, or from letters that are rescaled multiples of the matrix units | `TNLean/MPS/FundamentalTheorem/Reduction/RingEmbedding.lean` |
| `MPSTensor.right_intertwiner_eq_zero_of_ringCertificate`, `MPSTensor.left_intertwiner_eq_zero_of_ringCertificate`, `MPSTensor.mulVec_eq_zero_of_ringCertificate`, `MPSTensor.vecMul_eq_zero_of_ringCertificate` | helper theorems | Showing that a worked example admits no nonzero sitewise intertwiner `B i * X = X * C i` (or its left and bond-one vector forms) from a certificate matrix over an exact ring `R →+* ℂ` that selects a full-rank set of the sitewise equations, decided over `R`, instead of a hand-written `linear_combination` elimination | `TNLean/MPS/FundamentalTheorem/Reduction/AssemblyLemmas.lean` |
| `MPOTensor.exists_nat_eq_of_mpo_mulVec_mpv_eq_smul` | helper theorem | Concluding that a length-independent eigenvalue of the periodic operators of a matrix product operator on the periodic vectors of a normal tensor is a nonnegative integer | `TNLean/MPS/FundamentalTheorem/Reduction/IntegralRankOneAction.lean` |
| `MPSTensor.IsReduction.trans`, `MPSTensor.IsReduction.mulTensor_kronId`, `MPSTensor.IsReduction.mulTensor_idKron`, `MPSTensor.IsReduction.mulTensor_assoc_left`, `MPSTensor.IsDressedProportional.pullback`, `MPOTensor.assocInv_pentagon` | helper theorems | Composing reductions of stacked matrix product operators into fusion trees, and comparing two trees by a scalar against long words; rewrite tree boundaries with `kronId_mul`, `idKron_mul`, `Matrix.mul_assoc` and the `assocInv_*_assoc` naturality lemmas | `TNLean/MPS/Core/ReductionComposition.lean` |
