...
% the thin horizontal and vertical bonds are the virtual edges; the short thick
% diagonal stub at every vertex is the physical leg. The dangling boundary
% bonds are the open virtual indices.
\newcommand{\TNPEPSLatticeState}{%
    \begin{tikzpicture}[tn picture]
        ...
        \foreach \y in {0,1,2} {
            ...
            \draw[tn leg] (pl0\y.west) -- ++(-0.38,0);
            \draw[tn leg] (pl2\y.east) -- ++(0.38,0);
        }
        \foreach \x in {0,1,2} {
            \draw[tn leg] (pl\x0) -- (pl\x1) -- (pl\x2);
            \draw[tn leg] (pl\x0.south) -- ++(0,-0.38);
            \draw[tn leg] (pl\x2.north) -- ++(0,0.38);
        }
        \foreach \x in {0,1,2} {
            \foreach \y in {0,1,2} {
...
% Vertex balance of edge scalars. A scalar $z_e$ on each incident edge; the
% gauge is balanced at $v$ when the incident scalars multiply to one.
\newcommand{\TNPEPSVertexScalarBalance}{%
    \begin{tikzpicture}[tn picture]
        ...
        \TN@opdot{vbZS}{(0,-0.5)}
        \node at (-0.5,0.26) {$z_{e_1}$};
        \node at (0.5,0.26) {$z_{e_2}$};
        \node at (0.34,0.5) {$z_{e_3}$};
        \node at (0.34,-0.5) {$z_{e_4}$};
        \node at (2.25,0) {$\displaystyle\prod_{e \ni v} z_e = 1$};
    \end{tikzpicture}%
}
