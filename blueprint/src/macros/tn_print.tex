...
% Sector decomposition of a simple MPDO tensor and its neighbouring bond.
\newcommand{\TNEtaSectorDecomposition}{%
    \begin{tikzpicture}[tn picture]
        \node[draw, rounded corners, fill=blue!8, inner sep=2pt] (Uu) at (-2.55,0.76) {$U^\dagger$};
        \TN@tensordot{tnK}{(-2.55,0)}
        \node[draw, rounded corners, fill=blue!8, inner sep=2pt] (Ud) at (-2.55,-0.76) {$U$};
        \draw[tn phys leg] ($(Uu.north)+(-0.11,0)$) -- ++(0,0.36);
        \draw[tn phys leg] ($(Uu.north)+(0.11,0)$) -- ++(0,0.36);
        \draw[tn phys leg] (Uu.south) -- (tnK.north);
        \draw[tn phys leg] (tnK.south) -- (Ud.north);
        \draw[tn phys leg] ($(Ud.south)+(-0.11,0)$) -- ++(0,-0.36);
        \draw[tn phys leg] ($(Ud.south)+(0.11,0)$) -- ++(0,-0.36);
        \TN@leftleg{tnK}{0.58}
        \TN@rightleg{tnK}{0.58}
        \node at (-2.88,-0.24) {$K$};
        \node at (-1.43,0) {$=$};
        \node at (-0.68,0) {$\displaystyle\bigoplus_k$};
        \TN@tensordot{tnL}{(0.30,0)}
        \TN@tensordot{tnR}{(1.42,0)}
        \draw[tn leg] (tnL.west) -- ++(-0.52,0);
        \draw[tn leg] (tnL.east) -- (tnR.west);
        \draw[tn leg] (tnR.east) -- ++(0.52,0);
        \TN@upleg{tnL}{0.50}
        \TN@downleg{tnL}{0.50}
        \TN@upleg{tnR}{0.50}
        \TN@downleg{tnR}{0.50}
        \node at (0.30,0.78) {$l_k$};
        \node at (1.42,0.78) {$r_k$};
        \node at (2.48,0) {$,\qquad$};
        \TN@tensordot{tnRh}{(3.20,0)}
        \TN@tensordot{tnLk}{(4.32,0)}
        \draw[tn leg] (tnRh.east) -- (tnLk.west);
        \TN@upleg{tnRh}{0.50}
        \TN@downleg{tnRh}{0.50}
        \TN@upleg{tnLk}{0.50}
        \TN@downleg{tnLk}{0.50}
        \node at (3.20,0.78) {$r_k$};
        \node at (4.32,0.78) {$l_h$};
        \node at (3.76,-0.80) {$\eta_{k,h}=r_k l_h\geq 0$};
    \end{tikzpicture}%
}
...
\newcommand{\TNMPDOSectorFactorization}{%
    \begin{tikzpicture}[tn picture]
        ...
        \TN@tensordot{sfR}{(2.45,0)}
        \draw[tn leg] (sfL.west) -- ++(-0.62,0);
        \draw[tn leg] (sfL.east) -- (sfR.west);
        ...
        \node at (2.45,0.78) {$(r_k)_{\alpha_3}$};
    \end{tikzpicture}%
}
