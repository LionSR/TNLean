...
The compression theorems of Chapter~\ref{ch:asymmetric_examples} rely on
several explicit coordinate constructions: explicit gauge transformations and
integer arithmetic certificates, adapted bases and scaled matrix units for
normality certificates, and a sign-weighted pair testing the sharpness of the
remainder bound.
...
coordinate space, one summand per slot. For explicit bond coordinates, this
map is given by an invertible matrix and a labelling of coordinates by slots.
...
The reduction of the square of the CZX tensor printed in the review
(Section~\ref{sec:asymex_czx_review}) replaces the letters by a diagonal
block of an invariant subspace, following the construction of the canonical
form in~\cite[Section~IV]{Cirac2021Matrix}. When the complementary diagonal
block vanishes, the replacement is a compression by rectangular matrices,
and the following lemma shows that it preserves every nonempty word.

\begin{lemma}[Compression onto an absorbing corner]
    \label{lem:asymex_absorbing_compression}
    \lean{Kraus.mul_evalWord_of_left_absorb,
        Kraus.evalWord_mul_of_right_absorb,
        Kraus.evalWord_compress_of_left_absorb,
        Kraus.trace_evalWord_compress_of_left_absorb,
        Kraus.evalWord_compress_of_right_absorb,
        Kraus.trace_evalWord_compress_of_right_absorb}
    \leanok
    \uses{def:mps_tensor}
    Let $B^0,\ldots,B^{d-1}\in\MN{D}$, $W\in\mathrm{M}_{k\times D}(\C)$ and
    $V\in\mathrm{M}_{D\times k}(\C)$, and write $\widetilde B^i=WB^iV$.
    If $VWB^i=B^i$ for every $i$, then every nonempty word $w$ satisfies
    $VWB^w=B^w$,
    \begin{align}
        \widetilde B^w & =WB^wV,
        \notag
    \end{align}
    and $\tr(\widetilde B^w)=\tr(B^w)$. The same conclusions hold, with
    $B^wVW=B^w$ in place of $VWB^w=B^w$, if $B^iVW=B^i$ for every $i$.
\end{lemma}

\begin{proof}
    \leanok
    For the left condition, $VWB^w=B^w$ follows from the first letter of
    $w$. By induction on the length, for $w=aw'$ with $w'$ nonempty,
    \begin{align}
        \widetilde B^a\widetilde B^{w'}
         & =WB^a(VWB^{w'})V=WB^aB^{w'}V.
        \notag
    \end{align}
    Cyclicity of the trace then gives
    $\tr(WB^wV)=\tr(VWB^w)=\tr(B^w)$. The right condition is symmetric:
    $B^wVW=B^w$ follows from the last letter, the induction step uses
    $B^aVW=B^a$, and $\tr(WB^wV)=\tr(B^wVW)=\tr(B^w)$.
\end{proof}

\section{Adapted bases and scaled matrix units}%
\label{sec:asymex_rfp_basis}

The stacked bond spaces of vertical components are tensor squares.
Reordering the four bond bits separates the two middle bits, which form
the inner bond, from the two outer bits; the intermediate
...
same target as in Theorem~\ref{thm:asym_unblocked_coefficients}. The results
below establish tensor decompositions and periodic trace identities, not a
...
The sign-weighted pair $(A,-A)$ of bond dimension two and a bond-dimension-five
source $B$ carrying both targets together with a one-dimensional zero slot
illustrate the sharpness of Theorem~\ref{thm:asym_multiblock}: the remainder
bound is attained (Theorem~\ref{thm:asymex_parity_compression}), the periodic
trace identity $\tr(B^w)=(1+(-1)^{|w|})\tr(A^w)$~\eqref{eq:asymex_parity_trace}
acts as an even-length filter (Theorem~\ref{thm:asymex_parity_trace}), and a
word-level compression need not arise from sitewise intertwiners
($B^iV=V(-A^i)\implies V=0$, Theorem~\ref{thm:asymex_parity_no_intertwiner}).
...
Refs.~\cite[Section~II]{PerezGarcia2008StringOrder} and~\cite[Appendix~A]{Cirac2021Matrix},
the state can be understood directly from its tensor letters, with an on-site
$\Z_2$ symmetry reflected on the virtual bond space:
$P=\ketbra{0}{0}$ projects onto virtual coordinate $0$, while
$X=\ketbra{0}{1}+\ketbra{1}{0}$ exchanges the two virtual coordinates.
...
\end{align}
The sum of periodic amplitudes
$\tr(A^w)+\tr((-A)^w)=(1+(-1)^{|w|})\tr(A^w)$ therefore vanishes on all odd
lengths $|w|$. The following five-dimensional tensor realizes that sum without
being a direct sum in its displayed coordinates.
