...
The sign-weighted pair $(A,-A)$ of bond dimension two and a bond-dimension-five source $B$ carrying both targets together with a one-dimensional zero slot illustrate the sharpness of Theorem~\ref{thm:asym_multiblock}: the remainder bound is attained (Theorem~\ref{thm:asymex_parity_compression}), the periodic trace identity acts as a length filter (Theorem~\ref{thm:asymex_parity_trace}), and a word-level compression need not arise from sitewise intertwiners (Theorem~\ref{thm:asymex_parity_no_intertwiner}).
...
$\Z_2$ symmetry reflected on the virtual bond space. The matrix $P$
