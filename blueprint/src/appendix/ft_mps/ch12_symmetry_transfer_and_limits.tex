...
$\E_u$ (Definition~\ref{def:twisted_transfer_map}) acts on virtual-boundary
functionals, complementing the physical endpoint correlators of
Definition~\ref{def:physical_string_order_param}.
...
\begin{proof}\leanok
    The corresponding theorem in the companion quantum-channel volume~\cite[``Unitary freedom in Kraus operators'']{QICLean2026Blueprint} applies to the
...
Because $A$ and its companion $B_u$ share the same transfer map, they admit a
common trace-preserving gauge.
...
\begin{proof}
    ...
    the corresponding theorem in the companion quantum-channel volume~\cite[``Unitary freedom in Kraus operators'']{QICLean2026Blueprint}.
\end{proof}
...
\begin{definition}[Construction of the common trace-preserving gauge]
    ...
    \lean{MPSTensor.twistedTPGaugeSetup}
    \leanok
    \uses{def:twisted_tp_gauge_setup, lem:transfer_map_twisted_companion, thm:psd_fp_unique}
...
\begin{theorem}[Twisted eigenvalues survive the common gauge]
    ...
    \leanok
    \uses{def:twisted_tp_gauge_setup, lem:twisted_transfer_eq_mixed_transfer}
...
\section{Universality of virtual-boundary nondecay}
\label{sec:app_symmetry_universality}

For injective on-site symmetric tensors with canonical normalizations,
virtual-boundary nondecay is invariant under symmetry-preserving equivalences.
