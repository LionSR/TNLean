...
\begin{proof}
    ...
    For equal bond dimensions, gauge-phase separation and normalized
    irreducible-TP overlap theory give arbitrarily long chain lengths at which
    the two periodic MPVs are not proportional. The parent-Hamiltonian
    intersection argument then makes their length-$3(L_0+1)$ ground spaces
    disjoint. Nondegeneracy of the trace pairing converts this directness into
    surjectivity of the displayed pair-evaluation map. If the bond dimensions
    differ, the dimension branch of the same direct-sum argument gives the
    pair-trace conclusion directly. The common length is independent of the
    pair, so the two cases prove the assertion for every $k\ne j$.
\end{proof}
