...
Transfer-map powers, weighted direct sums, and matrix product vectors commute
with blocking, enabling the common-length reduction of
...
\begin{theorem}[Left-canonical normalization under blocking]
    \label{thm:left_canonical_blocking}
    \lean{MPSTensor.leftCanonical_blockTensor}
    \leanok
    \uses{def:blocked_tensor}
    If a finite matrix family satisfies
    \begin{align}
        \sum_i (A^i)^\dagger A^i & =\Id,
        \notag
    \end{align}
    then every blocked family satisfies the same identity.
\end{theorem}

\begin{proof}\leanok
    Expand the blocked matrices as products indexed by words and sum one
    letter at a time.
\end{proof}

\begin{theorem}[Irreducibility from a faithful fixed point]
    \label{thm:channel_posdef_fixed_point_unique_irreducible}
    \lean{isIrreducibleMap_of_channel_posDef_fixedPoint_unique}
    \leanok

    Let $E$ be a channel with a positive-definite fixed point. If all nonzero
    positive semidefinite fixed points of $E$ are proportional, then $E$ is
    irreducible.
\end{theorem}

\begin{proof}\leanok
    A nontrivial invariant projection would produce a positive semidefinite
    fixed point not proportional to the faithful one.
\end{proof}
...
\begin{lemma}[Fixed-point criteria for irreducibility after primitive blocking]%
    ...
    \lean{Kraus.hasPrimitiveFixedPoint_of_peripheralPrimitive_of_irreducible}
...
\begin{proof}\leanok
    \uses{def:primitive_mps, thm:left_canonical_blocking, thm:channel_posdef_fixed_point_unique_irreducible, thm:transfer_channel}
...
\begin{theorem}[Blocking preserves irreducibility for primitive irreducible blocks]%
    ...
    \leanok
    \uses{def:blocked_tensor, def:irreducible_tensor, def:primitive_mps, lem:blocked_irreducibility_direct_support, thm:transfer_channel}
...
\begin{proof}\leanok
    \uses{def:primitive_mps, lem:blocked_irreducibility_direct_support, thm:transfer_channel}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The primitive fixed-point theorem supplies this complementary-power
    convergence.
...
\begin{lemma}[Positive-length blocking of a weighted block sum]%
    ...
    \uses{def:blocked_tensor, def:block_diagonal_tensor, def:same_mpv2_pos, thm:same_mpv2_pos_block_tensor, thm:block_tensor_to_tensor_from_blocks}
...
Support isometries project a blocked tensor onto its cyclic corners, embedding
multiple cyclic-sector families into a single common blocked alphabet.
...
\begin{theorem}[Supported corner compression with a support isometry]%
    ...
    \begin{proof}\leanok
        ...
        Apply Theorem~\cite[``Compression of a supported finite Kraus family'']{QICLean2026Blueprint} to the
...
\begin{theorem}[Cyclic sectors after removing zero blocks]%
    ...
    \leanok
    \uses{thm:tp_gauge_arbitrary, def:same_mpv2_pos, def:primitive_irreducible_cyclic_sectors, thm:cyclic_sector_decomp_irr_tp_prim_irr}
...
\begin{proof}\leanok
    \uses{thm:tp_gauge_arbitrary, thm:cyclic_sector_decomp_irr_tp_prim_irr}
...
\begin{lemma}[Reindexed nonzero-part identity]%
    ...
    \uses{thm:common_blocked_cyclic_sector_reindexed_samempv, thm:common_blocked_cyclic_sector_block_tensor_common_reindexed}
...
For families of irreducible, trace-preserving, primitive blocks with non-zero
weights and positive bond dimensions, the asymptotic overlap condition
$O_{A_kA_k}(N)\to1$ is a derived consequence, and ordering the blocks by
non-increasing weight modulus produces a normal canonical form.
...
\begin{proof}\leanok
    \uses{def:is_normal_canonical_form, thm:primitive_overlap}
...
\begin{remark}[Scope of Theorem~\ref{thm:exists_normal_canonical_form}]
    Theorem~\ref{thm:exists_normal_canonical_form} reorganizes an already
    reduced family of primitive, irreducible blocks into normal canonical form
    by sorting their weight moduli. The reduction from an arbitrary MPS tensor,
    including the structural bond-dimension bound, is established by
    Theorem~\ref{thm:tp_primitive_blockdecomp} in
    Section~\ref{sec:reduction_to_normal_form} following~\cite[Section~2.3]{Cirac2016MPDO_arXiv}.
\end{remark}
