...
Reducing orthogonal projections yield support isometries that compress tensors to
corner blocks, preserving matrix product vectors across direct sums (supporting
Theorems~\ref{thm:projector_closure_irreducible_corners} and
\ref{thm:projector_closure_canonical_form}).
...
companion quantum-channel volume~\cite[``Rectangular Gram equality gives a unitary'']{QICLean2026Blueprint}.
...
\begin{lemma}[Canonical hypotheses descend to dependent direct summands]
    ...
    \leanok
    \uses{lem:matrix_sigma_block_inclusion, def:invariant_projector_closure, def:no_periodic_vectors}
...
Positive Perron eigenpairs determine the spectral normalization and unital
gauge transformations of irreducible blocks.
...
\begin{proof}
    ...
    for positive maps (Theorem~\cite[``Positive-eigenvalue specialization'']{QICLean2026Blueprint}) gives a
...
\begin{proof}
    ...
    Theorem~\cite[``Spectral radius is the Perron eigenvalue'']{QICLean2026Blueprint} identifies its spectral radius
...
\begin{theorem}[Irreducibility under rescaled conjugation]
    ...
    \leanok
    \uses{def:irreducible_tensor, lem:support_proj_range_factorization}
...
\begin{theorem}[Unital Schwarz data for an irreducible corner]
    ...
    \leanok
    \uses{def:irreducible_tensor, def:transfer_map, thm:unital_gauge_of_pd_fixed_point, thm:irreducible_tensor_smul_conj}
...
\begin{proof}
    \leanok
    \uses{thm:unital_gauge_of_pd_fixed_point, thm:irreducible_tensor_smul_conj, thm:transfer_psd_fp_pd_irred}
