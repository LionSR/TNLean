...
\begin{theorem}[Self-transfer]\label{thm:mixed_self}
    \lean{Kraus.mixedTransferMap_self}
    \leanok
...
\begin{lemma}[Iterated mixed transfer]\label{thm:mixed_pow}
    \lean{Kraus.mixedTransferMap_pow_apply}
    \leanok
...
Theorem~\ref{thm:overlap_trace} and Theorem~\ref{thm:overlap_trace_rect}
combine Lemma~\ref{thm:mixed_pow} and Lemma~\cite[``Trace expansion over matrix units'']{QICLean2026Blueprint} the
same way, over the square and rectangular matrix-unit bases respectively:
expanding $\Tr(T)$ by the trace-expansion formula
...
the corresponding definition in the companion quantum-channel volume~\cite[``Fixed-point projection'']{QICLean2026Blueprint} (Chapter~\ref{ch:channels}),
\begin{align}
    P(X)
     & =\frac{\tr(X)}{\tr(\rho)}\rho,
    \notag
...
(the corresponding theorem in the companion quantum-channel volume~\cite[``Power decomposition'']{QICLean2026Blueprint}, Chapter~\ref{ch:channels}) gives
