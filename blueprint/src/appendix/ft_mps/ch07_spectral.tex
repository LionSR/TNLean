...
The spectral theory of MPS transfer maps in Chapter~\ref{ch:spectral} relies on
several structural steps: algebraic mixed-transfer identities, peripheral
intertwiners for gauge rigidity, Banach-algebra decay bounds for overlaps, the
rank-one Perron projection for primitive channels, and cyclic-sector
decompositions.

\section{Mixed-transfer powers and overlap traces}
\label{sec:app_spectral_mixed_trace}

The mixed transfer operator $F_{AB}$ of Definition~\ref{def:mixed_transfer}
and its rectangular form $F_{AB}^{\mathrm{rect}}$ of
Definition~\ref{def:mixed_transfer_rect} satisfy exact word expansions and trace
identities.
...
Transporting a modulus-one mixed-transfer eigenvector through separate unital
gauges produces Kraus-level intertwiners that force equal bond dimension once
invertible, establishing the gauge rigidity of
Theorem~\ref{thm:modulus_one_gauge_irr_tp} and
Theorem~\ref{thm:transfer_operator_gap_rect_irr_tp}.
...
The spectral-radius gap $\rho_{\spec}(F_{AB}) < 1$ translates into
operator-norm decay and asymptotic orthogonality of matrix product vectors
through standard Banach-algebra convergence.
...
Let $E:\MN{D}\to\MN{D}$ be a trace-preserving linear map with a fixed point
$\rho\neq0$, $\tr(\rho)\neq0$, underlying the primitive overlap convergence in
Theorem~\ref{thm:primitive_overlap}. Write
...
The cyclic decomposition for irreducible finite Kraus maps proceeds by
normalizing a peripheral eigenvector to a unitary, forming its discrete Fourier
spectral projections, and tracking the cyclic shift of the resulting corners
under the channel.
