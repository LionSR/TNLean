...
Linear-algebraic change of basis, geometric power-sum extrapolation, and sector
bookkeeping provide the tools for comparing alternative BNT decompositions. A
sector has copy weights $\mu_{j,q}\ne0$, coefficient
...
\begin{proof}\leanok\uses{lem:bnt_finite_index_overlap_orthonormal_li, lem:bnt_invertible_change_basis}
    By Lemma~\ref{lem:bnt_finite_index_overlap_orthonormal_li}, both families are
...
\begin{proof}
    ...
    By the corresponding theorem in the companion quantum-channel volume~\cite[``Newton--Girard trace recursion'']{QICLean2026Blueprint}, the characteristic
...
\begin{lemma}[Sector-decomposition coefficient is not eventually zero]
    ...
    \lean{MPSTensor.SectorDecomposition.coeff_mul_not_eventually_zero}
    \leanok
    \uses{def:sector_weight_data, lem:finite_geometric_sum_eventual_vanishing}
    For any sector decomposition $P$ and any sector $j$, the coefficient
    $c_{P,j}^{(N)}=\sum_q\mu_{j,q}^N$ is not eventually zero in $N$.  More
    generally, for every positive integer $p$, the subsequence
    $c_{P,j}^{(pN)}$ is not eventually zero.
\end{lemma}
...
\begin{proof}\leanok
    ...
    impossible.  For the subsequence, apply the same argument to the nonzero
    bases $\mu_{j,q}^p$.
\end{proof}
...
The equal-MPV matching theorems in Chapter~\ref{ch:bnt} specialize an eventual
proportionality argument; matching in both directions produces a bijection
between the sector sets.

\begin{theorem}[Exact sector matching under eventual proportionality]
    \label{thm:sector_bnt_proportional_exact_match}
    \lean{MPSTensor.exists_block_match_exact_of_eventuallyProportional}
    \leanok
    \uses{def:sector_bnt_cf, lem:sector_bnt_combined_family_eventually_li, lem:sector_decomposition_coeff_not_eventually_zero}
...
\begin{proof}\leanok
    \uses{lem:sector_bnt_combined_family_eventually_li, lem:sector_decomposition_coeff_not_eventually_zero, cor:sector_bnt_cf_basis_normal, thm:cpsv_normal_overlap_dichotomy, thm:gauge_phase_mpv_scaling}
    Suppose the chosen sector decays against every sector of $Q$, and separate
    all such $P$-sectors from the remaining ones. For each remaining sector,
    choose a $Q$-sector with non-decaying overlap. Both blocks are normal by the
    canonical-form hypotheses. Theorem~\ref{thm:cpsv_normal_overlap_dichotomy}
    rules out its decay alternative and supplies equal bond dimensions and a
    gauge-phase relation with a unit phase. Gauge-phase covariance therefore
    expresses that $P$-sector MPV as a geometric scalar multiple of the chosen
    $Q$-sector MPV.

    Substitute these relations into the eventual proportionality identity.
    The combined-family Gram criterion makes the decaying residue together
    with the $Q$ family eventually linearly independent. Exact coefficient
    extraction then makes the chosen sector coefficient eventually zero,
    contradicting its power-sum nonvanishing. Thus a non-decaying partner
    exists. A second application of
    Theorem~\ref{thm:cpsv_normal_overlap_dichotomy} rules out decay and gives
    the required dimension equality and gauge-phase equivalence; no
    modulus-one overlap limit is assumed.
\end{proof}

\begin{theorem}[Full-basis matching under eventual proportionality]
    \label{thm:sector_bnt_proportional_full_basis_match}
...
\begin{proof}\leanok
    ...
    Theorem~\ref{thm:sector_bnt_proportional_exact_match} to each sector of
