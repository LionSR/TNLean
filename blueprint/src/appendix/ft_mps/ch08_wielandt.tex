...
The quantitative Wielandt argument of Chapter~\ref{ch:wielandt} rests on
structural properties of exact-word spans, cumulative spans, one-step
augmentations, blocking, and complementary transfer-map gaps.

\section{Exact-word and block-injectivity support}
\label{sec:app_wielandt_exact_word_support}

Exact-word products satisfy direct concatenation factorizations and dimension
bounds in low dimensions.

\begin{lemma}[Word span products]\label{lem:word_span_mul}
    \lean{Kraus.wordSpan_mul_le}
    \leanok
    \uses{def:word_span}
...
\begin{proof}\leanok
    ...
    Lemma~\cite[``Two-sided span from one nonzero matrix'']{QICLean2026Blueprint}, with $R$ and $S$ chosen
...
Cumulative word spans form an ascending chain of subspaces whose stabilization
is governed by finite dimensionality and Burnside's theorem.
...
\begin{remark}[Burnside input for cumulative spanning]
    \uses{thm:irreducible_tensor_to_action, thm:burnside_matrix, lem:algspan_cumulative_span}
...
\begin{lemma}[Irreducible action gives an irreducible tensor]
    ...
    \lean{Kraus.isIrreducibleFamily_of_isIrreducibleAction}
    \leanok
...
\begin{lemma}[The algebra span reaches a finite cumulative span]
    ...
    \lean{Kraus.exists_cumulativeSpan_eq_top_of_algSpan_eq_top}
    \leanok
...
\begin{theorem}[Nonzero trace product]\label{thm:nonzero_trace_word}
    \lean{Kraus.exists_nonzero_trace_word}
    \leanok
    \uses{def:normal}
...
\begin{theorem}[Eigenvalue extraction]\label{thm:eigenvalue_extraction}
    \lean{Kraus.exists_word_eigenvector}
    \leanok
    \uses{def:normal}
...
\begin{proof}
    \leanok
    \uses{thm:nonzero_trace_word}
    By Theorem~\ref{thm:nonzero_trace_word}, there is a word $w_0$ with
    $\tr(A^{w_0})\neq0$ and $|w_0|\le D^2$.
    By Theorem~\cite[``Nonzero trace implies eigenvector'']{QICLean2026Blueprint}, $A^{w_0}$ has a nonzero
    eigenvector.
\end{proof}

\begin{theorem}[Vector span stabilisation]\label{thm:vector_span_stable}
    \lean{Kraus.cumulativeVectorSpan_stable}
    \leanok
...
\begin{lemma}[Vector span from matrix span]\label{lem:vector_span_from_matrix}
    \lean{Kraus.cumulativeVectorSpan_eq_top_of_cumulativeSpan_eq_top}
    \leanok
...
Augmenting a Kraus family by a matrix already in its one-step span preserves all
exact word spans while allowing the adjunction of an invertible generator or an
eigenvector.

\begin{definition}[One-step augmentation]\label{def:one_step_augment}
    \lean{Kraus.oneStepAugment}
    \leanok
    \uses{def:mps_tensor}
...
\begin{lemma}[Kraus operators lie in the one-step span]\label{lem:kraus_mem_word_span_one}
    \lean{Kraus.apply_mem_wordSpan_one}
    \leanok
    \uses{def:word_span}
...
\begin{lemma}[Redundant one-step augmentation]\label{lem:one_step_augment_word_span}
    \lean{Kraus.oneStepAugment_apply_mem_wordSpan_one,
        Kraus.wordSpan_oneStepAugment_one,
        Kraus.wordSpan_oneStepAugment_eq}
    \leanok
...
\begin{corollary}[Wielandt case (2), invertible Kraus operator]%
    ...
    \leanok
    \uses{lem:kraus_mem_word_span_one, thm:wielandt_case2_one_step_span, cor:wielandt_case2_one_step_index}
...
\begin{corollary}[Wielandt case (3), non-invertible Kraus operator]%
    ...
    \leanok
    \uses{lem:kraus_mem_word_span_one, thm:wielandt_case3_one_step_span, cor:wielandt_case3_one_step_index}
    Let $A$ be an MPS tensor with bond dimension $D\ge1$, satisfying
    $\sum_i(A^i)^\dagger A^i=\Id$, and suppose that $A$ is primitive in the
    sense of Definition~\ref{def:paper_primitive}.
...
Blocking preserves tensor normality and embeds exact word spans, allowing word
eigenvectors to be transferred to single-index eigenvectors of blocked tensors.
...
\begin{proof}
    \leanok
    \uses{thm:isNormal_blockTensor, thm:blockTensor_single_eigenvector, thm:eigenvector_spreading, lem:blocking_transfer, thm:lemma2b_assembly}
...
\begin{proof}
    ...
    By Theorem~\cite[``Nonzero trace implies eigenvector'']{QICLean2026Blueprint},
...
A strict spectral-radius bound on the complementary transfer map implies
overlap convergence, non-vanishing of iterated transfer maps, and fixed-point
uniqueness.
...
\begin{theorem}[Peripheral primitivity and irreducibility give normalized overlap]
    ...
    \leanok
    \uses{def:irreducible_tensor, def:mpv_overlap}
...
\begin{lemma}[Fixed-point uniqueness from a complementary transfer-map gap]%
    ...
    \lean{Kraus.HasComplementaryFixedPointGap.fixedPoint_unique}
    \leanok
...
\begin{lemma}[Complement powers tend to zero]%
    ...
    \lean{Kraus.HasComplementaryFixedPointGap.complement_pow_tendsto_zero}
    \leanok
    \uses{def:primitive_mps}
...
\begin{lemma}[Primitive tensors with positive-definite fixed point have irreducible transfer map]%
    ...
    \lean{Kraus.HasComplementaryFixedPointGap.isIrreducibleMap_of_posDef}
    \leanok
    \uses{def:primitive_mps}
...
When the identity matrix lies in the one-step span $S_1(A)$, exact word spans
grow monotonically and coincide with cumulative spans.
...
\begin{remark}[Identity-in-the-one-step-span condition]
    ...
    matrix lies in the span of the Kraus operators $\{A^i\}_{i=0}^{d-1}$. While
    tensor normalization $\sum_i(A^i)^\dagger A^i=\Id$ is quadratic, the
    one-step condition $\Id\in S_1(A)$ is linear in the Kraus operators.
...
\begin{theorem}[Word-span monotonicity from one-step padding]%
    ...
    \lean{Kraus.wordSpan_mono_succ_of_one_mem_wordSpan_one}
    \leanok
    \uses{def:word_span}
...
\begin{theorem}[Cumulative span equals word span under one-step padding]%
    ...
    \lean{Kraus.cumulativeSpan_eq_wordSpan_of_one_mem_wordSpan_one}
    \leanok
