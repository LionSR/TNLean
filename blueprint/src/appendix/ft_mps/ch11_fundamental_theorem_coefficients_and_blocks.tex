...
Sector pairings in both directions determine a finite bijection between BNT
canonical forms, and normalization forces every intertwining gauge scalar to be
a unit phase.
...
\begin{theorem}[Bijective matching under eventual proportionality]
    ...
    \leanok
    \uses{def:sector_bnt_cf, thm:sector_bnt_proportional_full_basis_match, lem:sector_bnt_bijection_from_matches}
...
Gauge-phase equivalences select unit phases for each sector, allowing eventual
coefficient identification from the linear independence of the BNT basis.
...
\begin{proof}\leanok
    \uses{lem:sector_bnt_norm_phase_of_matched_mpv, lem:sector_bnt_coeff_identity_via_matched_mpv_phase}
...
For proportional total MPVs, coefficient extraction carries a global
length-dependent scalar $c_N$ relating the two MPV families across all sectors.
...
Block-diagonal gauge conjugations assemble into a global similarity
transformation on the direct-sum tensor (Section~\ref{sec:ft_equal_global_gauge}).
...
\begin{theorem}[Global gauge from a copy-weight matching]%
    ...
    \leanok
    \uses{def:sector_bnt_copy_weight_matching, thm:sector_bnt_global_gauge_of_matched_weights}
    ...
    in terms of copy-weight matchings.
\end{theorem}
...
When copy weights or coefficients satisfy explicit matching relations beyond
proportionality, global direct-sum gauges can be constructed directly (see
Remark~\ref{rem:proportional_length_scalar_gap} regarding the general
...
\begin{theorem}[Additional proportional global gauge under copy-weight matching]%
    ...
    \leanok
    \uses{def:sector_bnt_copy_weight_matching, thm:sector_bnt_proportional_sector_match_witnesses, thm:sector_bnt_global_gauge_of_copy_weight_matching}
...
\begin{theorem}[Additional proportional global gauge under coefficient identities]%
    ...
    \leanok
    \uses{def:sector_bnt_copy_weight_matching_of_coeff_identity, thm:sector_bnt_proportional_global_gauge_of_copy_weight_matching}
    Let $P$ and $Q$ be BNT canonical forms whose total tensors generate
    eventually nonzero proportional MPV families.
