...
\begin{align}
    ...
    \widetilde P_{k,q}
     & =P_{\beta(k)},
    \label{eq:matched_p_weight_tensor}
\end{align}
...
\begin{definition}[Matched weights, tensors, and repeated gauges]
    ...
    $\widetilde\mu$, $\widetilde P$, and $(k,q)\mapsto X_k$ defined in
    (\ref{eq:matched_p_weight_tensor}) on the flattened copy coordinates of $Q$.
\end{definition}
...
\begin{theorem}[Equal-MPV gauge with all matched witnesses]
    ...
    \leanok
    \uses{thm:sector_bnt_equal_global_gauge, lem:ft_total_dim_eq_of_match, def:ft_matched_flat_data}
...
A permutation matrix relates the matched-coordinate ordering to the literal
block ordering of $P$ in Theorem~\ref{thm:sector_bnt_equal_mps_gaugeEquiv_literal}.
...
\begin{lemma}[Permutation conjugation reindexes both coordinates]
    \label{lem:ft_perm_matrix_conj_submatrix}
    \lean{MPSTensor.permMatrix_conj_eq_submatrix}
    \leanok
    For a permutation $\rho$ and a square matrix $M$, conjugation by the
    permutation matrix reindexes both coordinates by $\rho$:
    \begin{align}
        \Pi_\rho M\Pi_{\rho^{-1}}
         & =\bigl(M_{\rho(i),\rho(j)}\bigr)_{i,j}.
...
\begin{proof}
    \leanok
    Left multiplication by $\Pi_\rho$ selects row $\rho(i)$ in row $i$, and
    right multiplication by $\Pi_{\rho^{-1}}$ selects column $\rho(j)$ in
    column $j$. The two selections give the displayed submatrix.
\end{proof}

The coordinate equivalence of Definition~\ref{def:ft_sector_flat_equiv} induces
the permutation $\rho$ on the total bond space. Applying
(\ref{eq:app_ft_perm_conjugation}) to $P_{\mathrm{tot}}^i$ produces the
matched-coordinate tensor, and composing $G_\rho$ with the block-diagonal gauge
yields the literal gauge transformation of
...
Between left-canonical irreducible sectors, any gauge-phase equivalence can be
refined to a unitary gauge with the same phase, extending across all copies in a
sector.
...
\begin{lemma}[Unitary gauge for left-canonical irreducible tensors]
    ...
    \leanok
    \uses{def:left_canonical_tensor, def:gauge_phase_equiv, def:irreducible_tensor, lem:left_canonical_irreducible_same_phase_unitary_gauge}
...
\begin{lemma}[Unitary gauge in matched sector coordinates]
    ...
    \leanok
    \uses{lem:sector_bnt_equal_matched_copy_weight_witnesses, lem:left_canonical_irreducible_same_phase_unitary_gauge, thm:sector_bnt_global_gauge_of_copy_weight_matching, lem:unitary_global_gauge_of_blocks}
    ...
    reordered block tensor, then
    $Q_{\mathrm{flat}}^i=XP_{\mathrm{match}}^iX^{-1}$.
\end{lemma}
\begin{proof}\leanok
    \uses{lem:sector_bnt_equal_matched_copy_weight_witnesses, lem:left_canonical_irreducible_same_phase_unitary_gauge, lem:unitary_global_gauge_of_blocks}
