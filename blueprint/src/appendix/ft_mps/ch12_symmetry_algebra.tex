Let $I=\{0,\ldots,d-1\}$ be the physical index set, $\MN{D}$ the bond algebra,
and $G$ a group. All sums below are finite. We treat permutation symmetries on
$I$, gauge scalar uniqueness for injective tensors, and factor-system cocycle
relations.
...
\begin{definition}[Physical-index rotation]%
    ...
    \lean{MPSTensor.rotatePhysical, MPSTensor.rotatePhysical_apply}
    \leanok
    \uses{def:mps_tensor}
    Given a matrix $M \in \C^{m\times d}$ and an MPS tensor~$A$ with
    physical dimension~$d$, the \emph{physical-index rotation} is the tensor
    with physical dimension~$m$ given by
    $(A_M)^i := \sum_j M_{ij}\, A^j$ for $i=0,\ldots,m-1$.
...
\begin{lemma}[Matrix product vector under physical rotation]
    ...
    For $M\in\C^{m\times d}$ and a length-$N$ physical configuration
    $s=(s_0,\ldots,s_{N-1})\in\{0,\ldots,m-1\}^N$, physical rotation
    satisfies
    \begin{align}
...
\begin{lemma}[Blocking commutes with physical maps]
    \label{lem:block_tensor_rotate_physical}
    \lean{MPSTensor.blockTensor_rotatePhysical}
    \leanok
    \uses{def:rotate_physical, def:blocked_tensor}
    For $W\in\C^{m\times n}$, a tensor~$C$ with physical dimension~$n$, and
    $L\ge0$, blocking the rotated tensor over $L$ sites is rotating the
    blocked tensor by the $L$-fold Kronecker power of $W$:
    \begin{align}
        (C_W)^{[L]} & =\bigl(C^{[L]}\bigr)_{W^{\otimes L}}.
        \label{eq:app_block_tensor_rotate_physical}
    \end{align}
\end{lemma}

\begin{proof}\leanok
    Decode a blocked physical index as a word $b$ of length~$L$.  Substituting
    $(C_W)^{b_k}=\sum_{v_k}W_{b_kv_k}C^{v_k}$ into the ordered product and
    distributing gives one summand $\bigl(\prod_kW_{b_kv_k}\bigr)
    C^{v_0}\cdots C^{v_{L-1}}$ for each word~$v$.  Reindexing the sum over
    words by the blocked index gives the right-hand side of
    (\ref{eq:app_block_tensor_rotate_physical}), whose coefficients are the
    entries of $W^{\otimes L}$.
\end{proof}

\section{Gauge ratios and scalar uniqueness}
\label{sec:app_symmetry_gauge_ratios}

For injective tensors, any two similarity transformations yielding the same
target tensor differ by a scalar multiple.
...
\begin{theorem}[Projective multiplication law for gauge matrices]%
    ...
    \leanok
    \uses{lem:gauge_ratio_isScalar, def:injective, def:on_site_symmetric, lem:twisted_tensor_mul}
    ...
    Invertibility of both sides forces $c(g,h) \in \C^{\times}$.
    Diagrammatically, both candidate gauges realize the twisted tensor
    $\widetilde{A}_{gh}^i$:
    \begin{center}
        \tenkzkernel
