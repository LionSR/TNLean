...
overlap. Asymptotic Gram-matrix criteria govern the linear independence of
matrix product vector families in the BNT characterization and matching
arguments.
...
\begin{proof}\leanok
    Irreducibility follows from Lemma~\ref{lem:bond_dimension_one_irreducible}. The identity transfer map
...
\begin{lemma}[Perron gauges preserve primitivity]
    ...
    \lean{Kraus.isPrimitive_mapLM_tpGauge_iff}
    \leanok
...
\begin{corollary}[Positive-length geometric proportionality phase]
    ...
    \leanok
    \uses{def:nt_cpsv, def:mpv_state, def:nonzero_proportional_mpv}
...
\begin{proof}\leanok
    \uses{lem:nonzero_proportional_mpv_basic, thm:normal_proportional_mpv_geometric_phase}
...
Stating the threshold explicitly yields an existential formulation.
...
\begin{theorem}[Existential BNT independence from overlap limits]%
    ...
    \leanok
    \uses{lem:bnt_finite_index_overlap_orthonormal_li}
...
\begin{proof}\leanok
    \uses{lem:bnt_finite_index_overlap_orthonormal_li}
    This is Lemma~\ref{lem:bnt_finite_index_overlap_orthonormal_li}, restated with an
...
Eventual linear independence converts equality of linear combinations of MPV
states into equality of their scalar coefficients, supplying the identification
used in Chapter~\ref{ch:ft_proof}.
