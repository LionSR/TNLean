...
\begin{proof}
    \leanok
    \uses{lem:starSubalgebra_sameIsotype_iso}
    An isomorphism of $W$ onto $W'$ has an inverse, obtained by projecting onto $W'$,
    inverting on the subspace, and including back into $W$. This inverse is an intertwiner
    from $W'$ into $W$ that is injective on $W'$ with image $W$, so the two pieces are of the
    same type in the reverse order.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{lem:starSubalgebra_sameIsotype_iso}
    Composing an isomorphism of $W$ onto $W'$ with an isomorphism of $W'$ onto $W''$ gives an
    intertwiner from $W$ into $W''$ that is injective on $W$ with image $W''$, which is
    nonzero on $W$ because $W''$ is nonzero.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{lem:starSubalgebra_orthogonal_reducibility}
    Decompose $x$ as the sum of its projection onto $W$ and its orthogonal part. Each member
    of $S$ sends the projection into $W$, since $W$ is invariant, and sends the orthogonal
    part into the orthogonal complement, which is invariant by orthogonal reducibility
    (Lemma~\ref{lem:starSubalgebra_orthogonal_reducibility}). Projecting the image onto $W$
    therefore recovers the member applied to the projection of $x$.
\end{proof}
...
\begin{proof}
    ...
    not of the same type, the projection vanishes on $W$, that is, $W$ lies in the orthogonal
    complement of $W'$.
\end{proof}
