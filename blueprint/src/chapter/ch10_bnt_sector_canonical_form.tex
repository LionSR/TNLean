...
\begin{theorem}[Exact reindexing of heterogeneous weighted block sums]%
    ...
    $D_k=D'_{e(k)}$. If, after this dimension identification,
    \begin{align}
        \mu_kA_k^i
        &=\Pi_k\left(\mu'_{e(k)}B_{e(k)}^i\right)\Pi_k^{-1}
        \label{eq:heterogeneous_block_local}
    \end{align}
    for every block and physical index, where $\Pi_k$ is the corresponding
    coordinate identification, then the induced equivalence $E$ of flattened
    dependent block coordinates has a permutation matrix $\Pi_E$. For every
    physical index $i$,
    \begin{align}
        \bigoplus_{k=1}^{r}\mu_kA_k^i
        &=\Pi_E\left(\bigoplus_{\ell=1}^{r'}
          \mu'_{\ell}B_{\ell}^i\right)\Pi_E^{-1}.
...
\begin{theorem}[Exact BNT reconstruction of prepared normal blocks]%
    ...
    bond coordinates, and $X\in\GL_{D_{\mathrm{tot}}(P)}(\C)$. The first
    ...
    \begin{align}
        ...
        &=\Pi_E\left(XP^iX^{-1}\right)\Pi_E^{-1}.
