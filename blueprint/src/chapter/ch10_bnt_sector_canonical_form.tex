...
\begin{proof}
    ...
    Positive blocking preserves normality of each representative. At every
    blocked length $N$, concatenation identifies the coefficients of
    $V^{(N)}(A^{[p]})$ and $V^{(N)}(A_j^{[p]})$ with those of
    $V^{(Np)}(A)$ and $V^{(Np)}(A_j)$, respectively. The spanning
    coefficients at length $Np$ therefore give those at blocked length $N$,
    and the same configuration-space identification preserves eventual
    linear independence.
\end{proof}
