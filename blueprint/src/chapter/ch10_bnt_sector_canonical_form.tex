...
\begin{theorem}[Active representative sector decomposition]
    ...
    \uses{thm:cpsv_canonical_form_active_bnt_refinement,
        thm:cpsv_bnt_blocked_simultaneous_span, def:sector_weight_data}
    In the notation of
    ...
    $N$, the two tensors generate the same matrix product vector:
    \begin{align}
        V^{(N)}(T_{\mathrm{grp}})
        &=V^{(N)}(P_{\mathrm{act}}).
...
\begin{proof}\leanok
    \uses{thm:cpsv_canonical_form_active_bnt_refinement,
        thm:cpsv_bnt_blocked_simultaneous_span, lem:paper_decomp_formula}
...
\begin{theorem}[Gauge realization of prepared phase-equivalent NTs]%
    ...
    bond dimensions $D_X$ and $D_Y$. Fix $\zeta\in\C\setminus\{0\}$ and suppose
    that, for every positive length $N$ and every physical word $\sigma$,
    \begin{align}
        V^{(N)}(Y)_\sigma
        &=\zeta^N V^{(N)}(X)_\sigma.
        \label{eq:prepared_phase_equiv_fixed_scalar}
    \end{align}
    Then $D_X=D_Y$, and after identifying the bond spaces there exists
    $G\in\GL_{D_Y}(\C)$ such that, for every physical index $i$,
    \begin{align}
        Y^i&=\zeta G X^i G^{-1}.
        \label{eq:prepared_phase_equiv_fixed_scalar_gauge}
    \end{align}
    The fixed scalar $\zeta$ is not asserted to be positive, and the gauge is
    not asserted to be unitary.
\end{theorem}
...
\begin{proof}\leanok
    ...
    The gauge-phase theorem gives an invertible $G$ and a nonzero scalar $\eta$
    such that $Y^i=\eta G X^iG^{-1}$. Hence
    $V^{(N)}(Y)_\sigma=\eta^N V^{(N)}(X)_\sigma$ for every $N$ and $\sigma$.
    The normalized self-overlaps tend to one, so there are consecutive positive
    lengths $M$ and $M+1$ with words for which the corresponding coefficients of
    $V^{(M)}(X)$ and $V^{(M+1)}(X)$ are nonzero. Comparing these identities with
    (\ref{eq:prepared_phase_equiv_fixed_scalar}) gives
    $\zeta^M=\eta^M$ and $\zeta^{M+1}=\eta^{M+1}$. Cancelling the nonzero
    $M$-th power yields $\zeta=\eta$, which proves
...
\begin{theorem}[Exact BNT reconstruction of prepared normal blocks]%
    ...
    \leanok
    \uses{thm:paperbnt_collapsed_prepared,
        def:block_diagonal_tensor, def:sector_weight_data}
    Under the hypotheses of
...
\begin{proof}\leanok
    \uses{def:mpv_phase_class_data,
        thm:paperbnt_collapsed_prepared,
        lem:prepared_collapsed_bnt_sector_decomp_total_dim,
        thm:prepared_phase_equiv_gauge,
        lem:tensor_from_blocks_globalGauge_conj,
        thm:heterogeneous_block_sum_reindex,
        def:global_gauge_of_blocks}
...
\begin{proof}\leanok
    \uses{thm:cpsv_exact_active_reconstruction,
        thm:cpsv_normal_tp_gauge,
        lem:gauge_equiv_from_block_gauge,
        thm:paperbnt_supplier_prepared_exact}
