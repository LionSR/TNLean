...
\begin{definition}[Active BNT sector presentation]
    ...
    sector decomposition $P$ such that every copy has positive multiplicity
    and nonzero weight, every representative $C_j$ is normal, the
    representative matrix product vectors are eventually linearly
    independent, and
    \begin{align}
        V^{(N)}(A)&=V^{(N)}(P) &&(N>0).
        \label{eq:cpsv_active_bnt_sector_presentation}
    \end{align}
    This is the active form of the canonical normalization at
    \cite[lines~217--246]{Cirac2016MPDO_arXiv}: dormant listed candidates are
    not members of the basis, whereas repeated nonzero copies and their
...
\begin{proof}\leanok
    Proposition~2.7 matches every representative of either presentation with
    one in the other. Eventual linear independence makes both matching maps
    injective. Since the two index sets are finite, they have equal cardinality,
    and either matching map is a bijection. Normal-tensor gauge rigidity gives
    the asserted gauge-phase relation for each matched pair.
\end{proof}
...
\begin{proof}\leanok
    \uses{thm:prepared_phase_equiv_gauge,
        thm:peripheral_primitive_irreducible_overlap_one,
...
\begin{proof}\leanok
    \uses{def:mpv_phase_class_data,
        ...
        lem:prepared_phase_equiv_fixed_scalar_gauge,
