...
The basis-of-normal-tensors canonical form of
\cite[Section~2]{Cirac2016MPDO_arXiv} is defined on a sector decomposition with
...
\begin{lemma}[Sector decomposition formula]%
    ...
    \begin{align}
        V^{(N)}(A_{\mathrm{tot}})_\sigma
         & =
        \sum_{j=1}^{g} c_N^{(j)} V^{(N)}(A_j)_\sigma.
...
\begin{theorem}[BNT existence for CPSV canonical form]
    ...
    family is obtained by choosing one representative from each
    matrix-product-vector phase class of the blocks $A_1,\ldots,A_r$.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:cpsv_canonical_form_bnt_characterization, def:mpv_phase_class_data, thm:cpsv_normal_mpv_phase_gauge}
    Partition the finite block family by matrix-product-vector phase
    equivalence. Choose one representative from each class. The
    representatives are normal and have positive bond dimensions. Distinct
    representatives are not gauge-phase equivalent. For every block, the
    normal-tensor phase theorem identifies its bond dimension with that of
    its representative and realizes their phase relation by a similarity and
    a unit-modulus scalar.
...
\begin{theorem}[Structured BNT refinement of CPSV canonical form]
    \label{thm:cpsv_canonical_form_bnt_refinement}
    ...
    \lean{MPSTensor.CPSVCanonicalFormData.BNTRefinement.copy}
    \lean{MPSTensor.CPSVCanonicalFormData.classCopyEquiv,
    ...
    \lean{MPSTensor.CPSVCanonicalFormData.BNTRefinement.groupedWeight,
    ...
    \end{align}
    be literal CPSV canonical-form data. There are phase classes $j$, copy
    ...
    \end{align}
    Each class has a representative $C_j$. If the copy
    $(j,q)$ corresponds to the block index $k$, then its bond dimension
    ...
    identification. Define the grouped tensor by
    \begin{align}
        T_{\mathrm{grp}}^i
    ...
    \end{align}
    If $G=\bigoplus_kX_k$ is the block-diagonal gauge, then, letter by letter,
    \begin{align}
...
\begin{proof}\leanok
    ...
    Enumerate the block indices by their phase classes and choose one normal
    ...
    that the representatives form a basis of normal tensors. Reindexing the
    dependent block coordinates gives $\Phi$ and
...
% lines 1135--1148.
\begin{definition}[BNT sector presentation]
    \label{def:cpsv_bnt_sector_presentation}
    \lean{MPSTensor.IsBNTSectorPresentation}
    \lean{MPSTensor.IsBNTSectorPresentation.%
        isCPSVBasisOfNormalTensors}
    \lean{MPSTensor.IsBNTSectorPresentation.%
        blocks_not_gaugePhaseEquiv}
    \lean{MPSTensor.IsBNTSectorPresentation.of_sameMPV₂Pos}
    \lean{MPSTensor.IsBNTSectorPresentation.%
        exists_pos_mpvState_ne_zero}
    \leanok
    \uses{def:sector_weight_data, def:nt_cpsv, def:same_mpv2_pos}
    A \emph{BNT sector presentation} of a tensor $A$ is a nonempty
    ...
    \cite[lines~217--246]{Cirac2016MPDO_arXiv}: repeated copies and their
    weights remain explicit.
\end{definition}

\begin{theorem}[Uniqueness of BNT presentations]
    \label{thm:cpsv_bnt_presentation_uniqueness}
    \lean{MPSTensor.IsBNTSectorPresentation.%
        equiv_of_sameMPV₂Pos}
    \leanok
    \uses{def:cpsv_bnt_sector_presentation, def:same_mpv2_pos}
    Two BNT presentations of the same positive-length matrix product
...
\begin{proof}\leanok
    ...
    direct-sum tensor of the first presentation, using the
...
% lines 1835--1858.
\begin{theorem}[Representative sector decomposition]
    \label{thm:cpsv_representative_sector_decomposition}
    \lean{MPSTensor.CPSVCanonicalFormData.BNTRefinement.%
    ...
    \lean{MPSTensor.CPSVCanonicalFormData.BNTRefinement.%
        groupedTensor_isBNTSectorPresentation}
    \lean{MPSTensor.CPSVCanonicalFormData.BNTRefinement.%
        source_sameMPV₂Pos_groupedTensor}
    \lean{MPSTensor.CPSVCanonicalFormData.BNTRefinement.%
        isBNTSectorPresentation}
    \leanok
    \uses{thm:cpsv_canonical_form_bnt_refinement, thm:cpsv_bnt_blocked_simultaneous_span, def:cpsv_bnt_sector_presentation}
    In the notation of
    Theorem~\ref{thm:cpsv_canonical_form_bnt_refinement}, let
    $k(j,q)$ be the listed block corresponding to the
    class-copy pair $(j,q)$. The representatives $C_j$ define a sector
    decomposition $P_{\mathrm{rep}}$ with copy weights
    \begin{align}
        \lambda_{j,q}
         & =\nu_{k(j,q)}\zeta_{k(j,q)}.
        \label{eq:cpsv_active_representative_weight}
    \end{align}
    Every copy weight in $P_{\mathrm{rep}}$ is nonzero. The representatives have simultaneous
    word-tuple separation at every sufficiently large length.

    For every positive length $N$, the grouped tensor $T_{\mathrm{grp}}$ of
    Theorem~\ref{thm:cpsv_canonical_form_bnt_refinement} and the
    representative sector tensor generate the same matrix product vector.
    The original tensor has the same positive-length matrix product
    vectors as both:
    \begin{align}
        V^{(N)}(T_{\mathrm{grp}})
         & =V^{(N)}(P_{\mathrm{rep}}).
    ...
    \end{align}
    Hence $P_{\mathrm{rep}}$ is a BNT sector presentation of $A$
    whenever at least one listed block occurs.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:cpsv_canonical_form_bnt_refinement, thm:cpsv_bnt_blocked_simultaneous_span, thm:coisometry_reconstruction_same_mpv_pos, thm:gauge_invariance, lem:paper_decomp_formula}
    The class-copy equivalence reindexes the listed blocks.
    ...
    simultaneous word-tuple separation for the representatives. Reindexing
    the finite sum by $(j,q)$
...
\begin{definition}[BNT canonical form]%
    ...
    \leanok
    \uses{def:sector_weight_data, def:bnt, def:bnt_cpsv}
    Let $P$ be a sector decomposition. In addition to spanning and eventual
    linear independence, the canonical form specifies irreducible
    left-canonical representatives, self-overlap normalization, pairwise
    gauge-phase separation, and weight normalization. Explicitly, each basis
    block has positive bond dimension, is irreducible, and is left-canonical;
    the normalized self-overlap of every basis block converges to $1$; the
    basis MPV family is eventually linearly independent; distinct
    same-dimension basis blocks are not gauge-phase equivalent after identifying
    equal bond dimensions; and the \cite[Section~2.3]{Cirac2016MPDO_arXiv}
    canonical-form normalization holds:
    \begin{align}
        ...
        \notag
    \end{align}
    The MPV expansion takes the two-layer form
    \begin{align}
        V^{(N)}(P)_\sigma
         & =
        \sum_{j=1}^{g}
    ...
    \cite[Definition~IV.2]{Cirac2021Matrix}. The canonical-form normalization
    accommodates equal-modulus and repeated-copy sectors, such as
    $C \oplus (-C)$ and $C \oplus (1/2) C$.
\end{definition}
...
\begin{lemma}[Existence of a unit-modulus weight from the canonical-form normalization]%
...
\begin{theorem}[A BNT canonical form determines an algebraic BNT]
    ...
    basis of normal tensors for the total tensor associated with $P$.
\end{theorem}
...
\begin{theorem}[Positive blocking preserves a basis of normal tensors]
    \label{thm:cpsv_bnt_positive_blocking}
    \lean{MPSTensor.IsCPSVBasisOfNormalTensors.blockTensor}
    \leanok
    \uses{def:bnt_cpsv, def:blocked_tensor, thm:cpsv_normal_tensor_positive_blocking}
    If $(A_j)_{j=1}^{g}$ is a basis of normal tensors for $A$ and $p\geq1$,
    then $(A_j^{[p]})_{j=1}^{g}$ is a basis of normal tensors for $A^{[p]}$.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:cpsv_normal_tensor_positive_blocking}
    Positive blocking preserves normality of each representative. Fix a
    blocked length $N$. If the $m$-th blocked letter is
    $J_m=(i_{m,1},\ldots,i_{m,p})$, define the configuration reindexing
    $\tau_p$ by concatenation:
    \begin{align}
        \tau_p(J_1,\ldots,J_N)
         & :=(i_{1,1},\ldots,i_{1,p},\ldots,
        i_{N,1},\ldots,i_{N,p}).
        \label{eq:cpsv_bnt_blocked_configuration_reindexing}
    \end{align}
    For every blocked configuration $\sigma$ and every representative $A_j$,
    the coefficient identities are
    \begin{align}
        \braket{\sigma}{V^{(N)}(A^{[p]})}
         & =\braket{\tau_p(\sigma)}{V^{(Np)}(A)},
        \label{eq:cpsv_bnt_blocked_tensor_coefficient} \\
        \braket{\sigma}{V^{(N)}(A_j^{[p]})}
         & =\braket{\tau_p(\sigma)}{V^{(Np)}(A_j)}.
        \label{eq:cpsv_bnt_blocked_representative_coefficient}
    \end{align}
    The spanning coefficients at length $Np$ therefore give those at blocked
    length $N$, and the same configuration reindexing preserves eventual
    linear independence.
\end{proof}
...
When each sector contains a single copy ($r_j = 1$), the sector decomposition
reduces to a weighted direct sum.

\subsection{Prepared-block construction of BNT canonical forms}%
\label{subsec:paperbnt_supplier}

After blocking, an MPS tensor reduces to a direct sum of trace-preserving,
primitive, irreducible, and injective blocks. Quotienting gauge-phase-equivalent
blocks into sectors and normalizing weights produces the canonical form of
...
\begin{proof}\leanok
    ...
    adjusted weights still have modulus at most one, and at least one has
    modulus one. Thus Definition~\ref{def:sector_bnt_cf} holds.
\end{proof}
...
\begin{theorem}[BNT from normal blocks with normalized weights]%
    ...
    \begin{enumerate}
        ...
        \item at least one weight has modulus one: $|\mu_k| = 1$ for some $k$.
    \end{enumerate}
...
\begin{theorem}[Sector BNT form of normalized CPSV canonical data]%
    \label{thm:cpsv_sector_bnt}
    \lean{MPSTensor.CPSVCanonicalFormData.exists_isBNTCanonicalForm}
    \leanok
    ...
    \end{align}
    The total dimension $D_{\mathrm{tot}}(P)$ equals the sum of the retained
    block dimensions.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:cpsv_sector_bnt_exact, thm:coisometry_reconstruction_same_mpv_pos, thm:gauge_invariance}
    Apply Theorem~\ref{thm:cpsv_sector_bnt_exact}. Its reconstruction
...
\begin{theorem}[Exact coisometric BNT reconstruction]%
    \label{thm:cpsv_sector_bnt_exact}
    \lean{MPSTensor.CPSVCanonicalFormData.exists_isBNTCanonicalForm_exact}
    \leanok
    \uses{def:cpsv_canonical_form, def:sector_bnt_cf}
    Let $A$ have CPSV canonical-form data satisfying the separate normalization
    condition~(\ref{eq:cpsv_weight_normalization}), and assume $A\ne0$. There
    are a BNT canonical-form sector decomposition $P$, a coisometry
...
\begin{proof}\leanok
    \uses{def:cpsv_canonical_form, thm:cpsv_normal_tp_gauge, lem:gauge_equiv_from_block_gauge, thm:paperbnt_supplier_prepared_exact}
    ...
    \end{align}
    Put every normal block into a trace-preserving primitive irreducible
    gauge $Y$, and apply
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This proves (\ref{eq:cpsv_active_exact_bnt_coisometry}). The block
...
\begin{theorem}[Equal-ambient gauge equivalence for CPSV canonical forms]%
    ...
    \end{align}
    The result applies to tensors sharing the ambient bond dimension $D$.
\end{theorem}

The cited sources supply the retained canonical-form comparison. The passage back
from the retained tensors to the ambient tensors is the coisometric lifting of
Theorem~\ref{thm:coisometry_reconstruction_gauge_equiv}.

\begin{proof}\leanok
    \uses{thm:cpsv_sector_bnt_exact, thm:coisometry_reconstruction_same_mpv_pos, thm:cpgsv_equal_case_source, thm:coisometry_reconstruction_gauge_equiv, thm:gauge_invariance}
    Apply Theorem~\ref{thm:cpsv_sector_bnt_exact} to obtain
...
Combining the after-blocking preparation with the weight normalization of
Theorem~\ref{thm:paperbnt_weight_normalization} gives the arbitrary-input
statement.
...
\begin{theorem}[Normalized BNT form after blocking for a nonzero MPV family]%
    ...
    weight choice, which is realized rather than assumed. The nonzero-family
    hypothesis is the standing convention of
    \cite[Section~2.3]{Cirac2016MPDO_arXiv}: the unit-modulus weight chosen
    there presupposes a nonempty weight family. The normalization and the
    proportional-MPV comparison are documented in
