...
\begin{lemma}[Gauge equivalence of weighted block sums]%
    \label{lem:sector_bnt_gauge_equiv_from_block_gauge}
    \lean{MPSTensor.gaugeEquiv_toTensorFromBlocks_of_blockGauge}
    \leanok
    \uses{def:block_diagonal_tensor, def:gauge_equiv}
    Suppose two block families have the same weights and corresponding blocks
    are gauge equivalent. Then their weighted block-diagonal tensors are gauge
    equivalent.
\end{lemma}

\begin{proof}\leanok
    \uses{lem:tensor_from_blocks_globalGauge_conj, def:global_gauge_of_blocks}
    Assemble the block gauges into a block-diagonal invertible matrix and apply
    it simultaneously to every physical letter.
\end{proof}
...
\begin{proof}\leanok
    \uses{thm:cpsv_exact_active_reconstruction,
        thm:cpsv_normal_tp_gauge,
        lem:sector_bnt_gauge_equiv_from_block_gauge,
