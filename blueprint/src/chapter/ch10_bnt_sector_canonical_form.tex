...
\begin{lemma}[Gauge realization with the fixed phase scalar]%
    ...
    Let $X$ and $Y$ satisfy the hypotheses of
    Theorem~\ref{thm:prepared_phase_equiv_gauge}. Fix
    $\zeta\in\C\setminus\{0\}$ and suppose that, for every positive length $N$
...
\begin{theorem}[Active-sector BNT form of normalized CPSV canonical data]%
    ...
    \leanok
    \uses{thm:cpsv_active_sector_bnt_exact, thm:gauge_invariance}
...
\begin{proof}\leanok
    ...
    $B^i=XP^iX^{-1}$. In every positive-length cyclic contraction, the adjacent
    factors $UU^\dagger$ cancel, so $A$ and $B$ generate the same matrix product
    vectors. Theorem~\ref{thm:gauge_invariance} gives the same conclusion for
    $B$ and $P$. Transitivity yields $\mc{V}^+(A)=\mc{V}^+(P)$, while the
