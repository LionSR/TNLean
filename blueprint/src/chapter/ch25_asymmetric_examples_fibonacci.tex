...
The Fibonacci fusion rules have two labels, the vacuum $1$ and $\tau$,
with $\tau\otimes\tau=1\oplus\tau$. The string-net construction
of~\cite[Appendix~D.1]{Bultinck2017Anyons} gives MPO blocks of bond
dimensions two and three. We describe their concrete two-label
realization, its action on periodic states, and its asymmetric
compressions. As in the presentation of MPS examples and MPO symmetries
in~\cite{Cirac2021Matrix}, we begin with the local tensors and their
...
$s=\sigma=\varphi^{-1/2}=\sqrt{(\sqrt5-1)/2}$.
...
The physical alphabet here has dimension two. It is not the full
triple-index alphabet of the categorical string-net construction.
Identifying operators on those different physical spaces requires an
explicit map between the spaces. In particular, the convention
$\sigma=\varphi^{-1/2}$ in these letters must not be replaced by
$\varphi^{-1}$.
...
construction of~\cite[Appendix~D.1]{Bultinck2017Anyons}, which combines
the vacuum and $\tau$ blocks with weights
$w_1=1/(1+\varphi^2)$ and $w_\tau=\varphi/(1+\varphi^2)$.
The single vacuum block considered here should not be identified with
that weighted construction.
...
\begin{proof}
    ...
    Embedding these identities into $\C$ proves the required
    word-spanning property. No normalization of the transfer-map
    spectral radius is asserted.
\end{proof}
...
This realizes the nonnegative-integer action discussed
in~\cite{GarreRubio2023MPOSym}; it does not assert that the two vectors
form a basis at each $N$. The positive-length hypothesis is essential:
at $N=0$ the periodic trace is the bond dimension, so
$O_0(B_1)=2$, $O_0(B_\tau)=3$, and the empty amplitudes of $A$ and
$C$ are $1$ and $2$. Neither the vacuum projection identity nor
(\ref{eq:asymex_fib_action_product}) extends to that length.

\subsection{Exact arithmetic and asymmetric compression}

The physical operator and state identities above reduce to finite
matrix calculations. Their coefficients belong to a quartic ring,
which also contains the compression maps for the stacked square.
...
\begin{theorem}[Normality of the two Fibonacci blocks]
    ...
    Both blocks are normal: their length-three words span the full
