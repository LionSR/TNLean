...
\begin{proof}\leanok
    ...
    neighboring operators $\eta_{q,h}$.  Expand every $\eta_{q,h}$ in matrix
    units, separating each coefficient into a right factor at one site and a
    left matrix unit at the next.  For a nonzero physical space, their common
    finite index is nonempty and gives a positive virtual dimension.  For the
    zero-dimensional physical space, take one virtual state and no physical
    sectors.  Contracting adjacent factors reconstructs every
    $\eta_{q,h}$, hence the physical-sector bond is exactly $B$.  The cyclic
    contraction formula then gives the product of the translates of $B$ at
    every length $N\geq2$.
\end{proof}
