Theorem~\ref{thm:periodic_ft} formalizes the equal-case Fundamental Theorem of
\cite[lines~643--656]{DeLasCuevas2017Irreducible} for tensors in
irreducible form. Theorem~\ref{thm:periodic_ft_equal_case_trace_preserving}
handles the case where each block's transfer map is trace-preserving; the
reduction to this normalization via blockwise similarity (\cite[eq.~\emph{unital},
    line~316]{DeLasCuevas2017Irreducible}) is carried out in the proof.
...
\begin{theorem}[Periodic Fundamental Theorem]
    ...
    dimensions $D_j$ and $D_k$. The blocks need only have irreducible transfer
    maps of spectral radius one
    (\cite[lines~248--261]{DeLasCuevas2017Irreducible}); no trace-preserving
    normalization is imposed, and the periodicity $m_j$ of each block is
    derived from the peripheral spectrum
...
\begin{proof}
    ...
    Theorem~\ref{thm:spectral_periodic_period_existence} yields a positive
    period $m_j$ for each block $A_j$ and $n_k$ for each block $B_k$. Applying
    Theorem~\ref{thm:periodic_ft_supplied_periods} with these derived periods
    gives every conclusion.
\end{proof}

\begin{remark}[Conventions on length range and scalars]
    The matrix-product-vector identities are taken at positive lengths, following the convention that the empty word is not a physical chain; this extends to length zero since the empty-word coefficient equals the bond dimension. The scalars $\xi_j$ are made explicit rather than absorbed into $S_{\pi(j)}$ as in \cite[lines~667--671]{DeLasCuevas2017Irreducible}, allowing the statement to speak directly about the given multiplicity matrices.
\end{remark}
...
\begin{theorem}[Periodic Fundamental Theorem, periods supplied]%
    ...
    \uses{def:spectral_periodic_tensor, def:same_mpv2_pos,
        def:scalar_gauge_equiv}
    Under the hypotheses of Theorem~\ref{thm:periodic_ft}, suppose in addition
    that positive integers $m_j$ and $n_k$ are given such that each $A_j$ is
    spectrally $m_j$-periodic and each $B_k$ is spectrally $n_k$-periodic
    (Definition~\ref{def:spectral_periodic_tensor}). Then the conclusions of
    Theorem~\ref{thm:periodic_ft} hold for these specified periods.
\end{theorem}
...
\begin{proof}\leanok
    ...
    the two direct-sum tensors generate the same matrix-product vectors as
...
\begin{theorem}[Global multiplicity gauge and similarity]%
    ...
    This establishes the global intertwining relation of
    \cite[lines~689--690]{DeLasCuevas2017Irreducible}, with $Y$ the direct sum
    $\bigoplus_{j\in J} T_j\otimes Y_j$ of the copy reorderings and the
...
\begin{proof}\leanok
    ...
    direct-sum tensors are related by the permutation of the matched copies
...
\begin{theorem}[Fundamental theorem for MPS with trace-preserving blocks, equal case]%
    ...
    the phase into $S_j$; the scalar is present, as the period-one pair
    $B_j=e^{i\theta}A_j$ shows.

    This establishes
    \cite[Theorem~3.8, lines~643--693]{DeLasCuevas2017Irreducible} for tensors
    whose block maps are trace preserving
    (\cite[eq.~\emph{unital}, line~316]{DeLasCuevas2017Irreducible}). The
    periodic overlap dichotomy and the vanishing of off-period blocks apply
    directly in this trace-preserving setting, from which
    Theorem~\ref{thm:periodic_ft} is deduced.
\end{theorem}
...
\begin{proof}\leanok
    ...
    Theorem~\ref{thm:periodic_equalcase_global_zgauge} constructs the
