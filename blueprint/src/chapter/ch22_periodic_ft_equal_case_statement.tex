...
\begin{theorem}[Periodic Fundamental Theorem]
    ...
    $ZA^i=YB^iY^{-1}$ for all $i$.\end{theorem}

\begin{remark}
    The matrix-product-vector identities are stated at positive chain
    lengths $L \ge 1$. The periods $m_j$ are produced from irreducibility
    and spectral radius one by
    Theorem~\ref{thm:spectral_periodic_period_existence}, matching
    \cite[lines~257--258]{DeLasCuevas2017Irreducible}. The scalars $\xi_j$
    are made explicit rather than absorbed into $S_{\pi(j)}$
    \cite[lines~667--671]{DeLasCuevas2017Irreducible}, keeping the
    multiplicity matrices $R_j$ and $S_k$ unscaled.
\end{remark}
\begin{proof}
    \leanok
...
\begin{theorem}[Periodic Fundamental Theorem, periods supplied]%
    ...
    \leanok
    \uses{def:spectral_periodic_tensor, def:same_mpv2_pos,
    Theorem~\ref{thm:periodic_ft} with the periods $m_j$ and $n_k$ supplied
    ...
    Definition~\ref{def:spectral_periodic_tensor}.
\end{theorem}
