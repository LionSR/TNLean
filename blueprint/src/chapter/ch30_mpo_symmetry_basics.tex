...
%%   FusionRing,SymmetricBoundary,Examples}.lean (Sections sec:mposym_fusion
%%   and sec:mposym_symmetric).
...
\begin{remark}[Group operators from a three-cocycle and the anomaly]%
    ...
    (Theorem~\ref{thm:mposym_closed_fusion_onsite}). Hence, for such a
    representation, a nontrivial class forbids closed fusion. In the examples
    of Chapter~\ref{ch:asymmetric_examples} with a nontrivial class, the
    stacked tensor reduces onto $T_{gh}$ up to a nilpotent remainder. It also forbids an invariant normal matrix
...
\begin{theorem}[Closed fusion forces an on-site representation]%
    \label{thm:mposym_closed_fusion_onsite}
    \lean{MPOTensor.GroupFamily.IsOnSite,
        MPOTensor.GroupFamily.FusionData.IsClosed,
        MPOTensor.bondDim_le_one_of_mpo_eq_one,
        MPOTensor.eq_ite_one_of_mpo_eq_one,
        MPOTensor.bondDim_le_one_of_mulTensor_eq_ite,
        MPOTensor.GroupFamily.bondDim_eq_one_of_closed_fusion,
        MPOTensor.GroupFamily.isOnSite_of_closed_fusion,
        MPOTensor.GroupFamily.FusionData.isTrivialGaugeClass_omega_of_closed_fusion}
    \leanok
    \uses{def:injective, def:mpo_to_mps, def:mpdo_operator_product,
        def:mpug_anomaly_three_cochain, thm:mpug_anomaly_three_cochain_spec}
    Let $\{T_g\}_{g\in G}$ be matrix product operator tensors of a group $G$
    whose doubled-index tensors are injective, with $U_gU_h=U_{gh}$ and
    $U_e=\Id$ on every nonempty chain, and let $(F^<_{g,h},F^>_{g,h})$ be
    fusion tensors reducing $T_gT_h$ onto $T_{gh}$. Suppose the fusion is
    closed, $(T_gT_h)^{ij}=F^>_{g,h}\,T_{gh}^{ij}\,F^<_{g,h}$ for all $g,h$
    and all physical indices $i,j$. Then every $T_g$ has bond dimension one,
    the representation is on-site, $U_g=u_g^{\otimes N}$ with
    $(u_g)_{ij}=T_g^{ij}$, and the anomaly three-cocycle $\omega$ of the
    fusion tensors is a coboundary. The on-site conclusion does not use the
    product law.
    % Source anchor: arXiv:2203.12563, Papers/2203.12563/REsubmission.tex,
    %   lines 1211--1213 (Lemma and proof), 641--656 (closed fusion,
    %   eq. fusiontensorG), 658 and 842 (on-site operators).
\end{theorem}

\begin{proof}\leanok
    \uses{def:mpug_anomaly_three_cochain, thm:mpug_anomaly_three_cochain_spec}
    The tensor $T_e$ is injective with $\tr T_e^{ij}=\delta_{ij}$ and
    $\tr(T_e^{ij}T_e^{kl})=\delta_{ij}\delta_{kl}$, so
    $\tr(XY)=\tr X\,\tr Y$ on the whole matrix algebra; testing on two
    off-diagonal matrix units shows that $T_e$ has bond dimension one and
    $T_e^{ij}=\delta_{ij}$. Closed fusion then gives
    $(T_gT_{g^{-1}})^{ij}=\delta_{ij}P$ and
    $(T_{g^{-1}}T_g)^{ij}=\delta_{ij}Q$ with
    $\tr P=\tr(F^<_{g,g^{-1}}F^>_{g,g^{-1}})\neq0$. Decomposing the triple
    product $T_gT_{g^{-1}}T_g$ in the two ways gives
    $P\otimes T_g^{ij}=T_g^{ij}\otimes Q$, hence $P\otimes X=X\otimes Q$ for
    every matrix $X$ by injectivity. If $T_g$ had bond dimension at least
    two, testing on $X=E_{cc}$ with $c\neq a$ would kill every diagonal
    entry of $P$ in the block $a$, and $\tr P$ would vanish. With every bond
    dimension one, the periodic operator is the product of the one-site
    entries, and the fusion tensors are nonzero scalars $v(g,h)$. The two
    fusion trees are then proportional with ratio
    $v(gh,k)v(g,h)/(v(g,hk)v(h,k))$, which is $\omega(g,h,k)$ by
    Theorem~\ref{thm:mpug_anomaly_three_cochain_spec}; this is the
    coboundary of $v^{-1}$.
\end{proof}
...
The source defines the symmetry by invariance of the states with arbitrary
boundary conditions; the statements below use only its consequence for
periodic boundary conditions~\cite[Section~3]{GarreRubio2023MPOSym}; see
...
\begin{example}[Ising fusion rule]%
    ...
    \lean{IsingTwist.fusionMatrix_isingFusion_psi,
        IsingTwist.fusionMatrix_isingFusion_sigma,
        IsingTwist.charpoly_fusionMatrix_isingFusion_sigma,
        IsingTwist.spectrum_fusionMatrix_isingFusion_sigma,
        IsingTwist.isingFusion_dimension_relation,
        IsingTwist.perronFrobeniusDim_isingFusion_sigma,
        IsingTwist.perronFrobeniusDim_isingFusion_psi,
        IsingTwist.perronFrobeniusDim_isingFusion_one,
        IsingTwist.isInvertibleLabel_isingFusion_psi,
        IsingTwist.not_isInvertibleLabel_isingFusion_sigma,
        IsingTwist.mpo_isingBlock_sigma_mul_sub}
    \leanok
    \uses{def:mposym_pf_dimension, lem:mposym_pf_dimension_positive,
        thm:asymex_ising_sigma_fusion, thm:asymex_ising_sigma_square,
        ex:mpo_symmetry_ising}
...
\begin{proof}
    ...
    \end{align}
    Let $\lambda_i$ be the nonzero eigenvalues of $Q$, counted with algebraic
    multiplicity. Since $\tr(Q^k) = \sum_i \lambda_i^k = c_{y_0}$ for all
    $k\geq1$, every $\lambda_i$ equals $1$, so $c_{y_0}$ is their total
    algebraic multiplicity and lies in $\N$. This extends the power-sum argument of
...
\begin{example}[Anomalous cyclic group of order three]%
    ...
    which every label is invertible. The fusion ring does not detect the
    nontrivial three-cocycle class $[\omega]\neq 1\in H^3(\Z_3,\C^\times)$ of
...
normal tensors can still be an eigenvector of every operator. When the
algebra is that of a fusion category, such a superposition with positive
weights exists for every module \cite[Section~5]{GarreRubio2023MPOSym}; it
is the boundary state compatible with a two-dimensional bulk carrying the
operators as virtual symmetries. The fusion category enters only through its
fusion ring.

\begin{definition}[Fusion ring]%
    \label{def:mposym_fusion_ring}
    \lean{MPOTensor.IsFusionRing}
    \leanok
    \uses{def:fusion_unit_invertible}
    Structure constants $N_{ab}^c\in\N$ on a finite label set $I$ form a
    \emph{fusion ring} with unit $e$ and duality $a\mapsto a^*$ if they are
    associative,
    $\sum_xN_{ab}^xN_{xc}^f=\sum_xN_{bc}^xN_{ax}^f$ for all $a,b,c,f$, the
    label $e$ is a unit in the sense of
    Definition~\ref{def:fusion_unit_invertible}, the duality is an
    involution, $N_{ab}^e=\delta_{b,a^*}$, and
    $N_{b^*a^*}^{c^*}=N_{ab}^c$. These are the identity element and dual
    elements of the fusion category of a physical symmetry
    \cite[Section~5]{GarreRubio2023MPOSym}.
    % Source anchor: arXiv:2203.12563, line 1236.
\end{definition}

\begin{theorem}[Positive regular element of a fusion ring]%
    \label{thm:mposym_fusion_ring_regular}
    \lean{MPOTensor.IsFusionRing.exists_pos_mul,
        MPOTensor.IsFusionRing.exists_pos_regular,
        MPOTensor.IsFusionRing.perronFrobeniusDim_pos,
        Matrix.exists_eq_smul_of_pos_of_mulVec_eq}
    \leanok
    \uses{def:mposym_fusion_ring, thm:mposym_pf_eigenvector,
        lem:mposym_pf_dimension_positive}
    For a fusion ring there are positive numbers $r_b$ with
    $\sum_br_bN_{ab}^c=d_a\,r_c$ for all labels $a$ and $c$, where $d_a$ is
    the Perron--Frobenius dimension of $a$. Moreover $d_a>0$ for every $a$.
\end{theorem}

\begin{proof}
    \leanok
    The coefficient of $c$ in $(b\times b^*)\times c$ is at least
    $N_{bb^*}^eN_{ec}^c=1$, and by associativity it equals the coefficient
    of $c$ in $b\times(b^*\times c)$; so for all $b,c$ some label $a$ has
    $N_{ba}^c>0$. Hence the matrix $P_{bc}=\sum_aN_{ba}^c$ of right
    multiplication by the sum of all labels has positive entries. By
    Theorem~\ref{thm:mposym_pf_eigenvector} applied to the transpose of $P$,
    there is a nonzero row vector $r\geq0$ with $rP=\rho r$, and positivity of
    $P$ forces every $r_c>0$. An eigenvector of a positive matrix with
    positive entries spans its eigenspace: if $wP=\rho w$ and $t=\min_c
        w_c/r_c$, then $w-tr$ is a nonnegative eigenvector with a vanishing entry,
    and positivity of $P$ forces $w=tr$. Left multiplication by $a$, the
    matrix $N_a$, commutes with $P$ by associativity, so $rN_a$ is again an
    eigenvector of $P$ for $\rho$ and $rN_a=t_ar$. By
    Lemma~\ref{lem:mposym_pf_dimension_positive}, $t_a=d_a$. Finally
    $d_ar_a=\sum_br_bN_{ab}^a\geq r_eN_{ae}^a=r_e>0$.
\end{proof}

\begin{theorem}[A positive common eigenvector of a nonnegative integer representation]%
    \label{thm:mposym_nimrep_pos_eigenvector}
    \lean{MPOTensor.IsNIMRep.exists_pos_left_eigenvector,
        MPOTensor.IsNIMRep.spectralRadius_eq_perronFrobeniusDim}
    \leanok
    \uses{thm:mposym_fusion_ring_regular, def:fusion_character_nimrep}
    Let $N$ be a fusion ring with unit $e$ and let $M_{a,x}^y$ be a
    nonnegative integer representation of it on a finite set $K$ with
    $M_{e,x}^y=\delta_{xy}$. Then there are positive numbers $v_x$ with
    \begin{align}
        \sum_xv_xM_{a,x}^y & =d_a\,v_y
        \label{eq:mposym_nimrep_eigenvector}
    \end{align}
    for all labels $a$ and all $y\in K$. If $K$ is nonempty, $d_a$ is the
    spectral radius of the matrix $M_a$. No indecomposability of the
    representation and no commutativity of the fusion ring is needed.
    % Source anchor: arXiv:2203.12563, lines 1801--1803.
\end{theorem}

\begin{proof}
    \leanok
    Let $r$ be as in Theorem~\ref{thm:mposym_fusion_ring_regular} and
    $S=\sum_br_bM_b$. By (\ref{eq:mpo_nimrep}),
    $M_bM_a=\sum_cN_{ab}^cM_c$, so
    $SM_a=\sum_c\left(\sum_br_bN_{ab}^c\right)M_c=d_aS$. The column sums
    $v_y=\sum_xS_{xy}$ therefore satisfy (\ref{eq:mposym_nimrep_eigenvector}),
    and $v_y\geq r_eM_{e,y}^y=r_e>0$. A positive left eigenvector of the
    nonnegative matrix $M_a$ has the spectral radius as its eigenvalue, by
    the argument of Lemma~\ref{lem:mposym_pf_dimension_positive}.
\end{proof}

\begin{theorem}[Positive symmetric boundary state]%
    \label{thm:mposym_symmetric_boundary_state}
    \lean{MPOTensor.IsMPOSymmetricFamily.mpo_mulVec_sum_eq_smul,
        MPOTensor.exists_pos_symmetric_boundary_state}
    \leanok
    \uses{thm:mposym_nimrep_pos_eigenvector, thm:mpo_symmetric_nimrep}
    Let $(A_x)_{x\in K}$, with $K$ nonempty, be normal tensors of positive bond dimension whose
    periodic vectors are linearly independent at some length $N_0\geq1$,
    symmetric under a fusion algebra whose structure constants form a fusion
    ring with unit $e$, and suppose $M_{e,x}^y=\delta_{xy}$. Then there are
    positive numbers $v_x$ such that for every label $a$ and every
    $N\geq1$,
    \begin{align}
        O_a^{(N)}\ket{\psi^{(N)}} & =d_a\ket{\psi^{(N)}},         &
        \ket{\psi^{(N)}}          & =\sum_xv_x\ket{V^{(N)}(A_x)},
        \notag
    \end{align}
    with $d_a>0$, and $\ket{\psi^{(N_0)}}\neq0$. This is the symmetric boundary state of
    \cite[Section~5]{GarreRubio2023MPOSym}, whose eigenvalue $r_a$ is the
    spectral radius of $M_a$.
    % Source anchor: arXiv:2203.12563, lines 1795--1805 (symboundary).
\end{theorem}

\begin{proof}
    \leanok
    By Theorem~\ref{thm:mpo_symmetric_nimrep} the coefficients $M_{a,x}^y$
    form a nonnegative integer representation. Take $v$ from
    Theorem~\ref{thm:mposym_nimrep_pos_eigenvector}. Then
    $O_a^{(N)}\ket{\psi^{(N)}}=\sum_xv_x\sum_yM_{a,x}^y\ket{V^{(N)}(A_y)}
        =\sum_y\left(\sum_xv_xM_{a,x}^y\right)\ket{V^{(N)}(A_y)}
        =d_a\ket{\psi^{(N)}}$. Since the periodic vectors are linearly independent at length $N_0$ and $v\neq0$, $\ket{\psi^{(N_0)}}\neq0$.
\end{proof}

\begin{example}[The Fibonacci boundary state]%
    \label{ex:mposym_fibonacci_boundary_state}
    \lean{FibonacciCompression.isFusionRing_fibNim,
        FibonacciCompression.fibDim_left_eigenvector,
        FibonacciCompression.fibBoundaryState,
        FibonacciCompression.mpo_fibBlock_mulVec_boundary,
        FibonacciCompression.mpo_fibTau_mulVec_boundary,
        FibonacciCompression.mpo_fibOne_mulVec_boundary}
    \leanok
    \uses{thm:mposym_symmetric_boundary_state, ex:mpo_symmetry_fibonacci,
        ex:mposym_fibonacci_dimension}
    The Fibonacci structure constants form a fusion ring with unit $1$ and
    both labels self-dual. For the family of
    Example~\ref{ex:mpo_symmetry_fibonacci}, $M_\tau=N_\tau$ and
    $v=(1,\varphi)$, so
    $\ket{\psi^{(N)}}=\ket{V^{(N)}(A_1)}+\varphi\ket{V^{(N)}(A_\tau)}$
    satisfies $O_1^{(N)}\ket{\psi^{(N)}}=\ket{\psi^{(N)}}$ and
    $O_\tau^{(N)}\ket{\psi^{(N)}}=\varphi\ket{\psi^{(N)}}$ for every
    $N\geq1$. The eigenvalue $d_\tau=\varphi$ is not an integer.
    % Source anchor: arXiv:2203.12563, lines 1795--1805 (r_a = d_a when
