...
\begin{definition}[Fusion matrices and Perron--Frobenius dimension]%
    ...
    \notready
    % Planned Lean: TNLean/MPS/Symmetry/MPOSymmetry/Dimension.lean,
    %   MPOTensor.fusionMatrix and MPOTensor.perronFrobeniusDim.
    \uses{def:mpo_fusion_algebra}
...
\begin{proof}
    This is the Perron--Frobenius theorem for nonnegative matrices. For
    $\varepsilon>0$ the matrix $N_a+\varepsilon J$, with $J$ the all-ones
    matrix, has positive entries, so it has a positive eigenvector
    $v_\varepsilon$ of norm one for the eigenvalue
    $\rho(N_a+\varepsilon J)$. A limit point $v$ of $v_\varepsilon$ as
    $\varepsilon\to0$ is nonnegative of norm one, and continuity of the
    spectral radius gives $N_av=\rho(N_a)v=d_av$.
\end{proof}

\begin{lemma}[A positive left eigenvector gives the Perron--Frobenius dimension]%
    \label{lem:mposym_pf_dimension_positive}
    \notready
    % Planned Lean: MPOTensor.perronFrobeniusDim_eq_of_pos_left_eigenvector.
    \uses{def:mposym_pf_dimension}
...
\begin{proof}
    \uses{thm:mposym_pf_eigenvector}
    Write $\delta$ for the row vector with entries $\delta_b$, so that
    $\delta N_a=\lambda\delta$. By
    Theorem~\ref{thm:mposym_pf_eigenvector} there is a column vector
    $v\neq0$ with nonnegative entries and $N_av=d_av$. Then
    $\lambda\,\delta v=\delta N_av=d_a\,\delta v$, and $\delta v>0$ because
    every $\delta_b$ is positive and $v$ is nonnegative and nonzero. Hence
    $\lambda=d_a$. The second statement is the first with
    $\lambda=\delta_a$.
\end{proof}
...
\begin{example}[Fibonacci fusion rule and the golden ratio]%
    ...
    \notready
    % Planned Lean: FibonacciCompression.perronFrobeniusDim_fibFusion_tau.
