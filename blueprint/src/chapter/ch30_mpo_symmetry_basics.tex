...
\begin{remark}[Groups represented by matrix product unitaries]%
    ...
    \uses{def:mpu, def:mpug_group_representation}
    ...
    periodic operator is unitary at every length, and a finite group is
    represented by matrix product unitaries when
...
\begin{remark}[Group operators from a three-cocycle and the anomaly]%
    ...
    class forbids an on-site form. It also forbids a symmetric normal matrix
    product state (Theorem~\ref{thm:mpoalg_no_symmetric_normal}), so a local
    Hamiltonian with such a symmetry is critical or has a degenerate ground
    space \cite{ChenLiuWen2011CZX,GarreRubio2023MPOSym,Cirac2021Matrix}.
...
\begin{definition}[Fusion matrices and Perron--Frobenius dimension]%
    ...
    $d_a=\rho(N_a)$. By the Perron--Frobenius theorem for nonnegative
    matrices, $d_a$ is an eigenvalue of $N_a$ with a nonzero eigenvector with
    nonnegative entries.
...
\begin{proof}
    ...
    $\delta N_a=\lambda\delta$. Let $v\neq0$ be a column vector with
    nonnegative entries and $N_av=d_av$. Then
...
\begin{example}[Fibonacci fusion rule and the golden ratio]%
    ...
    \uses{def:mposym_pf_dimension, lem:mposym_pf_dimension_positive,
        thm:asymex_fib_anomaly_fusion_algebra, def:asymex_fib_fsymbols}
    The Fibonacci labels are $1$ and $\tau$, with unit $1$ and
    $\tau\times\tau=1+\tau$. In the order $(1,\tau)$,
    \begin{align}
        N_\tau & =\begin{pmatrix}0&1\\1&1\end{pmatrix},
        \label{eq:mposym_fibonacci_matrix}
    \end{align}
    ...
    $b=\tau$, and $\tau\times\tau$ also contains $\tau$. The operators of Section~\ref{sec:asymex_fibonacci}
    realize this rule at every length
...
\begin{example}[Ising fusion rule]%
    ...
    The Ising labels are $1$, $\psi$ and $\sigma$, with unit $1$,
    $\psi\times\psi=1$, $\sigma\times\psi=\psi\times\sigma=\sigma$ and
    $\sigma\times\sigma=1+\psi$. In the order $(1,\psi,\sigma)$,
    \begin{align}
...
which already excludes symmetric normal states for the Fibonacci rule.
...
\begin{remark}[Symmetric superpositions of several normal states]%
    ...
    normal tensors can still be an eigenvector of every operator. If the
    matrices $M_a=\bigl(M_{a,x}^y\bigr)$ of
    Theorem~\ref{thm:mpo_symmetric_nimrep} have a common eigenvector $v$ with
    positive entries, $M_av=r_av$, then
...
\begin{remark}[Zipper and pulling-through conditions]%
    ...
    a time. In the setting of the source the two identities are equivalent to
    the reconstruction
    $(T_aT_b)^{ik}=\sum_{c,\mu}X_{ab,\mu}^cT_c^{ik}X_{ab,\mu}^{c+}$, and
    closing the chain then gives (\ref{eq:mpo_fusion_algebra}) at every
    ...
    and~\ref{def:mpoalg_action_tensors}), and shows that fusion tensors with vanishing
    remainder, the situation of the zipper condition, are unique up to
    invertible multiplicity matrices
