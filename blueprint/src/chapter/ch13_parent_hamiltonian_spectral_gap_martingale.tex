...
\begin{proof}
    ...
    \end{align}
    Take $a_i=\varepsilon$ and $B_{ij}=\varepsilon(1-\delta_{ij})$. Since $B$
    is symmetric with nonnegative entries, its norm is attained on vectors
    with nonnegative entries, so the displayed estimate gives
    $\lVert B\rVert\leq\varepsilon(m-1)\leq1-\varepsilon$, which is less
