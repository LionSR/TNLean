...
\begin{theorem}[C1--C3 energy estimate with no mass below the threshold]
    ...
    \cite{Nachtergaele1996Spectral} with conditions C1, C2, and C3 imposed
    only on $n_0\leq n<N$, which is where the source imposes them. The added
...
% docs/paper-gaps/nachtergaele96_theorem_2_1_lower_endpoint.tex.
\begin{theorem}[The threshold conditions alone do not give the estimate]
    \label{thm:nachtergaele_threshold_conditions_insufficient}
    \lean{FrustrationFree.not_unrestrictedNachtergaeleEstimate}
    \leanok
    For every $k$ there are, on a complex Hilbert space of dimension $k+2$,
    nested orthogonal projections $(G_n)_{n\geq0}$ with $G_0=\mathbf 1$,
    orthogonal projections $Q_n$, local operators $H_n$, an operator $H$, and
    a vector $v\perp\operatorname{ran}(G_{k+2})$ such that, with $l=0$,
    $n_0=1$, $N=k+2$, $\gamma_1=d_1=1$, and $\epsilon_0=0$, all the
    hypotheses of
    Theorem~\ref{thm:nachtergaele_c1_c3_threshold_energy_estimate} hold and
    \begin{align}
        \re\langle Hv,v\rangle
    ...
    \end{align}
    Hence conditions C1, C2, and C3 on the range $n_0\leq n<N$ at which
    \cite{Nachtergaele1996Spectral} imposes them do not imply the conclusion
    of its Theorem~2.1(i).
\end{theorem}
...
\begin{proof}
    ...
    condition C3 with $\epsilon_0=0$. Summing $H_n$ over $1\leq n<k+2$ gives
    $H$ without its weight at $e_0$, which is condition C1 with $d_1=1$. For
    ...
    martingale mass of $v$ sits at the excluded index $n=0$, the single index
    at which condition C2 fails.
\end{proof}
