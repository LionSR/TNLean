...
\begin{lemma}[Blocked local terms at block-aligned starts]
    \label{lem:blocked_original_aligned_local_terms}
    ...
    chain of length $Np$.  The parent interaction of $B$ on $N$ blocked sites is
    conjugate under $U$ to the parent interaction of $A$ on $Np$ sites, and for
    every blocked site $i$,
    \begin{align}
        U h^{B,2}_i U^{-1}
         & =h^{A,2p}_{pi}.
    \end{align}
...
\begin{theorem}[Uniform gap for normal MPS parent Hamiltonians]
    ...
    point, at range $L=l+1$ and for every $N\ge2m$, with no divisibility
    restriction on the chain length.  The unrestricted statement still requires
    the passage from that hypothesis to an arbitrary normal tensor.
\end{theorem}
...
\begin{proof}
    ...
    \discussion{5926}
    \uses{thm:primitive_parent_hamiltonian_finite_range_knabe_gap, thm:anticommutator_gap_bound, thm:parent_hamiltonian_gapped}
    The primitive case with a positive-definite fixed point is
    Theorem~\ref{thm:primitive_parent_hamiltonian_finite_range_knabe_gap}.  It
    remains to reduce an arbitrary normal tensor to that case while preserving
