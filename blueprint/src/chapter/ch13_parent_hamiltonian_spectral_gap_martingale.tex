...
\begin{theorem}[C1--C3 energy estimate with no mass below the threshold]
    ...
    Theorem~\ref{thm:nachtergaele_c1_c3_threshold_energy_estimate}, and
    ...
    \cite{Nachtergaele1996Spectral} with conditions C2 and C3 imposed only on
    $n_0\leq n<N$, which is where the source imposes them, and C1 imposed on
    that same range rather than on the wider range $l\leq n<N$ of its own
    window sum. The added
...
% one-site chain with a weakened leftmost interaction, restricted to its
% single-excitation subspace; see
% docs/paper-gaps/nachtergaele96_theorem_2_1_lower_endpoint.tex.
\begin{theorem}[The source conditions on their own ranges do not give the
    ...
    \leanok
    In the notation of
    Theorem~\ref{thm:nachtergaele_c1_c3_threshold_energy_estimate}: for every
    $k$ there are, on a complex Hilbert space of dimension $k+2$, nested
    orthogonal projections $(G_n)_{n\geq0}$ with $G_0=\mathbf 1$, orthogonal
    projections $Q_n$, local operators $H_n$, an operator $H$, and a vector
    $v\perp\operatorname{ran}(G_{k+2})$ such that, with $l=0$, $n_0=1$,
    $N=k+2$, $\gamma_1=d_1=1$, and $\epsilon_0=0$, the required commutations
    hold, condition C1 holds on the range $l\leq n<N$ carried by its own
    window sum, conditions C2 and C3 hold on the range $n_0\leq n<N$ from
    ...
    \end{align}
    Hence condition C1 on $l\leq n<N$ together with conditions C2 and C3 on
    $n_0\leq n<N$, the ranges at which \cite{Nachtergaele1996Spectral}
    imposes them, do not imply the conclusion of its Theorem~2.1(i).
\end{theorem}
...
\begin{proof}
    ...
    diagonal in it. Let $G_n$ have weight $1$ at $e_i$ when $n\leq i$ and
    weight $0$ otherwise, so that $G_0=\mathbf 1$, $G_{k+2}=0$, and $E_n$ is
    the projection onto $e_n$. Let $c_0=\tfrac12$ and $c_i=1$ for $i\geq1$,
    let $H$ have weight $c_i$ at $e_i$, let $H_n$ have weight $c_n$ at $e_n$
    and weight $0$ elsewhere, and let $Q_n=\mathbf 1-E_n$. All operators are
    diagonal, so the required commutations hold, and $Q_nE_n=0$ gives
    condition C3 with $\epsilon_0=0$. Summing $H_n$ over the whole C1 range
    $0\leq n<k+2$ gives $H$ itself, and the weights $c_i$ are nonnegative, so
    condition C1 holds with $d_1=1$, with equality in its upper bound. For
    $n\geq1$ one has $c_n=1$, so
    $\lVert(\mathbf 1-Q_n)x\rVert^2=\lVert E_nx\rVert^2$ equals
    $\re\langle H_nx,x\rangle$, which is condition C2 with $\gamma_1=1$.
    Take $v=e_0$. Then $v\perp\operatorname{ran}(G_{k+2})=\{0\}$ and
    $\re\langle Hv,v\rangle=c_0=\tfrac12$, while $\lVert v\rVert^2=1$, which
...
\begin{theorem}[Full-range C1--C3 energy estimate]
    ...
    Theorem~\ref{thm:nachtergaele_c1_c3_below_threshold_energy_estimate} at
...
\begin{proof}
    ...
    Apply
    Theorem~\ref{thm:nachtergaele_c1_c3_below_threshold_energy_estimate} with
    $n_0=0$. Its added vanishing hypothesis is then vacuous, and its
    conclusion is (\ref{eq:nachtergaele_c1_c3_full_range_energy_estimate}).
\end{proof}
...
\begin{proof}
    \leanok
    \uses{thm:primitive_fnw_projector_defect_geometric, thm:wordspan_blk_inj_succ}
    The geometric estimate already supplies a positive injective length.
    Choose $p$ above it and large enough that
    $c\lambda^p<1$ and $c\lambda^p(1+c\lambda^p)/(1-c\lambda^p)<7/16$; the
    rational coefficient tends to zero because $\lambda<1$.  Block injectivity
    propagates to $p$, and the geometric whole-increment estimate applies with
    $r=K$, $m=p$, and $\ell=p$ for every prefix length $K$.
\end{proof}
