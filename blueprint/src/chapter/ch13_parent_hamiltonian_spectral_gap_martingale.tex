...
\begin{theorem}[Block injectivity from a simultaneous word span]
...
\begin{proof}
    ...
    Theorem~\ref{thm:block_injective_of_word_tuple_span}, so each boundary
...
\begin{proof}\leanok
    ...
    For every positive length $R$, the local spaces satisfy
    \begin{align}
...
\begin{theorem}[Canonical gap for every canonical decomposition]
    ...
    \leanok
    Let $A$ be a tensor in canonical form. Fix any canonical decomposition of
...
\begin{proof}\leanok
    ...
    Set $B=\bigoplus_jB_j$. Its blocks satisfy the hypotheses of the uniform
...
\begin{proof}\leanok
    ...
    Fix one canonical decomposition of $A$ and take its distinct normal
    representatives, which form a basis of normal tensors for $A$. The
    preceding theorem applies to this fixed choice for every $S$ and every
    $R\geq S+1$.
\end{proof}
