...
\begin{theorem}[Full-range C1--C3 energy estimate]
    ...
    \end{align}
    \textbf{Scope restriction (full finite range):}
    the source's C1 sum starts only at the corresponding window length, and
...
\begin{proof}
    ...
    Equation~\eqref{eq:nachtergaele_c1_c3_full_range_energy_estimate}. If
    $\epsilon_l=0$, then $c_2=0$ is not an admissible weighted parameter.
    \textbf{Local fix (zero C3 constant):} instead, C3 gives $Q_nE_n=0$, so
    the second inner product in
...
\begin{corollary}[Norm gap from the full-range C1--C3 estimate]
    \label{cor:nachtergaele_c1_c3_full_range_norm_gap}
...
\begin{proof}
    ...
    Equation~\eqref{eq:nachtergaele_c1_c3_full_range_norm_gap} by $\kappa$.
    For an
