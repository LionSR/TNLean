...
\begin{theorem}[The source conditions on their own ranges do not give the
    ...
    orthogonal projections $(G_n)_{n\geq0}$ with $G_0=\mathbf 1$ and
    $G_n\neq0$ for every $n\leq N$, nonzero orthogonal projections $Q_n$,
    local operators $H_n$, an operator $H$, and a vector
    $v\perp\operatorname{ran}(G_N)$ such that, with $l=0$, $n_0=1$,
    $N=k+1$, $\gamma_1=d_1=1$, and $\epsilon_0=0$, the required commutations
    hold, condition C1 holds on the range $l\leq n<M$ carried by its own
    window sum, both at $M=N$ and, in its non-negativity clause, at $M=n_0$,
    conditions C2 and C3 hold on the range $n_0\leq n<N$ from which they are
    imposed, and
    \begin{align}
        \re\langle Hv,v\rangle
    ...
    \end{align}
    Hence condition C1 on the range of its own window sum together with
    conditions C2 and C3 on $n_0\leq n<N$, the ranges at which
    \cite{Nachtergaele1996Spectral} imposes them, do not imply the conclusion
    of its Theorem~2.1(i), even when every ground space is nontrivial, as
    condition C2 also requires.
\end{theorem}
...
\begin{proof}
    ...
    otherwise, so that $G_0=\mathbf 1$, every $G_n$ with $n\leq N$ is nonzero
    because it retains $e_{k+1}$, $G_N$ is the projection onto $e_{k+1}$, and
    $E_n$ is the projection onto $e_n$ for $n<N$. Let $c_0=\tfrac12$, let
    $c_i=1$ for $1\leq i\leq k$, and let $c_{k+1}=0$; let $H$ have weight
    $c_i$ at $e_i$, let $H_n$ have weight $c_n$ at $e_n$ and weight $0$
    ...
    Summing $H_n$ over the whole C1 range $0\leq n<N$ gives $H$ itself, and
    the weights $c_i$ are nonnegative, so condition C1 holds with $d_1=1$,
    with equality in its upper bound, and its non-negativity clause holds over
    every initial range. For $1\leq n<N$ one has $c_n=1$, so
    ...
    $e_{k+1}$, and $\re\langle Hv,v\rangle=c_0=\tfrac12$, while
