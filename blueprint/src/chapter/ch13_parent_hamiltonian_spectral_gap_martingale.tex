...
\begin{theorem}[Tail boundary map on a prefix--suffix pair]\label{thm:tail_boundary_map_append}
    \lean{MPSTensor.tailBoundaryMap_append, MPSTensor.reassocTailBoundaryMapES_apply_threeBlock}
    \leanok
    ...
    \begin{align}
        \Gamma^{\mathrm{tail}}_{K,L}(Y)(u,\tau)
         & =\Gamma_L(Y_u)(\tau).
    \end{align}
    Grouping three successive intervals of lengths $K,L,Q$ gives
    $\Gamma^{\mathrm{tail}}_{K,L+Q}(Y)(u,w,v)=\Gamma_{L+Q}(Y_u)(w,v)$
    in the common physical coordinates.
\end{theorem}
...
\begin{theorem}[Uniform decay of the individual-block projector error]
    ...
    \leanok
    Let $A$ be a normalized primitive tensor with a positive-definite fixed matrix.
...
\begin{theorem}[Mixed boundary Gram identity]
    ...
    and let $\Gamma_A(n)$ and $\Gamma_B(n)$ be their boundary maps with
    Hilbert--Schmidt norms on the boundary matrix spaces. Define
    $\mathcal E_{BA}(X)=\sum_i B^iX(A^i)^*$ on
    $M_{D_B,D_A}(\mathbb C)$. For matrix units $E_{ab}$ and $E_{ce}$,
    \begin{align}
        \langle\Gamma_A(n)E_{ab},\Gamma_B(n)E_{ce}\rangle
         & =\bigl[\mathcal E_{BA}^n(E_{ca})\bigr]_{eb}.
    \end{align}
    Thus $\Gamma_A(n)^*\Gamma_B(n)$ is the rectangular reshuffling of
    $\mathcal E_{BA}^n$. This is the boundary-coordinate form of
...
\begin{proof}
    ...
    \end{align}
    The word expansion of $\mathcal E_{BA}^n$ gives the assertion.
\end{proof}
...
\begin{theorem}[Mixed boundary Gram decay]
    ...
    If $\mathcal E_{BA}^n(X)\to0$ for every boundary matrix $X$, then
    $\lVert\Gamma_A(n)^*\Gamma_B(n)\rVert\to0$.
    In particular this holds when the spectral radius of
    $\mathcal E_{BA}$ is less than one.
\end{theorem}
...
\begin{proof}
    ...
    $\mathcal E_{BA}$, as in equation~\textup{limP12} in the proof of the
...
\begin{theorem}[Ground-space overlap from mixed Gram bounds]
    ...
    \leanok
    Suppose that $\Gamma_A(n)$ and $\Gamma_B(n)$ are injective, and put
    $G_A(n)=\Gamma_A(n)^*\Gamma_A(n)$ and
    $G_B(n)=\Gamma_B(n)^*\Gamma_B(n)$. For vectors
    $x\in\operatorname{ran}\Gamma_A(n)$ and
    $y\in\operatorname{ran}\Gamma_B(n)$,
    \begin{align}
        |\langle x,y\rangle|
         & \leq\sqrt{\lVert G_A(n)^{-1}\rVert}\,
        \lVert\Gamma_A(n)^*\Gamma_B(n)\rVert\,
        \sqrt{\lVert G_B(n)^{-1}\rVert}\,
...
\begin{proof}
    \leanok
    Write $x=\Gamma_A(n)u$ and $y=\Gamma_B(n)v$. The left inverse
    $G_A(n)^{-1}\Gamma_A(n)^*$ has norm
    $\sqrt{\lVert G_A(n)^{-1}\rVert}$, so
    $\lVert u\rVert\leq\sqrt{\lVert G_A(n)^{-1}\rVert}\lVert x\rVert$;
    the analogous estimate holds for $v$. Apply these bounds to
    $|\langle u,\Gamma_A(n)^*\Gamma_B(n)v\rangle|$.
...
\begin{theorem}[Uniform decay of block ground-space overlaps]
    ...
    $\operatorname{spr}(\mathcal E_{BA})<1$.
    For every $\varepsilon>0$, there is $n_0$ such that for every
    $n\geq n_0$, $x\in\mathcal G_n(A)$, and $y\in\mathcal G_n(B)$,
    \begin{align}
        |\langle x,y\rangle| & \leq\varepsilon\lVert x\rVert\lVert y\rVert.
    \end{align}
    This is the identity-observable conclusion obtained from the
    spectral-radius estimate in the proof of the lemma \textup{disjoint}
...
\begin{proof}
    ...
    \uses{thm:mixed_boundary_gram_decay, thm:ground_space_overlap_mixed_gram,
        thm:primitive_ground_space_gram_inverse_tendsto,
        thm:primitive_ground_space_gram_eventual_inverse_bound}
    The self-Gram operators converge to invertible limits determined by
    $\rho$ and $\sigma$. Their inverses therefore converge and are bounded
    for all sufficiently large $n$. By
    Theorem~\ref{thm:mixed_boundary_gram_decay}, the middle factor in
    Theorem~\ref{thm:ground_space_overlap_mixed_gram} tends to zero.
    The resulting coefficient is eventually less than $\varepsilon$,
...
\begin{theorem}[Asymptotic orthogonality of inequivalent blocks]
    ...
    \uses{thm:block_ground_space_overlap_decay, thm:transfer_operator_gap_irr_tp,
        thm:transfer_operator_gap_rect_irr_tp, lem:irreducible_map_from_primitive}
    Let $A$ and $B$ be trace-preserving tensors with positive-definite
    ...
    every $x\in\mathcal G_n(A)$ and $y\in\mathcal G_n(B)$ satisfy
    \begin{align}
        |\langle x,y\rangle| & \leq\varepsilon\lVert x\rVert\lVert y\rVert.
    \end{align}
...
\begin{proof}
    ...
    irreducibility. For equal bond dimensions, inequivalence gives a strict
    mixed-transfer spectral gap. For unequal bond dimensions, a peripheral
    mixed-transfer eigenvector would force equality of the bond dimensions,
    so the gap is again strict. Apply
    Theorem~\ref{thm:block_ground_space_overlap_decay}.
\end{proof}
...
\begin{theorem}[Lower Gram estimate]
    ...
    $B\in M_m(\mathbb R)$ have zero diagonal. Suppose that
...
\begin{theorem}[Overlap with a sum of subspaces]
    ...
    Let $a\in\mathbb R^m$ and $B\in M_m(\mathbb R)$, where $B$ has zero
...
\begin{proof}
    ...
    \end{align}
    Thus the preceding Cauchy--Schwarz argument applies with lower Gram
    bound $1-\varepsilon(m-1)>0$ and
    $\lVert a\rVert=\varepsilon\sqrt m$. Finally,
...
\begin{proof}
    \leanok
    The configuration decomposition preserves the inner product and norm.
...
\begin{theorem}[Overlap of extended boundary ranges]
    ...
    and $V_A$ the range of $\Gamma^{\mathrm{tail}}_{K,L+Q}(A)$,
...
\begin{proof}
    \leanok
    \uses{thm:ph_middle_interval_overlap, thm:tail_boundary_map_append, thm:left_boundary_map_append}
...
\begin{proof}
    \leanok
    \uses{thm:ph_extended_boundary_overlap, thm:ph_joint_projector_comparison, thm:spectator_boundary_hilbert_factorization}
...
\begin{proof}
    ...
    $\eta/(r+1)$. Taking adjoints equates the two orders of each individual
    projector error. The interval comparison therefore gives
    \begin{align}
        \lVert R-PQ\rVert
...
\begin{theorem}[Uniform projector error decay for primitive blocks]
    ...
    with positive bond dimensions and positive definite invariant densities.
    Suppose no two distinct members are related by a gauge transformation
    and a phase. Use the interval projectors of
...
\begin{theorem}[Interval spaces of a block-diagonal tensor]
    ...
    \end{align}
    These identities also hold under the natural reassociation of three
    consecutive intervals of lengths $K,L,Q$.
\end{theorem}
\begin{proof}
    \leanok
    \uses{lem:block_sum_local_ground_space}
    The local identity is $G_L(B)=\sum_jG_L(A_j)$.
    Expand a vector on the free sites in their finite coordinate basis.
    Interchanging the sum over the coordinates with the sum over the blocks
    gives both assertions. The linear isometry reassociating three
    consecutive intervals preserves sums of subspaces.
\end{proof}
...
\begin{proof}
    ...
    \uses{thm:ph_primitive_block_interval_defect_decay,
        lem:block_sum_local_ground_space, thm:ph_block_sum_interval_spaces}
    The three ground spaces of $B$ are the sums of the corresponding block
    ground spaces. Their orthogonal projectors are therefore precisely the
    three joint projectors in the primitive-block decay estimate. Apply
    that estimate with the prescribed $\eta$.
\end{proof}
...
\begin{lemma}[Locality and the initial grouped differences]
    ...
    \leanok
    Set \(E_n=G_n-G_{n+1}\). For \(0\leq m,n<M\), one has
    \[
        [E_m,Q_n]=0\qquad\text{if }m<n-1\text{ or }m>n.
    \]
    Moreover, \(E_0=0\) and \(Q_1E_1=0\).
\end{lemma}
\begin{proof}
    \leanok
...
\begin{theorem}[Grouped open-chain coercivity]
    ...
    projections above. Suppose that
    \[
        \|Q_nE_n\|\leq\epsilon<\frac1{\sqrt2}
        \qquad(0\leq n<M),
        \qquad \epsilon\geq0.
    \]
    Then, for every \(v\perp\ker H_N\),
    \[
...
\begin{proof}
    \leanok
    \uses{lem:grouped_open_ground_projection_locality,thm:nachtergaele_c1_c3_full_range_norm_gap}
    Apply the full-range martingale estimate to the grouped filtration,
    with window width one. Each local window Hamiltonian is the projection
    \(I-Q_n\), so condition C2 has constant one. The nonzero local window
    terms are distinct summands of \(H_N\); positivity gives condition C1
    with constant one. The locality lemma supplies the commutation
    condition. The assumed norm bound supplies C3, and the terminal
    projection is the projection onto \(\ker H_N\).
\end{proof}

\begin{theorem}[Energy counting for grouped open windows]
...
\begin{theorem}[Unit local gap for grouped open windows]
...
\begin{proof}
    ...
    Apply the full-range local gap bound with $l=2p-1$ and index
    $p(n+1)-1<N$.
\end{proof}

\begin{theorem}[Prefix ground spaces with free right sites]
...
\begin{proof}
    \leanok
    Decompose a vector according to the last $r$ physical coordinates.
    The prefix Hamiltonian acts on each component by the length-$n$ open
    Hamiltonian, so its kernel is characterized component by component.
\end{proof}

\begin{theorem}[Reassociated suffix ground space]
...
\begin{proof}
    \leanok
    Reassociation preserves the ordered physical coordinates.
    A parent interaction is the orthogonal projection onto the complement
    of the local boundary-condition space, acting trivially on the other sites.
\end{proof}

\begin{theorem}[Active grouped projector error]
    ...
    sites, and let $F$ project onto the whole-chain ground space. Then
    \begin{align}
        \|T(P-F)\|=\|F-PT\|.
        \notag
    \end{align}
    The right-hand side is the three-interval projector error in
...
\begin{theorem}[Periodic gaps from divisible open intervals]
...
\begin{proof}
    ...
    Choose $n$ with $(R-1)^2<n\gamma$ and then choose $M\geq n+2R$
    for which the open-chain bound holds. Set $m=pM-R+1$. Then
    $m\geq\max\{n,R\}$ and the active open volume is $m+R-1=pM$.
    The finite-range Knabe comparison gives the positive constant
    $\delta=(m\gamma-(R-1)^2)/(m-R+1)$ and the asserted bound for every
    $N\geq2m$.
\end{proof}

\begin{theorem}[Spectator extension of a grouped projection difference]
...
\begin{proof}
    ...
    Splitting the configuration into its active and spectator coordinates
    conjugates each of the three Hamiltonians to its fiberwise extension.
    Kernel projections commute with this conjugation and with fiberwise
    extension. Composition and subtraction also commute with fiberwise
    extension. Conjugation preserves the operator norm, and
    fiberwise extension does not increase it.
\end{proof}

\begin{theorem}[Grouped C3 from a three-interval estimate]
    \label{thm:grouped_c3_from_projector_defect}
    \lean{MPSTensor.grouped_martingaleDifference_norm_le_of_projector_defect,
        MPSTensor.grouped_martingaleDifference_norm_le_of_two_le}
    \leanok
...
\begin{proof}
    ...
    The products at indices zero and one vanish. For $n\geq2$,
    put $K=p(n-1)$. The active interval has length $p(n+1)$,
    and its preceding prefix has length $pn$. The kernel hypothesis
    and the active projector identity give the desired bound on this
    interval. Adjoining the remaining $pM-p(n+1)$ sites does not
    increase the operator norm.
\end{proof}

\begin{theorem}[Uniformly gapped interaction ranges for primitive block sums]
    ...
    \leanok
    Let $A_1,\ldots,A_r$ be normalized primitive tensors with
...
\begin{proof}
    ...
    \uses{thm:ph_primitive_blocks_open_kernel,
        ...
        thm:grouped_open_parent_gap,thm:periodic_gap_from_divisible_open}
    Choose $p$ above the prescribed bound and the open-kernel threshold,
    and sufficiently large that the three-interval projector error is at
    most $1/2$, uniformly in the two outer lengths. Set $R=2p$.
    The preceding theorem gives grouped C3 with $\epsilon=1/2$.
    Since $1/2<1/\sqrt2$, the grouped open-chain estimate gives the
    positive coefficient $(1-\sqrt2/2)^2$ at every length $pM$
    with $M\geq2$. The finite-range comparison then supplies a positive
    periodic gap at every sufficiently large length.
\end{proof}

\begin{theorem}[Operator order after adjoining free sites]
...
\begin{proof}
    \leanok
    Expand the inner product in the product orthonormal basis and sum first
    over the additional coordinates. Positivity follows term by term;
    the order assertion follows by applying positivity to $U-T$.
    The scalar identity follows on each coordinate. Unitary conjugation
    preserves and reflects positivity, hence also operator order.
\end{proof}
...
\begin{lemma}[Comparison on a finite interval]
    ...
    Suppose that the kernel of the open parent Hamiltonian $K_{R,W}$ is
...
\end{lemma}
\begin{proof}\leanok
    The restriction of $K_{R,W}$ to its kernel complement is injective.
    Finite dimensionality therefore gives a positive lower bound for its
    smallest singular value. Positivity turns this into the lower
    quadratic-form bound. The operator norm gives the upper bound.
    Both quadratic forms vanish on the common kernel.
\end{proof}

\begin{lemma}[Comparison with the kernel projection]
    \label{lem:positive-kernel-projection-comparison}
    \lean{LinearMap.IsPositive.exists_pos_smul_orthogonal_ker_projection_le, LinearMap.IsPositive.exists_pos_le_smul_orthogonal_ker_projection}
    \leanok
    Let $H$ be a positive operator on a finite-dimensional complex Hilbert
    space. If $P$ is the orthogonal projection onto $(\ker H)^\perp$,
    then $aP\leq H\leq bP$ for some $a,b>0$.
\end{lemma}
\begin{proof}\leanok
...
\begin{theorem}[Transfer to a prescribed interaction range]
    \label{thm:parent-gap-fixed-range-transfer}
    \lean{MPSTensor.exists_parentHamiltonianES_gap_of_larger_range}
    \leanok
    \uses{lem:parent-finite-interval-comparison, thm:periodic-parent-range-comparison, lem:positive-comparison-kernel-gap}
...
\end{theorem}
\begin{proof}\leanok
    Choose $\kappa,C>0$ such that
    $\kappa h_W\leq K_{R,W}\leq Ch_W$. Summing all cyclic translates
    gives
    \begin{align}
        \kappa H_{W,N}\leq (W-R+1)H_{R,N}\leq C H_{W,N}
        \notag
    \end{align}
    for sufficiently large $N$. The periodic kernels therefore agree.
    A gap $\gamma$ at range $W$ transfers to the gap
...
\begin{theorem}[Periodic comparison from an open interval]
...
\begin{proof}\leanok
    \uses{lem:full-interval-parent-interaction,
        thm:cyclic_active_block_local_conjugacy,
        thm:cyclic_knabe_block_open_parent_hamiltonian}
    Extend each local inequality by the identity on the complementary sites
    and conjugate by the cyclic identification of the chosen interval.
    Both operations preserve operator order. Summing over all cyclic
    origins gives the asserted bounds: each range-$R$ interaction occurs
    exactly $m$ times, once for each of its possible positions in the
    open interval.
\end{proof}

\begin{theorem}[Uniform gap above an interaction threshold]
...
\begin{proof}\leanok
    ...
    Choose $R_0$ so that every open parent Hamiltonian of range at least
    $R_0$ has the expected local ground space. Fix $R\geq R_0$, and choose
    a uniformly gapped range $W\geq2R$. On $W$ sites, the range-$R$
    Hamiltonian and the orthogonal projection onto the complement of the local
...
\begin{theorem}[Gap at the common block-injectivity range]
    ...
    \leanok
    Let $B=\bigoplus_{j=1}^r\mu_j A_j$, where $r\geq2$, all coefficients
...
\begin{proof}\leanok
    ...
    $R$ with the local MPS space. Choose a uniformly gapped range
...
\begin{theorem}[Positive gap at a fixed length]
...
\begin{proof}
    ...
    is injective. Its inverse on its image is bounded, since the domain is
    finite dimensional. Thus $\lVert v\rVert\leq C_N\lVert
        H_{N,L}^{\mathrm{ES}}(A)v\rVert$ for some $C_N>0$; take
    $\delta_N=C_N^{-1}$. If the kernel complement is zero, any positive
    constant suffices.
\end{proof}

\begin{theorem}[Extension of an eventual uniform gap]
...
\begin{proof}
    ...
    Take the minimum of $\delta$ and the finitely many positive constants
    $\delta_N$ supplied by Theorem~\ref{thm:parent_hamiltonian_fixed_length_gap}
    for $N<M$. Equivalently, induct on $M$, replacing $\delta$ by
    $\min\{\delta,\delta_{M-1}\}$ at each step.
\end{proof}

\begin{theorem}[Gap uniform in every chain length]
    ...
    \leanok
    Let $B=\bigoplus_{0\leq j<r}\mu_j A_j$, where $r\geq2$, all coefficients
    are nonzero, and the normalized primitive tensors $A_j$ are pairwise
    inequivalent under similarity and phase. Assume positive physical and bond
    dimensions, positive definite recorded invariant densities, and a common
    block-injectivity length $L_0\geq1$. For every
...
\begin{theorem}[Nonzero eigenvalues of the transfer remainder]
    \label{thm:fnw-transfer-eigenvalue-from-remainder}
    \lean{MPSTensor.IsPrimitiveMPS.fnwTransfer_hasEigenvalue_of_remainder}
    \leanok
    Let $A$ be primitive with positive bond dimension, let $E$ be its observable
    transfer map, and let $P$ be its stationary projection. Every nonzero
...
\begin{theorem}[FNW decay rates from transfer eigenvalues]
    ...
    \leanok
    Let $A$ be primitive with positive bond dimension and positive-definite
    stationary density $\rho$. Suppose $\lambda>0$ and every eigenvalue
    $\nu\neq1$ of its observable transfer map satisfies $|\nu|<\lambda$.
    Then the spectral radius of the transfer remainder in the $\rho$-weighted
    norm is less than $\lambda$. If $\tr\rho=1$, there is a constant
    $C_\lambda>0$ such that, for every integer $n\geq1$,
    \begin{align}
        a(n)\leq C_\lambda\lambda^n.
    \end{align}
    This establishes the eigenvalue prescription following
    \cite[Section~6, equation~(6.1)]{Nachtergaele1996Spectral}. % The dimension-only constant in the source projector estimate remains separate.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Prescribed FNW prefactors after an overlap threshold]
    ...
    \leanok
    Let $A$ be primitive with positive bond dimension and a positive-definite
    trace-one stationary density $\rho$. Suppose $\lambda>0$ exceeds the modulus
...
\begin{proof}\leanok
    ...
    Choose an intermediate rate $\mu$ strictly between the remainder spectral
...
\begin{theorem}[Eventual projector bound with a prescribed prefactor]
    ...
    For every $c>0$, there exists an onset $M$ such that, for every overlap
    ...
    \begin{align}
        ...
        -P_{\mathcal G_{r+m+\ell}}\right\|
        \leq \frac{c\lambda^m(1+c\lambda^m)}{1-c\lambda^m}.
    \end{align}
    The onset depends on the state, rate, and prefactor; it is not asserted to
    be the injectivity threshold. This is the version of
    \cite[Section~6, equation~(6.1)]{Nachtergaele1996Spectral} with the additional
    onset stated explicitly, as documented in the martingale-overlap paper-gap note.
\end{theorem}
\begin{proof}\leanok
    \uses{thm:fnw-eventual-prescribed-prefactor,thm:fnw_projector_defect_geometric}
    Increase the onset until the overlap exceeds an injectivity length and
    $c\lambda^m<1$. The preceding mixing estimate then satisfies the hypotheses
    of the fixed-overlap FNW projector estimate, uniformly in the two exterior
    lengths.
\end{proof}
...
\begin{definition}[Weighted matrix-unit path vectors]
    ...
    \lean{MPSTensor.FNWDimensionConstant.matrixUnitTensor,MPSTensor.FNWDimensionConstant.pathConfig,MPSTensor.FNWDimensionConstant.pathVector}
    \leanok
...
\begin{theorem}[Path contractions]
    ...
    \end{align}
    A factor depending on $b$ acts on the first physical site, and a
    factor depending on $e$ acts on the last physical site.
\end{theorem}
\begin{proof}\leanok
    \uses{def:weighted-matrix-unit-path-vectors}
    Distinct vertex paths give distinct physical basis vectors. Their
    inner products therefore vanish. The product of four consecutive
    matrix units is the matrix unit joining the two endpoints, which
    gives the second identity after taking the boundary trace.
\end{proof}

\begin{theorem}[Spectator multipliers preserve interval spaces]
    ...
    \leanok
    Multiplying an open-boundary vector by a function of the sites
    outside a prefix or tail interval preserves membership in that interval's
    ground space tensored with the spectator space.
\end{theorem}
\begin{proof}\leanok
    Factor the matrix product at the boundary of the interval. The
    multiplier is absorbed into the boundary matrix indexed by the
    spectator configuration.
\end{proof}

\begin{theorem}[Matrix-unit boundary vectors]
    \label{thm:matrix-unit-path-ground-space}
    \lean{MPSTensor.FNWDimensionConstant.pathVector_one_eq_groundSpaceMap}
    \leanok
    Let $t$ be a real matrix indexed by a finite set, and let the physical
    letters be ordered pairs of indices. Set $A_{ij}=t_{ij}E_{ij}$.
    For fixed initial and terminal indices $a,f$, define
    \begin{align}
        v_{a,f}
         & = \sum_{b,c,e} t_{ab}t_{bc}t_{ce}t_{ef}
        \lvert(a,b),(b,c),(c,e),(e,f)\rangle.
    \end{align}
    Then $v_{a,f}=\Gamma_4(E_{fa})$, where
    $\Gamma_4(X)$ has coordinates $\operatorname{tr}(A_{i_0}A_{i_1}A_{i_2}A_{i_3}X)$.
\end{theorem}
\begin{proof}
    \leanok
    \uses{def:weighted-matrix-unit-path-vectors}
    The product of four matrix units is zero unless consecutive edges join.
    For a path from $a$ to $f$, multiplication by $E_{fa}$ and taking the trace
    leaves precisely the product of its four edge weights. Summing over the
    three internal vertices gives the assertion.
\end{proof}

\begin{theorem}[Normalization and orthogonality of marked paths]
...
\begin{proof}\leanok
    ...
    Expand its self-inner product and its inner product with an arbitrary
    boundary vector. The latter equals the indicated scalar sum multiplied
    by the boundary matrix entry at $(f,a)$.
\end{proof}

\begin{theorem}[A matrix coefficient bound for a projection defect]
...
\begin{proof}\leanok
    ...
    equals $\langle y,(P_UP_V-P_W)x\rangle$. Apply Cauchy--Schwarz and the
    operator norm bound. The two operator norms agree by taking adjoints.
\end{proof}

\begin{theorem}[The dimension-four coefficient at overlap two]
...
\begin{theorem}[A positive Markov matrix with a nilpotent remainder]
    \label{thm:fnw_dimension_markov_matrix}
    \lean{MPSTensor.FNWDimensionConstant.markovMatrix_pos, MPSTensor.FNWDimensionConstant.markovMatrix_sum_identities, MPSTensor.FNWDimensionConstant.markovMatrix_pow_two, MPSTensor.FNWDimensionConstant.markovMatrix_pow_three, MPSTensor.FNWDimensionConstant.remainderMatrix_nilpotent}
    \leanok
    Let $e=(1,1,1,1)^{\mathsf T}$ and let
    $u=(1,1,-1,-1)^{\mathsf T}$, $v=(1,-1,1,-1)^{\mathsf T}$,
    $w=(1,-1,-1,1)^{\mathsf T}$. Put $Q=ee^{\mathsf T}/4$ and
    $P=Q+(uv^{\mathsf T}+vw^{\mathsf T})/16$.
    Then $P$ is strictly positive and doubly stochastic, and
    \begin{align}
        P^2     & =Q+uw^{\mathsf T}/64, & P^3            & =Q,   \\
        Pu      & =0,                   & w^{\mathsf T}P & =0,   \\
        (P-Q)^3 & =0,                   & (P-Q)^2        & \ne0.
    \end{align}
    This is an explicit construction for examining the prescribed-rate coefficient
    in \cite[Section~6, equation~(6.1)]{Nachtergaele1996Spectral}.
\end{theorem}
\begin{proof}
    \leanok
...
\begin{theorem}[Scalar path sums]
    \label{thm:fnw_dimension_markov_path_sums}
    \lean{MPSTensor.FNWDimensionConstant.realMarkovMatrix_path_sum, MPSTensor.FNWDimensionConstant.realMarkovMatrix_path_hadamardU_sum, MPSTensor.FNWDimensionConstant.realMarkovMatrix_path_hadamardUW_sum, MPSTensor.FNWDimensionConstant.markovAmplitude_sq, MPSTensor.FNWDimensionConstant.markovAmplitude_pos}
    \leanok
    For the matrix $P$ above and every pair of endpoints $a,f$, set
    ...
    \end{align}
    Moreover $t_{ij}=\sqrt{P_{ij}}$ is strictly positive and $t_{ij}^2=P_{ij}$.
\end{theorem}
\begin{proof}
    \leanok
    \uses{thm:fnw_dimension_markov_matrix}
    Apply the identities for $P^2$ and $P^3$, and use $u_b^2=w_e^2=1$.
    The assertions concerning square roots follow from positivity.
\end{proof}

\begin{theorem}[A dimension-four physical projection defect]
...
\end{theorem}
\begin{proof}\leanok
    \uses{thm:weighted-matrix-unit-path-contractions,thm:matrix-unit-path-ground-space,thm:spectator-multiplier-interval-space,thm:fnw-path-vector-norm-orthogonal,thm:fnw-projection-witness-bound,thm:fnw_dimension_markov_path_sums}
    ...
    \end{align}
    The mark $u_b$ acts on the first physical site, so $x$ belongs to
    the range of $G_{[1,3]}$. Similarly, $y$ belongs to the range of
    $G_{[0,2]}$. The path contraction identities give
...
\begin{theorem}[Weighted matrix-unit normalization]
...
\begin{theorem}[Transfer map of a weighted matrix-unit tensor]
...
\begin{proof}
    \leanok
    Multiply the matrix units in $\sum_{i,j}A_{ij}XA_{ij}^{\dagger}$.
    Only the diagonal entries of $X$ contribute.
\end{proof}

\begin{theorem}[The positive Markov matrix-unit channel]
    \label{thm:fnw_dimension_markov_channel}
    \lean{MPSTensor.FNWDimensionConstant.markovTensor_isInjective, MPSTensor.FNWDimensionConstant.markovTensor_isTP, MPSTensor.FNWDimensionConstant.markovTensor_transferMap_one, MPSTensor.FNWDimensionConstant.markovTensor_transferMap_cube}
    \leanok
    For the four-dimensional matrix $P$ above, let
    ...
    \end{align}
    These are the channel properties of the explicit construction used to examine
    \cite[Section~6, equation~(6.1)]{Nachtergaele1996Spectral}.
\end{theorem}
\begin{proof}
    \leanok
...
\begin{theorem}[A faithful primitive fixed point]
    \label{thm:fnw-dimension-markov-primitive}
    \lean{MPSTensor.FNWDimensionConstant.markovDensity_posDef,MPSTensor.FNWDimensionConstant.markovDensity_trace,MPSTensor.FNWDimensionConstant.markovDensity_fixed,MPSTensor.FNWDimensionConstant.markovTensor_isPrimitiveMPS,MPSTensor.FNWDimensionConstant.markovTensor_fnwTransfer_eigenvalue_eq_zero}
    \leanok
...
\begin{theorem}[Failure of the dimension-only coefficient at a prescribed rate]
    ...
    \end{align}
    Thus the dimension-only coefficient and the prescribed-rate assertion
    following Nachtergaele's bound cannot hold simultaneously at this fixed
    overlap length; see \cite{gap:cpgsv21_martingale_overlap}.
\end{theorem}
\begin{proof}\leanok
    \uses{thm:fnw-dimension-four-physical-defect,thm:fnw-dimension-four-coefficient,thm:fnw-dimension-markov-primitive}
    Take the weighted matrix-unit tensor associated with the transition
    matrix above and the stationary density $I/4$. Its transfer map becomes
...
\begin{theorem}[Separation from simultaneous injectivity]
    \label{thm:block-word-span-separation}
    \lean{MPSTensor.not_gaugePhaseEquiv_of_wordTupleSpanTop}
    \leanok
    Let $A^0,\ldots,A^{r-1}$ be tensors with positive bond dimensions. Suppose that
    their simultaneous word evaluations at length $S$ span the product of their
    matrix algebras. Then no two distinct blocks are gauge-phase equivalent.
    This is a consequence of the simultaneous block-injectivity condition in
...
\begin{proof}
    ...
    boundary map at that length is injective, so its value at the identity matrix,
    the periodic matrix product vector, is nonzero. Gauge-phase equivalent blocks
    would have proportional periodic vectors, contradicting independence.
\end{proof}

\begin{theorem}[A uniform gap at the simultaneous injectivity range]
    ...
    Suppose their word tuples of a supplied positive length $S$ span the full
    product matrix algebra. Let $B=\bigoplus_{j=0}^{r-1}\mu_jA^j$, where every
    $\mu_j$ is nonzero. For each interaction range $R\geq S+1$, there is a
    constant $\gamma_R>0$ such that
    \begin{align}
    ...
    Hamiltonian of $B$.
    This is the block-injective gap assertion of
...
\begin{proof}
    ...
    Normalize the blocks while retaining simultaneous injectivity at $S$ and
    all local ground spaces. Simultaneous injectivity forces distinct blocks
    to be inequivalent. The grouped martingale estimate and finite-range
    periodic comparison provide a gapped interaction range $W\geq2R$.
    The open range-$R$ Hamiltonian on $W$ sites has precisely the full local
    MPS space as its kernel, since $R\geq S+1$. Two-sided comparison of the
...
\begin{theorem}[Canonical parent Hamiltonians and normal representatives]
...
\begin{proof}\leanok
    ...
    The positive-length local space of $A$ is the sum of the local spaces
    of its normal representatives, which is also the local space of $B$.
...
\begin{theorem}[Canonical gap at a supplied simultaneous injectivity length]
    ...
    \lean{MPSTensor.CPSVCanonicalFormData.exists_parentHamiltonianES_uniform_gap_of_wordTupleSpanTop,
        MPSTensor.IsCPSVCanonicalForm.exists_bnt_uniform_gap_of_wordTupleSpanTop}
    \leanok
    Let $A$ be a tensor in canonical form. One may choose a basis of normal
    tensors $B_0,\ldots,B_{g-1}$ such that the following holds for every positive
    integer $S$: if the simultaneous length-$S$ word tuples span the product
    of the block matrix algebras, then, for every $R\geq S+1$, there exists
    $\gamma>0$ such that, for every chain length $N$ and every vector
    $v\perp\ker H_{R,N}(A)$,
    \begin{align}
        \gamma\|v\|\leq\|H_{R,N}(A)v\|.\notag
    \end{align}
    The representatives are chosen once, before $S$ and $R$.
    This includes canonical forms with repeated blocks, as in
    \cite[Section~IV.C]{Cirac2021Matrix}.
\end{theorem}
\begin{proof}\leanok
    \uses{thm:canonical-parent-hamiltonian-representatives,thm:block-parent-uniform-gap-simultaneous-injectivity}
    Choose the normal representatives of a canonical decomposition. Their
    direct sum has a positive uniform gap at every range $R\geq S+1$ by
    simultaneous injectivity. The preceding Hamiltonian equality transfers
    the same constant to the original tensor, without changing the range or
    the chain length.
\end{proof}

\begin{theorem}[Comparison of positive parent interactions]
    ...
    \cite[Section IV.C]{Cirac2021Matrix}. Write $q_R=I-P_{G_R(A)}$ for the
...
\begin{proof}\leanok
    ...
    Apply the finite-dimensional comparison with the projection onto the
    orthogonal complement of the kernel of $h$. This projection is $q_R$.
\end{proof}
...
\begin{theorem}[Order and positivity of periodic interaction sums]
...
\begin{theorem}[Canonical parent projections give the canonical Hamiltonian]
...
\begin{theorem}[Uniform comparison of periodic parent Hamiltonians]
    ...
    $G_R(A)$. Write $H_N$ for the canonical periodic parent Hamiltonian.
    There are $\kappa,C>0$, independent of $N$, such that
    \begin{align}
        \kappa H_N\leq H_N(h)\leq C H_N.
    \end{align}
\end{theorem}
\begin{proof}
    \leanok
    \uses{thm:parent-interaction-comparison,thm:periodic-fixed-interaction-order,thm:periodic-fixed-interaction-canonical}
    Extend the two local operator inequalities by the identity, conjugate by the
    cyclic coordinate identifications, and sum over all translated intervals.
\end{proof}

\begin{theorem}[Uniform gap for a fixed positive parent interaction]
...
\begin{proof}
    ...
    $\kappa P_R\leq h\leq C P_R$, where $P_R$ is the canonical parent projection.
    ...
    equality of the kernels. Restriction to their common orthogonal complement
    therefore transfers a canonical gap $\gamma$ to the gap $\kappa\gamma$.
    This is the projector reduction of \cite[Section~IV.C]{Cirac2021Matrix}.
\end{proof}

\begin{theorem}[Positive parent interactions at the simultaneous injectivity range]
    \label{thm:canonical-positive-parent-gap-simultaneous-injectivity}
    \lean{MPSTensor.CPSVCanonicalFormData.exists_parentInteraction_uniform_gap_of_wordTupleSpanTop,MPSTensor.IsCPSVCanonicalForm.exists_bnt_parentInteraction_uniform_gap_of_wordTupleSpanTop}
    \leanok
    Let $A$ be a tensor in canonical form, with its multiplicities retained, and
    choose its normal representatives $B_j$, indexed by $0\leq j<r$.
    Suppose that $S>0$ and the tuples of length-$S$ words span the direct sum of
    the matrix algebras of these representatives. For every $R\geq S+1$ and
    ...
    \end{align}
    The representatives can be chosen once, independently of $S$, $R$, and $h$.
\end{theorem}
\begin{proof}
    \leanok
    \uses{thm:canonical-gap-simultaneous-injectivity,thm:positive-parent-interaction-uniform-gap}
    Apply the canonical projector gap at range $R$, then the comparison theorem
    for the fixed interaction $h$. The constants depend on $h$ but not on $N$.
\end{proof}
