...
\begin{lemma}[Physical Gram convergence]
    \label{lem:physical_gram_convergence}
    \notready
    \discussion{5883}
    \uses{def:fixed_point_proj, thm:ground_space_gram_eq_gram_reshuffle,
        thm:gram_reshuffle_norm_sq_le_card_cube_mul_operator_norm_sq}
    Let $\mathcal E_A$ be trace preserving, let
    $\mathcal E_A(\rho)=\rho$ with $\tr(\rho)\ne0$, and suppose that the
    spectral radius of $\mathcal E_A-P_\rho$ is strictly smaller than one.
    Then the physical Gram operators
    converge to the reshuffled fixed-point projection:
    \begin{align}
        \mathcal K_n
        &\longrightarrow
        \mathscr R(P_\rho),
        ...
        \notag
    \end{align}
    The quantitative estimate is obtained from geometric decay of the
    complementary transfer powers and the dimension-explicit reshuffling bound.
    In the unweighted Frobenius boundary coordinates the limit is generally not
    the identity.
\end{lemma}
...
\begin{lemma}[Limiting Gram metric and inverse-Gram control]
    ...
    \notready
    \discussion{5922}
    \uses{lem:physical_gram_convergence, def:inverse_gram,
...
\begin{lemma}[Uniform FNW overlapping-window contraction]
    ...
    For an injective blocked tensor, let $P^{\mathrm{tail}}_{K,l+1}$ and
...
\begin{lemma}[Nachtergaele condition C3 at a blocked length]
    ...
    There is a positive blocked overlap length $l$ for which the uniform
    projector defect $\varepsilon_l$ satisfies
    \begin{align}
...
\begin{lemma}[Blocked open-chain input for the cyclic martingale condition]
    ...
    Let $B$ be the injective tensor obtained from $A$ by the chosen blocking,
    and let $L>1$ be the resulting cyclic interaction range.  The blocked
    open-chain estimate of
    Lemma~\ref{lem:nachtergaele_c3_blocked_length} implies, uniformly for
    $N\ge2L$ and every overlapping off-diagonal pair of cyclic windows, that
    \begin{align}
        h_i h_j+h_jh_i
        &\ge
        ...
        \notag
    \end{align}
    in quadratic form.  The comparison includes the windows crossing the
    periodic cut and keeps the blocking unit explicit.  This is a consequence
    of the reduced ground-space projector defect; it does not use an all-vector
    excitation-compression estimate.
\end{lemma}
