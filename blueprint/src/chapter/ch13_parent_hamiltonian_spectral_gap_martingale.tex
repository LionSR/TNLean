...
\begin{lemma}[Right-spectator extension]
    ...
    \leanok
    Let $T$ be an operator on a finite-dimensional active Hilbert space and let
    $S$ be a finite set of right-spectator configurations.  The operator
...
\begin{theorem}[Nonwrapping interactions with right spectators]
    ...
    \leanok
    \uses{lem:right-spectator-extension, lem:right-spectator-kernel-projection}
...
\begin{theorem}[Fixed-volume C3 ground projections with right spectators]
    ...
    Suppose that $A$ is block injective at length $L_0>0$ and
    $L_0+1\leq n$.  The kernel of the range-$(L_0+1)$ prefix Hamiltonian on
    the first $n$ sites of the $(n+1)$-site volume is the left open-chain
    ground space.  The kernel of the full final length-$L$ suffix window is
    the tail open-chain ground space.  After adjoining $r$ right spectators,
    their orthogonal projections satisfy
    \begin{align}
        U G_{[0,n)}^{(n+1+r)}U^{-1}
         & =(G_{[0,n)}^{(n+1)})^{\oplus[d]^r},       \\
        U G_{[K,K+l+1)}^{(K+l+1+r)}U^{-1}
         & =(G_{[K,K+l+1)}^{(K+l+1)})^{\oplus[d]^r}.
    \end{align}
\end{theorem}
...
\begin{proof}
    ...
    The full suffix interval contains one interaction; its cyclic restriction
...
\begin{theorem}[C3 boundary indices in the fixed final volume]
    ...
    \leanok
    \uses{def:fixed-volume-prefix-ground-projections}
    Take interaction length $L=l+1$ and define
    $E_n=G_n^{(N)}-G_{n+1}^{(N)}$.  Then the two boundary ranges in the
...
\begin{proof}
    \leanok
    \uses{def:fixed-volume-prefix-ground-projections}
    If $n<l$, neither prefix $[0,n)$ nor $[0,n+1)$ contains a full
...
\begin{theorem}[Norm bound for the whole-increment centered projector sandwich]
    ...
    \leanok
    \uses{def:whole_increment_centered_projector_sandwich, def:whole_increment_centered_residual, def:inverse_gram, def:ground_space_map_es}
...
\begin{proof}
    \leanok
    \uses{def:whole_increment_centered_projector_sandwich, thm:reassociated_tail_inverse_gram, thm:spectator_boundary_direct_sum_gram, thm:reassociated_tail_pseudoinverse_bound, thm:left_spectator_pseudoinverse_bound}
...
\begin{definition}[Centered physical C3 sandwich]
    ...
    \leanok
    \uses{def:c3_centered_overlap_factors_residual, thm:tail_boundary_map_factorization, thm:left_boundary_map_factorization}
...
\begin{theorem}[Physical C3 assembly and centered bound]
    ...
    \leanok
    \uses{def:c3_centered_physical_sandwich, def:c3_correction_sandwiches, def:open_chain_ground_subspaces_es, def:ground_space_es}
...
\begin{proof}
    \leanok
    \uses{thm:c3_projector_gram_error_telescope, thm:spectator_boundary_direct_sum_gram, thm:tail_spectator_pseudoinverse_bound, thm:left_spectator_pseudoinverse_bound}
...
\begin{theorem}[Spectator-uniform geometric C3 projector defect]
    ...
    \leanok
    \discussion{6134}
    \uses{def:primitive_mps, def:open_chain_ground_subspaces_es, def:ground_space_es}
...
\begin{theorem}[Prefix-uniform geometric whole-increment projector defect]
    ...
    \leanok
    \uses{def:primitive_mps, def:reassociated_tail_boundary_map, def:spectator_boundary_maps_es, def:ground_space_es}
    Let $A$ be primitive with positive-definite fixed point $\rho$ and nonzero
    bond dimension.  There are constants $C,c,r>0$, with $r<1$, such that for
...
\begin{proof}
    \leanok
    \uses{thm:primitive_ground_space_map_geometric_inverse_gram_bounds, thm:primitive_tail_virtual_map_norm_uniform, thm:fixed_point_gram_metric_is_unit, thm:primitive_whole_increment_centered_residual_bound, thm:whole_increment_left_virtual_norm, thm:whole_increment_centered_projector_sandwich_bound, thm:whole_increment_correction_bounds, thm:whole_increment_finite_gram_corrections, thm:injective_range_projector_inverse_gram, thm:range_ground_space_map_es}
...
\begin{theorem}[Uniform whole-increment defect at the coefficient $7/16$]
    ...
    \leanok
    \uses{thm:whole_increment_ground_projection_defect_geometric, thm:normal_from_primitive_posdef, thm:wordspan_blk_inj_succ}
    Let $A$ be primitive with positive-definite fixed point $\rho$ and nonzero
...
\begin{proof}
    \leanok
    \uses{thm:fnw_overlapping_window_contraction, thm:normal_from_primitive_posdef, thm:wordspan_blk_inj_succ, thm:nachtergaele_open_chain_projector_defect}
...
\begin{theorem}[Original-site gap on block-divisible periodic chains]
    ...
    \leanok
    \uses{thm:martingale_mps, lem:positive_gap_transfer_loewner, lem:blocked_original_aligned_sparse_sum}
...
\begin{theorem}[Uniform gap for normal MPS parent Hamiltonians]
    ...
    \notready
    \discussion{5926}
    \uses{thm:martingale_mps_original_site_multiples, thm:anticommutator_gap_bound, thm:parent_hamiltonian_gapped}
...
\begin{theorem}[Gap bound from cyclic overlap compression]
    ...
    \leanok
    \uses{def:parent_ground_space_es, def:parent_hamiltonian_es, thm:parent_ground_space_es_kernel, rem:euclidean_local_projector_ingredients, lem:local_term_es_average, lem:transported_es_positive, lem:transported_es_local_projections, lem:transported_es_nonoverlap_positive, lem:parent_hamiltonian_es_quadratic_reduction, lem:parent_hamiltonian_ordered_rowsum_gap, lem:parent_hamiltonian_finite_overlap_ordered_cross_term_quadratic, lem:parent_hamiltonian_finite_overlap_ordered_cross_term_gap, lem:cyclic_window_overlap_cardinality, lem:cyclic_window_nonoverlap_positive, lem:parent_hamiltonian_cyclic_window_ordered_cross_term_gap, lem:parent_hamiltonian_cyclic_overlap_norm_gap, thm:overlap_norm_gap_bound_constant}
...
\begin{proof}
    ...
    sufficient norm-compression formulation. It is the constant-compression
    statement of Theorem~\ref{thm:overlap_norm_gap_bound_constant} at
...
\begin{proof}
    \leanok
    The cyclic-window compression reduction applies with $\gamma=1-\eta m$ to
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Since $\eta m<1$ the gap constant $1-\eta m$ is positive, and the
    quadratic-form criterion converts the compression estimate into the stated
    norm lower bound. The preceding fixed-coefficient theorem is the
    specialization $\eta=(1-1/(4L))/(2(L-1))$.
\end{proof}
