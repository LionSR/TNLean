...
\begin{proof}
    ...
    \begin{align}
        ...
        W_j & =G_{K+L+Q}(A_j),
    \end{align}
    and let $P,Q,R$ be the orthogonal projectors onto $\sum_jU_j$,
    $\sum_jV_j$, and $\sum_jW_j$, as in
    Theorem~\ref{thm:ph_block_interval_projectors}. The local identity at
    ...
    \begin{align}
        ...
        P_{[K+1,K+L+Q]}(B) & =P_{\mathscr A_{K,L,Q}(\mathcal H_K\otimes
                G_{L+Q}(B))}=P_{\sum_jV_j}=Q.
    \end{align}
    Hence the left side of the asserted bound is $\lVert R-PQ\rVert$, and
...
\begin{proof}
    ...
    obtained by fixing these coordinates to $s\in\{1,\ldots,d\}^r$.
    The prefix Hamiltonian acts on each component $v_s$ by the length-$n$
    open Hamiltonian, whose kernel is $G_n(A)$. Hence
    \begin{align}
        v\in G_n(A)\otimes(\C^d)^{\otimes r}
         & \iff v_s\in G_n(A)\ \text{for every } s\in\{1,\ldots,d\}^r.
    \end{align}
\end{proof}
