...
\begin{theorem}[The source conditions on their own ranges do not give the
    ...
    own window sum, conditions C2 and C3 hold on the range $n_0\leq n<N$ from
    which they are imposed, every ground space is the kernel of its own
    Hamiltonian, that is $\ker H_{\Lambda_M}=\operatorname{ran}(G_M)$ for
    $M\leq N$ and $\ker H_n=\operatorname{ran}(Q_n)$ for $n_0\leq n<N$, and
    \begin{align}
...
\begin{proof}
    ...
    the $e_i$ with $i\neq n$ whenever $1\leq n<N$, which is
    $\operatorname{ran}(Q_n)$. For $1\leq n<N$ one has $c_n=1$, so
