...
\begin{theorem}[Mixed boundary Gram identity]
    ...
    $M_{D_B\times D_A}(\C)$. For matrix units $E_{ab}$ and $E_{ce}$,
    \begin{align}
...
\begin{theorem}[Uniform decay of block ground-space overlaps]
    ...
    $n\geq n_0$, $x\in\mathcal G_n^{\mathrm{ES}}(A)$, and $y\in\mathcal G_n^{\mathrm{ES}}(B)$,
    \begin{align}
...
\begin{proof}
    ...
    self-Gram operators $G_A(n)$ and $G_B(n)$ converge to invertible limits
    $G_A(\infty)$ and $G_B(\infty)$ determined by $\rho$ and $\sigma$, and
    their inverses converge to $G_A(\infty)^{-1}$ and $G_B(\infty)^{-1}$.
    Since the limits are invertible, so are $G_A(n)$ and $G_B(n)$, and hence
    $\Gamma_A(n)$ and $\Gamma_B(n)$ are injective, for all
    ...
    \begin{align}
        c(n)
         & =\sqrt{\lVert G_A(n)^{-1}\rVert}\,
        \lVert\Gamma_A(n)^*\Gamma_B(n)\rVert\,
        \sqrt{\lVert G_B(n)^{-1}\rVert}
        \longrightarrow
        \sqrt{\lVert G_A(\infty)^{-1}\rVert}\cdot0\cdot
        \sqrt{\lVert G_B(\infty)^{-1}\rVert}=0.
    \end{align}
...
\begin{theorem}[Asymptotic orthogonality of inequivalent blocks]
    ...
    every $x\in\mathcal G_n^{\mathrm{ES}}(A)$ and $y\in\mathcal G_n^{\mathrm{ES}}(B)$ satisfy
    \begin{align}
        |\langle x,y\rangle| & \leq\varepsilon\lVert x\rVert\lVert y\rVert.
    \end{align}
...
\begin{theorem}[Uniform gap for normal MPS parent Hamiltonians]
    ...
    \leanok
    \uses{def:normal, def:l_blk_injective, def:parent_hamiltonian_es}
    Let $D>0$ and let $A$ be a normal tensor. There exist $l>1$ and
    $\gamma>0$ such that $A$ is $l$-block injective and, for every
    $N\geq2(l+1)$ and every $v\in(\ker H_{N,l+1}^{\mathrm{ES}}(A))^\perp$,
    \begin{align}
        \gamma\lVert v\rVert
         & \leq \lVert H_{N,l+1}^{\mathrm{ES}}(A)v\rVert.
        \notag
    \end{align}
    This proves the normal-tensor case of the uniform gap assertion in
    \cite[lines~2183--2187]{Cirac2021Matrix}, at a suitable interaction range.
\end{theorem}
...
\begin{proof}
    ...
    Theorem~\ref{thm:normal_parent_hamiltonian_finite_range_knabe_gap}
    supplies $l>1$, $m\geq l+1$, and $\delta>0$ with the required norm
    inequality for $N\geq2m$.
    Apply Theorem~\ref{thm:parent_hamiltonian_eventual_gap} with
    $L=l+1$ and $M=2m$, and restrict the conclusion to $N\geq2(l+1)$.
\end{proof}
