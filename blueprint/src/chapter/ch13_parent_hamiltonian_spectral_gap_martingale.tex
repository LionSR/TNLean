...
\begin{theorem}[Gap at the common block-injectivity range]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    for every $N\geq N_R$ and every $v\perp\ker H_{R,N}(B)$. This removes
    the restriction to the range constructed by the overlap argument, as
    recorded in~\cite{gap:cpgsv21_block_parent_interaction_range}.
\end{theorem}
\begin{proof}\leanok
...
\begin{proof}\leanok
    ...
    multiply four consecutive letters:
    \begin{align}
...
\begin{proof}\leanok
    \uses{def:tail_boundary_map, def:left_boundary_map}
    Factor the matrix product at the boundary of the interval:
    \begin{align}
...
\begin{theorem}[Marked paths in interval spaces]
...
\begin{proof}\leanok
    ...
    By Theorems~\ref{thm:weighted-matrix-unit-path-contractions}
    ...
    on the last site. Theorem~\ref{thm:spectator-multiplier-interval-space}
...
\begin{proof}\leanok
    ...
    Theorem~\ref{thm:fnw-marked-path-interval-spaces}, $x$ belongs to the
...
\begin{theorem}[Positive parent interactions at the simultaneous injectivity range]
    ...
    \leanok
    Let $A$ be a tensor in canonical form with positive physical dimension, with
    its multiplicities retained, and choose its normal representatives $B_j$,
    indexed by $0\leq j<r$.
