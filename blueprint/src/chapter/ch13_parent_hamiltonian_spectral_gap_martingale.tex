...
\begin{proof}
    ...
    For the second bound, use $\lVert P\rVert\leq1$ and the first bound:
    \begin{align}
        \lVert X\rVert & \leq\lVert X-P\rVert+\lVert P\rVert
        \leq\frac{\delta}{1-\delta}+1 \notag                 \\
                       & =\frac{1}{1-\delta}. \notag
    \end{align}
\end{proof}
...
\begin{theorem}[Comparison of joint projectors]
    ...
    \leanok
    \uses{thm:ph_off_diagonal_projectors}
...
\begin{lemma}[Locality and the initial grouped differences]
    ...
    \leanok
    Set $E_n=G_n-G_{n+1}$. For $0\leq m,n<M$ with $m<n-1$ or $m>n$,
    one has
    \begin{align}
        [E_m,Q_n] & =0.
    \end{align}
    In addition, $E_0=0$ and $Q_1E_1=0$.
\end{lemma}
\begin{proof}
    \leanok
...
\begin{theorem}[The first two grouped differences need no estimate]
...
\begin{proof}
    ...
    \begin{align}
        Q_0E_0 & =0, & Q_1E_1 & =0,
    \end{align}
    so both norms are $0\leq\epsilon$.
\end{proof}
...
\begin{proof}
    ...
    $I-Q_n$, so Theorem~\ref{thm:grouped_open_suffix_c2} gives condition C2
    with constant one. The nonzero local window terms are distinct summands
    of $H_N$, so Theorem~\ref{thm:grouped_open_parent_c1} gives condition C1
    with constant one. Lemma~\ref{lem:open_parent_hamiltonian_es_positive}
    supplies positivity of $H_N$. The locality lemma supplies the
    commutation condition. The assumed norm bound supplies C3, and the terminal
...
\begin{proof}
    ...
    Decompose a vector $v$ on $n+r$ sites according to the last $r$
    physical coordinates, writing $v_s$ for the vector on $n$ sites
    obtained by fixing these coordinates to $s\in\{0,\ldots,d-1\}^r$.
    The prefix Hamiltonian acts on each component $v_s$ by the length-$n$
...
\begin{proof}
    ...
    Write $\sigma$ for the reassociation isometry from the physical space
    on $K+(L+Q)$ sites to that on $(K+L)+Q$ sites. It preserves the ordered
    physical coordinates, so for every family $(S_n)_n$ of subspaces of the
    $n$-site physical spaces,
    \begin{align}
    ...
    $(\C^d)^{\otimes K}\otimes G_{L+Q}(A)$, and the display transports this
...
\begin{proof}
    ...
    on $n$ sites. By
    Theorem~\ref{thm:open-hamiltonian-right-spectator-conjugacy}, $\Phi$
...
\begin{proof}
    ...
    Theorem~\ref{thm:grouped_c3_from_second_index} it suffices to treat
    $n\geq2$. For such $n$, put $K=p(n-1)$. The active interval has
    length $p(n+1)$, and its preceding prefix has length $pn$. The kernel
    hypothesis and the active projector identity give the desired bound on
    this interval. Adjoining the remaining $pM-p(n+1)$ sites does not
...
\begin{proof}
    ...
    By Lemma~\ref{lem:parent-finite-interval-comparison}, choose
    $\kappa,C>0$ such that
    $\kappa h_W\leq K_{R,W}\leq Ch_W$. Summing all cyclic translates
    (Theorem~\ref{thm:periodic-parent-range-comparison}) gives
    \begin{align}
...
\begin{theorem}[Nonzero eigenvalues of the transfer remainder]
    ...
    Let $A$ be primitive with nonzero bond dimension, let $E$ be its observable
...
\begin{theorem}[Eventual projector bound with a prescribed prefactor]
    ...
    onset stated explicitly~\cite{gap:cpgsv21_martingale_overlap}.
\end{theorem}
\begin{proof}\leanok
