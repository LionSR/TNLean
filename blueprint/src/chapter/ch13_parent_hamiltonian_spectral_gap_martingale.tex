...
\begin{theorem}[Left-canonical normalization of fixed-length words]
    ...
    \end{align}
    where $\Omega_Q=\{0,\ldots,d-1\}^Q$ is the set of physical words of
    length $Q$.
\end{theorem}
...
\begin{theorem}[Whole-increment mixed Gram in unreassociated coordinates]
    \label{thm:whole_increment_mixed_gram_coordinates}
    \lean{MPSTensor.inner_tailBoundaryMapES_adjoint_leftBoundaryMapES_q}
    \leanok
    \uses{def:tail_boundary_map, def:left_boundary_map,
        def:ground_space_gram, def:eval_word}
    Let $\Omega_n=\{0,\ldots,d-1\}^n$.  For boundary families
    $X=(X_u)_{u\in\Omega_K}$ and $Z=(Z_j)_{j\in\Omega_Q}$, the canonical
    coordinate identification from $(K+L)+Q$ to $K+(L+Q)$ gives
    \begin{align}
      \left\langle X,
        (\Gamma^{\mathrm{tail}}_{K,L+Q})^\dagger
        \mathscr A_{K,L,Q}^{-1}
    ...
    \end{align}
    where $U$ is Frobenius vectorization and
    $\mathcal K_L=(\Gamma_L^{\mathrm{ES}})^\dagger
    \Gamma_L^{\mathrm{ES}}$.
\end{theorem}
...
\begin{proof}
    ...
    \uses{def:tail_boundary_map, def:left_boundary_map,
        def:ground_space_gram, def:eval_word}
    Split each physical word into blocks of lengths $K$, $L$, and $Q$.
    Word multiplicativity and trace cyclicity leave the length-$L$ Gram pairing
    with boundary matrices $A^jX_u$ and $Z_jA^u$.
\end{proof}
...
\begin{theorem}[Whole-increment mixed Gram after reassociation]
    ...
    \leanok
    \uses{def:reassociated_tail_boundary_map,
        thm:whole_increment_mixed_gram_coordinates}
    With the notation of
    Theorem~\ref{thm:whole_increment_mixed_gram_coordinates},
    \begin{align}
      \left\langle X,
...
\begin{definition}[Whole-increment centered residual]
    ...
    \uses{def:reassociated_tail_boundary_map,
        ...
        def:whole_increment_right_overlap_factor}
    Let $\mathcal K_\infty=\mathscr R(P_\rho)$.  Define
    \begin{align}
      R_{K,L,Q}
      &:=(\widetilde\Gamma^{\mathrm{tail}}_{K;L,Q})^\dagger
...
\begin{theorem}[Primitive whole-increment centered residual bound]
    ...
    Every primitive MPS tensor satisfies the left-canonical centered residual
    estimate of
    Theorem~\ref{thm:whole_increment_centered_residual_bound_left_canonical}.
\end{theorem}
...
\begin{theorem}[Limiting overlap product]
    ...
    \uses{def:whole_increment_left_overlap_factor,
        ...
        thm:whole_increment_right_overlap_factor_apply,
        thm:limiting_metric_inverse_virtual_adjoints}
    For a positive-definite fixed point, the product
...
\begin{theorem}[Whole-increment finite-Gram cancellations]
    ...
    \uses{thm:left_finite_gram_cancellation,
        thm:reassociated_tail_boundary_factorization,
        thm:inverse_gram_identities}
    The left finite Gram cancels its inverse to give
    $(\Gamma_{K+L+Q}^{\mathrm{ES}})^\dagger$.  The reassociated tail finite
    Gram satisfies the analogous cancellation.
\end{theorem}
...
\begin{definition}[Whole-increment tail finite-Gram correction]
    ...
    \leanok
    The tail correction is the physical sandwich containing
...
\begin{definition}[Whole-increment full inverse-Gram correction]
    ...
    \leanok
    The full correction is the physical sandwich containing
...
\begin{definition}[Whole-increment left finite-Gram correction]
    ...
    \leanok
    The left correction is the physical sandwich containing
...
\begin{theorem}[Whole-increment virtual correction telescope]
    ...
    The exact virtual residual equals $R_{K,L,Q}$ minus the three terms obtained
    by replacing, one at a time,
    \begin{align}
      \mathcal K_{L+Q},\qquad
      (\mathcal K_{K+L+Q})^{-1},\qquad
      \mathcal K_{K+L}
    \end{align}
    by their limiting values.
\end{theorem}
...
\begin{definition}[Whole-increment centered projector sandwich]
    ...
    \leanok
    \uses{def:whole_increment_centered_residual}
    The centered physical sandwich is obtained by placing $R_{K,L,Q}$ between
    the reassociated tail pseudoinverse and the left boundary pseudoinverse.
\end{definition}
...
\begin{theorem}[Whole-increment physical correction telescope]
    ...
    The whole-increment projector defect is the centered physical sandwich minus
    the tail Gram correction, the full inverse-Gram correction, and the left
    Gram correction.
\end{theorem}
...
\begin{theorem}[Whole-increment correction bounds]
    ...
    Each correction is bounded by the product of its Gram error, the adjacent
    virtual-map norms, and the square roots of the neighboring inverse-Gram
    norms.  The full correction additionally contains
    $\lVert\mathcal K_\infty\rVert$ and
    $\lVert\mathcal K_\infty^{-1}\rVert$.
\end{theorem}
