...
\begin{theorem}[Uniform whole-increment defect at the coefficient $7/16$]
    ...
    \leanok
    \uses{thm:primitive_fnw_projector_defect_geometric, thm:normal_from_primitive_posdef, thm:wordspan_blk_inj_succ}
    ...
    that $A$ is $p$-block injective and, for every $K>0$,
    \begin{align}
        \left\lVert
...
\begin{proof}
    \leanok
    \uses{thm:primitive_fnw_projector_defect_geometric, thm:normal_from_primitive_posdef, thm:wordspan_blk_inj_succ}
    Choose $p$ above a positive injective length and large enough that
    ...
    $r=K$, $m=p$, and $\ell=p$ for every positive prefix length $K$.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{thm:uniform_whole_increment_defect_seven_sixteenths}
    Apply the prefix-uniform theorem at $K=p>0$.  The three consecutive regions
...
\begin{theorem}[Uniform open-chain projector-defect threshold]
    ...
    uniformly in the positive left length $K$ by $\epsilon_l$, and
    \begin{align}
    ...
    estimate, gives the corresponding anticommutator bound for every $K>0$.  If
    $A$ is already a blocked representative, one site of $A$ is one
    blocked-chain filtration step.
\end{theorem}
\begin{proof}
    \leanok
    \uses{thm:primitive_fnw_openchain_defect_geometric, thm:normal_from_primitive_posdef, thm:wordspan_blk_inj_succ, thm:nachtergaele_open_chain_projector_defect}
    The open-chain geometric estimate supplies $c>0$ and $0<\lambda<1$.  Since
    \begin{align}
    ...
    that length and above $1$ so that $c\lambda^l<1$ and the last display is
    less than $1$.  Block injectivity propagates to $l$, and the geometric
