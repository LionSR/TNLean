...
\begin{lemma}[Restriction and combination of a physical configuration]
...
\begin{proof}
    \leanok
    At a vertex in $R$ the combined configuration reads $\omega_R$, and at a
...
\begin{theorem}[Bond-inserted state as an edge insertion]
    ...
    \leanok
    \uses{thm:peps_deformedRegionState_bondInsertedRegionInsert, thm:peps_regionInsertedCoeff_eq_smul_edgeInsertedCoeff, lem:peps_assembleRegionSigma_restrict}
...
\begin{proof}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Lemma~\ref{lem:peps_assembleRegionSigma_restrict} identifies the combined
...
\begin{proof}
    ...
    \begin{align}
        \Psi^{\mathrm{as}}_{B,R_i,I_{B,R_i,e,M_i}}(\omega)
         & =p_B(R_i)
        E_B(e,\omega,\mathcal O_{R_i,e}(M_i)).
        \notag
    \end{align}
