...
\begin{theorem}[Equal zero tails give full nonzero-part MPV equality]%
    ...
    Let the two tensors be written as an all-zero leftover block plus a nonzero
    part, and suppose the original tensors have the same MPV family. If
    $z_A=z_B$, then the nonzero parts have the same MPV family: for every length
    $N$ and every configuration $\sigma$,
    \[
        V^{(N)}(A_{\mathrm{nz}})_\sigma
        =
        V^{(N)}(B_{\mathrm{nz}})_\sigma.
...
\begin{proof}\leanok
    \uses{lem:sameMPV2_live_of_zeroTail_eq}
    Apply Lemma~\ref{lem:sameMPV2_live_of_zeroTail_eq} to the two zero-tail
    decompositions. The equality $z_A=z_B$ identifies the all-zero summands, so
    cancellation gives the nonzero-part equality at every length.
\end{proof}
