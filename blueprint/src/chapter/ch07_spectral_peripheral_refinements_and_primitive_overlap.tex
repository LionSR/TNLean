...
\begin{proof}
    \leanok
    \uses{thm:overlap_trace, thm:qpf_irreducible,
        thm:peripheral_multiplicity_one, thm:eigenvalue_bound}
    By the overlap--trace identity \ref{eq:spectral_overlap_trace},
    \begin{align}
        O_{AA}(mn) & =\Tr\!\left(\E_A^{mn}\right).
        \notag
    \end{align}
    We first justify the multiplicities without imposing unitality on
    $\E_A$. Since $A$ is left-canonical, $\E_A$ is trace-preserving, and its
    Frobenius adjoint $\E_A^\dagger$ is unital. Irreducibility also passes to
    the adjoint: a subspace invariant under every adjoint Kraus operator has
    orthogonal complement invariant under every original Kraus operator.
    Theorem~\ref{thm:qpf_irreducible} supplies a positive-definite fixed point
    $\rho$ of $\E_A$, which is the adjoint fixed point required when applying
    Theorem~\ref{thm:peripheral_multiplicity_one} to the unital map
    $\E_A^\dagger$.

    Adjoint eigenvalues are complex conjugates, and rank--nullity for
    $\E_A-\lambda\Id$ and its adjoint gives
    \begin{align}
        \dim\ker(\E_A-\lambda\Id)
         & =\dim\ker(\E_A^\dagger-\overline\lambda\Id).
        \notag
    \end{align}
    Thus the one-dimensional peripheral eigenspaces of $\E_A^\dagger$ give
    one-dimensional peripheral eigenspaces of $\E_A$. Complete positivity and
    trace preservation make
    $\E_A$ contractive on Hermitian matrices in trace norm. Decomposing an
    arbitrary matrix into its Hermitian and skew-Hermitian parts therefore
    gives a uniform bound on the powers of $\E_A$. A peripheral eigenvalue
    cannot have a nontrivial Jordan block, since such a block would make the
    powers grow polynomially. Hence every peripheral eigenvalue of $\E_A$ has
    algebraic multiplicity one.

    By hypothesis, these peripheral eigenvalues are exactly the $m$-th roots
    $1,\gamma,\ldots,\gamma^{m-1}$. Write the trace of a power as the sum of
    the corresponding powers of all eigenvalues, counted with algebraic
    multiplicity. Every nonperipheral eigenvalue $\nu$ satisfies $|\nu|<1$ by
    the peripheral-spectrum definition and the eigenvalue bound, so its
    contribution $\nu^{mn}$ tends to zero.
    For the peripheral roots,
    \begin{align}
        \sum_{q=0}^{m-1}(\gamma^q)^{mn}
         & =\sum_{q=0}^{m-1}1=m.
        \notag
    \end{align}
    The nonperipheral contribution vanishes, and hence
    $O_{AA}(mn)\longrightarrow m$.
\end{proof}
