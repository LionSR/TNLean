...
The first two results in this section concern finite Kraus maps and hence assume
complete positivity. They do not supply the abstract Schwarz-map case of Wolf
Theorem~6.6; see
\texttt{docs/paper-gaps/wolf\_thm6\_6\_kraus\_scope.tex}.

% Scope restriction (complete positivity): finite-Kraus specialization.
\begin{theorem}[Transfer-map peripheral eigenvalues: cyclic structure]
    ...
    \leanok
    \uses{def:transfer_map, def:unital_kraus, def:irreducible_cp,
...
\begin{proof}
    \leanok
    \uses{thm:peripheral_finite, lem:peripheral_closed_under_mul_group,
...
\begin{theorem}[Peripheral eigenvalues form a cyclic group for Kraus maps]%
    ...
    Let $E(X)=\sum_i K_iXK_i^\dagger$ be the Kraus map of a finite matrix
    family on $\MN{D}$. Assume that $E$ is irreducible, unital, and
    trace-preserving and has a positive-definite adjoint fixed point. Then
...
\begin{theorem}[Kraus-map period divides bond dimension]%
    ...
    Let $E(X)=\sum_i K_iXK_i^\dagger$ be the Kraus map of a finite matrix
    family on $\MN{D}$. Assume that $E$ is irreducible, unital, and
    trace-preserving and has a positive-definite adjoint fixed point. Let
