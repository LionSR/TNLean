...
\begin{definition}[The once-blocked CZX tensor]
    ...
    \lean{MPOTensor.CZX.decoratedSiteTensor, MPOTensor.CZX.tensor,
        MPOTensor.CZX.tensor_eq_blockTensor, MPOTensor.CZX.complementSite,
        MPOTensor.CZX.complement, MPOTensor.CZX.edgeExponent,
        MPOTensor.CZX.cyclicExponent}
    \leanok
    ...
    is the two-site block $B^{ab,cd}=C^{a,c}C^{b,d}$ of
    \begin{align}
        C^{u,d}_{\ell r}
         & =\delta_{u,1-d}\delta_{\ell,u}(-1)^{u+\ell r}.
    \end{align}
    Here the weight is $W_{\ell r}=(-1)^{\ell r}=\sqrt{2}H_{\ell r}$,
    where $H$ is the normalized Hadamard matrix, and the factor $(-1)^u$
    is the output Pauli $Z$. Thus, if $P$ denotes the normalized tensor
    printed in \cite[lines~1906--1943]{Cirac2017MPU}, then
    ...
    define its complement pointwise and set
    \begin{align}
        E(s) & =\sum_{j=0}^{N-1}
        (a_j+b_j+a_jb_j+b_ja_{j+1}),
    \end{align}
...
\begin{theorem}[Coordinates and unitarity of the exact CZX tensor]
    ...
    \begin{align}
        ...
        (-1)^{a+b+ab+br}.
    \end{align}
    Its periodic operator is unitary for every $N>0$, with entries
    \begin{align}
        O_N(B)_{s,t} & =\delta_{s,\overline t}(-1)^{E(s)}.
    \end{align}
...
\begin{proof}
    \leanok
    Multiplication of the two constituent matrices forces the intermediate
    bond to be $b$, giving the displayed coordinates. In the cyclic trace
    expansion, the left-bond delta at site $j$ forces the virtual path to
    have value $a_j$. All other paths vanish. The remaining product is zero
    unless $s=\overline t$, and otherwise its exponent is $E(s)$.
    Thus the operator is the monomial matrix for complementation, with
    column phase $(-1)^{E(\overline t)}$. Each phase has absolute value one,
    which proves unitarity.
\end{proof}
