...
\begin{theorem}[Left-end growth]
    \label{thm:mpug_truncated_symmetry_left_growth}
    \lean{MPOTensor.SourceFactors.truncatedSymmetry_cons}
    \leanok
    \discussion{7319}
    \uses{def:mpug_truncated_symmetry, def:mpug_staircase_gates}
    Let $\mathcal U$ have source factors as in
    Definition~\ref{def:mpug_truncated_symmetry}, and construct $U^N,w_L$
    from these same factors. For $N\geq0$, physical words $\sigma,\tau$
    of length $N$, source indices $l,r$, and physical indices $i,a,b,c$,
    \begin{align}
    ...
    \end{align}
    The right endpoint is unchanged. This is the left-end factorization
    in the growth diagram of
    \cite[lines~2101--2174]{FrancoRubio2025MPUGauging}. No simplicity
    or unitarity assumption on $\mathcal U$ is required.
\end{theorem}
\begin{proof}\leanok
    \uses{def:mpug_truncated_symmetry, def:mpug_staircase_gates,
        def:mpo_eval_word, thm:mpu_source_factorization_entries}
    Expand $\mathcal U^{i\sigma,b\tau}
        =\mathcal U^{i,b}\mathcal U^{\sigma,\tau}$ in the endpoint
    contraction. The second source-cut factorization gives
    \begin{align}
        \sum_\gamma(Y_2)_{l,(a,\gamma)}\mathcal U^{i,b}_{\gamma,\alpha}
         & =\sum_t(w_L)_{(l,i),(a,t)}(Y_2)_{t,(b,\alpha)}.
    \end{align}
    Substitution and regrouping of the finite sums leave $U^N$ with
    the same right endpoint factor $Y_1$.
\end{proof}
...
\begin{proof}\leanok
    ...
    supplies the recorded two-letter insertion. The one-letter consequence
    of Theorem~\ref{thm:mpu_all_later_simple_blockings} and its coordinate
    expansion give
    \begin{align}
