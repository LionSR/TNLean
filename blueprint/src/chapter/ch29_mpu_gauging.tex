...
$N\geq3$ blocked sites. The argument is the orbit and holonomy calculation of
the planning note on the growth of the gauge-invariant subspace, whose labels
are named in the Lean sources. Nothing here identifies the displayed tuple
...
\begin{definition}[Trivial holonomy on a fiber]
    ...
    \lean{TNLean.Algebra.fiberFlip,
        ...
        TNLean.Algebra.TrivialHolonomy}
    \leanok
    ...
    \end{align}
    for all $j,k$ and $\gamma\in\mathbb F_2^r$. For involutive monomial
    operators $T_j=M(\sigma_j,\varphi_j)$ this is the statement that
    $(T_jT_k)^2$ acts as the identity on the fiber.
\end{definition}
...
\begin{proof}\leanok
    ...
    Theorem~\ref{thm:mpug_hypercube_potential} for the unimodular phases
...
\begin{definition}[The displayed CZX circuit tuple]
    ...
    this identification.
    % Source anchor: arXiv:2502.20257, lines 4503--5183 (CZX model, modified
    % fusion operator); planning note labels eq:p4-w-def, eq:p4-lambda-def,
    % eq:p4-tilde-lambda-def, eq:p4-paper-tuple.
\end{definition}
...
\begin{theorem}[Phase tables of the CZX fusion operators]
    ...
    $e(\overline x)=f(x)+1$, and $w$ and $\tilde\lambda$ are unitary.
\end{theorem}
...
\begin{proof}\leanok
    ...
    and unitarity follows from Theorem~\ref{thm:mpug_monomial_matrix_laws}
    because all phases are unimodular.
\end{proof}
...
\begin{theorem}[Local monomial action of the CZX Gauss operator]
    ...
    $\sigma(s)=(x_0+1,x_1+1,a+1,x_2,x_3,a'+1)$. Substituting
    (\ref{eq:mpug_czx_tuple}) into (\ref{eq:mpug_gauss_operator}) gives the
    nontrivial local Gauss operator
    \begin{align}
        G & =\tilde\lambda^\dagger\otimes\ketbra{1,1}{0,0}
        +w^\dagger\otimes\ketbra{1,0}{0,1}
        +w\otimes\ketbra{0,1}{1,0}
        +\tilde\lambda\otimes\ketbra{0,0}{1,1},
        \label{eq:mpug_czx_local_gauss}
    \end{align}
    which is the monomial operator
    \begin{align}
...
\begin{theorem}[Permutation-orbit classification]
    ...
    unchanged. Hence the fiber
    $S_{p,b}=\{s:p(s)=p,\ b(s)=b\}$ has $2^N$ elements and is one free
    transitive orbit of the group $\mathbb F_2^N$ generated by the
    $\sigma_j$.
\end{theorem}
...
\begin{theorem}[Involution and neighboring holonomy]
    ...
    $k\notin\{j-1,j,j+1\}$. In terms of the phases
    $\varphi_j(s)=i^{q_j(s)}$, (\ref{eq:mpug_czx_involution}) says that
    $G_j$ is an involution and (\ref{eq:mpug_czx_holonomy}) is the
    neighboring holonomy $(G_jG_{j+1})^2\ket{s}=(-1)^{b_{j+1}(s)}\ket{s}$.
\end{theorem}
...
\begin{theorem}[Fixed spaces on the orbit fibers]
    ...
    \lean{MPOTensor.CZX.trivialHolonomy_fiberPhase_iff}
    \leanok
...
\begin{theorem}[The placed CZX Gauss operators]
    ...
    (\ref{eq:mpug_czx_local_gauss}) placed on the bond $(j,j+1)$. For
...
\begin{theorem}[Exact dimension for the displayed CZX circuit tuple]
    ...
    \end{align}
    More precisely, the common invariant space is the direct sum of
    one-dimensional fixed spaces indexed by $p\in\mathbb F_2^N$, one for each
    fiber $S_{p,0}$. This answers the growth question of
