...
\begin{definition}[Generalized coboundary and cocycle]
    ...
    \cite[lines~5908--5916 and Lemma~\texttt{lemma:G\_cocycle}]
        {FrancoRubio2025MPUGauging}. No transitivity or finiteness assumption
    is imposed on $\mathsf X$.
    % Source anchor: arXiv:2502.20257, lines 5908--5916 and lemma:G_cocycle.
\end{definition}
...
\begin{definition}[Regular-action matrices]
    ...
    Suppose that $G$ is finite. On the group algebra $\C G$, define
    $X^x_g$ by
    \begin{align}
        X^x_g\ket{k}
...
\begin{theorem}[Finite-group power trivialization]
    ...
    \textbf{Local fix (determinant numerator).}
    The source repeats $\det X^x_g$ as the second numerator factor. Taking
    determinants in~(\ref{eq:mpug_generalized_regular_multiplication}) gives
    the corrected factor $\det X^x_h$ displayed in
...
\begin{proof}\leanok
    ...
    Write $P_{g^{-1}}$ for the permutation matrix of left multiplication by
    $g^{-1}$ and
    $D^x_g=\operatorname{diag}(\Omega^{k^{-1}x}_{g,k})_{k\in G}$.
    The row-to-column convention for permutation matrices gives
    \begin{align}
        X^x_g
        & =P_{g^{-1}}D^x_g.
    \end{align}
    Since $\det P_{g^{-1}}\in\{1,-1\}$ and every diagonal entry of $D^x_g$
