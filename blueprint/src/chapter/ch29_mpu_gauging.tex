...
\begin{definition}[Generalized coboundary and cocycle]
    ...
    \textbf{Local fix (two-cochain coboundary index).}
    In the source definition of $df$ for $f^x_{g,h}$, the final denominator
    factor is printed as $f^x_{gh,x}$, with a block label in a group-index
    position. The corrected factor is $f^x_{gh,k}$, which gives
    (\ref{eq:mpug_generalized_cocycle}) when $d\Omega=1$.
