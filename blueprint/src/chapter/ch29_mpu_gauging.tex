...
\begin{theorem}[Unitary gauges for the dagger and the group inverse]
    ...
    \uses{def:mpug_group_representation, def:mpu_normalized_flattening,
        def:left_canonical_tensor, thm:mpug_physical_adjoint_inverse_mpv,
    ...
    $\{\mathcal U_g\}_{g\in G}$ be a commonly blocked representation by
    simple injective MPU tensors whose normalized flattenings are
    left-canonical.  Then
    \begin{align}
        D_g & =D_{g^{-1}}.
    ...
    equation~\textup{\texttt{eq:defT}} in
    \cite[lines~1552--1557]{FrancoRubio2025MPUGauging}.
\end{theorem}
...
\begin{theorem}[Scalar relation for the dagger--inverse gauges]
    ...
    equation~\textup{\texttt{eq:intro_sigma}} and its immediate consequences
