...
$3\times2$ coordinate rectangles. We specify the underlying geometry: the nested
regions $R\subseteq S$ with $S\setminus R$ a single site, which realize the
one-site-different injective regions required at each vertex,
together with the local window $T$ around an edge. The union closure of
Section~\ref{sec:peps_normal_union} builds these regions from injective
rectangles, so rectangular injectivity guarantees their injectivity.
...
\begin{lemma}[The normal regions $R$ and $S$ are unions of contiguous
    ...
    \leanok
    \uses{def:peps_squareLatticeCoordinateRegions, lem:peps_squareLatticeCoordinateRegion_identities}
...
\begin{definition}[Rectangular covers of square-lattice regions]%
    ...
    \leanok
    \uses{def:peps_squareLatticeCoordinateRegions, lem:peps_normalSquareEdgeComplementRegion}
    ...
    finite-lattice complementary block $A_3$. The panel below illustrates the
    two permitted shapes of a cover member.
    \begin{tenkzequation}
        ...
        \begin{tenkz}[rows={ket,ket,ket,ket}, cols=4,
                west=open, east=open, north=open, south=open]
            \tnmark[form=enclosure, species=selected]{(1,1) .. (2,3)}{$2\times3$}
            \tnmark[form=enclosure, species=secondary]{(2,3) .. (4,4)}{$3\times2$}
        \end{tenkz}
    \end{tenkzequation}
\end{definition}
...
\begin{lemma}[The present local $T$-window and $A_3$ cover obstructions]%
    ...
    \leanok
    \uses{def:peps_normalSquareRectangleCover, lem:peps_normalSquareRegionT_window_complement}
...
\begin{lemma}[Injectivity from a rectangular cover]%
    ...
    \leanok
    \uses{def:peps_normalSquareRectangleCover, def:peps_normalRectangleInjectivityHypotheses, lem:peps_regionInjectivityUnionClosure_finite}
