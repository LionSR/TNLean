\begin{remark}[Basic-vector coefficients and nearest-neighbor local-term commutation]%
    ...
    \leanok
    \uses{thm:trace_eval_word_eq_sum_cyclic, lem:appendixB_basis_change_parent_interaction, def:appendixB_two_site_basic_space, def:appendixB_qax, def:appendixB_qxb, lem:appendixB_qax_qxb_parent_interaction, lem:appendixB_three_site_parent_lifts, lem:appendixB_lift_commutator_parent_terms, lem:nncph_from_adjacent_two_site_commutation}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The source places each virtual pair across neighboring sites and applies an
    on-site physical isometry, as in
    \cite[equations~(3.17)--(3.18) and Appendix~B]{Cirac2016MPDO_arXiv}.
    ...
    Lemma~\ref{lem:nncph_from_adjacent_two_site_commutation} gives this
    pairwise commutation directly from adjacent commutation. Together with
    Lemma~\ref{lem:parent_hamiltonian_ff}, this gives the zero-energy equation
    for $V^{(N)}(A)$. Neither conclusion requires a disjoint physical-pair
    factorization. The full source implication additionally includes the
...
\begin{remark}[Cyclic contraction and disjoint physical pairs]
    \label{rem:cyclic_contraction_not_disjoint_pairs}
    \uses{rem:product_pair_projectors}
    The unit pair-index isometry and cyclic coefficient formula in
    Remark~\ref{rem:product_pair_projectors} do not imply factorization of a
    fixed physical-word coefficient over disjoint physical pairs.
    For example, take virtual indices $0,1$, physical labels $(a,b)$, and
    $U^{(a,b)}_{x,y}=\delta_{x,a}\delta_{y,b}$, with
    $A^{(a,b)}=\lambda_a E_{a,b}$ and $\lambda_0,\lambda_1>0$.
    The pair-index isometry holds. For the word
    $\sigma=((0,1),(1,0),(1,0),(0,1))$, the cyclic coefficient is
    \begin{align}
        V^{(4)}(A)_\sigma
         & =
        C_4(\sigma)=0.
        \notag
    \end{align}
    In contrast, the product over disjoint physical pairs is
    \begin{align}
        \prod_{p=0}^{1}\phi_2(\sigma_{2p},\sigma_{2p+1})
         & =
        (\lambda_0\lambda_1)(\lambda_1\lambda_0)
        =\lambda_0^2\lambda_1^2>0.
        \notag
    \end{align}
\end{remark}
...
\begin{theorem}[Spatial block actions of the two-site parent interaction]%
    ...
    middle site by this unitary, the first identity below holds on the first
    pair and the second on the last pair:
    \begin{align}
        Q_A
         & =
