...
In the injective case the chain ground space equals $\spn\{V^{(N)}(A)\}$, so the
parent Hamiltonian has a unique ground state on every periodic chain. In the
normal case the same conclusion is the payoff of the section, given the
closed-boundary comparison and the length hypotheses $N\ge2$ and $L_0<L\le N$.
