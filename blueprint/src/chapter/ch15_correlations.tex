...
Two of the three results below remain deferred because they require explicit
constants, while the spectral-gap statement from injectivity is already available.
...
\begin{theorem}[Exponential convergence of primitive channels]%
    ...
    Fix a primitive trace-preserving MPS tensor $A$ with positive
...
\begin{remark}[Status of quantitative bounds]\label{rem:quantitative_status}
    ...
    Two of the three theorems above remain open at the quantitative level,
    while the spectral gap from injectivity is already established.
    ...
    The missing step is to extract explicit constants from the Jordan-form
    analysis and the Wielandt-bound argument.
\end{remark}
