...
\begin{proof}\leanok
    ...
    Substituting the decomposition above and using the isometry relation
...
\begin{theorem}[Full structural form for RFP tensors]%
    ...
    point admits the decomposition $A^i = X \Lambda U^i X^{-1}$, where
    $\Lambda$ is diagonal positive with $\tr(\Lambda) = 1$ and $U$ is an
    isometry in the physical indices.
    The Lean statement lives in \texttt{Archive/RFPStructuralFull.lean}
    pending the Kraus-to-isometry extraction step after the rank-one
    idempotent CP classification (tracked by issue \#233).
\end{theorem}
