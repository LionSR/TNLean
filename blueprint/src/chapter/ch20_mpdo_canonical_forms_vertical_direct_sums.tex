...
\begin{proof}[Proof outline]
    \uses{thm:cpsv_canonical_form_active_bnt_refinement,
        ...
        thm:cpsv_full_coordinate_marked_trace_bridge,
    ...
    \end{align}
    whenever the corresponding first-site equality has been established on
    $T_{\mathrm{grp}}$.

    The remaining step is to derive that grouped first-site equality from
    (\ref{eq:cpsv_prop_4_13_first_site}) through the listed block gauge $G$ and
    the ambient coisometry $V$, and then to carry
    (\ref{eq:cpsv_prop_4_13_active_separation}) back to the vertically viewed
    tensor. This must give
    \begin{align}
        \widetilde MP&=P\widetilde MP.
        \label{eq:cpsv_prop_4_13_missing_coisometry_transport}
    \end{align}
    This listed-gauge and ambient-coisometry implication remains open. If an
    argument instead removes the zero-weight coordinates, it must also
    construct an explicit coisometric selector and
    prove the corresponding reconstruction identity. Once this passage is
    established, $(\Id-P)\widetilde MP=0$, so every one-sided invariant
    ...
    identity used in the proof of Proposition~4.13. The full-coordinate trace
    bridge of Theorem~\ref{thm:cpsv_full_coordinate_marked_trace_bridge} and
    the marked form of
    Theorem~\ref{thm:cpsv_retained_representative_grouped_lemma_l} separate the
    representative marks once the required full-coordinate trace equality is
    known. It remains to derive that equality from the physical compression
    identity through $G$ and $V$. In particular, the passage to the local
    ...
    \end{align}
    remains open. Normal-tensor rigidity would then make the relative Gram
    matrix scalar:
...
% retained-coordinate Lemma L are complete. Passage through the listed gauge
% and ambient coisometry into the vertical tensor, including the physical
% Gram-conjugation identity and final assembly, remains open.
