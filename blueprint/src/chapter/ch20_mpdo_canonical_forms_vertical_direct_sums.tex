...
\begin{proof}
    \leanok
    \uses{thm:mpdo_vertical_projector_closure_literal_cpsv,
...
\begin{theorem}[Vertical normal sectors grouped into BNT representatives]
    ...
    Let $M$ generate matrix product density operators and suppose that $M$
    is in normalized BNT-refined horizontal form. Partition its normalized
    vertical sectors by equality of their matrix product vector families up to
    a fixed nonzero scalar per site. There are normal representatives
    $M_\alpha$, an enumeration
...
\begin{theorem}[Literal CPSV vertical sectors grouped into BNT representatives]
    ...
    \uses{def:mpdo, def:cpsv_canonical_form, def:vertical_tensor_mpdo,
        def:bnt_cpsv, def:mpv_phase_class_data}
    Let $M$ generate matrix product density operators, and suppose that its
    doubled-index MPS tensor is in literal CPSV canonical form. Partition the
    normalized vertical sectors by equality of their matrix product vector
    families up to a fixed nonzero scalar per site. There are normal representatives $M_\alpha$, an enumeration
    $k=(\alpha,q)$ of the original sectors, invertible matrices
    $X_{\alpha,q}$, and complex numbers $\zeta_{\alpha,q}$ of unit modulus,
    with $X_{\alpha,1}=\Id$ and $\zeta_{\alpha,1}=1$, such that
    \begin{align}
...
\begin{proof}[Proof outline]
    ...
    groups the tensors $B_k$ into actual representative families over normal
    tensors $M_\alpha$. It retains the physical isometries and both reducing
    identities, gives the exact reconstruction, and proves that every grouped
    coefficient $\mu_{\alpha,k}$ is positive. Its proof uses the nonzero
    finite-chain compression of each range projection together with the sector
    trace identity and positivity of the generated density operator.

    The remaining Figure~8 step would apply the conditional pairwise theorem
    to these actual corners. Theorem~\ref{thm:cpsv_literal_pairwise_vertical_gram_conjugation}
    gives
    \begin{align}
...
% inserted-tensor equality, invariant-projector closure, literal periodic
% exclusion, the normal vertical block decomposition, literal sector-compression
% separation, and the conditional pairwise Figure-8/Gram identities are complete.
% Literal vertical phase-class grouping is complete. Grouped Figure-8 normalization
