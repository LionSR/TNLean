...
\begin{proof}[Proof outline]
    ...
    It remains to group the tensors $B_k$ into actual vertical representative
    families over tensors $M_\alpha$. After such a grouping, let
    $P_{\alpha,k}$ be the range projection of the corner indexed by
    $(\alpha,k)$. Theorem~\ref{thm:cpsv_literal_sector_compression_nonzero}
    gives a length at which its compression is nonzero. Since $M$ generates
    matrix product density operators, that compression is positive
    semidefinite; Theorem~\ref{thm:mpdo_sector_compression_trace_pos} then
    makes its trace positive. The sector trace identity has the form
    \begin{align}
...
% Actual vertical phase-class grouping, normalization of the grouped sectors, and
% the final coisometry remain open.
