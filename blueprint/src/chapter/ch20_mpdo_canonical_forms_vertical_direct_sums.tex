...
\begin{theorem}[Per-copy horizontal hypotheses do not control diagonal normality]
    ...
    block weight satisfying the per-copy finite-length separation
...
\begin{theorem}[Invariant vertical projections in literal CPSV canonical form]
    ...
    \leanok
    \uses{def:mpdo, def:cpsv_canonical_form, def:vertical_tensor_mpdo, def:invariant_projector_closure, thm:representative_one_sub_mp_zero, thm:cpsv_original_coordinate_marked_lemma_l}
...
\begin{proof}\leanok
    \uses{thm:representative_one_sub_mp_zero, thm:cpsv_original_coordinate_marked_lemma_l}
...
\begin{theorem}[Invariant vertical projections are reducing]
    ...
    \leanok
    \uses{def:mpdo, def:horizontal_cf_mpdo, def:vertical_tensor_mpdo, def:invariant_projector_closure, thm:representative_one_sub_mp_zero}
...
\begin{theorem}[Irreducible reducing sectors of the vertically viewed tensor]
    ...
    \leanok
    \uses{thm:mpdo_vertical_projector_closure, thm:projector_closure_irreducible_corners}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The vertically viewed tensor $\widetilde M$ and the unit-weight direct sum
...
\begin{theorem}[Spectrally normalized vertical sectors with physical isometries]
    ...
    \leanok
    \uses{thm:mpdo_vertical_irreducible_reducing_sectors, thm:mpdo_vertical_no_periodic_vectors_horizontal_cf, lem:irreducible_transfer_perron_pair, lem:irreducible_block_spectral_normalization}
...
\begin{theorem}[Normal vertical corners in literal CPSV canonical form]
    \label{thm:mpdo_literal_normal_vertical_corners}
    \lean{MPSTensor.IsCPSVCanonicalForm.%
        exists_normal_verticalBlockDecomp_with_isometry}
    \leanok
    \uses{def:mpdo, def:cpsv_canonical_form, def:vertical_tensor_mpdo, def:normal}
    Let $M$ generate matrix product density operators, and suppose that its
    doubled-index MPS tensor is in literal CPSV canonical form. Then there are
    positive integers $d_k$, positive scalars $\nu_k$, normal tensors $B_k$ of
    bond dimension $d_k$, and isometries
    $V_k:\C^{d_k}\to\C^d$ with pairwise orthogonal ranges. For every sector
    $k$ and every vertical letter $v$,
    \begin{align}
        \widetilde M_vV_k         & =V_k(\nu_kB_k^v),
        \notag                                                     \\
        V_k^\dagger\widetilde M_v & =(\nu_kB_k^v)V_k^\dagger,
    ...
    \begin{align}
        \widetilde M_v
         & =\sum_kV_k(\nu_kB_k^v)V_k^\dagger.
        \label{eq:mpdo_literal_normal_vertical_reconstruction}
    \end{align}
    The omitted orthogonal complement is an all-zero sector. This is the
    normal-corner intermediate between the periodic exclusion at
    lines~1889--1893 and the grouped vertical representation used at line~1898
    of \cite[Proposition~4.13]{Cirac2016MPDO_arXiv}.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:mpdo_vertical_projector_closure_literal_cpsv, thm:mpdo_vertical_no_periodic_vectors_literal_cpsv, thm:projector_closure_canonical_form}
    Theorem~\ref{thm:mpdo_vertical_projector_closure_literal_cpsv} gives
    invariant-projector closure for $\widetilde M$, and
    Theorem~\ref{thm:mpdo_vertical_no_periodic_vectors_literal_cpsv} excludes
...
\begin{definition}[Grouped vertical BNT decomposition with physical isometries]
    ...
    \leanok
    \uses{def:vertical_tensor_mpdo, def:mpv_phase_class_data, def:bnt_cpsv, def:mpdo_sector_projector, def:mpdo_vertical_loops}
    ...
    Definition~\ref{def:mpdo_sector_projector}, and for every
...
\begin{theorem}[Vertical normal sectors grouped into BNT representatives]
    ...
    \leanok
    \uses{def:mpdo, def:horizontal_cf_mpdo, def:vertical_bnt_grouping_with_isometry}
...
\begin{proof}\leanok
    \uses{thm:mpdo_vertical_normal_sectors_with_isometries, thm:cpsv_normal_mpv_phase_gauge, thm:cpsv_normal_separated_eventually_li, thm:mpdo_grouped_sector_compression_nonzero, def:mpdo_sector_projector, thm:mpdo_sector_weight_pos}
...
\begin{theorem}[Literal vertical normal sectors grouped into BNT representatives]
    \label{thm:cpsv_literal_vertical_bnt_grouping_with_isometries}
    \lean{MPSTensor.IsCPSVCanonicalForm.%
        exists_verticalBNTGrouping_with_isometry}
    \leanok
    \uses{def:mpdo, def:cpsv_canonical_form, def:vertical_bnt_grouping_with_isometry}
    Under the same hypotheses, partition the normalized vertical sectors by
    equality of their matrix product vector families up to a fixed nonzero
    scalar per site. There are normal representatives $M_\alpha$, an enumeration
    $k=(\alpha,q)$ of the original sectors, invertible matrices
    ...
    \begin{align}
        B_{\alpha,q}^v
         & =\zeta_{\alpha,q}X_{\alpha,q}
        M_\alpha^vX_{\alpha,q}^{-1}.
        \label{eq:cpsv_literal_vertical_gauge_phase}
    \end{align}
    The representatives are pairwise gauge-phase distinct and form a basis of
    normal tensors for the weighted block tensor
    $\bigoplus_k\nu_kB_k$. Every grouped coefficient
    $\nu_{\alpha,q}\zeta_{\alpha,q}$ is positive. With the physical isometries
    from Theorem~\ref{thm:mpdo_literal_normal_vertical_corners}
    retained unchanged, one has both reducing identities, exact compression,
    and the literal reconstruction
    \begin{align}
        \widetilde M_v
        ...
        X_{\alpha,q}M_\alpha^vX_{\alpha,q}^{-1}\bigr)
        V_{\alpha,q}^\dagger.
        \label{eq:cpsv_literal_vertical_grouped_reconstruction}
    \end{align}
    For each pair $(\alpha,q)$, the range projector of $V_{\alpha,q}$ obeys
    the representative-loop identity and gives a nonzero compression at some
    finite length. This is the decomposition and positivity argument of
...
\begin{proof}\leanok
    \uses{thm:mpdo_literal_normal_vertical_corners, thm:cpsv_normal_mpv_phase_gauge, thm:cpsv_normal_separated_eventually_li, thm:mpdo_range_projection_corner_nonzero, thm:cpsv_literal_sector_compression_nonzero, def:mpdo_sector_projector, thm:mpdo_sector_weight_pos}
    Take one representative from each phase class.
    Theorem~\ref{thm:cpsv_normal_mpv_phase_gauge} gives equality of bond
    dimensions and (\ref{eq:cpsv_literal_vertical_gauge_phase}) for every copy.
    Distinct representatives are gauge-phase separated by construction, so
    Theorem~\ref{thm:cpsv_normal_separated_eventually_li} gives eventual linear
    independence. The phase-class regrouping of the finite sum gives the BNT
    expansion. The representative copy is assigned the identity gauge and
    ...
    that every phase has unit modulus. Substitution into the two intertwining
    identities, the compression identity, and the reconstruction
    (\ref{eq:mpdo_literal_normal_vertical_reconstruction}) proves all remaining
    algebraic assertions without altering any physical isometry. Cyclicity of
    trace gives the sector-loop identities. The nonzero-corner separation
    theorem supplies a nonzero compression for every sector, and the sector
    trace identity and MPDO positivity then make every grouped coefficient
...
\begin{lemma}[A retained vertical phase class exists]
    ...
    \leanok
    \uses{def:horizontal_cf_mpdo, thm:mpdo_vertical_bnt_grouping_with_isometries, lem:mpv_phase_class_nonempty}
    Under the hypotheses of
...
\begin{proof}
    \leanok
    \uses{def:horizontal_cf_mpdo, def:vertical_tensor_mpdo, lem:sector_bnt_weight_unit_exists}
...
\begin{theorem}[Algebraic BNT from a grouped vertical decomposition]
    ...
    \leanok
    \uses{def:vertical_tensor_mpdo, def:bnt, def:bnt_cpsv, def:mpv_phase_class_data}
...
\begin{proof}
    \leanok
    \uses{thm:cpsv_bnt_is_bnt, lem:bnt_of_same_mpv2_pos, thm:mpv_decomposition}
...
\begin{proof}\leanok
    \uses{thm:mpdo_reflected_marked_chain, thm:cpsv_original_coordinate_marked_lemma_l}
    For $Z=X,Y$, set
    \begin{align}
        f^Z_{(r,s),(i,j)}
...
\begin{proof}\leanok
    \uses{thm:cpsv_literal_pairwise_vertical_gram_conjugation, thm:normal_tensor_gram_rigidity}
...
% They are distinct from the Appendix C.4 mixed-prefix comparison in Chapter 21.
...
\begin{theorem}[Vertical canonical form from grouped sectors and Gram dressing]
    ...
    \leanok
    \uses{def:vertical_bnt_grouping_with_isometry, def:grouped_corner_gram_dressing, def:vertical_cf_mpdo}
...
\begin{proof}\leanok
    \uses{thm:grouped_sector_gram_scalar_of_dressing, thm:mpdo_vertical_grouped_is_bnt, thm:normalized_grouped_sector_maps_of_gram, thm:vertical_cf_of_grouped_orthogonal_sectors}
...
\begin{theorem}[Vertical canonical form of a canonical MPDO]
    ...
    \end{align}
    In \cite[Proposition~4.13]{Cirac2016MPDO_arXiv}, $U$ is called an isometry
    and (\ref{eq:cpsv_prop_4_13_vertical_form}) is displayed. In this rectangular
    orientation, $U$ maps the physical space onto the retained sector space and
    satisfies (\ref{eq:cpsv_prop_4_13_coisometry}), making $U$ a coisometry and
    $U^\dagger$ an isometric embedding. The reverse identity
    (\ref{eq:cpsv_prop_4_13_reconstruction}) supplies the exact reconstruction
    on the physical space.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:vertical_cf_of_grouping_and_gram_dressing, thm:cpsv_literal_vertical_bnt_grouping_with_isometries, thm:cpsv_literal_pairwise_vertical_gram_conjugation}
...
% The literal theorem above and the stronger horizontal theorem below share
% the grouped-sector BNT and coisometry results.
...
\begin{theorem}[BNT-refined horizontal form implies vertical canonical form]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This establishes vertical canonical form under the normalized BNT-refined
    horizontal hypothesis of Definition~\ref{def:horizontal_cf_mpdo},
    complementing the literal CPSV formulation of
    Theorem~\ref{thm:vertical_cf_of_cpsv_canonical_form}.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:vertical_cf_of_grouping_and_gram_dressing, thm:mpdo_vertical_bnt_grouping_with_isometries, thm:mpdo_pairwise_vertical_gram_conjugation}
