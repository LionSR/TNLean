...
\begin{definition}[Explicit $\eta$-data from the sector tensors]%
    ...
    the positivity from the projected chain identity: for every cyclic
    sector assignment $k_1,\ldots,k_N$, the corresponding block of the
    conjugated chain equals $\bigotimes_{n=1}^{N}\eta_{k_n,k_{n+1}}$ and is
    positive semidefinite, and the sector tensors can be chosen so that
    each factor is positive semidefinite
