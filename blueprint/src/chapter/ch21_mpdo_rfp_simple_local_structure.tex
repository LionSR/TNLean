...
\begin{theorem}[Pairwise commutativity of periodic bond translates]
    \label{thm:mpdo_physical_bond_translates_comm}
    \lean{MPOTensor.PhysicalSectorFactorization.physicalBond_translates_comm}
    \leanok
    \uses{thm:mpdo_periodic_adjacent_physical_bonds_comm,
      thm:mpdo_physical_two_site_crossed_bonds_comm}
    Let $N\geq 2$. For all sites $i,j\in\mathbb Z/N\mathbb Z$, the translated
    bonds commute:
    \[
      B_{i,i+1}B_{j,j+1}=B_{j,j+1}B_{i,i+1}.
    \]
...
\begin{proof}
    \leanok
    For $N=2$, apply
    Theorem~\ref{thm:mpdo_physical_two_site_crossed_bonds_comm}. Suppose $N\geq3$.
    Two length-two cyclic intervals are either equal, adjacent, or disjoint.
    Equal intervals commute trivially. Adjacent intervals reduce to the
    three-site identity of
    Theorem~\ref{thm:mpdo_periodic_adjacent_physical_bonds_comm}. Operators on
    disjoint intervals act on disjoint tensor factors and hence commute.
\end{proof}
...
\begin{theorem}[Positive translation-invariant bond datum]
    ...
    \uses{thm:mpdo_physical_two_site_bond_pos,
      thm:mpdo_physical_bond_translates_comm}
...
\begin{proof}
    ...
    is Theorem~\ref{thm:mpdo_physical_bond_translates_comm}.
\end{proof}
