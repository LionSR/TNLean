...
\begin{definition}[Sector-adapted chain coordinates]
    ...
    \[
        {\cal H}_k=\prod_{n=0}^{N-1}
          \bigl(B^L_{k_n}\times B^R_{k_n}\bigr).
    \]
    ...
    \[
        \Phi_N:\{0,\ldots,d-1\}^N
          \longrightarrow
          \coprod_{k_0,\ldots,k_{N-1}}{\cal H}_k.
    \]
\end{definition}

\begin{definition}[Physical basis change of an MPO tensor]
    \label{def:mpdo_conjugate_physical}
    \lean{MPOTensor.conjugatePhysical}
    \leanok
    Let $U$ be a unitary matrix on the physical space. The physical basis
    change of ${\cal K}$ is the tensor $\widetilde{\cal K}$ defined by
    \[
        \widetilde\kappa_{\beta,\alpha}^{\,ij}
        =\bigl(U\kappa_{\beta,\alpha}U^\dagger\bigr)_{ij}.
    \]
    Its horizontal bond indices are unchanged.
\end{definition}
...
\begin{theorem}[A fixed sector block is the cyclic neighboring product]
    ...
    \uses{def:mpdo_conjugate_physical,
        ...
        thm:mpdo_sector_word_eta_contraction}
    ...
    configuration $k$, the compression of
    $M_N(\widetilde{\cal K})$ to ${\cal H}_k$ is $\Omega_k$.
\end{theorem}

\begin{proof}
    \leanok
    Expand the closed horizontal trace of the MPO word. The sector
    factorization writes its local entries as products of entries of $l_{k_n}$
    and $r_{k_n}$. Theorem~\ref{thm:mpdo_sector_word_eta_contraction} then
    regroups the cyclic contraction into the entries of $\Omega_k$.
\end{proof}
...
\begin{theorem}[Sector-adapted MPO decomposition]
    ...
    Under the same sector factorization hypothesis,
    \[
        \Phi_N M_N(\widetilde{\cal K})\Phi_N^{-1}
...
\begin{figure}[ht]
    ...
    this algebraic decomposition; no positivity is asserted.}
...
\begin{proof}
    ...
    choose a site at which their sector labels are unequal. The local
    block-diagonal factorization vanishes there, and hence every summand in
    the closed horizontal trace is zero.
\end{proof}
