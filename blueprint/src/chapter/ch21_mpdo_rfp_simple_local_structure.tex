...
\begin{lemma}[Primitivity from returns of lengths two and three]%
    \label{lem:matrix_primitive_of_two_three_returns}
    \lean{Matrix.IsIrreducible.isPrimitive_of_pow_two_diagonal_pos_of_pow_three_diagonal_pos}
    \leanok
...
\begin{proof}
    ...
    For a chosen path of length $m$, the difference
    $2L+2-m$ is at least two and can therefore be written as $2a+3b$ with
    $a,b\geq0$. Append $a$ returns of length two and $b$ returns of length
    three at the terminal index. This gives a positive path of common length
    $2L+2$ between every ordered pair, so every entry of $T^{2L+2}$ is
    positive.
\end{proof}

\begin{definition}[Active sector trace data]%
    \label{def:mpdo_active_sector_trace_matrix}
    \lean{MPOTensor.PhysicalSectorFactorization.ActiveSector}
    \lean{MPOTensor.PhysicalSectorFactorization.ActiveSectorEntryIndex}
    \lean{MPOTensor.PhysicalSectorFactorization.activeSectorVirtualMatrixFamily}
    ...
    \lean{MPOTensor.PhysicalSectorFactorization.rephase_neighboringOperator_ne_zero_iff}
    \leanok
    For a physical-sector factorization with nonnegative sector weights
    $(p_k)_k$, let $I_+=\{k:p_k\ne0\}$. On this index set define
    \[
        T_{k,h}=\operatorname{Re}\tr(\eta_{k,h}).
    \]
    If every neighboring operator is positive semidefinite, then $T$ is
    entrywise nonnegative, and $T_{k,h}>0$ exactly when
    $\eta_{k,h}\ne0$. Reciprocal rephasing within each sector preserves the
    sector virtual matrices and the nonzero neighboring support.
\end{definition}

\begin{lemma}[Strong connectivity of active sector support]%
    \label{lem:mpdo_active_sector_support_strongly_connected}
    \lean{MPOTensor.PhysicalSectorFactorization.activeSectorVirtualSubmodule}
    \lean{MPOTensor.PhysicalSectorFactorization.activeSectorSet}
    \lean{MPOTensor.PhysicalSectorFactorization.activeSectorVirtualMatrixFamily_span_eq_top}
    \lean{MPOTensor.PhysicalSectorFactorization.activeSectorVirtualSubmodule_sup_compl_eq_top}
    \lean{MPOTensor.PhysicalSectorFactorization.activeSectorVirtualSubmodule_ne_bot}
    \lean{MPOTensor.PhysicalSectorFactorization.activeSectorVirtualSubmodule_eq_oneSiteSectorSpace}
    \lean{MPOTensor.PhysicalSectorFactorization.mul_eq_zero_of_mem_activeSectorVirtualSubmodule}
    \lean{MPOTensor.PhysicalSectorFactorization.activeSector_reflTransGen_neighboringOperator_ne_zero}
    \lean{MPOTensor.PhysicalSectorFactorization.exists_active_neighboringOperator_ne_zero}
    \lean{MPOTensor.PhysicalSectorFactorization.activeSectorTraceMatrix_isIrreducible}
    \leanok
    Suppose that the active-sector virtual matrices span $\MN{D}$ and that
    every active sector contains a nonzero virtual matrix. Then the support of
    the active trace matrix is strongly connected; equivalently, the active
    trace matrix is irreducible whenever the neighboring operators are
    positive semidefinite.
\end{lemma}

\begin{proof}
    \uses{thm:mpdo_directed_cut_one_site_spaces,
        lem:matrix_one_sided_product_obstruction}
    \leanok
    Fix an active sector $k$ and let $S$ be the active sectors reachable from
    it. If an active sector lay outside $S$, the virtual matrix spaces
    generated by $S$ and its complement would both be nonzero and their sum
    would be $\MN{D}$. There is no neighboring edge from $S$ to its
    complement, so every product from the first space to the second vanishes.
    This contradicts
    Lemma~\ref{lem:matrix_one_sided_product_obstruction}.
\end{proof}

\begin{lemma}[Two- and three-step active returns]%
    \label{lem:mpdo_active_sector_two_three_returns}
    \lean{MPOTensor.PhysicalSectorFactorization.exists_active_two_cycle}
    \lean{MPOTensor.PhysicalSectorFactorization.activeSectorTraceMatrix_pow_two_diag_pos}
    \lean{MPOTensor.PhysicalSectorFactorization.activeSectorTraceMatrix_pow_three_diag_pos}
    \lean{MPOTensor.PhysicalSectorFactorization.activeSectorTraceMatrix_isPrimitive}
    \leanok
    Under active-sector spanning and sector nonvanishing, every active sector
    lies on a closed support walk of length two. If every nonzero edge closes
    through a third active sector, then every active sector also lies on a
    closed support walk of length three. For positive semidefinite neighboring
    operators, the active trace matrix is primitive.
\end{lemma}

\begin{proof}
    \uses{lem:matrix_primitive_of_two_three_returns,
        thm:mpdo_sector_support_triangle_closure}
    \leanok
    For an active sector choose a nonzero virtual matrix $A$. Nondegeneracy of
    the trace pairing and active-sector spanning give a virtual matrix $B$ in
    an active sector with $\tr(AB)\ne0$. Cyclicity gives
    $\tr(BA)\ne0$, and hence the two corresponding neighboring edges form a
    closed walk of length two. Triangle closure supplies a closed walk of
    length three. Apply
    Lemma~\ref{lem:matrix_primitive_of_two_three_returns}.
\end{proof}
...
\begin{theorem}[Primitivity of the active sector trace matrix]%
    ...
    \lean{MPOTensor.exists_rephased_inverseMap_activeSectorTraceMatrix_isPrimitive}
    \lean{MPOTensor.exists_positive_physicalSectorFactorization_activeSectorTraceMatrix_isPrimitive_of_isSAL}
    \leanok
    \uses{def:mpdo, def:injective, def:is_sal,
...
\begin{proof}
    \uses{def:mpdo_eta_recurrent_support,
        ...
        lem:mpdo_active_sector_support_strongly_connected,
        lem:mpdo_active_sector_two_three_returns,
        lem:matrix_one_sided_product_obstruction}
    \leanok
