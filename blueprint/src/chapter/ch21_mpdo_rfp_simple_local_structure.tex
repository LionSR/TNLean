...
\begin{theorem}[Positive rescaling of a finite Kronecker product]%
    ...
    \leanok
    \uses{thm:kronecker_factor_positive_rescaling}
    ...
    \]
    Moreover, if two nonzero scalar multiples of the same nonzero matrix are
    positive semidefinite, then the quotient of their scalars is a positive
    real number. Thus the phase of a positive rescaling is unique.
\end{theorem}
...
\begin{proof}
    ...
    For uniqueness, Hermiticity of $c_1M$ and $c_2M$ gives
    $c_2\overline{c_1}=c_1\overline{c_2}$. Hence $c_1/c_2$ is real. It cannot
    be negative: otherwise $c_1M$ and its negative would both be positive
    semidefinite, forcing $c_1M=0$. Since $c_1$ and $M$ are nonzero, the
...
\begin{theorem}[Positive rescaling around one sector cycle]%
    ...
    Fix a cyclic sector assignment $k_0,\ldots,k_{N-1}$ such that every
    operator $\eta_{k_n,k_{n+1}}$ is nonzero. If the corresponding cyclic
    tensor product is positive semidefinite, then there are nonzero scalars
    $c_n$ satisfying
    \[
        \prod_{n=0}^{N-1}c_n=1,
...
\begin{proof}
    \leanok
    \uses{thm:finite_kronecker_positive_rescaling}
...
\begin{theorem}[Coherent positive choice of the neighboring operators]%
    ...
    \uses{def:mpdo, def:mpdo_sector_eta,
        ...
        thm:finite_kronecker_positive_rescaling}
    Let ${\cal K}$ be an injective simple MPDO satisfying the strong area
    law. The sector tensors in the factorization of
...
\begin{proof}
    \uses{thm:mpdo_sector_adapted_decomposition,
        thm:finite_kronecker_positive_rescaling}
    ...
    theorem chooses positive phases on any one nonzero cycle. What remains is
    to prove that these choices agree on common edges of different cycles,
    using the recurrence and sector structure furnished by injectivity. This
    compatibility is the coherent-choice step asserted at
...
\begin{theorem}[Primitivity of the concrete sector trace matrix]%
    ...
    Let ${\cal K}$ be an injective simple MPDO satisfying the strong area
    law, and choose the positive neighboring operators of
...
\begin{proof}
    \uses{lem:mpdo_local_orthogonality_contraction}
    If $T$ were reducible after the standard removal of periodic components,
    its sectors would split into two nonempty communicating classes. The
    corresponding orthogonal one-site projectors satisfy the three vanishing
    conditions in Lemma~\ref{lem:mpdo_local_orthogonality_contraction}. Let
    ${\cal A}_1$ and ${\cal A}_2$ be the matrix spaces generated by the two
    projected tensor families. The one-site vanishings give
    ...
    contraction and matrix-algebra contradiction remain to be formalized.
\end{proof}
