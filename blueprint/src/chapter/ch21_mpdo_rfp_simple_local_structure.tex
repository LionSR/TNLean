...
\begin{proof}
    \leanok
    \uses{thm:mpdo_sector_factorization}
    Substituting the sector factorization for each of the two slices, the
    outer sector tensors carry the outer virtual indices, and the bond sum
    collects the inner pair of sector tensors into the neighboring operator.
\end{proof}
...
\begin{proof}
    \leanok
    The trace of the bond summand $(r_k)_{\gamma}\otimes(l_h)_{\gamma}$ is
    the product $\tr[(r_k)_{\gamma}]\,\tr[(l_h)_{\gamma}]$, and summing over
    the bond gives the pairing of the closed tensors.
\end{proof}
...
\begin{definition}[Explicit $\eta$-data from the sector tensors]%
    ...
    \leanok
    \uses{def:mpdo_sector_eta, def:mpdo_closed_sector_tensors,
        def:mpdo_explicit_eta_operators, def:mpdo_explicit_eta_trace_matrix}
    If every neighboring operator $\eta_{k,h}$ of
    Definition~\ref{def:mpdo_sector_eta} is positive semidefinite, the
    family constitutes explicit neighboring $\eta$-data, and its trace
    matrix is the pairing of the closed sector tensors,
    $T_{k,h}=\tr(\eta_{k,h})=(r_k|l_h)$. The source obtains the positivity
    from the projected chain identity: for every cyclic sector assignment
    $k_1,\ldots,k_N$, the corresponding block of the conjugated chain equals
    $\bigotimes_{n=1}^{N}\eta_{k_n,k_{n+1}}$ and is positive semidefinite,
