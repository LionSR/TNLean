...
\begin{theorem}[Outer inverse-map contraction of a closure]
    ...
    \[
        ...
        R_{\beta_3,\alpha_1}.
    \]
    Consequently, for any matrix $U$ on the middle site, every entry of
    $R_{\beta_3,\alpha_1}\,U\kappa_{\beta_1,\alpha_3}U^\dagger$ expands into
    the corresponding outer inverse-map contraction of the state conjugated
    by $U$ on the middle site.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:mpdo_inverse_map_three_site_contraction}
    The first identity evaluates the double inverse-map contraction on the
    closure entries. The second follows by expanding the matrix products
    entrywise and reordering the finite sums.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{thm:mpdo_closure_outer_contraction}
    The block equality of the Hayashi decomposition makes the middle-site
    contraction between two distinct sectors vanish. Applying the inverse
    tensor at the first and third sites and moving the finite sums through
    the middle-site change of basis expresses the left-hand side as a sum of
    such vanishing contractions.
\end{proof}
