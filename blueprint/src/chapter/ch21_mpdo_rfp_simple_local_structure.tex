...
\begin{theorem}[Four-site SAL marginal has local $\eta$-structure]
    ...
    local $\eta$-structure on its middle site.

    This assertion is restricted to the four-site chain. It is the $N=4$
    specialization of the three-site decomposition in
    \cite[Appendix~C.2]{Cirac2016MPDO_arXiv}, which treats all chain lengths in
    the range of its four-region argument.
\end{theorem}
...
\begin{proof}
    ...
    Saturation gives equality in strong subadditivity for the three singleton
    factors in flattened block coordinates. Relabel each singleton factor by
    its canonical bijection with the one-site index. By
    Theorem~\ref{thm:ssa_equality_reindex_factors}, the resulting operator
    $\sigma_3^{(4)}(K)$ still satisfies the equality. Positivity and unit trace
    follow from the normalized reduced state, so
...
\begin{proof}
    ...
    \[
        ...
        R_{\beta_3,\alpha_1},
    \]
    which is the first identity. The second follows by expanding the matrix
    products entrywise and reordering the finite sums.
\end{proof}
...
\begin{proof}
    ...
    Expand the Hayashi block equality at fixed indices $(k,l,r)$ and
    $(k,l',r')$, and apply the inverse tensor at the first and third sites.
    Moving the two finite sums through the middle-site change of basis reduces
    the inner contraction to
    \[
        {\cal K}^{i_2j_2}_{\beta_1,\alpha_3}
        R_{\beta_3,\alpha_1}
    \]
    by the preceding theorem. Summing over $i_2,j_2$ gives the transformed
    physical slice on the left, while the Hayashi block equality gives the
    product $p_k A^{(k)}B^{(k)}$ on the right.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{thm:mpdo_reduced_four_three_tail}
    This restates the closed-word formula for the entries of
    $\sigma_3^{(4)}({\cal K})$ on the product of the three site indices.
\end{proof}
...
\begin{theorem}[Vanishing of the off-diagonal Hayashi sectors]
    ...
    \uses{def:mpdo_three_site_closure, def:mpdo_hayashi_inverse_factors}
    In the setting of
    Theorem~\ref{thm:mpdo_inverse_map_hayashi_sector_comparison}, every
    off-diagonal sector satisfies
    \[
        R_{\beta_3,\alpha_1}
...
\begin{theorem}[Existence of the sector tensors]
    ...
    sector tensors $l_k$ and $r_k$ such that
    \[
        U_B\,\kappa_{\beta_1,\alpha_3}\,U_B^\dagger
        =\bigoplus_k (l_k)_{\beta_1}\otimes(r_k)_{\alpha_3}
    \]
    for all virtual indices $\beta_1,\alpha_3$. The remaining steps of
