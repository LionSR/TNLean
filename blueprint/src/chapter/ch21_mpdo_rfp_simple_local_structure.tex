...
\begin{corollary}[Positive rescaling of the concrete sector operators around
    ...
    and fix an $\eta$-structure and a nonzero tail entry. For a cyclic sector
    assignment $k_0,\ldots,k_{N-1}$, suppose that every concrete operator
    $\eta_{k_n,k_{n+1}}$ obtained from these data is nonzero. Then there are
    nonzero scalars $c_n$ such that
    \[
        \prod_{n=0}^{N-1}c_n=1,
        \qquad c_n\eta_{k_n,k_{n+1}}\geq0
        \quad(0\leq n<N).
