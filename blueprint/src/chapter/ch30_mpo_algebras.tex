...
\begin{corollary}[Uniqueness of a single-target reduction]%
    ...
    Let $B$ be a tensor, $C$ a normal tensor, and $(W,V)$, $(\widetilde
        W,\widetilde V)$ two reductions of $B$ onto $C$ whose residual tensors
    $B-VCW$ and $B-\widetilde VC\widetilde W$ are nilpotent with a common
    bound $N_0$. Then there is a unique $\lambda\in\C^\times$ with
    \begin{align}
        \widetilde WB^{\mathbf w} & =\lambda\,WB^{\mathbf w},   &
        B^{\mathbf w}\widetilde V & =\lambda^{-1}B^{\mathbf w}V
...
\begin{theorem}[Uniqueness of zipper fusion tensors up to multiplicity gauge]%
    ...
    $(W_{ab}^{c,\mu},V_{ab}^{c,\mu})$ and
    $(\widetilde W_{ab}^{c,\mu},\widetilde V_{ab}^{c,\mu})$ be two families
    of fusion tensors for $(a,b)$ with vanishing remainder,
    \begin{align}
        (T_aT_b)^{\mathbf w}
         & =\sum_{c,\mu}V_{ab}^{c,\mu}T_c^{\mathbf w}W_{ab}^{c,\mu}
        =\sum_{c,\mu}\widetilde V_{ab}^{c,\mu}T_c^{\mathbf w}
        \widetilde W_{ab}^{c,\mu}
...
\begin{proof}
    ...
    $\widetilde W_{ab}^{c,\mu}$ on the left and $V_{ab}^{e,\nu}$ on the right
    and use biorthogonality of both families:
    $T_c^{\mathbf w}\,\widetilde W_{ab}^{c,\mu}V_{ab}^{e,\nu}
        =\widetilde W_{ab}^{c,\mu}V_{ab}^{e,\nu}\,T_e^{\mathbf w}$ for every
    ...
    $e=c$. This gives $\widetilde W_{ab}^{c,\mu}V_{ab}^{e,\nu}=
...
\begin{theorem}[Uniqueness of zipper action tensors up to multiplicity gauge]%
    ...
    vanishing remainder, $(T_a\cdot A_x)^w=\sum_{y,i}V_{ax}^{y,i}A_y^w
        W_{ax}^{y,i}$ for every nonempty word, are related by a unique
...
\begin{remark}[Uniqueness fails with a nilpotent remainder]%
    ...
    \begin{align}
        B^i & =\begin{pmatrix}C^i & R^i\\ 0 & 0\end{pmatrix},
            &
        V   & =\begin{pmatrix}\Id_D\\ 0\end{pmatrix},
            &
        W_y & =\begin{pmatrix}\Id_D & y\end{pmatrix}
        \label{eq:mpoalg_nonunique}
    \end{align}
    for $y\in\C^D$. Then $W_yV=\Id_D$, $W_yB^wV=C^w$ for every word $w$, and
    the remainder $B^i-VC^iW_y$ has vanishing products of length two, so
    every $(W_y,V)$ is a reduction of $B$ onto $C$ with nilpotent remainder.
    For $y\neq0$ the compressions $W_y$ and $W_0$ are not proportional,
    although $W_yB^w=W_0B^w$ for every nonempty word; with a single target,
    ...
    $\widetilde W_\mu=W_\mu-ZW_\nu$. Then
    $\widetilde W_\mu B^{\mathbf w}=W_\mu B^{\mathbf w}-ZW_\nu B^{\mathbf w}$
    on every long word, which is not a combination of the
    $W_\kappa B^{\mathbf w}$ with scalar coefficients unless $Z$ is scalar.
