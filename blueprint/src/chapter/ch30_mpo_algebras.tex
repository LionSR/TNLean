...
\begin{lemma}[Integrality of multi-block word-trace relations]%
    \label{lem:mpoalg_multiblock_trace_integral}
...
\begin{proof}
    ...
    Each word module of $A_y$ is simple. An element of the augmentation ideal
    of the word algebra acts on the block $y_0$ as a matrix unit and
    annihilates the other blocks; by the trace relation, every positive power
    of its action on the word module of $B$ has trace $c_{y_0}$. A matrix
    whose positive powers all have the same trace has all its nonzero
    eigenvalues equal to $1$, so that trace is the multiplicity of the
    eigenvalue $1$, a nonnegative integer.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{lem:mpoalg_multiblock_trace_integral}
    ...
    integer (Lemma~\ref{lem:mpoalg_multiblock_trace_integral}). Applying
