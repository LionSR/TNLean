...
A matrix product operator symmetry is given by a finite family of operators
satisfying the fusion rule~\eqref{eq:mpo_fusion_algebra}~\cite{GarreRubio2023MPOSym}.
Under periodic boundary conditions, the symmetry acts on periodic matrix product
states as an invariant subspace~\cite[Section~3]{GarreRubio2023MPOSym}; see
...
\begin{definition}[Unit and invertible labels]%
    ...
    $N_{ab}^c=N_{ba}^c=\delta_{ce}$ for all $c$. Outside the on-site case
    where $O_e^{(N)}=\Id$, the unit operator is in general a projector
    $(O_e^{(N)})^2=O_e^{(N)}$ \cite[Section~4]{GarreRubio2023MPOSym}.
...
\begin{proof}
    ...
    \end{align}
    Since $\tr(Q^k) = \sum_i \lambda_i^k = c_{y_0}$ for all $k\geq1$, the
    nonzero eigenvalues satisfy $\lambda_i=1$, so
    $c_{y_0}=\operatorname{rank}(Q)\in\N$, extending the power-sum argument of
    Theorem~\ref{thm:asymex_fib_anomaly_trace_integral} to multiple blocks.
\end{proof}
...
\begin{corollary}[Rank-one obstruction]%
    ...
    $c_e=1$. This obstruction depends only on the fusion ring;
    Theorem~\ref{thm:mpoalg_no_symmetric_normal} gives the cohomological
    obstruction for group-like algebras with a nontrivial three-cocycle.
\end{corollary}
...
\begin{example}[Anomalous cyclic group of order three]%
    ...
    which every label is invertible. The fusion ring isomorphism
    $\C[\Z_3]\cong \C^3$ does not detect the nontrivial three-cocycle
    $[\omega]\neq 1\in H^3(\Z_3,\mathrm{U}(1))$ of this representation; the
    obstruction to invariant normal states comes from
