...
\begin{definition}[Associator]%
    ...
    \begin{align}
        V^{g(hk)}(T_gT_hT_k)^{\mathbf w}
         & =\omega(g,h,k)\,V^{(gh)k}(T_gT_hT_k)^{\mathbf w}
...
\begin{proof}
    ...
    $x(yz)$ by $(xy)z$ at the cost of $\omega(x,y,z)$. Along the two paths of
    the pentagon this gives
    \begin{align}
        V^{g(h(kl))}X & =\omega(g,h,kl)\,V^{(gh)(kl)}X
        =\omega(g,h,kl)\,\omega(gh,k,l)\,V^{((gh)k)l}X,
        \notag                                                          \\
        V^{g(h(kl))}X & =\omega(h,k,l)\,V^{g((hk)l)}X
        =\omega(h,k,l)\,\omega(g,hk,l)\,V^{(g(hk))l}X
        \notag                                                          \\
                      & =\omega(h,k,l)\,\omega(g,hk,l)\,\omega(g,h,k)\,
        V^{((gh)k)l}X.
        \label{eq:mpoalg_pentagon_paths}
    \end{align}
    Since $V^{((gh)k)l}XW^{((gh)k)l}=T_{ghkl}^{\mathbf w}$ is nonzero for some
...
\begin{theorem}[Gauge dependence of the associator]%
    ...
    Replacing the fusion tensors by $(\beta_{g,h}V_{g,h},
        \beta_{g,h}^{-1}W_{g,h})$ with $\beta\colon G^2\to\C^\times$ replaces
...
\begin{proof}
    ...
    (\ref{eq:mpoalg_composite_compressions}) multiplies $V^{g(hk)}$ by
    $\beta_{g,hk}\beta_{h,k}$ and $V^{(gh)k}$ by $\beta_{g,h}\beta_{gh,k}$,
...
\begin{remark}[F-symbols with multiplicity under the zipper condition]%
    ...
    algebra, $F^{ghk}_{ghk}=\omega(g,h,k)$ and the pentagon equation is
...
\begin{definition}[L-symbols of an action]%
    ...
    defined by
    \begin{align}
        V_{gh,x}(V_{g,h}\otimes\Id)(T_gT_h\cdot A_x)^w
         & =L^x_{g,h}\,V_{g,h\cdot x}(\Id\otimes V_{h,x})
        (T_gT_h\cdot A_x)^w
...
\begin{proof}
    ...
    \begin{align}
        V^{(g(hk))x}Y & =L^x_{g,hk}\,V^{g((hk)x)}Y
        =L^x_{g,hk}\,L^x_{h,k}\,V^{g(h(kx))}Y,
        \notag                                                           \\
        V^{(g(hk))x}Y & =\omega(g,h,k)\,V^{((gh)k)x}Y
        =\omega(g,h,k)\,L^x_{gh,k}\,V^{(gh)(kx)}Y
        \notag                                                           \\
                      & =\omega(g,h,k)\,L^x_{gh,k}\,L^{k\cdot x}_{g,h}\,
        V^{g(h(kx))}Y.
        \label{eq:mpoalg_l_paths}
    \end{align}
    Since $V^{g(h(kx))}YW^{g(h(kx))}=A_{ghk\cdot x}^w$ is nonzero for some
