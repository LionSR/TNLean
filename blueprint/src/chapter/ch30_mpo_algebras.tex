...
\begin{theorem}[Uniqueness of fusion tensors up to multiplicity gauge]%
    ...
    \end{align}
    If $N_{ab}^c=1$, then $Y_{ab}^c$ is a nonzero scalar. The sources state
    uniqueness of the fusion tensors themselves up to this gauge
    \cite[Section~2]{GarreRubio2023MPOSym}, \cite[Section~4]{Molnar2022MPOAlgebrasI};
    with a nilpotent remainder only the boundary-dressed form
    of \cite[Theorem~22]{Molnar2018SemiInjective} holds
...
\begin{remark}[Fusion tensors are not unique up to scalars]%
    \label{rem:mpoalg_nonunique}
    With only a nilpotent remainder, two reductions onto the same normal
    tensor need not be proportional. Let $C$ be normal of bond dimension $D$,
    let $R^i\in\C^{D\times1}$ be arbitrary, and let
    \begin{align}
    ...
    the remainder $B^i-VC^iW_y$ has vanishing square products, so every
    $(W_y,V)$ is a reduction of $B$ onto $C$ with nilpotent remainder. For
    $y\neq0$ the compressions $W_y$ and $W_0$ are not proportional, while
    $W_yB^w=W_0B^w$ for every nonempty word, since the last row of $B^i$
    vanishes. Only the boundary-dressed identities of
    Theorem~\ref{thm:mpoalg_fusion_gauge_uniqueness} survive. The zipper
    condition of \cite{Bultinck2017Anyons}, which asks that the fusion tensors
    intertwine each letter of the stacked tensor with the letter of the
    block, excludes such remainders; under it the fusion tensors are unique
...
\begin{remark}[F-symbols with multiplicity under the zipper condition]%
    ...
    and Remark~\ref{rem:mpoalg_nonunique} shows that the fusion tensors
    themselves are then not determined up to the multiplicity gauge. An
    $F$-move holds only after dressing with long words, as in
    (\ref{eq:mpoalg_associator}); its formulation with multiplicity is not
    stated here.
