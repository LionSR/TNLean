\begin{definition}[Incident form of the region resonance]
    ...
    in-region endpoint operator is in incident-matrix form, namely
    $\exists Y\colon W_X=\mc{I}_f(Y)$.
\end{definition}
...
\begin{definition}[Region insertion transfer]
    ...
    of maps
    $T_f:\MN{D_A(f)}\to\MN{D_B(f)}$ and
    $S_f:\MN{D_B(f)}\to\MN{D_A(f)}$.
    For every inserted matrix and every pair of physical configurations, these
    maps satisfy
    \begin{align}
        C_A(R,f,X;\sigma,\tau)
    ...
    $T_f(XY)=T_f(X)T_f(Y)$ and $T_f(\Id)=\Id$.
\end{definition}
...
\begin{definition}[Region physical-to-virtual realization]
    ...
    \begin{align}
        O_{A,R,f}(X^T) \mc{T}_{B,v}(c)
         & =
        \mc{T}_{B,v}\!\left(\mc{I}_f(T_f(X)^T)c\right).
        \notag
    \end{align}
...
\begin{theorem}[Realized region transfers are multiplicative and unital]
    ...
    multiplicative: $T_f(XY)=T_f(X)T_f(Y)$. If in addition the two PEPS tensors
    have the same state and the same bond dimensions, all bond dimensions of
    $B$ are positive, and the blocked tensors of $B$ on $R$ and its complement
    are injective, then the identity insertion gives $T_f(\Id)=\Id$.
\end{theorem}
\begin{proof}\leanok
    ...
    image,
    $\mc{I}_f(T_f(XY)^T)
        =\mc{I}_f((T_f(X)T_f(Y))^T)$.
    Reading the incident matrix on the boundary leg gives
    $T_f(XY)=T_f(X)T_f(Y)$. For the identity, the coefficient
    $C_A(R,f,\Id;\sigma,\tau)$ is the interior bond product times the state
    coefficient of $A$, hence equals $C_B(R,f,\Id;\sigma,\tau)$ by equality of
...
\begin{definition}[Region insertion transfer from realized maps]
    ...
    \leanok
    \uses{def:peps_region_transfer_realizes, thm:peps_region_transfer_realized_maps}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    together with $T_f(XY)=T_f(X)T_f(Y)$ and $T_f(\Id)=\Id$.
\end{definition}
...
\begin{theorem}[Incident form gives the region insertion transfer and gauge]
    ...
    \leanok
    \uses{def:peps_region_resonate_reconcile, thm:peps_hform_of_region_incident_form, def:peps_same_state, def:IsVertexInjective, def:peps_regionInjectivityData}
    ...
    $T_f(X)=Z_f\,\iota_f(X)\,Z_f^{-1}$, where
    $\iota_f:\MN{D_A(f)}\simeq\MN{D_B(f)}$ is the identification induced
...
\begin{proof}\leanok
    ...
    yield the transfer datum, and the one-bond algebra theorem reads it as
...
\begin{theorem}[Coefficient transfer gives incident form]
    ...
    $C_A(R,f,X;\sigma,\tau)=C_B(R,f,Y;\sigma,\tau)$ for all $\sigma,\tau$.
    Then the virtual pullback of $X$ is in incident-matrix form on $f$.
\end{theorem}
\begin{proof}\leanok
...
\begin{proof}\leanok
    ...
    region block. The two left inverses define the coefficient $K_X$.
    Substituting (\ref{eq:peps_transfer_coeff_form}) into the double contraction
    gives the defining double sum for $C_B(R,f,Y;\sigma,\tau)$.
\end{proof}
...
\begin{theorem}[Incident form of the transfer coefficient]
    ...
    \leanok
    \uses{thm:peps_region_transfer_coeff_double_factorization, thm:peps_hform_of_region_incident_form, thm:peps_region_insertion_virtual_pullback_realizes}
    ...
    $W_X=\mc{I}_f(Y^T)$, then the transfer coefficient $K_X$ has the
...
\begin{proof}\leanok
    ...
    the row $v_X^\sigma$. If $W_X=\mc{I}_f(Y^T)$, this row is precisely the
    ...
    configuration, giving the stated formula.
\end{proof}
...
\begin{theorem}[Coefficient transfer matches coefficients and preserves the identity]
    ...
    For every inserted matrix $X$ and every pair of physical configurations,
    the chosen coefficient transfer satisfies
    $C_A(R,f,X;\sigma,\tau)
        =C_B(R,f,T_f(X);\sigma,\tau)$.
    ...
    blocked maps of $B$ are injective, then $T_f(\Id)=\Id$.
\end{theorem}
\begin{proof}\leanok
    The coefficient equality is the defining property of the chosen transfer.
    For $X=\Id$, both region-inserted coefficients are the corresponding interior
    bond products times the common PEPS state coefficient, so
    $C_A(R,f,\Id;\sigma,\tau)=C_B(R,f,\Id;\sigma,\tau)$.
    ...
    the chosen matrix $T_f(\Id)$ is equal to $\Id$.
\end{proof}
...
\begin{definition}[Region insertion transfer from coefficient transfers]
    ...
    \leanok
    \uses{def:peps_region_insertion_transfer, def:peps_coeff_transfer_map, thm:peps_coeff_transfer_map_properties}
    ...
    is multiplicative, $T_f(XY)=T_f(X)T_f(Y)$, then $T_f$ and $S_f$ form a
...
\begin{theorem}[Per-edge gauge from coefficient transfers]
    ...
    $X\in\MN{D_A(f)}$, one has
    $T_f(X)=Z_f\,\iota_f(X)\,Z_f^{-1}$, where
    $\iota_f:\MN{D_A(f)}\simeq\MN{D_B(f)}$ is the identification induced by
...
\begin{theorem}[Determinacy of a region insertion transfer]
    ...
    for every inserted matrix and every pair of physical configurations.
    Then $T_1(M)=T_2(M)$ for every $M$.
\end{theorem}
\begin{proof}\leanok
    For every pair of physical configurations, the two coefficient identities
    give the dense equality chain
    \begin{align}
    ...
    \begin{align}
        \left(\forall\sigma,\tau,
        C_B(R,f,T_1(M);\sigma,\tau)
        =
        C_B(R,f,T_2(M);\sigma,\tau)\right)
         & \Longrightarrow T_1(M)=T_2(M).
        \notag
    \end{align}
\end{proof}
...
\begin{proof}\leanok
    ...
    $T_i(M):=Z_i\,\iota(M)\,Z_i^{-1}$. Its coefficient identity is
    $C_A(R,f,M;\sigma,\tau)=C_B(R,f,T_i(M);\sigma,\tau)$.
    Theorem~\ref{thm:peps_regionInsertedCoeff_transferMap_unique} gives
    $T_1(M)=T_2(M)$. Substituting the definitions of $T_1$ and $T_2$ yields
    $Z_1\,\iota(M)\,Z_1^{-1}=Z_2\,\iota(M)\,Z_2^{-1}$.
\end{proof}
