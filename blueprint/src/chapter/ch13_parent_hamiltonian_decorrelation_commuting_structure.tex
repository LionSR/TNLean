\begin{definition}[Parent commuting Hamiltonian on the tripartite space]
    ...
    $\ran P=\ran\widehat P_{AX}\cap\ran\widehat P_{XB}$.
    Thus $Q_{AX}=\Id-P_{AX}$ and $Q_{XB}=\Id-P_{XB}$ are exactly the
    commuting parent terms of
...
\begin{theorem}[Ground-space intersection and projector product]
    ...
    $\widehat P_{AX}=P_{AX}\otimes\Id_B$ and
    $\widehat P_{XB}=\Id_A\otimes P_{XB}$.
    ...
    \begin{align}
        \ran P
        =\ran\widehat P_{AX}\cap\ran\widehat P_{XB}
        \quad\Longleftrightarrow\quad
        \widehat P_{AX}\widehat P_{XB}=P.
        \label{eq:parent_tripartite_projector_product}
    \end{align}
...
\begin{proof}
    ...
    equal ranges are equal, giving the forward implication in
    (\ref{eq:parent_tripartite_projector_product}); the reverse implication
    follows by substituting the product identity.
\end{proof}
...
\begin{theorem}[Parent commuting Hamiltonian--complex-linear decorrelation equivalence]
    ...
    \leanok
    \uses{def:tripartite_decorrelation, def:tripartite_parent_commuting_hamiltonian, thm:tripartite_support_products, thm:tripartite_ground_space_intersection_iff_product}
...
\begin{proof}
    ...
    $\widehat P_{XB}(\Id-P)\widehat P_{AX}=0$.
...
\begin{theorem}[Parent commuting Hamiltonian--observable decorrelation equivalence]
    ...
    \leanok
    \uses{def:tripartite_observable_decorrelation, thm:tripartite_observable_decorrelation_iff, thm:tripartite_parent_iff_complex_linear_decorrelated}
    Let $P=P^\dagger=P^2$. Then $P$ is the ground-space projector of a
    parent commuting Hamiltonian on $AX$ and $XB$ if and only if regions
...
\begin{definition}[Decorrelation]\label{def:is_decorrelated}
    ...
    when $P_K\circ O_A\circ(\Id-P_K)\circ O_B\circ P_K=0$
    for all $O_A\in\mathcal{O}_A$ and $O_B\in\mathcal{O}_B$.
\end{definition}

\begin{definition}[Idempotent-product parent-commuting structure]
    \label{def:has_commuting_parent_ham}
...
\begin{theorem}[Idempotence gives tautological idempotent-product data]%
    ...
    If $P_K\circ P_K=P_K$, then $P_K$ has idempotent-product data.
\end{theorem}
...
\begin{proof}
    ...
    \begin{align}
        P_{AX}\circ P_K
        &=P_{AX}\circ P_{AX}\circ P_{XB} =P_{AX}\circ P_{XB} =P_K.
        \notag
    \end{align}
\end{proof}
...
\begin{proof}
    ...
    \begin{align}
        P_K\circ P_{XB}
        &=P_{AX}\circ P_{XB}\circ P_{XB} =P_{AX}\circ P_{XB} =P_K.
        \notag
    \end{align}
\end{proof}
...
\begin{proof}
    \leanok
    \uses{def:has_commuting_parent_ham}
    By commutativity and the idempotence of $P_{XB}$,
    \begin{align}
        P_{XB}\circ P_K
        &=P_{XB}\circ P_{AX}\circ P_{XB} =P_{AX}\circ P_{XB}\circ P_{XB} =P_{AX}\circ P_{XB} =P_K.
        \notag
    \end{align}
\end{proof}
...
\begin{proof}
    \leanok
    \uses{def:has_commuting_parent_ham}
    By commutativity and the idempotence of $P_{AX}$,
    \begin{align}
        P_K\circ P_{AX}
        &=P_{AX}\circ P_{XB}\circ P_{AX} =P_{AX}\circ P_{AX}\circ P_{XB} =P_{AX}\circ P_{XB} =P_K.
        \notag
    \end{align}
\end{proof}
...
\begin{theorem}[Commuting complementary projectors]%
    ...
    $Q_{AX}=\Id-P_{AX}$ and $Q_{XB}=\Id-P_{XB}$, then
...
\begin{proof}
    \leanok
    \uses{def:has_commuting_parent_ham}
    Expanding both products and using
    $P_{AX}\circ P_{XB}=P_{XB}\circ P_{AX}$ shows that the complementary
    projectors commute.
\end{proof}
...
\begin{proof}
    ...
    The key identity is
    $P_{XB}\circ(\Id-P_{AX}\circ P_{XB})\circ P_{AX}=0$.
    Substituting $P_K=P_{AX}\circ P_{XB}$ and using
    $O_A\circ P_{XB}=P_{XB}\circ O_A$ and
    $O_B\circ P_{AX}=P_{AX}\circ O_B$ gives
    \begin{align}
        P_K\circ O_A\circ(\Id-P_K)\circ O_B\circ P_K
        &=
        P_{AX}\circ O_A\circ
        \bigl[
          P_{XB}\circ(\Id-P_{AX}\circ P_{XB})\circ P_{AX}
        \bigr]
...
\begin{theorem}[Decorrelating idempotent-product data]%
    ...
    $P_K$ has idempotent-product data, the $A$-observables commute with
    $P_{XB}$, and the $B$-observables commute with $P_{AX}$, then regions
...
\begin{proof}
    ...
    endomorphisms $P_{AX}$ and $P_{XB}$ supplied by the idempotent-product
    data.
\end{proof}
...
\begin{lemma}[Frustration-free identity]\label{lem:frustration_free_ham_eq}
    ...
    \begin{align}
        (\Id-P)+(\Id-Q)-(\Id-P)\circ(\Id-Q)
        =\Id-P\circ Q.
        \label{eq:parent_frustration_free_identity}
    \end{align}
\end{lemma}
...
\begin{proof}
    ...
    (\ref{eq:parent_frustration_free_identity}) and cancelling gives the
    result.
\end{proof}
...
\begin{theorem}[Idempotent-product Hamiltonian identity]%
    ...
    If $P_K=P_{AX}\circ P_{XB}$ is idempotent-product data, then
    \begin{align}
        Q_{AX}+Q_{XB}-Q_{AX}\circ Q_{XB}
        =\Id-P_K,
        \label{eq:parent_idempotent_product_hamiltonian}
    \end{align}
    where $Q_{AX}=\Id-P_{AX}$ and $Q_{XB}=\Id-P_{XB}$ are the
...
\begin{proof}
    ...
    Apply Lemma~\ref{lem:frustration_free_ham_eq} to $P_{AX}$ and $P_{XB}$,
    and substitute $P_{AX}\circ P_{XB}=P_K$ to obtain
    (\ref{eq:parent_idempotent_product_hamiltonian}).
\end{proof}
...
\begin{theorem}[Ground-space membership]\label{thm:mem_ground_iff}
    ...
    \begin{align}
        ...
        \text{ and }
        P_{XB}v=v.
        \label{eq:parent_ground_space_membership}
    \end{align}
...
\begin{proof}
    ...
    ($\Rightarrow$) If $P_Kv=v$, then
    \begin{align}
        P_{AX}(P_Kv)
        &=(P_{AX}\circ P_K)v =P_Kv =v
        \notag
    \end{align}
    by left absorption, and similarly for $P_{XB}$.

    ($\Leftarrow$) If $P_{AX}v=v$ and $P_{XB}v=v$, then
    \begin{align}
        P_Kv
        &=(P_{AX}\circ P_{XB})v =P_{AX}(P_{XB}v) =P_{AX}v =v.
        \notag
    \end{align}
    This proves (\ref{eq:parent_ground_space_membership}).
\end{proof}
