...
\begin{theorem}[Gauge invariance of simultaneous fixed-length product spans]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Both directions hold for general invertible gauges without unitarity
    constraints, allowing span certificates in left-canonical form to be
    transported to unital gauges.
\end{theorem}
...
\begin{lemma}[Direct-sum block injectivity at the source length]
    ...
    \leanok
    \uses{def:blockwise_product_word_span, def:blocks_not_gauge_phase_equiv, lem:bnt_direct_sum_injective_prefix_word_span}
...
\begin{theorem}[Simultaneous one-letter span after blocking a BNT]
    ...
    \leanok
    \uses{def:bnt_cpsv, def:blocked_tensor, def:blockwise_product_word_span}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This establishes simultaneous block injectivity following
    \cite[lines~317--345]{Cirac2016MPDO_arXiv}.
\end{theorem}

\begin{proof}\leanok
    \uses{lem:cpsv_bnt_block_dimensions_positive, thm:cpsv_bnt_blocks_not_gaugePhaseEquiv, thm:cpsv_normal_tp_gauge, thm:normal_tp_primitive_irred, thm:common_exact_block_injectivity_normal_left_canonical, thm:wordspan_blk_inj_succ, lem:bnt_direct_sum_block_separation_word_span, lem:gauge_invariance_block_injective}
...
\begin{proof}
    \leanok
    \uses{lem:cpsv_bnt_block_dimensions_positive, thm:cpsv_bnt_blocks_not_gaugePhaseEquiv, thm:cpsv_normal_tp_gauge, thm:normal_tp_primitive_irred, thm:exact_pow_four_block_injectivity_normal_left_canonical, thm:wordspan_blk_inj_succ, lem:bnt_direct_sum_block_separation_word_span, thm:word_tuple_span_gauge_invariance}
...
\begin{theorem}[Total dimension of the BNT representatives]
    \label{thm:cpsv_bnt_dimension_bound}
    \lean{MPSTensor.CPSVCanonicalFormData.sum_representative_dim_le}
    \leanok
    \uses{def:cpsv_canonical_form, thm:cpsv_canonical_form_bnt_refinement}
    Let literal canonical-form data of ambient bond dimension $D$ have
...
\begin{proof}
    ...
    In every phase class, choose its first copy. The class-copy
...
\begin{theorem}[Sharp block-injective form for literal CPSV canonical form]
    ...
    \leanok
    \uses{def:cpsv_canonical_form, def:bnt_cpsv, def:blocked_tensor, def:blockwise_product_word_span}
...
\begin{proof}
    \leanok
    \uses{thm:cpsv_canonical_form_bnt_refinement, thm:cpsv_bnt_dimension_bound, thm:cpsv_bnt_sharp_span_ambient_cap, thm:cpsv_bnt_blocked_simultaneous_span}
    Choose one representative from every matrix-product-vector phase
    class. These representatives form a basis of normal tensors, and their
    total bond dimension is at most $D$ by
    Theorem~\ref{thm:cpsv_bnt_dimension_bound}. Apply
