...
For physical dimension $d$, the local contraction admits separate horizontal
virtual dimension $D_{\mathrm h}$ and vertical virtual dimension
$D_{\mathrm v}$.
...
\begin{definition}[Local PEPS boundary tensor]
    ...
    This local ket--bra contraction corresponds to the diagram in
    \cite[lines~721--724]{Cirac2016MPDO_arXiv}. Under the PEPS renormalization
    hypothesis \cite[lines~716--724]{Cirac2016MPDO_arXiv}, this contraction
    represents the boundary tensor, with the common contracted inward leg denoted
    by $b$.
\end{definition}
...
\begin{theorem}[Component formula for the local boundary tensor]
    ...
    the bond coordinates are $(l,l')$ and $(r,r')$. Legs are ordered as
    left, right, up, down, physical, with horizontal ket--bra indices grouped
    into a single bond coordinate.
\end{theorem}
...
\begin{theorem}[The local PEPS boundary tensor is an LPDO]
    \label{thm:peps_local_boundary_tensor_is_lpdo}
    \lean{TNLean.PEPS.localBoundaryMPOTensor_isLPDO}
    \leanok
    \uses{def:peps_local_boundary_tensor, def:lpdo}
    The local boundary tensor $E_A$ is a locally purifiable density operator.
    One may take ancillary dimension $dD_{\mathrm v}$ with coordinate $(s,b)$,
    purifying bond dimension $D_{\mathrm h}$, and purifying matrices
    \begin{align}
        B^{(u;(s,b))}_{l,r}
         & = A^s_{l,r,u,b}.
        \notag
    \end{align}
    Under the PEPS renormalization hypothesis \cite[lines~716--724]{Cirac2016MPDO_arXiv},
    this contraction represents the boundary tensor. The Gram factorization
    exhibits $E_A$ as an LPDO for every rank-five tensor $A$, independently of
    any transfer fixed point or renormalization channel.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:peps_local_boundary_tensor_formula, def:lpdo}
    Identify each MPO bond coordinate with a pair of horizontal PEPS virtual
    coordinates. Substituting the displayed purifying matrices into the LPDO
    contraction and summing over the ancillary pair $(s,b)$ gives exactly
    (\ref{eq:peps_local_boundary_tensor_component}).
\end{proof}

\begin{corollary}[The local PEPS boundary tensor is an MPDO]
    \label{cor:peps_local_boundary_tensor_is_mpdo}
    \lean{TNLean.PEPS.localBoundaryMPOTensor_isMPDO}
    \leanok
    \uses{def:peps_local_boundary_tensor, def:mpdo}
    For every nonempty periodic chain, the matrix product operator generated
    by $E_A$ is positive semidefinite.
\end{corollary}

\begin{proof}
    \leanok
    \uses{thm:peps_local_boundary_tensor_is_lpdo, thm:lpdo_is_mpdo}
    Apply the general implication from local purification to matrix product
    density operators.
\end{proof}
...
\begin{definition}[Boundary fixed point]
    ...
    strip of local ket-bra transfer tensors. A boundary fixed point is a left or
    ...
    passage to the local boundary tensor and channels.
\end{definition}
...
\begin{theorem}[PEPS renormalization induces a boundary MPO fixed point]
    ...
    \notready
    \discussion{7371}
    \uses{def:peps_local_boundary_tensor, def:peps_transfer_fixed_point,
        def:peps_rfp_relation, def:peps_boundary_coarse_graining,
...
\begin{proof}
    ...
    \uses{def:peps_transfer_fixed_point, def:peps_rfp_relation,
        ...
        def:is_rfp_via_ts}
    The domain and codomain of the physical isometry, the virtual-space
    identifications, the transition from the finite-cylinder operators
    $\sigma_L,\sigma_R$ to the local tensor, and the normalization of the
    refinement channel are unspecified in \cite{Cirac2016MPDO_arXiv}; complete
    positivity, trace preservation, and the intertwining identities
    $\mathcal S[M_2(X)]=M_1(X)$ and $\mathcal T[M_1(X)]=M_2(X)$ await complete
    algebraic formulation \cite{gap:cpsv16_peps_boundary_rfp_specification}.
\end{proof}
