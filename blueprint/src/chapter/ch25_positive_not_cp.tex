...
\begin{proof}
    ...
    $\re\tr(W\rho)=\sum_i\re\tr\!\bigl(W\ket{\psi_i}\bra{\psi_i}\bigr)$,
    a sum of nonnegative terms, contradicting $\re\tr(W\rho)<0$.  This
...
\begin{proof}
    ...
    $\tr(W\ket{\psi}\bra{\psi})$; the witness condition makes it nonnegative on every
...
\begin{proof}
    ...
    $\tr(\tau_T\rho)=\tr\bigl(\rho\,(T\otimes\Id)(\ket{\Omega}\bra{\Omega})\bigr)$.  The
...
\begin{proof}
    ...
    \[
        \bra{\Omega}(T\otimes\Id)(\rho)\ket{\Omega}=\tr(W\rho),
    \]
    whose real part is negative.  A positive semidefinite matrix has nonnegative quadratic
    forms, so $(T\otimes\Id)(\rho)$ is not positive semidefinite.
\end{proof}
