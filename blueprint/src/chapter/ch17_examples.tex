...
\begin{remark}\label{rem:cluster_injective_blocked}
    ...
    the length-$2$ blocked tensor is injective.  Explicitly, the
    four products
    \[
        A^{00} = \tfrac{1}{2}\ketbra{+}{+},\quad
        A^{01} = \tfrac{1}{2}\ketbra{+}{-},\quad
        A^{10} = \tfrac{1}{2}\ketbra{-}{+},\quad
        A^{11} = \tfrac{1}{2}\ketbra{-}{-}
    \]
    (where $\ket{\pm} = \tfrac{1}{\sqrt{2}}(\ket{0}\pm\ket{1})$)
    are linearly independent and span~$\MN{2}$.
\end{remark}

\begin{theorem}[$\Z_2 \times \Z_2$ on-site symmetry of the cluster-state tensor]\label{thm:cluster_z2z2_symmetric}
    \notready
    \uses{def:cluster_tensor, def:on_site_symmetric}
    The cluster-state tensor is on-site symmetric under
    $\Z_2 \times \Z_2$, generated by the Pauli matrices $\sigma_x$
    and $\sigma_z$ acting on the physical index.
    The projective phase is the non-trivial element of
    $H^2(\Z_2 \times \Z_2, \mathrm{U}(1)) \cong \Z_2$, witnessing
