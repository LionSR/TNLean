...
\begin{lemma}[Valence-bond form of the cluster-state tensor]\label{lem:cluster_valence_bond}
    \lean{MPSTensor.clusterTensorVBS, MPSTensor.clusterTensorVBS_eq_smul}
    \leanok
    \uses{def:cluster_tensor_review}
    ...
    $\ket{0}\bra{00} + \ket{1}\bra{11}$. On a chain this is the tensor
    $\ketbra{s}{s} H$ with
    $H = \bigl(\begin{smallmatrix} 1 & 1 \\ 1 & -1 \end{smallmatrix}\bigr)$,
    which equals $\sqrt{2}\,A^s$.
\end{lemma}
...
\begin{proof}
    ...
    \ref{thm:cluster_string_order} and~\ref{thm:cluster_zcl}. For the string
    order, the four blocked matrices
    $\tfrac{1}{\sqrt{2}}\ketbra{0}{+}$, $\tfrac{1}{\sqrt{2}}\ketbra{1}{+}$,
    $\tfrac{1}{\sqrt{2}}\ketbra{0}{-}$ and $-\tfrac{1}{\sqrt{2}}\ketbra{1}{-}$
    satisfy
    $\sum_i A^i (A^i)^\dagger = \sum_i (A^i)^\dagger A^i = \Id$.
\end{proof}
