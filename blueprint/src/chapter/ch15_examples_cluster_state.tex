...
\begin{proof}
    \leanok
    \uses{lem:cluster_review_gauge, thm:cluster_not_injective, thm:cluster_normal,
        thm:cluster_z2z2_symmetric, thm:cluster_gauge_anticomm,
        thm:has_string_order_of_symmetric_injective, thm:cluster_string_order,
        thm:cluster_zcl}
...
\begin{theorem}[Doubly degenerate entanglement spectrum of the cluster state]
    \label{thm:cluster_entanglement_spectrum_even}
    \lean{MPSTensor.clusterBlocked_even_finrank_eigenspace}
    \leanok
    \uses{thm:spt_entanglement_spectrum_even, thm:cluster_z2z2_symmetric, thm:cluster_gauge_anticomm, rem:cluster_injective_blocked}
    Let $\Lambda$ be a positive definite fixed point of the adjoint transfer map
    of the length-$2$ blocked cluster-state tensor.  Every eigenspace of
    $\Lambda$ has even dimension.
\end{theorem}
\begin{proof}
    \leanok
    \uses{thm:spt_entanglement_spectrum_even, thm:cluster_z2z2_symmetric, thm:cluster_gauge_anticomm, rem:cluster_injective_blocked}
    The blocked tensor is injective and satisfies $\E(\Id)=\Id$; the two
    generators of $\Z_2\times\Z_2$ act by unitary permutation matrices with
    anticommuting virtual gauges $\sigma_z$ and $\sigma_x$.  Apply
