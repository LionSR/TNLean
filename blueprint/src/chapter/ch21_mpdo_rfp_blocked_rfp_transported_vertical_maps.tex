\begin{lemma}[Two-site support after refinement]
    ...
    \leanok
    \uses{def:mpdo_vertical_sector_precompression, lem:mpdo_normalized_retained_vertical_sector_embedding, lem:mpdo_retained_vertical_weighted_contraction, lem:mpdo_vertical_bond_contraction_reconstruction, thm:mpdo_one_site_closure_two_site_blocking}
...
\begin{proof}\leanok
    ...
    \begin{align}
        Z_T(V_1(X))
         & =\mathcal R^{-1}\!\left[T\!\left(U_1^\dagger B_1(X)U_1\right)\right]
        =\mathcal R^{-1}\!\left[T\!\left(\mathcal M^{(1)}(X)\right)\right]
    ...
    blocked-index relabelling theorem. Therefore
    \begin{align}
...
\begin{proof}\leanok
    ...
    \begin{align}
        Z_S(V_2(X))
         & =S\!\left(\mathcal R\!\left[U_2^\dagger B_2(X)U_2\right]\right)
...
\begin{lemma}[Trace preservation on one-site canonical-form contractions]
    ...
    \leanok
    \uses{lem:mpdo_vertical_sector_t_image_preservation, lem:mpdo_vertical_sector_precompression_positive_trace}
    Under the hypotheses of
    Lemma~\ref{lem:mpdo_vertical_sector_t_image_preservation}, suppose in
    addition that $U_1U_1^\dagger=1$ and that $T$ is completely positive and
    trace preserving. Then
    $\Trsec(\widetilde T(V_1(X)))=\Trsec(V_1(X))$.
\end{lemma}

\begin{proof}\leanok
    Cyclicity of trace and the support identity of
    Lemma~\ref{lem:mpdo_vertical_sector_t_image_preservation} give
    \begin{align}
...
\begin{lemma}[Trace preservation on two-site canonical-form contractions]
    ...
    \leanok
    \uses{lem:mpdo_vertical_sector_s_image_preservation, lem:mpdo_vertical_sector_precompression_positive_trace}
    Under the hypotheses of
    Lemma~\ref{lem:mpdo_vertical_sector_s_image_preservation}, suppose in
    addition that $U_2U_2^\dagger=1$ and that $S$ is completely positive and
    trace preserving. Then
...
\begin{proof}\leanok
    Cyclicity of trace and the support identity of
    Lemma~\ref{lem:mpdo_vertical_sector_s_image_preservation} give
    \begin{align}
...
\begin{theorem}[Action of the normalized vertical-sector maps]
    ...
    \leanok
    \uses{def:mpdo_transported_vertical_sector_maps, lem:mpdo_normalized_retained_vertical_sector_embedding, lem:mpdo_retained_vertical_sector_partial_trace, thm:mpdo_vertical_coordinate_map_transport}
    ...
    \begin{align}
        T_{\mathrm v}\!\left(\bigoplus_\alpha\mu_\alpha\otimes X_\alpha\right)
         & =\bigoplus_\beta\nu_\beta\otimes Y_\beta,
        \notag                                          \\
        S_{\mathrm v}\!\left(\bigoplus_\beta\nu_\beta\otimes Y_\beta\right)
...
\begin{proof}\leanok
    ...
    \begin{align}
        \widetilde T\!\left(\bigoplus_\alpha m_\alpha X_\alpha\right)
         & =\widetilde R_2\!\left(T_{\mathrm v}\!\left(\bigoplus_\alpha\mu_\alpha\otimes X_\alpha\right)\right)
        =\widetilde R_2\!\left(\bigoplus_\beta\nu_\beta\otimes Y_\beta\right)
        =\bigoplus_\beta n_\beta Y_\beta.
        \notag
    \end{align}
    ...
    \begin{align}
        \widetilde S\!\left(\bigoplus_\beta n_\beta Y_\beta\right)
         & =\widetilde R_1\!\left(S_{\mathrm v}\!\left(\bigoplus_\beta\nu_\beta\otimes Y_\beta\right)\right)
        =\widetilde R_1\!\left(\bigoplus_\alpha\mu_\alpha\otimes X_\alpha\right)
        =\bigoplus_\alpha m_\alpha X_\alpha.
        \notag
    \end{align}
\end{proof}
...
\begin{lemma}[Fixed contraction families of the transported composites]
    ...
    \end{align}
    Under the hypotheses of
    Theorem~\ref{thm:mpdo_transported_vertical_sector_maps}, the transported
    composites satisfy
    \begin{align}
    ...
    fixed-point spaces \cite[Appendix~C.4, lines~1974--1980]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
    Theorem~\ref{thm:mpdo_transported_vertical_sector_maps} gives
    $\widetilde T(V_1(X))=V_2(X)$ and $\widetilde S(V_2(X))=V_1(X)$. Hence
    \begin{align}
        (\widetilde S\circ\widetilde T)(V_1(X))
...
\begin{theorem}[Faithful fixed point from fixed product generators]
    ...
    $V_{j_1}\cdots V_{j_L}$ in $\mathcal M$. Then $F$ has a positive-definite
    fixed point. This is the local full-matrix intermediate implication used
    in \cite[Appendix~C.4, lines~1980--1995]{Cirac2016MPDO_arXiv}; it is not
...
\begin{proof}\leanok
    ...
    $QV_jQ=V_j$. Since $Q$ is a projection, this implies
    $QV_j=V_j=V_jQ$. By induction, for every $k\geq1$,
    \begin{align}
    ...
    $Q=Q^2=Q\mathbf 1Q=\mathbf 1$. Thus $\rho$ is positive definite, and the
    construction of $P$ gives $F(\rho)=\rho$.
\end{proof}
...
\begin{proof}\leanok
    ...
    $\Delta=\mathbf 1-F^*(\mathbf 1)$. For every positive semidefinite $X$,
    trace nonincrease gives
    $\tr(\Delta X)=\tr(X)-\tr(F(X))\geq0$.
    ...
    $\tr(\rho\Delta)=0$, so positive definiteness of $\rho$ forces
    $\Delta=0$. Thus $F^*(\mathbf 1)=\mathbf 1$, which is equivalent to trace
    preservation.
\end{proof}
...
\begin{proof}\leanok
    ...
    $\pi\circ\iota=\id_{\mathcal A}$ gives, whenever $\widehat F$ is trace
    preserving,
    \begin{align}
        \tr_{\mathcal A}(F(X))
         & =\tr(\widehat F(\iota(X)))
...
\begin{theorem}[Trace preservation on a finite sum from fixed products]
    ...
    $\tr_{\mathcal A}(X)=\sum_{\alpha\in I}\tr(X_\alpha)$.
...
\begin{proof}\leanok
    \uses{thm:positive_map_trace_preserving_from_fixed_product_span, thm:direct_sum_extension_trace_transfer}
    ...
    \begin{align}
        ...
        \notag                                             \\
        \iota(\mathbf 1_{\mathcal A}) & =\mathbf 1,
        \notag                                             \\
        \widehat F(\iota(X))          & =\iota(F(X)).
        \notag
    \end{align}
...
\begin{proof}\leanok
    ...
    \begin{align}
        H_1 & =X+X^\dagger,
        \notag                             \\
        H_2 & =\mathrm{i}(X-X^\dagger),
        \notag                             \\
        X   & =\frac12(H_1-\mathrm{i}H_2).
        \notag
    \end{align}
...
\begin{theorem}[Trace preservation of the transported vertical-sector maps]
    ...
    \begin{align}
        ...
        \notag                                                               \\
        \widetilde M^{(i)}_\ell                & =U_i^\dagger(B_i)_\ell U_i,
        \notag                                                               \\
    ...
    appropriate total sector trace. For the individual maps,
    \begin{align}
    ...
    \begin{align}
        \sum_\alpha\tr
        ((\widetilde S\widetilde T)(X)_\alpha)
         & =\sum_\alpha\tr(X_\alpha),
        \notag                        \\
        \sum_\beta\tr
        ((\widetilde T\widetilde S)(Y)_\beta)
         & =\sum_\beta\tr(Y_\beta).
        \notag
    \end{align}
...
\begin{proof}\leanok
    \uses{thm:positive_direct_sum_map_trace_preserving_from_fixed_product_span, thm:direct_sum_trace_preservation_sandwich, lem:mpdo_transported_vertical_sector_fixed_generators, thm:mpdo_vertical_bond_contractions_generate_sector_algebra, thm:mpdo_transported_vertical_sector_trace_loss}
...
\begin{lemma}[Retained-coordinate block-diagonal inclusion]
    ...
    inclusion is $\iota_{\mathrm v}=W\circ\iota$.
\end{lemma}
...
\begin{lemma}[Full-matrix factorization of transported refinement]
    ...
    \leanok
    \uses{def:mpdo_transported_vertical_sector_maps, lem:mpdo_vertical_sector_block_diagonal_reindex, lem:mpdo_vertical_sector_blocks_reindex}
    ...
    \begin{align}
        ...
        \label{eq:rfp_refinement_extension}
    \end{align}
\end{lemma}

\begin{proof}\leanok
    Substitute the inclusion and extraction identities of
    Lemma~\ref{lem:mpdo_vertical_sector_block_diagonal_reindex} and
    Lemma~\ref{lem:mpdo_vertical_sector_blocks_reindex} into
...
\begin{theorem}[Complete positivity and adjoint Schwarz for the transported maps]
    ...
    \leanok
    \uses{def:mpdo_transported_vertical_sector_maps, lem:mpdo_transport_t_vertical_coordinates_cp, lem:mpdo_transport_s_vertical_coordinates_cp, lem:mpdo_transported_vertical_sector_t_extension, lem:mpdo_transported_vertical_sector_s_extension, thm:mpdo_vertical_sector_partial_trace_kraus, thm:mpdo_transported_vertical_sector_composites_trace_preserving}
    Under the hypotheses of
...
\begin{proof}\leanok
    ...
    coordinates. The full-matrix extension of the transported refinement map
    has the factorization (\ref{eq:rfp_refinement_extension}). Here
    $\widehat R_1$ is normalized state preparation,
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    and (\ref{eq:rfp_coarse_extension}) prove complete positivity of
    $\widetilde S$. These factorizations are exact on the full matrix
    algebras: $\pi_iW_i^{-1}$ is precisely extraction of the retained diagonal
    sector blocks. Thus no range inclusion is used.

    Composition gives complete positivity of
    $\widetilde S\widetilde T$ and $\widetilde T\widetilde S$.
    Theorem~\ref{thm:mpdo_transported_vertical_sector_composites_trace_preserving}
    gives trace preservation of these two composites.
...
\begin{theorem}[Transported vertical-sector composites are identities]
    ...
    \leanok
    \uses{thm:mpdo_transported_vertical_sector_composites_trace_preserving, thm:mpdo_transported_vertical_sector_composites_schwarz, lem:mpdo_transported_vertical_sector_fixed_generators, thm:mpdo_vertical_bond_contractions_generate_sector_algebra, thm:mpdo_fixed_contractions_trace_adjoint_schwarz_identity}
    Under the hypotheses of
    Theorem~\ref{thm:mpdo_transported_vertical_sector_composites_trace_preserving},
...
\begin{proof}\leanok
    ...
    two transported maps.
    Theorem~\ref{thm:mpdo_transported_vertical_sector_composites_trace_preserving}
    and
    Theorem~\ref{thm:mpdo_transported_vertical_sector_composites_schwarz}
    give trace preservation and the Schwarz inequality for $F^*$,
    respectively.
    Lemma~\ref{lem:mpdo_transported_vertical_sector_fixed_generators} gives
    $F(V_i(X))=V_i(X)$ for every bond matrix $X$.
    Theorem~\ref{thm:mpdo_vertical_bond_contractions_generate_sector_algebra}
    gives a positive integer $L_i$ such that
    $C_{L_i}(V_i)=\mathcal A_i$.
    Applying
    Theorem~\ref{thm:mpdo_fixed_contractions_trace_adjoint_schwarz_identity}
    first to $F=\widetilde S\circ\widetilde T$ and then to
    $F=\widetilde T\circ\widetilde S$ proves
    (\ref{eq:mpdo_vsector_ST_id}) and
