...
\begin{definition}[Translation-invariant two-site bond]
    ...
    A positive semidefinite two-site bond $B\ge0$ whose
...
\begin{definition}[$\eta$-local single-bond commuting-form data]
    ...
    The one-bond $\eta$-local form constructed in
    \cite[Appendix~C.2]{Cirac2016MPDO_arXiv} consists of
...
\begin{theorem}[Adjacent translates are the two overlapping lifts]
    ...
    \leanok
    \uses{def:mpdo_three_site_overlapping_coordinates, def:mpdo_translation_invariant_bond_data}
...
\begin{theorem}[A commuting bond admits unitary block actions]
    ...
    \leanok
    \uses{def:mpdo_three_site_overlapping_coordinates, thm:mpdo_eta_bond_overlapping_lifts}
...
\begin{proof}\leanok
    ...
    Theorem~\cite[``Explicit unitary block actions for commuting overlaps'']{QICLean2026Blueprint}.
\end{proof}
...
\begin{theorem}[Scaled finite-chain cyclic neighboring-operator form]
    ...
    \leanok
    \uses{def:mpdo_eta_local_structure_data, thm:mpdo_commuting_bond_positive_eta_decomposition, thm:mpdo_eta_pair_bond_cyclic_product_transport}
    ...
    \cite[Appendix~C.2, lines~1571--1576]{Cirac2016MPDO_arXiv}, whereas the
    subsequent passage at
    \cite[Appendix~C.2, lines~1603--1605]{Cirac2016MPDO_arXiv} suppresses this
    factor when invoking equation \emph{sigmaNK2}.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpdo_commuting_bond_positive_eta_decomposition, thm:mpdo_eta_pair_bond_cyclic_product_transport}
...
\begin{remark}[An arbitrary proportional presentation need not have unit scalar]
    ...
    This example is not a normal injective block. More generally, an arbitrary
    proportional commuting-bond presentation need not be made coefficient-free
    by a single local rescaling. In the SAL-to-commuting-form direction, the proof
    chooses the neighboring operators from the source tensor and obtains scalar one
    directly, yielding the source-selected normal representative in
...
\begin{definition}[Fixed tensor for a bond-product family]
    ...
    \leanok
    \uses{def:mpdo_translation_invariant_bond_data, thm:mpdo_cyclic_edge_weight_tensor}
...
\begin{theorem}[Fixed tensor representation of a commuting bond]
    ...
    \leanok
    \uses{def:mpdo_translation_invariant_bond_data, def:mpdo_eta_local_structure_data, def:mpdo_fixed_bond_product_tensor, thm:mpdo_commuting_bond_positive_eta_decomposition}
...
\begin{theorem}[Positive physical sectors of the selected fixed tensor]
    ...
    Let $C$ be the selected fixed tensor of
    Theorem~\ref{thm:mpdo_eta_fixed_bond_product_tensor}. There are
    physical sectors
...
\begin{theorem}[Eventual proportionality from a fixed product tensor]
    ...
    \leanok
    \uses{def:mpdo_eta_local_structure_data, def:mpdo_fixed_bond_product_tensor, thm:mpdo_eta_fixed_bond_product_tensor}
...
\begin{theorem}[A normal rescaling gives a geometric phase]
    ...
    \leanok
    \uses{def:mpdo_eta_local_structure_data, def:mpdo_fixed_bond_product_tensor, def:nt_cpsv}
...
\begin{proof}\leanok
    \uses{thm:mpdo_eta_fixed_product_tensor_eventual_proportionality, lem:mpv_smul, thm:normal_proportional_mpv_geometric_phase, lem:mpdo_three_first_site_contraction_coefficients}
...
\begin{proof}\leanok
    \uses{thm:mpdo_eta_normal_rescaling_geometric_phase, thm:cpsv_normal_mpv_phase_gauge}
    Normality gives
