...
\section{Zero-block separation and honest block decomposition}%
...
\section{Arbitrary-input TP-gauge pipeline}%
\label{sec:tp_gauge_arbitrary}

\begin{theorem}[Arbitrary-input TP-gauge pipeline with zero-block separation]%
	...
	\]
	This is the furthest honest arbitrary-input pipeline step available
...
\begin{theorem}[Compression to a supported projection sector]%
	...
	\leanok
	Let $A$ be a left-canonical MPS tensor with
...
\begin{proof}\leanok
	\uses{thm:blockdecomp_commuting_projs}
	The Kadison--Schwarz equality condition for a TP map implies
	that a projection fixed by $\E_A^\dagger$ commutes with every
	Kraus operator.
	Apply Theorem~\ref{thm:blockdecomp_commuting_projs}.
\end{proof}

\section{Assembly: arbitrary tensor to primitive block decomposition}%
...
\begin{remark}[Canonical form pipeline: current status]%
	\label{rem:canonical_pipeline_gaps}
	\leanok
	The core pipeline steps are in place.
	The arbitrary-input pipeline
