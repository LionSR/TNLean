...
\begin{remark}[Remaining steps toward normal canonical form]%
	...
	can be written directly with the weighted common-length cyclic sectors.  The
	downstream assembly theorem
	Theorem~\ref{thm:after_blocking_sector_common_blocks_proportional_decomposition_zero_tail}
	now records the next comparison step: if those common-length live sectors are
	...
	finite-length span input is obtained through the common phase-cover theorem.
	Thus the remaining canonical-form inputs are precisely the physical-label
	compatibility, any additional blocking needed for injectivity, and the final
	BNT proportional-decomposition data for the common-length live sectors.
