...
sense of \cite{Cirac2017MPDO_AnnPhys}. What remains is the passage from an
arbitrary tensor to such weighted primitive data. The reduction draws on
\cite{PerezGarcia2007Matrix}, \cite{Cirac2021Matrix}, and the
positive-map spectral theory of~\cite[Ch.~6]{Wolf2012Quantum}
(especially Thms.~6.2--6.5 for irreducibility and PF eigenvectors).
...
\begin{theorem}[CFII continuation after irreducible block decomposition]%
	...
	MPV preservation).
	This theorem stops at the older CFII stage; it does not yet
	produce the normal-canonical-form output of the normal-canonical route.
\end{theorem}
...
\begin{theorem}[PSD eigenvector for positive maps]%
	...
	\cite[Thm.~6.5]{Wolf2012Quantum}
	(spectral-radius eigenvalue with a PSD eigenvector for any
	positive map); see also~\cite{Evans1978Spectral}.
	The formal proof applies Brouwer's fixed-point theorem to the
...
\begin{theorem}[Scaling preserves irreducibility]%
	...
	The full similarity version --- $E'(X) = c \cdot C^{-1} E(C X C^\dagger)(C^\dagger)^{-1}$
	is irreducible whenever $E$ is irreducible, $c > 0$, and $C$ is invertible ---
	is now fully formalized; see Lemma~\ref{lem:similarity_irred}
	in Chapter~\ref{ch:qpf}.
\end{theorem}
...
\begin{theorem}[PosDef adjoint eigenvector for irreducible tensors]%
	...
	(\cite[Thm.~6.5]{Wolf2012Quantum}, used in
	Theorem~\ref{thm:pf_eigenvector_existence}) with the
...
\section{Status of the canonical form reduction}\label{sec:pipeline_status}

\begin{remark}[Status of the canonical form reduction]%
		\label{rem:canonical_pipeline_gaps}
		\notready
		The Perron--Frobenius, TP-gauge, CFII, periodicity-removal, and
		normal-canonical-form packaging steps are now in place. What remains is
		the upstream passage from an arbitrary tensor to the primitive weighted
		block data required by Theorem~\ref{thm:exists_normal_canonical_form}.

		The block-injective route toward Definition~\ref{def:is_canonical_form}
		still requires turning blocked primitive TP data into injective blocks and
		eventual block injectivity.

		On the normal-canonical side,
		Theorem~\ref{thm:exists_normal_canonical_form} already provides the
		downstream packaging once primitive weighted blocks with positive
		dimensions and pairwise distinct nonzero weight moduli are given. The
		remaining gap is that Theorem~\ref{thm:pipeline_handoff} stops earlier:
		it gives only the CFII continuation after irreducible block
		decomposition, not an end-to-end construction from arbitrary tensors.
\end{remark}
