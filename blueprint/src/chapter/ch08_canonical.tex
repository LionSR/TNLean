...
The block-injective route aims at injective TP-normalized blocks and the
canonical form predicate of Chapter~\ref{ch:block_perm}. The normal-canonical
route packages a weighted family of irreducible TP-normalized blocks with
...
CFII data and the normal canonical form packaging.
...
\begin{definition}[Block-diagonal tensor from blocks]\label{def:tensor_from_blocks}
	...
	operators are $A^i = \bigoplus_{k=1}^r \mu_k A_k^i$.
\end{definition}
...
\begin{theorem}[Global multi-block fundamental theorem]\label{thm:multi_block_global}
	...
	\leanok
	\uses{def:tensor_from_blocks, def:gauge_equiv, thm:multi_block_ft}
	If all block tensors $A_k$ are injective and the per-block
	same-MPV condition holds ($A_k$ and $B_k$ generate the same
	MPV family for each~$k$),
	then the block-diagonal tensors $\bigoplus_k \mu_k A_k$ and
	$\bigoplus_k \mu_k B_k$ are gauge equivalent.
\end{theorem}

\begin{proof}\leanok
	\uses{thm:multi_block_ft}
	By Theorem~\ref{thm:multi_block_ft}, each pair $(A_k, B_k)$
	is gauge equivalent via some $X_k$.
	Block-diagonal conjugation by $X = \bigoplus_k X_k$ then gives
	the global gauge equivalence.
\end{proof}
...
\begin{definition}[Irreducible tensor]\label{def:irreducible_tensor}
	...
	if it has no nontrivial invariant orthogonal projection,
	i.e.\ $\lnot\mathsf{HasInvariantProj}(A)$
	(Definition~\ref{def:has_inv_proj}).
\end{definition}
...
\begin{proof}
	...
	(If one prefers a positive-eigenvector route without assuming TP
...
\begin{remark}\leanok
	...
	dependency for the formalization. In the current project it is formalized
	directly as the theorem tagged above, rather than imported from Mathlib;
	if Mathlib later acquires a suitable finite-dimensional Burnside theorem,
	this chapter could be retargeted to that library result.
\end{remark}
...
\begin{theorem}[CFII continuation after irreducible block decomposition]%
	...
	MPV preservation).
	The conclusion is CFII data; the normal-canonical-form
	packaging requires additional downstream steps.
\end{theorem}
...
\begin{proof}\leanok
		...
		steps may require a common blocking period; under the present hypotheses
...
\begin{proof}\leanok
	For $N = 0$, $\operatorname{evalWord}(\mathbf{0}, []) = \Id_{D}$,
...
\begin{remark}[Canonical form reduction: current status]%
	\label{rem:canonical_pipeline_gaps}
	\leanok
	The core reduction steps are in place.
	The arbitrary-input reduction
	(Theorem~\ref{thm:tp_gauge_arbitrary}) produces a zero tail plus
	left-canonical live blocks. After a common blocking step
	(Theorem~\ref{thm:tp_primitive_blockdecomp}), the blocked blocks
	have primitive transfer maps.

	The remaining steps to full normal canonical form require:
	establishing tensor irreducibility for each blocked block
	(blocking does not in general preserve tensor irreducibility),
	and establishing pairwise distinct weight norms.
