...
\begin{lemma}[Orbit-sum construction of the sector lift]%
	...
	In particular $R$ witnesses the orbit-sum lift condition used by the
	sector-irreducibility argument.
\end{lemma}
...
\begin{theorem}[Unconditional sector irreducibility]%
	...
	\leanok
	\uses{thm:orbit_sum_hLift_unconditional}
	Let $A$ be a left-canonical MPS tensor with irreducible adjoint transfer map,
...
\begin{proof}\leanok
	\uses{thm:orbit_sum_hLift_unconditional}
	Apply the unconditional orbit-sum lift from
	Theorem~\ref{thm:orbit_sum_hLift_unconditional} and then the cyclic-sector
