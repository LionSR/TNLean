...
\begin{theorem}[Equal-norm blocks with non-decaying overlap are gauge equivalent]
	\label{thm:gauge_equiv_equal_norm_blocks}
	\lean{MPSTensor.gaugePhaseEquiv_of_equal_norm_blocks}\leanok
	\uses{thm:gauge_equiv_nondecay}
	Given trace-preserving irreducible blocks with a non-decaying cross-overlap, they have equal bond dimensions and are gauge equivalent up to a unit-modulus phase.
	The non-decaying-overlap hypothesis is an additional assumption, typically derived from structural properties of the decomposition (cyclic-sector origin, Fundamental Theorem matching, etc.).
	Note: the MPV-level hypothesis connecting blocks through a parent tensor is \emph{not} sufficient on its own (see issue \#299 for a counter-example).
\end{theorem}
