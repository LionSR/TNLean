...
\begin{proof}\leanok
		Contrapositive of the overlap-decay theorem: if the mixed transfer
		...
		via the trace identity $\text{LinearMap.trace}((F_{AB})^N) = \langle V^{(N)}(A), V^{(N)}(B)\rangle$
		the overlap itself would tend to~$0$, contradicting the hypothesis.
\end{proof}

\begin{theorem}[Source-faithful equalMPS gauge recovery]%
		...
		This is the source-faithful direction of \cite[Lemma~equalMPS,
		lines~1085--1117]{Cirac2016MPDO_arXiv} for tensors of equal bond
		dimension; no proportionality of MPVs is required.
\end{theorem}
...
\begin{theorem}[Sector decomposition from TP primitive irreducible blocks]
	...
	\uses{thm:nt_overlap_norm_one_gauge, thm:bnt_grouping_gauge_equiv}
