...
\begin{remark}[Connections to the strong predicates]
	The module provides one connecting lemma,
	\texttt{IsNormalTensor.of\_irreducible\_and\_primitive}, which
	combines an \texttt{IsIrreducibleTensor} witness with an
	\texttt{IsPrimitive (transferMap A)} witness. Connections from
	these paper-faithful predicates to the strong predicates
	\texttt{IsNormalCanonicalForm} and \texttt{IsBNT} are not given
	in \texttt{PaperDefs.lean}: they require either a
	fundamental-theorem-level dependency or the algebraic-to-spectral
	normality implication, both of which appear in downstream
	chapters.
\end{remark}
...
\begin{remark}[Prepared-block constructors for the canonical-form predicate]\leanok
	The Lean formalization records two prepared-block constructors that package
	pre-normalized data into the canonical-form predicate.  The first
	(\texttt{MPSTensor.isCanonicalForm\_of\_primitive}) starts from
	injective, left-canonical, strictly-ordered nonzero-weight blocks each
	admitting a primitive fixed point.  The second
	(\texttt{MPSTensor.isCanonicalForm\_of\_peripheralPrimitive}) replaces the
	primitive fixed-point hypothesis by the peripheral-spectrum primitivity
	condition $\sigma_\partial(\E_{A_k}) = \{1\}$.  In both cases the canonical-form
	overlap clause $O_{A_k A_k}(N)\to 1$ is derived from the other hypotheses via
	the spectral-gap criterion of Chapter~\ref{ch:spectral}.  These statements
	assume the blocks are already in the required normal form and are not the
...
\begin{remark}\label{rem:primitivity_reduces_assumptions}\leanok
	The peripheral-primitive prepared-block constructor above shows that
...
The proof-level statements about projection multiplicativity, sector primitivity
and irreducibility, and common-alphabet bookkeeping are internal to the
period-removal argument; their Lean homes are
\texttt{MPS/CanonicalForm/SectorComparison/} and
\texttt{MPS/Periodic/SectorIrreducibility/}.
The three source-facing endpoint statements are presented here.
...
Two structural after-blocking statements relocated to
Chapter~\ref{ch:canonical_after_blocking}, and one block-injectivity statement
relocated to Chapter~\ref{ch:wielandt}, take an arbitrary tensor or
TP-primitive-irreducible block to a primitive block decomposition.  The
source-facing residue in this chapter is the normality of a TP-primitive
irreducible block.
...
\label{rem:canonical_pipeline_gaps}
	...
	The corresponding intermediate statements, including the common-alphabet
	cyclic-sector family and the after-blocking zero-tail and common-sector
	transport theorems, are collected in
	Chapter~\ref{ch:canonical_after_blocking}; the present chapter records only
	the source-facing endpoints
...
\subsection{Prepared-block BNT construction}%
\label{subsec:paperbnt_supplier}

The paper-faithful BNT predicate (Definition~\ref{def:bnt_paper}, formalized
as \texttt{IsBNTCanonicalForm} in Chapter~\ref{ch:bnt}) represents the CPSV16
basis-of-normal-tensors structure with full multiplicity and span fields.  The
following theorem is the prepared-block direction: a constructor from
pre-normalized block hypotheses to a BNT-equipped sector decomposition.
...
\begin{remark}[Arbitrary-input direction: open formalization gap]%
  ...
  arbitrary-input direction.

  \begin{enumerate}
    \item \textbf{Zero-tail conventions.}
      ...
      with a separate length-zero dimension identity.  Reducing to the exact
      \texttt{SameMPV\textsubscript{2}} statement requires matching the
      zero-tail bond dimension after pruning all-zero blocks, which fails once
      zero blocks are removed by Theorem~\ref{thm:zero_block_separation}.
...
\subsection{Intermediate constructions referenced from other chapters}%
\label{subsec:internal_lean_only_labels}

The following statements are intermediate constructions formalized in the Lean
modules \texttt{MPS/Core/BlockingInfrastructure.lean},
\texttt{MPS/Core/BlockingTransfer.lean},
\texttt{MPS/CanonicalForm/BlockingViaAdjoint.lean},
\texttt{MPS/CanonicalForm/FromPeripheralPrimitive.lean},
\texttt{MPS/CanonicalForm/SectorComparison/CommonBlockedCyclicSectorFamily.lean},
\texttt{MPS/CanonicalForm/SectorComparison/CommonBlockedCyclicSectorConstruction.lean},
and \texttt{MPS/Periodic/SectorIrreducibility/HLift.lean}.  They are not
source-paper theorems but are consumed from the Fundamental-Theorem and
after-blocking chapters and from
Chapter~\ref{ch:periodic_ft_apps}.  Each is recorded here as a short label-only
pointer so that the dependency graph remains complete; the mathematical content
is documented in the Lean source.

\begin{remark}[Canonical form from peripheral primitivity (Lean code only)]\label{thm:canonical_from_primitive}\leanok
	Prepared-block constructor \texttt{MPSTensor.isCanonicalForm\_of\_peripheralPrimitive};
	see the prepared-block remark in
	Section~\ref{sec:canonical_from_primitivity}.
\end{remark}
\begin{remark}[Adjoint primitivity equivalence (Lean code only)]\label{thm:primitive_adjoint_equiv}\leanok
	\texttt{MPSTensor.IsPrimitive.adjoint\_iff}: primitivity is invariant under
	taking adjoints, since the peripheral eigenvalues of $E^\dagger$ are the
	conjugates of those of~$E$.
\end{remark}
\begin{remark}[Iterated blocking at the transfer-map level (Lean code only)]\label{thm:iterated_blocking_transfer}\leanok
	\texttt{MPSTensor.transferMap\_blockTensor\_mul}: $\E_{(A^{[m]})^{[n]}}=\E_{A^{[mn]}}$.
\end{remark}
\begin{remark}[Primitivity under blocking-divisibility (Lean code only)]\label{thm:primitive_blocking_divisible}\leanok
	\texttt{MPSTensor.isPrimitive\_transferMap\_blockTensor\_of\_dvd}: if $\E_{A^{[p_0]}}$
	is primitive and $p_0 \mid p$ then $\E_{A^{[p]}}$ is primitive.
\end{remark}
\begin{remark}[Blocking assembled weighted blocks (Lean code only)]\label{thm:block_tensor_to_tensor_from_blocks}\leanok
	\texttt{MPSTensor.sameMPV₂\_blockTensor\_toTensorFromBlocks}: blocking commutes
	with the assembled weighted block sum.
\end{remark}
\begin{remark}[Nonzero blocked weights (Lean code only)]\label{lem:block_weights_ne_zero}\leanok
	\texttt{MPSTensor.blockWeights\_ne\_zero}: a positive power of a nonzero
	complex weight remains nonzero after blocking.
\end{remark}
\begin{remark}[Primitive irreducible cyclic-sector data (Lean code only)]\label{def:primitive_irreducible_cyclic_sectors}\leanok
	\texttt{MPSTensor.HasPrimitiveIrreducibleCyclicSectors}: predicate recording
	the period-removal length~$m$, the unit-weight $A^{[m]}$ block-tensor
	decomposition, and the TP+primitive+irreducible+positive-dimension fields.
\end{remark}
\begin{remark}[Primitive cyclic sectors of irreducible TP tensors (Lean code only)]\label{thm:has_primitive_irreducible_cyclic_sectors_tp_irr}\leanok
	\texttt{MPSTensor.hasPrimitiveIrreducibleCyclicSectors\_of\_TP\_of\_isIrreducibleTensor};
	restatement of Theorem~\ref{thm:cyclic_sector_decomp_irr_tp_prim_irr} as the
	above predicate.
\end{remark}
\begin{remark}[Common reblocked cyclic-sector family (Lean code only)]\label{thm:exists_common_blocked_cyclic_sector_family}\leanok
	\texttt{MPSTensor.exists\_commonBlockedCyclicSectorFamily\_of\_hasPrimitiveIrreducibleCyclicSectors};
	reblocks every per-block cyclic sector to a common physical alphabet.
\end{remark}
\begin{remark}[Common reblocking at a prescribed multiple (Lean code only)]\label{thm:exists_common_blocked_cyclic_sector_family_common_multiple}\leanok
	\texttt{MPSTensor.exists\_commonBlockedCyclicSectorFamily\_of\_commonMultiple};
	specifies the common physical length explicitly.
\end{remark}
\begin{remark}[Common-flat unit weight is nonzero (Lean code only)]\label{thm:common_blocked_cyclic_sector_flat_weight_ne_zero}\leanok
	The unit weight of the common reblocked cyclic-sector family is nonzero
	(formalized as \texttt{MPSTensor.CommonBlockedCyclicSectorFamily.commonFlatWeight\_ne\_zero}).
\end{remark}
\begin{remark}[Common-alphabet structural properties (Lean code only)]\label{thm:common_blocked_cyclic_sector_derived_properties}\leanok
	Lemmas in
	\texttt{CommonBlockedCyclicSectorFamily.lean} establishing that the
	common-alphabet sectors are trace-preserving, have primitive transfer maps,
	are tensor-irreducible, and have positive bond dimensions.
\end{remark}
\begin{remark}[Common-alphabet flattening (Lean code only)]\label{thm:common_blocked_cyclic_sector_reindexed_samempv}\leanok
	Common-alphabet flattening lemmas in
	\texttt{CommonBlockedCyclicSectorFamily.lean}: iterated block
	$(B_k^{[m_k]})^{[e_k]}$ agrees with the directly-blocked tensor after word
	identification.
\end{remark}
\begin{remark}[Coordinate-grouping predicate (Lean code only)]\label{def:common_blocked_cyclic_sector_grouped_block_cast}\leanok
	\texttt{MPSTensor.CommonBlockedCyclicSectorFamily.groupedBlockCastAgrees}:
	the canonical identification of the common alphabet with the iterated alphabet
	agrees with grouping a direct word of length $p=m_k e_k$ into $e_k$
	consecutive length-$m_k$ words.
\end{remark}
\begin{remark}[Digit-level flattening (Lean code only)]\label{thm:common_blocked_cyclic_sector_flatten_word_of_block_cast}\leanok
	\texttt{MPSTensor.CommonBlockedCyclicSectorFamily.flattenWordOfBlock\_cast\_eq};
	digit-level cast lemma for $p=mn$ words.
\end{remark}
\begin{remark}[Direct vs.\ iterated blocking comparison (Lean code only)]\label{thm:common_blocked_cyclic_sector_blocked_word_comparison}\leanok
	Bundle of common-alphabet reindexing lemmas in
	\texttt{CommonBlockedCyclicSectorFamily.lean} comparing direct blocking with
	the common-sector form.
\end{remark}
\begin{remark}[Reindexed nonzero-part transport (Lean code only)]\label{thm:common_blocked_cyclic_sector_reindexed_nonzero_part}\leanok
	The common-alphabet nonzero-part transport lemmas in
	\texttt{CommonBlockedCyclicSectorFamily.lean}.
\end{remark}
\begin{remark}[Unconditional sector irreducibility (Lean code only)]\label{thm:sector_irred_unconditional}\leanok
	\texttt{MPSTensor.isIrreducibleOnCorner\_of\_cyclic\_decomp\_mps}: each
	cyclic-sector corner of the adjoint blocked transfer map is irreducible.
\end{remark}
...
logically to the after-blocking fundamental-theorem proof chain rather than to
...
artifact of the build dependency order; a future reorganization may move them
to a dedicated after-blocking module.  Until then, readers should consult
Chapters~\ref{ch:ft_proof} and~\ref{ch:canonical_after_blocking} for the
after-blocking construction documentation.
