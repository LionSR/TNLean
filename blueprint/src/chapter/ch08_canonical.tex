...
\begin{definition}[Norm-class enumeration in decreasing order]
	...
	For any finite family $(\mu_k)_{k=1}^r$ of complex weights, the function
	$\mathrm{normClassGroupingData}$ produces a norm-class partition (in the
	sense of Definition~\ref{def:norm_class_grouping_data}) with
	representative values arranged in strictly decreasing order
...
\begin{itemize}
  ...
  \item \textbf{Thm.\ ``Interpretation of $\Lambda$'' (lines~987--993).}
    ...
    \emph{Not covered:} \cref{thm:CFII_data} produces $\Lambda$, but the
...
\lean{MPSTensor.isCanonicalForm_of_peripheralPrimitive}\leanok
