...
\begin{proof}
	...
	Each block is injective and TP-normalized. Peripheral-spectrum primitivity
	therefore implies the spectral-gap primitive condition used in
	Chapter~\ref{ch:spectral}, and hence the required self-overlap convergence.
	This supplies the final field in Definition~\ref{def:is_canonical_form}.
\end{proof}
...
\begin{proof}\leanok
		...
		condition is derived rather than stored as an extra field.
\end{proof}
...
\begin{proof}\leanok
		\uses{def:is_normal_canonical_form}
		The theorem first performs a common blocking of the already primitive
		blocks; in the present statement this blocking is trivial, so one may
		take $p = 1$. It then reorders the blocks by decreasing weight norm.
		The resulting irreducibility, TP, primitivity, nonzero-weight, and
		positive-dimension data are exactly those required by
