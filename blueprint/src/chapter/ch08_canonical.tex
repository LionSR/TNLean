...
\begin{theorem}[Unital gauge from a full-rank fixed point]%
    ...
    \leanok
    Let $\rho$ be a positive definite fixed point of
    $\E_A(X)=\sum_i A^i X(A^i)^\dagger$.  For
    $B^i := \rho^{-1/2}A^i\rho^{1/2}$,
    \[
        \sum_i B^i(B^i)^\dagger = \Id.
    \]
    Moreover $A$ and $B$ generate the same MPV family.

    This is the full-rank fixed-point gauge used in
    \cite[Theorem~Th:TIcanonical, lines~767--769]
        {PerezGarcia2007Matrix}.
\end{theorem}

\begin{proof}\leanok
    The positive square root of $\rho$ is invertible.  Substituting
    $B^i=\rho^{-1/2}A^i\rho^{1/2}$ into
    $\sum_i B^i(B^i)^\dagger$ and using $\E_A(\rho)=\rho$
    gives the identity.  The change from $A$ to $B$ is a similarity
    transformation, so cyclicity of the trace gives equality of all
    finite-ring MPV coefficients.
\end{proof}
