...
\begin{remark}[Canonical form reduction: remaining steps]%
		\label{rem:canonical_pipeline_gaps}
		\notready
		The Perron--Frobenius (Chapter~\ref{ch:qpf}), TP-gauge, CFII,
		periodicity-removal, and normal-canonical-form packaging steps are
		in place. The upstream passage from an arbitrary tensor to the
		primitive weighted block data required by
		Theorem~\ref{thm:exists_normal_canonical_form} remains open.

		On the block-injective side, turning blocked primitive TP data into
		injective blocks and establishing eventual block injectivity are the
		open steps toward Definition~\ref{def:is_canonical_form}.

		On the normal-canonical side,
		Theorem~\ref{thm:exists_normal_canonical_form} provides the
		downstream packaging once primitive weighted blocks with positive
		dimensions and pairwise distinct nonzero weight moduli are given.
		Theorem~\ref{thm:pipeline_handoff} gives the CFII continuation after
		irreducible block decomposition; the end-to-end construction from
		arbitrary tensors requires additional work.
\end{remark}
