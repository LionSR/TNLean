...
\begin{theorem}[Rectangular equalMPS dimension recovery]%
		\label{thm:nt_overlap_norm_one_dim_eq}
		\lean{MPSTensor.dim_eq_of_overlap_norm_tendsto_one_of_irreducible_TP}
		\leanok
		\uses{def:irreducible_tensor, def:tp_kraus, def:mpv_overlap}
		Let $A$ and $B$ be irreducible MPS tensors of bond dimensions $D_1$
		and $D_2$, satisfying
		$\sum_i (A^i)^\dagger A^i = \Id$ and
		$\sum_i (B^i)^\dagger B^i = \Id$.  If
		$\|\langle V^{(N)}(A)\,|\,V^{(N)}(B)\rangle\|\to 1$, then
		$D_1=D_2$.

		This is the bond-dimension conclusion of \cite[Lemma~equalMPS,
		lines~1085--1117]{Cirac2016MPDO_arXiv}, in the left-canonical
		normal-tensor form used in the proof there.
\end{theorem}

\begin{proof}\leanok
		\uses{thm:overlap_decay_rect_irr_tp}
		If $D_1\neq D_2$, Theorem~\ref{thm:overlap_decay_rect_irr_tp}
		gives $\langle V^{(N)}(A),V^{(N)}(B)\rangle\to0$.  Taking norms
		gives convergence to~$0$, contradicting the assumed convergence
		of the norm to~$1$.
\end{proof}
