...
\begin{corollary}[Irreducible action implies eventual cumulative spanning]%
    \label{cor:irreducible_action_cumulative_span}
...
The module \texttt{MPS/CanonicalFormExistence1606.lean} assembles
the sorry-free components of the canonical-form construction
following \cite[\S2.3 and Appendix~A]{Cirac2016Density}.
...
\begin{remark}[Missing steps for full canonical form existence]%
    \label{rem:canonical_pipeline_gaps}
    \notready
    The pipeline in this section stops at the ``handoff'' point:
    irreducible blocks with \emph{conditional} CFII data.
    The remaining steps for a complete canonical form existence
    theorem are:
    \begin{enumerate}
        \item \emph{TP-gauge existence:}
              put each irreducible block into the TP gauge
              (Perron--Frobenius / spectral rescaling).
              This is not yet formalized.
        \item \emph{CFII $\to$ doubly-stochastic glue:}
              the CFII diagonal PD fixed point provides
              a weight for the adjoint fixed-point peripheral
              closure
              (Lemma~\ref{lem:peripheral_closed_powers_irred_fp}),
              but the current blocking-primitivity theorem
              (Theorem~\ref{thm:blocking_primitivity})
              assumes the stronger doubly-stochastic condition.
              A generalization using the adjoint-fixed-point
              version would close this gap.
        \item \emph{Aperiodicity $\to$ fixed-length spanning:}
              the Burnside bridge is now available.
              Irreducible tensor blocks act irreducibly on $\C^D$
              (Theorem~\ref{thm:irreducible_tensor_to_action}),
              and irreducible action implies eventual \emph{cumulative} spanning
              $\exists N,\ T_N(A) = \MN{D}$
              (Corollary~\ref{cor:irreducible_action_cumulative_span}).
              What is still missing is an argument upgrading
              $\exists N,\ T_N(A) = \MN{D}$ to normality, i.e.
              $\exists L,\ S_L(A) = \MN{D}$.
              This requires an aperiodicity/peripheral-spectrum input
              (typically obtained by blocking to a primitive transfer map);
              see Remark~\ref{rem:prim_definition_mismatch}
              in Chapter~\ref{ch:wielandt}.
    \end{enumerate}
\end{remark}
