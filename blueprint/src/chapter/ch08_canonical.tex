...
\begin{theorem}[Translation-invariant canonical form]%
    ...
    $p \ge 1$, MPV-equivalent to a weighted direct sum
    \[
        A^{[p]} \sim
        \mathbf{0}_{d^{[p]},D_0}
        \oplus
        \bigoplus_{k=1}^{r} \mu_k B_k.
    \]
    Here $\mathbf{0}_{d^{[p]},D_0}$ is the all-zero block of bond dimension
    $D_0$, the weights $\mu_k$ are nonzero, and the blocks $B_k$ have positive
    bond dimensions, are irreducible and left-canonical,
    \[
