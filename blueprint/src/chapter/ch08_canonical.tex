...
\begin{theorem}[Normal canonical form after sorting]
	...
	\uses{def:normal_canonical_form, thm:bnt_sorted_blockdecomp}
...
\begin{theorem}[Left-canonical primitive blocking from irreducibility]
	...
	\uses{thm:primitive_from_peripheral}
...
\begin{theorem}[Zero-word evaluation for the zero tensor]
	...
	Every finite word evaluates to zero when all Kraus operators are zero.
\end{theorem}
...
\begin{theorem}[Zero MPV for the zero tensor]
	...
	The MPS value of every spin word is zero for the zero tensor.
\end{theorem}

\begin{theorem}[Nonzero Kraus operator in irreducible positive dimension]
	\label{thm:nonzero_kraus_irred}
	\lean{MPSTensor.exists_nonzero_kraus_of_isIrreducibleTensor}\leanok
	\uses{def:irreducible_tensor}
	An irreducible tensor of positive bond dimension has at least one nonzero Kraus operator.
\end{theorem}
