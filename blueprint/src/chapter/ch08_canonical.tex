...
\section{Canonical form existence pipeline (\cite{Cirac2017MPDO_AnnPhys})}%
...
\begin{theorem}[PSD eigenvector for positive maps (axiom)]%
    ...
    The standard proof applies the Brouwer fixed-point theorem
    to the normalization map
    $\rho \mapsto E(\rho) / \tr(E(\rho))$
    on the compact convex set of density matrices.
    This fixed-point argument has not yet been formalized;
    accordingly this result is currently assumed as an axiom.
\end{theorem}
...
\begin{remark}[Canonical form pipeline]%
    ...
    which requires Brouwer's fixed-point theorem and is
    currently assumed as an axiom (marked $\star$ below).
    \begin{enumerate}
