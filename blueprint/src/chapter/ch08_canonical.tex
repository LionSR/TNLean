...
\begin{proof}
    ...
    Our proof avoids this algebraic detour:
    we iterate the invariant-projection splitting with
    termination by strong induction on~$D$.
\end{proof}
...
\begin{remark}\label{rem:primitivity_reduces_assumptions}
    ...
    This corresponds to \cite[Appendix~A]{Cirac2016Density}.
\end{remark}
...
\begin{remark}\label{rem:blocking_primitivity_appendixA}
    ...
    step in \cite[Appendix~A]{Cirac2016Density}.
\end{remark}

\section{Canonical form existence pipeline (\cite{Cirac2016Density})}%
\label{sec:canonical_pipeline_1606}

This section assembles the components of the canonical-form
construction following \cite[\S2.3 and Appendix~A]{Cirac2016Density}.
...
\begin{theorem}[Irreducible block decomposition (pipeline)]%
    ...
    Every MPS tensor $A$ is $\mathrm{SameMPV}_2$-equivalent
    to a block-diagonal tensor $\bigoplus_{k=1}^r A_k$
    with each block irreducible.
    This re-exports Theorem~\ref{thm:irred_decomp}.
\end{theorem}

\begin{proof}\leanok\uses{thm:irred_decomp}
    Direct application of Theorem~\ref{thm:irred_decomp}.
\end{proof}
...
\begin{theorem}[CFII data for irreducible TP blocks]%
    ...
    \leanok
    \uses{def:irreducible_tensor, thm:form_II, thm:samempv_unitary}
    ...
    of \cite[Appendix~A]{Cirac2016Density}.
\end{theorem}
...
\begin{theorem}[Pipeline handoff: block decomposition with conditional CFII]%
    ...
    $(A_k)_{k=1}^r$ that are $\mathrm{SameMPV}_2$-equivalent to~$A$.
...
\cite[Appendix~A]{Cirac2016Density}.

\begin{theorem}[PSD eigenvector for positive maps (axiom)]%
    \label{thm:pf_eigenvector_existence}
    \lean{exists_posSemidef_eigenvector}
    \notready
    \uses{def:positive_map}
    Let $E : \MN{D} \to \MN{D}$ be a nonzero positive linear map
    with $D > 0$.
    ...
    The standard proof uses the Brouwer fixed-point theorem
    applied to the normalization map
    $\rho \mapsto E(\rho) / \tr(E(\rho))$
    on the compact convex set of density matrices.
    Since no proof of Brouwer's FPT is available in the
    underlying library, this result is stated as an axiom.
\end{theorem}
...
\begin{theorem}[PosDef adjoint eigenvector for irreducible tensors]%
    ...
    \uses{def:irreducible_tensor, def:transfer_map,
          thm:pf_eigenvector_existence, thm:irred_map_smul,
          thm:adjoint_transfer_ne_zero}
    ...
    Uses Theorem~\ref{thm:pf_eigenvector_existence} (axiom).
\end{theorem}
...
\begin{proof}\leanok
    ...
    $\E_A^\dagger \neq 0$.
    By Theorem~\ref{thm:pf_eigenvector_existence},
    there exist $\sigma_0 \ge 0$ (PSD, nonzero)
...
\begin{remark}[Canonical form pipeline]%
    ...
    Every step is proved except for step~2,
    which requires a fixed-point theorem not available in
    the library (marked $\star$ below).
    \begin{enumerate}
        ...
        \item[$\star$2.] PF eigenvector
              ...
              Requires Brouwer's fixed-point theorem.
        \item TP gauge
