...
\begin{definition}[Primitive irreducible cyclic-sector structure]%
\label{def:primitive_irreducible_cyclic_sectors}
	...
	\uses{def:blocked_tensor, def:tensor_from_blocks, def:same_mpv2,
		def:tp_kraus, def:primitive_channel, def:transfer_map,
...
\label{thm:tp_primitive_irreducible_extra_blocking}
	...
	\leanok
	\uses{def:blocked_tensor, def:tp_kraus, def:primitive_channel,
		def:transfer_map, def:irreducible_tensor,
		thm:blocked_irreducible_tp_primitive}
	If a sector block of positive bond dimension is already trace-preserving, has
	primitive transfer map, and is tensor-irreducible, then every later positive
	blocking length $k$ preserves all three properties. This $k$ is the subsequent
