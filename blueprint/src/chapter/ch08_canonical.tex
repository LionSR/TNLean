...
invariant-subspace decomposition. Two parallel routes coexist in the codebase.
The legacy route ends in injective TP-normalized blocks and feeds the canonical
form predicate of Chapter~\ref{ch:block_perm}. The new D1 route ends in
irreducible TP-normalized blocks with primitive transfer maps and packages them
as a normal canonical form in the sense of \cite{Cirac2017MPDO_AnnPhys}. The
reduction procedure follows
...
\section{Normal canonical form route}\label{sec:normal_canonical_form}
...
\begin{definition}[Normal canonical form predicate]\label{def:is_normal_canonical_form}
		...
		\end{enumerate}
		Here ``normal'' is used in the sense of
		\cite{Cirac2017MPDO_AnnPhys}: irreducible blocks with trivial
		peripheral spectrum after TP normalization. It is not the same as
		Definition~\ref{def:normal}, which means eventual block injectivity.
\end{definition}
...
\begin{proof}\leanok
		...
		This is the main structural advantage of the D1 predicate:
		primitivity is part of the data, so the self-overlap convergence is
		derived from irreducibility, TP normalization, and the spectral gap of
		the transfer map, rather than being stored as an extra field.
\end{proof}
...
\begin{proof}\leanok
		A modulus-one eigenvector for the mixed transfer map saturates the
		channel Schwarz inequality. Irreducibility then upgrades this
		eigenvector to an intertwiner between the two Kraus families, which is
		exactly gauge-phase equivalence.
\end{proof}
...
\begin{theorem}[Normal canonical form after common blocking]%
		\label{thm:exists_normal_canonical_form}
		\lean{MPSTensor.exists_normalCanonicalForm}
		\uses{def:is_normal_canonical_form, def:blocked_tensor}
		For every MPS tensor $A$, there exist a blocking length $p > 0$,
		a block family $(A_k)$ over the blocked physical space,
		and nonzero weights $(\mu_k)$ with strictly decreasing moduli such that
		the blocked tensor $A^{[p]}$ generates the same MPV family as the
		block-diagonal tensor $\bigoplus_k \mu_k A_k$, and
		$(\mu_k, A_k)$ satisfies the normal canonical form predicate.
\end{theorem}

\begin{proof}
		Decompose $A$ into irreducible blocks, put each block into TP gauge by
		Theorem~\ref{thm:tp_data_irreducible}, block until each transfer map is
		primitive by Theorem~\ref{thm:blocking_primitivity}, then take a common
		blocking length and reorder the surviving blocks by decreasing $|\mu_k|$.
		The remaining Lean work is the bookkeeping that packages these already
		formalized components into one theorem.
\end{proof}
...
\begin{remark}[Canonical form pipeline]%
		...
		\notready
		The Perron--Frobenius, TP-gauge, CFII, and periodicity-removal ingredients
		are now formalized. What remains is to assemble them into the repository's
		end-stage block predicates.

		The legacy route still aims at Definition~\ref{def:is_canonical_form}. For
		this route one still needs to turn blocked primitive TP data into the stronger
		injective blocks and eventual block-injectivity statements used by the legacy
		CF-BNT assembly.

		The new D1 route packages the same data directly as
		Definition~\ref{def:is_normal_canonical_form}. Its intended wrapper is
		Theorem~\ref{thm:exists_normal_canonical_form}, and that theorem is now the
		remaining sorry in this route. Once such normal-canonical-form data are
		available, the NT block-separation, BNT, and proportional Fundamental
		Theorem statements in Chapters~\ref{ch:block_perm} and~\ref{ch:assembly}
		apply without passing through the aperiodicity bridge of
		Chapter~\ref{ch:wielandt}.
\end{remark}
