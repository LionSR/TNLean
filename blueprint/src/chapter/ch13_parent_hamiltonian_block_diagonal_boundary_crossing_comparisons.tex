\begin{lemma}[Cyclic restrictions of block-diagonal boundary vectors]
    ...
    $N$ sites to $L$ sites,
    \begin{align}
        R_{i,\tau}\!\left(\Gamma_N^B\!\left(\bigoplus_{j=1}^gX_j\right)\right)
         & =
        \sum_{j=1}^g
...
\begin{lemma}[Local direct-sum constraint from block-diagonal boundary conditions]
    ...
    \leanok
    \uses{lem:block_diagonal_boundary_cyclic_restriction_sum, thm:block_sum_local_ground_space}
...
\begin{proof}
    ...
    before decomposition lies in $G_L(B)$.
    Lemma~\ref{lem:block_diagonal_boundary_cyclic_restriction_sum} rewrites this
    local vector as the displayed sum, and
    Theorem~\ref{thm:block_sum_local_ground_space} identifies $G_L(B)$ with
    $\bigvee_jG_L(A_j)$.
...
\begin{proof}
    \leanok
    \uses{def:ground_space_parent, rem:cyclic_window_operators, thm:contiguous_restriction_ground_space_map}
    ...
    $A_{\mathrm{right}}=A^j_{\tau_{i+L}}\cdots A^j_{\tau_{N-1}}$. Trace
    cyclicity gives
    \begin{align}
        R_{i,\tau}\!\left(\Gamma_N^{A_j}(\mu_j^NX_j)\right)
         & =
        \Gamma_L^{A_j}\!\left(A_{\mathrm{right}} \mu_j^NX_j\,A_{\mathrm{left}}\right).
        \notag
    \end{align}
...
\begin{lemma}[Boundary-crossing coefficient formula]
    ...
    \begin{align}
        \beta
         & =\sigma_{N-i}\cdots\sigma_{L-1},
        \notag                              \\
        \rho
         & =\tau_a\cdots\tau_{i-1},
        \notag                              \\
        \alpha
         & =\sigma_0\cdots\sigma_{N-i-1}.
        \notag
    \end{align}
...
\begin{proof}
    ...
    Since the interval crosses the cut, $\omega=\beta\rho\alpha$. Hence
    \begin{align}
...
\begin{proof}
    ...
    $\sum_j\tr\!\left(A^j_{\alpha\beta}C^j_{i,\tau}\right)$. Trace cyclicity
    rewrites this as
    $\sum_j\tr\!\left(A^j_\beta C^j_{i,\tau}A^j_\alpha\right)$. On the other
    hand, Lemma~\ref{lem:block_diagonal_boundary_crossing_coefficient_formula}
...
\begin{lemma}[Boundary representation gives fixed-tail boundary traces]
    ...
    \leanok
    \uses{lem:block_diagonal_boundary_local_sum_mem, lem:block_diagonal_boundary_crossing_trace_decomposition}
...
\begin{proof}
    ...
    sum of the corresponding block restrictions in $\bigvee_jG_L(A_j)$.
    Applying
...
\begin{theorem}[Fixed boundary-crossing $C^j,D^j$ comparison from the local block-sum constraint]
    ...
    \leanok
    \uses{lem:block_diagonal_boundary_crossing_trace_decomposition, lem:fixed_complementary_word_boundary_compatibility_from_traces}
    Assume $0<N$, $L\le N$, and let the cyclic interval beginning at $i$
    ...
    \begin{align}
        ...
        \label{eq:ph_fixed_boundary_crossing_comparison}
    \end{align}
...
\begin{proof}
    ...
    $X_j$ replaced by $\mu_j^NX_j$, gives
    (\ref{eq:ph_fixed_boundary_crossing_comparison}).
\end{proof}
...
\begin{theorem}[$C^j,D^j$ comparison for a block-diagonal boundary representation]
    ...
    \leanok
    \uses{lem:block_diagonal_boundary_crossing_trace_decompositions_boundary, lem:fixed_complementary_word_boundary_compatibility_from_traces}
    Let
    \begin{align}
        \psi
         & \in
...
\begin{theorem}[Fixed boundary-crossing component in the local block space]
    ...
    \leanok
    \uses{thm:block_diagonal_boundary_crossing_pgvwc_comparison, lem:block_diagonal_boundary_crossing_matrix_identity}
    Assume $0<N$, $L\le N$, and let the cyclic interval beginning at $i$
...
\begin{proof}
    ...
    gives matrices $C^j_{i,\tau}$ satisfying
    (\ref{eq:ph_fixed_boundary_crossing_comparison}). Thus
    $E=C^j_{i,\tau}$ satisfies the boundary matrix identity required in
...
\begin{lemma}[Boundary-crossing cyclic intervals from the boundary matrix identity]
    ...
    \begin{align}
        ...
        A^j_{\tau_a}\cdots A^j_{\tau_{i-1}}
         & =
        A^j_\beta E.
        \label{eq:ph_boundary_crossing_matrix_identity}
    \end{align}
    Then
    \begin{align}
        R_{i,\tau}\!\left(\Gamma_N^{A_j}(\mu_j^NX_j)\right)
...
\begin{proof}
    \leanok
    \uses{lem:block_diagonal_boundary_crossing_coefficient_formula, def:ground_space_parent, rem:cyclic_window_operators}
    ...
    past the cut, inside $\{0,\ldots,a-1\}$. The full $N$-site word appearing
    in the restriction is then
    $\beta\,\tau_a\cdots\tau_{i-1} \alpha$. Trace cyclicity gives
    \begin{align}
        \tr\!\left(A^j_\beta A^j_{\tau_a}\cdots A^j_{\tau_{i-1}}
        A^j_\alpha \mu_j^NX_j\right)
         & =
        \tr\!\left(A^j_\alpha \mu_j^NX_j A^j_\beta
        A^j_{\tau_a}\cdots A^j_{\tau_{i-1}}\right) =
        \tr\!\left(A^j_\alpha A^j_\beta E\right),
        \notag
    \end{align}
    where the second equality follows from
    (\ref{eq:ph_boundary_crossing_matrix_identity}). The last expression is
    the coefficient of $\Gamma_L^{A_j}(E)$ at the word $\alpha\beta$. Hence the
    restricted vector lies in $G_L(A_j)$.
\end{proof}
...
\begin{lemma}[Last crossing-window component trace form]
    ...
    \begin{align}
        \rho
         & =\tau_{m+1}\cdots\tau_{M-1},
        \notag                              \\
        C^j_a
         & =A^j_\rho A^j_a(\mu_j^{M+1}X_j).
        \notag
    \end{align}
...
\begin{proof}
    ...
    full word in the trace is $w\,b\,\rho\,a$. Hence the $j$-th coefficient is
    \begin{align}
        \tr\!\left(A^j_wA^j_bA^j_\rho A^j_a(\mu_j^{M+1}X_j)\right)
         & =
        \tr\!\left(A^j_bA^j_\rho A^j_a(\mu_j^{M+1}X_j)A^j_w\right),
        \notag
    \end{align}
    where the equality follows from trace cyclicity. This is the displayed
...
\begin{lemma}[Last crossing-window coefficient comparison]
    ...
    \leanok
    \uses{lem:block_diagonal_last_crossing_trace_form, lem:blockwise_boundary_identities_from_left_trace_membership}
    ...
    \begin{align}
        A^j_b\bigl(    A^j_\rho A^j_a(\mu_j^{M+1}X_j)
        \bigr)
         & =
        A^j_bE_jA^j_a.
        \label{eq:ph_last_crossing_coefficient_comparison}
    \end{align}
\end{lemma}
...
\begin{proof}
    ...
    gives (\ref{eq:ph_last_crossing_coefficient_comparison}).
\end{proof}
...
\begin{lemma}[Last crossing-window component membership]
    ...
    \leanok
    \uses{lem:block_diagonal_last_crossing_component_trace_form, lem:block_diagonal_last_crossing_coefficients_from_sum_membership, lem:left_boundary_component_mem_ground_space}
    In the setting of Lemma~\ref{lem:block_diagonal_last_crossing_trace_form},
    assume that the length-$m$ simultaneous word tuples span the product
    algebra and that
    \begin{align}
...
\begin{proof}
    ...
    gives matrices $E_j$ satisfying
    (\ref{eq:ph_last_crossing_coefficient_comparison}). By
    Lemma~\ref{lem:block_diagonal_last_crossing_component_trace_form}, the
