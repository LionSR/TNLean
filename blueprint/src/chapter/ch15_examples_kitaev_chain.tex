...
\begin{lemma}[The twist squares to minus the identity]\label{lem:kitaev_twist_sq}
    \lean{MPSTensor.kitaevTensor_zero, MPSTensor.kitaevTensor_one,
        MPSTensor.kitaevTwist_mul_self, MPSTensor.kitaevTensor_one_mul_self,
        MPSTensor.kitaevTwist_mul_kitaevTensor}
    \leanok
    \uses{def:kitaev_tensor}
    $A^0=\mathbb 1$, $A^1=Y$, $(A^1)^2=Y^2=-\mathbb 1$, and $Y$ commutes with
    $A^0$ and $A^1$
    \cite{Bultinck2017FermionicMPS}.
\end{lemma}
...
\begin{theorem}[Periodic amplitudes]\label{thm:kitaev_amplitudes}
    ...
    \begin{align}
        ...
        \begin{cases}
            ...
            0,           & k \text{ odd},
        \end{cases}
        \label{eq:kitaev_amplitudes_untwisted} \\
        ...
        \begin{cases}
            ...
            2(-1)^{(k+1)/2}, & k \text{ odd}.
        \end{cases}
        \label{eq:kitaev_amplitudes_twisted}
    \end{align}
...
\begin{proof}
    \leanok
    By Lemma~\ref{lem:kitaev_eval_word} the products are $Y^k$ and $Y^{k+1}$. Since
...
\begin{corollary}[Complementary parity supports]\label{cor:kitaev_parity_supports}
    ...
    By~\eqref{eq:kitaev_amplitudes_twisted}, $\tr(YA^w)\neq0 \iff k\equiv 1\pmod 2$,
    matching the odd fermion parity of~\eqref{eq:kitaev_chain_state};
    by~\eqref{eq:kitaev_amplitudes_untwisted}, $\tr(A^w)\neq0 \iff k\equiv 0\pmod 2$.
    Consequently, $\tr(YA^w)\tr(A^w)=0$ for every word $w$.
\end{corollary}
...
\begin{lemma}[Open-boundary closures]\label{lem:kitaev_open_boundary}
    ...
    \begin{align}
        M_{00}=M_{11}, \qquad M_{01}=-M_{10}.
        \label{eq:kitaev_open_closures}
    \end{align}
    Of the four ways to close the virtual indices at the two ends of an open
    chain, the two diagonal closures give the same amplitudes and the two
    off-diagonal closures differ by a sign~\cite{Bultinck2017FermionicMPS}.
\end{lemma}
...
\begin{theorem}[The ordinary contraction is reducible and not normal]%
    ...
    \lean{MPSTensor.kitaevTensor_mulVec_common_eigenvector,
        MPSTensor.kitaevTensor_not_isIrreducibleFamily,
    ...
    with eigenvalues $1$ and $i$, and the orthogonal projection onto the line it
    spans is a nontrivial projection invariant under both matrices. The tensor is
    not injective, and it is not normal: no blocking length makes the products
    span the full matrix algebra
    \cite[Section~III.A.4]{Cirac2021Matrix}.
\end{theorem}

\begin{proof}
    \leanok
    The line spanned by $(1,i)$ is invariant under $A^0=\mathbb 1$ and under
    $A^1=Y$. The linear functional $M\mapsto M_{00}-M_{11}$ vanishes on every
    product by~\eqref{eq:kitaev_open_closures} (Lemma~\ref{lem:kitaev_open_boundary}), while
    the matrix unit $E_{00}=\ketbra{0}{0}$ satisfies
    $(E_{00})_{00}-(E_{00})_{11}=1\neq0$, so
    $\operatorname{span}\{A^w : |w|=N\} \subsetneq \MN{2}$ for every $N$.
\end{proof}

\begin{theorem}[Sum of two injective matrix product states]%
    \label{thm:kitaev_two_injective}
    \lean{MPSTensor.kitaevSectorTensor, MPSTensor.kitaevSectorTensor_evalWord,
        MPSTensor.kitaevSectorTensor_isInjective, MPSTensor.kitaevTwistedCoeff_eq_add}
    \leanok
    \uses{def:kitaev_twisted_coeff, def:injective, lem:kitaev_eval_word}
    For $c\in\mathbb C$ let $a_c$ be the tensor of bond dimension one with
    $a_c^0=1$ and $a_c^1=c$; it is injective. Write $a_\pm=a_{\pm i}$. For
    every word $w$,
    \begin{align}
        \tr(YA^{i_1}\cdots A^{i_N})
        =
        i\,a_+^{i_1}\cdots a_+^{i_N}
        -i\,a_-^{i_1}\cdots a_-^{i_N},
        \label{eq:kitaev_sum_two_injective}
    \end{align}
    so the ordinary contraction of the twisted state is a sum of two injective
    matrix product states \cite[Section~III.A.4]{Cirac2021Matrix}.
\end{theorem}

\begin{proof}
    \leanok
    With $k$ the number of letters $1$ in $w$, the right-hand side is
    $i^{k+1}+(-i)^{k+1}$, which is the closed form of the twisted amplitude.
\end{proof}
