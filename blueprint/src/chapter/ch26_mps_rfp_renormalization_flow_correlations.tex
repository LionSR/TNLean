...
\begin{proof}
    \leanok
    \uses{def:ft_perm_gl, lem:ft_perm_gl_identities, def:mpdo,
    ...
    \begin{align}
        (\omega X A^{[2]}X^{-1})\otimes
        (\overline\omega\,\overline X\,\overline{A^{[2]}}\,\overline X^{-1})
        &=(X\otimes\overline X)M^{[2]}(X\otimes\overline X)^{-1},
        \notag
    \end{align}
    since $\omega\overline\omega=1$. If the transfer map were idempotent, all of
    its positive powers would equal the transfer map itself. Its dyadic powers
    would therefore form a constant, hence convergent, sequence. This
    contradicts the preceding phase-obstruction theorem, which shows that the
    dyadic orbit of $e_{21}$ alternates between the distinct factors $\omega$
    and $\omega^2$.
\end{proof}
