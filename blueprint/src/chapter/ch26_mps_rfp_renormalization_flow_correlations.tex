...
% CPSV16 Appendix D (`RFP-gauge`), lines 2094--2107.
% Provenance: counterexample formalization completed in issue #5920.
\begin{theorem}[Printed blocking-up-to-gauge equivalence]
    \label{thm:cpsv_rfp_gauge_printed_status}
    \notready
    \uses{thm:cpsv_canonical_form_flow_phase_obstruction, def:mps_transfer_idempotence}
    Let $\omega=e^{2\pi i/3}$, let
    \begin{align}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Thus blocking returns $A$ up to the swap gauge and a phase.  Nevertheless,
    the transfer map of $A$ is not idempotent.  Hence the pure-state
    equivalence asserted after the diagram labelled ``RFP-gauge'' in
    \cite[Appendix~D, lines~2094--2107]{Cirac2016MPDO_arXiv} is false.  The
    mixed-state question following the assertion therefore does not extend a
    valid pure-state theorem.
\end{theorem}
...
\begin{definition}[One-letter doubled-MPO virtual-gauge condition]
    ...
    For an MPO tensor $M$ with one physical component, the Appendix~D
    blocking diagram holds up to virtual gauge if $M^{[2]}$ and $M$ are related
    by simultaneous similarity on the virtual indices. For a pure tensor $A$,
    this condition is imposed on the doubled tensor
    \begin{align}
        M&=A\otimes\overline A.
        \notag
    \end{align}
    Both physical matrix algebras are one-dimensional, so every physical
    unitary-conjugation channel in the diagram is the identity. Similarity is
    symmetric and hence gives both directions of the diagram in
    \cite[Appendix~D, equation~\texttt{RFP-gauge}]{Cirac2016MPDO_arXiv}.
\end{definition}
...
\begin{definition}[One-letter MPS blocking witness]
    ...
    are an invertible matrix $X$ and a scalar $\zeta$ with $|\zeta|=1$ such
    that
    \begin{align}
        A&=\zeta X A^{[2]}X^{-1}.
        \notag
...
\begin{theorem}[An MPS blocking witness gives the doubled-MPO condition]
    ...
    Every one-letter MPS blocking witness induces the exact doubled-MPO
    virtual-gauge condition.
\end{theorem}
\begin{proof}
    \leanok
    \uses{def:doubled_virtual_gauge}
    Tensor the identity $A=\zeta X A^{[2]}X^{-1}$ with its complex conjugate.
    The virtual matrices combine to $X\otimes\overline X$, while the amplitude
    factor is $\zeta\overline\zeta=|\zeta|^2=1$.
\end{proof}
...
\begin{theorem}[The printed pure blocking-up-to-gauge equivalence is false]
    ...
    satisfying the source normalization convention. It also satisfies the literal
    one-letter virtual-gauge condition in the source diagram, but its transfer
    map is not idempotent. Hence the canonical-restricted pure-state
    equivalence claimed after
    \cite[Appendix~D, equation~\texttt{RFP-gauge}]{Cirac2016MPDO_arXiv} is false.
\end{theorem}
\begin{proof}
    \leanok
    ...
    Swapping the two virtual coordinates changes
    $\operatorname{diag}(1,\omega^2)$ into
    $\operatorname{diag}(\omega^2,1)$; multiplication by $\omega$ gives $A$.
    Writing $M=A\otimes\overline A$, doubling cancels the phase through
    \begin{align}
        ...
        \notag
    \end{align}
    since $\omega\overline\omega=1$. For every $n\geq0$, idempotence would imply
    \begin{align}
        \E_A^2=\E_A
        &\Longrightarrow \E_A^{2^n}=\E_A.
        \notag
    \end{align}
    The dyadic powers would therefore form a constant, hence convergent,
    sequence. This contradicts the preceding phase-obstruction theorem, which
    shows that the dyadic orbit of $e_{21}$ alternates between the distinct
    factors $\omega$ and $\omega^2$.
\end{proof}

\begin{remark}
    The counterexample refutes the printed pure equivalence. It does not decide
    the mixed-state continuation in the following source paragraph.
\end{remark}
...
\begin{theorem}[Per-block auxiliary equivalence]%
    ...
    \leanok
    \uses{def:is_canonical_form, def:mps_transfer_idempotence, def:is_zcl, thm:zcl_iff_idempotent_transfer}
