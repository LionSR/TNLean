...
\begin{theorem}[Printed blocking-up-to-gauge equivalence]
    ...
    \notready
    \uses{thm:cpsv_canonical_form_flow_phase_obstruction,
...
\begin{definition}[One-letter MPS blocking witness]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This is a sufficient MPS-level witness, not the source condition itself.
\end{definition}
...
\begin{theorem}[An MPS blocking witness gives the doubled-MPO condition]
    ...
    \leanok
    \uses{def:one_letter_mpdo_virtual_gauge_rfp,
        def:blocked_gauge_phase_fixed_point}
...
\begin{theorem}[The printed pure blocking-up-to-gauge equivalence is false]
    ...
    \lean{MPSTensor.cubePhaseTensor_normalized_canonical_gauge_not_rfp}
    \lean{MPSTensor.cpsv16_pure_rfp_gauge_equivalence_false}
    \leanok
    \uses{def:blocked_tensor, def:doubled_tensor, def:mpdo,
        def:one_letter_mpdo_virtual_gauge_rfp, def:cube_phase_swap_gauge,
        def:cpsv_canonical_form, def:mps_transfer_idempotence}
    ...
    The printed pure spectral argument cannot support the mixed-state
    continuation in the following source paragraph. This counterexample does
    not decide the mixed-state question.
\end{theorem}
\begin{proof}
    \leanok
    \uses{def:ft_perm_gl, lem:ft_perm_matrix_conj_submatrix, def:mpdo,
        lem:bond_dimension_one_identity_transfer_normal,
        def:cpsv_canonical_form, thm:blocked_gauge_phase_to_doubled_mpdo,
        thm:pure_doubled_tensor_rfp_via_ts,
        thm:cpsv_canonical_form_flow_phase_obstruction}
