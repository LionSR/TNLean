...
\begin{definition}[One-letter blocking up to virtual gauge and phase]
    \label{def:blocked_gauge_phase_fixed_point}
    \lean{MPSTensor.IsBlockedGaugePhaseFixedPoint}
    \leanok
    \uses{def:blocked_tensor}
    A tensor $A$ with one physical component is fixed by two-site blocking up
    to virtual gauge and physical phase if there are an invertible matrix $X$
    and a scalar $\zeta$ with $|\zeta|=1$ such that
    \begin{align}
        A&=\zeta X A^{[2]}X^{-1}.
        \notag
    \end{align}
    Since both physical spaces are one-dimensional, this is the one-letter MPS
    lift of the pure-state diagram in
    \cite[Appendix~D, equation~\texttt{RFP-gauge}]{Cirac2016MPDO_arXiv}.
    It suffices for the counterexample below; no equivalence with the full
    doubled-MPDO $T/S$ diagram is asserted.
\end{definition}
...
\begin{theorem}[The printed pure blocking-up-to-gauge equivalence is false]
    ...
    \lean{MPSTensor.cubePhaseTensor_isBlockedGaugePhaseFixedPoint}
    ...
    \leanok
    \uses{def:blocked_gauge_phase_fixed_point, def:cpsv_canonical_form,
        def:mps_transfer_idempotence,
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Define the virtual coordinate swap by
    \begin{align}
        S&=\begin{pmatrix}0&1\\1&0\end{pmatrix}.
        \notag
    \end{align}
    Then $\omega^3=1$, $\omega\ne1$, and direct multiplication gives
    \begin{align}
        A&=\omega S A^2S^{-1}.
        \notag
    \end{align}
    The tensor is in CPSV canonical form and its weights satisfy the source
    normalization convention. It satisfies the blocking-up-to-gauge
    condition with a unit phase and the orientation of the source diagram, but
    its transfer map is not idempotent. Hence the canonical-restricted
    pure-state equivalence claimed after the displayed blocking identity in
...
\begin{proof}\leanok
    ...
    On the off-diagonal matrix unit $e_{21}$, the transfer-map powers carry the
    factors $\omega$ and $\omega^2$, so the transfer map cannot be idempotent.
\end{proof}
