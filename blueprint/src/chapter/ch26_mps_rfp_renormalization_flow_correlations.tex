...
% CPSV16 Example 3.4 (`Ex:ZCL`), lines 450--465.
\begin{theorem}[Printed CID but non-fixed-point example]
    \label{thm:cpsv_example34_status}
    \notready
    \discussion{5916}
    \uses{def:source_physical_cid, def:mps_transfer_idempotence}
    Consider the tensor
    \begin{align}
        A^0&=\begin{pmatrix}1&0\\0&1/\sqrt2\end{pmatrix}, &
        A^1&=\begin{pmatrix}0&0\\0&1/\sqrt2\end{pmatrix}.
        \notag
    \end{align}
    Its length-$N$ matrix product vector is
    \begin{align}
        \ket{0}^{\otimes N}+\ket{+}^{\otimes N}.
        \notag
    \end{align}
    Example~3.4 of \cite{Cirac2016MPDO_arXiv} asserts that its physical
    correlations are independent of separation, while it is not a
    renormalization fixed point.  The blocked local vectors satisfy
    $\langle\widetilde 0\mid\widetilde +\rangle=2^{-n/2}$, which prevents an
    exact blocking isometry at finite block length.
\end{theorem}
