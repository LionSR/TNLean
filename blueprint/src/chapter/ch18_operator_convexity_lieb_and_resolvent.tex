...
\begin{proof}\leanok
    ...
    \[
        \langle \opvec K^\top,\,
            (A^s\otimes(B^\top)^{1-s})\opvec K^\top\rangle
...
\begin{theorem}[Lieb concavity, sub-boundary exponents]
    ...
    Let $x,y\ge 0$ with $x+y\le 1$. For any matrix~$K$ and
    positive-semidefinite matrices $A_1,A_2,B_1,B_2$, the map
    $(A,B)\mapsto\Re\tr(K^\dagger A^x K B^y)$ is jointly concave:
    \[
        t\,\Re\tr(K^\dagger A_1^x K B_1^y)
        +(1-t)\Re\tr(K^\dagger A_2^x K B_2^y)
        \le
        \Re\tr\!\bigl(K^\dagger(tA_1+(1-t)A_2)^x
          K(tB_1+(1-t)B_2)^y\bigr)
    \]
    for every $t\in[0,1]$.
...
\begin{proof}\leanok
    ...
    If $0<r\le 1$, set $s=x/r$. Then $x=rs$ and $y=r(1-s)$, with
    ...
    $C\mapsto C^r$ gives
    \[
      tA_1^r+(1-t)A_2^r\le (tA_1+(1-t)A_2)^r
    \]
    and the analogous inequality for $B$. The boundary
    positive-semidefinite theorem gives concavity at the left-hand powered
    inputs, and monotonicity of
