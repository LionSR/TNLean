...
\begin{lemma}[Window-level edge-insertion injectivity]%
    ...
    \leanok
    \uses{thm:peps_regionInsertedCoeff_injective, lem:peps_horizontal_staircase_window_injectivity, thm:peps_horizontal_staircase_reference_edge_crossing}
    ...
    \begin{align}
        ...
        \label{eq:peps_window_fit}
    \end{align}
    Suppose also that
    \begin{align}
        2L+1\le W,\qquad 2K+1\le H.
        \label{eq:peps_window_complement_fit}
    \end{align}
...
\begin{proof}
    ...
    map on $W_{\mathrm L}$. The single-window complement is also injective
    under the size hypotheses (\ref{eq:peps_window_fit})
    and (\ref{eq:peps_window_complement_fit}). Since
    $e\in\partial W_{\mathrm L}$,
...
\begin{theorem}[Window witness from a region insertion transfer]%
    ...
    \leanok
    \uses{def:peps_region_insertion_transfer, def:peps_window_edge_coeff_identity_witness, def:peps_window_edge_coeff_identity_witness_of_hypotheses}
    ...
    $2\le L$, $2\le K$, $1\le a$, $a+2L\le W$, and $b+2K-1\le H$,
    together with $2L+1\le W$ and $2K+1\le H$.
    Let $W_{\mathrm L}$ be the left end window and let $e$ be the reference
    ...
    \begin{align}
        ...
        \label{eq:peps_window_transfer_coeff}
    \end{align}
\end{theorem}
\begin{proof}
    \leanok
    The transfer determines a gauge $Z$ by the region insertion algebra:
    $T_e(M)=Z\,\iota_e(M)\,Z^{-1}$. Its coefficient-matching property
    gives (\ref{eq:peps_window_transfer_coeff}). The window and complement
    injectivity hypotheses are those required by
...
\begin{theorem}[Window witness from single-vertex realization]%
    ...
    \leanok
    \uses{def:peps_region_transfer_realizes, def:peps_region_insertion_transfer_of_realizes, thm:peps_windowEdgeCoeffIdentityWitness_of_transfer}
    Let $A$ and $B$ be PEPS tensors on a torus whose periods $W,H$ satisfy
    $1<W$ and $1<H$. Assume the one-orientation $L\times K$
    window-injectivity hypotheses for both tensors, union closure for
