...
\begin{theorem}[Scalar reduction of the residual two-tensor gauges]%
    ...
    For nonzero virtual leg spaces $\alpha,\beta,\gamma$, if an
    endomorphism of $\alpha\otimes\beta\otimes\gamma$ lies simultaneously in
    the three residual subspaces $\Id_\alpha\otimes\End(\beta\otimes\gamma)$,
    $\End(\alpha\otimes\beta)\otimes\Id_\gamma$, and the middle-identity space
    of operators acting as $\Id_\beta$ on the middle leg and as an endomorphism
    of $\alpha\otimes\gamma$ on the outer legs
    (Definition~\ref{def:peps_threeLegMiddleIdentityForm}), then it is a scalar
    multiple of the identity: there exists $\lambda\in\C$ such that
    \begin{align}
        T = \lambda\,\Id_{\alpha\otimes\beta\otimes\gamma}.
        \notag
    \end{align}
