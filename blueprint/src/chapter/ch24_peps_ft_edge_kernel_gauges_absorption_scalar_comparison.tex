\begin{theorem}[Absorbing edge gauges into the second PEPS]%
    ...
    \leanok
    \uses{def:peps_absorbEdgeGauges, thm:peps_absorbEdgeGauges_bondDim, thm:peps_absorbEdgeGauges_component}
...
\begin{theorem}[Post-absorption edge insertion equality]%
    ...
    \leanok
    \uses{thm:peps_edge_gauge_absorption, def:peps_postAbsorptionEdgeInsertionEquality}
...
\end{theorem}
\begin{proof}\leanok
    \uses{thm:peps_exists_edgeGaugeFamily, thm:peps_edge_gauge_absorption, def:peps_postAbsorptionEdgeInsertionEquality}
...
\begin{theorem}[Gauge consistency]%
    ...
    \leanok
    \uses{thm:peps_exists_edgeGaugeFamily, thm:peps_postAbsorptionEdgeInsertionEquality, thm:localGauge_exists, def:gaugeVertex}
...
\begin{definition}[Middle-identity three-leg residual form]%
    ...
    outer two legs and as the identity on the middle $\beta$ leg, with matrix entries
    $\delta_{bb'}W_{ac,a'c'}$ for $(a,b,c),(a',b',c')\in\alpha\times\beta\times\gamma$.
    This is the third residual form in \cite[Section~3]{Molnar2018NormalPEPS}.
\end{definition}
...
\begin{theorem}[Scalar reduction of the residual two-tensor gauges]%
    ...
    For nonzero virtual leg spaces $\alpha,\beta,\gamma$, let
    $Z\in\End(\beta\otimes\gamma)$, $U\in\End(\alpha\otimes\beta)$, and
    $W\in\End(\alpha\otimes\gamma)$ have equal induced three-leg residual forms
    (Definitions~\ref{def:peps_threeLegLeftIdentityForm}--\ref{def:peps_threeLegMiddleIdentityForm})
    on $\alpha\otimes\beta\otimes\gamma$, so that
    \begin{align}
        \delta_{aa'}Z_{bc,b'c'} = U_{ab,a'b'}\delta_{cc'} = W_{ac,a'c'}\delta_{bb'} \notag
    \end{align}
    for all $a,a'\in\alpha$, $b,b'\in\beta$, $c,c'\in\gamma$.
    Then there exists $\lambda\in\C$ such that all three residual matrices are scalar:
    \begin{align}
        Z = \lambda\,\Id_{\beta\otimes\gamma}, \qquad
        U = \lambda\,\Id_{\alpha\otimes\beta}, \qquad
        W = \lambda\,\Id_{\alpha\otimes\gamma}. \notag
    \end{align}
    \textbf{Local fix (nonzero virtual legs):}
    The scalar reduction requires that each virtual leg space $\alpha,\beta,\gamma$
    be nonzero; if any leg space is empty, the compatibility conditions hold vacuously
    and admit non-scalar solutions (such as $Z=\diag(1,2)$ when $\alpha=\varnothing$,
    $|\beta|=1$, $|\gamma|=2$). This models the nonzero-dimensional virtual bond
    spaces of \cite[Section~3]{Molnar2018NormalPEPS}; documented in
    \cite{gap:peps_injective_ft_section3_route}
    (\texttt{docs/paper-gaps/peps_injective_ft_section3_route.tex}).
\end{theorem}
\begin{proof}\leanok
    Write the common operator as $T$. The equality
    $\delta_{aa'}Z_{bc,b'c'}=U_{ab,a'b'}\delta_{cc'}$ gives
    $Z_{bc,b'c'}=0$ whenever $c\neq c'$ (by taking $a=a'$). Hence $Z$ is
    diagonal in the $\gamma$-index. Similarly, the equality
    $\delta_{aa'}Z_{bc,b'c'}=W_{ac,a'c'}\delta_{bb'}$ gives
    $Z_{bc,b'c'}=0$ whenever $b\neq b'$. Thus $Z_{bc,b'c'}=0$ unless
    $b=b'$ and $c=c'$.
    ...
    $Z=\lambda\Id_{\beta\otimes\gamma}$. Substitution into the compatibility
    equalities gives $U=\lambda\Id_{\alpha\otimes\beta}$ and
    $W=\lambda\Id_{\alpha\otimes\gamma}$.
\end{proof}
...
\begin{definition}[Matrix unit for coefficient extraction]%
    ...
    \mathlibok % Mathlib: Matrix.single.
...
\begin{theorem}[One-bond two-injective comparison]%
    ...
    \leanok
    \uses{thm:peps_twoBlockInsertedCoeff_singletonBond, thm:peps_twoBlockPointwiseProductCancellation}
...
\begin{proof}
    ...
    \cite[Section~3]{Molnar2018NormalPEPS}, where it is formulated as the
    compatibility of the three residual gauges $Z$, $U$, $W$
