...
\begin{theorem}[Scalar reduction of the residual two-tensor gauges]%
    ...
    $\End(\alpha\otimes\beta)\otimes\Id_\gamma$, and
    $\End(\alpha\otimes\gamma)\otimes\Id_\beta$, then it is a scalar multiple of
    the identity: there exists $\lambda\in\C$ such that
    \begin{align}
        T = \lambda\,\Id_{\alpha\otimes\beta\otimes\gamma}.
        \notag
    \end{align}
    % Local fix (nonzero virtual legs):
    ...
    % and admit non-scalar solutions (e.g. Z=\diag(1,2) when \alpha=\varnothing,
    ...
    % docs/paper-gaps/peps_injective_ft_section3_route.tex \cite{gap:peps_injective_ft_section3_route}.
\end{theorem}
\begin{proof}\leanok
