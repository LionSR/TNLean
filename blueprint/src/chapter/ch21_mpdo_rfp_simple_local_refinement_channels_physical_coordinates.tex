...
% which remains unresolved. Corrected rank-one criteria under positive
% semidefiniteness or linear independence are given by
...
% Documented in docs/paper-gaps/cpgsv17_pf_rank_one.tex.
\begin{theorem}[Blocking channels from SAL and ZCL (normalization obstruction)]
    ...
    supplied by the rank-one step of Lemma~C.5. The raw obstruction does not
    satisfy the normal-tensor standing hypothesis, while its normal
    representative loses literal ZCL. Thus the printed route remains
    unresolved at this normalization boundary; no alternative proof under the
    exact source hypotheses is known.
\end{theorem}
