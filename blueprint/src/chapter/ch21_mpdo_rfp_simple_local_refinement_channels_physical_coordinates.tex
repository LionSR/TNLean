...
\begin{theorem}[Conditional two-site blocking theorem from neighboring trace factorization]
    ...
    projector-controlled assembly when one factorization is supplied for each
    representative. The printed route obtains those factorizations from the
    false Proposition~C.12, so this conditional theorem does not prove the
    bare implication (ii)$\Rightarrow$(v). It also does not exclude different
    channel maps; see \cite{gap:cpgsv17_pf_rank_one}.
\end{theorem}
