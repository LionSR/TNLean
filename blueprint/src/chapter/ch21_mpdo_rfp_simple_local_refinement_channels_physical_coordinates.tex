...
% The conditional
% blocking theorem thm:mpdo_blocked_original_tensor_is_rfp_via_ts instead
% assumes a neighboring trace factorization.
% Documented in docs/paper-gaps/cpgsv17_pf_rank_one.tex.
\begin{theorem}[Blocking channels from SAL and ZCL (all-sector Case-II boundary)]
    ...
    input is Theorem~\ref{thm:mpdo_sal_zcl_bnt_sector_structure}, namely the
    all-sector factorization by physical isometries, closed sector tensors,
    positive neighboring operators, and rank-one coefficients. Once that
    factorization is available, Proposition~C.7 gives the two channels.
