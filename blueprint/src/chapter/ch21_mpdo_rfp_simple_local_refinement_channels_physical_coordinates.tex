...
\begin{theorem}[Blocking channels from representative trace factorizations]
    ...
    Theorem~\ref{thm:mpdo_sal_zcl_blocking_channels} nor excludes a different
    construction of its two channel maps.
\end{theorem}
...
\begin{theorem}[Two-site blocking from SAL and zero correlation length]
    ...
    common value into each representative. The printed proof at
    lines~1815--1816 then projects onto the mutually orthogonal outer physical
    sectors and invokes the one-representative channel construction. Their
    combination through the outer physical projections, including the
    trace-preserving completion outside their sum, is
    Theorem~\ref{thm:mpdo_absorbed_bnt_channel_support_completion}. Given one
    neighboring trace factorization for every absorbed representative, the
    ...
    source obtains these factorizations from Proposition~C.12, but
    Theorem~\ref{thm:mpdo_sal_zcl_bnt_sector_structure} refutes that
    intermediate assertion under the complete standing hypotheses; see
    \cite{gap:cpgsv17_pf_rank_one}. Hence the factorization is not a remaining
    proof obligation: the printed route fails. The counterexample does not
    exclude the existence of different channel maps, so the conclusion above
    is at present neither proved nor refuted.
\end{theorem}
