...
\begin{theorem}[Conditional composition of absorbed BNT channels]
    ...
    \lean{MPOTensor.blockTwo_isRFPViaTS_of_commonWeightAbsorbedBNTChannels}
    \leanok
    \uses{def:is_rfp_via_ts, def:mpdo_two_site_blocking,
        def:mpdo_common_sector_weight_absorption,
    ...
    \begin{align}
      ...
      \notag\\
      P_s\widetilde{\mathcal K}_s
        &=\widetilde{\mathcal K}_s
        =\widetilde{\mathcal K}_sP_s .
      \notag
    \end{align}
...
\begin{proof}\leanok
    \uses{thm:mpdo_absorbed_representative_boundary_decomposition,
        thm:mpdo_projective_resolution_channel_cptp}
    At each of the two closure lengths, expand the arbitrary-boundary closure
    as the sum of the absorbed-representative closures. Compression by the
    first-site projection annihilates every summand except the matching one,
    by two-sided physical support and $P_sP_t=0$ for $s\ne t$. Apply the
    corresponding representative channel to this surviving term and sum over
    $s$. The projective resolution makes both assembled maps trace-preserving
    and completely positive. The two resulting sums are respectively the
    length-two and length-four closures of $K$ with the original boundary.
\end{proof}
...
\begin{proof}\leanok
    ...
    vectors then give normalized rank-one density matrices on the two output
    spaces.

    Let $Q=\sum_sP_s$. Pairwise orthogonality makes $Q$ an orthogonal
    projection. Two-sided support of each absorbed representative implies
    that the length-two and length-four closures of the complete tensor are
    supported on the first-site lifts of $Q$.

    On the active corner, sum the representative channels after compression
    by the corresponding first-site projections. Each sum is completely
    positive and preserves exactly the trace selected by $Q$. Complete it on
    the orthogonal input complement by measuring $I-Q$ and preparing the
    fixed density matrix in the corresponding output space. The resulting map is
    trace-preserving and completely positive. Its complementary term vanishes
    on the supported closure, so the two channel equations reduce to those of
    the active sum and hence to the representative equations.
\end{proof}
