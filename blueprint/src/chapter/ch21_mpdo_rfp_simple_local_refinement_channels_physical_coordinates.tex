...
% one neighboring trace factorization. Theorem 4.9(iv) supplies such data
% separately for every outer BNT element, but the projector-controlled channel
% assembly is not yet formalized. Documented in
% docs/paper-gaps/cpgsv17_pf_rank_one.tex and tracked in issue #6632.
...
% from one neighboring trace factorization, including the rank-one identity
% tr(eta_{k,h}) = a_k b_h. Theorem 4.9(iv) supplies this data separately for
% each outer BNT element; the controlled assembly of those channels is not yet
% formalized. Documented in docs/paper-gaps/cpgsv17_pf_rank_one.tex and tracked
% in issue #6632.
\begin{theorem}[Conditional two-site blocking theorem from neighboring trace factorization]
    ...
    supplied factorization. The all-BNT implication
    \cite[Theorem~4.9, (iv)$\Rightarrow$(v)]{Cirac2016MPDO_arXiv} additionally
    requires the channels to be assembled along the mutually orthogonal outer
    physical supports.
\end{theorem}
...
% neighboring trace factorization. Even after such factorizations are obtained
% for every BNT element, the outer-sector channel assembly remains open.
...
\begin{theorem}[Blocking channels from SAL and ZCL (all-sector Case-II boundary)]
    ...
    The explicit bond-one counterexample to absorbed normality does not
    contradict this final statement; it invalidates only the printed route to
    its structural input. Deriving the channels from SAL and literal zero
    correlation length therefore requires a replacement for that structural
    step.
\end{theorem}
