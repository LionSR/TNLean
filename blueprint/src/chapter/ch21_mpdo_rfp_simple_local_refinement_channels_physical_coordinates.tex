...
% The supported-sector normal Case-I rank-one theorem is complete. The printed
% proof through prop2to3 still must show that every BNT tensor inherits MPDO
% positivity, SAL, and literal ZCL, and then combine the physical isometries,
% left--right tensors, and coefficients required by implication (ii) => (v).
% The conditional
...
\begin{theorem}[Blocking channels from SAL and ZCL (all-sector Case-II boundary)]
    ...
    The supported-sector rank-one theorem supplies the normal Case-I
    coefficient step. What remains is to prove that every BNT tensor inherits
    MPDO positivity, SAL, and literal ZCL, and then to combine the physical
    isometries, closed sector tensors, positive neighboring operators, and
    rank-one coefficients into the all-sector structure required by
    Proposition~C.7. It remains to establish this Case-II passage.
\end{theorem}
