\begin{definition}[Refinement in ordinary physical coordinates]
    ...
    \leanok
    \uses{def:mpdo_physical_sector_t_map, def:mpdo_physical_sector_closure_coordinates}
...
\begin{definition}[Physical coarse-graining]
    ...
    \leanok
    \uses{def:mpdo_physical_sector_s_map, def:mpdo_physical_sector_closure_coordinates}
...
\begin{definition}[Two-step physical refinement]
    ...
    \leanok
    \uses{def:mpdo_neighboring_trace_factorization, def:mpdo_physical_t_map, def:mpdo_localized_physical_t_map}
...
\begin{proof}\leanok
    \uses{thm:mpdo_physical_t_map_cptp, thm:mpdo_localized_physical_t_map_cptp}
...
\begin{proof}\leanok
    \uses{thm:mpdo_physical_t_closure_identity, thm:mpdo_localized_physical_t_closure_identity}
...
\begin{definition}[Two-step physical coarse-graining]
    ...
    \leanok
    \uses{def:mpdo_physical_s_map, def:mpdo_localized_physical_s_map}
    Define
    \begin{align}
        \widehat{\mathcal S}^2
...
\begin{proof}\leanok
    \uses{thm:mpdo_physical_s_map_cptp, thm:mpdo_localized_physical_s_map_cptp}
...
\begin{proof}\leanok
    \uses{thm:mpdo_localized_physical_s_closure_identity, thm:mpdo_physical_s_closure_identity}
...
\begin{theorem}[Blocked sector-coordinate tensor is a renormalization fixed point]
    ...
    \leanok
    \uses{def:is_rfp_via_ts, def:mpdo_two_site_blocking, def:mpdo_neighboring_trace_factorization}
    Let $F$ be a physical-sector factorization whose neighboring operators
...
\begin{proof}\leanok
    \uses{thm:mpdo_physical_t_squared_map_cptp, thm:mpdo_physical_t_squared_closure_identity, thm:mpdo_physical_s_squared_map_cptp, thm:mpdo_physical_s_squared_closure_identity, thm:mpdo_one_site_closure_two_site_blocking, thm:mpdo_two_site_closure_two_site_blocking}
...
\begin{theorem}[Conditional two-site blocking theorem from neighboring trace factorization]
    ...
    absorbed, these one-factorization channels can be inputs to the
    projector-controlled channel construction when one factorization is supplied
    for each representative. The printed route obtains those factorizations
    from the false Proposition~C.12, so this conditional theorem does not prove
    the bare implication (ii)$\Rightarrow$(v). It also does not exclude
    different channel maps; see \cite{gap:cpgsv17_pf_rank_one}.
\end{theorem}
...
% **False source result; proof-path drift:** Positivity, SAL, and literal ZCL
% descend to every common-weight absorbed BNT representative.  The completed
% ambient counterexample thm:mpdo_sal_zcl_bnt_sector_structure refutes the
% printed per-representative factorization under the full standing hypotheses.
% The source first projects into each local
% physical subspace labelled by j and then applies the blocked channel pair
% supplied by that factorization; this composition is formalized in #6632.  The
% project-derived direct sector-mixing alternative formerly tracked in #6793
% is not a separate source obligation.
...
\begin{theorem}[Absorbed-representative boundary decomposition]
    ...
    \leanok
    \uses{thm:mpdo_canonical_block_sum_arbitrary_boundary, def:mpdo_common_sector_weight_absorption, def:phys_close}
...
\begin{theorem}[Conditional composition of absorbed BNT channels]
    ...
    \leanok
    \uses{def:is_rfp_via_ts, def:mpdo_two_site_blocking, def:mpdo_common_sector_weight_absorption, thm:mpdo_absorbed_representative_boundary_decomposition}
...
\begin{theorem}[Completion outside the absorbed BNT supports]
    ...
    \lean{MPOTensor.sum_firstSiteMatrix,
        MPOTensor.sum_projection_twoSidedSupport_physicalSlice,
        MPOTensor.exists_chainCoordinateRFP_of_orthogonalSectorDecomposition,
        MPOTensor.blockTwo_isRFPViaTS_of_commonWeightAbsorbedBNTChannels_of_orthogonalSupports}
    \leanok
    \uses{thm:mpdo_absorbed_representative_boundary_decomposition, thm:mpdo_absorbed_bnt_channel_assembly, def:is_rfp_via_ts}
...
\begin{proof}\leanok
    \uses{thm:mpdo_absorbed_representative_boundary_decomposition, thm:mpdo_absorbed_bnt_channel_assembly, thm:matrix_support_completion_cptp, lem:mpdo_compressed_cp_sum, lem:mpdo_projector_controlled_trace_selection}
...
% This conditional theorem assumes the per-representative trace
% factorizations. Proposition C.12's claim that SAL and ZCL supply them is
% false under the complete standing hypotheses; see
% docs/paper-gaps/cpgsv17_pf_rank_one.tex.
\begin{theorem}[Blocking channels from representative trace factorizations]
    \label{thm:mpdo_absorbed_factorization_blocking_channels}
    \lean{MPOTensor.%
        blockTwo_isRFPViaTS_of_bntLayerOrthogonal_of_physicalSectorFactorization_of_commonWeight}
    \lean{MPOTensor.PhysicalSectorFactorization.NeighboringTraceFactorization.ofOneSector}
    \lean{MPOTensor.%
        blockTwo_isRFPViaTS_of_bntLayerOrthogonal_of_oneSectorFactorization_of_commonWeight}
    \leanok
    \uses{def:mpdo_physical_sector_factorization, def:mpdo_neighboring_trace_factorization, def:mpdo_bnt_layer_orthogonality, def:mpdo_common_sector_weight_absorption, def:is_rfp_via_ts, def:mpdo_two_site_blocking}
    Let $M$ be exactly gauge equivalent, by an invertible virtual matrix $G$,
    to a canonical BNT sum whose repeated-copy weights agree within each
    fixed representative and have been absorbed into tensors
    $(\mathcal K_s)_s$. Suppose that the simultaneous one-site tuples of the
    decomposition span the product of the virtual matrix algebras and that
    distinct absorbed representatives satisfy the unstarred layer
    orthogonality identities. If every $\mathcal K_s$ carries a
    physical-sector factorization with positive semidefinite neighboring
    operators $\eta^s_{k,h}$ and real families $(a^s_k)_k,(b^s_k)_k$ with
    \begin{align}
        \tr(\eta^s_{k,h}) & =a^s_kb^s_h,
                          &
        \sum_ka^s_kb^s_k  & =1,
        \notag
    \end{align}
    then $M^{[2]}$ is a renormalization fixed point via trace-preserving
    completely positive maps. In particular, this conclusion holds whenever
    ...
    factorizations are assumed here. They cannot in general be derived from
    saturation of the area law and zero correlation length:
    Theorem~\ref{thm:mpdo_sal_zcl_bnt_sector_structure} refutes that
    representativewise assertion under the complete standing hypotheses; see
    \cite{gap:cpgsv17_pf_rank_one}. Thus this conditional result neither proves
    Theorem~\ref{thm:mpdo_sal_zcl_blocking_channels} nor itself excludes a
    different construction of its two channel maps. The independent
    trace-norm argument in that theorem refutes the source conclusion.
\end{theorem}

\begin{proof}\leanok
    \uses{cor:mpdo_common_weight_bnt_layer_supports, thm:mpdo_blocked_original_tensor_is_rfp_via_ts, thm:mpdo_absorbed_bnt_channel_support_completion}
    Positivity of the neighboring operators makes every absorbed
    representative generate positive semidefinite periodic operators, and
    one-site spanning makes it injective. With the unstarred layer
    identities, Corollary~\ref{cor:mpdo_common_weight_bnt_layer_supports}
    gives pairwise orthogonal two-sided outer physical supports, realizing
    the projection onto each local subspace in the closing proof of the
    source. Each supplied trace factorization gives a blocked channel pair
    for its representative by
    Theorem~\ref{thm:mpdo_blocked_original_tensor_is_rfp_via_ts}. The
    projector-controlled combination of these pairs across the outer
    supports, completed trace-preservingly outside their sum, is
    Theorem~\ref{thm:mpdo_absorbed_bnt_channel_support_completion}. In the
    single-sector case the sole trace condition $\tr(\eta^s)=1$ realizes the
...
% failure of the printed representativewise factorization route.  Documented
% in docs/paper-gaps/cpsv16_prop_c15_blocking_channel_false.tex.
\begin{theorem}[The SAL--ZCL blocking-channel conclusion is false]
    \label{thm:mpdo_sal_zcl_blocking_channels}
    \lean{MPOTensor.CPSVBlockingChannelCounterexample.%
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Nevertheless, no trace-preserving completely positive map sends every
    two-site closure of $K$ to the corresponding three-site closure with the
    same virtual boundary. Nor does such a map exist from two sites to four
    sites. Consequently the two-site blocking $K^{[2]}$ is not a
    renormalization fixed point in the sense of Definition~4.1. Thus
    Proposition~C.15 and its twice-applied contribution to the condition-(ii)
    route to Theorem~4.9(v) are false as printed
    \cite{gap:cpsv16_prop_c15_blocking_channel_false}.
\end{theorem}
