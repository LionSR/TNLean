...
\begin{proof}\leanok
  ...
  Conversely, use two-sided integer induction.  The zero case is the identity
  translation.  In the successor step, compose the induction hypothesis with
  commutation by $\tau_1$.  In the predecessor step, use
  $\tau_{-1}=\tau_1^{-1}$ and compose commutation by $\tau_{-1}$ with the
  induction hypothesis.  The additive action law identifies the resulting
  composites with the required integer translations.
\end{proof}
