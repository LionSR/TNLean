...
\begin{corollary}[Necessary time-reversal invariants]
  ...
  path. For each endpoint, suppose the tensors $\mathcal U$, $\mathcal U_2$,
  and $\mathcal U_4$, together with the physical-adjoint comparison tensors
  $\mathcal U_2^\sharp$ and $\mathcal U_4^\sharp$, are simple and carry their
  own reduced full-support source data. Then admissible strict equivalence
  requires equality of both $\sigma^{(1)}$ and $\sigma^{(2)}$.
