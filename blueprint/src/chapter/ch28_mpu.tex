...
\begin{lemma}[Physical adjoints and double layers commute with blocking]
    \label{lem:mpu_double_layer_blocking}
...
\begin{theorem}[Blocked double-layer diagonal is a transfer power]
    ...
    \uses{def:mpu_double_layer,def:mpu_normalized_flattening,
        def:mpu_transfer_matrix,lem:mpu_double_layer_blocking}
...
\begin{proof}\leanok
    \uses{lem:mpu_double_layer_blocking}
...
\begin{theorem}[Identity plus traceless physical decomposition]
    \label{thm:mpu_WIsom}
    \lean{MPOTensor.exists_identity_traceless_basis,
    ...
    matrices $E,S_\alpha$ such that
    \begin{align}
        \sigma_0&=\Id,&
        \tr(\sigma_\alpha)&=0\quad(\alpha\ne0),\\
...
\begin{theorem}[Mixed residual traces and the rank-one sandwich]
    \label{thm:mpu_mixed_residual_trace}
    \lean{MPOTensor.IsMPU.trace_prod_mixedResidualFactor_doubleLayerTensor_eq_zero,
        MPOTensor.rankOne_sandwich_eq_zero_of_mixed_trace}
    \leanok
    \uses{thm:mpu_WIsom,thm:mpu_multilinear_physical_contraction,
        thm:mpu_normalized_transfer_stabilization}
    Every closed product containing at least one residual coefficient has
    zero trace.  In the formal MPU interface this conclusion is obtained for
    lengths $N>1$, because MPU unitarity is assumed at exactly those lengths;
    the source states the mixed identity for every positive length, including
    one residual letter.  If the stabilized transfer operator is rank one,
    $E=|\rho)(\Phi|$, then every mixed word $P$ satisfying the corresponding
    trace identity obeys
    \begin{align}
        EPE&=0.
    \end{align}
    This is equation \texttt{ESE=0} in
    \cite[Proposition~III.3(ii)]{Cirac2017MPU}.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpu_multilinear_physical_contraction}
    Represent identity factors by $d^{-1}\Id$ and residual factors by their
    traceless physical arguments.  The multilinear identity vanishes because
    one factor on the right has trace zero.  Finally,
    \begin{align}
        |\rho)(\Phi|P|\rho)(\Phi|
          &=\tr\!\left(P|\rho)(\Phi|\right)|\rho)(\Phi|,
    \end{align}
    and the scalar trace is zero.
\end{proof}
