...
\begin{theorem}[Trace powers above one determine the nonzero spectrum]
    ...
    \end{align}
    Thus $1\in\sigma(A)$ and every nonzero spectral value equals
    $1$~\cite[Proposition~III.1]{Cirac2017MPU}.
\end{theorem}
...
\begin{theorem}[Identification with chosen reduced-CFII fixed points]
    ...
    $\rho$ need not be diagonal, corresponding to the reduced representative in
    \cite[proof of Proposition~III.2(ii)]{Cirac2017MPU}.
\end{theorem}
...
\begin{theorem}[Normality of a full-support normalized MPU representative]
    ...
    required because adjoining a zero virtual direct summand preserves
    periodic operators while introducing a nontrivial invariant projection.
\end{theorem}
...
\begin{definition}[Trace-normalized source factors for the right shift]
    ...
    \begin{align}
        M_i & =X_i'Y_i',
            ...
            & Y_i'Z_i'                               & =\Id.
    \end{align}
    This gives the source-factor metric normalization of
...
The printed statement of \cite[Lemma~7]{Gross2012IndexTheory} states only that
$\mathcal A$ is a subalgebra; however, the commutant characterization requires
$\mathcal A$ to be closed under the involution, as in Schumacher and Werner
\cite[lines~1161--1171]{SchumacherWerner2004QCA}. We therefore formulate the
result for a star-subalgebra; see the paper-gap
note~\cite{gap:gnvw12_support_algebra_star_closure}. Furthermore, while Lemma~7
treats arbitrary finite-dimensional $C^*$-algebras, the statements below take
full matrix factors $\mathcal B_1=M_I(\mathbb C)$ and $\mathcal B_2=M_J(\mathbb
C)$, which suffice for the MPU spin chain where each one-site algebra is
isomorphic to $M_d(\mathbb C)$ \cite[lines~2320--2334]{Cirac2017MPU}; see
...
\begin{theorem}[Commutation of overlapping support algebras]
    ...
    full complex matrix algebras, which suffices for the MPU application
    \cite[lines~2300--2306, 2313--2334]{Cirac2017MPU}; see
    \cite{gap:gnvw12_support_algebra_full_matrix_scope}.
\end{theorem}
...
\begin{theorem}[Product of two full matrix factors]
    ...
    support algebras in Gross et al.~\cite[lines~1270--1308]{Gross2012IndexTheory};
    see \cite{gap:gnvw12_cscom_single_block_scope}.
\end{theorem}
...
\begin{definition}[Expansion of a blocked region]
    ...
    order within the block, matching the finite-region site grouping in
...
\begin{definition}[Algebraic-local site-blocking equivalence]
    ...
    \end{align}
    This corresponds to the algebraic-local change of site grouping in
    \cite[lines~2308 and 2313--2320]{Cirac2017MPU}.
\end{definition}
...
\begin{definition}[Site-indexed support algebras]
    ...
    While GNVW allow the cell size $d(y)$ to depend on $y$, the present
    definition treats a homogeneous chain with fixed size $d$; see
...
\begin{theorem}[Norm-closed generation by the support algebras]
    ...
    (\ref{eq:qca_support_algebra_topological_generation}) in the homogeneous
    fixed-$d$ setting; see
    \cite{gap:gnvw12_site_dependent_adjacent_generation_scope}.
\end{theorem}
...
\begin{theorem}[Two-site translation covariance of the support algebras]
    ...
    cell; translation by one does not preserve the even pairing $E_x$, as
    illustrated by the shift automorphism at
    \cite[lines~1267--1268]{Gross2012IndexTheory} where the even and odd
    support algebras have different dimensions.
    ...
    The statement treats the one-dimensional translation-invariant setting
    \cite{gap:sw04_support_covariance_scope}, with homogeneous cell size $d$
    \cite{gap:gnvw12_site_dependent_adjacent_generation_scope}.
\end{theorem}
