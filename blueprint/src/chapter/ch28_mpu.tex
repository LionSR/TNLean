...
\begin{theorem}[Exact interface for quantum cellular automata]
  ...
  \leanok
  \uses{def:qca}
  For $d>0$, a star-algebra automorphism $\omega$ of $\mathcal A$ is a quantum
  cellular automaton if and only if it is translation covariant and has finite forward
  propagation.  These two properties therefore construct a quantum cellular
...
\begin{theorem}[Identity quantum cellular automaton]
  ...
  \leanok
  \uses{def:qca}
  For $d>0$, the identity automorphism of $\mathcal A$ is a quantum cellular automaton.
\end{theorem}
...
Closure under forward composition is deferred to issue~\#6481, which supplies
composition of finite propagation bounds.  No inverse-locality assertion is
needed for that closure statement.
