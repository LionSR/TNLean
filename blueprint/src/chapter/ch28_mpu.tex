...
\begin{theorem}[Open periodic-tail source expansion]
    ...
    \leanok
    \uses{def:mpo_family, thm:mpu_source_v_open_two_site, thm:mpu_source_factor_recovery}
...
\begin{proof}\leanok
    \uses{thm:mpu_source_v_open_two_site, thm:mpu_source_factor_recovery}
...
\begin{theorem}[Closed source-cut formula for an auxiliary mixed-kernel Gram]
    \label{thm:mpu_mixed_y1_x2_gram_closed_source_cuts}
...
\begin{theorem}[Auxiliary mixed-kernel Gram entry with the second-cut metric]
    ...
    \leanok
    \uses{thm:mpu_mixed_y1_x2_gram_closed_source_cuts, thm:mpu_second_cut_metric}
...
\begin{proof}\leanok
    \uses{thm:mpu_mixed_y1_x2_gram_closed_source_cuts, thm:mpu_second_cut_metric}
    Substitute the second-cut metric factorization into the closed source-cut
    formula and expand the second-cut matrix products. Reordering the finite
    sums gives the displayed expression.
\end{proof}
...
\begin{theorem}[Complete source-$v$ network]
    ...
    \end{align}
    for all $i,j,k,\ell$. The weight is transposed, as in
    Theorem~\ref{thm:mpu_source_u_transpose_fixed_pair_trace}: the source
    gate of a tensor whose transfer fixed point is $\rho$ is the gate built
    from $\rho^{\mathsf T}$. This is the equivalence in
...
\begin{theorem}[Source-$v$ isometry of a simple tensor]
    ...
    \lean{MPOTensor.IsMPUSimple.sourceV_transpose_isIsometry,
    ...
    double layer with normalized diagonal $E$, and let $\rho>0$ satisfy
    $E^J=\lvert\rho)(\Phi\rvert$ for some $J>0$, where
    $(\Phi\rvert=\operatorname{vec}(\Id_D)^{\mathsf T}$. Let $v$ be the source
    operator of $\mathcal U$ built from the weight $\rho^{\mathsf T}$. Then
...
\begin{proof}\leanok
    ...
    $W^{ij}W^{k\ell}=W^{ij}\lvert\rho)(\Phi\rvert W^{k\ell}$. Applying
    Theorem~\ref{thm:mpu_source_v_complete_contraction} to the weight
    $\rho^{\mathsf T}$, whose transposed column-stacking vector is
    $\lvert\rho)$, gives the dressed Gram equation for the factors built from
    $\rho^{\mathsf T}$, and Theorem~\ref{thm:mpu_source_v_dressed_gram_cancel}
    cancels the dressing. Since $v^\dagger v=\Id$ on
...
\begin{proof}\leanok
    \uses{thm:mpu_source_v_dressed_gram_cancel, thm:mpu_source_v_complete_contraction, thm:mpu_supplied_witness_overlapping_reblocking}
    ...
    Theorem~\ref{thm:mpu_supplied_witness_overlapping_reblocking} supplies
...
\begin{proof}
    ...
    Suppose first that $\mathcal U$ is simple. The stabilized transfer power
    supplies the pair $\lvert\rho)(\Phi\rvert$, and
    Theorem~\ref{thm:mpu_simple_source_v_isometry} gives $v^\dagger v=\Id$
    for the source operator built from the weight $\rho^{\mathsf T}$, hence
    $r\ell\leq d^2$. Together with the standing source-$u$ isometry, this
    gives $r\ell=d^2$.
    % The source gate for the fixed point ρ is the formal gate built from the
    % weight ρᵀ; see docs/paper-gaps/mpu_source_cut_orientation.tex. The
    % statement above must fix this weight when the theorem is formalized.
...
\begin{definition}[Trace-normalized shift weight]
    ...
    $\rho_{\mathrm{aux}}=\Id_{d^2}$ after tensoring. They compute both the
    auxiliary mixed kernels and the paper gates. The latter carry reciprocal
    factors $d$ and $d^{-1}$ because these identity weights differ from
    $\rho_d$ and $\rho_{d^2}$ when $d>1$.
