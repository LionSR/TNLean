...
\end{align}
Thus this layer reuses the established operations
$X\mapsto X^{\mathrm L}_{rs}$ and $X\mapsto X^{\mathrm R}_{ij}$; it does not
introduce new notions of a slice or a block.  The construction below is the
full complex matrix-algebra specialization of the coefficient construction in
Schumacher--Werner, quant-ph/0405174, lines~1137--1169, and of the leastness
and commutant statements in GNVW Lemma~\emph{lem:spp}.  Its MPU application
is the support algebra $\mathcal R_{2x}$ discussed in
\cite[lines~2320--2334]{Cirac2017MPU}.  No full-matrix structure theorem,
dimension formula, Margolus gate, depth-two circuit, MPU standard form, or
index comparison is asserted here.
...
\begin{theorem}[Tensor-submodule criterion for left support]
    \label{thm:qca_left_support_tensor_submodule}
    \lean{Matrix.left_support_algebra_le_iff_le_tensor_submodule}
    \leanok
    \uses{def:qca_left_support_algebra}
    Let $I=\{1,\ldots,p\}$ and $J=\{1,\ldots,q\}$.  For every
    star-subalgebra $\mathcal B\subseteq M_p(\mathbb C)$,
    \begin{align}
        \operatorname{Spp}_{\mathrm L}(\mathcal A)\subseteq\mathcal B
        \quad\Longleftrightarrow\quad
        \mathcal A&\subseteq
    ...
    matrices whose left coefficient slices lie in $\mathcal B$.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:qca_left_support_minimality}
    Membership in the tensor-product submodule is equivalent to membership of
    every left coefficient in $\mathcal B$.  Apply
...
\begin{definition}[Kronecker-product submodule]
    ...
    \end{align}
    This is the honest tensor-product subspace used below, rather than a
...
\begin{proof}\leanok
    \uses{def:qca_kronecker_submodule}
    Each slice of $A\otimes B$ is a scalar multiple of $A$, and each block is
    a scalar multiple of $B$, so the forward implication extends by linearity.
...
\begin{theorem}[The one-sided tensor submodule as a Kronecker span]
    \label{thm:qca_kronecker_submodule_top}
    \lean{Matrix.kroneckerSubmodule_top_eq_tensorSubmodule}
    \leanok
    \uses{def:qca_kronecker_submodule}
    Let $I=\{1,\ldots,p\}$ and $J=\{1,\ldots,q\}$.  For every submodule
...
\begin{proof}\leanok
    \uses{thm:qca_mem_kronecker_submodule_iff}
    In Theorem~\ref{thm:qca_mem_kronecker_submodule_iff}, take the right
    submodule to be the full matrix space.  The right-block condition is then
    automatic, leaving precisely the left-slice criterion.
\end{proof}
...
\begin{proof}\leanok
    \uses{thm:qca_right_support_minimality,
...
\begin{theorem}[Two-sided support-algebra containment]
    ...
    \begin{align}
        ...
        \operatorname{Spp}_{\mathrm R}(\mathcal A).
    \end{align}
    This is the full complex matrix-algebra specialization of equation
    \emph{a2Supp} in Schumacher--Werner, quant-ph/0405174,
    lines~1161--1168, and of the symmetric conclusion of GNVW
    Lemma~\emph{lem:spp}.  It is an inclusion in an actual tensor-product
    span, not merely the conjunction of the two one-sided inclusions.  The MPU
    appendix applies this construction to $\mathcal R_{2x}$ in
    \cite[lines~2320--2334]{Cirac2017MPU}.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:qca_mem_kronecker_submodule_iff,
        thm:qca_left_coefficient_mem_support,
        thm:qca_right_coefficient_mem_support}
...
\begin{proof}\leanok
    Compare the $((i,r),(j,s))$ entries of the two products.  The identity
    matrix removes the sum over the second tensor coordinate, leaving exactly
    the $(i,j)$ entry of the coefficient commutator.
\end{proof}
...
\begin{proof}\leanok
    Compare the $((i,r),(j,s))$ entries of the two products.  The identity
    matrix removes the sum over the first tensor coordinate, leaving exactly
    the $(r,s)$ entry of the coefficient commutator.
\end{proof}
...
\begin{proof}\leanok
    \uses{thm:qca_left_coefficient_mem_support,
        thm:qca_left_kronecker_commute_iff,def:qca_left_coefficient_set,
        def:qca_left_support_algebra}
...
\begin{proof}\leanok
    \uses{thm:qca_operator_block_adjoint,
        thm:qca_right_coefficient_mem_support,
        thm:qca_right_kronecker_commute_iff,def:qca_right_coefficient_set,
        def:qca_right_support_algebra}
