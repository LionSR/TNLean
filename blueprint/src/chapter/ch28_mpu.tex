...
For the transfer-matrix spectrum argument of Proposition~\ref{prop:normal-tensor}
in \cite[lines~349--354]{Cirac2017MPU} we need the algebraic fact that a matrix
whose positive powers all have trace~$1$ must have characteristic polynomial
$X^{n-1}(X-1)$.

\begin{theorem}[Rank-one diagonal idempotent]
  \label{thm:mpu_diagonal_one_at}
  \lean{Matrix.diagonalOneAt, Matrix.trace_diagonalOneAt,
    Matrix.diagonalOneAt_pow, Matrix.trace_pow_diagonalOneAt,
    Matrix.charpoly_diagonalOneAt}
  \leanok
  Fix an index $i_0$ in $\{1,\dots,n\}$.  The diagonal matrix with a single
  $1$ at position $i_0$ and zeros elsewhere satisfies
  \begin{itemize}
    \item $M^k = M$ for every $k\ge 1$,
    \item $\tr(M^k) = 1$ for every $k\ge 1$,
    \item $\chi_M(X) = X^{\,n-1}(X-1)$.
  \end{itemize}
\end{theorem}
...
\begin{theorem}[Finite-range trace-power lemma]
  ...
  \leanok
  \uses{thm:newton_girard, thm:mpu_diagonal_one_at}
...
\begin{proof}\leanok
  ...
  Theorem~\ref{thm:mpu_diagonal_one_at}.  For $1\le k\le n$ we have
