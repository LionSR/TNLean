...
\begin{definition}[Auxiliary mixed source-factor kernels]
    ...
    \begin{align}
        a_{(l,r),(i_1,i_2)}
         & =\sum_\beta (Y_1)_{r,(\beta,i_1)}(X_2)_{(\beta,i_2),l},    \\
        b_{(j_1,j_2),(r,l)}
         & =\sum_\alpha (X_1)_{(j_1,\alpha),r}(Y_2)_{l,(j_2,\alpha)}.
    \end{align}
...
\begin{theorem}[Open periodic-tail source expansion]
    ...
    \lean{MPOTensor.mpo_finAddTwo_eq_sum_sourceV_sourceY₁_sourceY₂_evalWord,
        MPOTensor.mpo_finAddTwo_eq_sum_sourceV_sourceY₁_sourceX₂_openTail}
    \leanok
...
\begin{definition}[Auxiliary source-$v$ open-tail coefficient]
    ...
    \end{align}
    This coefficient occurs after the external source-$v$ entries have been
    separated from the open-tail expansion above. The entry $v_{q,(r,l)}$ is
    unstarred, while $v_{p,(r',l')}$ is starred, and the display contains the
    unique normalization factor $d^{-K}$. The coefficient is not itself a
    source gate.
\end{definition}
...
\begin{theorem}[Expanded normalized source-$v$ metric]
    \label{thm:mpu_mixed_y1_x2_expanded_metric}
    \lean{MPOTensor.IsMPU.normalized_sourceV_openTail_metric}
    \leanok
    ...
    $v,Y_1,X_2$ and the open tail gives
    \begin{align}
        d^{-K}\sum_{\tau,j,\zeta}
        ...
        \overline{\mathcal A_{p;\tau,j,\zeta}}
         & =\delta_{p,q},
    \end{align}
    where $\mathcal A_{q;\tau,j,\zeta}$ is the full source-$v$ expansion with
    the $q$ entry unstarred. This theorem evaluates the displayed expansion
    directly and does not identify it with the Gram matrix of $u$.
\end{theorem}
...
\begin{theorem}[Closed source-cut formula for an auxiliary mixed-kernel Gram]
    \label{thm:mpu_mixed_y1_x2_gram_closed_output_letter}
    \lean{MPOTensor.sourceY₁X₂_gram_apply_eq_closed_source_cuts}
    \leanok
    \uses{def:mpu_mixed_source_kernels, def:mpu_source_weight,
        def:mpu_weighted_source_factors}
    Let $\mathcal U$ be an MPO tensor, let $\rho$ be positive definite, and
    let $p,q\in\Fin d\times\Fin d$. Then
    \begin{align}
        \sum_{l,r}a_{(l,r),q}\overline{a_{(l,r),p}}
         & =\sum_{\beta,\delta}
        (M_1^\dagger W_\rho M_1)_{(\delta,p_1),(\beta,q_1)}
        (X_2X_2^\dagger)_{(\beta,q_2),(\delta,p_2)}.
    \end{align}
    Thus $q$ labels the unstarred entry and $p$ labels the starred entry. This
    is an auxiliary mixed-cut identity, not the source-$u$ network in
    \cite[Eq.~\texttt{uUnitary}]{Cirac2017MPU}.
\end{theorem}

\begin{proof}\leanok
    \uses{def:mpu_source_matrix_one, thm:mpu_source_factor_recovery,
        thm:mpu_source_x1_range_projection}
    Expand both source-$a$ entries and contract their common source indices.
    Weighted recovery of $Y_1$ replaces $Y_1^\dagger Y_1$ by
    $M_1^\dagger W_\rho M_1$, while the remaining $X_2$ factors form the
    range projector $X_2X_2^\dagger$.
\end{proof}
...
\begin{proof}
    ...
    Theorem~\ref{thm:mpu_mixed_kernels_two_site_factorization} gives the exact
