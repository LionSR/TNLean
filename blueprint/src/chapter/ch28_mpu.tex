...
\begin{lemma}[The source operator $u$ is an isometry]
  ...
  \notready
  \discussion{5708}
  \uses{def:mpu_source_uv,thm:mpu_source_factor_normalizations,
    thm:mpu_source_u_expanded_metric,thm:mpu_bounded_simple_blocking}
...
\begin{theorem}[Fundamental Theorem of MPU]
  ...
  $(u_{\mathcal V},v_{\mathcal V})$. They generate the same MPU,
  \begin{align}
    U^{(N)}&=V^{(N)}\qquad\text{for every }N,
  \end{align}
  if and only if their standard forms are related by the local unitary gauges
  of equations \textup{(SFuu)} and \textup{(SFvv)} of
  \cite{Cirac2017MPU}: there are unitaries $x$, $y$, and $z$ on the three
  corresponding source and canonical-form-II legs. In particular, the source
  unitaries obey the local relation
  \begin{align}
    u_{\mathcal V}&=(x\otimes y)u_{\mathcal U},
  \end{align}
  and the two canonical-form-II tensors are related on their virtual leg by
  $z$ and on their source legs by $x$ and $y$ exactly as in
  \textup{(SFvv)}.
\end{theorem}
...
\begin{definition}[Equivalence after blocking and ancillas]
  ...
  $d_b$. They are equivalent if there are a blocking length $k$ and coprime
  positive integers $p_a,p_b$ with $p_ad_a=p_bd_b$ such that
  $\mathcal U^{(p_a)}_k$ and $\mathcal V^{(p_b)}_k$ are strictly equivalent,
  where
  \begin{align}
...
\begin{proposition}[Local characterization of chirality]
  \label{PropChiral}
  \notready
  \discussion{5714}
  \uses{FundamentalMPU,IndexTh,SF}
  The MPU generated by a tensor in standard form satisfies
...
\begin{definition}[Equivalence under a symmetry]
  ...
  Two tensors $\mathcal U$ and $\mathcal V$ are equivalent under a symmetry
  $\mathcal S$ if there are ancilla dimensions $p_a,p_b$ and a common
  blocking length $k$ such that $\mathcal U^{(p_a)}_k$ and
...
\begin{lemma}[Trace formula for the chirality sign]
  \label{eq:sigma-from-trace}
  \notready
  \discussion{5714}
  \uses{PropChiral}
  For a chirality-invariant MPU in standard form, the sign $\sigma$ in
...
\begin{proposition}[Well-definedness of the chirality sign]
  ...
  constant under continuous deformations preserving chirality.
\end{proposition}

\begin{proposition}[Blocking parity of the chirality sign]
...
\begin{corollary}[Necessary chirality invariants]
  ...
  Strict equivalence of two chirality-invariant MPUs requires equality of
...
\begin{proposition}[Chirality class and the MPS sign]
  \label{prop:dagger-mps-class-and-mpu-class-equal}
  \notready
  \discussion{5714}
  \uses{PropChiral,prop:sigma1-and-sigma2-equal}
  For an MPU in standard form invariant under chirality, the MPS sign
...
\begin{proposition}[All phases under conjugation]
  ...
  \begin{align}
    ...
    v'_\pm&=(\Id_{r\ell/4}\otimes R)v_\pm,
  \end{align}
...
\begin{proposition}[The other three chirality classes]
...
\begin{lemma}[Transformation with swaps and symmetry equivalence]
  ...
  \notready
  \discussion{5715}
  \uses{def:threeMPU,def:threeMPU2,def:equivalent-symmetry}
...
\begin{proposition}[Conjugation phases of $U_2$ and $U_3$]
  ...
  \notready
  \discussion{5715}
  \uses{def:threeMPU,conjphs,prop:conjsym-strict-equiv}
...
\begin{proposition}[Time-reversal and transposition phases of $U_2$ and $U_3$]
  ...
  \notready
  \discussion{5715}
  \uses{def:threeMPU2,lemma:sym-trafo-swap,
...
\begin{proposition}[Conjugation phases of $U_1$ and $U_2$]
  ...
  conjugation exactly when
  \begin{align}
    d&=4k+2\qquad(k=0,1,2,\ldots).
  \end{align}
