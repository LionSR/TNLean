...
\begin{proof}\leanok
  ...
  leaves one nonzero virtual configuration.  Thus the matrix entry in row
  $\sigma$ and column $\tau$ is
  \begin{align}
    \langle\sigma\vert T^{(N)}\vert\tau\rangle
      &=\begin{cases}
          1,&\tau_n=\sigma_{n+1}\text{ for every }n\pmod N,\\
          0,&\text{otherwise}.
        \end{cases}
  \end{align}
  Hence the output configuration is
