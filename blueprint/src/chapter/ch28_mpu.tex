...
\begin{definition}[Trace-normalized shift weight]
    ...
    \leanok
    For a virtual space of dimension $d$, set
    \begin{align}
        W_d & = d^{-1}\Id_d.
    \end{align}
    On the product virtual space of the tensor-product shift examples, the
    corresponding weight is $W_{d^2}=(d^2)^{-1}\Id_{d^2}$. These weights are
    reserved for the paper-facing source-factor calculations and are distinct
    from the identity weights used by the supplied mixed-kernel formulas below.
\end{definition}
...
\begin{theorem}[Positivity and normalization of the shift weight]
    ...
    For every $d\geq0$, the matrix $W_d$ is positive semidefinite. If $d>0$,
    then $W_d$ is positive definite and
    \begin{align}
        \tr(W_d) & =1.
    \end{align}
    The same statements hold for $W_{d^2}$.
\end{theorem}
