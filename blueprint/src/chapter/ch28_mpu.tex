...
\begin{proof}
  \uses{thm:mpu_admissible_conjugation_odd_rank,
    ...
    prop:mpu_admissible_conjugation_standard_forms,
    def:mpu_admissible_symmetry_equivalence}
  If either $r$ or $\ell$ is odd, only Case I can occur. After one blocking,
  the endpoint determinants satisfy
  \begin{align}
    \det(u_i^{(2)}v_i^{(2)})&=\det(u_iv_i)^{2r\ell}=1.
  \end{align}
  The odd-rank theorem therefore gives one admissible equivalence class, namely
  Case I. If $r$ and $\ell$ are both even, one blocking gives
  \begin{align}
  ...
  \end{align}
  Blocking preserves Case I and Case II. The even-rank theorem then shows that
  the blocked endpoints are admissibly strictly equivalent exactly when they
  belong to the same case. This is precisely admissible equivalence of the
  original endpoints.
\end{proof}
