...
\begin{theorem}[Admissible conjugation symmetry when one source rank is odd]
  ...
  admissible path datum whose common blocking length $m$ satisfies $m\leq D^4$.
  Then $\mathcal U_1$ and $\mathcal U_2$ are admissibly strictly equivalent
...
\begin{corollary}[Admissible equivalence under conjugation]
  ...
  For $i=1,2$, suppose the original one-site endpoint tensor
  $\mathcal U_i$ and its conjugate comparison tensor are simple and each
  carries reduced full-support source data. Suppose also that the comparison
  blockings used below carry reduced full-support source data and that the
  resulting standard-form interpolation is admissible. Then
...
\begin{proof}
  ...
  $\ell^{(2)}$ for the blocked source ranks. Rank scaling gives
  \begin{align}
    r^{(2)}&=dr,
  \end{align}
  and
  ...
  equivalence. If both blocked ranks are even, the even-rank theorem applies
  instead; the preserved Case I condition and the two separate determinant
  identities again give admissible strict equivalence. Thus the odd-source-rank
  branch has a single admissible equivalence class without imposing a parity
  condition on the blocked ranks.

  If $r$ and $\ell$ are both even, the blocked ranks remain even. The same two
  separate determinant identities hold, and blocking preserves Case I and
  Case II. The even-rank theorem therefore shows that the blocked endpoints are
  admissibly strictly equivalent exactly when they belong to the same case.
  This is precisely admissible equivalence of the original endpoints.
\end{proof}
