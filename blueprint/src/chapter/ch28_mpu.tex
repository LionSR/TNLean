...
\begin{definition}[Reduced full-support source datum]
  ...
  \uses{def:mpu,def:mpu_normalized_flattening,def:cpsv_cfii,
    ...
    thm:mpu_reduced_cfii_transfer_stabilization,
    thm:mpu_normalized_transfer_fin_one}
  A reduced full-support source datum for an MPU tensor $\mathcal U$ of
  physical dimension $d>0$ and positive bond dimension consists of an integer
  $J>0$, a positive definite matrix $\rho$ with $\tr(\rho)=1$, and
  ...
  \end{enumerate}
  This definition does not assert that an arbitrary representative with
  $D>1$ admits the required canonical-form-II data and does not replace
  $\mathcal U$ by another tensor.
\end{definition}
...
\begin{definition}[Standard form]
  ...
  \notready
  \discussion{6019}
  % **Scope restriction (reduced full-support source datum):** the planned
  % standard form uses Theorem~\ref{ThmFund1} only for a tensor carrying the
  % datum in Definition~\ref{def:mpu_reduced_full_support_source_datum}.
  % Documented in docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{ThmFund1,def:mpu_source_uv,
    def:mpu_reduced_full_support_source_datum}
  Let $\mathcal U$ be a simple tensor equipped with a reduced full-support
  source datum. The standard form of the two-site
...
\begin{theorem}[Fundamental Theorem of MPU]
  ...
  % carry the data required by Definition~\ref{SF}. Documented in
  % docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{SF,ThmFund1,def:mpu_reduced_full_support_source_datum}
  Let $\mathcal U$ and $\mathcal V$ be simple tensors equipped with reduced
  full-support source data and written in standard form, with
...
\begin{definition}[Index]
  ...
  equipped with a reduced full-support source datum, and let
...
\begin{proposition}[Well-definedness of the index]
  ...
  \uses{def:index,ThmFund1,thm:mpu_source_cut_rank_blocking,
    ...
    def:mpu_reduced_full_support_source_datum}
  The index is independent of the chosen admissible simple blocking. More
  precisely, if $k\geq k_0$, both $\mathcal U_{k_0}$ and $\mathcal U_k$ are
  simple, and each is equipped with a reduced full-support source datum, then
  \begin{align}
    r_k&=d^{k-k_0}r_{k_0},&
...
\begin{lemma}[Composition additivity]
  ...
  generates the composition of the MPUs
  generated by $\mathcal U$ and $\mathcal U'$, then
  \begin{align}
    \operatorname{ind}(\mathcal W)
      &=\operatorname{ind}(\mathcal U)+\operatorname{ind}(\mathcal U').
  \end{align}
  This is the composition argument in the proof of
  \cite[Theorem~IV.3(ii)]{Cirac2017MPU}.
  % Source lines: paper_v2.tex 837--845.
\end{lemma}
...
\begin{definition}[Admissible equivalence and path datum]
  ...
  When the path is used under a symmetry, the datum also includes continuously
  varying reduced full-support source data for every transformed comparison
  tensor to which the fundamental theorem is applied. These are the conjugate,
  physical-adjoint, transposed, or more general symmetry-image tensors required
  by the argument, blocked at the same length. The transformed blocked tensors
  themselves are simple; no preservation under the transformation is assumed.
  ...
  joined by an admissible strict-equivalence path.
\end{definition}
...
\begin{proposition}[Constancy of the source ranks]
  ...
  % simple tensor must carry the datum required by Theorem~\ref{ThmFund1}.
  % Documented in docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{ThmFund1,thm:mpu_bounded_simple_blocking,
    def:mpu_reduced_full_support_source_datum,
...
\begin{theorem}[Index Theorem]
  ...
  \uses{index-well-defined,lem:mpu_index_tensor,
    ...
    def:mpu_admissible_equivalence_datum,prop:continuity-index,SF,
    FundamentalMPU,def:mpu_reduced_full_support_source_datum}
  On the class of tensors and operations carrying the source data specified in
  Definition~\ref{def:mpu_admissible_equivalence_datum}, the MPU index has the
...
\begin{proposition}[Conjugation symmetry]
  ...
  % tensors compared by Theorem~\ref{FundamentalMPU} carry reduced full-support
  % source data. Documented in
  % docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{FundamentalMPU,SF,def:mpu_reduced_full_support_source_datum}
  Let a tensor and its conjugate comparison tensor both be equipped with
  reduced full-support source data and written in standard form. The generated
...
\begin{proposition}[Orthogonal and symplectic standard forms]
  ...
  \uses{prop1conj,lemma:conjclass-normalform-continuous,FundamentalMPU,
    def:mpu_reduced_full_support_source_datum}
  Let an MPU in standard form be invariant under complex conjugation, and
...
\begin{corollary}[Reduction of the symplectic case to orthogonal unitaries]
  ...
  % source-data-restricted standard form of Proposition~\ref{corconj}.
  % Documented in docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{corconj,def:mpu_reduced_full_support_source_datum}
  In the admissible Case II of Proposition~\ref{corconj}, one can write
  \begin{align}
...
\begin{proposition}[Local characterization of time reversal]
  ...
  \uses{FundamentalMPU,IndexTh,SF,
    def:mpu_reduced_full_support_source_datum}
  Let an MPU of index zero be in standard form. Suppose its one-site tensor
...
\begin{definition}[Strict equivalence under a symmetry]
  ...
  witnessing path carries an admissible equivalence datum for both the actual
  blocked path tensor and every conjugate, physical-adjoint, transposed, or
  other symmetry-image comparison tensor used along the path.
\end{definition}
...
\begin{definition}[Equivalence under a symmetry]
  ...
  equivalence datum for the actual blocked path and every transformed
  comparison path tensor used by the symmetry argument.
\end{definition}
...
\begin{lemma}[Standard-form path criterion]
  ...
  \uses{def:strictly-equivalent-symmetry,coruvSF,FundamentalMPU,
    def:mpu_admissible_equivalence_datum}
  Two MPUs $U_1^{(N)}$ and $U_2^{(N)}$ described by admissible simple tensors
  are admissibly strictly equivalent under $\mathcal S$ if and only if there
  are continuous families of unitaries $\widetilde u(p)$ and
  $\widetilde v(p)$ whose endpoints generate $U_1^{(N)}$ and $U_2^{(N)}$,
  whose generated MPU is invariant under $\mathcal S$ for every $p\in[0,1]$,
  and whose actual blocked tensor path and every transformed comparison path
  tensor carry the continuously varying source data of an admissible
  equivalence datum.
\end{lemma}
...
\begin{theorem}[Conjugation symmetry when one source rank is odd]
  ...
  \uses{corconj,lem:equivalent-symmetry,
    def:mpu_admissible_equivalence_datum}
  Suppose that either $r$ or $\ell$ is odd and that the standard-form
  interpolation determined below carries an admissible path datum. Two MPUs
  $\mathcal U_1$ and $\mathcal U_2$ are admissibly strictly equivalent under
  conjugation symmetry if and only if
  \begin{align}
...
\begin{corollary}[Equivalence of conjugation-symmetric MPUs after blocking]
  ...
  \notready
  \discussion{5713}
  % **Scope restriction (blocking and admissible path):** the blocked tensors
  % carry their own source data and the resulting symmetry path is admissible.
  % No preservation under blocking is asserted. Documented in
  % docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{conjphs,def:equivalent-symmetry,
    def:mpu_admissible_equivalence_datum}
  All MPU tensors in the odd-source-rank case whose required blocked tensors
  carry reduced full-support source data, and whose resulting standard-form
  interpolation is admissible, are admissibly equivalent under conjugation
  symmetry.
\end{corollary}
...
\begin{theorem}[Strict conjugation equivalence for even source ranks]
  ...
  % Definition~\ref{def:mpu_admissible_equivalence_datum}. Documented in
  % docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{corconj,cor:sympl-to-orthogonal,lem:equivalent-symmetry,
    lemma:conjclass-normalform-continuous,
    def:mpu_admissible_equivalence_datum}
  Suppose that $r$ and $\ell$ are both even and that the standard-form
  interpolation determined below carries an admissible path datum. Choose
  admissible standard forms of two conjugation-invariant MPUs as in
  Proposition~\ref{corconj}. They are admissibly strictly equivalent under
...
\begin{corollary}[Equivalence under conjugation]
  ...
  \notready
  \discussion{5713}
  % **Scope restriction (blocking and admissible path):** all blocked endpoints
  % carry their own source data and the symmetry path is admissible. Documented
  % in docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{prop:conjsym-strict-equiv,def:equivalent-symmetry,
    def:mpu_admissible_equivalence_datum}
  Suppose the comparison blockings carry reduced full-support source data and
  the resulting standard-form interpolation is admissible. Then two MPU
  tensors are admissibly equivalent under conjugation symmetry if and only if
...
\begin{lemma}[Trace formula for the time-reversal sign]
  ...
  % the blocked source data required by Proposition~\ref{PropChiral}.
  % Documented in docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{PropChiral,def:mpu_reduced_full_support_source_datum}
...
\begin{proposition}[Well-definedness of the time-reversal sign]
  ...
  \uses{eq:sigma-from-trace,lem:equivalent-symmetry,
    def:mpu_reduced_full_support_source_datum,
    def:mpu_admissible_equivalence_datum}
  Suppose an admissible time-reversal-preserving path is given such that, at
  every path point, the one-site tensor, its actual two-site block, and the
  physical-adjoint comparison of that two-site block are simple and carry
  continuously varying reduced full-support source data. Then the sign
...
\begin{proposition}[Blocking parity of the time-reversal sign]
  ...
  be the sign of the resulting admissible standard form. Then
  \begin{align}
    \sigma^{(k)}
      &=\begin{cases}
        \sigma^{(1)},&k\text{ odd},\\
...
\begin{corollary}[Necessary time-reversal invariants]
  ...
  \uses{prop:sigma1-and-sigma2-equal,def:strictly-equivalent-symmetry,
    def:equivalent-symmetry,def:mpu_reduced_full_support_source_datum,
    def:mpu_admissible_equivalence_datum}
  Let two time-reversal-invariant MPUs be compared by an admissible symmetry
  path. For each endpoint, suppose the one-site tensor, the two-site tensor,
  and the physical-adjoint comparisons of both tensors are simple and carry
  their own reduced full-support source data. Then admissible strict
  equivalence requires equality of both $\sigma^{(1)}$ and $\sigma^{(2)}$.
  Admissible equivalence after blocking and adding ancillas requires equality
  of $\sigma^{(2)}$ for endpoint and comparison tensors carrying the same
  data after those operations.
\end{corollary}
...
\begin{proposition}[Conjugation class and the MPS sign]
  ...
  \uses{corconj,cor:equiv-under-conjugation,
    def:mpu_admissible_equivalence_datum}
  For an admissible MPU invariant under conjugation, the sign
...
\begin{proposition}[Time-reversal class and the MPS sign]
  ...
  \notready
  \discussion{5714}
  % **Scope restriction (two-site blocked source data):** the two-site blocked
  % tensor carries the datum required by Proposition~\ref{PropChiral}.
  % Documented in docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{PropChiral,prop:sigma1-and-sigma2-equal,
    def:mpu_reduced_full_support_source_datum,
    def:mpu_admissible_equivalence_datum}
  Let an MPU in admissible standard form be invariant under time reversal.
  Suppose its one-site tensor $\mathcal U$, its actual two-site block
  $\mathcal U_2$, and the physical-adjoint comparison tensors required at one
  and two sites are simple and each carries its own reduced full-support source
  datum. Then the MPS sign $G\overline G=\pm\Id$ equals the sign
  $\sigma^{(2)}$.
\end{proposition}
...
\begin{proposition}[All phases under conjugation]
  ...
  \uses{conjphs,prop:conjsym-strict-equiv,corconj,
    cor:sympl-to-orthogonal,def:mpu_reduced_full_support_source_datum,
    def:mpu_admissible_equivalence_datum}
  Assume the standard-form tensors displayed below carry reduced full-support
  source data and that the comparison paths used to classify them are
  admissible. Let
  \begin{align}
    u_+&=v_+=\Id_{r\ell},&
...
\begin{proposition}[CZX]
  ...
  \notready
  \discussion{5715}
  % **Scope restriction (blocked CZX data):** the one-site and two-site tensors
  % used to define the two signs carry reduced full-support source data.
  % Documented in docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{prop:sigma1-and-sigma2-equal,
    prop:dagger-mps-class-and-mpu-class-equal,
    def:mpu_reduced_full_support_source_datum,
    def:mpu_admissible_equivalence_datum}
  Suppose the one-site CZX tensor, its actual two-site block, and the
  physical-adjoint comparison tensors required at one and two sites are simple
  and each carries its own reduced full-support source datum. The CZX MPU is
  defined by
  \begin{align}
...
\begin{proposition}[The other three time-reversal classes]
  ...
  \uses{example:czx,prop:sigma1-and-sigma2-equal,
    def:mpu_reduced_full_support_source_datum,
    def:mpu_admissible_equivalence_datum}
  For each displayed example, suppose its one-site tensor, its actual two-site
  block, and the physical-adjoint comparison tensors required at one and two
  sites are simple and each carries its own reduced full-support source datum.
  The standard forms
  \begin{align}
    u&=v=\Id,\\
    u&=i\Id,&v&=\Id,\\
...
\begin{lemma}[Transformation with swaps and symmetry equivalence]
  ...
  \uses{threeMPU,threeMPU2,def:equivalent-symmetry,
    def:mpu_reduced_full_support_source_datum,
    def:mpu_admissible_equivalence_datum}
  For every endpoint in the comparison, suppose the one-site tensor, its
  actual two-site block, and every physical-adjoint, transposed, conjugate, or
  symmetry-image comparison tensor used at one or two sites are simple and
  carry their own reduced full-support source data. Suppose also that all
  comparison paths carry these data continuously. Then two MPUs
  $U_i^{(N)}$ and $U_j^{(N)}$ are in the same phase under time reversal
  combined with the local swap, transposition combined with the local swap,
  or conjugation if and only if the corresponding
...
\begin{proposition}[Conjugation phases of $U_2$ and $U_3$]
  ...
  and the standard-form comparison paths are admissible. Then $U_2^{(N)}$ and
  $U_3^{(N)}$ are both in the
...
\begin{proposition}[Time-reversal and transposition phases of $U_2$ and $U_3$]
  ...
  \uses{threeMPU2,lemma:sym-trafo-swap,
    ...
    def:mpu_reduced_full_support_source_datum,
    def:mpu_admissible_equivalence_datum}
  Suppose the one-site and actual two-site endpoint tensors, together with
  their physical-adjoint and transposed comparison tensors at both blocking
  lengths, are simple and carry their own reduced full-support source data.
  Suppose the displayed comparison paths carry these data continuously. Then
  the MPUs $\widetilde U_2^{(N)}$ and $\widetilde U_3^{(N)}$ are in the same
  admissible phase under both time reversal and transposition.
\end{proposition}
...
\begin{proposition}[Conjugation phases of $U_1$ and $U_2$]
  ...
  \uses{threeMPU2,lemma:sym-trafo-swap,conjphs,
    ...
    def:mpu_admissible_equivalence_datum}
  For admissible comparisons, the MPUs $U_1^{(N)}$ and $U_2^{(N)}$ are in
  different strict phases under
...
\begin{proposition}[Ancilla equivalence of $U_1$ and $U_2$]
  ...
  \uses{threeMPU2,lemma:sym-trafo-swap,
    ...
    def:mpu_reduced_full_support_source_datum,
    def:mpu_admissible_equivalence_datum}
  After adjoining the ancillas, suppose the one-site endpoint tensors, their
  actual two-site blocks, and their physical-adjoint and transposed comparison
  tensors at both blocking lengths are simple and carry their own reduced
  full-support source data. Suppose the displayed comparison paths carry these
  data continuously. After adding one ancilla of dimension $d$ per site,
  $\widetilde U_1^{(N)}$ and $\widetilde U_2^{(N)}$ are admissibly strictly equivalent
