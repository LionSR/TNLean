...
\begin{definition}[Source tensors $u$ and $v$]
    ...
    \begin{align}
        u_{(l,r),(i_1,i_2)}
         & =\sum_\beta (Y_2)_{l,(i_1,\beta)}(Y_1)_{r,(i_2,\beta)},    \\
        v_{(j_1,j_2),(r,l)}
         & =\sum_\alpha (X_1)_{(\alpha,j_1),r}(X_2)_{(\alpha,j_2),l}.
    \end{align}
...
\begin{definition}[Auxiliary mixed source-factor kernels]
    \label{def:mpu_mixed_source_kernels}
    \lean{MPOTensor.SourceFactors.sourceY₁X₂,
        MPOTensor.SourceFactors.sourceX₁Y₂,
        MPOTensor.SourceFactors.sourceY₁X₂_apply,
        MPOTensor.SourceFactors.sourceX₁Y₂_apply,
        MPOTensor.sourceFactors_sourceY₁X₂,
        MPOTensor.sourceFactors_sourceX₁Y₂,
        MPOTensor.sourceY₁X₂,MPOTensor.sourceX₁Y₂,
        MPOTensor.sourceY₁X₂_apply,MPOTensor.sourceX₁Y₂_apply,
        MPOTensor.sourceY₁X₂_conjTranspose_apply,
        MPOTensor.sourceX₁Y₂_conjTranspose_apply}
    \leanok
    \uses{def:mpu_weighted_source_factors}
    Define the auxiliary mixed contractions
    \begin{align}
        a_{(l,r),(i_1,i_2)}
    ...
    \end{align}
    These kernels are not the paper gates $u$ and $v$.
\end{definition}

\begin{theorem}[Auxiliary mixed-kernel two-site factorization]
    \label{thm:mpu_mixed_kernels_two_site_factorization}
    \lean{MPOTensor.trace_mul_eq_sourceY₁X₂_mul_sourceX₁Y₂_swap,
        MPOTensor.mpo_two_pair_entry_eq_sourceY₁X₂_mul_sourceX₁Y₂_swap,
        MPOTensor.mpo_two_reindex_eq_sourceY₁X₂_transpose_mul_sourceX₁Y₂_swap_transpose}
    \leanok
    \uses{def:mpu_mixed_source_kernels, def:mpo_family}
    Identify a two-site physical configuration with an ordered pair in
    $\Fin d\times\Fin d$. Then
    \begin{align}
        \operatorname{reindex}(U^{(2)})
         & =a^{\mathsf T}\bigl(\operatorname{swap}_{r,\ell}b\bigr)^{\mathsf T}.
    \end{align}
    This is an auxiliary algebraic factorization; it is not the staggered
    source-gate circuit of CPSV17 or FBC25.
\end{theorem}
...
\begin{proof}\leanok
    ...
    \begin{align}
        \sum_{l,r}a_{(l,r),(i_1,i_2)}b_{(j_1,j_2),(r,l)}.
    \end{align}
...
\begin{theorem}[Open two-site source factorization]
    \label{thm:mpu_mixed_y1_x2_open_two_site}
    \lean{MPOTensor.SourceFactors.mul_apply_eq_sum_X₁_mul_sourceY₁X₂_mul_Y₂,
        MPOTensor.SourceFactors.blockTwo_apply_eq_sum_X₁_mul_sourceY₁X₂_mul_Y₂,
        MPOTensor.sum_sourceX₁_mul_sourceY₁X₂,
        MPOTensor.mul_apply_eq_sum_sourceX₁_mul_sourceY₁X₂_mul_sourceY₂,
        MPOTensor.mul_apply_eq_sum_sourceX₁_mul_sourceY₁X₂_mul_sourceY₂_pair,
        MPOTensor.blockTwo_apply_eq_sum_sourceX₁_mul_sourceY₁X₂_mul_sourceY₂}
    \leanok
    \uses{def:mpu_mixed_source_kernels, def:mpdo_two_site_blocking,
    ...
    \begin{align}
        ...
        a_{(l,r),q}(Y_2)_{l,(j_2,\gamma)}.
    \end{align}
    The intermediate partial contraction is
    \begin{align}
        \sum_r (X_1)_{(\alpha,j_1),r}a_{(l,r),(q_1,q_2)}
...
\begin{proof}\leanok
    \uses{def:mpu_mixed_source_kernels, thm:mpu_source_factorization_entries}
...
\begin{theorem}[Reflected open two-site source factorization]
    \label{thm:mpu_mixed_x1y2_open_two_site}
    \lean{MPOTensor.SourceFactors.mul_apply_eq_sum_X₂_mul_sourceX₁Y₂_reflected_mul_Y₁,
        MPOTensor.SourceFactors.blockTwo_apply_eq_sum_X₂_mul_sourceX₁Y₂_reflected_mul_Y₁,
        MPOTensor.mul_apply_eq_sum_sourceX₂_mul_sourceX₁Y₂_reflected_mul_sourceY₁,
        MPOTensor.blockTwo_apply_eq_sum_sourceX₂_mul_sourceX₁Y₂_reflected_mul_sourceY₁}
    \leanok
    \uses{def:mpu_mixed_source_kernels, def:mpdo_two_site_blocking,
        thm:mpu_source_factorization_entries}
    ...
    \begin{align}
        ...
        b_{(j_2,j_1),(r,l)}(Y_1)_{r,(i_2,\gamma)}.
    \end{align}
    Thus the physical row of $b$ is $(j_2,j_1)$, while its kernel column is
...
\begin{theorem}[Open periodic-tail source expansion]
    \label{thm:mpu_mixed_y1_x2_open_periodic_tail}
    \lean{MPOTensor.mpo_finAddTwo_eq_sum_sourceX₁_sourceY₁X₂_sourceY₂_evalWord,
        MPOTensor.mpo_finAddTwo_eq_sum_sourceX₁_sourceY₁X₂_sourceX₂_openTail}
    \leanok
    \uses{def:mpo_family, thm:mpu_mixed_y1_x2_open_two_site, thm:mpu_source_factor_recovery}
...
\begin{proof}\leanok
    \uses{thm:mpu_mixed_y1_x2_open_two_site, thm:mpu_source_factor_recovery}
...
\begin{definition}[Auxiliary mixed-kernel open-tail coefficient]
    \label{def:mpu_mixed_y1_x2_open_tail_coefficient}
    \lean{MPOTensor.sourceY₁X₂OpenTailCoefficient}
    \leanok
    \uses{thm:mpu_mixed_y1_x2_open_periodic_tail, def:mpu_weighted_source_factors}
    For
    \begin{align}
        A^{\tau,\zeta}_{i,j_2}
    ...
    \end{align}
    This coefficient comes from the auxiliary $Y_1$--$X_2$ open-tail
    expansion above. The external factor $a_{(l,r),q}$ is unstarred, while
    $a_{(l',r'),p}$ is starred, so the display contains the unique
    normalization factor $d^{-K}$. It is not the coefficient of the paper gate
    $u=Y_2\mathbin{-}Y_1$ in \cite[Section~III]{Cirac2017MPU}.
\end{definition}

\begin{definition}[Auxiliary forward mixed-kernel network]
    \label{def:mpu_mixed_y1_x2_forward_kernel}
    \lean{MPOTensor.sourceY₁X₂ForwardKernel}
    \leanok
    \uses{thm:mpu_mixed_y1_x2_open_periodic_tail}
...
\begin{theorem}[Normalized output-layer tail entry]
    \label{thm:mpu_output_layer_tail_entry}
...
\begin{theorem}[Forward-kernel double-layer packaging]
    \label{thm:mpu_mixed_y1_x2_forward_kernel_packaging}
    \lean{MPOTensor.sourceY₁X₂ForwardKernel_eq_outputLayer_mul_normalizedDiagonal_pow}
    \leanok
    \uses{def:mpu_mixed_y1_x2_forward_kernel,
        thm:mpu_output_layer_tail_entry,
...
\begin{proof}\leanok
    \uses{thm:mpu_output_layer_tail_entry, thm:mpu_output_first_double_layer_entry}
...
\begin{theorem}[Expanded normalized source-$a$ metric]
    \label{thm:mpu_mixed_y1_x2_expanded_metric}
    \lean{MPOTensor.IsMPU.normalized_sourceY₁X₂_openTail_metric}
    \leanok
    \uses{thm:mpu_mixed_y1_x2_open_periodic_tail, thm:mpu_normalized_output_tail_coisometry}
    Assume $d>0$, let $\mathcal U$ be an MPU, and let $\rho$ be positive
    definite.  Expanding both $(K+2)$-site periodic entries through
    $X_1,a,X_2$ and the open tail gives
    \begin{align}
        d^{-K}\sum_{\tau,j,\zeta}
        ...
        =\delta_{p,q},
    \end{align}
    where $\mathcal A_{q;\tau,j,\zeta}$ is the full source-$a$ expansion with
...
\begin{theorem}[Auxiliary closed output-letter formula for mixed-kernel Gram entries]
    \label{thm:mpu_mixed_y1_x2_gram_closed_output_letter}
    \lean{MPOTensor.sourceY₁X₂_gram_apply_eq_closed_output_letter}
    \leanok
    \uses{def:mpu_mixed_source_kernels, def:mpu_double_layer, def:mpu_weighted_source_factors}
    ...
    \begin{align}
        \sum_{l,r}a_{(l,r),q}\overline{a_{(l,r),p}}
    ...
    each doubled bond is ordered as $(\text{ket},\text{bra})$. This is an
    auxiliary mixed-cut identity, not the source-$u$ network in
    \cite[Eq.~\texttt{uUnitary}]{Cirac2017MPU}.
\end{theorem}

\begin{proof}\leanok
    \uses{def:mpu_source_matrix_one, thm:mpu_source_weight_posdef, thm:mpu_source_factor_recovery, thm:mpu_source_x1_range_projection, thm:mpu_output_first_double_layer_entry}
    Expand both source-$a$ entries and contract their common source indices.
...
\begin{theorem}[Auxiliary mixed-kernel Gram entry with the second-cut metric]
    \label{thm:mpu_mixed_y1_x2_gram_second_cut_metric}
    \lean{MPOTensor.sourceY₁X₂_gram_apply_eq_secondCutMetric}
    \leanok
    \uses{thm:mpu_mixed_y1_x2_gram_closed_output_letter, thm:mpu_second_cut_metric}
    ...
    $H_2=Z_2Z_2^\dagger$, the ordinary source-$a$ Gram entry is
    \begin{align}
        \sum_{l,r}a_{(l,r),q}\overline{a_{(l,r),p}}
...
\begin{theorem}[Supplied-witness overlapping reblocking]
    ...
    \leanok
    \uses{def:mpu, def:mpu_double_layer, thm:mpdo_closed_chain_physical_blocking}
...
\begin{proof}\leanok
    \uses{lem:mpu_blocking, thm:mpu_closed_double_layer, thm:mpu_double_layer_blocking, lem:mpu_adjoint_mul}
...
\begin{theorem}[Normalized diagonal under output reflection]
    ...
    \leanok
    \uses{def:mpu_double_layer, def:mpdo_physical_adjoint, def:mpu_reflected_blocked_transfer_coordinates}
...
\begin{theorem}[Reflected transfer-power identities]
    ...
    \leanok
    \uses{def:mpu_double_layer, def:mpdo_physical_adjoint, def:mpdo_physical_blocking, def:mpu_normalized_flattening}
...
\begin{theorem}[Reflected supplied contractions]
    ...
    \leanok
    \uses{def:mpu, def:mpu_double_layer, def:mpdo_physical_adjoint, def:mpdo_physical_blocking, def:mpu_normalized_flattening}
...
\begin{proof}\leanok
    \uses{thm:mpu_matching_direct_block_contractions, thm:mpu_physical_adjoint_identities, thm:mpu_reflected_transfer_power, thm:mpu_supplied_witness_overlapping_reblocking}
...
\begin{remark}[Scope of the closed source-$a$ formulas]
    The first theorem rewrites the ordinary source-$a$ Gram entry, while the
    second independently rewrites the normalized closed output tail. They do
    not identify these two expressions or justify inserting source-cut range
    projections through the intervening chain. No such identification is part
    of the paper-gate proof: $a=Y_1\mathbin{-}X_2$ is not the paper gate
    $u=Y_2\mathbin{-}Y_1$. Neither theorem asserts an ambient
    $X_2X_2^\dagger=\Id$, $M_2M_2^\dagger=\Id$, or
    $Y_2Y_2^\dagger=\Id$ identity.
\end{remark}

\begin{theorem}[Auxiliary mixed-network boundary contraction]
    \label{thm:mpu_mixed_y1_x2_terminal_boundary}
    \lean{MPOTensor.sourceY₁X₂_boundary_sandwich}
    \leanok
    \uses{def:mpu_mixed_y1_x2_open_tail_coefficient, thm:mpu_source_factor_normalizations}
...
\begin{proof}\leanok
    ...
    This is valid source-factor algebra inside the auxiliary mixed network; it
    is not identified with the terminal step of the paper's source-$u$
    isometry argument.
\end{proof}
...
\begin{remark}[Scope of the open-tail algebra]
    ...
    ordinary Gram matrix $a^\dagger a$. No such equality is claimed as part of
    the paper-gate argument. The paper-facing proof must instead begin with
    $u=Y_2\mathbin{-}Y_1$. No result uses $X_2X_2^\dagger=\Id$,
    $M_2M_2^\dagger=\Id$, or
    $Y_2Y_2^\dagger=\Id$.
\end{remark}
...
\begin{theorem}[Paper-$v$ open-leg contraction]
    ...
    \begin{align}
        ...
        =\sum_\gamma U^{p_1z_1}_{\gamma a_1}U^{z_2p_2}_{\gamma a_2}.
    \end{align}
...
\begin{theorem}[Complete reduced source-$v$ contraction]
    ...
    % Documented in docs/paper-gaps/mpu_canonical_form_full_support.tex.
    \uses{def:mpu_simple, def:mpu_source_uv, def:mpu_source_v_dressing, def:mpu_reduced_full_support_source_datum, thm:mpu_supplied_projector_simple2}
...
\begin{proof}
    \uses{thm:mpu_normalized_input_tail_isometry, thm:mpu_normalized_diagonal_power_fixed_pair, thm:mpu_simple_two_letter_diagonal_tail, thm:mpu_source_v_four_u_expansions, thm:mpu_source_v_mul_source_y_tensor_entry, thm:mpu_source_v_dressed_entries}
...
\begin{proof}
    ...
    The implication from unitary $u$ to unitary $v$ still requires the exact
    staggered CPSV17 factorization of $U^{(2)}$ through the paper gates
    $u=Y_2\mathbin{-}Y_1$ and $v=X_1\mathbin{-}X_2$. That source-facing
    factorization is not yet formalized. The checked factorization through the
    auxiliary kernels $a=Y_1\mathbin{-}X_2$ and $b=X_1\mathbin{-}Y_2$ is a
    different identity and is not used here.
...
\begin{proof}\leanok
    \uses{thm:mpu_mixed_y1_x2_open_two_site,
        thm:mpu_mixed_x1y2_open_two_site,
    ...
    blocking with direct four-site blocking. For the right cut, factor the
    two-site block of each tensor through the auxiliary mixed kernel
    $a=Y_1\mathbin{-}X_2$. The diagonal cut through the two blocked factors
    retains one physical leg and one right source leg from each factor. Thus, for
    ...
    \end{align}
    For the left cut, use the reflected auxiliary mixed kernel
    $b=X_1\mathbin{-}Y_2$, with physical order $(j_2,j_1)$ and
    kernel-column order $(r,\ell)$. For suitable rectangular matrices $A_{2,4}$ and $B_{2,4}$,
    \begin{align}
...
\begin{definition}[Trace-normalized shift weight]
    ...
    trace one. For the scalar identity choice on the bond space of a single
    shift, this fixes
    \begin{align}
        \rho_{\mathrm{paper}}=W_d & =d^{-1}\Id_d
    \end{align}
    whenever $d>0$. The same formula is defined at $d=0$, although it then has
    trace zero. The tensors $U_2$ and $U_3$ have bond dimension $d^2$, so their
    corresponding scalar identity weight is
    \begin{align}
        W_{d^2} & =(d^2)^{-1}\Id_{d^2}.
    \end{align}
    This is the scalar trace-one specialization of the source weight used in
    \cite[equation labelled~\texttt{Y1Y1X1X1}]{Cirac2017MPU}.

    The supplied auxiliary mixed-kernel witnesses below instead use
    $\rho_{\mathrm{aux}}=\Id_d$ for each constituent shift and
    $\rho_{\mathrm{aux}}=\Id_{d^2}$ after tensoring. They compute the
    $Y_1$--$X_2$ and $X_1$--$Y_2$ contractions, not the paper gates. For
    $d>1$, these identity weights differ from $W_d$ and $W_{d^2}$.
...
\begin{theorem}[Positivity and normalization of the shift weight]
    ...
    For every $d\in\mathbb N$, the matrix $W_d$ is positive semidefinite. If
    $d>0$, then $W_d$ is positive definite and
    \begin{align}
        \tr(W_d) & =1.
    \end{align}
    Under the same hypothesis, $W_{d^2}$ is positive definite with trace one;
...
\begin{definition}[Supplied source factors for the three shift MPUs]
    ...
    \lean{MPOTensor.identitySourceFactors,
        ...
        MPOTensor.SourceFactors.sourceY₁X₂_independentTensorProductOfIdentityWeight_apply,
        MPOTensor.SourceFactors.sourceX₁Y₂_independentTensorProductOfIdentityWeight_apply,
        MPOTensor.sourceY₁X₂_identitySourceFactors_apply,
        MPOTensor.sourceX₁Y₂_identitySourceFactors_apply,
        MPOTensor.sourceY₁X₂_rightShiftSourceFactors_apply,
        MPOTensor.sourceX₁Y₂_rightShiftSourceFactors_apply,
        MPOTensor.sourceY₁X₂_leftShiftSourceFactors_apply,
        MPOTensor.sourceX₁Y₂_leftShiftSourceFactors_apply,
...
\begin{theorem}[One-site supplied mixed kernels for $U_1$ and $U_3$]
    ...
    \lean{MPOTensor.shiftExampleU₁SourceY₁X₂RowEquiv,
        MPOTensor.shiftExampleU₁SourceX₁Y₂ColumnEquiv,
        MPOTensor.shiftExampleU₃SourceY₁X₂SwapShuffle,
        MPOTensor.shiftExampleU₃SwapSourceY₁X₂RowEquiv,
        MPOTensor.shiftExampleU₃SwapSourceX₁Y₂ColumnEquiv,
        MPOTensor.shiftExampleU₃SwapSourceY₁X₂RowEquiv_apply,
        MPOTensor.shiftExampleU₃SwapSourceX₁Y₂ColumnEquiv_apply,
        MPOTensor.shiftExampleU₁_sourceY₁X₂_apply,
        MPOTensor.shiftExampleU₁_sourceX₁Y₂_apply,
        MPOTensor.shiftExampleU₁_sourceY₁X₂_sourceX₁Y₂_eq_identityTensorIdentity,
        MPOTensor.shiftExampleU₃_sourceY₁X₂_eq_swap_apply,
        MPOTensor.shiftExampleU₃_sourceX₁Y₂_eq_swap_apply,
        MPOTensor.shiftExampleU₃_sourceY₁X₂_sourceX₁Y₂_eq_swap}
    \leanok
    \uses{def:threeMPU_supplied_source_factors, def:mpu_mixed_source_kernels}
    Assume $d>0$. In the source coordinates of the supplied factorizations,
    the auxiliary kernels satisfy
    \begin{align}
        a_1 & =\Id\otimes\Id, & b_1 & =\Id\otimes\Id, \\
        a_3 & =\mathbb S,     & b_3 & =\mathbb S.
    \end{align}
    These are auxiliary computations near
    \cite[lines~2009--2016]{Cirac2017MPU}; they do not identify $a,b$ with the
    paper gates $u,v$. The $U_3$ equalities use the source-coordinate
    presentation and are distinct from the blocked four-spin presentation
    below.
\end{theorem}
...
\begin{proof}\leanok
    ...
    \begin{align}
        [a_1]_{((a,b),(c,e)),((i,j),(k,l))}
         & =[b_1]_{((i,j),(k,l)),((a,b),(c,e))}                  \\
...
\begin{theorem}[Auxiliary mixed kernels in the two-site factorizations of $U_2$]
    \label{thm:threeMPU_U2_supplied_mixed_kernels_two}
    \lean{MPOTensor.shiftExampleU₂SourceY₁X₂RowEquiv,
        MPOTensor.shiftExampleU₂SourceX₁Y₂ColumnEquiv,
        MPOTensor.shiftExampleU₂_sourceY₁X₂_fourSpin_apply,
        MPOTensor.shiftExampleU₂_sourceY₁X₂_reindex_eq_identitySwapIdentity,
        MPOTensor.shiftExampleU₂_sourceX₁Y₂_fourSpin_apply,
        MPOTensor.shiftExampleU₂_sourceX₁Y₂_reindex_eq_swapSwap_mul_identitySwapIdentity,
        MPOTensor.shiftExampleU₂_blockTwo_apply_eq_sum_X₁_mul_sourceY₁X₂_mul_Y₂,
        MPOTensor.shiftExampleU₂_blockTwo_apply_eq_sum_X₂_mul_sourceX₁Y₂_reflected_mul_Y₁}
    \leanok
    \uses{def:threeMPU_supplied_source_factors, def:mpu_mixed_source_kernels,
        thm:mpu_mixed_y1_x2_open_two_site, thm:mpu_mixed_x1y2_open_two_site}
    Assume $d>0$. The two auxiliary open-leg factorizations of the blocked
    tensor have, as their respective mixed kernels,
    \begin{align}
        a_2^{(2)} & =\Id\otimes\mathbb S\otimes\Id, \\
        b_2^{(2)} & =(\mathbb S\otimes\mathbb S)
    ...
    These are auxiliary computations near
    \cite[lines~2018--2026]{Cirac2017MPU}, not identifications of the paper
    gates $u$ and $v$.
\end{theorem}
...
\begin{theorem}[Auxiliary mixed kernels in the two-site factorizations of $U_3$]
    \label{thm:threeMPU_U3_supplied_mixed_kernels_two}
    \lean{MPOTensor.shiftExampleU₃SourceY₁X₂RowEquiv,
        MPOTensor.shiftExampleU₃SourceX₁Y₂ColumnEquiv,
        MPOTensor.shiftExampleU₃_sourceY₁X₂_fourSpin_apply,
        MPOTensor.shiftExampleU₃_sourceY₁X₂_reindex_eq_identitySwapIdentity_mul_swapSwap,
        MPOTensor.shiftExampleU₃_sourceX₁Y₂_fourSpin_apply,
        MPOTensor.shiftExampleU₃_sourceX₁Y₂_reindex_eq_identitySwapIdentity,
        MPOTensor.shiftExampleU₃_blockTwo_apply_eq_sum_X₁_mul_sourceY₁X₂_mul_Y₂,
        MPOTensor.shiftExampleU₃_blockTwo_apply_eq_sum_X₂_mul_sourceX₁Y₂_reflected_mul_Y₁}
    \leanok
    \uses{def:threeMPU_supplied_source_factors, def:mpu_mixed_source_kernels,
    ...
    tensor have, as their respective mixed kernels,
    \begin{align}
        a_3^{(2)} & =(\Id\otimes\mathbb S\otimes\Id)
        (\mathbb S\otimes\mathbb S),                 \\
        b_3^{(2)} & =\Id\otimes\mathbb S\otimes\Id.
    \end{align}
    ...
    These are auxiliary computations near
    \cite[lines~2028--2034]{Cirac2017MPU}, not identifications of the paper
    gates $u$ and $v$.
\end{theorem}
...
\begin{proof}\leanok
    ...
    corresponding identifications $\pi,\iota_a,\iota_b$, the exact equalities
    are
    \begin{align}
...
\begin{proposition}[Standard forms of the three shift MPUs]
    ...
    $v=X_1\mathbin{-}X_2$. The preceding auxiliary mixed-kernel computations
    concern $a=Y_1\mathbin{-}X_2$ and $b=X_1\mathbin{-}Y_2$ instead and do not
    establish this proposition. A source-faithful paper-gate computation
    remains.
