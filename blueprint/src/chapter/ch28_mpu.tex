...
$\mathcal B_1$ and $\mathcal B_2$, whereas the statements below take
$\mathcal B_1=M_I(\mathbb C)$ and $\mathcal B_2=M_J(\mathbb C)$.
The word ``subalgebra'' in Lemma~7 is used in the $C^*$-algebra sense, so
$\mathcal A$ is closed under the involution; this convention is explicit in
the precursor \cite[Section~4.3]{SchumacherWerner2004QCA}.  Thus the
star-subalgebra hypothesis below is not an additional restriction.  The
full-matrix restriction holds for the MPU spin chain because every one-site
algebra is isomorphic to $M_d(\mathbb C)$
\cite[lines~2320--2334]{Cirac2017MPU}.  The general-factor GNVW theorem is not
...
\begin{theorem}[Tensor-submodule criterion for left support]
    ...
    \leanok
    \uses{def:qca_left_support_algebra}
    Let $I=\{0,\ldots,p-1\}$ and $J=\{0,\ldots,q-1\}$.  For every
...
\begin{theorem}[The one-sided tensor submodule as a Kronecker span]
    ...
    \leanok
    \uses{def:qca_kronecker_submodule}
    Let $I=\{0,\ldots,p-1\}$ and $J=\{0,\ldots,q-1\}$.  For every submodule
