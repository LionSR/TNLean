...
\begin{theorem}[Counterexample to the unrestricted source-$v$ Gram bridge]
    \label{thm:mpu_source_v_gram_bridge_counterexample}
    \lean{MPOTensor.SourceVCounterexample.sourceYTensor_gram_ne_inserted,
        MPOTensor.SourceVCounterexample.bondMatrix,
    ...
    \leanok
    \uses{def:mpu,def:mpu_simple,def:mpu_source_v_dressing,
    ...
    \end{align}
    Writing $W$ for the unique double-layer letter, these witnesses satisfy
    \begin{align}
    ...
    counterexample establishes only that the unrestricted bridge is false; it
    does not assert a corrected Gram equality under those hypotheses.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpu_source_v_four_u_expansions,
        thm:mpu_reduced_cfii_transfer_stabilization}
    Put $A=U^{00}$. Since $A^2=A$ and $\tr(A)=1$, every periodic MPO is the
    one-dimensional identity, so $U$ is an MPU. Expanding both sides of the
    supplied rank-one contraction proves $W^2=W\lvert b)(\Phi\rvert W$;
