...
\begin{proof}\leanok
    \uses{lem:mpu_iterated_physical_blocking,
        thm:mpu_normalized_transfer_stabilization,
        thm:mpu_normalized_transfer_fin_one, thm:mpu_second_block_simple,
        def:mpu_physical_reindexing}
...
\begin{theorem}[Open periodic-tail source expansion]
    ...
    \leanok
    \uses{def:mpo_family, thm:mpu_source_u_open_two_site, thm:mpu_source_factor_recovery}
...
\begin{proof}\leanok
    \uses{thm:mpu_source_u_open_two_site, thm:mpu_source_factor_recovery}
...
\begin{definition}[Normalized open-tail coefficient]
    ...
    \leanok
    \uses{thm:mpu_source_u_open_periodic_tail, def:mpu_weighted_source_factors}
    For
    \begin{align}
        A^{\tau,\zeta}_{i,j_2}
...
\begin{theorem}[Forward-kernel double-layer packaging]
    ...
    \leanok
    \uses{def:mpu_source_u_forward_kernel, thm:mpu_source_u_output_layer_tail_entry, thm:mpu_output_first_double_layer_entry}
...
\begin{proof}\leanok
    \uses{thm:mpu_source_u_output_layer_tail_entry, thm:mpu_output_first_double_layer_entry}
...
\begin{theorem}[Expanded normalized source-$u$ metric]
    ...
    \leanok
    \uses{thm:mpu_source_u_open_periodic_tail, thm:mpu_normalized_output_tail_coisometry}
...
\begin{theorem}[Closed output-letter formula for source-$u$ Gram entries]
    ...
    \leanok
    \uses{def:mpu_source_uv, def:mpu_double_layer, def:mpu_weighted_source_factors}
...
\begin{theorem}[Source-$u$ Gram entry with the second-cut metric]
    ...
    \leanok
    \uses{thm:mpu_source_u_gram_closed_output_letter, thm:mpu_source_u_second_cut_metric}
...
\begin{theorem}[Terminal source-factor boundary contraction]
    ...
    \leanok
    \uses{def:mpu_source_u_open_tail_coefficient, thm:mpu_source_factor_normalizations}
...
\begin{theorem}[One-sided contraction of the dressed source operator]
    ...
    \leanok
    \discussion{6071}
    \uses{def:mpu_source_v_dressing, def:mpu_source_uv, thm:mpu_source_factorization_entries}
...
\begin{proof}\leanok
    \uses{thm:mpu_transfer_matrix_power_trace, thm:overlap_trace, thm:mixed_self}
...
\begin{proof}\leanok
    \uses{lem:transfer_map_corner_intertwine, thm:mpu_dependent_block_inclusion_identities, thm:mpu_shifted_trace_power_moments, thm:mpu_shifted_trace_power_spectrum, thm:mpu_trace_power_all, thm:mpu_transfer_matrix_power_trace, thm:mpu_transfer_matrix_eigenvalues}
...
\begin{proof}\leanok
    \uses{thm:mpu_normalized_transfer_trace, thm:mpu_shifted_transfer_trace_single_block, thm:mpu_transfer_eigenvalue_one, thm:mpu_ambient_block_isometry_intertwining, thm:mpu_unique_full_support_geometry, thm:matrix_posdef_coisometric_transport, lem:transfer_map_corner_intertwine, thm:transfer_matrix_trace_preserving_identity_left_fixed, thm:mpu_normalized_transfer_stabilization, thm:mpu_stabilized_fixed_pair_uniqueness}
...
\begin{theorem}[Counterexample to the unrestricted source-$v$ Gram identification]
    ...
    \lean{MPOTensor.SourceVCounterexample.sourceYTensor_gram_ne_inserted,
        ...
        MPOTensor.SourceVCounterexample.not_hasFullSupport}
    \leanok
    \uses{def:mpu_source_v_gram_counterexample_data, def:mpu, def:mpu_simple, def:mpu_double_layer, def:mpu_source_v_dressing, def:mpu_normalized_flattening, def:transfer_map, def:cpsv_cfii, def:mpu_full_support}
    ...
    support. Thus this example is outside the reduced, full-support
...
\begin{proof}\leanok
    \uses{thm:mpu_supplied_witness_overlapping_reblocking, thm:mpu_source_v_four_u_expansions, thm:mpu_reduced_cfii_transfer_stabilization}
    ...
    full-support canonical-form-II data, which excludes this example.
\end{proof}
...
\begin{proof}
    \uses{thm:mpu_reflected_transfer_power, thm:mpu_matching_direct_block_contractions, thm:mpu_supplied_witness_overlapping_reblocking, thm:mpu_reflected_supplied_contractions, thm:mpu_source_x1_range_projection, thm:mpu_source_x2_range_projection, thm:mpu_source_u_gram_second_cut_metric, thm:mpu_source_u_terminal_boundary, thm:mpu_normalized_output_tail_closed_trace}
...
\begin{proof}
    \uses{lem:mpu_admissible_source_u_isometry, thm:mpu_supplied_projector_simple2, thm:mpu_supplied_witness_overlapping_reblocking, thm:mpu_source_v_complete_reduced_contraction, thm:mpu_source_uv_two_site_factorization, thm:mpu_source_v_dressed_gram_cancel}
...
\begin{lemma}[Four-site composition source-rank bounds]
    ...
    \leanok
    \uses{def:mpdo_operator_product, def:mpdo_physical_blocking,
...
\begin{proof}\leanok
    \uses{thm:mpu_source_u_open_two_site,
        thm:mpu_source_v_open_two_site, thm:mpdo_blocking_product,
    ...
    two-site block of each tensor through its source tensor $u$. The diagonal
    cut through the two blocked factors retains one physical leg and one right
    ...
    \end{align}
    For the left cut, use the reflected $v$ factorization, with its physical
...
\begin{lemma}[Composition additivity]
    ...
    \uses{SF, def:index, index-well-defined,
        lem:mpu_four_site_composition_source_rank_bounds}
...
\section{Bipartite support algebras}

\begin{definition}[Tensor-product subspace of matrix spaces]
    \label{def:qca_kronecker_submodule}
    \lean{Matrix.kroneckerSubmodule}
    \leanok
    Let $\mathcal S\subseteq M_m(\mathbb C)$ and
    $\mathcal T\subseteq M_n(\mathbb C)$ be linear subspaces.  Their
    tensor-product subspace inside $M_{mn}(\mathbb C)$ is
    \begin{align}
        \mathcal S\otimes_{\mathrm{lin}}\mathcal T
        &:=\operatorname{span}\{L\otimes R:
             L\in\mathcal S,\ R\in\mathcal T\}.
    \end{align}
    % Source: References/0910.3675v2/QCI12.tex, equation a2Supp,
    % lines 1211--1219.
\end{definition}

\begin{definition}[Left and right support algebras]
    \label{def:qca_bipartite_support_algebras}
    \lean{StarSubalgebra.leftCoefficientSet,
        StarSubalgebra.rightCoefficientSet,
        StarSubalgebra.leftSupportAlgebra,
        StarSubalgebra.rightSupportAlgebra}
    \leanok
    Let $\mathcal A\subseteq M_m(\mathbb C)\otimes M_n(\mathbb C)$ be a
    star-subalgebra.  For $X\in\mathcal A$, define its left and right
    coefficient matrices by
    \begin{align}
        X^{\mathrm L}_{ij}(a,b)
          &:=X((a,i),(b,j)),                                      \\
        X^{\mathrm R}_{ab}(i,j)
          &:=X((a,i),(b,j)).
    \end{align}
    The left support algebra $\operatorname{Spp}_{\mathrm L}(\mathcal A)$
    is the star-algebra generated by all $X^{\mathrm L}_{ij}$, and the right
    support algebra $\operatorname{Spp}_{\mathrm R}(\mathcal A)$ is generated
    by all $X^{\mathrm R}_{ab}$.  These are the full-matrix coefficient
    construction of the support-algebra lemma of \cite{Gross2012IndexTheory} and
    its factor-swapped form.  The earlier coefficient-space construction is
    given by \cite[equations~\texttt{a2supp} and~\texttt{a2Supp}]
    {SchumacherWerner2004QCA}.
    % Source: References/0910.3675v2/QCI12.tex, Lemma lem:spp,
    % lines 1181--1191, and equation a2Supp, lines 1211--1219.
    % Precursor: Papers/quant-ph_0405174/qca.tex, equations a2supp and a2Supp,
    % lines 1137--1169.
\end{definition}

\begin{lemma}[Matrix-unit coefficient expansions]
    \label{lem:qca_bipartite_coefficient_expansions}
    \lean{Matrix.eq_sum_bipartiteSlice_kronecker_single,
        Matrix.eq_sum_single_kronecker_operatorBlock}
    \leanok
    \uses{def:qca_bipartite_support_algebras}
    For every $X\in M_m(\mathbb C)\otimes M_n(\mathbb C)$,
    \begin{align}
        X&=\sum_{i,j=1}^{n}X^{\mathrm L}_{ij}\otimes E_{ij}
          =\sum_{a,b=1}^{m}E_{ab}\otimes X^{\mathrm R}_{ab}.
    \end{align}
    % Source: References/0910.3675v2/QCI12.tex, Lemma lem:spp,
    % lines 1188--1191, and equation a2Supp, lines 1211--1219.
\end{lemma}

\begin{proof}\leanok
    Compare the $((a,i),(b,j))$ matrix entry.  In either sum, exactly one
    matrix unit contributes, and its contribution is $X((a,i),(b,j))$.
\end{proof}

\begin{theorem}[Leastness of the support algebras]
    \label{thm:qca_bipartite_support_algebra_least}
    \lean{StarSubalgebra.bipartiteSlice_mem_leftSupportAlgebra,
        StarSubalgebra.operatorBlock_mem_rightSupportAlgebra,
        StarSubalgebra.leftSupportAlgebra_le_iff,
        StarSubalgebra.rightSupportAlgebra_le_iff,
        StarSubalgebra.leftSupportAlgebra_le_iff_tensorSubmodule,
        StarSubalgebra.rightSupportAlgebra_le_iff_tensorSubmodule}
    \leanok
    \uses{def:qca_bipartite_support_algebras,
        lem:qca_bipartite_coefficient_expansions}
    If $\mathcal C\subseteq M_m(\mathbb C)$ is a star-subalgebra, then
    \begin{align}
        \operatorname{Spp}_{\mathrm L}(\mathcal A)\subseteq\mathcal C
        \quad\Longleftrightarrow\quad
        X^{\mathrm L}_{ij}\in\mathcal C
        \quad\text{for all }X\in\mathcal A\text{ and all }i,j.
    \end{align}
    Equivalently,
    \begin{align}
        \operatorname{Spp}_{\mathrm L}(\mathcal A)\subseteq\mathcal C
        \quad\Longleftrightarrow\quad
        \mathcal A\subseteq
        \mathcal C\otimes_{\mathrm{lin}}M_n(\mathbb C).
    \end{align}
    The analogous statements hold for the right support algebra after
    interchanging the two tensor factors.
    % Source: References/0910.3675v2/QCI12.tex, Lemma lem:spp,
    % lines 1181--1207, and equation a2Supp, lines 1211--1219.
\end{theorem}

\begin{proof}\leanok
    The support algebra is generated by precisely the displayed coefficient
    matrices.  The first equivalence is therefore the universal property of
    the generated star-algebra.  The second follows from the first
    matrix-unit expansion in
    Lemma~\ref{lem:qca_bipartite_coefficient_expansions}; the right-factor
    statement follows from the second expansion.
\end{proof}

\begin{theorem}[Two-sided support-algebra containment]
    \label{thm:qca_bipartite_two_sided_support_containment}
    \lean{StarSubalgebra.mem_kroneckerSubmodule_supportAlgebras,
        StarSubalgebra.le_kroneckerSubmodule_supportAlgebras}
    \leanok
    \uses{def:qca_kronecker_submodule,
        def:qca_bipartite_support_algebras,
        thm:qca_bipartite_support_algebra_least}
    The algebra $\mathcal A$ is contained in the tensor-product subspace of
    its two support algebras:
    \begin{align}
        \mathcal A
        &\subseteq
    ...
    \end{align}
    This is equation \texttt{a2Supp} of
    \cite{Gross2012IndexTheory} in the full complex matrix setting.
    % Source: References/0910.3675v2/QCI12.tex, equation a2Supp,
    % lines 1211--1219.
\end{theorem}

\begin{proof}\leanok
    Choose a basis $(L_k)_k$ of the left support algebra and extend its
    coordinate functionals to $M_m(\mathbb C)$.  For $X\in\mathcal A$, set
    \begin{align}
        R_k(i,j)&:=\varphi_k(X^{\mathrm L}_{ij}).
    \end{align}
    Expanding each $X^{\mathrm L}_{ij}$ in matrix units shows that $R_k$ is a
    linear combination of the right coefficient matrices
    $X^{\mathrm R}_{ab}$, so $R_k$ belongs to the right support algebra.
    Expanding each left coefficient in the basis $(L_k)_k$ then gives
    \begin{align}
        X&=\sum_k L_k\otimes R_k,
    \end{align}
    which is the required membership in the tensor-product subspace.
\end{proof}

\begin{theorem}[Commutants of the support algebras]
    \label{thm:qca_bipartite_support_algebra_commutants}
    \lean{StarSubalgebra.centralizer_leftSupportAlgebra,
        StarSubalgebra.centralizer_rightSupportAlgebra}
    \leanok
    \uses{def:qca_bipartite_support_algebras,
        lem:qca_bipartite_coefficient_expansions}
    The commutants of the two support algebras are
    \begin{align}
        \operatorname{Spp}_{\mathrm L}(\mathcal A)'
          &=\{B\in M_m(\mathbb C):B\otimes\mathbf 1_n\in\mathcal A'\}, \\
        \operatorname{Spp}_{\mathrm R}(\mathcal A)'
          &=\{C\in M_n(\mathbb C):\mathbf 1_m\otimes C\in\mathcal A'\}.
    \end{align}
    % Source: References/0910.3675v2/QCI12.tex, Lemma lem:spp,
    % lines 1192--1208, and equation a2Supp, lines 1211--1219.
\end{theorem}

\begin{proof}\leanok
    By the matrix-unit expansions, commutation of an identity extension with
    $X\in\mathcal A$ is equivalent to commutation with every coefficient
    matrix of $X$.  Thus a matrix commuting with the generated support algebra
    has the required identity extension.  Conversely, coefficientwise
    commutation, together with the same statement for the adjoint, propagates
    from the generators to the support algebra.  The second identity follows
    after interchanging the tensor factors.
\end{proof}
...
\begin{proof}\leanok
    \uses{def:qca_two_sided_finite_propagation, lem:qca_propagation_neighborhood_mono, lem:qca_finite_propagation_symmetric_interval}
...
\begin{theorem}[Forward and two-sided finite propagation]
    ...
    \leanok
    \uses{def:qca_finite_propagation, def:qca_two_sided_finite_propagation}
    Let $d>0$ and let $\omega$ be a star-algebra automorphism of $\mathcal A$.
    Then
    \begin{align}
...
\begin{proof}\leanok
    \uses{lem:qca_two_sided_common_neighborhood, thm:qca_inverse_propagation}
