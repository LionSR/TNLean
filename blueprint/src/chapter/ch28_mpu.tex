...
\begin{theorem}[Reduced representative under a supplied positive fixed pair]
  ...
  \end{align}
  All positivity, support, absorption, transfer, and normalization assumptions
  in this statement are supplied explicitly; the source derives them by
  blocking to $D^4$ sites and passing through the isometry of the
  Proposition~IV.5 argument, and this restriction is recorded in
  \path{docs/paper-gaps/mpu_reduced_representative_supplied_fixed_pair.tex}.
  % Source lines: paper_v2.tex 747--783.
\end{theorem}

\begin{proof}\leanok
  For every nonempty word $w$, idempotence of $T$ and the identities
  $TQ=PQ$, $PT=T$ give
  \begin{align}
    \widetilde W_w&=TW_wT.
  \end{align}
  ...
  Expanding the transfer map yields
  \begin{align}
    E_{\widetilde W}(X)
      &=T E_W(TXT)T
  ...
  \end{align}
  Support absorption and trace cyclicity preserve the pairing normalization.
  Finally, $\operatorname{ran}T\subseteq\operatorname{ran}P$, so the left
  compression is positive definite. For the right compression, transversality
  gives $\ker Q\cap\operatorname{ran}T=\{0\}$; hence every nonzero vector in
  the isometric coordinates has a nonzero component in the support of $R$.
\end{proof}
