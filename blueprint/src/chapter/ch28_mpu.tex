...
\begin{theorem}[Open two-site source factorization]
    \label{thm:mpu_mixed_y1x2_open_two_site}
...
\begin{theorem}[Open periodic-tail source expansion]
    \label{thm:mpu_mixed_y1x2_open_periodic_tail}
    ...
    \leanok
    \uses{def:mpo_family, thm:mpu_mixed_y1x2_open_two_site, thm:mpu_source_factor_recovery}
...
\begin{proof}\leanok
    \uses{thm:mpu_mixed_y1x2_open_two_site, thm:mpu_source_factor_recovery}
...
\begin{definition}[Auxiliary mixed-kernel open-tail coefficient]
    \label{def:mpu_mixed_y1x2_open_tail_coefficient}
    \lean{MPOTensor.sourceY₁X₂OpenTailCoefficient}
    \leanok
    \uses{thm:mpu_mixed_y1x2_open_periodic_tail, def:mpu_weighted_source_factors}
    For
    \begin{align}
        A^{\tau,\zeta}_{i,j_2}
...
\begin{definition}[Auxiliary forward mixed-kernel network]
    \label{def:mpu_mixed_y1x2_forward_kernel}
    \lean{MPOTensor.sourceY₁X₂ForwardKernel}
    \leanok
    \uses{thm:mpu_mixed_y1x2_open_periodic_tail}
...
\begin{theorem}[Forward-kernel double-layer packaging]
    \label{thm:mpu_mixed_y1x2_forward_kernel_packaging}
    \lean{MPOTensor.sourceY₁X₂ForwardKernel_eq_outputLayer_mul_normalizedDiagonal_pow}
    \leanok
    \uses{def:mpu_mixed_y1x2_forward_kernel,
...
\begin{theorem}[Expanded normalized source-$a$ metric]
    \label{thm:mpu_mixed_y1x2_expanded_metric}
    \lean{MPOTensor.IsMPU.normalized_sourceY₁X₂_openTail_metric}
    \leanok
    \uses{thm:mpu_mixed_y1x2_open_periodic_tail, thm:mpu_normalized_output_tail_coisometry}
...
\begin{theorem}[Auxiliary closed output-letter formula for mixed-kernel Gram entries]
    \label{thm:mpu_mixed_y1x2_gram_closed_output_letter}
...
\begin{theorem}[Auxiliary mixed-kernel Gram entry with the second-cut metric]
    \label{thm:mpu_mixed_y1x2_gram_second_cut_metric}
    \lean{MPOTensor.sourceY₁X₂_gram_apply_eq_secondCutMetric}
    \leanok
    \uses{thm:mpu_mixed_y1x2_gram_closed_output_letter, thm:mpu_second_cut_metric}
...
\begin{theorem}[Auxiliary mixed-network boundary contraction]
    \label{thm:mpu_mixed_y1x2_terminal_boundary}
    \lean{MPOTensor.sourceY₁X₂_boundary_sandwich}
    \leanok
    \uses{def:mpu_mixed_y1x2_open_tail_coefficient, thm:mpu_source_factor_normalizations}
...
\begin{proof}\leanok
    \uses{thm:mpu_mixed_y1x2_open_two_site,
...
\begin{theorem}[Auxiliary mixed kernels in the two-site factorizations of $U_2$]
    ...
    \leanok
    \uses{def:threeMPU_supplied_source_factors, def:mpu_mixed_source_kernels,
        thm:mpu_mixed_y1x2_open_two_site, thm:mpu_mixed_x1y2_open_two_site}
