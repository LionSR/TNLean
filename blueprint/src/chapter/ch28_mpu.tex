...
\begin{definition}[Product-index source weight]
    ...
    identified without this reindexing.
\end{definition}
...
\begin{definition}[Reduced full-support source datum]
  ...
  physical dimension $d>0$ and bond dimension $D>1$ consists of chosen
  canonical-form-II data for the normalized flattening of this same tensor,
  with its active blocks filling the ambient bond space. It includes the
  normalized fixed pair $\rho>0$ and $(\Phi\rvert$ supplied by
  Theorem~\ref{thm:mpu_reduced_cfii_transfer_stabilization}, so that
  \begin{align}
    \tr(\rho)&=1,&
    E^{D^2-1}&=\lvert\rho)(\Phi\rvert.
  \end{align}
  This definition does not assert that an arbitrary representative admits
  such data and does not replace $\mathcal U$ by another tensor.
\end{definition}
...
\begin{theorem}[Complete reduced source-$v$ contraction]
  ...
  source datum. Let $\rho$ and $(\Phi\rvert$ be its normalized fixed pair, so
  that
  \begin{align}
    E^{D^2-1}&=\lvert\rho)(\Phi\rvert.
  \end{align}
...
\begin{lemma}[Tensor-product additivity]
  ...
  \uses{def:index,index-well-defined,thm:mpu_tensor_product_source_ranks,
    def:mpu_reduced_full_support_source_datum}
  Suppose the simple blocks used to define the indices of $\mathcal U$,
  $\mathcal V$, and $\mathcal U\otimes\mathcal V$ each carry reduced
  full-support source data. Then
  \begin{align}
...
\begin{lemma}[Composition additivity]
  ...
  \uses{SF,def:index,index-well-defined,
    ...
    def:mpu_reduced_full_support_source_datum}
  Suppose the simple blocks used to define the indices of $\mathcal U$,
  $\mathcal U'$, and their composition tensor $\mathcal W$ each carry
  reduced full-support source data. If $\mathcal W$ generates the composition
  of the MPUs generated by $\mathcal U$ and $\mathcal U'$, then
  \begin{align}
...
\begin{definition}[Admissible equivalence and path datum]
  ...
  length $m$ and, for every $p$, a simple canonical-form tensor representing
  $\mathcal W(p)_m$, equipped with a reduced full-support source datum. The
  simple tensors and their source data are required to vary continuously with
  $p$, after fixed identifications of the finite-dimensional spaces involved.
  In particular, the blocked endpoint tensors carry their own source data.
...
\begin{proposition}[Constancy of the source ranks]
  ...
  Suppose that the path carries an admissible equivalence datum. Then the
  ranks $r(x)$ and $\ell(x)$ of the corresponding simple blocked tensors are
  constant on $[0,1]$.
\end{proposition}
...
\begin{corollary}[Continuous choice of standard form]
  ...
  \uses{prop:continuity-index,IndexTh,SF,ThmFund1,
    def:mpu_reduced_full_support_source_datum,
    def:mpu_admissible_equivalence_datum}
  For every continuous path $\mathcal U(p)$ of MPU tensors carrying an
  admissible equivalence datum, there is
  an integer $k_0\leq D^4$ and continuous paths
...
\begin{proposition}[Local characterization of time reversal]
  ...
  \uses{FundamentalMPU,IndexTh,SF,
    def:mpu_reduced_full_support_source_datum}
  Let an MPU of index zero be in standard form. Suppose its two-site blocked
  tensor and the adjoint comparison tensor carry reduced full-support source
  data, and write its one-site
...
\begin{proposition}[Local characterization of transposition]
  ...
  \uses{FundamentalMPU,def:index,SF,
    def:mpu_reduced_full_support_source_datum}
  Let an MPU of index zero be in standard form. Suppose its two-site blocked
  tensor and the transposed comparison tensor carry reduced full-support
  source data, and write its one-site
  physical space as $\mathbb C^{d_0}\otimes\mathbb C^{d_0}$. It satisfies
...
\begin{definition}[Strict equivalence under a symmetry]
  ...
  witnessing path carries an admissible equivalence datum.
\end{definition}
...
\begin{lemma}[Standard-form path criterion]
  ...
  and whose tensor path carries an admissible equivalence datum.
\end{lemma}
...
\begin{lemma}[Trace formula for the time-reversal sign]
  ...
  For a time-reversal-invariant MPU in admissible standard form, whose
  two-site blocked comparison carries reduced full-support source data, the
  sign $\sigma$ in
...
\begin{proposition}[Well-definedness of the time-reversal sign]
  ...
  \uses{eq:sigma-from-trace,lem:equivalent-symmetry,
    def:mpu_admissible_equivalence_datum}
  The sign $\sigma=\pm1$ is independent of the admissible standard-form gauge
  and is constant under admissible continuous deformations preserving time
  reversal.
\end{proposition}
...
\begin{proposition}[Blocking parity of the time-reversal sign]
  ...
  \uses{prop:sigma-well-defined,PropChiral,lem:mpu_blocking,
    def:mpu_reduced_full_support_source_datum}
  Suppose every blocked tensor under consideration carries the required
  reduced full-support source data. Let $\sigma^{(k)}$ be the sign of the
  admissible standard form obtained after blocking $k$ sites. Then
  \begin{align}
    \sigma^{(k)}
      &=\begin{cases}
        \sigma^{(1)},&k\text{ odd},\\
...
\begin{corollary}[Necessary time-reversal invariants]
  ...
  \uses{prop:sigma1-and-sigma2-equal,def:strictly-equivalent-symmetry,
    def:equivalent-symmetry,def:mpu_admissible_equivalence_datum}
  Admissible strict equivalence of two time-reversal-invariant MPUs requires
  equality of both $\sigma^{(1)}$ and $\sigma^{(2)}$. Admissible equivalence
  after blocking and adding ancillas requires equality of $\sigma^{(2)}$.
\end{corollary}
...
\begin{proposition}[Time-reversal class and the MPS sign]
  ...
  \uses{PropChiral,prop:sigma1-and-sigma2-equal,
    def:mpu_reduced_full_support_source_datum}
  For an MPU in admissible standard form invariant under time reversal, whose
  two-site blocked tensor carries reduced full-support source data, the MPS
  sign $G\overline G=\pm\Id$ equals the sign $\sigma^{(2)}$.
\end{proposition}
...
\begin{proposition}[CZX]
  ...
  \uses{prop:sigma1-and-sigma2-equal,
    ...
    def:mpu_reduced_full_support_source_datum}
  Suppose the one-site and two-site CZX tensors carry the required reduced
  full-support source data. The CZX MPU is defined by
  \begin{align}
...
\begin{proposition}[The other three time-reversal classes]
  ...
  \uses{example:czx,prop:sigma1-and-sigma2-equal,
    def:mpu_reduced_full_support_source_datum}
  Suppose the displayed standard forms and their required blockings carry
  reduced full-support source data. The standard forms
  \begin{align}
    u&=v=\Id,\\
    u&=i\Id,&v&=\Id,\\
...
\begin{lemma}[Transformation with swaps and symmetry equivalence]
  ...
  \uses{threeMPU,threeMPU2,def:equivalent-symmetry,
    def:mpu_admissible_equivalence_datum}
  For admissible phase comparisons, two MPUs $U_i^{(N)}$ and $U_j^{(N)}$
  are in the same phase under time
...
\begin{proposition}[Time-reversal and transposition phases of $U_2$ and $U_3$]
  ...
  \uses{threeMPU2,lemma:sym-trafo-swap,
    ...
    def:mpu_admissible_equivalence_datum}
  Suppose the displayed comparison paths carry admissible equivalence data.
  Then the MPUs $\widetilde U_2^{(N)}$ and $\widetilde U_3^{(N)}$ are in the
  same admissible phase under both time reversal and transposition.
\end{proposition}
...
\begin{proposition}[Ancilla equivalence of $U_1$ and $U_2$]
  ...
  \uses{threeMPU2,lemma:sym-trafo-swap,
    def:strictly-equivalent-symmetry,
    def:mpu_admissible_equivalence_datum}
  Suppose the ancilla-enlarged tensors carry reduced full-support source data
  and the displayed comparison paths are admissible. After adding one ancilla
  of dimension $d$ per site,
