...
\begin{definition}[Join of the site-indexed support algebras]
    ...
    \end{align}
    Each $\widehat{\mathcal R}_y$ is contained in $G$.  This is the algebra
    generated by all $\mathcal R_y$ in
    \cite[lines~1276--1279]{Gross2012IndexTheory}.  It is generated by
    finite-dimensional subalgebras, so it is not norm closed.
\end{definition}
...
\begin{theorem}[Norm-closed generation by the support algebras]
    ...
    because $\alpha$ is an automorphism.  The $\mathcal R_y$ are
    finite-dimensional, so their algebraic join cannot contain the completion;
    the generation statement is therefore the norm-closed one in
    (\ref{eq:qca_support_algebra_topological_generation}).  This is reusable
...
\begin{proof}\leanok
    \uses{thm:qca_support_algebra_topological_generation}
