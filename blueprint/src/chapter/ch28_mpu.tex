...
\begin{proposition}[Constancy of the source ranks]
  ...
  Suppose that every associated simple canonical-form tensor is equipped with
  a reduced full-support source datum. Then the ranks $r(x)$ and $\ell(x)$ of
  its two source cuts are constant on $[0,1]$.
\end{proposition}
...
\begin{theorem}[Index Theorem]
  ...
  \uses{index-well-defined,lem:mpu_index_tensor,
    ...
    def:mpu_reduced_full_support_source_datum}
  On the class of tensors, blockings, products, compositions, and paths for
  which the required reduced full-support source data are supplied, the MPU
...
\begin{corollary}[Continuous choice of standard form]
  ...
  in canonical form, suppose one can choose a common simple blocking whose
  reduced full-support source data vary continuously with $p$. Then there is
