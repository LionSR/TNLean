...
\begin{definition}[Double layer]
    ...
    indices without reversing the order of the virtual chain, as in the upper
    layer of the double-layer diagrams in \cite[Section~II]{Cirac2017MPU}.
\end{definition}
...
\begin{theorem}[A simple tensor generates matrix product unitaries]
    ...
    If $\mathcal U$ is simple, then for every integer $N > 1$, the periodic operator $U^{(N)}$ is unitary:
    \begin{align}
        \bigl(U^{(N)}\bigr)^\dagger U^{(N)} = \Id, \quad U^{(N)} \bigl(U^{(N)}\bigr)^\dagger = \Id.
        \notag
    \end{align}
    In particular, $\mathcal U$ generates matrix product unitaries \cite[Proposition~III.3(i)]{Cirac2017MPU}.
\end{theorem}
...
\begin{theorem}[Identity plus traceless physical decomposition]
    ...
    \end{align}
    where the second physical argument is traceless.  This is the
    decomposition in \cite[Proposition~III.3(ii)]{Cirac2017MPU}.
\end{theorem}
...
\begin{theorem}[Positive-length mixed residual traces]
    ...
    coefficient.  This is the mixed-trace property in
...
\begin{theorem}[Length-$D^2$ residual products vanish]
    ...
    \end{align}
    This establishes the vanishing of long residual products in
    \cite[Proposition~III.3(ii)]{Cirac2017MPU}, with the exact length $D^2$.
\end{theorem}
...
\begin{definition}[Three-form residual subspace]
    ...
    \end{align}
    This is the basis-independent sum of the three forms in
    \cite[Proposition~III.3(ii)]{Cirac2017MPU}.
\end{definition}
...
\begin{theorem}[Simplicity at every later direct blocking]
    ...
    \leanok
    \uses{def:mpu, def:mpu_simple}
    Let $\mathcal U$ generate an MPU.  If specified vectors $a,b$ satisfy the
    two-letter insertion identity
    \begin{align}
        W^{ij}W^{k\ell} & =W^{ij}|b)(a|W^{k\ell},
    \end{align}
    then the same vectors satisfy
    \begin{align}
        (a|W^{ij}|b) & =\delta_{ij}.
    \end{align}
    Moreover, if the direct length-$k$ block is simple for some $k>0$, then
    its direct length-$k'$ block is simple for every $k'\geq k$, without any
    divisibility condition on $k'$ by $k$.  This is the corollary following
    \cite[Proposition~III.3]{Cirac2017MPU}.
\end{theorem}
...
\begin{theorem}[Trace powers above one determine the nonzero spectrum]
    ...
    \end{align}
    Thus $1$ occurs and every nonzero eigenvalue equals $1$, matching
    the nonzero spectrum of \cite[Proposition~III.1]{Cirac2017MPU}.
\end{theorem}
...
\begin{theorem}[Independent tensor products preserve matrix product unitarity]
    ...
    If $\mathcal U$ and $\mathcal V$ generate matrix product unitaries, then their independent tensor product $\mathcal U\boxtimes\mathcal V$ generates matrix product unitaries: for every $N > 1$,
    \begin{align}
        (\mathcal U\boxtimes\mathcal V)^{(N)} \bigl((\mathcal U\boxtimes\mathcal V)^{(N)}\bigr)^\dagger = \Id.
        \notag
    \end{align}
    This is the tensoring operation in the proof of \cite[Theorem~IV.6(ii)]{Cirac2017MPU}.
\end{theorem}
...
\begin{definition}[Product-index source weight]
    ...
    \cite[Section~III.C, lines~479--487]{Cirac2017MPU}.
...
\begin{definition}[Source tensors $u$ and $v$]
    ...
    These are the triangle contractions in
    \cite[Section~III]{Cirac2017MPU}.
\end{definition}
...
\begin{theorem}[Paper-gate two-site factorization]
    ...
    reverses the two physical input coordinates. This is the two-site
    factorization through the gates $u$ and $v$ in \cite[Section~III]{Cirac2017MPU}.
\end{theorem}
...
\begin{theorem}[Closed double-layer trace for the normalized input tail]
    ...
    the closed input-tail expression in \cite[lines~550--556]{Cirac2017MPU}.
\end{theorem}
...
\begin{theorem}[Full support under physical relabeling]
    ...
    Suppose that canonical-form data have full support, $\sum_{k=1}^r D_k = D$.
    Then for any bijection $\pi \colon \{0,\dots,d'-1\} \to \{0,\dots,d-1\}$, the relabeled data also satisfy full support:
    \begin{align}
        \sum_{k=1}^r D_k = D.
        \notag
    \end{align}
...
\begin{theorem}[Unique block with full support]
    ...
    Let canonical-form data have $r=1$ block and full support $\sum_{k=1}^r D_k = D$.
    Then the block bond dimension satisfies $D_1 = D$, and the reconstructed ambient tensor $A$ is irreducible.
\end{theorem}
...
\begin{theorem}[Normality of a unique full-support block]
    ...
    \uses{def:mpu_full_support}
    Let canonical-form data have $r=1$ block and full support $\sum_{k=1}^r D_k = D$.
    If the transfer map $\E_A$ has spectral radius $\rho(\E_A) = 1$ and is primitive,
    then the ambient tensor $A$ is normal.
\end{theorem}
...
\begin{proof}\leanok
    \uses{def:blocked_tensor, def:left_canonical_tensor, thm:left_canonical_blocking, thm:cpsv_physical_reindexing}
...
\begin{theorem}[Identification with chosen reduced-CFII fixed points]
    ...
    fixed-point equations of \cite[equations~(6a)--(6b)]{Cirac2017MPU};
    this establishes the fixed-point structure for the reduced representative after
    blocking \cite[proof of Proposition~III.2(ii)]{Cirac2017MPU}.
\end{theorem}
...
\begin{theorem}[Normality of a full-support normalized MPU representative]
    ...
    This yields the representative form of
    \cite[Proposition~III.1]{Cirac2017MPU} under the full-support condition.
\end{theorem}
...
\begin{definition}[Reduced full-support source datum]
    ...
    For $D>1$, this formalizes the fixed-pair convention of
    \cite[Proposition~III.2(ii)]{Cirac2017MPU}, with $D=1$ as the
...
\begin{theorem}[Staggered Gram expansion of the source tensor $u$]
    ...
    the four-tensor expression obtained from $u=Y_2\mathbin{-}Y_1$ and the
    identities in \cite[Section~III.C]{Cirac2017MPU}.
\end{theorem}
...
\begin{theorem}[Complete source-$v$ network]
    ...
    equals $\lvert\rho)$ and the displayed equivalence is the identity in the
    proof of \cite[Theorem~III.8]{Cirac2017MPU}; see \cite{gap:mpu_source_cut_orientation}.
\end{theorem}
...
\begin{proof}\leanok
    \uses{thm:mpug_simple_canonical_boundary_contractions,
        thm:mpu_source_v_complete_contraction, thm:mpu_source_v_dressed_gram_cancel}
    Theorem~\ref{thm:mpug_simple_canonical_boundary_contractions} gives
    the insertion identity for the recorded canonical boundaries:
...
\begin{proof}
    ...
    is documented in \cite{gap:mpu_canonical_form_full_support}.
    ...
    corrected source-cut orientation \cite{gap:mpu_source_cut_orientation}.
...
\begin{definition}[Doubled-virtual contraction and factor-free sandwich]
    ...
    \end{align}
    This is the contraction in \cite[Proposition~IV.5]{Cirac2017MPU}.
    % Source lines: paper_v2.tex 807--812.
\end{definition}
...
\begin{theorem}[Index Theorem]
    ...
    For MPU tensors under the standing canonical-form convention, the index $\operatorname{ind}(\mathcal U) = \tfrac12\log_2(r/\ell)$ defined from any simple blocking satisfies:
    \begin{enumerate}
        \item Invariance under blocking: $\operatorname{ind}(\mathcal U^{[L]}) = \operatorname{ind}(\mathcal U)$ for all $L \ge 1$.
        \item Additivity: $\operatorname{ind}(\mathcal U\boxtimes\mathcal V) = \operatorname{ind}(\mathcal U) + \operatorname{ind}(\mathcal V)$, and $\operatorname{ind}(\mathcal W) = \operatorname{ind}(\mathcal U) + \operatorname{ind}(\mathcal U')$ for composition.
        \item Continuity: $\operatorname{ind}(\mathcal W(0)) = \operatorname{ind}(\mathcal W(1))$ along any continuous path $p \mapsto \mathcal W(p)$ of MPU tensors.
        \item Classification: $\operatorname{ind}(\mathcal U) = \operatorname{ind}(\mathcal V) \iff \mathcal U \sim \mathcal V$.
    \end{enumerate}
    This is the unrestricted statement of
...
\begin{lemma}[Symmetric branch of the unitary congruence normal form]
    ...
    \begin{align}
        x & = S^T S.
    \end{align}
    This is the symmetric branch of \cite[Lemma~V.2]{Cirac2017MPU}.
...
\begin{lemma}[Forgetting symmetry from strict equivalence]
    ...
    If $\mathcal U$ and $\mathcal V$ are strictly equivalent under a symmetry
    $\mathcal S$ via a continuous path $\mathcal W(p)$ with respect to
    $h\colon d_a=d_b$, then they are strictly equivalent via the same path:
    \begin{align}
        \mathcal W(0) = \operatorname{reind}_{h^{-1}}(\mathcal U), \quad \mathcal W(1) = \mathcal V.
        \notag
    \end{align}
\end{lemma}
...
\begin{theorem}[Forgetting symmetry from coherent stabilized equivalence]
    ...
    Fixed-bond equivalence under a coherent symmetry family implies ordinary equivalence:
    \begin{align}
        \mathcal U \sim_{\mathcal S} \mathcal V \implies \mathcal U \sim \mathcal V.
        \notag
    \end{align}
\end{theorem}
...
\begin{lemma}[Admissible standard-form path criterion]
    ...
    the length-one admissible witness for the displayed endpoint families.
    This provides the reduced-source form of the lemma following the
    symmetry-equivalence definitions in \cite{Cirac2017MPU}.
    % Source lines: paper_v2.tex 1366--1380.
\end{lemma}
...
\begin{theorem}[Normalization obstruction in the printed CZX tensor]
    \label{thm:czx_printed_normalization_obstruction}
    % Principal declaration: MPOTensor.CZX.printed2017Tensor_not_isMPU.
    \lean{MPOTensor.CZX.printed2017Tensor, MPOTensor.CZX.printed2017Tensor_two_zero_row, MPOTensor.CZX.printed2017Tensor_not_isMPU}
    \leanok
    \discussion{7738}
    \uses{def:mpu}
    Let $u,d,\ell,r\in\{0,1\}$ and set
    $P^{ud}_{\ell r}=\delta_{u,1-d}\delta_{u,\ell}2^{-1/2}(-1)^{\ell r}$,
    the tensor printed in the CZX example of \cite{Cirac2017MPU}.
    In the ordinary periodic trace contraction, the two-site operator
    generated by $P$ has all-zero output row
    \begin{align}
        \operatorname{MPO}_2(P)_{00,b} & =\tfrac12\delta_{b,11}.
    \end{align}
    In particular, $P$ does not generate a matrix product unitary.
    The normalization discrepancy and comparison with the Z-decorated
    tensor are detailed in \cite{gap:mpu_czx_tensor_normalization}.
    % Source: paper_v2.tex, example:czx, lines 1914--1930.
    % False-source note: docs/paper-gaps/mpu_czx_tensor_normalization.tex.
\end{theorem}
\begin{proof}
    \leanok
    \uses{lem:mpu_mul_adjoint}
    The copy deltas fix both virtual indices to zero. The physical bit
    flips then force the input to be $11$, and the two Hadamard entries
    contribute $1/2$. If the operator were unitary, its all-zero diagonal
    entry after multiplication by its adjoint would give
    \begin{align}
        1 & =\sum_b\left|\operatorname{MPO}_2(P)_{00,b}\right|^2
...
\begin{remark}
    The normalized Hadamard in the following printed CZX formulas has
    the obstruction above. In contrast, the blue tensors in
    the CZX tensor diagram of \cite{FrancoRubio2025MPUGauging}
    are $\sqrt{2}H$, and this diagram also has a $Z$ gate on each
    output qubit. Let $A=\sqrt2 P$, let $A_2$ denote its two-site block,
    and let $B$ denote that displayed Z-decorated tensor. The output-leg
    relation is
    \begin{align}
        B & =(Z\otimes Z)A_2.
        \label{eq:mpu_czx_decorated_block}
    \end{align}
    The coordinate derivation of (\ref{eq:mpu_czx_decorated_block}) is given in
    \cite{gap:mpu_czx_tensor_normalization}.
\end{remark}
...
\begin{proposition}[CZX]
    ...
    \notready
    \discussion{5715}
    \uses{prop:sigma1-and-sigma2-equal, prop:dagger-mps-class-and-mpu-class-equal, thm:czx_printed_normalization_obstruction}
    The literal normalized tensor printed below is not an MPU, by
    Theorem~\ref{thm:czx_printed_normalization_obstruction}.
    The following is the source classification claim, not an assertion
    of unitarity for that literal tensor; its normalization and comparison
    with the Z-decorated tensor are discussed in
    \cite{gap:mpu_czx_tensor_normalization}.
...
\begin{proposition}[Admissible CZX data]
    ...
    % Documented in docs/paper-gaps/mpu_canonical_form_full_support.tex.
    \uses{prop:mpu_admissible_sigma_blocking_parity, prop:mpu_admissible_mps_time_reversal_class, def:mpu_reduced_full_support_source_datum, def:mpu_admissible_equivalence_datum, thm:czx_printed_normalization_obstruction}
    The literal normalized tensor printed below is not an MPU, by
    Theorem~\ref{thm:czx_printed_normalization_obstruction}.
    The following records the supplied-data classification claim;
    it does not remove that obstruction or identify a corrected tensor.
    See \cite{gap:mpu_czx_tensor_normalization}.
...
\begin{definition}[Trace-normalized source factors for the right shift]
    ...
    \cite[Section~III.C]{Cirac2017MPU} for the right shift.
\end{definition}
...
\begin{theorem}[The paper source tensor $u$ for the right shift]
    ...
    \cite[Section~III]{Cirac2017MPU} for this basic MPU. It is the explicit
    normalization and orientation regression for
...
\begin{theorem}[Finite-chain formulas for the three shift MPUs]
    ...
    For every chain length $N \ge 1$, the periodic operators generated by $\mathcal U_1, \mathcal U_2, \mathcal U_3$ satisfy:
    \begin{align}
        U_1^{(N)} & = (\Id\otimes\Id)^{\otimes N}, \notag       \\
        U_2^{(N)} & \cong T^{(N)\dagger}\otimes T^{(N)}, \notag \\
        U_3^{(N)} & \cong T^{(N)}\otimes T^{(N)\dagger}. \notag
    \end{align}
    Moreover, each tensor is a matrix product unitary: $(U_a^{(N)})^\dagger U_a^{(N)} = \Id$ for all $N > 1$ and $a \in \{1, 2, 3\}$.
\end{theorem}
...
\begin{proof}\leanok
    \uses{thm:mpu_swap_matrix_unitary}
    The swap is unitary by Theorem~\ref{thm:mpu_swap_matrix_unitary}. Kronecker
    products preserve isometries, and a square isometry is unitary. Thus both
    factors are unitary. Closure of the unitary matrices under multiplication
    gives the two ordered products.
\end{proof}
...
for its commutant. The construction below adapts the coefficient construction of
\cite[Section~4.3]{SchumacherWerner2004QCA} and the leastness and commutant
statements of \cite[Lemma~7]{Gross2012IndexTheory}, requiring $\mathcal A$ to be
a star-subalgebra for involution closure of the commutant; see
\textbf{Scope restriction (full matrix factors):}
While \cite[Lemma~7]{Gross2012IndexTheory} and \cite{SchumacherWerner2004QCA} permit arbitrary
finite-dimensional $C^*$-algebras $\mathcal B_1$ and $\mathcal B_2$, the development below specializes
to full matrix factors $\mathcal B_1=M_I(\mathbb C)$ and $\mathcal B_2=M_J(\mathbb C)$. This suffices
for the MPU spin chain because every one-site algebra is isomorphic to $M_d(\mathbb C)$
\cite[lines~2320--2334]{Cirac2017MPU}; documented in
...
\begin{theorem}[Commutation of overlapping support algebras]
    ...
    \end{align}
    This specializes the commuting support lemma of
    \cite[lines~1174--1194]{SchumacherWerner2004QCA} and
    \cite[Section~7.1, lines~1221--1246]{Gross2012IndexTheory} to full complex matrix factors.

    \textbf{Local fix (star-closure and outer factor basis corrections):}
    For the generated $C^*$-algebras, the input must be closed under the
    involution; documented in \cite{gap:gnvw12_support_algebra_star_closure}
    (\texttt{docs/paper-gaps/gnvw12_support_algebra_star_closure.tex}).
    In both printed coefficient proofs, the basis element $e'_\nu$ belongs to the
    third factor, rather than the second factor as printed; documented in
    \cite{gap:gnvw12_support_algebra_outer_factor_basis}
    (\texttt{docs/paper-gaps/gnvw12_support_algebra_outer_factor_basis.tex}).
    ...
    \cite[lines~2300--2306 and 2313--2334]{Cirac2017MPU}; documented in
    \cite{gap:gnvw12_support_algebra_full_matrix_scope}
    (\texttt{docs/paper-gaps/gnvw12_support_algebra_full_matrix_scope.tex}).
\end{theorem}
...
\begin{theorem}[Product of two full matrix factors]
    ...
    \end{align}
    This is the unital single-block specialization of
    \cite[lines~2119--2138]{SchumacherWerner2004QCA},
    used for the adjacent support algebras in
    \cite[lines~1270--1308]{Gross2012IndexTheory}; see
    \cite{gap:gnvw12_cscom_single_block_scope}.
\end{theorem}
...
\begin{theorem}[Algebraic-local relative commutant]
    ...
    \end{align}
    This characterizes support in the strictly local algebra.
\end{theorem}
...
\begin{theorem}[Density of the canonical completion map]
    ...
    Let $A$ be a normed star algebra, and let $j \colon A \to \widehat{A}$ be the canonical star-algebra homomorphism into its completion. Then $j$ has dense range:
    \begin{align}
        \overline{j(A)} = \widehat{A}.
        \notag
    \end{align}
\end{theorem}
...
\begin{theorem}[Scalar center of the homogeneous quasi-local algebra]
    ...
    \begin{align}
        \{z\in\mathcal A\mid za=az\text{ for every }a\in\mathcal A\}
         & =\mathbb C\mathbf 1.
    \end{align}
    This establishes triviality of the centre for the homogeneous spin chain.
\end{theorem}
...
\begin{theorem}[Finite local-algebra blocking formulas]
    ...
    For any matrix $M$, the blocked matrix entries satisfy
    \begin{align}
        (\Phi(M))_{i,j} = M_{\varphi^{-1}(i),\varphi^{-1}(j)},
        \notag
    \end{align}
    and the blocking map preserves the operator norm: $\|\Phi(M)\| = \|M\|$. In particular, $\Phi$ is an isometry.
\end{theorem}
...
\begin{definition}[Algebraic-local site-blocking equivalence]
    ...
    \end{align}
    This formalizes the algebraic-local change of site grouping used in
    \cite[lines~2308 and 2313--2320]{Cirac2017MPU}.
\end{definition}
...
By regrouping sites into blocks of size $L$, a finite propagation
neighborhood is normalized to nearest-neighbor interaction $[-1,1]\cap\mathbb Z$
(see \cite[Section~3]{Cirac2017MPU}).
...
\begin{definition}[Nearest-neighbour pair regions]
    ...
    \end{align}
    The order $L_x,P_x$ is the left--right factor order in
...
\begin{definition}[Site-indexed support algebras]
    ...
    composition.
    Formula (\ref{eq:qca_RR2x}) corresponds to the support algebra decomposition in
    ...
    not claimed here; documented in
    \cite{gap:gnvw12_site_dependent_adjacent_generation_scope}
    (\texttt{docs/paper-gaps/gnvw12_site_dependent_adjacent_generation_scope.tex}).
\end{definition}
...
\begin{theorem}[Physical support of the site-indexed support algebras]
    ...
    This is the homogeneous specialization of the localization in
...
\begin{theorem}[Norm-closed generation by the support algebras]
    ...
    because $\alpha$ is an automorphism.  This establishes generation in
    the homogeneous setting; see
    \cite{gap:gnvw12_site_dependent_adjacent_generation_scope}.
\end{theorem}
...
\begin{theorem}[Pairwise commutation of distinct support algebras]
    ...
    \end{align}
    This establishes the homogeneous fixed-$d$ case of the
    consecutive/disjoint argument in
    \cite[lines~1270--1274]{Gross2012IndexTheory} and
    \cite[lines~1174--1194]{SchumacherWerner2004QCA} for the
    homogeneous four-site localization in
...
\begin{theorem}[Two-site translation covariance of the support algebras]
    ...
    \end{align}
    The displacement is two sites of the original chain, corresponding to one blocked cell.
    In general, one-site translation $\tau_1$ interchanges the even and odd pairings rather than preserving them;
    for instance, for the unilateral shift automorphism \cite[lines~1267--1268]{Gross2012IndexTheory},
    the even support algebra is a two-site full matrix algebra while the odd support algebra reduces to the scalars.
    ...
    and odd factors are defined as in
    ...
    The statement treats the one-dimensional translation-covariant case on a
    homogeneous chain \cite[lines~2292--2298]{Cirac2017MPU}; see
    \cite{gap:sw04_support_covariance_scope, gap:gnvw12_site_dependent_adjacent_generation_scope}.
\end{theorem}
