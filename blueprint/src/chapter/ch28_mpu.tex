...
\begin{theorem}[Full support under physical relabeling]
    ...
    Then for any bijection $\pi \colon \{1,\dots,d'\} \to \{1,\dots,d\}$, the relabeled data also satisfy full support:
    \begin{align}
        \sum_{k=1}^r D_k = D.
        \notag
    \end{align}
...
\begin{theorem}[Index Theorem]
    ...
    For MPU tensors under the standing canonical-form convention, the index $\operatorname{ind}(\mathcal U) = \log(r/\ell)$ defined from any simple blocking satisfies:
    \begin{enumerate}
...
\begin{lemma}[Forgetting symmetry from strict equivalence]
    ...
    If $\mathcal U$ and $\mathcal V$ are strictly equivalent under a symmetry $\mathcal S$ via a continuous path $\mathcal W(p)$, then they are strictly equivalent via the same path:
    \begin{align}
        \mathcal W(0) = \operatorname{reind}(\mathcal U), \quad \mathcal W(1) = \mathcal V.
        \notag
    \end{align}
\end{lemma}
...
**Scope restriction (full matrix factors):**
The support-algebra results in \cite[Lemma~7]{Gross2012IndexTheory} allow arbitrary
finite-dimensional $C^*$-algebras $\mathcal B_1$ and $\mathcal B_2$, whereas the
formalization treats full matrix factors $\mathcal B_1=M_I(\mathbb C)$ and
$\mathcal B_2=M_J(\mathbb C)$. This restriction suffices for the MPU spin chain
where every single-site algebra is $M_d(\mathbb C)$ \cite[lines~2320--2334]{Cirac2017MPU};
documented in `docs/paper-gaps/gnvw12_support_algebra_full_matrix_scope.tex` \cite{gap:gnvw12_support_algebra_full_matrix_scope}.
...
\begin{theorem}[Commutation of overlapping support algebras]
    ...
    using star-subalgebra inputs and third-factor basis elements as in
    \cite{gap:gnvw12_support_algebra_star_closure, gap:gnvw12_support_algebra_outer_factor_basis, gap:gnvw12_support_algebra_full_matrix_scope}.
\end{theorem}
...
\begin{definition}[Site-indexed support algebras]
    ...
    For the MPU spin chain, the cell size is taken to be homogeneous; see
