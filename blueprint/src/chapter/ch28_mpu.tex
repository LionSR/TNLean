...
\begin{lemma}[Iterated physical blocking]
    ...
    Iterating blocking at lengths $m$ and $n$, then applying the canonical
    regrouping of the physical indices, agrees with direct blocking at length
    $mn$.
\end{lemma}
...
\begin{proof}\leanok
    ...
    For the right cut, factor the two-site block of each tensor through its
    source tensor $u$. The diagonal cut through the two blocked factors retains
    one physical leg and one right source leg from each factor. Hence the
    resulting matrix factors through
    \begin{align}
        (\mathbb C^d\otimes\mathbb C^{r[\mathcal U]})
        \otimes
        (\mathbb C^d\otimes\mathbb C^{r[\mathcal V]}),
    \end{align}
    whose dimension is $d^2r[\mathcal U]r[\mathcal V]$. Matrix rank is at most
    the dimension of this intermediate space. For the left cut, use the
    reflected $v$ factorization. Its physical order $(j_2,j_1)$ and source
    order $(r,l)$ give the analogous intermediate dimension
    $d^2\ell[\mathcal U]\ell[\mathcal V]$. Finally, identify iterated two-site
    blocking with direct four-site blocking by the standard physical
    reindexing; both source-cut ranks are invariant under this relabeling.
\end{proof}
