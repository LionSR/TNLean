...
\begin{proof}\leanok
    ...
    generators $EA$, $AE$, and $AEB$ of the three summands.  Associativity,
    $E^2=E$, and the sandwich identity $EAE=0$, also applied to products in
    $\mathfrak S$, give both formulas in the nine possible pairs of generator
    types.
\end{proof}

\begin{theorem}[Exact simplicity after the second blocking]
    \label{thm:mpu_second_block_simple}
    \lean{MPOTensor.IsMPU.blockTensor_sq_isMPUSimple_of_normalizedDiagonal_eq_vecMulVec}
    \leanok
...
\begin{proof}\leanok
    ...
    $R_{ij}\in\mathcal T$.  The trace identity for $R_{ij}E$ gives
    \texttt{simple1}, and the insertion identity
    $R_{ij}R_{k\ell}=R_{ij}ER_{k\ell}$, together with $E^2=E$, gives
    \texttt{simple2} after expanding both coefficients.
\end{proof}
...
\begin{theorem}[Bounded blocking to simplicity]
    ...
    \begin{align}
        ...
        &&0<J\leq D^2-1.
    \end{align}
    If $D=1$, the length-one block is simple.  Simplicity is preserved by
    every further positive blocking and by a bijective relabeling of the
    physical basis.
\end{theorem}
...
\begin{proof}\leanok
    ...
    makes the subsequent length-$D^2$ block simple.  Canonically grouping the
    two blocking stages identifies it with direct length $JD^2$ blocking.
    For $D>1$,
    \begin{align}
        JD^2&<(D^2)D^2=D^4.
    \end{align}
    The one-dimensional transfer matrix is already the identity.  For further
    blocking, repeated use of \texttt{simple2} inserts the same rank-one
    projector between adjacent letters, while repeated \texttt{simple1}
    multiplies the sitewise Kronecker deltas.
\end{proof}

\begin{corollary}[The fixed-point simple1 contraction]
    \label{cor:mpu_simple1}
    \lean{MPOTensor.IsMPU.simple1}
    \leanok
    ...
    \end{align}
    The unblocked tensor need not satisfy \texttt{simple2}.
\end{corollary}
