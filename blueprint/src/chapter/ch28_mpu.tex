...
\begin{definition}[Reduced full-support source datum]
  ...
  \end{enumerate}
  For $D>1$, this packages the fixed-pair convention in
...
\begin{theorem}[Index Theorem]
  ...
  \notready
  \discussion{6023}
  \uses{def:index,index-well-defined,def:equivalent-tensors}
...
\begin{theorem}[Admissible Index Theorem]
  ...
  % data. Documented in docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{prop:mpu_admissible_index_well_defined,lem:mpu_index_tensor,
...
\begin{proposition}[Admissible local characterization of time reversal]
  ...
  % docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{thm:mpu_admissible_fundamental,thm:mpu_admissible_index,
    def:mpu_admissible_standard_form,
    def:mpu_reduced_full_support_source_datum}
...
\begin{proof}
  \uses{thm:mpu_admissible_index,thm:mpu_admissible_fundamental}
  Under time reversal, $\mathcal U_2$ and $\mathcal U_2^\sharp$ generate the
  same MPU. Physical adjunction exchanges the two source cuts, so
  $r(\mathcal U_2^\sharp)=\ell(\mathcal U_2)$ and
  $\ell(\mathcal U_2^\sharp)=r(\mathcal U_2)$.
  Blocking invariance and equality of the admissible indices therefore give
  \begin{align}
    \operatorname{ind}(\mathcal U)
      &=\operatorname{ind}(\mathcal U_2)
       =\operatorname{ind}(\mathcal U_2^\sharp)
       =-\operatorname{ind}(\mathcal U_2)
       =-\operatorname{ind}(\mathcal U).
  \end{align}
  Hence $\operatorname{ind}(\mathcal U)=0$. With this obstruction derived,
  apply the admissible fundamental theorem to the supplied standard forms of
  $\mathcal U_2$ and $\mathcal U_2^\sharp$ and follow the source proof. The
  converse follows by contracting the displayed local relation around every
  even periodic chain.
\end{proof}
...
\begin{proof}
  ...
  the same MPU. Transposition exchanges the two source cuts, so
  $r(\mathcal U_2^{\mathsf T})=\ell(\mathcal U_2)$ and
  $\ell(\mathcal U_2^{\mathsf T})=r(\mathcal U_2)$.
  Blocking invariance and equality of the admissible indices give
  \begin{align}
    \operatorname{ind}(\mathcal U)
      &=\operatorname{ind}(\mathcal U_2)
       =\operatorname{ind}(\mathcal U_2^{\mathsf T})
       =-\operatorname{ind}(\mathcal U_2)
       =-\operatorname{ind}(\mathcal U).
  \end{align}
  Thus $\operatorname{ind}(\mathcal U)=0$. Apply the admissible fundamental
  theorem to the supplied standard forms of $\mathcal U_2$ and
...
\begin{lemma}[Admissible standard-form path criterion]
  ...
  are admissibly strictly equivalent under $\mathcal S$ through a witness whose
  common blocking length $m$ satisfies $m\leq D^4$ if and only if there are
  continuous families of unitaries $\widetilde u(p)$ and $\widetilde v(p)$
  whose endpoints generate $U_1^{(N)}$ and $U_2^{(N)}$, whose generated MPU is
  invariant under $\mathcal S$ for every $p\in[0,1]$, and whose actual blocked
  tensor path and the fixed conjugate, physical-adjoint, transposed, and
  $\mathcal S$-image comparison paths carry the continuously varying source
  data of an admissible equivalence datum with the same length $m\leq D^4$.
\end{lemma}
...
\begin{theorem}[Admissible conjugation symmetry when one source rank is odd]
  ...
  interpolation determined below carries an admissible path datum whose common
  blocking length $m$ satisfies $m\leq D^4$. Two MPUs
...
\begin{corollary}[Admissible equivalence of conjugation-symmetric MPUs after blocking]
  ...
  % No preservation under blocking is asserted. Documented in
  % docs/paper-gaps/mpu_canonical_form_full_support.tex.
  \uses{thm:mpu_admissible_conjugation_odd_rank,def:mpu_admissible_symmetry_equivalence,
    def:mpu_admissible_equivalence_datum}
...
\begin{theorem}[Admissible strict conjugation equivalence for even source ranks]
  ...
  interpolation determined below carries an admissible path datum whose common
  blocking length $m$ satisfies $m\leq D^4$. Choose
...
\begin{corollary}[Admissible equivalence under conjugation]
  ...
  \uses{thm:mpu_admissible_conjugation_even_rank,def:mpu_admissible_symmetry_equivalence,
    def:mpu_admissible_equivalence_datum}
  ...
  they belong to the same case of Proposition~\ref{prop:mpu_admissible_conjugation_standard_forms}.
\end{corollary}
...
\begin{corollary}[Admissible necessary time-reversal invariants]
  ...
  path $p\mapsto\mathcal W(p)$. Suppose that the five path families
  ...
  ancilla-blocked witness path, not a reduced or equivalent representative.
  Require continuous reduced full-support source data along
  $\mathcal W_{\mathrm{anc}}(p)$, its actual two- and four-site blocks, and
  the physical-adjoint comparisons of those two blocks. Then the two
  endpoints have the same $\sigma^{(2)}$.
\end{corollary}
...
\begin{proposition}[Admissible conjugation class and the MPS sign]
  ...
  conjugation-symmetry path datum with one common positive blocking length
  $m\leq D^4$: the actual path $\mathcal W(p)_m$ and its fixed conjugate,
  physical-adjoint, transposed, and conjugation-image comparison paths are
  simple and carry continuously varying source data. Then the sign
...
\begin{definition}[Three topological-insulator-inspired MPUs]
  ...
  \end{align}
  % Source lines: paper_v2.tex 1980--2042.
\end{definition}
...
\begin{lemma}[Admissible transformation with swaps and symmetry equivalence]
  ...
  in that comparison. Suppose it carries an admissible symmetry-path datum
  with one common positive blocking length $m\leq D^4$. Thus the actual
  blocked path $\mathcal W(p)_m$ and the four fixed comparison paths
  $\overline{\mathcal W(p)_m}$, $\mathcal W(p)_m^\sharp$,
  $\mathcal W(p)_m^{\mathsf T}$, and $\mathcal S[\mathcal W(p)_m]$ are simple
  and carry continuously varying reduced full-support source data. Suppose
...
\begin{proposition}[Admissible time-reversal and transposition phases of $U_2$ and $U_3$]
  ...
  an admissible symmetry-path datum with one common positive blocking length
  $m\leq D^4$. Thus the actual blocked path
  $\mathcal W_{\mathcal S}(p)_m$ and its fixed conjugate, physical-adjoint,
...
\begin{proposition}[Admissible conjugation phases of $U_1$ and $U_2$]
  ...
  \uses{threeMPU2,lem:mpu_admissible_swap_symmetry_equivalence,
    ...
    def:mpu_reduced_full_support_source_datum,
    def:mpu_admissible_equivalence_datum}
  Suppose the endpoint tensors for $U_1$, $U_2$, $\widetilde U_1$, and
  $\widetilde U_2$, together with their conjugate comparison tensors, are
  simple and carry their own reduced full-support source data. For every
  strict comparison used below, let $p\mapsto\mathcal W(p)$ be the actual
  source-constructed interpolation between the corresponding endpoint
  tensors. Suppose it carries an admissible conjugation-symmetry path datum
  with one common positive blocking length $m\leq D^4$: the actual blocked
  path and its fixed conjugate, physical-adjoint, transposed, and
  conjugation-image comparison paths are simple and carry continuously
  varying source data. Require the same datum for the actual ancilla-blocked
  interpolation used in the final equivalence claim. Under these hypotheses,
...
\begin{proposition}[Admissible ancilla equivalence of $U_1$ and $U_2$]
  ...
  \uses{threeMPU2,lem:mpu_admissible_swap_symmetry_equivalence,
    ...
    def:mpu_reduced_full_support_source_datum,
    def:mpu_admissible_equivalence_datum}
  After adding one ancilla of dimension $d$ per site and performing the
  source blocking, let $p\mapsto\mathcal W_{\mathcal S}(p)$ be the actual
  source-constructed ancilla-blocked interpolation from $\widetilde U_1$ to
  $\widetilde U_2$, for either time reversal or transposition. Suppose this
  interpolation carries an admissible symmetry-path datum with one common
  positive blocking length $m\leq D^4$. Thus the actual blocked path
  $\mathcal W_{\mathcal S}(p)_m$ and its fixed conjugate, physical-adjoint,
  transposed, and $\mathcal S$-image comparison paths are simple and carry
  continuously varying reduced full-support source data. Suppose also that
  the one- and two-site ancilla-enlarged endpoint tensors used in the local
  comparison carry their own data. Then $\widetilde U_1^{(N)}$ and
  $\widetilde U_2^{(N)}$ are admissibly strictly equivalent under both time
  reversal and transposition.
\end{proposition}
