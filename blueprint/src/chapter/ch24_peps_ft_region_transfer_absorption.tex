...
\begin{theorem}[Region vertex product across an inserted site]%
    ...
    \begin{align}
        \prod_{w\in R\cup\{v\}}A_w(\zeta|_{\inc(w)},\sigma_w)
         & =
        A_v(\zeta|_{\inc(v)},\sigma_v)
        \prod_{w\in R}A_w(\zeta|_{\inc(w)},\sigma|_R{}_w).
        \notag
    \end{align}
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Region gauge absorption]%
    ...
    \leanok
    \uses{thm:peps_regionInsertedCoeff_eq_smul_edgeInsertedCoeff, thm:peps_edge_gauge_absorption, def:applyGauge}
...
\begin{proof}\leanok
    ...
    ($\mathcal O_{R,f}\circ\mathcal O_{R,f}=\id$), gives the
    displayed equality.
\end{proof}
...
\begin{align}
    C_A(R,f,M;\sigma,\tau)
     & =
    C_B(R,f,Z\,\phi_{h_f}(M)\,Z^{-1};\sigma,\tau),
    \notag
\end{align}
...
\begin{theorem}[Matrix reconciliation for the absorbing gauge]%
    ...
    bond space of $f$,
    \begin{align}
        \mathcal O_{R,f}\bigl(        X_f^{\mathsf T} \mathcal O_{R,f}(N)
        (X_f^{-1})^{\mathsf T}\bigr)
         & =
        Z\,N\,Z^{-1}.
        \notag
...
\begin{theorem}[Region absorbed equality from the conjugation identity]%
    ...
    \leanok
    \uses{def:peps_regionInsertedCoeff, def:applyGauge, def:peps_absorbedBoundaryGauge, thm:peps_regionEdgeOrient_absorbedBoundaryGauge, thm:peps_regionInsertedCoeff_applyGauge}
    ...
    \begin{align}
        C_A(R,f,M;\sigma,\tau)
         & =
        C_{B^X}(R,f,\phi_{h_f}(M);\sigma,\tau).
        \notag
    \end{align}
    This is the region analog of
...
\begin{theorem}[Edge absorbed equality from the region absorbed equality]%
    ...
    \leanok
    \uses{def:peps_regionInsertedCoeff, def:peps_edgeInsertedCoeff, def:applyGauge, thm:peps_regionInsertedCoeff_eq_smul_edgeInsertedCoeff}
...
\begin{proof}\leanok
    ...
    $\mathcal O_{R,f}\circ\mathcal O_{R,f}=\id$, the compatibility
...
\begin{theorem}[Region absorbed equality from the edge absorbed equality]%
    ...
    \begin{align}
        C_A(R,e,M;\sigma,\tau)
         & =
        C_{B^X}(R,e,\phi_{h_e}(M);\sigma,\tau).
        \notag
    \end{align}
