...
\begin{theorem}[Printed purification fixed-point equivalence]
    ...
    \notready
    \discussion{5917}
    \uses{def:mpdo_purification_rfp_witness, def:mpo_source_zcl,
        thm:global_prfp_not_source_zcl,
        thm:nonzero_global_prfp_not_source_zcl}
    Let an MPO tensor admit the global purification equation of
    Definition~\ref{def:mpdo_global_purification_equation}.  Theorem~4.4 of
    \cite{Cirac2016MPDO_arXiv} states that being a purification
    ...
    including a nonzero MPDO example, which fail the source zero-correlation
    relation.  Thus the printed equivalence is not covered by the corrected
    local purification theorems.  The obstruction is documented in
    \path{docs/paper-gaps/cpsv16_purification_rfp_definition.tex}.
\end{theorem}
