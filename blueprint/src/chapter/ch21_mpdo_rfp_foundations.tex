...
\begin{definition}[Source zero correlation length for MPO tensors]
    ...
    $\lambda>0$, the following conditions hold:
    \begin{align}
        \mathcal T_M&\ne 0,
        \notag\\
...
\begin{theorem}[Source zero correlation length requires a nonzero bond space]
    \label{thm:mpo_source_zcl_bond_dim_ne_zero}
    \lean{MPOTensor.IsSourceZCL.bondDim_ne_zero}
    \leanok
    \uses{def:mpo_source_zcl}
    If an MPO tensor has source zero correlation length, then its bond
