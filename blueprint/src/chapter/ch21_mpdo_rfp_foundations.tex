...
\begin{proof}\leanok
    \uses{def:is_rfp}
    The purifying tensor $A$ has transfer matrix $\sum_p \overline{A_p}\otimes A_p$.
    The pure-state renormalization fixed point condition makes this transfer
    matrix idempotent. Closing the ket and bra physical legs of $M$ rewrites
    $\mathcal T_M$ as a reindexing of that transfer matrix by the bond
    identification together with the Kronecker factor swap, and idempotence of a
    matrix is preserved under reindexing by a bijection of the bond space.
\end{proof}
