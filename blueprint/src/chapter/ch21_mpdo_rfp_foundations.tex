...
\begin{definition}[Global purification equation]
    ...
    purifying tensor $A$ if, for every length $N$,
    \[
        \rho^{(N)}(M)=\rho^{(N)}_p(A).
    \]
    Equivalently,
...
\begin{definition}[Trace-preserving spin reduction for a purification RFP]
    ...
    \leanok
    \uses{def:mpdo_is_prfp, def:is_rfp_via_ts}
    A purification RFP has trace-preserving spin reduction when, in addition,
    the spin degrees of freedom carry the trace-preserving completely positive
    maps of Definition~\ref{def:is_rfp_via_ts}. This records the structure
    obtained after tracing the ancillary indices in
