...
\begin{definition}[Periodic two-point MPDO correlation]
    ...
    This unnormalized contraction matches \cite[lines~490--496]{Cirac2016MPDO_arXiv}.
\end{definition}
...
\begin{theorem}[Zero correlation length removes positive wrapping-gap dependence]
    ...
    \end{align}
    This establishes independence of the periodic correlation under variations
    of the ring length at fixed observable separation, including adjacent
    marked sites ($g_{\mathrm{mid}}=0$), in the setting of
    \cite[Definition~4.2, lines~735--742]{Cirac2016MPDO_arXiv}. The restriction
    to strictly positive wrapping gaps is documented in
...
\begin{corollary}[Zero correlation length gives positive-gap distance independence]
    ...
    \end{align}
    For equal gap sums, this establishes correlation independence under varying
    observable distance on a fixed ring when the observables are nonadjacent on
    both complementary arcs \cite[lines~433--500]{Cirac2016MPDO_arXiv}; the
    positive-gap regime is documented in
    \cite{gap:cpsv16_mpdo_zcl_correlation_length_boundary}.
\end{corollary}
...
\begin{definition}[Nonzero up-to-scalar physical-trace relation]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This scale-invariant condition excludes the zero transfer while remaining
    invariant under positive rescaling of $M$, extending the literal $\lambda=1$
    diagram of \cite[Definition~4.2]{Cirac2016MPDO_arXiv}
    \cite{gap:cpsv16_zcl_canonical_form_normalization}.
\end{definition}
...
\begin{proof}
    \leanok
    This is Definition~\ref{def:mpo_source_zcl} with $\lambda=1$.
\end{proof}
...
\begin{theorem}[Renormalization fixed points have idempotent physical-trace transfer]
    ...
    (i)$\Rightarrow$(ii) in
    \cite[Theorem~4.9, lines~851--893]{Cirac2016MPDO_arXiv} and
    \cite[Appendix~C, lines~1333--1340]{Cirac2016MPDO_arXiv}.
\end{theorem}
...
\begin{theorem}[An idempotent physical-trace transfer without fixed-tensor
    ...
    This tensor corresponds to the $p=1/2$ model of
    \cite[lines~709--838]{Kato2024Exact} conjugated by Hadamard matrices on each
    physical site. While its physical-trace transfer is idempotent, the one- and
    two-site physical closures prevent any trace-preserving completely positive
    coarse-graining channel from satisfying $\mathcal S[K_2(X)]=K_1(X)$.
    Canonical form and area-law saturation for $K$ are established in
    Theorem~\ref{thm:kato_deformed_mpdo_cf_sal_capstone}.
\end{theorem}

\begin{proof}
    \leanok
    \uses{lem:mpdo_trace_loop_chain, def:cpsv_example412_site_sign, def:cpsv_example412_configuration_sign}
...
\begin{theorem}[A canonical-form, area-law-saturating MPDO without
    ...
    The canonical-form decomposition follows
...
\begin{proof}
    ...
    fixed point. Together with canonical form and area-law saturation, this
    proves the claim.
\end{proof}
...
\begin{proof}
    \leanok
    Apply Theorem~\ref{thm:rfp_via_ts_physical_trace_idempotent} and take the
    positive scalar in Definition~\ref{def:mpo_source_zcl} to be $1$.
\end{proof}
...
\begin{definition}[Existence of a purifying fixed-point tensor]
    ...
    For an MPO tensor $M$, require the existence of a spin--ancilla tensor $A$
    such that $\rho^{(N)}(M)=\rho^{(N)}_p(A)$ for every $N>0$ and
    $\widehat A$ is a pure-state renormalization fixed point.
    This condition records the density-operator equation of
...
\begin{definition}[Trace-preserving spin reduction]
    ...
    \cite[``Trace-preserving completely positive map'']{QICLean2026Blueprint}.
    This formalizes the structure described immediately after Definition~4.3 in
...
\begin{definition}[Purification renormalization fixed point]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    and $\widehat A$ is a pure-state renormalization fixed point. This defines a
    purification renormalization fixed point (PRFP) in the one-site
    presentation of \cite[lines~744--763]{Cirac2016MPDO_arXiv}.
\end{definition}
...
\begin{lemma}[Physical-trace transfer of the maximally mixed tensor]
    \label{lem:phys_trace_transfer_witnessM}
    \lean{MPOTensor.physTraceTransfer_witnessM}
    \leanok
    \uses{def:mpo_physical_trace_transfer}
    For the maximally mixed tensor $M_{\mathrm w}$, closing the ket and bra
    physical legs gives
...
\begin{theorem}[The maximally mixed tensor satisfies the nonzero
        physical-trace relation]
    \label{thm:witness_source_zcl}
    \lean{MPOTensor.isSourceZCL_witnessM}
    \leanok
    \uses{def:mpo_source_zcl, thm:prfp_not_doubled_index_zcl}
    The maximally mixed tensor $M_{\mathrm w}$ satisfies the nonzero
...
\begin{theorem}[A purification RFP has idempotent physical-trace transfer]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This establishes the forward implication from PRFP to literal zero
    correlation length \cite[Theorem~4.4]{Cirac2016MPDO_arXiv}.
\end{theorem}
...
\begin{theorem}[A local purification gives the global purification equation]
    ...
    For every purification renormalization fixed point $M$, its purifying
    tensor $A$ satisfies $\rho^{(N)}(M)=\rho^{(N)}_p(A)$ for every $N>0$,
    and $\widehat A$ is a pure-state renormalization fixed point.
\end{theorem}
...
% Gaps: docs/paper-gaps/cpsv16_purification_rfp_definition.tex and
...
\begin{theorem}[Purification fixed-point equivalence]
    ...
    \uses{def:mpdo_is_prfp, def:mpo_physical_trace_idempotence}
    \notready
    For an MPO tensor with the one-site purification presentation of
    \cite[lines~744--763]{Cirac2016MPDO_arXiv}, Theorem~4.4 asserts the
    equivalence of three conditions: the purifying tensor is a pure-state
    renormalization fixed point, the physical-trace transfer is idempotent, and
    the generated density operators have the repeated-copy form of
    \cite[lines~765--773]{Cirac2016MPDO_arXiv}.

    The equivalence between the first two conditions is
    Theorem~\ref{thm:prfp_iff_lpdo_physical_trace_idempotent}, and
    Theorem~\ref{thm:prfp_has_purification_rfp_witness} yields the global
    purification equation. The repeated-copy formula involves phases indexed by
    $(j,q)$, while the physical isometry of \cite[lines~546--563]{Cirac2016MPDO_arXiv}
    depends only on $j$; the physical embedding of the repeated copies is
