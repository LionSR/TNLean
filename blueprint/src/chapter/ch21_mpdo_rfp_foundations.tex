...
\begin{definition}[Bare global purification RFP]
    ...
    \cite[lines~756--764]{Cirac2016MPDO_arXiv}; the repaired nondegenerate
...
\begin{theorem}[Fixed local purification: pure RFP iff normalized physical-trace idempotence]
    ...
    \leanok
    \uses{def:lpdo, def:mpdo_purification_tensor,
    ...
    \]
    Then $A$ is a pure-state renormalization fixed point if and only if
    \[
...
\begin{proof}
    \leanok
    Let $K'$ be the transfer matrix of $A$. The local contraction identifies
    $\mathcal T_M$ with $K'$ after two bijective reindexings: interchange of
    the two Kronecker factors and the bond identification $e$. Since bijective
    reindexing commutes with matrix multiplication,
    \[
        (K')^2=K'
...
\begin{proof}
    \leanok
    The repaired predicate supplies both the local purification RFP condition
