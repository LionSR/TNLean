...
\begin{theorem}[Printed purification fixed-point equivalence]
    ...
    \notready
    \uses{def:mpdo_global_purification_equation, def:mpdo_is_prfp, def:mpo_source_zcl, def:mpdo_purification_rfp_witness, thm:global_prfp_not_source_zcl, thm:nonzero_global_prfp_not_source_zcl}
    ...
    The global positive-length equations do not detect nilpotent hidden bond
    sectors.  Theorems~\ref{thm:global_prfp_not_source_zcl} and
    \ref{thm:nonzero_global_prfp_not_source_zcl} give purification fixed points,
    including a nonzero matrix product density operator, whose physical-trace
    transfer fails the source zero-correlation relation.  Hence the stated
    equivalence is false for the global positive-length condition.
\end{theorem}
