...
\begin{definition}[Nonzero purification density]
    \label{def:mpdo_nonzero_purification_density}
    \lean{MPOTensor.HasNonzeroPurificationDensity}
    \leanok
    \uses{def:mpdo_purification_density}
    A purifying tensor has nonzero purification density if
    \[
        \rho^{(N)}_p(A)\ne 0
    \]
    for at least one positive length $N$. This excludes the degenerate zero
    purification, which is not part of the source PRFP--ZCL comparison.
\end{definition}
...
\begin{definition}[Purification RFP witness]
    ...
    \leanok
    \uses{def:mpdo_global_purification_equation,
        def:mpdo_nonzero_purification_density, def:is_rfp}
    ...
    \]
    The reduced density $\rho^{(N)}_p(A)$ is required to be nonzero for at least
    one positive length.
...
\begin{definition}[Purification RFP]
    ...
    \]
    with nonzero reduced density for at least one positive length, and
    $\widehat A$ is a pure-state renormalization fixed point. This is
    Definition~4.4 of \cite[lines~756--764]{Cirac2016MPDO_arXiv}, with the
    degenerate zero purification excluded.
\end{definition}
