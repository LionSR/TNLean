...
\begin{definition}[Global-family purification RFP witness]
    ...
    point. This auxiliary condition packages the density-operator equation at
...
\begin{definition}[Purification renormalization fixed point]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    and $\widehat A$ is a pure-state renormalization fixed point. Definition~4.3
    literally refers to the positive-length purification equation, but the
    surrounding discussion continues with the one-site presentation displayed
    in \cite[lines~744--763]{Cirac2016MPDO_arXiv}. Here PRFP denotes that
    standing one-site presentation; the positive-length equation is its MPDO
    interpretation.
\end{definition}
...
\begin{theorem}[A purification RFP has idempotent physical-trace transfer]
    ...
    length equation in Theorem~4.4 requires no nonzero assumption.
\end{theorem}
...
\begin{theorem}[Purification form and physical-trace idempotence characterize PRFP]
    ...
    \leanok
    \uses{def:mpdo_is_prfp, def:lpdo, thm:purification_tensor_rfp_iff_physical_trace_idempotent, thm:prfp_physical_trace_idempotent}
...
\begin{theorem}[Nondegenerate PRFP equivalence]
    ...
    \leanok
    \uses{def:is_nondegenerate_prfp, thm:prfp_iff_lpdo_physical_trace_idempotent}
    An MPO tensor is a nondegenerate purification renormalization fixed point if
...
\begin{theorem}[A nondegenerate purification RFP satisfies the nonzero
    ...
    \leanok
    \uses{def:is_nondegenerate_prfp, thm:source_zcl_of_prfp}
...
\begin{theorem}[Purification fixed-point equivalence]
    ...
    \uses{def:mpdo_is_prfp, def:mpo_physical_trace_idempotence, thm:prfp_iff_lpdo_physical_trace_idempotent, thm:prfp_has_purification_rfp_witness}
    \notready
