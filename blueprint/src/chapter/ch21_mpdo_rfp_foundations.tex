...
\begin{definition}[Existence of a purifying fixed-point tensor]
    ...
    This auxiliary condition records the density-operator equation of
    \cite[line~751]{Cirac2016MPDO_arXiv}, complementing the one-site
...
\begin{theorem}[Purification fixed-point equivalence]
    ...
    The repeated-copy formula uses phases indexed by $(j,q)$, whereas the
    physical isometry in \cite[lines~546--563]{Cirac2016MPDO_arXiv} is indexed
    only by $j$. The source does not specify how the copy index $q$ enters
    that isometry or the physical direct sum. Thus the displayed formulas
    do not determine a common physical realization of the repeated copies;
    this ambiguity is recorded in \cite{gap:cpsv16_rfp_isometry_scope}.
\end{theorem}
