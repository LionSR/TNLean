...
\begin{theorem}[Physical-trace transfer is idempotent under the local purification RFP condition]
    ...
    \leanok
    \uses{def:is_local_purification_rfp, def:mpo_physical_trace_transfer, def:mps_transfer_idempotence}
...
\begin{theorem}[Local purification form and transfer idempotence characterize a local purification RFP]
    ...
    \leanok
    \uses{def:is_local_purification_rfp, def:lpdo, thm:purification_tensor_rfp_iff_physical_trace_idempotent, thm:phys_trace_transfer_sq_of_local_purification_rfp}
...
\begin{theorem}[Nondegenerate PRFP equivalence]
    ...
    \leanok
    \uses{def:is_nondegenerate_prfp, def:lpdo, thm:local_purification_rfp_iff_lpdo_physical_trace_idempotent, thm:phys_trace_transfer_sq_of_local_purification_rfp}
    An MPO tensor is a nondegenerate purification renormalization fixed point if
...
\begin{proof}\leanok
    \uses{thm:phys_trace_transfer_sq_of_local_purification_rfp, thm:mpo_source_zcl_of_physical_trace_idempotent}
...
\begin{theorem}[A nondegenerate purification RFP satisfies the nonzero
    ...
    \leanok
    \uses{def:is_nondegenerate_prfp, thm:source_zcl_of_local_purification_rfp}
...
\begin{corollary}[A nondegenerate purification RFP satisfies the global PRFP predicate]
    ...
    \leanok
    \uses{def:is_nondegenerate_prfp, thm:is_prfp_of_local_purification_rfp}
    Every nondegenerate purification renormalization fixed point satisfies the
...
\begin{theorem}[Printed purification fixed-point equivalence]
    \label{thm:cpsv_theorem44_printed_status}
    \uses{def:mpdo_global_purification_equation, def:mpdo_is_prfp, def:mpo_physical_trace_idempotence, def:mpdo_purification_rfp_witness, thm:nonzero_global_prfp_not_physical_trace_idempotence}
    \notready
    Let an MPO tensor admit the global purification equation of
    Definition~\ref{def:mpdo_global_purification_equation}.  According to
    \cite[Theorem~4.4]{Cirac2016MPDO_arXiv}, being a purification
    renormalization fixed point, having zero correlation length, and admitting
    the displayed trace-preserving purification form are equivalent.

    This equivalence is false under the printed hypotheses.  Positive-length
    equations do not detect nilpotent hidden bond sectors.
    Theorem~\ref{thm:nonzero_global_prfp_not_physical_trace_idempotence} gives a
    nonzero matrix product density operator that is a purification fixed point
    but fails the literal physical-trace idempotence equation of
    Definition~\ref{def:mpo_physical_trace_idempotence}.  The corrected local
    purification equivalences above require a tensor-level purification identity
    and do not recover the printed global statement.
\end{theorem}
...
\begin{theorem}[The bare global PRFP predicate does not imply the nonzero
    ...
    \leanok
    \uses{def:mpdo_is_prfp, def:mpo_source_zcl, def:is_local_purification_rfp, thm:is_prfp_of_local_purification_rfp}
...
\begin{theorem}[A nonzero global PRFP need not satisfy the up-to-scalar
    ...
    \leanok
    \uses{def:mpdo, def:mpdo_is_prfp, def:mpo_physical_trace_transfer, def:mpo_source_zcl}
...
\begin{theorem}[A nonzero global PRFP need not satisfy literal physical-trace
        idempotence]
    \label{thm:nonzero_global_prfp_not_physical_trace_idempotence}
    \lean{MPOTensor.exists_isPRFP_isMPDO_physTraceTransfer_ne_zero_not_isPhysicalTraceIdempotent}
    \leanok
    \uses{def:mpdo, def:mpdo_is_prfp, def:mpo_physical_trace_transfer, def:mpo_physical_trace_idempotence}
    There is an MPO tensor which is an MPDO, satisfies the positive-length
    global purification RFP predicate, and has nonzero physical-trace transfer,
    but its physical-trace transfer is not idempotent.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:nonzero_global_prfp_not_source_zcl, thm:mpo_source_zcl_of_physical_trace_idempotent}
    Use the tensor from
    Theorem~\ref{thm:nonzero_global_prfp_not_source_zcl}.  If its nonzero
    physical-trace transfer $\mathcal T_M$ were idempotent, then
    \[
        \mathcal T_M^2=\mathcal T_M=1\cdot\mathcal T_M,
    \]
    which is the scalar-one case of the up-to-scalar relation refuted by that
    theorem.
\end{proof}
