...
\begin{figure}[ht]
    ...
    \caption{Zero correlation length as idempotence of the physical trace:
        closing the ket and bra legs of $M$ gives the transfer matrix
        $\mathcal T_M$, and the one-site transfer equals the virtual product
        of two copies, $\mathcal T_M=\mathcal T_M^2$. This is the defining
        picture of \cite[Definition~4.2, lines~736--741]{Cirac2016MPDO_arXiv};
        Definition~\ref{def:mpo_source_zcl} keeps this identity up to a
        positive scalar $\lambda$, and the drawn $\lambda=1$ case is
        Theorem~\ref{thm:mpo_source_zcl_of_physical_trace_idempotent}.}
    \label{fig:mpdo_zcl_idempotence}
\end{figure}
