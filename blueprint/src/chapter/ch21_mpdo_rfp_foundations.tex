...
\end{center}
Here closing the ket and bra legs gives $\mathcal T_M$, and juxtaposition is
virtual multiplication.  Thus the displayed identity is precisely
$\mathcal T_M=\mathcal T_M^2$.  This is the normalized identity in
...
\begin{lemma}[Forgetting the recorded spin reduction]
    ...
    \[
        \operatorname{PRFP}_{\mathrm{tp}}(M)
...
\begin{proof}
    ...
    \[
        (K')^2=K'
        \quad\Longleftrightarrow\quad
        \mathcal E_{\widehat A}\circ\mathcal E_{\widehat A}
          =\mathcal E_{\widehat A}.
    \]
\end{proof}
...
\begin{theorem}[A nondegenerate purification RFP has source ZCL]
    ...
    Every nondegenerate purification renormalization fixed point has source zero
    correlation length.
\end{theorem}
...
\begin{proof}\leanok
    Applying the mixed-product property $(A\otimes B)(C\otimes D)=(AC)\otimes(BD)$
    repeatedly, the length-$N$ product of the contracted tensor entries
    distributes over the ancillary index sum:
    \[
        \prod_{l=0}^{N-1}\Bigl(\sum_{k} A^{(\sigma_l,k)}\otimes\overline{A^{(\tau_l,k)}}\Bigr)
        = \sum_{\kappa\colon[N]\to[d_K]}
            \Bigl(\prod_{l} A^{(\sigma_l,\kappa_l)}\Bigr)
            \otimes \overline{\Bigl(\prod_l A^{(\tau_l,\kappa_l)}\Bigr)}.
    \]
    Tracing this sum and using
    $\operatorname{tr}(X\otimes\overline Y)
    =\operatorname{tr}(X)\,\overline{\operatorname{tr}(Y)}$ gives the coefficient
...
\begin{theorem}[The bare global PRFP predicate does not imply source ZCL]
    ...
    purification RFP predicate which does not have source zero correlation
    length.
\end{theorem}
