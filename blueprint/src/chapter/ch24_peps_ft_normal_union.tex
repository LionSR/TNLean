...
\begin{lemma}[Disjointness in the four-region decomposition]%
    ...
    \leanok
    \uses{def:peps_regionUnionDecomposition, lem:peps_regionUnionDecomposition_identities}
...
\begin{lemma}[Union of injective regions]%
    ...
    \notready
    \uses{def:IsVertexInjective, lem:peps_regionUnionDecomposition_identities, lem:peps_regionUnionDecomposition_disjoint, lem:peps_injectiveRegion_union_contractionChain}
    ...
    \begin{center}
        ...
        \begin{tenkz}[rows={wire}, cols=1, bonds=none]
            \tn[at=(1,1), name=unionLeft, label pos=w,
                ports={45:virtual, 0:virtual, -45:virtual,
                        90:physical}]{A\setminus B}
            \tn[at=1.3 ne of unionLeft, name=unionOverlap, label pos=e,
                ports={-135:virtual, -35:virtual, -70:virtual,
                        90:physical}]{A\cap B}
            \tn[at=3.5 e of unionLeft, name=unionRight, label pos=e,
                ports={180:virtual, 135:virtual, -135:virtual,
                        90:physical}]{B\setminus A}
            \tn[at=2.25 e of 1.4 s of unionLeft, name=unionOutside,
                label pos=s,
                ports={150:virtual, 110:virtual, 45:virtual,
                        90:physical}]{(A\cup B)^c}
            \tnwire{unionLeft.45}{unionOverlap.-135}
            \tnwire[name=unionAcross]{unionLeft.0}{unionRight.180}
            \tnwire{unionLeft.-45}{unionOutside.150}
            \tnwire{unionOverlap.-35}{unionRight.135}
            % The two bonds cross on the page without braiding; the
            % declared order is the drawing's, not the mathematics'.
            \tnwire[name=unionDown,
                cross={under at crossing of unionDown and unionAcross}]
            {unionOverlap.-70}{unionOutside.110}
            \tnwire{unionRight.-135}{unionOutside.45}
        \end{tenkz}
    \end{center}
    ...
    \begin{center}
        \tenkzkernel
        \begin{tenkz}[rows={wire}, cols=1, bonds=none]
            \tn[at=(1,1), name=proofZeroLeft, label pos=w,
                ports={45:virtual, 0:virtual, -45:virtual,
                        90:physical}]{P_0}
            \tn[at=1 ne of proofZeroLeft, name=proofZeroOverlap,
                label pos=e,
                ports={-135:virtual, -45:virtual, -90:virtual,
                        90:physical}]{P_1}
            \tn[at=2 e of proofZeroLeft, name=proofZeroRight,
                label pos=e,
                ports={180:virtual, 135:virtual, -135:virtual,
                        90:physical}]{P_2}
            \tn[at=1 se of proofZeroLeft, name=proofZeroX, label pos=s,
                ports={135:virtual, 90:virtual, 45:virtual}]{X}
            \tnwire{proofZeroLeft.45}{proofZeroOverlap.-135}
            \tnwire[name=proofAcross]{proofZeroLeft.0}{proofZeroRight.180}
            \tnwire{proofZeroLeft.-45}{proofZeroX.135}
            \tnwire{proofZeroOverlap.-45}{proofZeroRight.135}
            % The page crossing carries no braiding; the order is the
            % drawing's choice.
            \tnwire[name=proofDown,
                cross={under at crossing of proofDown and proofAcross}]
            {proofZeroOverlap.-90}{proofZeroX.90}
            \tnwire{proofZeroRight.-135}{proofZeroX.45}
        \end{tenkz}
        \,$=0$\quad$\xrightarrow{A^{-1}}$\quad
        \begin{tenkz}[rows={wire}, cols=1, bonds=none]
            \tn[at=(1,1), name=proofOneRight, label pos=e,
                ports={-135:virtual, 135:virtual, -90:virtual,
                        90:physical}]{P_2}
            % X stands off the -135 bearing of P_2, so the open -135 stub
            % of P_2 is not swallowed by the bond running down to X.
            \tn[at=1.2 s of 0.6 w of proofOneRight, name=proofOneX,
                label pos=s,
                ports={45:virtual, 90:virtual, 150:virtual}]{X}
            \tnwire{proofOneRight.-90}{proofOneX.45}
        \end{tenkz}
        \,$=0$\quad$\xrightarrow{A\cap B}$\quad
        \begin{tenkz}[rows={wire}, cols=1, bonds=none]
            \tn[at=(1,1), name=proofTwoOverlap, label pos=e,
                ports={-135:virtual, -90:virtual, -45:virtual,
                        90:physical}]{P_1}
            \tn[at=0.9 e of 0.75 s of proofTwoOverlap,
                name=proofTwoRight, label pos=e,
                ports={135:virtual, -135:virtual, 180:virtual,
                        90:physical}]{P_2}
            \tn[at=1.5 s of proofTwoOverlap, name=proofTwoX, label pos=s,
                ports={90:virtual, 45:virtual, 135:virtual}]{X}
            \tnwire{proofTwoOverlap.-90}{proofTwoX.90}
            \tnwire{proofTwoOverlap.-45}{proofTwoRight.135}
            \tnwire{proofTwoRight.-135}{proofTwoX.45}
        \end{tenkz}
...
\begin{lemma}[Union of two injective regions with positive bonds]%
    ...
    This extends Lemma~\ref{lem:peps_injectiveRegion_union} to arbitrary
    overlapping regions when all virtual bond dimensions are positive.
\end{lemma}
\begin{proof}\leanok
...
\begin{theorem}[Gauge preservation of blocked-region injectivity]
    ...
    \leanok
    \uses{def:applyGauge, thm:peps_regionBlockedWeight_applyGauge, thm:peps_sum_regionBoundaryCoupling_mul_inv}
...
\begin{theorem}[Union closure for a vertex-injective tensor]%
    ...
    \leanok
    \uses{def:IsVertexInjective, def:peps_regionInjectivityDataOf, thm:peps_regionBlockedTensorInjective_of_isVertexInjective}
...
\begin{theorem}[Union closure from the overlapping union lemma]%
    ...
    \leanok
    \uses{def:peps_regionInjectivityDataOf, lem:peps_injectiveRegion_union_pos}
...
\begin{lemma}[Indexed inside union under closure]%
    ...
    \leanok
    \uses{def:peps_regionInjectivityData, lem:peps_regionUnionDecomposition_identities}
