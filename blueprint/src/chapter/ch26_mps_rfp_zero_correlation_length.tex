...
\begin{lemma}[Positive-gap restriction, unfolded]%
    ...
    A tensor $A$ satisfies positive-gap zero correlation length if and only if
    \begin{align}
        \operatorname{IsBNT}(A) \land \operatorname{IsPositiveGapPhysicalCID}(A) \land \operatorname{IsBNTLocallyOrthogonal}(A).
        \notag
    \end{align}
\end{lemma}
\begin{proof}\leanok
...
\begin{theorem}[Two source gaps in the zero-correlation-length characterization]
    ...
    The source derivation of the forward implication uses $\E_A^n=\E_A$, which
    requires $n\geq1$; adjacent regions insert $\E_A^0=\Id$, creating the gap
    documented in \cite{gap:cpsv16_pure_zcl_adjacent_gap_cid_scope}.
...
\begin{lemma}[Source BNT zero correlation length implies its positive-gap form]%
    ...
    If a BNT family $A$ has zero correlation length for all disjoint physical regions, then
    \begin{align}
        \operatorname{IsPhysicalBNTZCL}(A) \implies \operatorname{IsPositiveGapBNTZCL}(A).
        \notag
    \end{align}
\end{lemma}
\begin{proof}\leanok
