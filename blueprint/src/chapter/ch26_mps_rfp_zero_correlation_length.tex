...
\begin{theorem}[Two source gaps in the zero-correlation-length characterization]
    ...
    space of a nearest-neighbor commuting parent Hamiltonian
    \cite[Theorems~3.8 and~3.10]{Cirac2016MPDO_arXiv}.
    Both directions of the biconditional fail as printed, and with them the
    ...
    literal CPSV canonical-form tensor. Its transfer map is idempotent, so it is
    a renormalization fixed point, but its adjacent two-region expectation is
    $1$, while the expectation after an allowed one-site shift is $0$.
    Hence transfer idempotence does not imply physical zero correlation length
    for the unrestricted quantification over disjoint regions, regardless of
    the commuting-parent condition.
...
\begin{proof}
    ...
    and with it the implication from fixed points to zero correlation length.
\end{proof}
...
\begin{remark}[Remaining obstructions in the printed converse]
    ...
    insertions.  Moreover, the claim that $\E_j^2\ne\E_j$ yields a nonzero eigenvalue
    $\lambda$ with $|\lambda|<1$ omits non-idempotent operators with nilpotent
    Jordan parts at eigenvalue zero. At multiplicity one,
    Theorem~\ref{thm:positive_gap_bnt_zcl_unconditional_converse} establishes the
    converse using arbitrary trace probes, while
    Theorem~\ref{thm:zcl_subleading_pair_excludes_cid} analyzes the spectral
    case under the explicit hypothesis of a nonzero subleading eigenpair.
\end{remark}
