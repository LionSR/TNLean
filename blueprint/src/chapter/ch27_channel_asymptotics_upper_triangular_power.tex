...
\begin{lemma}[Noncommutative upper-triangular word bound]
    \label{lem:wolf_upper_triangular_word_bound}
    \lean{Matrix.wolf_eq_105_seminorm}
    \leanok
    Let $D\ge 1$, let $\Lambda,N\in M_D(\mathbb C)$, assume that $\Lambda$ is
...
\begin{lemma}[Upper-triangular power estimate]
    \label{lem:wolf_upper_triangular_power_estimate}
    \lean{Matrix.wolf_eq_103, Matrix.wolf_eq_103_refined}
    \leanok
    \uses{lem:wolf_upper_triangular_word_bound}
    Under the hypotheses of Lemma~\ref{lem:wolf_upper_triangular_word_bound},
...
\begin{proof}
    \leanok
    \uses{lem:wolf_upper_triangular_word_bound}
    Apply Lemma~\ref{lem:wolf_upper_triangular_word_bound}.  For
    ...
    $\nu(\Lambda)=0$ without an additional convention.  The formal statement
    therefore records the exact range in which the displayed natural-power
    derivation is meaningful; the source-scope defect is documented in
    \path{docs/paper-gaps/wolf_ch8_eq_8_103_negative_exponent.tex}.
\end{proof}
