\begin{lemma}[Last crossing-window coefficients from a block-diagonal chain vector]
    ...
    \leanok
    \uses{lem:block_diagonal_boundary_local_sum_mem, lem:block_diagonal_last_crossing_coefficients_from_sum_membership}
    Let $B=\bigoplus_j\mu_jA^j$, and assume $\mu_j\ne0$ for every block
    $j$. Suppose
    \begin{align}
        \psi
         & =
        \Gamma_{M+1}^{B}\!\left(\bigoplus_jX_j\right),
        \notag \\
        \psi
         & \in
        \mathcal G_{M+1,m+2}(B).
        \notag
    \end{align}
    ...
    $A^j_b(A^j_\rho A^j_a(\mu_j^{M+1}X_j)) = A^j_bE_jA^j_a$
...
\begin{lemma}[Last crossing-window component membership from a chain vector]
    ...
    \leanok
    \uses{lem:block_diagonal_boundary_local_sum_mem, lem:block_diagonal_last_crossing_component_membership_from_sum_membership}
    Let $B=\bigoplus_j\mu_jA^j$, and assume $\mu_j\ne0$ for every block
    $j$. Suppose
    \begin{align}
        \psi
...
\begin{lemma}[Boundary-crossing intervals from a right boundary-matrix identity]
    ...
    Assume $0<N$ and $L\le N$, and let the cyclic interval beginning at $i$
...
\begin{proof}
    \leanok
    \uses{lem:block_diagonal_boundary_crossing_matrix_identity}
    Let $E:=Y A^j_{\tau_a}\cdots A^j_{\tau_{i-1}}$. Then, for every word
    $\beta$ on the segment $0,\ldots,a-1$,
    \begin{align}
...
\begin{lemma}[Boundary-crossing cyclic intervals suffice]
    ...
    \leanok
    \uses{def:chain_ground_space, lem:block_diagonal_boundary_nonwrapping_component}
    Assume $0<N$ and $L\le N$. Fix block-diagonal boundary conditions
    $X_j$. Suppose that, for every block $j$, every cyclic interval beginning
...
\begin{proof}
    ...
    Lemma~\ref{lem:block_diagonal_boundary_nonwrapping_component}. In the
    second case, use the displayed hypothesis.
\end{proof}
...
\begin{lemma}[Right boundary-matrix identities give single-block periodic-boundary constraints]
    ...
    \leanok
    \uses{lem:block_diagonal_boundary_crossing_right_boundary_matrix, lem:block_diagonal_boundary_component_crossing_windows_suffice}
    Assume $0<N$ and $L\le N$. Fix block-diagonal boundary conditions
...
\begin{lemma}[Spanning matrix identities give single-block constraints]
    ...
    \leanok
    \uses{thm:word_span_common_boundary_matrix, lem:block_diagonal_boundary_component_right_boundary_matrices}
    Assume $0<N$ and $L\le N$, and assume that, for every block $j$, the
    length-$N-L$ word products of $A_j$ span the full matrix algebra. Suppose
    that, for every block $j$, every boundary-crossing interval beginning at
    $i$, and every outside word $\rho$ of length $N-L$, there is a matrix
    $E_{j,i,\rho}$ such that, for every word $\beta$ before the cut of length
    $i+L-N$,
...
\begin{lemma}[Matrix identities under block injectivity give single-block constraints]
    ...
    \leanok
    \uses{lem:block_diagonal_boundary_component_complementary_word_identities, thm:wordspan_propagation_unital}
    Assume $0<N$, $L\le N$, and $L+L_0\le N$. Suppose each block $A_j$
    is $L_0$-block-injective and satisfies the normalization equation
    $\sum_a A^j_aA^{j\dagger}_a=\Id$. Suppose also that the boundary-crossing
    matrix identities of
    Lemma~\ref{lem:block_diagonal_boundary_component_complementary_word_identities}
    hold for every outside word $\rho$ of length $N-L$. Then, for every block
    $j$,
    $\Gamma_N^{A_j}(\mu_j^NX_j) \in \mathcal G_{N,L}(A_j)$.
\end{lemma}
...
\begin{lemma}[Normalization for words]
    ...
    If $\sum_a A_aA_a^\dagger=\Id$, then, for every length $M$,
...
\begin{lemma}[Compatibility hypotheses under block injectivity give single-block constraints]
    ...
    \leanok
    \uses{lem:sum_evalWord_mul_conjTranspose_evalWord, lem:complementary_word_boundary_identities_from_pgvwc, lem:block_diagonal_boundary_component_complementary_word_identities_injective}
    Assume $0<N$, $L\le N$, and $L+L_0\le N$. Suppose each block $A_j$
    is $L_0$-block-injective and satisfies
    $\sum_a A^j_aA^{j\dagger}_a=\Id$. Fix block-diagonal boundary conditions
    $X_j$. For every boundary-crossing interval beginning at $i$, suppose
    there are matrices $C^j_{i,\rho}$, indexed by outside words $\rho$ of
    length $N-L$, such that, for every word $\beta$ before the cut of length
    $i+L-N$,
...
\begin{proof}
    ...
    $\sum_\rho A^j_\rho A^{j\dagger}_\rho = \Id$
...
\begin{lemma}[Boundary representation and crossing constraints give periodic components]
    ...
    \leanok
    \uses{thm:block_diagonal_boundary_crossing_pgvwc_comparison_chain_ground_space, lem:block_diagonal_boundary_component_pgvwc_comparison_injective}
    Assume $0<N$, $L\le N$, and $L+L_0\le N$. Suppose each block $A_j$
    is $L_0$-block-injective and satisfies
    $\sum_a A^j_aA^{j\dagger}_a=\Id$. Let
    \begin{align}
        \psi
         & \in
...
\begin{lemma}[Boundary trace comparisons give single-block constraints]
    ...
    \leanok
    \uses{lem:pgvwc_cde_identities_from_trace_decompositions, lem:block_diagonal_boundary_component_complementary_word_identities_injective}
    Assume $0<N$, $L\le N$, and $L+L_0\le N$. Suppose each block $A_j$
    is $L_0$-block-injective and satisfies
    ...
    product algebra $\prod_j\MN{D_j}$. Fix complex numbers $\mu_j$ and
    block-diagonal boundary conditions $X_j$. For every boundary-crossing
    ...
    $\beta$ before the cut, outside word $\rho$, and middle word $w$,
    \begin{align}
        \sum_j\tr\!\left(A^j_\beta C^j_{i,\rho}A^j_w\right)
         & =
        \sum_j
...
\begin{proof}
    ...
    \uses{lem:pgvwc_cde_identities_from_trace_decompositions, lem:block_diagonal_boundary_component_complementary_word_identities_injective}
    Applying
    Lemma~\ref{lem:pgvwc_cde_identities_from_trace_decompositions}
    with the boundary matrix $D^j_\beta=(\mu_j^NX_j)A^j_\beta$, for every
    block $j$ and every boundary-crossing interval $i$, gives a matrix
...
\begin{lemma}[Boundary trace comparisons with a block-diagonal representation]
    ...
    $\psi$ has such a representation, there are matrices $C^j_{i,\rho}$ such
    that, for every boundary-crossing interval $i$, every outside word $\rho$
    of length $N-L$, every word $\beta$ of length $i+L-N$ before the cut, and
    every word $w$ of length $m$,
    \begin{align}
        \sum_j\tr\!\left(A^j_\beta C^j_{i,\rho}A^j_w\right)
         & =
        \sum_j
