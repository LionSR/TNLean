...
\begin{lemma}[Bond-dimension equality from normal torus rectangular injectivity]%
    \label{lem:peps_bondDim_eq_of_normalTorusRectangularInjectivity}
    ...
    \begin{proof}
        \leanok
        The proof constructs horizontal and vertical reference edges and
        builds one-edge blocking data for each tensor at these edges. The
        three-region blocking around each reference edge supplies an injective
        chain, and the one-edge isomorphism argument forces the bond dimensions
        at the reference edges to agree. Translation invariance then propagates
        the equality to every edge of the torus.
    \end{proof}
\end{lemma}
