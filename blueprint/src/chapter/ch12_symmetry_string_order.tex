...
nonidentity unitary string~\cite{PerezGarcia2008StringOrder}.  The formal
development also studies a restricted virtual-boundary nondecay predicate and
a virtual local-covariance relation.  These notions are stated separately
below; equivalence with physical state symmetry requires additional bridges.
...
\begin{definition}[Stationary-boundary block-twist functional]\label{def:string_order_param}
    ...
    The formal \emph{stationary-boundary block-twist functional} is
    \begin{align}
...
\begin{definition}[Periodic finite-chain overlap of PGWSVC08]
    ...
    functional $T_L(A,u,\Lambda)$ requires a thermodynamic-boundary or
    normalization bridge that is not presently formalized.  Thus
...
\begin{proof}
    ...
    $\E_A(V^\dagger V)=V^\dagger V$, hence purity forces
    $V^\dagger V=\Id$.  Applying the same equality argument to two peripheral
    eigenvectors shows that their quotient is a peripheral fixed direction of
    $\E_A$; purity makes that direction unique, proving the final assertion.
\end{proof}
...
\begin{theorem}[PGWSVC08 Theorem 1: physical string-order criterion]
    ...
    \end{align}
    In the present development, the transfer criterion is treated on the
    stronger one-site-injective surface.  The endpoint-realizability map from
    arbitrary virtual matrices to physical endpoint operators is not yet
    formalized, so the literal physical equivalence remains open.
\end{theorem}

\begin{proof}
    Lemma~\ref{lem:pgwsvc08_twisted_spectral_radius} shows that a nonzero limit
    requires a unique peripheral eigenvalue.  If $V$ and $Y$ are its right and
    left eigenmatrices, the canonical fixed-point identities give
    $Y=\Lambda V^\dagger$, and the asymptotic correlator factors as
    \begin{align}
       \tr\!\bigl(Y\E_y(\Id)\bigr)
       \tr\!\bigl(\Lambda\E_x(V)\bigr).
       \notag
    \end{align}
    Choosing $y=x^\dagger\widetilde u$ reduces nonvanishing to the second
    condition in (\ref{eq:pgwsvc08_theorem1}); if it holds for $n,m$, take
    $x=|n\rangle\langle m|$.  The reverse implication follows by the same
    factorization.  Formal completion requires the physical endpoint bridge
...
\begin{theorem}[PGWSVC08 Theorem 2: physical state symmetry]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The formal theorem below proves the corresponding virtual local-covariance
    criterion for a one-site-injective tensor; the bridge from that predicate
    to invariance of all physical reduced states is absent.
\end{theorem}
...
\begin{definition}[Virtual local covariance with boundary invariance]\label{def:local_symmetry}
    ...
    \leanok
    \uses{def:mps_tensor}
    The formal virtual local-covariance predicate holds for $A,u,\Lambda$ if
...
\begin{definition}[Virtual-boundary nondecay]\label{def:has_string_order}
    ...
    The formal virtual-boundary nondecay predicate holds for $u$ and
...
\begin{theorem}[Transfer-map covariance implies local physical intertwining]%
    ...
    The formal signature assumes that $A$ is one-site injective, that
    $VV^\dagger=\Id$, and that the transfer map is covariant.  It concludes
    that there is $u$ with $uu^\dagger=\Id$ satisfying the local physical
    intertwining relation.  The injectivity argument is unused, so the proof
    establishes the stronger implication under covariance and
    $VV^\dagger=\Id$ alone, although the public declaration still carries the
    redundant injectivity hypothesis.
\end{theorem}
