...
String order parameters probe symmetry-protected topological phases through
non-local correlators with a unitary string and endpoint operators
\cite{PerezGarcia2008StringOrder}. For injective canonical matrix product
states, this physical behavior provides a diagnostic correspondence with
virtual-boundary nondecay and virtual local covariance.
...
\begin{definition}[Periodic finite-chain symmetry overlap]
    ...
    \end{align}
    Under thermodynamic-boundary and normalization hypotheses, the normalized
    stationary-boundary functional $T_L(A,u,\Lambda)$ corresponds to this
    periodic overlap.
\end{definition}
...
\begin{definition}[Physical endpoint string order]
    ...
    Definition~\ref{def:has_physical_string_order_with} holds.
\end{definition}

\begin{lemma}[Twisted spectral-radius bound and peripheral rigidity]
    \label{lem:pgwsvc08_twisted_spectral_radius}
    \notready
    \uses{def:twisted_transfer_map}
    This is Lemma~1 of~\cite{PerezGarcia2008StringOrder}.  Let $A$ represent
    a pure finitely correlated state in the canonical gauge
...
\begin{theorem}[Physical endpoint criterion for string order]
    \label{thm:pgwsvc08_physical_string_order}
    \notready
    \uses{def:has_physical_string_order, lem:pgwsvc08_twisted_spectral_radius}
    This is Theorem~1 of~\cite{PerezGarcia2008StringOrder}.  A pure finitely
    correlated state has physical string order if and only if
    ...
    \end{align}
    For an injective tensor, sufficiently long physical endpoint operators realize
    the corresponding virtual boundary matrices.
\end{theorem}
...
\begin{theorem}[Physical state symmetry and twisted spectral radius]
    \label{thm:pgwsvc08_physical_symmetry}
    \notready
    \uses{lem:pgwsvc08_twisted_spectral_radius, thm:psd_fp_unique}
    This is Theorem~2 of~\cite{PerezGarcia2008StringOrder}.  For a pure
    finitely correlated state and a unitary $u\ne\Id$, the physical
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    For one-site-injective tensors, this physical symmetry is equivalent to
    virtual local covariance.
\end{theorem}

\begin{proof}
    If $\rho(\E_u)=1$, Lemma~\ref{lem:pgwsvc08_twisted_spectral_radius} gives
    a unitary $V$ satisfying the local intertwining relation.  Substitution of
    that relation into the adjoint transfer map, together with
    $\E_A^\dagger(\Lambda)=\Lambda$, gives
    $\E_A^\dagger(V\Lambda V^\dagger)=V\Lambda V^\dagger$.
    This positive fixed point has trace one, so uniqueness gives
    $V\Lambda V^\dagger=\Lambda$, equivalently
...
\begin{definition}[Virtual-boundary nondecay]\label{def:has_string_order}
    ...
    This virtual-boundary condition asserts the existence of boundary matrices
    with non-vanishing transfer overlap under the twist $u$. The relationship
    with physical endpoint operators (Definition~\ref{def:has_physical_string_order})
    is analyzed in \cite{gap:pgwsvc08_string_order_virtual_boundary}.
\end{definition}
...
\begin{theorem}[Transfer-map covariance implies local physical intertwining]%
    ...
    Covariance and coisometry of $V$ alone suffice for the conclusion.
\end{theorem}
...
\begin{proof}
    ...
    (the corresponding theorem in the companion quantum-channel volume~\cite[``Rectangular Kraus freedom'']{QICLean2026Blueprint}) converts
...
\begin{proof}
    \leanok
    \uses{lem:unitary_kraus_mixing, lem:twisted_transfer_eq_mixed_transfer, thm:left_canonical_gauge, thm:eigenvalue_bound}
...
\begin{theorem}[Modulus-one twisted eigenvalue implies gauge-phase equivalence]%
    ...
    \leanok
    \uses{def:twisted_transfer_map, def:twisted_mixed_companion, def:gauge_phase_equiv, def:injective}
    Let $A$ be an injective MPS tensor with $\E_A(\Id)=\Id$, and let $u$
    satisfy $u u^\dagger = \Id$.
    If
    \begin{align}
...
\begin{proof}
    \leanok
    \uses{lem:unitary_kraus_mixing, lem:twisted_transfer_eq_mixed_transfer, thm:left_canonical_gauge, thm:modulus_one_gauge_irr_tp}
    By Lemma~\ref{lem:twisted_transfer_eq_mixed_transfer}, the twisted transfer
...
\begin{theorem}[Virtual-boundary nondecay gives companion gauge-phase equivalence]%
    ...
    \leanok
    \uses{def:has_string_order, def:twisted_mixed_companion, def:gauge_phase_equiv, def:injective}
...
\begin{proof}
    \leanok
    \uses{lem:boundary_string_order_tendsto_zero, thm:twisted_transfer_spectral_radius_le_one, thm:twisted_transfer_modulus_one_gauge_phase}
...
\begin{proof}
    \leanok
    \uses{thm:twisted_transfer_eigen_of_virtual_unitary, def:string_order_boundary_param}
...
\begin{theorem}[Virtual local covariance iff a unitary peripheral eigenmatrix exists]%
    ...
    \leanok
    \uses{def:local_symmetry, def:twisted_transfer_map, def:injective, thm:twisted_transfer_spectral_radius_le_one}
...
\begin{proof}
    \leanok
    \uses{thm:twisted_transfer_eigen_of_virtual_unitary, thm:twisted_transfer_modulus_one_gauge_phase, thm:virtual_unitary_of_twisted_gauge_phase, thm:boundary_state_invariant_of_virtual_unitary}
...
\begin{proof}
    \leanok
    \uses{thm:string_order_implies_twisted_gauge_phase, thm:virtual_unitary_of_twisted_gauge_phase, thm:boundary_state_invariant_of_virtual_unitary, lem:has_string_order_of_local_symmetry}
...
\begin{proof}
    \leanok
    \uses{thm:string_order_implies_twisted_gauge_phase, thm:virtual_unitary_of_twisted_gauge_phase}
