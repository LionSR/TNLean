...
$A^{i_1}\cdots A^{i_p}$. We distinguish the number $N$ of blocked sites from
the number $Np$ of original sites.
...
\begin{lemma}[Local spaces under coisometric reconstruction]
    ...
    \leanok
    \uses{def:ground_space_parent, thm:block_sum_local_ground_space}
...
\begin{proof}
    ...
    Theorem~\ref{thm:block_sum_local_ground_space} gives the sum over all
