...
\begin{theorem}[Sharp blocking gives a nearest-neighbour parent Hamiltonian]
    ...
    \leanok
    \uses{thm:cpsv_literal_propblockinj, def:blocked_tensor, def:parent_hamiltonian_ground_space_spanning}
    ...
    $1\leq p\leq 3D^5$ such that, for every $N>2$,
    \begin{align}
    ...
    \end{align}
    Thus the actual blocked tensor has a nearest-neighbour parent Hamiltonian
...
\begin{proof}
    \leanok
    \uses{thm:cpsv_literal_propblockinj}
    ...
    Block injectivity identifies each periodic chain ground space with the
    corresponding matrix product vector. The direct-sum intersection theorem
    then identifies the parent-Hamiltonian kernel with the span on the
    right-hand side of
    (\ref{eq:cpsv_blocked_nearest_neighbor_parent_hamiltonian}). Finally,
    repeated phase-gauge copies and the rectangular ambient coisometry preserve
    every positive-length local space, so the equality transports from the
    direct sum to $A^{[p]}$.
\end{proof}
