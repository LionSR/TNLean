...
\begin{lemma}[Boundary equations from a common complement word]
    ...
    \[
        \Res^{\rho}_{i,L_0+1}\psi=\Gamma_{L_0+1}(Y_\rho),
        \qquad
        \Res^{\tau}_{i,L_0+1}\psi=\Gamma_{L_0+1}(Y_\tau).
    \]
...
\begin{proof}
    ...
    \end{align}
    Multiply equations~(\ref{eq:ph_first_letter_plus}) and
    (\ref{eq:ph_first_letter_minus}) on the right by $A^\sigma$ and use
    the assumed product equation for the chosen pair $j,\sigma$.
\end{proof}
