...
\begin{proof}
    ...
    \begin{align}
        ...
        &=
        Y_M(\tau^+_\eta(\mu))A^j,
        \label{eq:ph_first_letter_transport_plus}\\
        Y_{M+1-L_0}(\rho^-_{j,\sigma})A^j
        &=
        Y_{M+1-L_0}(\tau^-_\eta(\mu))A^j.
        \label{eq:ph_first_letter_transport_minus}
    \end{align}
    Multiply \eqref{eq:ph_first_letter_transport_plus} and
    \eqref{eq:ph_first_letter_transport_minus} on the right by $A^\sigma$ and use
