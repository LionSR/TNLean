...
For an injective tensor with $1<L\le N$ and $N\ge2$ (or an $L_0$-block-injective
tensor with $L_0>0$, $L_0<L\le N$, and $N\ge2$), the periodic matrix product
vector $V^{(N)}(A)=\Gamma_N(\Id)$ satisfies every cyclic window constraint
$\mathcal G_{N,L}(A)$. Together with the boundary-matrix commutation
$X A^j=A^jX$, this establishes that the periodic ground space is spanned by
$V^{(N)}(A)$.
