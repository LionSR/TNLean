...
forces $X$ to commute with every $A^j$ via the boundary-crossing
compatibilities $C^+_\tau A^j X=Y_\tau A^j$ and $X A^j C^-_\tau=A^jY_\tau$.
...
For an injective tensor with $D\ge1$, $1<L\le N$, and $N\ge2$ (or an
$L_0$-block-injective tensor with $D\ge1$, $L_0>0$, $L_0<L\le N$, and $N\ge2$),
the periodic matrix product vector $V^{(N)}(A)=\Gamma_N(\Id)$ satisfies every
cyclic window constraint $\mathcal G_{N,L}(A)$. Together with the
boundary-matrix commutation $X A^j=A^jX$, this establishes that the periodic
ground space is spanned by $V^{(N)}(A)$.
