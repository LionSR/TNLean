\section{The GHZ state}\label{sec:ghz}
%% ===================================================================

The Greenberger--Horne--Zeilinger (GHZ)
state~\cite{GreenbergerHorneZeilinger1989,Greenberger1990Bell} is the
prototypical non-injective, renormalization-fixed-point
MPS~\cite[Section~III.A]{Cirac2021Matrix}. On a $d$-level system it is an MPS
with bond dimension $d$~\cite[Appendix~A, ``The GHZ state'']{Cirac2021Matrix}.

\begin{definition}[GHZ tensor on a $d$-level system]\label{def:ghz_tensor_d}
    \lean{MPSTensor.ghzTensorD,
        MPSTensor.ghzTensorD_apply,
        MPSTensor.ghzTensorD_eq_diagonal}
    \leanok
    \uses{def:mps_tensor}
    Set $D = d$. The GHZ tensor on a $d$-level system is the family
    $\{A^i\}_{i=0}^{d-1}\subset\MN{d}$ with entries
    \begin{align}
        A^{i}_{\alpha\beta} & = \delta_{i=\alpha=\beta},
        \notag
    \end{align}
    that is, $A^i=\ketbra{i}{i}$~\cite[Appendix~A, ``The GHZ
        state'']{Cirac2021Matrix}.
\end{definition}

\begin{definition}[GHZ state]\label{def:ghz_state_d}
    \lean{MPSTensor.ghzStateD}
    \leanok
    The (unnormalized) GHZ state on $N$ sites of a $d$-level system is
    \begin{align}
        \ket{\mathrm{GHZ}} & = \sum_{i=0}^{d-1}\ket{i,i,\ldots,i}.
        \notag
    \end{align}
\end{definition}

\begin{theorem}[The GHZ state is an MPS]\label{thm:ghz_d_mpv}
    \lean{MPSTensor.mpv_ghzTensorD}
    \leanok
    \uses{def:ghz_tensor_d, def:ghz_state_d, def:mpv}
    For every $N$, the matrix product vector of the tensor of
    Definition~\ref{def:ghz_tensor_d} on $N$ sites is the GHZ state
    $\sum_{i=0}^{d-1}\ket{i,\ldots,i}$~\cite[Appendix~A, ``The GHZ
        state'']{Cirac2021Matrix}. For $N=0$ both sides equal $d$.
\end{theorem}

\begin{proof}
    \leanok
    A product $A^{\sigma_1}\cdots A^{\sigma_N}$ is the diagonal matrix whose
    $x$-th entry is $1$ if $\sigma_1=\cdots=\sigma_N=x$ and $0$ otherwise; its
    trace counts the $x$ with this property.
\end{proof}

\begin{theorem}[GHZ transfer map on a $d$-level system]\label{thm:ghz_d_transfer}
    \lean{MPSTensor.ghzTensorD_transferMap,
        MPSTensor.ghzTensorD_isTransferIdempotent}
    \leanok
    \uses{def:ghz_tensor_d, def:transfer_map, def:mps_transfer_idempotence}
    The transfer map of the GHZ tensor on a $d$-level system extracts the
    diagonal, $\E_A(X)_{\alpha\beta}=\delta_{\alpha\beta}X_{\alpha\alpha}$. In
    particular $\E_A^2=\E_A$, so the tensor is a renormalization fixed point.
\end{theorem}

\begin{proof}
    \leanok
    Since $A^i=\ketbra{i}{i}$, the term $A^iX(A^i)^\dagger$ is
    $X_{ii}\ketbra{i}{i}$. Extracting the diagonal is idempotent.
\end{proof}

\begin{theorem}[GHZ tensor is not injective for $d\ge2$]\label{thm:ghz_d_not_injective}
    \lean{MPSTensor.ghzTensorD_not_isInjective}
    \leanok
    \uses{def:ghz_tensor_d, def:injective}
    For $d\ge2$ the GHZ tensor on a $d$-level system is not injective.
\end{theorem}

\begin{proof}
    \leanok
    Every $A^i$ has vanishing $(0,1)$ entry, while the all-ones matrix does not.
\end{proof}

\begin{theorem}[Permutation and $\Z_d$ on-site symmetry]\label{thm:ghz_d_symmetric}
    \lean{MPSTensor.ghzTensorD_isOnSiteSymmetric_perm,
        MPSTensor.ghzCyclicShiftAction,
        MPSTensor.ghzTensorD_isOnSiteSymmetric_cyclic}
    \leanok
    \uses{def:ghz_tensor_d, def:on_site_symmetric}
    Let the symmetric group on $\{0,\ldots,d-1\}$ act on $\C^d$ by permutation
    matrices. The GHZ tensor on a $d$-level system is on-site symmetric under
    this action. In particular, for $d\ge1$ it is on-site symmetric under the
    cyclic group $\Z_d$ acting by the cyclic shift
    $\ket{i}\mapsto\ket{i+1 \bmod d}$.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:ghz_d_mpv}
    The twisted tensor of a permutation $\pi$ is the relabelling
    $i\mapsto A^{\pi^{-1}(i)}$, so its matrix product vector is the GHZ state
    evaluated on the relabelled configuration, which equals the GHZ state.
\end{proof}

For $d=2$ the tensor of Definition~\ref{def:ghz_tensor_d} is the two-level GHZ
tensor of Definition~\ref{def:ghz_tensor}, which is the representative used in
the rest of this section.

\begin{definition}[Two-level GHZ tensor]\label{def:ghz_tensor}
    \lean{MPSTensor.ghzTensor,
        MPSTensor.ghzTensor_apply}
    \leanok
    \uses{def:mps_tensor}
    Set $d = D = 2$. The GHZ tensor is the family
    $\{A^0, A^1\} \subset \MN{2}$ defined by
    \begin{align}
        A^0 & = \ketbra{0}{0} = \diag(1,0),
        \qquad
        A^1 = \ketbra{1}{1} = \diag(0,1).
        \notag
    \end{align}
    For a system of $N$ sites, the resulting MPV is the GHZ state
    \begin{align}
        \ket{\mathrm{GHZ}_N}
         & = \ket{0}^{\otimes N} + \ket{1}^{\otimes N}.
        \notag
    \end{align}
\end{definition}

\begin{lemma}[The two-level case]\label{lem:ghz_tensor_d_two}
    \lean{MPSTensor.ghzTensorD_two}
    \leanok
    \uses{def:ghz_tensor_d, def:ghz_tensor}
    The GHZ tensor on a $d$-level system with $d=2$ is the tensor of
    Definition~\ref{def:ghz_tensor}.
\end{lemma}

\begin{proof}
    \leanok
    Both tensors have $A^i=\ketbra{i}{i}$ for $i\in\{0,1\}$.
\end{proof}

\begin{definition}[Two-site computational vector]\label{def:two_site_ket}
    \lean{MPSTensor.twoSiteKet}
    \leanok
    For $a,b\in\{0,1\}$, the vector
    $\ket{ab}\in(\{0,1\}^{2}\to\C)$ is the indicator of the configuration
    $(\sigma_1,\sigma_2)=(a,b)$.
\end{definition}

\begin{definition}[Constant product vector]\label{def:constant_ket}
    \lean{MPSTensor.constantKet}
    \leanok
    For $a\in\{0,1\}$ and $N\ge0$,
    \begin{align}
        \ket{a}^{\otimes N}(\sigma_1,\ldots,\sigma_N)
         & =
        \begin{cases}
            1, & (\sigma_1,\ldots,\sigma_N)=(a,\ldots,a), \\
            0, & \text{otherwise}.
        \end{cases}
        \notag
    \end{align}
\end{definition}

\begin{theorem}[GHZ transfer map is idempotent]\label{thm:ghz_transfer_idempotent}
    \lean{MPSTensor.ghz_transferMap_idempotent}
    \leanok
    \uses{def:ghz_tensor, def:transfer_map}
    The transfer map of the GHZ tensor satisfies $\E_A^2 = \E_A$.
\end{theorem}

\begin{proof}
    \leanok
    The transfer map acts by $\E_A(X)_{ij} = \delta_{ij} X_{ij}$
    (extraction of the diagonal). This operation is idempotent.
\end{proof}

\begin{theorem}[GHZ tensor is a renormalization fixed point]\label{thm:ghz_is_rfp}
    \lean{MPSTensor.ghz_isTransferIdempotent}
    \leanok
    \uses{def:ghz_tensor, def:mps_transfer_idempotence, thm:ghz_transfer_idempotent}
    The GHZ tensor is a renormalization fixed point.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:ghz_transfer_idempotent}
    The claim follows from Theorem~\ref{thm:ghz_transfer_idempotent}.
\end{proof}

\begin{theorem}[GHZ tensor is not injective]\label{thm:ghz_not_injective}
    \lean{MPSTensor.ghz_not_isInjective}
    \leanok
    \uses{def:ghz_tensor, def:injective}
    The GHZ tensor is not injective: $\spn_{\C}\{A^0, A^1\}$ is the
    two-dimensional space of diagonal matrices in~$\MN{2}$, which does
    not equal~$\MN{2}$.
\end{theorem}

\begin{proof}
    \leanok
    Every element of $\spn_{\C}\{A^0, A^1\}$ has zero off-diagonal
    entries, so the matrix with all entries equal to~$1$ is not in the
    span.
\end{proof}

\begin{theorem}[$\Z_2$ on-site symmetry of the GHZ tensor]\label{thm:ghz_z2_symmetric}
    \lean{MPSTensor.ghz_isOnSiteSymmetric_Z2}
    \leanok
    \uses{def:ghz_tensor, def:on_site_symmetric}
    Let $\Z_2 = \{1, \sigma_x\}$ act on $\C^2$ by the Pauli-$X$
    matrix~$\sigma_x$. The GHZ tensor is on-site symmetric under this
    action: for each $g \in \Z_2$, the twisted tensor $\widetilde A_g$
    defines the same MPV family as $A$, i.e.\
    $\mc V(A)=\mc V(\widetilde A_g)$.
\end{theorem}

\begin{proof}
    \leanok
    The identity element is trivial. For the generator
    $g = \sigma_x$, the twisted tensor satisfies
    $\widetilde A_g^i = A^{1-i}$, i.e.\ the two matrices are swapped.
    Conjugation by $\sigma_x$ implements the same swap, giving
    gauge equivalence and hence the same MPV family.
\end{proof}

\begin{definition}[Virtual $\Z_2$ representation of the GHZ tensor]\label{def:ghz_virtual_z2_action}
    \lean{MPSTensor.ghzVirtualZ2Action}
    \leanok
    \uses{def:ghz_tensor}
    The virtual $\Z_2$ representation on the bond space is the homomorphism
    $\Z_2 \to \MN{2}$ sending the identity to $I$ and the generator to
    $\sigma_z = \diag(1, -1)$.
\end{definition}

\begin{theorem}[Virtual $\Z_2$ symmetry of the GHZ tensor]\label{thm:ghz_virtual_z2}
    \lean{MPSTensor.ghz_virtual_Z2_symmetric}
    \leanok
    \uses{def:ghz_tensor, def:ghz_virtual_z2_action}
    The matrix $\sigma_z$ commutes with every GHZ matrix $A^i$. A non-scalar
    matrix commuting with all $A^i$ is the hallmark of a non-injective
    ($\Z_2$-injective) tensor: the on-site symmetry of
    Theorem~\ref{thm:ghz_z2_symmetric} swaps the two one-dimensional injective
    blocks $A^0 \leftrightarrow A^1$, while $\sigma_z$ acts on each block by a
    phase.
\end{theorem}

\begin{proof}
    \leanok
    Both $A^0$ and $A^1$ are diagonal, so each commutes with the diagonal
    matrix $\sigma_z$.
\end{proof}

\begin{theorem}[GHZ order parameter is a transfer-map fixed point]\label{thm:ghz_order_param}
    \lean{MPSTensor.ghz_orderParam_fixed}
    \leanok
    \uses{def:ghz_tensor, def:transfer_map}
    The order-parameter operator $\sigma_z$ is a fixed point of the GHZ transfer
    map, $\E_A(\sigma_z) = \sigma_z$.
\end{theorem}

\begin{proof}
    \leanok
    Since $\E_A(X) = \sum_i A^i X (A^i)^{\dagger}$ and both $A^i$ are diagonal,
    the claim is the entrywise computation $\E_A(\sigma_z) = \sigma_z$.
\end{proof}

\begin{remark}
    A primitive (injective) tensor has a one-dimensional transfer-map
    fixed-point space, spanned by a positive matrix that equals the identity only
    in a normalized gauge. That the GHZ transfer map fixes the additional
    non-scalar operator $\sigma_z$ is the transfer-matrix signature of the extra
    non-decaying mode behind its long-range ferromagnetic correlations.
\end{remark}

\begin{theorem}[GHZ zero correlation length]\label{thm:ghz_zcl}
    \lean{MPSTensor.ghz_isZCL}
    \leanok
    \uses{def:is_zcl, thm:ghz_is_rfp, thm:zcl_iff_idempotent_transfer}
    The GHZ tensor has zero correlation length: its idempotent transfer map
    (Theorem~\ref{thm:ghz_is_rfp}) gives correlations independent of the
    separation. This contrasts with the AKLT state
    (Theorem~\ref{thm:aklt_correlation_length}), whose subleading eigenvalue
    $-\tfrac13$ produces a finite correlation length $1/\log 3$.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:zcl_iff_idempotent_transfer, thm:ghz_is_rfp}
    The claim follows from the idempotence of the GHZ transfer map and the
    equivalence of zero correlation length with transfer-map idempotence
    (Theorem~\ref{thm:zcl_iff_idempotent_transfer}).
\end{proof}

%% ===================================================================
