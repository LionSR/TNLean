...
\begin{theorem}[Full-ring boundary comparison]
    ...
    Let $A$ be $L_0$-block-injective with $L_0>0$, and let
...
\begin{proof}
    ...
    Move the cut by $L_0$ sites and write the translated state as
    $\Gamma_{L_0+1}(Y)$. Splitting each full-ring word into an $L_0$-site word
    $\beta$ and one letter $j$ gives
    \[
        XA^j=A^jY.
    \]
    Repeating this comparison at all $L_0+1$ cuts moves every full-ring word
    around the trace and returns to the original cut. Hence $X$ commutes with
    every word of length $L_0+1$, and block injectivity makes $X$ scalar.
\end{proof}
...
\begin{theorem}[Minimal-ring ground-space containment]
    ...
    \leanok
    \uses{thm:full_ring_boundary_intertwines, def:mpv_submodule}
    If $A$ is $L_0$-block-injective with $L_0>0$, then
    \[
...
\begin{proof}
    ...
    If $N=L_0+1$, Theorem~\ref{thm:full_ring_ground_space_containment}
    gives the claim. If $N>L_0+1$, the open-chain containment gives
