...
\begin{theorem}[Fixed-length vector spanning implies matrix spanning]%
    ...
    \lean{Kraus.wordSpan_eq_top_of_vectorSpreadSpan_eq_top_of_rankOneBasis}
    \leanok
    \uses{def:word_span}
    Assume:
    \begin{enumerate}
...
\begin{proof}
    \leanok
    The blocked physical alphabet is a reindexing of words of length $L$.
    Decoding blocked indices therefore identifies the two spanning families,
    and hence the two injectivity conditions are equivalent.
\end{proof}

\begin{lemma}[Blocking transfer]\label{lem:blocking_transfer}
    \lean{MPSTensor.wordSpan_eq_top_of_blockTensor_wordSpan_eq_top}
    \leanok
    \uses{def:word_span, def:blocked_tensor, lem:isNBlkInjective_iff_blockTensor_isInjective}
...
\begin{theorem}[Bounded injective blocking for left-canonical normal tensors]%
    ...
    \leanok
    \uses{def:normal, def:blocked_tensor, thm:wielandt_general, thm:wielandt_proposition_three}
...
\begin{theorem}[Exact quantum-Wielandt blocking length]
    ...
    \leanok
    \uses{thm:bounded_injective_blocking_normal_left_canonical, lem:blocking_transfer, thm:wordspan_blk_inj_succ}
    Let $A$ be a left-canonical normal MPS tensor with bond dimension $D>0$.
...
\begin{corollary}[Uniform injective blocking for separated normal-canonical families]%
    ...
    \leanok
    \uses{thm:normal_tp_primitive_irred, thm:bounded_injective_blocking_normal_left_canonical}
    ...
    \textbf{Scope restriction (one-copy representative family):}
    Here the family is a basis of normal-canonical representatives in the sense
    of Definition~\ref{def:normal_cf_bnt}, with distinct blocks and
    non-increasing weight moduli. The representative family is assumed to have
    been selected; this statement does not reconstruct the full CPSV sector
    decomposition with repeated copies inside one sector. Documented in
    \cite{gap:cpsv16_ft_one_copy_scope_restriction}
    (\texttt{docs/paper-gaps/cpsv16_ft_one_copy_scope_restriction.tex}).
\end{corollary}
...
\begin{remark}[Quantitative and blocked rank-one arguments]
    ...
    blocked construction, Theorem~\ref{thm:wielandt_lemma2b}, which first
    extracts a rank-one operator in a blocked tensor and then applies the
    fixed-length matrix-spanning argument; its fixed-length conclusion is a
    consequence of its normality hypothesis, not the quantitative statement of
    source Lemma~2(b).
\end{remark}
...
\begin{definition}[Primitive MPS tensor]\label{def:primitive_mps}
    ...
    \leanok
    \uses{def:mps_tensor, def:transfer_map}
    Assume $D>0$.
...
\begin{theorem}[Normalizing the primitive fixed point to unit trace]
    \label{thm:primitive_fixed_point_trace_normalization}
    \lean{fixedPointProj_smul}
    \lean{Matrix.trace_inv_trace_smul}
    \lean{Matrix.PosDef.inv_trace_smul}
    \lean{MPSTensor.IsPrimitiveMPS.smul_inv_trace}
    \leanok
    \uses{def:primitive_mps}
    The fixed-point projection $P$ depends only on the ray through $\rho$: for
    every nonzero scalar $c$ with $\tr(c\rho)\neq0$, the projection associated
    to $c\rho$ equals the projection associated to $\rho$.  Write
    \begin{align}
        \sigma & =\frac{\rho}{\tr(\rho)}
        \label{eq:primitive_fixed_point_normalized}
    \end{align}
    for the normalized representative of the ray through a matrix of nonzero
    trace.  Then $\tr(\sigma)=1$; if $\rho>0$ then also $\sigma>0$; and if $A$
    is primitive with positive-definite fixed point $\rho$, then $A$ is
    primitive with fixed point $\sigma$.
\end{theorem}

\begin{proof}
    \leanok
    \uses{def:primitive_mps}
    Both projections send $X$ to $\tr(X)\rho/\tr(\rho)$, so they agree.  The
    trace of $\sigma$ is $\tr(\rho)/\tr(\rho)=1$, and a positive-definite
    matrix stays positive definite under multiplication by the positive real
    $\tr(\rho)^{-1}$.  For the gap certificate, the trace-preservation clause
    does not involve $\rho$; positivity, nonvanishing and the fixed-point
    equation are preserved by a positive real rescaling; and the complementary
    spectral radius is unchanged because the two projections coincide.
\end{proof}
...
\emph{subspaces} of $\C^D$ under a finite matrix family.

\begin{definition}[Algebra span]\label{def:alg_span}
    \lean{Matrix.algSpan}\leanok
    For a finite matrix family $A$, let $\algspn(A)$ be the unital
...
\begin{definition}[Invariant submodule]\label{def:invariant_submodule}
    \lean{Matrix.IsInvariantSubmodule}\leanok
...
\begin{definition}[Irreducible action]\label{def:irreducible_action}
    \lean{Matrix.IsIrreducibleAction}\leanok
    \uses{def:invariant_submodule}
    The matrices in $A$ act \emph{irreducibly} on $\C^D$ if every
...
\begin{theorem}[Irreducible tensor implies irreducible action]\label{thm:irreducible_tensor_to_action}
    \lean{Kraus.isIrreducibleAction_of_isIrreducibleFamily}\leanok
...
\begin{theorem}[Burnside's theorem for matrix algebras]\label{thm:burnside_matrix}
    \lean{Matrix.burnside_matrix}\leanok
...
\begin{lemma}[Primitive tensors with positive-definite fixed point are strongly irreducible]%
    ...
    \leanok
    \uses{def:primitive_mps}
    If $A$ is a primitive MPS tensor with positive-definite fixed point~$\rho$,
    then it satisfies the strengthened condition in
    Definition~\ref{def:strongly_irreducible_paper}: $\E_A$ is irreducible,
    the fixed point $\rho$ is positive definite, and the peripheral spectrum
    of $\E_A$ is $\{1\}$. The source clause is
    \cite[Proposition~3(c)]{Sanz2010Quantum}; the definition used here records
    irreducibility separately.
\end{lemma}
