...
\begin{theorem}[Normalizing the primitive fixed point to unit trace]
    ...
    to $c\rho$ equals the projection associated to $\rho$.  Consequently, if
    $A$ is primitive with positive-definite fixed point $\rho$, then $A$ is
    primitive with the normalized fixed point
    \begin{align}
        \sigma & =\frac{\rho}{\tr(\rho)},
        \qquad \sigma>0,\qquad \tr(\sigma)=1.
        \notag
    \end{align}
\end{theorem}
...
\begin{proof}
    ...
    trace-preservation clause does not involve $\rho$; positivity, nonvanishing
    and the fixed-point equation are preserved by a positive real rescaling;
    and the complementary spectral radius is unchanged because the two
