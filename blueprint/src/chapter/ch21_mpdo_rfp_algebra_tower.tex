...
\begin{definition}[Algebras of blocked adjoint fixed points]%
    ...
    if there exist support algebras and maps as above such that, for every
    positive blocking size $n$,
    \begin{align}
        \mathcal{A}_n & = \Fix(E_n^{\dagger}),
        \notag
    \end{align}
    where $E_n$ is the blocked transfer map. We call this equality
...
not the algebras $\Fix(E_n^\dagger)$ defined above.
...
\begin{remark}[Stabilized fixed points and the vertical algebra condition]%
    ...
    one-site and two-site vertical canonical forms is essential to the maps
...
matrices $\chi_{\alpha,\beta,\gamma}$. The definitions below allow arbitrary
complex diagonal entries. They describe the trace-power identity alone;
positivity and the product law for $O_L(M_\alpha)$ are separate conditions.
