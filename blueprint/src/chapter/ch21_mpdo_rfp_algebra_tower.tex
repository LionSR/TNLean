...
\begin{definition}[Blocked fixed-point-algebra tower]%
    ...
    An MPO tensor $M$ satisfies the \emph{blocked fixed-point-algebra tower} used
    here when there exist blocked fixed-point-algebra data such that, for every
...
\begin{theorem}[Doubled-index ZCL yields a stationary algebra tower under a faithful fixed point]%
...
\begin{theorem}[Adjoint fixed points descend through the algebra tower]%
    ...
    \leanok
    \uses{def:mpdo_blocked_fixed_point_algebra_tower}
    If an MPO tensor satisfies the blocked fixed-point-algebra tower, then for
...
\begin{proof}\leanok
    ...
    $E_0 = \id$. This step does not require the blocked fixed-point-algebra data.
\end{proof}
...
\begin{theorem}[Finite-order adjoint eigenvalues for algebra towers]%
    ...
    \leanok
    \uses{def:mpdo_blocked_fixed_point_algebra_tower, thm:mpdo_adjoint_fix_descent,
        lem:mpdo_adjoint_blocked_eigenvector}
    Assume the blocked fixed-point-algebra tower holds. If $X\ne 0$,
...
\begin{theorem}[Adjoint fixed-point equality for algebra towers]%
    ...
    If an MPO tensor satisfies the blocked fixed-point-algebra tower, then
...
\begin{theorem}[Algebra data give the stationary fixed-point tower]%
    ...
    point, if $M$ satisfies the blocked fixed-point-algebra tower, then the
...
\begin{theorem}[A blocked fixed-point-algebra tower need not be ZCL]%
    \label{thm:mpdo_algebra_tower_not_imply_zcl}
    \lean{MPOTensor.exists_hasBlockedAdjointFixedPointAlgebraTower_not_isZCL}
    \leanok
    \uses{def:mpdo_blocked_fixed_point_algebra_tower, def:mpo_is_zcl,
    ...
    positive-definite fixed point, and a blocked fixed-point-algebra tower,
    while the doubled-index transfer map is not idempotent.
\end{theorem}
\begin{proof}\leanok
    ...
    $\Fix((E^n)^\dagger)=\Fix(E^\dagger)$. Thus the blocked fixed-point-algebra tower exists. On the other hand,
    $E^2(X)_{01}=\frac{49}{625}X_{01}\neq-\frac7{25}X_{01}=E(X)_{01}$.
    Therefore $E^2\neq E$, so the doubled-index ZCL condition fails.
\end{proof}

\begin{remark}[Why the algebra tower does not imply doubled-index ZCL]%
    \label{rem:mpdo_algebra_tower_to_zcl_gap}
    \uses{def:mpdo_blocked_fixed_point_algebra_tower, def:mpo_is_zcl,
        thm:mpdo_algebra_tower_not_imply_zcl, thm:newton_girard}
    ...
    $E^2=E$, as Theorem~\ref{thm:mpdo_algebra_tower_not_imply_zcl} shows. More
    ...
    Theorem~\ref{thm:mpdo_algebra_tower_not_imply_zcl} shows that the
