...
\begin{theorem}[Positive per-block extensions are unitarily implemented]%
    \label{thm:positive_perBlockLinearExtension_unitary_conj}
    \lean{MPSTensor.exists_unitary_conj_of_positive_perBlockLinearExtension}
    \leanok
    \uses{def:perBlockLinearExtension}
    Let $T_k:\MN{D_k}\to\MN{D_k}$ be the per-block linear extension of an
    injective MPS family and a matched family with the same closed matrix
    product vectors.
    If $T_k$ is positive, then there is a unitary matrix $U_k$ such that
    \begin{align}
        T_k(M)&=U_kMU_k^\dagger
        \label{eq:positive_perBlockLinearExtension_unitary_conj}
    \end{align}
    for every $M\in\MN{D_k}$.
\end{theorem}
...
\begin{proof}\leanok
    ...
    The per-block extension is multiplicative and bijective. Positivity makes
    the resulting algebra automorphism preserve adjoints. The unitary
    Skolem--Noether theorem then gives
    (\ref{eq:positive_perBlockLinearExtension_unitary_conj}).
\end{proof}
