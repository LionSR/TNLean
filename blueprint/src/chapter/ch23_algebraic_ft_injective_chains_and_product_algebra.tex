...
\begin{theorem}[Injective-chain Fundamental Theorem up to a global phase]%
    ...
    \leanok
    \uses{thm:fundamentalTheorem_injective_chain, def:gauge_phase_equiv, def:non_ti_chain}
    Let $\mathbf{A} = (A_0, \ldots, A_{n-1})$ and
...
\begin{proof}
    \leanok
    \uses{lem:chainCombinedTensor_smul_chain, thm:gauge_invariance, thm:fundamentalTheorem_injective_chain}
...
\begin{proof}
    \leanok
    \uses{lem:blocked_chain_isInjective, thm:fundamentalTheorem_injective_chain}
...
\begin{remark}[Relation to normal-MPS corollary]
    Theorem~\ref{thm:fundamentalTheorem_blockedChain} establishes gauge
    equivalence only for the blocked chains; recovering sitewise gauges for the
    original unblocked tensors is a separate unblocking step yielding the
    normal-MPS corollary of \cite{Molnar2018NormalPEPS}, outside this
    blocked-chain statement.
\end{remark}
...
$k$-th coordinate, as in Definition~\cite[``Block ideal'']{QICLean2026Blueprint}.
...
\begin{theorem}[Ring isomorphisms match block ideals]\label{thm:ring_perm}
    \lean{TwoSidedIdeal.exists_blockEquiv_of_ringEquiv_pi_simple}
    \lean{TwoSidedIdeal.ringEquiv_pi_simple_permutes_blockIdeals}
    \leanok
...
\begin{proof}
    ...
    Theorem~\cite[``Summand matching and dimension preservation'']{QICLean2026Blueprint} identifies the matrix sizes
    $D_{\pi(k)} = D_k$.
    For each $k$, Theorem~\cite[``Paired component maps are bijections'']{QICLean2026Blueprint} gives a bijective
    ...
    Skolem--Noether (Theorem~\cite[``Skolem--Noether'']{QICLean2026Blueprint}) makes each such
    automorphism inner.
...
\begin{remark}[Ring level versus algebra level]\label{rem:ring_vs_alg}
    ...
    Theorem~\cite[``Paired component maps are bijections'']{QICLean2026Blueprint} stays at this ring level: once the
    block ideals are permuted, one can extract the corresponding single-block
    maps without using scalar multiplication.
    Theorems~\cite[``Summand matching and dimension preservation'']{QICLean2026Blueprint}
...
\begin{proof}\leanok
    \uses{lem:perBlockLinearExtension_mul, lem:perBlockLinearExtension_bijective}
...
\begin{definition}[Product algebra automorphism]%
    ...
    \leanok
    \uses{def:perBlockLinearExtension, lem:perBlockLinearExtension_mul, lem:perBlockLinearExtension_bijective}
