...
\begin{proof}\leanok
    ...
    non-decaying cross-overlap. Normal-block overlap rigidity supplies a
    scalar sequence $\alpha_j(N)$ such that
    \begin{align}
        V^{(N)}(P_j)
         & =\alpha_j(N)V^{(N)}(Q_{k(j)}).
        \notag
    \end{align}
    ...
    $k_0$ has non-decaying cross-overlap with $P_{j_0}$. The unequal-dimension
    and equal-dimension overlap dichotomies then give
