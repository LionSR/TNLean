...
% lines 1184--1188.
The exact matching of Section~\ref{sec:sector_bnt_strong_match} produces a
coefficient identity on each matched pair. By Newton--Girard power-sum
recovery, this identity determines the per-copy weight multisets with
multiplicity \cite[Appendix~A]{Cirac2016MPDO_arXiv}.

\subsection{Matched-sector weight multiset equality}

For a matched pair of sectors with
$c_N^{(j_0)}(P) = \zeta^N c_N^{(\tilde k_0)}(Q)$ for all $N > N_0$ and
$\zeta\ne0$, power-sum recovery yields multiset equality of the per-copy
weights, inducing an explicit copy permutation that realizes
$\mu_{j,q} = \nu_{j,q}\, e^{i\varphi_j}$ \cite[Appendix~A]{Cirac2016MPDO_arXiv}.
...
\begin{lemma}[Matched-sector weight multiset equality]%
    ...
    \leanok
    \uses{def:sector_weight_data, thm:power_sum_multiset}
...
The exact matching shows that equal MPV families force each basis sector
of one canonical form to pair with a unique basis sector of the other
...
\begin{theorem}[Exact single-sector matching]
    ...
    \leanok
    \uses{def:sector_bnt_cf, def:same_mpv2_pos, thm:sector_bnt_proportional_exact_match}
...
\begin{theorem}[Strong existential matching, full-basis form]
    ...
    \leanok
    \uses{def:sector_bnt_cf, def:same_mpv2_pos, thm:sector_bnt_proportional_full_basis_match}
...
Applying the matching in both directions yields a bijection between the basis
sectors: the basis-distinctness clause ensures injectivity both ways, so
$g_a = g_b$ and $B_k = e^{i\phi_k} X_k A_{j_k} X_k^{-1}$ on the full basis
\cite[Appendix~A]{Cirac2016MPDO_arXiv}.
...
\begin{theorem}[Bijective matching by symmetry]
    ...
    \leanok
    \uses{thm:sector_bnt_forall_unit_k_exists_j_nondecaying_overlap_of_sameMPV, lem:sector_bnt_bijection_from_matches, def:sector_bnt_cf, def:same_mpv2_pos}
    Let $P, Q$ be two BNT canonical forms with the same MPV family at every
...
\begin{proof}\leanok
    \uses{thm:sector_bnt_forall_unit_k_exists_j_nondecaying_overlap_of_sameMPV, lem:sector_bnt_bijection_from_matches}
