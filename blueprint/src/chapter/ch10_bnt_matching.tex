...
\begin{proof}\leanok
    ...
    equivalence, hence a scalar $\omega_{j'} \in \C^\times$ with
    $V^{(N)}(P_{j'}) = \omega_{j'}^{N} V^{(N)}(Q_{k(j')})$, so
    $V^{(N)}(P_{j'})$ lies in the span of $\{V^{(N)}(Q_k)\}_k$. The family
    $\{P_{j'} : j'\in T\}\cup\{Q_k\}_k$ then has all pairwise cross-overlaps
    decaying, hence is eventually linearly independent
    (Lemma~\ref{lem:sector_bnt_combined_family_eventually_li}). Rewriting the
    common-MPV identity over this family,
    \begin{align}
        ...
        \notag
    \end{align}
    exact coefficient extraction forces $c_N^{(j)} = 0$ at each fixed large
    $N$, contradicting
