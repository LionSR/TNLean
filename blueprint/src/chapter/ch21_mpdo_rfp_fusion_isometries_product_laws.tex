...
\begin{theorem}[BNT characterizations of MPDO renormalization fixed points]
    ...
    \begin{enumerate}
        ...
        \item[(iii)] $M$ has the BNT fusion clause.
    \end{enumerate}
    Then
    \begin{align}
        \textnormal{(i)} & \iff\textnormal{(ii)},
                         &
        \textnormal{(i)} & \iff\textnormal{(iii)}.
        \label{eq:mpdo_cpsv_bnt_rfp_equivalence}
    \end{align}
    These are the two equivalences of
    \cite[Theorem~4.14, lines~972--993]{Cirac2016MPDO_arXiv}.  Every BNT label
    of the fusion clause carries a nonempty multiplicity space and positive
    weights, so the product sectors it ranges over are the ones the source's
    basis of normal tensors supplies.
