...
\begin{definition}[Normal square-lattice blocking regions]%
    ...
    \leanok
    \uses{def:peps_normalRectangleInjectivityHypotheses, def:peps_regionInjectivityData, lem:peps_normalSquareRegionRS_injective_of_unionClosure, lem:peps_normalSquareRegionT_injective_of_cover}
    ...
    The statement here specifies the corresponding abstract hypothesis:
    it records the rectangular injectivity assumptions, the
...
\begin{theorem}[Normal edge blocking to a three-site injective chain]%
    ...
    \notready
    \uses{lem:peps_injectiveRegion_union, def:peps_normalSquareBlockingRegions, def:peps_normalEdgeBlockingHypotheses, thm:peps_normalEdgeBlockingHypotheses_injectiveChain, thm:peps_edgeBlockedInsertionAlgebraIsomorphism}
    ...
    \begin{tenkzequation}
        ...
        \begin{tenkz}[rows={ket}, cols=3, physical=up, boundary=periodic]
            ...
            \tnmark[form=enclosure, species=selected]{(1,1)}{}
            \tnmark[form=enclosure, species=secondary]{(1,2)}{}
            \tnmark[form=enclosure, species=complement]{(1,3)}{}
        \end{tenkz}
    \end{tenkzequation}
\end{theorem}
...
\begin{theorem}[Translationally invariant normal gauge absorption]%
    ...
    \notready
    \uses{thm:peps_normalEdgeBlockingToInjectiveChain, thm:peps_edge_gauge_absorption}
...
\begin{theorem}[Fundamental Theorem for normal square-lattice PEPS]%
    ...
    \notready
    \uses{thm:peps_tiNormalGaugeAbsorption, thm:peps_oneVertexComplementComparison, def:GaugeEquivPEPS}
    ...
    statement of the source is established in
...
\begin{theorem}[Fundamental Theorem for normal square-lattice PEPS,
    ...
    \leanok
    \uses{thm:fundamentalTheorem_normalSquarePEPS, def:peps_normalRectangleInjectivityHypotheses, def:applyGauge, def:peps_edgeInsertedCoeff}
