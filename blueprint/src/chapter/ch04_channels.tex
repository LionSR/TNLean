...
\begin{proof}
    \leanok
    Specialise \texttt{kraus\_dual\_eq\_of\_map\_eq} at $Y = \Id$.
\end{proof}
...
\begin{theorem}[Thm.~2.1 item 4: Rectangular Kraus freedom (necessary direction)]\label{thm:kraus_rectangular_freedom}
    ...
    for all $X$ and $r_2 \le r_1$, then there exists a rectangular isometry
    $V$ ($r_1 \times r_2$, $V^\dagger V = \Id$) such that
...
\begin{proof}
    ...
    range, and extend to a full isometry $V$ with $V^\dagger V = \Id$.
\end{proof}
...
\begin{theorem}[Thm.~2.1 item 4: Rectangular Kraus freedom (general index variant)]\label{thm:kraus_rectangular_freedom'}
    ...
    Variant of \Cref{thm:kraus_rectangular_freedom} with general
...
\begin{proof}
    ...
    \Cref{thm:kraus_rectangular_freedom}.
\end{proof}
...
\begin{proof}
    ...
    Reverse direction: $|\det(U\,\cdot\, U^\dagger)| = |\det U|^{2d^2} = 1$.
    Forward direction: $|\det T| = 1$ and Kadison--Schwarz forces each Kraus
    operator $K_i$ to be a scalar multiple of a unitary; the
    trace-preservation constraint then pins $T = U\,\cdot\, U^\dagger$.
\end{proof}
