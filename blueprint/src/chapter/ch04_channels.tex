...
\begin{proof}\leanok
    Regard $E$ as the associated positive linear map. Positive linear maps
    between matrix $C^*$-algebras preserve self-adjoint elements, and the
    self-adjoint matrices are exactly the Hermitian matrices.
\end{proof}
...
\begin{proof}\leanok
    The involution on complex matrices is conjugate transpose, so the two
    formulations have the same self-adjointness equation and the same
    idempotence equation.
\end{proof}
...
\begin{proof}\leanok
    Reinterpret $P$ as a self-adjoint idempotent in the involutive algebra
    $\MN{D}$. Its complement $\Id-P$ is again a self-adjoint idempotent, hence
