...
\begin{definition}[Sum of the largest eigenvalues]
    ...
    zero, so the value stabilizes at $\operatorname{Re}\tr(A)$ once $k$ reaches
    $D$.
...
\begin{proof}\leanok
    ...
    eigenvalues. Then $\operatorname{Re}\tr(PA)=\sum_{i\le k}\lambda_i=S_k(A)$.
\end{proof}
...
\begin{theorem}[Ky Fan's maximum principle]
    ...
    eigenvalues is the largest value of $\operatorname{Re}\tr(PA)$ attained by an
    orthogonal projection $P$ of rank $k$:
    \[
        S_k(A) = \max_{P^2=P=P^\dagger,\ \tr P = k}
            \operatorname{Re}\tr(PA) .
    \]
    This is Ky Fan's maximum principle \cite{Fan1949Theorem}. For a positive
