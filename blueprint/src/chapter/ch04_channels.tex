...
\begin{theorem}[Ky-Fan achievability]
...
\begin{proof}\leanok
    ...
    eigenvalues. Then $\operatorname{Re}\tr(PA)=\sum_{i\le k}\lambda_i=\|A\|_{(k)}$.
\end{proof}

\begin{theorem}[Ky-Fan upper bound]
...
\begin{proof}\leanok
    ...
    and $\operatorname{Re}\tr(PA)=\sum_i w_i\lambda_i$ where $w_i$ are the
...
\begin{proof}\leanok
    ...
    rank-$k$ projection whose trace against $A$ equals $\|A\|_{(k)}$, so the value
    is attained. The upper bound (Theorem~\ref{thm:ky_fan_upper_bound}) shows that
    no rank-$k$ projection exceeds it. Hence $\|A\|_{(k)}$ is the maximum.
\end{proof}
