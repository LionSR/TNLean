...
\begin{definition}[Completely positive map]\label{def:cp_map}
    ...
    \]
    By Choi's theorem, this is equivalent to $E \otimes \id_n$ being
    positive for all~$n$.
\end{definition}
...
\begin{theorem}[Injectivity implies irreducibility]\label{thm:injective_irreducible}
    ...
    \leanok
    \uses{def:injective, def:transfer_map, def:irreducible_cp, thm:transfer_cp}
    If $A$ is an injective MPS tensor, then its transfer map
    $\E_A$ (which is CP by Theorem~\ref{thm:transfer_cp}) is irreducible.
\end{theorem}
...
\begin{theorem}[Transfer map is CP]\label{thm:transfer_cp}
    ...
    is completely positive (its Kraus operators are the matrices
    $\{A^i\}$).
\end{theorem}

\begin{proof}\leanok
    Immediate from the definition: the transfer map is defined
    as a Kraus map with operators $K_i = A^i$.
\end{proof}

\begin{theorem}[Transfer map is a channel]\label{thm:transfer_channel}
    \lean{MPSTensor.transferMap_isChannel}
    \leanok
    \uses{def:transfer_map, def:channel, thm:transfer_cp}
...
\begin{proof}\leanok\uses{thm:transfer_cp}
    Complete positivity is Theorem~\ref{thm:transfer_cp}.
    Trace preservation:
...
\begin{definition}[Peripheral eigenvalues]\label{def:peripheral_eigenvalues}
    ...
    \textbf{Note:} The Lean definition hardcodes $|\lambda| = 1$
    rather than $|\lambda| = \rho(E)$
    (the spectral radius).  This is correct for channels
    but would need generalisation for maps with spectral
    radius $\neq 1$.
\end{definition}
...
\begin{theorem}[Peripheral eigenvalues are roots of unity]\label{thm:peripheral_roots_of_unity}
    ...
    By the multiplicative domain theory
    (\cite[Prop.~6.2]{Wolf2012Quantum}),
    if $\mu$ is a peripheral eigenvalue then $\mu^n$ is an
    eigenvalue for every $n \ge 1$.
    In finite dimensions the eigenvalue set is finite,
    so $\{\mu^n\}$ is finite, forcing $\mu$ to be a root of unity.
\end{theorem}

\begin{proof}
    \leanok
    The hypothesis of the Lean theorem is that $\mu^n$ is an
    eigenvalue for every~$n$.
    By the pigeonhole principle on the finite eigenvalue set,
    $\mu^a = \mu^b$ for some $a \neq b$, giving $\mu^{|a-b|} = 1$.
\end{proof}
...
\begin{theorem}[Peripheral powers imply root of unity]\label{thm:peripheral_powers}
    ...
    \leanok
    \uses{def:peripheral_eigenvalues, thm:peripheral_roots_of_unity}
...
\begin{remark}\label{rem:periodicity_removal_spectral}
...
\begin{lemma}[Powering collapses peripheral eigenvalues]\label{lem:peripheral_pow_singleton}
    ...
    \leanok
    \uses{def:peripheral_eigenvalues, thm:one_peripheral}
...
\begin{lemma}[Peripheral eigenvalues closed under powers]\label{lem:peripheral_closed_powers_irred_ds}
    ...
    \leanok
    \uses{def:unital_kraus, def:tp_kraus, def:irreducible_cp, thm:ks_peripheral,
          thm:psd_fp_pd_irred}
...
\begin{lemma}[Peripheral closure via adjoint fixed point]%
    ...
    \uses{def:unital_kraus, def:kraus_adjoint_map, def:irreducible_cp,
          thm:ks_peripheral, thm:psd_fp_pd_irred}
...
\begin{theorem}[Roots of unity via adjoint fixed point]%
    ...
    every peripheral eigenvalue of $E$ is a root of unity.
\end{theorem}
...
\begin{remark}\label{rem:adjoint_fp_more_general}
    Lemma~\ref{lem:peripheral_closed_powers_irred_fp}
    ...
    argument in \cite[Appendix~A]{Cirac2016Density},
...
\begin{definition}[Primitive map]\label{def:primitive_channel}
    ...
    For channels this corresponds to \cite[Thm.~6.11]{Wolf2012Quantum}.
    The Lean definition applies to any linear endomorphism, not only
    channels.
\end{definition}
...
\begin{theorem}[Non-peripheral eigenvalues of primitive maps]\label{thm:primitive_compl_lt_one}
    \lean{compl_eigenvalue_norm_lt_one_of_primitive}
    \leanok
    \uses{def:primitive_channel}
    If a linear map is primitive, then every eigenvalue $\lambda \neq 1$
    satisfies $|\lambda| < 1$.
\end{theorem}
