...
\begin{remark}[Bi-canonical special case]\label{rem:bicanonical_special_case}\leanok
    ...
    by Theorem~\ref{thm:peripheral_powers}. This is the simplest route, but
...
%% =========================================================
\section{Representations: infrastructure}
\label{sec:representations_infrastructure}
%% =========================================================

The following infrastructure supports the Choi--Jamiol\-kowski isomorphism
...
\begin{definition}[Partial traces]\label{def:partial_trace}
    ...
    \[
        ...
        (\operatorname{tr}_B(X))_{ij}
          = \sum_{k} X_{(i,k),(j,k)}.
    \]
    In Lean these are \texttt{Matrix.traceLeft} and
    \texttt{Matrix.traceRight} respectively.
\end{definition}
...
\begin{definition}[Maximally entangled state]\label{def:maximally_entangled}
    ...
    In Lean this is \texttt{Matrix.omegaProj}.
\end{definition}
...
\begin{definition}[Tensor product of a map with identity]\label{def:tensor_map_id}
    ...
    bipartite slice.
    In Lean this is \texttt{Matrix.tensorMapId}.
\end{definition}
...
%% =========================================================
\section{Quantum dynamical semigroups (Wolf Ch.~7, Workstream A)}
\label{sec:quantum_dynamical_semigroups}
%% =========================================================

This section develops the theory of one-parameter semigroups of quantum
channels following~\cite[Ch.~7]{Wolf2012Quantum}.
The main structural result (Proposition~\ref{thm:continuousDynSemigroup_eq_exp})
is that every norm-continuous dynamical semigroup on a finite-dimensional
algebra is the matrix exponential of a generator.

\emph{Note:} The full semigroup theory, including GKSL/Lindblad generators
(Props.~7.2--7.4, Thm.~7.1) is developed in
Chapter~\ref{ch:semigroup}.

\begin{definition}[Dynamical semigroup]\label{def:dyn_semigroup}
    \lean{IsDynSemigroup}
    \leanok
...
\begin{definition}[Continuous dynamical semigroup]\label{def:continuous_dyn_semigroup}
    \lean{IsContinuousDynSemigroup}
    \leanok
    \uses{def:dyn_semigroup}
    A dynamical semigroup $T$ is \emph{norm-continuous} if the map
    $t \mapsto T(t)$ is continuous with respect to the operator norm.
    In finite dimensions, norm-continuity is equivalent to strong
    continuity~\cite[§7.1]{Wolf2012Quantum}.
\end{definition}

\begin{definition}[Exponential semigroup]\label{def:exp_semigroup}
    \lean{expSemigroup}
    \leanok
    \uses{def:dyn_semigroup}
    ...
    \[
        ...
        \quad t \ge 0,
    \]
    where $e^{tL} = \sum_{n=0}^\infty \frac{(tL)^n}{n!}$ is the
    operator exponential.
    The family $t \mapsto e^{tL}$ is a dynamical semigroup
    (Theorem~\ref{thm:expSemigroupCLM_add}) and is
    norm-continuous
    (Theorem~\ref{thm:expSemigroupCLM_continuous}).
\end{definition}

\begin{theorem}[Semigroup law for the exponential]\label{thm:expSemigroupCLM_add}
    \lean{expSemigroupCLM_add}
    \leanok
    \uses{def:exp_semigroup}
    For all $t, s \in \R$,
    \[
...
\begin{proof}
    \leanok
    $t \cdot L$ and $s \cdot L$ commute (both are scalar multiples of
    $L$), so $e^{(t+s)L} = e^{tL+sL} = e^{tL}\cdot e^{sL}$ by the
    Banach-algebra addition formula for commuting elements.
\end{proof}

\begin{theorem}[Continuity of the exponential semigroup]\label{thm:expSemigroupCLM_continuous}
    \lean{expSemigroupCLM_continuous}
    \leanok
    \uses{def:exp_semigroup}
    The map $t \mapsto e^{tL}$ is continuous in the operator norm.
\end{theorem}

\begin{proof}
    \leanok
    The map $t \mapsto tL$ is continuous (as a real-to-complex-scalar
    multiple), and the matrix exponential is analytic on the whole
    algebra, hence continuous.
\end{proof}

\begin{theorem}[Derivative of the exponential semigroup]\label{thm:hasDerivAt_expSemigroupCLM}
    \lean{hasDerivAt_expSemigroupCLM}
    \leanok
    \uses{def:exp_semigroup}
    For all $t \in \R$,
    \[
...
\begin{proof}
    \leanok
    By the chain rule, differentiating $t \mapsto e^{(t : \C) \cdot L}$
    via the real-to-complex inclusion and the Mathlib lemma
    \texttt{hasDerivAt\_exp\_smul\_const}.
\end{proof}

\begin{theorem}[Generator uniqueness]\label{thm:generator_unique}
    \lean{generator_unique}
    \leanok
    \uses{def:exp_semigroup}
...
\begin{proof}
    \leanok
    The two functions agree on $[0,\infty)$, so they have the same
    right-derivative at $t = 0$ within $[0,\infty)$.
    The set $[0,\infty)$ has unique differentials at $0$, so the
    derivatives $L$ and $L'$ agree.
\end{proof}

\begin{theorem}[Prop.~7.1: Every continuous semigroup is exponential]\label{thm:continuousDynSemigroup_eq_exp}
    \lean{continuousDynSemigroup_eq_exp}
    \uses{def:continuous_dyn_semigroup, def:exp_semigroup,
          thm:generator_unique}
...
\begin{proof}
    By finite-dimensionality, $\MN{D}$ is a Banach algebra and every
    norm-continuous semigroup on a Banach algebra is uniformly
    continuous; the generator $L = \lim_{h \downarrow 0} (T_h - \id)/h$
    exists in the operator norm, and the fundamental theorem of
    calculus for ODEs gives $T_t = e^{tL}$.
    (Formalization in progress.)
\end{proof}

\begin{theorem}[Lem.~7.1: Duhamel formula]\label{thm:duhamel_formula}
    \lean{duhamel_formula}
    \uses{def:exp_semigroup, thm:hasDerivAt_expSemigroupCLM}
...
\begin{proof}
    ...
    (Theorem~\ref{thm:hasDerivAt_expSemigroupCLM}),
    and $e^{tL'} - e^{tL} = f(t) - f(0) = \int_0^t f'(s)\,ds$.
    (Formalization in progress.)
\end{proof}

\begin{theorem}[Cor.~7.1: Perturbation bound]\label{thm:perturbation_bound}
    \lean{perturbation_bound}
    \uses{thm:duhamel_formula}
    For $t \ge 0$,
    \[
        \bigl\|e^{tL'} - e^{tL}\bigr\|
          \le t\,\|\Delta\|
...
\begin{proof}
    ...
    (Lemma~\ref{thm:duhamel_formula}):
    \[
        \bigl\|e^{tL'} - e^{tL}\bigr\|
          \le \int_0^t \|e^{(t-s)L}\|\,\|\Delta\|\,\|e^{sL'}\|\,ds
          \le t\,\|\Delta\|
             \cdot \sup_s\|e^{sL}\|
             \cdot \sup_s\|e^{sL'}\|.
    \]
