...
\begin{definition}[Unique positive fixed point]
    \label{def:unique_positive_fixed_point}
    \lean{HasUniqueFixedPoint}
    \leanok
    A linear map $E : \MN{D} \to \MN{D}$ has a \emph{unique positive fixed point}
    $\rho$ if $E(\rho) = \rho$, the matrix $\rho$ is positive definite,
    and every positive semidefinite fixed point of $E$ is a scalar multiple of $\rho$.
\end{definition}
...
\begin{theorem}[Canonical dilation as a packaged Naimark dilation]
    ...
    $P_i = \one_D \otimes |i\rangle\langle i|$ satisfy the packaged axioms
    of a Naimark dilation.
\end{theorem}

\begin{proof}
    \leanok
    Combine the previously proved identities $V^\dagger V = \one_D$,
    $P_i^2 = P_i$, $P_i^\dagger = P_i$, $P_i P_j = 0$ for $i \ne j$,
    $\sum_i P_i = \one$, and $V^\dagger P_i V = E_i$.
\end{proof}
...
\begin{proof}\leanok
    \uses{thm:fixedPoints_in_multiplicativeDomain}
    ...
    Closure under multiplication: if $E(X) = X$ and $E(Y) = Y$, then
    Theorem~\ref{thm:fixedPoints_in_multiplicativeDomain} places $X$ in the
    multiplicative domain, so $E(XY) = E(X) E(Y) = XY$.
\end{proof}

\begin{remark}\leanok
    The multiplicative-domain step used here is recorded explicitly in
    Theorem~\ref{thm:fixedPoints_in_multiplicativeDomain}, so the fixed-point
    algebra statement no longer relies on a later chapter for its formulation.
\end{remark}
