...
\begin{center}
    ...
    \begin{tenkz}[rows={ket,ket,ket}, cols=3, frame=plane, physical=up,
        ...
        \tn[at=(1,2), species=marked, label pos=nw]{A}
    \end{tenkz}
\end{center}
...
\begin{definition}[Translation-invariant torus tensor]%
    ...
    $A^{\tau_{a,b}}=A$ for every translation $\tau_{a,b}$. This is
...
\begin{theorem}[State coefficient of a translation-invariant torus tensor]%
    ...
    \leanok
    \uses{def:peps_torus_translation_invariant, thm:peps_stateCoeff_transport}
...
\begin{theorem}[Tensor translation invariance implies state translation invariance]%
    ...
    A translation-invariant torus tensor $A$ represents a translation-invariant
    state: for every lattice translation $\tau_{a,b}$ and physical configuration $\sigma$,
    \begin{align}
        \Coeff_A(\sigma\circ\tau_{a,b}) & = \Coeff_A(\sigma). \notag
    \end{align}
\end{theorem}
\begin{proof}\leanok
...
\begin{lemma}[Boundary windows for the staircase reference edge]%
    ...
    \leanok
    \uses{lem:peps_reference_edge_left_endpoint_staircase_window, lem:peps_reference_edge_right_endpoint_staircase_window}
