...
\begin{theorem}[Rank under faithful scalar extension]
    ...
    matrix over $K$.  Then
    \[
      \rank_{L}\!\left((\iota(A_{ij}))_{ij}\right)
      =\rank_{K} A.
    \]
\end{theorem}
\begin{proof}\leanok
...
\begin{definition}[Normalized diagonal distribution at fixed length]
    ...
    \[
      W_{x,y}=\re\rho^{(L+R)}(M)_{(x,y),(x,y)},\qquad
...
\begin{remark}[Boundedness of the mutual information]
    ...
    $\rank(q)^L$, although its mutual information vanishes. The
...
\begin{proof}\leanok
    ...
    $W(u, w) = \langle u\,w\,|\,V^{(N)}(A)\rangle = \tr
    ...
    having $D^2$ columns, so $\rank\rho_L \le \rank W
        \le D^2$.
\end{proof}
