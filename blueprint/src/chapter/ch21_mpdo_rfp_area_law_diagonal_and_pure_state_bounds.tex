...
\begin{proof}\leanok
    ...
    \end{align}
    The support projection of $\Delta$ fixes $v$. Indeed, $T$ is positive
    definite, while the support projection of $R^T$ fixes $R^T$; after
    transposition this gives $R P_{\supp(R^T)}^T=R$. This support condition is
    essential when $\rho$ is singular: it removes the zero spectral weights
    before Jensen's inequality is applied.

    Theorems~\ref{thm:log_kronecker_support},
    ...
    The auxiliary divergence and its vector-state Jensen argument follow the
    $\tau=\rho$, $\alpha=1,2$ specialization of
    \cite[arXiv:1306.3142v4, Definition~5 and proof of Theorem~5]{MullerLennert2013Renyi}.
    The final Cauchy--Schwarz estimate is the direct endpoint proof used here;
    it is not an application of Beigi's interpolation theorem.
\end{proof}
