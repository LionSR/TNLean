...
\begin{lemma}[Positive physical trace and non-Hermitian Bell similarity]%
    ...
    The tensor $A$ is injective and proportional to a normal tensor. The matrix $X$
...
\begin{theorem}[Physical-trace and Bell Hermiticity force a scalar Gram matrix]%
    ...
    $P$ with the two-site operator of the undeformed physical model.
\end{theorem}
...
\begin{theorem}[Canonical form in the original physical direction]%
    ...
    four bond-one normal tensors, one supported on each symbol, all with weight one.
    Hence $M$ itself is in canonical form, without a change of basis or blocking.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Normalized one-element vertical BNT and its algebra]%
    ...
    left-canonical and is a normal tensor with transfer spectral radius one.
    A vertical basis of normal tensors consists of the single tensor $M_0=\widehat A$.
...
\begin{theorem}[One-element vertical BNT algebra for the physical tensor]%
    ...
    The physical tensor $M$ has a vertical basis of normal tensors with one label, one copy,
