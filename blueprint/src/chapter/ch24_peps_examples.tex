...
$G$-injectivity and to be the regular representation for $G$-isometry;
each instance below uses a regular representation. The source's tensors
are not normalized, and the constant $c$ absorbs the normalization.
...
On a torus the differences of plaquette colorings exhaust only the Gauss-law
configurations of trivial holonomy, and this statement is not formalized.
...
\begin{theorem}[The primal virtual labels fuse to the identity]%
    ...
    every representation $\pi$ of $G$, and the tensor is invariant under the
    virtual action of every one-dimensional representation $\chi$, by $\chi$
    on the top and right legs and by $\chi^{-1}$ on the down and left legs
    \cite[Appendix~A, ``The Toric Code and quantum double
    models'']{Cirac2021Matrix}.
\end{theorem}
