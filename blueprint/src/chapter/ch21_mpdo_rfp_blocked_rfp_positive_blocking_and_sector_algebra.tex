...
\begin{theorem}[Blocked word evaluation of an MPO tensor]
    \label{thm:mpo_eval_word_block}
    \lean{MPOTensor.evalWord_blockTensor_ofFn}
    \leanok
    \uses{def:mpdo_physical_blocking, def:mpo_eval_word}
    Let $L\geq0$, and let $\sigma=(\sigma_0,\ldots,\sigma_{m-1})$ and
    $\tau=(\tau_0,\ldots,\tau_{m-1})$ be words of the same length $m$ in the
    blocked alphabet, so that each $\sigma_k$ and each $\tau_k$ is a word of
    length $L$ in $\{0,\ldots,d-1\}$. Write $\widetilde\sigma$ and
    $\widetilde\tau$ for the concatenations
    $\sigma_0\sigma_1\cdots\sigma_{m-1}$ and $\tau_0\tau_1\cdots\tau_{m-1}$, words of
    length $mL$. Then
    \begin{align}
        (M^{[L]})^{\sigma,\tau}
         & =M^{\widetilde\sigma,\widetilde\tau}.
        \notag
    \end{align}
    This is the counterpart for MPO tensors of
    Lemma~\ref{lem:eval_word_block}, with the ket and bra words blocked
    separately.
\end{theorem}

\begin{proof}\leanok
    \uses{lem:mpo_eval_word_append}
    Induct on $m$. For $m=0$ both sides are the identity. For $m\geq1$,
    split off the first blocked letters $\sigma_0,\tau_0$: by
    Definition~\ref{def:mpdo_physical_blocking},
    $(M^{[L]})^{\sigma_0,\tau_0}=M^{\sigma_0,\tau_0}$, and since $\sigma_0$
    and $\tau_0$ have the same length $L$,
    Lemma~\ref{lem:mpo_eval_word_append} gives
    $M^{\sigma_0\widetilde\sigma',\,\tau_0\widetilde\tau'}
        =M^{\sigma_0,\tau_0}M^{\widetilde\sigma',\widetilde\tau'}$ for the
    remaining words $\sigma',\tau'$.
\end{proof}
...
\begin{proof}\leanok
    \uses{thm:mpo_eval_word_block, lem:eval_word_block}
    The word of $N$ blocked letters $\sigma=(\sigma_0,\ldots,\sigma_{N-1})$
    has concatenation $\Phi_{N,L}(\sigma)$, and similarly for $\tau$; this
    concatenation is the blocked-configuration bijection of
    Lemma~\ref{lem:eval_word_block}. By
    Theorem~\ref{thm:mpo_eval_word_block},
    \begin{align}
        (M^{[L]})^{\sigma,\tau}
         & =M^{\Phi_{N,L}(\sigma),\Phi_{N,L}(\tau)}.
        \notag
    \end{align}
...
\begin{lemma}[Closed MPO entries with residual sites]
    \label{lem:mpdo_physical_blocking_residuals}
    \lean{MPOTensor.mpo_apply_prefix_blockTensor_suffix}
    \leanok
    \uses{def:mpdo_physical_blocking, def:mpo_family, def:mpo_eval_word}
    Let a chain consist of a prefix of $p$ sites, $m$ complete blocks of
    length $L$, and a suffix of $s$ sites. Write $M^{\alpha,\beta}$ for the
    ordered product of matrices indexed by two equally long physical
    words, and write $\widetilde\sigma,\widetilde\tau$ for the concatenations of
    blocked words $\sigma,\tau$. Then, for matching row and column
    configurations,
    \begin{align}
        \rho^{(p+mL+s)}(M)_{i^-\widetilde\sigma i^+,\,
            j^-\widetilde\tau j^+}
         & =\operatorname{tr}\!\left(
        M^{i^-,j^-}\cdot (M^{[L]})^{\sigma,\tau}\cdot M^{i^+,j^+}
        \right).
    \end{align}
    This identity retains both residual products, including when the chain
    length is not divisible by $L$. It extends the closed-chain blocking
    identity used in
    \cite[Appendix~C.4, lines~1952--2017]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
    \uses{lem:mpo_eval_word_append, thm:mpo_eval_word_block, lem:eval_word_block}
    The concatenations $\widetilde\sigma,\widetilde\tau$ are identified with
    the flattened blocked configurations by Lemma~\ref{lem:eval_word_block}.
    Applying word concatenation at the two boundaries
    (Lemma~\ref{lem:mpo_eval_word_append}) and then evaluating the middle
    segment as a word of blocked matrices (Theorem~\ref{thm:mpo_eval_word_block})
    gives
    \begin{align}
        M^{i^-\widetilde\sigma i^+,\,j^-\widetilde\tau j^+}
         & = M^{i^-,j^-}\cdot M^{\widetilde\sigma,\widetilde\tau}
        \cdot M^{i^+,j^+} \notag                                  \\
         & = M^{i^-,j^-}\cdot (M^{[L]})^{\sigma,\tau}
        \cdot M^{i^+,j^+}.
        \notag
    \end{align}
    Taking the virtual trace yields the stated entry identity.
\end{proof}
...
\begin{theorem}[Concrete two-site blocking as length-two blocking]
    ...
    \leanok
    \uses{def:mpdo_physical_blocking, def:mpdo_two_site_blocking, def:mpdo}
...
\begin{theorem}[MPO blocking preserves literal CPSV doubled-index form]
    ...
    \leanok
    \uses{def:mpdo_physical_blocking, def:mpdo_two_site_blocking, thm:mpdo_blocking_doubled_index, thm:mpdo_block_two_as_positive_blocking, thm:cpsv_physical_reindexing, thm:cpsv_literal_canonical_form_positive_blocking}
...
\begin{theorem}[Literal one-site vertical decomposition]
    ...
    \leanok
    \uses{def:mpdo, def:cpsv_canonical_form, def:vertical_tensor_mpdo, def:bnt_cpsv}
...
\begin{proof}\leanok
    \uses{thm:cpsv_literal_vertical_bnt_grouping_with_isometries, thm:normalized_grouped_sector_maps_of_dressing, lem:same_mpv_exact_isometric_decomposition}
...
\begin{theorem}[Literal two-site vertical decomposition]
    ...
    \leanok
    \uses{def:mpdo_two_site_blocking, def:vertical_tensor_mpdo, def:bnt_cpsv}
...
\begin{proof}\leanok
    \uses{thm:mpdo_literal_cpsv_positive_blocking, thm:mpdo_block_two_as_positive_blocking, thm:mpdo_literal_cpsv_vertical_decomposition}
...
\begin{corollary}[Paired literal vertical decompositions]
    ...
    \leanok
    \uses{thm:mpdo_literal_cpsv_vertical_decomposition, thm:mpdo_literal_cpsv_vertical_decomposition_block_two}
    Under the hypotheses of
...
\begin{lemma}[Two-site blocking preserves normalized BNT-refined horizontal form]
    ...
    \leanok
    \uses{def:horizontal_cf_mpdo, def:mpdo_two_site_blocking, thm:sector_bnt_cf_block_tensor}
...
\begin{theorem}[One-site and two-site vertical canonical forms]
    ...
    \leanok
    \uses{thm:vertical_cf_of_horizontal_cf, thm:mpdo_block_two_as_positive_blocking, lem:mpdo_horizontal_cf_block_two}
...
\begin{proof}\leanok
    By Theorem~\ref{thm:mpdo_block_two_as_positive_blocking} and
    Lemma~\ref{lem:mpdo_horizontal_cf_block_two}, $K^{[2]}$ is in normalized
...
\begin{definition}[Products of weighted vertical contractions]
    ...
    \leanok
    \uses{def:mpdo_normalized_vertical_sector_maps, def:mpdo_vertical_bond_matrix_contraction}
...
\begin{lemma}[Weighted contraction of matrix-unit words]
    ...
    \leanok
    \uses{def:mpdo_weighted_vertical_contraction_products, lem:mpdo_vertical_bond_contraction_encoded_matrix_unit}
...
\begin{lemma}[Transfer of simultaneous word spanning]
    ...
    \leanok
    \uses{def:mpdo_vertical_sector_scaling_equivalence, lem:mpdo_weighted_contraction_matrix_unit_words}
...
\begin{proof}\leanok
    \uses{lem:mpdo_weighted_contraction_span_transfer, thm:cpsv_bnt_blocked_simultaneous_span}
...
\begin{proof}\leanok
    \uses{lem:mpdo_vertical_sector_retraction, thm:mpdo_nonzero_weight_vertical_bond_contractions_generate_sector_algebra}
    Since every multiplicity space is nonzero and every diagonal entry is
    positive,
    $m_\alpha
        =\sum_{q=0}^{\mult(\alpha)-1}\mu_{\alpha,q}>0$.
    Apply Theorem~\ref{thm:mpdo_nonzero_weight_vertical_bond_contractions_generate_sector_algebra}
    to the nonzero family $c_\alpha=m_\alpha=\tr(\mu_\alpha)$.
\end{proof}
...
\begin{theorem}[Fixed contractions and the dimension of their fixed-point space]
    ...
    \leanok
    \uses{def:mpdo_weighted_vertical_contraction_products, def:mpdo_vertical_fixed_point_products}
    Let $F$ be a linear endomorphism of
...
\begin{theorem}[Faithful fixed family in a finite product]
    ...
    This provides the intermediate step for the fixed-point argument in
    \cite[Appendix~C.4, lines~1980--1993]{Cirac2016MPDO_arXiv}.
\end{theorem}
...
\begin{theorem}[Identity criterion from fixed contractions]
    ...
    \leanok
    \uses{thm:mpdo_direct_sum_faithful_fixed_family, thm:mpdo_density_blocks_fixed_point_product_dimension, thm:mpdo_fixed_contractions_product_dimension}
    Let $F$ be a positive trace-preserving endomorphism of
...
\begin{proof}\leanok
    ...
    $\mathcal F=\Fix(F)$.
    Since $C_L(V_c)=\mathcal A$ and $V_c(X)\in\mathcal F$,
    Theorem~\ref{thm:mpdo_fixed_contractions_product_dimension} gives $F=\id_{\mathcal A}$.
\end{proof}
