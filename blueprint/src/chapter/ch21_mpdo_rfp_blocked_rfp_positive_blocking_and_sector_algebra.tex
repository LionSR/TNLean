...
\begin{lemma}[Blocked word evaluation of an MPO tensor]
    \label{lem:mpo_eval_word_block}
    \lean{MPOTensor.evalWord_blockTensor_ofFn}
    \leanok
    \uses{def:mpdo_physical_blocking, def:mpo_eval_word}
    Let $L\geq0$, and let $\sigma=(\sigma_1,\ldots,\sigma_m)$ and
    $\tau=(\tau_1,\ldots,\tau_m)$ be words of the same length $m$ in the
    blocked alphabet, so that each $\sigma_k$ and each $\tau_k$ is a word of
    length $L$ in $\{0,\ldots,d-1\}$. Write $\widehat\sigma$ and
    $\widehat\tau$ for the concatenations
    $\sigma_1\sigma_2\cdots\sigma_m$ and $\tau_1\tau_2\cdots\tau_m$, words of
    length $mL$. Then
    \begin{align}
        (M^{[L]})^{\sigma,\tau}
         & =M^{\widehat\sigma,\widehat\tau}.
        \notag
    \end{align}
...
\begin{proof}\leanok
    ...
    split off the first blocked letters $\sigma_1,\tau_1$: by
    Definition~\ref{def:mpdo_physical_blocking},
    $(M^{[L]})^{\sigma_1,\tau_1}=M^{\sigma_1,\tau_1}$, and since $\sigma_1$
    and $\tau_1$ have the same length $L$,
    Lemma~\ref{lem:mpo_eval_word_append} gives
    $M^{\sigma_1\widehat\sigma',\,\tau_1\widehat\tau'}
        =M^{\sigma_1,\tau_1}M^{\widehat\sigma',\widehat\tau'}$ for the
...
\begin{proof}\leanok
    \uses{lem:mpo_eval_word_append, lem:mpo_eval_word_block}
...
\begin{lemma}[Closed MPO entries with residual sites]
    ...
    words, and write $\widehat\sigma,\widehat\tau$ for the concatenations of
    blocked words $\sigma,\tau$. Then, for matching row and column
    configurations,
    \begin{align}
        \rho^{(p+mL+s)}(M)_{i^-\widehat\sigma i^+,\,
            j^-\widehat\tau j^+}
...
\begin{proof}\leanok
    ...
    segment as a word of blocked matrices (Lemma~\ref{lem:mpo_eval_word_block})
    gives
    \begin{align}
        M^{i^-\widehat\sigma i^+,\,j^-\widehat\tau j^+}
         & = M^{i^-,j^-}\cdot M^{\widehat\sigma,\widehat\tau}
        \cdot M^{i^+,j^+} \notag                              \\
