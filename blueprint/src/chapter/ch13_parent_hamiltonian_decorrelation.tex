...
The first source-faithful step toward the converse is independent of
decorrelation.  For a positive semidefinite operator on a bipartite space, the
...
\begin{definition}[Lift from the left tensor factor]
    \label{def:lift_left_factor}
    \lean{Matrix.liftLeftFactor}
    \leanok
    If $M$ acts on a finite-dimensional space $H_L$, its lift to
    $H_L\otimes H_R$ is
    \[
        M\otimes \mathbf 1_R.
    \]
...
\begin{lemma}[Adjoint identity for the right partial trace]
    ...
    \lean{Matrix.trace_liftLeftFactor_mul}
    \leanok
    \uses{def:lift_left_factor}
...
\begin{theorem}[Left absorption by the marginal support projector]
    ...
    \lean{Matrix.PosSemidef.liftLeftFactor_supportProj_mul_self}
    \leanok
    \uses{def:lift_left_factor,lem:trace_lift_left_factor_mul}
...
\begin{proof}
    \leanok
    The complementary projector $Q_L=\mathbf 1_L-P_L$ annihilates the
    marginal.  The partial-trace adjoint identity shows that its lift has zero
    expectation against $\rho$.  Positivity then implies
    $(Q_L\otimes\mathbf 1_R)\rho=0$, which is equivalent to the claim.
\end{proof}

\begin{theorem}[Right absorption by the marginal support projector]
    \label{thm:marginal_support_right_absorption}
    \lean{Matrix.PosSemidef.mul_liftLeftFactor_supportProj_self}
    \leanok
    \uses{def:lift_left_factor,thm:marginal_support_left_absorption}
...
\begin{proof}
    \leanok
    Take the adjoint of the left absorption identity and use that both $\rho$
    and the lifted support projector are Hermitian.
\end{proof}
