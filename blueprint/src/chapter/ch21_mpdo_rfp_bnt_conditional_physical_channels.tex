...
\begin{theorem}[BNT algebra clause implies a renormalization fixed point]
    ...
    Definition~\ref{def:is_rfp_via_ts}.  This establishes implication
    (ii)$\Rightarrow$(i) of
    \cite[Theorem~4.14, lines~972--993]{Cirac2016MPDO_arXiv}.

    \textbf{Local fix (mixed prefix gauge normalization):}
    The common marked comparison normalizing the exact sector gauges to unitaries
    is supplied by Theorem~\ref{thm:mpdo_bnt_mixed_prefix_unitary_sector_conjugacy};
    documented in \cite{gap:cpsv16_two_site_sector_unitary_gauge_gap}
    (\texttt{docs/paper-gaps/cpsv16_two_site_sector_unitary_gauge_gap.tex}).

    \textbf{Local fix (zero-sector complement):}
    The physical maps are completed on discarded complements as documented in
    \cite{gap:cpgsv17_vertical_isometry_zero_sector}
    (\texttt{docs/paper-gaps/cpgsv17_vertical_isometry_zero_sector.tex}).
\end{theorem}
