...
\begin{figure}[ht]
    \centering
    \[
        A^{i_1}A^{i_2}=\sum_jV^{i_1i_2}_jA^j
    \]
    \begin{center}
        ...
        % legs at={1,2} keeps one open up leg per column (i_1, i_2); the
        % vertical V-A bond it seals below is the summed index j.  Both
        ...
        \begin{tenkz}[rows={op:none, ket}]
            \tn[pill, wide=2, legs at={1,2}, up={$i_1$,$i_2$}]{V} & \\
            \tn[wide=2]{A^j} &
        \end{tenkz}
    \end{center}
    \medskip
    \[
        \sum_{i_1,i_2}(V^\dagger)^j_{i_1i_2}A^{i_1}A^{i_2}=A^j
    \]
...
\begin{proof}\leanok
    ...
    \[
        ...
        \qquad(n\ge 1).
    \]
    By normality, the words of some length $N$ span $\MN{D}$.  If $N\ge1$,
    the preceding inclusion shows that the one-site span is already
    $\MN{D}$.  If $N=0$, then $\spn\{\Id\}=\MN{D}$, and any nonzero $A^j$
    spans this space.  Thus $A$ is injective in either case.
\end{proof}
...
\begin{proof}\leanok
    ...
    \[
        \E_B(Y)=P_\rho(Y)
        =\frac{\operatorname{tr}(Y)}{\operatorname{tr}(\rho)}\rho.
    \]
    Set
    \[
        \Lambda_\alpha
          =\sqrt{\frac{D\rho_{\alpha,\alpha}}{\operatorname{tr}(\rho)}},
        \qquad
...
\begin{proof}\leanok
    ...
    \[
        \operatorname{tr}\!\left(N\E_A^2(Z)\right)
        =\operatorname{tr}\!\left(N\E_A(Z)\right).
    \]
...
\begin{proof}\leanok
    ...
    \[
        \begin{aligned}
        C(X,Y;n)
        &=\operatorname{tr}\!\left(Y\E_A^n(X\rho^R)\right)
          -\operatorname{tr}(X\rho^R)\operatorname{tr}(Y\rho^R)\\
        &=\operatorname{tr}\!\left(Y\E_A(X\rho^R)\right)
          -\operatorname{tr}(X\rho^R)\operatorname{tr}(Y\rho^R)\\
        &=C(X,Y;1).
        \end{aligned}
    \]
