...
\begin{definition}[Sequential scheme with an ancilla]
    ...
    \end{align}
    where $(A^{[k]}_i)_{\alpha\beta}=A^{[k]}_{i,\alpha,\beta}$.
    The scheme is
    \begin{enumerate}
...
\begin{lemma}[Isometric open-boundary representation]
    ...
    bound in which every site map is an isometry:
    \begin{align}
...
\begin{theorem}[Sequential generation with an ancilla]
    ...
    \uses{def:sequential_scheme_ancilla, lem:obc_left_isometric_representation,
    ...
    \end{enumerate}
    This is \cite[Theorem~``Sequential generation with ancilla'',
        Section~5.1]{PerezGarcia2007Matrix}.
\end{theorem}
\begin{proof}
...
\begin{theorem}[Sequential generation without an ancilla]
    ...
    \uses{def:sequential_scheme_no_ancilla, thm:sequential_generation_ancilla}
    ...
    \cite[Theorem~``Sequential generation without ancilla'',
        Section~5.2]{PerezGarcia2007Matrix}.
\end{theorem}
\begin{proof}
    ...
    as the ancilla of the $k$-th step, followed by a swap of sites $k+1$ and
    $k+2$.
\end{proof}
