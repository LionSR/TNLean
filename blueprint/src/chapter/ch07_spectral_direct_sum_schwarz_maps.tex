...
\begin{proof}
    ...
    trace adjoints determine a star-algebra isomorphism
    $\Phi:\mathcal A\simeq\mathcal B$. This isomorphism permutes the minimal
    nonzero central idempotents, and hence induces an equivalence
    $\sigma:I\simeq J$ with $\Phi(I_i)=J_{\sigma(i)}$. The restriction of
    $\Phi$ to these ideals is a complex-linear bijection
    \begin{align}
        \MN{D_i}
        &\simeq \MN{E_{\sigma(i)}}.
        \notag
    \end{align}
    Comparing complex dimensions gives $D_i^2=E_{\sigma(i)}^2$. Since all
    block dimensions are positive, $D_i=E_{\sigma(i)}$, which proves
