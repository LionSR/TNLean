...
\begin{theorem}[$\Z_2 \times \Z_2$ on-site symmetry of the blocked AKLT tensor]\label{thm:aklt_blocked_z2z2_symmetric}
    ...
    \leanok
    \uses{def:aklt_blocked, def:on_site_symmetric, thm:aklt_z2z2_symmetric}
    The length-$2$ blocked AKLT tensor is on-site symmetric under
    $\Z_2 \times \Z_2$.  Each group element acts on the $9$-dimensional blocked
    physical space by the tensor square $P_g \otimes P_g$ of the corresponding
    single-site spin-$1$ $\pi$-rotation $P_g$.  Blocking preserves the matrix
    product vector family, so the symmetry of the single-site tensor
    (Theorem~\ref{thm:aklt_z2z2_symmetric}) lifts to the blocked tensor.
\end{theorem}
...
\begin{theorem}[The AKLT state has string order under its $\Z_2 \times \Z_2$ symmetry]\label{thm:aklt_string_order}
    ...
    \leanok
    \uses{def:has_string_order, def:aklt_blocked}
...
\begin{proof}
    ...
    \uses{thm:has_string_order_of_symmetric_injective, thm:aklt_blocked_injective,
        thm:aklt_blocked_z2z2_symmetric}
    ...
    $\sum_i (A^i)^\dagger A^i = \Id$; these identities propagate to length-$2$
    words, so the blocked transfer map is unital and $\Lambda$ is a fixed point of
    its adjoint.  These are exactly the hypotheses of
