...
\begin{theorem}[The blocked AKLT tensor is injective]\label{thm:aklt_blocked_injective}
    ...
    $\MN{2}$.  This is the blocked-tensor form of the normality of the
    single-site tensor (Theorem~\ref{thm:aklt_normal}).
\end{theorem}
...
\begin{theorem}[$\Z_2 \times \Z_2$ on-site symmetry of the blocked AKLT tensor]\label{thm:aklt_blocked_z2z2_symmetric}
    ...
    $P_g$ (Definition~\ref{def:aklt_blocked_z2z2_action}).  Writing $A$ for the
    single-site tensor and $A^{[2]}$ for its length-$2$ blocking, twisting the
    blocked tensor by $P_g \otimes P_g$ equals blocking the twisted single-site
    tensor,
    \[
...
\begin{proof}
    ...
    \uses{thm:has_string_order_of_symmetric_injective, thm:aklt_blocked_injective,
        thm:aklt_blocked_z2z2_symmetric, def:transfer_map}
