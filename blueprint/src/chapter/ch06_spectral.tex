...
\begin{proof}
    ...
    $B^i = \bar{\mu} X^{-1} A^i X$ for all~$i$.

    In~\cite{PerezGarcia2007Matrix}, this result is stated as
    part of the proof of the single-block fundamental theorem.
    The argument above follows the same strategy
    (KS inequality $\to$ multiplicative domain $\to$ Skolem--Noether)
    adapted to the mixed transfer setting.
\end{proof}
...
\begin{theorem}[Self-correlation persists]\label{thm:self_persist}
    ...
    If $\rho$ is the unique PSD fixed point of $\E_A$, then
...
\begin{proof}
    ...
    Since $\rho(E - P) < 1$, $(E - P)^n \to 0$,
    so $\tr(E^n) \to \tr(P)$.
    Trace preservation gives $\tr(P(X)) = \tr(X) \cdot
    \frac{\tr(\rho)}{\tr(\rho)} = \tr(X)$ for the
    trace functional, so $\tr(P) = 1$.
\end{proof}
