...
\begin{theorem}[Rectangular intertwining forces equal bond dimension]%
    ...
    $\mu \in \mathbb{C}$ such that, for every physical index~$i$,
    \[
        X (B^i)^\dagger = \mu \, (A^i)^\dagger X,
        \qquad
        A^i X = \mu \, X B^i
...
\begin{proof}\leanok
    ...
    $K_{j,i}=\ket{j}\bra{i}\,S$ indexed by $(j,i)\in\{1,\dots,D\}^2$.
    Using $\ket{j}\bra{i}\,M\,\ket{i}\bra{j} = M_{ii}\,\ket{j}\bra{j}$ and
    summing first over $i$ and then over $j$,
    \[
        \sum_{j,i} K_{j,i}\,X\,K_{j,i}^\dagger
        = \sum_j \ket{j}\bra{j}\,\operatorname{tr}(SXS)
        = \operatorname{tr}(\sigma X)\,\Id,
    \]
    where we used cyclicity of the trace and $S^2=\sigma$.  Hence
    $E_\sigma = (\operatorname{tr}\sigma)^{-1}\cdot\bigl(X\mapsto\sum_{j,i}K_{j,i}XK_{j,i}^\dagger\bigr)$
    is the Kraus map scaled by the nonnegative real number
...
\begin{theorem}[$\sigma$-trace preservation of the scalar conditional expectation]
    ...
    \[
        \operatorname{tr}\bigl(\sigma\,E_\sigma(X)\bigr)
        = \operatorname{tr}(\sigma X).
    \]
\end{theorem}
\begin{proof}\leanok
    Direct computation:
    $\operatorname{tr}\bigl(\sigma\cdot\tfrac{\operatorname{tr}(\sigma X)}{\operatorname{tr}\sigma}\Id\bigr)
    = \tfrac{\operatorname{tr}(\sigma X)}{\operatorname{tr}\sigma}\operatorname{tr}\sigma
    = \operatorname{tr}(\sigma X)$.
\end{proof}
...
\begin{proof}
    ...
    hence Artinian. By~\cite[Thm.~6.14]{Wolf2012Quantum}, it suffices to show
