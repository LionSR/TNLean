...
\begin{theorem}[Trace of iterated mixed transfer]\label{thm:mixed_trace_identity}
    \lean{MPSTensor.trace_mixedTransferMap_pow_identity}
    \leanok
    \uses{thm:mixed_pow, def:coeff}
    For all $X \in \MN{D}$ and $N \ge 0$,
    \[
        \tr(F_{AB}^N(X))
        = 
...
\begin{proof}\leanok\uses{thm:mixed_pow}
    Apply the trace to each summand in
    Theorem~\ref{thm:mixed_pow} and use
    $\tr(M N^\dagger) = \sum_{p,q} M_{pq} \overline{N_{pq}}$.
\end{proof}
