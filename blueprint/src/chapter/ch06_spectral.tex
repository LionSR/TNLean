...
\begin{lemma}[Wedderburn block decomposition for $*$-subalgebras]%
    \label{lem:starSubalgebra_hasWedderburnBlockDecomp}
    \lean{Kraus.starSubalgebra_hasWedderburnBlockDecomp}
    \leanok
    \uses{thm:fixedPointAlgebra_isSemisimpleRing}
    Every finite-dimensional $*$-subalgebra of~$\MN{D}$ admits a Wedderburn
    block decomposition: there exist block dimensions $d_1,\dots,d_n$ with
    $\sum_k d_k \le D$ and a $\C$-algebra isomorphism to
    $\prod_{k=1}^{n} M_{d_k}(\C)$.
\end{lemma}
...
\begin{proof}
    \leanok
    \uses{thm:fixedPointAlgebra_isSemisimpleRing}
    The subalgebra is semisimple and finite-dimensional, so the
    Wedderburn--Artin theorem gives an isomorphism
    $S \cong \prod_{k} M_{d_k}(\C)$. The dimension bound
    $\sum_k d_k \le D$ follows by transporting the diagonal matrix units
    from each factor into~$\MN{D}$: they form pairwise orthogonal nonzero
    idempotents, whose column images are linearly independent.
\end{proof}
...
\begin{theorem}[Fixed-point algebra admits Wedderburn block decomposition]%
    ...
    \leanok
    \uses{lem:starSubalgebra_hasWedderburnBlockDecomp}
...
\begin{proof}
    ...
    Lemma~\ref{lem:starSubalgebra_hasWedderburnBlockDecomp} to the
    adjoint-fixed-point $*$-subalgebra.
\end{proof}
