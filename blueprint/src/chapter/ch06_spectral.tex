...
\begin{proof}
    ...
    Choose positive fixed points of the transfer maps of $A$ and $B$
    (Theorem~\ref{thm:qpf}) and gauge the two tensors separately to
    unital Kraus families (Theorem~\ref{thm:ds_gauge_unital}).
    Gauge $X$ accordingly and embed it as an off-diagonal block matrix
    for the block Kraus family with diagonal blocks $A'^i$ and $B'^i$.
    The KS equality for peripheral eigenvectors
    (Theorem~\ref{thm:ks_peripheral}) and the resulting Kraus
    commutation relation (Theorem~\ref{thm:kraus_commute}) yield
    intertwining identities between $A'^i$, $B'^i$, and the gauged
    eigenvector. Injectivity and the multiplicative-domain argument
    force this intertwiner to be invertible; Skolem--Noether
...
\begin{proof}
    ...
    modulus-one eigenvector $X$.

    Gauge $A$ and $B$ separately to unital Kraus families and apply the
    same block-embedding Kadison--Schwarz argument as in the square case.
    It shows that $\ker(X)$ is invariant under all matrices on
    $\C^{D_2}$, so $X$ is injective and therefore $D_2 \le D_1$.
