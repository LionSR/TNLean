...
\begin{proof}
    ...
    The map $M \mapsto X' M (X')^{-1}$ is a $\CC$-algebra automorphism
...
\begin{proof}
    ...
    modulus-one eigenvector $X \in M_{D_1 \times D_2}(\CC) \setminus \{0\}$.
    ...
    for all~$i$.
    Now consider $\ker(X') \subseteq \CC^{D_2}$. If $v \in \ker(X')$,
    then $X' B'^i v = A'^i X' v = 0$, so $B'^i v \in \ker(X')$ for
    every~$i$.  Since $B$ is injective, $\{B'^i\}$ spans $M_{D_2}(\CC)$,
    so $\ker(X')$ is invariant under every $D_2 \times D_2$ matrix and
    is therefore either $\{0\}$ or $\CC^{D_2}$.  Since $X' \ne 0$,
    we have $\ker(X') = \{0\}$, so $X'$ is injective as a linear
    map $\CC^{D_2} \to \CC^{D_1}$ and $D_2 \le D_1$.
