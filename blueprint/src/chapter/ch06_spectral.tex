...
\begin{proof}
    ...
    for all~$n$.
    Since $F_{AB}^n(X) = \mu^n X$, the left-hand side equals
    $|\mu|^{2n}\|X\|_{\mathrm{HS}}^2$.
    Dividing by $\|X\|_{\mathrm{HS}}^2 > 0$ gives
    $|\mu|^{2n} \le 1$ for all $n \ge 0$,
    hence $|\mu| \le 1$.
...
\begin{theorem}[Block separation principle]\label{thm:block_separation}
    ...
    equivalent. If $\tr(F_{AB}^N(\Id)) = 0$ for all $N \ge 1$,
    then $F_{AB}(\Id) = 0$.
\end{theorem}

\begin{proof}\leanok
    Taking $N = 1$ gives $\tr(F_{AB}(\Id)) = 0$.
    The full sequence of vanishing trace conditions for all
    $N \ge 1$, together with the cross-correlation decay
    (Theorem~\ref{thm:cross_decay}), forces the spectral
    components of $F_{AB}(\Id)$ to all vanish,
    giving $F_{AB}(\Id) = 0$.
\end{proof}
