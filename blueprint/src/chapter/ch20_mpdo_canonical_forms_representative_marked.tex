...
\begin{theorem}[Representative-grouped Lemma L]
    ...
    \leanok
    \uses{def:representative_word_tuple_span, def:sector_weight_data, cor:nonvanishing_finite_power_sum}
...
\begin{proof}
    \leanok
    \uses{def:representative_word_tuple_span, lem:paper_decomp_formula, cor:nonvanishing_finite_power_sum}
...
\begin{theorem}[Algebraic marked-letter extension of representative separation]
    ...
    \leanok
    \uses{def:representative_word_tuple_span, def:marked_representative_direct_sum, lem:marked_representative_trace_expansion, cor:nonvanishing_finite_power_sum}
    Let $(C_j^s)$ and $(E_j^s)$ be two marked families over the same sector
    decomposition. Fix a tail length $L$, and suppose that, for every marked
    index $s$ and every unmarked word $w$ of length $L$,
    \begin{align}
        \tr\!\left(C_{\mathrm{tot}}^s A_{\mathrm{tot}}^w\right)
    ...
    representative word-tuple separation, agreement for every marked index
    and every positive-length unmarked word implies $C_j=E_j$ for every $j$.

    This extends the argument of \cite[Appendix~C.3, lines~1848--1858]{Cirac2016MPDO_arXiv}
    from first-site physical marks to general marked matrices in the marked slot.
\end{theorem}

\begin{proof}
    \leanok
    \uses{def:representative_word_tuple_span, lem:marked_representative_trace_expansion, cor:nonvanishing_finite_power_sum}
...
\begin{theorem}[Full-coordinate marked trace identity]
    ...
    \leanok
    \uses{thm:cpsv_canonical_form_bnt_refinement, thm:cpsv_representative_sector_decomposition, def:marked_representative_direct_sum, lem:marked_representative_trace_expansion}
    Let $T_{\mathrm{grp}}$ be the full grouped tensor of
    Theorem~\ref{thm:cpsv_canonical_form_bnt_refinement}, and let
    $P_{\mathrm{rep}}$ be its representative sector decomposition.
    For a marked family $(C_j^s)_j$, define the marked tensor
    $\widehat C_{\mathrm{grp}}$ on all grouped listed coordinates as follows.
    The copy $(j,q)$ carries the marked block
    ...
    \begin{align}
        \tr\!\left(\widehat C_{\mathrm{grp}}^sT_{\mathrm{grp}}^w\right)
         & =\tr\!\left(C_{\mathrm{rep}}^sP_{\mathrm{rep}}^w\right)
        \notag                                                     \\
         & =\sum_j\left(\sum_q\lambda_{j,q}^{|w|+1}\right)
        \tr\!\left(C_j^sC_j^w\right).
        \label{eq:cpsv_full_coordinate_marked_trace_identity}
    \end{align}
    The exponent $|w|+1$ includes the factor at the marked site.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:cpsv_representative_sector_decomposition, lem:marked_representative_trace_expansion}
    Reindex the finite sum over listed coordinates by the
    class-copy pairs $(j,q)$ and use the bond-dimension identifications to
    replace each block by its unit-phase representative. The marked
...
\begin{theorem}[Retained-coordinate representative-grouped Lemma L]
    ...
    \leanok
    \uses{thm:cpsv_full_coordinate_marked_trace_identity, thm:cpsv_representative_sector_decomposition, thm:representative_grouped_marked_separation, thm:representative_grouped_insert_eq}
    Let $(C_j^s)_j$ and $(E_j^s)_j$ be two marked families on the
    representatives. Form their full-coordinate marked tensors. If
    \begin{align}
        \tr\!\left(\widehat C_{\mathrm{grp}}^sT_{\mathrm{grp}}^w\right)
         & =\tr\!\left(\widehat E_{\mathrm{grp}}^sT_{\mathrm{grp}}^w\right)
        \label{eq:cpsv_retained_marked_trace_agreement}
    \end{align}
    for every $s$ and every word $w$ of positive length, then $C_j=E_j$ for
    every representative $j$.
    ...
    \end{align}
    for every representative $j$ and every physical letter $i$.
\end{theorem}
...
\begin{proof}\leanok
    ...
    agreement for $P_{\mathrm{rep}}$. Its representatives have eventual
    ...
    $P_{\mathrm{rep}}$ because their matrix product vectors agree at every
...
\begin{theorem}[Original-coordinate marked Lemma L]
    ...
    \leanok
    \uses{thm:cpsv_canonical_form_bnt_refinement, thm:cpsv_full_coordinate_marked_trace_identity, thm:cpsv_retained_representative_grouped_lemma_l}
    Let $A$ be in literal CPSV canonical form. For canonical-form data and
    its BNT refinement, write
    \begin{align}
        A^i        & =V^\dagger B^iV,
        \notag                        \\
        VV^\dagger & =\Id.
    ...
    \end{align}
    where $B^i=GT_{\mathrm{grp}}^iG^{-1}$ and $G$ is the listed block gauge.
    If marked matrices satisfy $C=V^\dagger EV$, then for every
    word $w$, including the empty word,
    \begin{align}
        \tr(CA^w) & =\tr(EB^w).
        \label{eq:cpsv_original_marked_trace}
    \end{align}
    The inserted marked matrix ensures that the closed chain is nonempty.

    For a linear physical mark $f$, the linear marking of
    $T_{\mathrm{grp}}$ is exactly its full grouped marked tensor. Each
    copy $(j,q)$ carries
    $\lambda_{j,q}f(C_j)$, where
    $\lambda_{j,q}=\nu_{k(j,q)}\zeta_{k(j,q)}$. For two linear marks $f$ and $g$, put
    ...
    \end{align}
    for every marked letter $u$ and every word $w$ of positive length, then
    $F_A=G_A$ on the original bond space.

    This positive-tail formulation corresponds to the argument in
    \cite[Appendix~C.3, lines~1835--1858]{Cirac2016MPDO_arXiv}: first-site agreement
    at positive system sizes suffices to separate representatives at a nonzero tail
    length without requiring empty-word data.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:cpsv_full_coordinate_marked_trace_identity, thm:cpsv_retained_representative_grouped_lemma_l}
    ...
    gives the grouped marked tensor.
    ...
    representatives. The resulting equality of the full grouped
...
\begin{lemma}[First-site actions under physical blocking]
    ...
    \leanok
    \uses{def:first_site_action_physical_blocking, def:blocked_tensor, def:l_blk_injective}
    This identity connects the block-injectivity argument
    \cite[lines~318--345]{Cirac2016MPDO_arXiv} with Lemma~L
    \cite[Appendix~C.3, lines~1835--1858]{Cirac2016MPDO_arXiv}.
...
\begin{theorem}[Representative separation after injective blocking]
    ...
    \leanok
    \uses{def:sector_bnt_cf, def:sector_decomposition_block_tensor, def:representative_word_tuple_span, thm:representative_grouped_insert_eq}
...
\begin{proof}
    \leanok
    \uses{lem:bnt_direct_sum_block_separation_word_span, lem:block_separating_equations_from_pairwise_separation, lem:product_word_span_from_bnt_block_separation, thm:representative_grouped_insert_eq}
    ...
    representatives. The preceding selector argument then applies to $P^{[p]}$.
...
\begin{corollary}[Marked-letter separation for a BNT canonical form]
    ...
    \leanok
    \uses{def:sector_bnt_cf, thm:postblocked_representative_grouped_insert_eq, thm:representative_grouped_marked_separation}
    ...
    every unmarked word $w$ of positive length, then $C_j^s=E_j^s$ for every
    representative $j$ and every marked index $s$.
\end{corollary}

\begin{proof}
    \leanok
    \uses{thm:postblocked_representative_grouped_insert_eq, thm:representative_grouped_marked_separation}
...
\begin{corollary}[Physical-letter marked separation in BNT-refined horizontal form]
    ...
    \leanok
    \uses{def:horizontal_cf_mpdo, cor:bnt_marked_representative_separation}
    ...
    $w$ of positive length, then $C^u=E^u$ for every $u$.

    Restricting the marks to the span of the physical letters isolates the
    physical-insertion consequence of
    \cite[Appendix~C.3, lines~1835--1858]{Cirac2016MPDO_arXiv} needed for the
    contractions in \cite[Proposition~4.13, lines~1909--1919]{Cirac2016MPDO_arXiv}.
\end{corollary}
