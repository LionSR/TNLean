...
\begin{definition}[Multiplicity spaces for a fixed final label]%
    ...
    $F$ in~\cite[Section~3.4]{Bultinck2017Anyons}. Under the length-independent
    integer specialization, the dimensions of the positive diagonal matrices
    match those fusion multiplicities. The corresponding spaces for a fixed
    final label are $\mathcal H^{\mathrm L}_{\varepsilon}\otimes\C^{D_\varepsilon}$
    and $\mathcal H^{\mathrm R}_{\varepsilon}\otimes\C^{D_\varepsilon}$.
\end{definition}
...
\begin{theorem}[Equality of multiplicity dimensions for a fixed final label]%
    ...
    establishing the numerical associativity statement of
...
\begin{theorem}[Intertwiners at an injective final sector]%
    ...
    Here the Kronecker decomposition holds for any given intertwiner $C$ between
    the fixed-final-label representations.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Unitarity of the multiplicity matrix at a fixed final label]%
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Thus $F_\varepsilon$ is invertible (with opposite orientation to the
...
\begin{figure}[ht]
    ...
    \caption{The fixed-final comparison in the graphical order of
        ...
        described in \cite[lines~995--1010]{Cirac2016MPDO_arXiv}.}
