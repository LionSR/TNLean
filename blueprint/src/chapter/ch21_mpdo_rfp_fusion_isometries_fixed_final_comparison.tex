\begin{definition}[One fusion layer with full support]%
    ...
    Suppose that the fusion isometries have full support, and let $P_\gamma$
    act on the bond space of $M_\gamma$. Define
    \begin{align}
        ...
        \notag
    \end{align}
    The terminal contraction is unchanged. Full support means that the
    active support is the whole product bond space.

    Full support is an additional restriction here, not a hypothesis of the
    source statement for arbitrary chain lengths. This distinction is
    documented in
    \cite{gap:cpsv16_topological_projector_recursion}.
\end{definition}

\begin{theorem}[Projection property with full support]%
    ...
    \leanok
    \uses{def:mpdo_full_support_virtual_projector_one_fusion, thm:mpdo_pair_fusion_bnt_completeness}
    Suppose that $\{M_\alpha\}_\alpha$ is a BNT and that the structure
    coefficients
    $c_{\alpha,\beta,\gamma}^{(L)}
        =\tr(\chi_{\alpha,\beta,\gamma}^{L})$,
    with positive diagonal $\chi_{\alpha,\beta,\gamma}$, are independent of
    $L>0$. If each $P_\gamma$ is a self-adjoint idempotent on the bond space,
...
\begin{proof}\leanok
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Conjugation also preserves self-adjointness.
\end{proof}
...
\begin{theorem}[Left iterated fusion completeness]%
    ...
    \leanok
    \uses{thm:mpdo_pair_fusion_selector_completeness, def:mpdo_left_fusion_isometry}
...
\begin{theorem}[Right iterated fusion completeness]%
    ...
    \leanok
    \uses{thm:mpdo_pair_fusion_selector_completeness, def:mpdo_right_fusion_isometry}
...
\begin{theorem}[Right adjoint inverse for the full triple-fusion comparison]%
    ...
    \leanok
    \uses{lem:mpdo_triple_fusion_comparison_left_range, thm:mpdo_left_fusion_selector_completeness}
...
\begin{theorem}[Left adjoint inverse for the full triple-fusion comparison]%
    ...
    \leanok
    \uses{lem:mpdo_triple_fusion_comparison_right_range, thm:mpdo_right_fusion_selector_completeness}
    Under the same hypotheses,
    $C_{\alpha,\beta,\gamma}^{\dagger}C_{\alpha,\beta,\gamma}=1$.
    This proves invertibility on the full direct sums, but not yet
    invertibility of the multiplicity matrix for a fixed final label.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Vanishing between distinct final sectors]%
    ...
    \leanok
    \uses{def:mpdo_triple_fusion_comparison, def:mpdo_final_label_selector_words}
    Suppose that the structure coefficients
    $c_{\alpha,\beta,\gamma}^{(L)}
        =\tr(\chi_{\alpha,\beta,\gamma}^{L})$,
    with positive diagonal $\chi_{\alpha,\beta,\gamma}$, are independent of
    $L>0$ and that common word selectors exist. If
    $\varepsilon\ne\varepsilon'$, then the block of the full comparison from
    the right final sector $\varepsilon'$ to the left final sector
    $\varepsilon$ vanishes:
    $P^{\mathrm L}_{\varepsilon}
        C_{\alpha,\beta,\gamma}
        P^{\mathrm R}_{\varepsilon'}=0$.
    The word-selector identities are additional hypotheses. This result
    does not establish invertibility of a diagonal block, identify it with
    an $F$-matrix, or prove the pentagon identity.
\end{theorem}
\begin{proof}\leanok
    ...
    the corresponding bond-space block of the full letterwise identity gives
    $M_\varepsilon^{ij}X=X M_{\varepsilon'}^{ij}$.
    ...
    the whole off-diagonal final-sector block vanishes.
\end{proof}

\begin{definition}[Multiplicity spaces for a fixed final label]%
    ...
    $(f,\lambda,\sigma)$ index spaces in the row and column convention for
    $F$ in~\cite[Section~3.4]{Bultinck2017Anyons}. Identifying the dimensions
    of the positive diagonal matrices with those fusion multiplicities
    requires the length-independent integer specialization and is not
    asserted here. The corresponding spaces for a fixed final label are
    $\mathcal H^{\mathrm L}_{\varepsilon}\otimes\C^{D_\varepsilon}$ and
    $\mathcal H^{\mathrm R}_{\varepsilon}\otimes\C^{D_\varepsilon}$.
\end{definition}

\begin{theorem}[Equality of multiplicity dimensions for a fixed final label]%
    ...
    \leanok
    \uses{def:mpdo_final_sector_multiplicity_spaces, def:mpdo_bnt_label_linear_independent, def:mpdo_bnt_label_length_independent}
    Suppose that the structure coefficients
    $c_{\alpha,\beta,\gamma}^{(L)}
        =\tr(\chi_{\alpha,\beta,\gamma}^{L})$,
    with positive diagonal $\chi_{\alpha,\beta,\gamma}$, are independent of
    $L>0$ and that the operators $O_L(M_\alpha)$ are linearly independent
    for all sufficiently large $L$. Then, for each final label $\varepsilon$,
    \begin{align}
        ...
        \notag
    \end{align}
    Consequently a matrix between these two multiplicity spaces is square.
    This is the numerical associativity statement of
    \cite[lines~237--241]{Bultinck2017Anyons}; it does not assert completeness
    of the fusion maps or invertibility of their comparison at a fixed final
    label.
\end{theorem}
\begin{proof}\leanok
    \uses{thm:mpdo_bnt_label_coeff_associativity, thm:mpdo_bnt_label_length_independent_natural}
    Choose a positive length beyond the linear-independence threshold.
    Associativity of the product of the operators $O_L(M_\alpha)$ gives
    equality of the two sums of products of structure coefficients.
    Length independence and positivity identify every coefficient with the
    size of its corresponding diagonal $\chi$-matrix. The two sums are
    precisely the dimensions of the two multiplicity spaces.
\end{proof}
...
\begin{definition}[Triple-fusion maps with fixed final label]%
    ...
    \leanok
    \uses{def:mpdo_final_sector_multiplicity_spaces, def:mpdo_left_fusion_isometry, def:mpdo_right_fusion_isometry}
    The maps $U^{\mathrm L}_{\alpha,\beta,\gamma;\varepsilon}$ and
    $U^{\mathrm R}_{\alpha,\beta,\gamma;\varepsilon}$ are obtained by
    restricting the rows of the two iterated fusion isometries to the
    summands with final label $\varepsilon$. After the canonical
    reassociation of the triple bond space, their adjoints have the
    orientation and bracketing pattern of weighted versions of the two
    fusion maps in~\cite[Section~3.4]{Bultinck2017Anyons}.
\end{definition}
...
\begin{theorem}[Left triple fusion at a fixed final label]%
    ...
    \leanok
    \uses{def:mpdo_final_sector_fusion_maps, def:mpdo_operator_product, thm:mpdo_left_fusion_apply, lem:matrix_block_conjugation_restriction}
...
\begin{proof}\leanok
    ...
    precisely the summands with final label $\varepsilon$.
\end{proof}
...
\begin{theorem}[Right triple fusion at a fixed final label]%
    ...
    \leanok
    \uses{def:mpdo_final_sector_fusion_maps, def:mpdo_operator_product, thm:mpdo_right_fusion_apply, lem:matrix_block_conjugation_restriction}
...
\begin{theorem}[Intertwiners at an injective final sector]%
    ...
    right final-sector space to the left final-sector space. Under
    ...
    This conclusion concerns the assumed intertwiner $C$; it does not
    construct an invertible $C$ or $F$. It also does not identify the
    dimensions of the positive diagonal matrices $\chi$ with the fusion
    multiplicities in the row and column convention for $F$
    in~\cite[Section~3.4]{Bultinck2017Anyons}.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Kronecker form and separation at a fixed final label]%
    ...
    \leanok
    \uses{def:mpdo_triple_fusion_comparison, def:mpdo_final_label_selector_words, def:mpdo_final_sector_multiplicity_spaces}
    Suppose that the structure coefficients
    $c_{\alpha,\beta,\gamma}^{(L)}
        =\tr(\chi_{\alpha,\beta,\gamma}^{L})$,
    with positive diagonal $\chi_{\alpha,\beta,\gamma}$, are independent of
    $L>0$ and that common word selectors separate the final labels.
    ...
    for the block from the right final sector $\varepsilon'$ to the left
    final sector $\varepsilon$. There is a rectangular matrix
    $F_\varepsilon:\mathcal H^{\mathrm R}_{\varepsilon}
        \to\mathcal H^{\mathrm L}_{\varepsilon}$ such that, after the canonical
    ...
    $F$-matrix of~\cite[Section~3.4]{Bultinck2017Anyons}; no pentagon identity
    is asserted.
\end{theorem}
\begin{proof}\leanok
    \uses{thm:mpdo_triple_fusion_comparison_intertwines, thm:mpdo_triple_fusion_comparison_final_sector_vanishing, thm:mpdo_injective_fixed_final_intertwiner}
    Taking the $\varepsilon$--$\varepsilon$ block of the full comparison
...
\begin{theorem}[Unitarity of the comparison at each final sector]%
    ...
    \leanok
    \uses{def:mpdo_final_label_selector_words, def:mpdo_final_sector_multiplicity_spaces, def:mpdo_triple_fusion_comparison}
    Suppose that the structure coefficients
    $c_{\alpha,\beta,\gamma}^{(L)}
        =\tr(\chi_{\alpha,\beta,\gamma}^{L})$,
    with positive diagonal $\chi_{\alpha,\beta,\gamma}$, are independent of
    $L>0$ and that common selectors exist at a positive length.
    For every final label $\varepsilon$, write
    $C_\varepsilon
        =P^{\mathrm L}_{\varepsilon}
        C_{\alpha,\beta,\gamma}
        P^{\mathrm R}_{\varepsilon}$. Then
    \begin{align}
        C_\varepsilon C_\varepsilon^\dagger
         & =1,
        \label{eq:mpdo_final_sector_right_unitary} \\
        C_\varepsilon^\dagger C_\varepsilon
         & =1.
        \label{eq:mpdo_final_sector_left_unitary}
    \end{align}
    The simultaneous selector identities are additional hypotheses.
    The conclusion gives invertibility at a fixed final label, as obtained
    from the simultaneous inverse in~\cite[lines~247--277]{Bultinck2017Anyons}.
\end{theorem}
\begin{proof}\leanok
    \uses{thm:mpdo_triple_fusion_comparison_right_inverse, thm:mpdo_triple_fusion_comparison_left_inverse, thm:mpdo_triple_fusion_comparison_final_sector_vanishing}
    Order each full direct sum first by its final label. In the product
    $CC^\dagger$, the $\varepsilon$-diagonal block is
    $\sum_{\varepsilon'}
        C^{\varepsilon,\varepsilon'}
    ...
    $\varepsilon$-diagonal block of $C^\dagger C=1$ gives
    (\ref{eq:mpdo_final_sector_left_unitary}).
\end{proof}

\begin{theorem}[Unitarity of the multiplicity matrix at a fixed final label]%
    \label{thm:mpdo_triple_fusion_multiplicity_unitary}
    \lean{MPOTensor.BNTFusionIsometryFamily.%
        exists_tripleFusionComparison_finalSector_eq_kronecker_one_and_unitary}
    \leanok
    \uses{def:mpdo_final_label_selector_words, def:mpdo_final_sector_multiplicity_spaces, def:mpdo_triple_fusion_comparison, lem:matrix_kronecker_identity_cancellation}
    ...
    $F_\varepsilon:\mathcal H^{\mathrm R}_{\varepsilon}
        \to\mathcal H^{\mathrm L}_{\varepsilon}$ such that
    \begin{align}
        C_\varepsilon
    ...
    row and column convention for $F$ in
    \cite[Section~3.4]{Bultinck2017Anyons}; no pentagon
    identity is asserted.
\end{theorem}
\begin{proof}\leanok
    \uses{thm:mpdo_triple_fusion_comparison_final_sector_kronecker, thm:mpdo_triple_fusion_comparison_final_sector_unitary, lem:matrix_kronecker_identity_cancellation}
    Injectivity gives
    $C_\varepsilon=F_\varepsilon\otimes1_{D_\varepsilon}$.
    Substituting this expression into
    (\ref{eq:mpdo_final_sector_right_unitary}) and
    (\ref{eq:mpdo_final_sector_left_unitary}) gives
    \begin{align}
        (F_\varepsilon F_\varepsilon^\dagger)
        \otimes1_{D_\varepsilon}
         & =1,
        \notag \\
        (F_\varepsilon^\dagger F_\varepsilon)
        \otimes1_{D_\varepsilon}
...
\begin{definition}[Simultaneous MPO block left inverse]
    ...
    \leanok
    Let $M_c^{il}\in \MN{D_c}$ be a finite family of MPO tensors.
    A simultaneous block left inverse consists of coefficients
...
\begin{theorem}[Simultaneous inverse from a one-letter product-algebra span]%
    ...
    \leanok
    \uses{def:mpdo_block_left_inverse, thm:cpsv_bnt_blocked_simultaneous_span}
    Let $A_c^q\in \MN{D_c}$ be a finite family of tensors. If
    the one-letter tuples $(A_c^q)_c$, indexed by $q$, span the full product
    algebra $\prod_c \MN{D_c}$, then there are coefficients
    $C_{(c,x,y),q}$ such that
    \begin{align}
    ...
    coefficients form the simultaneous block left inverse used in
    \cite[lines~269--277]{Bultinck2017Anyons}. For a BNT, the full
    product-algebra span is obtained after the common blocking described in
...
\begin{proof}\leanok
    ...
    its $(x',y')$ entry in block $d$ gives
    $\sum_q C_{(c,x,y),q}(A_d^q)_{x'y'}
        =\delta_{cd}\delta_{xx'}\delta_{yy'}$.
    Reindexing the doubled physical letter by the ket--bra pair gives the
    MPO form.
\end{proof}
