%% ==================================================
%% Chapter: Polar decomposition of a blocked tensor
...
\chapter{Polar Decomposition of a Blocked Tensor}\label{ch:ldpolar}

The log-depth preparation of matrix product states by Malz, Styliaris, Wei
and Cirac (arXiv:2307.01696) starts from the exact identity $B=VP$ for the
tensor $B$ of $q$ blocked sites, read as a map from the virtual pair space
$\C^{D^2}$ to the physical space. This chapter records the polar
...
\section{Polar decomposition of a matrix}
...
\begin{definition}[Polar factors of a matrix]
    ...
    \leanok
    Let $M\in\C^{m\times k}$. Write $f(M^\dagger M)$ for the functional
    calculus of the positive semidefinite matrix $M^\dagger M$. The
    \emph{positive part} is $P=(M^\dagger M)^{1/2}$, the \emph{support
        projector} is $\Pi=\chi(M^\dagger M)$ with $\chi(0)=0$ and $\chi(x)=1$
    for $x\ne 0$, and the \emph{partial isometry} is
    $V=M\,g(M^\dagger M)$ with $g(x)=x^{-1/2}$ for $x>0$ and $g(0)=0$.
    This is the decomposition $B=VP$ of arXiv:2307.01696, paragraph
    ``Approximation through the fixed-point state'' and Supplemental
...
\section{A tensor as a map on the virtual pair space}
...
\begin{proof}
    ...
    evaluating it on the matrix units gives $x=0$. arXiv:2307.01696 assumes
...
\section{Polar factors of a tensor}
...
\begin{definition}[Polar factors of a tensor]
    ...
    tensor $P$ of arXiv:2307.01696, eqs.~(eq:key\_approximation)
    and~(eq:phi\_pos).
\end{definition}
...
\begin{theorem}[Polar decomposition of a tensor]
    ...
    $B=P_V$. Moreover $V^\dagger V=\Pi$, the matrix $P$ is positive
    semidefinite, $\Pi$ is a Hermitian idempotent, and
...
\begin{theorem}[Transfer map of the positive part]
    ...
    the first equality of arXiv:2307.01696, eq.~(eq:B\_TM), which is stated
...
\begin{theorem}[Polar factors of an injective tensor]
    ...
    definite, as in arXiv:2307.01696, paragraph ``Approximation through the
    fixed-point state''.
\end{theorem}
\begin{proof}
    \leanok
...
\begin{proof}
    \leanok
    \uses{thm:ldpolar_transfer}
    $\E_P=\E_{A^{[q]}}$ by Theorem~\ref{thm:ldpolar_transfer}, and
    $\E_{A^{[q]}}=\E_A^q$, as noted after arXiv:2307.01696, eq.~(eq:B).
\end{proof}
...
\begin{theorem}[Isometric form of the blocked periodic state]
    ...
    extension to non-normal tensors'', before normalization.
\end{theorem}
\begin{proof}
    \leanok
