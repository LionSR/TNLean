...
\begin{theorem}[Coefficient comparison for transported vertical sectors]
    ...
    \cite[Appendix~C.4]{Cirac2016MPDO_arXiv}, comparing the traces of the
...
\begin{theorem}[Simultaneous representations of the blocked vertical operator]
    ...
    \textbf{Local fix (blocked trace ratio exponent):} The exponent
    $(m_\gamma/n_\gamma)^L$ deliberately corrects the source's printed
    $m_\gamma^L/n_\gamma$ in \cite[line~2013]{Cirac2016MPDO_arXiv}; documented in
    \cite{gap:cpsv16_blocked_operator_trace_ratio_exponent}
    (\texttt{docs/paper-gaps/cpsv16_blocked_operator_trace_ratio_exponent.tex}).
\end{theorem}
...
\begin{theorem}[Coverage of active product corners by the blocked BNT]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The choices may be made simultaneously.

    \textbf{Scope restriction (active product BNT):} The conclusion gives
    only one-way coverage of active product corners and does not assert that
    every blocked BNT label occurs; documented in
    \cite{gap:cpsv16_bnt_uniqueness_zero_coefficient}.

    \textbf{Local fix (Figure-11 fixed-pair support and fusion coisometry):}
    Fixed-pair support follows from exact squared reconstruction and the canonical
    direct-sum corner; a zero pair contributes an empty active family. The
    retained-row map is a coisometry onto the active direct sum. Documented in
...
\begin{definition}[Original-label corner family]
    ...
    A copy pair with no active corner is represented by an empty family; see
...
\begin{theorem}[Positive original-label corner decomposition]
    ...
    Empty active families remain empty, and the corresponding raw product
    vanishes; see \cite{gap:cpsv16_figure11_per_pair_support}.
\end{theorem}
...
\begin{theorem}[Fusion coisometries from original-label corners]
    ...
    product corners and coisometries following
    \cite{gap:cpsv16_bnt_uniqueness_zero_coefficient, gap:cpsv16_figure11_per_pair_support, gap:cpsv16_figure11_fusion_coisometry}.
\end{theorem}
...
\begin{theorem}[Fusion coisometries from normalized product corners]
    ...
    reconstruction for every product tensor $M_\alpha M_\beta$
    \cite[Appendix~C.4, lines~2020--2029]{Cirac2016MPDO_arXiv}.
\end{theorem}
...
\begin{theorem}[Positive fusion from transported vertical sectors]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This is the positive fusion decomposition of
    \cite[Appendix~C.4, lines~2020--2029]{Cirac2016MPDO_arXiv}.
\end{theorem}
