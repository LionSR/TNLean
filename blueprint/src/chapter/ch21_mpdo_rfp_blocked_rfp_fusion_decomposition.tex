...
\begin{proof}\leanok
    \uses{lem:cpsv_bnt_block_dimensions_positive, thm:mpdo_transported_vertical_sector_composites_trace_preserving, thm:mpdo_transported_vertical_sector_composites_schwarz, thm:mpdo_transported_vertical_sector_composites_identity}
    ...
    the total block trace, and
    Theorem~\ref{thm:mpdo_transported_vertical_sector_composites_identity}
    gives both inverse laws.
    Apply Theorem~\cite[``Unitary action of an invertible CPTP map'']{QICLean2026Blueprint} to this
...
\begin{proof}\leanok
    \uses{lem:mpdo_vertical_bond_contraction_encoded_matrix_unit, lem:mpdo_vertical_sector_retraction, thm:mpdo_transported_vertical_sector_unitary_relabeling, thm:mpdo_transported_vertical_sector_maps}
...
\begin{theorem}[Simultaneous representations of the blocked vertical operator]
    ...
    \leanok
    \uses{thm:mpdo_transported_vertical_sector_coefficient_comparison, def:vertical_cf_mpdo, def:mpdo_two_site_blocking}
    ...
    In \cite[Appendix~C.4, line~2013]{Cirac2016MPDO_arXiv}, the exponent is
    printed as $m_\gamma^L/n_\gamma$; the tensor scaling requires
    $(m_\gamma/n_\gamma)^L$ (see
    \cite{gap:cpsv16_blocked_operator_trace_ratio_exponent}).
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpdo_operator_product_mpo, lem:mpv_vertical_assembled, thm:mpdo_transported_vertical_sector_coefficient_comparison, thm:coisometry_reconstruction_same_mpv_pos}
    The two-site vertical canonical form first gives
    $O_L(B)=\sum_\delta\tr(\nu_\delta^L)
        O_L(A^{(2)}_\delta)$.
    Reindex this sum by $\delta=\sigma(\alpha)$. The tensor-letter identity
    from Theorem~\ref{thm:mpdo_transported_vertical_sector_coefficient_comparison}
    multiplies every letter of $A^{(1)}_\alpha$ by
...
\begin{theorem}[Canonical hypotheses for the retained vertical product]
    ...
    \leanok
    \uses{thm:mpdo_blocked_vertical_operator_representations, def:mpdo_two_site_blocking}
...
\begin{theorem}[Canonical hypotheses for every retained copy pair]
    ...
    \leanok
    \uses{thm:mpdo_retained_vertical_product_canonical_hypotheses, lem:canonical_hypotheses_dependent_direct_summand}
...
\begin{theorem}[Normal-corner decomposition of a retained copy pair]
    ...
    \leanok
    \uses{thm:mpdo_weighted_vertical_product_block_canonical_hypotheses, thm:projector_closure_canonical_form}
...
\begin{proof}\leanok
    ...
    closure and periodic-exclusion properties of
    Theorem~\ref{thm:mpdo_weighted_vertical_product_block_canonical_hypotheses}. Its
...
\begin{theorem}[Existence and inclusions of retained-product spectral data]
    ...
    \leanok
    \uses{def:mpdo_retained_product_spectral_family, thm:mpdo_weighted_vertical_product_block_normal_decomposition, thm:mpdo_retained_vertical_product_canonical_hypotheses, lem:matrix_sigma_block_inclusion}
...
\begin{theorem}[Retained-product spectra from literal CPSV canonical form]
    ...
    \leanok
    \uses{def:mpdo, def:cpsv_canonical_form, def:mpdo_retained_product_spectral_family, thm:mpdo_literal_cpsv_positive_blocking, thm:mpdo_vertical_projector_closure_literal_cpsv, thm:mpdo_vertical_no_periodic_vectors_literal_cpsv}
...
\begin{theorem}[Flattened reconstruction of the retained product]
    ...
    \leanok
    \uses{def:mpdo_retained_product_spectral_family, lem:matrix_sigma_block_inclusion, lem:same_mpv_exact_isometric_decomposition}
...
\begin{theorem}[Coverage of active product corners by the blocked BNT]
    ...
    \leanok
    \uses{thm:mpdo_retained_product_flat_reconstruction, thm:mpdo_retained_vertical_product_canonical_hypotheses, thm:cpsv_bnt_characterization}
    Suppose that the one-site vertical tensor has the stated exact
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The choices may be made simultaneously, giving one-way coverage of the
    active product corners \cite{gap:cpsv16_bnt_uniqueness_zero_coefficient}.
    Here fixed-pair support follows from exact squared reconstruction and the
    canonical direct-sum corner, with a zero pair contributing an empty active
    family \cite{gap:cpsv16_figure11_per_pair_support}.
\end{theorem}
...
\begin{theorem}[Distinguished blocked reference corners]
    ...
    \leanok
    \uses{thm:mpdo_active_product_corner_blocked_bnt_coverage, lem:matrix_sigma_block_inclusion}
...
\begin{theorem}[Active product positivity from sector separation]
    ...
    \leanok
    \uses{def:mpdo, thm:mpdo_active_product_corner_blocked_bnt_coverage, thm:mpdo_blocked_reference_corners, thm:mpdo_retained_product_spectral_family_exists, thm:mpdo_sector_weight_pos}
...
\begin{theorem}[Positivity of active product coefficients]
    ...
    \leanok
    \uses{thm:mpdo_active_product_corner_blocked_bnt_coverage, thm:mpdo_blocked_reference_corners, thm:mpdo_retained_product_spectral_family_exists, thm:mpdo_grouped_sector_compression_nonzero, thm:mpdo_sector_weight_pos, thm:mpdo_active_product_coefficient_phase_pos_of_sector_separation}
...
\begin{proof}\leanok
    ...
    range-projection corner at some finite chain length. Apply
    Theorem~\ref{thm:mpdo_active_product_coefficient_phase_pos_of_sector_separation}
    with this separation property.
\end{proof}
...
\begin{theorem}[Literal positivity of active product coefficients]
    ...
    \leanok
    \uses{def:mpdo, def:cpsv_canonical_form, thm:mpdo_active_product_corner_blocked_bnt_coverage, thm:mpdo_blocked_reference_corners, thm:cpsv_literal_sector_compression_nonzero, thm:mpdo_sector_weight_pos, thm:mpdo_active_product_coefficient_phase_pos_of_sector_separation}
...
\begin{theorem}[Unitary normalization of active product gauges]
    ...
    \leanok
    \uses{thm:mpdo_active_product_corner_blocked_bnt_coverage, thm:mpdo_retained_product_spectral_family_exists, lem:mpdo_horizontal_cf_block_two, thm:mpdo_block_two_as_positive_blocking, thm:mpdo_active_product_coefficient_phase_pos, thm:mpdo_blocked_reference_corners, thm:mpdo_pairwise_vertical_gram_conjugation, thm:normal_tensor_gram_rigidity}
    Under the hypotheses of
    Theorem~\ref{thm:mpdo_literal_cpsv_active_product_coefficient_phase_pos},
    for every active product corner $j$ there is a real number $\omega_j>0$ such that
    \begin{align}
        X_j^\dagger X_j
         & =\omega_j\Id,
        \notag                 \\
...
\begin{theorem}[Literal unitary normalization of active product gauges]
    ...
    \leanok
    \uses{def:mpdo, def:cpsv_canonical_form, thm:mpdo_active_product_corner_blocked_bnt_coverage, thm:mpdo_blocked_reference_corners, thm:mpdo_literal_cpsv_active_product_coefficient_phase_pos, thm:cpsv_literal_positive_relative_gram_scalar}
    Under the hypotheses of
    Theorem~\ref{thm:mpdo_literal_cpsv_active_product_coefficient_phase_pos},
    for every active product corner $j$ there is a real number $\omega_j>0$ such
    that
    \begin{align}
        X_j^\dagger X_j
         & =\omega_j\Id,
...
\begin{proof}\leanok
    ...
    distinguished blocked reference corner. Their coefficients are positive by
    Theorem~\ref{thm:mpdo_literal_cpsv_active_product_coefficient_phase_pos}
    and by positivity of the blocked multiplicity
...
\begin{definition}[Original-label corner family]
    ...
    A copy pair with no active corner is represented by an empty family
...
\begin{theorem}[Positive original-label corner decomposition]
    ...
    \leanok
    \uses{def:mpdo_original_corner_family, thm:mpdo_active_product_corner_blocked_bnt_coverage, thm:mpdo_transported_vertical_sector_coefficient_comparison, lem:mpdo_normalized_vertical_sector_positive_trace, thm:mpdo_active_product_coefficient_phase_pos, thm:mpdo_active_product_gauge_unitary_normalization, thm:normalized_grouped_sector_maps_of_dressing}
    ...
    For copy pairs with empty active support, exact weighted reconstruction and
    positivity of the outer copy weight imply that the corresponding raw
    product vanishes \cite{gap:cpsv16_figure11_per_pair_support}.
\end{theorem}
...
\begin{theorem}[Fusion coisometries from original-label corners]
    ...
    \leanok
    \uses{def:mpdo_original_corner_family, def:mpdo_bnt_fusion_coisometry_family, thm:vertical_sector_coisometry}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Empty label fibers contribute zero-dimensional diagonal blocks, yielding
    the decomposition of \cite[Appendix~C.4, lines~2020--2029]{Cirac2016MPDO_arXiv}.
    Here only active product corners are retained
    \cite{gap:cpsv16_bnt_uniqueness_zero_coefficient}, with fixed-pair support
    handled by empty families \cite{gap:cpsv16_figure11_per_pair_support}
    and the retained-row map acting as a coisometry onto the active direct sum
...
\begin{theorem}[Fusion coisometries from normalized product corners]
    ...
    \leanok
    \uses{thm:mpdo_original_corner_family_exists, thm:mpdo_original_corner_family_to_fusion_coisometry}
    ...
    reconstruction for every product tensor $M_\alpha M_\beta$. This yields the
    active-corner construction of
    \cite[Appendix~C.4, lines~2020--2029]{Cirac2016MPDO_arXiv}, retaining
    active product corners \cite{gap:cpsv16_bnt_uniqueness_zero_coefficient},
    handling fixed-pair support \cite{gap:cpsv16_figure11_per_pair_support},
    and using coisometric fusion onto the active subspace
    \cite{gap:cpsv16_figure11_fusion_coisometry}.
\end{theorem}

\begin{proof}\leanok
    The positive coefficients and normalized gauges give an original-label
    corner family. Apply the block-row construction of
    Theorem~\ref{thm:mpdo_original_corner_family_to_fusion_coisometry} to its
...
\begin{theorem}[Positive fusion from transported vertical sectors]
    ...
    \leanok
    \uses{lem:cpsv_bnt_block_dimensions_positive, thm:mpdo_transported_vertical_sector_coefficient_comparison, thm:mpdo_retained_product_spectral_family_exists, thm:mpdo_active_product_corner_blocked_bnt_coverage, thm:mpdo_active_product_coefficient_phase_pos, thm:mpdo_active_product_gauge_unitary_normalization, thm:mpdo_original_corner_family_exists, thm:mpdo_original_corner_family_to_fusion_coisometry, thm:mpdo_normalized_product_corners_to_fusion_coisometry}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This establishes the positive fusion decomposition of
    \cite[Appendix~C.4, lines~2020--2029]{Cirac2016MPDO_arXiv}, accounting for
    active product corners \cite{gap:cpsv16_bnt_uniqueness_zero_coefficient},
    fixed-pair support \cite{gap:cpsv16_figure11_per_pair_support}, and
    coisometric fusion \cite{gap:cpsv16_figure11_fusion_coisometry}.
\end{theorem}
...
\begin{theorem}[Positive fusion from literal CPSV canonical form]
    ...
    \leanok
    \uses{def:mpdo, def:cpsv_canonical_form, thm:mpdo_transported_vertical_sector_coefficient_comparison, thm:mpdo_literal_cpsv_retained_product_spectral_family, thm:mpdo_active_product_corner_blocked_bnt_coverage, thm:mpdo_literal_cpsv_active_product_coefficient_phase_pos, thm:mpdo_literal_cpsv_active_product_gauge_unitary_normalization, thm:mpdo_original_corner_family_exists, thm:mpdo_original_corner_family_to_fusion_coisometry, thm:mpdo_normalized_product_corners_to_fusion_coisometry}
    Let $M$ generate matrix product density operators, and suppose that its
    ...
    This establishes the positive fusion decomposition of
    \cite[Appendix~C.4, lines~2020--2029]{Cirac2016MPDO_arXiv}, with active
    corners and coisometric fusion following
    \cite{gap:cpsv16_bnt_uniqueness_zero_coefficient,
        gap:cpsv16_figure11_per_pair_support, gap:cpsv16_figure11_fusion_coisometry}.
\end{theorem}
...
\begin{theorem}[Idempotent trace scalars from blocked representations]
    ...
    \leanok
    \uses{def:mpdo_bnt_label_idempotent_coefficient_form, def:mpdo_bnt_fusion_coisometry_family, def:mpdo_bnt_eventual_power_sums_multiplicity_comparison, thm:mpdo_blocked_vertical_operator_representations}
...
\begin{theorem}[Positive fusion decomposition of vertical products]
    ...
    \leanok
    \uses{def:is_rfp_via_ts, thm:vertical_cf_of_horizontal_cf, thm:mpdo_transported_vertical_product_positive_fusion_decomposition}
    ...
    the positive fusion decomposition of
    \cite[Appendix~C.4, lines~2020--2029]{Cirac2016MPDO_arXiv} on the active
    corners \cite{gap:cpsv16_bnt_uniqueness_zero_coefficient,
        gap:cpsv16_figure11_per_pair_support, gap:cpsv16_figure11_fusion_coisometry}.
\end{theorem}
...
\begin{proof}\leanok
    ...
    Theorem~\ref{thm:mpdo_transported_vertical_product_positive_fusion_decomposition}
    then supplies the positive diagonal family, the row
...
\begin{theorem}[Two-site closure of the two-site blocking]
    ...
    \leanok
    \uses{def:phys_close, def:mpdo_two_site_blocking, def:mpdo_four_site_physical_closure}
