...
\begin{theorem}[Transfer map preserved by physical rotation]%
    ...
    $\mathcal{E}_{\operatorname{rotatePhysical}\, u\, A}
     = \mathcal{E}_A$.
\end{theorem}
...
\begin{theorem}[Left-canonical preserved by physical rotation]%
    ...
    $\operatorname{rotatePhysical}\, u\, A$ is left-canonical.
\end{theorem}
...
\begin{theorem}[Irreducibility preserved by physical rotation]%
    ...
    $\operatorname{rotatePhysical}\, u\, A$ is irreducible.
\end{theorem}
...
\begin{theorem}[Periodicity preserved by physical rotation]%
    ...
    $\operatorname{rotatePhysical}\, u\, A$ is periodic with the same
    period.
\end{theorem}
...
\begin{theorem}[SameMPV preserved by physical rotation]%
    ...
    $\mathcal{V}(\operatorname{rotatePhysical}\, u\, A)
     = \mathcal{V}(\operatorname{rotatePhysical}\, u\, B)$.
\end{theorem}
...
\begin{theorem}[Physical rotation distributes over block assembly]%
    ...
    \leanok
    \uses{def:rotate_physical}
    $\operatorname{rotatePhysical}\, u\,
    (\operatorname{toTensorFromBlocks}\, \mu\, A)
    = \operatorname{toTensorFromBlocks}\, \mu\,
    (k \mapsto \operatorname{rotatePhysical}\, u\, (A_k))$.
\end{theorem}

\begin{proof}\leanok
    \uses{def:rotate_physical}
    Pointwise matrix-entry computation using linearity of
    \texttt{blockDiagonal'} and \texttt{reindex}.
\end{proof}
...
\begin{theorem}[Irreducible form preserved by physical rotation]%
    ...
    $\operatorname{rotatePhysical}\, u\, A$ is in irreducible form~II
    with the same block structure and rotated blocks.
\end{theorem}
...
\begin{theorem}[String order holds for injective symmetric MPS]%
    ...
    \uses{thm:virtual_rep_injective,
          thm:modulus_one_gauge}
    Let $A$ be an injective tensor, symmetric under a unitary
    representation $U$, with canonical normalisations $\E_A(\Id)=\Id$,
    $\E_A^\dagger(\Lambda)=\Lambda$, and $\Lambda$ positive definite
    with unit trace.
    Then string order exists for every group element $g$: the
    twisted transfer map $\E_{U(g)}$ has the virtual gauge matrix as
    a fixed point with eigenvalue~$1$.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:virtual_rep_injective,
          thm:modulus_one_gauge,
          thm:string_order_iff_local_symmetry}
    ...
    Since $|1|=1$, the modulus-one rigidity theorem yields
    gauge-phase equivalence, from which one extracts a virtual unitary
    $W$ and a unit-modulus phase $\mu$ satisfying the local symmetry
    conditions.  The boundary-state invariance
    $W^\dagger\Lambda W=\Lambda$ then gives local symmetry, which
    implies string order.
\end{proof}
...
\begin{theorem}[String order is an SPT invariant]%
    ...
    If $A$ and $B$ are injective, both on-site symmetric under $U$,
    and both satisfy the canonical normalisation hypotheses, then for
    every $g\in G$, string order exists for $A$ with twist $U(g)$ if
    and only if it exists for~$B$.  In particular, since
