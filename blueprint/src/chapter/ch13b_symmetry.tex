...
\begin{corollary}[Physical symmetry implies virtual gauge]%
    ...
    \notready
    \uses{def:same_state_reduction_hyp, def:injective}
    ...
    Fix $n \ge 3$ and suppose the $n$-site translation-invariant state
    satisfies
    \[
...
\begin{proof}
    \uses{thm:ti_reduction_of_sameState}
    Define $B^i := \sum_j U_{ij} A^j$.
    Since $U$ is invertible, $\{B^i\}$ spans $\MN{D}$, so $B$ is
    injective.
    The symmetry condition means the two constant chains $(A, \ldots, A)$ and
    $(B, \ldots, B)$ of length $n$ generate the same chain state.
    Theorem~\ref{thm:ti_reduction_of_sameState} then gives the required
    virtual gauge relation.
\end{proof}
