...
\begin{lemma}[Spectral radius $<1$ forces boundary-string decay]%
    ...
    If $\rho(\E_u) < 1$, then for every boundary state $\Lambda$ and all
    boundary matrices $X,Y \in \MN{D}$ one has
    \[
...
\begin{theorem}[String order implies gauge-phase equivalence with the companion tensor]%
    ...
    If string order exists for $u$, then $A$ is gauge-phase equivalent to the
    companion tensor $B_u$.
\end{theorem}
...
\begin{theorem}[Gauge-phase equivalence with the companion tensor yields a virtual unitary]%
    ...
    exist a unitary $V$ and a phase $\mu$ with $|\mu|=1$ such that
    \[
        \sum_j u_{ij} A^j = \mu\, V A^i V^\dagger
        \qquad\text{for all } i.
    \]
...
\begin{theorem}[Boundary-state invariance of the virtual unitary]%
    ...
    If $V$ is a unitary and $\mu$ a phase satisfying
    \[
        \sum_j u_{ij} A^j = \mu\, V A^i V^\dagger
        \qquad\text{for all } i,
    \]
...
\begin{theorem}[Local symmetry iff a unitary peripheral eigenmatrix exists]%
    ...
    Assume $u u^\dagger = \Id$, $\E_A(\Id)=\Id$, $\Lambda$ is positive
...
\begin{theorem}[Virtual unitary from string order]%
    ...
    For an injective MPS, assume $u u^\dagger = \Id$ and $\E_A(\Id)=\Id$.
    If string order exists for $u$, then there exists a virtual unitary $V$
    and a unit-modulus scalar $\mu$ such that
