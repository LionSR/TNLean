...
\begin{proof}
    ...
    injective.
    The symmetry condition means the TI chains $(A, \ldots, A)$ and
    $(B, \ldots, B)$ of length $n$ generate the same state.
...
\begin{definition}[Local symmetry]\label{def:local_symmetry}
    ...
    \emph{Remark.} In the Lean formalization, local symmetry is defined
    via the existence of a virtual unitary witness $V$ and phase $\mu$
    with $\|\mu\| = 1$ satisfying $V^\dagger \Lambda V = \Lambda$ and a
    twisted Kraus-mixing equation, rather than the scalar string-order
...
\begin{definition}[String order]\label{def:has_string_order}
    ...
    \emph{Remark.} In the Lean formalization, string order is defined
    with boundary witness matrices $X$ and $Y$ rather than the scalar
    parameter $R_L(u)$.
\end{definition}
...
\begin{proof}
    ...
    unitary change of Kraus operators; see Section~\ref{sec:kraus_representation}.
\end{proof}
...
\begin{theorem}[Spectral radius bound]\label{thm:twisted_transfer_spectral_radius_le_one}
    ...
    $\tr(\Lambda)=1$, and $\E_A^\dagger(\Lambda)=\Lambda$. Then every
    eigenvalue $\lambda$ of $\E_u$ satisfies $|\lambda| \le 1$.
\end{theorem}
...
\begin{theorem}[Local symmetry $\Leftrightarrow$ spectral radius one]%
    ...
    For a pure FCS, assume $u u^\dagger = \Id$,
    $\E_A(\Id)=\Id$, $\Lambda$ is positive definite,
    $\tr(\Lambda)=1$, and $\E_A^\dagger(\Lambda)=\Lambda$. Then $u$ is
    a local symmetry iff $\rho(\E_u) = 1$.
\end{theorem}
...
\begin{theorem}[Virtual unitary from string order]%
    ...
    For an injective MPS, assume $u u^\dagger = \Id$,
    $\E_A(\Id)=\Id$, $\Lambda$ is positive definite,
    $\tr(\Lambda)=1$, and $\E_A^\dagger(\Lambda)=\Lambda$. If string
    order exists for $u$, then there exists a virtual unitary $V$ and a
    unit-modulus scalar $\mu$ such that
...
\begin{theorem}[String order holds for injective symmetric MPS]%
    ...
    Then string order exists for every group element $g$: the twisted
    transfer map $\E_{U(g)}$ has the virtual gauge matrix as a fixed
    point with eigenvalue~$1$.
\end{theorem}
