...
\begin{theorem}[String order is an SPT invariant]%
    ...
    \uses{def:is_same_spt_phase,
          thm:has_string_order_of_symmetric_injective}
    If $A$ and $B$ are injective and in the same SPT phase
    (Definition~\ref{def:is_same_spt_phase}), and both satisfy the
    canonical normalisation hypotheses, then for every $g\in G$,
    string order exists for $A$ with twist $U(g)$ if and only if it
    exists for $B$.  On-site symmetry of both tensors is derived from
    the same-phase witness via gauge equivalence.  The cohomology
    condition is not needed for string order \emph{existence} (which
    holds universally for injective symmetric tensors) but becomes
    essential for finer SPT invariants such as string-order parameter
    values.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:has_string_order_of_symmetric_injective}
    The same-phase hypothesis provides virtual representation
    intertwining for both tensors; via gauge equivalence and
    \texttt{GaugeEquiv.sameMPV} this yields on-site symmetry.
    Both directions then follow from
