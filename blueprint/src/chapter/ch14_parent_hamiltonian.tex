...
\begin{definition}[Ground space for the block-sum tensor]
    ...
    \]
    The periodic parent-Hamiltonian ground space considered in this section is
    \[
        \ker H_{N,L}(B)
        =
        \mathcal G_{N,L}(B).
...
\begin{lemma}[Each normal-tensor vector lies in the Hamiltonian ground space]
    ...
    $\ker H_{N,L}(\bigoplus_k \mu_k A_k)$.
\end{lemma}
...
\begin{proof}
    ...
    gives its membership in $\ker H_{N,L}(\bigoplus_k\mu_k A_k)$.
\end{proof}
...
\begin{lemma}[Forward inclusion for the direct-sum tensor]
    ...
    blockwise periodic-chain ground spaces is contained in the
    parent-Hamiltonian ground space of the block-sum tensor:
    \[
        \sum_{j} \mathcal G_{N,L}(A_{j})
        \subseteq
        \ker H_{N,L}\!\left(\bigoplus_k \mu_k A_k\right).
    \]
\end{lemma}
...
\begin{proof}
    ...
    every $j$, and the parent-Hamiltonian ground space is
...
\begin{theorem}[Containment in the space generated by a basis of normal tensors]
    ...
    If $A^i=\bigoplus_j\mu_j A_j^i$ is in block-injective canonical form, with
    $A_1,\ldots,A_g$ a basis of normal tensors, and if $1 < L$ and
    $N \ge L + 1$, then
    \[
        \ker H_{N,L}\!\left(\bigoplus_j \mu_j A_j\right)
        \subseteq
        \spn\{V^{(N)}(A_j): j=1,\ldots,g\}.
    \]
\end{theorem}
...
\begin{proof}
    ...
    Lemma~\ref{lem:bnt_mpv_mem_ground_space} shows that each vector
    $V^{(N)}(A_j)$ lies in
    $\ker H_{N,L}(\bigoplus_j \mu_j A_j)$. Since the parent-Hamiltonian ground
    space is a linear subspace, it therefore contains every linear combination
