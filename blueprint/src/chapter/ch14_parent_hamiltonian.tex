...
\begin{theorem}[MPV nonvanishing for block-injective tensors]%
  ...
  If $A$ is $L_0$-block-injective with $L_0 > 0$ and $N \ge L_0 + 1$,
  then $\mathrm{mpv}(A) \ne 0$ in the $N$-site space.  The proof uses a
  descent argument: assuming all traces of length-$N$ products vanish,
  trace nondegeneracy forces all products of length $N - L_0$ to vanish,
  and induction reaches a contradiction.
\end{theorem}
...
\begin{theorem}[Chain ground space for block-injective tensors]%
  ...
  If $A$ is $L_0$-block-injective and the window size satisfies
  $L \ge 2L_0$, then on the periodic chain with $N \ge 2$ sites,
  the chain ground space equals the MPV submodule:
  $\mathcal{G}_L(A) = \operatorname{span}\{\mathrm{mpv}(A)\}$.
\end{theorem}
...
\begin{theorem}[Chain ground space for normal tensors]%
  ...
  \uses{def:chain_ground_space, def:mpv_submodule, def:l_blk_injective,
        def:normal}
  If $A$ is normal and $L_0$-block-injective, the window requirement
  is reduced from $2L_0$ to $L_0 < L$.  The normality hypothesis enables
  the range reduction via the structure theory of normal MPS (peripheral
  spectrum, canonical form).  See~\cite[\S IV.C]{Cirac2021Matrix}.
\end{theorem}
