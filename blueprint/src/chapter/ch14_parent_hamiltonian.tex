...
\begin{theorem}[Boundary equality implies containment in the MPS line]
    ...
    $\eta$ used outside the complementary sites. Suppose that, for all letters
    $a,b$ and all complementary words $\mu$,
    \[
        A^\mu A^bX=Y^+_{\tau^+_\eta(\mu)}A^b,
        \qquad
        XA^aA^\mu=A^aY^-_{\tau^-_\eta(\mu)},
    \]
    and that the two boundary matrices satisfy
    \[
        Y^+_{\tau^+_\eta(\mu)} = Y^-_{\tau^-_\eta(\mu)}
    \]
...
\begin{proof}
    ...
    Put
    \[
        Y_\mu = Y^+_{\tau^+_\eta(\mu)}
              = Y^-_{\tau^-_\eta(\mu)}.
    \]
    The two one-sided equations become
    \[
        A^\mu A^bX=Y_\mu A^b,
        \qquad
        XA^aA^\mu=A^aY_\mu.
