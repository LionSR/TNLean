...
\begin{theorem}[RFP implies NNCPH]\label{thm:rfp_implies_nncph}
    ...
    \cite[Theorem~3.10]{Cirac2021Matrix}, plus the bridge from that
    product-of-pairs structure to commutation of the formal local parent
    projectors.
\end{theorem}
...
\begin{theorem}[NNCPH implies RFP]\label{thm:nncph_implies_rfp}
    ...
    \cite[Theorem~3.10]{Cirac2021Matrix}.  In the current formal
    normalization, the statement must also include a normalization hypothesis:
    the commuting-parent-Hamiltonian condition is invariant under nonzero
