...
\cite[lines~2113--2117]{Cirac2021Matrix}, this means that for every
$X=(X_j)_j\in\bigoplus_{j=1}^g \MN{D_j}$ there are coefficients
$c_\omega(X)$ such that
\[
    X_j=\sum_{\omega} c_\omega(X) A_j^\omega.
\]
The corresponding parent-Hamiltonian ground space is the vector space
generated by the matrix product vectors of a basis of normal tensors
\cite[lines~2121--2128]{Cirac2021Matrix}.

\begin{definition}[Parent ground space for block-injective canonical form]
    \label{def:parent_hamiltonian_ground_space_bnt}
    \notready
    \uses{def:chain_ground_space}
    For a basis of normal tensors $A=(A_j)_{j=1}^g$ and nonzero weights $\mu_j$,
    write
    \[
        \operatorname{ParentGround}_{N,L}(A)
        :=
        \operatorname{ChainGround}_{N,L}\!\left(\bigoplus_{j=1}^g \mu_j A_j\right).
    \]
\end{definition}

\begin{lemma}[Definition as a chain ground space]
    \label{lem:parent_hamiltonian_ground_space_eq}
    \notready
    \uses{def:parent_hamiltonian_ground_space_bnt}
    By definition, this subspace is the chain ground space of
    $\bigoplus_{j=1}^g \mu_j A_j$.
\end{lemma}

\begin{definition}[Vector space generated by a basis of normal tensors]
    \label{def:bnt_span}
    \notready
    For a basis of normal tensors $A_1,\ldots,A_g$, set
    \[
        \operatorname{BNTSpan}_{N}(A)
        :=
...
\begin{definition}[Finite blockwise physical--virtual correspondence]
    \label{def:route_b_product_word_span}
    \lean{MPSTensor.WordTupleSpanTop}
    \leanok
    \uses{rem:word_tuple_span_top}
    For a fixed length $m$, the finite-length form of block injectivity is
    \[
        \spn\{(A_1^\omega,\ldots,A_g^\omega): |\omega|=m\}
        =
        \prod_{j=1}^g \MN{D_j}.
    \]
    Equivalently, for every block-diagonal boundary condition
    $X=(X_j)_j\in\bigoplus_{j=1}^g \MN{D_j}$ there are coefficients
    $c_\omega(X)$ satisfying
    \[
        X_j=\sum_{|\omega|=m} c_\omega(X) A_j^\omega.
    \]
    The remaining mathematical step is to derive such a positive length from
    the canonical-form hypotheses and the basis of normal tensors.
\end{definition}

\begin{lemma}[Each basis tensor vector lies in the parent ground space]
    ...
    the vector $V^{(N)}(A_j)$ lies in $\operatorname{ParentGround}_{N,L}(A)$.
\end{lemma}
...
\begin{proof}
    ...
    gives its membership in the parent ground space of the direct sum.
\end{proof}
...
\begin{lemma}[Periodic block lift]
    ...
    \[
        ...
        \subseteq
        \operatorname{ChainGround}_{N,L}\!\left(\bigoplus_k \mu_k A_k\right).
    \]
\end{lemma}
...
\begin{lemma}[Periodic block decomposition: forward inclusion]
    ...
    periodic-chain ground spaces is contained in the direct-sum tensor's periodic-chain
    ground space:
    \[
...
\begin{lemma}[Forward inclusion for the direct-sum tensor]
    ...
    ground spaces is contained in the parent ground space:
    \[
        \bigsqcup_{j} \operatorname{ChainGround}_{N,L}(A_{j})
...
\begin{proof}
    ...
    parent ground space is the chain ground space of $\bigoplus_j \mu_j A_j$ by
    definition.
\end{proof}
...
\begin{lemma}[Block-diagonal commutant reduction from span extension]
    ...
    family commutes with each $P_j$, hence is block diagonal. Applied after pulling a
    direct-sum tensor back along the direct-sum reindexing, length-$m$ word commutation
...
\begin{lemma}[Dimension step for two-block direct-sum image spaces]
    ...
    blockwise word span required by the block-separation argument. Since
    ...
    to the normal-CF-BNT hypotheses once one-site injectivity is supplied
    explicitly, since the normal-CF-BNT predicate carries the irreducibility and
...
\begin{lemma}[Blockwise word span from block-separating equations]
    ...
    coefficient-equation conclusion does not require the full canonical-form/BNT
    hypotheses.
...
\begin{lemma}[Conditional reduction to block-diagonal boundary conditions]
    ...
    $\bigsqcup_j \mathcal G_{N,L}(A_j)$. Consequently, if every vector in
    ...
    normal tensors.  This is the block-diagonal boundary-condition form of
    inverting and re-growing tensors in \cite[lines~2126--2128]{Cirac2021Matrix}.
\end{lemma}
...
\begin{lemma}[Conditional inverting and re-growing step from blockwise word span]
    ...
    and boundary representation remain hypotheses here; they are not derived by
    this lemma.
\end{lemma}
...
\begin{proof}
    ...
    The blockwise word-span witness gives the sector-projection span by
    ...
    conditional reduction to block-diagonal boundary conditions from
    Lemma~\ref{lem:conditional_boundary_matrix_block_split_projection_span} then
    gives the reverse inclusion. The equality and span forms are the
    corresponding projection-span results after this word-span to projection-span
    reduction, followed by the blockwise uniqueness argument already built into the
    projection-span theorem.
\end{proof}

\begin{lemma}[Conditional inverting and re-growing step from basis tensor separation]
    ...
    hypotheses imply containment in $\spn\{V^{(N)}(A_j): j=1,\ldots,g\}$. The
    separation hypothesis and the boundary representation remain explicit: this
    lemma does not derive the coefficients $c_\omega^{(k)}$ from the basis of
    normal tensors, nor the wrapping/open-chain boundary representation. This is
    the block-injective ground-space argument of
...
\begin{proof}
    ...
    canonical-form/BNT hypotheses plus the displayed separation equations into
    the blockwise word-span condition at the positive length $1 + S$.
    Lemma~\ref{lem:route_b_block_split_from_product_word_span} then applies
    the conditional inverting and re-growing step with the boundary matrix
    commuting with the same length-$(1 + S)$ direct-sum words. The equality and
    span containment are exactly the corresponding word-span consequences after
    this replacement.
\end{proof}

\begin{theorem}[Containment in the space generated by basis tensors]
    ...
    If $A$ is in canonical-form/BNT and $1 < L$ and $N \ge L + 1$, then
    \[
        \operatorname{ParentGround}_{N,L}(A)
        \subseteq
        \spn\{V^{(N)}(A_j): j=1,\ldots,g\}.
...
\begin{proof}
    ...
    chain ground space of $\bigoplus_j \mu_j A_j$. The source argument uses the
    same inverting and re-growing procedure as in the injective case, with the
    inverse projecting to block-diagonal boundary conditions. In formulas, this
    is the passage from a commuting boundary matrix to
    \[
        X=\bigoplus_j X_j,
        \qquad
    ...
    and Lemma~\ref{lem:route_b_block_split_from_bnt_selectors} gives the
...
\begin{theorem}[Ground space generated by a basis of normal tensors]
    \label{thm:gs_eq_bnt_span}
    \notready
    \uses{thm:parent_ground_space_le_bnt_span, lem:bnt_mpv_mem_ground_space,
        def:parent_hamiltonian_ground_space_bnt, def:bnt_span}
    If $1 < L$ and $N \ge L + 1$, then for a canonical-form/BNT family
    \[
        \operatorname{ParentGround}_{N,L}(A)
        =
        \spn\{V^{(N)}(A_j): j=1,\ldots,g\}.
