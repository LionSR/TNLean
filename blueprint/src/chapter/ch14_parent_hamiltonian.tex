...
\begin{definition}[Canonical parent interaction]\label{def:parent_interaction}
    ...
    \leanok
    \uses{def:ground_space_parent}
    The parent interaction at range $L$ is the canonical positive projector
    \[
...
\begin{lemma}[Frustration-freeness of the MPV]\label{lem:parent_hamiltonian_ff}
    ...
    The MPS vector is a frustration-free ground state:
    \[
        h_i V^{(N)}(A)=0
        \quad\text{for all }i.
    \]
...
\begin{definition}[MPV submodule]\label{def:mpv_submodule}
    ...
    \leanok
    \uses{def:mpv}
    The one-dimensional space spanned by the MPS vector
...
\begin{theorem}[Boundary contraction in the MPS span from boundary comparison]
...
\begin{theorem}[Conditional range reduction from boundary comparison]
    ...
    \[
        ...
        Y^-_{\tau^-_\eta(\mu)}
    \]
    for every physical letter $j$ and every complementary word $\mu$. Then
    \[
...
\begin{theorem}[Normal-range reduction: containment in the MPS span]
