...
\begin{definition}[Periodic chain ground space]\label{def:chain_ground_space}
  ...
  \[
    \mathcal{G}_{N,L}(A) := \bigl\{\psi \in (\{0,\dots,d{-}1\}^N \to \C)
    : \text{for every cyclic window } [i,i{+}L{-}1],\;
    \psi|_{[i,i+L-1]} \in G_L(A)\bigr\},
  \]
  where $\psi|_{[i,i+L-1]}$ denotes the restriction obtained by fixing
  all physical indices outside the window to arbitrary values and summing
  over the outside configurations.
...
\begin{proof}
  ...
  Repeating the argument with words of length $N - 2L_0$ (using
  $\tr(A^{w'} A^{u_1} A^{u_2}) = \tr(A^{w'} \cdot 0) = 0$) and iterating,
...
\begin{theorem}[Chain ground space for block-injective tensors]%
  ...
  $\mathcal{G}_{N,L}(A) = \operatorname{span}\{V^{(N)}(A)\}$.
\end{theorem}
...
\begin{theorem}[Chain ground space for normal tensors]%
  ...
  spectrum, canonical form).  See~\cite[\S IV.C]{Cirac2021Matrix}. The
  normality hypothesis allows the window range to be reduced from $2L_0$
  to $L_0 + 1$ via the peripheral-spectrum structure of the transfer map
...
\begin{theorem}[Unique ground state on the periodic chain]%
  ...
  For an injective tensor~$A$ on a periodic chain of $N \ge L$ sites with
...
\begin{proof}
  ...
  $X \in \MN{D}$ with
  \[
    \psi(\sigma_1,\dots,\sigma_N) = \tr(A^{\sigma_1}\cdots A^{\sigma_N} X).
  \]
  ...
  \textbf{Step 2 (PBC forces $X$ scalar).}
  By translation invariance with periodic boundary conditions, the
  representation above holds for \emph{every} cyclic rotation of the
  sites.  In particular, for any one-letter prefix
  $u = (i_1)$ and suffix
  $w = (i_2,\dots,i_N)$:
  \[
    \psi(u,w) = \tr(A^u A^w X) = \tr(A^w X A^u),
  \]
  by trace cyclicity.  Since $A$ is injective, the products
  $\{A^u : |u| = 1\}$ span $\MN{D}$
  (Definition~\ref{def:injective}).
  Comparing the two representations and using nondegeneracy of the
  trace pairing, $X$ commutes with every element of $\MN{D}$.
  By Schur's lemma for the full matrix algebra, $X = \lambda I$ for
  some $\lambda \in \C$.
...
\begin{theorem}[RFP implies NNCPH]\label{thm:rfp_implies_nncph}
  ...
  this result requires the structural theory of renormalization
  fixed points needed for \cite[Theorem~3.10]{Cirac2021Matrix}.
\end{theorem}
