...
\begin{theorem}[Intersection property]\label{thm:ground_space_intersection}
  ...
  If $A$ is injective and $L \ge 1$, then a state $\psi$ on $L+1$ sites
...
\begin{definition}[MPV submodule]\label{def:mpv_submodule}
  ...
  \leanok
  \uses{def:mpv}
  The one-dimensional subspace spanned by the MPV state
...
\begin{definition}[Unique ground state]\label{def:has_unique_ground_state}
  ...
  \leanok
  A submodule $S$ has a unique ground state if $\dim S \le 1$.
\end{definition}
...
\begin{theorem}[Unique ground state for block-injective tensors]%
  ...
  If $A$ is $L_0$-block-injective, the parent Hamiltonian with interaction
  range $2L_0$ has a unique ground state on the periodic chain.
\end{theorem}
...
\begin{theorem}[Optimal unique ground state for normal tensors]%
  ...
  If $A$ is normal (hence $L_0$-block-injective for some $L_0$) and
