...
\begin{definition}[Route B use of the product-word span condition]\label{def:route_b_product_word_span}
    ...
    \]
    span the whole product algebra $\prod_j \MN{D_j}$.  The issue~\#934 task is to
    derive this condition for some positive length from the separated
    canonical-form/BNT hypotheses.  It matches the block-injective span
    requirement in CPGSV21 \S IV.C, where lines 2123--2128 state that the parent
    ground space is generated by the basis of normal tensors after blocking and
    then improve the bound by ``restricting to boundary conditions $L$ and $R$
...
\begin{lemma}[Conditional Route B block split from BNT selector words]
    ...
    Let $A$ be in canonical-form/BNT, and suppose length-$S$ block-selector
    words are given.  If every assembled periodic-chain ground vector has a
    boundary matrix representation commuting with all length-$(1+S)$ assembled
    words, then
    \[
        \mathcal G_{N,L}\bigl(\operatorname{Assemble}(\mu, A)\bigr)
        \subseteq \bigsqcup_j \mathcal G_{N,L}(A_j).
    ...
    boundary representation remain explicit: this lemma does not derive the
    issue~\#934 selector existence theorem, nor the wrapping/open-chain boundary
    representation.  It is the formal composition matching the CPGSV21
    block-injective ground-space statements at lines 2123--2128 and the line
    2126 instruction to restrict the regrowing argument to block-diagonal
    boundary conditions.
\end{lemma}
...
\begin{proof}
    ...
    assembled chain ground space.  Lemma~\ref{lem:route_b_block_split_from_bnt_selectors}
    now gives the conditional Route B composition once finite BNT selector words
    and the wrapping-window commuting boundary matrix are available.  What remains
    open is to derive those selector words from BNT separation alone and to supply
    the compatible boundary representation.  With the block decomposition available,
