...
\noindent\emph{Remark.} The Lean formalization currently proves the injective
special case (with $L_0 = 1$); the block-injective generalization is future
work.
...
\noindent\emph{Remark.} The blueprint phrases the previous lemma and theorem
in terms of the Hamiltonian kernel $\ker(H_N(A,L_0{+}1))$. The
current Lean formalization instead uses the chain-ground-space object
\texttt{parentHamiltonianGroundSpace} (defined via
\texttt{chainGroundSpace}); a bridge theorem between these two formulations is
not yet stated explicitly here.
