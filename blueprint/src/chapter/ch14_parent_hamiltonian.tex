...
\begin{definition}[Translated local term]\label{def:local_term_parent}
  ...
  position~$i$ (with periodic boundary conditions). Diagrammatically,
  \[
    \TNLocalTermWindow{\sigma_i}{\sigma_{i+L-1}}{L}{N}{h_i}
  \]
  the dashed window marks the $L$ consecutive sites on which the local
  projector acts, while the surrounding chain is left unchanged. The range
  $L$ and ambient chain length~$N$ are carried by the surrounding notation.
\end{definition}
...
\begin{proof}
  \leanok
  The ``invert-and-regrow'' argument:
  \[
    \TNWindowOverlap{[1,L]}{[2,L+1]}{[2,L]}
    \qquad
...
\begin{proof}
  \leanok
  The induction repeatedly merges two neighbouring admissible windows into
  a larger one:
  \[
    \TNWindowHighlight{\sigma_i}{\sigma_{i+L-1}}{[i,i+L-1]}{N}
    \qquad
    \TNWindowOverlap{[i,i+L-1]}{[i+1,i+L]}{[i+1,i+L-1]}
  \]
...
\begin{proof}
  \notready
  The relevant geometry is a family of overlapping local windows:
  \[
    \TNMartingaleWindow{L}{i}{n}
    \qquad
    \TNWindowOverlap{[i,i+L-1]}{[j,j+L-1]}{d_N(i,j)}
  \]
  From $h_i^2 = h_i$,
  \[
...
\begin{proof}
  ...
  \notready
  The local picture is again an overlap of two translated projector windows:
  \[
    \TNMartingaleWindow{L}{i}{N}
    \qquad
    \TNWindowOverlap{[i,i+L-1]}{[j,j+L-1]}{L-d_N(i,j)}
  \]
...
\begin{proof}
  ...
  sites).
  Diagrammatically the proof passes to the blocked chain and then applies
  the martingale window estimate at range~$2$:
  \[
    \TNBlockedChain{L_0}{N/L_0}
    \qquad
    \TNMartingaleWindow{2}{j}{N/L_0}
  \]
