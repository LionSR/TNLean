...
\begin{definition}[Route B product-word span witness]\label{def:route_b_product_word_span}
    \lean{MPSTensor.RouteBProductWordSpan}
    \lean{MPSTensor.HasRouteBProductWordSpan}
    \leanok
    For a fixed length $m$, the Route B product-word span condition says that the
    simultaneous block-word evaluations
    \[
        \omega \longmapsto \bigl(A_j^{\omega}\bigr)_j
    \]
    span the whole product algebra $\prod_j \MN{D_j}$.  The existential form
    \texttt{HasRouteBProductWordSpan} records a positive length $m$ with this property.
    This is the finite-span witness that issue~\#934 is meant to derive from the
    separated canonical-form/BNT hypotheses.  It matches the block-injective span
...
\begin{lemma}[Route B block split from product-word span]
    \label{lem:route_b_block_split_from_product_word_span}
    \lean{MPSTensor.chainGroundSpace_toTensorFromBlocks_le_iSup_of_wordTuple_span_eq_top}
    \lean{MPSTensor.chainGroundSpace_toTensorFromBlocks_eq_iSup_of_wordTuple_span_eq_top}
    \lean{MPSTensor.parentHamiltonianGroundSpace_le_bntSpan_of_wordTuple_span_eq_top}
    \leanok
    ...
    containment in $\operatorname{BNTSpan}_{N}(A)$.
\end{lemma}
...
\begin{proof}
    ...
    gives the reverse inclusion.  The equality adds the already-proved forward
    inclusion
    Lemma~\ref{lem:isup_chain_ground_space_block_le_assembled}.  The BNT-span
    containment is the parent-ground-space version of the same reverse inclusion,
    followed by the blockwise uniqueness endgame already built into the
