...
\begin{proof}
    ...
    \[
        Y_\mu = Y^+_{\tau^+_\eta(\mu)}
              = Y^-_{\tau^-_\eta(\mu)}.
    \]
    For $\tau=\tau^+_\eta(\mu)$ and $\tau=\tau^-_\eta(\mu)$, the common-complement
    identity gives \(C^+_{\tau^+_\eta(\mu)}=C^-_{\tau^-_\eta(\mu)}=\mu\).
    The two one-sided equations therefore become
    \[
        A^\mu A^bX=Y_\mu A^b,
        \qquad
        XA^aA^\mu=A^aY_\mu.
