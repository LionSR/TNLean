...
\begin{theorem}[Optimal unique ground state for normal tensors]%
    ...
    \leanok
    \uses{thm:chain_ground_space_eq_mpv_normal, thm:mpv_ne_zero_blk_injective}
...
\begin{proof}
    \leanok
    \uses{thm:chain_ground_space_eq_mpv_normal, thm:mpv_ne_zero_blk_injective}
    By Theorem~\ref{thm:chain_ground_space_eq_mpv_normal}, the chain ground
    space equals the MPV submodule $\spn_{\C}\{\ket{V^{(N)}(A)}\}$.
    Since $A$ is $L_0$-block-injective with $L_0 > 0$ and $N \ge L_0 + 1$,
    the MPV is nonzero by Theorem~\ref{thm:mpv_ne_zero_blk_injective}.
    A one-dimensional subspace spanned by a nonzero vector has
    $\dim = 1$.
\end{proof}
...
\begin{theorem}[RFP implies NNCPH]\label{thm:rfp_implies_nncph}
    ...
    \textbf{Status:} Scaffold; requires the structural theory of
    renormalization fixed points (Appendix~B) needed for
    \cite[Theorem~3.10]{Cirac2021Matrix}.
\end{theorem}
...
\begin{theorem}[NNCPH implies RFP]\label{thm:nncph_implies_rfp}
    ...
    \notready
    \uses{def:is_rfp, def:is_nncph, def:normal}
    If $A$ is normal and its parent Hamiltonian with $L = 2$ has
    commuting local terms for every chain length $N \ge 2$, then $A$ is
    a renormalization fixed point (RFP).
    \textbf{Status:} Scaffold; requires the ground-space characterisation
    of Beigi (J.~Phys.~A 45, 025306, 2012) needed for
    \cite[Theorem~3.10]{Cirac2021Matrix}.
\end{theorem}
