...
\begin{lemma}[Martingale condition for MPS ($L=2$)]\label{lem:martingale_mps}
  \notready
  \uses{thm:martingale_criterion, def:parent_interaction,
        def:local_term_parent, lem:ground_space_intersection, def:injective}
  Let $A$ be injective and fix interaction range $L = 2$ as in
  Definition~\ref{def:parent_interaction}.  Let $h_2(A) := \Pi_{G_2(A)^\perp}$
  be the $2$-site parent interaction, and for each chain length and site $i$
  ...
  $d_N(i,j) = 1$, where $d_N(i,j) := \min(|i-j|,\, N-|i-j|)$ is the
  cyclic distance on the ring (i.e.\ whose $2$-site supports share
  exactly one site under periodic boundary conditions),
  the spectral gap of $H_N = \sum_i h_i$ satisfies
  $\lambda_{\min}^+(H_N) \ge \gamma$ for some $\gamma > 0$ depending
  only on $A$ (not on $N$).
\end{lemma}
...
\begin{proof}
  ...
  \notready
  With $L = 2$, two terms $h_i, h_j$ with $d_N(i,j) = 1$ have supports
  overlapping in exactly one site.  Since $A$ is injective, the
  intersection property (Lemma~\ref{lem:ground_space_intersection})
  gives $\ker(h_i) \cap \ker(h_j) = G_{[i,i+2]}(A)$.
  ...
  exactly $G_{[i,i+2]}(A)$; in particular there is no non-trivial
  vector in one kernel that is orthogonal to the other yet lies in both.
  Hence the Friedrichs angle $\theta$ between $\ker(h_i)$ and
  $\ker(h_j)$ is strictly positive (for the fixed injective tensor $A$),
  and the Friedrichs cosine $\alpha := \cos\theta < 1$.
  Because $L = 2$, every pair with $d_N(i,j) = 1$ has the same overlap
  structure, so $\alpha$ depends only on $A$ and not on $i$, $j$, or $N$.

  \textbf{Spectral gap via the finite-size criterion.}
  The Nachtergaele finite-size
  criterion~\cite{Nachtergaele1996Spectral} establishes the spectral
  gap without requiring the Friedrichs cosine to be below any specific
  threshold other than $\alpha < 1$.  The criterion proceeds as follows:
  consider the Hamiltonian $h_i + h_{i+1}$ restricted to three
  consecutive sites.  By the intersection property, its ground space is
  $G_{[i,i+2]}(A)$.  Since $h_i$ and $h_{i+1}$ are projectors on a
  finite-dimensional space and $\alpha < 1$, the operator $h_i + h_{i+1}$
  has a strictly positive spectral gap
  $\epsilon(A) := \lambda_{\min}^+(h_i + h_{i+1}) > 0$
  above its ground space.  (Explicitly, the smallest nonzero eigenvalue
  of the sum of two projectors with Friedrichs cosine $\alpha$ is
  $1 - \alpha > 0$.)
  The finite-size criterion then transfers this local gap to the full
  periodic chain: there exists $\gamma > 0$, depending only on
  $\epsilon(A)$ and the local dimension (hence only on $A$), such that
  $\lambda_{\min}^+(H_N) \ge \gamma$ for all $N$ sufficiently
  large~\cite{Nachtergaele1996Spectral, Fannes1992Finitely}.

  \emph{Remark (martingale constants).}
  Alternatively, the martingale criterion
  (Theorem~\ref{thm:martingale_criterion}) can be applied with
  $c_{ij} := \tfrac{1}{2}$ for overlapping pairs (row-sum
  $= 2 \cdot \tfrac{1}{2} = 1$), yielding $\gamma = 1 - 2\alpha$.
  This gives $\gamma > 0$ only when $\alpha < \tfrac{1}{2}$, which is
  a stronger condition than $\alpha < 1$.  The finite-size criterion
  avoids this restriction and applies to all injective MPS tensors.
...
\begin{proof}
  ...
  injective tensor $A^{[L_0]}$ with $L = 2$ to obtain a uniform
  spectral gap $\gamma > 0$ depending only on $A^{[L_0]}$ (hence
  only on $A$ and $L_0$), not on $N$.
  This gives $\lambda_{\min}^+(H_N) \ge \gamma$~\cite{Fannes1992Finitely,
