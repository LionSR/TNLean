...
\begin{center}
    ...
    % contracted; the physical legs remain open.
    % Boundary signature: (virtual-west=0 | virtual-east=0 |
    % physical-up=3 | physical-down=0), both sides closed by $W_s$,
    % $V_s$.
    \tenkzkernel
    ...
    % $V_t$, joined with no intervening tensor.
    % Contraction set: the single virtual bond between the two rings.
    % Boundary signature: (virtual-west=0 | virtual-east=0 |
    % physical-up=0 | physical-down=0): no open leg remains.
...
\begin{center}
    ...
    % legs $(i,j)$, read as one pair alphabet, remain open.
    % Boundary signature: (virtual-west=0 | virtual-east=0 |
    % physical-up=3 | physical-down=3) on the left, collapsing to the
    % pair alphabet, (physical-up=3), on the right.
    \tenkzkernel
...
\begin{center}
    ...
    % square node in each pair; the physical leg $i$ remains open.
    % Boundary signature: (virtual-west=1 | virtual-east=0 |
...
\begin{center}
    ...
    % physical legs remain open.
    % Boundary signature: (virtual-west=0 | virtual-east=0 |
    % physical-up=3 | physical-down=0), both sides closed by $W$, $V$.
    \tenkzkernel
    \begin{tenkz}[rows={wire}, cols=5, west=open, east=open]
        \tn[skin=ring]{W} &
        \tn[label pos=s, ports={90:physical:$i_1$}]{B} &
