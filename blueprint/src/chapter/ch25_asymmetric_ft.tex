...
\begin{corollary}[Single-block case: the normal-target reduction]
    ...
    \uses{def:mpv, def:normal}
    Let $A$ be a normal tensor of bond dimension $D_A$, let $B$ be an
    arbitrary tensor of bond dimension $D_B$, and let $\lambda\in\C^\times$
    satisfy $\ket{V^{(N)}(B)}=\lambda^N\ket{V^{(N)}(A)}$ for every $N\geq1$.
    Then $D_B\geq D_A$, and there are maps $V:\C^{D_A}\to\C^{D_B}$ and
    $W:\C^{D_B}\to\C^{D_A}$ with $WV=\id$ and
    \begin{align}
        WB^{i_1}\cdots B^{i_N}V & =\lambda^NA^{i_1}\cdots A^{i_N}
        \label{eq:asym_single_block_compression}
    \end{align}
    for every $N\geq1$ and every word; equivalently, the rescaled tensor
    $\lambda^{-1}B$ compresses exactly onto $A$. The remainder
    $R^i=B^i-\lambda\,VA^iW$ generates a nilpotent non-unital algebra, with
    the explicit bound
    \begin{align}
...
\begin{proof}
    ...
    $C=\lambda A$, which is normal because $\lambda\neq0$. Clauses (iv) and
    (v) give the compression pair and (\ref{eq:asym_single_block_compression}),
    clause (vii) gives $D_B=D_A+z\geq D_A$, and clause (vi) gives
    (\ref{eq:asym_single_block_nilpotency}) with $r=|S|+z=1+(D_B-D_A)$.
\end{proof}

Corollary~\ref{cor:asym_single_block} is Propositions~20 and~21 of
\cite{Molnar2018SemiInjective} in the present notation (their $V,W$ are the
present $W,V$), with three repairs and one strengthening. First, the
hypothesis $\lambda\neq0$ is stated explicitly: it is absent from the
printed Proposition~20, whose conclusion fails at $\lambda=0$, since the
zero tensor $B=0$ satisfies the hypothesis while no compression onto a
nonzero normal target exists. Second, the printed conclusion carries no
scalar, because the source proof silently normalizes $\lambda=1$; the
weighted form above makes the normalization explicit. Third, the remainder
