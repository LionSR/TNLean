...
\begin{proof}\leanok\uses{lem:bnt_finite_index_overlap_orthonormal_li, thm:bnt_perm_same_card}
...
The asymptotic orthonormality of overlaps matches the blocks up to a
permutation and gauge phases. Section~\ref{sec:sector_bnt_strong_match}
establishes the exact sector counterpart used in the Fundamental Theorem
(Chapter~\ref{ch:ft_proof}).

The overlap dichotomy also shows that the MPV states of distinct normal
blocks are non-proportional at arbitrarily large lengths, which enters the
...
the corresponding theorem in the companion quantum-channel volume~\cite[``Newton--Girard trace recursion'']{QICLean2026Blueprint}. It identifies a characteristic polynomial
...
% CPSV16 Lemma A.5 (`Lem:app_simple`), lines 1155--1163.
% Applied to the nonzero coefficients of a canonical form; the nonzero
% hypothesis is the nonzero-coefficient local correction recorded in
% docs/paper-gaps/cpsv16_bnt_uniqueness_zero_coefficient.tex; the
% alternative routes are in docs/paper-gaps/cpsv16_power_sum_alternative_route.tex.
\begin{theorem}[Finite-range equal power sums imply equal multisets]
    ...
    it also holds at $N=0$ then $x_a=x_b$ and the full lists agree. The
    nonzero variant is the one applied to the nonzero coefficients of a
    canonical form, which is how \cite[Lemma~A.5]{Cirac2016MPDO_arXiv} is used.
\end{theorem}
