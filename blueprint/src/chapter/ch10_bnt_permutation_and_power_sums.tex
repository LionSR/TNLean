...
\begin{lemma}[Printed finite power-sum comparison]
    ...
    for every exponent up to the larger list length.  Lemma~A.5 of
    \cite{Cirac2016MPDO_arXiv} asserts that the list lengths agree and that the
    two multisets are equal.

    This is false when zero entries are allowed: the lists $(1)$ and $(1,0)$
    have equal power sums for every positive exponent.  The valid alternatives
    either assume every entry is nonzero, compare the power sum at exponent
    zero as well, or conclude equality only after deleting zero entries.  These
    alternatives are formalized in
    Theorem~\ref{thm:bounded_power_sum_multiset} and documented in
