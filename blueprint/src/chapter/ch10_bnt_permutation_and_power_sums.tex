...
% Gap: docs/paper-gaps/cpsv16_power_sum_alternative_route.tex.
\begin{lemma}[Printed finite power-sum comparison]
    \label{lem:cpsv_power_sum_printed_status}
    \notready
    \uses{thm:bounded_power_sum_multiset}
    Suppose two finite lists of complex numbers have equal positive power sums
    for every exponent up to the larger list length.  According to
    \cite[Lemma~A.5]{Cirac2016MPDO_arXiv}, the list lengths agree and the two
    multisets are equal.

    This is false when zero entries are allowed: the lists $(1)$ and $(1,0)$
    have equal power sums for every positive exponent.  The conclusion becomes
    valid if every entry is nonzero, if the exponent-zero power sum is also
    compared, or if zero entries are deleted before comparing the multisets.
    The nonzero version is Theorem~\ref{thm:bounded_power_sum_multiset}.
\end{lemma}
