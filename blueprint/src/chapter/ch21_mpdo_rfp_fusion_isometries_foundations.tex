...
A linear factorization of the doubled-index blocked transfer map $E_n$
through a subspace, with the retract identities stated below, exists
precisely when $E_n$ is idempotent. Requiring such a factorization at every
positive blocked size is equivalent to idempotence of $E_1$. This condition
is distinct from the physical renormalization fixed-point condition in
Definition~\ref{def:is_rfp_via_ts} and
\cite[Definition~4.1, lines 638--660]{Cirac2016MPDO_arXiv}. For a tensor $M$
in CF generating MPDOs, the latter requires tpCPMs $S,T$ on the physical
indices with $S[M_2(X)]=M_1(X)$ and $T[M_1(X)]=M_2(X)$ for every boundary
matrix $X$. The physical maps constructed in the source proof likewise act
on physical indices
\cite[lines 1974--2085]{Cirac2016MPDO_arXiv}. The fusion maps in
\cite[Theorem~4.14(ii)--(iii), lines 972--991]{Cirac2016MPDO_arXiv}
concern the vertically read tensors. Below, their support restrictions and
exact reconstruction are stated explicitly, separately from the stronger
condition that the maps are isometries on the whole product bond space.
...
\begin{proof}\leanok
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Since $E_1$ is the original transfer map, this is exactly doubled-index
