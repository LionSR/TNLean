...
A linear factorization of the doubled-index blocked transfer map $E_n$ through
a subspace with the retract identities of (\ref{eq:rfp_transfer_retract}) exists
precisely when $E_n$ is idempotent. Requiring such a factorization at every
positive blocked size is equivalent to idempotence of $E_1$. This
transfer-retract condition is distinct from the physical renormalization
fixed-point condition in Definition~\ref{def:is_rfp_via_ts} and
\cite[Definition~4.1, lines~638--660]{Cirac2016MPDO_arXiv}. For an MPDO tensor
$M$ in CF, the latter requires tpCPMs $S,T$ on the physical indices satisfying
$S[M_2(X)]=M_1(X)$ and $T[M_1(X)]=M_2(X)$ for every boundary matrix $X$, acting
on physical indices as in \cite[lines~1974--2085]{Cirac2016MPDO_arXiv}. By
contrast, the fusion maps in
\cite[Theorem~4.14(ii)--(iii), lines~972--991]{Cirac2016MPDO_arXiv} act on the
vertically read tensors, distinguishing support restrictions and exact
reconstruction from full-support isometries.
...
\begin{definition}[Transfer-retract datum]%
    ...
    MPO tensor $M$ consists of a subspace $\mathcal{A}_n \subseteq \MN{D}$
    ...
    \end{align}
    where $E_n$ denotes the blocked transfer map. No complete positivity or
    trace preservation is required of these linear maps.
\end{definition}

\begin{theorem}[Transfer-retract data imply transfer-map idempotence]%
    \label{thm:mpdo_transfer_retract_implies_zcl}
    \lean{MPOTensor.TransferRetractData.isZCL}
    \leanok
    \uses{def:mpdo_fusion_isometry, def:mpo_is_zcl}
    A one-site transfer-retract datum implies doubled-index transfer-map
    idempotence.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[One-site transfer-retract criterion]%
    \label{thm:mpdo_one_site_transfer_retract_iff_zcl}
    \lean{MPOTensor.transferRetractData_one_iff_isZCL}
    \leanok
    \uses{def:mpdo_fusion_isometry, def:mpo_is_zcl}
    A one-site transfer-retract datum exists if and only if the doubled-index
    MPO transfer map is idempotent.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Transfer retracts at positive blocked sizes]%
    \label{thm:mpdo_transfer_retract_of_zcl}
    \lean{MPOTensor.transferRetractData_of_isZCL}
    \leanok
    \uses{def:mpdo_fusion_isometry, def:mpo_is_zcl}
    If the doubled-index transfer map is idempotent, then transfer-retract data
    exist at every positive blocked size.
\end{theorem}
\begin{proof}\leanok
    Every positive blocked transfer map equals a positive power of the
    idempotent one-site map and is therefore idempotent. Factoring it through
    its range gives the transfer-retract datum.
\end{proof}
...
\begin{definition}[Operator product of MPO tensors]%
    ...
    joins two vertically read tensors along an original horizontal bond.
    It is the tensor written $M_\alpha M_\beta$ in
...
\begin{theorem}[The operator family is multiplicative]%
    ...
    \leanok
    \uses{def:mpdo_operator_product, def:mpo_family, lem:mpdo_operator_product_eval_word}
    For every length $L$, the operator generated by the product tensor
...
\begin{lemma}[Blockwise conjugation and restriction]
    ...
    If $r$ is a bijective reindexing of the row space of a matrix
...
\begin{definition}[Fusion isometries on the full product bond space]%
    ...
    Let $\{M_\gamma\}$ be a finite family of tensors with a common physical
    dimension and bond dimensions depending on $\gamma$. Fusion isometries
    on the full product bond space consist of diagonal matrices
    $\chi_{\alpha,\beta,\gamma}$ with positive entries and maps
    $U_{\alpha,\beta}$ satisfying
    $U_{\alpha,\beta}^\dagger U_{\alpha,\beta}=1$ and, site by site,
    \begin{align}
    ...
    \cite[Theorem~4.14(iii)]{Cirac2016MPDO_arXiv}. The definition does not
    require $\{M_\gamma\}$ to be a BNT. In applications to an MPDO tensor,
    these tensors come from the vertical BNT of
    \cite[Proposition~4.13]{Cirac2016MPDO_arXiv}; their physical dimension
    in the vertical reading is the bond dimension of the original tensor.
    Statement (iii) of the source theorem also requires the vector
    $m$, with $m_\alpha=\tr(\mu_\alpha)$ for the positive diagonal
    multiplicity matrices $\mu_\alpha$ of the vertical decomposition, to
    be an idempotent for the multiplication induced by
    $c_{\alpha,\beta,\gamma}^{(1)}
        =\tr(\chi_{\alpha,\beta,\gamma})$. This condition is recorded
    separately in
    Definition~\ref{def:mpdo_bnt_label_idempotent_coefficient_form}.

    The condition $U_{\alpha,\beta}^\dagger U_{\alpha,\beta}=1$ on the whole
    product bond space is an additional restriction. The unrestricted
    source statement permits a common zero corner. Its support convention
    must therefore be made explicit: the map is a coisometry onto the
    retained direct sum, and exact reconstruction is required. This
    correction is documented in
    \cite{gap:cpsv16_figure11_fusion_coisometry}.
\end{definition}

\begin{definition}[Fusion coisometries onto the retained direct sum]
    ...
    For the same labelled tensors and positive diagonal matrices, put
    $G_{\alpha,\beta}^{ij}
        =\bigoplus_\gamma\chi_{\alpha,\beta,\gamma}\otimes M_\gamma^{ij}$.
    Fusion coisometries onto the retained direct sum are maps
    $U_{\alpha,\beta}$ satisfying
    ...
    $U_{\alpha,\beta}^\dagger U_{\alpha,\beta}=1$ for every pair, these maps
    are fusion isometries on the full product bond space.

    The coisometry condition makes explicit the retained-row orientation
    of the source's vertical CF decomposition. Exact reconstruction states
    that the discarded common corner is zero. This correction to the
    full-support interpretation is documented in
    \cite{gap:cpsv16_figure11_fusion_coisometry}.
\end{definition}

With $G_{\alpha,\beta}^{ij}$ denoting the retained direct-sum letter,
\cite[Theorem~4.14(iii), equation~(89), lines~986--991]
{Cirac2016MPDO_arXiv} gives the following fusion identity:
...
\begin{lemma}[Weighted zipper identities on the active support]
    ...
    A fusion coisometry onto the retained direct sum satisfies
    \begin{align}
        U_{\alpha,\beta}(M_\alpha M_\beta)^{ij}
         & =G_{\alpha,\beta}^{ij}U_{\alpha,\beta},
        \notag                                              \\
...
\begin{theorem}[Weighted zipper identities]%
    ...
    $G_{\alpha,\beta}^{ij}
        ...
        \otimes M_\gamma^{ij}$.
    The fusion isometry satisfies the two weighted zipper identities
    \begin{align}
        U_{\alpha,\beta}(M_\alpha M_\beta)^{ij}
         & =G_{\alpha,\beta}^{ij}U_{\alpha,\beta},
        \notag                                              \\
    ...
    separation of the final labels are still needed to construct an invertible
    $F$-move.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Length-independent zipper identities and reconstruction]%
    ...
    \leanok
    \uses{def:mpdo_bnt_fusion_isometry_family, def:mpdo_bnt_label_length_independent, thm:mpdo_bnt_label_length_independent_natural, thm:mpdo_bnt_weighted_zipper}
    Suppose the structure coefficients
    $c_{\alpha,\beta,\gamma}^{(L)}
        =\tr(\chi_{\alpha,\beta,\gamma}^{L})$, with positive diagonal
    $\chi_{\alpha,\beta,\gamma}$, are independent of $L>0$. Then
    $\chi_{\alpha,\beta,\gamma}=1_{r_{\alpha,\beta,\gamma}}$ for every triple
    of labels, where $r_{\alpha,\beta,\gamma}$ is the dimension of the
    corresponding multiplicity space. If
    \begin{align}
        H_{\alpha,\beta}^{ij}
         & =
        \bigoplus_\gamma
    ...
    \end{align}
    These are the identity-weight specialization of the fusion and zipper
    relations in
    ...
    discussed in \cite[line~1010]{Cirac2016MPDO_arXiv}.
\end{theorem}
\begin{proof}\leanok
    ...
    Theorem~\ref{thm:mpdo_bnt_weighted_zipper}. For reconstruction, insert
...
\begin{theorem}[Common positive blocking of BNT fusion isometries]
    ...
    \leanok
    \uses{thm:cpsv_bnt_blocked_simultaneous_span, thm:mpdo_blocking_doubled_index, thm:mpdo_blocking_product, thm:mpdo_bnt_length_independent_zipper}
    Suppose that the labelled doubled-index tensors form a BNT and admit
    fusion isometries on the full product bond spaces. Suppose also that
    $c_{\alpha,\beta,\gamma}^{(L)}
        =\tr(\chi_{\alpha,\beta,\gamma}^{L})$, with positive diagonal
    $\chi_{\alpha,\beta,\gamma}$, is independent of $L>0$. Then there is
    one integer $L>0$ such that the tuples of letters of the blocked tensors
    $M_\gamma^{[L]}$ span the full product of their matrix algebras.
    Hence every $M_\gamma^{[L]}$ is injective.
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    where $r_{\alpha,\beta,\gamma}$ is the dimension of the multiplicity
    space on which $\chi_{\alpha,\beta,\gamma}$ acts.
    The original fusion isometry satisfies
    \begin{align}
...
\begin{proof}\leanok
    ...
    replaces every $\chi_{\alpha,\beta,\gamma}$ by the identity on its
    multiplicity space. The two zipper identities and reconstruction then
    follow by multiplying the blocked fusion
    identity (\ref{eq:rfp_blocked_fusion}) by
    $U_{\alpha,\beta}$ or $U_{\alpha,\beta}^\dagger$ on the appropriate side.
\end{proof}

The three displayed diagrams represent, respectively, the two unweighted
zipper identities and their local reconstruction consequence. The symbol
$H$ denotes $H_{\alpha,\beta}^{ij}$, and the open physical legs carry the
indices $i,j$. The reconstruction relies directly on the isometry condition
$U_{\alpha,\beta}^{\dagger}U_{\alpha,\beta}=1$. The zipper orientation is that of
\cite[Section~3, Equations~(19)--(21)]{Bultinck2017Anyons}; the MPDO identity
is given in \cite[Theorem~4.14(iii), lines~986--991]{Cirac2016MPDO_arXiv},
with identity weights in the length-independent case discussed in
