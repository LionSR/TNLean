...
The proof here is purely algebraic. The trace-pairing maps introduced
below are auxiliary devices attached to the chosen generating families
$\{A^i\}$ and $\{B^i\}$; they are used only to construct the linear
map that is later shown to be a gauge transform.
...
\begin{definition}[Trace pairing map]\label{def:trace_pairing_map}
    ...
    \leanok
    \uses{def:mps_tensor}
    For an MPS tensor~$A$, attach to the chosen family $\{A^i\}$ the
