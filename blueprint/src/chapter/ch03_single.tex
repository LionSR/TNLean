...
This chapter proves that if an injective MPV tensor~$A$ generates
the same MPV family as another tensor~$B$ of the same bond dimension,
then $A$ and $B$ are gauge equivalent. The formalization still uses the
historical name \texttt{MPSTensor} for these tensors.
The proof recorded here is purely algebraic and does not use the later
