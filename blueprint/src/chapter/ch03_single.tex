...
proved in the companion quantum-channel volume~\cite[``Nondegeneracy of the trace pairing'']{QICLean2026Blueprint}.
...
\begin{proof}
    \leanok
    \uses{def:mpv}
    By (\ref{eq:mps_mpv_component}), the configuration $\sigma = w$ gives
    $V^{(|w|)}(A)_w = \tr(A^w)$. Equality of the matrix-product vectors gives
    $V^{(N)}(A) = V^{(N)}(B)$ for all~$N$, so
    $\tr(A^w) = \tr(B^w)$.
\end{proof}

The trace pairing against the family $\{A^i\}$ connects word traces to the
algebra structure of $\MN{D}$.
...
\begin{proof}
    ...
    the corresponding theorem in the companion quantum-channel volume~\cite[``Nondegeneracy of the trace pairing'']{QICLean2026Blueprint}.
\end{proof}
...
\begin{lemma}[Injectivity transfers across a range inclusion]%
    ...
    If $\ker \Phi = \{0\}$ and $\range \Phi \subseteq
        \range \Psi$, then $\ker \Psi = \{0\}$.
\end{lemma}

\begin{proof}
    \leanok
    By rank--nullity, $\ker \Phi = \{0\}$ gives
    $\dim \range \Phi = \dim V$, and the range inclusion gives
    \begin{align}
        \dim V & = \dim \range \Phi \le \dim \range \Psi \le \dim V.
        \notag
    \end{align}
    so $\dim \range \Psi = \dim V$ and $\ker \Psi = \{0\}$.
\end{proof}
...
\begin{theorem}[The linear extension exists and is unique]\label{thm:linear_ext}
    ...
    then there is a unique linear map $T : \MN{D} \to \MN{D}$ with
    $T(A^i) = B^i$ for all~$i$.
\end{theorem}
...
\begin{proof}
    ...
    The $\{A^i\}$ span $\MN{D}$, so $\range \Phi_A \subseteq
        \range \Phi_B$. With $\ker \Phi_A = \{0\}$
...
\begin{proof}
    \leanok
    \uses{thm:trace_pairing_injective, lem:same_mpv_trace, lem:injectivity_transfer}
    Length-two trace agreement gives
    $\Phi_B(T(A^i)) = \Phi_B(B^i) = \Phi_A(A^i)$; since the $\{A^i\}$ span
    $\MN{D}$, this extends to $\Phi_B \circ T = \Phi_A$, so
    $\range \Phi_A = \range(\Phi_B \circ T) \subseteq \range \Phi_B$. With
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Hence injectivity of $\Phi_B$ yields
    $T(A^i A^j) = B^i B^j$. Extend in two stages: for fixed~$j$, the maps
    $M \mapsto T(M A^j)$ and $M \mapsto T(M) B^j$ agree on the spanning
    $\{A^i\}$, hence $T(M A^j) = T(M) B^j$ for all~$M$; then for fixed~$M$,
    the maps $N \mapsto T(M N)$ and $N \mapsto T(M) T(N)$ agree on the
    spanning $\{A^j\}$, hence $T(M N) = T(M) T(N)$ for all~$N$.
\end{proof}

\section{Inner automorphism and the single-block theorem}

A nonzero multiplicative linear map on $\MN{D}$ is bijective by simplicity,
preserves the unit by surjectivity, and is given by conjugation by an
invertible matrix by the Skolem--Noether theorem.

\begin{theorem}[A nonzero multiplicative endomorphism of $\MN{D}$ is bijective]%
    \label{thm:simplicity}
    \lean{Matrix.linear_mul_endomorphism_bijective}
    \leanok
...
\begin{proof}
    \leanok
    Choose~$X$ with $T(X) = \Id$. Then
    \begin{align}
        T(\Id) & = T(X)T(\Id) = T(X\Id) = T(X) = \Id.
        \notag
    \end{align}
\end{proof}
...
\begin{theorem}[Single-block Fundamental Theorem]\label{thm:ft_single}
    ...
    there is $X \in \GL_D(\C)$ with $B^i = X A^i X^{-1}$ for all~$i$. In
    ...
    \begin{center}
        ...
        % lines 1173--1178, states A^i = X B^i X^{-1}; the displayed orientation
        % follows by swapping the tensors and replacing X with X^{-1}.
        % Ink-to-index map: i labels the free physical leg, while the west
        % and east wires carry the free virtual indices of B^i.
        % Contraction set: exactly the two bonds X--A and A--X^{-1} are summed.
...
\begin{proof}
    \leanok
    \uses{thm:linear_ext, thm:linear_ext_mul, thm:simplicity, lem:trace_ne_zero, lem:linear_map_to_alg}
    Theorem~\ref{thm:linear_ext} gives the unique linear~$T$ with
    $T(A^i) = B^i$, and Theorem~\ref{thm:linear_ext_mul} makes it
    multiplicative. It is nonzero: otherwise $B^i = 0$ for all~$i$, against
    Lemma~\ref{lem:trace_ne_zero}. By Theorem~\ref{thm:simplicity}, $T$ is
    bijective, and by Lemma~\ref{lem:linear_map_to_alg} it fixes~$\Id$, so $T$
    is a $\C$-algebra automorphism. The Skolem--Noether theorem
    \cite[``Skolem--Noether'']{QICLean2026Blueprint} gives
    $X \in \GL_D(\C)$ with $T(M) = X M X^{-1}$; evaluating at $M = A^i$
    gives $B^i = X A^i X^{-1}$.
\end{proof}
