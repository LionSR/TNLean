...
\begin{definition}[$Z$-gauge equivalence]
    ...
    Theorem~\ref{thm:periodic_ft} refines this global relation to block-multiplicity diagonal gauges $Z_j$.
\end{definition}
...
\begin{remark}[$Z$-gauge is trivial for period-$1$ blocks]
    ...
    Nontrivial $Z$-gauges can only appear when $m>1$.
\end{remark}

\begin{theorem}[Blocking a $\Z_m$-gauge gives an ordinary gauge]
    ...
    If $A$ and $B$ are $\Z_m$-gauge equivalent, then their $m$-blocked
    tensors $A^{[m]}$ and $B^{[m]}$ are gauge equivalent in the ordinary sense.
\end{theorem}

\begin{proof}\leanok
    Write the $\Z_m$-gauge relation as
    $ZA^i=YB^iY^{-1}$ with $Z^m=\Id$ and $[A^i,Z]=0$.
...
\begin{corollary}[Blocking a $\Z_m$-gauge preserves the MPV family]
    ...
    If $A$ and $B$ are $\Z_m$-gauge equivalent, then the blocked tensors
