\begin{definition}[Retained coordinates for the vertical-sector algebra]
    ...
    this space by $\bigoplus_\alpha Y_\alpha$.
...
\begin{lemma}[Retraction in retained vertical-sector coordinates]
    ...
    \leanok
    \uses{def:mpdo_retained_vertical_sector_coordinates, lem:mpdo_retained_vertical_sector_extraction, lem:mpdo_vertical_sector_retraction}
...
\begin{lemma}[Weighted vertical contraction in retained coordinates]
    ...
    \leanok
    \uses{def:mpdo_retained_vertical_sector_coordinates, def:mpdo_vertical_bond_matrix_contraction}
...
\begin{lemma}[Vertical contraction and physical closure]
    ...
    one-site physical closure: $\widetilde M(X)=M(X)$.
\end{lemma}
...
\begin{proof}\leanok
    Applying Lemma~\ref{lem:mpdo_bond_contraction_block_sum} to the block form
...
\begin{lemma}[Contraction of a letterwise forward identity]
    ...
    \leanok
    \uses{def:phys_close, def:mpdo_vertical_bond_matrix_contraction, lem:mpdo_bond_contraction_composition, lem:mpdo_vertical_bond_contraction_phys_close}
    Let $B_{ab}$ be an arbitrary vertical tensor. If
    $U\widetilde M_{ab}U^\dagger=B_{ab}$ for every pair $(a,b)$, then, for
    every bond matrix $X$, $U M(X)U^\dagger=B(X)$.
\end{lemma}
...
\begin{lemma}[Contraction of a letterwise reconstruction identity]
    ...
    \uses{def:phys_close, def:mpdo_vertical_bond_matrix_contraction, lem:mpdo_bond_contraction_composition, lem:mpdo_vertical_bond_contraction_phys_close}
    Let $B_{ab}$ be an arbitrary vertical tensor. If
    $\widetilde M_{ab}=U^\dagger B_{ab}U$ for every pair $(a,b)$, then, for
    every bond matrix $X$, $M(X)=U^\dagger B(X)U$.
\end{lemma}
...
\begin{lemma}[Contraction of the one-site vertical canonical form]
    ...
    \leanok
    \uses{def:vertical_cf_mpdo, def:phys_close, def:mpdo_vertical_bond_matrix_contraction, lem:mpdo_vertical_bond_contraction_forward, lem:mpdo_vertical_bond_contraction_reconstruction, lem:mpdo_vertical_bond_contraction_assembled}
    Suppose that
    \begin{align}
...
\begin{proof}\leanok
    ...
    \begin{align}
        M(X)
         & =\sum_{a,b}X_{ba}\widetilde M_{ab}
        \notag                                                                    \\
         & =U^\dagger\left(\sum_{a,b}X_{ba}
        \left(\bigoplus_\alpha\mu_\alpha\otimes M_\alpha\right)_{ab}\right)U
        \notag                                                                    \\
         & =U^\dagger\left(\bigoplus_\alpha\mu_\alpha\otimes M_\alpha(X)\right)U.
        \notag
    \end{align}
\end{proof}
...
\begin{theorem}[Action of the transported physical maps]
    ...
    \leanok
    \uses{def:mpdo_vertical_coordinate_map_transport, lem:mpdo_vertical_bond_contraction_forward, lem:mpdo_vertical_bond_contraction_reconstruction, thm:mpdo_one_site_closure_two_site_blocking}
...
\begin{definition}[Refinement and coarse-graining on vertical sectors]
    ...
    \leanok
    \uses{def:mpdo_normalized_vertical_sector_maps, def:mpdo_retained_vertical_sector_coordinates, def:mpdo_vertical_coordinate_map_transport}
    Define maps between the one-site and two-site simple-sector algebras by
    \begin{align}
        \widetilde T
         & =\widetilde R_2T_{\mathrm v}R_1,
        \notag                              \\
        \widetilde S
         & =\widetilde R_1S_{\mathrm v}R_2.
        \notag
    \end{align}
    The partial trace and diagonal-summand extraction define these maps on the
    full matrix algebras; on the weighted block-diagonal image they match the
    normalized maps in
    \cite[Appendix~C.4, lines~1972--1979]{Cirac2016MPDO_arXiv}.
\end{definition}
...
\begin{lemma}[Positivity and trace in normalized sector coordinates]
    ...
    \leanok
    \uses{def:mpdo_positive_vertical_sector_family, def:mpdo_retained_vertical_sector_coordinates}
    Suppose that every multiplicity space is nonzero and every diagonal entry
...
\begin{lemma}[Normalized embedding of multiplicity-trace-scaled sectors]
    ...
    \uses{def:mpdo_retained_vertical_sector_coordinates}
    Suppose that every multiplicity space is nonzero and every diagonal entry
    of $\mu_\alpha$ is positive. For $m_\alpha=\tr(\mu_\alpha)$,
    \begin{align}
...
\begin{lemma}[Partial trace of weighted vertical sectors]
    ...
    \begin{align}
        \widetilde R_\mu\!\left(\bigoplus_\alpha\mu_\alpha\otimes X_\alpha\right)
...
\begin{lemma}[Trace under coisometric compression]
    ...
    \leanok
    Let $U:\C^n\to\C^r$ satisfy $UU^\dagger=1$. If $Z\geq0$, then
    \begin{align}
        \tr(UZU^\dagger)
         & \leq\tr(Z),
        \notag                                 \\
        ...
        \notag
    \end{align}
    For every $W\in\C^{r\times r}$, one also has
    $\tr(U^\dagger WU)=\tr(W)$.
\end{lemma}
...
\begin{lemma}[Positivity and trace before active-sector compression]
    ...
    \leanok
    \uses{def:mpdo_transported_vertical_sector_maps, def:mpdo_positive_vertical_sector_family, def:mpdo_vertical_sector_precompression, lem:mpdo_normalized_vertical_sector_positive_trace}
...
\begin{proof}\leanok
    ...
    identities follow from Lemma~\ref{lem:mpdo_coisometric_compression_trace},
    Lemma~\ref{lem:mpdo_vertical_sector_precompression_positive_trace}, and
    trace preservation of the physical maps.
\end{proof}
...
\begin{theorem}[Positivity and trace loss of the transported sector maps]
    ...
    \leanok
    \uses{def:mpdo_transported_vertical_sector_maps, def:mpdo_positive_vertical_sector_family, def:mpdo_vertical_sector_precompression, lem:mpdo_normalized_retained_vertical_sector_embedding, lem:mpdo_retained_vertical_sector_partial_trace, lem:mpdo_coisometric_compression_trace, lem:mpdo_vertical_sector_precompression_positive_trace}
    ...
    \begin{align}
        U_2^\dagger U_2 Z_T(X)
         & =Z_T(X),
        \notag      \\
        U_1^\dagger U_1 Z_S(Y)
         & =Z_S(Y),
        \notag
    \end{align}
    respectively.

    The condition on vanishing trace loss characterizes when the transported
    maps preserve the total sector trace.
\end{theorem}
