...
\begin{definition}[AKLT tensor from singlet bonds]\label{def:aklt_tensor_review}
    \lean{Matrix.singletY, MPSTensor.akltRMPSpinOneState, MPSTensor.akltTensorRMP}
    \leanok
    \uses{def:mps_tensor}
...
\begin{theorem}[Doubly degenerate entanglement spectrum of the AKLT state]
    \label{thm:aklt_entanglement_spectrum_even}
    \lean{MPSTensor.akltBlocked_even_finrank_eigenspace}
    \leanok
    \uses{thm:spt_entanglement_spectrum_even, thm:aklt_blocked_injective, thm:aklt_gauge_anticomm, lem:twisted_tensor_block_gauge}
    Let $\Lambda$ be a positive definite fixed point of the adjoint transfer map
    of the length-$2$ blocked AKLT tensor.  Every eigenspace of $\Lambda$ has
    even dimension.
\end{theorem}
\begin{proof}
    \leanok
    \uses{thm:spt_entanglement_spectrum_even, thm:aklt_blocked_injective, thm:aklt_gauge_anticomm, lem:twisted_tensor_block_gauge}
    The blocked tensor is injective and satisfies $\E(\Id)=\Id$.  The two
    generators of $\Z_2\times\Z_2$ act by unitary Kronecker squares with virtual
    gauges $i\sigma_y$ and $\sigma_z$, which carry over from the single site by
    Lemma~\ref{lem:twisted_tensor_block_gauge} and anticommute.  Apply
    Theorem~\ref{thm:spt_entanglement_spectrum_even}.
\end{proof}
