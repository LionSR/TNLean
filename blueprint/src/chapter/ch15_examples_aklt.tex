...
\begin{definition}[Tensor in a new physical basis]\label{def:physical_basis_change}
    \lean{MPSTensor.physicalBasisChange}
    \leanok
    \uses{def:mps_tensor}
    Let $V\in\MN{d}$ and let $\ket{e_a}=\sum_s V_{sa}\ket{s}$ be its columns.
    The tensor $A$ expressed in the basis $\{\ket{e_a}\}$ is
    $B^{a}=\sum_{s}\overline{V_{sa}}\,A^{s}$.
\end{definition}

\begin{theorem}[Coefficients in a new physical basis]\label{thm:physical_basis_change_coefficients}
    \lean{MPSTensor.mpv_physicalBasisChange}
    \leanok
    \uses{def:physical_basis_change}
    For every $N$ and every $a_1,\dots,a_N$,
    $\tr(B^{a_1}\cdots B^{a_N})
    =\sum_{s}\prod_{k}\overline{V_{s_k a_k}}\,\tr(A^{s_1}\cdots A^{s_N})$.
    When $V$ is unitary this is the component of the matrix product state
    of $A$ along $\ket{e_{a_1}\cdots e_{a_N}}$.
\end{theorem}
\begin{proof}
    \leanok
    Expand each $B^{a_k}$ and use multilinearity of the product and the trace.
\end{proof}
...
\begin{definition}[The Pauli basis of the spin-$1$ site]\label{def:aklt_review_basis}
    \lean{MPSTensor.akltRMPBasis, MPSTensor.akltRMPBasisFixed,
        MPSTensor.akltRMPBasis_mem_unitaryGroup,
    ...
    \end{align}
    It is orthonormal, and so is the basis obtained by replacing $\ket{+}$ with
    $-\frac{i}{\sqrt2}(\ket{-1}+\ket{+1})$.
\end{definition}

\begin{theorem}[Pauli form of the AKLT tensor]\label{thm:aklt_review_pauli_form}
    \lean{MPSTensor.akltTensorRMPPauli, MPSTensor.physicalBasisChange_akltRMPBasis,
        MPSTensor.physicalBasisChange_akltRMPBasisFixed}
    \leanok
    \uses{def:aklt_tensor_review, def:physical_basis_change, def:aklt_review_basis,
        lem:aklt_review_matrices}
    Expressed in the basis (\ref{eq:aklt_review_basis}), the tensor
    (\ref{eq:aklt_review_tensor}) becomes
    \begin{align}
        A^{-} & = \tfrac{1}{\sqrt2}\sigma_x,
              & A^{+} & = -\tfrac{1}{\sqrt2}\sigma_y,
              & A^{0} & = \tfrac{1}{\sqrt2}\sigma_z.
        \notag
    \end{align}
    In the basis with $\ket{+}=-\frac{i}{\sqrt2}(\ket{-1}+\ket{+1})$ it becomes
    exactly $A^{-}=\sigma_x/\sqrt2$, $A^{+}=\sigma_y/\sqrt2$,
    $A^{0}=\sigma_z/\sqrt2$, the form printed in
    \cite[Appendix~A, ``The AKLT state'']{Cirac2021Matrix}. The source prints
    $A^{+}=\sigma_y/\sqrt2$ for the basis (\ref{eq:aklt_review_basis}); the
    sign is corrected in \cite{gap:rmp_aklt_pauli_basis_sign}.
\end{theorem}
\begin{proof}
    \leanok
    Since $\langle +|\pm1\rangle=-i/\sqrt2$, one has
    $A^{+}=-\frac{i}{\sqrt2}(A^{-1}+A^{+1})=-\frac{i}{\sqrt2}Y=-\sigma_y/\sqrt2$,
    while $A^{-}=(A^{-1}-A^{+1})/\sqrt2=\sigma_x/\sqrt2$; conjugating the phase
    of $\ket{+}$ reverses the sign of $A^{+}$.
\end{proof}
...
\begin{theorem}[Symmetries and normality of the singlet-bond AKLT tensor]%
    ...
    \uses{lem:aklt_review_gauge_bridge, thm:aklt_normal, thm:aklt_z2z2_symmetric,
        thm:aklt_so3_symmetric, thm:aklt_review_pauli_form, def:normal}
...
\begin{theorem}[The singlet-bond AKLT tensor lies in a non-trivial SPT phase]%
    ...
    \leanok
    \uses{def:aklt_tensor_review, thm:aklt_nontrivial_spt, def:aklt_omega}
...
\begin{proof}
    \leanok
    An entrywise computation for each of the four group elements.
\end{proof}
