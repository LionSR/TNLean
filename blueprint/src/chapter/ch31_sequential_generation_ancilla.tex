...
\begin{definition}[Deterministic transition scheme]
    ...
    \lean{MPSPreparation.IsTransitionInteraction,
        ...
        MPSPreparation.transitionStep_eq_of_isTransitionInteraction}
    \leanok
    \uses{def:seqanc_deterministic}
    ...
    of \cite[Section~5.1]{PerezGarcia2007Matrix}. Since every site starts in
    $\ket{0}$, only the action of $T$ on site inputs $\ket{0}$ enters, and a
    step leaving the site in $\ket{i}$ acts on the ancilla by the matrix with
    entries
    $\bra{x,i}T(W^{[k]}\otimes\Id)\ket{y,0}
        =\delta_{x_2,0}\,W^{[k]}_{(x_1,i),y}$.
\end{definition}
...
\begin{definition}[Sequential scheme without an ancilla]
    ...
    \leanok
    Let $N\ge 2$. Starting from $\ket{0}^{\otimes N}$, an operation $U^{[k]}$
    ...
    operations are arbitrary \cite[Section~5.2]{PerezGarcia2007Matrix}. Before
    $U^{[k]}$ acts, sites $k+1,\dots,N$ are still in $\ket{0}$, and site $k$
    is not touched afterwards, so the chain state after $U^{[k]}$ is
    $\ket{\chi_k}\otimes\ket{0}^{\otimes(N-k-1)}$ with
    $\chi_1=U^{[1]}\ket{0,0}$ and
...
\begin{theorem}[Sequential generation without an ancilla]
    ...
    Let $N\ge 2$. The states generated by sequential schemes without an
...
\begin{proof}
    \leanok
    \uses{thm:seqanc_sweep, thm:seqanc_sweep_obc, thm:seqanc_norm,
    ...
    open-boundary representation with bond dimension at most $d$ after
    Theorem~\ref{thm:seqanc_sweep_obc}. For unitary operations each
    ...
    site, and run the successive decompositions of
