...
\begin{proof}
    ...
    isometry condition. For the chain, write
    $Q^{\sigma}=Q_0^{\sigma_0}Q'^{\sigma'}$, where $Q'$ is the chain
    $Q_1,\dots,Q_{n-1}$ and $\sigma'=(\sigma_1,\dots,\sigma_{n-1})$. The
    vector $w=Q'^{\sigma'}v$ is supported below $b_1$ by
    Theorem~\ref{thm:seqanc_support_propagation}, so the one-site identity
    for $Q_0$ gives $\sum_{\sigma_0}\|Q_0^{\sigma_0}w\|^2=\|w\|^2$, and the
    induction hypothesis for $Q'$ gives $\sum_{\sigma'}\|Q'^{\sigma'}v\|^2=\|v\|^2$.
\end{proof}
