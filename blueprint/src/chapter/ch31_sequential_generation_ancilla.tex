...
at most $D$. The last two sections state the deterministic transition scheme
for $d=2$ and sequential generation without an ancilla, which are not yet
formalized.
...
\begin{definition}[Operations induced by unitaries]
    ...
    $U_0,\dots,U_{N-1}$, with $U_k$ acting on site $k$, and an initial
    ancilla state $\ket{\varphi_I}$, the ancilla component of the joint state
    at the configuration $\sigma$ is $A_0^{\sigma_0}\cdots
        A_{N-1}^{\sigma_{N-1}}\ket{\varphi_I}\in\C^D$, where $A_k$ is induced
    by $U_k$. This is \cite[Section~5.1]{PerezGarcia2007Matrix}.
\end{definition}
...
\begin{theorem}[One decomposition step]
    ...
    \lean{MPSPreparation.exists_isometryOn_mul}
    \leanok
...
\begin{proof}
    ...
    By induction on $n$. For $n=0$ take $r'=R_{-1}r$. Otherwise apply
    Theorem~\ref{thm:seqanc_one_step} to $Y^i=R_{-1}P_0^i$, which gives
    $Y^i=Q_0^iR_0$ with $R_0$ row-supported below $b_1$, and apply the
    induction hypothesis to $R_0$ and $P_1,\dots,P_{n-1}$. This is the
    construction by successive singular value decompositions of Schön,
    Solano, Verstraete, Cirac and Wolf, on which the proof in
    \cite[Section~5.1]{PerezGarcia2007Matrix} is based.
\end{proof}

\begin{theorem}[Isometric chains preserve the norm]
    \label{thm:seqanc_norm}
    \lean{MPSPreparation.sum_normSq_mulVec_of_isometryOn,
...
\begin{proof}
    \leanok
    \uses{thm:seqanc_support_propagation, thm:seqanc_eval_last}
    Expand $\sum_i\|Q^iv\|^2=\sum_{\beta,\beta'<b}\overline{v_\beta}v_{\beta'}
        \sum_{i,\alpha}\overline{Q^i_{\alpha\beta}}Q^i_{\alpha\beta'}$ and use the
    isometry condition. For the chain, peel off the last site, apply the
    one-site identity to $Q_{n-1}$ and $v$, and apply the induction
    hypothesis to the vector $Q_{n-1}^{\sigma_{n-1}}v$, which is supported
    below $b_{n-1}$.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{thm:varying_bond_obc_zero_pad_zero}
...
\begin{proof}
    ...
    \uses{thm:seqanc_sweep, thm:seqanc_coeff_corner, thm:seqanc_norm,
        thm:seqanc_support_propagation}
...
\begin{definition}[Deterministic transition scheme]
    ...
    \uses{def:seqanc_deterministic}
    Let $d=2$ and let the ancilla be $\C^D\otimes\C^2$. In every step an
    arbitrary unitary acts on the ancilla, followed by the fixed interaction
    of the ancilla with the current site
    \begin{align}
    ...
    \end{align}
    and the ancilla decouples after the last step. This is the third scheme
    of \cite[Section~5.1]{PerezGarcia2007Matrix}.
\end{definition}
...
\begin{definition}[Sequential scheme without an ancilla]
    ...
    Starting from $\ket{0}^{\otimes N}$, an operation $U^{[k]}$ acts on sites
    $k$ and $k+1$, for $k=1,\dots,N-1$ in turn. The scheme is deterministic if
    every $U^{[k]}$ is unitary and probabilistic if the operations are
    arbitrary \cite[Section~5.2]{PerezGarcia2007Matrix}.
\end{definition}
...
\begin{theorem}[Sequential generation without an ancilla]
    ...
    The states generated by sequential schemes without an ancilla, either
    deterministically or probabilistically (the latter up to normalization),
    are exactly the states with an open-boundary representation of bond
    dimension at most $d$. This is \cite[Section~5, Theorem ``Sequential
        generation without ancilla'']{PerezGarcia2007Matrix}.
\end{theorem}
\begin{proof}
    \uses{thm:seqanc_deterministic, thm:seqanc_probabilistic}
    For $k<N-1$ the matrices
    $A^{[k]}_{i,\alpha,\beta}=\bra{i,\beta}U^{[k]}\ket{\alpha,0}$ are the site
    matrices, with $\alpha=0$ at $k=1$; a singular value decomposition of
    $\bra{i,j}U^{[N-1]}\ket{\alpha,0}$ in the grouping $i\mid\alpha,j$ gives the
    last two sites. Conversely, run the deterministic scheme of
    Theorem~\ref{thm:seqanc_deterministic} using site $k+1$ as the ancilla of
    the $k$-th step, followed by a swap of sites $k+1$ and $k+2$.
\end{proof}
