...
\begin{lemma}[Similarity is an involution]%
	...
	the similarity transforms by $C$ and $C^{-1}$ compose to the identity:
	\[
		X \;\mapsto\; C\bigl(C^{-1}\, E(C\,X\,C^\dagger)\,(C^\dagger)^{-1}\bigr)\,C^\dagger
			\;=\; E(X).
	\]
\end{lemma}
...
\begin{proof}\leanok
	...
	$C \cdot (C^{-1} \, X \, (C^\dagger)^{-1}) \cdot C^\dagger$ collapses to $X$
	using $C C^{-1} = \Id$ and $(C^\dagger)^{-1} C^\dagger = \Id$, and the
	outer wrapping by $C \, (\cdot) \, C^\dagger$ then cancels the inner
...
\begin{proof}\leanok
	...
	Lemma~\ref{lem:similarity_involution}.  The reverse direction is
