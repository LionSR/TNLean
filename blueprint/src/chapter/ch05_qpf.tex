...
\begin{proof}
    ...
    \[
        \tr\left(B \cdot \sum_{k=0}^{D-1} \tbinom{D-1}{k} E^k(A)\right) > 0.
    \]
...
\begin{lemma}[Similarity is an involution]%
    ...
    \[
        X \mapsto C(C^{-1}\, E(C\,X\,C^\dagger)\,(C^\dagger)^{-1})\,C^\dagger
        = E(X).
    \]
\end{lemma}
...
\begin{theorem}[Scaling preserves irreducibility]%
    ...
    is also irreducible.
    This is the scalar special case ($C = \Id$) of~\cite[Proposition~6.6]{Wolf2012Quantum}.
\end{theorem}

\begin{proof}\leanok
    An invariant projection for $c E$ is an invariant projection
