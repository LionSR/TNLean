...
\begin{theorem}[Trace matrix of the sector-tensor $\eta$-data]
    ...
    \leanok
    \uses{def:mpdo_explicit_eta_of_sector_tensors, def:mpdo_explicit_eta_trace_matrix, def:mpdo_closed_sector_tensors}
...
\begin{definition}[Nonzero-weight sector trace matrix]%
    ...
    \leanok
    \uses{def:mpdo_one_site_sector_matrix_space, def:mpdo_minimal_neighboring_operator}
...
\begin{theorem}[Identification and spanning of active one-site matrices]%
    ...
    \leanok
    \uses{def:mpdo_active_sector_trace_matrix, thm:mpdo_sector_virtual_matrix_spanning}
...
\begin{theorem}[Complementary active-sector spaces]%
    ...
    \leanok
    \uses{def:mpdo_active_sector_trace_matrix, thm:mpdo_active_one_site_matrix_spanning, thm:mpdo_directed_cut_one_site_spaces}
...
\begin{theorem}[Strong connectivity of the active neighboring support]%
    ...
    \leanok
    \uses{thm:mpdo_active_sector_complementary_spaces, lem:matrix_one_sided_product_obstruction}
    Suppose that the active one-site matrices span $\MN{D}$ and each active
...
\begin{theorem}[Two-step return through every active sector]%
    ...
    \leanok
    \uses{thm:mpdo_active_one_site_matrix_spanning, thm:mpdo_active_trace_matrix_positive_entries}
    Suppose that the active one-site matrices span $\MN{D}$ and each active
...
\begin{theorem}[Irreducibility of the active trace matrix]%
    ...
    \leanok
    \uses{thm:mpdo_active_neighboring_support_strongly_connected, thm:mpdo_active_trace_matrix_positive_entries}
...
\begin{theorem}[Three-step returns and a common positive power]%
    ...
    \leanok
    \uses{thm:matrix_primitive_of_two_three_returns, thm:mpdo_active_sector_two_step_return, thm:mpdo_active_trace_matrix_irreducible, thm:mpdo_sector_support_triangle_closure}
    Suppose that the neighboring operators are positive semidefinite, the
...
\begin{theorem}[Retained witnesses for the active inverse-map factorization]
    \label{thm:mpdo_inverse_map_active_sector_witnesses}
    ...
    \leanok
    \uses{def:mpdo, def:injective, def:mpdo_eta_structure, def:mpdo_three_site_closure, def:mpdo_zero_weight_reparameterized_inverse_map_factorization}
    Let ${\cal K}$ be an injective tensor generating MPDOs, let $R$ close a
    three-site state $\rho$, and choose a Hayashi decomposition with weights
    $p_k$. Fix outer indices
    for which the corresponding entry of $R$ is nonzero, and let $F$ be the
    zero-weight reparameterized inverse-map factorization. There are coherent
    sector phases for which all neighboring operators are positive semidefinite
    and the active one-site matrices span $\MN{D}$. Every active sector contains
    a nonzero virtual matrix, every virtual matrix in a zero-weight sector
    vanishes, every nonzero neighboring edge closes through a third active
    sector, every inactive left tensor vanishes, and $I_*$ is nonempty.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:mpdo_eta_coherent_positive_choice, thm:mpdo_zero_weight_reparameterized_neighboring_operator, lem:mpdo_sector_tensor_left_zero_of_weight_zero, thm:mpdo_sector_virtual_matrix_spanning, thm:mpdo_active_one_site_matrix_spanning, thm:mpdo_sector_support_triangle_closure, thm:mpdo_physical_sector_rephasing_invariance}
    Choose the coherent positive phases. Injectivity gives the full virtual
    matrix span, while zero-weight reparameterization removes the inactive
    summands from the active span. A nonzero virtual matrix exists in every
    active sector, and the neighboring-support recurrence gives the three-edge
    return. Rephasing preserves the nonzero neighboring edges, so this return
    persists after the coherent rephasing. Inactive left tensors vanish by
    construction. Finally, $\sum_kp_k=1$ gives a nonempty active sector.
\end{proof}
...
\begin{theorem}[Primitivity of the active sector trace matrix]%
    ...
    \leanok
    \uses{def:mpdo, def:injective, def:is_sal, def:mpdo_active_sector_trace_matrix}
    Let ${\cal K}$ be an injective tensor that generates MPDOs and satisfies
    SAL, and choose the positive semidefinite neighboring operators of
    Theorem~\ref{thm:mpdo_inverse_map_active_sector_witnesses}. Restrict the sector
...
\begin{proof}
    \uses{def:mpdo_normalized_four_site_tail, def:mpdo_zero_weight_reparameterized_inverse_map_factorization, thm:mpdo_four_site_sal_eta_structure, thm:mpdo_normalized_four_site_tail_entry_ne_zero, thm:mpdo_three_site_closure_reduced, thm:mpdo_inverse_map_active_sector_witnesses, thm:mpdo_active_sector_three_step_return_and_primitivity}
    \leanok
    Theorem~\ref{thm:mpdo_four_site_sal_eta_structure} supplies the four-site
    Hayashi decomposition, and
    Theorem~\ref{thm:mpdo_normalized_four_site_tail_entry_ne_zero} supplies
    nonzero outer indices for the normalized four-site tail. Its reduced block
    state gives the required three-site closure; form the zero-weight
    reparameterized inverse-map factorization. Then
    Theorem~\ref{thm:mpdo_inverse_map_active_sector_witnesses} supplies one
    rephased factorization with positive neighboring operators, full active
    span, nonzero active virtual matrices, and the three-edge return condition.
    Apply
    Theorem~\ref{thm:mpdo_active_sector_three_step_return_and_primitivity} to
    conclude that the active trace matrix is primitive.
\end{proof}
...
\begin{theorem}[Zero-correlation-length relations on the primitive active trace matrix]%
    ...
    \leanok
    \uses{def:mpdo_active_sector_trace_matrix, lem:finset_sum_subtype_ne_zero, thm:mpdo_sector_trace_matrix_primitive, thm:mpdo_closed_sector_pairing_normalized_idempotent}
...
\begin{proof}
    \leanok
    \uses{thm:mpdo_inverse_map_active_sector_witnesses, thm:mpdo_active_sector_three_step_return_and_primitivity, lem:mpo_source_zcl_normalized_idempotent}
    Theorem~\ref{thm:mpdo_inverse_map_active_sector_witnesses} gives one
    rephased factorization with positive neighboring operators and vanishing
    left tensors on the zero-weight sectors. Apply
    Theorem~\ref{thm:mpdo_active_sector_three_step_return_and_primitivity} to
    obtain primitivity of its active trace matrix.
    Lemma~\ref{lem:mpo_source_zcl_normalized_idempotent} supplies normalized
    idempotence of the physical-trace transfer, from which the same active trace
    matrix satisfies the square--cube identity and the positive-power trace
    identities.
\end{proof}

\begin{lemma}[SAL makes the physical-trace transfer nonzero]
    \label{lem:mpdo_sal_phys_trace_transfer_nonzero}
    \lean{MPOTensor.IsSAL.physTraceTransfer_ne_zero}
    \leanok
    \uses{def:is_sal, def:mpo_physical_trace_transfer}
    If an MPO tensor ${\cal K}$ satisfies SAL, then its physical-trace
    transfer is nonzero:
    \begin{align}
        \mathcal T_{\cal K} & \ne0. \notag
    \end{align}
...
\begin{proof}\leanok
    ...
    The SAL normalization at one site gives a nonzero trace for the one-site
    periodic operator. Lemma~\ref{lem:mpdo_trace_loop_chain} identifies this
    trace with that of $\mathcal T_{\cal K}$, which therefore cannot vanish.
\end{proof}

\begin{theorem}[SAL and literal idempotence imply source zero correlation length]
    \label{thm:mpdo_sal_literal_idempotence_source_zcl}
    \lean{MPOTensor.IsSAL.isSourceZCL_of_physTraceTransfer_sq}
    \leanok
    \uses{def:is_sal, def:mpo_physical_trace_transfer, def:mpo_source_zcl}
    If ${\cal K}$ satisfies SAL and
    \begin{align}
        \mathcal T_{\cal K}^2 & =\mathcal T_{\cal K}, \notag
    \end{align}
    then ${\cal K}$ has source zero correlation length.
\end{theorem}

\begin{proof}\leanok
    \uses{lem:mpdo_sal_phys_trace_transfer_nonzero, thm:mpo_source_zcl_of_physical_trace_idempotent}
    The preceding lemma gives $\mathcal T_{\cal K}\ne0$. Apply
    Theorem~\ref{thm:mpo_source_zcl_of_physical_trace_idempotent}.
\end{proof}
...
\begin{theorem}[Literal-ZCL specialization of the active trace relations]%
    ...
    \leanok
    \uses{def:injective, def:is_sal, def:mpo_physical_trace_transfer, def:mpdo_physical_sector_factorization, def:mpdo_active_sector_trace_matrix}
    ...
    $(\widehat T^*)^2=(\widehat T^*)^3$ and, for every positive integer $N$,
    \begin{align}
        \tr((\widehat T^*)^N) & =\tr(\widehat T^*). \notag
    \end{align}
    No idempotence or rank-one conclusion is asserted.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpdo_sal_literal_idempotence_source_zcl, thm:mpdo_active_trace_matrix_zcl_relations}
    SAL makes the one-site periodic trace nonzero, and hence
    $\mathcal T_{\cal K}\ne0$.
    Theorem~\ref{thm:mpdo_sal_literal_idempotence_source_zcl} therefore gives
    source zero correlation length. Apply
...
\begin{theorem}[Virtual spanning does not force active-sector idempotence]
    ...
    \lean{MPOTensor.ActiveSectorSpanningCounterexample.factorization}
    \leanok
    \uses{def:mpdo_physical_sector_factorization, def:mpdo_active_sector_trace_matrix}
...
% This theorem concerns only the explicit witness above.  It proves that its
% selected four-sector inverse-map factorization is nonminimal.  The distinct
% four-letter rank-one neighboring-trace obstruction in
% thm:mpdo_sal_zcl_bnt_sector_structure_counterexample refutes only the
% low-level implication from the inherited analytic properties.  Its prepared
% ambient two-block BNT decomposition does not supply a simple-biCF
% reconstruction, so the source-context absorbed-representative assertion
% remains open in #6775.
\begin{theorem}[A one-sector factorization of the four-sector witness]
    \label{thm:mpdo_active_trace_witness_one_sector_factorization}
    \lean{MPOTensor.ActiveSectorSpanningCounterexample.oneSectorEquiv,
        MPOTensor.ActiveSectorSpanningCounterexample.oneSectorSign,
        MPOTensor.ActiveSectorSpanningCounterexample.oneSectorLeftTensor,
        MPOTensor.ActiveSectorSpanningCounterexample.oneSectorRightTensor,
        MPOTensor.ActiveSectorSpanningCounterexample.oneSectorLeftTensor_apply,
        MPOTensor.ActiveSectorSpanningCounterexample.oneSectorRightTensor_apply,
        MPOTensor.ActiveSectorSpanningCounterexample.oneSectorFactorization,
        MPOTensor.ActiveSectorSpanningCounterexample.%
        oneSector_neighboringOperator_eq,
        MPOTensor.ActiveSectorSpanningCounterexample.%
        oneSector_neighboringOperator_posSemidef,
        MPOTensor.ActiveSectorSpanningCounterexample.%
        oneSector_neighboringOperator_trace,
        MPOTensor.ActiveSectorSpanningCounterexample.%
        oneSectorNeighboringTraceFactorization,
        MPOTensor.ActiveSectorSpanningCounterexample.%
        exists_oneSector_neighboringTraceFactorization}
    \leanok
    \uses{def:mpdo_physical_sector_factorization, def:mpdo_neighboring_trace_factorization, thm:mpdo_virtual_spanning_active_trace_counterexample, thm:mpdo_inverse_map_nonzero_rectangular_remainder}
    The tensor in
    Theorem~\ref{thm:mpdo_virtual_spanning_active_trace_counterexample}
    also admits a physical-sector factorization with one sector.  Write each
    physical label as a pair $(x,y)\in\{0,1\}^2$ and put
    $Z=\operatorname{diag}(1,-1)$.  The left and right factors are
    \begin{align}
        l_0 & =\frac14 I, & l_1 & =Z, \notag         \\
        r_0 & =I,         & r_1 & =\frac18 Z. \notag
    \end{align}
    ...
    \begin{align}
        \eta
         & =I\otimes\frac14 I+\frac18 Z\otimes Z
        =\operatorname{diag}\left(\frac38,\frac18,\frac18,\frac38\right).
        \notag
    \end{align}
    In particular, $\eta$ is positive semidefinite and $\tr(\eta)=1$.
    Taking $a=b=1$ gives the normalized rank-one trace factorization.  Thus
    the four sectors selected by the inverse-map construction are nonminimal;
    their non-rank-one trace matrix is not an obstruction to the existence of
    another physical-sector factorization for this tensor.
\end{theorem}

\begin{proof}\leanok
    Reindex the four physical basis vectors by parity and quotient.  The
    diagonal entries of the tensor then factor as
    $(l_\beta)_{x,x}(r_\alpha)_{y,y}$, and all off-diagonal entries vanish.
    Contracting the common virtual label gives the displayed expression for
    $\eta$.  Its four diagonal entries prove positivity and trace one.
\end{proof}

% Project-derived from the twice-iterated sector-controlled maps in CPSV16,
% Appendix C.2; this is not a theorem printed in the source.
% Taxonomy: conditional helper.
\begin{theorem}[Pair-preserving trace obstruction]
    \label{thm:mpdo_pair_preserving_trace_obstruction}
    \lean{MPOTensor.PhysicalSectorFactorization.FourSiteMiddleIndex}
    \lean{MPOTensor.PhysicalSectorFactorization.fourSiteNeighboringOperator}
    \lean{MPOTensor.PhysicalSectorFactorization.neighboringTraceMatrix}
    \lean{MPOTensor.PhysicalSectorFactorization.neighboringTraceMatrix_apply}
    \lean{MPOTensor.PhysicalSectorFactorization.%
        trace_fourSiteNeighboringOperator}
    \lean{MPOTensor.PhysicalSectorFactorization.%
        neighboringTraceMatrix_eq_pow_three_of_pairwise_coarsening}
    \lean{MPOTensor.PhysicalSectorFactorization.%
        neighboringTraceMatrix_eq_pow_three_of_pairwise_refinement}
    \leanok
    \uses{def:mpdo_physical_sector_factorization}
    Let $\eta_{k,h}$ be the neighboring operators of a physical-sector
    factorization, and define
    \begin{align}
        C_{k,h}
         & =\tr(\eta_{k,h}),
        \notag               \\
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Suppose that, for every fixed pair $(k,h)$, there is either a
    trace-preserving linear map from the four-site middle algebra to the
    two-site middle algebra which sends $\Theta_{k,h}$ to $\eta_{k,h}$, or a
    trace-preserving linear map in the reverse direction which sends
    $\eta_{k,h}$ to $\Theta_{k,h}$.  If the same direction is chosen for all
    pairs, then
    \begin{align}
        C & =C^3.
        \notag
...
\begin{proof}
    \leanok
    Multiplicativity of the trace on tensor products and additivity on direct
    sums give
    \begin{align}
        \tr(\Theta_{k,h})
        ...
        \notag
    \end{align}
    Trace preservation identifies this quantity with
    $\tr(\eta_{k,h})=C_{k,h}$ in either direction.  The entrywise identities
    give $C=C^3$.
\end{proof}
...
\begin{proof}
    ...
    bound is the corresponding theorem in the companion quantum-channel volume~\cite[``Forward Lorentz-cone trace inequality'']{QICLean2026Blueprint} applied
    to $\eta_{k,h}$.
\end{proof}
...
\begin{theorem}[Literal zero correlation length stabilizes the active
    ...
    \leanok
    \uses{def:mpdo_active_sector_trace_matrix, def:mpo_physical_trace_transfer, lem:finset_sum_subtype_ne_zero}
    Let ${\cal K}$ be an MPO tensor with a physical-sector factorization
...
\begin{proof}
    \leanok
    \uses{def:mpdo_active_sector_trace_matrix, thm:mpdo_active_sector_three_step_return_and_primitivity, thm:matrix_primitive_pow_two_rank_one}
...
\begin{lemma}[Indicator-path expansion of a matrix power]
    ...
    \lean{pow_apply_eq_sum_path_indicator}
    \leanok
...
\begin{theorem}[Trace of a matrix power as a sum over cyclic products]
    ...
    \lean{trace_pow_eq_sum_cyclic_product}
    \leanok
...
\begin{lemma}[Reduction to the active sector]
    ...
    \leanok
    \uses{def:mpdo_active_sector_trace_matrix, thm:mpdo_cyclic_neighboring_trace_products}
    Let ${\cal K}$ be an MPO tensor with a physical-sector factorization
...
\begin{theorem}[The overlap formula]
    ...
    \leanok
    \uses{lem:mpdo_active_sector_trace_sq_matrix, thm:mpdo_doubled_overlap_hilbert_schmidt, thm:mpdo_trace_pow_cyclic_product, lem:mpdo_trace_conjugate_physical_invariant, lem:mpdo_sum_prod_traceSq_active}
    Let ${\cal K}$ be an MPO tensor with a physical-sector factorization
...
\begin{proof}
    \leanok
    \uses{lem:mpdo_active_sector_trace_sq_matrix, thm:mpdo_doubled_overlap_hilbert_schmidt, thm:mpdo_trace_pow_cyclic_product, thm:mpdo_cyclic_neighboring_trace_products, lem:mpdo_trace_conjugate_physical_invariant, lem:mpdo_sum_prod_traceSq_active}
...
\begin{theorem}[The active sector is unique]
    ...
    \leanok
    \uses{def:mpdo_active_sector_trace_matrix, def:nt_cpsv, thm:mpdo_active_trace_matrix_literal_zcl_powers, thm:mpdo_overlap_formula}
...
\begin{proof}
    \leanok
    \uses{def:mpdo_active_sector_trace_matrix, lem:mpdo_active_sector_trace_sq_matrix, thm:mpdo_active_trace_matrix_literal_zcl_powers, thm:mpdo_active_sector_three_step_return_and_primitivity, thm:mpdo_overlap_formula, thm:mpdo_trace_pow_similarity_squared_diagonal, thm:matrix_trace_pow_tendsto_zero_of_nonneg_le_hadamard_self}
...
\begin{theorem}[The Case-I relation $(T^*)^2=T^*$]
    ...
    \leanok
    \uses{thm:mpdo_active_sector_singleton, thm:mpdo_active_trace_matrix_literal_zcl_powers}
    Let ${\cal K}$ be an MPO tensor of positive bond dimension $D$ with a
    physical-sector factorization and sector weights $p_k$, and let
    $I_*=\{k:p_k\ne0\}$. Suppose that every neighboring operator
    $\eta_{k,h}$ is positive semidefinite,
...
\begin{theorem}[Normalized Case-I rank-one coefficients]
    \label{thm:mpdo_casei_rank_one_coefficients}
    \lean{MPOTensor.activeSectorTraceMatrix_rank_one_coefficients_of_literal_ZCL}
    \leanok
    \uses{thm:mpdo_active_trace_matrix_pow_two_eq}
    Under the complete normal Case-I hypotheses of
    Theorem~\ref{thm:mpdo_active_trace_matrix_pow_two_eq}, there are real
    functions $a,b:I_*\to\mathbb R$ such that, for all $k,h\in I_*$,
    \begin{align}
        T^*_{k,h} & =a_kb_h,
        \notag
    \end{align}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This is the normal Case-I active-sector packaging of the displayed
    rank-one and normalization equations in
    \cite[Appendix~C.2, lines~1394--1401 and 1484--1499]{Cirac2016MPDO_arXiv}.
    It neither transfers a conclusion to Case~II nor uses the paper's
    constant-trace-powers rank-one argument.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:mpdo_active_sector_singleton, thm:mpdo_active_trace_matrix_pow_two_eq, thm:mpdo_active_trace_matrix_literal_zcl_powers}
    Write $I_*=\{k_0\}$. Strict positivity of $(T^*)^2$ and
    $(T^*)^2=T^*$ give
    \begin{align}
        0 & <(T^*_{k_0,k_0})^2=T^*_{k_0,k_0}.
        \notag
    \end{align}
    Thus the sole entry is $T^*_{k_0,k_0}=1$. Taking $a_k=b_k=1$ gives
    $T^*_{k,h}=a_kb_h$ and
    $\sum_{k\in I_*}a_kb_k=1$.
\end{proof}
...
\begin{theorem}[Strong-area-law form of the normalized Case-I coefficients]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    and that the underlying tensor is normal in the sense of
    Definition~\ref{def:nt_cpsv}. Then there is a physical-sector factorization
    with sector weights $p_k\geq0$ satisfying $\sum_kp_k=1$, for which every
    neighboring operator is positive semidefinite. There are real functions
    $a,b$ on
    $I_*=\{k:p_k\ne0\}$. For all $k,h\in I_*$,
    \begin{align}
        T^*_{k,h} & =a_kb_h,
        \notag
    \end{align}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This is the active-sector part of the normal Case-I conclusion of
    \cite[Appendix~C.2, Case~I, lines~1374--1381]{Cirac2016MPDO_arXiv}.
    The active Hayashi factorization comes from
    \cite[Appendix~C.2, Lemma~C.4, lines~1406--1471]{Cirac2016MPDO_arXiv};
    Theorem~\ref{thm:mpdo_casei_all_sector_rank_one_coefficients_of_is_sal}
    extends these coefficients to every sector label, as in Lemma~C.5.
    % Literal idempotence is essential here; the statement neither replaces it
    % by a scale-invariant zero-correlation-length condition nor asserts a
    % conclusion in Case~II.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:mpdo_casei_rephased_inverse_map_coefficient_witness}
    Apply
    Theorem~\ref{thm:mpdo_casei_rephased_inverse_map_coefficient_witness}
    and retain its active coefficient functions and equations.
\end{proof}

\begin{theorem}[Vanishing at a zero-weight incident sector]
    \label{thm:mpdo_rephased_zero_weight_incident_vanish}
    \lean{MPOTensor.rephase_zeroWeightReparameterized_neighboringOperator_eq_zero_of_incident}
    \leanok
    \uses{def:mpdo_zero_weight_reparameterized_inverse_map_factorization, def:mpdo_physical_sector_rephasing}
    Let $(\eta_{k,h})_{k,h}$ be the neighboring operators of the
    zero-weight-reparameterized inverse-map factorization, after a coherent
    sector rephasing, and let $(p_k)_k$ be its Hayashi weights. If
    $p_k=0$ or $p_h=0$, then
    \begin{align}
        \eta_{k,h} & =0.
        \notag
    \end{align}
    The reparameterization makes the right tensor of an inactive source sector
    zero, while its left tensor is already zero. Thus this is a statement about
    the retained reparameterized and rephased witness, not about the raw
    inverse-map factorization.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:mpdo_physical_sector_rephasing_invariance, thm:mpdo_zero_weight_reparameterized_neighboring_operator, lem:mpdo_sector_tensor_left_zero_of_weight_zero}
    Rephasing changes a neighboring operator by
    \begin{align}
        (\eta_z)_{k,h} & =z_k^{-1}z_h\eta_{k,h}.
        \notag
    \end{align}
    Hence \(\eta_{k,h}=0\) implies \((\eta_z)_{k,h}=0\). If \(p_k=0\),
    the reparameterized right tensor vanishes, so \(\eta_{k,h}=0\). If
    \(p_h=0\), the left tensor vanishes and again \(\eta_{k,h}=0\).
\end{proof}

\begin{theorem}[Rephased inverse-map Case-I coefficient witness]
    \label{thm:mpdo_casei_rephased_inverse_map_coefficient_witness}
    ...
    nonnegative weights $(p_k)_k$ satisfying $\sum_kp_k=1$, positive
    semidefinite neighboring operators $\eta_{k,h}$, and real functions
    $a,b$ on $I_*=\{k:p_k\ne0\}$. For all $k,h\in I_*$,
    \begin{align}
        T^*_{k,h} & =a_kb_h,
        \notag
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    For all ambient sector labels $k,h$,
    \begin{align}
        p_k=0\ \text{or}\ p_h=0 & \implies \eta_{k,h}=0.
        \notag
    \end{align}
    The incident-vanishing clause belongs to this chosen reparameterized and
    rephased factorization, not to the raw inverse-map factors. This is the
    normal Case-I construction from
    \cite[Appendix~C.2, Case~I, Lemmas~C.4--C.5,
        lines~1374--1505]{Cirac2016MPDO_arXiv}; the incident-vanishing clause is the
    chosen zero-weight convention for inactive summands.
\end{theorem}
...
\begin{proof}
    ...
    Its active-sector witnesses give the normalized rank-one equations, and
    Theorem~\ref{thm:mpdo_rephased_zero_weight_incident_vanish} gives the final
    incident-vanishing clause for the same factorization.
\end{proof}

\begin{theorem}[All-sector normalized Case-I rank-one coefficients]
    \label{thm:mpdo_casei_all_sector_rank_one_coefficients_of_is_sal}
    \lean{MPOTensor.exists_neighboringOperator_trace_rank_one_coefficients_of_isSAL_of_literal_ZCL}
    \leanok
    \uses{def:mpdo, def:injective, def:is_sal, def:mpo_physical_trace_transfer, def:nt_cpsv}
    Let ${\cal K}$ be an injective normal MPO tensor satisfying the strong area
    law and literal physical-trace idempotence,
    \begin{align}
        \mathcal T_{\cal K}^2 & =\mathcal T_{\cal K}.
        \notag
    \end{align}
    Then there is a physical-sector factorization with nonnegative weights
    $(p_k)_k$ satisfying $\sum_k p_k=1$, positive semidefinite neighboring
    operators $\eta_{k,h}$, and real families $(a_k)_k,(b_k)_k$ indexed by all
    sector labels. For all $k,h$,
    \begin{align}
        \operatorname{Re}\tr(\eta_{k,h}) & =a_kb_h, \notag \\
        \sum_k a_kb_k                    & =1. \notag
    \end{align}
    On the nonzero-weight sectors these are the coefficient families of
    Theorem~\ref{thm:mpdo_casei_rephased_inverse_map_coefficient_witness}; if
    $p_k=0$,
    then $a_k=b_k=0$. This is a project-derived realization of the all-index
    coefficient statement in \cite[Appendix~C.2, Lemma~C.5,
        lines~1473--1499]{Cirac2016MPDO_arXiv}. The source does not introduce the
    active--inactive split or ambient filler sectors used here. This theorem
    concerns literal zero correlation length only; it makes no assertion for
    the scale-invariant source-ZCL condition or in Case~II.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:mpdo_casei_rephased_inverse_map_coefficient_witness}
    Extend the coupled witness's active coefficient families by zero. In the chosen
    zero-weight-reparameterized and rephased factorization, $p_k=0$ makes
    every neighboring operator incident to $k$ vanish. Therefore the trace
    factorization is zero whenever either index is inactive, while it is the
    active factorization otherwise. Filtering the full diagonal sum to the
    nonzero-weight sectors gives
    \begin{align}
        \sum_k a_kb_k
         & =\sum_{k:p_k\ne0}a_kb_k
        =1.
