...
\begin{theorem}[Retained witnesses for the active inverse-map factorization]
    ...
    a nonzero virtual matrix, every nonzero neighboring edge closes through a
    third active sector, every inactive left tensor vanishes, and $I_*$ is
    nonempty.
\end{theorem}
...
\begin{proof}
    ...
    Theorem~\ref{thm:mpdo_active_sector_three_step_return_and_primitivity} to
    obtain primitivity of its active trace matrix. Source zero correlation
    length then gives a positive normalization of this same matrix satisfying
    the square--cube identity and the positive-power trace identities.
\end{proof}
...
\begin{theorem}[Strong-area-law form of the normalized Case-I coefficients]
    ...
    Literal idempotence is essential here; the statement
    neither replaces it by a scale-invariant zero-correlation-length condition
    nor asserts a conclusion in Case~II.
\end{theorem}
