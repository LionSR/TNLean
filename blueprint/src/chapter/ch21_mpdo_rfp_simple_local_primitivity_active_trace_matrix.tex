...
\begin{proof}
    ...
    Hayashi decomposition, and
    Theorem~\ref{thm:mpdo_normalized_four_site_tail_entry_ne_zero} supplies
    nonzero outer indices for the normalized four-site tail, and
    Theorem~\ref{thm:mpdo_three_site_closure_reduced} identifies the required
    three-site closure. Apply the zero-weight reparameterized inverse-map
    factorization and choose the coherent sector phases from
    Theorem~\ref{thm:mpdo_inverse_map_active_sector_witnesses}. Its neighboring
    operators are positive semidefinite, and its active one-site matrices,
    nonzero sector tensors, return condition, inactive sectors, and nonempty
    active sector satisfy the hypotheses of
    Theorem~\ref{thm:mpdo_casei_rank_one_coefficients}.
    Hence
    \begin{align}
        T^*_{k,h}&=a_kb_h,
        \notag\\
        ...
        \notag
    \end{align}
    for the resulting active sector set.
\end{proof}
...
\begin{theorem}[All-sector normalized Case-I rank-one coefficients]
    ...
    Theorem~\ref{thm:mpdo_casei_rank_one_coefficients_of_is_sal}; if $p_k=0$,
...
\begin{proof}
    \leanok
    \uses{thm:mpdo_casei_rank_one_coefficients_of_is_sal,
      thm:mpdo_rephased_zero_weight_incident_vanish}
    Extend the active coefficient families by zero. In the chosen
