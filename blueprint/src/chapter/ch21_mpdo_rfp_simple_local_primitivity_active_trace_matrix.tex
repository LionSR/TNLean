...
\begin{proof}
    ...
    \uses{thm:mpdo_physical_sector_rephasing_invariance,
      thm:mpdo_zero_weight_reparameterized_neighboring_operator,
      lem:mpdo_sector_tensor_left_zero_of_weight_zero}
    If the source sector has zero weight, its reparameterized right tensor is
    zero. If the target sector has zero weight, its left tensor is zero.
    Rephasing only rescales the neighboring operator, and hence preserves its
    vanishing.
\end{proof}
...
\begin{theorem}[Rephased inverse-map Case-I coefficient witness]
    ...
    \lean{MPOTensor.exists_rephased_inverseMap_activeSectorTraceMatrix_rank_one_coefficients_witnesses}
    \leanok
    \uses{def:mpdo, def:injective, def:is_sal,
      def:mpo_physical_trace_transfer, def:nt_cpsv}
