...
\begin{theorem}[Strong-area-law form of the normalized Case-I coefficients]
    ...
    Let ${\cal K}$ be an injective MPO tensor of positive bond dimension. Suppose
    ...
    with sector weights $p_k$, for which every neighboring operator is positive
    semidefinite, and there are real functions $a,b$ on
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This is the normal Case-I conclusion from
    \cite[Appendix~C.2, Case~I, lines~1374--1381]{Cirac2016MPDO_arXiv}:
    Lemma~C.4 supplies the active Hayashi factorization
    (lines~1406--1471), Lemma~C.5 gives the normalized coefficients
    (lines~1473--1499), and the corollary uses this rank-one input
    (lines~1501--1505). Literal idempotence is essential here; the statement
...
\begin{proof}
    \leanok
    \uses{thm:mpdo_casei_rank_one_coefficients}
    Choose the physical-sector factorization and the coherent sector phases
    supplied by the strong-area-law decomposition. Its neighboring operators
    are positive semidefinite, and its active one-site matrices, nonzero sector
    tensors, return condition, inactive sectors, and nonempty active sector
    satisfy the hypotheses of Theorem~\ref{thm:mpdo_casei_rank_one_coefficients}.
    Hence
    \begin{align}
        T^*_{k,h}&=a_kb_h,
        \notag\\
