...
\begin{lemma}[Indicator-path expansion of a matrix power]
    ...
    \leanok
    Let $S$ be a real square matrix indexed by a finite set $I$, and let
    $N\geq0$. Then, for $a,b\in I$,
    \begin{align}
...
\begin{theorem}[Trace of a matrix power as a sum over cyclic products]
    ...
    \lean{MPOTensor.trace_pow_eq_sum_cyclicProduct}
    \leanok
    \uses{lem:mpdo_pow_apply_path_indicator}
    Let $S$ be a real square matrix indexed by a finite set $I$, and
    let $N\geq1$. Then
    \begin{align}
        \tr\!\left(S^N\right)
