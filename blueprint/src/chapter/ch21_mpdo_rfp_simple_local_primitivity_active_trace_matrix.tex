...
\begin{definition}[Explicit $\eta$-data from the sector tensors]%
    ...
    Theorem~\ref{thm:mpdo_sector_adapted_decomposition}, with positive
    semidefiniteness assumed for the chosen representatives here.
...
\begin{theorem}[Existence of a positive factorization with full active span]
...
% thm:mpdo_sal_zcl_bnt_sector_structure_counterexample is the active block in
% the completed ambient counterexample
% thm:mpdo_sal_zcl_bnt_sector_structure, which refutes the source-context
% absorbed-representative assertion.
\begin{theorem}[A one-sector factorization of the four-sector tensor]
...
\begin{theorem}[Vanishing at a zero-weight incident sector]
    ...
    the chosen reparameterized and rephased factorization, not about the raw
...
\begin{theorem}[Existence of normalized Case-I rank-one coefficients]
...
\begin{proof}
    ...
    Its active trace matrix admits the normalized rank-one factorization, and
...
\begin{theorem}[All-sector normalized Case-I rank-one coefficients]
    ...
    \leanok
    \uses{def:mpdo, def:injective, def:is_sal, def:mpo_physical_trace_transfer, def:nt_cpsv}
    Let ${\cal K}$ be an injective normal MPO tensor satisfying saturation of
    the area law and literal physical-trace idempotence,
    \begin{align}
        \mathcal T_{\cal K}^2 & =\mathcal T_{\cal K}.
        \notag
    \end{align}
    ...
    $p_k=0$,
    then $a_k=b_k=0$. This realizes the all-index coefficient statement of
    \cite[Appendix~C.2, Lemma~C.5, lines~1473--1499]{Cirac2016MPDO_arXiv}
    for literal zero correlation length by extending the active-sector
    coefficients to inactive sectors by zero.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:mpdo_casei_rephased_inverse_map_coefficient_witness}
    Extend the active coefficient families of this factorization by zero.
    In the chosen zero-weight-reparameterized and rephased factorization, $p_k=0$ makes
...
\begin{theorem}[Rectangular form of the Case-I relation]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This expresses the operator identity corresponding to the $l_h,r_k$ factors
    in \cite[Appendix~C.2, Lemma~C.5, lines~1473--1499]{Cirac2016MPDO_arXiv}.
\end{theorem}
