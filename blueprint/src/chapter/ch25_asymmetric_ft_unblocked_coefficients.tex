...
weights, at every positive length and with no blocking. It does not assert
positivity of these weights. Normality means injectivity after some blocking,
as in Definition~\ref{def:normal}; the tensors need not be normalized.
Separation excludes gauge equivalence up to any nonzero complex scalar, not
only a unit-modulus phase.
