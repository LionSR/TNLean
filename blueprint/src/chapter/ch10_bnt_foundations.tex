...
\begin{proof}
    ...
    Theorem~\ref{thm:normal_proportional_mpv_geometric_phase} says that
    eventual nonzero proportionality of normalized normal representatives has
    a geometric phase and hence gives a gauge-phase equivalence. By
    Theorem~\ref{thm:cpsv_bnt_blocks_not_gaugePhaseEquiv}, distinct BNT
    representatives exclude this conclusion. Therefore, beyond every
    prescribed threshold, there is an $N$ for which no scalar $c_N$ gives the
    proportionality above. Choose such an $N>\max\{2,\ell\}$. The
    local-intersection equality gives exactly the forbidden proportionality,
