...
\begin{proof}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The injective uniqueness theorem, applied with the injective prefix
    $L_0$ and interaction range $L_0+1=\ell$, identifies these spaces for
    $N>\ell$ with the periodic MPS lines
    \begin{align}
    ...
    The normalized irreducible uniqueness theorem says conversely that two
    same-dimensional normalized representatives whose periodic MPS remain
    proportional at all sufficiently large lengths are gauge-phase equivalent.
    Equivalently, the BNT non-gauge-phase hypothesis supplies, beyond every
    prescribed threshold, an $N$ for which no scalar $c_N$ gives this
    proportionality. Choose such an $N>\max\{2,\ell\}$. The preceding
    parent-ground-space equality gives exactly the forbidden proportionality,
