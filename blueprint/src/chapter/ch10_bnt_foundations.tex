...
\begin{proof}
    \leanok
    \uses{def:bnt_cpsv, def:blockwise_product_word_span}
    Put $\ell=L_0+1$ and $S=3\ell$, and write
    $\mathcal G_n^q=\operatorname{ran}\Gamma_n^{A^q}$. Fix $k\ne j$.
    We first prove
    \begin{align}
        \mathcal G_S^k\cap\mathcal G_S^j&=\{0\}.
        \notag
    \end{align}
    Indeed, suppose that a nonzero vector belonged to both spaces and choose
    boundary matrices $X$ and $Y$ for its two trace representations. Then
    $X\ne0$ and the equality of the representations is the homogeneous
    three-block relation
    \begin{align}
        \tr(XA^k_{uvw})-\tr(YA^j_{uvw})&=0
        \qquad (|u|=|v|=|w|=\ell).
        \notag
    \end{align}
    The three-block factorization and length-$\ell$ block injectivity turn
    this relation into an inclusion of local spaces. After orienting the pair
    so that the first bond dimension is at least the second, it gives
    $\mathcal G_\ell^k\subseteq\mathcal G_\ell^j$. The full length-$\ell$
    word spans and the nondegeneracy of the matrix trace pairing make both
    maps $\Gamma_\ell^{A^q}$ injective. Consequently,
    \begin{align}
        D_k^2
        &=\dim\mathcal G_\ell^k
        \leq \dim\mathcal G_\ell^j
        =D_j^2.
        \notag
    \end{align}
    The chosen ordering forces equality, so $D_k=D_j$ and
    $\mathcal G_\ell^k=\mathcal G_\ell^j$. Using the injective
    length-$L_0$ boundary maps to close chains, injective uniqueness then makes
    the periodic matrix product vectors of the two normalized irreducible
    blocks proportional at arbitrarily large lengths. The BNT hypotheses exclude
    this: proportionality of normalized irreducible representatives forces a
    gauge-phase equivalence, whereas distinct same-dimensional representatives
    were assumed not to be gauge-phase equivalent. This contradiction proves
    the displayed trivial-intersection identity. If the bond dimensions are
    unequal, the same dimension inequality is already the contradiction.

    The full length-$S$ word spans similarly make each $\Gamma_S^{A^q}$
    injective. Hence
    \begin{align}
        ...
        \notag
    \end{align}
    is injective. All spaces are finite-dimensional, so its dual is
    surjective. Under the nondegenerate trace pairings, that dual sends word
...
\begin{proof}\leanok
    ...
    block injectivity at $p+1$ and at $3(p+1)$; equivalently, the corresponding
    homogeneous word spans are full, while the length-$p$ trace map is
    injective. Thus all hypotheses of the preceding
    Lemma~\ref{lem:bnt_direct_sum_injective_prefix_word_span} hold with
    $L_0=p$. Applying it gives the full simultaneous product-algebra span at
    the exact length
    \begin{align}
        L&=(p+1)+3(g-1)(p+1).
        \notag
    \end{align}
    Conjugation by the Perron gauges preserves this span, since it conjugates
    each component matrix by an invertible map. Finally, a one-letter
    evaluation of $A_j^{[L]}$ is precisely a length-$L$ word evaluation of
    $A_j$, which proves the asserted blocked one-letter span.
\end{proof}
