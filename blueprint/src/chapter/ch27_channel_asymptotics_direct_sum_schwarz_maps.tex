...
\begin{theorem}[Mutually inverse CPTP maps give a star equivalence]%
    ...
    \leanok
    \uses{def:kraus_map_between_direct_sums, lem:direct_sum_rectangular_trace_adjoint} Let $T:\mathcal A\to\mathcal B$ and $S:\mathcal B\to\mathcal A$ be
...
\begin{theorem}[Paired component maps for products of matrix algebras]
    ...
    \leanok
    \uses{thm:component_map_bijective}
...
\begin{proof}\leanok
    \uses{thm:component_map_bijective}
    Apply Theorem~\ref{thm:component_map_bijective} to the matched block
    ideals.
\end{proof}
...
\begin{proof}\leanok
    \uses{thm:unitary_skolem_noether_matrix, thm:corner_compression_partial_isometry_transport}
...
\begin{proof}\leanok
    \uses{thm:unitary_skolem_noether_matrix, thm:dim_preserved, thm:direct_sum_inverse_cptp_star_equiv}
...
\begin{proof}\leanok
    \uses{thm:direct_sum_inverse_cptp_star_equiv, thm:direct_sum_star_equiv_block_unitary, lem:direct_sum_block_unitary_trace_preserving, lem:direct_sum_trace_adjoint_star_equiv_inverse, lem:direct_sum_rectangular_trace_adjoint}
...
\begin{theorem}[Weighted block form of the adjoint Ces\`aro projection on full support]%
    ...
    \cite[Theorem~6.14]{Wolf2012Quantum}. The remaining projection step is
    documented in the companion completion record
...
\begin{theorem}[Density-block form of the Ces\`aro projection on full support]%
    ...
    asserted to be positive definite. The scope restriction and factor-order
    convention are documented in the companion completion record
    \cite{gap:wolf_theorem6_14_fixed_point_projection_gap}.
\end{theorem}
...
\begin{theorem}[Density-block form of fixed points on a finite direct sum]%
    ...
    and complementary zero summand of the general theorem remain open; the
    precise remaining step is documented in the companion completion record
    \cite{gap:wolf_theorem6_14_fixed_point_projection_gap}.
\end{theorem}
