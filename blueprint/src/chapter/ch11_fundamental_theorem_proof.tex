...
\begin{proof}\leanok
	The self-overlap is the conjugate of the Lean inner product of the MPV state
	with itself.  Hence its limit gives convergence of the squared state norm to
...
\label{lem:proportional_tensor_from_blocks_scalar_norm}
	...
	Suppose the weighted state sums for the two assembled BNT tensors are
...
\begin{lemma}[Normalized proportional weighted projections]%
	...
	\leanok
	If the weighted projections of two assembled BNT tensors are related by a
	scalar sequence \(c_N\), then after division by nonzero scalars
	\(\mu^N\) and \(\nu^N\) the normalized projections are related by the
	adjusted scalar \(c_N(\nu/\mu)^N\).
\end{lemma}
...
\label{lem:proportional_tensor_from_blocks_adjusted_weighted_inner_sequence}
	...
	For proportional assembled BNT tensors and any nonzero scalars
	\(\mu,\nu\in\C\), one may choose a nonzero scalar sequence \(c_N\)
	such that the normalized weighted projections are related by the adjusted
	scalar \(c_N(\nu/\mu)^N\).
\end{lemma}
...
\label{thm:proportional_bnt_nondecaying_overlap}
	...
	selected one-copy BNT representatives. Suppose that for every system size
	the two MPV families are proportional by a nonzero scalar depending on the
	size. Then every block \(A_j\) has a block \(B_k\) such that
