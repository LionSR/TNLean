...
\begin{definition}[Flattened block-diagonal gauge]%
    ...
    $\{0,\dots,\sum_k D_k-1\}$ of the assembled tensor. The result is an
...
\begin{proof}\leanok
    ...
    \[
        ...
        =\mathbb 1.
    \]
    Reindexing the dependent block coordinates by the flattened bond index
    preserves multiplication and adjoints.
\end{proof}
...
\begin{lemma}[Same-phase unitary assembly in matched sector coordinates]
    ...
    \[
        Q_k^i=\zeta_k U_kP_{\beta(k)}^iU_k^\dagger,
        \qquad
        \nu_{k,q}=\zeta_k^{-1}\mu_{\beta(k),\tau_k(q)},
        \qquad |\zeta_k|=1.
    \]
...
\begin{proof}\leanok
    ...
    Repeating $U_k$ on every copy gives
    $X=\bigoplus_{k,q}U_k$, which is unitary by
    Lemma~\ref{lem:unitary_global_gauge_of_blocks}.  The phase cancellation in
    the two displayed sector identities gives the matched-coordinate
    conjugation formula.
\end{proof}
...
\begin{corollary}[Canonical Form II unitary refinement in the equal case]%
    ...
    \leanok
    \uses{thm:sector_bnt_equal_global_gauge,
        lem:sector_bnt_equal_unitary_matched_gauge}
    Let $P$ and $Q$ be BNT canonical forms whose total tensors generate the
    ...
    \[
        Q_{\mathrm{tot}}^i=U P_{\mathrm{tot}}^i U^\dagger.
    \]
...
\begin{proof}\leanok
    ...
    \[
        ...
        \nu_{k,q}=\zeta_k^{-1}\mu_{\beta(k),\tau_k(q)}.
    \]
    Thus the phase cancels copy by copy.  Repeating $U_k$ on its matched copies
    gives
    \[
        X=\bigoplus_{k,q}U_k,
        \qquad X^\dagger X=\mathbb 1.
    \]
    If $\Pi$ is the permutation matrix restoring the original sector and copy
    order, then $\Pi^\dagger\Pi=\mathbb 1$.  Hence
    \[
        U=X\Pi,
        \qquad U^\dagger U=\mathbb 1,
    \]
    and conjugation by $U$ carries $P_{\mathrm{tot}}^i$ to
    $Q_{\mathrm{tot}}^i$.
\end{proof}
...
\begin{lemma}[Phase normalization for an irreducible gauge]
    ...
    \[
        B^i=\zeta X A^iX^{-1}
    \]
    for every $i$, then $|\zeta|=1$.
\end{lemma}
\begin{proof}\leanok
    Transporting a nonzero positive fixed point through $X$ gives a nonzero
    positive matrix $\sigma$ satisfying
    \[
        \mathcal E_B(\sigma)=|\zeta|^2\sigma.
    \]
    The map $\mathcal E_B$ also has a nonzero positive fixed point.  Its
    irreducibility therefore forces $|\zeta|^2=1$.
\end{proof}
...
\begin{lemma}[Unitary replacement with the same phase]
    ...
    \leanok
    \uses{def:left_canonical_tensor, def:irreducible_tensor,
        lem:left_canonical_irreducible_phase_norm}
    Let $A$ and $B$ be left-canonical irreducible tensors of common positive
    bond dimension satisfying
    \[
        B^i=\zeta X A^iX^{-1}
    \]
    for an invertible $X$ and a nonzero scalar $\zeta$.  There is a unitary
    matrix $U$, with the same scalar $\zeta$, such that $|\zeta|=1$ and
    \[
...
\end{lemma}
\begin{proof}\leanok
    \uses{lem:left_canonical_irreducible_phase_norm}
    Lemma~\ref{lem:left_canonical_irreducible_phase_norm} gives
    $|\zeta|=1$.  Substitution into the left-canonical identities shows that
    $X^\dagger X$ is a positive-semidefinite fixed point of the adjoint transfer
    map of $A$.  Irreducibility gives $X^\dagger X=c\mathbb 1$ for some $c>0$;
    hence $U=c^{-1/2}X$ is unitary and implements the same conjugation.
\end{proof}
...
\begin{proof}\leanok
    ...
    $B^i=\zeta X A^iX^{-1}$ with $X$ invertible and $\zeta\ne0$.
...
\begin{proof}\leanok
    ...
    \[
        Q_k^i=\zeta_k X_k P_{\beta(k)}^iX_k^{-1}.
    \]
    The matched tensors are left-canonical and irreducible. Hence
    Lemma~\ref{lem:left_canonical_irreducible_unitary_gauge} gives unitary
    matrices $U_k$ satisfying
    \[
        X_k P_{\beta(k)}^iX_k^{-1}
        =U_kP_{\beta(k)}^iU_k^\dagger,
    \]
    and therefore
    \[
        Q_k^i=\zeta_k U_kP_{\beta(k)}^iU_k^\dagger.
