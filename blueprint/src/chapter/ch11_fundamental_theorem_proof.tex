...
The restricted heterogeneous equal-case lemma, stated as
...
the block-diagonal MPV families.  The former common-structure theorem is not
recorded as a formalization of \cite[Corollary~II.2]{Cirac2016MPDO_arXiv}: it
assumed common weights, common block count, and common bond dimensions, while
the source theorem derives the matching data.  Theorem~II.1 still passes
through sector decompositions, copy-count recovery, and sector-weight multiset
comparison before one obtains a global block-diagonal gauge.
...
\begin{lemma}[Separated normal canonical-form hypotheses produce a common phase cover]%
	...
	distinct equal-dimension blocks, a proportional-decomposition witness
	(per-length coefficient arrays, a per-length nonzero proportionality
	scalar, unit-modulus dominant blocks on each side, uniform sub-dominant
	coefficient bounds, and the two BNT decomposition identities) gives a
	common MPV phase cover of the two basis families.
		This is the BNT construction of the common cover; it does not assume equality of
		finite-length spans.
	\end{lemma}

% The proof currently depends on the pending nonzero-overlap argument recorded in
% docs/paper-gaps/cpsv16_cf_normalization_and_proportional_comparison.tex and
% tracked in issue #1559.
\begin{proof}
	\uses{lem:common_phase_cover_of_proportional_decomposition,
...
\label{def:common_primitive_bnt_cover_hypotheses}
	\lean{MPSTensor.CommonPrimitiveBNTCoverHypotheses,
		...
		MPSTensor.tendsto_blockPowerCoeff_of_tendsto_pow,
		MPSTensor.IsNormalCanonicalFormBNT.blockTensor_of_notGpe,
	...
	families.  The construction from equal MPVs is formal in the assembled
	block-diagonal tensors and uses the correct expansion coefficients \(\mu_j^N\), so the convergence
	conditions and nonzero-limit hypotheses for those coefficient families remain explicit inputs.
...
\begin{lemma}[Physical relabelling of common primitive BNT hypotheses]%
	...
	equality of the residual all-zero block dimensions are preserved.  Applying
	this to the canonical bijection
...
\label{def:common_representative_bnt_cover_hypotheses}
	...
	BNT separation (no gauge-phase-equivalent distinct representatives), unit
	modulus of the dominant representative weight on each side as in the source
	normalization after \(\eqref{eq:II_CF1}\), the length-zero identity of
	the representative nonzero parts, one-site
	injectivity of each representative block, proportional-decomposition data
	linking the two representative families, and the resulting BNT comparison
	conclusion: a permutation of representatives, equality of the matched bond
	dimensions, and gauge-phase equivalences for the matched blocks.
\end{definition}
...
\label{thm:ofCommonRepresentatives_zeroTailIdentity}
	...
	Given representative common-sector families, if the representative weights
	are strictly ordered by norm, distinct representatives are BNT-separated,
	the dominant representative weight on each side has unit modulus as in the
	source normalization after \(\eqref{eq:II_CF1}\), the length-zero identity
	holds for the representative nonzero parts, each
	representative block is one-site injective, proportional-decomposition data
	links the two representative families, and the BNT comparison conclusion
	supplies a permutation, matched bond dimensions, and gauge-phase equivalences,
	then the length-zero identity for the residual all-zero block and the
	normal-CF-BNT conditions produce BNT matching hypotheses for the
...
\label{thm:ofCommonRepresentativeBNTCoverHypotheses}
	...
	to phase, the dominant representative weight on each side has unit modulus,
	the length-zero identity for the representative nonzero parts holds, and each
	representative block is one-site injective.  Assume also the
