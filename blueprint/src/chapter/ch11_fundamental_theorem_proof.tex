...
\begin{proof}\leanok
    ...
    (Lemma~\ref{thm:ft_proportional_nt}).
...
\begin{theorem}[Proportional comparison for restricted CF-BNT hypotheses]\label{thm:ft_proportional}
...
\begin{remark}[Coefficient arrays for canonical-form decompositions]
	...
	Lemma~\ref{thm:ft_proportional} come from the block-diagonal
	...
	Lemma~\ref{thm:ft_proportional} keeps these arrays explicit because
	Theorem~\ref{thm:bnt_perm_prop} is formulated in that abstract form.
\end{remark}

\begin{theorem}[Proportional comparison for restricted normal-CF-BNT hypotheses]\label{thm:ft_proportional_nt}
...
\begin{theorem}[Common-block equal-MPV comparison]\label{thm:ft_equal_same_structure}
...
\begin{lemma}[Equal MPVs imply proportional MPVs]\label{lem:sameMPV2_implies_proportionalMPV2}
	\lean{MPSTensor.sameMPV₂_implies_proportionalMPV₂}
	\leanok
	\uses{def:same_mpv2, def:proportional_mpv2}
	If two MPS tensors have the same MPV family, then they have proportional
	MPV families with proportionality constant~\(1\).  This trivial observation
	reduces the equal case of Corollary~II.2 to the proportional
	Fundamental Theorem~\ref{thm:ft_proportional}.
\end{lemma}
...
\begin{theorem}[Equal-MPV fundamental theorem (global gauge equivalence)]%
	\label{thm:ft_equalMPV_full}
	\lean{MPSTensor.fundamentalTheorem_equalMPV_full}
	\leanok
	\uses{thm:ft_proportional, lem:sameMPV2_implies_proportionalMPV2,
		def:cf_bnt, def:gauge_equiv}
	Under the same coefficient hypotheses as
	Theorem~\ref{thm:ft_proportional}, if the two MPV families of the assembled
	block-diagonal tensors are equal (not just proportional), then the
	phase-corrected weights match blockwise.  After reindexing the \(B\)-family
	by the permutation from the proportional theorem, the assembled weighted
...
\begin{remark}[Equal-case comparison after canonical reduction]\notready
	...
	% latter part of the argument.  Lemma~\ref{thm:ft_equal_same_structure}
