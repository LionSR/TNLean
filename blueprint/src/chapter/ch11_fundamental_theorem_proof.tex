...
\begin{proof}\leanok
    ...
    Lemma~\ref{lem:unitary_global_gauge_of_blocks}.  For every matched copy
    $(k,q)$,
    \[
        \nu_{k,q}Q_k^i
        =\zeta_k^{-1}\mu_{\beta(k),\tau_k(q)}\,
...
\begin{proof}\leanok
    ...
    \[
        Q_{\mathrm{tot}}^i=UP_{\mathrm{tot}}^iU^\dagger.
    \]
\end{proof}
...
\begin{proof}\leanok
    ...
    $X^\dagger X=c\mathbb 1$ for some $c>0$, so
    $X^{-1}=c^{-1}X^\dagger$ and $U=c^{-1/2}X$ is unitary. Moreover,
    \[
        UA^iU^\dagger
        =c^{-1}XA^iX^\dagger
        =XA^iX^{-1}.
