...
\begin{lemma}[Selected coefficient extraction from a residual-span exclusion]%
\label{lem:selected_coefficient_from_residual_span_sum}
	\lean{MPSTensor.eventually_selected_coefficient_eq_of_eventually_notMem_residual_span_sum}
	\leanok
	\uses{lem:eventual_selected_coefficient_from_residual_span}
	Let \(v_i^{(N)}\) and \(w_k^{(N)}\) be two finite families, and fix
	\(i_0\).  Suppose that, for all sufficiently large \(N\),
	\[
		\sum_i a_i^{(N)} v_i^{(N)}
		=
		b_0^{(N)} v_{i_0}^{(N)}
		+\sum_k b_k^{(N)} w_k^{(N)}.
	\]
	If \(v_{i_0}^{(N)}\) is not in the span of the residual family consisting
	of the vectors \(v_i^{(N)}\) with \(i\ne i_0\) together with all
	\(w_k^{(N)}\), then \(a_{i_0}^{(N)}=b_0^{(N)}\) for all sufficiently large
	\(N\).  This is the finite-sum form of the residual-span coefficient
	extraction needed after the BNT separation step
	\cite[Theorem~II.1, line~1182]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
	Split the left-hand sum into the selected term and the remaining
	\(v_i^{(N)}\)-terms.  Both residual sums lie in the span of the displayed
	residual family, so Lemma~\ref{lem:eventual_selected_coefficient_from_residual_span}
	applies.
\end{proof}

\begin{lemma}[Selected coefficient extraction from selected-plus-residual independence]%
\label{lem:selected_coefficient_from_residual_independence_sum}
	\lean{MPSTensor.eventually_selected_coefficient_eq_of_eventually_linearIndependent_residual_sum}
	\leanok
	\uses{lem:selected_coefficient_from_residual_span_sum,
		lem:eventual_selected_not_in_residual_span_from_li}
	Let \(v_i^{(N)}\) and \(w_k^{(N)}\) be two finite families, and fix
	\(i_0\).  Suppose that, for all sufficiently large \(N\),
	\[
		\sum_i a_i^{(N)} v_i^{(N)}
		...
		+\sum_k b_k^{(N)} w_k^{(N)}.
	\]
	If the family consisting of \(v_{i_0}^{(N)}\) together with the residual
	vectors \(v_i^{(N)}\), \(i\ne i_0\), and all \(w_k^{(N)}\) is eventually
	linearly independent, then \(a_{i_0}^{(N)}=b_0^{(N)}\) for all
	sufficiently large \(N\).  This is the coefficient-extraction form used
	after the selected block has been separated from the residual family
	\cite[Theorem~II.1, line~1182]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
	Eventual selected-plus-residual linear independence first gives the
	selected vector outside the residual span.  Then apply
	Lemma~\ref{lem:selected_coefficient_from_residual_span_sum}.
\end{proof}
...
\begin{lemma}[Selected weighted summand from phase substitution and a residual-span exclusion]%
\label{lem:selected_weighted_summand_from_phase_sum_residual_span}
	\lean{MPSTensor.eventually_selected_weighted_mpvState_eq_smul_of_phase_sum_and_notMem_residual_span}
	\leanok
	\uses{lem:selected_coefficient_from_residual_span_sum,
		lem:selected_weighted_summand_from_phase_coeff}
	Suppose a total weighted-state identity has the form
	...
	for all sufficiently large \(N\).  If
	\(\ket{V^{(N)}(B_0)}=\zeta^N\ket{V^{(N)}(A_{i_0})}\) for every \(N\), and
	\(\ket{V^{(N)}(A_{i_0})}\) is eventually outside the span of the remaining
	\(A_i\)-states together with the residual \(B_k\)-states, then
	\[
		\mu_{i_0}^N\ket{V^{(N)}(A_{i_0})}
		=
		c_N\nu_0^N\ket{V^{(N)}(B_0)}
	\]
	for all sufficiently large \(N\).  This is the residual-span version of
	the selected-summand bridge used in the fixed-block cancellation step
	\cite[Theorem~II.1, lines~1181--1183]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
	Substitute the phase relation for \(B_0\), apply
	Lemma~\ref{lem:selected_coefficient_from_residual_span_sum}, and then use
	Lemma~\ref{lem:selected_weighted_summand_from_phase_coeff}.
\end{proof}

\begin{lemma}[Selected weighted summand from phase substitution and residual independence]%
\label{lem:selected_weighted_summand_from_phase_sum_residual_independence}
	\lean{MPSTensor.eventually_selected_weighted_mpvState_eq_smul_of_phase_sum_and_li_residual}
	\leanok
	\uses{lem:selected_coefficient_from_residual_independence_sum,
		lem:selected_weighted_summand_from_phase_coeff}
	Suppose a total weighted-state identity has the form
	\[
		\sum_i \mu_i^N \ket{V^{(N)}(A_i)}
	...
	for all sufficiently large \(N\).  If
	\(\ket{V^{(N)}(B_0)}=\zeta^N\ket{V^{(N)}(A_{i_0})}\) for every \(N\), and
	the family consisting of \(\ket{V^{(N)}(A_{i_0})}\) together with the
	remaining \(A_i\)-states and the residual \(B_k\)-states is eventually
	...
	\[
		...
		c_N\nu_0^N\ket{V^{(N)}(B_0)}
	\]
	for all sufficiently large \(N\).  This is the selected-plus-residual
	independence form of the fixed-block selected-summand bridge
	\cite[Theorem~II.1, lines~1181--1183]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
	Substitute the phase relation for \(B_0\), apply
	Lemma~\ref{lem:selected_coefficient_from_residual_independence_sum}, and
	then use Lemma~\ref{lem:selected_weighted_summand_from_phase_coeff}.
\end{proof}
...
\begin{lemma}[Eventual tail proportionality from phase substitution and a residual-span exclusion]%
\label{lem:eventual_tail_proportional_from_phase_sum_residual_span}
	\lean{MPSTensor.eventuallyNonzeroProportionalMPV₂_tail_succAbove_of_phase_sum_notMem_residual_span}
	\leanok
	\uses{lem:selected_weighted_summand_from_phase_sum_residual_span,
		lem:eventual_proportional_weighted_tail_succAbove_package}
	Suppose the total weighted state sums are eventually related by a nonzero
	scalar sequence.  If a selected pair satisfies
	$\ket{V^{(N)}(B_{b_0})}=\zeta^N\ket{V^{(N)}(A_{a_0})}$ for every $N$,
	and \(\ket{V^{(N)}(A_{a_0})}\) is eventually outside the span of the
	remaining \(A\)-block states together with the remaining \(B\)-block
	states, then the two complements, reindexed by the insertion maps, are
	eventually nonzero proportional.  This is the residual-span version of the
	conditional algebraic peeling statement used after the fixed-block
	application of Lemma~\texttt{Lem1}
	\cite[Theorem~II.1, lines~1181--1185]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
	Rewrite the \(B\)-side weighted sum as the selected summand plus its
	complement.  The residual-span selected-summand lemma gives the selected
	weighted summand identity.  Subtract that identity from the total relation
	and reindex the two complements.
\end{proof}

\begin{lemma}[Eventual tail proportionality from phase substitution and residual independence]%
\label{lem:eventual_tail_proportional_from_phase_sum_residual_independence}
	\lean{MPSTensor.eventuallyNonzeroProportionalMPV₂_tail_succAbove_of_phase_sum_li_residual}
	\leanok
	\uses{lem:selected_weighted_summand_from_phase_sum_residual_independence,
		lem:eventual_tail_proportional_from_phase_sum_residual_span}
	Suppose the total weighted state sums are eventually related by a nonzero
	scalar sequence.  If a selected pair satisfies
	$\ket{V^{(N)}(B_{b_0})}=\zeta^N\ket{V^{(N)}(A_{a_0})}$ for every \(N\),
	and the family consisting of \(\ket{V^{(N)}(A_{a_0})}\) together with the
	remaining \(A\)-block states and the remaining \(B\)-block states is
	eventually linearly independent, then the two complements, reindexed by the
	insertion maps, are eventually nonzero proportional.  This is the
	selected-plus-residual independence form of the conditional peeling step
	\cite[Theorem~II.1, lines~1181--1185]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
	The selected-plus-residual independence gives the selected state outside
	the residual span.  Apply the residual-span tail-peeling lemma.
\end{proof}
...
\begin{lemma}[Eventual tail proportionality from the left residual-span exclusion]%
\label{lem:eventual_tail_proportional_from_phase_sum_residual_span_left}
	\lean{MPSTensor.eventuallyNonzeroProportionalMPV₂_tail_succAbove_of_phase_sum_notMem_residual_span_left}
	\leanok
	\uses{lem:selected_coefficient_from_residual_span_sum,
		lem:eventual_proportional_weighted_tail_succAbove_package}
	Suppose the total weighted state sums are eventually related by a nonzero
	scalar sequence, and suppose
	$\ket{V^{(N)}(B_{b_0})}=\zeta^N\ket{V^{(N)}(A_{a_0})}$ for every \(N\),
	with \(\zeta\ne0\).  If \(\ket{V^{(N)}(B_{b_0})}\) is eventually outside
	the span of the remaining \(B\)-block states together with the remaining
	\(A\)-block states, then the two complements, reindexed by the insertion
	maps, are eventually nonzero proportional.  This is the left-substituted
	residual-span form of the conditional peeling step
	\cite[Theorem~II.1, lines~1181--1185]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
	Use \(\zeta\ne0\) to rewrite the selected \(A\)-summand in terms of
	\(B_{b_0}\).  Residual-span coefficient extraction identifies the selected
	summands, and the tail-subtraction lemma gives eventual proportionality of
	the two complements.
\end{proof}

\begin{lemma}[Eventual tail proportionality from the left residual independence input]%
\label{lem:eventual_tail_proportional_from_phase_sum_residual_independence_left}
	\lean{MPSTensor.eventuallyNonzeroProportionalMPV₂_tail_succAbove_of_phase_sum_li_residual_left}
	\leanok
	\uses{lem:eventual_tail_proportional_from_phase_sum_residual_span_left}
	Suppose the total weighted state sums are eventually related by a nonzero
	scalar sequence, and suppose
	\(\ket{V^{(N)}(B_{b_0})}=\zeta^N\ket{V^{(N)}(A_{a_0})}\) for every \(N\),
	with \(\zeta\ne0\).  If the family consisting of
	\(\ket{V^{(N)}(B_{b_0})}\) together with the remaining \(B\)-block states
	and the remaining \(A\)-block states is eventually linearly independent,
	then the two complements, reindexed by the insertion maps, are eventually
	nonzero proportional.  This is the left-substituted selected-plus-residual
	independence form of the conditional peeling step
	\cite[Theorem~II.1, lines~1181--1185]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
	The selected-plus-residual independence gives the selected \(B\)-state
	outside the residual span.  Apply the left residual-span tail-peeling
	lemma.
\end{proof}
...
\begin{lemma}[Dominant-weight normalized BNT projections concentrate]%
\label{lem:dominant_weight_normalized_bnt_projection}
	\lean{MPSTensor.tendsto_normalized_weighted_mpvInner_sum_of_dominant}
	\lean{MPSTensor.tendsto_normalized_weighted_mpvInner_sum_of_leading_CFBNT}
	\leanok
	\uses{def:mpv_inner}
	Let \(A_j\) be a finite BNT family.  Fix a dominant weight
	\(\mu_{j_0}\ne 0\), and assume that every other weight ratio satisfies
	\(|\mu_j/\mu_{j_0}|<1\).  If the selected self-projection tends to \(1\)
	and all off-diagonal projections from the selected block tend to \(0\),
	then
	\[
		\mu_{j_0}^{-N}
		\sum_j \mu_j^N
		\langle V^{(N)}(A_{j_0}),V^{(N)}(A_j)\rangle
		\longrightarrow 1 .
	\]
	In particular, this holds for the leading block of a one-copy BNT
	canonical-form family.  \cite[Theorem~\texttt{thm1},
	lines~1170--1192]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
	After division by the dominant weight, the selected term has coefficient
	one.  The selected self-projection tends to \(1\), and every other term is
	a geometrically damped off-diagonal projection.  Summing these limits gives
	the asserted concentration.  In the BNT-leading case, strict ordering of
	the weight moduli supplies the geometric damping.
\end{proof}

\begin{lemma}[Phase-normalized dominant projections concentrate]%
\label{lem:phase_normalized_dominant_bnt_projection}
	\lean{MPSTensor.tendsto_phase_normalized_weighted_mpvInner_sum_of_dominant}
	\lean{MPSTensor.tendsto_phase_normalized_weighted_mpvInner_sum_of_leading_CFBNT}
	\leanok
	\uses{def:mpv_inner}
	Let \(A_0\) be an MPV block and let \(B_k\) be a finite family with
	distinguished block \(B_{k_0}\).  Suppose that \(|\zeta|=1\) and that, for
	every \(N\),
	\[
		\ket{V^{(N)}(B_{k_0})}
		=
		\zeta^N \ket{V^{(N)}(A_0)} .
	\]
	If every remaining weight ratio satisfies \(|\mu_k/\mu_{k_0}|<1\),
	the self-projection of \(A_0\) tends to \(1\), and the projections
	\(\langle V^{(N)}(A_0),V^{(N)}(B_k)\rangle\) tend to \(0\) for
	\(k\ne k_0\), then
	\[
		(\mu_{k_0}\zeta)^{-N}
		\sum_k \mu_k^N
		\langle V^{(N)}(A_0),V^{(N)}(B_k)\rangle
		\longrightarrow 1 .
	\]
	In the leading one-copy BNT case, assume also that both the \(A\)-family and
	the \(B\)-family are in BNT canonical form and take \(A_0\) and \(B_{k_0}\)
	to be their leading blocks.  The \(A\)-side BNT normalization supplies the
	self-projection limit, while \(B\)-side BNT separation supplies the
	off-diagonal decay and strict ordering supplies the geometric damping.
	\cite[Theorem~\texttt{thm1}, lines~1170--1192]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
	After division by \((\mu_{k_0}\zeta)^N\), the selected term is the
	self-projection of \(A_0\).  Each remaining term is a decaying projection
	multiplied by a geometrically damped coefficient.  The BNT-leading
	specialization obtains the decay from block separation, using the phase
	relation to transfer orthogonality from \(B_{k_0}\) to \(A_0\).
\end{proof}
...
\begin{lemma}[Dominant phase-adjusted proportional scalar converges to one]%
\label{lem:dominant_phase_adjusted_scalar_convergence}
	\lean{MPSTensor.exists_dominant_phase_adjusted_scalar_tendsto_one_of_eventuallyNonzeroProportionalMPV₂_CFBNT}
	\leanok
	\uses{lem:eventual_proportional_tensor_from_blocks_weighted_state_sequence,
		lem:eventual_proportional_tensor_from_blocks_weighted_inner_sequence,
		lem:dominant_weight_normalized_bnt_projection,
		lem:phase_normalized_dominant_bnt_projection}
	Let \((\mu^A_j,A_j)\) and \((\mu^B_k,B_k)\) be one-copy BNT
	canonical-form families with nonempty index sets, and suppose that the
	...
	\[
		c_N\left(\frac{\mu^B_{b_0}\zeta}{\mu^A_{a_0}}\right)^N
		\longrightarrow 1 .
	\]
	This is only the leading projected scalar convergence; it is not the exact
	selected-summand equality needed for tail subtraction.
	\cite[lines~1170--1192]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
	Project the eventual weighted-state proportionality against the leading
	\(A\)-block and divide by \((\mu^A_{a_0})^N\).  The \(A\)-side normalized
	projection tends to \(1\), while the phase relation makes the
	\(B\)-side projection normalized by \((\mu^B_{b_0}\zeta)^N\) tend to
	\(1\).  Dividing the two limiting identities gives the stated convergence
...
\begin{lemma}[Dominant tail difference vanishes after phase adjustment]%
\label{lem:dominant_phase_adjusted_tail_difference}
	\lean{MPSTensor.exists_dominant_tail_diff_tendsto_zero_of_eventuallyNonzeroProportionalMPV₂_CFBNT}
	\leanok
	\uses{lem:dominant_phase_adjusted_selected_difference}
	Let \((\mu^A_j,A_j)\) and \((\mu^B_k,B_k)\) be one-copy BNT
	canonical-form families with nonempty index sets, and suppose that the
	assembled weighted MPV families are eventually nonzero proportional.  If
	for every \(N\), the leading blocks satisfy
	...
	\[
		\left\|(\mu^A_{a_0})^{-N}
		\Bigg(
		\sum_{j\neq a_0}(\mu^A_j)^N\ket{V^{(N)}(A_j)}
		-
		c_N\sum_{k\neq b_0}(\mu^B_k)^N\ket{V^{(N)}(B_k)}
		\Bigg)\right\|
		\longrightarrow 0 .
	\]
	\cite[lines~1170--1192]{Cirac2016MPDO_arXiv}.
	% This is an asymptotic erased-tail statement; it does not give exact
	% proportionality of the two erased tails.
\end{lemma}

\begin{proof}\leanok
	Split each assembled weighted sum into its leading selected summand and the
	erased tail.  The total proportionality identity and
	Lemma~\ref{lem:dominant_phase_adjusted_selected_difference} show that the
	tail difference is the negative of the selected difference, after the same
	normalization.  Norms are unchanged by multiplication by \(-1\).
\end{proof}
...
\begin{lemma}[Eventual adjusted scalar and normalized projections]%
\label{lem:eventual_adjusted_scalar_norm_and_projection_sequence}
	\lean{MPSTensor.exists_adjusted_scalar_norm_and_inner_sequence_of_eventuallyNonzeroProportionalMPV₂}
	\leanok
	\uses{lem:eventual_proportional_tensor_from_blocks_weighted_state_sequence,
		lem:eventual_proportional_tensor_from_blocks_adjusted_scalar_norm,
		lem:eventual_proportional_tensor_from_blocks_weighted_inner_sequence}
	Suppose the assembled weighted block tensors are eventually proportional by
	nonzero scalars.  Fix nonzero scalars $\mu,\nu$, and assume that after
	division by $\mu^N$ and $\nu^N$ the two weighted state sums both have
	norm tending to $1$.  Then one may choose a single scalar sequence $c_N$,
	nonzero for all sufficiently large $N$, such that
	\[
		\left|c_N(\nu/\mu)^N\right|\longrightarrow 1
	\]
	and, for every tensor $X$, the normalized weighted projections against
	$\ket{V^{(N)}(X)}$ are eventually related by the same adjusted scalar
	$c_N(\nu/\mu)^N$.  This statement asserts only the scalar-normalization
	and projection consequences, not a fixed-block coefficient identity.
	\cite[lines~1170--1192]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
	Choose the scalar sequence from eventual proportionality of the weighted
	state sums.  The normalized state-norm hypotheses give the adjusted scalar
	modulus limit.  Projecting the same weighted-state identity against
	$\ket{V^{(N)}(X)}$ and dividing by $\mu^N$ and $\nu^N$ gives the
	normalized projection identity with that same scalar sequence.
\end{proof}
...
\label{lem:fixed_right_all_overlaps_decay_contradiction_fin_one}
	...
	\leanok
	\uses{def:cf_bnt, lem:fixed_right_lem1_input_eventual_proportional,
		lem:eventual_coeff_eq_of_eventual_li}
	If \(A\) and \(B\) are BNT canonical-form families and the \(B\)-family
	consists only of the fixed block \(B_0\), then the fixed-right
	all-overlaps-decay alternative contradicts eventual nonzero proportionality
	of the total weighted MPV families.  This is the no-tail case of the
	fixed-block cancellation step in
	\cite[Theorem~II.1, line~1182]{Cirac2016MPDO_arXiv}.
\end{lemma}
...
\label{lem:fixed_left_all_overlaps_decay_contradiction_fin_one}
	...
	\leanok
	\uses{def:cf_bnt, lem:fixed_left_lem1_input_eventual_proportional,
		lem:eventual_coeff_eq_of_eventual_li}
	If \(A\) and \(B\) are BNT canonical-form families and the \(A\)-family
	consists only of the fixed block \(A_0\), then the fixed-left
	all-overlaps-decay alternative contradicts eventual nonzero proportionality
	of the total weighted MPV families.  This is the no-tail case of the
	symmetric fixed-block cancellation step in
	\cite[Theorem~II.1, lines~1182--1185]{Cirac2016MPDO_arXiv}.
\end{lemma}
