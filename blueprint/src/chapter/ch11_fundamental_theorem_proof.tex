...
\begin{proof}\leanok
    ...
    Write $B^i=\zeta X A^iX^{-1}$.  Left canonicity implies that
    $X^{\dagger}X$ is a positive-semidefinite fixed point of the adjoint
    transfer map of $A$.  By irreducibility and uniqueness of such fixed
    points, $X^{\dagger}X=c\mathbb 1$ for some $c>0$.  Hence
    $U=c^{-1/2}X$ is unitary and
    $X A^iX^{-1}=U A^iU^{\dagger}$.  The irreducible normalization gives
    $|\zeta|=1$.
\end{proof}
...
\begin{corollary}[Canonical Form II unitary refinement of proportional matching]%
    ...
    Theorem~\ref{thm:cpgsv_multiblock_ft_source}, the matrices $Y_k$ may be
    chosen unitary.  Thus there are unitary matrices
    $U_k\in\mathrm{U}(\dim Q_k)$ and phases $\zeta_k\in\C$, with
    $|\zeta_k|=1$, such that, for every $k$ and every physical index~$i$,
    \[
        Q_k^i
        =
...
\begin{proof}\leanok
    ...
    The proportional sector-matching theorem gives an invertible gauge and a
    unit phase for each matched pair.  Both tensors in such a pair are
    left-canonical and irreducible.
    Lemma~\ref{lem:left_canonical_irreducible_unitary_gauge} therefore replaces
    the invertible gauge by a unitary matrix.
\end{proof}
