...
\begin{theorem}[Multi-block fundamental theorem for normal tensors]%
    ...
    be MPS tensors generating the same MPV family at every length, after
    canonical-form reduction into normal-tensor blocks. Write the block
    families as $(A_k)_{k=1}^{r}$ and $(B_\ell)_{\ell=1}^{s}$. Then
    $r=s$ after relabelling, and there are a permutation $\pi$ of
    $\{1,\ldots,r\}$, scalars $\zeta_k \in \C$ with $|\zeta_k|=1$, and
    invertible matrices $Y_k \in \GL_{D_k}(\C)$ such that, after identifying
    the matched bond dimensions, for every $k$ and every physical index $i$,
    \[
        B_k^i
        =
        \zeta_k\,Y_k\,A_{\pi(k)}^i\,Y_k^{-1}.
    \]
    In particular, the multisets of block dimensions and gauge-phase classes
    coincide.
\end{theorem}
...
\begin{theorem}[Equal canonical-form representations have matched gauge phases]%
    ...
    canonical-form representations of the same MPV family, then their block
    dimensions agree after a permutation $\pi$ of the block index set. For
    every $k$ and every physical index $i$, the matched normal-tensor
    ...
    \[
        ...
        \zeta_k\,Y_k\,A_{\pi(k)}^i\,Y_k^{-1},
    \]
    for some scalar $\zeta_k \in \C$ with $|\zeta_k|=1$ and some invertible
    matrix $Y_k \in \GL_{D_k}(\C)$.
\end{theorem}
