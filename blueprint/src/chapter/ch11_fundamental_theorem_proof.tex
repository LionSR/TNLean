...
\begin{proof}\leanok
    ...
    \[
        ...
        -
        \sum_{k}\Bigl(a^{(N)}_k-c_N\,c^{(k)}_N(Q)\Bigr)\,V^{(N)}(Q_k)
    \]
...
\cite[Theorem~4.5 and Corollary~IV.5]{Cirac2021Matrix}. We state it on the BNT
canonical-form surface (Definition~\ref{def:sector_bnt_cf}): the hypothesis is
the paper's ``$A$ and $B$ in canonical form'' with chosen bases of normal
tensors, and the conclusion is the per-normal-tensor block count, bond-dimension,
