...
\begin{lemma}[Phase-normalized dominant BNT projections concentrate]%
\label{lem:phase_normalized_dominant_bnt_projection}
	...
	\leanok
	\uses{def:mpv_inner}
	Let \(A_0\) be an MPV block and let \(B_k\) be a finite BNT family with
	dominant block \(B_{k_0}\).  Suppose
	\[
		\ket{V^{(N)}(B_{k_0})}
		=
		\zeta^N \ket{V^{(N)}(A_0)}
		\qquad\text{for all }N,\qquad |\zeta|=1 .
	\]
	...
	\[
		...
		\longrightarrow 1 .
	\]
	In the leading one-copy BNT case, BNT separation supplies the off-diagonal
	decay and strict ordering supplies the geometric damping.
