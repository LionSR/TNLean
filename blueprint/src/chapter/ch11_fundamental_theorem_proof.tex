...
\label{lem:selected_coefficient_from_two_family_linear_independence}
	...
	\cite[Theorem~II.1, lines~1181--1182]{Cirac2016MPDO_arXiv}
	(source label \texttt{thm1}), after applying
...
\label{lem:selected_weighted_summand_from_phase_sum_li}
	...
	\]
	for all sufficiently large \(N\).
	This packages the step in the proof of
...
\label{lem:eventual_tail_proportional_from_phase_sum_li_left}
	...
	\(\ket{V^{(N)}(B_{b_0})}=\zeta^N\ket{V^{(N)}(A_{a_0})}\) with
	\(\zeta\ne 0\).  If the family consisting of the remaining \(A\)-blocks
	together with all \(B\)-blocks is eventually linearly independent, then
	the two complements, reindexed by the insertion maps, are eventually
	nonzero proportional.
	This is the same recursive peeling step with the linearly independent
	family ordered as the remaining \(A\)-blocks together with all
	\(B\)-blocks; it is used for the symmetric half of
	\cite[Theorem~II.1, lines~1181--1185]{Cirac2016MPDO_arXiv}.
\end{lemma}
...
\begin{proof}\leanok
	...
	tail-subtraction package gives eventual proportionality of the two
	complements.
\end{proof}
...
\begin{proof}
	...
	vanishing overlaps with every block on the other side, the BNT expansion of
	the total MPV and Lemma~\ref{lem:bnt_overlap_orthonormal_li} would contradict
	the assumed proportionality of the total MPV families. The same argument
	with the two tensor families interchanged gives the symmetric assertion.
\end{proof}
