...
here on the basis-of-normal-tensors canonical-form surface. The multi-block
proportional theorem (Theorem~\ref{thm:cpgsv_multiblock_ft_source}) gives the
per-normal-tensor conclusion --- equal block counts $g_a=g_b$ and a per-block
unit phase and gauge $Q_k^i=\zeta_k Y_k P_{\beta(k)}^i Y_k^{-1}$ --- from
eventually nonzero proportional MPVs, using the BNT sector matching of
Chapter~\ref{ch:bnt}. The equal-case corollary
(Theorem~\ref{thm:cpgsv_equal_case_source}) sharpens this to a single global
conjugacy of the total tensors, by additionally combining the canonical-form
reduction, the repeated-copy coefficient comparison, and the block-diagonal
gauge construction. Collapsing the proportional per-block phases into a single
global gauge on the total tensors requires, in addition, the matched-sector
coefficient identity under merely proportional (rather than equal) MPVs; that
copy-weight step is recorded conditionally in Chapter~\ref{ch:bnt}
...
% surface from the sector-matching witnesses of Chapter~\ref{ch:bnt}. The
...
\begin{proof}\leanok
    ...
    the BNT canonical forms $P$ and $Q$. The exact linear-independence matching
    at fixed system size needs no per-sector unit-modulus hypothesis: the
    eventually nonzero proportionality only rescales the aggregate $Q$-side
    coefficients by the scalar $c_N$, leaving the isolated $P$-coefficient to be
    detected by BNT linear independence, which is nonzero infinitely often.
    The matching produces the bijection $\beta$, the bond-dimension equalities,
    the unit phases $\zeta_k$, and the block gauges $Y_k$ with
    $Q_k^i=\zeta_k Y_k P_{\beta(k)}^i Y_k^{-1}$.
\end{proof}

This is the multi-block theorem of
\cite[Theorem~II.1]{Cirac2017MPDO_AnnPhys}; see also
\cite[Theorem~4.5 and Corollary~IV.5]{Cirac2021Matrix}. The statement is
formalised on the BNT canonical-form surface (Definition~\ref{def:sector_bnt_cf}):
the hypothesis is the paper's ``$A$ and $B$ in canonical form'' with chosen bases
of normal tensors, and the conclusion is the per-normal-tensor block count,
bond-dimension, gauge-phase, and unit-phase data. The reduction of an
