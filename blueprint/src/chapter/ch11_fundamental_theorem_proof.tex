...
\begin{proof}\leanok
    ...
    Write $B^i=\zeta X A^iX^{-1}$.  Transporting a nonzero positive
    fixed point through $X$ gives a nonzero positive matrix $\sigma$ satisfying
    \[
        \mathcal E_B(\sigma)=|\zeta|^2\sigma.
    \]
    Since $\mathcal E_B$ also has a nonzero positive fixed point,
    irreducibility forces $|\zeta|^2=1$.  Substitution into the left-canonical
    identities then makes $X^{\dagger}X$ a positive-semidefinite fixed point of
    the adjoint transfer map of $A$.  Uniqueness gives
    $X^{\dagger}X=c\mathbb 1$ for some $c>0$.  Hence $U=c^{-1/2}X$ is unitary
    and $X A^iX^{-1}=U A^iU^{\dagger}$.
\end{proof}
