...
While the previous results focus on a repeated tensor, the site-dependent setting
treats chains carrying a distinct tensor per site. Windows are arcs of consecutive sites,
...
\end{lemma}
\begin{proof}\leanok
    \uses{lem:chain_overlapWindow_exists_bondOperator, lem:exists_arcTransport, lem:isWindowInjective_arc_span}
...
\end{lemma}
\begin{proof}\leanok
    \uses{lem:chain_exists_insertionHom}
    ...
    Skolem--Noether (Theorem~\cite[``Skolem--Noether'']{QICLean2026Blueprint}) it is conjugation
...
\begin{corollary}[Fundamental Theorem for site-dependent normal closed
    ...
    closing section of \cite{Molnar2018NormalPEPS} obtained from its
...
\end{corollary}
\begin{proof}\leanok
    \uses{lem:chain_exists_conjugation_of_sameState, lem:chain_overlapWindow_exists_bondOperator, lem:isWindowInjective_arc_span}
...
\begin{corollary}[Fundamental Theorem for injective closed MPS chains]%
    ...
    \leanok
    \uses{cor:fundamentalTheorem_normalMPSChain_of_overlap, lem:isWindowInjective_one_of_isInjective}
    Let $A_0,\ldots,A_{n-1}$ and $B_0,\ldots,B_{n-1}$ be two
...
\begin{corollary}[Gauge comparison with the cyclic shift]%
    ...
    \leanok
    \uses{cor:fundamentalTheorem_injectiveMPSChain_of_sameState, def:mps_chain_cyclic_shift}
...
\begin{corollary}[Gauge-to-first description from a cyclic-invariant state]%
    ...
    \leanok
    \uses{cor:fundamentalTheorem_injectiveMPSChain_cyclicShift, cor:exists_gauge_to_first_of_cyclicShift_gaugeEquiv}
    Let $A_0,\ldots,A_{n-1}$ be an injective site-dependent closed chain
    with positive bond dimension on $n\ge3$ sites. If its generated state
...
\begin{corollary}[Translation-invariant description of an injective closed MPS chain,
    ...
    \leanok
    \uses{cor:fundamentalTheorem_injectiveMPSChain_cyclicShift, def:mps_chain_cyclic_shift}
...
\begin{theorem}[Uniqueness of the injective closed-chain gauge]%
    ...
    \leanok
    \uses{cor:fundamentalTheorem_injectiveMPSChain_of_sameState, def:chain_arc_window_injective}
...
\begin{theorem}[Fundamental Theorem for normal PEPS, vertex-injective case]%
    ...
    \leanok
    \uses{thm:fundamentalTheorem_PEPS, def:peps_normalPEPSBlockingHypotheses, def:GaugeEquivPEPS, def:IsVertexInjective}
    ...
    from the injective Fundamental Theorem, without requiring the region blocking
    hypotheses.
\end{theorem}
\begin{proof}\leanok
