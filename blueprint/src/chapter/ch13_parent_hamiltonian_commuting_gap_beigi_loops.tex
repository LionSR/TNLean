\begin{theorem}[Two-site parent Hamiltonian on a single-site chain]
    ...
    \leanok
    Under the convention that an $L$-site interaction vanishes for $L>N$, the
...
\begin{proof}\leanok
    Since $2>1$, the interaction vanishes by convention, and the kernel of the zero
...
\begin{corollary}[Parent-ground-space dimension is the loop number]
    ...
    \leanok
    \uses{thm:beigi_ordered_cycle_ground_space_dimension, thm:beigi_eventually_constant_sector_graph, thm:nncph_eventual_ground_space_dimension}
...
\begin{theorem}[Transfer fixed point of a normalized Beigi loop tensor]
    ...
    \leanok
    \uses{def:beigi_normalized_minimal_loop_tensor, thm:beigi_minimal_loop_tensor_injective, def:mps_transfer_idempotence}
...
\begin{theorem}[Periodic vector of a positive Beigi loop]
    ...
    \leanok
    \uses{def:beigi_loop_product_state, def:beigi_loop_schmidt_support, def:beigi_normalized_minimal_loop_tensor, lem:mpv_rotate_physical}
...
\begin{theorem}[A positive Beigi loop gives a parent ground state]
    ...
    \leanok
    \uses{def:beigi_sector_graph, def:beigi_loop_product_state, thm:beigi_complement_product_conjugation, thm:beigi_complement_product_fixed_vector}
    ...
    belongs to the parent ground space.
\end{theorem}
...
\begin{theorem}[Positive Beigi loops span the parent ground space]
    ...
    \leanok
    \uses{thm:beigi_loop_product_state_ground_state, cor:beigi_parent_ground_space_loop_number}
...
\begin{theorem}[All-chain nearest-neighbor commuting parent-Hamiltonian ground spaces imply a renormalization fixed point]
    ...
    \leanok
    \uses{def:mps_transfer_idempotence, def:bnt, def:has_nncph_ground_spaces, def:normal, thm:beigi_loop_product_states_span_ground_space, thm:beigi_normalized_minimal_loop_tensor_rfp}
    ...
    \textbf{Scope restriction (NNCPH representative):}
    These two reverse-implication results cover only normal left-canonical
    and multiplicity-one unit-weight representatives, whereas
    \cite[Theorem~3.10]{Cirac2016MPDO_arXiv} is stated for general
    canonical-form tensors with repeated copies and raw weights; documented in
    \cite{gap:cpsv16_nncph_ground_state_scope}
    (\texttt{docs/paper-gaps/cpsv16_nncph_ground_state_scope.tex}).
\end{theorem}
...
\begin{lemma}[Common physical unitaries preserve mixed transfer maps]
    ...
    \lean{MPSTensor.mixedMapLM_rotatePhysical}
    \leanok
...
\begin{lemma}[Gauge-phase transport of mixed-transfer vanishing]
    ...
    \lean{MPSTensor.mixedMapLM_eq_zero_of_gaugePhaseEquiv}
    \lean{MPSTensor.mixedMapLM_cast_eq_zero_iff}
    \leanok
...
\begin{lemma}[Distinct normalized Beigi loops are locally orthogonal]
    ...
    \lean{MPSTensor.BeigiSectorGraphData.normalizedMinimalLoopTensor_mixedMapLM_eq_zero_of_ne}
    \leanok
...
\begin{theorem}[Multi-sector NNCPH ground spaces imply a fixed point for the basis direct sum]
    ...
    \leanok
    \uses{def:mps_transfer_idempotence, def:has_nncph_ground_spaces, thm:bnt_cf_basis_is_bnt_for_direct_sum, thm:beigi_loop_product_states_span_ground_space, thm:beigi_normalized_minimal_loop_tensor_rfp, lem:beigi_normalized_minimal_loop_mixed_transfer_zero, lem:mixed_transfer_gauge_phase_zero, thm:cpsv_normal_mpv_phase_gauge, thm:direct_sum_transfer_idempotent_iff_pairwise_mixed}
    ...
    \]
    Then $B$ is a renormalization fixed point.
    This establishes the reverse implication of
    \cite[Theorem~3.10]{Cirac2016MPDO_arXiv} for multiplicity-one unit-weight
    representatives.
\end{theorem}
...
\begin{theorem}[Fixed points and commuting parent ground spaces for the basis direct sum]
    ...
    \leanok
    \uses{thm:rfp_multiplicity_one_bnt_has_nncph_ground_spaces, thm:nncph_implies_rfp_basis_direct_sum}
    Let $A_1,\ldots,A_g$ be the distinct basis tensors of a BNT canonical
