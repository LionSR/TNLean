...
\begin{theorem}[Supported corner compression with a support isometry]%
    ...
    Let $A$ be a left-canonical tensor supported on an orthogonal projection
    ...
    \begin{proof}\leanok
        ...
        corner-letter identity and, after multiplying along a word and taking
        traces, the displayed matrix-product-vector formula.
    \end{proof}
\end{theorem}
...
\begin{theorem}[Commuting projections give sector isometries]%
    ...
    \begin{proof}\leanok
        ...
        $\sum_k P_k=\Id$ give the direct-sum decomposition, while summing the
        sector trace formulas gives the unit-weight matrix-product-vector
        identity.
    \end{proof}
\end{theorem}
...
\begin{theorem}[Adjoint-fixed projections give sector isometries]%
    ...
    \begin{proof}\leanok
        \uses{thm:commuting_projection_block_decomp_isometry}
        The adjoint fixed-point equation for $P_k$ gives commutation with each
        letter of $A$.  The result then follows from
        Theorem~\ref{thm:commuting_projection_block_decomp_isometry}.
    \end{proof}
\end{theorem}

\begin{theorem}[Period removal with support isometries]%
    ...
    In the period-removal cyclic-sector decomposition of
    Theorem~\ref{thm:cyclic_sector_decomp_after_blocking}, the compression
    maps may be chosen with rectangular support isometries $V_k$ satisfying
    \[
        V_k^\dagger V_k=\Id,\qquad V_kV_k^\dagger=P_k,\qquad
        \varphi_k(X)=V_kXV_k^\dagger .
    \]
    ...
    \begin{proof}\leanok
        ...
        The cyclic peripheral-spectrum decomposition provides the sector
        projections for the blocked tensor $A^{[m]}$.  Applying
