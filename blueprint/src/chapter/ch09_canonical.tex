...
The reduction of a translation-invariant matrix product state tensor to a
weighted direct sum of primitive blocks follows~\cite{PerezGarcia2007Matrix},
\cite{Cirac2016MPDO_arXiv}, \cite{Cirac2021Matrix}, and~\cite[Chapter~6]{Wolf2012Quantum}.
The transfer map of an arbitrary tensor may be reducible and periodic.
The reduction decomposes the bond space into irreducible subspaces. For each
irreducible block in left-canonical form, powering the transfer map collapses the
peripheral spectrum to roots of unity, whose cyclic projections decompose the
tensor into primitive, tensor-irreducible corners. When the transfer map has
period $p$, \cite[Theorem~5]{PerezGarcia2007Matrix} expresses the state vector
for $p\mid N$ as a sum of $p$ cyclic component chains, and gives zero when $p\nmid N$.
...
\begin{theorem}[Boundary traces and component-chain coefficients]
    ...
    \end{align}
    If $N>0$, then $\tr F_u(\sigma)$ is the coefficient of the component chain
    $B_u$ from Definition~\ref{def:pgvwc07_projector_component_chain}.
...
\begin{theorem}[Normalized periodic state-vector decomposition]
    ...
    (\ref{eq:pgvwc07_periodic_boundary_decomposition}) reduces to the boundary
    traces $\tr(Q_u)$ from (\ref{eq:pgvwc07_empty_boundary_decomposition}). If
    $p\nmid N$, then $\ket{V^{(N)}(A)}=0$.
\end{theorem}

\begin{proof}
    \leanok
    \uses{def:pgvwc07_projector_component_chain, thm:pgvwc07_boundary_trace_coefficients}
