...
\begin{theorem}[Supported corner compression with a support isometry]%
    ...
    \[
        ...
        \varphi(X)=VXV^\dagger.
    \]
    Moreover $\varphi(C^i)=A^i$ for every physical letter, and the compressed
    tensor has the same sector matrix-product vectors.
\end{theorem}
...
\begin{theorem}[Adjoint-fixed projections give sector isometries]%
    ...
    \[
        P_k A^i=A^i P_k,
    \]
    which is the input for the corner-letter identity
    \[
        \varphi_k(C_k^i)=P_k A^i P_k.
    \]
\end{theorem}
