\begin{definition}[Terminal spectral refinement of a fusion decomposition]%
    ...
    coisometry. Taking the direct sum over all labels and multiplicities
    at positive length $N$ defines the operator $P_s^{(N)}$.
...
\begin{theorem}[Terminal spectral projectors from a fusion decomposition]%
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This establishes the terminal spectral decomposition of
    \cite[lines~1003--1012]{Cirac2016MPDO_arXiv}.
...
\begin{proof}\leanok
    ...
    transport preserves orthogonality and the spectral sum.
    Corollary~\ref{cor:mpdo_fusion_clause_topological_density_factor_commutator}
    gives the final two identities.
\end{proof}
...
\begin{theorem}[Terminal spectral projectors for an RFP MPDO with
    ...
    This establishes the spectral decomposition of
    \cite[lines~1003--1012]{Cirac2016MPDO_arXiv}.
\end{theorem}
\begin{proof}\leanok
...
\begin{definition}[Commuting Gibbs form on the retained vertical space for
    ...
    nonzero vertical sectors, with reconstruction onto the physical space
    treated in \cite{gap:cpsv16_topological_gibbs_physical_complement}.
...
\begin{theorem}[Fusion decomposition gives a commuting Gibbs decomposition
    ...
    The formula holds for $R_L$ on the retained vertical space; the extension to
    the physical space is analyzed in
...
\begin{theorem}[Commuting Gibbs decomposition on the retained vertical space
    ...
    sectors, with the physical-space extension treated in
    \cite{gap:cpsv16_topological_gibbs_physical_complement}.
    The conclusion holds for $L=N+2$ (see
    \cite{gap:cpsv16_topological_gibbs_length_one} regarding length one).
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Physical commuting Gibbs decomposition for chains of at least
    ...
    coisometry, supplying the physical complement correction described in
    \cite{gap:cpsv16_topological_gibbs_physical_complement}.
    The conclusion holds for $L=N+2$ (see
    \cite{gap:cpsv16_topological_gibbs_length_one} regarding length one).
\end{theorem}
...
\begin{remark}[Length-one boundary of the two-site construction]%
    ...
    lower bound on $L$, whereas the two-site interaction convention requires
    $L\geq2$ (see \cite{gap:cpsv16_topological_gibbs_length_one}).
    % See also docs/paper-gaps/cpsv16_topological_gibbs_physical_complement.tex.
\end{remark}
