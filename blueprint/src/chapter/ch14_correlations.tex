...
\begin{theorem}[Exponential decay bound (interface)]%
    ...
    \leanok
    \uses{def:connected_correlator}
    If a constant $C_{XY}\in\R$ and $\lambda_2\in\C$ with
    $|\lambda_2|<1$ satisfy, for all $n\ge0$,
    \begin{align}
        |C(X,Y;n)|
        &\le C_{XY}\,|\lambda_2|^n,
...
\begin{definition}[Correlation length]\label{def:correlation_length}
    ...
    The correlation length associated with a nonzero subleading eigenvalue
    $\lambda_2$ of the transfer map, where $0<|\lambda_2|<1$, is
    \begin{align}
        \xi
        &= -\frac{1}{\log|\lambda_2|}. \notag
    \end{align}
    If all subleading spectral values vanish and correlations therefore
    vanish after finitely many transfer steps, one may use the limiting
    convention $\xi=0$.
\end{definition}
