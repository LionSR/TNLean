...
\begin{theorem}[Local orthogonal sums preserve SAL]
    ...
    \[
        \rho^{(N)}(M)=\sum_{x=0}^{g-1}n_x\rho^{(N)}(K_x).
    \]
    For $1\leq L\leq N$, write $\widehat\rho_{x,N,L}$ for the normalized
    marginal of $\rho^{(N)}(K_x)$ on the first $L$ sites.  Assume that the
    sector label is visible on the first retained site:
    \[
