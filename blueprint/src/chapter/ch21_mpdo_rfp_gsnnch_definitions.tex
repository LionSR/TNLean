...
\begin{theorem}[Printed Case-II commuting-form implication]
    ...
    Proposition~C.14 of \cite{Cirac2016MPDO_arXiv} states that a simple tensor
    whose matrix product density operators have the displayed GSNNCH form
    satisfies saturation of the area law.

    The proof applies Proposition~C.9 to each sector, but Proposition~C.9 also
    assumes zero correlation length.  The printed statement does not include
    the ambient or sectorwise zero-correlation hypothesis, and the commuting
    bond realization is unambiguous only from length two onward.  The corrected
    theorem below assumes ambient zero correlation length and the one-site
    positivity needed to complete the sector MPDOs.  This distinction is
    recorded in \path{docs/paper-gaps/cpsv16_commuting_form_to_sal.tex}.
\end{theorem}
