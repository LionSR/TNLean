...
\begin{theorem}[Local orthogonal sums preserve SAL]
...
\begin{proof}\leanok
    ...
    \begin{align}
        p_x
        &=\frac{n_x\tr(\rho^{(N)}(K_x))}
          {\tr(\rho^{(N)}(M))}.
        \notag
    \end{align}
    Positivity of the chain operators, positivity of $n_x$, and nonemptiness
    of the family give $p_x>0$ and a nonzero normalization for $M$. Every
    nonempty normalized marginal decomposes as
    \begin{align}
        \widehat\rho_{N,L}(M)
        &=\sum_x p_x\widehat\rho_{x,N,L}.
        \notag
    \end{align}
    Put $Q_x=P_x\otimes\Id^{\otimes(L-1)}$. The support identity gives
    $Q_x\widehat\rho_{x,N,L}=\widehat\rho_{x,N,L}$, while $P_xP_y=0$ for
    $x\ne y$ gives $Q_xQ_y=0$. Thus the summands have pairwise annihilating
    supports. Hence
    \begin{align}
        S_L(M)
        &=\sum_x(-p_x\log p_x+p_xS_L(K_x)).
        \notag
    \end{align}
    Substitution in $I_L=S_L+S_{N-L}-S_N$ gives
    \begin{align}
        I_L(M)
        &=-\sum_xp_x\log p_x+\sum_xp_xI_L(K_x).
        \notag
    \end{align}
    The first sum is independent of $L$, while sectorwise SAL gives
    $I_L(K_x)=I_{L+1}(K_x)$ for every $x$. Therefore
    $I_L(M)=I_{L+1}(M)$ throughout the SAL range.
\end{proof}
...
\begin{theorem}[Orthogonal fixed-bond sector sums with ZCL satisfy SAL]
    \label{thm:mpdo_orthogonal_fixed_bond_sector_sum_zcl_sal}
    \lean{MPOTensor.isSAL_of_orthogonalCommutingSectorFamily_of_isSourceZCL}
    \leanok
    \uses{def:mpdo_orthogonal_commuting_sector_family,
        def:mpdo_eta_local_structure_data, def:mpo_source_zcl,
        def:is_sal, def:mpdo, def:injective, def:normal}
    Let $K_x$ be a nonempty finite family and let $n_x$ be positive natural
    numbers. For each $x$, suppose that there is a positive two-site bond
    $C_x$, supported on $P_x\otimes P_x$, such that the projections $P_x$ are
    pairwise orthogonal, the translates of $C_x$ commute, and, for every
    $N\geq2$,
    \begin{align}
        \rho^{(N)}(K_x)
        &=\prod_{i=0}^{N-1}(C_x)_{i,i+1}.
        \notag
    \end{align}
    Suppose also that, for every $N>0$,
    \begin{align}
        \rho^{(N)}(M)
        &=\sum_x n_x\rho^{(N)}(K_x).
        \notag
    \end{align}
    Assume that every $K_x$ is injective and normal and generates MPDOs.
    For each $x$, suppose that there is a positive two-site bond $B_x$ whose
    translates commute and, for every $N\geq2$, a constant $c_{x,N}>0$ such
    that
    \begin{align}
        \rho^{(N)}(K_x)
        &=c_{x,N}\prod_{i=0}^{N-1}(B_x)_{i,i+1}.
        \notag
    \end{align}
    If the physical-trace transfer of every $K_x$ has zero correlation
    length, then $M$ saturates the area law.

    This is a scope-restricted result associated with the proposition at
    \cite[Appendix~C.2, lines~1801--1808]{Cirac2016MPDO_arXiv}. The source
    sector premise supplies the proportional fixed-bond form displayed using
    $B_x$, while the printed statement omits the sectorwise
    zero-correlation-length hypothesis used by its proof. The present theorem
    additionally assumes MPDO positivity,
    one-site injectivity, normality, and source zero correlation length for
    every sector. One-site injectivity is not a consequence of BNT normality
    alone, so the source application requires an additional blocking
    argument. The bonds $C_x$ give a separate scalar-one realization, stronger
    than the proportional realization by $B_x$, and the outer
    positive-multiplicity sum is assumed exactly.

    % **Scope restriction (sectorwise hypotheses and exact realization):**
    % The source has proportional ApprhoNComm data, whereas this theorem also
    % assumes per-sector MPDO, one-site injectivity, normality, source ZCL,
    % proportional fixed-bond data, an independent orthogonal commuting
    % sector family with exact scalar-one realization, and an exact positive-
    % multiplicity outer sum. One-site injectivity is not supplied merely by
    % BNT normality, leaving a blocking gap in the source application.
...
\begin{proof}\leanok
    ...
    to every $K_x$. Each sector therefore satisfies SAL. The conclusion
    follows from
    Corollary~\ref{cor:mpdo_orthogonal_commuting_sector_sum_sal}.
\end{proof}
