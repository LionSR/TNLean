...
\begin{theorem}[Proportional orthogonal commuting-bond sectors with ZCL satisfy SAL]
    ...
    % **Scope restriction (one-site injective sectors):** The printed
    % proposition applies the single-sector result to normal BNT sectors, but
    % this theorem still assumes one-site injectivity. The all-original-cut
    % bridge from eventual injectivity is not yet proved. See
...
\begin{remark}[Sectorwise source ZCL in the canonical BNT application]
    ...
    correlation length supplies the sectorwise hypothesis by
    Theorem~\ref{thm:mpdo_absorbed_bnt_sector_zcl}. This conclusion is specific
    to that canonical presentation. An arbitrary GSNNCH presentation does not
    inherit source zero correlation length, since that condition belongs to
    the tensor's physical-trace transfer rather than only to its periodic
    operators. The remaining source application is the passage from eventual
    injectivity to visibility at every original consecutive cut.

    % The remaining bridge is tracked in issue #5208. The literal source
    % proposition `prop4to2` is not complete until that bridge is proved.
\end{remark}
