...
\begin{theorem}[Proportional orthogonal commuting-bond sectors with ZCL satisfy SAL]
    ...
    % injectivity only after blocking. Under the present blocking and SAL
    % interfaces, blocked SAL supplies only block-aligned chains and cuts and
    % does not by itself establish unblocked SAL. Following the present proof
    % route requires all-original-cut visibility from eventual block
    % injectivity. The global outer sum and sector products now have the
    % positive proportionality allowed by ApprhoNComm.
    % Documented in docs/paper-gaps/cpsv16_commuting_form_to_sal.tex.
\end{theorem}
