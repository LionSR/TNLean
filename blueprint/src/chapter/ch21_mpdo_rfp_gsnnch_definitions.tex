...
\begin{theorem}[Normalized operators under an isometric physical inclusion]
    ...
    \lean{MPOTensor.trace_mpo_changePhysicalBasis_of_isometry,
        MPOTensor.normalizedMPO_changePhysicalBasis_of_isometry}
    \leanok
    \uses{def:block_entropy, def:mpdo_single_kraus_map,
        def:mpdo_sitewise_physical_matrix}
    With the same notation, the normalized periodic operators satisfy
    \begin{align}
      \sigma^{(N)}(K^V)
...
\begin{proof}\leanok
    ...
    The sitewise congruence preserves the trace because $V^{\otimes N}$ is an
    isometry.  It therefore commutes with the scalar normalization.
\end{proof}
...
\begin{theorem}[Prefix marginal of a sitewise isometric image]
    ...
    \uses{def:block_entropy, def:mpdo_single_kraus_map,
        def:mpdo_sitewise_physical_matrix}
    If a state on $L+K$ sites is conjugated by $V^{\otimes(L+K)}$, then its
    first-$L$-site marginal is conjugated by $V^{\otimes L}$.
\end{theorem}
