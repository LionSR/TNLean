...
\begin{theorem}[Proportional local orthogonal sums preserve SAL]
    ...
    This proves the proportional outer-sum part of the final inference in
    \cite[Appendix~C.2, lines~1801--1808]{Cirac2016MPDO_arXiv}. It retains
    sectorwise SAL and the local support identities as explicit assumptions.
...
\begin{proof}\leanok
    ...
    first tensor factor. Hence the full periodic product is fixed by the
    corresponding one-site action. Taking the partial trace over the final
    $N-L$ sites preserves this identity.
\end{proof}
...
\begin{proof}\leanok
    The preceding lemma supplies the marginal-support hypothesis of
    Theorem~\ref{thm:mpdo_local_orthogonal_sum_sal}; the remaining hypotheses
    are precisely the assumed outer-sector equality, positivity of the
    multiplicities, and sectorwise SAL.
\end{proof}
...
\begin{proof}
    \uses{thm:mpdo_translation_invariant_commuting_form_zcl_sal,
        cor:mpdo_orthogonal_commuting_sector_sum_sal}
    \leanok
    For every $x$, use the fixed sector bond and its exact periodic-product
    identity in the single-sector commuting-bond theorem to obtain SAL for
    $K_x$. The preceding orthogonal-sum corollary then gives SAL for $M$.
\end{proof}
