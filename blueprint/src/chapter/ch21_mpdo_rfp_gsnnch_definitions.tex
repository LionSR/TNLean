...
% 1801--1808, under the standing Case II biCF hypothesis at line 1628 and
% the surrounding source-ZCL hypothesis.
\begin{theorem}[Canonical BNT commuting sectors with ambient ZCL satisfy SAL]
    ...
    \leanok
    \uses{def:mpdo_proportional_orthogonal_commuting_sector_family,
      def:mpdo_common_sector_weight_absorption, def:mpo_source_zcl,
      def:is_sal, thm:mpdo_bnt_physical_support_restriction}
    Let $M$ admit a nonempty BNT presentation with representatives
    $A_x$, positive copy numbers $n_x$, and a block-diagonal gauge that
    reconstructs $M$ from the corresponding direct sum. Suppose that this
    presentation gives the same matrix product vectors as $M$ at every
    positive length, that the weights of all copies of $A_x$ agree with a
    common value $\mu_x$, and that the physical-trace transfer of
    every $A_x$ is nonnilpotent. Assume also that the one-letter tuples
    $(A_x^i)_x$ span the full product of the representative matrix algebras.
    Put $K_x=\mu_xA_x$.
...
\begin{proof}\leanok
    ...
    Theorem~\ref{thm:mpdo_absorbed_bnt_sector_zcl}. Finally, the canonical BNT
    identity gives
