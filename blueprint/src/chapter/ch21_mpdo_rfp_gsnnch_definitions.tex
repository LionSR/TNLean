...
\begin{theorem}[Orthogonal commuting-bond sectors with ZCL satisfy SAL]
    ...
    This is a restricted result associated with the proposition at
    \cite[Appendix~C.2, lines~1801--1808]{Cirac2016MPDO_arXiv}. The source
    sector premise gives a proportional fixed-bond identity, whereas the
    identity assumed here has scalar one. The theorem also assumes MPDO
    positivity, one-site injectivity, normality, and source zero correlation
    length for every sector, as well as the exact positive-multiplicity outer
    sum. One-site injectivity does not follow from BNT normality alone, so the
    source application requires an additional blocking argument.
