...
\begin{theorem}[A compatible sector decomposition with ambient ZCL satisfies SAL]
    ...
    This is \cite[Proposition~C.14]{Cirac2016MPDO_arXiv} with the ambient
    zero-correlation-length and one-site positivity hypotheses made explicit:
    the printed proposition omits the ambient hypothesis that its proof uses,
    through Proposition~C.9, to obtain sectorwise zero correlation length, and
    its commuting-bond realization is unambiguous only from length two onward,
    so positivity of $\rho^{(1)}(K_x)$ is the sharp additional boundary.
\end{theorem}
