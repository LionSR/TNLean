...
\begin{theorem}[Single-bond predicate iff single-bond commuting form]
    ...
    \leanok
    \uses{def:mpdo_has_commuting_bond_product, def:mpdo_has_commuting_form}
...
\begin{theorem}[Single-bond predicate with idempotence iff single-bond form with it]
    ...
    \leanok
    \uses{def:mpdo_has_commuting_bond_product_zcl, thm:mpdo_has_commuting_bond_product_iff_has_commuting_form}
...
\begin{theorem}[A single bond gives one GSNNCH sector]
    ...
    \leanok
    \uses{def:mpdo_source_gsnnch, def:mpdo_has_commuting_bond_product}
...
\begin{theorem}[Normalized operators under an isometric physical inclusion]
    \label{thm:mpdo_normalized_state_physical_isometry_transport}
    \lean{MPOTensor.normalizedMPO_changePhysicalBasis_of_isometry}
    \leanok
    \uses{def:block_entropy, def:mpdo_single_kraus_map, def:mpdo_sitewise_physical_matrix}
    Let $V:\C^d\to\C^e$ satisfy $V^*V=I$, and let $K^V$ be obtained from an MPO
    tensor $K$ by conjugating each physical matrix by $V$.  The normalized
    periodic operators satisfy
    \begin{align}
        \sigma^{(N)}(K^V)
         & =V^{\otimes N}\sigma^{(N)}(K)(V^{\otimes N})^*.
        \notag
    \end{align}
\end{theorem}
...
\begin{proof}\leanok
    ...
    Since $(V^{\otimes N})^*V^{\otimes N}=I$, cyclicity of the trace gives
    \begin{align}
        \tr\!\left[
            V^{\otimes N}\rho^{(N)}(K)(V^{\otimes N})^*\right]
         & =\tr\!\left[\rho^{(N)}(K)\right].
        \notag
    \end{align}
    Therefore the scalar normalization commutes with the congruence:
    \begin{align}
        \sigma^{(N)}(K^V)
         & =\frac{V^{\otimes N}\rho^{(N)}(K)(V^{\otimes N})^*}
        {\tr[\rho^{(N)}(K)]}
        =V^{\otimes N}\sigma^{(N)}(K)(V^{\otimes N})^*.
        \notag
    \end{align}
...
\begin{theorem}[Prefix marginal of a sitewise isometric image]
    \label{thm:mpdo_block_reduced_state_single_kraus_sitewise}
    \lean{MPOTensor.blockReducedState_singleKraus_sitewise}
    \leanok
    \uses{def:block_entropy, def:mpdo_single_kraus_map, def:mpdo_sitewise_physical_matrix}
    Let $V:\C^d\to\C^e$ satisfy $V^*V=I$.  If an operator on $L+K$ sites is
    conjugated by $V^{\otimes(L+K)}$, then its first-$L$-site marginal is
    conjugated by $V^{\otimes L}$.
\end{theorem}
...
\begin{theorem}[Characteristic polynomial under an isometric inclusion]
    ...
    \leanok
    \uses{def:mpdo_single_kraus_map, lem:matrix_rectangular_charpoly_cyclicity}
...
\begin{theorem}[Reduced states under an isometric physical inclusion]
    ...
    tensor $K$ by conjugating each physical matrix by $V$.  For every
    $0\leq L\leq N$, the contiguous reduced states satisfy
    \begin{align}
        \rho_L^{(N)}(K^V)=V^{\otimes L}\rho_L^{(N)}(K)(V^{\otimes L})^*.
        \notag
    \end{align}
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpdo_normalized_state_physical_isometry_transport, thm:mpdo_block_reduced_state_single_kraus_sitewise}
    The normalized periodic operators are related by the sitewise isometric
    congruence.  Splitting the chain into the retained interval and its
    complement gives
    \begin{align}
         & \tr_{N\setminus L}\!\left[
            \bigl(V^{\otimes L}\otimes V^{\otimes(N-L)}\bigr)
            \sigma^{(N)}(K)
        \bigl(V^{\otimes L}\otimes V^{\otimes(N-L)}\bigr)^*\right] \\
         & \qquad=V^{\otimes L}\rho_L^{(N)}(K)(V^{\otimes L})^*.
        \notag
    \end{align}
    Here $(V^{\otimes(N-L)})^*V^{\otimes(N-L)}=I$, so the isometry on the
    traced factor disappears under the partial trace.
\end{proof}
...
\begin{theorem}[One-site positivity completes proportional sector positivity]
    \label{thm:mpdo_proportional_commuting_sector_positivity}
    \lean{MPOTensor.ProportionalOrthogonalCommutingSectorFamily.%
    ...
    Every sector operator of length $N\geq2$ in a proportionally realized
    orthogonal commuting sector family is positive semidefinite. Consequently,
    if the one-site operator $\rho^{(1)}(K_x)$ is positive semidefinite for
    every sector $x$, then every $K_x$ generates matrix product density
    operators at all positive lengths.
\end{theorem}

\begin{proof}\leanok
    For $N\geq2$, the sector operator is a positive scalar multiple of a
    product of commuting positive bonds, hence is positive semidefinite. The
    one-site hypothesis supplies the only remaining positive chain length.
\end{proof}
...
\begin{definition}[Exact sector products as proportionally realized products]
    \label{def:mpdo_orthogonal_commuting_sector_family_to_proportional}
    \lean{MPOTensor.OrthogonalCommutingSectorFamily.toProportional}
    \leanok
    \uses{def:mpdo_orthogonal_commuting_sector_family, def:mpdo_proportional_orthogonal_commuting_sector_family}
    Every exactly realized orthogonal commuting sector family determines a
    proportionally realized orthogonal commuting sector family with the same
    one-site projections and two-site bonds.
\end{definition}

\begin{proof}\leanok
    Take $a_{x,N}=1$ for every sector $x$ and every $N\geq2$.
\end{proof}
...
\begin{theorem}[Positive factorizations on orthogonal physical restrictions]
    ...
    \leanok
    \uses{def:mpdo_physical_support_restriction, def:mpdo_physical_sector_factorization, def:mpdo_orthogonal_commuting_sector_family}
...
\begin{theorem}[SAL supplies orthogonally supported BNT sector products]
    ...
    \leanok
    \uses{thm:mpdo_bnt_physical_support_restriction, thm:mpdo_physical_support_restriction_bond, thm:mpdo_bnt_sector_projection_resolution, def:mpdo_orthogonal_commuting_sector_family}
...
\begin{theorem}[Orthogonal commuting sector products give the GSNNCH form]
    ...
    \leanok
    \uses{def:mpdo_source_gsnnch, def:mpdo_orthogonal_commuting_sector_family}
...
\begin{theorem}[Proportional commuting sector products have normalized-chain support]
    ...
    \leanok
    \uses{def:mpdo_proportional_orthogonal_commuting_sector_family, def:mpdo_vertical_one_site_actions}
    Let $\{K_x\}_{x=0}^{g-1}$ have a proportionally realized orthogonal
    commuting sector family with one-site projections $P_x$ and two-site
    bonds $B^{(x)}$. Then, for every sector $x$ and every $N\geq2$,
    \begin{align}
...
\begin{proof}\leanok
    Since $P_x$ is a projection and $B^{(x)}$ is supported on
    $P_x\otimes P_x$,
    \begin{align}
        (P_x\otimes\Id)B^{(x)}
         & =B^{(x)}.
        \notag
    \end{align}
    Thus the first translated bond, and hence the full product, satisfy
    \begin{align}
        (P_x\otimes\Id^{\otimes(N-1)})\tau_0(B^{(x)})
         & =\tau_0(B^{(x)}),
        \notag                                \\
        (P_x\otimes\Id^{\otimes(N-1)})
        \prod_{i=0}^{N-1}\tau_i(B^{(x)})
         & =\prod_{i=0}^{N-1}\tau_i(B^{(x)}).
        \notag
    \end{align}
    Multiplying the second identity by $a_{x,N}$ gives
    \begin{align}
        (P_x\otimes\Id^{\otimes(N-1)})\rho^{(N)}(K_x)
         & =\rho^{(N)}(K_x).
        \notag
    \end{align}
    Multiplying this identity by the scalar
    $\bigl(\operatorname{tr}\rho^{(N)}(K_x)\bigr)^{-1}$ gives
    \begin{align}
         & (P_x\otimes\Id^{\otimes(N-1)})
        \left[
            \bigl(\operatorname{tr}\rho^{(N)}(K_x)\bigr)^{-1}
            \rho^{(N)}(K_x)
            \right]
        \notag                            \\
         & \quad=
        ...
        \notag
    \end{align}
    The scalar multiples on both sides are the normalized chain states. This
    remains valid when the trace is zero, with the convention $0^{-1}=0$.
\end{proof}
...
\begin{theorem}[Proportional commuting sector products have prefix-marginal support]
    ...
    Let $\{K_x\}_{x=0}^{g-1}$ have a proportionally realized orthogonal
    commuting sector family with one-site projections $P_x$ and two-site
    bonds $B^{(x)}$. Then, for every sector $x$, every $N\geq2$, and every
    $1\leq L\leq N$,
    \begin{align}
...
\begin{proof}\leanok
    ...
    Write $\operatorname{Tr}_{L+1,\ldots,N}$ for the partial trace over the
    final $N-L$ sites. By definition,
    \begin{align}
        \widehat\rho_{x,N,L}
         & =\operatorname{Tr}_{L+1,\ldots,N}
        \bigl(\widehat\rho_{x,N,N}\bigr).
        \notag
    \end{align}
    The first-site action commutes with this partial trace. The preceding
...
\begin{lemma}[Commuting sector products have local marginal support]
    ...
    \leanok
    \uses{def:mpdo_orthogonal_commuting_sector_family, def:mpdo_vertical_one_site_actions}
...
\begin{theorem}[Proportional orthogonal commuting sector sums preserve SAL]
    ...
    \leanok
    \uses{thm:mpdo_proportional_local_orthogonal_sum_sal, thm:mpdo_proportional_orthogonal_commuting_sector_marginal_support, def:mpdo_proportional_orthogonal_commuting_sector_family}
...
\begin{corollary}[Orthogonal commuting sector sums preserve SAL]
    ...
    \leanok
    \uses{thm:mpdo_proportional_orthogonal_commuting_sector_sum_sal, def:mpdo_orthogonal_commuting_sector_family}
...
\begin{proof}\leanok
    \uses{def:mpdo_orthogonal_commuting_sector_family_to_proportional, thm:mpdo_proportional_orthogonal_commuting_sector_sum_sal}
...
% 1597--1619, and Proposition `prop4to2`, lines 1801--1808.
\begin{theorem}[Proportional orthogonal commuting-bond sectors with ZCL satisfy SAL]
    ...
    \leanok
    \uses{def:mpdo_proportional_orthogonal_commuting_sector_family, def:mpdo, def:injective, def:mpo_source_zcl, def:is_sal}
    Let $K_x$ be a nonempty finite family of injective tensors generating
    MPDOs. Suppose that there are pairwise orthogonal one-site
    ...
    % **Scope restriction (sectorwise source ZCL and positivity):** The
    % printed GSNNCH-only proposition omits the source-ZCL hypothesis used by
    % its proof. This general theorem assumes sector MPDO positivity, one-site
    % injectivity, and sectorwise source ZCL, but no normality.
    % See docs/paper-gaps/cpsv16_commuting_form_to_sal.tex.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpdo_translation_invariant_commuting_form_zcl_sal, thm:mpdo_proportional_orthogonal_commuting_sector_sum_sal}
...
% CPSV16 Proposition C.14 (`prop4to2`), lines 1801--1808.
% Provenance: source-label audit completed in issue #5917.
% Gap: docs/paper-gaps/cpsv16_commuting_form_to_sal.tex.
\begin{theorem}[Printed Case-II commuting-form implication]
    \label{thm:cpsv_prop_c14_printed_status}
    \notready
    \uses{def:mpdo_simple_tensor, def:is_sal, def:mpdo_source_gsnnch, def:mpo_source_zcl}
    According to \cite[Proposition~C.14]{Cirac2016MPDO_arXiv}, a simple tensor
    whose matrix product density operators have the displayed GSNNCH form
    satisfies saturation of the area law.

    The source proof applies Proposition~C.9 to each sector, but that
    proposition also assumes zero correlation length.  Proposition~C.14 does
    not include this ambient or sectorwise hypothesis, and its commuting-bond
    realization is unambiguous only from length two onward.  With ambient zero
    correlation length and one-site positivity for the absorbed sectors, the
    asserted area law follows from the theorem below.
\end{theorem}

% Source: arXiv:1606.00608, Appendix C.2, Proposition `prop4to2`, lines
% 1801--1808. The standing Case II biCF and source-ZCL hypotheses supply the
% sector-decomposition data below, but are not assumptions of this theorem.
\begin{theorem}[A compatible sector decomposition with ambient ZCL satisfies SAL]
    \label{thm:mpdo_canonical_bnt_proportional_sectors_zcl_sal}
    \lean{MPOTensor.%
        isSAL_of_commonWeightAbsorbedBasisMPOTensor_of_proportionalSectors_of_isSourceZCL}
    \leanok
    \uses{def:mpo_tensor, def:sector_weight_data, def:global_gauge_of_blocks, def:same_mpv2_pos, def:blockwise_product_word_span, def:mpdo_doubled_phys_trace_transfer, def:mpdo_common_sector_weight_absorption, def:mpdo_proportional_orthogonal_commuting_sector_family, def:mpo_source_zcl, def:is_sal}
    Let $S$ be a nonempty sector decomposition with representatives $A_x$,
    positive copy numbers $n_x$, and nonzero copy weights. Suppose its total
    bond dimension equals that of an MPO tensor $M$, and a block-diagonal gauge
    reconstructs $M$ from the tensor associated with $S$. Suppose also that
    this tensor has the same matrix product vectors as $M$ at every positive
    length, that all copy weights over $A_x$ have a common value $\mu_x$, and
    that the physical-trace transfer of every $A_x$ is nonnilpotent. Assume
    that the one-letter tuples $(A_x^i)_x$ span the full product of the
    representative matrix algebras, and put $K_x=\mu_xA_x$.

    Suppose that the tensors $K_x$ form a proportionally realized orthogonal
    commuting sector family and that each one-site operator
    $\rho^{(1)}(K_x)$ is positive semidefinite. If $M$ has source zero
    correlation length, then $M$ satisfies SAL.

    % **Scope restriction (ambient source ZCL and one-site positivity):**
    % Proposition `prop4to2` omits the ambient source-ZCL hypothesis used to
    % obtain sectorwise ZCL. The positive commuting-bond realization starts at
    % length two, so positivity of rho_1(K_x) is the sharp additional boundary;
    % positivity at every N >= 2 follows from the commuting bonds. See
    % docs/paper-gaps/cpsv16_commuting_form_to_sal.tex.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpdo_proportional_commuting_sector_positivity, thm:mpdo_proportional_orthogonal_commuting_sector_zcl_sal, thm:mpdo_bnt_physical_support_restriction, thm:mpdo_absorbed_bnt_sector_zcl}
    The one-site hypothesis and
    Theorem~\ref{thm:mpdo_proportional_commuting_sector_positivity} show that
    every $K_x$ generates matrix product density operators. The simultaneous
    one-letter span and the nonzero absorbed weights give one-site injectivity
    by Theorem~\ref{thm:mpdo_bnt_physical_support_restriction}. Ambient source
    zero correlation length descends to every $K_x$ by
    Theorem~\ref{thm:mpdo_absorbed_bnt_sector_zcl}. Finally, the supplied
    positive-length sector-decomposition identity gives
    $\rho^{(N)}(M)=\sum_x n_x\rho^{(N)}(K_x)$ for every $N>0$, so
...
\begin{theorem}[Orthogonal commuting-bond sectors with ZCL satisfy SAL]
    ...
    \leanok
    \uses{def:mpdo_orthogonal_commuting_sector_family, def:mpdo, def:injective, def:mpo_source_zcl, def:is_sal}
    Let $K_x$ be a nonempty finite family of injective tensors generating
    MPDOs. Suppose that there are pairwise orthogonal one-site
    projections $P_x$ and fixed translation-invariant positive commuting
    ...
    % injectivity and source ZCL, an orthogonal commuting sector
    % family with exact scalar-one realization, and the exact positive-
    % multiplicity outer sum. In the Case II application, the biCF one-letter
    % span supplies one-site injectivity. Sector MPDO positivity remains an
    % explicit input. No normality is assumed. The exact realization is stronger
...
\begin{corollary}[Orthogonal BNT sector products give the GSNNCH form]
    ...
    \leanok
    \uses{def:same_mpv2_pos, thm:mpdo_orthogonal_commuting_sectors_gsnnch}
...
\begin{corollary}[Sectorwise SAL gives the BNT GSNNCH form]
    ...
    \leanok
    \uses{def:same_mpv2_pos, thm:mpdo_bnt_sector_sal_commuting_family, cor:mpdo_bnt_orthogonal_commuting_sectors_gsnnch}
    Let $M$ be an MPO tensor and let its BNT canonical form have normal
    representatives $A_x$, copy numbers $n_x$, and copy-independent weights
    $\mu_x$. Put $\mathcal K_x=\mu_xA_x$, and assume that this canonical form
    generates the same matrix product vectors as $M$ at every positive length.
...
\begin{lemma}[Positivity of the sector sum]
    ...
    \leanok
    \uses{def:mpdo_source_gsnnch, lem:mpdo_gsnnch_bond_translates_comm, lem:mpdo_gsnnch_translate_pos}
...
\begin{lemma}[Translation invariance of the sector sum]
    ...
    \leanok
    \uses{def:mpdo_source_gsnnch, lem:mpdo_gsnnch_bond_translates_comm, lem:mpdo_gsnnch_shift_transport}
...
\begin{theorem}[Sector form with nonvanishing traces gives GSNNCH]
    ...
    \leanok
    \uses{def:mpdo_source_gsnnch, lem:mpdo_gsnnch_sector_sum_pos, lem:mpdo_gsnnch_form_normalized}
