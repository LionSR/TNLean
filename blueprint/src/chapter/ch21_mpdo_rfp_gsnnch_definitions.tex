...
\begin{proof}\leanok
    ...
    Hence
    \[
        \spn\{(\mathcal K_s)_{ij}\}_{i,j}
        \subseteq
        \spn\{(K_{P_s})_{pq}\}_{p,q}.
    \]
...
\begin{theorem}[Local orthogonal sums preserve SAL]
    ...
    \[
        \bigl(P_x\otimes\Id^{\otimes(L-1)}\bigr)
          \widehat\rho_{x,N,L}=\widehat\rho_{x,N,L}.
    \]
...
\begin{proof}\leanok
    ...
    \[
        \widehat\rho_{N,L}(M)=\sum_x p_x\widehat\rho_{x,N,L}.
    \]
    ...
    supports.  Hence
    \[
        S_L(M)=\sum_x\bigl(-p_x\log p_x+p_xS_L(K_x)\bigr).
    \]
    Substitution in $I_L=S_L+S_{N-L}-S_N$ gives
    \[
        I_L(M)= -\sum_xp_x\log p_x+\sum_xp_xI_L(K_x).
    \]
