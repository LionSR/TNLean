...
\begin{proof}
    ...
    $(P_L^{\mathrm{ES}}(A))^2=P_L^{\mathrm{ES}}(A)$ on that window.
    Lemma~\ref{lem:transported_es_positive} supplies symmetry, and the case
    $L>N$ is the zero projection by definition.
\end{proof}
...
\begin{theorem}[End-site and cyclic coordinate identifications]
    \label{thm:adjacent_local_term_coordinate_identifications}
    \lean{MPSTensor.restrictLast_eq_cyclicRestrictES_zero,
        MPSTensor.tailRestrictₗ_one_eq_restrictFirst,
        MPSTensor.restrictFirst_eq_cyclicRestrictES_one}
    \leanok
    \uses{def:restrict_last, def:restrict_first, def:tail_restrict_parent, rem:euclidean_local_projector_ingredients}
    Let $\iota_L$ denote the canonical identification of the $L$-site state
    space with its Hilbert-space realization, and let
    $R^{\mathrm{cyc}}_{i,\tau}$ denote cyclic restriction to the length-$L$
    window beginning at $i$, with background configuration $\tau$. For every
    $L\in\N$, every $(L+1)$-site Hilbert-space vector $v$, and every background
    configuration $\tau$,
    \begin{align}
        R^{\mathrm{last}}_{\tau_L}(\iota_{L+1}^{-1}v)
         & =\iota_L^{-1}R^{\mathrm{cyc}}_{0,\tau}v.
    \end{align}
    On three sites, for every one-site prefix $u$ and every state $\psi$,
    \begin{align}
        R^{\mathrm{tail}}_u\psi=R^{\mathrm{first}}_{u_0}\psi.
    \end{align}
    Finally, if $L>0$, then for every $(L+1)$-site Hilbert-space vector $v$
    and every background configuration $\tau$,
    \begin{align}
        R^{\mathrm{first}}_{\tau_0}(\iota_{L+1}^{-1}v)
         & =\iota_L^{-1}R^{\mathrm{cyc}}_{1,\tau}v.
    \end{align}
    No hypothesis on the physical or virtual dimension is required.
\end{theorem}

\begin{proof}
    \leanok
    \uses{def:restrict_last, def:restrict_first, def:tail_restrict_parent, rem:euclidean_local_projector_ingredients}
    Expand the restriction maps on a configuration. At site $0$, cyclic order
    is ordinary order and fixing the complement fixes the final coordinate.
    A one-site prefix is its unique coordinate, which gives the middle identity.
    If $L>0$, cyclic restriction beginning at site $1$ fixes the first coordinate
    and preserves the order of the remaining $L$ coordinates.
\end{proof}

\begin{theorem}[Adjacent three-site kernels and excitation projections]
    \label{thm:adjacent_two_site_local_terms_open_chain}
    \lean{MPSTensor.ker_localTermES_two_three_zero,
        MPSTensor.ker_localTermES_two_three_one,
        MPSTensor.openChainLeftGroundSpaceES_orthogonal_starProjection_eq_localTermES,
        MPSTensor.openChainTailGroundSpaceES_orthogonal_starProjection_eq_localTermES}
    \leanok
    \uses{def:open_chain_ground_subspaces_es, rem:euclidean_local_projector_ingredients}
    For a tensor $B$, let $h_0$ and $h_1$ be the two translated length-two parent
    interactions on the three-site Hilbert space. Then
    \begin{align}
        \ker h_0 & =\mathcal U_2(B),     &
        \ker h_1 & =\mathcal V_{1,2}(B).
    \end{align}
    Consequently, the orthogonal projectors onto the complementary excitation
    subspaces satisfy
    \begin{align}
        P_{\mathcal U_2(B)^\perp}     & =h_0, &
        P_{\mathcal V_{1,2}(B)^\perp} & =h_1.
    \end{align}
    These identities require neither injectivity nor a nonzero bond dimension.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:mem_open_chain_left_ground_space_es, thm:mem_open_chain_tail_ground_space_es, lem:local_term_es_kernel_restrictions, lem:mem_ground_space_es, lem:transported_es_local_projections, thm:adjacent_local_term_coordinate_identifications}
    At site $0$, fixing the final physical coordinate is the cyclic restriction
    defining $h_0$. At site $1$, fixing the one-site prefix is the first-site
    restriction and hence the cyclic restriction defining $h_1$. The kernel
    characterization of a translated local parent interaction therefore gives
    the two displayed kernel identities. Each $h_i$ is an orthogonal projector,
    so its range is the orthogonal complement of its kernel. Uniqueness of the
    orthogonal projector with a prescribed range gives the final two identities.
\end{proof}
...
\begin{theorem}[Zero energy gives the cyclic local constraints]
    ...
    \leanok
    \uses{def:parent_hamiltonian, def:parent_hamiltonian_es, def:chain_ground_space, lem:transported_es_local_projections, lem:local_term_es_kernel_restrictions, rem:projection_geometry_rowsum_identities}
...
\begin{lemma}[Cyclic constraints give zero energy]
    ...
    \leanok
    \uses{def:parent_hamiltonian, def:chain_ground_space, thm:chain_ground_space_implies_frustration_free}
...
\begin{theorem}[The finite-volume kernel is the cyclic constraint space]
    ...
    \leanok
    \uses{def:parent_hamiltonian, def:parent_hamiltonian_es, def:chain_ground_space, thm:parent_hamiltonian_kernel_le_chain_ground_space, lem:chain_ground_space_le_parent_hamiltonian_kernel, def:chain_ground_space_es}
    Assume $N>0$ and $L\le N$. Then
...
\begin{theorem}[Intersection property for two adjacent local kernels]
    ...
    \leanok
    \uses{thm:ground_space_intersection, lem:local_term_es_kernel_restrictions, lem:mem_ground_space_es}
...
\begin{proof}
    ...
    Remark~\ref{rem:projection_geometry_rowsum_identities} gives the
    non-negative ordered cross term for commuting symmetric projections.
\end{proof}
...
\begin{lemma}[Parent-Hamiltonian quadratic-form reduction]
    ...
    \leanok
    \uses{lem:transported_es_positive, thm:parent_ground_space_es_kernel, lem:quadratic_form_to_norm_bound}
...
\begin{lemma}[Two-projection compression on the reduced space]
    ...
    \leanok
    \uses{def:friedrichs_overlap_bound, lem:two_projection_friedrichs_compression}
...
\begin{lemma}[Directional compression on a reduced projection range]
    ...
    \leanok
    \uses{def:friedrichs_overlap_bound, lem:two_projection_friedrichs_product}
...
\begin{lemma}[Directional compression for kernel-complement projections]
    ...
    \leanok
    \uses{def:friedrichs_overlap_bound, lem:two_projection_friedrichs_directional}
...
\begin{lemma}[Finite-overlap projection norm-compression reduction]
    ...
    \leanok
    \uses{lem:finite_overlap_projection_rowsum_reduction, rem:projection_geometry_rowsum_identities}
    Let $P_i$ be a finite family of symmetric projections and let
