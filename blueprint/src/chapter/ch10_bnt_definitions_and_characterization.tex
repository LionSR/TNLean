...
\begin{theorem}[Characterization by canonical-form representatives]
    ...
    \begin{enumerate}
        ...
        \item no two distinct tensors $A_j$ and $A_{j'}$ are related in this
              way.
    \end{enumerate}
    This is \cite[Proposition~2.7]{Cirac2016MPDO_arXiv}, with one necessary
    local correction: the coverage clause is restricted to the active blocks.
    The source canonical-form syntax permits $\mu_k=0$, but such a summand
    contributes nothing to any positive-length MPV and therefore cannot be
    recovered from the BNT spanning property.
\end{theorem}
