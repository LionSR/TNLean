...
\begin{definition}[Basis of Normal Tensors (BNT)]\label{def:bnt}
    ...
    MPVs are linearly independent \cite[Definition~IV.2]{Cirac2021Matrix}.
...
\begin{definition}[Basis of normal tensors, coefficient form]\label{def:bnt_cpsv}
    ...
    In the coefficient form of a basis of normal tensors, each block $A_j$ is a
    normal tensor in the spectral sense (Definition~\ref{def:nt_cpsv}), the
    MPV of $A$ is given at every positive length by coefficients
    $c_j^{(N)} \in \C$ with
...
% Under the nonzero-coefficient convention every coefficient is nonzero, so
% this theorem formalizes the proposition; see
