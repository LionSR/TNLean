...
\begin{theorem}[Printed BNT characterization]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    be a tensor in canonical form.  Proposition~2.7 of
    \cite{Cirac2016MPDO_arXiv} states that a family $(A_j)_j$ is a basis of
    normal tensors for $A$ if and only if every listed normal block
    $\widetilde A_k$ is gauge-phase equivalent to some $A_j$ and no two
    distinct $A_j$ are gauge-phase equivalent.

    The statement is false under the literal displayed canonical-form
    convention, which does not require $\mu_k\ne0$.  A zero-weight block is
    invisible in every positive-length matrix product vector and need not be
    represented by a basis obtained from those vectors.  The corrected
    characterization restricts the coverage condition to the active indices
    $k$ with $\mu_k\ne0$; this is
    Theorem~\ref{thm:cpsv_bnt_characterization}.  The distinction is recorded
    in \path{docs/paper-gaps/cpsv16_bnt_characterization_active_blocks.tex}.
\end{theorem}
