...
\begin{theorem}[Failure of literal BNT uniqueness]
    ...
    \leanok
    \uses{def:bnt_cpsv, def:nt_cpsv, def:same_mpv2_pos, def:block_diagonal_tensor}
...
\begin{theorem}[Restricted BNT uniqueness under converse coverage]
    ...
    \leanok
    \uses{def:bnt_cpsv, def:nt_cpsv, def:same_mpv2_pos, def:block_diagonal_tensor, def:gauge_phase_equiv}
    Let $A$ have a canonical-form MPV expansion
    \begin{align}
        \ket{V^{(N)}(A)}
...
\begin{proof}\leanok
    \uses{thm:cpsv_bnt_characterization, thm:cpsv_bnt_blocks_not_gaugePhaseEquiv, thm:cpsv_normal_mpv_phase_gauge, lem:gauge_phase_equiv_cast_compose, def:gauge_phase_equiv}
