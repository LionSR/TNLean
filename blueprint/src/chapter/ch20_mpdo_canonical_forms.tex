...
\begin{definition}[Vertical canonical form for an MPO]\label{def:vertical_cf_mpdo}
    ...
    $\mu_\alpha=\diag(\omega_{\alpha,0},\dots,\omega_{\alpha,r_\alpha-1})$
    with $r_\alpha\geq1$,
...
\begin{theorem}[Reflected marked-chain identity]
    ...
    \[
    \begin{split}
      \tr\!\left(\mathcal R(A)^{rs}
        M^{\sigma_1\tau_1}\cdots M^{\sigma_L\tau_L}\right)
      ={}&\tr\!\left(\mathcal R(A^\sharp)^{rs}
...
\begin{lemma}[Diagonal tensor evaluates the density-operator diagonal]
    ...
    \]
    since both equal $\tr\bigl(M^{\sigma_1\sigma_1}\cdots
