...
\begin{proof}
    ...
    $L=Mr-1$.  Finally, equality of positive-length MPV families transports the
...
\begin{theorem}[A nonzero vertical corner has a nonzero sector compression]
    ...
    In particular, suppose that $A$ is a normal tensor, $V$ is an isometry,
    and the compression by $V$ is a nonzero scalar multiple of an invertible
    conjugate of $A$.  Then the range projector $VV^\dagger$ has a nonzero
    vertical corner and hence a nonzero first-site compression.
...
\begin{theorem}[Reflected marked-chain identity]
    ...
    Let $M$ generate positive semidefinite operators on every nonempty chain, let
...
\begin{theorem}[Pairwise Gram conjugation of grouped vertical corners]
    ...
    semidefinite operators on every nonempty chain.  Suppose two vertical corners
...
\begin{lemma}[Diagonal tensor as a block decomposition under horizontal form]
    ...
    product vector family with a block-diagonal assembly
    $\bigoplus_k \mu_k\, A_k$ of blocks $A_k$ on
...
\begin{theorem}[Irreducible reducing sectors of the vertically viewed tensor]
    ...
    $\bigoplus_k B_k$ have the same matrix product vectors at every positive
    length.
\end{theorem}
