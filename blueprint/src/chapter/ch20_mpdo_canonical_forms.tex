...
\begin{theorem}[Invariant projections commute with the density operators]
    ...
    vertically viewed tensor.  Then, with $P$ acting on the first spin,
    \[
        PH^{(N)} = PH^{(N)}P = H^{(N)}P
    \]
    for every $N\ge 1$.  This is the displayed chain in the proof of
...
\begin{proof}
    ...
    Splitting off the first site of a density-operator entry gives
    \[
    ...
    it by $(PMP)^{ij}$, which unfolds to the entry of $PH^{(N+1)}P$.  This
    proves $PH^{(N)}=PH^{(N)}P$.  Since $H^{(N)}$ is positive semidefinite
    and $P$ is self-adjoint, Theorem~\ref{thm:mpdo_opposite_corner_zero}
    gives $(\Id-P)H^{(N)}P=0$, hence
    $H^{(N)}P=PH^{(N)}P=PH^{(N)}$.
\end{proof}

\begin{theorem}[Invariant projections reduce blockwise]
    \label{thm:blockwise_opposite_insert_invariant_projection}
    \lean{MPOTensor.blockwise_opposite_insert_eq_of_invariant_projection}
    \leanok
    ...
    equivalently, the opposite corner $P^\perp MP$ vanishes blockwise.
\end{theorem}
...
\begin{proof}
    ...
    \uses{thm:mpdo_first_site_invariant_comm,
        thm:blockwise_opposite_insert_rotated_mpo}
    Theorem~\ref{thm:mpdo_first_site_invariant_comm} turns the hypothesis
    $PM=PMP$ into the entrywise operator identities $PH^{(N)}=PH^{(N)}P$ and
    $PH^{(N)}=H^{(N)}P$ for every size.  These are exactly the hypotheses of
...
\begin{lemma}[Commutation with a power of a positive semidefinite matrix]
    \label{lem:psd_commute_of_commute_pow}
    ...
    itself.  This is the identity excluding nontrivial $p$-periodic sectors
    in the proof of \cite[Proposition~4.13]{Cirac2016MPDO_arXiv}; the
    proposition applies it to an orthogonal projector $Q$, but only
    positivity of $H$ is used.
\end{lemma}
...
\begin{theorem}[Horizontal canonical form implies vertical canonical form]
    ...
    \uses{def:mpdo, def:horizontal_cf_mpdo, def:vertical_cf_mpdo,
        ...
        thm:blockwise_opposite_insert_rotated_mpo,
        thm:mpdo_first_site_invariant_comm,
        thm:blockwise_opposite_insert_invariant_projection,
        lem:psd_commute_of_commute_pow,
...
\begin{proof}
    \uses{def:mpdo, def:horizontal_cf_mpdo, def:vertical_cf_mpdo,
        ...
        lem:mpv_diagonal_tensor,
        lem:vertical_grouping_equiv}
    Theorem~\ref{thm:blockwise_opposite_insert_invariant_projection} shows
    that every self-adjoint $P$ with $PM=PMP$ on the vertically viewed tensor
    reduces the tensor blockwise: the opposite corner $P^\perp MP$ vanishes
    on every horizontal canonical-form block.
    Lemma~\ref{lem:psd_commute_of_commute_pow} supplies the operator identity
    for excluding nontrivial periodic sectors: an orthogonal projector
    commuting with $[H^{(N)}]^p$ commutes with $H^{(N)}$.  One must then
    apply the canonical-form theorem in the vertical direction to obtain a
