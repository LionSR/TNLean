...
\begin{remark}[Sharp finite-length block separation]
    ...
    representatives.  The common exact injective length is $D^4$, while the
    pair-separation argument gives selectors of length
    \[
        3(g-1)(D^4+1)\leq3(D-1)(D^4+1).
    \]
    Concatenating a separate injective prefix gives the longer length
    \[
        (3g-2)(D^4+1).
    \]
    The sharp bound in \cite[lines~340--345]{Cirac2016MPDO_arXiv} remains an
    open gap: one must obtain arbitrary matrices in the selected block at the
    selector length itself.  Once this direct-sum step is proved, the final
    estimate is
    \[
        3(D^4+1)(D-1)\leq3D^5.
    \]
