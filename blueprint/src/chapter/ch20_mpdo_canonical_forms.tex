...
\begin{proof}
    ...
    since $PH^{(N)}P = PH^{(N)}P^\dagger$.  A positive semidefinite matrix
    has nonnegative real trace, and its trace vanishes only if the matrix
    itself is zero; hence a nonzero compression has positive trace.
\end{proof}
