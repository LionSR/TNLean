...
\begin{proof}
    ...
    hypotheses now yield a finite representative-indexed witness.  Quantum
    Wielandt and positive-length propagation give the common exact injective
    length $D^4$, and the existing pair-separation argument constructs the
    source-length block selectors.  What remains open in the sharper
    proposition of \cite[lines~340--345]{Cirac2016MPDO_arXiv} is the direct-sum
    step which obtains the full product-algebra span at that same length,
    without concatenating a further injective prefix.  The exact numerical
