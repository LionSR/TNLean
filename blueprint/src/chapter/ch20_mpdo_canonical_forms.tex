...
\begin{lemma}[Power-sum flattening of repeated vertical blocks]
    ...
    \lean{MPOTensor.sameMPV₂_toTensorFromBlocks_verticalAssembledTensor_of_power_sums}
    \leanok
    ...
    \]
    for every block $\alpha$ and every length $N$, then the two assemblies
    generate the same matrix product vector family:
    \[
...
\begin{lemma}[Diagonal tensor to vertical assembly under power sums]
    ...
    \lean{MPOTensor.sameMPV₂_diagonalTensor_verticalAssembledTensor_of_power_sums}
    \leanok
    ...
    \[
        \mu_\alpha^N=\sum_j \omega_{\alpha j}^N
    \]
    for every block $\alpha$ and every length $N$, then the diagonal tensor
    $i\mapsto M^{ii}$ has the same matrix product vector family as the vertical
    assembly of the diagonal restrictions $A_\alpha^{\mathrm{diag}}$:
    \[
...
\begin{proof}\leanok
    \uses{lem:mpv_diagonal_tensor_eq_blocks,
        lem:vertical_flattening_power_sums}
    Lemma~\ref{lem:mpv_diagonal_tensor_eq_blocks} identifies the diagonal tensor
    with the single-weight block assembly
    $\bigoplus_\alpha \mu_\alpha A_\alpha^{\mathrm{diag}}$. Applying
    Lemma~\ref{lem:vertical_flattening_power_sums} to the diagonal restrictions
    replaces this assembly by the repeated vertical assembly with the same power
    sums.
\end{proof}
