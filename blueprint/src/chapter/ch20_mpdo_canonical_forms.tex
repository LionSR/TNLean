...
\begin{definition}[Single-site vertical loops]
    ...
    product $PM$.  On a chain of length $N+1$, the compression of the density
...
\begin{proof}\leanok
    ...
    $\bigl(\sum_a M^{aa}\bigr)$ times the sum over the remaining $N-1$
...
\begin{proof}\leanok
    ...
    Splitting each diagonal entry of $P_1H^{(N+1)}$ at the first site pairs
    $P$ against the first tensor and leaves the diagonal word evaluation of
    the remaining $N$ sites; summing the diagonal configurations closes the
    remaining loops by Lemma~\ref{lem:mpdo_trace_loop_chain}.  For the
...
\begin{definition}[Sector projectors of the vertical decomposition]
    ...
    $\bigoplus_{\alpha,k}\mu_{\alpha,k}X_{\alpha,k}M_\alpha
    X_{\alpha,k}^{-1}$ of the proof of
    ...
    $X_{\alpha,k}$ cancels under the single-site loop trace.  Constructing
...
\begin{theorem}[The sector trace identity]
    ...
    \]
    For the sector projectors $P_{\alpha,k}$ this is the trace
    identification
    $\tr(P_{\alpha,k}H^{(N)}P_{\alpha,k})=\mu_{\alpha,k}d_\alpha$ in the
    proof of \cite[Proposition~4.13]{Cirac2016MPDO_arXiv}; the source states
    it at the particular length where the compression does not vanish.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Positivity of the vertical weights]
    ...
    \[
        ...
        \mu_{\alpha,k}>0.
    \]
    This is the conclusion of the trace argument in the proof of
