...
\begin{definition}[Horizontal canonical form for an MPO]\label{def:horizontal_cf_mpdo}
    ...
    MPS tensor generates the same complete MPV family as a BNT canonical form
    \[
        \bigoplus_{j=1}^{g}\bigoplus_{q=1}^{r_j}\mu_{j,q}A_j,
    \]
    where $(A_j)_{j=1}^{g}$ is a minimal basis of normal tensors and each
    $\mu_{j,q}$ is nonzero.  Thus its length-$N$ matrix product vector is
    \[
        \sum_{j=1}^{g}
        \left(\sum_{q=1}^{r_j}\mu_{j,q}^{N}\right)V^{(N)}(A_j).
    \]
    In particular, repeated gauge-equivalent copies are grouped over their
    common representative, as in
    \cite[Section~2.3, lines~283--301]{Cirac2016MPDO_arXiv}.
\end{definition}
