...
\begin{theorem}[Commutation of the adjacent virtual bond projectors]
    ...
    % Formula: P_{01}P_{12}=P_{12}P_{01}, where each P_{rs} is the
    % rank-one projector onto the bond vector \varphi_{rs}.
    ...
    % half-indices of the corresponding bonds; the red regions denote the two
    % projector placements and introduce no indices.
    ...
    % Source verdict: the topology is the three-site restriction of the
    % basic-vector form in Equation (3.18) and II_RFP_MPS.png.  Appendix B
    % proves the projector identity algebraically and has no separate figure.
    \[
        \begin{tenkzfree}
            ...
            \tnregion[slot=selected, label={$P_{01}$}, label pos=south,
                name=bondProjectorZero]{bondPhiZero}
            \tnregion[slot=selected, label={$P_{12}$}, label pos=south,
...
\begin{proof}
    ...
    % Formula: (P_2)_{01}(P_2)_{12}
    % =U^{\otimes3}P_{01}P_{12}(U^*)^{\otimes3}; reversing the order replaces
    % P_{01}P_{12} by P_{12}P_{01}.
    ...
    % isometric bond structure.  The overlapping-projector transport is the
    % Appendix B algebra rendered in that source topology.
    \[
        \begin{tenkzfree}
            \tnput[boundary, ports={east:virtual}]
            ...
            \tnregion[slot=secondary, outline, label={$(P_2)_{01}$},
            ...
            \tnregion[group, outline, label={$(P_2)_{12}$},
...
\begin{theorem}[Transport of the three-site commutator]
    ...
    % Source verdict: the paper's proof at lines 1305--1307 invokes the
    % structural characterization and supplies no cyclic-chain picture.  The
    % old ring schematic therefore added unsupported tensor-network ink.
