...
\begin{theorem}[The two-site basic-vector image]
    ...
    Consequently,
    \[
        \ran(U^{\otimes2}I_\varphi)=G_2(\Lambda U).
    \]
\end{theorem}
...
\begin{proof}
    ...
    \]
    Expanding
    $\tr((\Lambda U^i)(\Lambda U^j)Y)$ gives the same sum, so
...
\begin{proof}
    ...
    $\ran(U^{\otimes2}I_\varphi)$.  Orthogonal projection onto
...
\begin{definition}[Tensor associated with a positive Beigi loop]
    ...
    $\varphi_a\in\ran P_{a,a}$.  In sector coordinates define
...
\begin{theorem}[Transfer fixed point of a normalized Beigi loop tensor]
    ...
    \[
        \mathcal E_a(Z)=\tr(X_a^*X_aZ)Y_aY_a^*,
        \qquad
        \mathcal E_a^2=\nu_a^2\mathcal E_a.
    \]
