...
\begin{theorem}[Renormalization fixed points have commuting nearest-neighbor local terms]\label{thm:rfp_implies_nncph}
    ...
    \uses{def:mps_transfer_idempotence, def:is_nncph, def:nt_cpsv,
        thm:cpsv_normal_tp_gauge, lem:rfp_parent_gauge_invariance}
...
\begin{proof}\leanok
    \uses{thm:cpsv_normal_tp_gauge, lem:rfp_parent_gauge_invariance,
        thm:left_canonical_rfp_chain_commutation}
    ...
    \]
    Cyclic transport and locality then give, for every $N>2$,
    \[
        h_i^{(N)}h_{i+1}^{(N)}=h_{i+1}^{(N)}h_i^{(N)},\qquad
        h_i^{(N)}h_j^{(N)}=h_j^{(N)}h_i^{(N)}
    \]
    whenever the first pair is adjacent or the two supports are disjoint.
