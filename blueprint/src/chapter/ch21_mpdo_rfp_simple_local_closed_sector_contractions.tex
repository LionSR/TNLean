...
\begin{theorem}[Closed-sector form of the physical-trace transfer]
    ...
    \leanok
    \uses{def:mpo_physical_trace_transfer, def:mpdo_closed_sector_pairing_operator}
...
\begin{corollary}[Physical-trace transfer from the inverse-map sectors]
    ...
    \leanok
    \uses{def:mpdo_injective_inverse_layer, def:mpdo_eta_structure, thm:mpdo_closed_sector_physical_trace_transfer}
...
\begin{proof}
    \leanok
    \uses{thm:mpdo_sector_factorization, thm:mpdo_closed_sector_physical_trace_transfer}
...
\begin{theorem}[Normalized closed-sector pairing operator]
    ...
    \leanok
    \uses{def:mpo_source_zcl, def:mpdo_injective_inverse_layer, def:mpdo_eta_structure, cor:mpdo_concrete_closed_sector_physical_trace_transfer, lem:mpo_source_zcl_normalized_idempotent}
    Let ${\cal K}$ be injective. Let $R$ and $\rho$ give a three-site
    closure of ${\cal K}$, choose a Hayashi decomposition of $\rho$, and fix
    virtual indices $\alpha_1,\beta_3$ such that
...
\begin{proof}
    \leanok
    \uses{cor:mpdo_concrete_closed_sector_physical_trace_transfer, lem:mpo_source_zcl_normalized_idempotent}
    Substitute
...
\begin{theorem}[Normalized closed-sector trace relations]
    ...
    \leanok
    \uses{def:mpdo_concrete_closed_sector_trace_matrix, thm:mpdo_closed_sector_pairing_normalized_idempotent}
    ...
    \end{align}
    For $\lambda=1$, this gives the trace-power display in
...
\begin{definition}[Explicit neighboring $\eta$-operator data]
    ...
    Fix a Hayashi decomposition $h_\eta$. An explicit neighboring
...
\begin{definition}[Cyclic tensor product of neighboring operators]
    ...
    \leanok
    \uses{def:mpdo_sector_chain_coordinates, def:mpdo_explicit_eta_operators}
...
\begin{theorem}[A fixed sector block is the cyclic neighboring product]
    ...
    \leanok
    \uses{def:mpdo_conjugate_physical, def:mpdo_cyclic_eta_tensor_product, def:mpdo_eta_of_sector_tensors}
...
\begin{theorem}[Sector-adapted MPO decomposition]
    ...
    \leanok
    \uses{def:mpdo_sector_chain_coordinates, def:mpdo_conjugate_physical, def:mpdo_cyclic_eta_tensor_product, def:mpdo_eta_of_sector_tensors}
