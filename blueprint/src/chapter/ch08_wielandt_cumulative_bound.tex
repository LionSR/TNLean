...
\begin{definition}[Strong irreducibility in Proposition~3]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    and $\E_A$ is irreducible. The formal predicate makes irreducibility
    explicit, matching the condition implicit in the source terminology.
    Thus the eigenvalue $1$ has a positive-definite fixed point and is the only
...
\begin{proof}\leanok
    ...
    $\rho$. The spreading condition makes $\rho$ positive definite and
    excludes nontrivial invariant subspaces. The cyclic peripheral-spectrum
    theorem then makes every peripheral eigenvalue a root of unity. Suppose
    $\E_A(X)=\lambda X$ with $|\lambda|=1$ and $\lambda\neq1$, and choose
    $p>0$ with $\lambda^p=1$. Paper primitivity persists under positive
    powers, so $\E_A^p$ has a unique positive fixed point of each trace.
    If $H$ is a trace-zero Hermitian fixed point of $\E_A^p$, choose
    $\varepsilon>0$ small enough that
    $\rho\pm\varepsilon H\succeq0$. These are fixed points with the same
    trace as $\rho$, so both equal $\rho$ and hence $H=0$. Trace preservation
    and $\lambda\neq1$ give $\tr(X)=0$. Apply the preceding argument to
    $X+X^\dagger$ and $\mathrm{i}(X-X^\dagger)$, which are Hermitian fixed
    points of $\E_A^p$. Both vanish, contradicting $X\neq0$.
