...
\begin{definition}[Fixed-length vector span of an MPS tensor]
    \label{def:mps_vector_spread_span}
    \lean{Kraus.vectorSpreadSpan}
    \leanok

    For an MPS tensor $A$, its fixed-length vector span is the fixed-length
    vector span of the underlying finite matrix family.
\end{definition}
...
\begin{definition}[Strengthened strong irreducibility]
    ...
    An MPS tensor $A$ is \emph{strongly irreducible} if there exists a matrix
    $\rho>0$ such that
    \begin{align}
        \E_A(\rho)                 & =\rho, \notag \\
        ...
        \notag
    \end{align}
    and $\E_A$ is irreducible. This formulation includes explicit irreducibility
    alongside the peripheral spectrum and fixed-point condition of
    Sanz et al.~\cite[Proposition~3(c)]{Sanz2010Quantum}; see
    \cite{gap:spwc10_wielandt_one_step_subspace}.
\end{definition}
...
\begin{proof}\leanok
    This is the corresponding theorem in the companion quantum-channel volume~\cite[``Powers on an eigenspace'']{QICLean2026Blueprint} for the eigenspace of $\mu$.
\end{proof}
...
\begin{proof}\leanok
    ...
    Apply Theorem~\cite[``Hermitian power-fixed point from a finite-order eigenmatrix'']{QICLean2026Blueprint} to the transfer
...
\begin{proof}\leanok
    ...
    This is Theorem~\cite[``Uniqueness of power-fixed positive matrices for a Kraus map'']{QICLean2026Blueprint}
...
\begin{theorem}[Strengthened form of the primitivity equivalence]
    ...
    \leanok
    \uses{def:paper_primitive, def:eventually_full_kraus_rank, def:strongly_irreducible_paper}
    Let $D>0$ and suppose
...
\begin{theorem}[Primitive-channel specialization of the index bound]
    ...
    \leanok
    \uses{def:wielandt_i_index, def:wielandt_q_index, thm:wielandt_proposition_three}
    Under the hypotheses of Theorem~\ref{thm:wielandt_proposition_three},
    paper primitivity implies
    \begin{align}
        q(\E_A) & \le\iota(A).
        \notag
    \end{align}
    This is the primitive-channel specialization of the universal inequality
    $q(\E)\le\iota$ of Sanz et al.~\cite[Proposition~1]{Sanz2010Quantum}; see
    \cite{gap:spwc10_wielandt_one_step_subspace}.
\end{theorem}
...
\begin{proof}\leanok
    ...
    The source establishes this by showing that $\E_A^n(\ket\psi\!\bra\psi)$
    has full rank for each pure state and ruling out multidimensional fixed
    spaces via Corollary~6.5 of~\cite{Wolf2012Quantum}. The present argument
    proceeds via the Proposition~3 equivalence
    (Theorem~\ref{thm:wielandt_proposition_three}).
\end{proof}

\begin{lemma}[Monotonicity of cumulative span]\label{lem:cumulative_mono}
    \lean{Kraus.cumulativeSpan_mono}
    \leanok
...
\begin{lemma}[Dimension bound]\label{lem:cumulative_finrank_le}
    \lean{Kraus.cumulativeSpan_finrank_le}
    \leanok
    \uses{def:cumulative_span}
...
\begin{theorem}[Cumulative span stabilisation]\label{thm:cumulative_stable}
    \lean{Kraus.cumulativeSpan_stable}
    \leanok
    \uses{def:cumulative_span}
...
\begin{theorem}[Dimension growth]\label{thm:cumulative_strict_mono}
    \lean{Kraus.cumulativeSpan_finrank_strict_mono}
    \leanok
    \uses{def:cumulative_span}
    If $T_n(A)\neq T_{n+1}(A)$, then
...
\begin{theorem}[Positive-length propagation of block injectivity]
    ...
    \lean{Kraus.wordSpan_succ_eq_mul_left}
    ...
    This combines the block-injectivity discussion in
    \cite[Section~II]{Cirac2016MPDO_arXiv} with
    \cite[Appendix~C.3, Lemma~L]{Cirac2016MPDO_arXiv}.
\end{theorem}
...
\begin{theorem}[Cumulative span reaches $\MN{D}$]\label{thm:cumulative_eq_top}
    ...
    \lean{Kraus.cumulativeSpan_eq_top}
    \leanok
    \uses{def:cumulative_span, def:normal}
    Let $D>0$. If $A$ is normal, then $T_{D^2}(A)=\MN{D}$.
    This cumulative estimate corresponds to the intermediate step in the proof of
...
\begin{theorem}[Eigenvector extraction for an MPS tensor]
    \label{thm:mps_eigenvector_from_full_cumulative_span}
    \lean{Kraus.exists_eigenvector_of_cumulativeSpan_eq_top}
    \leanok
    \uses{def:cumulative_span}
    The conclusion of
    Theorem~\cite[``Eigenvector extraction from a full cumulative span'']{QICLean2026Blueprint} holds for the
    matrix family of an MPS tensor whenever its cumulative span at level $N$
    is the full matrix algebra.
\end{theorem}

\begin{proof}\leanok

    Apply the finite-family theorem to the matrices of the tensor.
\end{proof}
...
\begin{theorem}[Sharp nonzero trace product for a primitive channel]%
    ...
    \leanok
    \uses{def:paper_primitive, def:wielandt_kraus_rank, thm:wielandt_proposition_three}
...
\begin{theorem}[Sharp nonzero trace product]%
    ...
    \lean{Kraus.exists_nonzero_trace_word_sharp}
    \leanok
    \uses{def:normal}
...
\begin{theorem}[Sharp nonzero trace product --- positive length]%
    ...
    \lean{Kraus.exists_nonzero_trace_word_sharp_pos}
    \leanok
...
\begin{definition}[Cumulative vector span]\label{def:cumulative_vector_span}
    \lean{Kraus.cumulativeVectorSpan}
    \leanok
    \uses{def:mps_tensor}
...
\begin{proof}
    \leanok
    \uses{thm:cumulative_eq_top, lem:vector_span_from_matrix, thm:vector_span_stable}
...
\begin{theorem}[Fixed-length eigenvector spreading for a primitive channel]
    ...
    \leanok
    \uses{def:paper_primitive, thm:wielandt_proposition_three, thm:eigenvector_spreading, lem:eigenvector_padding}
    Let $D>0$, suppose $\sum_i(A^i)^\dagger A^i=\Id$, and assume that
    $A$ is primitive in the sense of
...
\begin{theorem}[Wielandt case (2), one-step invertible element]%
    ...
    \leanok
    \uses{def:word_span, def:one_step_augment, lem:one_step_augment_word_span, lem:one_step_augment_normal_kraus_rank, thm:wielandt_case2_normal_generator, def:paper_primitive}
...
\begin{proof}
    \leanok
    \uses{thm:wielandt_proposition_three, lem:one_step_augment_word_span, lem:one_step_augment_normal_kraus_rank, thm:wielandt_case2_normal_generator}
...
\begin{theorem}[Wielandt case (3), one-step non-invertible element]%
    ...
    \leanok
    \uses{def:word_span, def:one_step_augment, lem:one_step_augment_word_span, lem:one_step_augment_normal_kraus_rank, thm:eigenvector_spreading, lem:rank_one_noninvertible_eigenvector, thm:lemma2b_assembly, def:paper_primitive}
    Let $A$ be an MPS tensor with bond dimension $D\ge1$, satisfying
    $\sum_i(A^i)^\dagger A^i=\Id$, and suppose that $A$ is primitive in the
    sense of Definition~\ref{def:paper_primitive}.
...
\begin{proof}
    \leanok
    \uses{thm:wielandt_proposition_three, lem:one_step_augment_word_span, lem:one_step_augment_normal_kraus_rank, thm:eigenvector_spreading, lem:rank_one_noninvertible_eigenvector, thm:lemma2b_assembly}
...
\begin{theorem}[Wielandt's inequality --- general bound]%
    ...
    \leanok
    \uses{def:paper_primitive, def:wielandt_i_index, def:wielandt_kraus_rank}
...
\begin{proof}
    \leanok
    \uses{thm:wielandt_proposition_three, thm:nonzero_trace_word_primitive_sharp, thm:isNormal_blockTensor, thm:wielandt_case2_normal_generator, cor:wielandt_case3_generator}
    Theorem~\ref{thm:wielandt_proposition_three} gives normality.
    ...
    Theorem~\cite[``Nonzero trace implies eigenvector'']{QICLean2026Blueprint}.
    ...
    the rank-one extraction argument of \cite[Lemma~2(b)]{Sanz2010Quantum}.
    In both cases, $\iota(A)\le D^2n\le D^2(D^2-d'+1)$.
\end{proof}

\begin{remark}[Quantitative and qualitative Wielandt bounds]%
    \leanok
    The quantitative bound $\iota(A)\le(D^2-d'+1)D^2$
    (\cite[Theorem~1, case~(1)]{Sanz2010Quantum}) gives an explicit upper bound
    on the injectivity length. For the Fundamental Theorem of MPS, the
    qualitative assertion that $S_N(A)=\MN{D}$ for some finite $N$ suffices.
\end{remark}
