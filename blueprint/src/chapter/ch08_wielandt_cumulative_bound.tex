...
\begin{theorem}[Irreducibility from vector spreading]
    ...
    \leanok
    \uses{def:vector_spread_span, def:irreducible_cp}
...
\begin{proof}\leanok
    \uses{thm:positive-map-conj-eigenvector, thm:root-eigenvector-power-fixed}
...
\begin{proof}\leanok
    \uses{thm:root-eigenvector-power-fixed, thm:positive-map-conj-power-fixed, thm:trace-preserving-eigenvector-trace-zero}
...
\begin{proof}\leanok
    \uses{thm:kraus_power_fixed_point_posdef_from_spreading, thm:posdef_fixedpoint_proportional}
...
\begin{proof}\leanok
    \uses{thm:channel-natural-power, thm:kraus-power-fixed-point-unique-from-spreading, thm:positive_fixedpoint_decomp}
...
\begin{proof}\leanok
    \uses{thm:kraus_posdef_fixed_point_from_tp_spreading, thm:kraus_primitive_from_tp_spreading, thm:kraus_irreducible_from_spreading}
...
\begin{theorem}[Eigenvector extraction for an MPS tensor]
    ...
    \lean{Kraus.exists_eigenvector_of_cumulativeSpan_eq_top}
    \leanok
    \uses{def:cumulative_span, thm:eigenvector_from_full_cumulative_span}
    The conclusion of
    Theorem~\ref{thm:eigenvector_from_full_cumulative_span} holds for the
    matrix family of an MPS tensor whenever its cumulative span at level $N$
    is the full matrix algebra.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:eigenvector_from_full_cumulative_span}
...
\begin{proof}\leanok
    \uses{thm:kraus_cumulative_eigenvector_spreading, lem:kraus_eigenvector_padding}
...
\begin{theorem}[Fixed-length eigenvector spreading for a primitive channel]
    ...
    \leanok
    \uses{def:paper_primitive, def:vector_spread_span, thm:wielandt_proposition_three, thm:eigenvector_spreading, lem:eigenvector_padding}
    Let $D>0$, suppose $\sum_i(A^i)^\dagger A^i=\Id$, and assume that
    $A$ is primitive in the sense of
...
\begin{theorem}[Sharp rank-one extraction for a primitive channel]%
    ...
    \leanok
    \uses{def:paper_primitive, thm:wielandt_proposition_three, def:fitting_decomp, thm:nilpotent_pow_bound}
    Let $D>0$, suppose $\sum_i(A^i)^\dagger A^i=\Id$, and assume that
    $A$ is primitive in the sense of
...
\begin{proof}
    \leanok
    \uses{thm:wielandt_proposition_three, thm:nonzero_trace_word_primitive_sharp, thm:eigenvector_from_trace, thm:isNormal_blockTensor, thm:wielandt_case2_normal_generator, cor:wielandt_case3_generator}
    Theorem~\ref{thm:wielandt_proposition_three} gives normality.
