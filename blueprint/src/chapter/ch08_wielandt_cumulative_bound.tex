...
\begin{definition}[Strong irreducibility in Proposition~3]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    and $\E_A$ is irreducible. Thus the eigenvalue $1$ has a
    positive-definite fixed point and is the only peripheral eigenvalue.
\end{definition}

\begin{theorem}[Equivalent primitivity conditions]
    ...
    \end{enumerate}
    This is~\cite[Proposition~3]{Sanz2010Quantum}.
\end{theorem}

\begin{proof}\leanok
    If $S_i(A)=\MN{D}$ and $\varphi\neq0$, then
    $S_i(A)\varphi=\C^D$, proving (2)$\Rightarrow$(1).
    For (1)$\Rightarrow$(3), first obtain a nonzero positive fixed point of
    the channel. The spreading condition makes this fixed point positive
    definite and excludes nontrivial invariant subspaces. If
    $\E_A(X)=\lambda X$ with $|\lambda|=1$, pass to a positive power for
    which $\lambda^p=1$. The Hermitian and anti-Hermitian parts of $X$ are
    trace-zero fixed points of $\E_A^p$; positivity and uniqueness of the
    positive fixed point force both parts to vanish unless $\lambda=1$.
    Hence the peripheral spectrum is $\{1\}$.
...
\begin{theorem}[Primitivity index is bounded by the full-rank index]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This is the inequality following
...
\begin{theorem}[Sharp nonzero trace product for a primitive channel]%
    ...
    \lean{MPSTensor.exists_nonzero_trace_word_of_isPrimitivePaper_sharp}
    \leanok
    \uses{def:paper_primitive, def:wielandt_kraus_rank,
        thm:wielandt_proposition_three, thm:cumulative_span_sharp}
...
\begin{proof}\leanok
    ...
    rank, hence normality. Starting from
    $\dim T_1(A)\ge\krausrk(A)$, each strict inclusion
    $T_n(A)\subsetneq T_{n+1}(A)$ raises the dimension. Stabilisation before
    reaching $\MN{D}$ would contradict eventual full Kraus rank. Thus
    $T_{D^2-\krausrk(A)+1}(A)=\MN{D}$. Express $\Id$ as a linear combination
    of the corresponding words. Since $\tr(\Id)=D\neq0$, at least one word
    has nonzero trace.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{thm:wielandt_proposition_three,
        thm:nonzero_trace_word_sharp_pos, thm:eigenvector_from_trace,
