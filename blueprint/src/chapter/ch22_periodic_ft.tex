...
\begin{lemma}[Global-gauge assembly]
    \label{lem:periodic_global_gauge_assembly}
    \lean{MPSTensor.repeatedBlocks_of_globalGauge}
    \leanok
    \uses{def:cyclic_sector_decomp, lem:periodic_sector_unitary}
    Let $A$ and $B$ carry the cyclic off-diagonal structure
    \[
...
\begin{proof}\leanok
    ...
    per-site proportionality over $u$ and reindexing by $v=u+q$ identifies this
    with $\zeta^{-1}A^i$.  The unit-modulus scalar and the unitary $U$ exhibit
    $A$ and $B$ as repeated blocks.
\end{proof}
