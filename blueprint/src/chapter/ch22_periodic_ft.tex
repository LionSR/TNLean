...
\begin{lemma}[Corner product of a flattened word]
    ...
    \[
        F_k^{\widehat W}=\varphi_k\bigl(\bar A_k^{\,W}\bigr),
    \]
...
\begin{proof}\leanok
    ...
    diagonal corner to $\varphi_k(\bar A_k^I)$, and multiplicativity of $\varphi$
    reassembles the letters:
    \[
        F_k^{\widehat W}=\prod_{I\in W}\varphi_k(\bar A_k^I)
            =\varphi_k\Bigl(\prod_{I\in W}\bar A_k^I\Bigr)=\varphi_k(\bar A_k^{\,W}).
    \]
...
\begin{lemma}[Ambient corner right inverse from the sector right inverse]
    ...
    From a sector right inverse $\Omega$ recovering the sector word evaluations
    $\bar A_k^{\,W}$ on the sector bond space
...
\begin{proof}\leanok
    ...
    \[
        ...
        =\varphi_k\Bigl(\sum_{W}\bigl(\Omega_k(\varphi_k^{-1}(Y))\bigr)_{W}\,
            \bar A_k^{\,W}\Bigr)
        =\varphi_k\bigl(\varphi_k^{-1}(Y)\bigr)=Y .
    \]
\end{proof}
...
\begin{lemma}[Sector contraction]
    ...
    \[
        \sum_{\boldsymbol\rho}\Bigl(\prod_k(\hat\Omega_{k+1}(X_k))_{\rho_k}\Bigr)
        \prod_k A_k^{\sigma_k}F_{k+1}^{\rho_k}=\prod_k A_k^{\sigma_k}X_k .
    \]
\end{lemma}
