...
\begin{lemma}[Unit-phase gauge relation gives repeated blocks]
    ...
    \[
        ...
        \qquad |\zeta|=1,
    \]
    with $X$ invertible. Then $A$ and $B$ satisfy the repeated-block relation
    \[
        A^i=\zeta^{-1}X^{-1}B^iX.
    \]
\end{lemma}
\begin{proof}\leanok
    ...
    \[
        X^{-1}B^iX=\zeta A^i;
    \]
    multiplication by $\zeta^{-1}$ proves the claim.
\end{proof}
...
\begin{lemma}[Cyclic $\Omega_u$-contraction for blocked sector gauges]%
    ...
    blocks gives
    \begin{equation}\label{eq:pft_intermediate_block_identity}
        A_u^{i_1}F_{u+1}^{\mathbf i_1}
        A_{u+1}^{i_2}F_{u+2}^{\mathbf i_2}
        \cdots
        A_{u+m-1}^{i_m}F_u^{\mathbf i_m}
    ...
    equation~(\ref{eq:pft_intermediate_block_identity}) gives
    \[
    ...
    sides of equation~(\ref{eq:pft_intermediate_block_identity}) and collecting the
    sector phases gives
    \[
        \begin{aligned}
        A_u^{i_1}\otimes A_{u+1}^{i_2}\otimes\cdots\otimes A_{u+m-1}^{i_m}
