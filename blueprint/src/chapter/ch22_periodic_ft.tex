...
\begin{lemma}[Per-sector unitary gauge]
    ...
    \leanok
    \uses{def:periodic_tensor}
    Let $A$ and $B$ be left-canonical irreducible tensors that are gauge-phase
    equivalent.  Then the gauge can be taken unitary: there is a unitary $U$ and
    a unit-modulus scalar $\zeta$ with
    \[
        B^i=\zeta\,U A^i U^{\dagger}\qquad\text{for every }i .
    \]
