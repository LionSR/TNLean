...
\begin{definition}[Flattening bijection on word indices]
    ...
    \[
        [d^m]^L \;\simeq\; [d]^{mL},
    \]
    obtained from the iterated blocking bijection.  Here $[q]^r$ denotes the
    set of words of length $r$ over a $q$-letter alphabet.
\end{definition}
...
\begin{lemma}[Ambient corner right inverse from the sector right inverse]
    ...
    \[
        \sum_{\mathbf j\in[d]^{mL}}
            (\hat\Omega_k(Y))_{\mathbf j}\,F_k^{\mathbf j}=Y .
    \]
\end{lemma}
