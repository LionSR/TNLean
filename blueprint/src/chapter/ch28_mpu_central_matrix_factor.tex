...
\begin{theorem}[Scalar centre forces a full matrix algebra of a definite size]
    ...
    algebra $M_I(\C)$ is the zero algebra, which is isomorphic to no
    $\MN{r}$ with $r\geq1$.

    \textbf{Scope restriction (single-block structure):}
    This proves the single matrix-block building block of the structure theorem for
    finite-dimensional $C^*$-algebras \cite[Proposition~Csform]{SchumacherWerner2004QCA},
    presented inside a matrix algebra, omitting the general direct-sum decomposition
    and abstract $C^*$-algebra generality; documented in
