...
\begin{theorem}[Canonical form of \texorpdfstring{$R$}{R}]%
    ...
    and $16$ letters $R^{pq}$ for $p,q\in\{0,\ldots,3\}$, is in canonical form
    as in Definition~\ref{def:cpsv_canonical_form}
...
\begin{proof}\leanok
    ...
    Thus $\mu^{-1}R^{\bullet\bullet}$ is a normal tensor with the required
...
\begin{theorem}[Non-nilpotency of the retained horizontal block]%
    ...
    block in this canonical form, where $\mu=\sqrt{337/512}$. Then
    \begin{align}
...
\begin{theorem}[Simplicity of the rescaling-stable dimer]
    ...
    the basis of normal tensors has one member, $\widehat R$, and its physical-trace
...
\begin{proof}\leanok
    ...
    canonical physical relabeling of $R^{\bullet\bullet}$. Refine the canonical form
    from Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_cf} to its
    basis-of-normal-tensors sector presentation. The only representative is $\widehat R$, whose
...
\begin{theorem}[The rescaling-stable tensor is a renormalization fixed
    ...
    renormalization fixed point in the sense of Definition~\ref{def:is_rfp_via_ts}.
    The refinement map $T$ from one site to two sites is completely positive
    and trace preserving, with
    ...
    eigenvector. The coarse-graining map $S$ from two sites to one site is completely positive
    and trace preserving, with four Kraus
...
\begin{theorem}[Vertical BNT and algebra structure for \texorpdfstring{$R$}{R}]%
    ...
    The tensor $R$ has a vertical basis of normal tensors with one element $M_0$.
...
\begin{theorem}[Closed-operator factorization and local diagonalization for \texorpdfstring{$R$}{R}]%
    ...
    identifies this matrix as the coefficient matrix
    $\chi_{0,0,0}$ of a vertical basis of normal tensors with one element.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Trace-power formula for the structure coefficients]%
    ...
    vertical basis of normal tensors $\{M_0\}$.
\end{theorem}
\begin{proof}\leanok
