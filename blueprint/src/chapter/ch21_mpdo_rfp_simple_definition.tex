...
\begin{lemma}[Blockwise form of the physical-trace transfer]
    ...
    $\mu_{j,q}$, the physical-trace transfer of the direct-sum tensor is the
...
\begin{definition}[Normalized simplicity in blocked canonical form]
    ...
    This definition fixes the canonical representative and incorporates the
    global unit-weight convention of~\cite[line~246]{Cirac2016MPDO_arXiv}. It
    instantiates the Appendix~C.2 hypothesis on the already-blocked tensor, in
    contrast to Definition~\ref{def:mpdo_simple_tensor} which quantifies over
    blocking lengths and presentations.
\end{definition}
...
\begin{lemma}[Normalized simple canonical forms are horizontal canonical]
    ...
    A tensor in normalized simple canonical form is in horizontal canonical form.
\end{lemma}

\begin{proof}\leanok
    The sector decomposition, its basis-of-normal-tensors property, and the
    block-diagonal gauge directly satisfy the requirements of a horizontal
    canonical form.
\end{proof}
...
\begin{theorem}[Normalized simple canonical forms are simple]
    ...
    Every tensor in normalized simple canonical form is simple in the sense of
