...
\begin{lemma}[Gauge-phase invariance of nilpotent physical trace]
    \label{lem:mpdo_doubled_phys_trace_nilpotent_gauge_phase}
    \lean{MPOTensor.%
...
\begin{theorem}[Presentation independence of BNT nonnilpotency]
    ...
    \uses{def:cpsv_active_bnt_sector_presentation,
        thm:cpsv_active_bnt_presentation_uniqueness,
        lem:mpdo_doubled_phys_trace_nilpotent_gauge_phase}
...
\begin{proof}\leanok
    ...
    Lemma~\ref{lem:mpdo_doubled_phys_trace_nilpotent_gauge_phase} to each
    matched pair.
\end{proof}
...
\begin{definition}[Source-simple tensor]
    ...
    \uses{def:mpdo, def:mpdo_physical_blocking,
        ...
        thm:mpdo_active_bnt_nonnilpotency_independent}
    ...
    Every displayed copy in $P$ has nonzero weight, so dormant candidates are
    absent. The blocking length is existential, and presentation independence
    follows from
...
\begin{lemma}[Source simplicity implies positive-length nontriviality]
    \label{lem:mpdo_source_simple_exists_nonzero}
...
\begin{proof}\leanok
    Choose a representative and a copy over it. Its copy weights are nonzero,
    so its power-sum coefficient cannot vanish at every sufficiently large
    length. At a large length where this coefficient is nonzero, eventual
...
\begin{theorem}[Scaling an active BNT presentation]
    ...
    \lean{MPSTensor.SectorDecomposition.scaleWeights,
        MPSTensor.IsActiveCPSVBasisOfNormalTensors.smul_left}
