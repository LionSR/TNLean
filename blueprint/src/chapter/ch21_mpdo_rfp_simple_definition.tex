...
\begin{theorem}[Vanishing separates the project simplicity predicates]
    ...
    \begin{align}
        M\text{ satisfies the project normalized-simple predicate}
        \quad\text{and}\quad
        M\text{ does not satisfy the project source-simple predicate}.
        \notag
    \end{align}
    Thus the positive-length nonvanishing hypothesis in the project bridge
...
\begin{proof}\leanok
    ...
    physical-trace transfer is non-nilpotent. This proves the project
    normalized-simple predicate. On the other hand, $\rho^{(1)}(M)=0$, which
    violates the additional positive-length nonvanishing clause in the project
    source-simple predicate.
\end{proof}

This separation concerns only the two project predicates. Since the simplicity
definition in~\cite[lines~815--823]{Cirac2016MPDO_arXiv} does not impose
positive-length nonvanishing, the example is not a counterexample to that
source definition. It asserts neither an RFP property nor any rigidity statement
for BNT structure coefficients.
