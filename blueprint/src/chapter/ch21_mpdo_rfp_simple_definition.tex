...
\begin{definition}[Source-simple tensor]
    ...
    $L$-site blocking has a finite basis of normal tensors
    ...
    imposes neither positive-length nonvanishing nor the global unit-weight
    normalization of~\cite[line~246]{Cirac2016MPDO_arXiv}, and it does not
    require a normalized horizontal canonical-form witness.
\end{definition}
...
\begin{proof}\leanok
    ...
    $L$, it multiplies the blocked tensor by $r^L$; the same normal basis spans
    the rescaled tensor after multiplying its length-$N$ coefficients by
    $r^{LN}$.
\end{proof}
...
\begin{proof}\leanok
    ...
    particular the family is positive semidefinite, while $\rho^{(1)}(M)=0$.
