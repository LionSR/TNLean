...
\begin{definition}[Source-simple tensor]
    ...
    \lean{MPOTensor.IsSourceSimple,
        MPOTensor.IsSourceSimple.exists_mpo_ne_zero}
    \leanok
    \uses{def:mpdo, def:mpo_family, def:mpdo_physical_blocking, def:bnt_cpsv,
        def:mpdo_doubled_phys_trace_transfer}
    A tensor generating MPDOs is \emph{source-simple} if its closed MPO is
    nonzero at some positive length and there is a positive physical blocking
    length $L$ such that the doubled-index tensor of the $L$-site blocking has
    a finite basis of normal tensors $B_1,\ldots,B_g$ for which every
    physical-trace transfer $\mathcal T_{B_j}$ is non-nilpotent. This records
    the standing active canonical-block convention of
    \cite[lines~217--246]{Cirac2016MPDO_arXiv} together with the simplicity
    condition of \cite[lines~815--822]{Cirac2016MPDO_arXiv}.

    Both the nonzero length and the blocking length are existential. Thus the
    definition excludes the identically zero closed-MPO family but permits
    isolated vanishing lengths. The existential nontriviality condition does
    not exclude dormant extra candidates in an arbitrary normal-basis
    presentation; in particular, it is not a claim that every member
    of every possible presentation is active. The definition also omits the
    global unit-weight normalization of
...
\begin{proof}\leanok
    ...
    Positive rescaling preserves the MPDO condition and the nonzero closed MPO
    at the chosen positive length. For a blocking length $L$, it multiplies the
    blocked tensor by $r^L$; the same normal basis spans the rescaled tensor
    after multiplying its length-$N$ coefficients by $r^{LN}$.
\end{proof}
...
\begin{proof}\leanok
    ...
    decomposition as the source basis. For every $N>0$,
    Lemma~\ref{lem:paper_decomp_formula} gives
    \begin{align}
        V^{(N)}(A_{\mathrm{tot}})_\sigma
        =\sum_{j=1}^{g} c_{S,j}^{(N)}V^{(N)}(A_j)_\sigma.
        \notag
    \end{align}
    Choose a sector carrying a unit-modulus copy. The basis vectors are
    linearly independent for all sufficiently large $N$, and its sector
    coefficient $c_{S,j}^{(N)}$ is not eventually zero by
    Lemma~\ref{lem:sector_bnt_coeff_not_eventually_zero}. At a sufficiently
    large length where that coefficient is nonzero, the displayed expansion
    is therefore a nonzero blocked-chain state. The canonical-form gauge and
    Theorem~\ref{thm:gauge_invariance} preserve this state. The closed-chain
    blocking identity in
    Theorem~\ref{thm:mpdo_closed_chain_physical_blocking} then unblocks it to a
    nonzero closed MPO at a positive multiple of $L$.

    The same gauge gives
...
\begin{theorem}[Normalized simplicity implies nonvanishing source simplicity
    ...
    \leanok
    \uses{def:mpdo_nonvanishing_source_simple_tensor,
        thm:mpdo_simple_implies_source_simple}
...
\begin{theorem}[Vanishing separates source simplicity from its strengthening]
    ...
    \lean{MPOTensor.SimpleVanishingCounterexample.M,
        ...
        MPOTensor.SimpleVanishingCounterexample.%
            M_isSimple_and_isSourceSimple_and_not_isNonvanishingSourceSimple}
    \leanok
    \uses{def:mpdo_source_simple_tensor,
