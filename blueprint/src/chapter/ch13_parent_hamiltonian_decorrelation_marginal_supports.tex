...
For $\rho \ge 0$ on $H_L\otimes H_R$ with $P_L$ the orthogonal projector
onto the support of $\tr_R\rho$, the identities
$(P_L\otimes\Id_R)\rho=\rho$ and $\rho(P_L\otimes\Id_R)=\rho$ provide
the $P_{AX}$ support-projector identities in
\cite[Appendix~D.2]{Cirac2016MPDO_arXiv} upon setting
$H_L=H_A\otimes H_X$ and $H_R=H_B$, as developed in
\cite{QICLean2026Blueprint}.
...
Similarly, for the support projector $P_R$ of $\tr_L\rho$, one has
$(\Id_L\otimes P_R)\rho=\rho$ and $\rho(\Id_L\otimes P_R)=\rho$.
...
\begin{theorem}[Observable and complex-linear decorrelation are equivalent]
    ...
    \leanok
    \uses{def:tripartite_observable_decorrelation, def:tripartite_decorrelation}
...
\begin{theorem}[Decorrelation for the reduced operators]
    ...
    \leanok
    \uses{def:tripartite_decorrelation, thm:tripartite_marginal_matrix_units}
...
\begin{theorem}[Reverse reduced-operator decorrelation]
    ...
    \leanok
    \uses{thm:tripartite_decorrelation_reverse, thm:tripartite_marginal_matrix_units}
...
\begin{theorem}[Decorrelation for the two marginal supports]
    ...
    \leanok
    \uses{def:tripartite_marginal_supports, thm:tripartite_marginal_complementary_product_zero, thm:tripartite_reverse_marginal_complementary_product_zero}
    If $P=P^\dagger$ and $A$ and $B$ are decorrelated, then
    \begin{align}
...
\begin{theorem}[Product of the two marginal support projectors]
    ...
    \leanok
    \uses{def:tripartite_marginal_supports, thm:tripartite_xb_support_absorption, thm:tripartite_ax_support_absorption, thm:tripartite_complementary_product_zero}
