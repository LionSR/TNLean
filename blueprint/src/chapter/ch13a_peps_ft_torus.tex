...
\end{theorem}
\begin{proof}\leanok
    By Theorem~\ref{thm:peps_stateCoeff_transport}, both transported
    coefficients are the original coefficients evaluated at the
    precomposed physical configuration $\sigma\circ\phi$. The assumed
    equality of the original state coefficients therefore gives the result.
\end{proof}
...
\end{theorem}
\begin{proof}\leanok
    Apply Theorem~\ref{thm:peps_stateCoeff_transport} to the translation
    automorphism and then replace $A^{\tau_{a,b}}$ by $A$ using translation
    invariance.
\end{proof}
...
\end{theorem}
\begin{proof}\leanok
    Translation invariance identifies the bond dimension at an edge with the
    bond dimension at each translated edge. Every horizontal edge is a
    translate of a right edge, and all right edges are translates of the
    reference right edge; the same argument applies to vertical edges using
    up edges. This gives the two displayed equalities.
\end{proof}
