...
ground space $G_m(A_j)$ on an open length-$m$ segment, and the blocks form the
direct sum $S_m:=\bigoplus_{j=1}^g G_m(A_j)$. On the cyclic chain the
parent Hamiltonian has the periodic ground space
$\mathcal G_{N,L}(\bigoplus_j\mu_j A_j)$. The results below establish the
open-segment recursion
$\C^d\otimes S_m\cap S_m\otimes\C^d=S_{m+1}$ and the resulting equality of the
cyclic-chain ground space with the span $\spn\{V^{(N)}(A_j):j=1,\ldots,g\}$ of
the basis-of-normal-tensors component vectors.
