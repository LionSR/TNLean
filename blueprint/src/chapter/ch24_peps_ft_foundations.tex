...
\begin{definition}[PEPS tensor on a graph]
    ...
    % and the last member of the arbitrary incident-edge family; the ellipsis
    % denotes all intervening legs, and the north leg is sigma(v).
    ...
    % Papers/1804.04964/paper_normal.tex:961--979; the ellipsis avoids a
    % square-lattice degree-four restriction.
    \begin{center}
        \begin{tenkzfree}
...
\begin{theorem}[Gauge invariance of PEPS state]%
    ...
    \[
        \sum_j X_e(j,i)\,\bigl((X_e^{-1})^\top\bigr)(j,k)
        =\delta_{ik}.
    \]
\end{theorem}
