    ...
    \begin{center}
        \begin{tenkzfree}
            \tnput[dot, ports={210:virtual,270:virtual,330:virtual,
                ...
                {localStar}{(0,0)}{A_v}
            \tnput[boundary, ports={30:virtual}, label pos=south]
                {localStarFirst}{(-10mm,-8mm)}{e_1}
            \tnput[boundary, ports={90:virtual}, label pos=west]
                {localStarTypical}{(0,-12mm)}{e_k}
            \tnput[boundary, ports={150:virtual}, label pos=south]
                {localStarLast}{(10mm,-8mm)}{e_{\deg(v)}}
            \tnput[boundary, ports={south:physical}, label pos=east]
                {localStarPhysical}{(0,12mm)}{\sigma(v)}
            \tnjoin{localStar.210}{localStarFirst.30}
            \tnjoin{localStar.270}{localStarTypical.90}
            \tnjoin{localStar.330}{localStarLast.150}
            \tnjoin{localStar.north}{localStarPhysical.south}
        \end{tenkzfree}
    \end{center}
\end{definition}
    ...
    \begin{center}
        \begin{tenkzfree}
            \tnput[boundary, ports={east:virtual}, label pos=south]
                {vertexInjectiveDomain}{(-18mm,0)}
                {\bigotimes_{e\ni v}\C^{D_e}}
            \tnput[box, ports={west:virtual,east:physical}]
                {vertexInjectiveMap}{(0,0)}{A_v}
            \tnput[boundary, ports={west:physical}, label pos=south]
                {vertexInjectiveCodomain}{(18mm,0)}{\C^d}
            \tnjoin{vertexInjectiveDomain.east}{vertexInjectiveMap.west}
            \tnjoin{vertexInjectiveMap.east}{vertexInjectiveCodomain.west}
        \end{tenkzfree}
        \qquad $\ker A_v=\{0\}$.
    \end{center}
\end{definition}
...
\begin{theorem}[Extensionality of physical-to-virtual pullback]%
    ...
    tensor map. For every coefficient function $c$,
    \begin{align}
        T_O=T_{O'}
