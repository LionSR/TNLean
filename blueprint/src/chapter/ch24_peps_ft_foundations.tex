...
This section fixes the objects of the chapter: the local tensors, the state they
generate, the gauge freedom on the bonds, and the vertex injectivity condition
together with the local inverse it provides.
...
\begin{definition}[PEPS tensor on a graph]
    ...
    The local tensor at $v$ has one physical leg and one virtual leg for each
    incident edge:
...
\begin{theorem}[Constant-cardinality fiber sum]
    ...
    monoid,
    $\sum_x g(m(x))=n\mathbin{\boldsymbol{\cdot}}\sum_{\eta}g(\eta)$,
...
An invertible matrix on an edge acts on its two endpoint tensors by opposite
oriented insertions. Absorbing such a matrix on every edge gives a gauge
transformation that leaves the generated state unchanged.

\begin{definition}[Edge gauge at a vertex]%
    \label{def:edgeGaugeAt}
    \lean{TNLean.PEPS.edgeGaugeAt}
    \leanok
    For an edge $e=(u,w)$ with $u<w$ and gauge $X_e\in\GL(D_e)$,
...
\begin{theorem}[Gauge equivalence implies same state]%
    ...
    If $A \sim B$ are gauge-equivalent PEPS tensors, then they represent the same state:
    \begin{align}
        \operatorname{state}(A) & = \operatorname{state}(B).
    \end{align}
\end{theorem}

\subsection{Vertex injectivity and the local inverse}

When the linear map from the coefficient space on virtual configurations to
$\C^d$, sending the basis vector at $\eta_v$ to $A_v(\eta_v,\cdot)$, is
injective, the family is linearly independent and the map has a left inverse.
This inverse turns a matrix inserted on an incident bond into an operation on
the physical leg, the construction that drives the gauge extraction in later
sections.
...
\begin{definition}[Vertex injectivity]%
    ...
    $(A_v(\eta,\cdot)\in\C^d)_{\eta}$ indexed by virtual configurations $\eta$
    on the edges incident to $v$ is linearly independent in $\C^d$.
    Equivalently, extending $A_v$ by linearity to a map from the free vector
    space on virtual configurations into the physical space $\C^d$ gives a
    linear map with trivial kernel. This matches the injectivity formulation
    of \cite[Section~3]{Molnar2018NormalPEPS}, where the local tensor is
    interpreted as a map from virtual configurations to the physical space.
...
\begin{lemma}[Trivial kernel in explicit coordinates]%
    ...
    virtual configurations at $v$ satisfying
    $\sum_{\eta}R(\eta)A_v(\eta,\tau)=0$ for every physical index $\tau$, then
    $R=0$.
\end{lemma}
\begin{proof}\leanok
    The hypothesis says that the local tensor map sends $R$ to the zero vector.
    Vertex injectivity makes this map injective, hence $R=0$.
\end{proof}
...
\begin{definition}[Local left inverse]%
    ...
    If the family of local physical vectors $(A_v(\eta,\cdot))_{\eta}$ indexed
    by the virtual configurations $\eta$ at $v$ is linearly independent, one may
    choose a linear map $\Psi_v$ from $\C^d$ to the coefficient space on local
    virtual configurations at $v$ such that $\Psi_v\circ\Phi_v=\Id$.
\end{definition}
...
\begin{theorem}[Local virtual operations on the image]%
    ...
    $v$, one has $O_T(\Phi_v(c))=\Phi_v(Tc)$.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Injectivity of local physical realization]%
    ...
    Suppose the family of local physical vectors at $v$ is linearly
    independent. If two virtual operations at $v$ have the same canonical
    physical realization, then they agree.
\end{theorem}
\begin{proof}\leanok
    Evaluate both physical realizations on vectors in the image of the local
    tensor map, where linear independence makes that map injective.
\end{proof}
...
\begin{definition}[Virtual pullback of a local physical operator]%
    ...
    its virtual pullback by $\Psi_v\circ O\circ\Phi_v$.
...
\begin{theorem}[Recovery of a virtual operation from a physical realization]%
    ...
    Suppose the family of local physical vectors at $v$ is linearly
    independent. If a physical operator $O$ realizes a virtual operation $T$ on
    every vector in the image of $\Phi_v$, then pulling back $O$ recovers $T$.
\end{theorem}
\begin{proof}\leanok
    After applying the local tensor map, both virtual operations have the same
    image. Linear independence makes the local tensor map injective, so the
...
\begin{theorem}[Extensionality of physical-to-virtual pullback]%
    ...
    \uses{def:peps_localTensorMap, thm:peps_localVirtualOpOfPhysicalOp_spec}
    Suppose the family of local physical vectors at $v$ is linearly
    independent. Two physical endpoint operations then have the same virtual
    pullback exactly when their projected actions agree on every vector in the
    image of the local tensor map. For every coefficient function $c$,
    \begin{align}
        T_O=T_{O'}
        \quad\Longleftrightarrow\quad
        P_vO\Phi_v(c)=P_vO'\Phi_v(c).
        \notag
    \end{align}
\end{theorem}
\begin{proof}\leanok
    Apply the local tensor map to the two virtual pullbacks and use the
    pullback specification. The converse follows from injectivity of the local
    tensor map; the forward direction is obtained by rewriting the two virtual
    pullbacks.
\end{proof}
...
\begin{theorem}[Projected local recovery]%
    ...
    Suppose that the compressed physical action $P_vOP_v$ agrees with the
...
\begin{proof}\leanok
    ...
    $P_vOP_v$. By hypothesis this is the physical realization of $T$.
...
\begin{theorem}[Projected local recovery equivalence]%
    ...
    The compressed physical action $P_vOP_v$ is the canonical physical
...
\begin{theorem}[Physical realization of one-edge virtual matrix action]%
    ...
    If the family of local physical vectors at $v$ is linearly independent,
    then a matrix acting on one edge incident to $v$ is realized by a physical
    operator on the physical space at $v$.
\end{theorem}
\begin{proof}\leanok
...
Fixing an edge $e=(u,v)$ splits the vertex set into the two endpoints and the
middle region $V\setminus\{u,v\}$. The state coefficient factors accordingly,
which is the starting point for the edge-middle reduction of the next section.
