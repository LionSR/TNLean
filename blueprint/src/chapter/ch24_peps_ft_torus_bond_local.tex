...
\begin{theorem}[Forward window coefficient transfer from bond locality]
    ...
    \leanok
    \uses{def:peps_regionInsertedCoeff, lem:peps_horizontal_staircase_window_injectivity, thm:peps_horizontal_staircase_reference_edge_crossing, thm:peps_region_transfer_coeff_double_factorization, thm:peps_region_transfer_coeff_incident_form}
    ...
    $2\le L$, $2\le K$, $1\le a$, $a+2L\le W_{\mathrm{torus}}$, and
    $b+2K-1\le H_{\mathrm{torus}}$, together with
    $2L+1\le W_{\mathrm{torus}}$ and $2K+1\le H_{\mathrm{torus}}$.
...
\begin{theorem}[Window witness from bond locality and multiplicativity]
    ...
    \leanok
    \uses{thm:peps_windowCoeffTransferAB_of_bondLocal, thm:peps_windowCoeffTransferBA_of_bondLocal, def:peps_coeff_transfer_map, thm:peps_region_edge_gauge_of_coeff_transfer, def:peps_window_edge_coeff_identity_witness, def:peps_window_edge_coeff_identity_witness_of_hypotheses}
    ...
    the forward theorem. If $T_e(MM')=T_e(M)T_e(M')$ for all
    $M,M'\in\MN{D_A(e)}$, then there exist
...
\begin{proof}
    ...
    and~\ref{thm:peps_windowCoeffTransferBA_of_bondLocal}. With this
    multiplicativity,
