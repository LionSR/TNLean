...
\begin{theorem}[Transitivity of balanced edge-scalar equivalence]%
    ...
    % The multiplication takes place in the endpoint equation; a repeated picture
    % would not add a contraction asserted by the source.
\end{theorem}
...
\begin{theorem}[Balanced edge scalars preserve the gauged tensor]%
    ...
    % The general gauge action is the valence-independent component equation in
    % Definition~\ref{def:gaugeVertex}; the old degree-four star was not a source
    % figure and did not represent an arbitrary vertex.
\end{theorem}
