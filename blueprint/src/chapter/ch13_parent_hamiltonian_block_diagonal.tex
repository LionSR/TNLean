...
\begin{theorem}[Open-chain ground space of trace-preserving blocks]
    ...
    \begin{align}
        \ker H^{\mathrm{open}}_{N,L}(B)=G_N(B)=\sum_jG_N(A^j).
        \notag
    \end{align}
\end{theorem}
\begin{proof}
    ...
    \begin{align}
        H^{\mathrm{open}}_{N,L}(B) & =\sum_{i\in I_{L,N}}h_{i,\mathrm{ES}}(B,L),
        \notag                                                                   \\
        h_{i,\mathrm{ES}}(B,L)     & \ge0.
        \notag
    \end{align}
    Hence, by Theorem~\ref{thm:open_parent_hamiltonian_zero_local_terms},
    \begin{align}
        \ker H^{\mathrm{open}}_{N,L}(B)
    ...
    $G_N(B)\subseteq\ker H^{\mathrm{open}}_{N,L}(B)$ by
...
\begin{theorem}[Three-block pairwise separation from block injectivity]
    ...
    \leanok
    \uses{def:pairwise_block_separating_words, def:l_blk_injective,
    ...
    $L_0+1$ and $S=3(L_0+1)$. Then for all $k\ne j$ there are coefficients
    $c_\omega$ with
    \begin{align}
...
\begin{theorem}[Eventual simultaneous word span of primitive blocks]
    ...
    \leanok
    \uses{def:primitive_mps, def:gauge_phase_equiv}
    Let $A^0,\ldots,A^{r-1}$ be trace-preserving primitive tensors with
    positive bond dimensions and positive definite fixed points.
...
\begin{proof}
    ...
    $3(p+1)$. Each block is irreducible and has primitive transfer map, so
    its self-overlap satisfies $O_{A^jA^j}(N)\to1$
    (Theorem~\ref{thm:peripheral_primitive_irreducible_overlap_one}).
    Theorem~\ref{thm:ph_three_block_pairwise_separation} with
...
\begin{theorem}[Nonzero rescalings preserve gauge-phase equivalence]
    \label{thm:gauge_phase_equiv_of_nonzero_rescalings}
    \lean{MPSTensor.gaugePhaseEquiv_of_smul_smul_cast}
    \leanok
    \uses{def:gauge_phase_equiv}
    Let $A$ and $B$ be tensors of the same bond dimension, and let
    $c,e\in\C$ be nonzero. If $cA$ and $eB$ are gauge-phase equivalent,
    then $A$ and $B$ are gauge-phase equivalent.
\end{theorem}
\begin{proof}
    \leanok
    If $eB^i=\zeta X(cA^i)X^{-1}$ for every $i$, with $\zeta\ne0$, then
    \begin{align}
        B^i=\frac{\zeta c}{e}\,XA^iX^{-1}
        \notag
    \end{align}
    for every $i$, and $\zeta c/e\ne0$.
\end{proof}
...
\begin{theorem}[Primitive representatives of inequivalent normal blocks]
    ...
    $\rho_i$ is its fixed point, and $G_L(A^i)=G_L(B^i)$ for every
...
\begin{proof}
    ...
    \uses{thm:normalized_primitive_gauge,
        ...
        thm:gauge_phase_equiv_of_nonzero_rescalings}
    ...
    transports the gauges, and
    Theorem~\ref{thm:gauge_phase_equiv_of_nonzero_rescalings} removes the
    nonzero rescalings), contrary to the hypothesis.
\end{proof}
...
\begin{theorem}[Blockwise replacement preserves parent Hamiltonians]
    ...
    every volume: $H^{\mathrm{ES}}_{N,L}(A)=H^{\mathrm{ES}}_{N,L}(B)$.
\end{theorem}
\begin{proof}
    \leanok
...
\begin{theorem}[Primitive block representatives with unchanged Hamiltonians]
    ...
    $H^{\mathrm{ES}}_{N,L}(A)=H^{\mathrm{ES}}_{N,L}(B)$ for every $L,N$, where
...
\begin{lemma}[Primitive normalization at a fixed C1 length]
    ...
    block injective at a common positive word length $L_0$, and that its members
    ...
    representative, with positive definite invariant matrix, which remains
    block injective at length $L_0$.
...
\begin{proof}
    ...
    preserve block injectivity at each fixed word length, preserve all local
...
\begin{theorem}[The printed open-chain threshold for primitive blocks]
    ...
    \leanok
    Let $d>0$, $r\ge2$, and $B=\bigoplus_{j=0}^{r-1}\mu_jA^j$, with all
    ...
    Suppose that each block is block injective at the supplied common length
    $L_0>0$. Then for
    \begin{align}
    ...
    $\ker H^{\mathrm{open}}_{N,R}(B)=G_N(B)$.
...
\begin{proof}
    ...
    \uses{lem:bnt_direct_sum_block_separation_word_span,
        thm:ph_tp_word_span_propagation,
        thm:ph_tp_block_open_ground_space}
    The simultaneous-span estimate of
    Lemma~\ref{lem:bnt_direct_sum_block_separation_word_span}, the direct-sum
    lemma of \cite[lines~1346--1408]{PerezGarcia2007Matrix}, gives the full
    product matrix algebra at length $3(r-1)(L_0+1)$. Trace-preserving propagation keeps
...
\end{theorem}
\begin{proof}\leanok
    \uses{thm:ph_primitive_representatives_parent_equality,
        thm:primitive_block_sum_parent_gap,
        thm:primitive_block_sum_parent_gap_threshold}
...
\begin{theorem}[The gap at the prescribed interaction range]
    ...
    physical and bond dimensions, block injective at a common positive word
    length $L_0$, and pairwise inequivalent under virtual conjugation and nonzero
