...
\begin{lemma}[Periodic constraints for a block-diagonal tensor]
    ...
    \leanok
    \uses{lem:block_sum_local_ground_space, lem:periodic_constraints_propagated_subspace_sequence}
    Let $B=\bigoplus_{j=1}^g\mu_jA_j$, where $\mu_j\ne0$ for
...
\begin{theorem}[Block intersections in the trace-preserving normalization]
    ...
    \begin{align}
        (S_{n+1}\otimes\C^d)\cap(\C^d\otimes S_{n+1})=S_{n+2}.
        \notag
    \end{align}
\end{theorem}
\begin{proof}
    ...
    Set $E_j=\sum_b(A^{j,b})^\dagger D_b^j$. Then $C_a^j=E_jA^{j,a}$,
    so each boundary summand lies in $G_{n+2}(A^j)$.
...
\begin{proof}
    ...
    Zero energy for a sum of positive local projectors is equivalent to
    satisfying every contiguous length-$L$ constraint. The block intersection
    identity extends these constraints one site at a time, giving membership
    in $G_N(B)$. Conversely, every boundary-condition vector satisfies each
    local constraint. Nonzero block weights identify $G_N(B)$ with the sum
    of the block spaces.
\end{proof}
...
\begin{proof}
    ...
    Primitivity gives normality, hence a common positive injective length $p$.
    Injectivity persists at every length at least $p$.
    The three-block separation argument at length $3(p+1)$ gives a word
    polynomial selecting one block against any other. Multiplying these
    polynomials gives selectors of a common length $s$ for the whole family.
    For $L\ge s+p$, prepend an arbitrary matrix-valued word polynomial of
    length $L-s$ to each selector. The resulting tuples span each factor of
    the product algebra separately.
\end{proof}
...
\begin{proof}
    ...
    threshold and apply the open-chain intersection theorem.
\end{proof}
...
\begin{proof}
    ...
    Normalize each block independently. The resulting nonzero rescaling
    and virtual gauge leave its local ground spaces unchanged.
    If two normalized blocks were equivalent, composing the gauges and
    dividing by the nonzero normalization factors would give an equivalence
    between the corresponding original blocks, contrary to the hypothesis.
\end{proof}
...
\end{lemma}
\begin{proof}\leanok
    Normalize each tensor by a nonzero scalar and an invertible virtual
    conjugation. These operations preserve injectivity at each fixed word
    length, preserve all local ground spaces, and preserve inequivalence.
    The ground space of a weighted block sum is the sum of the block ground
    spaces. Hence every local parent projection, and therefore every periodic
    parent Hamiltonian, is unchanged.
\end{proof}
...
\begin{theorem}[Cyclic closure of block boundary matrices]
    ...
    If a vector $\psi$ admits a representation as a sum of block boundary
    vectors at every cyclic cut, then $\psi$ belongs to the span of the
    periodic MPS vectors of the blocks.
...
\begin{proof}
    ...
    Choose boundary matrices $X_i^j$ at each cyclic cut $i$.
    Comparing adjacent cuts against all complementary words of length $N-1$
    ...
    linear combination of the periodic block vectors.
\end{proof}
...
\begin{theorem}[Periodic ground space at a simultaneous injectivity length]
    ...
    satisfies
    \begin{align}
        \ker H_{B,L,N}=\operatorname{span}\{V_N(A^j):j\}.
        \notag
    \end{align}
\end{theorem}
\begin{proof}
...
\begin{theorem}[Primitive normalization at a simultaneous injectivity length]
    ...
    Each component ground space is unchanged at every length. The spans of
    the periodic component vectors are likewise unchanged at every chain
    length. For any nonzero weights $\mu_j$, the periodic parent Hamiltonians
...
\begin{proof}\leanok
    \uses{thm:block_word_span_independent_rescaling, thm:word_tuple_span_gauge_invariance,
        thm:normalized_primitive_gauge, lem:block_sum_local_ground_space}
    ...
    and admits a primitive representative obtained by a nonzero rescaling
    and an invertible virtual conjugation. Both operations preserve the
    simultaneous word span at the specified length. They preserve every
    component ground space and multiply each periodic component vector by
    a nonzero scalar. Taking sums of the component ground spaces proves
    equality of the assembled parent Hamiltonians.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{lem:coisometric_reconstruction_local_space, lem:block_sum_local_ground_space,
...
\begin{theorem}[Canonical ground spaces at a simultaneous injectivity length]
    ...
    \begin{align}
        \ker H_{A,L,N}=\operatorname{span}\{V_N(B^j):0\leq j<g\}.
        \notag
    \end{align}
