...
\begin{proof}
    ...
    membership in $G_N(B)$. Conversely, every boundary-condition vector
    satisfies each local constraint. Nonzero block weights identify $G_N(B)$
    with the sum of the block spaces
...
\begin{theorem}[Eventual word span from block selectors]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Then for every $L\ge s+p$ the tuples $(A^{0,w},\ldots,A^{r-1,w})$, with
    $|w|=L$, span $\prod_j\MN{D_j}$.
\end{theorem}
\begin{proof}
    \leanok
    \uses{thm:wordspan_blk_inj_succ,
        lem:product_word_span_from_bnt_block_separation}
    Let $L\ge s+p$. Since $L-s\ge p$, every $A^j$ is block injective at length
    $L-s$ by Theorem~\ref{thm:wordspan_blk_inj_succ}. Lemma
...
\begin{proof}
    ...
    between the corresponding original blocks
    (Lemma~\ref{lem:gauge_phase_equiv_transport_left_right}), contrary to the
    hypothesis.
\end{proof}
