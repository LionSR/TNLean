...
\begin{proof}
    ...
    dissipators and collect the two scalar coefficients.
\end{proof}
...
\begin{theorem}[Trace symmetry of two-Pauli dissipation]
    ...
    \[
        \tr\bigl(X^\dagger\mathcal L_{a,b}(Y)\bigr)
        =\tr\bigl(\mathcal L_{a,b}(X)^\dagger Y\bigr).
    \]
\end{theorem}
...
\begin{proof}
    ...
    Theorem~\ref{thm:two_pauli_generator_spectrum} are nonzero, while the
    identity coefficient is unrestricted.
\end{proof}
...
\begin{definition}[Maximally mixed qubit state]
    ...
    \leanok
    The maximally mixed qubit state is
    \[
        \tau_1=\frac12\Id_2.
    \]
\end{definition}
...
\begin{theorem}[Faithful stationary state of two-Pauli dissipation]
    ...
    \[
        e^{t\mathcal L_{a,b}}(\tau_1)=\tau_1
    \]
    for every $t\ge0$. More generally, for $X\in\MN{2}$,
    \[
        \bigl(\forall t\ge0,\ e^{t\mathcal L_{a,b}}(X)=X\bigr)
        \quad\Longleftrightarrow\quad
        X\in\C\Id.
    \]
    The same equivalence holds for any autonomous semigroup $T_t$ satisfying
    $T_t=e^{t\mathcal L_{a,b}}$ at every nonnegative time.
\end{theorem}
...
\begin{proof}
    ...
    fixed for every nonnegative time lies in the generator kernel.
\end{proof}
...
\begin{theorem}[Conditional logarithmic-time trace-norm estimate]
    ...
    \[
        t\ge \frac{2}{\gamma_*}
        \bigl((2p+1)\log N+\log(2\log2)\bigr),
    \]
    then
    \[
        \lVert\rho_t-\omega\rVert_1\le N^{-p}.
    \]
...
\begin{proof}
    ...
    \]
    The time hypothesis implies
    $e^{-\gamma_*t/2}\le N^{-2p}/(2N\log2)$, so the squared trace norm is at
    most $N^{-2p}$. Both sides are nonnegative, and taking square roots gives
    the result.
\end{proof}
