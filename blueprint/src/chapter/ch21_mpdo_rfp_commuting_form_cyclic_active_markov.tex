...
% docs/paper-gaps/cpsv16_commuting_form_to_sal.tex.
\begin{theorem}[Adjacent-marginal coefficient extraction]
    ...
    \uses{def:mpdo, def:injective, def:normal, def:mpo_source_zcl,
      ...
      def:mpdo_suffix_sector_contraction,
...
\begin{proof}\leanok
    Trace one suffix site and then two suffix sites from adjacent normalized
    marginals. On a retained sector path from $h$ to $q$, the corresponding
    ...
    \[
        ...
        P_{h,q}(T_C^3)_{q,h},
    \]
    where $P_{h,q}\ne0$ is the product of the retained edge traces.
    Recurrence supplies such a path for every ordered pair. Cancellation
    shows that $T_C^2$ and $T_C^3$ have the same positive support.
    Irreducibility then implies that every entry of $T_C^2$ is positive.

    For fixed $q,h$, choose a positive summand
    $(T_C)_{h,r}(T_C)_{r,q}$ of $(T_C^2)_{h,q}$. The three-site retained word
    $h,r,q$ is visible. Thus the adjacent marginal identity at the single
    retained length three gives
    \[
        (T_C^3)_{q,h}=\frac{Z_5}{Z_4}(T_C^2)_{q,h},
    \]
    where $Z_N$ is the positive normalization of the selected tensor at
    length $N$. The ratio is independent of $q,h$; taking
    $\lambda=Z_5/Z_4$ proves the assertion.
\end{proof}
