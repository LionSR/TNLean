...
\begin{theorem}[Finite-chain reduced marginals under nonzero proportionality]
    ...
    \leanok
    \uses{thm:mpdo_finite_chain_normalized_proportionality, def:mpo_to_mps}
...
\begin{theorem}[Eventual equality of normalized finite-chain operators]
    ...
    \leanok
    \uses{def:eventually_nonzero_proportional_mpv, def:mpo_tensor, def:mpo_to_mps}
...
\begin{lemma}[Marginal stability under source zero correlation length]
    ...
    This establishes the first replacement of
...
\begin{theorem}[Sector decomposition of a suffix marginal]
    ...
    \leanok
    \uses{def:mpdo_suffix_sector_contraction, def:mpdo_physical_sector_chain_coordinates}
...
\begin{remark}
    ...
    Theorem~\ref{thm:mpdo_eta_local_sigma_nk2}, given the rank-one trace
    factorization.
\end{remark}
...
\begin{theorem}[A nonminimal sector factorization need not be primitive]
    ...
    \leanok
    \uses{def:mpdo_eta_local_structure_data, def:mpo_source_zcl, def:mpdo, def:injective, def:normal}
    ...
    This demonstrates that primitivity depends on the choice of sector
    factorization in \cite[Appendix~C.2, line~1613]{Cirac2016MPDO_arXiv};
    the same tensor admits a one-sector factorization whose trace matrix
    is primitive.
\end{theorem}
...
\begin{theorem}[Deletion of sectors absent from cyclic products]
    ...
    \leanok
    \uses{def:mpdo_cyclic_active_sector, def:mpdo_physical_sector_chain_coordinates}
...
\begin{theorem}[Trace-visible recurrence of selected fixed-product sectors]
    ...
    \leanok
    \uses{def:injective, def:mpdo_eta_local_structure_data, thm:mpdo_eta_fixed_bond_product_tensor, thm:mpdo_fixed_bond_positive_physical_sector_representative, def:mpdo_physical_sector_factorization, def:mpdo_cyclic_active_sector}
    Let $K$ be an injective tensor with an eta-local structure. Let $C$ be the
    exact fixed-product tensor selected from its commuting bond, and let $F$ be
    the associated positive physical-sector factorization. Then any two sectors
...
\begin{theorem}[Selected fixed-product reduced states]
    ...
    \leanok
    \uses{def:mpdo_eta_local_structure_data, thm:mpdo_fixed_bond_positive_physical_sector_representative}
...
\begin{theorem}[Adjacent selected fixed-product marginals]
    ...
    \leanok
    \uses{def:mpo_source_zcl, def:mpdo_eta_local_structure_data, thm:mpdo_selected_fixed_product_reduced_states}
...
\begin{theorem}[Iterated selected fixed-product marginals]
    ...
    \leanok
    \uses{def:mpo_source_zcl, def:mpdo_eta_local_structure_data, thm:mpdo_selected_fixed_product_reduced_states, thm:mpdo_source_zcl_iterated_marginal_stability}
    Under the hypotheses of
    Theorem~\ref{thm:mpdo_selected_fixed_product_adjacent_marginals}, for every
    $L>0$, the normalized $L$-site reduced states of the selected fixed-product
    tensor obtained from chain lengths $L+3$ and $L+1$ are equal.
\end{theorem}
...
\begin{theorem}[Positive traces of the selected fixed product]
    ...
    \leanok
    \uses{def:mpo_source_zcl, def:mpdo_eta_local_structure_data, thm:mpdo_fixed_bond_positive_physical_sector_representative}
    Let $K$ have an eta-local structure and source zero correlation length.
...
\begin{theorem}[Strict positivity from adjacent cyclic-sector marginals]
    ...
    \leanok
    \uses{def:mpdo_physical_sector_factorization, def:mpdo_cyclic_active_sector}
...
\begin{proof}\leanok
    \uses{def:mpdo_suffix_sector_contraction, thm:mpdo_suffix_marginal_block_expansion, thm:matrix_irreducible_square_support_stabilization}
...
\begin{proof}\leanok
    \uses{def:mpdo_suffix_sector_contraction, thm:mpdo_suffix_marginal_block_expansion, thm:mpdo_cyclic_active_square_positive_adjacent_marginals}
    By
...
\begin{theorem}[Selected fixed-product square--cube identity]
    ...
    \leanok
    \uses{def:injective, def:mpo_source_zcl, def:mpdo_eta_local_structure_data, def:mpdo_physical_sector_factorization, thm:mpdo_eta_fixed_bond_product_tensor, thm:mpdo_fixed_bond_positive_physical_sector_representative, def:mpdo_cyclic_active_sector}
    Let $K$ be an injective tensor with an eta-local structure and source zero
    correlation length. Let $F$ be the
...
\begin{proof}\leanok
    \uses{lem:mpdo_source_zcl_marginal_stability, thm:mpdo_eta_fixed_bond_product_tensor, thm:mpdo_fixed_bond_positive_physical_sector_representative, thm:mpdo_selected_fixed_product_cyclic_recurrence, thm:mpdo_selected_fixed_product_adjacent_marginals, thm:mpdo_selected_fixed_product_positive_traces, thm:mpdo_cyclic_active_square_cube_adjacent_marginals}
...
\begin{theorem}[Markov cuts from the selected fixed product]
    ...
    \leanok
    \uses{def:injective, def:mpo_source_zcl, def:mpdo_eta_local_structure_data, thm:mpdo_fixed_bond_positive_physical_sector_representative}
    Let $K$ be an injective tensor with an eta-local structure and source zero
    correlation length. Express $K$ in the one-site physical coordinates of the
    positive fixed-product factorization selected by the eta-local structure.
    For arbitrary $A,C\geq0$, the normalized marginal on consecutive regions of
    lengths $A$, $1$, and $C$, obtained from the periodic chain of length
    $A+C+2$, admits a Hayashi Markov decomposition.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpdo_selected_fixed_product_cyclic_recurrence, thm:mpdo_selected_fixed_product_adjacent_marginals, thm:mpdo_selected_fixed_product_iterated_marginals, thm:mpdo_selected_fixed_product_positive_traces, thm:mpdo_selected_fixed_product_reduced_states, thm:mpdo_selected_fixed_product_square_cube, thm:mpdo_cyclic_active_square_positive_adjacent_marginals, thm:mpdo_cyclic_active_explicit_fourth_region_blocks, thm:mpdo_cyclic_active_markov_of_fourth_region, thm:matrix_primitive_pow_two_rank_one}
...
\begin{theorem}[Rank-one cyclic-active two-step coefficient]
    ...
    \leanok
    \uses{def:mpo_source_zcl, thm:mpdo_cyclic_active_trace_matrix_primitive, thm:matrix_primitive_pow_two_rank_one}
    Under the hypotheses of
    ...
    $(\widehat T^2)_{k,h}=\sum_{q\in C}\widehat T_{k,q}\widehat T_{q,h}$.
\end{theorem}

\begin{proof}\leanok
    Set the left sector tensor equal to zero outside $C$. The span
    argument of Theorem~\ref{thm:mpdo_cyclic_active_trace_matrix_primitive} shows
    that this does not change the physical tensor, and the
...
\begin{theorem}[Cyclic-active three-boundary trace coefficient]
    ...
    \leanok
    \uses{def:mpdo_cyclic_active_sector, thm:mpdo_sector_closure_partial_traces}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Here $\tr_{\mathrm{mid}}$ traces both neighboring factors.
...
\begin{proof}\leanok
    \uses{thm:mpdo_cyclic_active_three_boundary_trace, thm:mpdo_cyclic_active_deletion}
...
\begin{theorem}[Cyclic-active fourth-region factorization]
    ...
    \leanok
    \uses{thm:mpdo_source_zcl_iterated_marginal_stability, thm:mpdo_cyclic_active_explicit_fourth_region_blocks, thm:mpdo_cyclic_active_three_boundary_trace, thm:mpdo_cyclic_active_deletion}
...
\begin{theorem}[Cyclic-active Markov decomposition at every cut]
    ...
    \leanok
    \uses{thm:mpdo_cyclic_active_fourth_region_factorization, thm:mpdo_cyclic_active_markov_of_fourth_region}
    Under the hypotheses of
...
\begin{theorem}[Physical-sector factorization and ZCL imply SAL]
    ...
    \leanok
    \uses{thm:mpdo_cyclic_active_markov_all_cuts, thm:mpdo_all_cut_markov_implies_sal, thm:mpdo_sal_physical_isometry_transport}
    ...
    % operator, so it remains a conditional explicit-factorization result.
    % The source-faithful CPSV16 Proposition 4to2 is
...
\begin{proof}\leanok
    ...
    length makes its trace nonzero. Apply
    Theorem~\ref{thm:mpdo_cyclic_active_markov_all_cuts} with
...
\begin{theorem}[A translation-invariant commuting bond and ZCL imply SAL]
    ...
    \lean{MPOTensor.EtaLocalStructureData.isSAL_of_isSourceZCL}
    \leanok
    \uses{def:mpdo_eta_local_structure_data, def:mpo_source_zcl, def:is_sal, def:mpdo, def:injective}
    Let $\mathcal K$ be an injective tensor generating MPDOs. Suppose
    that there is one
    ...
    % **Local fix (cyclic-active two-step coefficient):** Source lines
    % 1613--1616 use an invalid one-step rank-one inference. The proof instead
    % uses the positive rank-one factorization of the cyclic-active two-step
    % coefficient and transports the normalized marginals from the selected
    % fixed-product tensor to the source tensor. Documented in
    % docs/paper-gaps/cpsv16_commuting_form_to_sal.tex.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpdo_selected_fixed_product_markov_cuts, thm:mpdo_all_cut_markov_implies_sal, thm:mpdo_sal_physical_isometry_transport, lem:mpdo_sitewise_physical_coordinate_congruence, thm:mpdo_source_zcl_strict_positive_length_normalization}
    Express $\mathcal K$ in the one-site physical coordinates selected by
    the fixed bond. Positivity of every periodic operator is preserved by
    this isometry, and its trace is unchanged, hence nonzero at every
    positive length by source zero correlation length.

    For every $1\leq m\leq\lfloor N/2\rfloor$, apply
    Theorem~\ref{thm:mpdo_selected_fixed_product_markov_cuts} with
    $A=m-1$ and $C=N-m-1$. Since
    $A+C+2=N$, the resulting decomposition is the quantum Markov
    decomposition required at the $m$-th cut. The all-cut criterion gives
    saturation of the area law in the selected physical coordinates.
    Finally, isometric invariance of the chain and marginal entropies
    transports saturation back to $\mathcal K$.
\end{proof}
...
\begin{theorem}[$\eta$-local structure gives the one-sector GSNNCH form]
    ...
    \leanok
    \uses{thm:mpdo_eta_local_structure_has_commuting_form, thm:mpdo_commuting_form_data_has_gsnnch_form}
...
\begin{proof}\leanok
    At each chain length, apply the one-sector construction directly to the
    commuting-form data carried by the $\eta$-local structure.
\end{proof}
...
\begin{theorem}[Injective SAL tensors generate GSNNCH states]
    ...
    \leanok
    \uses{def:is_sal, cor:mpdo_eta_local_structure_of_is_sal, thm:mpdo_eta_local_structure_has_gsnnch_form, thm:mpdo_gsnnch_of_sector_form}
...
\begin{remark}[Positive physical-sector factorization]
    ...
    The doubled-index transfer condition enters only in the subsequent
    renormalization fixed point and single-bond branch conclusions.
\end{remark}
