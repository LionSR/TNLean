...
make this precise and establish the quotient $H^2(G,\U(1))$.
...
\begin{proof}\leanok
    ...
    $X_2(g^{-1}) = f(g)\,X_1(g^{-1})$.
    Substituting into the projective multiplication law and cancelling
    the invertible matrix $X_1(g\cdot h)$ yields
...
\begin{proof}\leanok\uses{cor:ft_ti_chain}
    The SameMPV hypothesis on the combined tensors, combined with
    translation invariance, forces the per-site gauge matrices to
    collapse to a single matrix $X$ (by the TI chain gauge collapse).
    Injectivity of $B$ follows from gauge equivalence to the
    injective tensor $A$.
\end{proof}
...
\begin{theorem}[Projective multiplication law for gauge matrices]%
    ...
    existential without the nonzero assertion.)
\end{theorem}
