...
This chapter develops the one-dimensional MPS results of~\cite{Molnar2018NormalPEPS}: the Fundamental Theorem for injective
...
\begin{lemma}[Blocking preserves injectivity]\label{lem:blocking_preserves_injectivity}
    ...
    is again injective. (In Lean, this is formalized for the
    repackaged combined tensor \texttt{chainCombinedTensor}, not the
    physically blocked product tensor.)
\end{lemma}
...
\begin{lemma}[Virtual bond gauge -- Lemma~1 of \protect\cite{Molnar2018NormalPEPS}]%
    ...
    $\mathbf{B} = (B_0, \ldots, B_{n-1})$ be two injective non-TI
    periodic chains of length $n \ge 3$ with common bond dimension
    $D \ge 1$ that generate the same chain state.
    Fix a bond position $k \in \{0, \ldots, n-1\}$.
...
\begin{proof}
    ...
    $\MN{D}$ by Theorem~\ref{thm:physRealize_mul}.
...
\begin{remark}
    The Lean formalization currently proves a 3-site special case under
    the stronger \texttt{SameMPV} hypothesis; the general chain-length
...
\begin{lemma}[Aligned gauges force proportionality -- Lemma~2 of \protect\cite{Molnar2018NormalPEPS}]%
    ...
    \end{align}
    Diagrammatically, Eq.~\eqref{eq:internal_bond} is the equality obtained
    by inserting $X$ on the bond between the two sites,
    \[
        \TNInternalTraceInsertion{i}{j}{X}
    \]
    while Eq.~\eqref{eq:external_bond} is the corresponding equality with
...
\begin{proof}
    ...
    the conditions in Eqs.~\eqref{eq:internal_bond} and~\eqref{eq:external_bond}
    give
    \[
...
Lemma~\ref{lem:tensor_proportional} yields the main theorem of~\cite{Molnar2018NormalPEPS} for non-translation-invariant injective MPS.
...
\begin{proof}
    ...
    a two-site chain; the agreement on both bonds gives exactly the
    hypotheses of Eqs.~\eqref{eq:internal_bond}--\eqref{eq:external_bond}
    in Lemma~\ref{lem:tensor_proportional}, yielding
...
\begin{corollary}[TI case]\label{cor:ft_ti_chain}
    ...
    $(A, A, \ldots, A)$ and $(B, B, \ldots, B)$ of length $n \ge 3$
...
\begin{proof}
    ...
    two constant chains to obtain $Z_0, \ldots, Z_{n-1}$ with
...
\begin{corollary}[Non-TI realisation of a TI state]%
    ...
    \notready
    \uses{thm:fundamentalTheorem_injective_chain, def:non_ti_chain}
    Let $\mathbf{A} = (A_0, \ldots, A_{n-1})$ be an injective non-TI
    ...
    the TI chain $(B, \ldots, B)$ generates the same state as $\mathbf{A}$.
...
\begin{proof}
    \uses{thm:fundamentalTheorem_injective_chain}
    The cyclic shift $\sigma(\mathbf{A}) = (A_1, \ldots, A_{n-1}, A_0)$
    generates the same state by hypothesis.
    Apply Theorem~\ref{thm:fundamentalTheorem_injective_chain}:
    there exist invertible $Z_0, \ldots, Z_{n-1}$ with
...
\begin{corollary}[Normal MPS Fundamental Theorem]%
    ...
    matrices $Z_0, \ldots, Z_{n-1} \in \GL_D(\C)$, unique up to a
...
\begin{proof}
    ...
    invertible gauges $\widehat{Z}_0, \ldots, \widehat{Z}_{m-1}$ with
    ...
    gauges, yielding $Z_0, \ldots, Z_{n-1}$ for the original tensors.
\end{proof}

\begin{remark}
    The tagged Lean theorem proves gauge equivalence of blocked constant
    chains; the stronger result for original site tensors is future
    work.
\end{remark}
...
%% ---------------------------------------------------------
\section{Fundamental Theorem for injective PEPS}%
\label{sec:peps_ft}
%% ---------------------------------------------------------
