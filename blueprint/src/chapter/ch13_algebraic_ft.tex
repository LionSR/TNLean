...
\begin{remark}[A stronger equal-MPV variant]
    ...
    The paper closes this gap by a blocking argument when $n \ge 3$.
    In the present blueprint, that reduction is kept separate from the main
    chain theorem.
\end{remark}
...
\begin{definition}[Twisted transfer map]\label{def:twisted_transfer_map}
    ...
    box $u$ couples the physical legs.
\end{definition}
...
\begin{definition}[String order parameter]\label{def:string_order_param}
    ...
    \]
    The boundary matrix $\Lambda$ is contracted into the virtual boundary, and
    the right boundary is traced.
\end{definition}
...
\begin{proof}
    ...
    operators using C1.  After that, the physical $u$-boxes disappear by the
    unitary Kraus-mixing identity.
\end{proof}
...
\begin{proof}
    ...
    This step depends on the spanning property and on CP-map structure, and is
    deferred here.
\end{proof}
...
\begin{proof}
    ...
    \[
        \TNStringOrderParameter{u}{L}
    \]
    through the iterates of the local cell \(\TNTwistedTransfer{u}\).
\end{proof}
...
\begin{theorem}[Virtual unitary from string order]%
    ...
    Diagrammatically this is the same local intertwining move as C1,
...
\begin{proof}\leanok\uses{def:piAlgEquiv, thm:skolem_noether}
    ...
    it is inner. The tensor-network picture records the same split into
    permutation of block ideals followed by inner conjugation on each
    surviving block.
\end{proof}
...
\begin{theorem}[Multi-block Fundamental Theorem]%
    ...
    \]
    assembling the per-block gauges into one global block-diagonal gauge.
\end{theorem}
...
\begin{proof}\leanok\uses{cor:ft_ti_chain}
    One may package the constant chain into a combined tensor and obtain a
    single uniform gauge
    \[
        \TNGaugeConjugation{X}{(k,i)}{X^{-1}}
    \]
    on the combined-site index $(k,i)$.
    Reading this back on each site gives $B^i = X A^i X^{-1}$.
...
\begin{theorem}[TI reduction from same state]%
    ...
    bridge hypothesis is available to
...
\begin{theorem}[Projective multiplication law for gauge matrices]%
    ...
    $c(g,h) \in \C^{\times}$. Diagrammatically, both
    \[
