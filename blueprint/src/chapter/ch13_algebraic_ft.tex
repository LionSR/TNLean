...
\begin{lemma}[Block separation from canonical-form data and strict weights]%
    ...
    $\rho = |\mu_1|/|\mu_0| < 1$.
    This is a strict same-structure comparison theorem, not the full
    PGVWC07 canonical-form uniqueness theorem
...
\begin{lemma}[Block separation from separated canonical data]%
    ...
    Then $\mc{V}(A_k)=\mc{V}(B_k)$ for every block $k$.
    This statement assumes the separated block data needed for the local
    peeling argument; it is not the PGVWC07 theorem, whose hypotheses are
    C1, uniqueness of the OBC canonical representation, and
    $N>2L_0+D^4$
    \cite[Theorem~\texttt{thm-uniq}, lines~1002--1015]{PerezGarcia2007Matrix}.
\end{lemma}
...
\begin{lemma}[Gauge comparison from separated canonical data]%
    ...
    This is the gauge consequence of the same-structure separated-data
    comparison above, not the PGVWC07 uniqueness theorem
    \cite[Theorem~\texttt{thm-uniq}, lines~1002--1015]{PerezGarcia2007Matrix}.
\end{lemma}
...
\begin{lemma}[Explicit gauge from separated canonical data]%
    ...
    This is the explicit-matrix form of the preceding same-structure
    comparison theorem, not a formalization of PGVWC07
    Theorem~\texttt{thm-uniq}
...
\begin{lemma}[Gauge comparison from canonical-form data and strict weights]%
    ...
    The common block structure and strict weight ordering are hypotheses
    here; PGVWC07 derives uniqueness from C1, OBC uniqueness, and the
    finite-size bound instead
    \cite[Theorem~\texttt{thm-uniq}, lines~1002--1015]{PerezGarcia2007Matrix}.
\end{lemma}
...
\begin{lemma}[Explicit gauge from canonical-form data and strict weights]%
    ...
    $B_k^i = X_k\, A_k^i\, X_k^{-1}$
    for every block $k$ and physical index $i$.
    This is the explicit strict same-structure form of the preceding
    gauge comparison, not PGVWC07 Theorem~\texttt{thm-uniq}
    \cite[lines~1002--1015]{PerezGarcia2007Matrix}.
\end{lemma}
...
\begin{lemma}[Heterogeneous equal-case block matching]%
    ...
    gauge-equivalence statement for the block-diagonal tensors.  It is the
    one-copy-per-sector restricted surface of CPSV16 Corollary~\texttt{II\_cor2};
    the source corollary also recovers multiplicities and assembles the global
    gauge
...
The block-permutation gauge-phase conclusion (see
Definition~\ref{def:block_matching_gauge_phase_data}) provides per-block
...
proportionality matrix} of
\cite[Corollary~\texttt{II\_cor2} and equation~\texttt{eq:II\_auxcor},
lines~1173--1199]{Cirac2016MPDO_arXiv}.
...
\begin{theorem}[Global conjugation from per-block gauge-phase data]\label{thm:global_x_conj}
    ...
    \leanok
    \uses{thm:block_matching_globalX_conj, def:global_gauge_of_blocks,
        def:gauge_equiv}
