...
\begin{theorem}[Fundamental Theorem for injective chains (combined-tensor MPV version)]%
...
\begin{corollary}[Translation-invariant collapse to one gauge]\label{cor:ft_ti_chain}
    ...
    \]
    for every physical index~$i$.
    In the current Lean theorem one can take $\lambda = 1$.
\end{corollary}
...
\begin{corollary}[Non-translation-invariant realisation of a translation-invariant state]%
...
\begin{theorem}[Explicit per-block gauges]%
    ...
    Theorem~\ref{thm:fundamentalTheorem_multiBlock_full}, there exist
...
\begin{theorem}[Product-algebra decomposition of the multi-block theorem]%
    ...
    Under the hypotheses of
    Theorem~\ref{thm:fundamentalTheorem_multiBlock_full}, the product-algebra
...
\begin{theorem}[Heterogeneous equal-case Fundamental Theorem]%
    ...
    If the assembled block-diagonal tensors generate the same MPV family, then
