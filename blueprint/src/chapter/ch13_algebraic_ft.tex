...
\begin{proof}\leanok\uses{lem:perBlockLinearExtension_mul}
    ...
    automatically bijective: surjectivity follows from the simplicity
    of $\MN{D_k}$, and injectivity from finite dimension.
...
\begin{theorem}[Block-permutation decomposition]%
    ...
    \[
        T_{\sigma(k)}(M) = X_k\, M\, X_k^{-1}
    \]
    for all $M \in \MN{D_k}$ and all $k$.
\end{theorem}
...
\begin{theorem}[Block separation from canonical form]%
    ...
    \leanok
    \uses{def:canonical_form, def:same_mpv}
...
\begin{theorem}[Projective multiplication law for gauge matrices]%
    ...
    $\widetilde{A}_g^i = X(g)\, A^i\, X(g)^{-1}$ for all~$i$.
    Then for all $g, h \in G$, there exists $c(g,h) \in \C^{\times}$
    such that
    \[
