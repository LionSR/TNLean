...
\begin{proof}
    ...
    diagrammatically one isolates the relevant window and compares the
    same local insertion across the two blocked chains:
    \[
        \TNWindowHighlight{\sigma_{k-1}}{\sigma_{k+1}}{[k-1,k+1]}{n}
        \qquad
...
\begin{proof}
    ...
    After the bond gauges have been aligned, the ring is reduced to the
...
\begin{proof}\leanok\uses{def:piAlgEquiv, thm:skolem_noether}
    ...
    At the assembled level this is the direct-sum map
    \[
        \TNDirectSumAssembly{\bigoplus_k \MN{D_k}}{\bigoplus_k \MN{D_{\sigma(k)}}}{\operatorname{diag}(X_k)}.
    \]
...
\begin{theorem}[Multi-block Fundamental Theorem]%
    ...
    \[
        \TNDirectSumAssembly{\bigoplus_k \mu_k A_k}{\bigoplus_k \mu_k B_k}{\bigoplus_k X_k}
    \]
...
\begin{theorem}[Block separation from canonical form]%
    ...
    \[
        \TNBlockTailEstimate{\mu_0^N V(A_0)}{\rho^N}
    \]
...
\begin{proof}
    ...
    The weighted block sum is first compared as
    \[
        \TNSummedBlocks{\mu}{N}
    \]
    and then reduced to the leading-block estimate
    \[
        \TNBlockTailEstimate{\mu_0^N V(A_0)}{\rho^N}.
    \]
...
\begin{proof}\leanok\uses{cor:ft_ti_chain}
    ...
    Diagrammatically one first blocks the constant chain and then reads
    the same gauge back on each local leg:
    \[
        \TNBlockedChain{n}{N}
        \qquad
