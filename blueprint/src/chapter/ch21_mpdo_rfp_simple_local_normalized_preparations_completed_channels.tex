...
\begin{theorem}[Positivity of the direct-sum neighboring operator]
    ...
    \leanok
    \uses{def:mpdo_eta_omega, def:mpdo_explicit_eta_operators, thm:matrix_pos_semidef_block_diagonal}
...
\begin{theorem}[Vanishing of zero-weight neighboring operators]
    ...
    \leanok
    \uses{thm:mpdo_eta_omega_pos, thm:mpdo_eta_omega_trace, def:mpdo_eta_rank_one_trace_factorization}
...
\begin{theorem}[Positivity of normalized neighboring operators]
    ...
    \leanok
    \uses{def:mpdo_normalized_eta_omega, thm:mpdo_eta_omega_pos, thm:mpdo_eta_zero_weight}
...
\begin{theorem}[Normalized active preparations]
    ...
    \leanok
    \uses{def:mpdo_normalized_eta_omega, thm:mpdo_normalized_eta_omega_pos, thm:mpdo_eta_omega_trace}
...
\begin{proof}
    ...
    $\tr(\Omega_{k,h})=a_kb_h$.
    Apply Lemma~\cite[``State preparation is trace-preserving completely positive'']{QICLean2026Blueprint} to each operator.
\end{proof}
...
\begin{lemma}[Nonemptiness of the Hayashi sector factors]
    ...
    \leanok
    \uses{def:mpdo_eta_structure, lem:mpdo_trace_one_nonempty}
...
\begin{lemma}[Nonemptiness of the neighboring auxiliary spaces]
    ...
    \leanok
    \uses{def:mpdo_eta_omega, lem:mpdo_hayashi_factor_nonempty, lem:mpdo_hayashi_sector_nonempty}
...
\begin{definition}[Completed neighboring-state preparation]
    ...
    \leanok
    \uses{def:mpdo_normalized_eta_omega, lem:mpdo_hayashi_auxiliary_nonempty, def:mpdo_faithful_uniform_density}
...
\begin{theorem}[Completed neighboring-state preparations are trace-preserving
    ...
    \leanok
    \uses{def:mpdo_completed_eta_preparation, thm:mpdo_normalized_eta_omega_preparation, thm:mpdo_faithful_uniform_density}
...
\begin{theorem}[Completed direct-sum preparations are trace-preserving
    ...
    \leanok
    \uses{def:mpdo_completed_omega_preparation, thm:mpdo_normalized_eta_omega_preparation, thm:mpdo_faithful_uniform_density}
...
\begin{theorem}[Diagonal-sector action of the completed neighboring preparation]
    ...
    \leanok
    \uses{def:mpdo_completed_eta_controlled_map, def:mpdo_completed_eta_preparation}
...
\begin{theorem}[The globally controlled neighboring-state preparation is
    ...
    \leanok
    \uses{def:mpdo_completed_eta_controlled_map, thm:mpdo_completed_eta_preparation_cptp}
...
\begin{theorem}[Diagonal-sector action of the completed direct-sum preparation]
    ...
    \leanok
    \uses{def:mpdo_completed_omega_controlled_map, def:mpdo_completed_omega_preparation}
    If $X_{k,h}$ is the $(k,h)$ diagonal block of $X$, then
    \begin{align}
...
\begin{theorem}[The globally controlled direct-sum preparation is
    ...
    \leanok
    \uses{def:mpdo_completed_omega_controlled_map, thm:mpdo_completed_omega_preparation_cptp}
...
\begin{remark}[Scope of the completed preparation]
    ...
    factorization for the inverse-map neighboring operators from saturation
    of the area law and zero correlation length is the Perron--Frobenius step
    asserted in
    \cite[Appendix~C.2, lines~1404--1410 and~1484--1498]
    {Cirac2016MPDO_arXiv}. Theorem~\ref{thm:mpdo_sal_zcl_bnt_sector_structure}
    shows that the Case-II representativewise reuse of this factorization in
    Proposition~C.12 is false under the complete standing hypotheses; it does
    not refute Lemma~C.5 on the normal Case-I surface (see
    \cite{gap:cpgsv17_pf_rank_one}).
    The completed maps therefore provide the global controlled preparation once
    that factorization is supplied, with the regrouping maps, shift maps,
    and closure identities developed in subsequent sections to complete the
    two-map statement of
    \cite[Appendix~C.2, lines~1510--1517]{Cirac2016MPDO_arXiv}.
\end{remark}
