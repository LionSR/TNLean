...
\begin{theorem}[Cluster three-site parent space is the stabilizer eigenspace]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    the $+1$ eigenspace of $K = \sigma^z_1\sigma^x_2\sigma^z_3$. Hence the
    three-site parent interaction, the orthogonal projector with kernel
    $\mathcal G_3(A)$, is the projector onto the $-1$ eigenspace of $K$,
    namely $h^{ZXZ} = \tfrac12(\Id - K)$
    \cite[local TeX lines~374--387]{PerezGarcia2007Matrix}.
\end{theorem}
...
\begin{theorem}[Cluster states are unique ground states of the $ZXZ$ interactions]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    so the cluster state is the unique ground state of
    $\sum_i \tfrac12(\Id - \sigma^z_i\sigma^x_{i+1}\sigma^z_{i+2})$
    \cite[local TeX lines~374--387]{PerezGarcia2007Matrix}.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:cluster_stabilizer_chain_ground_space, thm:cluster_normal, thm:parent_hamiltonian_unique_gs}
    ...
    Theorem~\ref{thm:parent_hamiltonian_unique_gs} with $L = 3 \le N$ gives
    $\mathcal G_{N,3}(A_{\mathrm{cl}}) = \C\,V^{(N)}(A_{\mathrm{cl}})$.
\end{proof}
...
\begin{remark}[Cluster-state stabilizer convention]\label{rem:cluster_stabilizer_convention}
    ...
    convention is the $+1$ eigenspace of $\sigma^z_1\sigma^x_2\sigma^z_3$, so
    the $ZXZ$ stabilizer of \cite[local TeX lines~374--387]{PerezGarcia2007Matrix}
    applies to both without any local-basis change.
\end{remark}
...
\begin{theorem}[AKLT two-site parent space]\label{thm:aklt_ground_space_two}
    ...
    \end{align}
    Thus $\mathcal G_2(A)$ is the four-dimensional span of
    $\ket{01}-\ket{10}$, $\ket{02}-\ket{20}$, $\ket{00}-2\ket{12}$, and
    $\ket{00}-2\ket{21}$.
    % This span is the spin-0 and spin-1 subspace of two spin-1 particles;
    % that identification is not part of the formal statement.
\end{theorem}
...
\begin{proof}
    \leanok
    \uses{thm:aklt_ground_space_intersection_two, thm:aklt_normal, thm:parent_hamiltonian_unique_gs}
