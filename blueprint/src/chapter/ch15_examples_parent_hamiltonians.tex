...
\begin{theorem}[Periodic ground space for the source cluster matrices]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This equality alone does not assert that the space is one-dimensional.
\end{theorem}
...
\begin{proof}
    ...
    By Theorem~\ref{thm:cluster_source_ground_space_three_eigenspace},
    the local $-1$ eigenspace is $\mathcal G_3(A_{\mathrm{src}})$.
    Thus $K_i\psi=-\psi$ for every $i$ exactly when every three-site
