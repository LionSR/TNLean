...
Following the parent-Hamiltonian construction of
\cite[lines~1988--2000]{Cirac2021Matrix} reviewed in
Chapter~\ref{ch:parent_hamiltonian}, originally introduced for finitely
correlated states by Fannes, Nachtergaele, and Werner~\cite{Fannes1992Finitely}
and developed for matrix product states by Perez-Garcia et
al.~\cite{PerezGarcia2007Matrix}, the range-$L$ local parent space is
\begin{align}
    \mathcal G_L(A)
     & =
    ...
    \notag
\end{align}
Then $\ker h_L = \mathcal G_L(A)$, the range-$L$ parent Hamiltonian is
$H_N^{(L)} = \sum_i (h_L)_i$, and $\GS(H)=\ker(H-E_0(H)\Id)$ denotes the ground
eigenspace.
...
\begin{definition}[Translated cluster stabilizers]\label{def:cluster_chain_stabilizer}
    ...
    \leanok
    On a periodic chain of $N$ sites, the stabilizer at site $i$ is the
    operator
    \begin{align}
        (K_i\psi)(\sigma)
        ...
        \notag
    \end{align}
    with site indices modulo $N$. For $N\ge3$ the sites $i$, $i+1$, $i+2$
    are distinct and $K_i = \sigma^z_i\sigma^x_{i+1}\sigma^z_{i+2}$; the
    results below assume $N\ge3$.
\end{definition}
...
\begin{proof}
    \leanok
    \uses{thm:cluster_ground_space_three_eigenspace, rem:cyclic_window_operators}
...
\begin{proof}
    ...
    \end{align}
    For a configuration $s=(s_0,\ldots,s_{N-1})$, adjacent inverse factors
    cancel in the ordered product:
    \begin{align}
        A_{\mathrm{src}}^{s_0}\cdots A_{\mathrm{src}}^{s_{N-1}}
         & =(\sqrt2)^N(-1)^{\sum_j s_j}
        G\bigl(A_{\mathrm{cl}}^{s_0}\cdots A_{\mathrm{cl}}^{s_{N-1}}\bigr)G^{-1}.
        \label{eq:cluster_source_product}
    \end{align}
    Taking the trace in (\ref{eq:cluster_source_product}) and using
    $\operatorname{tr}(GBG^{-1})=\operatorname{tr}(B)$ gives
    \begin{align}
        V^{(N)}(A_{\mathrm{src}})_s
         & =\operatorname{tr}\bigl(A_{\mathrm{src}}^{s_0}\cdots A_{\mathrm{src}}^{s_{N-1}}\bigr)\notag \\
         & =(\sqrt2)^N(-1)^{\sum_j s_j}
        \operatorname{tr}\bigl(A_{\mathrm{cl}}^{s_0}\cdots A_{\mathrm{cl}}^{s_{N-1}}\bigr)\notag       \\
    ...
    % The formal argument below uses norm preservation and the real quadratic
    % form; it does not invoke a separately formalized Hermiticity theorem.
...
\begin{remark}[Cluster-state stabilizer convention]\label{rem:cluster_stabilizer_convention}
    ...
    and eigenspace for the printed matrices; see
...
\begin{remark}[AKLT polynomial Hamiltonian and edge modes]\label{rem:aklt_parent_hamiltonian}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    up to an additive constant~\cite{AKLT1987Rigorous,AKLT1988ValenceBond};
    see also \cite[local TeX lines~340--349]{PerezGarcia2007Matrix} and
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    \cite[lines~1169--1174]{Cirac2021Matrix}; see
    \cite{gap:rmp_example_parent_hamiltonian_scope}.
