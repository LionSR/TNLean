...
\begin{proof}
    \leanok
    \uses{def:cp_map, def:irreducible_cp}
    Choose Kraus operators $K_i$ such that
    $E(X)=\sum_iK_iXK_i^\dagger$. Suppose $\rho$ is not positive definite,
    and let $Q$ be its support projection. Since $\rho=Q\rho Q$ and
    $\rho=E(\rho)$, the $(\Id-Q)$ corner satisfies
    \begin{align}
        0
         & =(\Id-Q)\rho(\Id-Q) \notag \\
         & =\sum_i
        \bigl((\Id-Q)K_iQ\bigr)\rho
        \bigl((\Id-Q)K_iQ\bigr)^\dagger.
        \notag
    \end{align}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Thus $K_iQ=QK_iQ$, and for every $X$ each summand
    $K_iQXQK_i^\dagger$ lies in the $Q$ corner. Therefore
    \begin{align}
        QE(QXQ)Q
         & =E(QXQ),
        \notag
