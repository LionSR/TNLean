...
Quantum Perron--Frobenius theory governs the fixed points and spectral structure
of positive and completely positive maps. For injective and irreducible transfer
maps, positive fixed points are unique up to scaling and strictly positive
definite, determining the canonical gauges. The ergodicity of irreducible
channels and the spectral characterization of irreducibility are treated in the
companion volume~\cite[``Ces\`aro convergence for irreducible channels'' and
    ``Irreducibility iff the restricted CP spectral
    properties hold'']{QICLean2026Blueprint}, following
Wolf~\cite[Chapter~6]{Wolf2012Quantum} and Evans~\cite{Evans1978Spectral}.
...
\begin{theorem}[Square-root trace-preserving gauge from an adjoint eigenvector]%
    ...
    \lean{Kraus.tpGauge_isTP_of_map_conjTranspose_eigenvector}
    \leanok
...
\begin{remark}\leanok
    The right-canonical (unital) gauge and the left-canonical (trace-preserving)
    gauge arise from different similarity transformations: the former uses a fixed
    point of the transfer map $\E_A$, whereas the latter uses a fixed point of the
    adjoint map $\E_A^\dagger$. In general, a transfer map cannot be brought into
    simultaneously unital and trace-preserving form by a single similarity transform.
\end{remark}
...
Every irreducible MPS tensor admits a positive definite eigenvector for its
adjoint transfer map with positive eigenvalue, which yields a trace-preserving
(left-canonical) gauge. The underlying fixed-point and positive-definiteness
theorems follow Wolf~\cite[Theorems~6.3 and 6.5]{Wolf2012Quantum} and
Evans~\cite{Evans1978Spectral}; the gauge construction follows
Cirac et al.~\cite[Appendix~A]{Cirac2016MPDO_arXiv}.
...
\begin{theorem}[TP-gauge existence for irreducible tensors]%
    ...
    This corresponds to the spectral rescaling and trace-preserving gauge
    construction of \cite[Appendix~A]{Cirac2016MPDO_arXiv}.
\end{theorem}
...
\begin{theorem}[Unital-gauge existence for irreducible tensors]%
    ...
    This corresponds to the Perron--Frobenius unital gauge of
    \cite[Theorem~4]{PerezGarcia2007Matrix} for an irreducible block.
\end{theorem}
