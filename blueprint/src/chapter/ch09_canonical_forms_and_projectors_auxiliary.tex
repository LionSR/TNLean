...
\begin{proof}\leanok
    ...
    Follow the proof of \cite[Theorem~4]{PerezGarcia2007Matrix}, beginning with
    the identically-zero case. If every positive-length MPV coefficient
    vanishes, return the empty retained family; all block conditions and the
    bond-dimension bound are then vacuous. Otherwise some length~$N$ has a
    nonzero coefficient, and therefore
    \begin{align}
        \Tr(\E_A^N)
        ...
        \notag
    \end{align}
    Thus $\E_A$ has a nonzero eigenvalue and positive spectral radius. Only in
    this nonzero branch do we divide the tensor by the square root of that
    radius.
    Perron--Frobenius then supplies a positive semidefinite fixed point. If it
    is full rank, conjugation by its positive square root gives the unital
    gauge
    \begin{align}
