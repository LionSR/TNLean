...
\begin{theorem}[Normalized exact form for a nonzero MPV family]%
    ...
    Suppose that $V^{(N)}(A)_\sigma\neq0$ for some $N>0$ and some closed
...
\begin{proof}\leanok
    \uses{thm:fp_inv_proj, thm:two_block_decomp,
        ...
        thm:irreducible_unital_scalar_fixed_point,
        thm:pgvwc07_dual_diag_unital}
    Follow the proof of \cite[Theorem~4]{PerezGarcia2007Matrix}. The nonzero
    coefficient gives
    \begin{align}
        \Tr(\E_A^N)
         & =\sum_\sigma\left|V^{(N)}(A)_\sigma\right|^2
        >0,
        \notag
    \end{align}
    so $\E_A$ has positive spectral radius. Divide the tensor by the square
    root of this radius. Perron--Frobenius supplies a positive semidefinite
    fixed point. If it is full rank, conjugation by its positive square root
    gives the unital gauge
    \begin{align}
        \sum_iA^i(A^i)^\dagger
         & =\Id.
        \notag
    \end{align}
    If the fixed point is singular, positivity makes its support invariant:
    ...
    induction stops only when every block has fixed-point space $\C\Id$.
    Apply the Perron argument to each dual map and diagonalize its
    positive-definite fixed point by a unitary. This preserves the unital
    identity and gives
    (\ref{eq:canonical_pgvwc_nonzero_block_conditions}). Finally absorb the
    positive spectral-radius factors into the weights and divide all weights
    by their largest modulus. This gives $|\mu_k|\le1$, a unit-modulus weight,
...
\begin{theorem}[Zero/nonzero alternative for the normalized exact form]%
    ...
    \begin{enumerate}
        \item $V^{(N)}(A)_\sigma=0$ for every $N>0$ and every closed
              word~$\sigma$;
...
interface below. The renormalization fixed-point analysis uses a separate
