...
\begin{theorem}[Equal BNT canonical forms are globally conjugate]%
    ...
    Compare \cite[Corollary~2.11]{Cirac2016MPDO_arXiv} and
    \cite[Corollary~IV.5]{Cirac2021Matrix}.  The BNT class in
    Definition~\ref{def:sector_bnt_cf} requires every basis representative to
    occur among the canonical-form summands with positive copy count and
    nonzero copy weights.  If an unrelated normal tensor is instead allowed to
    occur with coefficient zero at every positive length, it can change the
    ambient bond dimension without changing any positive-length MPV.  The
    dimension equality and global conjugacy follow under the stated hypotheses.
\end{theorem}
