...
% Gap: docs/paper-gaps/cpsv16_bnt_uniqueness_zero_coefficient.tex.
\begin{theorem}[Printed proportional fundamental theorem]
    ...
    The statement is false if inactive sectors are admitted.  Adjoining a
    normal tensor with coefficient zero changes the basis cardinality without
    changing the tensor or any positive-length matrix product vector.  For
    active nonzero sector data, the asserted matching follows from
...
% Provenance: source-label audit completed in issue #5917.
% Gaps: docs/paper-gaps/cpsv16_bnt_characterization_active_blocks.tex and
...
\begin{corollary}[Printed Canonical Form II unitary refinement]
    ...
    Canonical Form II does not exclude inactive zero-weight blocks, so the
    literal equal-MPV conclusion can still fail at the bond-dimension and
    global-unitary level.  After restricting to active nonzero sectors, the
    proportional sector gauges and the equal global gauge may be chosen
    unitary, as stated in the two corollaries below.
\end{corollary}
...
\begin{corollary}[Printed equal-MPV fundamental theorem]
    ...
    An inactive zero-weight block may be added to either tensor without
    changing any positive-length matrix product vector.  It changes the ambient
    bond dimension and therefore invalidates the literal dimension and global
    conjugacy conclusion.  For active nonzero sector data, the conclusion holds
    by Theorem~\ref{thm:cpgsv_equal_case_source}.
\end{corollary}
