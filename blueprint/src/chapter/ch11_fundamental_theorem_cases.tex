...
\cite[Theorem~2.10]{Cirac2016MPDO_arXiv}; see also
\cite[Theorem~IV.4 and Corollary~IV.5]{Cirac2021Matrix}.  The normalized,
left-canonical BNT surface of Definition~\ref{def:sector_bnt_cf} excludes an
unrelated normal representative whose coefficient vanishes at every length.
The literal BNT definition in the first reference does not exclude such a
representative, so its unrestricted cardinality conclusion requires this
additional converse-coverage condition.
