...
% CPSV16 Theorem 2.10 (`thm1`), lines 349--352 and Appendix A restatement
% at lines 1167--1170.
...
\begin{theorem}[Printed proportional fundamental theorem]
    \label{thm:cpsv_theorem210_printed_status}
    \lean{MPSTensor.fundamentalTheorem_proportional_canonicalForm}
    \leanok
    \uses{def:bnt_cpsv, def:cpsv_canonical_form, def:gauge_phase_equiv, thm:cpgsv_multiblock_ft_source}
    Let $A$ and $B$ be tensors in canonical form whose matrix product vectors
    are proportional at every positive length.  According to
    \cite[Theorem~2.10]{Cirac2016MPDO_arXiv}, arbitrary chosen bases of normal
    tensors for $A$ and $B$ have the same cardinality and are related by a
    bijective family of gauge-phase equivalences.

    Here the canonical-form decomposition is read with every coefficient
    $\mu_k$ nonzero, the convention implicit in the source normalization
    $|\mu_k|\le 1$ with at least one coefficient equal to one: a summand with
    coefficient zero contributes nothing to any matrix product vector and is
    not a block of the decomposition.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:cpgsv_multiblock_ft_source}
    With nonzero coefficients the canonical forms are BNT canonical forms, and
    the statement is Theorem~\ref{thm:cpgsv_multiblock_ft_source}.
\end{proof}
...
\begin{theorem}[Multi-block fundamental theorem for normal tensors]%
    ...
    \lean{MPSTensor.fundamentalTheorem_proportional_canonicalForm}
    \leanok
    \uses{def:sector_bnt_cf, def:gauge_phase_equiv, thm:sector_bnt_proportional_sector_match_witnesses}
    Let $P$ and $Q$ be BNT canonical forms (Definition~\ref{def:sector_bnt_cf}),
    with bases of normal tensors $(P_j)_{j=1}^{g_a}$ and $(Q_k)_{k=1}^{g_b}$.
    Every listed sector has a positive number of copies, and every copy weight
    is nonzero. Thus no normal tensor with identically zero coefficient is
    included in either basis. If the total tensors $P_{\mathrm{tot}}$,
    $Q_{\mathrm{tot}}$ generate eventually
...
\begin{lemma}[Phase normalization for an irreducible gauge]
    ...
    \leanok
    \uses{def:left_canonical_tensor, def:irreducible_tensor, thm:eigenvalue_unique_irred}
...
\begin{lemma}[Unitary replacement with the same phase]
    ...
    \leanok
    \uses{def:left_canonical_tensor, def:irreducible_tensor, lem:left_canonical_irreducible_phase_norm, thm:psd_fp_unique_irred}
...
% CPSV16 Corollary A.6 (`thm:Fundamental-CFII`), lines 1197--1199.
% Provenance: source-label audit completed in issue #5917.
% Local fix (nonzero coefficients): docs/paper-gaps/cpsv16_bnt_characterization_active_blocks.tex and
% docs/paper-gaps/cpsv16_global_vs_persector_unit_witness.tex.
\begin{corollary}[Printed Canonical Form II unitary refinement]
    \label{cor:cpsv_corollary_a6_printed_status}
    \lean{MPSTensor.ft_sector_bnt_proportional_unitary_sector_match_witnesses,
        MPSTensor.fundamentalTheorem_equal_canonicalForm_unitary}
    \leanok
    \uses{def:cpsv_cfii, thm:cpsv_theorem210_printed_status, cor:cpsv_corollary211_printed_status}
    According to \cite[Corollary~A.6]{Cirac2016MPDO_arXiv}, the gauge matrices
    in Theorem~2.10 and Corollary~2.11 may be chosen unitary when both tensors
    are in Canonical Form II.

    With the canonical forms read with nonzero coefficients, the proportional
    sector gauges and the equal global gauge may be chosen unitary, as stated
    in the two corollaries below.
\end{corollary}

\begin{proof}\leanok
    \uses{cor:sector_bnt_proportional_unitary_sector_match, cor:sector_bnt_equal_unitary_global_gauge}
    These are the two corollaries below.
\end{proof}
...
\begin{corollary}[Canonical Form II unitary refinement of proportional matching]%
    ...
    Theorem~\ref{thm:cpgsv_multiblock_ft_source}, all sectors are active and
    all copy weights are nonzero. There exist a bijection
...
\begin{proof}\leanok
    \uses{thm:sector_bnt_proportional_sector_match_witnesses, lem:left_canonical_irreducible_unitary_gauge}
...
% CPSV16 Corollary 2.11 (`II_cor2`), lines 354--361 and Appendix A
% restatement at lines 1172--1179.
% Provenance: source-label audit completed in issue #5917.
% Local fix (nonzero coefficients): docs/paper-gaps/cpsv16_bnt_characterization_active_blocks.tex.
\begin{corollary}[Printed equal-MPV fundamental theorem]
    \label{cor:cpsv_corollary211_printed_status}
    \lean{MPSTensor.fundamentalTheorem_equal_canonicalForm}
    \leanok
    \uses{thm:cpsv_theorem210_printed_status, def:cpsv_canonical_form, thm:cpgsv_equal_case_source}
    According to \cite[Corollary~2.11]{Cirac2016MPDO_arXiv}, two
    canonical-form tensors generating the same matrix product vectors at every
    positive length have equal bond dimensions and are conjugate by one
    invertible matrix.

    The canonical forms are read with nonzero coefficients, as in
    Theorem~\ref{thm:cpsv_theorem210_printed_status}.
\end{corollary}

\begin{proof}\leanok
    \uses{thm:cpgsv_equal_case_source}
    This is Theorem~\ref{thm:cpgsv_equal_case_source}.
\end{proof}
...
\begin{theorem}[Equal BNT canonical forms are globally conjugate]%
    ...
    MPV family for every positive system size~$N$. Every listed sector has a
    positive number of copies and every copy weight is nonzero, so inactive
    zero-weight summands are excluded. Then their total bond
...
\begin{corollary}[Canonical Form II unitary refinement in the equal case]%
    ...
    \leanok
    \uses{lem:sector_bnt_equal_unitary_matched_gauge, thm:sector_bnt_equal_mps_gaugeEquiv_literal}
    Let $P$ and $Q$ be BNT canonical forms whose total tensors generate the
    same MPV family at every positive length. Assume every listed sector has a
    positive number of copies and every copy weight is nonzero, as in
    Definition~\ref{def:sector_bnt_cf}; the literal zero-weight CFII statement
    is not included. Their total bond dimensions
