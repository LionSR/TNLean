\section{The proportional and equal cases}
\label{sec:ft_equal_proportional_cases}

% CPSV16 Theorem 2.10 (`thm1`), lines 349--352 and Appendix A restatement
% at lines 1167--1170.
\begin{theorem}[Printed proportional fundamental theorem]
    \label{thm:cpsv_theorem210_printed_status}
    \notready
    \discussion{5917}
    \uses{def:bnt_cpsv, def:cpsv_canonical_form}
    Let $A$ and $B$ be tensors in canonical form whose matrix product vectors
    are proportional at every positive length.  Theorem~2.10 of
    \cite{Cirac2016MPDO_arXiv} asserts that arbitrary chosen bases of normal
    tensors for $A$ and $B$ have the same cardinality and are related by a
    bijective family of gauge-phase equivalences.

    The literal statement is false.  The source definition permits adjoining
    to a basis a normal tensor with coefficient zero at every positive length;
    this changes the basis cardinality without changing the tensor or any
    matrix product vector.  The corrected theorem requires active, nonzero
    sector data and is Theorem~\ref{thm:cpgsv_multiblock_ft_source}.  The
    counterexample is recorded in
    \path{docs/paper-gaps/cpsv16_bnt_uniqueness_zero_coefficient.tex}.
\end{theorem}

\begin{theorem}[Multi-block fundamental theorem for normal tensors]%
    \label{thm:cpgsv_multiblock_ft_source}
    \lean{MPSTensor.fundamentalTheorem_proportional_canonicalForm}
    \leanok
    \uses{def:sector_bnt_cf, def:gauge_phase_equiv,
        thm:sector_bnt_proportional_sector_match_witnesses}
    Let $P$ and $Q$ be BNT canonical forms (Definition~\ref{def:sector_bnt_cf}),
    with bases of normal tensors $(P_j)_{j=1}^{g_a}$ and $(Q_k)_{k=1}^{g_b}$.
    Every listed sector has a positive number of copies, and every copy weight
    is nonzero. Thus no normal tensor with identically zero coefficient is
    included in either basis. If the total tensors $P_{\mathrm{tot}}$,
    $Q_{\mathrm{tot}}$ generate eventually
    nonzero proportional MPV families --- for all sufficiently large $N$ there is
    a nonzero scalar $c_N$ with
    $V^{(N)}(P_{\mathrm{tot}})=c_NV^{(N)}(Q_{\mathrm{tot}})$ --- then
    $g_a=g_b$. After relabelling the basis of normal tensors of $P$ by a
    bijection $\beta:J(Q)\simeq J(P)$, there are scalars $\zeta_k\in\C$ with
    $|\zeta_k|=1$ and invertible matrices $Y_k$ such that, for every $k$ and
    every physical index~$i$,
    \begin{align}
        Q_k^i
         & =\zeta_k\,Y_k\,P_{\beta(k)}^i\,Y_k^{-1}. \notag
    \end{align}
    Thus the paired normal tensors have the same bond dimension
    ($\dim P_{\beta(k)}=\dim Q_k$) and the same gauge-phase class.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:sector_bnt_proportional_sector_match_witnesses}
    Apply Theorem~\ref{thm:sector_bnt_proportional_sector_match_witnesses}.
    Its partition-and-substitution argument gives a bijection
    $\beta:J(Q)\simeq J(P)$, equal bond dimensions, invertible block gauges
    $X_k$, and phases $\zeta_k$ of unit modulus. Taking $Y_k=X_k$ gives exactly
    the asserted relations.
\end{proof}

% Compare \cite[Theorem~2.10]{Cirac2016MPDO_arXiv} and
% \cite[Theorem~IV.4]{Cirac2021Matrix}.  In
% Definition~\ref{def:sector_bnt_cf}, every basis representative occurs among the
% canonical-form summands, has positive copy count, and has nonzero copy weights.
% These hypotheses exclude an unrelated normal tensor whose coefficient is zero
% at every length.  If such a tensor is admitted, it can be adjoined without
% changing any MPV, and equality of the two basis cardinalities need not follow.

\subsection{Unitary gauges on normalized sectors}
\label{subsec:ft_unitary_sector_gauges}

\begin{lemma}[Phase normalization for an irreducible gauge]
    \label{lem:left_canonical_irreducible_phase_norm}
    \lean{MPSTensor.gaugePhase_scalar_norm_eq_one_of_leftCanonical_irreducible}
    \leanok
    \uses{def:left_canonical_tensor, def:irreducible_tensor,
        thm:eigenvalue_unique_irred}
    Let $A$ and $B$ be left-canonical tensors of common positive bond
    dimension, with $B$ irreducible. If $X$ is invertible, $\zeta\ne0$, and
    $B^i=\zeta XA^iX^{-1}$ for every $i$, then $|\zeta|=1$.
\end{lemma}

\begin{proof}\leanok
    \uses{thm:eigenvalue_unique_irred}
    Let $\rho$ be a nonzero positive fixed point of $\E_A$ and set
    $\sigma=X\rho X^\dagger$. Then $\sigma$ is nonzero and positive, and
    $\E_B(\sigma)=|\zeta|^2\sigma$. The map $\E_B$ also has a nonzero positive
    fixed point. Its irreducibility therefore forces $|\zeta|^2=1$.
\end{proof}

\begin{lemma}[Unitary replacement with the same phase]
    \label{lem:left_canonical_irreducible_same_phase_unitary_gauge}
    \lean{MPSTensor.exists_unitaryConj_of_gaugePhase_data_of_leftCanonical_irreducible}
    \leanok
    \uses{def:left_canonical_tensor, def:irreducible_tensor,
        lem:left_canonical_irreducible_phase_norm, thm:psd_fp_unique_irred}
    Let $A$ and $B$ be left-canonical irreducible tensors of common positive
    bond dimension satisfying $B^i=\zeta XA^iX^{-1}$ for an invertible $X$ and
    a nonzero scalar $\zeta$. There is a unitary matrix $U$, with the same scalar
    $\zeta$, such that $|\zeta|=1$ and $B^i=\zeta UA^iU^\dagger$ for every $i$.
\end{lemma}

\begin{proof}\leanok
    \uses{lem:left_canonical_irreducible_phase_norm, thm:psd_fp_unique_irred}
    Lemma~\ref{lem:left_canonical_irreducible_phase_norm} gives
    $|\zeta|=1$. Substitution into the left-canonical identities gives
    \begin{align}
        \sum_i A^{i\dagger}(X^\dagger X)A^i & =X^\dagger X. \notag
    \end{align}
    Thus $X^\dagger X$ is a positive-semidefinite fixed point of the adjoint
    transfer map of $A$. Irreducibility gives $X^\dagger X=c\Id$ for some
    $c>0$, so $X^{-1}=c^{-1}X^\dagger$ and $U=c^{-1/2}X$ is unitary. Moreover,
    \begin{align}
        UA^iU^\dagger
         & =c^{-1}XA^iX^\dagger
        =XA^iX^{-1}. \notag
    \end{align}
\end{proof}

% CPSV16 Corollary A.6 (`thm:Fundamental-CFII`), lines 1197--1199.
\begin{corollary}[Printed Canonical Form II unitary refinement]
    \label{cor:cpsv_corollary_a6_printed_status}
    \notready
    \discussion{5917}
    \uses{thm:cpsv_theorem210_printed_status,
        cor:cpsv_corollary211_printed_status}
    Corollary~A.6 of \cite{Cirac2016MPDO_arXiv} asserts that the gauge matrices
    in Theorem~2.10 and Corollary~2.11 may be chosen unitary when both tensors
    are in Canonical Form II.

    Canonical Form II does not remove inactive zero-weight blocks.  The literal
    equal-MPV conclusion therefore still fails at the bond-dimension and
    global-unitary level.  The proportional sector gauges and the equal global
    gauge are unitary on the corrected active, nonzero surface; these are the
    two results below.  The source boundary is recorded in
    \path{docs/paper-gaps/cpsv16_bnt_characterization_active_blocks.tex} and
    \path{docs/paper-gaps/cpsv16_global_vs_persector_unit_witness.tex}.
\end{corollary}

\begin{corollary}[Canonical Form II unitary refinement of proportional matching]%
    \label{cor:sector_bnt_proportional_unitary_sector_match}
    \lean{MPSTensor.ft_sector_bnt_proportional_unitary_sector_match_witnesses}
    \leanok
    \uses{def:sector_bnt_cf, thm:cpgsv_multiblock_ft_source}
    Under the hypotheses of
    Theorem~\ref{thm:cpgsv_multiblock_ft_source}, all sectors are active and
    all copy weights are nonzero. There exist a bijection
    $\beta:J(Q)\simeq J(P)$, unitary matrices
    $U_k\in\mathrm{U}(\dim Q_k)$, and phases $\zeta_k\in\C$ with
    $|\zeta_k|=1$, such that $\dim P_{\beta(k)}=\dim Q_k$ and, for every $k$
    and every physical index~$i$,
    \begin{align}
        Q_k^i
         & =\zeta_k\,U_k\,P_{\beta(k)}^i\,U_k^\dagger. \notag
    \end{align}
    This is the proportional part of
    \cite[Corollary~A.6, lines~1197--1199]{Cirac2016MPDO_arXiv}.
\end{corollary}

\begin{proof}\leanok
    \uses{thm:sector_bnt_proportional_sector_match_witnesses,
        lem:left_canonical_irreducible_unitary_gauge}
    The proportional sector-matching theorem gives a bijection $\beta$,
    invertible matrices $X_k$, and unit-modulus phases $\zeta_k$ with
    $Q_k^i=\zeta_kX_kP_{\beta(k)}^iX_k^{-1}$. The matched tensors are
    left-canonical and irreducible. Hence
    Lemma~\ref{lem:left_canonical_irreducible_unitary_gauge} gives unitary
    matrices $U_k$ satisfying
    $X_kP_{\beta(k)}^iX_k^{-1}=U_kP_{\beta(k)}^iU_k^\dagger$, and therefore
    $Q_k^i=\zeta_kU_kP_{\beta(k)}^iU_k^\dagger$.
\end{proof}

\subsection{The equal case}
\label{subsec:ft_equal_case}

% CPSV16 Corollary 2.11 (`II_cor2`), lines 354--361 and Appendix A
% restatement at lines 1172--1179.
\begin{corollary}[Printed equal-MPV fundamental theorem]
    \label{cor:cpsv_corollary211_printed_status}
    \notready
    \discussion{5917}
    \uses{thm:cpsv_theorem210_printed_status, def:cpsv_canonical_form}
    Corollary~2.11 of \cite{Cirac2016MPDO_arXiv} states that two canonical-form
    tensors generating the same matrix product vectors at every positive
    length have equal bond dimensions and are conjugate by one invertible
    matrix.

    Under the literal canonical-form display, an inactive zero-weight block
    may be added to either tensor.  Such a block changes the ambient bond
    dimension but contributes nothing at positive length.  The printed
    dimension and global-conjugacy conclusion therefore fails.  The corrected
    active, nonzero result is Theorem~\ref{thm:cpgsv_equal_case_source}; the
    source distinction is recorded in
    \path{docs/paper-gaps/cpsv16_bnt_characterization_active_blocks.tex}.
\end{corollary}

\begin{theorem}[Equal BNT canonical forms are globally conjugate]%
    \label{thm:cpgsv_equal_case_source}
    \lean{MPSTensor.fundamentalTheorem_equal_canonicalForm}
    \leanok
    \uses{def:sector_bnt_cf, thm:sector_bnt_equal_mps_gaugeEquiv_literal}
    Let $P$ and $Q$ be BNT canonical forms (Definition~\ref{def:sector_bnt_cf})
    whose total tensors $P_{\mathrm{tot}}$, $Q_{\mathrm{tot}}$ generate the same
    MPV family for every positive system size~$N$. Every listed sector has a
    positive number of copies and every copy weight is nonzero, so inactive
    zero-weight summands are excluded. Then their total bond
    dimensions coincide; writing the common value as~$D$, there is one
    invertible matrix $X\in\GL_D(\C)$ such that, for every physical index~$i$,
    \begin{align}
        Q_{\mathrm{tot}}^i
         & =X\,P_{\mathrm{tot}}^i\,X^{-1}. \notag
    \end{align}

%    Corollary~2.11 of \cite{Cirac2016MPDO_arXiv} is stated for a convention
%    that permits an additional normal summand with coefficient zero at every
%    positive length.  Such a summand can change the ambient bond dimension
%    without changing any positive-length MPV.  In
%    \cite[Corollary~IV.5]{Cirac2021Matrix}, the canonical summands have positive
%    coefficients, and the dimension and conjugacy conclusions agree with those
%    above.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:sector_bnt_equal_mps_gaugeEquiv_literal}
    This is Theorem~\ref{thm:sector_bnt_equal_mps_gaugeEquiv_literal}. The BNT
    comparison of Chapter~\ref{ch:bnt} provides the basis relabelling, the
    per-block gauge-phase equivalences, and the copy-weight multiset matching,
    which combine into the block-diagonal global gauge. The exact
    linear-independence matching needs no per-sector unit-modulus hypothesis, and
    the BNT canonical form needs no one-site injectivity.
\end{proof}

\begin{corollary}[Canonical Form II unitary refinement in the equal case]%
    \label{cor:sector_bnt_equal_unitary_global_gauge}
    \lean{MPSTensor.fundamentalTheorem_equal_canonicalForm_unitary}
    \leanok
    \uses{lem:sector_bnt_equal_unitary_matched_gauge,
        thm:sector_bnt_equal_mps_gaugeEquiv_literal}
    Let $P$ and $Q$ be BNT canonical forms whose total tensors generate the
    same MPV family at every positive length. Assume every listed sector has a
    positive number of copies and every copy weight is nonzero, as in
    Definition~\ref{def:sector_bnt_cf}; the literal zero-weight CFII statement
    is not included. Their total bond dimensions
    agree; writing the common value as $D$, there is a unitary matrix
    $U\in\mathrm{U}(D)$ such that, for every physical index~$i$,
    \begin{align}
        Q_{\mathrm{tot}}^i
         & = U P_{\mathrm{tot}}^i U^\dagger.
        \notag
    \end{align}
%    Under the same hypotheses on the canonical-form summands, this is the
%    unitary equal case considered in
%    \cite[Corollary~A.6, lines~1197--1199]{Cirac2016MPDO_arXiv}.  If an
%    additional summand with coefficient zero at every positive length is
%    admitted, positive-length MPVs do not determine the ambient bond dimension.
\end{corollary}

\begin{proof}\leanok
    \uses{lem:sector_bnt_equal_unitary_matched_gauge}
    For each matched sector, retain the phase in both identities
    \begin{align}
        Q_k^i
         & = \zeta_k U_kP_{\beta(k)}^iU_k^\dagger,
        \notag                                     \\
        \nu_{k,q}
         & = \zeta_k^{-1}\mu_{\beta(k),\tau_k(q)}.
        \notag
    \end{align}
    For every matched copy $(k,q)$,
    \begin{align}
        \nu_{k,q}\zeta_k
         & = \zeta_k^{-1}\mu_{\beta(k),\tau_k(q)}\zeta_k
        = \mu_{\beta(k),\tau_k(q)}.
        \notag
    \end{align}
    Hence $Q_{\mathrm{flat}}^i=XP_{\mathrm{match}}^iX^{-1}$. Repeating $U_k$
    on its matched copies gives
    \begin{align}
        X
         & = \bigoplus_{k,q}U_k,
        \notag                   \\
        X^\dagger X
         & = \mathbb 1.
        \notag
    \end{align}
    If $\Pi$ is the permutation matrix restoring the original sector and copy
    order, then $\Pi^\dagger\Pi=\mathbb 1$. Hence
    \begin{align}
        U
         & = X\Pi,
        \notag          \\
        U^\dagger U
         & = \mathbb 1,
        \notag
    \end{align}
    and the permutation of matched coordinates gives
    \begin{align}
        Q_{\mathrm{tot}}^i
         & = UP_{\mathrm{tot}}^iU^\dagger.
        \notag
    \end{align}
\end{proof}
