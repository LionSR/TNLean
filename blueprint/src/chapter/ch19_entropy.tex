...
\begin{proof}\leanok
    ...
    \]
    a sum of nonnegative terms, each bounded below by the tangent inequality
    $\log x\le x-1$ at $x=q_j/p_i$ together with the doubly stochastic telescope.
    ...
    matrix therefore intertwines the eigenvalue diagonals, and conjugating
    $\sigma$ into the eigenbasis of $\rho$ recovers $\rho$'s spectral diagonal,
    so $\rho=\sigma$.
\end{proof}
