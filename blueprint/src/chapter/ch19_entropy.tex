...
\begin{proof}\leanok
    ...
    $\tr(\mathcal R_\sigma(X))
      =\tr(P_\tau X)$ gives trace preservation.
\end{proof}
...
\begin{proof}\leanok
    ...
    $\ker(\tr_R\sigma)\subseteq
    \ker(\tr_R\rho)$. The preceding absorption lemma therefore
    yields
    $P_\tau(\tr_R\rho)P_\tau
      =\tr_R\rho$, where
    $\tau=\tr_R\sigma$. The supported-agreement theorem gives
    \[
        P_\tau(\tr_R\rho)P_\tau
        =\tr_R\rho
        \;\Longrightarrow\;
...
\begin{theorem}[The support Petz map recovers its reference]
    ...
    \[
        \mathcal R_\sigma(\tr_R\sigma)=\sigma.
    \]
\end{theorem}
...
\begin{theorem}[The completed Petz channel recovers its reference]
    ...
    For every positive semidefinite $\sigma$,
    \[
        \widehat{\mathcal R}_\sigma(\tr_R\sigma)=\sigma.
    \]
\end{theorem}
