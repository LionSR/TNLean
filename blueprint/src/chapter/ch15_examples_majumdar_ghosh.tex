...
\begin{remark}\label{rem:majumdar_ghosh_review_hamiltonian}
    By Theorem~\ref{thm:majumdar_ghosh_hamiltonian_eigen}, every vector of
    $\mathcal G_{N,3}(A)$ is an eigenvector of the spin-$\tfrac12$ chain
    $H=\sum \boldsymbol{S}_i\cdot\boldsymbol{S}_{i+1}
        +\tfrac12\sum\boldsymbol{S}_i\cdot\boldsymbol{S}_{i+2}$
    of \cite{Cirac2021Matrix} with eigenvalue $-\tfrac{3N}{8}$. If
    $-\tfrac{3N}{8}$ is the smallest eigenvalue of $H$, then the two-fold
    degeneracy of Theorem~\ref{thm:majumdar_ghosh_ground_space} reflects the
    spontaneous breaking of the one-site translation symmetry of this chain.
    % The lower bound H >= -3N/8 is not formalized here; tracked in #8117.
\end{remark}
