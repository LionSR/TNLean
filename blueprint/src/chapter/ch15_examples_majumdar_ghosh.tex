...
\begin{lemma}[The three-site term as a projector]%
    \label{lem:majumdar_ghosh_term_projector}
...
\begin{remark}\label{rem:majumdar_ghosh_term_total_spin}
    ...
    Lemma~\ref{lem:majumdar_ghosh_term_projector} is the projector $P^{3/2}$ onto
...
\begin{theorem}[Ground space of the Majumdar--Ghosh Hamiltonian]%
    ...
    \uses{def:majumdar_ghosh_hamiltonian, def:parent_hamiltonian,
        lem:majumdar_ghosh_term_projector, lem:transported_es_positive,
        thm:majumdar_ghosh_ground_space, thm:majumdar_ghosh_hamiltonian_eigen}
...
\begin{proof}
    \leanok
    By Theorem~\ref{thm:spin_exchange_swap}, $H_{N}+\tfrac{3N}{8}$ is a quarter of
    the sum over the windows $i,i+1,i+2$ of the three transpositions within the
    window, since each nearest-neighbour transposition occurs in two windows and
    each next-nearest-neighbour one in one. By
    Lemma~\ref{lem:majumdar_ghosh_term_projector}, the parent interaction on the
    window starting at $i$ is a third of the same sum of three transpositions,
    which gives~(\ref{eq:mg_hamiltonian_projectors}). The parent Hamiltonian is
    positive by Lemma~\ref{lem:transported_es_positive}, so pairing an
    eigenvector of $H_{N}$ with eigenvalue $\mu$ against itself shows
    $\mu+\tfrac{3N}{8}\ge0$. The eigenspace for $-\tfrac{3N}{8}$ is the kernel of
