...
resonating valence-bond state. In tensor network
representations~\cite{PerezGarcia2007Matrix,Cirac2021Matrix}, it exemplifies
a non-normal, non-injective tensor with a two-fold degenerate periodic ground
space.
...
\begin{definition}[Majumdar--Ghosh tensor]\label{def:majumdar_ghosh_tensor}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The three bond levels represent the two spin-$\tfrac12$ values of an open
    singlet partner together with the closed inter-dimer link; the relative sign
    $-\tfrac1{\sqrt2}$ reflects the antisymmetry of the singlet.
\end{definition}
...
\begin{remark}\label{rem:majumdar_ghosh_bond_dimension}
    The valence-bond shorthand of \cite{Cirac2021Matrix} uses the singlet
    matrix $Y$, which literally specifies a six-dimensional virtual space;
    the $D = 3$ representation in Definition~\ref{def:majumdar_ghosh_tensor}
    is a schematic reduction rather than a verified contraction; see
    \cite{gap:rmp_majumdar_ghosh_tensor_gap}.
\end{remark}
