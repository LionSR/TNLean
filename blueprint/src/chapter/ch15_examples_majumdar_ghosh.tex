...
normality in the following two lemmas.

\begin{lemma}[Graded injectivity of the boundary map]%
...
\begin{lemma}[Intersection property of the Majumdar--Ghosh tensor]%
...
\begin{proof}
    ...
    Lemma~\ref{lem:majumdar_ghosh_graded_injectivity}, its restriction with
    ...
    $\Gamma_{M-1}(A^jY_i)=\Gamma_{M-1}(Z_jA^i)$, and both matrices have parity
    $(-1)^{M-1}$. For $M\ge4$ the lemma gives $A^jY_i=Z_jA^i$; with
    $X=\sum_j(A^j)^\dagger Z_j$ the left-canonical equation gives $Y_i=XA^i$,
    hence $\psi=\Gamma_{M+1}(X)$. For $M=3$, the
...
\begin{proof}
    ...
    Lemma~\ref{lem:majumdar_ghosh_intersection}, $\psi=\Gamma_{N}(X)$ and its
    translate by one site is $\Gamma_{N}(Y)$, with $X$ and $Y$ commuting with
    $G$. Comparing the two cuts on the remaining $N-1$ sites gives
    $\Gamma_{N-1}(XA^a)=\Gamma_{N-1}(A^aY)$, so $XA^a=A^aY$ by
    Lemma~\ref{lem:majumdar_ghosh_graded_injectivity}. The entries of these two
...
\begin{proof}
    ...
    $G$. The entries of these equations force $X=0$.
\end{proof}
...
\begin{remark}\label{rem:majumdar_ghosh_review_hamiltonian}
    ...
    that lower bound is not formalized here.
\end{remark}
