\begin{theorem}[Representative and literal reduction from first-site contractions]
    ...
    \lean{MPSTensor.CPSVCanonicalFormData.BNTRefinement.%
        insertedTensor_eq_of_firstSiteActionAgree}
    \lean{MPSTensor.IsCPSVCanonicalForm.insertedTensor_eq_of_firstSiteActionAgree}
    \lean{MPOTensor.firstSiteActionAgree_braRight_ketLeftBraRight_of_invariant}
    ...
    \leanok
    \uses{def:mpdo, def:horizontal_cf_mpdo, def:mpdo_three_first_site_contractions, thm:representative_grouped_insert_eq, thm:postblocked_representative_grouped_insert_eq, thm:cpsv_original_coordinate_marked_lemma_l, thm:mpdo_first_site_invariant_comm}
    ...
    The corresponding literal-CF assertion also holds. Let $A$ be in
    literal CPSV canonical form. If the first-site actions of $Y,Z\in\MN{d}$
    agree on every positive-length matrix product vector of $A$, then
    \begin{align}
        YA & =ZA
        \label{eq:cpsv_literal_first_site_inserted}
    \end{align}
    on the original bond space. In particular, suppose that $M$ generates
    matrix product density operators, $P=P^\dagger$, and
    $P\widetilde M=P\widetilde MP$. Positivity gives agreement of the
    first-site actions associated with $\widetilde MP$ and
    $P\widetilde MP$ at every positive length. Hence
    (\ref{eq:cpsv_literal_first_site_inserted}) gives the opposite one-sided
    identity on the original bond space.

    This is the invariant-projection step of
    \cite[Proposition~4.13, lines~1873--1887]{Cirac2016MPDO_arXiv}, grouping
    copies over each normal representative as in Lemma~L
    \cite[Appendix~C.3]{Cirac2016MPDO_arXiv}; Hermiticity of $Q$ suffices for the
    implication.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:representative_grouped_insert_eq, thm:postblocked_representative_grouped_insert_eq, thm:cpsv_original_coordinate_marked_lemma_l, thm:mpdo_first_site_invariant_comm, thm:blockwise_opposite_insert_rotated_mpo}
    For the normalized BNT-refined assertion, positive-length MPV equality
    gives the first-site identity for the BNT sector decomposition. The representative-grouped
    ...
    $Q_1H^{(N)}=Q_1H^{(N)}Q_1=H^{(N)}Q_1$. These two identities give the
    corresponding first-site equalities for the BNT sector decomposition. Apply the
    ...
    For literal CPSV canonical form, first-site agreement is a marked-trace
    identity on the original tensor. Theorem~\ref{thm:cpsv_original_coordinate_marked_lemma_l}
    needs only the identities with a nonempty unmarked tail; it identifies the
    two linear marks and gives
    (\ref{eq:cpsv_literal_first_site_inserted}). In the invariant-projection
    case, positivity first yields
    $P_1H^{(N)}=P_1H^{(N)}P_1=H^{(N)}P_1$; its matrix coefficients are exactly
    the required first-site agreement.
\end{proof}
...
\begin{lemma}[Blockwise equality from agreement on the MPV family]
    ...
    \textbf{Scope restriction (block-injective specialization):}
    The block-injective hypothesis excludes gauge-equivalent repeated blocks,
    specializing the source lemma \cite[Lemma~L]{Cirac2016MPDO_arXiv}; documented in
...
\begin{theorem}[One-sided invariant corners of Hermitian operators]
    \label{thm:hermitian_opposite_corner_zero}
    \lean{Matrix.IsHermitian.opposite_corner_eq_zero}
    \leanok
    Let $H$ and $P$ be Hermitian matrices. If $PH=PHP$, then
    $(\Id-P)HP=0$.
\end{theorem}

\begin{proof}\leanok
    Taking adjoints gives $HP=PHP$, and the conclusion follows by
    subtraction.
\end{proof}
...
\begin{theorem}[One-sided invariant corners of positive operators]
    ...
    \leanok
    \uses{def:mpdo, thm:hermitian_opposite_corner_zero}
    Let $H$ be a nonempty-chain density operator generated by an MPDO, and
    let $P=P^\dagger$. If $PH=PHP$, then $(\Id-P)HP=0$. This is the operator
    identity used in the
...
\begin{theorem}[First-site contraction reduction]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    then $Y_RA_j=Y_CA_j$ for every minimal representative $A_j$. This application of
    Lemma~L specializes to the physical-index insertions of $P$, $PHP$, and $HP$ in
...
\begin{lemma}[Doubled-index insertions are vertical multiplications]
    ...
    \leanok
    \uses{def:mpdo_three_first_site_contractions, def:mpdo_vertical_one_site_actions}
...
\begin{theorem}[Representative reduction for the MPO contractions]
    ...
    \leanok
    \uses{def:mpdo_three_first_site_contractions, lem:mpdo_three_first_site_contraction_coefficients, thm:blockwise_opposite_insert_eq}
...
\begin{proof}
    \leanok
    \uses{lem:mpdo_three_first_site_contraction_coefficients, thm:blockwise_opposite_insert_eq}
    ...
    contractions. Equality of matrix product vectors gives these
    equalities for the chosen canonical-form decomposition. Apply
...
\begin{theorem}[One-sided invariance and density-operator commutation]
    ...
    \leanok
    \uses{def:mpdo, def:mpo_family, def:mpdo_vertical_one_site_actions, thm:mpdo_opposite_corner_zero}
...
\begin{theorem}[One-sided invariance on BNT representatives]
    ...
    \leanok
    \uses{def:mpdo, def:horizontal_cf_mpv_representation, def:mpdo_vertical_one_site_actions, thm:mpdo_first_site_invariant_comm, thm:blockwise_opposite_insert_rotated_mpo}
...
\begin{proof}
    \leanok
    \uses{thm:mpdo_first_site_invariant_comm, thm:blockwise_opposite_insert_rotated_mpo}
...
\begin{theorem}[Commutation with a power of a positive semidefinite matrix]
    \label{thm:psd_commute_of_commute_pow_matrix}
    \lean{Matrix.PosSemidef.commute_of_commute_pow}
    \leanok
    Let $H\geq0$, let $p\geq1$, and let $QH^p=H^pQ$. Then $QH=HQ$.
\end{theorem}

\begin{proof}\leanok
    Diagonalize $H$. The function $x\mapsto x^p$ is injective on the
    non-negative reals, so a matrix commutes with the diagonal matrix of the
    $p$-th powers exactly when it commutes with the diagonal matrix itself.
\end{proof}
...
\begin{theorem}[Commutation with a power of a positive semidefinite matrix]
    ...
    \leanok
    \uses{def:mpdo, thm:psd_commute_of_commute_pow_matrix}
    A matrix commuting with a nonzero power of a nonempty-chain density
    operator generated by an MPDO commutes with the
