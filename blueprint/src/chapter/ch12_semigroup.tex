...
\end{theorem}
\begin{proof}\leanok
    Assume for contradiction that a block-upper-triangular Lindblad
    decomposition exists.  Extract a nontrivial invariant projection $P$,
    show $(1-P)A P = 0$ for every generator by induction on elements of the
    $*$-subalgebra generated by $\{L_j\} \cup \{\kappa\}$,
    and derive a contradiction with the hypothesis that the generated
    algebra is all of $M_d(\CC)$.
\end{proof}
...
\end{theorem}
\begin{proof}\leanok
    Decompose the Kossakowski matrix spectrally and extract a Lindblad family
    with rank at most $\finrank(\mathfrak{su}(d))$.  Show that any
    block-upper-triangular Lindblad family in dimension $d$ with a nontrivial
    invariant projection of rank $k$ spans a subspace of dimension at most
    $d^2 - d$, contradicting the rank hypothesis.
\end{proof}
