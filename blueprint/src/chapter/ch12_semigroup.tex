...
\begin{definition}[Lindblad span]\label{def:lindblad_span}
    ...
    \uses{def:lindblad_form}
    The $\CC$-linear span of the Lindblad operators $\{L_j\}$.
\end{definition}
...
\begin{definition}[Hermitian-closed Lindblad span]\label{def:is_lindblad_span_hermitian_closed}
    ...
    i.e.\ it forms a $*$-subspace of $M_d(\CC)$.
\end{definition}
...
\begin{definition}[Trivial Lindblad commutant]\label{def:has_lindblad_span_trivial_commutant}
    ...
    \uses{def:lindblad_form}
    The commutant of the Lindblad family $\{L_j\}'$ equals $\CC \cdot \Id$,
    i.e.\ the Lindblad operators act irreducibly on $M_d(\CC)$.
\end{definition}
...
\begin{lemma}[$\kappa$ block vanishing from generator compression]%
    \label{lem:kappa_block_generator_compression}
...
\begin{lemma}[Full algebra generation forbids block-upper-triangular form]%
    \label{lem:full_algebra_gen_no_block_ut}
    \lean{full_algebra_generation_implies_no_blockUpperTriangular}\leanok
    \uses{def:lindblad_form, def:has_block_upper_triangular_lindblad,
          lem:kappa_block_generator_compression}
    Let $(H, \{L_j\})$ be a Lindblad form.
    If the $*$-algebra generated by $\{L_j\}$ and $\kappa$ is the entire
    matrix algebra $M_d(\CC)$, then the Lindblad data do not admit a
    block-upper-triangular decomposition.
\end{lemma}
\begin{proof}\leanok
    \uses{lem:kappa_block_generator_compression}
    Assume for contradiction that a block-upper-triangular decomposition
    exists with nontrivial projection $P$.
    By Lemma~\ref{lem:kappa_block_generator_compression},
    $(1-P)\kappa P = 0$.
    Induction on elements of the $*$-subalgebra shows
    $(1-P)A P = 0$ for every generator.  Since the generated algebra is
    all of $M_d(\CC)$, every matrix satisfies $(1-P)AP = 0$, forcing
    $P = 0$ or $P = \Id$ — a contradiction.
\end{proof}

\begin{lemma}[Hermitian span + trivial commutant forbids block-upper-triangular form]%
    \label{lem:herm_span_trivial_commutant_no_block_ut}
    ...
    commutant is $\CC \cdot \Id$, then the Lindblad data do not admit a
    block-upper-triangular decomposition.
\end{lemma}
\begin{proof}\leanok
    Assume for contradiction that a block-upper-triangular decomposition
...
\begin{theorem}[Cor.~7.2(1): full algebra generation implies non-reducibility]
    ...
    \uses{def:is_gksl_generator, def:lindblad_form,
          lem:full_algebra_gen_no_block_ut,
          thm:wolf_prop_7_6_three_iff_four}
    If the algebra generated by $\{L_j\}$ and $\kappa$ is the entire
    matrix algebra $M_d(\CC)$, then the QDS is not reducible.
    This is \cite[Cor.~7.2, condition~1]{Wolf2012Quantum}.
\end{theorem}
\begin{proof}\leanok
    \uses{lem:full_algebra_gen_no_block_ut, thm:wolf_prop_7_6_three_iff_four}
    By Lemma~\ref{lem:full_algebra_gen_no_block_ut}, no block-upper-triangular
...
\begin{theorem}[Cor.~7.2(2): Hermitian span + trivial commutant implies non-reducibility]
    ...
    \uses{def:is_gksl_generator, def:is_lindblad_span_hermitian_closed,
          def:has_lindblad_span_trivial_commutant,
          lem:herm_span_trivial_commutant_no_block_ut,
          thm:wolf_prop_7_6_three_iff_four}
...
\end{theorem}
\begin{proof}\leanok
    \uses{lem:herm_span_trivial_commutant_no_block_ut,
          thm:wolf_prop_7_6_three_iff_four}
    By Lemma~\ref{lem:herm_span_trivial_commutant_no_block_ut}, no
...
\begin{theorem}[Cor.~7.2(3): large Kossakowski rank implies non-reducibility]
    ...
    \lean{not_isReducible_of_kossakowski_rank_ge}\notready
