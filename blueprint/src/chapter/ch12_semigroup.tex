...
\begin{theorem}[Thm.~7.2: Adjoint kernel equals commutant (faithful state)]
    ...
    \lean{mem_commutant_of_mem_adjointKernel_of_hasFaithfulStationaryState}\leanok
...
\begin{theorem}[Thm.~7.2: Adjoint kernel equals commutant (full equivalence)]
    ...
    \lean{adjointKernel_eq_commutant_of_hasFaithfulStationaryState}\leanok
...
\begin{theorem}[Prop.~7.6, (4)$\to$(2): Block-upper-triangular implies rank-deficient kernel]
    ...
    \uses{def:is_gksl_generator, def:lindblad_form}
    Let $L$ be a GKSL generator with Lindblad form $(H, \{L_j\}, \kappa)$.
    If there exists a nontrivial projector $P$ such that
    $(1-P)L_j P = 0$ and $(1-P)\kappa P = 0$ for all $j$
    (i.e.\ the Lindblad data are block-upper-triangular with respect to $P$),
