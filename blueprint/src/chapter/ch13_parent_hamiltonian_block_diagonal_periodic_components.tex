\begin{lemma}[$C^j$ boundary comparison with a block-diagonal representation]
    ...
    \leanok
    \uses{lem:block_diagonal_boundary_trace_decomposition_from_pgvwc_comparison, lem:block_diagonal_boundary_periodic_components_from_trace_decomposition_boundary}
...
\begin{lemma}[Boundary trace comparisons give periodic-boundary single-block states]
    ...
    \leanok
    \uses{lem:bnt_block_diagonal_open_boundary_matrices, lem:block_diagonal_boundary_periodic_components_from_trace_decomposition_boundary}
...
\begin{lemma}[Basis-of-normal-tensors product span gives boundary-trace periodic components]
    ...
    \leanok
    \uses{lem:unital_block_separating_product_span, lem:block_diagonal_boundary_periodic_components_from_trace_decomposition}
    With the hypotheses and notation of
    Lemma~\ref{lem:bnt_block_diagonal_open_boundary_matrices}, assume in
...
\begin{lemma}[$C^j$ boundary comparison gives periodic single-block states]
    ...
    \leanok
    \uses{lem:bnt_block_diagonal_open_boundary_matrices, lem:block_diagonal_boundary_component_pgvwc_comparison_injective}
    With the hypotheses and notation of
    Lemma~\ref{lem:bnt_block_diagonal_open_boundary_matrices}, assume in
...
\begin{theorem}[Crossing-tail spans give periodic single-block states]
    ...
    \leanok
    \uses{lem:bnt_block_diagonal_open_boundary_matrices, lem:block_diagonal_boundary_periodic_components_from_crossing_boundary}
    With the hypotheses and notation of
    Lemma~\ref{lem:bnt_block_diagonal_open_boundary_matrices}, assume in
...
\begin{theorem}[Short crossing-tail spans give periodic single-block states]
    ...
    \leanok
    \uses{thm:block_diagonal_boundary_periodic_components_from_crossing_span, lem:unital_block_separating_product_span}
    With the hypotheses and notation of
    Lemma~\ref{lem:bnt_block_diagonal_open_boundary_matrices}, assume in
...
\begin{theorem}[Cyclic translations compose]
    \label{thm:parent_cyclic_translate_add}
    \lean{MPSTensor.cyclicTranslateState_add}
    \leanok
    \uses{def:parent_cyclic_translate_state}
    For offsets $s,t\in\mathbb Z/N\mathbb Z$ and a state $\psi$ on the
    periodic chain,
    \begin{align}
        T_s(T_t\psi) & =T_{s+t}\psi.
        \notag
    \end{align}
\end{theorem}
...
\begin{proof}
    ...
    For every configuration $\sigma$, the two sides evaluate $\psi$ on the
    same configuration because $T_t(T_s\sigma)=T_{s+t}\sigma$.
\end{proof}
...
\begin{lemma}[Cyclic translation preserves the periodic chain space]
    ...
    \leanok
    \uses{def:parent_cyclic_translate_configuration, def:parent_cyclic_translate_state}
...
\begin{lemma}[Comparison across a global cut]
    ...
    \leanok
    \uses{def:parent_cyclic_translate_state, lem:parent_cyclic_translate_append, lem:block_matrices_equal_from_trace_pairings}
...
\begin{theorem}[A global change of cut gives periodic single-block states]
    ...
    \lean{MPSTensor.exists_blockDiagonal_boundary_chainGroundSpace_pgvwc07_of_dualFixedPoint}
    \leanok
...
\begin{theorem}[Periodic block decomposition from a global cut]
    ...
    \lean{MPSTensor.chainGroundSpace_toTensorFromBlocks_eq_iSup_pgvwc07_of_dualFixedPoint}
    \leanok
    ...
    gauge and phase, and injective at a common
    length $L_0>0$. Assume
    $\sum_aA^j_aA^{j\,\dagger}_a=\Id$ and, for positive-definite $\Lambda_j$,
    ...
    % Repository source coordinates: Papers/quant-ph_0608197/MPSarchive.tex,
    % canonical normalization lines 742--763 and parent theorem lines 1424--1456.
\end{theorem}
...
\begin{theorem}[Periodic block decomposition and block-space independence]
    ...
    \lean{MPSTensor.chainGroundSpace_toTensorFromBlocks_eq_iSup_and_iSupIndep_pgvwc07_of_dualFixedPoint}
    \leanok
...
\begin{theorem}[Unique decomposition into open-boundary block spaces]
    ...
    \lean{MPSTensor.exists_unique_sum_groundSpace_of_chainGroundSpace_toTensorFromBlocks_pgvwc07_of_dualFixedPoint}
    \leanok
    Let $g\ge2$, assume the physical alphabet is nonempty, and let $A_j$
    be irreducible blocks with positive bond dimensions, pairwise inequivalent
    up to gauge and phase, and injective at a common
...
\begin{corollary}[Kernel containment from the global cut]
    ...
    \lean{MPSTensor.ker_parentHamiltonian_toTensorFromBlocks_le_bntMPSVectorSpan_pgvwc07_of_dualFixedPoint}
    \leanok
...
\begin{lemma}[Matrix identities give periodic single-block states]
    ...
    \leanok
    \uses{lem:bnt_block_diagonal_open_boundary_matrices, lem:block_diagonal_boundary_component_complementary_word_identities_injective}
    With the hypotheses and notation of
    Lemma~\ref{lem:bnt_block_diagonal_open_boundary_matrices}, assume in
    addition that $L+L_0\le N$. Let
    $\psi\in\mathcal G_{N,L}(\bigoplus_j\mu_jA_j)$. Suppose that whenever
