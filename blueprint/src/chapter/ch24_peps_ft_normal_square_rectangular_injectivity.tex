...
\begin{lemma}[Bond-dimension equality at interior edges from rectangular
        injectivity]%
    \label{lem:peps_bondDim_interiorEdge_eq_of_normalSquareRectangularInjectivity}
    ...
    \begin{proof}
        \leanok
        \uses{lem:peps_normalSquareInteriorEdgeBlocking}
        The interior-edge blocking datum of
        Lemma~\ref{lem:peps_normalSquareInteriorEdgeBlocking} supplies the
        edge-blocking data for both tensors at $e$. The normal blocking
        hypothesis and the positive bond dimensions then give, through the
        one-edge isomorphism argument, the equality of the bond dimensions.
    \end{proof}
\end{lemma}
