\begin{definition}[Normal rectangular injectivity hypotheses]%
    ...
    \leanok
    \uses{def:peps_regionInjectivityData, def:peps_squareLatticeCoordinateRegions}
...
\begin{lemma}[Coordinate rectangular injectivity gives injective rectangles]%
    ...
    \leanok
    \uses{def:peps_normalRectangleInjectivityHypotheses, lem:peps_squareLatticeCoordinateRegion_identities}
...
\begin{lemma}[Rectangular injectivity gives injective $T$-edge blocks]%
    ...
    \leanok
    \uses{def:peps_regionInjectivityData, def:peps_normalRectangleInjectivityHypotheses, lem:peps_normalSquareRegionTHole_rectangular_decomposition}
...
\begin{lemma}[Injectivity from a rectangular cover of the local $T$-region]%
    ...
    \leanok
    \uses{def:peps_normalSquareRectangleCover, def:peps_normalRectangleInjectivityHypotheses, lem:peps_normalSquareRectangleCover_injective, lem:peps_regionInjectivityUnionClosure_finite}
...
\begin{lemma}[Injectivity from a rectangular cover of the edge complement]%
    ...
    \leanok
    \uses{def:peps_normalSquareRectangleCover, def:peps_normalRectangleInjectivityHypotheses, lem:peps_normalSquareRectangleCover_injective, lem:peps_regionInjectivityUnionClosure_finite, lem:peps_normalSquareEdgeComplementRegion_coverOfT}
...
\begin{lemma}[A cover of $T$ extends to a cover of the edge complement]%
    ...
    \leanok
    \uses{def:peps_normalSquareRectangleCover, lem:peps_normalSquareEdgeComplementRegion_topCollar}
...
\begin{lemma}[The normalized edge complements have collars]%
    ...
    \leanok
    \uses{lem:peps_normalSquareEdgeComplementRegion, lem:peps_normalSquareRegionT_window_complement, def:peps_regionInjectivityData, lem:peps_squareLatticeRectangleInjectivity_rectangles}
    ...
    \begin{tenkzequation}
        ...
        \tenkzkernel
        \begin{tenkz}[rows={wire,wire,wire,wire,wire,wire,wire}, cols=5,
            ...
            \tn[skin=dot]{} & \tn[skin=dot]{} & \tn[skin=dot]{} &
            \tn[skin=dot]{} & \tn[skin=dot]{}
            \tnmark[form=enclosure, species=selected, tint, label pos=n]
            {(1,1) .. (6,5) - (3,1) .. (5,2) - (2,3) .. (3,5)}{$T$}
            \tnmark[form=enclosure, species=collar]{(6,1) .. (7,3)}{$C_1$}
            \tnmark[form=enclosure, species=collar]
            {(6,3) .. (7,5)}{$C_2$}
        \end{tenkz}
        $ = $
        \begin{tenkz}[rows={wire,wire,wire,wire,wire,wire,wire}, cols=5,
            ...
            \tn[skin=dot]{} & \tn[skin=dot]{} & \tn[skin=dot]{} &
            \tn[skin=dot]{} & \tn[skin=dot]{}
            \tnmark[form=enclosure, species=complement, tint, label pos=n]
            {(1,1) .. (7,5) - (3,1) .. (5,2) - (2,3) .. (3,5)}{$A_3$}
        \end{tenkz}
    \end{tenkzequation}
\end{lemma}
% Current source-comparison status: docs/paper-gaps/peps_normal_ft_section3_route.tex.
...
\begin{lemma}[A large rectangle is injective]%
    ...
    \leanok
    \uses{lem:peps_injectiveRegion_union, lem:peps_squareLatticeRectangleInjectivity_rectangles}
...
\begin{lemma}[Interior edge-complement injectivity]%
    ...
    \leanok
    \uses{lem:peps_normalSquareEdgeComplementRegion, lem:peps_normalSquareWideRectangle_injective, lem:peps_squareLatticeRectangleInjectivity_rectangles}
    ...
    \textbf{Scope restriction (open-boundary interior edges):}
    These edge-blocking constructions apply only to edges with interior margins
    in the open rectangular model, whereas \cite[Theorem~3]{Molnar2018NormalPEPS}
    obtains every edge via periodic translations on the torus; documented in
...
\begin{lemma}[Cover-free interior every-edge blocking]%
    ...
    \leanok
    \uses{lem:peps_normalSquareInteriorEdgeComplement_injective, lem:peps_normalSquareHorizontalTranslatedEdge_blockingData, def:peps_normalEdgeBlockingHypotheses}
    ...
    establishes the normal edge-blocking hypotheses, with the injective chain,
    endpoint membership, pairwise disjointness, and coverage at every edge.

    This establishes the edge-blocking hypotheses for edges with interior margins
    \cite{gap:peps_normal_ft_section3_route}.
\end{lemma}
% Current source-comparison status: docs/paper-gaps/peps_normal_ft_section3_route.tex.
\begin{proof}\leanok
    ...
    interior $A_3$ injectivity. Combining these per-edge data over all edges
...
\begin{lemma}[Normalized edge-blocking data]%
    ...
    \leanok
    \uses{def:peps_squareLatticeCoordinateRegions, lem:peps_normalSquareEdgeComplementRegion, lem:peps_normalSquareRegionT_edgeBlocks_injective, lem:peps_normalSquareEdgeComplementRegion_topCollar}
...
\begin{lemma}[Translated horizontal edge-blocking data]%
    ...
    \leanok
    \uses{def:peps_squareLatticeCoordinateRegions, lem:peps_normalSquareEdgeComplementRegion, lem:peps_normalSquareEdgeComplementRegion_injective_of_cover}
...
\begin{lemma}[Translated vertical edge-blocking data]%
    ...
    \leanok
    \uses{def:peps_squareLatticeCoordinateRegions, lem:peps_normalSquareEdgeComplementRegion_injective_of_cover}
...
\begin{theorem}[Bond-dimension equality at interior edges from rectangular
    ...
    \leanok
    \uses{def:peps_normalRectangleInjectivityHypotheses, def:peps_regionInjectivityData, def:peps_regionInjectivityDataOf, def:peps_same_state, lem:peps_normalSquareInteriorEdgeBlocking}
    ...
    \begin{proof}
        \leanok
        \uses{lem:peps_normalSquareHorizontalTranslatedEdge_blockingData, lem:peps_normalSquareVerticalTranslatedEdge_blockingData, thm:peps_bondDim_eq_of_normalBlocking}
