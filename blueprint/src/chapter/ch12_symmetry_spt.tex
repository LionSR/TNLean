...
The formal predicate in this section compares virtual cocycle labels.  It does
not define physical phase equivalence.  The latter is supplied by the
classification theorem of Chen--Gu--Wen and Schuch--P\'erez-Garc\'ia--Cirac
under fixed symmetry, injectivity, and unique-ground-state hypotheses.
...
\begin{definition}[Equality of virtual cocycle labels]
    ...
    $U:G\to\GL_d(\C)$, the formal predicate called \emph{same SPT phase}
...
\begin{remark}[Scalar and character conventions]
    ...
    The formal factor systems take values in $\C^\times$.  For unitary bond
...
\begin{theorem}[One-dimensional symmetric gapped-phase classification]
    ...
    \notready
    \uses{def:is_same_spt_phase, def:h2_cx}
    Fix a symmetry group $G$ and its unitary on-site action, up to adjoining
    symmetry-trivial local degrees of freedom.  In the injective MPS setting,
    equivalently for unique ground states of finite-range parent Hamiltonians,
    allow finite blocking, changes of physical and bond dimensions by trivial
    ancillary sectors, and smooth deformations within the injective class.
    Two tensors are connected by a symmetry-preserving uniformly
    gapped path if and only if their virtual projective representations define
    the same class in $H^2(G,\mathrm U(1))$
    \cite{ChenGuWen2011,Schuch2011Phases}.
\end{theorem}

\begin{proof}
    Along a symmetric gapped path, quasi-adiabatic continuation preserves the
    projective edge action, so the $\mathrm U(1)$ factor-system class is
    constant.  Conversely, choose gauges with the same cocycle representative,
    block until the tensors are injective on one site, enlarge the bond spaces
    by symmetry-trivial ancillas when their dimensions differ, and interpolate
    through symmetric injective tensors.  The parent-Hamiltonian construction
    then supplies a symmetric path with a uniform gap.  The formal development
    contains the virtual cocycle label but not the Hamiltonian-path,
    blocking/dimension-change, or gap-stability bridges, so this source
    classification theorem remains unformalized.
\end{proof}
