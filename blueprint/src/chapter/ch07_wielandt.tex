...
\begin{remark}[Explicit blocking-length estimates]\notready
    Theorem~\ref{thm:wielandt_bound} gives the $D^2$
    cumulative-span bound.
    The sharper explicit blocking-length estimates from
    \cite{Sanz2010Quantum} --- at most $D^4$ for injectivity and at
    most $3D^5$ for the biCF form --- are not currently formalized here.
\end{remark}
...
The results below first formalize the assembly step and then derive a
blocked fixed-length spanning theorem.
...
\begin{remark}[Relation with Lemma~2(b)]\label{rem:lemma2b_status}\leanok
    ...
    extraction statement for fixed-length spans with an assembly
    argument turning fixed-length vector spanning into fixed-length
    matrix spanning.
    Theorem~\ref{thm:lemma2b_assembly} formalizes the second step.
    Theorem~\ref{thm:rankone_extraction_blockTensor} supplies the
    rank-one input in blocked form, and
    Theorem~\ref{thm:wielandt_lemma2b} then derives a blocked,
    fixed-length word-span saturation result from that pipeline.
\end{remark}
...
\begin{remark}[Aperiodicity condition]\label{rem:aperiodicity}\leanok
    ...
    Kraus operators.
    For genuinely primitive tensors one expects aperiodicity to follow
    from peripheral-spectrum analysis, but that implication is not used
    in the present Wielandt bridge, so the condition is kept explicit.
\end{remark}
...
\begin{remark}[The D1 route bypasses the aperiodicity bridge]\label{rem:d1_bypasses_aperiodicity}
    The aperiodicity results in \S\ref{sec:cumspan_to_wordspan} are needed only for the
    legacy route through Definition~\ref{def:normal}, where one upgrades cumulative
    spanning to eventual block injectivity. The D1 route used later in
    Chapters~\ref{ch:canonical}--\ref{ch:assembly} does not pass through this bridge:
    it works directly with irreducible TP-normalized blocks whose blocked transfer maps
    are primitive, and packages them as normal canonical form in the sense of
    \cite{Cirac2017MPDO_AnnPhys}. The code also contains companion cumulative-span
    lemmas for this blocked irreducible setting, including blocked and
    eigenvector-spreading variants, but we do not spell them out separately here.
\end{remark}
