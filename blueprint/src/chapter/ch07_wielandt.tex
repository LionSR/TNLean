...
The formalization in \texttt{Wielandt/Lemma2b.lean} covers the
...
\begin{remark}[Status of Lemma~2(b)]\label{rem:lemma2b_status}
    ...
    What remains is the \emph{rank-one construction step}:
    showing that the Fitting/Jordan analysis of the eigenvalue
    word $A^{w_0}$
    (from Theorem~\ref{thm:eigenvalue_extraction})
    produces the required rank-one operators
    $\ketbra{\varphi}{e_j} \in S_m(A)$.
    Once this is done, combining with the eigenvector
    spreading (Theorem~\ref{thm:eigenvector_spreading})
    and eigenvector padding
    (Lemma~\ref{lem:eigenvector_padding})
    will yield the full Lemma~2(b) and hence the sharp
    Wielandt bound
    $S_{(D-1)^2 + 1}(A) = \MN{D}$.
\end{remark}
...
The module \texttt{Wielandt/RectangularSpan.lean} extends the
Lemma~2(b) infrastructure to the \emph{blocked tensor}~$A^{[L]}$
and develops the ``rectangular span'' used to produce
rank-one elements.
...
\begin{remark}[Remaining gap for Lemma~2(b) via blocking]%
    \label{rem:blocked_lemma2b_gap}
    \notready
    Theorem~\ref{thm:wielandt_blocked_assembly} reduces the full
    Lemma~2(b) of~\cite{Sanz2010Quantum} to producing a single
    rank-one element $\varphi \psi^T \in S_m(A^{[L]})$ for bounded~$m$.
    This is the core linear-algebraic step:
    the Fitting/Jordan analysis of high powers of the eigenvector
    word products should produce this rank-one element via the
    ``rectangular span'' developed in the same module.
    The rectangular span API (monotonicity, dimension bounds,
    Wielandt-type stabilisation) is formalized, but the final
    rank-one extraction is not yet completed.
\end{remark}
...
The module \texttt{Wielandt/PrimitivityToNormal.lean} studies the
relationship between the spectral-gap notion of primitivity
(\texttt{IsPrimitiveMPS}: unique PSD fixed point with spectral gap)
and normality.
...
\begin{remark}[Definition mismatch]\label{rem:prim_definition_mismatch}
    \notready
    The Lean definition \texttt{IsPrimitiveMPS} encodes only
    the spectral-gap condition: there exists a nonzero PSD
    fixed point $\rho$ and $\rho_{\mathrm{spec}}(\E_A - P_\rho) < 1$.
    The paper's notion of ``primitive''
    (\cite[Proposition~3]{Sanz2010Quantum}) additionally requires
    $\rho$ to be positive definite (full rank).
    Without this, \texttt{IsPrimitiveMPS} alone does \emph{not}
    imply irreducibility or normality.
    A counterexample ($D = 2$, $\rho = \mathrm{diag}(1,0)$)
    is documented in the Lean source.
    To recover the paper's full equivalence
    (primitive $\Leftrightarrow$ eventually full Kraus rank),
    one should assume $\rho$ is positive definite or,
    equivalently, work with \texttt{IsIrreducibleTensor}.
    The remaining gaps are:
    \begin{enumerate}
        \item \texttt{IsPrimitiveMPS} $+$ $\rho$ positive definite
              $\Rightarrow$ \texttt{IsIrreducibleTensor}
              (requires formalising the restricted subchannel argument).
        \item \texttt{IsIrreducibleTensor} $\Rightarrow$
              \texttt{IsNormal}
              (requires Burnside's theorem for matrix algebras,
              not yet in Mathlib).
    \end{enumerate}
\end{remark}