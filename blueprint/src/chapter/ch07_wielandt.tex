...
\begin{definition}[Word span]\label{def:word_span}
    ...
    \[
        ...
        \subseteq  \MN{D}.
    \]
    This is \cite[\S II]{Sanz2010Quantum}.
\end{definition}
...
\begin{theorem}[Cumulative span reaches $\MN{D}$ --- sharp bound]%
    ...
    If $A$ is normal, then
    $T_{D^2 - d' + 1}(A) = \MN{D}$,
    where $d' = \dim S_1(A) = \operatorname{krausRank}(A)$.
...
\begin{theorem}[Sharp nonzero trace product]%
    ...
    \uses{def:normal}
    If $A$ is normal, then there exists a word $w$ of length
    $\le D^2 - \operatorname{krausRank}(A) + 1$ such that
    $\tr(A^w) \neq 0$.
...
\begin{theorem}[Sharp nonzero trace product --- positive length]%
    ...
    of \emph{positive} length $1 \le |w| \le D^2 - \operatorname{krausRank}(A) + 1$
    such that $\tr(A^w) \neq 0$.
    This strengthens
    Theorem~\ref{thm:nonzero_trace_word_sharp} by requiring
    $|w| \ge 1$, which is needed for the blocking argument in
...
\begin{proof}
    ...
    so $\operatorname{wordSpan}(A, N) \subseteq V_n \neq \MN{D}$
...
\begin{theorem}[Wielandt chain]\label{thm:wielandt_chain}
    ...
    Every normal MPS tensor admits a Wielandt analysis consisting of
    data $(w_0, \mu, \varphi)$ with
    $K_{D-1}(A, \varphi) = \C^D$.
\end{theorem}
...
\begin{theorem}[Wielandt bound]\label{thm:wielandt_bound}
    ...
    fixed-length conclusion $S_N(A) = \MN{D}$, proved below in
    Theorem~\ref{thm:wielandt_lemma2b}.
\end{theorem}
...
\begin{theorem}[Wielandt's inequality --- general bound]%
    ...
    \[
        \iota(A) \;\le\;
        (D^2 - \operatorname{krausRank}(A) + 1) \cdot D^2,
    \]
...
\begin{proof}
    \leanok
    \uses{thm:nonzero_trace_word_sharp_pos, thm:isNormal_blockTensor}
    \begin{enumerate}
    ...
    \item \emph{$D \ge 2$:}
        ...
        where $d' = \operatorname{krausRank}(A)$.
        ...
        Theorem~\ref{thm:wielandt_lemma2b} only gives the qualitative
        conclusion $\exists N : S_N(B) = \MN{D}$ without this explicit bound.
...
\begin{remark}[Relation with Lemma~2(b)]\label{rem:lemma2b_status}\leanok
    ...
    extraction for fixed-length spans with an assembly
...
\begin{remark}[Blocking route to fixed-length spanning]%
    ...
    applying the fixed-length assembly step
...
\begin{definition}[Primitive MPS tensor]\label{def:primitive_mps}
	...
	This is weaker than the paper's notion, which requires
...
\begin{remark}[Primitivity: spectral gap vs.\ positive definiteness]%
    ...
    \cite[Proposition~3]{Sanz2010Quantum} identifies the paper's
...
\begin{theorem}[Primitive with PosDef fixed point implies irreducible tensor]%
    ...
    (see also \cite[\S2.3]{Cirac2017MPDO_AnnPhys}).
\end{theorem}
...
\begin{theorem}[Primitive MPS with PosDef fixed point is normal]%
    ...
    This is the key connection between the spectral-gap
    primitivity of Definition~\ref{def:primitive_mps} and
    normality, filling the gap that the aperiodicity-based route
    in \S\ref{sec:cumspan_to_wordspan} leaves open.
\end{theorem}
...
\begin{theorem}[Irreducible tensor plus aperiodicity implies normality]%
    ...
    and~\ref{thm:burnside_matrix} in Chapter~\ref{ch:canonical};
    this is a presentation-level forward dependency,
...
\begin{remark}[The normal-canonical route bypasses the aperiodicity bridge]\label{rem:d1_bypasses_aperiodicity}
    ...
    block-injective route through Definition~\ref{def:normal}, where one upgrades cumulative
    spanning to eventual block injectivity. The normal-canonical route used later in
    ...
    packages them as normal canonical form in the sense of
