...
\begin{proof}
    ...
    Starting from $T_1(A) = S_1(A)$ with dimension~$d'$,
...
\begin{proof}
    ...
    Since $V_{D^2 - d' + 1} = \MN{D}$ and
    $D \ge 2$ implies $\Id \notin V_{D^2 - d' + 1}$
    (dimension of $\MN{D}$ is $D^2 \ge 4$,
    but $\spn\{\Id\}$ has dimension~$1$),
    at least one generator has nonzero trace.
\end{proof}
...
\subsection{Primitivity bridge to strong irreducibility and normality}%
...
\begin{theorem}[Primitive MPS with PosDef fixed point is normal]%
    ...
    This is the key bridge between the spectral-gap
...
\begin{proof}
    ...
    The implication (c)$\Rightarrow$(b) of
    \cite[Proposition~3]{Sanz2010Quantum}
    (strong irreducibility implies eventually full Kraus rank)
    then gives normality.
    (Formally: strong irreducibility implies
    $\langle A \rangle_n = \MN{D}$ for large $n$,
    which is exactly the definition of normality.)
\end{proof}
...
\begin{remark}[Aperiodicity condition]\label{rem:aperiodicity}\leanok
    ...
    present Wielandt bridge, so the condition is kept explicit.
\end{remark}
