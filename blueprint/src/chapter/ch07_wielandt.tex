...
\begin{proof}\leanok
    ...
    is a scalar multiple of $E_{ip} C E_{qj}$, so the span~(\ref{eq:wld_two_sided_nonzero_span})
...
\begin{proof}\leanok
    ...
    Iterating the inclusion~(\ref{eq:wld_blocknormal_self_inclusion}) gives $S_N(A) \subseteq S_{kN}(A)$
...
\section{Block injectivity from TP/primitive/irreducible blocks (relocated from Chapter~\ref{ch:canonical})}%
...
\begin{remark}
    See Theorem~\ref{thm:isNormal_blockTensor} for the formalized statement that blocking preserves normality.
\end{remark}
