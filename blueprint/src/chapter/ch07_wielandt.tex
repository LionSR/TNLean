...
\begin{theorem}[Blocked fixed-length matrix spanning]\label{thm:wielandt_blocked_assembly}
    ...
    \noindent\emph{Remark.}
    The explicit bound $N = ((D{-}1) + m + (D{-}1)) \cdot L$ holds
    from the proof construction, but the Lean formalization currently
    only exports the existential conclusion $\exists N,\; S_N(A) = \MN{D}$.
\end{theorem}
...
\begin{theorem}[Primitive MPS with PosDef fixed point is normal]%
    ...
    normality, filling the gap that the aperiodicity-based argument
