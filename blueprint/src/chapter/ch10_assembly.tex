...
coefficient-convergence data in both the legacy CF-BNT route and the
new normal-CF-BNT route, and the equal theorem is the same-structure
CF-BNT specialization rather than the full paper-level equal-MPV theorem.
...
\begin{proof}
		...
		eventually linearly independent. This is the legacy injective route;
		Remark~\ref{rem:normalcf_bnt_gap} explains the corresponding gap for
		the NT companion Definition~\ref{def:normal_cf_bnt}.
\end{proof}

\begin{remark}[The `IsNormal` gap for normal-CF-BNT]\label{rem:normalcf_bnt_gap}
		Definition~\ref{def:normal_cf_bnt} uses ``normal'' in the paper's D1
		sense, not the eventual block-injective predicate of
		Definition~\ref{def:normal}. The field `IsBNT.normal` still asks for the
		latter. In Lean, the helper theorem from normal-CF-BNT data to a full BNT
		structure therefore takes separate blockwise normality as an extra
		hypothesis. The missing bridge is the aperiodicity step from primitive
		blocked TP data to eventual block injectivity:
		$\mathrm{IsPrimitive} \Rightarrow \Id \in S_1(A)$ has not been formalized.
...
\begin{proof}\leanok
		The proof follows the same coefficient-convergence and permutation
		pattern as Theorem~\ref{thm:ft_proportional}, but uses
		Definition~\ref{def:normal_cf_bnt} in place of injective CF-BNT data.
		Irreducibility plus TP normalization supply the necessary overlap
		asymptotics, so the same matching argument yields the block
		permutation and gauge-phase equivalence.
		Remark~\ref{rem:normalcf_bnt_gap} explains why this route does not need
		the full `IsBNT.normal` field.
\end{proof}
