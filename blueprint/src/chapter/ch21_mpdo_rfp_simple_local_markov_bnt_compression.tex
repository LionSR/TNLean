...
\begin{remark}[Sectorwise hypotheses in the canonical BNT application]
    ...
    hypothesis gives a simultaneous one-letter span, hence one-site
    injectivity of every representative. The chosen BNT representatives are
    normal. Thus no blocking argument is needed for these two hypotheses.

    The formal sector theorem still assumes separately that every absorbed
    sector generates matrix product density operators. This positivity must
    come from the surrounding positive sector data; it cannot be inferred from
    SAL of the ambient tensor, since SAL is the conclusion under discussion.
    When this positivity input is present, the sector theorem applies at every
    original cut. This is the corrected implication conditioned on ambient
    source zero correlation length. It does not prove the printed GSNNCH-only
    proposition, which omits that hypothesis. An arbitrary GSNNCH presentation
    need not inherit source zero correlation length from its periodic
    operators.
