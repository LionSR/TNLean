...
\begin{theorem}[Ambient ZCL descends to the canonical absorbed BNT elements]
    ...
    $K$ has source zero correlation length. Then the copy weights over each
    normal representative agree. For every representative $A_s$ with
    non-nilpotent physical-trace transfer $\mathcal B_s$, absorb the common
    weight and write $\mathcal K_s=\mu_sA_s$. Each $\mathcal K_s$ has source
    zero correlation length. More precisely,
    \begin{align}
...
\begin{remark}[Sectorwise hypotheses in the canonical BNT application]
    ...
    correlation length supplies sectorwise source zero correlation length by
    Theorem~\ref{thm:mpdo_absorbed_bnt_sector_zcl}. The standing biCF
    hypothesis gives a simultaneous one-letter span. Together with the
    nonzero common weight, Theorem~\ref{thm:mpdo_bnt_physical_support_restriction}
    therefore gives one-site injectivity of every absorbed representative, so
    no blocking argument is needed.

    Theorem~\ref{thm:mpdo_proportional_orthogonal_commuting_sector_zcl_sal}
    still assumes separately that every absorbed sector generates matrix
    product density operators and is normal with the chosen normalization.
    These hypotheses must come from the surrounding positive normalized sector
    data; neither may be inferred from SAL of the ambient tensor, since SAL is
    the conclusion under discussion. When these inputs are present, the
    theorem applies at every original cut. This is the corrected implication
    conditioned on ambient source zero correlation length. It does not prove
    the printed GSNNCH-only proposition, which omits that hypothesis. An
    arbitrary GSNNCH presentation need not inherit source zero correlation
    length from its periodic operators.

    % **Scope restriction (ambient source ZCL):** Proposition `prop4to2` omits
    % the ambient source-ZCL hypothesis used to obtain sectorwise ZCL. See
    % docs/paper-gaps/cpsv16_commuting_form_to_sal.tex.
\end{remark}
