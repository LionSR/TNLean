...
\begin{theorem}[Entrywise BNT--Markov identity]
    ...
    \leanok
    \uses{thm:mpdo_bnt_three_site_outer_inverse_collapse, thm:mpdo_bnt_outer_inverse_markov_block}
    ...
    This is the entrywise identity used
    in~\cite[Appendix~C.2, lines~1719--1732]{Cirac2016MPDO_arXiv}, retaining
    both the normal-sector and Markov-sector off-diagonal entries.
...
\begin{theorem}[Uniqueness from nonzero closing matrices]
    ...
    \leanok
    \uses{thm:mpdo_bnt_markov_key_formula, thm:mpdo_bnt_markov_label_exists, def:mpdo_bnt_markov_block}
...
\begin{theorem}[SAL implies local $\eta$-structure]
    ...
    strong subadditivity --- the local entropy consequence of saturation of
    the area law for a simple MPDO --- then it admits the local $\eta$-structure of
    Definition~\ref{def:mpdo_eta_structure}.
\end{theorem}

\begin{proof}
    \leanok
    This follows from the characterization of states saturating strong
    subadditivity~\cite{Hayden2004Structure}. Petz recovery with a product
    reference and the Koashi--Imoto decomposition yield the direct-sum
    decomposition into tensor products on the middle system, giving the local
    $\eta$-structure of Definition~\ref{def:mpdo_eta_structure}.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{thm:mpdo_sal_ssa_equality, thm:sal_implies_eta_structure, thm:quantum_markov_right_marginal, lem:mpdo_reduced_state_collapse_last}
...
\begin{theorem}[BNT sector projectors for the four-site SAL marginal]
    ...
    \leanok
    \uses{thm:mpdo_normalized_three_site_bnt_family_closure, thm:mpdo_bnt_sector_projection_resolution, thm:mpdo_four_site_sal_eta_structure}
    ...
    the corresponding theorem in the companion quantum-channel volume~\cite[``Hayashi SSA-equality characterization'']{QICLean2026Blueprint}; no additional
...
\begin{definition}[Sitewise physical action and BNT coefficients]
...
\begin{theorem}[Separating projectors with copy-independent weights]
    ...
    \leanok
    \uses{def:mpdo_completed_bnt_sector_projection, thm:mpdo_bnt_projector_local_selection}
...
\begin{proof}
    \leanok
    \uses{thm:mpdo_four_site_sal_bnt_sector_projectors, thm:mpdo_bnt_projector_local_selection}
...
\begin{theorem}[Separating projectors for a simple tensor satisfying ZCL and SAL]
    ...
    \leanok
    \uses{thm:mpdo_scale_invariant_weight_copy_independent, thm:mpdo_sal_bnt_separating_projectors_copy_independent}
...
\begin{theorem}[Positive-length BNT-sector compression]
    ...
    \leanok
    \uses{def:mpdo_sitewise_physical_matrix, thm:mpdo_bnt_projector_local_selection}
...
\begin{proof}\leanok
    \uses{thm:mpdo_bnt_projector_local_selection, thm:mpdo_four_site_sal_bnt_sector_projectors}
...
\begin{theorem}[Physical-trace transfer after common-weight absorption]
    \label{thm:mpdo_absorbed_bnt_sector_phys_trace_transfer}
    \lean{MPOTensor.commonWeightAbsorbedBasisMPOTensor_toMPSTensor}
    \lean{MPOTensor.physTraceTransfer_commonWeightAbsorbedBasisMPOTensor}
    \leanok
    \uses{def:mpo_physical_trace_transfer, def:mpdo_common_sector_weight_absorption}
    Suppose the weights over each BNT representative are copy-independent.
    For $\mathcal K_s=\mu_sA_s$ and
    $\mathcal B_s=\mathcal T_{A_s}$, one has
    \begin{align}
...
\begin{proof}\leanok
    \uses{lem:mpdo_doubled_phys_trace_transfer_to_mps}
    Substitute $\mathcal K_s=\mu_sA_s$ into the defining diagonal sum for the
    physical-trace transfer and factor out the scalar $\mu_s$.
\end{proof}
...
\begin{proof}\leanok
    \uses{def:global_gauge_of_blocks, lem:mpdo_doubled_phys_trace_transfer_to_mps, lem:mpdo_doubled_phys_trace_transfer_blockwise, lem:mpdo_copy_weight_eq_common_weight, thm:mpdo_absorbed_bnt_sector_phys_trace_transfer, lem:mpdo_simple_blockwise_zcl_equation}
...
\begin{theorem}[Scale-invariant source ZCL descends to absorbed BNT representatives]
    ...
    \leanok
    \uses{def:mpdo_simple_cf_tensor, def:mpo_source_zcl, def:mpdo_common_sector_weight_absorption}
    ...
    precisely, $\mathcal T_{\mathcal K_s}=\mu_s\mathcal B_s\ne0$ and,
    for some $\lambda>0$,
    \begin{align}
        \mathcal T_{\mathcal K_s}^{2}
...
\begin{proof}\leanok
    \uses{cor:mpdo_simple_scale_invariant_zcl_weight_copy_independent, lem:mpdo_scale_invariant_blockwise_zcl_equation, thm:mpdo_absorbed_bnt_sector_phys_trace_transfer}
    Corollary~\ref{cor:mpdo_simple_scale_invariant_zcl_weight_copy_independent}
    gives the common weight on every copy.
    Lemma~\ref{lem:mpdo_scale_invariant_blockwise_zcl_equation} then gives
    the displayed quadratic identity. The common weight is nonzero, and
    nonnilpotence of $\mathcal B_s$ implies $\mathcal B_s\ne0$, so the
...
\begin{remark}[Sectorwise hypotheses in the canonical BNT application]
    ...
    \uses{thm:mpdo_absorbed_bnt_sector_zcl, thm:mpdo_bnt_physical_support_restriction, thm:mpdo_proportional_commuting_sector_positivity, thm:mpdo_canonical_bnt_proportional_sectors_zcl_sal}
...
\begin{theorem}[Strict normalization at positive length]
    ...
    \leanok
    \uses{def:mpdo, def:mpo_source_zcl, lem:mpdo_trace_loop_chain, cor:mpdo_absorbed_bnt_sector_is_mpdo, thm:mpdo_absorbed_bnt_sector_zcl}
...
\begin{theorem}[Saturated area law for the absorbed BNT elements]
    ...
    \leanok
    \uses{def:is_sal, def:mpo_source_zcl, def:mpdo_common_sector_weight_absorption}
...
\begin{proof}\leanok
    \uses{lem:sal_const, thm:mpdo_tripartite_block_entropy_identifications, thm:mpdo_bnt_projector_local_selection, cor:mpdo_absorbed_bnt_sector_is_mpdo, thm:mpdo_source_zcl_strict_positive_length_normalization}
...
\begin{theorem}[Analytic properties of absorbed BNT representatives]
    ...
    \leanok
    \uses{def:injective, def:mpdo, def:is_sal, def:mpo_physical_trace_transfer, def:mpdo_common_sector_weight_absorption}
    ...
    BNT label $s$, set $\mathcal K_s=\mu_sA_s$. Then
...
\begin{proof}\leanok
    \uses{def:global_gauge_of_blocks, def:gauge_equiv, thm:gauge_invariance, lem:mpdo_trace_loop_chain, thm:mpo_source_zcl_of_physical_trace_idempotent, thm:mpdo_bnt_physical_support_restriction, cor:mpdo_absorbed_bnt_sector_is_mpdo, thm:mpdo_absorbed_bnt_sector_sal, thm:mpdo_absorbed_bnt_sector_literal_zcl}
    The block-diagonal gauge identifies the MPV families of $K$ and its
    weighted BNT direct sum. Saturation gives $\tr(\sigma^{(1)}(K))>0$. Since
