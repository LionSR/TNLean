...
\begin{theorem}[Ambient ZCL descends to the canonical absorbed BNT elements]
    ...
    \leanok
    \uses{def:mpdo_simple_cf_tensor,
...
\begin{remark}[Sectorwise hypotheses in the canonical BNT application]
    ...
    injectivity of every representative. Thus the sector theorem applies at
    every original cut without passing through blocked SAL.

    This does not prove the printed GSNNCH-only proposition. An arbitrary
    GSNNCH presentation need not inherit source zero correlation length, since
    that condition belongs to the tensor's physical-trace transfer rather than
    only to its periodic operators.
