...
Moln\'ar et al.~\cite{Molnar2018NormalPEPS} formulate the algebraic proof for periodic chains
whose tensors vary from site to site, whereas P\'erez-Garc\'ia et al.~\cite[Theorem~3]{PerezGarcia2007Matrix}
begin with a varying-bond open chain. Both fixed-length constructions are defined below.

\begin{definition}[Periodic non-TI chain data]\label{def:non_ti_chain}
    \lean{MPSChainTensor.coeff, MPSChainTensor.IsInjective}
    \leanok
    \uses{def:mps_tensor, def:injective, def:periodic_mps_tensor, def:periodic_relations}
...
\begin{definition}[Square padding of a varying-bond open chain]
    ...
    This square padding embeds the rectangular open chain into square matrices,
    whereas \cite[Theorem~3]{PerezGarcia2007Matrix} places the rectangular
    matrices into the off-diagonal blocks of an $ND\times ND$ cyclic matrix.
\end{definition}
...
whose size does not depend on the chain length. The system and its uniqueness
criteria follow \cite[Theorems~7 and~8]{PerezGarcia2007Matrix}.
...
\begin{lemma}[Chain of intertwiners between two solutions]
    ...
    intertwiners. This yields the internal part of the chain in
    \cite[Theorem~8]{PerezGarcia2007Matrix}; the full linear dependence required by
    Lemma~\ref{lem:pgvwc07_chain_combination} incorporates the boundary cuts.
\end{lemma}
...
\begin{theorem}[Obtaining the translation-invariant canonical form]
    ...
    \begin{enumerate}
        \item If the state admits a translation-invariant matrix product
              representation satisfying injectivity (condition C1), then the system has a
              solution.
    ...
    \end{enumerate}
    The unrestricted representation claim and exact conjugacy in
    \cite[Theorem~8]{PerezGarcia2007Matrix} carry a residual finite-ring phase defect
    \cite{gap:pgvwc07_quadratic_reconstruction_phase}, which is resolved by the explicit
    phase $\xi$ in (\ref{eq:pgvwc07_quadratic_reconstruction_phase_conjugacy}).
\end{theorem}
...
\begin{proof}
    ...
    under injectivity give $|x|=1$ and
...
intertwiner between the two local tensors. The cut-vector argument supplies
these intertwiners directly from injectivity, yielding the fixed-length
intertwiner and the uniqueness theorem. The printed conclusion is adjusted
on a finite ring by an $N$th-root phase
\cite{gap:pgvwc07_quadratic_reconstruction_phase, gap:pgvwc07_ti_uniqueness_scope}.
...
\begin{lemma}[Linear independence of the cut vectors]
    ...
    length $L$. If $\sum_iA_iA_i^\dagger=\Id$ as well, then $A$ is
...
\begin{theorem}[Uniqueness of the canonical form, corrected]
    ...
    Uniqueness of the open-boundary canonical representation is not needed;
    Theorem~\ref{thm:pgvwc07_fixed_length_intertwiner} obtains the intertwiners
    directly from injectivity, yielding the conclusion recorded in
...
\begin{lemma}[Virtual bond gauge -- 3-site case]%
    ...
    uniqueness of $Z$ up to scalar; the three-site formulation here isolates
    the local algebraic content.
\end{lemma}
