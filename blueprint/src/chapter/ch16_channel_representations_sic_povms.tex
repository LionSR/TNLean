...
\begin{theorem}[Operator-basis property]
    ...
    \leanok
    \uses{thm:sic_povm_overlap_bound_equality,
      thm:sic_povm_equality_linear_independent,
      thm:sic_povm_singleton_linear_independent}
    The $d^2$ projectors in a symmetric informationally complete family form a
    basis of $\MN{d}$.
\end{theorem}

\begin{proof}\leanok
    For $d\ge2$, the family realizes equality in the SIC--POVM overlap bound,
    so Theorem~\ref{thm:sic_povm_equality_linear_independent} gives linear
    independence. For $d=1$, the family is a nonzero singleton. Since
    $\dim\MN{d}=d^2$, linear independence implies the basis property.
\end{proof}

\begin{theorem}[Diagonal representation]
    \label{thm:sic_povm_diagonal_representation}
    \lean{SICPOVM.diagonalRepresentation}
    \leanok
...
\begin{theorem}[SIC Kraus channel]
    ...
    \lean{SICPOVM.krausChannel_identity}
    \lean{SICPOVM.krausMap_eq}
    \lean{SICPOVM.isChannel_krausMap}
    \leanok
    \uses{thm:sic_povm_diagonal_representation,
      thm:sic_povm_normalizations}
