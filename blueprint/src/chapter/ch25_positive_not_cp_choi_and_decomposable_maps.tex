...
\begin{lemma}[Decomposable maps preserve positivity on PPT states]
    \label{lem:decomposable_map_tensor_id_ppt}
...
\begin{proof}
    \leanok
    \uses{def:decomposable_positive_map, def:is_ppt,
        thm:cp_map_entrywise_complete_positive,
        thm:partial_transpose_left_involution}
    Write $\Phi=\Phi_{\mathrm{cp}}+\Phi_{\mathrm{ccp}}$ with
    $\Phi_{\mathrm{cp}}$ completely positive and $\Phi_{\mathrm{ccp}}$
    completely copositive. Complete positivity is entrywise positivity on
    block matrices, so the ampliation of $\Phi_{\mathrm{cp}}$ preserves
    positivity: $(\Phi_{\mathrm{cp}}\otimes\id)(\rho)\ge0$. For the second
    summand, ampliating $\Phi_{\mathrm{ccp}}$ after transposition equals
    ampliating $\Phi_{\mathrm{ccp}}$ after the first-factor partial transpose
    of the input,
    \begin{align}
...
\begin{lemma}[A PPT state detected by an ampliation witnesses indecomposability]
    \label{lem:indecomposable_of_ppt_witness}
    \lean{not_isDecomposablePositiveMap_of_isPPT_not_tensorMapId_posSemidef}
    \leanok
    \uses{def:decomposable_positive_map, def:is_ppt,
        lem:decomposable_map_tensor_id_ppt}
...
\begin{proof}
    ...
    Lemma~\ref{lem:decomposable_map_tensor_id_ppt}.
\end{proof}
