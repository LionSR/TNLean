...
\begin{theorem}[Gauge covariance of the F-matrices]%
    ...
    $N_{ab}^{c}\times N_{ab}^{c}$ matrix, and set
    \begin{align}
        X'^c_{ab,\mu}
         & =\sum_\nu (Y^c_{ab})_{\nu\mu}X^c_{ab,\nu},
         &
        X'^{c+}_{ab,\mu}
         & =\sum_\nu \bigl((Y^c_{ab})^{-1}\bigr)_{\mu\nu}X^{c+}_{ab,\nu}. \notag
    \end{align}
...
\begin{proof}\leanok
    \uses{def:mpdo_complete_zipper_fusion_family, thm:mpdo_complete_zipper_printed_fmove}
    Write $\Pi_{ab}=\bigoplus_c Y^c_{ab}\otimes\Id_{D_c}$. The new synthesis
    and analysis matrices are $X_{ab}\Pi_{ab}$ and $\Pi_{ab}^{-1}X^+_{ab}$.
    Since $\Pi_{ab}$ acts only on the multiplicity factors, it commutes with
    $\bigoplus_c\Id\otimes B_c^{ik}$. The biorthogonality, reconstruction and
    zipper identities of the new family follow.
