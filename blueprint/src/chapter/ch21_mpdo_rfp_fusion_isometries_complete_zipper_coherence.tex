...
\begin{theorem}[Complete zipper family from split compressions]%
    ...
    In particular, if the word traces of every product $B_{ab}$ are the sums of
    the word traces of $N_{ab}^{c}$ copies of each $B_c$, and the letters of
    every product tensor generate an algebra closed under conjugate
    transposition, then such compressions exist and the conclusion holds.
\end{theorem}
\begin{proof}\leanok
    \uses{def:mpdo_complete_zipper_fusion_family, thm:asym_multiblock}
    ...
    In the star-closed case the word module of each product is semisimple, so
...
\begin{theorem}[Simultaneous left inverse after blocking]%
    ...
    \leanok
    \uses{thm:mpdo_complete_zipper_of_compression}
    Let $B_1,\ldots,B_m$ be MPO blocks of positive bond dimension which,
    read as tensors on the doubled physical alphabet and rescaled by nonzero
    scalars, are normal tensors, and no two of which are gauge equivalent up
    to a nonzero scalar. Then there is $L>0$ such that the words of length
    $L$ span $\bigoplus_cM_{D_c}$ jointly, so the blocked tensors
    $B_c^{[L]}$ have a simultaneous left inverse at one blocked site. If
    ...
    inverse among the hypotheses. The same holds in the star-closed case.
...
\begin{proof}\leanok
    \uses{thm:mpdo_complete_zipper_of_compression}
    Rescaling a block by a nonzero scalar multiplies its words of length $L$
    ...
    and the coefficients expressing each matrix unit of each summand give the
    simultaneous left inverse. Each blocked letter of a product is a word of
    the unblocked letters. In the coordinates of a split compression, every
    unblocked letter is block diagonal with blocks $B_c^{ik}$, so every word
    is block diagonal with the corresponding words of $B_c$. The compression
