...
\begin{theorem}[Complete zipper family from split compressions]%
    ...
    the word traces of $N_{ab}^{c}$ copies of each $B_c$, and
    $(B_{ab}^i)^\dagger$ lies in the unital algebra generated by the
    $B_{ab}^j$ for every $i$, then such compressions exist and the conclusion
    holds.
\end{theorem}
\begin{proof}\leanok
    \uses{def:mpdo_complete_zipper_fusion_family, thm:asym_multiblock,
