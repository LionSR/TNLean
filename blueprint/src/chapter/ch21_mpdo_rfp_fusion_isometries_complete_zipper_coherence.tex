...
\begin{proof}\leanok
    ...
    For $G=G_{\mathrm R}^{-1}F^{abc}_dG_{\mathrm L}$, the $F$-move
    (\ref{eq:rfp_printed_fmove}) of the original family gives
    \begin{align}
        R'(G\otimes\Id)
         & =R\bigl(G_{\mathrm R}G_{\mathrm R}^{-1}F^{abc}_dG_{\mathrm L}
        \otimes\Id\bigr)
        =R(F^{abc}_d\otimes\Id)(G_{\mathrm L}\otimes\Id)
        =L(G_{\mathrm L}\otimes\Id)=L'.
        \notag
    \end{align}
    By the uniqueness just shown, applied to the new family,
    $F'^{abc}_d=G_{\mathrm R}^{-1}F^{abc}_dG_{\mathrm L}$, which is
    (\ref{eq:rfp_fmatrix_gauge}); multiplying on the left by $G_{\mathrm R}$
    gives the intertwining form $G_{\mathrm R}F'^{abc}_d=F^{abc}_dG_{\mathrm L}$.
    For the inverse, let
    $H=G_{\mathrm L}^{-1}(F^{abc}_d)^{-1}G_{\mathrm R}$. By
    (\ref{eq:rfp_fmatrix_gauge}),
    \begin{align}
        F'^{abc}_dH
         & =G_{\mathrm R}^{-1}F^{abc}_dG_{\mathrm L}G_{\mathrm L}^{-1}
        (F^{abc}_d)^{-1}G_{\mathrm R}=\Id,
        \notag                                                         \\
        (F'^{abc}_d)^{-1}
         & =(F'^{abc}_d)^{-1}\bigl(F'^{abc}_dH\bigr)
        =\bigl((F'^{abc}_d)^{-1}F'^{abc}_d\bigr)H=H.
        \notag
    \end{align}
\end{proof}
