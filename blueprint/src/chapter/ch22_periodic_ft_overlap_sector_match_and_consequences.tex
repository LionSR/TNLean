...
\begin{theorem}[Eventual linear independence of periodic vectors]%
    ...
    \textbf{Scope restriction (linear independence without spanning):}
    This theorem establishes eventual linear independence along the common-period
    subsequence without proving that these periodic vectors span the full periodic MPS space
    asserted in \cite[lines~589--609]{DeLasCuevas2017Irreducible}; documented in
    \cite{gap:dccsp17_periodic_overlap_route_alignment}
    (\texttt{docs/paper-gaps/dccsp17_periodic_overlap_route_alignment.tex}).
\end{theorem}
...
\begin{theorem}[Scalar-weight specialization of the mixed-overlap theorem]
    ...
    unit-modulus scalar.
    This specializes \cite[Theorem~3.4]{DeLasCuevas2017Irreducible} to one
    positive scalar per listed block and direct-sum equality at all lengths.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Finite matching from the periodic overlap dichotomy]
    ...
    $j\in J$.
    This isolates the finite matching step deduced from the overlap dichotomy
    in \cite[Theorem~3.4]{DeLasCuevas2017Irreducible}.
\end{theorem}
\begin{proof}\leanok
