...
\begin{theorem}[Eventual linear independence of periodic vectors]%
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Following \cite[lines~589--609]{DeLasCuevas2017Irreducible}, this
    establishes linear independence of the periodic vectors
    (\cite{gap:dccsp17_periodic_overlap_route_alignment}).
\end{theorem}
...
\begin{theorem}[Absorbing the phase of a repeated-block relation]
    ...
    is asserted for the direct-sum tensors
...
\begin{proof}
    ...
    \leanok
    Take $\xi$ to be the reciprocal of the phase from
    Theorem~\ref{thm:repeated_blocks_phase_power_mpv}, which
...
\begin{proof}
    ...
    unity, so Theorem~\ref{thm:roots_of_unity_determine_period} applies.
\end{proof}
...
\begin{proof}
    ...
    \leanok
    Repeated blocks have the same peripheral spectrum by
    Theorem~\ref{thm:repeated_blocks_peripheral_spectrum}.
    Theorem~\ref{thm:spectral_period_eq_of_peripheral_spectrum} then
    identifies their periods.
\end{proof}
...
\begin{theorem}[Non-decaying mixed overlaps for normalized bases of periodic
    ...
    \emph{Normalization.} In \cite[lines~313--332]{DeLasCuevas2017Irreducible},
    the trace-preserving normalization is obtained via a block-diagonal
    similarity from irreducible transfer maps of spectral radius one. Here,
    the basis tensors are taken to be normalized in advance;
    Theorem~\ref{thm:periodic_spectral_multiplicity_nondecaying_overlap} transfers
    the result to unnormalized spectrally periodic blocks.
\end{theorem}
\begin{proof}\leanok
...
\begin{proof}
    ...
    \leanok
    Normalize all representatives by their pure Perron gauges and combine
...
\begin{theorem}[Scalar-weight specialization of the mixed-overlap theorem]
    ...
    This specializes Theorem~\ref{thm:periodic_multiplicity_nondecaying_overlap}
    to block-diagonal tensors with a single scalar weight per block.
\end{theorem}
\begin{proof}\leanok
    Repeat the argument of
    Theorem~\ref{thm:periodic_multiplicity_nondecaying_overlap} with the
    singleton coefficients $\mu_j^N$ and $\nu_k^N$.  Their nonvanishing at
    every length replaces the geometric-extrapolation step.  Equality with the
    two scalar-weight direct sums transports the positive-length
    proportionality relation to the corresponding block-diagonal tensors.
\end{proof}
...
\begin{theorem}[Finite matching from the periodic overlap dichotomy]
    ...
    This is the finite matching step of
...
\begin{theorem}[Proportional Fundamental Theorem for irreducible-form MPS]
    ...
    This theorem matches the bases of periodic tensors, leaving the
    multiplicity matrices unconstrained. The periods $m_j$ and $n_k$ are
    derived from the transfer maps
...
\begin{theorem}[Proportional Fundamental Theorem, periods supplied]%
    ...
    \leanok
    \uses{def:nonzero_proportional_mpv, def:spectral_periodic_tensor}
    Under the hypotheses of Theorem~\ref{thm:periodic_ft_proportional}, suppose
    in addition that positive integers $m_j$ and $n_k$ are given such that each
    $A_j$ is spectrally $m_j$-periodic and each $B_k$ is spectrally $n_k$-periodic
    (Definition~\ref{def:spectral_periodic_tensor}). Then the conclusions of
    Theorem~\ref{thm:periodic_ft_proportional} hold for these specified periods.
\end{theorem}
\begin{proof}
