...
\begin{theorem}[Non-decaying mixed overlaps for scalar-weight irreducible-form
    representations]
    ...
    every $N\geq0$, as a scalar-weight block-diagonal tensor formed from a
    finite, pairwise non-repeated family of
    left-canonical periodic tensors
    $\{A_j\}_{j\in J}$ and $\{B_k\}_{k\in K}$, with a positive real scalar
    weight attached to each block. Suppose that for every $N\geq 1$ there is
    a scalar $c_N\ne0$ such that
    \begin{align}
    ...
    \emph{Scope restriction.} The irreducible forms in
    \cite[lines~286--332 and 575--585]{DeLasCuevas2017Irreducible} carry
    multiplicity matrices $R_j$ and $S_k$, use literal block-diagonal tensor
    equalities, and require only irreducible transfer maps of spectral radius
    one. The present theorem has one positive real scalar weight per block,
    records equality of matrix-product-vector families with the corresponding
    scalar-weight block-diagonal tensors,
    including equality at the empty word. This forces equality of the
    corresponding bond-dimension traces, whereas the source physical family is
    positive-length and allows a lower-dimensional representative. The theorem
    also assumes left-canonical normalization through periodicity. It therefore
    supplies the mixed-overlap step only for these witnesses and does not by
    itself prove the unrestricted proportional Fundamental Theorem. These
    distinctions are recorded in
    \path{docs/paper-gaps/1708_periodic_overlap_route_alignment.tex}.
    Removing the length and normalization restrictions requires replacing the
    empty-word equality by a positive-length comparison, applying the source's
    block-diagonal similarity to trace-preserving form and transport of the
    conclusion back to the original blocks; that reduction is not part of the
    present declaration.
\end{theorem}
\begin{proof}\leanok
    ...
    Expand the proportionality relation at length $pN$ in the two block
    families. Replace each term indexed by $j\notin T$ by its scalar multiple
    of the corresponding $B_{k(j)}$ term, and collect terms with the same
    value of $k(j)$. Linear independence then forces
    $\mu_{j_0}^{pN}=0$ at one sufficiently large positive $N$, contradicting
    the positivity of $\mu_{j_0}$. Hence $j_0$ has a non-decaying partner.
...
\begin{theorem}[Proportional Fundamental Theorem for irreducible-form MPS]
    ...
    % Formalization status: the scalar-weight theorem above proves only its
    % explicitly restricted overlap step. The unrestricted multiplicity-bearing
    % argument remains tracked in issue #5342.
\end{theorem}
