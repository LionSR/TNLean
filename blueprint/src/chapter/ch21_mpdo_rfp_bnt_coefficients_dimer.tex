...
\begin{theorem}[An RFP with length-dependent BNT structure coefficients]%
    ...
    There is a tensor $M$ in canonical form that generates matrix product
    density operators, is a renormalization fixed point, and
    has a vertical basis-of-normal-tensors presentation with one element and structure
    coefficient $c^{(L)}=(\sqrt{1/2})^L$.
    In particular, $c^{(1)}\ne c^{(2)}$, so these structure coefficients
    for the exhibited presentation depend on the length.
\end{theorem}
\begin{proof}\leanok
    ...
    The bond-dimension-two tensor with a one-element vertical basis of normal tensors
    generates positive operators on every nonempty chain, is in canonical form, and
    satisfies the displayed multiplication law and the corresponding
    tensor decomposition. The reverse implication of
    Theorem~\ref{thm:mpdo_bnt_algebra_tensor_to_rfp} supplies the two
    trace-preserving completely positive maps required for a renormalization fixed point. Finally,
...
\begin{theorem}[Length dependence under every uniform positive rescaling]%
    ...
    The unscaled coefficients are the structure coefficients of an explicit
    one-element vertical basis-of-normal-tensors presentation of $R$,
    motivated by the discussion in \cite[lines~995--1010]{Cirac2016MPDO_arXiv}.
\end{theorem}
\begin{proof}\leanok
...
The gates in this factorization act on an incoming bond and its receiving
flag across neighboring physical sites. After the unitary conjugation by $U_N$,
the bond and flag factors decouple, yielding the state-level product form.
...
The flag factor is identified using the density-normalized tensor
$\widehat M$; the constant multiplication coefficients for the unnormalized
matrices $\Id,Z$ differ by scaling from those for the spectrally normalized
...
\begin{theorem}[Channel equations for the mixed-Bell dimer]%
    ...
    Definition~\ref{def:mpdo_twisted_dimer_bond_channels} are trace-preserving
...
\begin{proof}\leanok
    ...
    of basis coordinates, and partial trace are trace-preserving completely
    positive maps.
...
These channel equations establish that the mixed-Bell bond dimer is a
renormalization fixed point, with nonzero bond-state eigenvalues $7/8$
and $1/8$.
...
The separation $H_N$ is on-site, while the conjugating operator $U_N$ couples
neighboring sites; at positive chain length $N \ge 1$, this decouples the bond
qubits from the site flags.
...
\begin{theorem}[The twisted quantum dimer is a renormalization fixed point]%
    ...
    point in the sense of Definition~\ref{def:is_rfp_via_ts}.
    The refinement map $\mathcal T$ is completely positive and trace preserving,
    with eight Kraus operators, labelled by the two prepared site flags
    ...
    Flag addition is modulo two.
    The coarse-graining map $\mathcal S$ is completely positive and trace
    preserving, with sixteen Kraus operators, labelled by a member of the orthonormal
