...
\begin{lemma}[Boundary identities from block-sum membership]
    ...
    \leanok
    \uses{lem:block_matrices_equal_from_trace_pairings, def:blockwise_product_word_span, def:ground_space_parent}
    Let $A^1,\ldots,A^g$ be block tensors whose length-$m$ simultaneous word
...
\begin{proof}
    \leanok
    \uses{lem:block_matrices_equal_from_trace_pairings, def:ground_space_parent, def:ground_space_map}
...
\begin{lemma}[Explicit $C^j,D^j,E^j$ identities from trace decompositions]
    ...
    \leanok
    \uses{lem:sum_evalWord_mul_conjTranspose_evalWord, lem:blockwise_word_boundary_compatibility_from_traces, lem:normalized_boundary_matrix_identities}
    Let $A^1,\ldots,A^g$ be block tensors with the common word-span property
...
\begin{lemma}[Complementary-word boundary identities from trace decompositions]
    ...
    \leanok
    \uses{lem:sum_evalWord_mul_conjTranspose_evalWord, lem:blockwise_word_boundary_compatibility_from_traces, lem:complementary_word_boundary_identities_from_pgvwc}
    Let $A^1,\ldots,A^g$ be block tensors with the common word-span property
