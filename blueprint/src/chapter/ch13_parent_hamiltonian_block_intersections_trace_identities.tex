...
\begin{lemma}[Sharp BNT block separation without an injective prefix]
    ...
    Let $A^1,\ldots,A^r$ satisfy the normalization, irreducibility,
    non-equivalence, injective-prefix, and full-word-span hypotheses of
    Lemma~\ref{lem:bnt_direct_sum_injective_prefix_word_span} for some
    $L_0>0$. If $r\geq2$, then the simultaneous block-word tuples at the
    source length $3(r-1)(L_0+1)$ span the full product algebra
    $\prod_{j=1}^r\MN{D_j}$.
\end{lemma}
...
\begin{proof}
    ...
    Lemma~\ref{lem:pair_product_span_arbitrary_first_value} gives a
    length-$3(L_0+1)$ tuple $M$ with $(M_k,M_j)=(X_k,0)$. For every
    $\ell\notin\{k,j\}$, choose a selector $N^{(\ell)}$ with
    $N^{(\ell)}_k=\Id$ and $N^{(\ell)}_\ell=0$, and put
    $C_t = M_t\prod_{\ell\notin\{k,j\}}N^{(\ell)}_t$.
    Then
    \begin{align}
        C_k &= X_k,
        ...
        C_j &= 0,
        \notag\\
        C_\ell &= M_\ell(\cdots)0(\cdots) = 0 \quad (\ell\notin\{k,j\}).
        \notag
    \end{align}
