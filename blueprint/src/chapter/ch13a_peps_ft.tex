...
\begin{theorem}[Per-edge uniqueness of the torus gauge up to a constant]%
    ...
    \begin{proof}
        ...
        determined by this identity, so the two conjugations agree on every
        bond matrix; a matrix commuting with the whole bond matrix algebra is
        a scalar, so the two absorbing matrices differ by a nonzero scalar.
        Unwinding the orientation adaptation --- a transpose on one branch, an
        inverse on the other --- gives the proportionality of the matrices
        themselves, with the constant inverted on the inverse branch, which
        the final scalar absorbs.
    \end{proof}
\end{theorem}
...
\begin{theorem}[Class constancy of the gauge comparison scalars]%
    ...
    \begin{proof}
        ...
        reference edge by a unique translation; applying the covariance of
        both families along this translation carries the reference
        proportionality to the edge --- unchanged when the translation
        preserves the stored endpoint order, since reindexing commutes with
        the scalar, and with the constant inverted when it swaps the order,
        since the transposed inverse of a proportionality inverts its
        constant.  At a right (up) edge the order is preserved exactly when
        the stored second endpoint is the cyclic successor of the first,
        which is the stated marker.
    \end{proof}
\end{theorem}
