...
\begin{lemma}[Normalized horizontal-edge blocking data]%
    ...
    closure give injectivity of all three blocks.
\end{lemma}
% Maintainer note: this is the normalized horizontal-edge instance.  The
% translated every-edge construction and the vertical-edge instance remain
% separate steps.
\begin{proof}\leanok
    The endpoint memberships are coordinate checks.  Rectangular injectivity
    gives the two edge blocks, the collar lemma gives the complement, and the
    disjointness and cover statements are the finite-lattice complement
    identities for the two removed edge blocks.
\end{proof}
