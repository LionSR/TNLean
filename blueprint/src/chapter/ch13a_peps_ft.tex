...
\begin{lemma}[The normalized edge complements have collars]%
    ...
    \(T\)-injectivity implies injectivity of the vertical complementary block.
\end{lemma}
...
\begin{lemma}[Normalized edge-blocking data]%
    ...
    injectivity is either kept as an explicit hypothesis or obtained from
    rotated \(T\)-injectivity by the vertical collar lemma.  In both cases the
    named blocking datum supplies the corresponding three injective regions for
    the edge-blocked chain.
\end{lemma}
% Maintainer note: vertical-edge complement injectivity is now reduced to
% rotated T-injectivity.  The translated every-edge construction remains a
% separate step.
\begin{proof}\leanok
