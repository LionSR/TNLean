...
\begin{theorem}[Edge blocking preserves injectivity]%
    ...
    spaces; without it the middle tensor can vanish identically while vertex
    injectivity survives vacuously, so the assertion is false.
\end{theorem}
\begin{proof}\leanok
    ...
    \[
        ...
        K_c(S\setminus\{j\}).
    \]
    The contraction at $j$ uses the star-bond split that separates the bonds
    incident to $j$ from the rest. The terminal step
