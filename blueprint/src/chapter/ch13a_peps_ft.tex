...
\begin{definition}[Complement two-block tensor]%
    ...
    \leanok
    \uses{def:peps_tensor, def:peps_vertexTwoBlock}
...
\end{theorem}
\begin{proof}\leanok
    Let $c=(c_\eta)_\eta$ be a family of coefficients on the open star of $v$.
    The vanishing condition for the complement tensor is
    \[
        \sum_{\eta} c_\eta\,T_{v^c}(*,\eta,\tau)=0
    \]
    for every physical configuration $\tau$ on $V\setminus\{v\}$. Expanding
    $T_{v^c}$ as a contraction over the internal virtual indices gives the
    kernel-descent condition
    \[
        K_{V\setminus\{v\}}(c)=0.
    \]
    Removing one vertex at a time with vertex injectivity yields
    \[
        K_{V\setminus\{v\}}(c)=0
        \Longrightarrow
        K_{\varnothing}(c)=0
        ...
        c_\eta=0
    \]
    for every star configuration $\eta$. Hence the complement two-block tensor
    is injective.
\end{proof}
