...
\begin{lemma}[The present local \(T\)-windows have no rectangular cover]%
    ...
    \(2\times3\) and \(3\times2\) rectangles.  The rotated normalized
    \(6\times5\) local-window model used for vertical edges has the analogous
    obstruction.
\end{lemma}
...
\begin{proof}\leanok
    ...
    argument with the two shapes interchanged.
\end{proof}
...
\begin{lemma}[The normalized edge complements have collars]%
    ...
    contiguous \(2\times3\) rectangles.  For the rotated vertical edge-blocking
    regions, the corresponding complement is the rotated local \(T\)-region
    together with the same two right-collar rectangles; hence rotated
...
\begin{definition}[Translated edge windows]%
    ...
    \lean{TNLean.PEPS.HasNormalSquareHorizontalEdgeMargins}
    \lean{TNLean.PEPS.HasNormalSquareVerticalEdgeMargins}
    ...
    upward representative.  The oriented margin-and-cover data package is the
    ...
    margin-cover package stores these named predicates, so such data force the
    corresponding horizontal and vertical margin inequalities.  In the present
    open rectangular coordinate model, this margin package does not apply to
...
\begin{theorem}[Assembly from translated edge windows]%
    ...
    If every edge of the finite square lattice is supplied with a translated
    edge window, then the corresponding one-edge blockings assemble into the
    normal edge-blocking hypotheses.  This is the conditional assembly step;
    constructing such windows for all edges of the \(7\times7\) lattice remains
    separate; the current open rectangular translated-window package does not
    ...
    assembled hypotheses is the datum supplied by the corresponding
    margin-and-cover input, and the assembled hypotheses give the three
...
\end{theorem}
\begin{proof}\leanok
    Apply the general assembly map for one-edge blocking data to the datum
...
\begin{definition}[Normal square-lattice blocking regions]%
    ...
    around every edge.  Conversely, for the coordinate square lattice, the
    rectangular hypotheses, union closure, and a rectangular cover of the
    displayed \(T\)-region give this abstract \(R,S,T\) package, with \(R\)
    and \(S\) obtained from their rectangular decompositions and \(T\) obtained
    from the supplied cover.  The source regions are:
    \begin{center}
