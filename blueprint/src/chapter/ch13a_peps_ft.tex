...
\begin{theorem}[Edge-blocked insertion algebra isomorphism]%
    ...
    the edge $e$ are injective, every bond dimension is positive, and the two
    PEPS tensors define the same state. Then
    \[
        \exists \Phi_e:\MN{D_A(e)}
        \simeq_{\mathrm{alg}}
        \MN{D_B(e)},\qquad
...
\begin{theorem}[Gauge consistency]%
    ...
    Assume every bond dimension is positive. Choose an edge $e=(u,v)$ and block
    all other vertices into one middle
...
\begin{proof}
    ...
    $W=\sum_i W_i^{(1)}\otimes W_i^{(2)}$. Equation~(\ref{eq:peps_residual_equality}) is exactly
    \[
        \sum_i I\otimes Z_i^{(1)}\otimes Z_i^{(2)}
        =
        \sum_i U_i^{(1)}\otimes U_i^{(2)}\otimes I
...
\begin{theorem}[Zero-bond obstruction to the unrestricted PEPS theorem]%
    \label{thm:peps_ft_zero_bond_obstruction}
    \lean{TNLean.PEPS.FundamentalTheoremCounterexample.fundamentalTheoremPEPSStatement_false}
    \leanok
    \uses{def:IsVertexInjective, def:peps_same_state, def:GaugeEquivPEPS}
    If zero-dimensional virtual bonds are allowed, vertex-injectivity and
    equality of all PEPS state coefficients do not imply gauge equivalence. More
    precisely, there are a finite graph, a physical dimension, and PEPS tensors
    $A,B$ such that
    \[
        A\ \text{and}\ B\ \text{are vertex-injective},\qquad
        A \sim_{\mathrm{state}} B,\qquad
        A\not\sim_{\mathrm{gauge}} B.
    \]
\end{theorem}
\begin{proof}\leanok
    Take the triangle graph. Let two of the three edge spaces have dimension
    zero for both tensors, and let the remaining edge have dimension $1$ for
    $A$ and dimension $2$ for $B$. The zero-dimensional edges make every local
    and global virtual-configuration set empty. Hence vertex-injectivity and
    equality of the represented state are vacuous. Gauge equivalence would
    nevertheless identify all virtual edge dimensions, which would force
    $1=2$ on the remaining edge.
\end{proof}
...
\begin{theorem}[Fundamental Theorem for injective PEPS, conditional on
    ...
    already identified. If $A$ and $B$ are vertex-injective with positive bond
    dimensions and give the same
...
\begin{theorem}[Fundamental Theorem for injective PEPS]%
    ...
    \uses{thm:fundamentalTheorem_PEPS_of_bondDim, def:GaugeEquivPEPS,
        def:IsVertexInjective}
    Let $A$ and $B$ be vertex-injective PEPS tensors with positive bond
    dimensions on a finite simple
    ...
    \cite{Molnar2018NormalPEPS}; the positive-bond condition is that source's
    standing assumption that injective PEPS have nonzero virtual bond spaces.
\end{theorem}
...
\begin{theorem}[Gauge uniqueness modulo balanced edge scalars]%
    ...
    If two gauge families relate the same vertex-injective PEPS tensor with
    positive bond dimensions to the
