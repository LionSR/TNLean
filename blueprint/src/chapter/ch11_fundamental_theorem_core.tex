...
normalization of Definition~\ref{def:sector_bnt_cf}.

The proportional theorem below allows nonzero projective proportionality for
all sufficiently large lengths; proportionality at every positive length is a
special case.
...
\begin{theorem}[Proportional sector matching with unit phases]%
    ...
    \leanok
    \uses{def:sector_bnt_cf, thm:sector_bnt_proportional_bijective_match, lem:sector_bnt_norm_phase_of_matched_mpv}
...
\begin{proof}\leanok
    \uses{thm:sector_bnt_proportional_bijective_match, lem:sector_bnt_norm_phase_of_matched_mpv}
...
\begin{definition}[Copy-weight matching from matched-sector coefficient identities]%
    ...
    \leanok
    \uses{def:sector_bnt_copy_weight_matching, lem:sector_bnt_matched_sector_weight_equiv}
...
\begin{theorem}[Equal-case sector and copy matching with differing bond dimensions]%
    ...
    \leanok
    \uses{def:same_mpv2_pos, def:sector_bnt_cf, def:gauge_phase_equiv, thm:sector_bnt_bijective_match_of_sameMPV, lem:sector_bnt_coeff_identity_via_matched_mpv_phase, def:sector_bnt_copy_weight_matching_of_coeff_identity}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This requires only the global weight normalization of
    Definition~\ref{def:sector_bnt_cf}. This is the sector-data form of
...
\begin{proof}\leanok
    \uses{thm:sector_bnt_bijective_match_of_sameMPV, lem:sector_bnt_coeff_identity_via_matched_mpv_phase, def:sector_bnt_copy_weight_matching_of_coeff_identity}
...
\begin{proof}\leanok
    \uses{lem:sector_bnt_equal_matched_copy_weight_witnesses, thm:sector_bnt_global_gauge_of_copy_weight_matching}
...
\begin{theorem}[Literal equal-MPV gauge after sector-coordinate permutation]%
    ...
    BNT form of
    \cite[Corollary~2.11 and Appendix~A, lines~1189--1192]
    {Cirac2016MPDO_arXiv}.
\end{theorem}
