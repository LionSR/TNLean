...
\begin{lemma}[Block separation core]\label{lem:block_sep_core}
    ...
    \]
    If $\sum_{k=1}^r \mu_k^N (V^{(N)}(A_k)_\sigma - V^{(N)}(B_k)_\sigma) = 0$
    for all $N$ and $\sigma$,
    then each pair $(A_k, B_k)$ generates the same MPV
    family~\cite[Appendix~\texttt{AppendixMPV}, lines~1054--1192]{Cirac2016MPDO_arXiv}.
\end{lemma}

\begin{proof}\leanok
    \uses{thm:overlap_decay, thm:overlap_decay_rect}
    By induction on the number of blocks~$r$.
    In the inductive step, take bilinear overlaps with the leading block $A_1$:
    from
    $\sum_{k=1}^r \mu_k^N (V^{(N)}(A_k)_\sigma - V^{(N)}(B_k)_\sigma) = 0$
    one obtains
    \[
        \sum_{k=1}^r \overline{\mu_k}^N
        (O_{A_1 A_k}(N) - O_{A_1 B_k}(N)) = 0.
    \]
    After division by $\overline{\mu_1}^N$,
    the contributions from $k \ge 2$ decay because
    $|\mu_k/\mu_1| < 1$ (strict weight ordering) and the bilinear overlaps
    $O_{A_1 A_k}(N)$ and $O_{A_1 B_k}(N)$ are bounded
    (Theorems~\ref{thm:overlap_decay_rect}
    and~\ref{thm:overlap_decay} supply the relevant bounds), leaving
    $O_{A_1 A_1}(N) - O_{A_1 B_1}(N) \to 0$.
    Condition~(5) gives $O_{A_1 A_1}(N) \to 1$, hence
    $O_{A_1 B_1}(N) \to 1$.
    Since $O_{A_1 B_1}(N) \to 1 \neq 0$, the bond dimensions must match:
    $D_{A_1} = D_{B_1}$; otherwise Theorem~\ref{thm:overlap_decay_rect}
    would give $O_{A_1 B_1}(N) \to 0$, a contradiction.
    With equal bond dimensions established, the contrapositive of
    Theorem~\ref{thm:overlap_decay} forces $A_1$ and $B_1$
    to be gauge-phase equivalent; the limit $O_{A_1 B_1}(N) \to 1$
    then makes the phase trivial, so $A_1$ and $B_1$
    generate the same MPV family.
    Peeling off the leading block and dividing by
    $\mu_1^N$, the induction hypothesis applies.
\end{proof}
...
\begin{lemma}[Block separation from canonical form]\label{lem:block_sep}
    ...
    $(A_k, B_k)$ generates the same MPV
    family~\cite[Appendix~\texttt{AppendixMPV}, lines~1054--1192]{Cirac2016MPDO_arXiv}.
\end{lemma}
...
\begin{proof}
    ...
    a decaying tail. Concretely, one isolates the leading component
    $\mu_1^N V^{(N)}(A_1)_\sigma$ and bounds the remaining contribution by
...
\begin{proof}\leanok
    \uses{thm:ncf_overlap_tendsto_one, thm:modulus_one_gauge_irr_tp, lem:block_sep}
    The proof follows the same peeling argument as Lemma~\ref{lem:block_sep},
    with Theorem~\ref{thm:modulus_one_gauge_irr_tp} (modulus-one eigenvalue rigidity
    ...
    Since both block families already have the same bond dimensions,
    the conclusion is direct per-block same MPV.
\end{proof}

\begin{lemma}[Canonical-form gauge comparison, same-structure case]\label{lem:ft_canonical}
    \lean{MPSTensor.fundamentalTheorem_canonicalForm}
    \leanok
    \uses{def:is_canonical_form, def:gauge_equiv, def:block_diagonal_tensor}
...
\begin{remark}[Relation to the literature]\label{rem:block_perm_literature}
    ...
    The separation part is the local strict-weight peeling argument used
    in~\cite[Appendix~\texttt{AppendixMPV}, lines~1054--1192]{Cirac2016MPDO_arXiv};
