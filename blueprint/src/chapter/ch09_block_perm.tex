...
\begin{lemma}[Per-block same MPV gives global gauge equivalence]\label{lem:per_block_gauge}
...
\begin{definition}[Canonical form conditions]\label{def:is_canonical_form}
    ...
    proof.
    This is a conditional local predicate, not the canonical-form existence
    theorem of PGVWC07, which starts from an arbitrary translation-invariant
    representation and constructs unital canonical blocks with positive
    full-rank invariant densities
    \cite[Theorem~\texttt{Th:TIcanonical}, lines~742--763]{PerezGarcia2007Matrix};
    see \path{docs/paper-gaps/pgvwc07_ti_canonical_form_scope.tex}.
\end{definition}
...
\begin{proof}\leanok
    ...
    In the inductive step, take bilinear overlaps with the leading block $A_0$:
    from
    $\sum_k \mu_k^N (V^{(N)}(A_k)_\sigma - V^{(N)}(B_k)_\sigma) = 0$
    one obtains
    \[
        \sum_k \overline{\mu_k}^N
        \bigl(O_{A_0 A_k}(N) - O_{A_0 B_k}(N)\bigr) = 0.
    \]
    After division by $\overline{\mu_0}^N$,
