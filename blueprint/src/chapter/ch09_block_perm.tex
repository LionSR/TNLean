...
\begin{proof}
    ...
    semisimple algebras;
    see \cite[§II.B]{Cirac2021Matrix} for the MPS context.
\end{proof}
...
\begin{definition}[Canonical form predicate]\label{def:is_canonical_form}
    ...
    see \cite[Thm.~6.11]{Wolf2012Quantum}
    and the discussion in Chapter~\ref{ch:spectral}.
\end{definition}
...
\begin{proof}
    ...
    In~\cite{PerezGarcia2007Matrix}, the separation argument
    is sketched informally using the ``Vandermonde structure''
    of the powers $\mu_k^N$.
    The formalization makes this precise by induction on $r$,
    with each step using the spectral gap to peel off the
    dominant block.
\end{proof}
