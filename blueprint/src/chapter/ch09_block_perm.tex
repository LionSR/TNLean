...
\begin{remark}[Normal-canonical companion]\label{rem:normal_cf_companion}
    Definition~\ref{def:is_canonical_form} is the legacy injective predicate.
    The D1 route instead uses the normal canonical form predicate from
...
\begin{lemma}[Block separation core]\label{lem:block_sep_core}
    ...
    (strict ordering, TP normalization, self-overlap convergence),
...
\begin{theorem}[Block separation from normal canonical form]\label{thm:block_sep_normal}
    ...
    and let $(B_k)$ be irreducible tensors satisfying the same TP
    normalization $\sum_i (B_k^i)^\dagger B_k^i = \Id$.
    If the block-diagonal tensors
    $\bigoplus_k \mu_k A_k$ and $\bigoplus_k \mu_k B_k$
    generate the same MPV family, then each pair
...
\begin{proof}\leanok
    \uses{thm:ncf_overlap_tendsto_one, thm:nt_modulus_one_gauge}
    The proof follows the same peeling argument as
    Theorem~\ref{thm:block_sep}. The only change is in the spectral step:
    injectivity is replaced by irreducibility plus TP normalization, and
    Theorem~\ref{thm:ncf_overlap_tendsto_one} supplies the diagonal overlap
    limit needed to identify the leading block.
\end{proof}
