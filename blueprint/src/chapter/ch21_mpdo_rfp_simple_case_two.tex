...
\begin{theorem}[Copy independence for a non-nilpotent sector decomposition]
    ...
    \leanok
    \uses{def:mpo_source_zcl, def:mpdo_doubled_phys_trace_transfer, lem:mpdo_doubled_phys_trace_transfer_to_mps, lem:mpdo_doubled_phys_trace_transfer_blockwise, lem:mpdo_simple_blockwise_zcl_equation}
...
\begin{proof}\leanok
    ...
    $X$ and $X^{-1}$ gives $\mathcal T_S^{2}=\lambda\mathcal T_S$.
    By
    ...
    \begin{align}
        ...
        \notag\\
        \mathcal T_{A_j}^{2}
        &=\frac{\lambda}{\mu_{j,q}}\mathcal T_{A_j}.
        \notag
    \end{align}
...
\begin{corollary}[Simple canonical-form tensors with zero correlation length]
    ...
    \leanok
    \uses{def:mpdo_simple_cf_tensor, def:mpo_source_zcl, thm:mpdo_simple_weight_copy_independent}
