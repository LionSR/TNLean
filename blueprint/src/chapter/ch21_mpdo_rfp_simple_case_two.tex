...
\begin{lemma}[Blockwise zero-correlation-length equation]
    ...
    is gauge-equivalent, through block-diagonal copy gauges, to an assembled
    sector decomposition. Every copy satisfies
    \begin{align}
...
\begin{proof}\leanok
    ...
    $X$ and $X^{-1}$ gives $\mathcal T_S^{2}=\mathcal T_S$.
    By
...
\begin{lemma}[Scale-invariant blockwise zero-correlation-length equation]
    ...
    gauge-equivalent, through block-diagonal copy gauges, to an assembled
    sector decomposition. There is a scalar $\lambda>0$ such that every copy
    satisfies
    \begin{align}
        (\mu_{j,q}\mathcal T_{A_j})^2
...
\begin{theorem}[Scale-invariant copy-independence generalization]
    ...
    remain independent of the copy index. This scale-invariant statement is
    distinct from the literal fixed-representative assertion of
    Theorem~\ref{thm:mpdo_simple_weight_copy_independent}.
\end{theorem}
...
\begin{theorem}[Nonvanishing of the common-weight coefficient]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This establishes coefficient nonvanishing for the choice
