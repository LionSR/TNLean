...
the same state. This section also identifies when the middle tensor is
injective, the hypothesis used by the edge-kernel argument of
Section~\ref{sec:peps_edge_kernel_gauges}.
...
\begin{definition}[Open middle tensor at an edge]%
    ...
    Fixing the residual boundary configurations at the endpoint vertices $u$
    and $v$ but leaving the bond $e$ out of the middle region, the open middle
    tensor is the contraction over all virtual indices away from $e$ that are
    compatible with those boundary configurations.
\end{definition}
...
\end{theorem}
\begin{proof}\leanok
    Reindex the ordinary middle configurations by the finite equivalence with
    open-middle configurations and compare the local tensor factors pointwise.
\end{proof}
...
\end{theorem}
\begin{proof}\leanok
    Substitute the identity matrix entry $\Id_{a,a}=1$ and apply the
    reindexing
    $C_{A,M_e}(\beta)=C^{\mathrm{open}}_{A,M_e}(\beta)$
...
\end{theorem}
\begin{proof}\leanok
    For $a\ne b$, the identity matrix entry $\Id_{a,b}=0$ makes the
    inserted summand vanish.
\end{proof}
...
\end{theorem}
\begin{proof}\leanok
    Split the inserted-boundary sum according to whether the two bond indices
    $a,b$ agree:
    \begin{align}
        C_A(e,\Id;\sigma)
        &=
        ...
        \notag
    \end{align}
    The off-diagonal sum vanishes because $\Id_{a,b}=0$ for $a\ne b$,
    and the diagonal sum reindexes to $C_A(e;\sigma)$ through the equivalence
    between diagonal inserted-boundary configurations and ordinary
    ...
    \begin{center}
        \begin{tenkzfree}
            ...
            \tnput[ports={235:virtual,155:virtual,north:physical}]
                {blockAthree}{(16mm,6mm)}{}
            \tnput[ports={-25:virtual,190:virtual,north:physical}]
                {blockAfour}{(4mm,15mm)}{}
            \tnput[ports={10:virtual,-70:virtual,-39:virtual,north:physical}]
                {blockAfive}{(-10mm,10mm)}{}
            \tnput[boundary, ports={south:physical}]{blockPone}{(-10mm,3mm)}{}
            \tnput[boundary, ports={south:physical}]{blockPtwo}{(8mm,3mm)}{}
            \tnput[boundary, ports={south:physical}]{blockPthree}{(16mm,14mm)}{}
            \tnput[boundary, ports={south:physical}]{blockPfour}{(4mm,23mm)}{}
            \tnput[boundary, ports={south:physical}]{blockPfive}{(-10mm,18mm)}{}
            \tnjoin[name=blockOneTwo]{blockAone.0}{blockAtwo.180}
            \tnjoin[name=blockTwoThree]{blockAtwo.55}{blockAthree.235}
            \tnjoin[name=blockThreeFour]{blockAthree.155}{blockAfour.-25}
            \tnjoin[name=blockFourFive]{blockAfour.190}{blockAfive.10}
            \tnjoin[name=blockFiveOne]{blockAfive.-70}{blockAone.110}
            \tnjoin[name=blockTwoFive]{blockAtwo.141}{blockAfive.-39}
            \tnjoin{blockAone.north}{blockPone.south}
            \tnjoin{blockAtwo.north}{blockPtwo.south}
            \tnjoin{blockAthree.north}{blockPthree.south}
            \tnjoin{blockAfour.north}{blockPfour.south}
            \tnjoin{blockAfive.north}{blockPfive.south}
            \tnregion[slot=selected, label={$A'_2$}, label pos=south]
                {blockAone}
            \tnregion[slot=secondary, label={$A'_1$}, label pos=north west]
                {blockAfive}
            \tnregion[slot=collar, label={$A'_3$}, label pos=north east]
                {blockAtwo,blockAthree,blockAfour,
                 blockTwoThree,blockThreeFour}
        \end{tenkzfree}
        $\quad\rightsquigarrow\quad$
        \tnpic[periodic, physical=up]{%
            \tn[label pos=south]{A'_1} &
            \tn[label pos=south]{A'_2} &
            \tn[label pos=south]{A'_3}}
    \end{center}
\end{definition}
...
\begin{proof}\leanok
    ...
    Write $\Coeff_A(\sigma)$ for the PEPS state coefficient and
    ...
    Thus $C_A(e;\sigma)=\Coeff_A(\sigma)$.
\end{proof}
...
\begin{proof}\leanok
    For every physical configuration $\sigma$,
    $C_A(e,\Id;\sigma)=\Coeff_A(\sigma)
        =\Coeff_B(\sigma)=C_B(e,\Id;\sigma)$.
\end{proof}
...
\begin{definition}[Singleton blocked region]%
    ...
    Its injectivity is $\inj(T_{A,\{v\}})$. The comparison with a
    region-injectivity predicate $\kappa$ records the implication
    $\inj(T_{A,\{v\}})\Longrightarrow\kappa(\{v\})$.
\end{definition}
...
\begin{theorem}[Vertex injectivity gives singleton-block injectivity]%
    ...
    injective. For every vertex $v$, $\inj(T_{A,\{v\}})$.
\end{theorem}
\begin{proof}\leanok
    For $R=\{v\}$, the contraction defining $T_{A,R}$ has one factor. Hence
    $T_{A,\{v\}}(\eta,\sigma)=A_v(\eta,\sigma)$.
    Thus $\inj(A_v)$ is exactly
    $\inj(T_{A,\{v\}})$. Vertex injectivity gives
    $\forall v,\inj(A_v)$, and therefore
    $\forall v,\inj(T_{A,\{v\}})$. For a fixed $v$, this is the
    displayed assertion.
\end{proof}
...
\begin{theorem}[Singleton comparison gives the region base case]%
    ...
    vertex $v$, $\inj(T_{A,\{v\}})\Longrightarrow\kappa(\{v\})$.
...
\begin{proof}\leanok
    ...
    $A\text{ vertex-injective}\Longrightarrow
        \inj(T_{A,\{v\}})$. The comparison hypothesis gives
    $\inj(T_{A,\{v\}})\Longrightarrow\kappa(\{v\})$. Their
...
\begin{theorem}[Finite regions from singleton regions and union closure]%
    ...
    $\inj(T_{A,\{v\}})\Longrightarrow\kappa(\{v\})$ for every
    vertex $v$, and
...
\begin{proof}\leanok
    ...
    $\inj(T_{A,u})$ and $\inj(T_{A,v})$, which are
...
\begin{proof}\leanok
    For each $e=(u,v)$,
    Theorem~\ref{thm:peps_edgeBlockedEndpointTensorMaps_injective} gives
...
\begin{proof}\leanok
    ...
    $\inj(T_{A,u})$ and $\inj(T_{A,v})$, while the
    hypothesis is $\inj(T_{A,M_e})$. Hence
...
\begin{proof}\leanok
    ...
    $\inj(T_{A,u})$ and $\inj(T_{A,v})$, while the
    hypothesis gives $\inj(T_{A,M_e})$. Hence
    $\inj(T_{A,u})\wedge\inj(T_{A,M_e})\wedge\inj(T_{A,v})$
...
\begin{proof}\leanok
    ...
    $\kappa(M_e)\Longrightarrow\inj(T_{A,M_e})$. Vertex injectivity
    supplies $\inj(T_{A,u})$ and $\inj(T_{A,v})$.
...
\begin{proof}\leanok
    ...
    $\kappa(M_e)\Longrightarrow\inj(T_{A,M_e})$ yields
    $\inj(T_{A,M_e})$ for every edge.
\end{proof}
...
\begin{proof}\leanok
    ...
    $\kappa(M_e)\Longrightarrow\inj(T_{A,M_e})$. Vertex
    injectivity gives $\inj(T_{A,u})$ and
    $\inj(T_{A,v})$. Thus
    $\inj(T_{A,u})\wedge\inj(T_{A,M_e})\wedge\inj(T_{A,v})$.
\end{proof}
...
\begin{theorem}[Edge-middle tensor from singleton regions, nonempty middle case]%
    ...
    comparison
    $\kappa(M_e)\Longrightarrow\inj(T_{A,M_e})$. If $A$ is
...
\begin{proof}\leanok
    ...
    comparison sends this to $\inj(T_{A,M_e})$.
\end{proof}
...
\end{theorem}
\begin{proof}\leanok
    The preceding theorem gives $\inj(T_{A,M_e})$. The endpoint
...
\begin{theorem}[All edge-blocked chains from singleton regions, cardinal form]%
    ...
    $\kappa(M_e)\Longrightarrow\inj(T_{A,M_e})$, and vertex
    injectivity of $A$. If $2<|V|$, then
    $V\setminus\{u,v\}\ne\varnothing$ for every edge $e=(u,v)$, and these
    hypotheses give $\inj(T_{A,V\setminus\{u,v\}})$ and
    edge-blocked three-site injectivity at every edge.
\end{theorem}
\begin{proof}\leanok
    For each $e=(u,v)$,
    Theorem~\ref{thm:peps_edgeMiddleVertices_nonempty_of_two_lt_card}
