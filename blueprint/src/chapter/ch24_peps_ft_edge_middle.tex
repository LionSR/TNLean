...
\begin{theorem}[Same state gives the same edge-blocked coefficients]%
    ...
    If two PEPS tensors $A$ and $B$ represent the same state, then their edge-blocked
    coefficients agree at every edge $e$ and every physical configuration $\sigma$:
    \begin{align}
        C_A(e;\sigma) & = C_B(e;\sigma).
    \end{align}
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Same state gives the same identity insertions]%
    ...
    If two PEPS tensors $A$ and $B$ represent the same state, then their edge-centered identity
    insertions agree at every edge $e$ and every physical configuration $\sigma$:
    \begin{align}
        C_A(e,\Id;\sigma) & = C_B(e,\Id;\sigma).
    \end{align}
\end{theorem}
\begin{proof}\leanok
    For every physical configuration $\sigma$,
...
\begin{theorem}[Endpoint injectivity in the edge-blocked chain]%
    ...
    If the original PEPS tensor $A$ is vertex-injective, then for each edge $e=(u,v)$,
    the two endpoint tensor maps in the edge-blocked three-site chain are injective:
    \begin{align}
        \inj(T_{A,u}) \quad\text{and}\quad \inj(T_{A,v}).
    \end{align}
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Endpoint injectivity at all edges]%
    ...
    \leanok
    \uses{def:IsVertexInjective, thm:peps_edgeBlockedEndpointTensorMaps_injective}
    If the original PEPS tensor $A$ is vertex-injective, then the two endpoint
    tensor maps in the edge-blocked three-site chain are injective at every edge $e=(u,v)$:
    \begin{align}
        \forall e=(u,v),\quad \inj(T_{A,u}) \quad\text{and}\quad \inj(T_{A,v}).
    \end{align}
\end{theorem}
\begin{proof}\leanok
    For each $e=(u,v)$,
...
\begin{theorem}[Reducing edge-blocked injectivity to the middle block]%
    ...
    For a vertex-injective PEPS tensor $A$, injectivity of the edge-blocked middle
...
\begin{theorem}[All edge-blocked chains from middle-tensor injectivity]%
    ...
    tensor $A$ gives an injective edge-blocked three-site chain at every edge $e=(u,v)$:
    \begin{align}
        \forall e=(u,v),\quad \inj(T_{A,u}) \wedge \inj(T_{A,M_e}) \wedge \inj(T_{A,v}).
    \end{align}
\end{theorem}
\begin{proof}\leanok
