...
\begin{theorem}[$\Z_2 \times \Z_2$ on-site symmetry of the AKLT tensor]\label{thm:aklt_z2z2_symmetric}
    ...
    of $\mathrm{SO}(3)$ generated by the $\pi$-rotations about two orthogonal
    axes.  The two generators act on the spin-$1$ physical index by commuting
    involutions, and each twisted tensor is gauge equivalent to the original
    through the virtual matrices $i\sigma_y$ and $\sigma_z$, which anticommute.
    The resulting projective phase is the non-trivial element of
    $H^2(\Z_2 \times \Z_2, \mathrm{U}(1)) \cong \Z_2$, witnessing a non-trivial
    SPT phase.  This $\Z_2 \times \Z_2$ is the smallest subgroup of
    $\mathrm{SO}(3)$ carrying a non-trivial $2$-cocycle.
\end{theorem}

\begin{theorem}[$\mathrm{SO}(3)$ on-site symmetry of the AKLT tensor]\label{thm:aklt_so3_symmetric}
    \notready
    \uses{def:aklt_tensor, def:on_site_symmetric, thm:aklt_z2z2_symmetric}
    ...
    witnessing a non-trivial SPT phase.  The full continuous
    $\mathrm{SO}(3)$ symmetry is not formalized; the
    $\Z_2 \times \Z_2$ subgroup of
    Theorem~\ref{thm:aklt_z2z2_symmetric} carries the same projective
    class and is the minimal protecting symmetry.
\end{theorem}
...
\begin{theorem}[$\Z_2 \times \Z_2$ on-site symmetry of the blocked cluster-state tensor]\label{thm:cluster_z2z2_symmetric}
    ...
    \leanok
    \uses{def:cluster_tensor, def:on_site_symmetric, rem:cluster_injective_blocked}
