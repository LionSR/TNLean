...
These boundary identifications are algebraic relations between matrix spaces and
coefficient functions, independent of any state-dependent boundary inner product.
...
\begin{lemma}[Ambient-space dimension]\label{lem:nsite_space_finrank}
    ...
    \leanok
    The ambient space of $L$-site tensors satisfies
    $\dim(\Fin{d}^{L}\to\C)=d^L$.
\end{lemma}
...
\begin{proof}
    ...
    Theorem~\ref{thm:ground_space_map_es_injective_block} then gives injectivity of
    $\Gamma_L^{\mathrm{ES}}$.
\end{proof}
...
These identities establish the algebraic equivalence between the two transfer-map
conventions.
...
\begin{proof}
    ...
    Write $E=E_{\mathrm{FNW},A}$ and $P=E_\infty^\rho$. The identities of
    Theorem~\ref{thm:fnw_limit_map_projection},
    Theorem~\ref{thm:fnw_transfer_absorbs_limit}, and
    Theorem~\ref{thm:fnw_limit_absorbs_transfer} give
...
These algebraic relations express the spectral projection and remainder
decomposition of the transfer map before introducing the $\rho$-weighted inner
product.
...
\begin{theorem}[Geometric power bound at a prescribed rate]
    ...
    \end{align}
    for every $n\in\N$.  This operator-theoretic estimate provides the
    prescribed-rate geometric decay used for the peripheral-spectrum remainder
    of the transfer map in \cite[Lemma~5.2]{Fannes1992Finitely}.
\end{theorem}
...
\begin{theorem}[Nonnegativity of the FNW mixing quantity]
    \label{thm:fnw_mixing_quantity_nonneg}
    \lean{MPSTensor.fnwWeightedOperatorNorm_nonneg}
    \lean{MPSTensor.fnwMixingQuantity_nonneg}
    \leanok
    \uses{def:fnw_mixing_quantity, def:fnw_weighted_remainder_data, def:fnw_trace_inverse_factor}
    The weighted operator norm is nonnegative, and hence so is $a(n)$ for every
    $n\in\N$.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:fnw_trace_inverse_factor_pos}
    A norm is nonnegative, and the trace factor $c_\rho$ is strictly positive.
\end{proof}
...
The theorem does not identify $C_\lambda$ with $k^2$ or identify the FNW auxiliary dimension $k$ with the chosen bond dimension $D$.
...
Theorem~\ref{thm:fnw_boundary_near_isometry_exact} and
Theorem~\ref{thm:fnw_boundary_norm_sq_succ} establish
(\ref{eq:fnw_boundary_near_isometry_exact}) and the right-boundary
recursion (\ref{eq:fnw_boundary_norm_sq_succ}) for
\cite[Lemma~5.3]{Fannes1992Finitely}. They do not prove Lemma~6.2. In its source
...
\begin{theorem}[Positivity from block injectivity]
    \label{thm:fnw_lower_boundary_block_injective_pos}
    \lean{MPSTensor.fnwLowerBoundaryConstant_pos_of_isNBlkInjective}
    \lean{MPSTensor.fnwLowerBoundaryConstant_pos_of_isNBlkInjective_of_le}
    \leanok
    \uses{def:l_blk_injective, def:fnw_lower_boundary_constant}
    Suppose that $A$ is block injective at length $L_0$. Then
    \begin{align}
        a_-(L_0) & >0.
        \label{eq:fnw_lower_boundary_block_injective_pos}
    \end{align}
    If $\E_A(\rho)=\rho$, then for every $m\geq L_0$,
    \begin{align}
        a_-(m) & >0.
        \label{eq:fnw_lower_boundary_block_injective_pos_mono}
    \end{align}
    This is the positivity input used in the proof of
    \cite[Lemma~6.2]{Fannes1992Finitely}.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:fnw_lower_boundary_singular, thm:fnw_lower_boundary_mono}
    Injectivity makes every singular value of $F_{L_0}$ positive. The least
    squared singular value is $a_-(L_0)$ by
    Theorem~\ref{thm:fnw_lower_boundary_singular}. Monotonicity then gives
    $a_-(m)\geq a_-(L_0)>0$ for $m\geq L_0$.
\end{proof}

These results establish the lower-boundary assertions of
\cite[Lemma~5.3]{Fannes1992Finitely}.

% FNW 1992, Lemma 6.2, especially equation (6.5).
\subsection{The linear overlap estimate in FNW Lemma 6.2}
\label{subsec:fnw_linear_overlap_estimate}

Fix consecutive block lengths $\ell,m,r$. Write $\mu^\ell$, $\mu^m$, and
$\mu^r$ for configurations on these blocks. The physical configuration is
$\mu^\ell\mu^m\mu^r$, with the first two blocks grouped before the right
block. All virtual matrix families below use the $\rho$-weighted norm from
(\ref{eq:fnw_weighted_norm}).

\begin{definition}[Weighted spectator families and three-block coordinates]
    \label{def:fnw_weighted_spectator_families}
    \lean{MPSTensor.FNWBoundaryFamilySpace}
    \lean{MPSTensor.fnwThreeBlockConfigEquiv}
    \lean{MPSTensor.fnwThreeBlockConfigEquiv_apply}
    \lean{MPSTensor.fnwThreeBlockConfigEquiv_symm_apply_apply}
    \leanok
    For a finite configuration space $S$, let
    \begin{align}
        \mathscr H_\rho(S)
         & :=\bigoplus_{\mu\in S}^{\ell^2}(\MN{D},\lVert\cdot\rVert_\rho).
        \label{eq:fnw_weighted_spectator_space}
    \end{align}
    The canonical three-block bijection sends
    $((\mu^\ell,\mu^m),\mu^r)$ to the concatenated configuration
    $\mu^\ell\mu^m\mu^r$ and has the evident inverse on such concatenations.
\end{definition}

\begin{definition}[Left and right overlap families]
    \label{def:fnw_overlap_families}
    \lean{MPSTensor.fnwLeftOverlapMap}
    \lean{MPSTensor.fnwRightOverlapMap}
    \lean{MPSTensor.fnwLeftOverlapMap_apply_append}
    \lean{MPSTensor.fnwRightOverlapMap_apply_append}
    \leanok
    \uses{def:fnw_weighted_spectator_families, def:fnw_boundary_map}
    For $\Phi\in\mathscr H_\rho(\{0,\ldots,d-1\}^r)$ and
    $\Psi\in\mathscr H_\rho(\{0,\ldots,d-1\}^\ell)$, define physical
    vectors $L_{\ell,m,r}\Phi$ and $R_{\ell,m,r}\Psi$ by
    \begin{align}
        (L_{\ell,m,r}\Phi)(\mu^\ell\mu^m\mu^r)
         & :=F_{\ell+m}(\Phi(\mu^r))(\mu^\ell\mu^m),
        \label{eq:fnw_left_overlap_family}           \\
        (R_{\ell,m,r}\Psi)(\mu^\ell\mu^m\mu^r)
         & :=F_{m+r}(\Psi(\mu^\ell))(\mu^m\mu^r).
        \label{eq:fnw_right_overlap_family}
    \end{align}
\end{definition}

\begin{definition}[Full-ground spectator families]
    \label{def:fnw_full_ground_spectator_families}
    \lean{MPSTensor.fnwLeftFullGroundFamily}
    \lean{MPSTensor.fnwRightFullGroundFamily}
    \lean{MPSTensor.fnwLeftFullGroundFamily_apply}
    \lean{MPSTensor.fnwRightFullGroundFamily_apply}
    \leanok
    \uses{def:fnw_weighted_spectator_families}
    For $B\in\MN{D}$, define the special spectator families
    \begin{align}
        \Phi_B(\mu^r)
         & :=Bv(\mu^r)^\dagger,
        \label{eq:fnw_left_full_ground_family} \\
        \Psi_B(\mu^\ell)
         & :=v(\mu^\ell)^\dagger B.
        \label{eq:fnw_right_full_ground_family}
    \end{align}
    These are the families obtained by splitting the length-$\ell+m+r$
    boundary vector at the two sides of the middle block.
\end{definition}

\begin{definition}[Middle-block boundary matrices]
    \label{def:fnw_middle_boundary_matrices}
    \lean{MPSTensor.fnwBoundaryMapCLM_append_eq_leftWord}
    \lean{MPSTensor.fnwBoundaryMapCLM_append_eq_rightWord}
    \lean{MPSTensor.fnwLeftMiddleBoundary}
    \lean{MPSTensor.fnwRightMiddleBoundary}
    \leanok
    \uses{def:fnw_overlap_families, def:fnw_boundary_map}
    Let $v(\mu)$ denote the product of the FNW letters along a word $\mu$.
    For each pair $(\mu^\ell,\mu^r)$, set
    \begin{align}
        X_{\mu^\ell,\mu^r}
         & :=v(\mu^\ell)^\dagger\Phi(\mu^r),
        \label{eq:fnw_left_middle_boundary}  \\
        Y_{\mu^\ell,\mu^r}
         & :=\Psi(\mu^\ell)v(\mu^r)^\dagger.
        \label{eq:fnw_right_middle_boundary}
    \end{align}
    More generally, for words $u$ and $\tau$ of lengths $K$ and $L$,
    \begin{align}
        F_{K+L}(B)(u\tau)
         & =F_L(v(u)^\dagger B)(\tau)
        =F_K(Bv(\tau)^\dagger)(u).
        \label{eq:fnw_boundary_word_splitting}
    \end{align}
    Thus the left word moves to the left of the boundary matrix and the right
    word moves to its right.
\end{definition}

\begin{theorem}[Middle-block decomposition and FNW aggregates]
    \label{thm:fnw_overlap_aggregate_decomposition}
    \lean{MPSTensor.inner_fnwLeftOverlapMap_fnwRightOverlapMap}
    \lean{MPSTensor.inner_conjTranspose_mul_mul_conjTranspose_eq_aggregateSummands}
    \lean{MPSTensor.fnwLeftOverlapAggregate}
    \lean{MPSTensor.fnwRightOverlapAggregate}
    \lean{MPSTensor.sum_inner_overlapFibers_eq_inner_aggregates}
    \lean{MPSTensor.inner_overlap_sub_inner_aggregates}
    \leanok
    \uses{def:fnw_middle_boundary_matrices, def:fnw_overlap_families}
    The weighted matrix identity
    \begin{align}
        \langle U^\dagger X,YV^\dagger\rangle_\rho
         & =\langle X\rho V\rho^{-1},UY\rangle_\rho
        \label{eq:fnw_aggregate_summand_pairing}
    \end{align}
    gives the source aggregates
    \begin{align}
        A_\varphi
         & :=\sum_{\mu^r}\Phi(\mu^r)\rho v(\mu^r)\rho^{-1},
        \label{eq:fnw_left_overlap_aggregate}               \\
        A_\psi
         & :=\sum_{\mu^\ell}v(\mu^\ell)\Psi(\mu^\ell).
        \label{eq:fnw_right_overlap_aggregate}
    \end{align}
    Then
    \begin{align}
        \langle L_{\ell,m,r}\Phi,R_{\ell,m,r}\Psi\rangle
         & =\sum_{\mu^\ell,\mu^r}
        \langle F_m(X_{\mu^\ell,\mu^r}),
        F_m(Y_{\mu^\ell,\mu^r})\rangle,
        \label{eq:fnw_overlap_middle_decomposition} \\
        \langle A_\varphi,A_\psi\rangle_\rho
         & =\sum_{\mu^\ell,\mu^r}
        \langle X_{\mu^\ell,\mu^r},Y_{\mu^\ell,\mu^r}\rangle_\rho.
        \label{eq:fnw_aggregate_pairing}
    \end{align}
    Consequently, the difference of these two pairings is the sum of the
    length-$m$ boundary defects over all spectator pairs.
\end{theorem}

\begin{proof}
    \leanok
    \uses{def:fnw_middle_boundary_matrices, def:fnw_overlap_families}
    Reindex the physical inner product by the three consecutive blocks and
    apply the two boundary-word splitting identities. For the aggregate
    pairing, use $\rho\rho^{-1}=\rho^{-1}\rho=I_D$ and cyclicity of trace,
    then distribute the two finite sums.
\end{proof}

\begin{theorem}[Spectator-family norm identities]
    \label{thm:fnw_spectator_family_norm_identities}
    \lean{MPSTensor.fnwTransferMap_pow_one}
    \lean{MPSTensor.trace_mul_fnwTransferMap_pow}
    \lean{MPSTensor.sum_norm_sq_conjTranspose_evalWord_mul}
    \lean{MPSTensor.sum_norm_sq_mul_conjTranspose_evalWord}
    \lean{MPSTensor.sum_norm_sq_fnwLeftMiddleBoundary}
    \lean{MPSTensor.sum_norm_sq_fnwRightMiddleBoundary}
    \leanok
    \uses{def:fnw_middle_boundary_matrices, thm:fnw_transfer_map_one, thm:trace_mul_fnw_transfer_map}
    Suppose that $A$ is left canonical and $\E_A(\rho)=\rho$. For every
    $N\geq0$ and $X\in\MN{D}$,
    \begin{align}
        E_{\mathrm{FNW},A}^N(I_D)
         & =I_D,
        \label{eq:fnw_word_unitality}        \\
        \tr\!\left(\rho E_{\mathrm{FNW},A}^N(X)\right)
         & =\tr\!\left(\E_A^N(\rho)X\right).
        \label{eq:fnw_power_trace_pairing}
    \end{align}
    Consequently,
    \begin{align}
        \sum_{\mu^\ell,\mu^r}
        \lVert X_{\mu^\ell,\mu^r}\rVert_\rho^2
         & =\lVert\Phi\rVert^2,
        \label{eq:fnw_left_middle_family_norm} \\
        \sum_{\mu^\ell,\mu^r}
        \lVert Y_{\mu^\ell,\mu^r}\rVert_\rho^2
         & =\lVert\Psi\rVert^2.
        \label{eq:fnw_right_middle_family_norm}
    \end{align}
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:fnw_transfer_map_one, thm:trace_mul_fnw_transfer_map}
    Unitality of $E_{\mathrm{FNW},A}^N$ gives the first word sum. The
    trace-pairing identity and stationarity of $\rho$ give the second. Summing
    over the remaining spectator index yields the two displayed identities.
\end{proof}

\begin{theorem}[Full-ground spectator identities]
    \label{thm:fnw_full_ground_spectator_identities}
    \lean{MPSTensor.fnwLeftOverlapMap_fullGroundFamily}
    \lean{MPSTensor.fnwRightOverlapMap_fullGroundFamily}
    \lean{MPSTensor.fnwLeftOverlapAggregate_fullGroundFamily}
    \lean{MPSTensor.fnwRightOverlapAggregate_fullGroundFamily}
    \lean{MPSTensor.norm_fnwLeftFullGroundFamily}
    \lean{MPSTensor.norm_fnwRightFullGroundFamily}
    \leanok
    \uses{def:fnw_full_ground_spectator_families, def:fnw_overlap_families,
        thm:fnw_spectator_family_norm_identities}
    Suppose that $A$ is left canonical and $\E_A(\rho)=\rho$. Then
    \begin{align}
        L_{\ell,m,r}\Phi_B
                           & =F_{\ell+m+r}(B)
        =R_{\ell,m,r}\Psi_B,
        \label{eq:fnw_full_ground_overlap_identities}   \\
        A_{\varphi,B}      & =B=A_{\psi,B},
        \label{eq:fnw_full_ground_aggregate_identities} \\
        \lVert\Phi_B\rVert & =\lVert B\rVert_\rho
        =\lVert\Psi_B\rVert.
        \label{eq:fnw_full_ground_family_norms}
    \end{align}
\end{theorem}
...
\begin{proof}
    ...
    \uses{def:fnw_full_ground_spectator_families, def:fnw_overlap_families,
        thm:fnw_spectator_family_norm_identities}
    Expand the finite words using the boundary convention from
    \cite[Equations~(5.5) and~(5.8)]{Fannes1992Finitely}. Trace cyclicity gives
    the right overlap identity. Stationarity of $\rho$ gives the left aggregate
    identity, while unitality of the FNW word map gives the right one and the
    two norm identities.
\end{proof}

\begin{theorem}[Termwise equation (5.9) estimate]
    \label{thm:fnw_overlap_linear_family_estimate}
    \lean{MPSTensor.norm_inner_overlap_sub_inner_aggregates_le}
    \lean{MPSTensor.norm_inner_overlap_sub_inner_aggregates_le_familyNorms}
    \leanok
    \uses{thm:fnw_overlap_aggregate_decomposition, thm:fnw_spectator_family_norm_identities, thm:fnw_boundary_near_isometry}
    If $\tr(\rho)=1$, then
    \begin{align}
        \bigl|\langle L_{\ell,m,r}\Phi,R_{\ell,m,r}\Psi\rangle
        -\langle A_\varphi,A_\psi\rangle_\rho\bigr|
         & \leq a(m)
        \left(\sum_{\mu^\ell,\mu^r}\lVert X_{\mu^\ell,\mu^r}\rVert_\rho^2\right)^{1/2}
        \left(\sum_{\mu^\ell,\mu^r}\lVert Y_{\mu^\ell,\mu^r}\rVert_\rho^2\right)^{1/2}.
        \label{eq:fnw_overlap_termwise_estimate}
    \end{align}
    If, in addition, $A$ is left canonical and $\E_A(\rho)=\rho$, then
    \begin{align}
        \bigl|\langle L_{\ell,m,r}\Phi,R_{\ell,m,r}\Psi\rangle
        -\langle A_\varphi,A_\psi\rangle_\rho\bigr|
         & \leq a(m)\lVert\Phi\rVert\lVert\Psi\rVert.
        \label{eq:fnw_overlap_linear_family_estimate}
    \end{align}
\end{theorem}
...
\begin{proof}
    \leanok
    \uses{thm:fnw_overlap_aggregate_decomposition, thm:fnw_spectator_family_norm_identities, thm:fnw_boundary_near_isometry}
    Apply (\ref{eq:fnw_boundary_near_isometry}) to every middle-boundary pair,
    sum the estimates, and use finite Cauchy--Schwarz. The identities
    (\ref{eq:fnw_left_middle_family_norm}) and
    (\ref{eq:fnw_right_middle_family_norm}) give the stated family norms.
\end{proof}

\begin{theorem}[Overlap norms and the lower boundary constant]
    \label{thm:fnw_overlap_lower_boundary_control}
    \lean{MPSTensor.norm_fnwLeftOverlapMap_sq}
    \lean{MPSTensor.norm_fnwRightOverlapMap_sq}
    \lean{MPSTensor.fnwLowerBoundaryConstant_mul_familyNorm_sq_le_leftOverlap}
    \lean{MPSTensor.fnwLowerBoundaryConstant_mul_familyNorm_sq_le_rightOverlap}
    \lean{MPSTensor.sqrt_fnwLowerBoundaryConstant_mul_familyNorm_le_leftOverlap}
    \lean{MPSTensor.sqrt_fnwLowerBoundaryConstant_mul_familyNorm_le_rightOverlap}
    \lean{MPSTensor.fnwLowerBoundaryConstant_mul_familyNorms_le_overlapNorms}
    \leanok
    \uses{def:fnw_overlap_families, thm:fnw_lower_boundary_optimal, thm:fnw_lower_boundary_mono}
    The overlap norms satisfy
    \begin{align}
        \lVert L_{\ell,m,r}\Phi\rVert^2
         & =\sum_{\mu^r}\lVert F_{\ell+m}(\Phi(\mu^r))\rVert^2,
        \label{eq:fnw_left_overlap_norm}                           \\
        \lVert R_{\ell,m,r}\Psi\rVert^2
         & =\sum_{\mu^\ell}\lVert F_{m+r}(\Psi(\mu^\ell))\rVert^2.
        \label{eq:fnw_right_overlap_norm}
    \end{align}
    The optimal lower-boundary inequality also gives
    \begin{align}
        a_-(\ell+m)\lVert\Phi\rVert^2
         & \leq\lVert L_{\ell,m,r}\Phi\rVert^2,
        \label{eq:fnw_left_overlap_lower_boundary} \\
        a_-(m+r)\lVert\Psi\rVert^2
         & \leq\lVert R_{\ell,m,r}\Psi\rVert^2.
        \label{eq:fnw_right_overlap_lower_boundary}
    \end{align}
    If $\E_A(\rho)=\rho$ and $a_-(m)>0$, monotonicity from $m$ to
    $\ell+m$ and from $m$ to $m+r$ gives
    \begin{align}
        \sqrt{a_-(m)}\lVert\Phi\rVert
         & \leq\lVert L_{\ell,m,r}\Phi\rVert,
        \label{eq:fnw_left_overlap_sqrt_lower_boundary}  \\
        \sqrt{a_-(m)}\lVert\Psi\rVert
         & \leq\lVert R_{\ell,m,r}\Psi\rVert,
        \label{eq:fnw_right_overlap_sqrt_lower_boundary} \\
        a_-(m)\lVert\Phi\rVert\lVert\Psi\rVert
         & \leq\lVert L_{\ell,m,r}\Phi\rVert
        \lVert R_{\ell,m,r}\Psi\rVert.
        \label{eq:fnw_overlap_lower_boundary_control}
    \end{align}
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:fnw_lower_boundary_optimal, thm:fnw_lower_boundary_mono}
    Reindex each physical norm by the spectator configuration. Apply
    (\ref{eq:fnw_lower_boundary_universal}) to every summand. Monotonicity
    gives $a_-(m)\leq a_-(\ell+m)$ and
    $a_-(m)\leq a_-(m+r)$. Multiply the resulting square-root estimates.
\end{proof}

\begin{theorem}[FNW equation (6.5), linear contribution]
    \label{thm:fnw_equation_6_5_linear}
    \lean{MPSTensor.norm_inner_overlap_sub_inner_aggregates_le_div_lowerBoundary}
    \leanok
    \uses{thm:fnw_overlap_linear_family_estimate, thm:fnw_overlap_lower_boundary_control}
    Suppose that $\tr(\rho)=1$, $A$ is left canonical,
    $\E_A(\rho)=\rho$, and $a_-(m)>0$. Then
    \begin{align}
        \bigl|\langle L_{\ell,m,r}\Phi,R_{\ell,m,r}\Psi\rangle
        -\langle A_\varphi,A_\psi\rangle_\rho\bigr|
         & \leq\frac{a(m)}{a_-(m)}
        \lVert L_{\ell,m,r}\Phi\rVert
        \lVert R_{\ell,m,r}\Psi\rVert.
        \label{eq:fnw_equation_6_5_linear}
    \end{align}
    This is the linear estimate in
    \cite[Equation~(6.5)]{Fannes1992Finitely}. Positivity of the actual lower
    boundary constant is assumed directly.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:fnw_overlap_linear_family_estimate, thm:fnw_overlap_lower_boundary_control}
    Divide (\ref{eq:fnw_overlap_lower_boundary_control}) by $a_-(m)$ and
    substitute the resulting family-norm bound into
    (\ref{eq:fnw_overlap_linear_family_estimate}).
\end{proof}


\subsection{The quadratic aggregate contribution in FNW Lemma 6.2}
\label{subsec:fnw_quadratic_aggregate_estimate}

\begin{definition}[Full boundary range]
    \label{def:fnw_boundary_range}
    \lean{MPSTensor.fnwBoundaryRange}
    \leanok
    \uses{def:fnw_boundary_clm}
    At length $N$, let
    \begin{align}
        \mathcal G_N & :=\operatorname{ran}F_N.
        \label{eq:fnw_boundary_range}
    \end{align}
    This is the full open-chain ground-space range used in the orthogonality
    step of \cite[Lemma~6.2]{Fannes1992Finitely}.
\end{definition}

\begin{theorem}[Unconverted aggregate estimates]
    \label{thm:fnw_aggregate_family_bounds}
    \lean{MPSTensor.norm_inner_fnwLeftOverlapAggregate_le_familyNorm}
    \lean{MPSTensor.norm_inner_fnwRightOverlapAggregate_le_familyNorm}
    \lean{MPSTensor.norm_fnwLeftOverlapAggregate_le_familyNorm}
    \lean{MPSTensor.norm_fnwRightOverlapAggregate_le_familyNorm}
    \leanok
    \uses{def:fnw_boundary_range, thm:fnw_full_ground_spectator_identities,
        thm:fnw_overlap_linear_family_estimate}
    Suppose that $\tr(\rho)=1$, $A$ is left canonical,
    $\E_A(\rho)=\rho$, and both $L_{\ell,m,r}\Phi$ and
    $R_{\ell,m,r}\Psi$ are orthogonal to $\mathcal G_{\ell+m+r}$. Then, for
    every $B\in\MN{D}$,
    \begin{align}
        |\langle A_\varphi,B\rangle_\rho|
         & \leq a(m)\lVert\Phi\rVert\lVert B\rVert_\rho,
        \label{eq:fnw_left_aggregate_dual_bound}         \\
        |\langle B,A_\psi\rangle_\rho|
         & \leq a(m)\lVert B\rVert_\rho\lVert\Psi\rVert.
        \label{eq:fnw_right_aggregate_dual_bound}
    \end{align}
    Consequently,
    \begin{align}
        \lVert A_\varphi\rVert_\rho
         & \leq a(m)\lVert\Phi\rVert,
        \label{eq:fnw_left_aggregate_family_bound} \\
        \lVert A_\psi\rVert_\rho
         & \leq a(m)\lVert\Psi\rVert.
        \label{eq:fnw_right_aggregate_family_bound}
    \end{align}
\end{theorem}
...
\begin{theorem}[One-sided aggregate conversion]
    \label{thm:fnw_aggregate_sqrt_lower_boundary}
    \lean{MPSTensor.norm_fnwLeftOverlapAggregate_le_div_sqrt_lowerBoundary}
    \lean{MPSTensor.norm_fnwRightOverlapAggregate_le_div_sqrt_lowerBoundary}
    \leanok
    \uses{thm:fnw_aggregate_family_bounds,
        thm:fnw_overlap_lower_boundary_control}
    Under the hypotheses of
    Theorem~\ref{thm:fnw_aggregate_family_bounds}, if $a_-(m)>0$, then
    \begin{align}
        \lVert A_\varphi\rVert_\rho
         & \leq\frac{a(m)}{\sqrt{a_-(m)}}
        \lVert L_{\ell,m,r}\Phi\rVert,
        \label{eq:fnw_left_aggregate_sqrt_bound} \\
        \lVert A_\psi\rVert_\rho
         & \leq\frac{a(m)}{\sqrt{a_-(m)}}
        \lVert R_{\ell,m,r}\Psi\rVert.
        \label{eq:fnw_right_aggregate_sqrt_bound}
    \end{align}
\end{theorem}
...
\begin{proof}
    ...
    \uses{thm:fnw_aggregate_family_bounds,
        thm:fnw_overlap_lower_boundary_control}
    Apply only the matching one-sided square-root inequality
    (\ref{eq:fnw_left_overlap_sqrt_lower_boundary}) or
    (\ref{eq:fnw_right_overlap_sqrt_lower_boundary}) to each family bound.
\end{proof}

\begin{theorem}[Quadratic aggregate contribution]
    \label{thm:fnw_aggregate_quadratic}
    \lean{MPSTensor.norm_inner_fnwOverlapAggregates_le_sq_div_lowerBoundary}
    \leanok
    \uses{thm:fnw_aggregate_sqrt_lower_boundary}
    Under the hypotheses of
    Theorem~\ref{thm:fnw_aggregate_family_bounds}, if $a_-(m)>0$, then
    \begin{align}
        |\langle A_\varphi,A_\psi\rangle_\rho|
        ...
        \lVert L_{\ell,m,r}\Phi\rVert
        \lVert R_{\ell,m,r}\Psi\rVert.
        \label{eq:fnw_aggregate_quadratic}
    \end{align}
    This is the quadratic contribution in the proof of
    \cite[Lemma~6.2]{Fannes1992Finitely}.
\end{theorem}
...
\begin{proof}
    \leanok
    \uses{thm:fnw_aggregate_sqrt_lower_boundary}
    Apply Cauchy--Schwarz to the aggregate pairing and multiply the two
    one-sided bounds. Since $\sqrt{a_-(m)}^2=a_-(m)$, the coefficient is
    exactly $a(m)^2/a_-(m)$.
\end{proof}

\begin{theorem}[Linear and quadratic overlap bound]
    \label{thm:fnw_overlap_linear_add_quadratic}
    \lean{MPSTensor.norm_inner_fnwOverlapMaps_le_linear_add_quadratic}
    \leanok
    \uses{thm:fnw_equation_6_5_linear, thm:fnw_aggregate_quadratic}
    Under the hypotheses of
    Theorems~\ref{thm:fnw_equation_6_5_linear} and
    \ref{thm:fnw_aggregate_quadratic},
    \begin{align}
        |\langle L_{\ell,m,r}\Phi,R_{\ell,m,r}\Psi\rangle|
         & \leq\left(\frac{a(m)}{a_-(m)}
        +\frac{a(m)^2}{a_-(m)}\right)
        \lVert L_{\ell,m,r}\Phi\rVert
        \lVert R_{\ell,m,r}\Psi\rVert.
        \label{eq:fnw_overlap_linear_add_quadratic}
    \end{align}
    The two source contributions are deliberately left unfactored.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:fnw_equation_6_5_linear, thm:fnw_aggregate_quadratic}
    Insert the aggregate pairing and apply the triangle inequality. The
    difference term is bounded by Equation~(6.5), and the aggregate term is
    bounded by Theorem~\ref{thm:fnw_aggregate_quadratic}.
\end{proof}

Write $P^{\mathrm{tail}}_{r;m,\ell}$ and
$P^{\mathrm{left}}_{r+m;\ell}$ for the orthogonal projectors onto the
reassociated tail-boundary range and the left-boundary range, respectively,
and write $G_{r+m+\ell}$ for the orthogonal projector onto the full ground
space.

\begin{theorem}[FNW projector defect with separate contributions]
    \label{thm:fnw_projector_defect_unfactored}
    \lean{MPSTensor.wholeIncrement_groundProjection_defect_le_fnw}
    \leanok
    \uses{def:reassociated_tail_boundary_map,
        def:spectator_boundary_maps_es, def:ground_space_es,
        def:fnw_mixing_quantity, def:fnw_lower_boundary_constant,
        def:left_canonical_tensor, def:l_blk_injective}
    Let $A$ have nonzero bond dimension, let $\rho$ be positive definite with
    $\tr(\rho)=1$, and suppose that $A$ is left canonical and
    $\mathcal E_A(\rho)=\rho$. Suppose that $L_0\geq1$ is an injective length.
    For every $r\geq0$, every $\ell\geq1$ and every $m\geq L_0$,
    \begin{align}
        \left\lVert
        P^{\mathrm{tail}}_{r;m,\ell}P^{\mathrm{left}}_{r+m;\ell}
        -G_{r+m+\ell}
        \right\rVert
         & \leq \frac{a(m)}{a_-(m)}+\frac{a(m)^2}{a_-(m)}.
        \label{eq:fnw_projector_defect_unfactored}
    \end{align}
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:fnw_overlap_linear_add_quadratic,
        thm:fnw_lower_boundary_block_injective_pos}
    In the source coordinates, let $U$ and $V$ be the two overlap ranges and
    let $W$ be the full boundary range. Then $W\subseteq U\cap V$. For
    $x\in U\cap W^\perp$ and $y\in V\cap W^\perp$,
    Theorem~\ref{thm:fnw_overlap_linear_add_quadratic} gives
    \begin{align}
        |\langle x,y\rangle|
         & \leq\left(\frac{a(m)}{a_-(m)}
        +\frac{a(m)^2}{a_-(m)}\right)\lVert x\rVert\lVert y\rVert.
    \end{align}
    Projecting a vector first onto $V\cap W^\perp$ and then onto
    $U\cap W^\perp$ converts this relative-overlap bound into the same
    operator-norm bound for $P_UP_V-P_W$. Global reversal is unitary and sends
    $U$, $V$, and $W$ to the three ranges in the statement, so it preserves
    the norm.
\end{proof}

\begin{theorem}[FNW projector-defect coefficient]
    \label{thm:fnw_projector_defect_factored}
    \lean{MPSTensor.wholeIncrement_groundProjection_defect_le_fnw_factored}
    \leanok
    \uses{thm:fnw_projector_defect_unfactored}
    Under the hypotheses of
    Theorem~\ref{thm:fnw_projector_defect_unfactored},
    \begin{align}
        \left\lVert
        P^{\mathrm{tail}}_{r;m,\ell}P^{\mathrm{left}}_{r+m;\ell}
        -G_{r+m+\ell}
        ...
        \label{eq:fnw_projector_defect_factored}
    \end{align}
...
\begin{proof}
    ...
    \uses{thm:fnw_projector_defect_unfactored}
    First apply
    Theorem~\ref{thm:fnw_projector_defect_unfactored}, which retains the two
    source contributions. Then use
    \begin{align}
...
\begin{theorem}[Primitive FNW projector-defect estimate]
    \label{thm:primitive_fnw_projector_defect}
    \lean{MPSTensor.IsPrimitiveMPS.exists_wholeIncrement_groundProjection_defect_le_fnw}
    \leanok
    \uses{def:primitive_mps, def:l_blk_injective,
        thm:fnw_projector_defect_unfactored}
    Let $A$ be primitive with positive-definite fixed point $\rho$ and
    $\tr(\rho)=1$. Then there is an injective length $L_0\geq1$ such that
    the bound (\ref{eq:fnw_projector_defect_unfactored}) holds whenever
    $m\geq L_0$ and $\ell\geq1$, for every $r\geq0$.
\end{theorem}
...
\begin{proof}
    ...
    Choose a positive injective length from primitivity. Left canonicality and
    the fixed-point equation are part of the primitive hypotheses, so
    Theorem~\ref{thm:fnw_projector_defect_unfactored} applies.
\end{proof}


\section{The geometric form of the FNW projector estimate}

Nachtergaele imports the projector estimate of
Theorem~\ref{thm:primitive_fnw_projector_defect} in the rational geometric form
\begin{align}
    \left\lVert
    G_{\Lambda_n}G_{\Lambda_{n+1}\setminus\Lambda_{n-l}}
    -G_{\Lambda_{n+1}}
    \right\rVert
     & \leq c\lambda^l\,\frac{1+c\lambda^l}{1-c\lambda^l},
    \label{eq:nachtergaele_geometric_projector_defect}
\end{align}
stated in \cite[lines~1180--1194]{Nachtergaele1996Spectral} and again for the
best overlap constant $A^\alpha_m$ in
\cite[Equation~(6.1)]{Nachtergaele1996Spectral}.  This bound follows from the
FNW coefficient $a(m)(1+a(m))/a_-(m)$, the lower-boundary inequality
$1-a(m)\leq a_-(m)$, and the prescribed-rate estimate $a(m)\leq c\lambda^m$.

The source's rate condition is now established by
Theorem~\ref{thm:fnw-transfer-eigenvalue-rate}: every positive rate exceeding
the moduli of the nonunit observable-transfer eigenvalues also exceeds the
weighted spectral radius of the transfer remainder. Every nonzero eigenvalue
of the remainder is a nonunit transfer eigenvalue, which gives the required
implication. In particular, the geometric estimate is available for every
source-admissible rate $0<\lambda<1$, with a positive rate-dependent prefactor.

\textbf{Local fix (dimension-only prefactor):}
The source's simultaneous assertions that $c=k^2$, the square of the auxiliary
dimension, and that every admissible rate may be prescribed require a correction
at the fixed intersection threshold. Theorem~\ref{thm:fnw-dimension-only-counterexample}
gives a physical counterexample. The valid replacement retains the
rate-dependent prefactor, or permits any prescribed positive prefactor after
an additional onset depending on the state, rate, and prefactor, as proved in
Theorem~\ref{thm:fnw-eventual-projector-prescribed-prefactor}. See
\cite{gap:cpgsv21_martingale_overlap}.

\begin{theorem}[Geometric FNW projector defect at a fixed length]
    \label{thm:fnw_projector_defect_geometric}
    \lean{MPSTensor.wholeIncrement_groundProjection_defect_le_fnw_geometric}
    \leanok
    \uses{thm:fnw_projector_defect_unfactored, def:fnw_mixing_quantity}
    Under the hypotheses of
    Theorem~\ref{thm:fnw_projector_defect_unfactored}, suppose in addition that
    $a(m)\leq c\lambda^m<1$.  Then
    \begin{align}
        \left\lVert
        P^{\mathrm{tail}}_{r;m,\ell}P^{\mathrm{left}}_{r+m;\ell}
        -G_{r+m+\ell}
        ...
        \label{eq:fnw_projector_defect_geometric}
    \end{align}
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:fnw_projector_defect_factored, thm:fnw_lower_boundary_estimate,
        thm:fnw_mixing_quantity_nonneg}
    The FNW coefficient is $a(m)(1+a(m))/a_-(m)$.  Equation~(5.9) gives
    $1-a(m)\leq a_-(m)$, and $a(m)\leq c\lambda^m<1$ makes the left-hand side
    strictly positive, so the denominator may be replaced by $1-c\lambda^m$.
    The numerator is nondecreasing in $a(m)$ on the nonnegative reals, so it is
    at most $c\lambda^m(1+c\lambda^m)$.
\end{proof}

\begin{theorem}[Geometric FNW projector defect for a primitive tensor]
    \label{thm:primitive_fnw_projector_defect_geometric}
    \lean{MPSTensor.IsPrimitiveMPS.exists_wholeIncrement_groundProjection_defect_le_fnw_geometric}
    \leanok
    \uses{def:primitive_mps, def:l_blk_injective,
        thm:fnw_projector_defect_geometric}
    Let $A$ be primitive with positive-definite fixed point $\rho$ and nonzero
    bond dimension.  There are constants $c>0$ and $0<\lambda<1$ and an
    injective length $L_0\geq1$ such that, whenever $m\geq L_0$ and
    $c\lambda^m<1$, the bound (\ref{eq:fnw_projector_defect_geometric}) holds for
    every prefix length $r$ and every $\ell\geq1$.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:fnw_projector_defect_geometric, thm:fnw_mixing_quantity_geometric,
        thm:primitive_fixed_point_trace_normalization,
        thm:fnw_weighted_remainder_spectral_radius_lt_one,
        thm:normal_from_primitive_posdef}
    Rescale the fixed point to unit trace; this changes neither the tensor nor
    the primitivity certificate.  The weighted remainder has spectral radius
    below one, so there is a rate $\lambda$ strictly between that radius and
    one; the prescribed-rate estimate supplies $c>0$ with $a(m)\leq c\lambda^m$
    for every $m\geq1$.  Normality supplies a positive injective length.  Apply
    the fixed-length geometric estimate.
\end{proof}

\begin{theorem}[Open-chain form of the geometric FNW projector defect]
    \label{thm:primitive_fnw_openchain_defect_geometric}
    \lean{MPSTensor.openChain_groundProjection_defect_eq_wholeIncrement}
    \lean{MPSTensor.IsPrimitiveMPS.exists_openChain_groundProjection_defect_le_fnw_geometric}
    \leanok
    \uses{def:primitive_mps, def:l_blk_injective,
        def:open_chain_ground_subspaces_es, def:ground_space_es,
        thm:primitive_fnw_projector_defect_geometric}
    A one-site suffix increment needs no reassociation, so the open-chain
    projector defect at overlap length $l$ and prefix length $K$ equals the
    whole-increment defect with $r=K$, $m=l$, and $\ell=1$.  Consequently, for
    a primitive tensor with positive-definite fixed point there are $c>0$,
    $0<\lambda<1$, and an injective length $L_0\geq1$ such that, whenever
    $l\geq L_0$ and $c\lambda^l<1$, for every prefix length $K$,
    \begin{align}
        \left\lVert
        ...
        \label{eq:primitive_fnw_openchain_defect_geometric}
    \end{align}
    This establishes the projector defect bound
    (\ref{eq:nachtergaele_geometric_projector_defect}) for primitive matrix product
    states.
\end{theorem}
...
\begin{proof}
    ...
    \uses{thm:primitive_fnw_projector_defect_geometric,
        thm:reassociated_tail_boundary_one_site,
        thm:spectator_boundary_hilbert_factorization}
    The two projectors of the whole-increment display at $\ell=1$ are the two
    projectors of the open-chain display.  Indeed
    \begin{align}
        \ran\widetilde\Gamma^{\mathrm{tail}}_{K;l,1}
        =\ran\Gamma^{\mathrm{tail}}_{K,l+1}=\mathcal V_{K,l+1}(A),
        \qquad
        \ran\Gamma^{\mathrm{left}}_{K+l,1}=\mathcal U_{K+l}(A),
        \label{eq:openchain_defect_range_identifications}
    \end{align}
    the first equality by
    (\ref{eq:reassociated_tail_boundary_one_site}) and the remaining
    two by Theorem~\ref{thm:spectator_boundary_hilbert_factorization}.  Taking
    orthogonal projectors onto the three subspaces gives
    \begin{align}
        P^{\mathrm{tail}}_{K;l,1}=P^{\mathrm{tail}}_{K,l+1}(A),
        \qquad
        P^{\mathrm{left}}_{K+l;1}=P^{\mathrm{left}}_{K+l}(A),
        \label{eq:openchain_defect_projector_identifications}
    \end{align}
    so the two defect operators are equal.  Substituting $\ell=1$ in the
    geometric estimate gives the display.
\end{proof}
...
the reshuffled fixed-point projection $\mathscr R(P_\rho)$, so the
limiting metric is generally nonidentity.
...
For a positive-definite fixed point $\rho$,
Theorem~\ref{thm:primitive_ground_space_gram_geometric_inverse_bounds} provides
quantitative inverse bounds for the limiting metric $\mathscr R(P_\rho)$.
...
\begin{proof}
    ...
    identities of Theorem~\ref{thm:inverse_gram_identities}
    identify $S_{\Gamma_n^{\mathrm{ES}}}$ with $\mathcal K_n^{-1}$, so the
    pointwise bounds of
    Theorem~\ref{thm:primitive_ground_space_gram_geometric_inverse_bounds} apply
    unchanged.
\end{proof}
...
\begin{theorem}[Eventual injectivity and inverse-Gram control]
    ...
    its inverse Gram operator is $\mathcal K_n^{-1}$, satisfying
    \begin{align}
...
\begin{proof}
    ...
    injectivity of $\Gamma_n^{\mathrm{ES}}$.  The inverse-Gram identities of
    Theorem~\ref{thm:inverse_gram_identities} then identify
    $S_{\Gamma_n^{\mathrm{ES}}}$ with $\mathcal K_n^{-1}$, and the estimate of
    Theorem~\ref{thm:primitive_ground_space_gram_eventual_inverse_bound} applies.
\end{proof}
...
\begin{proof}
    ...
    The vector $Q_Tx$ lies in $\range T$. Since
    $T^\dagger Q_Tx=(T^\dagger T)S_TT^\dagger x=T^\dagger x$, the adjoint identity
    gives $\langle x-Q_Tx,Ty\rangle=0$ for every $y$. Thus $x-Q_Tx$ is
    orthogonal to $\range T$, and the range--orthogonal decomposition
    characterizing the orthogonal projector gives $Q_T=P_{\range T}$.
\end{proof}
