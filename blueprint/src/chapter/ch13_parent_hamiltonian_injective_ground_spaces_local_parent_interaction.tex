...
\textbf{Scope restriction (FNW overlap bound rate and prefactor):}
Two features of the source's display are not reproduced, and the statements
below are phrased so as not to assert them.  The first is the rate condition:
the source prescribes that $\lambda$ may be any number with
$\lambda_i<\lambda<1$ for every eigenvalue $\lambda_i\neq1$ of the transfer
operator.  What is proved here instead admits any $\lambda$ strictly between
the weighted spectral radius of the transfer remainder and one.  The two
conditions would describe the same rates if the spectral radius were known to
equal the largest modulus among the nonunit transfer eigenvalues.  That is not
established here: what is available is the bound that every eigenvalue of the
remainder has modulus below one, which is not a description of its spectrum.
The theorems below therefore use the spectral-radius condition alone.  The
second is the prefactor: the source asserts it may be taken equal to $k^2$, the
dimension of the auxiliary space, whereas the constant produced below is the
existential rate-dependent one.  Documented in
\cite{gap:cpgsv21_martingale_overlap}
(\texttt{docs/paper-gaps/cpgsv21_martingale_overlap.tex}).
