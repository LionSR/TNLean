...
\begin{theorem}[Unconverted aggregate estimates]
    ...
    Suppose that $L_{\ell,m,r}\Phi$ and $R_{\ell,m,r}\Psi$ are orthogonal to
    $\mathcal G_{\ell+m+r}$. Then, for every $B\in\MN{D}$,
    \begin{align}
...
\begin{theorem}[One-sided aggregate conversion]
    ...
    Under the same orthogonality assumptions, if $a_-(m)>0$, then
    \begin{align}
        \lVert A_\varphi\rVert_\rho
...
\begin{theorem}[Quadratic aggregate contribution]
    ...
    If both overlap vectors are orthogonal to $\mathcal G_{\ell+m+r}$ and
    $a_-(m)>0$, then
    \begin{align}
        |\langle A_\varphi,A_\psi\rangle_\rho|
         & \leq\frac{a(m)^2}{a_-(m)}
...
These results prove the block-injectivity positivity input, the linear
coefficient $a(m)/a_-(m)$ from Equation~(6.5), and the separate quadratic
coefficient $a(m)^2/a_-(m)$ in the proof of FNW Lemma~6.2. They stop before
the common-subspace projector geometry and the final projector-defect estimate
of Lemma~6.2. No inverse or finite Gram matrix, pseudoinverse, periodic
coercivity, comparison of unequal kernels, cyclic replacement, dimension
prefactor, identity $a_-(m)=1-a(m)$, identification $k=D$, or claim
$C_\lambda=k^2$ enters these results.
% The final common-subspace projector geometry belongs to issue #7601.
