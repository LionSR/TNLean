...
\cite[Equation~(6.1)]{Nachtergaele1996Spectral}, where $\lambda$ may be any
number with $\lambda_i<\lambda<1$ for every eigenvalue $\lambda_i\neq1$ of the
transfer operator.  The results below assemble that display from the FNW
coefficient $a(m)(1+a(m))/a_-(m)$, the lower-boundary inequality
$1-a(m)\leq a_-(m)$, and the prescribed-rate estimate $a(m)\leq c\lambda^m$,
whose rate condition is that $\lambda$ exceed the weighted spectral radius of
the transfer remainder.
The source also asserts that the prefactor may be taken equal to $k^2$, the
dimension of the auxiliary space; that value is not asserted here, and the
prefactor stays the existential rate-dependent constant.
...
\begin{theorem}[Open-chain form of the geometric FNW projector defect]
    ...
    $l\geq L_0$, $c\lambda^l<1$, and $K>0$,
    \begin{align}
        \left\lVert
    ...
    \end{align}
    This is Equation~(\ref{eq:nachtergaele_geometric_projector_defect}).
\end{theorem}
