...
This proves only the linear contribution $a(m)/a_-(m)$. The
block-injectivity argument that supplies $a_-(m)>0$, the aggregate estimates
coming from orthogonality to the full ground space, the separate quadratic
contribution $a(m)^2/a_-(m)$, the common-subspace projector estimate, and the
final assertion of Lemma~6.2 remain open. No inverse or finite Gram matrix,
pseudoinverse, periodic coercivity, comparison of unequal kernels, cyclic
replacement, dimension prefactor, identity $a_-(m)=1-a(m)$, identification
$k=D$, or claim $C_\lambda=k^2$ enters the result above.
