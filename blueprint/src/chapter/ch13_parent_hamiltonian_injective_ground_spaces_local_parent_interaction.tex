...
\begin{theorem}[Right-boundary successor recursion]
    ...
    \end{align}
    This is the right-boundary recursion in
    \cite[Equation~(5.8)]{Fannes1992Finitely}, with
    $A^\mu=v(\mu)^\dagger$.
\end{theorem}
...
The boundary scalar-product identity, its exact defect form,
(\ref{eq:fnw_boundary_near_isometry_exact}), and
(\ref{eq:fnw_boundary_norm_sq_succ}) are now formalized. These results do not
include Lemma~5.3 or Lemma~6.2 of Ref.~\cite{Fannes1992Finitely}. They use no
unweighted finite-Gram substitution, periodic coercivity, comparison of
unequal kernels, cyclic substitute, dimension prefactor, projector-defect
terms $a/a_-+a^2/a_-$, identification $k=D$, or assertion
$C_\lambda=k^2$.
