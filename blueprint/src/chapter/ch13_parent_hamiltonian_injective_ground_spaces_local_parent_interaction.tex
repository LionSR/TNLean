...
the weighted spectral radius of the transfer remainder and one.  The remainder
has exactly the nonunit transfer eigenvalues together with zero, so the two
conditions describe the same rates once the spectral radius is known to equal
the largest modulus among those eigenvalues; that equality is not established
here, and the theorems below use only the spectral-radius condition.  The
