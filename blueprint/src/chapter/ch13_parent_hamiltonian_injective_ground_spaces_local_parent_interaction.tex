...
\cite[Lemma~5.3]{Fannes1992Finitely}. They do not yet estimate the overlap of
two three-block boundary families.
...
\begin{theorem}[Primitive FNW projector-defect estimate]
    ...
    Equation~(\ref{eq:fnw_projector_defect_unfactored}) holds whenever
...
\cite[Equation~(6.1)]{Nachtergaele1996Spectral}.  The results below assemble
that display from the FNW coefficient $a(m)(1+a(m))/a_-(m)$, the
...
\begin{theorem}[Geometric FNW projector defect for a primitive tensor]
    ...
    $c\lambda^m<1$, Equation~(\ref{eq:fnw_projector_defect_geometric}) holds for
...
\begin{theorem}[Open-chain form of the geometric FNW projector defect]
    ...
    \end{align}
    This is the inequality of
    Equation~(\ref{eq:nachtergaele_geometric_projector_defect}) with the rate
    and prefactor obtained as described at the head of this section, rather than
    under the source's eigenvalue prescription and with its value $k^2$.
\end{theorem}
...
\begin{proof}
    ...
    Equation~(\ref{eq:reassociated_tail_boundary_one_site}) and the remaining
