...
\begin{theorem}[Recovered dilation in ambient block coordinates]
    ...
    \begin{align}
        U_{BCE}
        =
        \bigoplus_j \mathbf 1_{b_j^L}\otimes U_j.
    \end{align}
    ...
    trace one.
    TNLean stores each sector in the reverse coordinate order from HJPW, so
    the corresponding Lean block matrix is
    $U_j\otimes\mathbf 1_{b_j^L}$.
...
\begin{theorem}[Hayashi SSA-equality characterization]
    ...
    all of $B$ and completes equations~(13)--(14), while
    Theorem~\ref{thm:hjpw_recovered_dilation_ambient_blocks} proves the
    recovery-dilation block action of equation~(15). Constructing the
    tripartite sector decomposition and assembling the quantum-Markov
    decomposition remain open. This forward implication
