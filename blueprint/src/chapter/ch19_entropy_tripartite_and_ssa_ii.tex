...
\begin{theorem}[Ambient recovered bipartite blocks]
    ...
    \begin{align}
        ...
        &=1.
    \end{align}
    This is TNLean's stored factor order, reverse to HJPW's
    $b_j^L\otimes b_j^R$ order. Directions complementary to the minimum
    joint support form
...
\begin{theorem}[Recovered dilation in ambient block coordinates]
    ...
    trace one.
    This is TNLean's stored order. In HJPW's order the same block form is
...
\begin{proof}
    ...
    with the Petz operation there. The supported reconstruction of
    $\rho_{AB}$ then transports the Petz recovery identity to the chosen
    unitary recovery.
\end{proof}
