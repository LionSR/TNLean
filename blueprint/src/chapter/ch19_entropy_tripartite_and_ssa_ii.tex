...
\begin{theorem}[Recovered dilation in ambient block coordinates]
    ...
    \begin{align}
        U_{BCE}
        =
        \bigoplus_j U_j\otimes\mathbf 1_{b_j^R}.
    \end{align}
    On every supported sector, $U_j$ dilates a trace-preserving completely
    positive map that sends $\sigma_j$ to a state on $b_j^L C$ whose
    $b_j^L$ marginal is $\sigma_j$. The resulting state is positive and has
    trace one.
...
\begin{proof}
    ...
    on $b_j^R$, while the induced operation on $b_j^L$ fixes $\sigma_j$.
