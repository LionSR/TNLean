...
\begin{theorem}[Recovered bipartite blocks on the joint support]
    ...
    % recovered-dilation theorem supplies equation (15).
\end{theorem}
...
\begin{theorem}[Recovered dilation in ambient block coordinates]
    \label{thm:hjpw_recovered_dilation_ambient_blocks}
    \lean{Matrix.exists_recoveredDilationBlockUnitary}
    \lean{Matrix.RecoveredConditionalDilationBlockForm.recoveredSectorState_posSemidef}
    \lean{Matrix.RecoveredConditionalDilationBlockForm.recoveredSectorState_trace}
    \lean{Matrix.RecoveredConditionalDilationBlockForm.partialTraceRight_recoveredSectorState}
    \leanok
    \uses{thm:hjpw_recovered_bipartite_ambient_blocks,
        thm:hjpw_recovered_conditional_joint_support_blocks}
    Let $\rho_{ABC}$ be a tripartite density matrix attaining equality in
    ...
    \end{align}
    from
    Theorem~\ref{thm:hjpw_recovered_bipartite_ambient_blocks}.
...
\begin{theorem}[Ambient recovered tripartite blocks]
    \label{thm:hjpw_recovered_tripartite_ambient_blocks}
    \lean{Matrix.RecoveredConditionalDilationBlockForm.ambient_tripartite_block_form}
    ...
    Theorems~\ref{thm:hjpw_recovered_bipartite_ambient_blocks} and
    \ref{thm:hjpw_recovered_dilation_ambient_blocks}. For each supported sector,
    put
    \begin{align}
        \widehat\rho_j
...
\begin{theorem}[SSA equality gives quantum Markov structure]
    ...
    \uses{def:hayashi_markov_decomposition,
        thm:hjpw_recovered_tripartite_ambient_blocks}
...
\begin{proof}\leanok
    \uses{def:hayashi_markov_decomposition,
        thm:hjpw_recovered_tripartite_ambient_blocks}
    By Theorem~\ref{thm:hjpw_recovered_tripartite_ambient_blocks}, put
    $W=\mathbf 1_A\otimes(U_B\otimes\mathbf 1_C)$. Then
    \begin{align}
        W^\dagger\rho_{ABC}W
        &=0_\perp\oplus
        ...
        \notag
    \end{align}
    where $\omega_j\geq0$, each supported recovered state
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    On a supported sector, $\overline\rho_j=\widehat\rho_j$ and
    $\overline\omega_j=\omega_j/p_j$ whenever $p_j>0$. Arbitrary normalized
    fillers are chosen only at this final boundary when $p_j=0$, including the
    complementary sectors; multiplication by $p_j$ makes every such choice
    invisible. The displayed direct sum is therefore a quantum Markov
