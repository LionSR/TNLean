...
\begin{theorem}[Maximal weight in convex decomposition]
    ...
    \[
        c \le \bigl\|\rho^{-1/2}\rho_1\rho^{-1/2}\bigr\|_\infty^{-1},
    \]
    where the inverse is taken on the range of $\rho$. This is
    \cite[Proposition, ``Maximal weight in convex decomposition'']
    {Wolf2012Quantum}.
\end{theorem}

\begin{proof}\leanok
    Necessity of the kernel inclusion: if $\rho v=0$ for some vector $v$,
    every term in $\rho=\sum_i\lambda_i\rho_i$ is positive semidefinite, so
    each summand's quadratic form at $v$ vanishes; in particular
    $\lambda_{i_1}\langle v,\rho_1v\rangle=0$, and since $\lambda_{i_1}=c>0$
    and $\rho_1$ is positive semidefinite, $\rho_1v=0$
    ...
    Finally $\rho-c\rho_1\ge0$ rewrites as $1\ge c\,\rho^{-1/2}\rho_1
    \rho^{-1/2}$, using the kernel inclusion to compress both sides onto
    the range of $\rho$ and conjugating by $\rho^{-1/2}$ there; by the
    $C^*$-identity this holds iff $c\|\rho^{-1/2}\rho_1\rho^{-1/2}\|_\infty
    \le 1$.
\end{proof}
