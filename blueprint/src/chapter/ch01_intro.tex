...
\begin{proof}\leanok
    ...
    testing against $\Omega$ gives the bound.
\end{proof}
...
\begin{lemma}[Hahn--Banach extension of $\tau$]
    ...
    $\tau'(A)\le\|A\|_\infty\,\tau'(\mathbb1)$ for every Hermitian
...
\begin{lemma}[Positivity of a dominated functional]
    ...
    $\varphi(A)\le\|A\|_\infty\,\varphi(\mathbb1)$ for every Hermitian
    $A\in\MN d$, with $\varphi(\mathbb1)\ge0$. Then $\varphi(A)\ge0$ for
    every positive semidefinite $A\in\MN d$.
\end{lemma}

\begin{proof}\leanok
    Apply the domination bound to $\|A\|_\infty\mathbb1-A$, which is
    positive semidefinite of norm at most $\|A\|_\infty$ whenever $A$ is
    positive semidefinite, and rearrange.
\end{proof}
...
\begin{lemma}[The Riesz matrix of the extension is positive semidefinite]
    ...
    \leanok
    \uses{lem:tau_extension, lem:tau_nonneg}
    Let $\tau'$ be as in Lemma~\ref{lem:tau_extension} and let $Y$ be the
    matrix representing $\tau'$ through the trace pairing,
    $\tau'(X)=\tr(YX)$. Then $Y$ is positive semidefinite.
\end{lemma}

\begin{proof}\leanok
    \uses{lem:tau_nonneg}
    By Lemma~\ref{lem:tau_domination} applied to $\tau'$ (which obeys the
    same bound as $\tau$ by Lemma~\ref{lem:tau_extension}) and
    Lemma~\ref{lem:tau_nonneg}, $\tau'$ is nonnegative on positive
    semidefinite matrices. Since $\tau'$ is also Hermiticity-preserving to
    conjugation, being built from the Hahn--Banach extension by
    decomposing into Hermitian and skew-Hermitian parts, $Y$ is Hermitian
    with nonnegative quadratic form, hence positive semidefinite.
\end{proof}
...
\begin{lemma}[Reconstructing a Kraus map from $\tau'$]
    ...
    \leanok
    \uses{lem:riesz_psd, def:is_kraus_cp}
    Let $\tau'$ be as in Lemma~\ref{lem:tau_extension} and let
    $Y=\sum_kv_kv_k^\dagger$ be a rank-one decomposition of its Riesz
    matrix (Lemma~\ref{lem:riesz_psd}). Then the map
    $T'(B)_{ij}:=n\cdot\tau'(B\otimes\ket i\bra j)$ equals
    $\sum_kK_kBK_k^\dagger$ for the reshaped operators $K_k:\C^m\to\C^n$,
    $K_k:=\sqrt n\,v_k^\dagger$ (viewing $v_k$ as an $m\times n$ matrix); in
    particular $T'$ is completely positive in rectangular Kraus form.
\end{lemma}
