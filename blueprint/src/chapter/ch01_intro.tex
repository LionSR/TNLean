...
\begin{definition}[Complete positivity on an operator system]
    \label{def:cp_on_operator_system}
    \lean{Matrix.IsCPOnOperatorSystem}
    \leanok
    \uses{def:operator_system}
    Let $S\subseteq\MN{m}$ be an operator system and $T:S\to\MN{n}$ a linear
    map. For a natural number $k$, write $T\otimes\mathrm{id}_k$ for the map
    ...
    every $k$ (the condition is vacuous at $k=0$).
\end{definition}
...
\begin{theorem}[Extending cp maps from operator systems]
    ...
    Let $S\subseteq\MN{m}$ be an operator system and $T:S\to\MN{n}$ a
    completely positive linear map on $S$. Then there is a completely
    positive map $T':\MN{m}\to\MN{n}$, in rectangular Kraus form, that
    agrees with $T$ on $S$.
\end{theorem}

\begin{proof}\leanok
    \uses{def:is_kraus_cp}
    Fix the maximally entangled vector $\Omega$ of dimension $n$ and define
    $\tau(A):=\braket{\Omega|(T\otimes\mathrm{id}_n)(A)|\Omega}$ for
    $A\in S\otimes\MN{n}$. Complete positivity of $T$ at level $n$ gives
    $\tau(A)\le f(A):=\|A\|_\infty\,\tau(\mathbb 1)$ for every Hermitian
    $A\in S\otimes\MN{n}$: $\|A\|_\infty\mathbb 1-A$ is positive
    semidefinite, so its image under $T\otimes\mathrm{id}_n$ is too, and
    testing against $\Omega$ gives the bound. Since $f$ is sublinear, the
    Hahn--Banach theorem extends $\tau$, first as a real-linear functional
    on the Hermitian matrices of $\MN{m}\otimes\MN{n}$ dominated by $f$
    throughout, and then, decomposing each matrix into its Hermitian and
    ...
    adjoint), to a $\C$-linear functional $\tau'$ agreeing with $\tau$ on
    all of $S\otimes\MN{n}$ and obeying the same bound on all Hermitian
    matrices of $\MN{m}\otimes\MN{n}$.

    Applying the bound to $\|A\|_\infty\mathbb 1-A$ for $A$ positive
    semidefinite shows $\tau'$ is nonnegative on positive semidefinite
    matrices. The matrix $Y$ representing $\tau'$ through the trace pairing
    is therefore positive semidefinite, hence a sum of rank-one terms
    $Y=\sum_k v_kv_k^\dagger$; reshaping each $v_k$ into an operator
    $K_k:\C^m\to\C^n$ gives a rectangular Kraus family for the map
    $T'(B)_{ij}:=n\cdot\tau'(B\otimes\ket i\bra j)$, which is exactly
    $\sum_kK_kBK_k^\dagger$. Wolf's inversion formula
    $\tau(B\otimes\ket i\bra j)=\tfrac1n\,T(B)_{ij}$ holds already for the
    original $T$ on $S$, and $\tau'$ agrees with $\tau$ on $S\otimes\MN{n}$,
    so $T'$ agrees with $T$ on $S$.
\end{proof}
