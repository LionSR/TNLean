...
block gauges~(\ref{eq:intro_matched_pair_gauge}) and the weight comparison then combine into a single gauge
conjugacy of the two weighted block-diagonal tensors~(\ref{eq:intro_canonical_form}). For a single injective
block this reduces to the familiar conclusion
$B^i = X A^i X^{-1}$ for all physical indices $i$, the single-block
specialization of~(\ref{eq:intro_matched_pair_gauge}).
