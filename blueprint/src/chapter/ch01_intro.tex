...
\begin{proof}\leanok
    \uses{thm:cp_iff_choi_posSemidef, lem:block_quadratic_form_nonneg_commuting}
    By Theorem~\ref{thm:cp_iff_choi_posSemidef} it suffices to show the Choi
    matrix of $T$ is positive semidefinite. Taking $[a_{ij}]$ in
    Lemma~\ref{lem:block_quadratic_form_nonneg_commuting} to be the
    coefficient family of the Choi matrix of the identity map exhibits the
    Choi-matrix quadratic form of $T$ as a block quadratic form with
    pairwise-commuting images (commutative range), so it is nonnegative for
    every $\{\psi_i\}$; this is exactly positive semidefiniteness of the
    Choi matrix.
\end{proof}
...
\begin{lemma}[Complete positivity is trace-adjoint invariant]
    ...
    \lean{PositiveOnAbelian.isCPMap_traceAdjointMap_iff}
    \leanok
