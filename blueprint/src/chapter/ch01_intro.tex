...
\begin{definition}[Ordered simple-spectrum perturbation]
    ...
    \begin{align}
        ...
        \label{eq:ch01_perturbed_eigenvalue}
    \end{align}
...
\begin{theorem}[Set spectrum determines multiplicities at simple spectrum]
    ...
    \lean{Matrix.IsHermitian.roots_charpoly_eq_of_spectrum_eq_of_roots_nodup}
    ...
    Then the root multisets of $\chi_A$ and $\chi_B$ are equal. Consequently,
    $\tr(B^k)=\tr(A^k)$ for every $k\in\N$.
\end{theorem}
...
\begin{proof}\leanok
    ...
    elements. Since the Hermitian polynomial $\chi_B$ splits and has degree
    $d$, its root multiset also has cardinality $d$. Hence it cannot repeat
    any root, and the two root multisets agree. Formula
    (\ref{eq:ch01_trace_power_sum}) then gives equality of trace powers.
\end{proof}
...
\begin{theorem}[Hermitian set-spectrum multiplicity bridge]
    \label{thm:hermitian_set_spectrum_charpoly}
    \lean{Matrix.charpoly_map_eq_of_preservesHermitianSpectrum}
    \leanok
