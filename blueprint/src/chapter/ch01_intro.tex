...
\begin{proof}\leanok
    ...
    Choose an orthonormal basis. Its rays have pairwise transition probability
    zero, so their images determine another orthonormal basis and hence a
    unitary $U$. After applying $U^{-1}$, the resulting map fixes the basis
    rays. The rays generated by $e_i+e_j$ determine relative moduli and real
    parts of phases. Those generated by $e_i+\mathrm{i}e_j$ determine the
    imaginary parts. Compatibility among triples of coordinates forces one
    global choice: all phases are preserved or all are conjugated. Thus the
    reduced map is respectively the identity or coordinatewise conjugation.
\end{proof}
