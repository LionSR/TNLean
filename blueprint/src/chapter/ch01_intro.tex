...
\begin{proof}\leanok
    Suppose first that $T$ has commutative range. Simultaneously
    diagonalizing the mutually commuting Hermitian family $\{T(a_{ij})\}$
    for an arbitrary positive semidefinite block matrix $[a_{ij}]$ reduces
    the block quadratic form $\sum_{i,j}\bra{\psi_i}T(a_{ij})\ket{\psi_j}$ to
    a sum of scalar terms, each nonnegative by positivity of $T$ and of
    $[a_{ij}]$; taking $[a_{ij}]$ to be the coefficient family of the Choi
    matrix of the identity map shows the Choi matrix of $T$ is positive
    semidefinite, which is complete positivity. If instead $T^*$ has
    commutative range, apply the codomain case to $T^*$ and use that $T$ is
    completely positive iff $T^*$ is.
\end{proof}
