\section{Normal blocks and overlap rigidity}

This section relates spectral normality to eventual block injectivity and
records the overlap dichotomy that detects bond-dimension and gauge-phase
matching between normal blocks.

\begin{theorem}[Normality forces positive bond dimension]
    \label{thm:cpsv_normal_bond_dim_pos}
    \lean{MPSTensor.IsNormalTensor.bondDim_ne_zero}
    \leanok
    \uses{def:nt_cpsv}
...
\end{theorem}
\begin{proof}
    \uses{lem:trace_mul_transfer_map_adjoint, lem:irreducible_transfer_perron_pair, thm:posDef_adjoint_eigenvector, thm:tp_gauge_from_adjoint_fixed, lem:tp_gauge_equiv}
    \leanok
...
\begin{theorem}[Spectral normality gives a pure trace-preserving gauge]
    ...
    \leanok
    \uses{def:nt_cpsv, thm:cpsv_normal_bond_dim_pos}
    Let $A$ be a normal tensor in the sense of
    \cite[Section~2.2]{Cirac2016MPDO_arXiv}. Its bond dimension is necessarily
    positive, and there is a positive definite matrix $\sigma$ such that the
    gauge obtained from $\sigma$ is trace-preserving, primitive, and
    irreducible. No scalar rescaling is needed.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:pure_perron_tp_gauge, lem:perron_gauge_primitive_iff}
...
\begin{theorem}[Spectral normality gives eventual block injectivity]
    ...
    \leanok
    \uses{def:nt_cpsv, def:normal, thm:cpsv_normal_bond_dim_pos}
    Every normal tensor in the sense of
    \cite[Section~2.2]{Cirac2016MPDO_arXiv} is injective after blocking by some
    finite length; its positive bond dimension follows from normality.
\end{theorem}
...
\begin{theorem}[A coefficient-form BNT is an algebraic BNT]
    ...
    \leanok
    \uses{def:bnt, def:bnt_cpsv, lem:cpsv_bnt_block_dimensions_positive, thm:cpsv_normal_eventually_injective}
...
\begin{proof}\leanok
    \uses{lem:cpsv_bnt_block_dimensions_positive, thm:cpsv_normal_eventually_injective}
...
\begin{theorem}[Phase-equivalent normal tensors are gauge-phase equivalent]
    ...
    \leanok
    \uses{def:nt_cpsv, def:mpv_block_phase_equiv, def:gauge_phase_equiv}
...
\begin{proof}\leanok
    \uses{thm:cpsv_normal_tp_gauge, thm:rect_overlap_norm_one_dim_eq, thm:nt_overlap_norm_one_gauge}
...
\begin{proof}\leanok
    \uses{thm:cpsv_normal_tp_gauge, thm:overlap_decay_rect_irr_tp, thm:overlap_decay_irr_tp, thm:gauge_invariance, thm:gauge_phase_mpv_scaling, thm:cpsv_normal_mpv_phase_gauge}
...
\begin{theorem}[Exact phase alternative for normal tensors]
    ...
    \lean{MPSTensor.overlap_tendsto_zero_of_not_mpvBlockPhaseEquiv}
    \leanok
...
\begin{proof}\leanok
    \uses{thm:cpsv_normal_overlap_dichotomy, thm:gauge_phase_mpv_scaling}
...
\begin{theorem}[Geometric proportionality phase for normal tensors]
    ...
    \leanok
    \uses{def:nt_cpsv, def:mpv_state, def:eventually_nonzero_proportional_mpv}
    Let $A$ and $B$ be normal tensors of positive, possibly different, bond
...
\begin{proof}\leanok
    \uses{thm:eventual_proportional_normalized_overlap_norm_one, thm:cpsv_normal_mpv_phase_alternative, thm:cpsv_normal_mpv_phase_gauge}
...
\begin{proof}\leanok
    \uses{thm:cpsv_normal_tp_gauge, lem:bnt_finite_index_overlap_orthonormal_li}
