...
\begin{proof}\leanok
    \uses{thm:mpdo_equiv_reindex_cptp, lem:is_kraus_cptp_comp, thm:mpdo_one_site_closure_two_site_blocking, thm:mpdo_two_site_closure_two_site_blocking}
...
\begin{theorem}[The normalized representative of Example 4.12 is not GSNNCH]
    ...
    \leanok
    \uses{def:cpsv_example412_literal_tensor, def:mpdo_source_gsnnch,
        thm:cpsv_example412_normalized_rfp_maps,
        thm:cpsv_example412_not_gsnnch}
...
% used in Appendix C.2.  The printed implication (ii)=>(iv) therefore needs a
% factorization argument that does not use absorbed normality; tracked in
% #6775.  Direct per-representative blocked channels are a separate possible
% route to (ii)=>(v), tracked in #6793, and their outer-sector composition is
% tracked in #6632.  Independently,
