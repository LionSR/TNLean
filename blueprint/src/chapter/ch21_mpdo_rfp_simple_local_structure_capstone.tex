...
\begin{theorem}[Finite-marginal obstruction to source zero correlation length]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Thus both displayed tensors fail the literal diagram of Definition~4.2
    at source lines 735--739.
...
\begin{proof}
    ...
    At length one, the exact partition function gives
    $\operatorname{tr}(\operatorname{mpo}(M,1))=
    \operatorname{tr}(\operatorname{mpo}(\widetilde M,1))=2$, so both
    physical-trace transfers are nonzero.  Literal idempotence would therefore
    imply the already-refuted up-to-scalar relation with scalar one.
\end{proof}
