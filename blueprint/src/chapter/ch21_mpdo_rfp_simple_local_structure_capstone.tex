...
\begin{lemma}[Rescaling invariance of the area-law quantities]
    ...
    Let $c$ be a nonzero complex number and let $L\le N$. Then
    \begin{align}
...
\end{lemma}
\begin{proof}\leanok
    \uses{thm:mpdo_normalized_mpo_scaling_invariance}
    ...
    functions of these reduced states. For $r>0$, the operators
    $\rho^{(N)}(rM)=r^N\rho^{(N)}(M)$ are positive semidefinite exactly when
    those of $M$ are, and their traces vanish exactly when those of $M$ do.
...
\end{theorem}
\begin{proof}\leanok
    \uses{lem:mpdo_area_law_scaling_invariance, thm:cpsv_example412_literal_sal, thm:cpsv_example412_normalized_rfp_maps, thm:cpsv_example412_normalized_not_gsnnch}
    Lemma~\ref{lem:mpdo_area_law_scaling_invariance} with $r=\frac12$
    transfers saturation from the printed tensor
    (Theorem~\ref{thm:cpsv_example412_literal_sal}) to $\widehat M$. The channel equations imply that the physical-trace transfer
    is idempotent. The commuting-form obstruction is the preceding theorem.
\end{proof}
