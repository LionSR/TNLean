...
\begin{theorem}[The five BNT identities give the GSNNCH form]
    ...
    copy numbers $n_x\geq1$, and arbitrary nonzero copy weights $\mu_{x,q}$.
    Suppose that the decomposition generates the same matrix product vectors
    as $M$ at every positive length.  Assume that the one-site words of each
    $A_x$ span its virtual matrix algebra and that, for distinct $x,y$,
    \begin{align}
