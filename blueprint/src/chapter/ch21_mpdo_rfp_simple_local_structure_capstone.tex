...
\begin{theorem}[Counterexample to the low-level absorbed-sector implication]
    ...
    \lean{MPOTensor.NonCartesianActiveSectorCandidate.transferMap_posDef_eigenvalue_le_one,
    ...
    two, and a nonzero scalar $\mu$ with $|\mu|\leq 1$, such that $K=\mu A$,
    the tensor $A$ is normal, and $K$ is injective, satisfies the strong area
    law, and obeys
    \begin{align}
        \mathcal T_K^2&=\mathcal T_K.
        \notag
...
\begin{proof}\leanok
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Pairing this inequality with a positive-definite Perron eigenmatrix of
    $\mathcal E_K$ shows that its Perron value $r$ is at most one. Perron
    normalization and a canonical gauge therefore express the tensor as
    $K=\mu A$ with $A$ normal and $|\mu|=\sqrt r\leq1$.
\end{proof}
