...
\begin{remark}[Normalization boundary in Lemma C.5]
    ...
    the paper's literal zero-correlation-length diagram. Thus the matrix
    inference is obstructed, while the exact source lemma remains unsettled.
\end{remark}
...
% assumed in Case I; its normal representative loses literal ZCL. Thus the
% source corollary remains unresolved. Corrected rank-one criteria under
% positive semidefiniteness or linear independence are given by
...
\begin{theorem}[BNT sector structure from SAL and ZCL (rank-one obstruction)]
    ...
    above, whose derivation invokes the unresolved rank-one step of Lemma~C.5.
    The available raw witness lies outside its normal-tensor standing
    hypothesis, and its normal representative loses literal ZCL. Thus no
    alternative proof of the complete sector structure under the exact source
    hypotheses is known.
\end{theorem}
