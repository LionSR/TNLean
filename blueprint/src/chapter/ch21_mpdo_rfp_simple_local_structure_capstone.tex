...
\begin{theorem}[Four properties of the normalized representative in Example 4.12]
    ...
    \lean{MPOTensor.isSAL_smul_ofReal_iff,
        MPOTensor.CPSVExample412NormalizedRFP.Mhat_isSAL,
    ...
    \leanok
    \uses{def:is_sal, def:mpo_physical_trace_idempotence, def:mpdo_source_gsnnch, def:is_rfp_via_ts, thm:cpsv_example412_literal_sal, thm:cpsv_example412_normalized_rfp_maps, thm:cpsv_example412_normalized_not_gsnnch}
    Saturation of the area law is invariant under rescaling of the tensor by a
    strictly positive real number. The normalized representative
...
\end{theorem}
\begin{proof}\leanok
    \uses{thm:cpsv_example412_literal_sal, thm:cpsv_example412_normalized_rfp_maps, thm:cpsv_example412_normalized_not_gsnnch}
    Rescaling by a nonzero scalar leaves every normalized reduced state
    unchanged, hence every block entropy and every mutual information; a
    strictly positive real factor also preserves positivity of the generated
    operators. Saturation therefore transfers from the printed tensor to
    $\widehat M$. The channel equations imply that the physical-trace transfer
