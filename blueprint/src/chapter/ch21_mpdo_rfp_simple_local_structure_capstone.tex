...
% Commit b222d6a2d certifies the scalar integer and logarithm inequalities only.
...
\begin{theorem}[Printed mixed-state separation examples]
    ...
    recorded in the accompanying paper-gap note gives $I_2>I_1$; its spectral,
    entropy, and tensor links remain to be formalized.  The printed universal
    exception list also omits $p=1$, where equality returns.
    ...
    accompanying paper-gap note instead gives $I_2>I_1$; its spectral, entropy,
    and tensor links remain to be formalized.  Global normalization does not
    change this conclusion.  An open or infinite stationary Markov-chain
    interpretation may explain the source claim, but the source does not state
    that interpretation.
