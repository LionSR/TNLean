...
\begin{remark}[Normalization boundary in Lemma C.5]
    ...
    structural package used by the following corollary.
\end{remark}
...
% has the displayed SAL, ZCL, and inverse-map data but is not the normal tensor
...
% still requires one all-sector package containing the physical isometry,
...
\begin{corollary}[Local sector structure from SAL and ZCL (all-sector boundary)]
    ...
    tensor satisfying the strong area law and zero correlation length. Then
    there are
    ...
    The raw obstruction above does not satisfy the normal-tensor standing
    hypothesis, while its normal representative loses literal ZCL. Thus the
    printed route remains unresolved at this normalization boundary; no
    alternative proof under the exact source hypotheses is known.
\end{corollary}
...
% **Normalization obstruction (Lemma C.5):**
% rem:mpdo_sal_zcl_rank_one_normalization_boundary records that the raw witness
% has the displayed SAL, ZCL, and inverse-map data but is not the normal tensor
% assumed in Case I; its normal representative loses literal ZCL. Hence the
% proposition remains unresolved under the exact source hypotheses. Corrected
% rank-one criteria under positive semidefiniteness or linear independence are
...
\begin{theorem}[BNT sector structure from SAL and ZCL (rank-one obstruction)]
    ...
    ZCL, and then to combine the resulting Case-I data into the all-sector
    structural package above. That passage is not yet formalized.
\end{theorem}
