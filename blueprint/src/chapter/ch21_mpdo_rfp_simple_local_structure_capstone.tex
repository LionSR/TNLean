...
\begin{theorem}[Printed mixed-state separation examples]
    ...
    fixed-parameter tensor; they are not printed in the source.  The tensor
    block-entropy identities and the resulting failure of saturation remain
    open.  We make no claim about the printed universal statement for
...
\begin{proof}
    ...
    Each reduced state is the isometric image of a diagonal bond-window
    distribution.  The diagonal weights give the listed nonzero roots.  The
    orthogonal complement contributes no zero roots for one site, eight for
    two sites, and forty-eight for three sites.  For four sites it contributes
    $4^4-2^4=240$ zero roots, while the eight odd-parity bond patterns supply
    the remaining eight.
\end{proof}

% Project-derived weight layer for CPSV16 Example 4.10, lines 897--905.
% Scope restriction: all four operator spectra are identified with the
% corrected tensor, but the tensor block-entropy identities remain open.
