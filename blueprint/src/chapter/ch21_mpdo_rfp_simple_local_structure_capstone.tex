...
\begin{theorem}[Common-weight BNT sectors have the GSNNCH form]
    ...
    \uses{thm:mpdo_ambient_sector_factorization_supported_bond,
        cor:mpdo_common_weight_bnt_layer_supports,
        cor:mpdo_bnt_orthogonal_commuting_sectors_gsnnch}
    Let $M$ have a BNT decomposition with normal representatives $A_x$,
    copy numbers $n_x$, and copy-independent weights $\mu_x$.  Put
    $\mathcal K_x=\mu_xA_x$, and suppose that this decomposition generates
    the same matrix product vectors as $M$ at every positive length.  Assume
    that the one-site words of the $A_x$ span their matrix algebras and that
    for distinct $x,y$,
    \begin{align}
        \mathcal K_x[\beta,\alpha]\,
        \mathcal K_y[\beta',\alpha']&=0
        \notag
    \end{align}
    for all virtual indices.  For every $x$, suppose that $\mathcal K_x$ has
    the physical-sector factorization
    \begin{align}
        U_x\mathcal K_xU_x^*
        &=\bigoplus_k l_{x,k}\otimes r_{x,k},
        \notag
    \end{align}
    ...
    This conclusion is restricted to copy-independent weights.  The five
    blockwise identities alone do not imply that restriction.
\end{theorem}

\begin{proof}
    \leanok
    The unstarred layer identities, one-site spanning, and positivity give
    pairwise orthogonal projections supporting the tensors
    $\mathcal K_x$ on both sides.  Theorem~
    \ref{thm:mpdo_ambient_sector_factorization_supported_bond} gives a
    positive commuting bond on each such projection whose periodic product
    is $\rho^{(N)}(\mathcal K_x)$.  Corollary~
    \ref{cor:mpdo_bnt_orthogonal_commuting_sectors_gsnnch} then applies the
    positive-length BNT identity
    \begin{align}
        \rho^{(N)}(M)
