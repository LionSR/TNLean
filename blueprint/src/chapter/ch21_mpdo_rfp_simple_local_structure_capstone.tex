...
\begin{theorem}[Neighboring positivity and trace factorization on the canonical
    ...
    \lean{MPOTensor.PhysicalSectorFactorization.FactorActiveSector,
        ...
        MPOTensor.PhysicalSectorFactorization.%
        ActivePhysicalCompressionWitness.neighboringOperator_trace,
        MPOTensor.PhysicalSectorFactorization.%
        ActivePhysicalCompressionWitness.neighboringTraceFactorization}
    \leanok
...
\begin{theorem}[Printed simple-MPDO implication chain]
    ...
    factorization for every absorbed representative. Proposition~C.7 constructs
    the channel pair belonging to one such factorization, and the proof of
    Proposition~C.15 first projects into each local physical subspace labelled
    by $j$ before applying those pairs. Thus the remaining source step is the
    existence of the per-representative factorizations; the outer-sector
    combination is already complete.
\end{theorem}
...
\begin{theorem}[Printed mixed-state separation examples]
    ...
    \uses{def:is_rfp_via_ts, def:is_sal, def:mpdo_source_gsnnch,
        ...
        thm:cpsv_example410_source_zcl,
        thm:cpsv_example410_four_site_spectrum,
        thm:cpsv_example410_tensor_entropy_obstruction,
        ...
        thm:cpsv_example412_not_gsnnch,
        thm:cpsv_example412_normalized_rfp_maps,
        thm:cpsv_example412_normalized_not_gsnnch}
    ...
    left-qubit label.  We interpret the second occurrence as the right-qubit
    label and fix $p=1/4$.  The resulting tensor is an MPDO and has source zero
    correlation length.  The full characteristic-polynomial root multisets of
    its normalized one-, two-, three-, and four-site reduced states are
    \begin{align}
        L=1: & \quad \{0^{[0]},(1/4)^{[4]}\}, \notag                 \\
        ...
        L=4: & \quad \{0^{[248]},(41/128)^{[1]},(15/128)^{[4]},
                           (9/128)^{[3]}\}. \notag
    \end{align}
    These formulas are project-derived consequences of the corrected
    fixed-parameter tensor; they are not printed in the source.  They give the
    four block entropies and the exact strict inequality
    ...
    \end{align}
    Hence the corrected tensor has source zero correlation length and does not
    saturate the area law.  We make no claim about the printed universal
    statement for $p\ne0,1/2$, whose exception list also omits $p=1$.
...
\begin{proof}
    \leanok
    \uses{def:cpsv_example410_corrected_tensor,
        thm:cpsv_example410_bond_distribution}
...
\begin{theorem}[Entropy obstruction for the corrected Example 4.10 tensor]
    ...
    \leanok
    \uses{def:cpsv_example410_corrected_tensor, def:block_entropy,
        def:mutual_info_chain, def:is_sal, def:mpo_source_zcl}
...
\begin{proof}
    \leanok
    \uses{thm:block_entropy_of_charpoly_roots,
        thm:cpsv_example410_source_zcl,
        thm:cpsv_example410_four_site_spectrum,
        thm:cpsv_example410_window_entropies,
        thm:cpsv_example410_mutual_information_defect,
        thm:cpsv_example410_log_ratio_pos, lem:sal_const}
...
\begin{theorem}[Bond-one physical-sector boundary]
    ...
    \leanok
    \uses{def:is_sal, def:mpo_physical_trace_idempotence,
...
% Once it is supplied, the Proposition C.7 channel construction and its
% outer-sector composition are formalized; the latter was tracked in #6632.
% Independently,
...
% The necessary condition thm:mpdo_neighboring_trace_unit_closure shows that
% any qualifying factorization of a tensor forces every nonempty closed chain
% of that tensor to have unit trace.  The per-representative assertion below
% therefore carries the implicit normalization tr(T_{K_s}) = 1 for each
% absorbed representative, which the line-1498 normalization asserts but the
% ambient normalization at lines 1755-1759 supplies only summed over
% representatives.  Whether the standing hypotheses of (ii) imply this
% per-representative unit trace is part of the remaining content of #6775.
...
\begin{proof}\leanok
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Pairing this positive-definite gap with a positive-definite Perron
    eigenmatrix of $\mathcal E_K$ gives a strictly positive trace pairing and
    shows that its Perron value $r$ is strictly below one. Perron
