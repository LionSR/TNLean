...
\begin{theorem}[Algebraic Case-II data for the absorption example]
    ...
    \uses{def:nt_cpsv, def:sector_bnt_cf,
        def:blockwise_product_word_span, def:horizontal_cf_mpdo,
        def:mpo_physical_trace_transfer}
    The two bond-one representatives in the coefficient-absorption example
    are normal and, with weights $(1/\sqrt2,1)$, form a normalized BNT
    canonical decomposition. Their simultaneous word-tuple span is the full
    two-block direct sum already at length one, so the representative family
    is in biCF. The ambient tensor is exactly the weighted block-diagonal
    assembly, both entrywise and as a doubled-index MPS tensor, and is in
    normalized BNT-refined horizontal form. Moreover, the doubled
    physical-trace transfer of neither representative is nilpotent.
...
\begin{proof}
    \leanok
    The first representative has two nonzero letters, both equal to
    $1/\sqrt2$, and the second has one nonzero letter equal to $1$.
    Their transfer maps are the identity, while their disjoint physical
    supports exclude equivalence by a virtual gauge and a phase. The letters
    $0$ and $8$ separately select the two scalar blocks, proving the
    simultaneous one-letter span. Direct evaluation of the weighted diagonal
    sum gives the ambient tensor and the identity block gauge. Finally, the
    two doubled physical-trace transfers are respectively the nonzero scalar
    $2/\sqrt2$ and the identity.
\end{proof}
