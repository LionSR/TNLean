...
\begin{theorem}[Printed simple-MPDO implication chain]
    ...
    \notready
    \discussion{5917}
    \uses{def:is_rfp_via_ts, def:is_sal, def:mpo_physical_trace_transfer,
    ...
    of normal tensors, and let $M=K^{[2]}$.  Theorem~4.9 of
    \cite{Cirac2016MPDO_arXiv} states the implication chain
    \begin{align}
    ...
    The forward fixed-point implication, the Markov decomposition, the channel
    constructions under the local factorization, and the sectorwise analytic
    inheritance are formalized.  The remaining Case-II step is the all-sector
    factorization: absorbing a coefficient $\mu_s$ multiplies the transfer
    spectral radius by $|\mu_s|^2$, while the source normalization does not
    imply $|\mu_s|=1$ for every sector.  The two not-ready nodes
    Theorems~\ref{thm:mpdo_sal_zcl_bnt_sector_structure} and
    \ref{thm:mpdo_sal_zcl_blocking_channels} record exactly this boundary.
\end{theorem}
...
\begin{theorem}[Printed mixed-state separation examples]
    ...
    \notready
    \discussion{5919}
    \uses{def:is_rfp_via_ts, def:is_sal, def:mpo_source_zcl}
    Examples~4.10--4.12 of \cite{Cirac2016MPDO_arXiv} give three proposed
    separations among zero correlation length, saturation of the area law, the
    fixed-point condition, and the simple commuting form.  Example~4.10 uses a
    four-spin ring of Bell pairs followed by correlated on-site bit-flip noise
    and claims zero correlation length without saturation of the area law.
    Example~4.11 uses the bond-weight matrix
    \begin{align}
        W&=\begin{pmatrix}1&1/2\\1/2&1\end{pmatrix}
        \notag
    \end{align}
    and claims saturation without zero correlation length.  Example~4.12 uses
    an explicit two-dimensional tensor generating
    \begin{align}
        \rho^{(N)}&=I^{\otimes N}+\sigma_z^{\otimes N}
        \notag
    \end{align}
    and claims saturation and zero correlation length without the simple
    commuting form.  The printed saturation claim in Example~4.11 requires an
    independent calculation and is not assumed here.
\end{theorem}

% **All-sector Case-II boundary:**
% Analytic inheritance is complete for every absorbed BNT representative.
% Coefficient absorption need not preserve normality, so the normal Case-I
% structural theorem cannot yet be applied sectorwise.
% Documented in
% docs/paper-gaps/cpgsv17_pf_rank_one.tex.
\begin{theorem}[BNT sector structure from SAL and ZCL (all-sector Case-II boundary)]
    ...
    $s$. Hence the normal Case-I result
    Corollary~\ref{cor:mpdo_sal_zcl_local_sector_structure} cannot yet be
    applied to every absorbed BNT representative. This normality step, and the
    resulting all-sector factorization, remain open.
\end{theorem}
