...
\begin{remark}[Normalization boundary in Lemma C.5]
    ...
    injective matrix product density operator tensor saturating the area law
    and having zero correlation length has real families $(a_k)_k,(b_k)_k$ with
    \begin{align}
...
% coarsened channel pair exists.  This is a project-derived example, not the
% construction used in the source proof of Proposition C.15.
% Taxonomy: proof-path drift.
\begin{theorem}[Pair-preserving obstruction for the four-sector tensor]
...
\begin{proof}
    ...
    This gives the physical isometry, the
...
\begin{definition}[Unstarred orthogonality of BNT layers]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The factor order is $\mathcal K_y\mathcal K_x$ without adjoints.
    This is the orthogonality relation in
...
\begin{definition}[Physical adjoint]
    ...
    indices retain their order. The doubled-bond coordinate exchange
    $(a,b)\mapsto(b,a)$, an involution of the doubled virtual space, is
    recorded here as the coordinate map accompanying this definition.
    For every periodic chain,
    \begin{align}
...
\begin{theorem}[Canonical restriction constructed from the active sector factors]
...
\begin{theorem}[Neighboring positivity and trace factorization on the canonical
        active restriction]
    ...
    \lean{MPOTensor.PhysicalSectorFactorization.FactorActiveSector,
        ...
        MPOTensor.PhysicalSectorFactorization.%
        activeRestrictedPhysicalSectorFactorization_neighboringOperator_trace,
        MPOTensor.PhysicalSectorFactorization.%
        activeRestrictedPhysicalSectorFactorization_neighboringTraceFactorization}
    \leanok
    \uses{thm:mpdo_active_physical_sector_restriction, def:mpdo_neighboring_trace_factorization}
    In the setting of
    ...
    Suppose moreover that the ambient neighboring operators admit real
    coefficients $a_k,b_h$ such that
    \begin{align}
        \tr(\eta_{k,h}) & =a_kb_h,
        \notag                     \\
        \sum_k a_kb_k   & =1.
        \notag
    \end{align}
    For the active-sector correspondence $j\mapsto k(j)$, set
    $\widetilde a_j=a_{k(j)}$ and $\widetilde b_j=b_{k(j)}$.  Then
    \begin{align}
        \tr(\eta^{\mathrm{act}}_{j,l})
         & =\widetilde a_j\widetilde b_l,
        \notag                            \\
        \sum_j\widetilde a_j\widetilde b_j
         & =1.
        \notag
    \end{align}
    \textbf{Scope restriction (active physical support compression):}
    The identities being preserved correspond to
    \cite[Theorem~4.9(iv), lines~864--889]{Cirac2016MPDO_arXiv}. Their
    transport through active physical support compression is an auxiliary
    construction rather than an assertion of that source theorem; documented in
    \cite{gap:cpsv16_active_physical_support_compression}.
\end{theorem}
...
\begin{theorem}[Normal BNT representatives, biCF, exact direct sum, and
...
\begin{theorem}[Coefficient absorption need not preserve normality]
    ...
    $(\mu_0,\mu_1)=(1/\sqrt2,1)$. Their weighted direct sum has
    \begin{align}
...
\begin{proof}
    ...
    \end{align}
    Equations~(\ref{eq:caseii-ambient-slices}) and
    (\ref{eq:caseii-ambient-mpo}) follow by multiplying the diagonal slices.
    There are $2^N$ binary strings and one terminal string, which gives
    (\ref{eq:caseii-ambient-trace}). For a virtual matrix $X$, the one-site and
    ...
    (\ref{eq:caseii-ambient-trace}), they imply saturation of the area law. Summing
    the diagonal slices gives (\ref{eq:caseii-ambient-zcl}), the literal
    ...
    (\ref{eq:caseii-absorbed-radius}) and the failure of the printed inference.
\end{proof}
...
\begin{remark}[Printed simple-MPDO implication chain]
    \label{thm:cpsv_theorem49_printed_status}
    For a simple tensor $K$ in block-injective canonical form with a basis of
    normal tensors and $M=K^{[2]}$,
    \cite[Theorem~4.9]{Cirac2016MPDO_arXiv} prints the chain
    (i)$\Rightarrow$(ii), (ii)$\Leftrightarrow$(iii), (iii)$\Rightarrow$(iv),
    (iv)$\Rightarrow$(v) between the fixed-point condition, SAL with zero
    correlation length, the commuting Gibbs form with zero correlation length,
    the blockwise local structure, and the fixed-point condition for $M$.
    Theorem~\ref{thm:mpdo_sal_zcl_bnt_sector_structure} refutes the
    representativewise assertion used in (ii)$\Rightarrow$(iv). Its ambient
    tensor has a two-representative BNT presentation in biCF. Independently,
    Theorem~\ref{thm:mpdo_theorem49_iv_to_v_counterexample} refutes the literal
    implication (iv)$\Rightarrow$(v). The representative-channel and
    outer-sector constructions relevant to Proposition~C.15 are proved
    conditionally for supplied absorbed-sector factorizations, but the printed
    route obtains those factorizations from the false representativewise
    assertion. More strongly,
    Theorem~\ref{thm:mpdo_sal_zcl_blocking_channels} gives a trace-norm
    counterexample to Proposition~C.15 itself and shows that the same ambient
    tensor has a two-site blocking which is not an RFP. These false-source and
    proof-path dispositions are documented in
    \cite{gap:cpgsv17_pf_rank_one,
        gap:cpsv16_prop_c15_blocking_channel_false}.
\end{remark}

% CPSV16 Examples 4.10--4.12, lines 897--942.
% Entropy corrections: docs/paper-gaps/cpsv16_examples_4_10_4_11_entropy.tex.
\begin{remark}[Printed mixed-state separation examples]
    \label{thm:cpsv_examples410_412_status}
    The separations asserted in
    \cite[Examples~4.10--4.12, lines~897--942]{Cirac2016MPDO_arXiv} are decided
    as follows.  Example~4.10, read with one left and one right bit flip and
    $p=1/4$ (Definition~\ref{def:cpsv_example410_corrected_tensor}), has source
    zero correlation length and does not saturate the area law
    (Theorems~\ref{thm:cpsv_example410_source_zcl}
    and~\ref{thm:cpsv_example410_tensor_entropy_obstruction}); no claim is made
    about the printed universal statement in $p$.  The literal finite periodic
    family of Example~4.11 fails both saturation and source zero correlation
    length (Theorems~\ref{thm:cpsv_example411_ambient_not_sal}
    and~\ref{thm:cpsv_example411_not_source_zcl}).  The literal tensor of
    Example~4.12 is unnormalized, so its fixed-point and zero-correlation-length
    assertions fail for that representative
    (Theorem~\ref{thm:cpsv_example412_literal_zcl_rfp_gap}); the halved tensor
    saturates the area law, has literal zero correlation length, and
    satisfies the two channel equations without supplying the unit-weight
    convention
    (Theorems~\ref{thm:cpsv_example412_normalized_rfp_maps}
    and~\ref{thm:cpsv_example412_normalized_four_properties}), and both
    representatives lie outside the full GSNNCH class
    (Theorems~\ref{thm:cpsv_example412_not_gsnnch}
    and~\ref{thm:cpsv_example412_normalized_not_gsnnch}).
\end{remark}

% Local correction of CPSV16 Example 4.10, lines 897--905.
% The source repeats the left-qubit label; this definition uses one left and
% one right bit flip and fixes p=1/4. See
% docs/paper-gaps/cpsv16_examples_4_10_4_11_entropy.tex.
\begin{definition}[Corrected tensor for Example 4.10]
    \label{def:cpsv_example410_corrected_tensor}
    \lean{MPOTensor.CPSVExample410Operator.channelCoeff,
        MPOTensor.CPSVExample410Operator.pairEquiv,
        MPOTensor.CPSVExample410Operator.correlatedFlip,
        MPOTensor.CPSVExample410Operator.purifier,
        MPOTensor.CPSVExample410Operator.purifierTerm,
        MPOTensor.CPSVExample410Operator.M}
    \leanok
    Identify the local four-dimensional spin with two qubits and write
    $U=\sigma_x\otimes\sigma_x$.  Starting from one Bell pair on every bond,
    apply independently at each site the channel
    \begin{align}
        \mathcal E(X) & =\tfrac14 UXU^*+\tfrac34X. \notag
    \end{align}
    Equivalently, use the local purification with Kraus amplitudes
    $\sqrt3/2$ for the identity and $1/2$ for the correlated flip.  This is
    distinct from the repeated-left expression printed in the source.
\end{definition}

\begin{theorem}[Source zero correlation length of the corrected tensor]
    \label{thm:cpsv_example410_source_zcl}
    \lean{MPOTensor.CPSVExample410Operator.M_isLPDO,
        MPOTensor.CPSVExample410Operator.M_isMPDO,
        MPOTensor.CPSVExample410Operator.traceProjector,
        MPOTensor.CPSVExample410Operator.physTraceTransfer_M,
        MPOTensor.CPSVExample410Operator.traceProjector_sq,
        MPOTensor.CPSVExample410Operator.physTraceTransfer_M_sq,
        MPOTensor.CPSVExample410Operator.physTraceTransfer_M_ne_zero,
        MPOTensor.CPSVExample410Operator.M_isSourceZCL}
    \leanok
    \uses{def:cpsv_example410_corrected_tensor, def:mpo_source_zcl}
    The corrected tensor is locally purified and hence positive at every
    chain length.  It has source zero correlation length.
\end{theorem}

\begin{proof}
    \leanok
    \uses{def:cpsv_example410_corrected_tensor, lem:complex_ofreal_sqrt_sq, thm:mpo_source_zcl_of_physical_trace_idempotent}
    Its physical-trace transfer is the nonzero projector
    \begin{align}
        P & =\frac12
        \begin{pmatrix}
            1 & 0 & 0 & 1 \\
...
\begin{theorem}[One- through four-site spectra of the corrected tensor]
    \label{thm:cpsv_example410_four_site_spectrum}
    \lean{MPOTensor.CPSVExample410Spectrum.charpoly_roots_one,
        MPOTensor.CPSVExample410Spectrum.charpoly_roots_two,
        MPOTensor.CPSVExample410Spectrum.charpoly_roots_three,
        MPOTensor.CPSVExample410Spectrum.charpoly_roots_four}
    \leanok
    \uses{def:cpsv_example410_corrected_tensor}
    The normalized reduced states on one, two, three, and four consecutive
    sites have characteristic-polynomial root multisets
    \begin{align}
        L=1: & \quad \{0^{[0]},(1/4)^{[4]}\}, \notag                 \\
...
\begin{proof}
    \leanok
    \uses{def:cpsv_example410_corrected_tensor, thm:cpsv_example410_bond_distribution}
    For $1\leq L\leq4$, the proof constructs a bond-window weight family
...
% Corrected fixed-parameter conclusion for CPSV16 Example 4.10.
% The source prints the channel and decimal entropies. The root multisets and
% exact entropy calculation below are project-derived.
\begin{theorem}[Entropy obstruction for the corrected Example 4.10 tensor]
    \label{thm:cpsv_example410_tensor_entropy_obstruction}
    \lean{MPOTensor.CPSVExample410Entropy.blockEntropy_one,
        MPOTensor.CPSVExample410Entropy.blockEntropy_two,
        MPOTensor.CPSVExample410Entropy.blockEntropy_three,
        MPOTensor.CPSVExample410Entropy.blockEntropy_four,
        MPOTensor.CPSVExample410Entropy.mutualInfo_two_sub_one,
        MPOTensor.CPSVExample410Entropy.mutualInfo_two_sub_one_pos,
        MPOTensor.CPSVExample410Entropy.M_not_isSAL,
        MPOTensor.CPSVExample410Entropy.M_isSourceZCL_and_not_isSAL}
    \leanok
    \uses{def:cpsv_example410_corrected_tensor, def:block_entropy, def:mutual_info_chain, def:is_sal, def:mpo_source_zcl}
    ...
    \begin{align}
        ...
        \frac{2^{32}7^7}{3^3 5^{20}}\right)>0.\notag
    \end{align}
    The tensor has source zero correlation length and does not saturate the
    area law. These identities concern only the corrected fixed value
    $p=1/4$.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:block_entropy_of_charpoly_roots, thm:cpsv_example410_source_zcl, thm:cpsv_example410_four_site_spectrum, thm:cpsv_example410_window_entropies, thm:cpsv_example410_mutual_information_defect, thm:cpsv_example410_log_ratio_pos, lem:sal_const}
    The four root multisets give the displayed block entropies. Substitution
    into $I_L=S_L+S_{4-L}-S_4$ gives the exact logarithmic ratio, which is
    positive by Theorem~\ref{thm:cpsv_example410_log_ratio_pos}. If the area
    law saturated, Lemma~\ref{lem:sal_const} would give $I_1=I_2$, a
    contradiction. The source zero-correlation-length statement is
    Theorem~\ref{thm:cpsv_example410_source_zcl}.
\end{proof}
...
\begin{definition}[Configuration sign]
    ...
    \lean{MPOTensor.configurationSign}
    \leanok
...
% CPSV16 line 792 normalization and Definition 4.6, lines 797 and 811--815.
\begin{lemma}[Rescaling invariance of the area-law quantities]
    \label{lem:mpdo_area_law_scaling_invariance}
    \lean{MPOTensor.reducedBlockState_smul,
        MPOTensor.blockEntropy_smul,
        MPOTensor.mutualInfoChain_smul,
        MPOTensor.isSAL_smul_ofReal_iff}
    \leanok
    \uses{def:block_entropy, def:mutual_info_chain, def:is_sal, thm:mpdo_normalized_mpo_scaling_invariance}
    Let $M$ be an MPO tensor, let $c$ be a nonzero complex number, and let
    $L\le N$ be natural numbers. Then
    \begin{align}
        \rho_L^{(N)}(cM) & =\rho_L^{(N)}(M).
        \notag
    \end{align}
    If $\rho^{(N)}(cM)$ and $\rho^{(N)}(M)$ are positive semidefinite, then
    \begin{align}
        S_L^{(N)}(cM) & =S_L^{(N)}(M),
                      & I_L(cM)        & =I_L(M).
        \notag
    \end{align}
    For every real $r>0$, the tensor $rM$ saturates the area law if and only
    if $M$ does.
\end{lemma}
\begin{proof}\leanok
    \uses{thm:mpdo_normalized_mpo_scaling_invariance, thm:mpdo_mpo_scalar_scaling, thm:mpdo_positive_rescaling_iff}
    By Theorem~\ref{thm:mpdo_normalized_mpo_scaling_invariance}, the
    normalized operator $\sigma^{(N)}(cM)$ equals $\sigma^{(N)}(M)$, so their
    partial traces agree; the block entropies and mutual informations are
    functions of these reduced states. For $r>0$,
    Theorem~\ref{thm:mpdo_positive_rescaling_iff} shows that $rM$ generates
    positive semidefinite operators exactly when $M$ does, and
    Theorem~\ref{thm:mpdo_mpo_scalar_scaling} gives
    $\rho^{(N)}(rM)=r^N\rho^{(N)}(M)$, so the traces of the two operators
    vanish together.
    The mutual-information clause of Definition~\ref{def:is_sal} transfers by
    the displayed identity.
\end{proof}

% CPSV16 Example 4.12, lines 932--939, with Definitions 4.1, 4.2, 4.6, and 4.8.
\begin{theorem}[Four properties of the normalized representative in Example 4.12]
    \label{thm:cpsv_example412_normalized_four_properties}
    \lean{MPOTensor.CPSVExample412NormalizedRFP.Mhat_isSAL,
        MPOTensor.CPSVExample412NormalizedRFP.Mhat_isPhysicalTraceIdempotent,
        MPOTensor.CPSVExample412NormalizedRFP.Mhat_isSAL_isPhysicalTraceIdempotent_not_isGSNNCH_isRFPViaTS}
    \leanok
    \uses{def:is_sal, def:mpo_physical_trace_idempotence, def:mpdo_source_gsnnch, def:is_rfp_via_ts, lem:mpdo_area_law_scaling_invariance, thm:cpsv_example412_literal_sal, thm:cpsv_example412_normalized_rfp_maps, thm:cpsv_example412_normalized_not_gsnnch}
    The normalized representative
    $\widehat M=\frac12 M$ of Example~4.12 saturates the area law, its
    physical-trace transfer is idempotent, it is not a Gibbs state of a
    nearest-neighbor commuting Hamiltonian, and it satisfies the two channel
    equations of Definition~4.1.

    This is a scope-restricted form of
    \cite[Example~4.12]{Cirac2016MPDO_arXiv}: the tensor is the halved
    representative rather than the printed one, and the fixed-point assertion
    is read as the two channel equations, without the global unit-weight
    canonical convention of the source
    \cite{gap:cpsv16_example_4_12_normalization,
        gap:cpsv16_unit_weight_rfp_scale_tension}.
\end{theorem}
\begin{proof}\leanok
    \uses{lem:mpdo_area_law_scaling_invariance, thm:cpsv_example412_literal_sal, thm:cpsv_example412_normalized_rfp_maps, thm:rfp_via_ts_physical_trace_idempotent, thm:cpsv_example412_normalized_not_gsnnch}
    Lemma~\ref{lem:mpdo_area_law_scaling_invariance} with $r=\frac12$
    transfers saturation from the printed tensor
    (Theorem~\ref{thm:cpsv_example412_literal_sal}) to $\widehat M$. By
    Theorem~\ref{thm:rfp_via_ts_physical_trace_idempotent}, the channel
    equations of Theorem~\ref{thm:cpsv_example412_normalized_rfp_maps} imply
    that the physical-trace transfer is idempotent. The commuting-form
    obstruction is
    Theorem~\ref{thm:cpsv_example412_normalized_not_gsnnch}.
\end{proof}

% CPSV16 Theorem 4.9 and Appendix C.2, lines 1381--1403, 1441--1450, and 1647--1782.
\begin{theorem}[Bond-one physical-sector boundary]
    \label{thm:mpdo_bond_one_physical_sector_factorization}
    \lean{MPOTensor.BondOnePhysicalSectorFactorization.sectorEquiv,
        MPOTensor.BondOnePhysicalSectorFactorization.factorization,
        MPOTensor.BondOnePhysicalSectorFactorization.physTraceTransfer_eq_one,
        MPOTensor.BondOnePhysicalSectorFactorization.neighboringOperator_eq_submatrix,
        MPOTensor.BondOnePhysicalSectorFactorization.neighboringOperator_pos,
        MPOTensor.BondOnePhysicalSectorFactorization.neighboringOperator_trace,
        MPOTensor.BondOnePhysicalSectorFactorization.neighboringTraceFactorization,
        MPOTensor.exists_physicalSectorFactorization_of_bondDim_one}
    \leanok
...
\end{theorem}
\begin{proof}\leanok
    \uses{lem:mpdo_sal_phys_trace_transfer_nonzero}
    Use one physical sector, with the full physical space as its left factor
    and a one-dimensional right factor. Its neighboring operator is the
    one-site periodic operator, so positivity follows from the MPDO condition
    included in SAL. The physical-trace transfer is a nonzero scalar. Literal
    idempotence therefore makes it equal to one, and the neighboring trace is
    normalized by $a_0=b_0=1$.
\end{proof}
...
\begin{theorem}[Counterexample to the low-level absorbed-sector implication]
    ...
    \lean{MPOTensor.NonCartesianActiveSectorCandidate.transferMap_posDef_eigenvalue_lt_one,
        MPOTensor.NonCartesianActiveSectorCandidate.transferMap_posDef_eigenvalue_le_one,
        MPOTensor.NonCartesianActiveSectorCandidate.exists_normalTensor_scalar_representation,
        MPOTensor.NonCartesianActiveSectorCandidate.full_lowLevel_counterexample}
    \leanok
    \uses{def:is_sal, def:mpo_physical_trace_transfer, def:mpdo_physical_sector_factorization, def:mpdo_neighboring_trace_factorization}
    There are tensors $K,A$ of physical dimension four and virtual dimension
    two, and a scalar $\mu$ with $0<|\mu|<1$, such that $K=\mu A$, the tensor
    $A$ is normal, and $K$ is injective, saturates the area law, and obeys
    \begin{align}
...
\begin{proof}\leanok
    ...
    neighboring operators are positive. For the positive test matrix
    ...
    normalization and a canonical gauge therefore express the tensor as
    $K=\mu A$ with $A$ normal and $0<|\mu|=\sqrt r<1$.
\end{proof}

\begin{theorem}[Prepared BNT decomposition for the rank-one neighboring-trace obstruction]
    \label{thm:mpdo_neighboring_trace_obstruction_prepared_two_block_bnt}
    \lean{MPOTensor.NeighboringTraceObstructionAmbientBlocks.exists_obstruction_terminal_twoBlock_BNT_witness,
        MPOTensor.NeighboringTraceObstructionAmbientBlocks.terminalBlock_isNormalTensor}
    \leanok
    \uses{def:sector_bnt_cf}
    There are a scalar $\mu$ with $0<|\mu|<1$ and two normal
    left-canonical blocks $B_{\mathrm{obs}}$ and $B_{\mathrm{term}}$, of bond
    dimensions two and one respectively, such that the embedded obstruction
    tensor is gauge equivalent to $\mu B_{\mathrm{obs}}$.
    The two-block decomposition with one copy of each block and coefficients
    $(\mu,1)$ is in normalized BNT canonical form. Its one-site physical
    supports $P_{\mathrm{obs}}$ and $P_{\mathrm{term}}$ are disjoint and
    resolve the ambient physical space:
    \begin{align}
        P_{\mathrm{obs}}P_{\mathrm{term}}
                                           & =P_{\mathrm{term}}P_{\mathrm{obs}}=0,
        \notag                                                                     \\
        P_{\mathrm{obs}}+P_{\mathrm{term}} & =1.
        \notag
    \end{align}
    This is a project-derived preparation of the counterexample, not a
    construction asserted in \cite{Cirac2016MPDO_arXiv}. It supplies the two
    normal blocks used in the ambient construction below.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpdo_sal_zcl_bnt_sector_structure_counterexample,
        thm:mpdo_isometric_physical_embedding}
    Embed the four-letter obstruction tensor into four coordinates of a
    five-dimensional physical space and place a bond-one product block on the
    remaining coordinate. The coordinate projections are complementary. The
    terminal block has coefficient one, while the first block retains the
    coefficient $\mu$. Their unequal bond dimensions separate the two BNT
    representatives.
\end{proof}
...
\begin{theorem}[Ambient neighboring-trace counterexample properties]
    \label{thm:mpdo_neighboring_trace_obstruction_ambient_properties}
    \lean{MPOTensor.NeighboringTraceObstructionAmbientCounterexample.%
    ...
    dimensions two and one, and each occurs once. The bond-two representative
    is one-site injective. Their coefficients are $\mu$ and $1$, where
    $0<|\mu|<1$. In particular, every coefficient has modulus at most one and
    the second coefficient has modulus one. The embedded four-letter
    obstruction is virtually gauge equivalent to $\mu A_0$.
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The obstruction and terminal sectors have one-site support projections
    $P_0,P_1$ satisfying
    \begin{align}
        P_0P_1=P_1P_0 & =0, \notag \\
        P_0+P_1       & =1. \notag
...
\begin{proof}\leanok
    ...
    Use the prepared obstruction and terminal blocks. The obstruction slices
    span $M_2(\mathbb C)$ and vanish on the terminal coordinate, whereas the
    terminal block equals one on that coordinate. Hence the pairs of one-site
    slices span $M_2(\mathbb C)\times\mathbb C$ in one letter. Both
    representatives are normal, and the coefficients give the stated global
    normalization.
...
\begin{theorem}[Universal obstruction for the absorbed representative]
    \label{thm:mpdo_neighboring_trace_obstruction_absorbed_no_factorization}
    \lean{MPOTensor.NeighboringTraceObstructionAmbientCounterexample.%
        firstCommonWeightAbsorbed_not_exists_neighboringTraceFactorization}
    \leanok
    \uses{thm:mpdo_sal_zcl_bnt_sector_structure_counterexample,
        thm:mpdo_neighboring_trace_factorization_presentation_invariant,
        thm:mpdo_neighboring_trace_factorization_isometric_embedding_descent}
    Let $\mu\neq0$ and let $B$ be a five-letter tensor such that $\mu B$ is
    virtually gauge equivalent to the embedded four-letter obstruction. Then
    the absorbed tensor $\mu B$ admits no physical-sector factorization with
    positive neighboring operators $\eta_{k,h}$ and real families
    $(a_k)_k,(b_h)_h$ satisfying
    \begin{align}
        \tr(\eta_{k,h}) & =a_kb_h, \notag \\
        \sum_k a_kb_k   & =1. \notag
    \end{align}
    The negation is universal over all physical-sector factorizations of
    $\mu B$.
\end{theorem}
...
\begin{proof}\leanok
    \uses{thm:mpdo_sal_zcl_bnt_sector_structure_counterexample,
        thm:mpdo_neighboring_trace_factorization_presentation_invariant,
        thm:mpdo_neighboring_trace_factorization_isometric_embedding_descent}
    Suppose such a factorization exists. Virtual gauge invariance gives the
    same neighboring-trace factorization for the embedded four-letter
    obstruction. The embedding is an isometry, and the original obstruction
    is injective with full one-site physical support. Isometric physical
    descent therefore gives a normalized rank-one neighboring-trace
    factorization of the original four-letter tensor. This contradicts
    Theorem~\ref{thm:mpdo_sal_zcl_bnt_sector_structure_counterexample}.
\end{proof}
...
\begin{theorem}[The printed absorbed-representative structure assertion is false]
    ...
    matrices simultaneously span $M_2(\mathbb C)\times\mathbb C$. The first
    representative is one-site injective, and the embedded four-letter
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Its two one-site sector supports are complementary. Nevertheless, the
    first absorbed representative $\mathcal K_0=\mu A_0$ has no
    physical-sector factorization with positive neighboring operators and
    real families $(a_k)_k,(b_h)_h$ satisfying
    \begin{align}
        \tr(\eta_{k,h}) & =a_kb_h, \notag \\
        \sum_k a_kb_k   & =1. \notag
    \end{align}
    Hence Proposition~C.12 and the representativewise assertion used in
    implication (ii)$\Rightarrow$(iv) of
    \cite[Theorem~4.9 and Appendix~C.2, lines~1628--1782]
    {Cirac2016MPDO_arXiv} are false under the stated hypotheses. This
    false-source result is documented in \cite{gap:cpgsv17_pf_rank_one}.
\end{theorem}
