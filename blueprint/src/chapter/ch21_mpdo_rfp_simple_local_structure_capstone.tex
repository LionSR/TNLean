...
\begin{theorem}[Printed simple-MPDO implication chain]
    ...
    After that normalization, the remaining missing statement is the
    projector-controlled assembly of the Proposition~C.7 channel pairs.
    Proposition~C.7 starts from one physical-sector factorization of the whole
    tensor. Separate factorizations of its BNT elements do not by themselves
    provide the projector-controlled combination required here.
\end{theorem}
...
% normality under all conditions used in Appendix C.2. Hence implication
% (ii)=>(iv) remains a missing statement; the printed inference has proof-path
% drift. Independently,
...
\begin{theorem}[BNT sector structure from SAL and ZCL (all-sector Case-II boundary)]
    ...
    $s$. Theorem~\ref{thm:mpdo_caseii_absorption_not_normal} gives an MPDO of
    positive normalization with SAL, literal ambient physical-trace
    idempotence and normalized fixed-representative simple canonical form; its
    representatives are in biCF. Its global weights are $(1/\sqrt2,1)$, and
    the first absorbed transfer radius is $1/2$. Therefore the printed assertion that absorption
    preserves normality is false. The claimed
    all-sector conclusion would instead require an alternative factorization
    argument that does not pass through absorbed-sector normality. This is a
    missing statement in implication (ii)$\Rightarrow$(iv), with proof-path
    drift in the printed argument. It is distinct from the literal implication
    (iv)$\Rightarrow$(v), which is refuted by
    Theorem~\ref{thm:mpdo_theorem49_iv_to_v_counterexample} because condition
    (iv) does not constrain unequal raw repeated-copy weights.
\end{theorem}
