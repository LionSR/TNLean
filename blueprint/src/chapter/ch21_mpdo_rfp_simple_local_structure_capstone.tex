...
\begin{theorem}[Printed simple-MPDO implication chain]
    ...
    In Case II, the implication from SAL and zero correlation length to the
    blockwise local structure requires the normal Case-I factorization for each
    absorbed BNT representative.  Absorbing a coefficient $\mu_s$ multiplies
    the transfer spectral radius by $|\mu_s|^2$, while the source normalization
    does not imply $|\mu_s|=1$ for every sector.  Thus a separate argument is
    required to establish normality of every absorbed sector.
\end{theorem}

% CPSV16 Examples 4.10--4.12, lines 897--942.
% Tracking: issue #5919.
% The SAL assertion in Example 4.11 and the RFP maps in Example 4.12 require
% independent verification from the displayed source data.
\begin{theorem}[Printed mixed-state separation examples]
    ...
    without saturation of the area law.  Example~4.11 uses the bond-weight
    matrix
    \begin{align}
        W&=\begin{pmatrix}1&1/2\\1/2&1\end{pmatrix}
        \notag
    \end{align}
    and claims saturation without zero correlation length.  This saturation
    claim requires an independent entropy calculation.
    ...
    a renormalization fixed point.  The displayed density formula alone does not
    construct the trace-preserving completely positive maps required for the
    last assertion; those maps must be exhibited separately.
\end{theorem}
