...
\begin{theorem}[Exact length-four effective spectra]
    ...
    \lean{MPOTensor.CPSVExample411Spectrum.effectiveReducedBlockState_four_eq_diagonal,
        MPOTensor.CPSVExample411Spectrum.transitionCounts_one,
        MPOTensor.CPSVExample411Spectrum.transitionCounts_two,
        MPOTensor.CPSVExample411Spectrum.transitionCounts_three,
        MPOTensor.CPSVExample411Spectrum.transitionCounts_four,
    ...
    \leanok
    \uses{thm:cpsv_example411_binary_block_probability}
    On the periodic ring of length four, the effective-binary reduced states
    have the following characteristic-polynomial root multisets:
    \begin{align}
...
\begin{proof}
    ...
    The reduced-state formula is diagonal.  Enumerating binary words of lengths
    one through four gives transition-count multiplicities
    $(2)$, $(2,2)$, $(2,4,2)$, and $(2,6,6,2)$, respectively.  Substitution in
    the exact diagonal formula gives the displayed roots and multiplicities.
\end{proof}
...
\begin{theorem}[Exact length-four ambient spectra]
    ...
    For $1\leq L\leq4$, isometric inclusion multiplies the effective
    characteristic polynomial by $X^{4^L-2^L}$.  Thus the full ambient root
    multiset is the effective multiset together with zero of multiplicity
    $2$, $12$, $56$, and $240$ for $L=1$, $2$, $3$, and $4$, respectively.
\end{theorem}
...
\begin{proof}
    ...
    \uses{thm:mpdo_sal_physical_isometry_transport,
        thm:cpsv_example411_length_four_effective_spectra}
    The reduced ambient state is the isometric congruence
    $V^{\otimes L}\rho_L(V^{\otimes L})^*$.  Rectangular characteristic-polynomial
    cyclicity and $(V^{\otimes L})^*V^{\otimes L}=I$ give the power of $X$ and
    preserve all effective roots.  Since the ambient and effective dimensions
    are $4^L$ and $2^L$, their difference gives the stated zero multiplicity.
\end{proof}
