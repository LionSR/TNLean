...
\begin{theorem}[Neighboring positivity and trace factorization on the canonical
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The identities being preserved are those of CPSV16 Theorem~4.9(iv),
    equations \texttt{tralktrrk} and \texttt{PsiPhi}, lines~864--889
    \cite{Cirac2016MPDO_arXiv}.  Their transport through the active physical
    support compression is an auxiliary construction, not an assertion of
    that source theorem; see
...
\begin{remark}[Printed simple-MPDO implication chain]
    ...
    implication (iv)$\Rightarrow$(v). Proposition~C.15 remains conditional:
    the representative-channel and outer-sector constructions are proved for
    supplied absorbed-sector factorizations, but the printed route obtains
    those factorizations from the false representativewise assertion. No
    counterexample to different blocking channels is claimed.
\end{remark}
...
\begin{proof}\leanok
    ...
    one-site periodic operator, so positivity follows from the strong area
    law. The physical-trace transfer is a nonzero scalar. Literal idempotence
    therefore makes it equal to one, and the neighboring trace is normalized
    by $a_0=b_0=1$.
\end{proof}
...
% Appendix C.2, lines 1628--1782. The finite simultaneous word span and the
% local orthogonal-sum entropy calculation are project-derived bridges.
\begin{theorem}[Ambient neighboring-trace counterexample properties]
    ...
    \lean{MPOTensor.NeighboringTraceObstructionAmbientCounterexample.%
        exists_ambient_standing_simpleBiCF_package}
    \leanok
    ...
    dimensions two and one, and each occurs once. Their coefficients are
    $\mu$ and $1$, where
    $0<|\mu|<1$. In particular, every coefficient has modulus at most one and
    the second coefficient is the single global unit-modulus coefficient. The
    embedded four-letter
    obstruction is virtually gauge equivalent to $\mu A_0$.

    The representatives form a BNT presentation in biCF. The tensor $K$ is the
    exact weighted direct sum $\mu A_0\oplus A_1$ with the identity global
    virtual gauge. It is simple, generates positive semidefinite periodic
    operators with strictly positive trace at every positive length, satisfies
    the strong area law, and has literal zero correlation length,
    \begin{align}
        \mathcal T_K^2 & =\mathcal T_K.
        \notag
    \end{align}
...
\begin{proof}\leanok
    \uses{thm:mpdo_neighboring_trace_obstruction_prepared_two_block_bnt,
        thm:sharp_bnt_block_separation, rem:word_tuple_span_top,
        thm:mpdo_local_orthogonal_sum_sal}
    Use the prepared obstruction and terminal blocks. Their normalized BNT
    form has a finite simultaneous word span by
    Theorem~\ref{thm:sharp_bnt_block_separation}; the separation property in
    Remark~\ref{rem:word_tuple_span_top} gives biCF. Both representatives are
    normal, and the coefficients give the stated global normalization.
    ...
    the asserted simple canonical presentation. Positivity and literal zero
    correlation length imply strict positivity of every nonempty periodic
    trace.
\end{proof}
...
% biCF, simple-canonical, MPDO, SAL, literal-ZCL, and
...
\begin{theorem}[The printed absorbed-representative structure assertion is false]
    ...
    \uses{thm:mpdo_neighboring_trace_obstruction_ambient_properties,
        ...
        def:mpdo_neighboring_trace_factorization}
    There is a five-letter MPO tensor $K$ of virtual bond dimension three
    with two normal left-canonical representatives $A_0,A_1$ forming a BNT
    presentation in biCF. The embedded four-letter obstruction is virtually
    gauge equivalent to $\mu A_0$. Each
    representative occurs once, with coefficients $\mu$ and $1$, where
    $0<|\mu|<1$; hence $1$ is the single global unit-modulus coefficient. The
    tensor has simple canonical form, generates positive semidefinite periodic
    operators with strictly positive trace at every positive length, satisfies
    ...
    {Cirac2016MPDO_arXiv} are false under the stated hypotheses.
\end{theorem}
