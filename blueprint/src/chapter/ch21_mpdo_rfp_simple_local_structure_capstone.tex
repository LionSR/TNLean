...
\begin{theorem}[Printed mixed-state separation examples]
    ...
    correlation length.  Its normalized four-site state has nonzero spectrum
    $(41/128)^{[1]},(15/128)^{[4]},(9/128)^{[3]}$, agreeing with the corrected
    bond-pattern calculation.  The analogous identifications for the one-,
    two-, and three-site reductions remain open, so the exact inequality
    $I_2>I_1$ for the weight families does not yet prove that this tensor fails
    saturation.  We make no claim about the printed universal statement for
...
\begin{theorem}[Four-site spectrum of the corrected tensor]
    \label{thm:cpsv_example410_four_site_spectrum}
    \lean{MPOTensor.CPSVExample410Spectrum.charpoly_roots_four}
    \leanok
    \uses{def:cpsv_example410_corrected_tensor}
    The normalized four-site reduced state has characteristic-polynomial
    roots
    \begin{align}
        0^{[248]},\quad
        (41/128)^{[1]},\quad
...
\begin{proof}
    ...
    The sixteen bond patterns define orthonormal Bell-network vectors.
    Exactly eight have even parity and may have nonzero weight; the eight
    odd-parity patterns have zero weight.  The normalized state is the
    isometric image of the diagonal bond-pattern distribution.  Thus its
    nonzero roots are precisely the nonzero bond weights, and the remaining
    $4^4-2^4=240$ ambient directions together with the eight zero-weight
    patterns give $248$ zero roots.
\end{proof}

% Project-derived weight layer for CPSV16 Example 4.10, lines 897--905.
% Scope restriction: the weight families are identified with the corrected
% tensor only for the full four-site state; shorter reductions remain open.
