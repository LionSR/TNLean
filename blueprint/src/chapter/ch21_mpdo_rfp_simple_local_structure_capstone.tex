...
% This node records the algebraic assertions tracked in #6082. The MPDO, SAL,
% and complete standing-hypothesis surface remain the separate task #6083.
...
\begin{theorem}[Printed simple-MPDO implication chain]
    ...
    hypotheses. Thus implication (ii)$\Rightarrow$(iv) remains a missing
    statement and the printed normality argument has proof-path drift.
    ...
    all clauses of condition~(iv), but fails condition~(v). Hence the literal implication
    (iv)$\Rightarrow$(v) is refuted and is an unfaithful statement. The viable
...
% normality under the source's global weight convention. Hence implication
% (ii)=>(iv) remains a missing statement with proof-path drift. Independently,
