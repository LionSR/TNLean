...
\begin{theorem}[BNT sector structure from SAL and ZCL (all-sector Case-II boundary)]
    ...
    $s$. Indeed, the two-sector bond-one construction recorded in
    \lean{MPOTensor.CaseIIAbsorptionCounterexample.printed_absorbed_normality_step_is_false}
    has global weights $(1/\sqrt2,1)$, literal ambient physical-trace
    idempotence, and a first absorbed transfer radius equal to $1/2$.
    Therefore the printed assertion that absorption preserves normality is
    false. This does not refute the theorem's final implication: the exact
    high-level packaging of the remaining standing hypotheses and an
    alternative all-sector argument are still absent. Hence the all-sector
    factorization remains not ready.
\end{theorem}
