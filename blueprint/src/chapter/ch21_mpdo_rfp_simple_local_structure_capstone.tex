...
% Tracking: issues #5919 and #5984.
% Entropy corrections: docs/paper-gaps/cpsv16_examples_4_10_4_11_entropy.tex.
% The RFP maps in Example 4.12 require independent verification.
\begin{theorem}[Printed mixed-state separation examples]
    ...
    reproduces all four printed entropies.  At $p=1/4$, the exact reduced
    spectra prove $I_2>I_1$.  The printed universal exception list also omits
    $p=1$, where equality returns.
    ...
    four-site periodic state, exact reduced spectra instead give $I_2>I_1$.
    Global normalization does not change this conclusion.  An open or infinite
    stationary Markov-chain interpretation may explain the source claim, but
    the source does not state that interpretation.
