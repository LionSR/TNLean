...
\begin{theorem}[Fixed-label recursive fusion identity]%
    ...
    \[
      ...
      =Q_N\bigl((M_\gamma^{i,j})_\gamma\bigr).
    \]
...
\begin{proof}\leanok\uses{thm:mpdo_bnt_length_independent_zipper}
    ...
    Summing the letterwise equality over equal physical indices gives the
    terminal-trace equality.  Under length independence every accumulated
    coefficient $w(h)$ is one, so self-adjointness and idempotence hold block
    by block.
\end{proof}
...
\begin{definition}[All-label recursive density operator]%
    ...
    $L$ appended labels after the initial site.
    \[
...
\begin{proof}\leanok
    ...
    matrices form the fixed-label product tensor.  Apply the recursive fusion
...
\begin{definition}[Terminal spectral refinement data]%
    ...
    space.  Its spectral indices are pairs
    \[
      s=(\gamma,k),\qquad
      0\leq k<D_\gamma.
    \]
...
\begin{theorem}[Terminal spectral projectors for a length-independent RFP
    ...
    \[
      (P_s^{(N)})^\dagger=P_s^{(N)},\qquad
      (P_s^{(N)})^2=P_s^{(N)},\qquad
      P_s^{(N)}P_t^{(N)}=0\quad(s\neq t),
    \]
    and
    \[
      T_N=\sum_s\lambda_s P_s^{(N)},\qquad
      A_NP_s^{(N)}=P_s^{(N)}A_N.
    \]
    Consequently,
    \[
      R_N=\sum_s\lambda_s A_NP_s^{(N)}.
    \]
...
\begin{proof}\leanok
    ...
    spectral theorem to obtain the nonnegative eigenvalues and rank-one
    ...
    projector identities.  Linearity gives the weighted reconstruction of $T_N$.
    Finally, $A_N$ is scalar on every copy-configuration block, hence it
    commutes with each $P_s^{(N)}$ and gives the displayed decomposition of
    $R_N$.
\end{proof}
...
\begin{definition}[Commuting topological Gibbs data above one site]%
    ...
    \]
    on two adjacent sites.  For a chain of length $L=N+2$, let
    \[
      H_L=\sum_{i=1}^{L}h_{i,i+1},
    \]
...
\begin{theorem}[Commuting Gibbs decomposition above one site]%
    ...
    \[
      [P_i^{(L)},e^{-H_L}]=0,
      \qquad
      R_L=\sum_{i=1}^{d}\lambda_iP_i^{(L)}e^{-H_L}.
    \]
...
\begin{proof}\leanok
    ...
    that sum with the exponential identity and the exact adjoint
...
\begin{theorem}[Unitarity of each fixed-final comparison corner]%
    ...
    \begin{align}
        ...
        \label{eq:mpdo_final_sector_comparison_right_unitary}\\
        C_\varepsilon^\dagger C_\varepsilon&=1.
        \label{eq:mpdo_final_sector_comparison_left_unitary}
    \end{align}
...
\begin{proof}\leanok
    ...
    \eqref{eq:mpdo_final_sector_comparison_right_unitary} and
    \eqref{eq:mpdo_final_sector_comparison_left_unitary}:
    \[
