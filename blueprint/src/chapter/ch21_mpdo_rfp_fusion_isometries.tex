...
\begin{theorem}[Fixed-label recursive fusion identity]%
    ...
    sequential fusion map $\widetilde U_N$ is a coisometry.  Letter by letter,
    it carries the left-associated product tensor to the recursive direct sum:
    \[
      \widetilde U_N M_{\alpha,\boldsymbol\beta}^{i,j}
    ...
    \textbf{Scope restriction (fixed initial label):} The theorem proves the
    recursive fusion and terminal-trace identities for each initial BNT label.
    It does not yet assemble the vertical-canonical coefficient over all
    initial labels, identify the resulting matrix with the physical MPDO, or
    prove the source commutator.  These remaining steps are recorded in
    \path{docs/paper-gaps/cpsv16_topological_projector_recursion.tex}.
\end{theorem}
\begin{proof}\leanok
