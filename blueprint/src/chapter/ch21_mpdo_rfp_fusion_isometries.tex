...
\begin{proof}
    ...
    \uses{thm:mpdo_fusion_same_length_product,
        def:mpdo_positive_bnt_label_chi_trace_power_form}
    The positive chi witness gives, for $L>0$,
    \[
        c^{(L)}_{\alpha,\beta,\gamma}
        =\operatorname{tr}(\chi_{\alpha,\beta,\gamma}^{L}),
    \]
    and positivity of every diagonal entry. The preceding theorem gives
    \[
        O_L(M_\alpha)O_L(M_\beta)
        =\sum_\gamma c^{(L)}_{\alpha,\beta,\gamma}O_L(M_\gamma),
    \]
    while the remaining hypothesis is
    \[
        m_\gamma=\sum_{\alpha,\beta}
        c^{(1)}_{\alpha,\beta,\gamma}m_\alpha m_\beta.
    \]
    These are precisely the three fields of the BNT algebra clause.
\end{proof}
