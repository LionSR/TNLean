...
\]
These are the $(e,\mu,\nu)$ and $(f,\lambda,\sigma)$ index spaces in the
equation (Fmove) of \cite{Bultinck2017Anyons}.

\begin{definition}[Triple-fusion maps with fixed final label]%
    \label{def:mpdo_final_sector_fusion_isometries}
    ...
    \lean{MPOTensor.BNTFusionIsometryFamily.leftFinalFusionIsometry}
    \lean{MPOTensor.BNTFusionIsometryFamily.rightFinalFusionIsometry}
    \leanok
    \uses{def:mpdo_left_fusion_isometry,
...
\begin{theorem}[Left triple fusion at a fixed final label]%
    ...
    \leanok
    \uses{def:mpdo_final_sector_fusion_isometries,
