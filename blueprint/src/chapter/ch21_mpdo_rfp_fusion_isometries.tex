...
Fix a final label $\varepsilon$.  The multiplicity indices of the two
bracketings are
\[
...
diagonal matrices with the fusion multiplicities of that equation requires
the length-independent integer specialization and is not asserted here.
...
\begin{definition}[Triple-fusion maps with fixed final label]%
    ...
    \lean{MPOTensor.BNTFusionIsometryFamily.LeftFinalIndex}
    \lean{MPOTensor.BNTFusionIsometryFamily.RightFinalIndex}
    \lean{MPOTensor.BNTFusionIsometryFamily.leftFinalRow}
    \lean{MPOTensor.BNTFusionIsometryFamily.rightFinalRow}
    ...
    $U^{\mathrm R}_{\alpha,\beta,\gamma;\varepsilon}$ are the restrictions
    of the two iterated fusion isometries to the summands with final label
    $\varepsilon$.  Their codomains are respectively
    $\mathcal H^{\mathrm L}_{\varepsilon}\otimes\C^{D_\varepsilon}$ and
    $\mathcal H^{\mathrm R}_{\varepsilon}\otimes\C^{D_\varepsilon}$.
    Their adjoints, after the canonical reassociation of the triple bond
    space, have the orientation and bracketing pattern of the weighted
    analogues of the two composites in equation (Fmove) of
