...
\begin{definition}[BNT algebra clause]%
    ...
    Fix BNT-label operators $O_L(M_\alpha)$, trace scalars
    $m_\alpha=\operatorname{tr}(\mu_\alpha)$, and diagonal matrices
    $\chi_{\alpha,\beta,\gamma}$. They satisfy the \emph{BNT algebra clause}
    when the diagonal entries are positive, the coefficients obey
    \[
        c^{(L)}_{\alpha,\beta,\gamma}
        =\operatorname{tr}(\chi_{\alpha,\beta,\gamma}^{L}),
    \]
    the same-length product law holds at every positive length,
    \[
    ...
    \cite[Theorem~4.14(ii), lines~972--985]{Cirac2016MPDO_arXiv}.
\end{definition}
...
\begin{theorem}[Fusion isometries imply the BNT algebra clause]%
    \label{thm:mpdo_bnt_fusion_implies_algebra_clause}
    \lean{MPOTensor.BNTFusionIsometryFamily.to_bntAlgebraClause}
    \leanok
    ...
    Then the concrete operators $O_L(M_\alpha)$ satisfy the BNT algebra clause
    with the same diagonal matrices and trace scalars. This is implication
    (iii)$\Rightarrow$(ii) of
...
\begin{proof}
    \leanok
    \uses{thm:mpdo_fusion_same_length_product,
        lem:mpdo_bnt_label_of_chi_positive_length_trace_power}
    Positivity is part of the fusion-isometry hypothesis. The trace-power
    identity follows from the canonical coefficients determined by $\chi$,
    the same-length product law is the preceding theorem, and the idempotent
    scalar law is the remaining hypothesis.
\end{proof}
