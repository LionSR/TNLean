...
\begin{proof}\leanok
    ...
    If $E_1 = S_1 \circ T_1$ and $T_1 \circ S_1 = \id$, then
...
\begin{proof}\leanok
    ...
    \[
        \tr\bigl(U_{\alpha,\beta} W
        U_{\alpha,\beta}^\dagger\bigr)
        = \tr\bigl(W U_{\alpha,\beta}^\dagger
        U_{\alpha,\beta}\bigr)
        = \tr(W).
    \]
    ...
    coefficients $\tr(\chi_{\alpha,\beta,\gamma}^{L})$.
\end{proof}
...
\begin{theorem}[Fixed-label recursive fusion identity]%
    ...
    \[
      ...
      =Q_N\bigl((\tr(M_\gamma))_\gamma\bigr).
    \]
...
\begin{definition}[All-label recursive density operator]%
    ...
    and set
    \[
      Q_{N,c}=Q_N\bigl((\tr(M_\gamma))_\gamma\bigr).
    \]
