...
\begin{proof}
    ...
    Thus $A_1^i = \lambda B_1^i$, and substituting into the first product
    identity~(\ref{eq:aft_product_identities_a}) forces $A_2^j = \lambda^{-1} B_2^j$.
\end{proof}
...
\begin{theorem}[Injective-chain Fundamental Theorem up to a global phase]%
    ...
    \begin{equation}\label{eq:aft_chain_gaugePhase_conj}
...
\begin{proof}
    ...
    gives the conclusion~(\ref{eq:aft_chain_gaugePhase_conj}).
\end{proof}
...
\begin{corollary}[Translation-invariant collapse to one gauge]\label{cor:ft_ti_chain}
    ...
    \begin{equation}\label{eq:aft_ti_collapse_conj}
...
\begin{proof}
    ...
    Then $\lambda^n = 1$ and the identity~(\ref{eq:aft_ti_collapse_conj}) is
    exactly the same gauge relation rewritten in the form used by the corollary.
\end{proof}
...
\begin{lemma}[Explicit direct-sum conjugation by the flattened gauge]%
    ...
    \begin{equation}\label{eq:aft_blockGauge_conj}
...
\begin{proof}\leanok
    ...
    identity~(\ref{eq:aft_blockGauge_conj}).
\end{proof}
...
\begin{lemma}[Gauge equivalence of direct sums from per-block MPV equality]%
    ...
    \begin{equation}\label{eq:aft_multiBlock_global_conj}
        \bigoplus_k \mu_k B_k^i
        = X \left(\bigoplus_k \mu_k A_k^i\right) X^{-1}.
    \end{equation}
\end{lemma}
...
\begin{proof}\leanok\uses{lem:multi_block_ft, lem:multi_block_global}
    ...
    With $X=\bigoplus_k X_k$, the identity~(\ref{eq:aft_multiBlock_global_conj})
