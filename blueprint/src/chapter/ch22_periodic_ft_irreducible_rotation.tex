...
\begin{theorem}[The period of an irreducible-form block]
    ...
    radius one (\cite[lines~248--261]{DeLasCuevas2017Irreducible}). Then
    there exists a positive integer $m$ such that $A$ is spectrally $m$-periodic,
    so that the peripheral eigenvalues of $\E_A$ are precisely the
    $m$-th roots of unity (\cite[lines~257--258]{DeLasCuevas2017Irreducible}).
\end{theorem}
\begin{proof}
...
\begin{definition}[Left-canonical periodic tensor]
    ...
    primitive transfer map (the corresponding definition in the companion quantum-channel volume~\cite[``Primitive map'']{QICLean2026Blueprint}).
\end{definition}
...
\begin{theorem}[Similarity of sector-decomposed tensors]
...
\end{theorem}
\begin{proof}
    \uses{def:ft_matched_flat_data, lem:tensor_from_blocks_globalGauge_conj}
    \leanok
...
\begin{theorem}[Sectorwise Perron normalization]
    ...
    \leanok
    \uses{def:spectral_periodic_tensor, def:periodic_sector_basis_replacement, def:periodic_tensor}
...
\end{theorem}
\begin{proof}
    \uses{thm:spectral_periodic_normalization, thm:periodic_sector_basis_replacement_gauge}
    \leanok
    Apply the positive Perron gauge to each representative and
    combine them via
...
\begin{remark}[Irreducible form allows higher periods]
    ...
    \notready
    Irreducible forms permit periodic blocks with period $m > 1$; for
    example, the antiferromagnetic state has period~$2$.
\end{remark}

Physical-index rotations act on periodic tensors and their transfer maps as
follows.
...
\begin{theorem}[Periodicity preserved by physical rotation]%
    ...
    \leanok
    \uses{def:rotate_physical, thm:transfer_map_rotate_physical, thm:left_canonical_rotate_physical, thm:irreducible_tensor_rotate_physical}
    If $uu^\dagger=\Id$ and $A$ is periodic with period~$m$, then
    $A_u$ is periodic with the same period.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:transfer_map_rotate_physical, thm:left_canonical_rotate_physical, thm:irreducible_tensor_rotate_physical}
    This combines transfer-map invariance
    (Theorem~\ref{thm:transfer_map_rotate_physical}), left-canonical preservation
    (Theorem~\ref{thm:left_canonical_rotate_physical}), and irreducibility
    (Theorem~\ref{thm:irreducible_tensor_rotate_physical}).
\end{proof}
...
\begin{theorem}[Irreducible form preserved by physical rotation]%
    ...
    \leanok
    \uses{def:rotate_physical, def:irreducible_form, thm:periodic_rotate_physical, thm:same_mpv_rotate_physical, thm:rotate_physical_blocks}
...
\begin{proof}\leanok
    \uses{thm:periodic_rotate_physical, thm:same_mpv_rotate_physical, thm:rotate_physical_blocks}
...
\begin{corollary}[Periodic symmetry yields finite-order $Z$-gauge equivalence]%
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Hence $A$ and $B$ are related by a $\mathbb Z_m$ virtual gauge.
\end{corollary}
