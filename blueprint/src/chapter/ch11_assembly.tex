...
\begin{proof}
	...
	At the block level this is the matching step
	\[
		\TNBlockPermutation{\pi}{X}
	\]
	that reindexes the separated sectors before comparing them one by one.
...
\begin{proof}
	...
	resulting global conjugacy is assembled as
	\[
		\TNDirectSumAssembly{\bigoplus_j \mu_j^A A_j}{\bigoplus_j \mu_{\pi(j)}^B B_{\pi(j)}}{\bigoplus_j X_j}
	\]
	so the
...
\begin{proof}
	...
	The common-structure case is therefore already in block-diagonal form:
	\[
...
\subsection{Multiplicity data with repeated blocks}%
\label{sec:route_b}
...
\label{thm:route_b_equal}
	...
	\end{enumerate}
	At the level of block contributions, the comparison is still the same
	weighted sector sum:
	\[
		\TNSummedBlocks{\mu}{N}
	\]
	with the coefficients now aggregated over the multiplicity layers.
\end{theorem}
...
\begin{theorem}[Periodic Fundamental Theorem]\notready
	...
	\]
	rather than by an ordinary aperiodic gauge alone.
...
\begin{remark}[Comparison with the 1606.00608 approach]\notready
	...
	approach would require the irreducible-form and $Z$-gauge infrastructure
