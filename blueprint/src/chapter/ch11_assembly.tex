...
\begin{proof}
	...
	Apply the bundled common-cover sector theorem to those data.
\end{proof}
...
\label{thm:after_blocking_sector_common_blocks_proportional_decomposition_zero_tail}
	...
	then the zero-tail-aware sector comparison conclusion holds.
\end{theorem}
...
\begin{proof}
	...
	\uses{lem:sameMPV2_live_of_zeroTail_eq,
		...
		thm:sector_basis_overlap_span_to_matching,
		thm:route_b_hetero_sector_matching}
	Zero-tail cancellation gives equality of the two live tensors, including at
	length zero. The proportional-decomposition comparison gives the primitive
	overlap-span hypotheses through the common phase-cover construction. The sector
	basis matching and heterogeneous sector comparison theorems then give the
	sector-weight conclusion.
\end{proof}
