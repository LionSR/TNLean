...
\label{thm:sector_basis_overlap_ortho_to_span}
	...
	and in addition the basis blocks of both sector decompositions are one-site
	injective, and the finite-length MPV spans of the two bases are equal at every
	system size, then the two bases satisfy the primitive overlap-span hypotheses.
\end{theorem}
...
\label{thm:after_blocking_sector_common_blocks_injective_span}
	...
	addition that the live blocks are one-site injective, and that any two sector
	decompositions that each represent the respective live block tensors, satisfy
	the BNT linear-independence condition, and carry the single-family
	overlap-orthogonality data have equal finite-length MPV spans. Then the same
	matched sector-weight conclusion holds.
\end{theorem}
