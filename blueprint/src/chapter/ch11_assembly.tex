...
\begin{remark}[Current formalized endpoint versus the literature]
	Theorem~\ref{thm:ft_equal_explicit} is the current Lean-backed
	equal-case upgrade of Theorem~\ref{thm:ft_proportional}.
	It still depends on explicit coefficient arrays with nonzero limits,
	and therefore does not yet recover Theorem~\ref{thm:ft_equal} or the
	full \cite[Corollary~IV.5]{Cirac2021Matrix}.
	Theorem~\ref{thm:ft_equal_same_structure} is a different formalized
	special case with a fixed common block structure.
	The general literature statement requires additional multiplicity
	bookkeeping and a power-sum step, as indicated in the proof sketch of
	Theorem~\ref{thm:ft_equal}.
\end{remark}
