...
\begin{proof}
    ...
    Our formalization uses the Fitting decomposition
    (generalized eigenspaces) as a substitute,
...
\begin{proof}
    ...
    hence $T_{L_0} = \MN{D}$).
    The proof follows \cite[Lemma~1]{Sanz2010Quantum}
    and \cite[Lemma~6.2]{Wolf2012Quantum}.
    Those references obtain the sharper bound $D^2 - d + 1$
    by starting the dimension count at $\dim S_1 = d$;
    our formalization uses the coarser bound $D^2$.
\end{proof}
...
\begin{proof}
    ...
    In \cite{Sanz2010Quantum} and \cite{Wolf2012Quantum},
    this step uses the Jordan normal form directly;
    our formalization uses the Fitting decomposition instead,
    as the full Jordan normal form is not available in Mathlib.
    This suffices because we only need the existence of an
    eigenvector, not the full Jordan block structure.
\end{proof}
