...
\begin{theorem}[Positive-length propagation of block injectivity]
    ...
    $A$ is $m$-block injective for every $m\geq L$.
...
\begin{proof}\leanok\uses{def:word_span}
    ...
    \[
        ...
        =S_{L+1}(A).
    \]
    Iterating this implication gives $S_m(A)=\MN{D}$ for every $m\geq L$.
\end{proof}
...
\begin{lemma}[Two-sided span from one nonzero matrix]
    ...
    \begin{equation}\label{eq:wld_two_sided_nonzero_span}
...
\begin{proof}\leanok
    ...
    is a scalar multiple of $E_{ip} C E_{qj}$, so the span in
...
\begin{theorem}[Sharp nonzero trace product --- positive length]%
    ...
    If $A$ is normal and $D \ge 1$, then there exists a word~$w$
...
\begin{corollary}[Wielandt case (2), index bound]%
    ...
    \[
        \iota(A) \le D^2 - \krausrk(A) + 1,
    \]
    where $\iota(A)=\min\{n\geq 1 : S_n(A)=\MN{D}\}$.
\end{corollary}

\begin{proof}
    \leanok
    \uses{thm:wielandt_case2_one_step_span}
    The word-span equality~(\ref{eq:wld_case2_one_step_wordspan_top}) gives an admissible value in the
...
\begin{theorem}[Wielandt's inequality --- general bound]%
    ...
    \]
    where $\iota(A) = \min\{n \geq 1 : S_n(A) = \MN{D}\}$ is the
...
\begin{proof}
    ...
    $d' = \krausrk(A)$.  The matrix $A^w$ has nonzero trace, hence a
    nonzero eigenvalue $\mu$ and eigenvector $\varphi$
    (Theorem~\ref{thm:eigenvector_from_trace}).  Set $n = |w|$ and let
    $B = A^{[n]}$ be the blocked tensor.  Then $B$ is normal
    (Theorem~\ref{thm:isNormal_blockTensor}), and the encoding of $w$ as a
    single block index gives $B^{i_0} = A^w$.
    \begin{enumerate}
    ...
    direct dimension growth, giving $S_{D^2 - k_B + 1}(B) = \MN{D}$ where
    $k_B = \krausrk(B)$.  In the non-invertible case, the bound
    $S_{D^2}(B) = \MN{D}$ follows from tracking indices through the rank-one
    extraction argument of \cite[Lemma~2(b)]{Sanz2010Quantum}; the separate
    qualitative fixed-length conclusion stated below does not supply this
    explicit bound.  In both cases
    $\iota(A) \le D^2 \cdot n \le D^2 \cdot (D^2 - d' + 1)$.
\end{proof}
...
\begin{proof}
    ...
    $(D^2 - \rank S_1(A) + 1)D^2$ is at most $D^4$. The passage from
...
\begin{theorem}[Exact quantum-Wielandt blocking length]
    ...
    \[
        S_{D^4}(A)=\MN{D};
    \]
    equivalently, $A$ is $D^4$-block injective.
\end{theorem}
...
\begin{proof}
    ...
    \uses{thm:bounded_injective_blocking_normal_left_canonical,
        lem:blocking_transfer,
        thm:wordspan_blk_inj_succ}
    Choose $L$ with $0<L\leq D^4$ and $S_L(A)=\MN{D}$, and write
    \[
        D^4=L+k.
    \]
...
\begin{proof}\leanok
    ...
    Then
    \begin{equation}\label{eq:wld_blocknormal_self_inclusion}
        M = M \cdot \Id = \sum_r c_r M A^{w_r}
        \in S_N(A) \cdot S_N(A) \subseteq S_{2N}(A)
    \end{equation}
    by Lemma~\ref{lem:word_span_mul}.
    Iterating the inclusion~(\ref{eq:wld_blocknormal_self_inclusion}) gives
    $S_N(A) \subseteq S_{kN}(A)$
...
\begin{definition}[Algebra span]\label{def:alg_span}
    ...
    $\C$-subalgebra of $\MN{D}$ generated by the matrices $\{A^i\}_{i=0}^{d-1}$:
    \[
        \algspn(A)
