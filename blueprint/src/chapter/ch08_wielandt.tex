...
\begin{theorem}[Primitive MPS tensors have nonzero word products]\label{thm:prim_nonzero_word}
    ...
    \leanok
    \uses{def:eval_word, def:normal}
    If $A$ is a primitive normal MPS tensor, then for every
    $n \ge 1$ there exists a word $w$ of length $\le n$
    with $A^w \neq 0$.
\end{theorem}

\begin{proof}\leanok
    Primitivity of the transfer map ensures that iterated
    powers of $\E_A$ do not vanish. Since
    $\E_A^n(\Id) = \sum_{|w|=n} A^w (A^w)^\dagger$,
    this sum is nonzero, so at least one $A^w$ is nonzero.
\end{proof}
...
\begin{theorem}[Iterated transfer does not vanish]\label{thm:transfer_pow_ne_zero}
    ...
    If $A$ is a primitive normal MPS tensor, then
    $\E_A^n \neq 0$ for all $n \ge 0$.
\end{theorem}

\begin{proof}\leanok
    For $n = 0$, $\E_A^0 = \Id \neq 0$.
    For $n \ge 1$, use Theorem~\ref{thm:prim_nonzero_word}:
    there exists a word~$w$ with $A^w \neq 0$, so
    $\E_A^n(\Id)$ has a nonzero summand.
\end{proof}