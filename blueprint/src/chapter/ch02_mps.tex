...
\section{Transfer maps and quantum-channel interfaces}\label{sec:channels}\label{ch:channels}
...
\begin{lemma}[Trace adjointness of a transfer map]
    ...
    \leanok
    \uses{def:transfer_map}
    For an MPS tensor $A$ and bond matrices $\sigma, \rho\in\MN{D}$,
    \begin{align}
        \tr(\sigma\,\E_A(\rho)) & = \tr(\E_{A^\dagger}(\sigma)\,\rho),
        \notag
    \end{align}
    where $\E_{A^\dagger}$ is the transfer map with Kraus operators $(A^i)^\dagger$.
\end{lemma}

\begin{proof}\leanok
    Apply trace adjointness for Kraus maps~\cite[``Trace adjointness of a Kraus map'']{QICLean2026Blueprint} to the Kraus family $\{A^i\}$.
\end{proof}
...
\begin{theorem}[Basic rectangular reduction identities]
    ...
    \lean{MPSTensor.IsReduction.mul_eq_one,
        ...
        MPSTensor.IsReduction.bondDim_le,
    ...
    identity alone implies $VW=1_{D_A}$ by taking the empty word.  Moreover,
    the target bond dimension cannot exceed the source bond dimension:
    $D_A\leq D_B$.
\end{theorem}
...
\begin{proof}
    ...
    instance reads $V1_{D_B}W=1_{D_A}$.  Since $V$ has a right inverse, it is
    surjective, and finite-dimensional linear algebra gives $D_A\leq D_B$.
\end{proof}

\begin{remark}\leanok
    Definition~\ref{def:same_mpv} specializes Definition~\ref{def:same_mpv2}
    to equal bond dimensions $D_1=D_2=D$.
\end{remark}
...
\begin{remark}\leanok
    ...
    Definition~\ref{def:normal} (eventual block injectivity) is equivalent to
    the spectral characterization of \cite[Definition~IV.1]{Cirac2021Matrix}
    (primitivity of the trace-preserving normalized transfer map $\E_A$); see
    \cite[Proposition~3]{Sanz2010Quantum}. In this development, the equivalence
    is established by Lemma~\ref{lem:primitive_implies_irreducible},
    Theorem~\ref{thm:normal_from_primitive_posdef},
    Theorem~\ref{thm:algspan_to_normal}, and
...
\begin{remark}\label{rem:cf_injective_vs_normal}\leanok
    The block-injective canonical form requires each block to be injective
    (Definition~\ref{def:injective}), which is stronger than the normal
    canonical form of \cite{Cirac2021Matrix} where blocks are normal in the
    spectral sense. Chapter~\ref{ch:canonical} develops the normal canonical
    form used in the multi-block theory.
\end{remark}
