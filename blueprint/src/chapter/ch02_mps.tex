...
\begin{lemma}[Rectangular factorization of a rank-one matrix]
    \label{lem:rank_one_rectangular_factorization}
    ...
    \leanok
    Let $M$ be a matrix on the product index $D_B\times D_A$.  If
    $\rank M=1$, then there are nonzero rectangular matrices
    $W\in\C^{D_B\times D_A}$ and $V\in\C^{D_A\times D_B}$ such that
    \begin{align}
        M_{(b,a),(b',a')}
         & =W_{ba}V_{a'b'}.
        \notag
...
\begin{proof}
    \leanok
    A linear map with one-dimensional range is the product of a scalar
    functional and a nonzero range vector.  In the standard bases this is an
    outer product.  Reshape its left vector as a $D_B\times D_A$ matrix and
    its right vector as a $D_A\times D_B$ matrix, preserving the displayed
    product-index order.
\end{proof}
...
\begin{theorem}[Equality-case existence of rectangular reductions]
    ...
    \cite[Proposition~20]{Molnar2018SemiInjective}.  The unrestricted
...
\begin{proof}
    ...
    \uses{lem:one_block_injective_of_injective,
        ...
        lem:rank_one_rectangular_factorization}
