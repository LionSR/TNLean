...
\begin{definition}[Residual algebra of a selected reduction]
    ...
    \leanok
    \uses{def:mps_rectangular_reduction, def:eval_word}
    ...
    This is Definition~8 of \cite{Molnar2018SemiInjective}.
\end{definition}
...
\begin{definition}[Residual nilpotency length]
    ...
    A natural number $L$ is a \emph{mixed-word nilpotency bound} for
    $\mathcal R(B,A;V,W)$ if every product of exactly $L$ elements of this
    algebra is zero.  The residual nilpotency length
    $\ell(B,A;V,W)$ is the infimum of the set of such bounds.  When a
    bound exists, this is its least value.
\end{definition}
...
\begin{theorem}[Positive residual words vanish under the reduction]
    ...
    \lean{MPSTensor.IsReduction.evalWord_mul_reductionResidual_mul_evalWord_eq_zero,
        MPSTensor.IsReduction.evalWord_reductionResidual_sandwich_eq_zero}
    \leanok
    \uses{def:mps_rectangular_reduction,
        def:mps_reduction_residual_algebra}
    Let $(V,W)$ be a reduction from $B$ to $A$.  For all words
    ...
    This is the residual-word identity preceding Proposition~21 in
...
\begin{proof}
    ...
    Expand the first residual letter as $N^i=B^i-WA^iV$.
    The two resulting terms have the same reduction after using $VW=1$.
    Induction on the positive residual word gives
    (\ref{eq:mps_reduction_residual_exterior_sandwich}); taking both exterior
    words empty gives (\ref{eq:mps_reduction_residual_sandwich}).
\end{proof}
...
\begin{definition}[Strict residual-window sums]
    ...
    \leanok
    \uses{def:mps_reduction_residual_algebra, def:eval_word}
    For a word $\mathbf i=(i_1,\ldots,i_n)$, define the strict-window sum by
    \begin{align}
        \mathcal W(\mathbf i)
    ...
    \end{align}
    The strict inequality $r<s$ means that every central $A$-word is
    nonempty.  The initial-window sum is the part with $r=0$.
\end{definition}
...
\begin{proof}
    ...
    are the pure residual word and the windows in
    (\ref{eq:mps_reduction_residual_expansion}).
\end{proof}

\begin{theorem}[Nilpotency of the selected-reduction residual algebra]
    \label{thm:mps_reduction_residual_algebra_nilpotent}
    \lean{MPSTensor.IsReduction.trace_evalWord_reductionResidual_eq_zero,
        ...
        MPSTensor.IsReduction.reductionResidualAlgebra_list_prod_eq_zero,
    ...
    element of this algebra is nilpotent.  Every mixed product of $D_B$
    algebra elements is zero.  Consequently $D_B$ is a mixed-word
    nilpotency bound, the least bound $\ell(B,A;V,W)$ is itself a bound, and
    \begin{align}
        \ell(B,A;V,W) & \leq D_B.
    ...
    equality is essential; a reduction alone does not imply nilpotency.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:mps_reduction_residual_sandwich}
    Closed-chain equality and cyclicity of trace show that each nonempty
    residual word has trace zero.  Linearity extends this identity to the
    generated nonunital algebra.  The vanishing traces of positive powers
    imply that each algebra element is nilpotent.  The finite-dimensional
    nil-algebra theorem then makes every product of $D_B$ elements vanish.
    The well-ordering of $\N$ gives the least bound and
    (\ref{eq:mps_reduction_residual_nilpotency_length_bound}).
\end{proof}

\begin{theorem}[Consequences of an arbitrary residual bound]
    \label{thm:mps_reduction_residual_bound_consequences}
    \lean{MPSTensor.IsReduction.reductionResidualAlgebra_list_prod_eq_zero_of_bound_le_length,
        MPSTensor.IsReduction.evalWord_reductionResidual_eq_zero_of_bound_le_length}
    \leanok
    \uses{def:mps_reduction_residual_nilpotency_length}
    If $L$ is a mixed-word nilpotency bound, then every product of at least
    $L$ elements of $\mathcal R(B,A;V,W)$ vanishes.  In particular,
    \begin{align}
        N^{\mathbf i} & =0
...
\begin{proof}
    \leanok
    Split a product after its first $L$ factors.  The prefix is zero by the
    bound, so the full product is zero.  Apply this to the list of residual
    letters in a word.
\end{proof}
...
\begin{theorem}[Bound-truncated contracted residual expansions]
    ...
    \uses{def:mps_rectangular_reduction,
        def:mps_reduction_residual_nilpotency_length}
    Let $(V,W)$ be a reduction from $B$ to $A$, let $L$ be any mixed-word
    nilpotency bound for its residual algebra, and write
    $\mathbf i=(i_1,\ldots,i_n)$.  Then
    \begin{align}
        VB^{i_1}\cdots B^{i_n}
...
\begin{theorem}[Exterior-buffer identity]
    ...
    mixed-word nilpotency bound for its residual algebra.  If the central word
    ...
    residual bound enter this identity.
\end{theorem}
...
\begin{proof}
    ...
    \uses{thm:mps_reduction_residual_sandwich,
        thm:mps_reduction_residual_expansion,
        thm:mps_reduction_residual_bound_consequences}
    Expand the first letter of the nonempty central word as
    $B^i=N^i+WA^iV$, and apply the corrected expansion to both exterior
    words.  The pure residual terms vanish by the two buffer inequalities.
    Every remaining unwanted term contains a positive residual word between
    $V$ and $W$, so Theorem~\ref{thm:mps_reduction_residual_sandwich} removes
    it.  The surviving term is (\ref{eq:mps_reduction_exterior_buffer}).
\end{proof}
