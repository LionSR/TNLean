...
\begin{definition}[Residual nilpotency length]
    ...
    Definition~8 of \cite{Molnar2018SemiInjective} instead takes the least
    $L$ for which every residual-letter word $N^{i_1}\cdots N^{i_L}$ is
    zero.  The two formulations are equivalent.  A mixed-product bound
    applies to the residual generators.  Conversely, each algebra element is
    a linear combination of positive residual words, so a product of $L$
    algebra elements is a linear combination of residual words of length at
    least $L$; each such word vanishes after factoring off its first $L$
    letters.
\end{definition}
...
\begin{definition}[Strict residual-window sums]
    ...
    \end{align}
    Since $VW=1$, for $\mathbf i=(i_1,\ldots,i_n)$ these recursions give
    \begin{align}
        \mathcal W(\mathbf i)
    ...
    \end{align}
    The strict inequality $r<s$ makes the central $A$-word nonempty, and
    $\mathcal I$ is the part with $r=0$.
\end{definition}
