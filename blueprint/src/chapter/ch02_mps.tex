...
\begin{proof}
    ...
    word.  A positive residual word between two selected $WA^iV$ blocks
    vanishes, while adjacent selected blocks combine by $VW=1$.
\end{proof}
...
\begin{proof}
    ...
    sandwiched residual terms vanish and $VW=1$.  For the right contraction,
    decompose the first letter and apply the reduction identity to the tail.
\end{proof}
...
\begin{theorem}[Recursive and explicit strict-window sums]
    ...
    \leanok
    \uses{def:mps_reduction_strict_window_sums,
        thm:mps_reduction_all_cut_contracted,
        thm:mps_reduction_recursive_residual_expansion}
...
\begin{theorem}[Dimension bound for the residual nilpotency length]
    ...
    a residual-word nilpotency bound.  The least bound exists, is itself a
    bound, and satisfies
    \begin{align}
        \ell(B,A;V,W) & \leq D_B.
...
\begin{proof}
    \leanok
    \uses{thm:mps_reduction_residual_bound_consequences}
