...
\begin{theorem}[Covariance of structure coefficients for vertical BNT decompositions]%
    ...
    Suppose that two vertical basis-of-normal-tensors decompositions of tensors satisfy the
...
\end{theorem}
\begin{proof}\leanok
    The algebra conditions for the two vertical basis-of-normal-tensors decompositions give the
...
\end{theorem}
\begin{proof}\leanok
    For the vertical basis-of-normal-tensors decompositions, the covariance formula gives
    \begin{align}
        {c'}^{(L)}_{\alpha,\beta,\gamma}
         & =\left(\frac{s_\alpha s_\beta}{s_\gamma}\right)^L
        c^{(L)}_{e(\alpha),e(\beta),e(\gamma)}
...
\begin{definition}[Horizontally gauge-equivalent tensors with distinct vertical normalizations]%
    ...
    basis-of-normal-tensors decompositions whose normalized representatives and positive
...
\begin{theorem}[Horizontal periodic equality alone does not determine vertical BNT coefficients]%
    ...
    Their chosen vertical basis-of-normal-tensors decompositions nevertheless have structure
    coefficients
    \begin{align}
        c_P^{(L)} & =1,
        \notag                               \\
    ...
    While Theorem~\ref{thm:mpdo_bnt_label_coeff_unique} guarantees uniqueness of
    structure coefficients for a fixed operator family, this counterexample shows
    that periodic operator equality alone does not constrain the structure coefficients
    of independent vertical decompositions across nonunitary gauge changes.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Associativity constraint in trace-power form]%
    ...
    constructed from unitary fusion categories give the family of
    renormalization fixed point MPDOs described there.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[The dimer RFP has length-dependent coefficients in its chosen vertical BNT decomposition]%
    ...
    $M^i=\bigoplus_k\mu_k M_k^i$ with normalized normal tensor blocks $M_k$, is a
    renormalization fixed point, and has a vertical basis-of-normal-tensors decomposition
    satisfying the algebra condition whose structure coefficients depend on
    the positive length $L$. One may take $M=R$ and the single-element
    basis-of-normal-tensors decomposition already constructed, for which
    \begin{align}
    ...
    Theorem~4.14 of \cite{Cirac2016MPDO_arXiv} without requiring the unit-weight
    normalization of \cite{gap:cpsv16_unit_weight_rfp_scale_tension}.
\end{theorem}
...
\begin{proof}\leanok
    ...
    Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_ismpdo} and
    Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_cf} show that $R$ generates
    MPDOs and has the direct-sum form with normalized normal tensor blocks.
    Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_is_rfp_via_ts} constructs
    the completely positive trace-preserving maps $S,T$ satisfying
    $S[R_2(X)]=R_1(X)$ and $T[R_1(X)]=R_2(X)$ for all $X$, while
    Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_bnt_algebra_clause}
    provides its single-element vertical basis-of-normal-tensors decomposition
