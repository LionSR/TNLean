...
\begin{theorem}[The dimer RFP has length-dependent coefficients in its chosen vertical BNT decomposition]%
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This gives an example for the variant of the question after
    Theorem~4.14 in which the additional normalization of the canonical
    weights is not required, for this chosen vertical decomposition.
    It does not assert that length dependence is invariant under a change
    of decomposition. Although the blocks are normalized NT, the statement
    does not supply the source's additional normalization of the canonical
    weights: every weight has modulus at most one and at least one has
    modulus one.
