\section{Physical blocking and transfer idempotence}

\begin{definition}[Physical blocking isometry]
    \label{def:physical_blocking_isometry}
    \lean{MPSTensor.HasPhysicalBlockingIsometry}
    \leanok
    An MPS tensor $A$ is a \emph{renormalization fixed point} if there exists an
    isometry $V\colon \C^d\to\C^{d^2}$ with coefficients $V_{(i_1,i_2),j}$
    satisfying $V^\dagger V=\Id$ such that for all physical indices
    $i_1,i_2\in\{0,\ldots,d-1\}$,
    \begin{align}
        A^{i_1}A^{i_2}
         & =\sum_{j=0}^{d-1}V_{(i_1,i_2),j}A^j.
        \label{eq:corr_physical_blocking_isometry}
    \end{align}
    This expresses the physical two-site blocking relation $AA=A$ of
    \cite[Definition~3.2]{Cirac2016MPDO_arXiv}.
\end{definition}

\begin{definition}[Idempotent transfer map]\label{def:mps_transfer_idempotence}
    \lean{MPSTensor.IsTransferIdempotent}
    \leanok
    \uses{def:transfer_map}
    The transfer map $\E_A$ of an MPS tensor $A$ satisfies \emph{transfer map idempotence} when
    $\E_A\circ\E_A=\E_A$.
\end{definition}

\begin{theorem}[A physical blocking isometry gives transfer idempotence]
    \label{thm:physical_blocking_implies_transfer_idempotence}
    \lean{MPSTensor.isTransferIdempotent_of_kraus_isometry}
    \leanok
    \uses{def:mps_transfer_idempotence}
    Suppose there exists an isometry
    $V\colon \C^d\to\C^{d^2}$, written as coefficients $V_{(i_1,i_2),j}$, such
    that
    \begin{align}
        A^{i_1}A^{i_2}
         & =\sum_{j=0}^{d-1}V_{(i_1,i_2),j}A^j
        \label{eq:corr_kraus_isometry_decomp}
    \end{align}
    for all $i_1,i_2\in\{0,\ldots,d-1\}$.
    Then $\E_A^2=\E_A$.
\end{theorem}
\begin{proof}\leanok
    Substituting the decomposition (\ref{eq:corr_kraus_isometry_decomp}) into
    the double Kraus sum gives, for every bond matrix $X$,
    \begin{align}
        \E_A^2(X)
         & =\sum_{i_1,i_2}(A^{i_1}A^{i_2})X
        (A^{i_1}A^{i_2})^\dagger
        \notag                                             \\
         & =\sum_{j,k}\left(\sum_{i_1,i_2}
        V_{(i_1,i_2),j}\overline{V_{(i_1,i_2),k}}\right)A^jX(A^k)^\dagger
        \notag                                             \\
         & =\sum_{j,k}(V^\dagger V)_{j,k}A^jX(A^k)^\dagger
        =\sum_{j,k}\delta_{j,k}A^jX(A^k)^\dagger
        =\sum_jA^jX(A^j)^\dagger
        =\E_A(X).
        \notag
    \end{align}
\end{proof}

\begin{theorem}[Kraus-isometry criterion for transfer idempotence]%
    \label{thm:kraus_isometry_iff_transfer_idempotence}
    \lean{MPSTensor.isTransferIdempotent_iff_kraus_isometry}
    \leanok
    \uses{def:mps_transfer_idempotence, thm:physical_blocking_implies_transfer_idempotence}
    The transfer map of $A$ is idempotent if and only if there exists an
    isometry $V\colon \C^d\to\C^{d^2}$, written as coefficients
    $V_{(i_1,i_2),j}$ with $V^\dagger V=\Id$, such that
    \begin{align}
        A^{i_1}A^{i_2}
         & =\sum_{j=0}^{d-1}V_{(i_1,i_2),j}A^j
        \label{eq:corr_AA_isometry}
    \end{align}
    for all $i_1,i_2\in\{0,\ldots,d-1\}$.
\end{theorem}
\begin{proof}\leanok
    \uses{thm:physical_blocking_implies_transfer_idempotence}
    The backward implication is
    Theorem~\ref{thm:physical_blocking_implies_transfer_idempotence}. For the
    forward implication, expanding $\E_A^2=\E_A$ as a Kraus sum gives, for
    every $X$,
    \begin{align}
        \sum_{i_1,i_2}(A^{i_1}A^{i_2})X(A^{i_1}A^{i_2})^\dagger
         & =\sum_jA^jX(A^j)^\dagger.
        \notag
    \end{align}
    Rectangular Kraus freedom~\cite[``Rectangular Kraus freedom with general finite index types'']{QICLean2026Blueprint} then supplies the isometry $V$
    relating the two Kraus families.
\end{proof}

\begin{theorem}[Physical and transfer-map fixed-point criteria]
    \label{thm:physical_blocking_isometry_iff_transfer_idempotence}
    \lean{MPSTensor.isTransferIdempotent_iff_hasPhysicalBlockingIsometry}
    \leanok
    \uses{def:physical_blocking_isometry, def:mps_transfer_idempotence}
    The transfer map $\E_A$ of an MPS tensor $A$ is idempotent, $\E_A^2 = \E_A$, if and only if there exists an isometry $V \colon \C^d \to \C^d \otimes \C^d$ such that
    \begin{align}
        A^{i_1} A^{i_2} = \sum_j V_{(i_1, i_2), j} A^j.
        \notag
    \end{align}
\end{theorem}
\begin{proof}\leanok
    \uses{thm:kraus_isometry_iff_transfer_idempotence}
    Follows from Theorem~\ref{thm:kraus_isometry_iff_transfer_idempotence}
    and Definition~\ref{def:physical_blocking_isometry}.
\end{proof}

\begin{definition}[Limit of the renormalization flow]
    \label{def:appears_as_renormalization_flow_limit}
    \lean{MPSTensor.AppearsAsRenormalizationFlowLimit}
    \leanok
    \uses{def:transfer_map}
    Let $A=(A^i)_{i=0}^{d-1}$ be an MPS tensor with bond dimension $D$. The
    physical-isometry class of $A$ \emph{appears as a limit of the
        renormalization flow} if there exist a physical dimension $e$ and an MPS
    tensor $B=(B^a)_{a=0}^{e-1}$ of bond dimension $D$ such that
    \begin{align}
        \widehat{\E}_B^{\,2^n}
         & \longrightarrow \widehat{\E}_A.
        \notag
    \end{align}
    Here $\widehat{\E}_A$ and $\widehat{\E}_B$ are the transfer matrices, and
    convergence is taken in the standard topology on the finite-dimensional
    matrix space $\MN{D^2}$. The physical dimension may change under blocking,
    while the bond dimension $D$ is preserved. Transfer matrices represent the
    quotient by physical isometries
    \cite[lines~382--394 and 1205--1209]{Cirac2016MPDO_arXiv}.
\end{definition}

\begin{theorem}[A dyadic power limit is idempotent]
    \label{thm:dyadic_power_limit_idempotent}
    \lean{isIdempotentElem_of_tendsto_pow_two_pow}
    \leanok
    Let $R$ be a Hausdorff topological monoid with continuous multiplication.
    If $E^{2^n}\to P$, then $P^2=P$.
\end{theorem}
\begin{proof}\leanok
    Continuity of multiplication gives
    $E^{2^n}E^{2^n}\to P^2$. On the other hand,
    $E^{2^n}E^{2^n}=E^{2^{n+1}}\to P$. Uniqueness of limits gives
    $P^2=P$.
\end{proof}

\begin{theorem}[Limits of the renormalization flow]
    \label{thm:renormalization_flow}
    \lean{MPSTensor.IsTransferIdempotent.of_appearsAsRenormalizationFlowLimit,
        MPSTensor.IsTransferIdempotent.appearsAsRenormalizationFlowLimit,
        MPSTensor.appearsAsRenormalizationFlowLimit_iff_isTransferIdempotent,
        MPSTensor.appearsAsRenormalizationFlowLimit_iff_hasPhysicalBlockingIsometry}
    \leanok
    \uses{def:appears_as_renormalization_flow_limit, def:physical_blocking_isometry}
    A tensor class appears as a limit of the two-site renormalization flow if
    and only if it has a physical blocking isometry. Equivalently, there is an
    isometry $V\colon\C^d\to\C^{d^2}$ such that
    \begin{align}
        A^{i_1}A^{i_2}
        =\sum_{j=0}^{d-1}V_{(i_1,i_2),j}A^j
        \notag
    \end{align}
    for every $i_1,i_2$. This is
    \cite[Theorem~3.1 and lines~1205--1209]{Cirac2016MPDO_arXiv}.

    % Local fix (renormalization-flow indices): The printed display sums the
    % free indices i_1,i_2 and has the undefined output index j_1. The equation
    % above is the well-formed identity specified by the renormalization step
    % at lines 389--394, the source diagrams, and the Appendix proof. This
    % changes neither the hypotheses nor the conclusion and is documented in
    % docs/paper-gaps/cpsv16_renormalization_flow_index_typo.tex.
\end{theorem}
\begin{proof}\leanok
    \uses{thm:dyadic_power_limit_idempotent, thm:physical_blocking_isometry_iff_transfer_idempotence}
    If $\widehat{\E}_B^{\,2^n}\to\widehat{\E}_A$, then
    Theorem~\ref{thm:dyadic_power_limit_idempotent} gives
    $\widehat{\E}_A^2=\widehat{\E}_A$. Since the transfer-matrix
    representation is injective, $\E_A^2=\E_A$. Theorem~\ref{thm:physical_blocking_isometry_iff_transfer_idempotence}
    supplies the physical blocking isometry. Conversely, if $A$ has such an
    isometry, its transfer map is idempotent by
    Theorem~\ref{thm:physical_blocking_isometry_iff_transfer_idempotence}.
    Setting $B=A$ makes the dyadic orbit constant:
    $\widehat{\E}_A^{\,2^n}=\widehat{\E}_A\to\widehat{\E}_A$.
\end{proof}

\begin{figure}[ht]
    \centering
    \begin{center}
        % A^{i_1}A^{i_2} = sum_j V^{i_1 i_2}_j A^j (eq:corr_kraus_isometry_decomp,
        % CPSV16 Thm 3.1, lines 396-412): two copies of A on physical legs
        % i_1, i_2 fuse through the isometry V into a single copy of A on
        % physical leg j.  V is a wide pill whose outer slots keep one open
        % up leg each (i_1, i_2), while its centre slot carries the summed
        % index j down into A.  Both sides carry the same boundary signature
        % (virtual-west=1 | virtual-east=1 | physical-up=2).
        \tenkzkernel
        \begin{tenkz}[cols=2, west=open, east=open]
            \tn[ports={90:physical:$i_1$}]{A} & \tn[ports={90:physical:$i_2$}]{A}
        \end{tenkz}
        $ = $
        \begin{tenkz}[rows={op,wire}, cols=3, bonds=none]
            \tn[skin=pill, wide=3, name=V,
                ports={90@1:physical:$i_1$, 90@3:physical:$i_2$,
                        270@2:physical}]{V} \\
            & \tn[name=A,
                ports={180:virtual, 0:virtual, 90:physical:$j$}]{A^j} &
            \tnwire{A.90}{V.270@2}
        \end{tenkz}
    \end{center}
    \begin{center}
        % sum_{i_1,i_2} (V^dagger)^j_{i_1 i_2} A^{i_1} A^{i_2} = A^j
        % (CPSV16 Thm 3.1, lines 396--412, source image II_UAAA.png):
        % V^dagger has one open upper port j and two lower ports, at the
        % blocked columns, contracted separately with A^{i_1}, A^{i_2}.
        % The boundary signature (virtual-west=1 | virtual-east=1 |
        % physical-up=1 | physical-down=0) certifies that no second upper
        % port is introduced.
        \tenkzkernel
        \begin{tenkz}[rows={op,ket}, cols=2]
            \tn[skin=pill, wide=2, ports={90:physical:$j$}]{V^\dagger} \\
            \tn[ports={180:virtual}]{A} & \tn[ports={0:virtual}]{A}
        \end{tenkz}
        $ = $
        \begin{tenkz}[cols=1, west=open, east=open]
            \tn[ports={90:physical:$j$}]{A}
        \end{tenkz}
    \end{center}
    \begin{align}
        \sum_{i_1,i_2}(V^\dagger)^j_{i_1,i_2}A^{i_1}A^{i_2}
         & =A^j.
        \label{eq:corr_AA_isometry_adjoint}
    \end{align}
    \caption{The fusion identity of the renormalization fixed-point
        characterization: two contracted copies of $A$ equal a single copy of
        $A$ with the isometry $V$ merging the summed index $j$ into the
        physical pair $(i_1,i_2)$, as stated in
        (\ref{eq:corr_AA_isometry}); conversely, applying $V^\dagger$
        to the two physical legs of the contracted pair recovers the single
        tensor, as stated in
        (\ref{eq:corr_AA_isometry_adjoint}). The source draws the
        isometry as $U$
        \cite[Theorem~3.1, lines~396--412]{Cirac2016MPDO_arXiv}.}
    \label{fig:rfp_kraus_isometry}
\end{figure}
