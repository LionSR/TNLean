...
This correspondence is an isomorphism between the insertion algebras of the two
endpoints, and an isomorphism of matrix algebras is conjugation by an invertible
matrix $Z_e$ (Skolem--Noether), the edge gauge. Assembling the $Z_e$ into a
single gauge acting at the vertices leaves a nonzero residual scalar $c_{v,e}$
at each endpoint, and equality of the two PEPS states forces these scalars to
balance around every vertex,
\[
    \prod_{e\ni v}c_{v,e}=1,
\]
so on a connected graph the per-edge gauges assemble into the global gauge
family that relates the two tensors.
