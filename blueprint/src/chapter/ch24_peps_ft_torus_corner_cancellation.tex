...
\begin{definition}[Corner extension of a region insert]
    ...
    \begin{align}
        ...
        C_{\mu,\sigma|_R}
        \Phi_{R,S}(\mu,\sigma|_{S\setminus R},\nu).
        \notag
    \end{align}
...
\begin{theorem}[State preservation under corner extension]
    ...
    $\Psi_{A,R,C}=\Psi_{A,S,\Ext_{R\subseteq S}(C)}$.
\end{theorem}
\begin{proof}
    \leanok
...
\begin{theorem}[Consecutive horizontal windows have equal corner extensions]
    ...
    \leanok
    \uses{def:peps_corner_extension, thm:peps_corner_extension_state_preserves}
    ...
    bounds $2\le L$, $2\le K$, $2L+1\le W$, and $2K+1\le H$.
...
\begin{theorem}[Patch-extended equality of the staircase end windows]
    ...
    \leanok
    \uses{def:peps_horizontal_staircase_patch, thm:peps_corner_extension_state_preserves}
    Assume positive bond dimensions and the bounds
    $0<L$, $0<K$, $2L\le W$, and $2K\le H$.
...
\begin{lemma}[Two-block merge collapse]
    ...
    \begin{align}
        F_1(\xi)
         & =
        \prod_{x\in B_1}A_x(\xi|_{\partial x},\sigma_1(x)),
        \notag \\
        F_2(\xi)
         & =
        \prod_{x\in B_2}A_x(\xi|_{\partial x},\sigma_2(x)).
        \notag
    \end{align}
    Then the
...
\begin{theorem}[Composition of corner-extension coefficient kernels]
    ...
    \begin{align}
        \sum_{\nu\in\partial S}
        \Phi_{R,S}(\mu,\sigma|_{S\setminus R},\nu)
        \Phi_{S,P}(\nu,\sigma|_{P\setminus S},\omega)
         & =
        P_{V\setminus S}
        \Phi_{R,P}(\mu,\sigma|_{P\setminus R},\omega).
        \notag
    \end{align}
...
\begin{theorem}[Linearity and transitivity of corner extension]
    ...
    \leanok
    \uses{def:peps_corner_extension, thm:peps_corner_extension_coefficient_composition}
...
\begin{definition}[Combination of subregion physical configurations]
    ...
    configuration on $S\setminus R$ combine into a physical configuration on
    $S$:
    \begin{align}
...
\begin{theorem}[Restrictions of combined subregion configurations]
    ...
    $\operatorname{As}_{R,S}(\sigma_R,\sigma_Q)|_R=\sigma_R$ and
...
\begin{theorem}[Kernel triviality of corner extension from the added block]
    ...
    \leanok
    \uses{def:peps_corner_extension, def:peps_assemble_subregion_physical_config, thm:peps_assemble_subregion_physical_config_restricts}
    ...
    $\Ext_{R\subseteq S}(C)=0$, then $C=0$.
\end{theorem}
\begin{proof}
    \leanok
    ...
    $\rho:\partial R \simeq \partial(V\setminus R)$ and
    $\tau:\partial S \simeq \partial(V\setminus S)$ be the
    complement-boundary bijections, and define
    $c_\eta=C_{\rho^{-1}(\eta),\sigma_R}$ for
    $\eta\in\partial(V\setminus R)$.
    ...
    \begin{align}
        ...
        \left(\sum_{\eta\in\partial(V\setminus R)}
        c_\eta
        \mathbf 1_{\{\exists q:
            \partial_{V\setminus R}q=\eta,
            \partial_{V\setminus S}q=\omega,
            \partial_Qq=\beta\}}\right) W_Q(\beta,\sigma_Q)
        \notag \\
         & =
        \Gamma
        \sum_{\eta\in\partial(V\setminus R)}
    ...
    $\rho(\mu)$. Set
    $\omega=\partial_{V\setminus S}q$ and $\beta=\partial_Q q$.
...
\begin{theorem}[Cancellation of a shared injective corner]
    ...
    \leanok
    \uses{thm:peps_corner_extension_linear_transitive, thm:peps_extend_insert_kernel_trivial_added_injective}
    ...
    $\Ext_{R\subseteq S}(C_1)=\Ext_{R\subseteq S}(C_2)$, then $C_1=C_2$.
\end{theorem}
\begin{proof}
    \leanok
    By linearity, $\Ext_{R\subseteq S}(C_1-C_2)=0$.
...
\begin{theorem}[Staircase-pair cancellation on the torus]
    ...
    Assume the size bounds $2L\le W$ and $2K\le H$, that $Q$ is blocked-tensor
    injective, and that all bond dimensions are positive.
    If two inserts $X$ and $Y$ on $S$ have equal extensions to $S\cup Q$,
    $\Ext_{S\subseteq S\cup Q}(X)=\Ext_{S\subseteq S\cup Q}(Y)$,
...
\begin{proof}
    ...
    completed corner, and therefore $(S\cup Q)\setminus S=Q$.
