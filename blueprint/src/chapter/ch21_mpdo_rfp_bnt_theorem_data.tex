...
\begin{definition}[Virtual per-block linear extension]%
    ...
    product vectors, the per-block linear extension is the unique linear map
...
\begin{proof}\leanok
    \uses{thm:mpdo_virtual_perBlockLinearExtension_mul,
        thm:mpdo_virtual_perBlockLinearExtension_bijective,
        thm:unitary_skolem_noether_matrix}
    The per-block map is a bijective algebra endomorphism. Positivity makes it
...
\begin{theorem}[Closed matrix product vectors do not force a positive virtual extension]%
    ...
    \discussion{5284}
    \uses{def:injective, def:same_mpv,
        def:mpdo_virtual_perBlockLinearExtension,
...
\begin{proof}\leanok
    \uses{def:mpdo_virtual_perBlockLinearExtension,
        thm:mpdo_positive_perBlockLinearExtension_unitary,
        lem:mpdo_nonunitary_pair_no_positive_isometric_realization}
...
\begin{theorem}[Gauge-dressed physical corner of a matched two-site sector]
    ...
    \textbf{Scope restriction (gauge-dressed corner):} This theorem by itself
    constructs only the physical corner in
...
\begin{theorem}[Raw physical corner of a matched one-site sector]
    ...
    $a_\gamma>0$ such that
    \begin{align}
        W_\gamma^\dagger\widetilde M^vW_\gamma
        &=a_\gamma M_\gamma^v
        \label{eq:mpdo_bnt_exact_sector_one_site_raw_corner}
    \end{align}
    for every vertical letter $v$.  Here the bond space of $M_\gamma$ is
    identified with that of its matched two-site sector.  This is the
    distinguished positive-weight copy in the one-site vertical
    decomposition of
...
\begin{proof}\leanok
    ...
    copy.  Transport along the equality of the paired bond dimensions.
\end{proof}
...
\begin{theorem}[Reflected marked chains with a two-site initial block]
    ...
    \textbf{Local fix (mixed one-site/two-site prefix):} This is the common
    unblocked-tail comparison needed when
...
\begin{proof}\leanok
    ...
    $(X^{-1})^\dagger$.  The left side becomes the Gram dressing, while the
    gauge factors cancel on the reflected side.
\end{proof}
...
\begin{theorem}[Oblique physical-letter realization of a matched two-site sector]
    ...
    \end{align}
    This supplies the physical-letter span needed for the direct comparison
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This is the obstruction in the direct two-site argument.  The mixed-prefix
    theorem below instead compares the one-site raw corner with the two-site
    gauge corner while retaining the same one-site tail.
\end{theorem}
...
\begin{theorem}[Conditional Gram identity from the positive-tail reflected target]
    ...
    \textbf{Scope restriction (conditional reflected target):} This theorem
    assumes that the identity-dressed physical-letter family has the same
    reflected-adjoint target as the gauge-dressed marked chains at every
    positive tail length. The oblique construction alone cannot provide this
    identity: adjoint reflection exchanges $L_\gamma$ and $R_\gamma$ and is
    governed by the Gram-dressed compression
...
\begin{lemma}[Normalization of conjugation by a scalar Gram matrix]
    \label{lem:normalized_conjugation_of_scalar_gram}
    \lean{Matrix.normalized_conj_eq_conj_inv_of_gram_eq_smul_one}
    \leanok
    If an invertible matrix $Z$ satisfies
    $Z^\dagger Z=\omega\Id$ with $\omega>0$, then
    $U=\omega^{-1/2}Z$ is unitary and conjugation by $U$ equals conjugation
    by $Z$.
\end{lemma}

\begin{proof}\leanok
    The Gram identity identifies $Z^{-1}$ with
    $\omega^{-1}Z^\dagger$; the scalar factors cancel in the conjugation.
\end{proof}
...
\begin{theorem}[Conditional unitary conjugacy from the positive-tail reflected
    ...
    \leanok
    \uses{def:mpdo, def:cpsv_canonical_form,
        def:mpdo_bnt_two_site_exact_sector_gauge, lem:normalized_conjugation_of_scalar_gram}
    ...
    while their common positive-tail reflected target is an assumption of
    this theorem. Appendix~C.4, lines~2048--2057, does not itself spell out
...
\begin{proof}\leanok
    \uses{thm:mpdo_bnt_conditional_identity_marked_gram,
        lem:normalized_conjugation_of_scalar_gram}
...
\begin{theorem}[Mixed-prefix Gram identity and unitary sector conjugacy]
    ...
    canonical form, and let the one-site and two-site sectors be paired by the
    exact gauges $Z_\gamma$ above.  Then, for every sector $\gamma$ and every
    vertical letter $v$,
    \begin{align}
        (Z_\gamma^\dagger Z_\gamma)M_\gamma^v
        (Z_\gamma^\dagger Z_\gamma)^{-1}
    ...
    \end{align}
    The identity-dressed marked chains therefore also have the positive-tail
    reflected target used in the conditional statements above.

    \textbf{Local fix (mixed one-site/two-site prefix):} The proof compares a
    one-site raw corner with a two-site gauge corner while retaining the same
    unblocked tail.  This supplies the common comparison omitted when
    \cite[Proposition~4.13, lines~1909--1919]{Cirac2016MPDO_arXiv} is invoked
...
\begin{proof}\leanok
    \uses{thm:mpdo_bnt_exact_sector_one_site_raw_corner,
        thm:mpdo_bnt_exact_sector_gauge_blocked_corner,
        thm:mpdo_bnt_exact_sector_identity_oblique_physical_span,
        thm:mpdo_two_site_prefix_gram_reflected_marked_chain,
        thm:mpdo_reflected_marked_chain,
        thm:cpsv_original_coordinate_marked_lemma_l,
        thm:mpdo_bnt_conditional_identity_marked_gram,
    ...
    identity applied to the gauge corner identifies the marked chains of
    ...
    The ordinary one-site reflected identity applied to the raw corner gives
    exactly the same target for $M_\gamma$.

    Both marked tensors are linear combinations of the one-site physical
    letters.  Indeed, the raw mark is obtained from the pair
    $(W_\gamma,W_\gamma)$, whereas the Gram-dressed mark is obtained from
    \begin{align}
        \bigl(W_\gamma G_\gamma^\dagger,
              W_\gamma G_\gamma^{-1}\bigr).
        \notag
    \end{align}
