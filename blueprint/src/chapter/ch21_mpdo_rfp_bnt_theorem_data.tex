...
\begin{definition}[Bond-two singleton deformation]%
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Its terminal matrix and the two-site Bell projector are
    \begin{align}
    ...
    \end{align}
    For
    $X=\frac12\begin{pmatrix}3&1\\1&3\end{pmatrix}$, set
...
\begin{lemma}[Terminal-positive nonunitary deformation]%
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    while $P_X$ is not Hermitian. Thus terminal positivity permits this
    nonunitary deformation at one site, but the corresponding two-site
    similarity is incompatible with positivity.
\end{lemma}
...
\begin{proof}\leanok
    ...
    $X^\dagger X\ne\Id$. Finally, the $(00,01)$ and $(01,00)$ entries of
    $P_X$ are unequal, so $P_X$ is not Hermitian.
\end{proof}

\begin{theorem}[One-site and two-site Hermiticity force a scalar Gram matrix]%
...
\begin{proof}\leanok
    ...
    one-site similarity gives $GJ=JG$. Hermiticity of the two-site similarity
    gives
...
\begin{definition}[Exact invertible gauges for the source-derived two-site sectors]%
    ...
    % Issue 5282 proves the common-target Gram step conditionally, and issue
    % 5286 proves the ensuing scalar-to-unitary normalization.  Issue 5284,
...
\begin{theorem}[Gauge-dressed physical corner of a matched two-site sector]
    ...
    % Issue 5282 proves the common-target Gram identity from a supplied
...
\begin{theorem}[Conditional unitary conjugacy from an identity-dressed marked
    ...
    \path{docs/paper-gaps/cpsv16_two_site_sector_unitary_gauge_gap.tex}.
    % Issue 5282 is the conditional Gram theorem, and issue 5286 composes it
