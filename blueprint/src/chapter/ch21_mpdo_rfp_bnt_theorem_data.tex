...
\begin{theorem}[Closed matrix product vectors do not force a positive virtual extension]%
    ...
    \leanok
    \discussion{5284}
    \uses{def:injective, def:same_mpv, def:perBlockLinearExtension,
...
\begin{proof}\leanok
    \uses{def:perBlockLinearExtension,
        ...
        lem:mpdo_nonunitary_pair_no_positive_isometric_realization}
    If the per-block extension were positive,
    Theorem~\ref{thm:positive_perBlockLinearExtension_unitary} would give a
    unitary $U$ satisfying $B^i=UA^iU^\dagger$ for every letter~$i$.
