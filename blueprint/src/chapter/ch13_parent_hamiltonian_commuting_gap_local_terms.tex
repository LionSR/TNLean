...
\begin{definition}[Overlapping two-site supports]\label{def:overlapping_two_site_supports}
    ...
    \leanok
    \uses{def:nsite_space_parent, def:cyclic_window_support, def:cyclic_windows_overlap}
...
\begin{theorem}[Matrices of the two adjacent factor actions]
    ...
    \leanok
    \uses{def:overlapping_two_site_supports, def:three_coordinate_identification}
...
\begin{theorem}[Three-site translated local terms]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This identifies the two translated parent interactions with the local actions
    in \cite[Definition~D.2]{Cirac2016MPDO_arXiv}.
\end{theorem}
...
\begin{theorem}[Three-site commutation from overlapping supports]
    ...
    \leanok
    \uses{def:overlapping_two_site_supports, thm:local_term_two_three_ax_xb_lifts}
...
\begin{lemma}[Overlap reduction for commuting local terms]
    ...
    \leanok
    \uses{def:is_commuting_parent_ham, def:is_nncph, def:cyclic_windows_overlap}
...
\begin{proof}\leanok
    \uses{lem:cyclic_window_nonoverlap_positive, lem:transported_es_nonoverlap_positive}
...
\begin{lemma}[Adjacent two-site translates suffice for nearest-neighbor commutation]
    ...
    \leanok
    \uses{lem:two_site_cyclic_window_overlap_cases, lem:commuting_parent_ham_overlap_reduction}
    ...
    \end{align}
    Then all translated length-two local terms commute.
\end{lemma}
...
\begin{remark}[Elementary consequences of the commuting definition]
    ...
    general commuting condition. The ground-vector condition combines this
    commuting equation with the frustration-free equation for $V^{(N)}(A)$.
