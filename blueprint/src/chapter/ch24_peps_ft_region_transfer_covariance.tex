...
\begin{theorem}[Covariance of the region insertion coefficient]%
    ...
    \leanok
    \uses{def:peps_regionInsertedCoeff, def:peps_tensor_transport, thm:peps_regionComplementWeight_transport, thm:peps_sameAwayFromBond_regionBoundaryConfigMap}
    Let $\phi:G\simeq G'$ be a graph isomorphism and let $f$ be a crossing edge
...
\begin{theorem}[Bond dimension at a translated crossing edge]%
    ...
    \leanok
    \uses{def:peps_torus_translation_invariant, def:peps_torus_geometry_translation}
...
\begin{theorem}[Translation covariance of the coefficient identity]%
    ...
    \leanok
    \uses{thm:peps_regionInsertedCoeff_transport, thm:peps_regionInsertedCoeff_congrTensor, thm:peps_bondDim_boundaryEdgeMap_translate, def:peps_torus_translation_invariant}
...
\end{theorem}
\begin{proof}\leanok
    \uses{thm:peps_regionBlockedWeight_vertexComplement, thm:peps_regionBlockedTensorInjective_of_isVertexInjective}
...
\begin{theorem}[Injectivity of the complement two-block tensor]%
    ...
    \leanok
    \uses{def:IsVertexInjective, def:peps_complementTwoBlock, thm:peps_vertexComplementTensorInjective}
    If $A$ is vertex-injective and every virtual bond space has positive
...
\begin{theorem}[One-vertex versus complement comparison]%
    ...
    \leanok
    \uses{thm:peps_postAbsorptionEdgeInsertionEquality, thm:peps_twoInjectiveTensorInsertionComparison}
...
\begin{theorem}[Fundamental Theorem for injective PEPS, conditional on
    ...
    \leanok
    \uses{thm:gaugeConsistency, thm:peps_oneVertexComplementComparison, def:GaugeEquivPEPS, def:IsVertexInjective}
...
\end{theorem}
\begin{proof}\leanok
    Gauge consistency supplies the edge gauges directly, which combine with the
    identified bond dimensions to yield the gauge equivalence.
\end{proof}
...
\begin{theorem}[Fundamental Theorem for injective PEPS]%
    ...
    \uses{thm:fundamentalTheorem_PEPS_of_bondDim, def:GaugeEquivPEPS, def:IsVertexInjective}
