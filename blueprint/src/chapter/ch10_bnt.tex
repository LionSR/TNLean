...
\begin{proof}
    ...
    nonnegative summand of $D=\sum_k r_kD_k$, so $D_j\leq D$.
\end{proof}
...
\begin{theorem}[Common exact quantum-Wielandt length for BNT representatives]
    ...
    $1\leq j\leq g$, the representative $A_j$ is $D^4$-block injective:
    \[
        S_{D^4}(A_j)=\MN{D_j}.
    \]
\end{theorem}
...
\begin{proof}
    ...
    \uses{def:sector_bnt_cf,
        ...
        thm:exact_pow_four_block_injectivity_normal_left_canonical,
        thm:wordspan_blk_inj_succ}
    Let $m_j$ be the peripheral period of $A_j$.  Along its multiples, the
    irreducible left-canonical self-overlap satisfies
    \[
        O_{A_jA_j}(m_jn)\longrightarrow m_j.
    \]
    The BNT normalization gives the same limit equal to $1$, hence $m_j=1$
    and $A_j$ is normal.  Therefore
    \[
        S_{D_j^4}(A_j)=\MN{D_j}.
    \]
