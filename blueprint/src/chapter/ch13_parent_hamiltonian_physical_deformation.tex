...
mixing its physical matrices, $A^i\mapsto\sum_j\Lambda_{ij}A^j$~\cite[Section~IV.C]{Cirac2021Matrix}.
Under this action, the deformed local interaction
...
chain Hamiltonians correspond one-to-one through $\Lambda^{\otimes N}$.

\textbf{Scope restriction (periodic-chain specialization):}
The ground-space correspondence under physical deformation is stated in
\cite[Section~IV.C]{Cirac2021Matrix} for tensor network states on a general
graph; here it is formalized for matrix product states on the periodic chain.
Documented in \cite{gap:cpgsv21_physical_deformation_general_graph}
(\texttt{docs/paper-gaps/cpgsv21_physical_deformation_general_graph.tex}).
