...
\begin{definition}[Normalized BNT-refined horizontal form for an MPO]
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    This is the strengthened, normalized BNT-refined form of the canonical
    decomposition in \cite[Section~2.3]{Cirac2016MPDO_arXiv}: unlike the literal
    predicate (Definition~\ref{def:cpsv_canonical_form}), the direct sum exhausts
    the full bond dimension $D$, each copy weight is nonzero and normalized, and
    gauge-equivalent normal blocks are grouped over a minimal BNT representative
    with an independent gauge for each copy.
...
\begin{theorem}[BNT-refined horizontal form gives the BNT MPV representation]
    ...
    \leanok
    \uses{def:horizontal_cf_mpdo, def:horizontal_cf_mpv_representation}
...
\begin{definition}[Vertical canonical form for an MPO]\label{def:vertical_cf_mpdo}
    ...
    \leanok
    \uses{def:mpo_tensor, def:vertical_tensor_mpdo, def:bnt, def:block_diagonal_tensor}
...
\begin{remark}[Finite-word span and trace separation]%
    ...
    \lean{MPSTensor.HasFiniteWordTraceSeparation}
    \lean{MPSTensor.hasFiniteWordTraceSeparation_of_wordTupleSpanTop}
    \lean{MPSTensor.exists_wordTupleSpanTop_of_hasFiniteWordTraceSeparation}
    \lean{MPSTensor.hasFiniteWordTraceSeparation_iff_exists_wordTupleSpanTop}
    \lean{MPSTensor.hasFiniteWordTraceSeparation_of_wordEntryFamily_linearIndependent}
    ...
    \end{align}
    The spanning and separation conditions are equivalent after existentially
    quantifying the length. Spanning at length~$L$ gives separation at that
    length by nondegeneracy of the product trace pairing. Conversely, if the
    span were proper, a nonzero linear functional $f$ would annihilate it.
    Nondegeneracy of the matrix trace pairing represents $f$ as
    \begin{align}
        f\bigl((M_k)_k\bigr) & =\sum_k\tr(\Delta_kM_k), \notag
    \end{align}
    for some tuple $(\Delta_k)_k$, contradicting separation. This finite-word
    property generalizes the one-letter simultaneous span in the source
    definition of block-injective canonical form to length-$L$ words.
    ...
    $(k,a,b)$. Linear independence of this scalar family implies the
    product-algebra spanning condition, and hence trace separation.
    ...
    produces the spanning condition at length $L+S$, hence the product-algebra
    spanning and trace-separation properties.
\end{remark}
...
\begin{theorem}[Finite-word trace-separation criterion]
    \label{thm:propblockinj_abstract}
    \lean{MPSTensor.hasFiniteWordTraceSeparation_of_propBlockInjective}
    \leanok
    \uses{rem:word_tuple_span_top}
    If a block family has a common injective word length and finite words that
    select each block from the others, then the family has finite-word trace
    separation.
\end{theorem}
