...
\begin{theorem}[Duplicate scalar blocks obstruct finite separation]%
    \label{thm:duplicate_scalar_blocks_counterexample}
    \lean{MPSTensor.duplicateScalarBlocks_counterexample}
    \leanok
    \uses{rem:word_tuple_span_top}
    There is a two-copy scalar-block family whose blocks are injective and left
    canonical and whose weights are nonzero, but which has neither block-injective
    canonical form nor a length with linearly independent word-entry family.
\end{theorem}
