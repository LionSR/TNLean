...
\begin{lemma}[Block-window equation at the closing boundary]
    ...
    $G_{L_0+1}(A)$. Then there are matrices $Y_\nu$, indexed by complementary
    words $\nu$ of length $M+1-(L_0+1)$, such that for every word $\alpha$ of
    length $L_0$,
    \[
        X A^\alpha A^\nu=A^\alpha Y_\nu .
    \]
...
\begin{proof}
    ...
    \cite[lines~2078--2079]{Cirac2021Matrix}. It remains to derive this
    block-window equation from the closing-boundary local constraints recorded
    above.
\end{proof}
