...
\begin{proof}
    ...
    \]
    Taking $M=\Id$ gives
    \begin{equation}\label{eq:ph_normal_closing_word_value}
        Y_v = X A^v .
    \end{equation}
    Taking $M=A^j$ and substituting equation~(\ref{eq:ph_normal_closing_word_value})
    gives, for every word $v$ of length $K$,
    \[
        (X A^j-A^jX)A^v=0
    \]
...
\begin{proof}
    ...
    gives
    \begin{equation}\label{eq:ph_normal_closing_trace_window}
        \tr\left(A^jX A^\alpha A^\nu\right)
        =
        \tr\left(A^jA^\alpha Y_\nu\right)
    \end{equation}
    ...
    \]
    For every physical letter $j$,
    equation~(\ref{eq:ph_normal_closing_trace_window}) gives
    \[
        \tr(A^j Z)=0 .
    \]
