...
\begin{proof}
    ...
    \]
    Taking $M=\Id$ gives
    \begin{equation}\label{eq:ph_normal_closing_boundary_word_value}
        Y_v = X A^v .
    \end{equation}
    Taking $M=A^j$ and substituting \eqref{eq:ph_normal_closing_boundary_word_value}
...
\begin{proof}
    ...
    gives
    \begin{equation}\label{eq:ph_normal_closing_boundary_trace_window}
        \operatorname{tr}\left(A^jX A^\alpha A^\nu\right)
        =
        \operatorname{tr}\left(A^jA^\alpha Y_\nu\right)
    \end{equation}
    ...
    \]
    For every physical letter $j$,
    \eqref{eq:ph_normal_closing_boundary_trace_window} gives
    \[
