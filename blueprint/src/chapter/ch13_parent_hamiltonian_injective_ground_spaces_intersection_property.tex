...
The following two statements hold for every tensor $A$; injectivity enters
only through the choice of the matrices $R_j$ in the second one.

\begin{theorem}[Overlap of one-site restrictions]%
    \label{thm:ground_space_restriction_overlap}
    \lean{MPSTensor.groundSpaceMap_mul_eq_of_restrict}
    \leanok
    \uses{def:ground_space_map}
    Let $\psi$ be a vector on $K+2$ sites and let $Y_i,Z_j\in\MN{D}$ satisfy
    $\psi(i,\tau)=\Gamma_{K+1}(Y_i)(\tau)$ and
    $\psi(\tau,j)=\Gamma_{K+1}(Z_j)(\tau)$ for all letters $i,j$ and all words
    $\tau$ of length $K+1$. Then
    \begin{align}
        \Gamma_{K}(A^jY_i) & =\Gamma_{K}(Z_jA^i)
        \notag
    \end{align}
    for all letters $i,j$.
\end{theorem}
...
\begin{proof}
    \leanok
    For a word $\sigma$ of length $K$, cyclicity of the trace gives
    \begin{align}
        \Gamma_{K}(A^jY_i)(\sigma)
         & =\tr(A^\sigma A^jY_i)
        =\psi(i,\sigma,j)
        =\tr(A^iA^\sigma Z_j)
        =\Gamma_{K}(Z_jA^i)(\sigma).
        \notag
    \end{align}
\end{proof}

\begin{theorem}[Growing back from intertwined boundary matrices]%
    \label{thm:ground_space_grow_back}
    \lean{MPSTensor.mem_groundSpace_succ_of_intertwine}
    \leanok
    \uses{def:ground_space_map, def:ground_space_parent}
    Let $\psi$ be a vector on $K+1$ sites and let $Y_i,Z_j\in\MN{D}$ satisfy
    $\psi(i,\tau)=\Gamma_{K}(Y_i)(\tau)$ for all letters $i$ and words $\tau$
    of length $K$, and $A^jY_i=Z_jA^i$ for all letters $i,j$. If matrices
    $R_j\in\MN{D}$ satisfy $\sum_jR_jA^j=\Id$, then $\psi\in G_{K+1}(A)$.
\end{theorem}

\begin{proof}
    \leanok
    Put $X=\sum_jR_jZ_j$. Then
    \begin{align}
        Y_i
         & =\sum_jR_jA^jY_i
        =\sum_jR_jZ_jA^i
...
\begin{theorem}[One inclusion of the intersection property]\label{thm:ground_space_intersection}
    ...
    \leanok
    \uses{thm:ground_space_map_injective, def:in_left_ground, def:in_right_ground, def:ground_space_parent, def:injective}
...
\begin{proof}
    ...
    Comparing the left and right restrictions on their common $(L-1)$-site
    overlap determines a boundary matrix $X'$ that reconstructs the
    full $(L+1)$-site word:
    \begin{center}
    ...
    \begin{enumerate}
        ...
        \item Matching on the $(L-1)$-site overlap
              (Theorem~\ref{thm:ground_space_restriction_overlap}) and using
              injectivity of $\Gamma_{L-1}$ gives $A^jY_i = Z_jA^i$ for all
              $i,j$.
        \item Since $A$ is injective, the matrices $\{A^j\}$ span $\MN{D}$
              ...
              This is Theorem~\ref{thm:ground_space_grow_back} with
              $R_j=c_j\Id$.
...
\begin{theorem}[Intersection property]\label{thm:ground_space_iff}
    ...
    \leanok
    \uses{lem:ground_space_in_left_ground, lem:ground_space_in_right_ground, thm:ground_space_intersection}
...
\begin{remark}[Contiguous interval restrictions]\label{rem:contiguous_window_operators}
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    These identities include the boundary cases $s=0$ and $M=N$, the deletion
    of the first or last site, and the composition of fixing $K$ prefix sites
    followed by restricting to $[s+K,s+K+L-1]$.
\end{remark}
...
\begin{remark}[Cyclic interval restrictions]\label{rem:cyclic_window_operators}
    ...
    \lean{MPSTensor.cyclicCfg_cyclicForwardSite_apply}
    \lean{MPSTensor.cyclicCfg_extractWindow}
    \lean{MPSTensor.update_cyclicCfg}
    ...
    $\rho^{\mathrm{cyc}}_{i,L,\omega}(i+r)=\omega(r)$ with site labels taken modulo $N$.
    Reconstructing a configuration from its local restriction returns the original configuration:
    taking $\rho=\sigma$ and $\omega(r)=\sigma(i+r)$ for $r<L$,
    \begin{align}
        \sigma^{\mathrm{cyc}}_{i,L,\omega}
         & = \sigma.
        \notag
    \end{align}
    Modifying a single inserted site commutes with the cyclic extension: for $r<L$ and a
    letter $x$, writing $\omega[r\mapsto x]$ for the word $\omega$ with its
    value at $r$ replaced by $x$,
    \begin{align}
        \rho^{\mathrm{cyc}}_{i,L,\omega}[\,i+r\mapsto x\,]
         & = \rho^{\mathrm{cyc}}_{i,L,\omega[r\mapsto x]}.
        \notag
    \end{align}
    Deleting the final site of an $(L+1)$-interval gives the $L$-interval
    beginning at $i$; for $L+1\le N$, deleting the first site gives the
    $L$-interval beginning at $i+1$; and the cyclic formula agrees with the
    contiguous restriction when the interval does not wrap around the ring.
\end{remark}
...
\begin{theorem}[One-site restrictions of cyclic-window boundary conditions]
    ...
    In particular, when the boundary words are given by $a_r=\rho(i_0+r)$ and
    $b_r=\rho(i_0+r+L+1)$ modulo $N$, this identity reads $Y_0A^a=A^bY_n$.
\end{theorem}
...
\begin{remark}[Ground-space contiguous-window restriction]
    ...
    \uses{thm:contiguous_restriction_ground_space_map, def:ground_space_map, def:ground_space_parent, rem:contiguous_window_operators}
...
\begin{proof}
    \leanok
    \uses{thm:mem_open_chain_left_ground_space_es, thm:mem_open_chain_tail_ground_space_es, lem:mem_ground_space_es, thm:ground_space_extend_right_block_injective, lem:ground_space_in_left_ground, lem:ground_space_in_tail_ground}
...
\begin{theorem}[Open-chain iteration of the intersection property]%
    ...
    \leanok
    \uses{thm:ground_space_extend_right_block_injective, def:l_blk_injective, rem:contiguous_window_operators}
...
\begin{proof}
    \leanok
    \uses{thm:ground_space_extend_right_block_injective, def:tail_restrict_parent, rem:contiguous_window_operators}
    ...
    Definition~\ref{def:tail_restrict_parent} and
    Remark~\ref{rem:contiguous_window_operators} identify every fixed-prefix
    suffix with one of the assumed length-$L_0+1$ intervals.
...
\begin{theorem}[Open-chain containment from cyclic local constraints]%
    ...
    \leanok
    \uses{lem:cyclic_window_range_antitonicity, thm:normal_open_chain_range_reduction}
