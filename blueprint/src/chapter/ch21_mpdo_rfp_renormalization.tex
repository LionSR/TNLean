...
\begin{theorem}[Pure-state specialization of the physical-map fixed point]
    ...
    \leanok
    \uses{def:is_rfp_via_ts, def:mps_transfer_idempotence,
...
\begin{proof}
    ...
    \uses{thm:physical_blocking_isometry_iff_transfer_idempotence,
      thm:rfp_via_ts_physical_trace_idempotent,
...
\begin{theorem}[Fibonacci rank obstruction cited against strong fixed points]
    ...
    Let $A$ be the vacuum-sector boundary tensor of the Fibonacci model
    described in Appendix~D of \cite{Cirac2016MPDO_arXiv}, and set
    \begin{align}
        \tau_\pm&=\frac{1\pm\sqrt5}{2}.
        \notag
    \end{align}
    The source claims that for every $N>0$,
    \begin{align}
        \operatorname{rank}\rho^{(N)}(A)
        &=\tau_+^{2N}+\tau_-^{2N},
        \notag
    \end{align}
    and that this sequence is not geometric: there do not exist natural
    ...
    exclude such $r$ and $s$.  The diagram labelled ``Strong-RFP'' does not yet
    specify an unambiguous algebraic predicate, so its general rank-growth
    implication is not included in this statement.
\end{theorem}
...
\begin{theorem}[Pure-state recovery of transfer-map idempotence]%
    ...
    \leanok
    \uses{def:mpo_is_zcl, def:mps_transfer_idempotence,
