...
\begin{theorem}[Tensor extension preserves trace-preserving complete positivity]
    ...
    \leanok
    \uses{def:mpdo_id_tensor_map, thm:mpdo_equiv_reindex_cptp, lem:is_kraus_cptp_comp}
...
\begin{theorem}[Diagonal blocks of an orthogonally controlled map]
    ...
    \leanok
    \uses{def:mpdo_controlled_kraus_map, def:mpdo_rectangular_kraus_map}
...
\end{lemma}
\begin{proof}\leanok
    \uses{lem:mpdo_rectangular_kraus_map_cp}
...
\begin{theorem}[Projective-resolution control preserves channels]
    ...
    \lean{isKrausCPTP_sum_comp_singleKrausMap_of_starProjectionResolution,
        isKrausCPTP_sum_comp_singleKrausMap_of_projectiveResolution}
    \leanok
    \uses{def:orthogonal_projection_channel, def:mpdo_single_kraus_map}
...
\begin{theorem}[Kraus action of state preparation]
    ...
    \leanok
    \uses{def:mpdo_rectangular_kraus_map, def:mpdo_state_preparation_kraus}
...
\begin{lemma}[Positive state preparation is completely positive]
    ...
    \leanok
    \uses{thm:mpdo_state_preparation_kraus_action}
...
\begin{proof}
    \leanok
    \uses{thm:mpdo_rectangular_kraus_map_cptp, thm:mpdo_state_preparation_kraus_action, thm:mpdo_state_preparation_kraus_resolution}
...
\begin{proof}
    \leanok
    \uses{thm:physical_blocking_isometry_iff_transfer_idempotence, thm:rfp_via_ts_physical_trace_idempotent, thm:matrix_support_completion_cptp, thm:mpdo_single_kraus_map_cptp}
...
\begin{proof}
    \leanok
    \uses{def:cpsv_fibonacci_open_boundary_operator, def:cpsv_fibonacci_periodic_count, thm:cpsv_fibonacci_periodic_count_golden_ratio, thm:cpsv_fibonacci_open_boundary_rank_status, thm:to_mpo_diagonal_rank}
