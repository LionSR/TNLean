...
The physical renormalization of matrix product density operators rests on
channel extensions, projector-controlled positive sums, and physical closures of
...
\begin{proof}
    ...
    so closure under composition proves the claim.
\end{proof}
...
% Projection-first channel construction in CPSV16, Appendix C.2, lines 1810--1817.


The state-preparation map $\mathcal P_\rho(X)=X\otimes\rho$ from operators on
$A$ to operators on $A\otimes B$, used in the maps $\mathcal T_1$ and
$\mathcal S_1$ of
\cite[Appendix~C.2, lines~1527--1533 and~1551--1555]{Cirac2016MPDO_arXiv},
is stated in \cite[``State-preparation map'']{QICLean2026Blueprint}.
...
In the mixed-state renormalization fixed point of
\cite[Definition~4.1]{Cirac2016MPDO_arXiv}, two local channels extend to
periodic chains of arbitrary length by acting on the first one or two sites and
leaving the remaining sites unchanged.
...
\begin{definition}[Renormalization fixed point via trace-preserving maps]
    ...
    An MPO tensor $M$ is a \emph{renormalization fixed point} (RFP) if there exist
    ...
    $M_2(X)_{(i_1i_2)(j_1j_2)}=\tr(M^{i_1j_1}M^{i_2j_2}X)$ are the one- and
    two-site physical operators obtained by closing one or two tensors with $X$.
    In \cite[Definition~4.1]{Cirac2016MPDO_arXiv}, this condition is formulated
    for tensors in canonical form; the zero-correlation-length consequence
    concerns the physical-trace transfer (Definition~\ref{def:mpo_physical_trace_transfer})
    rather than idempotence of the doubled-index transfer map.
...
\begin{theorem}[Pure-state specialization of the physical-map fixed point]
    ...
    the refinement channel is given by
    \begin{align}
    ...
    pure-state specialization stated in
    \cite[lines~668, 675--687]{Cirac2016MPDO_arXiv}.
\end{theorem}
\begin{proof}
    \leanok
...
\begin{definition}[Fibonacci vacuum-sector support tensor]
    ...
    \leanok
    \uses{def:mps_tensor}
    The Fibonacci vacuum-sector support tensor
    \cite[Appendix~D, lines~2116--2121]{Cirac2016MPDO_arXiv} has physical labels
    given by triples $i,j,k\in\{0,1\}$ and virtual labels
...
\begin{theorem}[Global action of the renormalization maps]
    ...
    Thus the renormalization channels act on periodic rings of arbitrary
    length \cite[Appendix~C, lines~1333--1341]{Cirac2016MPDO_arXiv}.
\end{theorem}
