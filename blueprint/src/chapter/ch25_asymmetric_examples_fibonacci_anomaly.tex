...
\begin{theorem}[Word-trace relations against a normal tensor are integral]%
    ...
    positive bond dimension, over the same alphabet. If every nonempty word
    trace of $B$ is one scalar $c$ times the corresponding word trace of
    $A$, $\tr(B^w)=c\,\tr(A^w)$ for every nonempty word $w$, then $c$ is a
    nonnegative integer.
\end{theorem}

\begin{proof}
    \leanok
    Normality writes a matrix unit of $A$ as a combination
    $\sum_ux_uA^u$ of words of one positive length $\ell$; the same
    combination $N=\sum_ux_uB^u$ of words of $B$ then satisfies
    $\tr(N^k)=c\,\tr\bigl((\sum_ux_uA^u)^k\bigr)=c$ for every positive $k$,
    and a scalar whose every positive power of one fixed matrix has this
    constant trace is a nonnegative integer.
\end{proof}
