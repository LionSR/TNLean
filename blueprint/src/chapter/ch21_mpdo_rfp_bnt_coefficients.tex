...
\begin{proof}\leanok
    ...
    matrices via the rank-one contraction identity and closing the trace
    at the boundary. Positivity is the Hadamard product of the rank-one
    bond-matching indicator (positive semidefinite as an outer product)
    with the pullback of the Kronecker power of $W$ (positive semidefinite
    by congruence along an arbitrary reindexing), scaled by the
    nonnegative scalar $(25/32)^N$.
\end{proof}
...
\begin{proof}\leanok
    \uses{def:mpdo}
    Direct computation: the uniform and alternating vectors are
    eigenvectors of the explicit $2\times2$ matrix $W$, and the identity,
    the traceless diagonal, and the two off-diagonal matrix units are
    eigenvectors of $\varphi$ by direct expansion of the letter matrices.
\end{proof}
