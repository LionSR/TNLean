...
gauge equivalence. This theorem alone gives neither a circuit-depth bound
nor a classification of the factors as renormalization fixed-point tensors.
Positive length is
...
\begin{theorem}[Identification of the flag factor]%
    ...
    both renormalization channel equations. For $N>0$, $\omega_N$ is
    positive semidefinite and has trace one. At $N=0$, both sides of the
    displayed identity are the scalar $2$.
\end{theorem}
\begin{proof}\leanok
    \uses{thm:cpsv_example412_normalized_rfp_maps}
    Substitute the identity and Pauli $Z$ matrices in the definition of
    $\omega_N$. Its expression $2^{-N}(I^{\otimes N}+Z^{\otimes N})$
    is the closed operator of $\widehat M$; the empty case is evaluated
    directly. Positivity, normalization, and the two channel equations
...
\begin{theorem}[Normalization of the bond--flag factorization]%
    ...
    \[
        \operatorname{tr}D_N=\operatorname{tr}\rho^{(N)}(T)=1.
    \]
\end{theorem}
\begin{proof}\leanok
    \uses{thm:mpdo_twisted_dimer_flag_factor, thm:mpdo_twisted_dimer_unitary_factorization}
    The Bell projectors are positive rank-one operators of trace one,
    and their weights in $\sigma$ sum to one. Positivity and trace
    multiply under tensor products and are preserved by unitary
    changes of coordinates. Apply these facts to $D_N$ and then use
    Theorem~\ref{thm:mpdo_twisted_dimer_unitary_factorization}.
\end{proof}

The flag identification uses $\widehat M$, not the unnormalized tensor
printed in Example~4.12. It does not classify the coefficients of an
attached normalized vertical presentation. In particular, constant
multiplication coefficients for the raw representatives $I,Z$ should not
be confused with those for the spectrally normalized representatives
$I/\sqrt2,Z/\sqrt2$.
...
\begin{definition}[The mixed-Bell bond dimer]%
    ...
    \leanok
    \uses{def:mpdo_twisted_dimer_controlled_flip}
    Let $C=\begin{pmatrix}1/2&3/8\\3/8&1/2\end{pmatrix}$.
    ...
    Both physical and virtual dimensions are four. For $N>0$, let
    $B_N$ be the permutation of basis configurations given by
    \[
        B_N\ket{((l_m,r_m))_m}=\ket{((r_{m-1},l_m))_m},
    \]
    with cyclic indices. Its inverse sends a bond configuration $(b_m)_m$
...
\begin{theorem}[Closed operators of the mixed-Bell dimer]%
    ...
    particular, $Q$ is an MPDO. The identity excludes $N=0$, where the
    empty closed operator is $4$ and the empty bond product is $1$.
\end{theorem}
\begin{proof}\leanok
    \uses{def:mpdo_twisted_dimer_bond_tensor, thm:mpdo_twisted_dimer_factor_normalization}
    Multiplication of matrix units forces the right qubit of each
    letter to match the left qubit of the next letter, in both matrix
    arguments. Taking the trace closes this condition around the ring.
    The surviving coefficient is the product of the corresponding
    entries of $C$, hence of $\sigma$ on the matched bond subspace.
...
\begin{theorem}[Channel equations for the mixed-Bell dimer]%
    ...
    Definition~\ref{def:mpdo_twisted_dimer_bond_channels} are completely
    positive and trace preserving. For every one-site operator $Y$,
    \[
        \mathcal S_Q(\mathcal T_Q(Y))=Y.
    \]
    For every virtual boundary matrix $A\in M_4(\mathbb C)$, the
    one-site and two-site physical closures satisfy
    \[
        \mathcal T_Q(Q_1(A))=Q_2(A),
    \]
    \[
        \mathcal S_Q(Q_2(A))=Q_1(A).
    \]
    Thus $Q$ is a renormalization fixed point in the sense of
    Definition~\ref{def:is_rfp_via_ts}.
\end{theorem}
\begin{proof}\leanok
    \uses{thm:mpdo_twisted_dimer_factor_normalization, def:mpdo_twisted_dimer_bond_tensor}
    Preparation of the positive trace-one state $\sigma$, permutation
    of basis coordinates, and partial trace are quantum channels.
    Tracing out the prepared state proves the retraction identity.
    For the refinement equation, the product of two tensor letters
    vanishes unless both intermediate qubits match. On the matched
    subspace its coefficient is $C_{ll'}C_{uu'}$, exactly the entry of
    the one-site closure tensored with the inserted bond state. This
    proves the identity for arbitrary boundary matrices. Applying the
    retraction gives the coarse-graining equation.
\end{proof}

These channel equations independently classify the exact bond dimer as a
renormalization fixed point; that classification does not follow merely
from its closed-operator factorization. The nonzero bond eigenvalues are $7/8$
and $1/8$. No simplicity, canonical-form normalization, or attached
vertical coefficient formula for $Q$ is asserted here.
...
\begin{theorem}[Attachment of the two fixed-point factors]%
    ...
    density-normalized tensor of Example~4.12. For every $N>0$,
    \[
        D_N=H_N^\dagger
        \bigl(\rho^{(N)}(Q)\otimes\rho^{(N)}(\widehat M)\bigr)H_N,
    \]
    \[
        \rho^{(N)}(T)=U_NH_N^\dagger
        \bigl(\rho^{(N)}(Q)\otimes\rho^{(N)}(\widehat M)\bigr)
        H_NU_N^\dagger.
    \]
    Both factor tensors are MPDO renormalization fixed points, by
...
\end{theorem}
\begin{proof}\leanok
    \uses{thm:mpdo_twisted_dimer_bond_product, thm:mpdo_twisted_dimer_bond_rfp, thm:mpdo_twisted_dimer_flag_factor, thm:mpdo_twisted_dimer_unitary_factorization}
    The coordinate permutations satisfy
    $(B_N\otimes I)H_N=S_NE_N$: both read the incoming bonds
    $(r_{m-1},l_m)$ and the unchanged flags $f_m$.
    Substitute the bond-product formula and the flag identification
    into the definition of $D_N$. The second identity then follows
    from Theorem~\ref{thm:mpdo_twisted_dimer_unitary_factorization}.
    The channel classifications of the two factors were proved
    independently in the cited theorems.
\end{proof}

The separation $H_N$ is on-site, but the conjugating operator $U_N$ acts
across neighboring sites. The displayed identities therefore do not
...
$4\cdot2=8$; the factor attachment requires positive length.
