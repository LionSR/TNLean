...
\begin{theorem}[Spectral identification of the local factor and the
    ...
    % Scope note: the existential tensor-attached BNT algebra clause for R
    % is obtained below from the general Theorem 4.14 equivalence. Identifying
    % that witness with the explicit one-label coefficient and diagonal data
    % displayed here remains open.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[The rescaling-stable tensor is a renormalization fixed
    ...
    condition directly. The following theorem obtains an existential
    tensor-attached BNT algebra clause from the general equivalence.
\end{theorem}
\begin{proof}\leanok
...
% Source: arXiv:1606.00608, Theorem 4.14(i)--(ii), lines 972--985,
% and Appendix C.4, lines 1929--2085.
\begin{theorem}[Existential tensor-attached BNT algebra clause for $R$]%
    \label{thm:mpdo_bnt_label_rescaling_stable_bnt_algebra_clause}
    \lean{MPOTensor.RescalingStableLengthDependentRFP.%
        R_hasBNTAlgebraTensorClause}
    \leanok
    \uses{def:mpdo_bnt_algebra_tensor_clause}
    The tensor $R$ admits a tensor-attached vertical BNT presentation. Thus
    there are component tensors $(M_\alpha)_\alpha$, multiplicity matrices
    $(\mu_\alpha)_\alpha$, coefficients $c^{(L)}_{\alpha,\beta,\gamma}$,
    and positive diagonal matrices $\chi_{\alpha,\beta,\gamma}$ such that,
    for every $L>0$,
    \begin{align}
        O_L(M_\alpha)O_L(M_\beta)
        ...
        \notag
    \end{align}
    The coefficients and multiplicities obey
    \begin{align}
        c^{(L)}_{\alpha,\beta,\gamma}
            &=\tr\!\left(\chi_{\alpha,\beta,\gamma}^{L}\right).
        \notag
    \end{align}
    With $m_\alpha=\tr(\mu_\alpha)$, the length-one coefficients satisfy
    \begin{align}
        m_\gamma
        ...
        \notag
    \end{align}
    This is the existential tensor attachment in
    \cite[Theorem~4.14(i)--(ii), lines~972--985, and Appendix~C.4,
      lines~1929--2085]{Cirac2016MPDO_arXiv}. It does not identify this
    presentation with the explicit one-label coefficient and diagonal data
    below.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpdo_bnt_label_rescaling_stable_cf,
      thm:mpdo_bnt_label_rescaling_stable_ismpdo,
      thm:mpdo_bnt_label_rescaling_stable_is_rfp_via_ts,
      thm:mpdo_cpsv_bnt_rfp_equivalence}
    Apply the forward implication of
    Theorem~\ref{thm:mpdo_cpsv_bnt_rfp_equivalence} to the literal canonical
    form, positivity, and renormalization fixed-point properties of $R$.
\end{proof}

\begin{theorem}[The closed operator of $R$ attaches $\chi$ exactly]%
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    So the one-label BNT structure-coefficient block arising from the
    closed operator of $R$ equals $\chi$ exactly, closing the attachment
    of $\chi$ to $R$ used by
    Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_length_dependence}.
    Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_bnt_algebra_clause}
    supplies an existential tensor-attached presentation. The closed-operator
    factorization here does not identify its labels, component operators,
    coefficients, or diagonal matrices with the explicit one-label data
    (documented in
...
\begin{theorem}[Uniform $\chi$-trace-power form of R's one-label structure
    coefficient]%
    ...
    The coefficient family $c^{(L)}_{0,0,0}$ attached to $R$ in
    Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_length_dependence}
    is a positive BNT-label $\chi$ trace-power witness
    (Definition~\ref{def:mpdo_positive_bnt_label_chi_trace_power_form}) with
    $\chi=\operatorname{diag}(1,\tfrac{7}{25})$: the SAME diagonal matrix
    $\chi$ is used at every positive length $L$, only the exponent changes,
    so by Lemma~\ref{lem:mpdo_positive_bnt_label_chi_eq_trace_matrix_pow}
    \begin{align}
        c^{(L)}&=\tr(\chi^L),
        & L&>0.
        \notag
    \end{align}
    Moreover this trace-power coefficient equals $\tr(W^L)$, the trace of
    the $L$-th ordinary matrix power of $R$'s own local factor $W$ from
    ...
    $c^{(L)}=\tr(\chi^L)=1+(7/25)^L$ already displayed in
    Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_length_dependence}, this
    gives $\tr(W^L)=1+(7/25)^L$ as well. The matrix power $W^L$ here is the
    ordinary $L$-fold matrix product of the fixed $2\times2$ matrix $W$,
    distinct from the $N$-fold Kronecker power $W_N$ appearing inside
    $\rho^{(N)}(R)$ itself
    (Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_chi_attachment}); the
    two agree only at $L=N=1$. This closes the example-level uniform
    BNT-label $\chi$-trace-power statement of
    \cite[Theorem~4.14(ii)]{Cirac2016MPDO_arXiv} for $R$'s one-label
    structure coefficient, at the coefficient level. It is not the
    operator-multiplication closure law
    $O_L(M_0)O_L(M_0)=c^{(L)}O_L(M_0)$ for $R$'s original closed operator;
    indeed that direct identification is false. The existential presentation
    in Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_bnt_algebra_clause}
    instead uses the closed operators of its vertical BNT components. Their
    explicit identification with the one-label data remains open in
    \path{docs/paper-gaps/cpgsv17_blocked_chi_uniformity.tex}.
\end{theorem}
\begin{proof}\leanok
