...
\end{theorem}
\begin{proof}\leanok
    The difference of the two coefficient systems gives a vanishing linear
    combination of the operators, so every coefficient of the difference
    vanishes.
\end{proof}
...
\end{theorem}
\begin{proof}\leanok
    Expand the two associations of the triple product
    $O_L(M_\alpha)O_L(M_\beta)O_L(M_\gamma)$ through the product law and
    compare the coefficients of the linearly independent operators.
\end{proof}
...
\begin{theorem}[Associativity constraint in trace-power form]%
    ...
    \uses{thm:mpdo_bnt_label_coeff_associativity,
        def:mpdo_bnt_label_positive_length_chi_trace_power_form}
    For a coefficient system in trace-power form, the associativity
    constraint is a restriction on the family of diagonal matrices:
    \[
        \sum_\delta
