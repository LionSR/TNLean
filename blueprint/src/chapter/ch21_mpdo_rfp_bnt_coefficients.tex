...
\begin{theorem}[A length-dependent coefficient family stable under every
    ...
    The theorem establishes a coefficient-level obstruction for an explicit
    family defined alongside the displayed tensor; it does not attach that
    family to a vertical BNT presentation. Positivity, literal CPSV canonical
    form, and the renormalization fixed-point property of $R$ are proved
    separately. The tensor is a project example motivated by the
...
\begin{theorem}[Closed-operator factorization and local diagonalization for $R$]%
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    Thus the hand-defined matrix $\chi$ diagonalizes the local factor $W$ in
    the closed-operator formula. Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_bnt_algebra_clause}
    independently supplies an existential tensor-attached presentation. The
    factorization here does not identify that presentation's labels, component
    operators, coefficients, or diagonal matrices with the explicit one-label
    data (documented in
...
\begin{theorem}[Uniform $\chi$-trace-power form of the explicit one-label
    ...
    $W_L$ inside $\rho^{(L)}(R)$. The theorem does not identify this explicit
    coefficient model with the coefficients or diagonal data of the
    existential presentation in
    Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_bnt_algebra_clause}.
    That identification remains open in
    \path{docs/paper-gaps/cpgsv17_blocked_chi_uniformity.tex}.
\end{theorem}
\begin{proof}\leanok
