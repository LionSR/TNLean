...
\begin{theorem}[Uniform $\chi$-trace-power form of R's one-label structure
    ...
    \begin{align}
        c^{(L)}&=\tr(\chi^L)=\tr(W^L)=1+\left(\frac{7}{25}\right)^L,
        & L&>0.
        \notag
    \end{align}
    The matrix power $W^L$ here is the ordinary $L$-fold matrix product of
    the fixed $2\times2$ matrix $W$, distinct from the $N$-fold Kronecker
    power $W_N$ appearing inside $\rho^{(N)}(R)$ itself
...
\end{theorem}
\begin{proof}\leanok
    The coefficient family is canonically determined by $\chi$
    (Lemma~\ref{lem:mpdo_bnt_label_of_chi_positive_length_trace_power}), so
    it satisfies the positive-length trace-power condition with respect to
    $\chi$ directly. For the trace identity, the Walsh--Hadamard identity
    $H\chi H=2W$ gives the intertwining relation $H\chi=WH$ after
    cancelling one factor of $H$ using $HH=2\cdot1$; this lifts to every
    power, $H\chi^L=W^L H$, by induction. Conjugating back through $H$ and
    its half-inverse and applying the cyclic property of the trace gives
    $\tr(W^L)=\tr(\chi^L)$ for every $L$.
\end{proof}
