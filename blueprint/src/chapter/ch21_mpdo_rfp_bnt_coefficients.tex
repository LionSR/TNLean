...
\begin{theorem}[The rescaling-stable dimer fails normalized simplicity]
    ...
    \leanok
    \uses{def:mpdo_simple_tensor, def:horizontal_cf_mpdo, thm:mpdo_bnt_label_rescaling_stable_length_dependence}
...
\begin{proof}\leanok
    \uses{lem:mpdo_bnt_label_rescaling_stable_transfer_quasi_idempotent, thm:mpdo_blocking_doubled_index, thm:cpsv_physical_reindexing, thm:transfer_map_block_tensor_quasi_idempotent, thm:mpdo_horizontal_cf_quasi_idempotent_scalar, lem:mpdo_simple_tensor_horizontal_cf}
    By
...
\begin{theorem}[Non-nilpotency of the retained horizontal block]%
    ...
    \leanok
    \uses{def:mpdo_doubled_phys_trace_transfer, def:mpdo_source_simple_tensor, thm:mpdo_bnt_label_rescaling_stable_length_dependence, thm:mpdo_bnt_label_rescaling_stable_cf}
...
\begin{theorem}[Source simplicity of the rescaling-stable dimer]
    ...
    \leanok
    \uses{def:mpdo_source_simple_tensor, thm:mpdo_bnt_label_rescaling_stable_length_dependence}
...
\begin{proof}\leanok
    \uses{thm:cpsv_active_representative_sector_decomposition, thm:mpdo_bnt_label_rescaling_stable_ismpdo, thm:mpdo_bnt_label_rescaling_stable_cf, thm:mpdo_bnt_label_rescaling_stable_non_nilpotent}
...
\begin{theorem}[Nonvanishing source simplicity of the rescaling-stable dimer]
    \label{thm:mpdo_bnt_label_rescaling_stable_nonvanishing_source_simple}
    \lean{MPOTensor.RescalingStableLengthDependentRFP.%
        R_isNonvanishingSourceSimple}
    \leanok
    \uses{def:mpdo_nonvanishing_source_simple_tensor, thm:mpdo_bnt_label_rescaling_stable_length_dependence}
    The dimer tensor $R$ is nonvanishing source-simple.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpdo_bnt_label_rescaling_stable_source_simple, thm:mpdo_bnt_label_rescaling_stable_ismpdo}
    Source simplicity follows from the preceding theorem. For every positive
    length $N$, evaluating the entry identity of
    Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_ismpdo} at the all-zero
    configuration gives
    \begin{align}
        \rho^{(N)}(R)_{\vec 0,\vec 0}
        &=\left(\frac{25}{32}\right)^N
          \left(\frac{16}{25}\right)^N
          =\left(\frac12\right)^N\ne0.
        \notag
    \end{align}
    Hence the closed operator does not vanish.
\end{proof}

\begin{theorem}[Source and normalized simplicity give opposite verdicts]
    \label{thm:mpdo_bnt_label_rescaling_stable_simplicity_verdicts}
    \lean{MPOTensor.RescalingStableLengthDependentRFP.%
        R_isNonvanishingSourceSimple,
        MPOTensor.RescalingStableLengthDependentRFP.%
        R_not_isSimple}
    \leanok
    \uses{def:mpdo_nonvanishing_source_simple_tensor, def:mpdo_simple_tensor, thm:mpdo_bnt_label_rescaling_stable_length_dependence}
    The dimer tensor $R$ is nonvanishing source-simple, and hence source-simple,
    but it does not satisfy normalized fixed-representative simplicity.
\end{theorem}

\begin{proof}\leanok
    \uses{thm:mpdo_bnt_label_rescaling_stable_nonvanishing_source_simple, thm:mpdo_bnt_label_rescaling_stable_not_simple}
    Combine
    Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_nonvanishing_source_simple}
    with Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_not_simple}.
\end{proof}
