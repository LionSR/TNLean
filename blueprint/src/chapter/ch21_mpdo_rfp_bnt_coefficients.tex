...
\begin{theorem}[Spectral identification of the local factor and the
    ...
    % statement of Theorem 4.14(ii) (Cirac2016MPDO_arXiv) for R itself:
    % that requires the tensor-attached BNT algebra-structure witness for
    % R, which is future work. The renormalization fixed-point property of
    % Definition is_rfp_via_ts is established directly below, without that
    % witness; the literal CPSV canonical form of $R$ is established
    % separately below (Theorem thm:mpdo_bnt_label_rescaling_stable_cf).
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[The closed operator of $R$ attaches $\chi$ exactly]%
    ...
    of $\chi$ to $R$ used by
    Theorem~\ref{thm:mpdo_bnt_label_rescaling_stable_length_dependence}.
    This is not the uniform same-length product-algebra statement of
    \cite[Theorem~4.14(ii)]{Cirac2016MPDO_arXiv} itself; that statement
    requires the tensor-attached BNT algebra-structure witness for $R$,
    which is future work (documented in
...
\begin{theorem}[Uniform $\chi$-trace-power form of R's one-label structure
    ...
    $O_L(M_0)O_L(M_0)=c^{(L)}O_L(M_0)$ of that theorem for $R$'s own closed
    operator; that remains the tensor-attached BNT algebra-structure
    witness documented as future work in
    \path{docs/paper-gaps/cpgsv17_blocked_chi_uniformity.tex}.
\end{theorem}
\begin{proof}\leanok
