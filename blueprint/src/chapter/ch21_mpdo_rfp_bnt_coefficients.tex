...
\begin{definition}[BNT-label operator family]%
    ...
    % of the factors M_alpha^{a_r b_r} are contracted by the trace.
    % Contraction set: precisely the cyclic matrix-index contraction encoded by
    % \operatorname{tr}; no physical index is summed.
    ...
    \[
        ...
        =\operatorname{tr}\!\left(
            M_\alpha^{a_1b_1}\cdots M_\alpha^{a_Lb_L}
          \right),
        \qquad
        \mathbf a=(a_1,\ldots,a_L),\quad
...
\begin{proof}\leanok
    ...
    \begin{align*}
        (O_L(M_\alpha)\, O_L(M_\beta))\, O_L(M_\gamma)
         & = \sum_{\varepsilon'}
        \Bigl(\sum_\delta c^{(L)}_{\alpha,\beta,\delta}\,
        c^{(L)}_{\delta,\gamma,\varepsilon'}\Bigr)
        O_L(M_{\varepsilon'}),   \\
        O_L(M_\alpha)\,(O_L(M_\beta)\, O_L(M_\gamma))
         & = \sum_{\varepsilon'}
        \Bigl(\sum_\delta c^{(L)}_{\beta,\gamma,\delta}\,
        c^{(L)}_{\alpha,\delta,\varepsilon'}\Bigr)
        O_L(M_{\varepsilon'}).
    \end{align*}
