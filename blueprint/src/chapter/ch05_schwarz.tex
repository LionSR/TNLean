...
\begin{lemma}[Diagonal Jensen inequality]\label{lem:diagonal_jensen_of_convexOn}
    ...
    \leanok
    Let $A$ be a positive semidefinite matrix, let $v$ be a unit vector, and
    let $f$ be convex on $[0,\infty)$. Then
    \[
        f\!\left(\langle v,Av\rangle\right)
        \le \langle v,f(A)v\rangle .
    \]
\end{lemma}

\begin{proof}
    \leanok
    Diagonalize $A$ by the spectral theorem. The squared moduli of the
    coordinates of $v$ in the eigenbasis form a probability distribution, and
    the eigenvalues of $A$ lie in $[0,\infty)$. The claim is then the scalar
    Jensen inequality applied to these weights and eigenvalues.
\end{proof}
