...
\begin{theorem}[Positive retractions onto one matrix factor]
    ...
    \[
        E(A)=\Id_m\otimes
        \operatorname{tr}_m\!\left((\rho\otimes\Id_d)A\right).
    \]
...
\begin{proof}
    \leanok
    \uses{thm:positive_functional_density_representation}
    ...
    \]
    Hence $R(Y\otimes P_\psi)$ belongs to the nonnegative ray generated by
    $P_\psi$.  Comparing the rays associated with
    $e_i\pm e_j$ and $e_i\pm \mathrm{i}e_j$ shows that there is a scalar
    $c_Y$ such that $R(Y\otimes X)=c_YX$ for every $X\in\MN{d}$.
    ...
    \[
        \operatorname{tr}_m\!\left((\rho\otimes\Id_d)(Y\otimes X)\right)
