...
\begin{proof}
    ...
    For the upper bound, the orthogonal projection $P$ onto the column space of
    $B$ has rank at most $n$ and fixes $B$, so the Cauchy--Schwarz inequality
    gives $|\tr(C^\dagger B)|^2\le \tr(P\rho)$, which the positive-semidefinite
    form of Ky Fan's maximum principle bounds by $\|\rho\|_{(n)}$.  For the
    converse, scaling the projection onto the $n$ largest eigenvalues of $\rho$
    by $C$ yields a normalized coefficient matrix of rank at most $n$ whose
    squared overlap equals $\|\rho\|_{(n)}$.
\end{proof}
