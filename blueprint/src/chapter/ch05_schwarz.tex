...
\begin{proof}
    ...
    \]
    For $i\ne j$, the parallelogram identity and its image under $R$ are
    \[
    ...
    \]
    Their $(i,i)$, $(j,j)$, and $(i,j)$ entries give
    \[
        c_{e_i+e_j}=c_{e_i-e_j}=c_{e_i}=c_{e_j}.
    \]
    The same calculation with $e_i\pm\mathrm{i}e_j$, followed by rank-one
    polarization, therefore gives a scalar $c_Y$ such that
