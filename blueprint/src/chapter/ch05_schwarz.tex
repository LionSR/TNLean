...
\begin{definition}[Schmidt rank]\label{def:schmidt_rank}
    ...
    \[
        \SR(\psi)=\rank(C_\psi).
    \]
    We write $\SR(\psi)\le r$ for the corresponding bounded-rank
    condition.
\end{definition}
...
\begin{lemma}[Monotonicity of bounded Schmidt rank]\label{lem:schmidt_rank_mono}
    ...
    \leanok
    \uses{def:schmidt_rank}
    If $\SR(\psi)\le k$ and $k\le l$, then
    $\SR(\psi)\le l$.
\end{lemma}
...
\begin{theorem}[Square representative with exact Schmidt rank]
    ...
    \[
      ...
      \rank X=\SR(\psi).
    \]
    In particular, if $\SR(\psi)\le r$, then $X$ may be chosen
    with $\rank X\le r$.
\end{theorem}
...
\begin{theorem}[Converse Schmidt-rank Choi test]
    ...
    \]
    for every $\psi\in\C^D\otimes\C^D$ with
    $\SR(\psi)\le k$, then $T$ is $k$-positive.
\end{theorem}
...
\begin{proof}
    ...
    $\SR(R_X^\dagger\eta)\le k$.  Thus every quadratic form of
...
\begin{proof}
    ...
    $\tr(E^*(\rho)e_{ij})=(E^*(\rho))_{ji}$, while the definition
    of $E^*$ gives
    $(E^*(\rho))_{ji}=\tr(\rho E(e_{ij}))$.
\end{proof}
...
\begin{proof}
    ...
    identity
    $\tr(E^*(\rho)X)=\tr(\rho E(X))$, applied first
...
\begin{proof}
    ...
    $\tr(Axx^\dagger)=x^\dagger A x$ and the hypothesis give
...
\begin{lemma}[Rank-one domination]
    ...
    $\psi\in\C^D$.  If
    \[
        A\leq c\ketbra{\psi}{\psi},
    \]
    then there is a nonnegative scalar $a$ such that
    \[
        A=a\ketbra{\psi}{\psi}.
    \]
\end{lemma}
...
\begin{proof}
    ...
    \[
        PAP=\langle\psi,A\psi\rangle P,
        \qquad
        A=s^{-2}\langle\psi,A\psi\rangle P.
    \]
    The coefficient is nonnegative because $A\geq0$ and $s>0$.
\end{proof}
...
\begin{theorem}[Positive retractions onto one matrix factor]
    ...
    \[
        E(\Id_m\otimes X)=\Id_m\otimes X
    \]
    for every $X\in\MN{d}$.  Then there is a density matrix
    $\rho\in\MN{m}$ such that, for every $A\in\MN{md}$,
    \[
        E(A)=\Id_m\otimes
...
\begin{proof}
    ...
    \[
        0\leq R(Y\otimes P_\psi)\leq\tr(Y)P_\psi.
    \]
    Hence, for each $v\in\C^d$, there is a scalar $c_v$ such that
    \[
        R(Y\otimes P_v)=c_vP_v.
    \]
    By rank-one ray scalar rigidity, applied to the map
    ...
    \[
        R(Y\otimes X)=f(Y)X
    \]
    for all $Y\in\MN{m}$ and $X\in\MN{d}$.  Positivity of $R$ gives
    $f(Y)\geq0$ when $Y\geq0$, and the retraction property gives
    $f(\Id_m)=1$.  The density representation of positive matrix functionals
    therefore supplies a density matrix $\rho$ satisfying
    $f(Y)=\tr(\rho Y)$.

    Finally, on simple tensors,
    \[
        \tr_m\!\left((\rho\otimes\Id_d)(Y\otimes X)\right)
        =\tr(\rho Y)X=R(Y\otimes X).
    \]
    Since matrix units are simple tensors, linearity proves the formula for
    every $A\in\MN{md}$.
\end{proof}
...
\begin{proof}
    ...
    $\tr(\rho_{pq}E(X_{qp}))$, and summing the blocks back gives
    $\tr(\rho E^{(k)}(X))$.  Nondegeneracy of the trace pairing
...
\begin{proof}
    In finite dimensions, $\ran(A) \subseteq \ran(B)$
