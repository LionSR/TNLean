...
\begin{definition}[Reduced density of an eigenvector]
    ...
    \]
    Writing $C_i$ for the coefficient matrix of $\phi_i$, one has
    $\rho_i = C_i C_i^\dagger$, so $\rho_i$ is positive semidefinite.
\end{definition}

\begin{lemma}[Eigenbasis Rayleigh expansion]
    \label{lem:eigenbasis_rayleigh}
    \lean{Matrix.IsHermitian.rayleigh_re}
    \leanok
...
\begin{lemma}[Eigenbasis Parseval identity]
    ...
    \lean{Matrix.IsHermitian.sum_normSq_eigenvector_overlap_re}
    \leanok
