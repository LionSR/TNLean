...
\begin{corollary}[Periodic symmetry yields gauge equivalence up to a root of unity]%
    ...
    unity. Let $A$ be in irreducible form~II and
...
\begin{proof}\leanok
    Apply the assumed equivalence to $A$ and $B$, which are in irreducible
    form~II with $\mc{V}(A) = \mc{V}(B)$.
\end{proof}
...
\begin{proof}\leanok\uses{lem:gauge_equiv_twisted}
    ...
    \begin{center}
        ...
        % lines 328--349, especially Condition C1 and its local consequence,
        % identifies the physical action with virtual conjugation.
...
\begin{theorem}[Virtual representation theorem for injective MPS]%
    ...
    \begin{center}
        % Formula/source: Papers/0802.0447/StringOrder-v10.tex,
        % lines 328--349, transfers the on-site action to a virtual gauge.
...
\begin{proof}\leanok
    ...
    \begin{center}
        % Formula/source: Papers/0802.0447/StringOrder-v10.tex,
        % lines 328--349, gives the local physical-to-virtual gauge relation.
...
\begin{definition}[String order parameter]\label{def:string_order_param}
    ...
    \begin{center}
        % Formula/source: Papers/0802.0447/StringOrder-v10.tex,
        % equations EU and SOPMP, lines 164--181, give
        % R_L(u)=tr(\Lambda E_u^L(Id)).
