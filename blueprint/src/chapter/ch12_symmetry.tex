...
\begin{corollary}[Periodic symmetry yields gauge equivalence up to a root of unity]%
    ...
    Suppose that for any two tensors in irreducible form~II with the same MPV
    family there is $m > 0$ making them gauge equivalent up to an $m$-th root of
    unity (Theorem~\ref{thm:periodic_ft}). Let $A$ be in irreducible form~II and
...
\begin{proof}\leanok
    ...
    Apply the assumed equivalence (Theorem~\ref{thm:periodic_ft}) to $A$
    and $B$, which are in irreducible form~II with $\mc{V}(A) = \mc{V}(B)$.
\end{proof}
...
\begin{definition}[On-site symmetry]\label{def:on_site_symmetric}
    ...
    representation $U : G \to \GL_{d}(\C)$ if $\mc{V}(A) = \mc{V}(\widetilde{A}_g)$
    for every $g \in G$.
\end{definition}
...
\begin{proof}\leanok\uses{def:twisted_tensor}
    Since $U(1) = \Id_d$, each component reduces to
...
\begin{theorem}[Virtual symmetry equation]\label{thm:virtual_symmetry_eq}
    ...
    If $A$ is injective and on-site symmetric under $U$, then
    for every $g \in G$ there exist an invertible matrix $X(g) \in \GL_{D}(\C)$
...
\begin{definition}[Boundary string order parameter]%
    ...
    produce every virtual matrix. That comparison is not formalized here,
    and the results below are stated for the virtual form.
...
\begin{definition}[String order]\label{def:has_string_order}
    ...
    boundary choice. The definition in~\cite{PerezGarcia2008StringOrder}
    quantifies existentially over a local unitary $u \ne \Id$ and physical
    endpoint operators $x,y$, and requires a positive limit
    $\lim_{N\to\infty}|S_N| > 0$ rather than a lower bound uniform in the
    length; that paper also notes that the correlator for a particular
    endpoint choice can vanish even when the symmetry is present.
    % Deviation recorded in docs/paper-gaps/pgwsvc08_string_order_virtual_boundary.tex.
\end{definition}
...
\begin{theorem}[Modulus-one twisted eigenvalue implies gauge-phase equivalence]%
    ...
    satisfy $u u^\dagger = \Id$.
    If
    \begin{equation}\label{eq:symmetry_twisted_eigen_mod_one}
        \E_u(X) = \lambda X
    \end{equation}
...
\begin{proof}
    ...
    Gauging both families by that fixed point puts them in a common
    trace-preserving gauge.
    In that gauge the eigenvalue equation~(\ref{eq:symmetry_twisted_eigen_mod_one})
    becomes a modulus-one peripheral
...
\begin{theorem}[Gauge-phase equivalence with the companion tensor yields a virtual unitary]%
    ...
    physical index~$i$,
    \begin{equation}\label{eq:symmetry_phased_covariance}
        \sum_j u_{ij} A^j = \mu\, V A^i V^\dagger.
    \end{equation}
\end{theorem}
...
\begin{proof}
    ...
    equation~(\ref{eq:symmetry_phased_covariance}), with $|\mu|=1$.
\end{proof}
...
\begin{theorem}[Boundary-state invariance of the virtual unitary]%
    ...
    index~$i$,
    \begin{equation}\label{eq:symmetry_boundary_covariance}
        \sum_j u_{ij} A^j = \mu\, V A^i V^\dagger,
    \end{equation}
    then
    \[
...
\begin{proof}
    ...
    $V^\dagger \Lambda V$ is another positive
...
(Remark~\ref{rem:spt_classification_not_formalized}). The theorems of
...
\begin{remark}[The classification theorem is not formalized here]%
    \label{rem:spt_classification_not_formalized}
    ...
    \cite[Section~II.F]{Schuch2011Phases}). Neither direction of that
    equivalence is established here: this chapter constructs the cocycle
    label and proves its gauge independence
