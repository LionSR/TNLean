...
On the square lattice the injective regions are built from coordinate
rectangles. This section fixes the geometry: the regions $R$ and $S$ and the
local window $T$ whose complements differ in a single site, the configuration
the normal theorem requires at each vertex. The union closure of
Section~\ref{sec:peps_normal_union} turns a rectangular injectivity hypothesis
into injectivity of these regions and their complements.
