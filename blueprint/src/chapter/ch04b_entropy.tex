...
\texttt{Entropy} namespace, wraps the sanctioned strong-subadditivity
axiom, and also packages the sanctioned Hayashi SSA-equality
characterization into a consumer-facing quantum-Markov-decomposition
API. The external inputs are accepted as given; their full proofs are
tracked separately in the entropy umbrella issues.
...
\begin{theorem}[Strong subadditivity, namespaced wrapper]\label{thm:entropy_strong_subadditivity}
    ...
    the \texttt{TNLean.Entropy.*} bootstrap. The underlying axiom's
...
\begin{definition}[Quantum Markov decomposition, namespaced]
    ...
    \texttt{Entropy.QuantumMarkovDecomposition} is the stable
    consumer-facing name for the decomposition record of
