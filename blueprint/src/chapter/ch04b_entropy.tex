...
\begin{theorem}[Hayashi SSA-equality characterization]
    ...
    \textbf{Status:} Recorded in the formalization as a sanctioned
    external-input axiom. This is the equality criterion needed
...
This section records the entropy, mutual-information, and Markov-decomposition
declarations used later in the formalization. They restate the previously
introduced formulas and sanctioned external inputs without changing the
mathematical content. The external inputs are accepted as given; their full
proofs are tracked separately in the entropy umbrella issues.

\begin{definition}[Von Neumann entropy, restated]\label{def:entropy_von_neumann_entropy}
    \lean{Entropy.vonNeumannEntropy}
    \leanok
    \uses{def:von_neumann_entropy}
    This declaration records the same formula
    \[
        S(\rho) = -\sum_i \lambda_i \log \lambda_i
    \]
    as in Definition~\ref{def:von_neumann_entropy}.
\end{definition}

\begin{theorem}[Strong subadditivity, restated]\label{thm:entropy_strong_subadditivity}
    ...
    \]
    This restates the single sanctioned strong-subadditivity input already
    recorded in Theorem~\ref{thm:strong_subadditivity}; no new external
    assumption is introduced here. The proof of the underlying inequality
    (Lieb--Ruskai 1973) is deferred to the umbrella entropy issue.
\end{theorem}

\begin{definition}[Quantum Markov decomposition, restated]
    ...
    This declaration records the same decomposition datum as
...
\begin{theorem}[SSA equality iff quantum Markov decomposition]
    ...
    This restates Theorem~\ref{thm:hayashi_ssa_equality_characterization};
    no new external assumption is introduced here.
\end{theorem}

\begin{definition}[Mutual information, restated]\label{def:entropy_mutual_information}
