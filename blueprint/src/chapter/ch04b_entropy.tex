...
\section{Entropy namespace}

The following statements are the \texttt{Entropy}-namespace versions of the
entropy results required for the MPDO arguments
\cite[Appendix~C]{Cirac2021Matrix}.

\begin{definition}[Von Neumann entropy]\label{def:entropy_von_neumann_entropy}
...
\begin{theorem}[Strong subadditivity]\label{thm:entropy_strong_subadditivity}
...
\begin{definition}[Quantum Markov decomposition]\label{def:entropy_quantum_markov_decomposition}
    \lean{Entropy.QuantumMarkovDecomposition}
    \leanok
    \uses{def:hayashi_markov_decomposition}
    The quantum Markov decomposition of a tripartite state, as in
    Definition~\ref{def:hayashi_markov_decomposition}.
\end{definition}

\begin{theorem}[SSA equality and quantum Markov decomposition]
    ...
    subadditivity holds iff $\rho_{ABC}$ admits a quantum
    Markov decomposition on the middle subsystem $B$.
\end{theorem}

\begin{definition}[Mutual information]\label{def:entropy_mutual_information}
...
\begin{theorem}[Subadditivity from SSA with trivial middle system]\label{thm:entropy_subadditivity_trivial_B}
