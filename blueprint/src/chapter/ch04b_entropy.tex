...
\section{\texorpdfstring{\texttt{TNLean.Entropy}}{TNLean.Entropy} namespace bootstrap}

The \texttt{TNLean/Entropy/} directory (issue~\#613) provides a stable
namespace surface for the Simple MPDO RFP track (issue~\#236). It
re-exports the von Neumann entropy and mutual information under the
\texttt{Entropy} namespace, and records strong subadditivity as an
axiom in the same namespace so that the axiom boundary is explicit to
downstream modules. The axiom is accepted as given; its proof (from
Lieb--Ruskai 1973) is tracked separately in issue~\#239.
...
\begin{theorem}[Strong subadditivity, axiomatized]\label{thm:entropy_strong_subadditivity}
    ...
    \]
    This statement is accepted as an \emph{axiom} in the current Lean
    development; the proof (Lieb--Ruskai 1973) is deferred to the
    umbrella entropy issue. The \leanok tag marks the axiom itself as
    intentionally part of the Lean development surface.
\end{theorem}
