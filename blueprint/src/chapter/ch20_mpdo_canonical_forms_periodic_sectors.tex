...
\begin{theorem}[No periodic-sector projector for a matrix product density
    ...
    \leanok
    \uses{def:mpdo, def:mpdo_periodic_sector_projector, thm:psd_commute_of_commute_pow}
...
\begin{definition}[Periodic vectors supply projectors]
    ...
    \leanok
    \uses{def:vertical_tensor_mpdo, def:no_periodic_vectors, def:mpdo_periodic_sector_projector}
    ...
    peripheral spectrum
    (Theorem~\ref{thm:mpdo_displaced_invariant_projector}); the single
    non-commutation instance suffices for the periodic-sector contradiction
    below.
...
\begin{theorem}[No periodic vectors in the vertical direction]
    ...
    \leanok
    \uses{def:mpdo, def:vertical_tensor_mpdo, def:no_periodic_vectors, def:mpdo_periodic_vector_yields_projector, thm:mpdo_no_periodic_sector_projector}
...
\begin{proof}
    \leanok
    \uses{def:mpdo_periodic_vector_yields_projector, thm:mpdo_no_periodic_sector_projector}
...
\begin{theorem}[Vertical letters of the stacked tensor]
    ...
    \leanok
    \uses{def:mpo_stacked_layers, def:vertical_tensor_mpdo, def:mpdo_vertical_one_site_actions}
...
\begin{definition}[Periodic-sector projector from cyclic data]
    ...
    \leanok
    \uses{def:mpdo, def:mpdo_periodic_sector_projector, def:mpdo_vertical_one_site_actions, def:mpo_stacked_layers}
...
\begin{proof}
    \leanok
    \uses{thm:mpo_stacked_vertical_letters, thm:mpo_stacked_layers_power, thm:mpdo_first_site_invariant_comm}
...
\begin{definition}[Periodic vectors supply cyclic projectors]
    ...
    \leanok
    \uses{def:vertical_tensor_mpdo, def:no_periodic_vectors, def:mpdo_vertical_one_site_actions}
...
\begin{proof}
    \leanok
    \uses{thm:spectral_unital_gauge_schwarz_setup, lem:eigenvalue_transport_unital_gauge, lem:kraus_cyclic_shift, lem:support_proj_range_factorization}
    ...
    Theorem~\cite[``Cyclic decomposition for irreducible finite Kraus maps'']{QICLean2026Blueprint} produces
...
\begin{theorem}[A displaced idempotent fails to commute at some length]
    ...
    \leanok
    \uses{def:cpsv_canonical_form, def:mpo_family, def:mpdo_vertical_one_site_actions}
    ...
    \end{align}
    This existential non-commutation statement supplies the single length
    needed for the contradiction in
    \cite[Proposition~4.13, lines~1888--1893]{Cirac2016MPDO_arXiv}.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:representative_one_sub_mp_zero, lem:mpdo_first_site_insertions_vertical_mul}
...
\begin{theorem}[Literal canonical form excludes vertical periodic sectors]
    ...
    \leanok
    \uses{def:mpdo, def:cpsv_canonical_form, def:vertical_tensor_mpdo, def:no_periodic_vectors}
...
\begin{proof}
    \leanok
    \uses{thm:mpdo_displaced_invariant_projector, thm:cpsv_literal_displaced_noncommutation, thm:mpo_stacked_vertical_letters, thm:mpo_stacked_layers_power, thm:mpdo_first_site_invariant_comm, thm:psd_commute_of_commute_pow}
...
\begin{theorem}[BNT-refined horizontal form excludes vertical periodic sectors]
    ...
    \leanok
    \uses{def:mpdo, def:horizontal_cf_mpdo, def:vertical_tensor_mpdo, def:no_periodic_vectors, thm:representative_one_sub_mp_zero, lem:mpdo_first_site_insertions_vertical_mul, thm:mpdo_displaced_invariant_projector, thm:mpo_stacked_layers_power, thm:mpdo_first_site_invariant_comm, thm:psd_commute_of_commute_pow}
    ...
    The normalized BNT-refined horizontal hypothesis is stronger than the literal
    CPSV canonical-form hypothesis; documented in
    \cite{gap:cpgsv17_vertical_cf_grouping}.
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:representative_one_sub_mp_zero, lem:mpdo_first_site_insertions_vertical_mul, thm:mpdo_displaced_invariant_projector, thm:mpo_stacked_layers_power, thm:mpdo_first_site_invariant_comm, thm:psd_commute_of_commute_pow}
    ...
    with $H^{(N+1)}$, which yields the contradiction in
    \cite[Proposition~4.13, lines~1888--1893]{Cirac2016MPDO_arXiv} using a
    single noncommuting length.
\end{proof}
...
\begin{theorem}[No periodic vectors for an injective matrix product density
    ...
    \leanok
    \uses{def:mpdo, def:injective, def:vertical_tensor_mpdo, def:no_periodic_vectors, thm:mpdo_displaced_invariant_projector, thm:mpdo_commute_of_isInjective}
