\begin{definition}[One fusion layer of the topological projector]%
    ...
    \end{align}
    Summing the forward fusion identity over equal horizontal
    operator indices and distributing matrix multiplication
    gives (\ref{eq:rfp_terminal_trace}).
...
\begin{definition}[Recursive fusion histories and their sequential circuit]%
    ...
    has length $N-1$; the operators $W_N$ and $Q_N$ are indexed by this total
...
\begin{theorem}[Fixed-label recursive fusion identity]%
    ...
    In Theorem~\ref{thm:mpdo_topological_density_decomposition}, the direct
    sum is taken over all labels and multiplicity coordinates. The commutator
    is a separate assertion; see
...
\begin{theorem}[Positivity of the terminal transfer matrices]%
    ...
    in \cite[lines~1010--1016]{Cirac2016MPDO_arXiv}.

    The trace closes the horizontal operator leg and leaves the two bond
    indices of the vertically read tensor $M_\gamma$ open. See
    \cite{gap:cpsv16_topological_projector_recursion}.
\end{theorem}
\begin{proof}\leanok
...
\begin{definition}[Recursive density operator summed over all labels]%
    ...
    The embedding $\widetilde U$ in \cite[line~999]{Cirac2016MPDO_arXiv}
    corresponds to $W_{N,c}^\dagger$ on this block.
...
\begin{theorem}[Density decomposition of an RFP MPDO]%
    ...
    \end{align}
    The statement applies to positive lengths under the normalized BNT-refined
    horizontal hypothesis.
    % See docs/paper-gaps/cpgsv17_vertical_cf_grouping.tex.

    For a rectangular vertical map, (\ref{eq:rfp_density_reverse}) uses
    exact reconstruction from the retained nonzero sectors rather than a
    square-unitary identity. See
    \cite{gap:cpgsv17_vertical_isometry_zero_sector}.
\end{theorem}
\begin{proof}\leanok
    ...
    over all configurations. Local reconstruction applied at every site
    gives (\ref{eq:rfp_density_reverse}).
\end{proof}
...
\begin{corollary}[Commuting density factors of an RFP MPDO]%
    ...
    \end{align}
    Equivalently, $[A_N,T_N]=0$, as stated in
    \cite[lines~1000--1002]{Cirac2016MPDO_arXiv}.
