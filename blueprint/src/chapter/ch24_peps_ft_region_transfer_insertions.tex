...
\begin{theorem}[Injectivity of the complement-region two-block tensor]%
    ...
    $\inj(T_{A,V\setminus R})\Longrightarrow\inj(B_{A,R^c})$.
\end{theorem}
\begin{proof}\leanok
    ...
    $\partial R$, then
    $\eta_1^c=\eta_2^c\Longrightarrow\eta_1=\eta_2$.
...
\begin{proof}
    ...
    $C_A(R,f,M-M';\sigma,\tau)=0$ for every $\sigma,\tau$.
    Injectivity of the region and complement blocked tensor maps reads off every
    entry of $M-M'$ from configurations agreeing away from $f$, so $M-M'=0$
    and hence $M=M'$.
\end{proof}
...
\begin{theorem}[Bond insertion as a deformed region state]
    ...
    \leanok
    \uses{def:peps_bond_inserted_region_insert, def:peps_regionInsertedCoeff}
    For every physical configuration $\sigma$ on $R$ and $\tau$ on
...
\begin{theorem}[Region insertion as an edge insertion]
    ...
    \begin{align}
        C_B(R,f,N;\sigma,\tau)
         & =
        p_B(R)
        E_B(f,\omega_{R,\sigma,\tau},\mathcal O_{R,f}(N)).
        \notag
    \end{align}
...
\begin{theorem}[Bond insertion through a corner extension]
    ...
    \leanok
    \uses{thm:peps_deformedRegionState_bondInsertedRegionInsert, thm:peps_corner_extension_state_preserves}
    ...
    \begin{align}
        C_A(R,f,M;\sigma|_R,\sigma|_{V\setminus R})
         & =
        \Psi_{A,S,
            \Ext_{R\subseteq S}(I_{A,R,f,M})}(\sigma).
        \notag
    \end{align}
...
\begin{theorem}[Region insertion as a two-block insertion]%
    ...
    \leanok
    \uses{def:peps_regionInsertedCoeff, def:peps_regionTwoBlock, def:peps_regionComplementTwoBlock}
...
\begin{theorem}[Same two-block insertions from region insertions]%
    ...
    \leanok
    \uses{thm:peps_twoBlockInsertedCoeff_eq_regionInsertedCoeff, thm:peps_regionInsertedCoeff_reindexTensor}
    ...
    $(B_{A,R},B_{A,R^c})$ and $(B_{B^h,R},B_{B^h,R^c})$
...
\begin{theorem}[Complement-side reading of a region insertion]
    ...
    \leanok
    \uses{thm:peps_twoBlockInsertedCoeff_swap, thm:peps_twoBlockInsertedCoeff_eq_regionInsertedCoeff}
...
\begin{theorem}[Incident form gives the transferred endpoint form]
    ...
    $W=\mathcal I_f(P)$ is in incident-matrix form on the boundary leg $f$,
    then the transfer matrix read off from $W$ reinserts to $W$.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Virtual pullback realizes the endpoint operator]
    ...
    \leanok
    \uses{def:peps_same_state, def:IsVertexInjective, def:peps_localTensorMap, thm:peps_physical_to_virtual_insertion}
...
\begin{theorem}[Complement-side cast and endpoint realization]
    ...
    \leanok
    \uses{def:peps_region_out_endpoint_block_inverse, thm:peps_regionInsertedCoeff_complement_reading}
...
\begin{theorem}[Region resonate identity and blocked read-offs]
    ...
    \leanok
    \uses{def:peps_region_out_endpoint_block_inverse, thm:peps_regionInsertedCoeff_complement_cast}
    ...
    \begin{align}
        P_R\,O^{\mathrm{in}}_f(X) \Psi_B(R;\sigma,\tau)
         & =
        P_{R^c}\,O^{\mathrm{out}}_f(X)
        \Psi_B(R^c;\tau,\sigma^{cc}).
        \notag
    \end{align}
