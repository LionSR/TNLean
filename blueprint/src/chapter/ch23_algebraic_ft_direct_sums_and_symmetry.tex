...
they combine already-known block gauges, and they record the easy consequences
...
corresponding global block-diagonal gauge is given by
...
\begin{lemma}[Gauge equivalence of direct sums with phase-weight absorption]%
    ...
    \uses{lem:tensor_from_blocks_globalGauge_conj, def:gauge_equiv, def:gauge_phase_equiv}
...
\begin{remark}
    See Definition~\ref{def:block_diagonal_tensor} for the block-diagonal tensor
    construction. This subsection records the direct-sum corollaries where the
    pairing between blocks is already fixed; the derivation of the block
    permutation from the global same-MPV condition appears in
...
\begin{remark}[Block-diagonal gauge from matched blocks]
    This lemma constructs a block-diagonal gauge from pre-matched blockwise gauge
    equivalences, whereas the full Fundamental Theorem for direct sums derives
    the block correspondence and permutation from the global state.
\end{remark}
...
\begin{lemma}[Gauge equivalence of direct sums from per-block MPV equality]%
    ...
    \leanok
    \uses{def:injective, def:same_mpv, def:gauge_equiv, def:block_diagonal_tensor, lem:tensor_from_blocks_globalGauge_conj}
...
\begin{lemma}[MPV invariance under block permutation]\label{lem:mpv_perm_blocks}
    ...
    The $\mathrm{SameMPV}_2$ family of a weighted block-diagonal tensor is
    invariant under permuting the block index set and transporting the weights
    correspondingly.
\end{lemma}
...
The scalar uniqueness and gauge-composition results used by the virtual
representation theorem are proved in
Section~\ref{sec:app_symmetry_gauge_ratios}.
