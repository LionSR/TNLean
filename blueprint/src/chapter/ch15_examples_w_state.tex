...
excitation~\cite{DurVidalCirac2000}. Unlike the preceding examples it has no
translation-invariant representation with bond dimension bounded independently
of $N$~\cite[Section~III.D]{Cirac2021Matrix}; it is presented with open boundary
...
\begin{remark}[No fixed-bond translation-invariant family]%
    ...
    \uses{def:w_tensor}
    The open-boundary representation in Definition~\ref{def:w_tensor} uses
    boundary vectors. Ref.~\cite{Cirac2021Matrix} observes that a
    translation-invariant periodic representation requires a bond dimension
    growing with system size ($D^3\log D=\Omega(N)$ via the quantitative quantum
    Wielandt inequality); this lower bound is an unformalized target
    (see~\cite{gap:rmp_w_state_ti_bound}) requiring representation theory and
    quantitative Wielandt bounds not yet in the library.
\end{remark}
