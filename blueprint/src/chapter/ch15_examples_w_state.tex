...
\begin{lemma}[At most two product states]\label{lem:w_state_two_product_states}
    \lean{MPSTensor.not_sum_pow_mul_pow_eq}
    \leanok
    \uses{def:w_indicator}
    Let $r\leq2$, let $x_1,\ldots,x_r$ and $y_1,\ldots,y_r$ be complex numbers, and
    let $c\neq0$. Suppose $N\geq2$, and $N\geq3$ when $r=2$. Then the equalities
    \begin{align}
        \sum_{k=1}^{r}y_k^{m}x_k^{N-m} & =
        \begin{cases}
            c & m=1,   \\
            0 & m=2,3,
        \end{cases}
...
\begin{proof}
    ...
    and $m=2$ gives $c\,y_1=0$, so $y_1=0$ and again $c=0$. For $r=2$ both
    sequences $m\mapsto y_k^mx_k^{N-m}$ are annihilated by the recurrence with
    characteristic polynomial $(x_1S-y_1)(x_2S-y_2)$; applied to the cases
    $m=1,2,3$ it gives $c\,y_1y_2=0$. Hence one of the $y_k$ vanishes, and the
...
\begin{proof}
    ...
    \uses{lem:w_state_cut_rank, lem:pgvwc07_block_tuple_span,
        lem:pgvwc07_cut_rank_lower_bound, lem:w_state_two_product_states,
    ...
    most two blocks, each of size one by one. By
    Lemma~\ref{lem:w_state_two_product_states}, a sum of at most two
