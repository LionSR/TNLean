...
\begin{definition}[Physical maps in vertical canonical coordinates]
    \label{def:mpdo_vertical_coordinate_channel_transport}
    \lean{MPOTensor.transportTToVerticalCoordinates}
    \lean{MPOTensor.transportSToVerticalCoordinates}
    \leanok
    \uses{def:is_rfp_via_ts, def:mpdo_vertical_bond_matrix_contraction}
    ...
    changes, as in
    \cite[Appendix~C.4, lines~1974--1979]{Cirac2016MPDO_arXiv}.  They are
...
\begin{lemma}[Action of the transported physical maps]
    \label{lem:mpdo_vertical_coordinate_channel_transport}
    \lean{MPOTensor.reindex_physClose2_eq_physClose1_blockTwo}
    ...
    \leanok
    \uses{def:mpdo_vertical_coordinate_channel_transport,
      lem:mpdo_vertical_bond_contraction_forward,
      lem:mpdo_vertical_bond_contraction_reconstruction,
    ...
    identities for $\widetilde T$ and $\widetilde S$ at lines~1974--1979 of
    the source.
\end{lemma}
...
\begin{proof}\leanok
    ...
    \begin{aligned}
        T_{\mathrm v}(B_1(X))
          &=U_2J(T(M_1(X)))U_2^\dagger \\
          &=U_2J(M_2(X))U_2^\dagger \\
          &=U_2M^{[2]}_1(X)U_2^\dagger
           =B_2(X).
    \end{aligned}
    \]
    ...
    \begin{aligned}
        S_{\mathrm v}(B_2(X))
          &=U_1S(J^{-1}(M^{[2]}_1(X)))U_1^\dagger \\
          &=U_1S(M_2(X))U_1^\dagger \\
          &=U_1M_1(X)U_1^\dagger
           =B_1(X).
    \end{aligned}
    \]
