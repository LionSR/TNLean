...
\begin{theorem}[Canonical hypotheses for every retained copy pair]
    ...
    $((\alpha,q),(\beta,r))$ is
    \[
      \mu_{\alpha,q}\mu_{\beta,r}
      \bigl(M_\alpha M_\beta\bigr).
    \]
...
\begin{theorem}[Existence and inclusions of retained-product spectral data]
    ...
    spectral data exist.  The canonical
    ...
    tensor.  These explicit composite inclusions retain the range projections
...
\begin{proof}\leanok
    ...
    identities.  Finally, the Kronecker square of the vertical coisometry is a
    coisometry, so its adjoint preserves the isometry relation.
\end{proof}
...
\begin{theorem}[Flattened reconstruction of the retained product]
    ...
    \[
      C^i=\sum_j W_j\bigl(\lambda_j A_j^i\bigr)W_j^\dagger,
      \qquad
      W_j^\dagger W_l=\delta_{jl},
      \qquad
      C^iW_j=W_j(\lambda_jA_j^i),
      \qquad
      W_j^\dagger C^i=(\lambda_jA_j^i)W_j^\dagger.
    \]
...
\begin{theorem}[Coverage of active product corners by the blocked BNT]
    ...
    \lean{MPOTensor.exists_blockedBNT_gaugePhase_of_flatBlock}
    \lean{MPOTensor.FlatBlockedBNTComparison}
    \lean{MPOTensor.exists_flatBlockedBNTComparison}
    \leanok
    ...
    one such that
    \[
      A_j^i=\zeta_jX_jA^{(2),i}_{\gamma(j)}X_j^{-1}
    \]
    after transporting the blocked tensor along the dimension equality.  The
    choices may be made simultaneously.  No converse coverage or surjectivity
    of $j\mapsto\gamma(j)$ is asserted.

    \textbf{Local fix (Figure-11 fixed-pair support):} Only active product
    corners are classified.  A blocked BNT label unused by a fixed product
    pair has an empty multiplicity fiber.  This is documented in
