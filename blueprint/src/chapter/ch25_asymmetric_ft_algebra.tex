...
\begin{theorem}[Nilpotency from vanishing trace powers]
    ...
    $u\in\operatorname{End}(V)$. If $\tr(u^k)=0$ for all $k\ge 1$, then $u$ is
    nilpotent.
\end{theorem}
...
\begin{theorem}[Identification of a simple quotient with a target block]
    ...
    This provides the quotient identification used with the
    positive-word character lemma (Lemma~\ref{lem:asym_character}) below.
\end{theorem}
...
\begin{lemma}[A family of normal blocks with zero total word trace is
    ...
    Let $\{C_s\}_{s\in S}$ be a finite family of normal tensors of positive
    bond dimensions. If for every nonempty word $w\ne\emptyset$,
    \begin{align}
        \sum_{s\in S}\tr(C_s^w)=0,
        \notag
    \end{align}
    then $S=\emptyset$.
\end{lemma}
