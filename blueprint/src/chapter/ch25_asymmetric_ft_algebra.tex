...
\begin{proposition}[Periodic data of a compression]
    ...
    \lean{MPSTensor.MultiBlockCompression.trace_evalWord_eq_sum,
        MPSTensor.MultiBlockCompression.trace_evalWord_eq_sum_smul,
    ...
    When the blocks are rescaled tensors $C_s=\mu_sA_s$, this reads
    $\tr(B^w)=\sum_{s\in S}\mu_s^{|w|}\tr(A_s^w)$.
\end{proposition}
...
\begin{proof}
    ...
    slots. For $C_s=\mu_sA_s$ a word of length $|w|$ gives
    $C_s^w=\mu_s^{|w|}A_s^w$.
\end{proof}
