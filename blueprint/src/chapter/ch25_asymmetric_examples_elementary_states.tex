...
The upper-triangular source tensor of bond dimension four below generates the
same positive-length states, with two scalar diagonal sectors for the constant
product configurations, providing an explicit instance of the multi-block
...
amplitudes $(1,2)$. Repeating the scalar block twice multiplies every
periodic coefficient by two; the upper-triangular source $B$ of
Definition~\ref{def:asymex_repeated} generates the same vector as this
repeated target,
\begin{align}
    \ket{V^{(N)}(B)}
     & =2\ket{\phi}^{\otimes N}
...
that proves the factor two of Theorem~\ref{thm:asymex_repeated_state}.
