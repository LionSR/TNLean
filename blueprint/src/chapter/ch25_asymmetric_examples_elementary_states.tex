...
For $N>0$, the unnormalized Greenberger--Horne--Zeilinger (GHZ)
state~\cite{GreenbergerHorneZeilinger1989,Greenberger1990Bell} on
$(\C^2)^{\otimes N}$ is
\begin{align}
    \ket{\mathrm{GHZ}_N}
...
with the same positive-length states, whose two scalar diagonal sectors
generate the constant product configurations.
...
source $B$ in Definition~\ref{def:asymex_repeated} generates the same vector as
...
the tensor power is the scalar $1$.
...
(Theorem~\ref{thm:asymex_repeated_not_gauge}).
