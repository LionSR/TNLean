...
the direct sum $C_0\oplus C_1$ of two scalar sectors, as in
Ref.~\cite[Appendix~A]{Cirac2021Matrix} and Definition~\ref{def:ghz_tensor}.
The upper-triangular source tensor $B$ of bond dimension four
(Definition~\ref{def:asymex_ghz}) satisfies $\ket{V^{(N)}(B)} = \ket{\mathrm{GHZ}_N}$
for all $N>0$, providing an explicit instance of the multi-block
compression theorem (Theorem~\ref{thm:asym_multiblock}).
...
$\ket{\phi}=\ket{0}+2\ket{1}$, so the bond-one tensor $A$ has local
amplitudes $(1,2)$. The direct sum $A\oplus A$ doubles every amplitude,
and the upper-triangular source $B$ of Definition~\ref{def:asymex_repeated}
generates the same vector:
\begin{align}
    \ket{V^{(N)}(B)}
     & =\ket{V^{(N)}(A\oplus A)} = 2\ket{\phi}^{\otimes N}
    =2(\ket{0}+2\ket{1})^{\otimes N}.
    \label{eq:asymex_repeated_state_eq}
\end{align}
...
\begin{theorem}[Product-state amplitudes and multiplicity]
    ...
    \begin{align}
        \ket{V^{(N)}(A)}
         & =(\ket{0}+2\ket{1})^{\otimes N}, \label{eq:asymex_repeated_A_state} \\
        \ket{V^{(N)}(B)}
         & =2\ket{V^{(N)}(A)}. \label{eq:asymex_repeated_B_state}
    \end{align}
...
The source $B$ and target $A\oplus A$ satisfy
$\ket{V^{(N)}(B)}=\ket{V^{(N)}(A\oplus A)}$ at all lengths $N\geq0$
by~\eqref{eq:asymex_repeated_state_eq}. However, they are not gauge
equivalent: $B^i\neq X(A^i\oplus A^i)X^{-1}$ for any invertible $X$
(Theorem~\ref{thm:asymex_repeated_not_gauge}), even though
$\tr(B^w)=2\tr(A^w)$ for all nonempty words $w$
(Theorem~\ref{thm:asymex_repeated_compression}). The compression theorem
acts only on the two diagonal copies of $A$, yielding the trace identity
$\tr(B^w)=2\tr(A^w)$ that proves the factor two in~\eqref{eq:asymex_repeated_amplitude}.
...
A Jordan extension of a repeated block of arbitrary bond dimension shares
every periodic trace with the direct sum of two copies of the block, while a
single boundary matrix $Q$ detects it:
