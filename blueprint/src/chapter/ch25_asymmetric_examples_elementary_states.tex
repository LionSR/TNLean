\section{The GHZ state as a compression target}%
\label{sec:asymex_ghz}

The Greenberger--Horne--Zeilinger (GHZ)
state~\cite{GreenbergerHorneZeilinger1989,Greenberger1990Bell} is the
equal superposition of the two constant product configurations; for
$N>0$ sites, its unnormalized form on $(\C^2)^{\otimes N}$ is
\begin{align}
    \ket{\mathrm{GHZ}_N}
     & =\ket{0}^{\otimes N}+\ket{1}^{\otimes N}.
    \notag
\end{align}
Here $\ket{0},\ket{1}$ form the orthonormal local basis, following the
product-state and GHZ conventions of Ref.~\cite[Appendix~A]{Cirac2021Matrix}.
The standard tensor generating this state has bond dimension two and is
the direct sum of two scalar sectors, as in
Ref.~\cite[Appendix~A]{Cirac2021Matrix} and in
Definition~\ref{def:ghz_tensor} of Chapter~\ref{ch:examples}. The
upper-triangular source tensor of bond dimension four studied below is a
construction of this development: it is not minimal, it generates the
same positive-length states, and its two scalar diagonal sectors
generate the constant product configurations. It serves as the first
instance of the multi-block compression theorem
(Theorem~\ref{thm:asym_multiblock}).

Throughout this section and the next, for a tensor $T=\{T^i\}_{i=0}^1$
of bond dimension $D$, the periodic trace boundary defines
\begin{align}
    V^{(N)}(T)_\sigma
...
The source generates the GHZ state at every positive length, with the
same amplitudes and the same norm as the standard tensor; the two
tensors differ only in their empty-length coefficients, which count
...
The amplitude identity rests on the compression of the source onto its
two scalar sectors. The source has bond dimension four while both
sectors are scalar, so two of its four diagonal positions are zero
slots; the compression theorem accounts for them in the dimension count
and delivers the periodic trace identity behind
Theorem~\ref{thm:asymex_ghz_state}.
...
The compression is genuinely a word-level statement. A sitewise
intertwiner for a sector would be a vector fixed by one letter of the
source and annihilated by the other; the following theorem shows that
the sector $C_0$ admits none in either direction.
...
\begin{theorem}[The first sector has no sitewise intertwiner]
    ...
    $C_0$ therefore vanish, so the word-level compression of
    Theorem~\ref{thm:asymex_ghz_compression} yields no nonzero
...
A product state $\ket{\phi}^{\otimes N}$ is the simplest matrix product
state: it is generated by a tensor of bond dimension one whose entries
are the local amplitudes of $\ket{\phi}$, as in the product-state
construction of Ref.~\cite[Appendix~A]{Cirac2021Matrix}. Here
$\ket{\phi}=\ket{0}+2\ket{1}$, so the bond-one tensor has local
amplitudes $(1,2)$. The repeated block and its upper-triangular source
are a construction of this development: repeating the scalar block
twice multiplies every periodic coefficient by two, and the source $B$
of Definition~\ref{def:asymex_repeated} generates the same vector as
this repeated target,
\begin{align}
    \ket{V^{(N)}(B)}
     & =2\ket{\phi}^{\otimes N}
    ...
    \notag
\end{align}
The tensors $A$ and $B$ in Definition~\ref{def:asymex_repeated} generate these
states via the trace boundary~(\ref{eq:asymex_periodic_amplitude}); at $N=0$
the tensor power is the scalar $1$. The example shows a target with a
repeated block, where the compression theorem records the multiplicity
of the block, and a source that is not gauge equivalent to the target
even though the two generate equal periodic vectors at every length.
...
\subsection{Compression and the absence of a gauge}

The source and the direct sum $A\oplus A$ generate the same periodic
vectors at all lengths, including $N=0$. However, they are not gauge
equivalent: the off-diagonal entry of $B^0$ contributes nothing to any
periodic word trace, but prevents conjugation to the diagonal target
(Theorem~\ref{thm:asymex_repeated_not_gauge}). The compression theorem
sees only the two diagonal copies of $A$, and it is the trace identity
it supplies that proves the factor two of
Theorem~\ref{thm:asymex_repeated_state}.
...
The off-diagonal entry of $B^0$ is one instance of a general mechanism
of this development, the Jordan extension of a repeated block of
arbitrary bond dimension: the extension shares every periodic trace
with the direct sum of two copies of the block, and a single boundary
matrix $Q$ detects it.
...
\begin{theorem}[Equal periodic traces and different boundary families]
    ...
    arbitrary tensor $A$. Then $B$ and $C$ have equal periodic traces for
    every word, but the functions $v\mapsto\tr(QB^v)$ and
