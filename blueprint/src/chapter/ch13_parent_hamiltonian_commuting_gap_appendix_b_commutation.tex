\begin{theorem}[Commutation of adjacent two-site basic support projectors]
    ...
    \leanok
    \uses{thm:appendixB_virtual_bond_projectors_commute, thm:appendixB_physical_isometry_tensor_power, thm:appendixB_two_site_basic_support_projector}
    ...
    \cite[Definition~3.9, Equations~(3.16)--(3.18), and
        Appendix~B]{Cirac2016MPDO_arXiv}.
\end{theorem}
...
\begin{theorem}[The (AX) factor action]
    ...
    \leanok
    \uses{def:appendixB_qax, thm:pair_lift_kronecker_matrices, def:three_coordinate_identification}
...
\begin{theorem}[The (XB) factor action]
    ...
    \leanok
    \uses{def:appendixB_qxb, thm:pair_lift_kronecker_matrices, def:three_coordinate_identification}
    On the homogeneous three-single-site coefficient space
...
\begin{lemma}[Appendix B coefficient representatives and the parent interaction]
    ...
    \leanok
    \uses{lem:appendixB_basis_change_parent_interaction, def:appendixB_qax, def:appendixB_qxb}
...
\begin{lemma}[Appendix B two-site representative idempotents]
    ...
    \leanok
    \uses{lem:parent_interaction_idempotent, lem:appendixB_qax_qxb_parent_interaction}
...
\begin{theorem}[Appendix B overlapping two-site commutation]
    ...
    \leanok
    \uses{thm:appendixB_two_site_basic_support_projectors_commute, thm:appendixB_two_site_basic_support_projector, lem:appendixB_qax_qxb_idempotent, def:overlapping_two_site_supports}
    ...
    Definition~\ref{def:overlapping_two_site_supports}, providing the
    coefficient-space analog of the commutation condition in
    \cite[Definition~D.2]{Cirac2016MPDO_arXiv}.
\end{theorem}
...
\begin{theorem}[Three-site intersection of adjacent parent kernels]
    ...
    \leanok
    \uses{thm:local_term_two_three_ax_xb_lifts, thm:ground_space_intersection, def:parent_interaction, def:injective}
...
\begin{theorem}[Appendix B coefficient-space analog of the Appendix D kernel datum]
    ...
    \leanok
    \uses{thm:appendixB_overlapping_two_site_commutation, thm:three_site_ground_space_adjacent_parent_kernels, lem:appendixB_qax_qxb_parent_interaction}
...
\begin{theorem}[Appendix B adjacent commutation on periodic chains]
    ...
    \leanok
    \uses{thm:appendixB_overlapping_two_site_commutation, thm:three_site_commutator_chain_transport}
...
\begin{theorem}[Left-canonical RFP commutation and zero energy]
    ...
    \leanok
    \uses{thm:appendixB_adjacent_chain_commutation, lem:nncph_from_adjacent_two_site_commutation}
...
\begin{proof}
    \leanok
    The Appendix~B structural theorem supplies a datum whose adjacent
    translated parent interactions commute by
    Theorem~\ref{thm:appendixB_adjacent_chain_commutation}.
    Lemma~\ref{lem:nncph_from_adjacent_two_site_commutation} gives pairwise
    commutation directly. The
...
\begin{lemma}[Definition D.2 projectors identified with Appendix B representatives]
    ...
    \leanok
    \uses{def:overlapping_two_site_supports, def:appendixB_qax, def:appendixB_qxb, lem:appendixB_overlapping_condition_parent_terms}
...
\begin{lemma}[Three-site lifts of the Appendix B coefficient representatives]
    ...
    \leanok
    \uses{thm:local_term_two_three_ax_xb_lifts, lem:appendixB_qax_qxb_parent_interaction}
...
\begin{lemma}[Appendix B lifted commutator gives three-site local-term commutation]
    ...
    \leanok
    \uses{lem:appendixB_overlapping_condition_from_lift_commutator, lem:appendixB_overlapping_condition_parent_terms}
    If the Appendix~B $AX$ and $XB$ coefficient representatives satisfy the
