...
\begin{theorem}[Direct-sum quasi-idempotence on every mixed corner]%
    ...
    \lean{MPSTensor.%
        mixedMapLM_comp_self_eq_smul_of_transferMap_comp_self_eq_smul}
    \leanok
...
\begin{proof}\leanok
    \uses{thm:block_transfer_sum_quasi_idempotent_of_direct_sum, lem:block_diagonal_transfer_sum_to_block}
...
\begin{lemma}[Pairwise idempotence of the block transfer sum]%
    ...
    \lean{MPSTensor.blockTransferSum_idempotent_of_pairwise_mixedMapLM}
    \leanok
...
\begin{theorem}[Pairwise mixed-transfer criterion for a direct sum]%
    ...
    \lean{MPSTensor.isTransferIdempotent_directSumTensor_iff_pairwise_mixedMapLM_isIdempotentElem}
    \leanok
    \uses{def:mps_transfer_idempotence, def:mixed_transfer_rect, lem:block_diagonal_transfer_sum_to_block, lem:block_transfer_sum_idempotent_of_pairwise_mixed}
...
\begin{theorem}[Phase coherence for literal repeated blocks]%
    ...
    \leanok
    \uses{def:mps_transfer_idempotence, lem:mixed_transfer_rect_smul, thm:direct_sum_transfer_idempotent_iff_pairwise_mixed}
...
\begin{theorem}[Cross-block transfer vanishing for a direct-sum fixed point]%
    ...
    \leanok
    \uses{def:is_bnt_locally_orthogonal, def:mps_transfer_idempotence, def:mixed_transfer_rect}
    ...
    distinct components $j\ne j'$, the mixed transfer operator vanishes:
    \begin{align}
        \E_{j,j'}=\sum_i A_j^i\otimes\overline{A_{j'}^i}=0.
        \notag
    \end{align}
    This establishes the cross-block ($\delta_{j,j'}$) orthogonality of
    \cite[Theorem~3.11 and Corollary~3.12]{Cirac2016MPDO_arXiv}.
    The passage to the residual tensors $U_j$ is given by
    Theorem~\ref{thm:residual_isometry_of_rfp_direct_sum}.
\end{theorem}
\begin{proof}\leanok
    \uses{lem:block_diagonal_transfer_sum_to_block, thm:transfer_operator_gap_irr_tp, thm:transfer_operator_gap_rect_irr_tp, lem:idempotent_spectral_radius_zero}
...
\begin{definition}[Joint isometry condition]%
    ...
    \leanok
    A family $(U_j)_j$ satisfies the \emph{joint isometry condition} if
    \begin{align}
        ...
        \notag
    \end{align}
    Each block satisfies unit pair-index orthonormality ($j=j'$), and the
    cross-block sums between distinct blocks vanish ($j\ne j'$), as in
    \cite[Theorem~3.11]{Cirac2016MPDO_arXiv}.
\end{definition}
...
\begin{lemma}[Covariance of the mixed transfer operator]%
    ...
    \lean{MPSTensor.mixedMapLM_conj_apply}
    \leanok
    \uses{def:mixed_transfer_rect}
...
\begin{lemma}[Vanishing after removing the diagonal factors]%
    ...
    \lean{MPSTensor.mixedMapLM_eq_zero_of_conj}
    \leanok
    \uses{def:mixed_transfer_rect}
...
\begin{lemma}[Entrywise cross-block isometry sum]%
    ...
    \lean{MPSTensor.residual_isometry_entry_of_mixedMapLM_eq_zero}
    \leanok
    \uses{def:mixed_transfer_rect}
...
\begin{theorem}[Joint isometry condition for a direct-sum fixed point]%
    ...
    \leanok
    \uses{def:is_residual_isometry_family, def:mps_transfer_idempotence, def:is_isometry_canonical_form}
    Let $(A_j)_j$ be a family of normal, irreducible, left-canonical blocks with
    bond dimensions $D_j\ge1$, no two of which are gauge-phase equivalent. If
    the direct sum $\bigoplus_j A_j$ is a renormalization fixed point, then
    there exist invertible matrices $X_j$, trace-one positive diagonal
    matrices $\rho_j=\diag(\lambda_{j,\alpha})$, and tensors $U_j$ with
    \begin{align}
        A_j^i & =X_j\sqrt{\rho_j}\,U_j^iX_j^{-1},
        \notag
    \end{align}
    such that $(U_j)_j$ satisfies the joint isometry condition
    (Definition~\ref{def:is_residual_isometry_family}). This formalizes the
    isometry equations of \cite[Corollary~3.12]{Cirac2016MPDO_arXiv} for
    distinct normal blocks.
...
\end{theorem}
\begin{proof}\leanok
    \uses{thm:bnt_locally_orthogonal_of_rfp_direct_sum, thm:isometry_canonical_form_of_rfp_nt, lem:mixed_transfer_conj_apply, lem:isrfp_block_of_rfp_direct_sum, lem:mixed_transfer_eq_zero_of_conj, lem:residual_isometry_entry}
...
\begin{theorem}[Direct sum of square-root fixed-point blocks]%
    ...
    \leanok
    \uses{def:is_isometry_canonical_form, def:mps_transfer_idempotence, def:mixed_transfer_rect}
    Let $(B_k)_k$ be a family of tensors in square-root fixed-point form whose
    cross-block mixed transfer operators vanish ($\E_{j,j'}=0$ for all
    $j\ne j'$). Then the direct sum $\bigoplus_k B_k$ is a renormalization
    fixed point. This establishes the converse direction of the pure-state
    fixed-point characterization \cite[Section~3]{Cirac2016MPDO_arXiv} for
    distinct blocks.
...
\end{theorem}
\begin{proof}\leanok
    \uses{thm:is_rfp_of_isometry_canonical_form, lem:block_diagonal_transfer_sum_to_block}
...
\begin{theorem}[Distinct-block fixed-point characterization]%
    ...
    \leanok
    \uses{def:mps_transfer_idempotence, def:is_isometry_canonical_form, def:mixed_transfer_rect, def:normal, def:irreducible_tensor, def:gauge_phase_equiv}
    Let $(B_k)_k$ be a family of normal, irreducible, left-canonical blocks with
    $D_k\ge1$ for all $k$, no two of which are gauge-phase equivalent. Then
    the direct sum $\bigoplus_k B_k$ is a renormalization fixed point if and only
    if each block has the square-root fixed-point form and all cross-block mixed
    transfer operators vanish:
    \begin{align}
        \left(\bigoplus_k B_k\text{ is an RFP}\right)
         & \iff
        ...
        \notag
    \end{align}
    This is the distinct-block characterization of pure-state renormalization
    fixed points \cite[Theorem~3.11]{Cirac2016MPDO_arXiv}.
...
\end{theorem}
\begin{proof}\leanok
    \uses{thm:isometry_canonical_form_of_rfp_nt, lem:isrfp_block_of_rfp_direct_sum, thm:bnt_locally_orthogonal_of_rfp_direct_sum, thm:is_rfp_of_isometry_canonical_form_direct_sum}
...
\begin{corollary}[BNT basis direct-sum characterization]%
    ...
    \leanok
    \uses{def:sector_bnt_cf, def:mps_transfer_idempotence, def:is_isometry_canonical_form, def:mixed_transfer_rect, def:is_residual_isometry_family}
    Let $P$ satisfy the auxiliary BNT sector hypotheses, with distinct basis tensors $(B_j)_j$.
    Then the multiplicity-one direct sum $\bigoplus_j B_j$ is a
    renormalization fixed point if and only if every $B_j$ has the square-root
    fixed-point form and $\E_{j,j'}=0$ for all $j\ne j'$.

    In this case, there exist invertible matrices $X_j$, trace-one positive
    diagonal matrices $\rho_j=\diag(\lambda_{j,\alpha})$, and tensors $U_j$
    such that $B_j^i=X_j\sqrt{\rho_j}\,U_j^iX_j^{-1}$, where $(U_j)_j$
    satisfies the joint isometry condition
    (Definition~\ref{def:is_residual_isometry_family}).
    % The physical-space construction of the repeated-copy index is recorded in
    % docs/paper-gaps/cpsv16_rfp_isometry_scope.tex.
\end{corollary}
\begin{proof}\leanok
    \uses{thm:rfp_directSumTensor_iff, thm:postblocked_representative_grouped_insert_eq, thm:normal_tp_primitive_irred}
    ...
    the corresponding theorem in the companion quantum-channel volume~\cite[``Primitivity equals period one'']{QICLean2026Blueprint}, and normality follows from
...
\begin{theorem}[A fixed nonzero scaling of a normal tensor]%
    ...
    \leanok
    \uses{def:nt_cpsv, def:mps_transfer_idempotence, thm:spectral_radius_smul, thm:idempotent_spectral_radius_one}
...
\begin{theorem}[Blocks of a CPSV fixed point]%
    \label{thm:cpsv_weight_norm_one_and_block_rfp}
    \lean{MPSTensor.CPSVCanonicalFormData.weight_norm_one_and_block_rfp}
    \leanok
    \uses{def:cpsv_canonical_form, def:mps_transfer_idempotence, thm:rfp_coisometry_reconstruction_iff, thm:normal_scaled_rfp, lem:isrfp_block_of_rfp_direct_sum}
...
\end{theorem}
\begin{proof}\leanok
    \uses{thm:rfp_coisometry_reconstruction_iff, lem:isrfp_block_of_rfp_direct_sum, thm:normal_scaled_rfp}
    By Theorem~\ref{thm:rfp_coisometry_reconstruction_iff}, the direct sum
    $\bigoplus_k\mu_kA_k$ is a renormalization fixed point.
    Lemma~\ref{lem:isrfp_block_of_rfp_direct_sum} implies that each block
    $\mu_kA_k$ is also a renormalization fixed point. Since $\mu_k\ne0$,
    Theorem~\ref{thm:normal_scaled_rfp} gives both conclusions.
\end{proof}

% CPSV16 Corollary 3.12 (`III_cor3`), lines 583--590.
% Under the nonzero-coefficient convention every basis element occurs with a
% nonzero weight, so the phase-class statement below is the formalization of
% the corollary; the square-root diagonal is a local correction.
\begin{theorem}[Phase-class residual isometries]%
    \label{thm:cpsv_phase_class_residual_isometries}
    \lean{MPSTensor.CPSVCanonicalFormData.BNTRefinement.%
        exists_residualIsometryFamily_of_isTransferIdempotent}
    \lean{MPSTensor.IsCPSVCanonicalForm.%
        exists_bntRefinement_residualIsometryFamily_of_isTransferIdempotent}
    \leanok
    \uses{def:cpsv_canonical_form, def:is_residual_isometry_family}
    Let $A$ be a CPSV canonical-form renormalization fixed point. Choose one
    representative $C_j$ from each matrix-product-vector phase class.
    Then there are invertible matrices $X_j$, positive numbers
    $\lambda_{j,\alpha}$ with $\sum_\alpha\lambda_{j,\alpha}=1$, and tensors
    $U_j$ such that
    \begin{align}
        C_j^i
         & =X_j\diag\!\left(\sqrt{\lambda_{j,\alpha}}\right)
        U_j^iX_j^{-1},
        \notag                                                         \\
        \sum_i(U_j^i)_{\alpha,\beta}
        \overline{(U_{j'}^i)_{\alpha',\beta'}}
        ...
        \notag
    \end{align}
    If no classes are present, the statement holds vacuously.

    \textbf{Local fix (square-root factor in canonical form):}
    The formula uses the square-root weights $\sqrt{\lambda_{j,\alpha}}$ rather
    than the bare diagonal printed in \cite[line~547]{Cirac2016MPDO_arXiv}, as
    required by the reference-tensor construction; documented in
    \cite{gap:cpsv16_rfp_isometry_scope}
    (\texttt{docs/paper-gaps/cpsv16_rfp_isometry_scope.tex}).
\end{theorem}
\begin{proof}\leanok
    \uses{thm:cpsv_canonical_form_bnt_refinement, thm:rfp_coisometry_reconstruction_iff, thm:cpsv_weight_norm_one_and_block_rfp, thm:normal_rfp_iff_isometry_canonical_form, thm:bnt_locally_orthogonal_of_rfp_direct_sum, lem:mixed_transfer_eq_zero_of_conj, lem:residual_isometry_entry}
    The retained weighted direct sum is a renormalization fixed point. For each
    representative, Theorem~\ref{thm:cpsv_weight_norm_one_and_block_rfp}
    gives $|\mu_j|=1$ and $\E_{C_j}^2=\E_{C_j}$. By
    Theorem~\ref{thm:normal_rfp_iff_isometry_canonical_form}, each normal fixed
    point $C_j$ admits the displayed square-root form. Putting the
    representatives independently in left-canonical gauge, distinct phase-class
    representatives are not gauge-phase equivalent. Since the direct sum is a
    fixed point, Theorem~\ref{thm:bnt_locally_orthogonal_of_rfp_direct_sum}
    gives
    \begin{align}
        \E_{\mu_jC_j,\mu_{j'}C_{j'}} & =0
        \qquad (j\ne j').
        \notag
    \end{align}
    Cancelling the unit-modulus scalars and gauge transformations yields
    $\E_{C_j,C_{j'}}=0$. Removing the positive diagonal factors and evaluating
    on matrix units gives the off-diagonal residual orthogonality. Combined
    with the within-block orthonormality of each $U_j$, this yields the joint
    isometry condition.
\end{proof}
...
\begin{theorem}[Square-root characterization of fixed points]
    ...
    \notready
    \uses{def:cpsv_canonical_form, def:bnt_cpsv, def:mps_transfer_idempotence, def:is_isometry_canonical_form, def:is_residual_isometry_family}
    Cirac et al.~\cite[Theorem~3.11]{Cirac2016MPDO_arXiv} assert that a tensor $A$
    in canonical form is a renormalization fixed point if and only if it can be
    written as
    \begin{align}
        A^i
    ...
    both the physical and virtual spaces \cite[lines~561--562]{Cirac2016MPDO_arXiv}.
    However, it does not provide an explicit coordinate embedding for the physical
    routing of $q$, a dimension constraint, or a $q$-indexed isometry equation.
    Consequently, this statement remains unformalized pending clarification of the
    coordinate-level routing in the source.

    \textbf{Local fix (square-root factor in canonical form):}
    CPSV prints a bare $\Lambda_j$ in Theorem~3.11, whereas the reference tensor
    at line~1300 has coefficients $\sqrt{(\rho_j)_{\alpha,\alpha}}$, which
    forces the factor $\sqrt{\rho_j}$ used above. This repair is documented in
    \cite{gap:cpsv16_rfp_isometry_scope}
    (\texttt{docs/paper-gaps/cpsv16_rfp_isometry_scope.tex}).
\end{theorem}
