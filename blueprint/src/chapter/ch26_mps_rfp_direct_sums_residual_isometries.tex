...
\begin{lemma}[Scaling of a rectangular mixed transfer operator]%
    ...
    \lean{Kraus.mixedTransferMap₂_smul}
    \leanok
    \uses{def:mixed_transfer_rect}
...
\begin{theorem}[An arbitrary BNT need not possess a joint residual isometry]%
    ...
    \leanok
    \uses{def:cpsv_canonical_form, def:bnt_cpsv, def:mps_transfer_idempotence, def:is_residual_isometry_family}
...
\begin{theorem}[Square-root characterization of fixed points]
    ...
    \notready
    \uses{def:cpsv_canonical_form, def:bnt_cpsv, def:mps_transfer_idempotence, def:is_isometry_canonical_form, def:is_residual_isometry_family, thm:phase_flip_tensor_not_rfp}
...
% The arbitrary-BNT statement is refuted by an inactive BNT member. The full
% corrected conclusion for active phase-class representatives is proved above.
\begin{corollary}[Basis tensors of a fixed point]
    \label{cor:cpsv_iii_cor3_status}
    \notready
    \uses{def:cpsv_canonical_form, def:bnt_cpsv, def:mps_transfer_idempotence, def:is_isometry_canonical_form, def:is_residual_isometry_family, thm:cpsv_weight_norm_one_and_block_rfp, thm:cpsv_phase_class_residual_isometries, thm:cpsv_cor312_arbitrary_bnt_counterexample}
    ...
    The corrected active conclusion is complete. For the normal blocks $(A_k)_k$
    occurring with nonzero weights $\mu_k$ in literal CPSV canonical-form data,
    Theorem~\ref{thm:cpsv_weight_norm_one_and_block_rfp} proves
    \begin{align}
        |\mu_k| & =1,
                & \E_{A_k}^2 & =\E_{A_k}.
        \notag
    \end{align}
    After choosing one representative $C_j$ from each active phase class,
    Theorem~\ref{thm:cpsv_phase_class_residual_isometries} supplies
    invertible $X_j$, positive trace-one diagonal data $\rho_j$, and tensors
    $U_j$ such that
    \begin{align}
        C_j^i & =X_j\sqrt{\rho_j}\,U_j^iX_j^{-1},
        \notag                                                              \\
        \sum_i(U_j^i)_{\alpha,\beta}
        \overline{(U_{j'}^i)_{\alpha',\beta'}}
              & =\delta_{j,j'}\delta_{\alpha,\alpha'}\delta_{\beta,\beta'}.
        \notag
    \end{align}
    Thus both the diagonal square-root forms and the simultaneous off-diagonal
    equations hold for the active representatives. The corollary remains
    not ready as printed because its quantifier ranges over an arbitrary BNT.
