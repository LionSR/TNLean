...
\begin{theorem}[Cross-block transfer vanishing for a direct-sum fixed point]%
    ...
    the normal-tensor blocks. The passage to the residual tensors $U_j$ is given
    by Theorem~\ref{thm:residual_isometry_of_rfp_direct_sum}.
...
\begin{proof}\leanok
    ...
    $\mu_kA_k$ a renormalization fixed point. Since $\mu_k\ne0$,
...
\begin{proof}\leanok
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The normal-tensor characterization supplies the displayed square-root form.
    Put the representatives independently in left-canonical gauge and retain
    the unit-modulus factors $\mu_j$. Distinct phase-class representatives are
    not gauge-phase equivalent, while idempotence of the retained direct sum
    restricts to every mixed pair. Hence
    Theorem~\ref{thm:bnt_locally_orthogonal_of_rfp_direct_sum} gives
    \begin{align}
        ...
        \notag
    \end{align}
    Cancelling the nonzero scalar factors and the invertible gauges yields
    $\E_{C_j,C_{j'}}=0$. Removing the positive diagonal factors and evaluating
    at matrix units gives the off-diagonal residual equation. Together with
    the within-block orthonormality, this is the joint isometry condition.
\end{proof}
...
\begin{theorem}[Square-root characterization of fixed points]
    ...
    determine a source-faithful coordinate-level predicate. The routed
    characterization therefore remains not ready pending a source
