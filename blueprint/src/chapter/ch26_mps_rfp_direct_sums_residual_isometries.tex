...
\begin{theorem}[Active phase-class residual isometries]%
    \label{thm:cpsv_active_phase_class_residual_isometries}
    ...
    \lean{MPSTensor.IsCPSVCanonicalForm.%
        exists_activeBNT_residualIsometryFamily_of_isTransferIdempotent}
    \leanok
    ...
    representative $C_j$ from each active matrix-product-vector phase class.
    ...
    \begin{align}
        ...
        \notag
    \end{align}
    The conclusion also covers the case in which there are no active classes.
\end{theorem}
\begin{proof}\leanok
    \uses{thm:cpsv_canonical_form_active_bnt_refinement, thm:rfp_coisometry_reconstruction_iff, thm:cpsv_active_weight_norm_one_and_block_rfp, thm:normal_rfp_iff_isometry_canonical_form, thm:bnt_locally_orthogonal_of_rfp_direct_sum, lem:mixed_transfer_eq_zero_of_conj, lem:residual_isometry_entry}
    The retained weighted direct sum is a renormalization fixed point. For each
    representative, Theorem~\ref{thm:cpsv_active_weight_norm_one_and_block_rfp}
    gives
    \begin{align}
        |\mu_j| & =1,
                & \E_{C_j}^2 & =\E_{C_j}.
...
\begin{corollary}[Basis tensors of a fixed point]
    ...
    \notready
    \uses{def:cpsv_canonical_form, def:bnt_cpsv, def:mps_transfer_idempotence, def:is_isometry_canonical_form, def:is_residual_isometry_family, thm:cpsv_active_weight_norm_one_and_block_rfp, thm:cpsv_active_phase_class_residual_isometries, thm:cpsv_cor312_arbitrary_bnt_counterexample}
    ...
    Theorem~\ref{thm:cpsv_active_weight_norm_one_and_block_rfp} proves
    \begin{align}
        |\mu_k| & =1,
                & \E_{A_k}^2 & =\E_{A_k}.
        \notag
    ...
    Theorem~\ref{thm:cpsv_active_phase_class_residual_isometries} supplies
