...
\begin{proof}\leanok
    \uses{thm:ldp_tree_factorization}
    The twisted tensor $\widetilde A_g$ is obtained from $A$ by the physical
    map $U_g$, and $U^{[L]}_g$ is the $L$-fold Kronecker power of $U_g$.
    Blocking commutes with physical maps: blocking $W\cdot C$ over $L$ sites
    gives $W^{\otimes L}$ applied to $C^{[L]}$, as in
    Theorem~\ref{thm:ldp_tree_factorization}.  The case $W=U_g$, $C=A$ is
