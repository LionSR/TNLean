...
\begin{proof}\leanok
    \uses{thm:block_tensor_rotate_physical}
    The twisted tensor $\widetilde A_g$ is the physical rotation of $A$ by
    $U(g)$, and $U^{[L]}(g)=U(g)^{\otimes L}$.  Hence
    (\ref{eq:symmetry_twist_block_comm}) is the case $W=U(g)$, $C=A$ of
    Theorem~\ref{thm:block_tensor_rotate_physical}.
\end{proof}
