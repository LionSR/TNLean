...
When an MPS is invariant under an on-site group action, the resulting virtual
gauge transformations form a projective representation on the bond space.
...
\begin{definition}[MPV-family invariance under physical twists]\label{def:on_site_symmetric}
    ...
    An MPS tensor $A$ has \emph{on-site symmetry} under a representation
    $U:G\to\GL_d(\C)$ if $\mc{V}(A)=\mc{V}(\widetilde A_g)$ for every $g\in G$.
    This equality of periodic MPV families provides the condition for constructing
    virtual symmetry gauges.
\end{definition}

\begin{lemma}[On-site symmetry under rescaling and gauge]\label{lem:on_site_symmetric_smul_gauge}
    \lean{MPSTensor.IsOnSiteSymmetric.smul, MPSTensor.IsOnSiteSymmetric.of_gaugeEquiv}
    \leanok
    \uses{def:on_site_symmetric, def:gauge_equiv, thm:gauge_invariance}
    If $A$ has on-site symmetry under $U$, then so does $cA$ for every
    $c\in\C$, and so does every tensor gauge equivalent to $A$.
\end{lemma}

\begin{proof}\leanok
    \uses{def:twisted_tensor, thm:gauge_invariance}
    Twisting commutes with rescaling, $\widetilde{(cA)}_g=c\,\widetilde A_g$, and
    the $N$-site coefficients of $cB$ are $c^N$ times those of $B$. If
    $B^i=XA^iX^{-1}$, then
    $(\widetilde B_g)^i=\sum_j U(g)_{ij}XA^jX^{-1}=X(\widetilde A_g)^iX^{-1}$, so
    $\widetilde B_g$ is gauge equivalent to $\widetilde A_g$, and
    Theorem~\ref{thm:gauge_invariance} gives
    $\mc{V}(B)=\mc{V}(A)=\mc{V}(\widetilde A_g)=\mc{V}(\widetilde B_g)$.
\end{proof}
...
\begin{lemma}[Twisting commutes with blocking]
    ...
    \leanok
    \uses{def:block_kron_action, def:blocked_tensor, def:twisted_tensor, lem:eval_word_twisted_tensor}
...
\begin{theorem}[MPV-family symmetry is preserved by blocking]
    ...
    \leanok
    \uses{def:on_site_symmetric, def:block_kron_action, lem:twisted_tensor_block_comm}
...
\begin{lemma}[Virtual gauges survive blocking]
    \label{lem:twisted_tensor_block_gauge}
    \lean{MPSTensor.twistedTensor_blockTensor_eq_gauge}
    \leanok
    \uses{lem:twisted_tensor_block_comm}
    If $R\in\GL_{D}(\C)$ satisfies $(\widetilde A_g)^i=R\,A^i\,R^{-1}$ for all
    $i$, then $(\widetilde{(A^{[L]})}^{\,U^{[L]}}_g)^I=R\,(A^{[L]})^I\,R^{-1}$
    for every blocked index $I$.
\end{lemma}

\begin{proof}\leanok
    By (\ref{eq:symmetry_twist_block_comm}) the left side is a product of $L$
    matrices $(\widetilde A_g)^{i_k}=R\,A^{i_k}R^{-1}$, and the inner factors
    $R^{-1}R$ cancel.
\end{proof}
...
\begin{remark}[Virtual representation via the Fundamental Theorem]%
    \label{rem:injective_symmetry_ft_route}
    \uses{thm:ft_single, lem:gauge_equiv_twisted, lem:gauge_unique_up_to_scalar, lem:twisted_tensor_mul, thm:virtual_rep_injective}
    On-site symmetry identifies $A$ and $\widetilde{A}_g$ as defining the same MPV
    family. The single-block Fundamental Theorem yields the gauge matrices in
    Lemma~\ref{lem:gauge_equiv_twisted}; gauge uniqueness and twist functoriality
    then establish the projective representation of
    Theorem~\ref{thm:virtual_rep_injective}.
\end{remark}
...
\begin{lemma}[Gauge-phase uniqueness]\label{lem:gauge_phase_unique}
    \lean{MPSTensor.gauge_phase_unique}
    \leanok
    \uses{def:injective, lem:center_scalar}
    Let $A$ be an injective tensor with $D\ge1$.  If $X,Y\in\GL_{D}(\C)$ and
    $c\in\C$ satisfy $X^{-1}A^iX=c\,Y^{-1}A^iY$ for all $i$, then $c=1$ and
    $X=u\,Y$ for some $u\in\C^{\times}$ \cite[Section~III.A]{Cirac2021Matrix}.
\end{lemma}

\begin{proof}\leanok
    \uses{lem:center_scalar}
    The matrix $Z=YX^{-1}$ satisfies $ZA^i=c\,A^iZ$ for every $i$, hence
    $ZM=c\,MZ$ for every matrix $M$ by linearity.  Taking $M=\Id$ gives
    $(1-c)Z=0$, so $c=1$ because $Z$ is invertible.  Then $Z$ commutes with
    $\MN{D}$, so it is a nonzero scalar by Lemma~\ref{lem:center_scalar}.
\end{proof}
...
\begin{theorem}[Virtual representation theorem for injective MPS]%
    ...
    \leanok
    \uses{def:injective, def:on_site_symmetric, def:projective_rep, def:scalar_cocycle, lem:gauge_unique_up_to_scalar, lem:twisted_tensor_mul}
...
\begin{proof}
    \leanok
    \uses{def:on_site_symmetric, lem:gauge_equiv_twisted, lem:gauge_unique_up_to_scalar, lem:twisted_tensor_mul}
...
Projective representations are classified up to projective equivalence by the
second cohomology group $H^2(G,\C^{\times})$ of cocycles modulo coboundaries.
...
\begin{theorem}[Gauge independence of the cocycle class]%
    ...
    \leanok
    \uses{thm:virtual_rep_injective, lem:gauge_unique_up_to_scalar, def:cohomologous_to}
...
\begin{definition}[Agreement of factor-system cohomology classes]\label{def:projectively_equivalent}
    ...
    Two projective representations are \emph{projectively equivalent} when their
    factor systems are cohomologous in $H^2(G,\C^\times)$.
\end{definition}
...
under a change of cocycle representative, so a value different from $1$
witnesses non-triviality of the class.
...
\begin{definition}[Commutator phase]\label{def:comm_phase}
    ...
    $\beta_\omega(g,h):=\omega(g,h) \omega(h,g)^{-1}$, defined for any
    $\C^{\times}$-valued $2$-cochain.
\end{definition}
