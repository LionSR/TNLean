...
\begin{theorem}[Reduction of forward canonicalization to the blocked equal-case stage]
    ...
    Assume the following continuation statement: whenever a blocked tensor $C$
    in irreducible form~II has the same MPV family as a blocked tensor $A^{[p]}$,
    there exists a tensor $A'$ with
    $\sum_i (A'^i)^\dagger A'^i = \Id$ and
    $\mathcal E_C = \mathcal E_{A'^{[p]}}$.
    Then every $p$-refinable tensor $B$ in irreducible form~II admits such a
    left-canonical root $A'$ with $\mathcal E_B = \mathcal E_{A'^{[p]}}$.
\end{theorem}
...
\begin{proof}\leanok
    ...
    family as $A^{[p]}$. The continuation statement then gives a
    left-canonical tensor $A'$ with
