...
\begin{theorem}[$p$-refinability iff transfer-map $p$-divisibility]%
    ...
    Assume the two left-canonical root hypotheses used in the one-way statements:
...
\begin{proof}\leanok
    ...
    single biconditional. The forward and reverse directions each assume the
    corresponding left-canonical root hypothesis, and together they give the full
...
\begin{theorem}[Transfer-map $p$-divisibility implies $p$-refinability]%
    ...
    \uses{def:irreducible_form, def:p_refinable, def:p_divisible_channel}
    Let $B$ be an MPS tensor in irreducible form~II and let $p \ge 1$.
    Assume the reverse left-canonical root hypothesis: there is a tensor $A$ with
...
\begin{proof}\leanok
    From the reverse left-canonical root hypothesis, extract $A$ with
