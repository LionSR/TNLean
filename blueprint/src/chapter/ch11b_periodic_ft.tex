...
\begin{proof}\leanok
    ...
    \end{equation}
    equality of the two finite sets in~(\ref{eq:pft_periodic_set_equality}) gives $m_A=m_B$, a contradiction. Thus
...
\begin{proof}\notready
    ...
    \begin{equation}\label{eq:pft_decay_mpv_equality}
        ...
        \braket{V^{(N)}(A)}{V^{(N)}(A)}.
    \end{equation}
    Along multiples of the period $m_a$,~(\ref{eq:pft_decay_mpv_equality}) would imply
    \[
        0
        =
        \lim_{N\to\infty}
...
\begin{proof}\leanok
    ...
    \end{equation}
    Since $(g h) k = g (h k)$, both sides of~(\ref{eq:pft_assoc_scalar_multiple}) are scalar multiples of the
