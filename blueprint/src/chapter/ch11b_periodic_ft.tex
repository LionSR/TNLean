...
\begin{theorem}[Blocked Z-gauge extraction from the periodic equal-case FT]
    ...
    at the blocked dimensions $(\text{blockPhysDim}\,d\,p, D)$, then for any
    blocked tensor $C$ and blocked root $A^{[p]}$ which are both already in
    irreducible form~II and have the same MPV family, there is an integer
    $m \ge 1$ and a $\mathbb Z_m$-gauge equivalence between them.
\end{theorem}

\begin{proof}\leanok
    This is a direct application of the equal-case Fundamental Theorem to the
    pair $(C,A^{[p]})$. The irreducible-form hypothesis on $A^{[p]}$ is an
    explicit assumption here; it is not inferred from equality of MPV families.
\end{proof}
