...
\subtitle{{\small 1228 lines $\cdot$ 12 modules $\cdot$ 0 sorries\;\;$\cdot$\;\;Mathlib v4.27.0\;\;$\cdot$\;\;arXiv:2011.12127}}
\date{}
\author{}
...
\inprog{Multi-block decomposition}\\
\future{Full corollary}
\end{columns}
\end{frame}
...
\vspace{0.3em}
\textbf{Expansion factor:} $\sim$3 pages of paper proof $ \longrightarrow $ 1228 lines of Lean $ \approx  10\times$ (typical)
\end{frame}
...
\node[box,done] (s1) at (0,3.2) {\proved{Single-block fundamental theorem}};
\node[box,done] (s2) at (0,2.4) {\proved{Canonical form + toTensor construction}};
\node[box,done] (s3) at (0,1.6) {\proved{MPV decomposition: $V^{(N)} = \sum_k \mu_k^N V_k^{(N)}$}};
\node[box,done] (s4) at (0,0.8) {\proved{Vandermonde separation of blocks}};
\node[box,next] (s5) at (0,0) {\inprog{Block matching: equal sums $\Rightarrow$ pairwise match}};
\node[box,next] (s6) at (0,-0.8) {\inprog{Apply single-block theorem per matched pair}};
\node[box,fut] (s7) at (0,-1.6) {\future{Assemble global gauge transform (full Corollary 4.2)}};
\end{tikzpicture}
\end{center}

\vspace{0.2em}
{\small\textbf{Remaining challenge:} Block matching requires showing that blocks from $A$'s canonical form can be put in bijection with blocks from $B$'s, with matching $\mu_k$ (up to phase).}
\end{frame}
...
\vspace{0.8em}
\textcolor{leangreen}{\textbf{1228 lines\quad$\cdot$\quad 12 modules\quad$\cdot$\quad 0 sorries\quad$\cdot$\quad 0 axioms}}
\end{center}
\end{frame}
