...
- Proof integrity (BLOCKER): no `sorry`, `admit`, `native_decide` on non-trivial goals, `unsafeCast`, or new axioms. See the lean-conventions skill's `PROOF_INTEGRITY.md` reference (`the lean-conventions skill's PROOF_INTEGRITY reference (docs/PROOF_INTEGRITY.md holds only the project addendum)` holds only the project addendum).
